var schoolNames = [
    'วัดชนะสงคราม',
    'อนุบาลวัดปรินายก',
    'พระตำหนักสวนกุหลาบ',
    'ราชวินิต',
    'วัดโบสถ์',
    'วัดพลับพลาชัย',
    'วัดโสมนัส',
    'ประถมนนทรี',
    'วัดด่าน',
    'มหาวีรานุวัตร',
    'วัดชัยชนะสงคราม',
    'อนุบาลสามเสนฯ',
    'ทุ่งมหาเมฆ',
    'วัดเวตวันธรรมาวาส',
    'ดาราคาม',
    'พญาไท',
    'อนุบาลพิบูลเวศม์',
    'สตรีวิทยา',
    'มัธยมวัดเบญจมบพิตร',
    'สตรีวัดมหาพฤฒาราม',
    'สวนกุหลาบวิทยาลัย',
    'วัดราชาธิวาส',
    'พุทธจักรวิทยา',
    'โยธินบูรณะ',
    'วัดบวรนิเวศ',
    'วัดราชบพิธ',
    'วัดน้อยนพคุณ',
    'เบญจมราชาลัย',
    'ราชวินิต มัธยม',
    'มัธยมวัดมกุฏกษัตริย์',
    'วัดสังเวช',
    'เศรษฐเสถียร',
    'เตรียมอุดมศึกษา',
    'วัดสระเกศ',
    'เทพศิรินทร์',
    'สายปัญญา ในพระราชินูปถัมภ์',
    'พระโขนงพิทยาลัย',
    'วชิรธรรมสาธิต',
    'เจ้าพระยาวิทยาคม',
    'ไตรมิตรวิทยาลัย',
    'สามเสนวิทยาลัย',
    'สุรศักดิ์มนตรี',
    'กุนนทรีรุทธารามวิทยาคม',
    'พิบูลประชาสรรค์',
    'วัดสุทธิวราราม',
    'ยานนาเวศวิทยาคม',
    'สตรีศรีสุริโยทัย',
    'โสตศึกษาทุ่งมหาเมฆ',
    'ศีลาจารพิพัฒน์',
    'สุวรรณสุทธารามวิทยา',
    'ราชนันทาจารย์สามเสนวิทยาลัย 2',
    'ปทุมคงคา',
    'สายน้ำผึ้ง',
    'ศรีอยุธยา',
    'สันติราษฎร์วิทยาลัย',
    'มักกะสันพิทยา',
    'มัธยมวัดธาตุทอง',
    'สิริรัตนาธร',
    'วัดอุทัยธาราม',
    'วัดใหม่ช่องลม',
    'พิบูลอุปถัมภ์',
    'บางบัว',
    'บ้านหนองบอน',
    'วัดมหาบุศย์',
    'ไทยรัฐวิทยา 75',
    'พระยาประเสริฐสุนทราศรัย',
    'รัตนโกสินทร์สมโภชบางเขน',
    'บางกะปิ',
    'สตรีเศรษฐบุตรบำเพ็ญ',
    'มัธยมวัดหนองจอก',
    'เทพลีลา',
    'เศรษฐบุตรบำเพ็ญ',
    'บดินทรเดชา (สิงหเสนี) 4',
    'มัธยมวัดบึงทองหลาง',
    'พรตพิทยพยัต',
    'เทพศิรินทร์ร่มเกล้า',
    'รัตนโกสินทร์สมโภชลาดกระบัง',
    'จันทร์หุ่นบำเพ็ญ',
    'เตรียมอุดมศึกษาพัฒนาการ รัชดา',
    'นวมินทราชูทิศ กรุงเทพมหานคร',
    'บดินทร์เดชา (สิงห์ สิงหเสนี) 2',
    'บางกะปิสุขุมนวพันธ์อุปถัมภ์',
    'หอวัง',
    'สารวิทยา',
    'ราชดำริ',
    'เตรียมอุดมศึกษาพัฒนาการ',
    'ดอนเมืองทหารอากาศบำรุง',
    'สีกัน (วัฒนานันท์อุปถัมภ์)',
    'ดอนเมืองจาตุรจินดา',
    'ลาดปลาเค้าพิทยาคม',
    'สตรีวิทยา 2',
    'ราชวินิตบางเขน',
    'ฤทธิยะวรรณาลัย',
    'ฤทธิยะวรรณาลัย2',
    'ศรีพฤฒา',
    'เตรียมอุดมศึกษาน้อมเกล้า',
    'นวมินทราชินูทิศ เตรียมอุดมศึกษา น้อมเกล้า',
    'บดินทรเดชา (สิงห์ สิงหเสนี)',
    'นวมินทราชินูทิศ บดินทร์เดชา',
    'นวมินทราชินูทิศ เบญจมราชาลัย',
    'นวมินทราชินูทิศ สตรีวิทยา 2',
    'วัดประยุรวงศาวาส',
    'วัดเจ้ามูล',
    'วัดสังข์กระจาย',
    'วัดหงส์รัตนาราม',
    'ประถมทวีธาภิเศก',
    'วัดช่างเหล็ก',
    'วัดอมรินทราราม',
    'โฆสิตสโมสร',
    'ราชวินิตประถมบางแค',
    'วัดนาคปรก',
    'วัดหนัง',
    'อนุบาลวัดนางนอง',
    'ศึกษานารี',
    'ทวีธาภิเศก',
    'สุวรรณพลับพลาพิทยาคม',
    'วัดอินทราราม',
    'วัดประดู่ในทรงธรรม',
    'วัดน้อยใน',
    'มัธยมวัดดาวคนอง',
    'ฤทธิณรงค์รอน',
    'โพธิสารพิทยากร',
    'ธนบุรีวรเทพีพลารักษ์',
    'วัดปากน้ำวิทยาคม',
    'มหรรณพาราม',
    'ชิโนรสวิทยาลัย',
    'สตรีวัดระฆัง',
    'สุวรรณารามวิทยาคม',
    'มัธยมวัดดุสิตาราม',
    'มัธยมวัดนายโรง',
    'สวนอนันต์',
    'พิทยาลงกรณ์พิทยาคม',
    'รัตนโกสินทร์สมโภชบางขุนเทียน',
    'ทวีธาภิเศก 2',
    'วัดนวลนรดิศ',
    'วัดรางบัว',
    'สตรีวัดอัปสรสวรรค์',
    'ไชยฉิมพลีวิทยาคม',
    'จันทร์ประดิษฐารามวิทยาคม',
    'มัธยมวัดหนองแขม',
    'บางปะกอกวิทยาคม',
    'แจงร้อนวิทยา',
    'วัดบวรมงคล',
    'วิมุตยารามพิทยากร',
    'ปัญญาวรคุณ',
    'ราชวินิตบางแคปานขำ',
    'ทวีวัฒนา',
    'นวมินทราชินูทิศ สตรีวิทยา พุทธมณฑล',
    'วัดพุทธบูชา',
    'อิสลามวิทยาลัยแห่งประเทศไทย',
    'ศึกษานารีวิทยา',
    'นวลนรดิศวิทยาคม รัชมังคลาภิเษก',
    'มัธยมวัดสิงห์',
    'วัดราชโอรส',
    'บางมดวิทยา',
    'วัดราษฎร์โพธิ์ทอง',
    'คลองแสนสุข',
    'บ้านคลองหลวง',
    'คลองสำโรง',
    'วัดบางด้วน',
    'วัดบางนางเกรง',
    'วัดสุขกร',
    'คลองเสาธง',
    'คลองบางปู',
    'ชุมชนวัดราษฎร์บำรุง',
    'พิบูลประชาบาล',
    'วัดตำหรุ มิตรภาพที่ 65',
    'เอี่ยมสุรีย์',
    'นิลรัตน์อนุสรณ์',
    'วัดบางโปรง',
    'คลองมหาวงก์',
    'คลองบางปิ้ง',
    'วัดไตรสามัคคี',
    'อินทรัมพรรย์อนุสรณ์',
    'พร้านีลวัชระ',
    'อนุบาลวัดพิชัยสงคราม',
    'วัดแพรกษา',
    'นาคดีอนุสรณ์',
    'คลองใหม่',
    'วัดคลองเก้า',
    'วัดด่านสำโรง',
    'มหาภาพกระจาดทองอุปถัมภ์',
    'คลองกระทุ่มราษฎร์อุทิศ',
    'วัดคันลัด',
    'วัดป่าเกด',
    'วัดบางกระสอบ',
    'วัดบางกอบัว',
    'วัดราษฎร์รังสรรค์',
    'พลอยจาตุรจินดา',
    'วัดครุนอก',
    'วัดครุใน',
    'บางครุ',
    'อนุบาลสุขสวัสดิ์',
    'วัดชมนิมิตร',
    'บ้านบางจาก',
    'วัดบางน้ำผึ้งใน',
    'วัดบางน้ำผึ้งนอก',
    'วัดกลาง',
    'วัดรวก',
    'วัดบางกะเจ้ากลาง',
    'วัดกองแก้ว',
    'วัดบางขมิ้น',
    'วัดบางหญ้าแพรก',
    'วัดท้องคุ้ง',
    'ฉัตรทิพย์เทพวิทยา',
    'วัดแหลม',
    'วัดบางฝ้าย',
    'วัดบางหัวเสือ',
    'วัดโยธินประดิษฐ์',
    'วัดมหาวงษ์',
    'วัดสำโรงเหนือ',
    'วัดปุณหังสนาวาส',
    'วัดสวนส้ม',
    'ชุมชนวัดสาขลา',
    'วัดศรีคงคาราม',
    'บ้านขุนสมุทรไทย',
    'วัดคู่สร้าง',
    'คลองนาเกลือน้อย',
    'วัดใหญ่',
    'วัดแค',
    'วัดคลองสวน',
    'วัดคลองมอญ',
    'คลองสองพี่น้อง',
    'อนุบาลพระสมุทรเจดีย์',
    'วัดไตรมิตรวราราม',
    'ป้อมพระจุลจอมเกล้า',
    'วัดแหลมฟ้าผ่า',
    'บ้านขุนสมุทร',
    'สมุทรปราการ',
    'สตรีสมุทรปราการ',
    'มัธยมด่านสำโรง',
    'บางเมืองเขียนผ่องอนุสรณ์',
    'หาดอมราอักษรลักษณ์วิทยา',
    'นวมินทราชินูทิศ สวนกุหลาบวิทยาฯ',
    'มัธยมวัดศรีจันทร์ประดิษฐ์',
    'วัดทรงธรรม',
    'ราชประชาสมาสัยฝ่ายมัธยมฯ',
    'เตรียมอุดมศึกษาน้อมเกล้า สมุทรปราการ',
    'วิสุทธิกษัตรี',
    'สาขลาสุทธีราอุปถัมภ์',
    'ป้อมนาคราชสวาทยานนท์',
    'มัธยมวัดใหม่สมุทรกิจวิทยาคม',
    'สุเหร่าคลองใหม่',
    'เฉลิมมณีฉายวิทยาคาร',
    'วัดมงคลโคธาวาส',
    'วัดสร่างโศก',
    'วัดสว่างอารมณ์',
    'วัดสีล้ง',
    'ไทยรัฐวิทยา 71 (ก่อสร้างคลองด่าน)',
    'วัดปานประสิทธาราม',
    'บ้านท้องคุ้ง',
    'วัดนิยมยาตรา',
    'ตลาดคลองสวน',
    'คลองกระแซงเตย',
    'คลองพระยานาคราช',
    'วัดสุคันธาวาส',
    'วัดคอลาด',
    'คลองหลุมลึก',
    'คลองสะบัดจาก',
    'ปากคลองชวดใหญ่',
    'วัดบางบ่อ',
    'อนุบาลชุมชนบางบ่อ',
    'คลองกันยา',
    'วัดบางพลีน้อย',
    'วัดเกาะแก้ว',
    'วัดนาคราช',
    'ธรรมศรีสุวรรณดิษฐ์',
    'วัดโคธาราม',
    'วัดลาดหวาย',
    'วัดบางเพรียง',
    'ตลาดบางพลีน้อย',
    'คลองบ้านระกาศ',
    'วัดบางนางเพ็ง',
    'ชุมชนวัดบ้านระกาศ',
    'ตลาดปากคลองเจ้า',
    'วัดกาหลง',
    'วัดเปร็งราษฎร์บำรุง',
    'คลองปลัดเปรียง',
    'วัดสลุด',
    'วัดหนามแดง',
    'วัดบางโฉลงนอก',
    'วัดบางโฉลงใน',
    'หนองงูเห่าศาสตร์ประเสริฐ',
    'คลองบางน้ำจืด',
    'วัดราษฎร์บูรณะ',
    'คลองบางกะอี่',
    'คลองบางกะสี',
    'สุเหร่าบางปลา',
    'สุเหร่าบางกะสี',
    'วัดราษฎร์นิยมธรรม',
    'คลองบางแก้ว',
    'คลองบางกระบือ',
    'ธนสิทธิ์อนุสรณ์',
    'เตรียมปริญญานุสรณ์',
    'วัดบางพลีใหญ่กลาง',
    'วัดบางพลีใหญ่ใน',
    'วัดกิ่งแก้ว',
    'คลองลาดกระบัง',
    'เทวะคลองตรง',
    'พรหมพิกุลทอง',
    'วัดหนองปรือ',
    'ละมูลรอดศิริ',
    'วัดบัวโรย',
    'วัดเสาธงกลาง',
    'วัดเสาธงนอก',
    'วัดมงคลนิมิตร',
    'สุเหร่าบ้านไร่',
    'เคหะบางพลี (10 ปี สปช.)',
    'รัตนโกสินทร์ 9',
    'คลองเจริญราษฎร์',
    'วัดหัวคู้',
    'ปากคลองมอญ',
    'วัดจรเข้ใหญ่',
    'วัดศรีวารีน้อย',
    'บางบ่อวิทยาคม',
    'เปร็งวิสุทธาธิบดี',
    'หลวงพ่อปานคลองด่านอนุสรณ์',
    'นวมินทราชินูทิศ เตรียมอุดมศึกษาฯ',
    'บางพลีราษฎร์บำรุง',
    'พูลเจริญวิทยาคม',
    'ราชวินิตบางแก้ว',
    'บางแก้วประชาสรรค์',
    'สมุทรพิทยาคม',
    'วัดลานนาบุญ',
    'วัดบัวขวัญ(มีทับราษฎร์บำรุง)',
    'ท่าทรายประชาอุปถัมภ์',
    'วัดตำหนักใต้(วิลาศโอสถานนท์นุเคราะห์)',
    'ทานสัมฤทธิ์วิทยา',
    'ประชาอุปถัมภ์',
    'วัดแดง(นนทคุณพิพัฒน์ราษฎร์อุปถัมภ์)',
    'ชุมชนวัดไทรม้า',
    'วัดแคนอก(คณะราษฎร์บำรุง)',
    'วัดแจ้งศิริสัมพันธ์',
    'ชุมชนวัดสมรโกฏิ(อยู่พูนราษฎร์บำรุง)',
    'อนุราชประสิทธิ์',
    'วัดบางระโหง(ภิรมย์ศิริ)',
    'วัดประชารังสรรค์',
    'วัดปราสาท',
    'วัดแคใน(ชื่นแย้มราษฎร์บำรุง)',
    'ไทยรัฐวิทยา 55 (วัดโบสถ์ดอนพรหม)',
    'วัดฝาง',
    'วัดทางหลวงโพธิ์ทอง',
    'วัดโชติการาม',
    'วัดตึก(จำลองศิลป์วิทยา)',
    'วัดบางรักน้อย',
    'ป่าไม้อุทิศ 9',
    'วัดพุฒิปรางค์ปราโมทย์',
    'วัดเฉลิมพระเกียรติ(พิบูลบำรุง)',
    'อนุบาลนนทบุรี',
    'กลาโหมอุทิศ',
    'วัดปากน้ำ(พิบูลสงคราม)',
    'วัดจันทร์(ผาดไสวประชาอุทิศ)',
    'วัดลุ่ม',
    'วัดเชิงกระบือ',
    'วัดสำโรง(หิรัญราษฎร์ภักดีวิทยา)',
    'วัดชลอ',
    'วัดบางขนุน(ทัดสุวรรณกลึงประชาสรรค์)',
    'วัดบางไกรนอก',
    'ชุมชนวัดบางไกรใน',
    'วัดซองพลู(วิฑิตราชประชานุสรณ์)',
    'วัดโบสถ์',
    'วัดสิงห์(แจ่มชื่นวิทยาคม)',
    'ชุมชนวัดส้มเกลี้ยง',
    'วัดรวก(พร้อมพิทยาคาร)',
    'วัดบางอ้อยช้าง(นันทศรีวิบูลย์บำรุง)',
    'วัดอุบลวนาราม',
    'วัดสุนทรธรรมิการาม',
    'อนุบาลบางกรวย',
    'มหาสวัสดิ์(ราษฎร์บำรุง)',
    'วัดโคนอนราษฎร์บำรุง',
    'วัดสนามนอก',
    'วัดสัก',
    'วัดใหม่ผดุงเขต',
    'นุ่มประสงค์วิทยา',
    'วัดเขมาภิรตาราม',
    'ศรีบุณยานนท์',
    'สตรีนนทบุรี',
    'เตรียมอุดมศึกษาพัฒนาการบางใหญ่',
    'เบญจมราชานุสรณ์',
    'นนทบุรีพิทยาคม',
    'มัธยมวัดเพลง นนทบุรี',
    'รัตนาธิเบศร์',
    'บดินทรเดชา (สิงห์ สิงหเสนี) นนทบุรี',
    'เทพศิรินทร์ นนทบุรี',
    'ศึกษาสงเคราะห์บางกรวย',
    'วัดพิกุลเงิน',
    'วัดสังวรพิมลไพบูลย์(จันทรานุกูล)',
    'วัดพระเงิน(อิสราวิทย์อุปกรณ์)',
    'ชุมชนวัดบางโค(แม่นางวิทย์อุปการี)',
    'อนุบาลบางใหญ่',
    'วัดศรีราษฎร์(เอนกนาครราษฎร์บำรุง)',
    'ชุมชนวัดต้นเชือก(ถาวรวิทย์อุปกฤต)',
    'ตลาดบางคูลัด',
    'วัดท่าบันเทิงธรรม',
    'วัดเอนกดิษฐาราม',
    'บ้านดอนตะลุมพุก(เจริญจิตรวิทยา)',
    'คลองโยงเวทีอุปถัมภ์',
    'บ้านใหม่(ศรีอินทร์พงษ์วิทยาคาร)',
    'สามแยกบางคูลัด(จั่นเพชรวิทยาคาร)',
    'เฉลิมพระเกียรติ 60 พรรษาฯ',
    'วัดเสาธงหิน(วันครู พ.ศ.2504)',
    'วัดอินทร์',
    'แก้วอินทร์สุธาอุทิศ',
    'เต็มรักศึกษา',
    'บางคูลัด',
    'คล้ายสอนศึกษา',
    'ชุมชนไมตรีอุทิศ',
    'ประชารัฐบำรุง',
    'แสงประเสริฐ',
    'จันทร์ทองเอี่ยม',
    'ชุมชนวัดบางไผ่',
    'วัดลาดปลาดุก',
    'วัดบางรักใหญ่(สายอักษรศรี)',
    'วัดโมลี(นันทวิมล)',
    'วัดมะเดื่อ',
    'ระดิ่งหินประชาสรรค์',
    'สุเหร่าลากค้อน',
    'สุเหร่าปากคลองลำรี',
    'สุเหร่าเขียว',
    'ประสานสามัคคีวิทยา',
    'แสงประทีปรัฐบำรุง',
    'ซอและฮ์ศึกษา',
    'วัดลำโพ',
    'เจริญรัฐอุปถัมภ์',
    'วัดคลองขุนศรี',
    'บ้านคลองพระพิมล',
    'วัดยอดพระพิมล',
    'บ้านคลองโต๊ะนุ้ย',
    'วัดคลองขวาง(จรูญชนม์ราษฎร์บำรุง)',
    'สมบูรณ์ศาสตร์พัฒนา',
    'วัดมะสง มิตรภาพที่ 55',
    'วัดคลองเจ้า',
    'ชุมชนวัดไทรน้อย',
    'วัดคลองตาคล้าย',
    'บ้านคลองฝรั่งฯ',
    'วัดไทรใหญ่',
    'ดีมากอุปถัมภ์',
    'วัดปลายคลองขุนศรี',
    'วัดสโมสร',
    'บ้านคลองหนึ่ง',
    'บ้านราษฎร์นิยม',
    'ชุมชนวัดราษฎร์นิยม',
    'วัดลากค้อน',
    'วัดเพรางาย',
    'บ้านหนองเพรางาย',
    'วัดเสนีวงศ์',
    'วัดปรมัยยิกาวาส(แสนสวัสดิ์วิทยาคาร)',
    'วัดศาลากุล',
    'คลองเกลือ',
    'วัดท่าเกวียน(ศึกษาประชาสรรค์)',
    'สามัคคีวิทยา(เชื้อผู้ดีอุปถัมภ์)',
    'วัดศรีเขตนันทาราม',
    'วัดอินทาราม(สงค์วอนอุทิศ)',
    'วัดสพานสูง(รถไฟนุเคราะห์)',
    'รุ่งเรืองวิทยา',
    'วัดบางบัวทอง(โสภณวิทยานุเคราะห์)',
    'วัดเชิงเลน',
    'ชลประทานสงเคราะห์',
    'ประเสริฐอิสลาม',
    'วัดตำหนักเหนือ(ชื่นวิทยานุสรณ์)',
    'วัดตาล',
    'วัดสาลีโขภิตาราม',
    'วัดกู้(นันทาภิวัฒน์วิทยา)',
    'วัดผาสุกมณีจักร',
    'วัดโพธิ์บ้านอ้อย',
    'ผาสุกมณีจักร มิตรภาพที่ 116',
    'วัดสลักเหนือ',
    'ไทยรัฐวิทยา95 (วัดโพธิ์ทองบน)',
    'วัดกลางเกร็ด',
    'วัดบ่อ(นันทวิทยา)',
    'วัดบางพูดใน(นนทวิทยา)',
    'วัดสิงห์ทอง',
    'วัดใหญ่สว่างอารมณ์(สโมสรสหมิตรอุทิศ)',
    'ไทรน้อย',
    'ปากเกร็ด',
    'ราษฎร์นิยม',
    'สวนกุหลาบวิทยาลัย นนทบุรี',
    'เตรียมอุดมศึกษาน้อมเกล้า นนทบุรี',
    'โพธินิมิตวิทยาคม',
    'นวมินทราชินูทิศ หอวัง นนทบุรี',
    'สตรีนนทบุรีบางใหญ่(นนทกิจพิศาล)',
    'บางบัวทอง',
    'โสตศึกษาจังหวัดนนทบุรี',
    'วัดบางกุฎีทอง',
    'ชุมชนวัดบางกะดี',
    'วัดชินวราราม',
    'วัดบางนางบุญ',
    'วัดบางคูวัด',
    'วัดบางเดื่อ',
    'วัดไพร่ฟ้า',
    'อนุบาลปทุมธานี',
    'วัดฉาง',
    'วัดโคก',
    'วัดหงส์ปทุมาวาส',
    'วัดเวฬุวัน',
    'วัดบางพูน',
    'วัดดาวเรือง',
    'วัดเปรมประชากร',
    'วัดราษฎร์ศรัทธาทํา',
    'วัดโพธิ์เลื่อน',
    'วัดโบสถ์(บวรธรรมกิจวิทยา)',
    'วัดมะขาม(ศรีวิทยาคาร)',
    'ขจรทรัพย์อํารุง',
    'วัดเทียนถวาย',
    'ชุมชนวัดเสด็จ',
    'วัดรังสิต',
    'สุลักขณะ',
    'วัดนาวง',
    'อินทรนาวีราษฎร์อุปถัมภ์',
    'วัดอู่ข้าว',
    'ปากคลองสอง',
    'ลําสนุ่น',
    'สังข์อํ่าวิทยา',
    'คลองสอง',
    'วัดกล้าชอุ่ม',
    'สามัคคีราษฎร์บํารุง',
    'วัดเกิดการอุดม',
    'วัดกลางคลองสาม',
    'บ้านบึง',
    'ไทยรัฐวิทยา 69 (คลองหลวง)',
    'วัดสว่างภพ',
    'วัดมงคลพุการาม',
    'วัดตะวันเรือง',
    'บึงเขาย้อน',
    'วัดเพิ่มทาน',
    'วัดมูลเหล็ก',
    'เจริญวิทยา',
    'วัดหว่านบุญ',
    'วัดผลาหาร',
    'ลิ้นจี่อุทิศ',
    'ประถมศึกษาธรรมศาสตร์',
    'บุญค้มราษฎร์บํารุง',
    'จารุศรบํารุง',
    'วัดพืชนิมิตร',
    'วัดคุณหญิงส้มจีน',
    'วันครู 2502',
    'ชุมชนวัดบางขัน',
    'คลองหนึ่ง(แก้วนิมิตร)',
    'คลองห้า (พฤกษชัฏ ฯ)',
    'ชุมชนวัดหัตถสารเกษตร',
    'บางชวดอนุสรณ์',
    'วัดศิริจันทราราม',
    'บ้านคลองเจ้าเมือง',
    'คลองพระอุดม',
    'วัดสุวรรณจินดาราม',
    'วัดบ่อทอง',
    'วัดจันทาราม',
    'บางโพธิ์ใหม่',
    'คลองบางโพธิ์',
    'วัดลํามหาเมฆ',
    'วัดบ่อเงิน',
    'สังฆรักษ์บํารุง',
    'ชุมชนวัดบัวแก้วเกษร',
    'วัดบัวสุวรรณประดิษฐ์',
    'วัดลาดหลุมแก้ว',
    'บ้านคลองขวางบน',
    'วัดบัวขวัญ',
    'วัดสุทธาวาส',
    'ชุมชนวัดหน้าไม้',
    'คลองลาดช้าง',
    'วัดเนกขัมมาราม',
    'สว่างราษฏร์บํารุง',
    'คลองลากค้อน',
    'สามวาวิทยา',
    'วัดบางนา',
    'ชุมชนวัดไก่เตี้ย',
    'วัดสามัคคิยาราม',
    'วัดสหราษฏร์บำรุง(วงษ์กล้าแข็ง)',
    'คลองสระ(สภาวิทยาอุทิศ)',
    'ศาลาพัน',
    'วัดเมตารางค์',
    'วัดบัวหลวง',
    'วัดบ้านพร้าวใน',
    'วัดท้ายเกาะ',
    'วัดเชิงท่า',
    'ประชาบำรุง(ทำสะดวกอนุสรณ์)',
    'สี่แยกบางเตย',
    'สุเหร่าใหม่เจริญ',
    'บางโพธิ์เหนือ',
    'วัดสองพี่น้อง',
    'วัดป่างิ้ว',
    'วัดปทุมทอง',
    'วัดถั่วทอง',
    'คลองบ้านพร้าว',
    'วัดบางเตยใน',
    'ชุมชนวัดจันทน์กะพ้อ',
    'วัดบางเตยนอก(แก่นจำเนียรอนุสรณ์)',
    'วัดสะแก',
    'ปทุมวิไล',
    'คณะราษฎร์บำรุงปทุมธานี',
    'ปทุมธานี (นันทมุนีบำรุง)',
    'หอวัง ปทุมธานี',
    'มัธยมวัดหัตถสารเกษตร',
    'สวนกุหลาบวิทยาลัย รังสิต',
    'ธรรมศาสตร์คลองหลวงวิทยาคม',
    'บัวแก้วเกษร',
    'วรราชาทินัดดามาตุวิทยา',
    'สุนทโรเมตตาประชาสรรค์',
    'จุฬาภรณราชวิทยาลัย ปทุมธานี',
    'สามโคก',
    'ชัยสิทธาวาส (พัฒน์สายบำรุง)',
    'เตรียมอุดมศึกษาน้อมเกล้า ปทุมธานี',
    'ชุมชนวัดพิชิตปิตยาราม',
    'วัดสระบัว',
    'วัดมูลจินดาราม',
    'วัดเขียนเขต',
    'วัดขุมแก้ว',
    'วัดแสงสรรค์',
    'ทองพูลอุทิศ',
    'ชุมชนประชาธิปัตย์วิทยาคาร',
    'ธัญญสิทธิศิลป์',
    'วัดนาบุญ',
    'วัดอัยยิการาม',
    'วัดนพรัตนาราม',
    'แสนชื่นปานนุกูล',
    'คลอง 11ศาลาครุ(เทียมอุปถัมภ์)',
    'วัดจตุพิธวราวาส',
    'วัดราษฎรบํารุง',
    'วัดเจริญบุญ',
    'วัดจุฬาจินดาราม',
    'หิรัญพงษ์อนุสรณ์',
    'วัดศรีคัคณางค์',
    'วัดสุขบุญฑริการาม',
    'วัดสอนดีศรีเจริญ',
    'วัดพวงแก้ว',
    'นิกรราษฎร์บํารุงวิทย์',
    'วัดศรีสโมสร',
    'ศาลาลอย',
    'ชุมชนบึงบา',
    'วัดแสงมณี',
    'วัดปทุมนายก',
    'อยู่ประชานุเคราะห์',
    'แสนจําหน่ายวิทยา',
    'วัดธรรมราษฏร์เจริญผล',
    'คลองสิบสาม(ผิวศรีราษฏร์บำรุง)',
    'นิกรราษฎร์บูรณะ',
    'ราษฎร์สงเคราะห์วิทยา',
    'ชุมชนประชานิกรอํานวยเวทย์',
    'วัดโปรยฝน',
    'วัดลาดสนุ่น',
    'วัดโพสพผลเจริญ',
    'ขจรเนติยุทธ',
    'วัดประยูรธรรมาราม',
    'วัดโสภณาราม',
    'รวมราษฎร์สามัคคี',
    'ร่วมจิตประสาท',
    'ร่วมใจประสิทธิ์',
    'ชุมชนเลิศพินิจพิทยาคม',
    'วัดราษฎร์ศรัทธาราม',
    'สหราษฎร์บํารุง',
    'วัดแจ้งลําหิน',
    'วัดประชุมราษฏร์',
    'วัดเกตุประภา',
    'วัดสุวรรณ',
    'วัดพิรุณศาสตร์',
    'กลางคลองสิบ',
    'วัดทศทิศ',
    'วัดอดิศร',
    'ชุมชนวัดทําเลทอง',
    'วัดพืชอุดม',
    'วัดนิเทศน์',
    'วัดปัญจทายิกาวาส',
    'วัดคลองชัน',
    'วัดกลางคลองสี่',
    'วัดมงคลรัตน์',
    'วัดชัยมังคลาราม',
    'วัดธัญญะผล',
    'เจริญดีวิทยา',
    'วัดสมุหราษฎร์บํารุง',
    'วัดดอนใหญ่',
    'วัดลานนา',
    'ธัญรัตน์',
    'สายปัญญารังสิต',
    'ธัญบุรี',
    'หนองเสือวิทยาคม',
    'เตรียมอุดมศึกษาพัฒนาการ ลำลูกกา',
    'เทพศิรินทร์คลองสิบสาม ปทุมธานี',
    'นวมินทราชินูทิศ สวนกุหลาบวิทยาลัย ปทุมธานี',
    'มัธยมสังคีตวิทยา กรุงเทพมหานคร',
    'วัดทรงกุศล',
    'วัดช่างทอง',
    'ประชาสามัคคี',
    'บ้านคลองตะเคียน หมู่ 2',
    'วัดกลางคลองสระบัว',
    'วัดพนัญเชิง',
    'ประตูชัย',
    'อนุบาลพระนครศรีอยุธยา',
    'วัดไผ่',
    'วัดพระงาม',
    'วัดกษัตราธิราช',
    'วัดไก่เตี้ย',
    'วัดเกตุ (ประชาราษฎร์บำรุง)',
    'ปฐมวิทยาคาร',
    'วัดกุฎีลาย',
    'วัดทำใหม่',
    'วัดกลาง (ปากกรานสามัคคี)',
    'วัดช้าง',
    'วัดปากกราน',
    'วัดพระญาติการาม',
    'วัดประดู่ทรงธรรม',
    'วัดใหญ่ชัยมงคล',
    'สุดินสหราษฎร์',
    'ลุมพลีชนูปถัมภ์',
    'วัดช้างใหญ่',
    'วัดตูม',
    'วัดสว่างอารมณ์',
    'ตำหนักเพนียด',
    'พรพินิตพิทยาคาร',
    'วัดพุทไธศวรรย์',
    'วัดหันตรา',
    'วัดดุสิดาราม',
    'วัดบัวงาม',
    'วัดจําปา',
    'วัดจงกลณี (ประสิทธิ์วิทยา)',
    'วัดสฎางค์',
    'วัดไก่จ้น(ชลประทานอุปถัมภ์)',
    'วัดหัวหิน',
    'วัดสะตือ',
    'วัดถลุงเหล็ก(ปูนซิเมนต์ไทยอุปถัมภ์)',
    'ชุมชนวัดบ้านร่อม',
    'วัดดอนประดู่',
    'วัดแค',
    'วัดเทพคันธาราม"เลื่อนเทียนประสิทธิ์สงเคราะห์',
    'วัดสามเรือน"จนทสโรอนุสสรณ์"',
    'วัดโพธิ์เอน "ราษฎร์สามัคคีวิทยา"',
    'วัดบึง',
    'วัดวังแดงเหนือ "แจ่มวิทยาคาร"',
    'วัดศักดิ์ "ราษฎร์สามัคคี"',
    'วัดศาลาลอย',
    'วัดนางคุ่ม',
    'ประชาผดุงวิทย์ (ฟ้อน ฟุ้ง อุทิศ)',
    'บ่อแร่',
    'หนองขนาก "ตุรงค์เรือง"',
    'วัดทุ่งมน',
    'วัดบ้านแถว "ตุรงค์เรืองประชานุสรณ์"',
    'ชุมชนวัดเสด็จ(คุณศรีประเสริฐอุปถัมภ์)',
    'วัดทองทรงธรรม (สามัคคีนฤมิต)',
    'วัดมเหยงค์',
    'วัดน้อย (สร้อยเกลียวอุปถัมภ์)',
    'วัดตะโหนด (ประชารักษ์เวสสถิตยวิทยา)',
    'นครหลวง (พิบูลประเสริฐวิทย์)',
    'วัดโพธิ์ทอง (วาสนานุกูล)',
    'วัดราษฎร์บำเพ็ญ (กริ่ง สง่า อุปภัมก์)',
    'วัดบ้านดาบ',
    'วัดลาย (ทวีปัญญา)',
    'วัดเรือแข่ง (ประชาอุปถัมภ์)',
    'วัดบ้านชุ้ง (อดุลประชาวิทย์)',
    'วัดละมุด (ร.ส.พ.ราษฎร์บํารุง)',
    'วัดทอง (สามัคคยานุสรณ์)',
    'วัดสำมะกัน (เปี่ยมวิทยาราษฎร์บำรุง)',
    'วัดปรีดาราม',
    'วัดใหญ่ (สามไถผาสุขประเสริฐ)',
    'วัดมาบพระจันทร์ (รัฐบำรุงประชาวิทยา)',
    'บ้านหนองโคก (อดุลถวิลราษฎร์บำรุง)',
    'วัดจอมเกษ',
    'วัดค่าย (เล็กจุฬาประชานุสรณ์)',
    'วัดตาลเอน (โศถนชนูปถัมภ์)',
    'วัดศรีภวังค์',
    'วัดผึ่งแดด (บุญสืบวิชชนูปถัมภ์)',
    'วัดทางกลาง (ประชาสามัคคีอุปถัมภ์)',
    'วัดพระงาม',
    'วัดบางเดื่อ',
    'วัดโตนด (บุญชื่นอุปถัมภ์)',
    'วัดเทพอุปการามฯ',
    'วัดเกาะเลิ่ง',
    'วัดวรนายกรังสรรค์',
    'วัดนาค',
    'วัดบางเพลิง (คงสกุลมากอุทิศ)',
    'วัดแก้วตา',
    'วัดโขดเขมาราม',
    'วัดบ้านม้า',
    'วัดโพธิ์ทอง',
    'วัดอินกัลยา',
    'วัดประมุง (แจ่มราษฎร์รังสรรค์)',
    'วัดนนทรีย์',
    'วัดโคก (ธูปะเตมีย์อุปถัมภ์)',
    'วัดม่วง',
    'วัดโพธิ์หอม',
    'วัดดาวคะนอง',
    'วัดเสาธงเก่า (ถึงสุขประชาสรรค์)',
    'วัดดอกไม้',
    'วัดบ้านแจ้ง',
    'วัดตลาด',
    'วัดไก่',
    'วัดพระแก้ว โสภณประสิทธิ์วิทยาคาร',
    'ชุมชนโคกม่วงเผอิญศรีภูธรอุปภัมภ์',
    'วัดโคกกรวด',
    'วัดโคกสังข์ "ประชานุกูล"',
    'บ้านดอนข่อย "คงสมโอษฐ์ราษฎร์บำรุง"',
    'ตะโกดอนหญ้านาง',
    'วัดวิมลสุนทร',
    'วัดภาชี "สุนทรอุปถัมภ์"',
    'วัดหนองกลาง',
    'วัดนาอุ่น',
    'วัดหัวคุ้ง',
    'วัดดอนกลาง',
    'วัดหนองบัว',
    'วัดอุทการาม',
    'วัดเจริญธรรม เผอิญศรีภูธรอุปถัมภ์',
    'ชุมชนวัดระโสม',
    'วัดมาบโพธิ์',
    'วัดผดุงธรรม',
    'วัดหนองนาง',
    'วัดหนองเครือบุญ',
    'วัดธรรมสินธุ์โสภา',
    'วัดหนองเป้า',
    'วัดธรรมจริยา',
    'วัดศรีประชา',
    'วัดวงษ์สวรรค์',
    'กลางคลอง 27',
    'วัดยมนาตามธรรม',
    'สุวพรรณสนิทวงศ์พิทยา',
    'วัดสุคันธาราม (สุคันธวิทยาคาร)',
    'บ้านสร้าง (สำริต ภู่เงิน อนุสรณ์)',
    'บ้านบ่อตาโล่',
    'วัดห้วยจระเข้ (เกตุอร่ามอุปถัมภ์)',
    'มุสลิมบำรุง',
    'วัดศิวาราม',
    'พระอินทร์ศึกษา (กล่อมสกุลอุทิศ)',
    'วัดพะยอม',
    'ชลประทานอนุเคราะห์',
    'วัดบ้านช้าง',
    'บ้านกะทุ่มลาย',
    'วังน้อยวิทยาภูมิ',
    'จรูญกิมลี้กิจจาทรอนุสรณ์',
    'วัดลาดทราย',
    'วัดกุฎีประสิทธิ์',
    'วัดลำพระยา (แช่มช้อยวิทยา)',
    'วัดสว่างอารมณ์',
    'วัดมณฑลประสิทธิ์ (อาจธวัชประชานุกูล)',
    'สิทธิพยากรณ์',
    'คอตันคลอง 27',
    'บ้านลำแดง',
    'วัดสะแก',
    'วัดโตนด',
    'บ้านเป็ด',
    'วัดคานหาม',
    'วัดโคกมะยม',
    'เชาวน์วัศ',
    'วัดกะสังข์ (พิศิษฎ์นวการอุปถัมภ์)',
    'วัดขุนทิพย์ (สาครราษฎร์บำรุง)',
    'บ้านช้าง',
    'วัดบ้านหีบ',
    'วัดนางชี',
    'วัดไทรงาม',
    'วัดพรานนก',
    'วัดโพธิ์สาวหาญ',
    'วัดโคกโพธิ์',
    'วัดสามบัณฑิต',
    'วัดดอนพุดซา',
    'วัดกุ่มแต้',
    'วัดอุทัย (เชาวนวิทยา)',
    'บ้านข่อยโทน',
    'วัดหนองน้ำส้ม (เจียนวิทยาคาร)',
    'บ้านคู้คด',
    'วัดหนองไม้ซุง',
    'วัดสนามทองมิตรภาพที่ 180',
    'วัดโคกช้าง(ราษฎร์บำรุง)',
    'วัดจำปา',
    'วัดโตนดเตี้ย',
    'วัดตะบอง (ธรรมาภิรมอุปถัมภ์)',
    'วัดโบสถ์อนุกูลสังฆกิจ',
    'วัดเจ้าปลุก (เติมรัฐประชาสรรค์)',
    'วัดท่าตอเลิศบุญยงค์วิทยา',
    'วัดน้ำเต้า',
    'วัดบางสงบ (กองวารีวิทยานุสรณ์)',
    'วัดสุวรรณเจดีย์เลิศบุญยงค์อนุสรณ์',
    'วัดโพธิ์ประสิทธิ์ (วิบูลย์บำรุง)',
    'ชุมชนวัดสุเมธ',
    'วัดกระโจมทอง',
    'วัดอุโลม (วิสูตรอนุสรณ์)',
    'วัดโพธิ์',
    'วัดปากคลอง (ประชามหาราช)',
    'วัดนครโปรดสัตว์',
    'วัดไก่เตี้ย',
    'วัดโบสถ์',
    'วัดเขียนลาย (ทองคำวิทยานุสรณ์)',
    'วัดท้าวอู่ทอง',
    'โพธิ์ราษฎร์บำรุง',
    'ชุมชนวัดสำพะเนียง (แร่ผดุงวิทยา)',
    'วัดหลวงพ่อเขียว',
    'อยุธยาวิทยาลัย',
    'จอมสุรางค์อุปถัมภ์',
    'อยุธยานุสรณ์',
    'ปากกรานพิทยา',
    'ท่าเรือ(นิตยานุกูล)',
    'ท่าหลวงวิทยานุกูล',
    'นครหลวง(อุดมรัชต์วิทยา)',
    'ท่าช้างวิทยาคม',
    'บางปะหัน',
    'ภาชี (สุนทรวิทยานุกูล)',
    'วิเชียรกลิ่นสุคนธ์อุปถัมภ์',
    'วังน้อย (พนมยงค์วิทยา)',
    'อุทัย',
    'หนองน้ำส้มวิทยาคม',
    'มหาราช (ประชานิมิต)',
    'บ้านแพรกประชาสรรค์',
    'วัดกระแชง "จันทไพโรจน์วิทยา"',
    'วัดปากน้ำ (ไตรคามสามัคคี)',
    'วัดป่าคา (เจริญอรุณราษฎร์)',
    'วัดอนุกุญชราราม',
    'วัดศิริสุขาราม',
    'วัดช่างเหล็ก (พิบูลเกียรติ)',
    'ไทยรัฐวิทยา 2 (วัดช้างใหญ่)',
    'บ้างเชียงรากน้อย',
    'ศรีบางไทร',
    'ไทยรัฐวิทยา 3 (วัดบางไทร)',
    'วัดเทพสนทวารี',
    'วัดสง่างาม',
    'บ้านบางพลี (บุศย์บูรณะ)',
    'สินสังวาลย์อุทิศ',
    'วัดแก้ว (ประโชติวิทยาภรณ์)',
    'วัดไทรโสภณ (ปราสาททองโอสถสงเคราะห์)',
    'วัดทางยาว',
    'วัดนาคสโมสร (โบราณญาณบํารุง)',
    'วัดโพธิ์แตงใต้ "ปัญญานุวัตรวิทยา"',
    'วัดท่าซุงทักษิณาราม (เอิบ มหาเดชน์อุปถัมย์)',
    'วัดไม้ตราสมาชิการาม',
    'วัดเชิงเลน',
    'วัดโบสถ์สมพรชัย',
    'วัดสนามไชย',
    'บางไทร',
    'วัดหน้าต่างใน (จงนิลอุปถัมภ์)',
    'วัดสุนทราราม',
    'วัดขุนจ่าธรรมาราม',
    'วัดใหม่ (ประพิตรประชาคาร)',
    'วัดโพธิ์',
    'วัดอินทาราม (คำเทศวิทยาคาร)',
    'วัดปราสาททอง',
    'วัดบุญกันนาวาส',
    'วัดไทรน้อย',
    'วัดสีกุก (สุ่นสามัคคีราษฎร์วิทยา)',
    'วัดน้ำเต้า (อุดมราษฎร์นิมิต)',
    'วัดบางปลาหมอ (ราษฎร์รังสรรค์)',
    'วัดโคก',
    'วัดโบสถ์',
    'วัดบางบาล',
    'ประชากรรังสฤษฏ์',
    'วัดกอไผ่ (ปฏิพัทธ์สหราษฎร์)',
    'วัดอัมพวา (วัฒนราษฎร์อุปถัมภ์)',
    'วัดจุฬามณี (ชุณหะจัทนประชาสรรค์)',
    'บ้านหาดทราย',
    'วัดขวิด',
    'วัดพระขาว (ประชานุเคราะห์)',
    'วัดแจ้ง',
    'วัดม่วงหวาน(ส่วนกระบวนยุทธประชาสรรค์)',
    'วัดตะกู (เมตตาชนูปการ)',
    'วัดนกกระจาบ (ศิริสรณ์ประชาสรรค์)',
    'วัดไผ่ล้อม',
    'วัดเชิงท่า',
    'วัดทำเลไทยโปรดสัตว์',
    'วัดวิเวกวายุพัด',
    'คลองจิก (สุขสินนุเคราะห์)',
    'ราษฎร์นิรมิตร',
    'เชียงรากน้อย',
    'วัดธรรมนาวา',
    'วัดเปรมปรีชา',
    'วัดตลาด (อุดมวิทยา)',
    'ราษฎร์บํารุง',
    'วัดราษฎร์ศรัทธาธรรม',
    'บ้านบางกระสั้น',
    'วัดบางเคียน',
    'วัดบ้านกลิ้ง',
    'วัดชีปะขาว "รัฐราษฎร์บูรณะ"',
    'วัดศรีโพธิ์ (เริงทรัพย์วิทยาคาร)',
    'วัดขนอนบ้านกรด',
    'วัดสุทธิรุจิราราม',
    'เจ้าฟ้าสร้าง',
    'บ้านพลับ (วิสุทธิวังราช)',
    'วัดโพธิ์',
    'ปราสาททองวิทยา',
    'วัดชุมพลนิกายาราม',
    'ชุมชนวัดกำแพง',
    'วัดบ้านสร้าง',
    'วัดทำใหม่',
    'วัดบ้านหว้า (ชมพูวิทยา)',
    'เสริมปัญญา (ไวยกัญญาอนุเคราะห์)',
    'วัดลาดระโหง',
    'วัดยม',
    'วัดสามเรือน',
    'วัดฤาไชย (ฤาไชยศึกษา)',
    'วัดโคกทอง (บวรวิทยา)',
    'วัดโพธิ์เผือก',
    'วัดดอนลาน',
    'วัดตาลานเหนือ',
    'วัดท่าดินแดง',
    'วัดนาคู (จันทศึกษาคาร)',
    'วัดบ้านแค',
    'วัดย่านอ่างทอง (นิยมประมุขป',
    'วัดตาลานใต้ (เจริญวิทยา)',
    'วัดราษฎร์นิยม (นิติเกษตรบำรุง)',
    'วัดโพธิ์ (ผักไห่วิทยาคาร)',
    'วัดลาดชิด (ลาดชิดวิทยาคาร)',
    'วัดไผ่ล้อม (โสภณวิทยา)',
    'วัดคงษา (ประชา-รัฐอุปถัมภ์)',
    'วัดลำตะเคียน (วิริยศึกษา)',
    'วัดลาดชะโด',
    'วัดหน้าโคก (วิสุทธิศึกษา)',
    'วัดบ้านอ้อ (บ้านอ้อวิทยาคาร)',
    'วัดอมฤต (สิทธิประสิทธิ์)',
    'รอซีดี',
    'มาลาอีสงเคราะห์',
    'วัดราษฎร์ปุณณาราม',
    'ประสิทธิ์วิทยา',
    'สามัคคีวิทยา',
    'วัดโสภณเจติการาม',
    'คู้สลอด 2 (ชมแช่มวิทย์อุปถัมภ์)',
    'วัดฉัตรทอง (เลื่อนประชานุกูล)',
    'สอนดี (ประชารัฐอนุสรณ์)',
    'วัดบันลือธรรม(ลำกะประชารัฐ)',
    'รุ่งวิทยาประชาอุปถัมภ์',
    'ลาดบัวหลวง (นิ่มนวลอุทิศ)',
    'วัดอินทอารี(สำนักงานสลากกินแบ่งสงเคราะห์289)',
    'วัดหญ้าไทร (ราษฎร์ประสิทธิ์)',
    'อุปลพันธุ์โรจนประสิทธิ์',
    'วัดสุทธาวาส',
    'วัดลาดบัวหลวง (สหมิตรศึกษา)',
    'วัดตรีพาราสีมาเขต',
    'สัตตปทุมบำรุง',
    'วิทยานนท์',
    'วัดราษฎร์บำรุง',
    'หงสประภาสประสิทธิ์',
    'วัดลาดประทุมคงคาราม',
    'วัดปทุมวัน (โสนิกรประชาอนุสรณ์)',
    'ลาดบัวหลวง "ประชาบรรลือฤทธิ์"',
    'วัดเจ้าเจ็ดใน',
    'วัดเจ้าเจ็ดนอก',
    'ชายนาพัฒนา (สำเนียง สนิทอุปถัมภ์)',
    'วัดกลางคลองวัฒนาราม',
    'วัดไชยภูมิ',
    'วัดแก้วสุวรรณ',
    'วัดปิ่นแก้ว',
    'วัดจรเข้ไล่',
    'วัดบางนมโค (ปานอุทิศ)',
    'วัดมารวิชัย (เกษมปกาศิตอนุสรณ์)',
    'วัดสุธาโภชน์ (ไวทย์วรวิทย์)',
    'วัดโบสถ์(ศุภพิทยาคาร)',
    'บ้านแถววิทยาคาร',
    'วัดหนองลำเจียก',
    'วัดโบสถ์ (วงศ์พานิช)',
    'วัดโพธิ์ (แจ่มวิทยาคาร)',
    'วัดกลาง',
    'วัดกระโดงทอง (พิบูลประสิทธิ์)',
    'วัดรางจระเข้',
    'จรัสวิทยาคาร (มิตรภาพที่ 57)',
    'จุฬาราษฎร์วิทยา',
    'วัดอู่ตะเภา',
    'วัดเจ้าแปดทรงไตรย์',
    'ลาดงาวิทยาคม (ปลั่ง ฤกษ์จารี)',
    'ราษฎร์ศุภประดิษฐ์',
    'วัดสามกอ',
    'วัดบ้านแพน "ศรีรัตนานุกูล"',
    'วัดสามตุ่ม',
    'วัดคู้สลอด',
    'วัดหัวเวียง "เขมสุทธิวิทยาคาร"',
    'วัดบางกะทิง(พิศิษฐ์วิทยา)',
    'วัดบันไดช้าง',
    'วัดประดู่โลกเชษฐ์',
    'วัดบางซ้ายใน',
    'โคกตาพรหม',
    'วัดบางซ้ายนอก',
    'วัดมฤคทายวัน',
    'วัดสุคนธาราม',
    'วัดเทพมงคล',
    'วัดฤกษ์บุญมี',
    'วัดวังชะโด',
    'คอตัน',
    'วัดเศวตศิลาราม',
    'วัดใหม่ต้านทาน',
    'วัดสามเพลง',
    'วัดทางหลวง (เทพราษฎร์รังสรรค์)',
    'วัดดอนพัฒนาราม',
    'วัดใหม่หนองคต (ราษฎร์แต่งตั้ง)',
    'คชเวกวิทยา',
    'บางไทรวิทยา',
    'บางบาล',
    'บางปะอิน(ราชานุเคราะห์ ๑)',
    'บางปะอิน',
    'อุดมศีลวิทยา',
    'ผักไห่(สุทธาประมุข)',
    'ลาดชะโดสามัคคี',
    'วัดโพธิ์ผักไห่ (เวชพันธุ์อนุสรณ์)',
    'ลาดบัวหลวงไพโรจน์วิทยา',
    'เสนา(เสนาประสิทธิ์)',
    'สาคลีวิทยา',
    'ลาดงาประชาบำรุง',
    'บางซ้ายวิทยา',
    'วัดโพธิวงษ์',
    'วัดโคศุภราช',
    'วัดจำปาหล่อ',
    'วัดโพทูล',
    'วัดสุวรรณ',
    'วัดตาลเจ็ดช่อ',
    'อนุบาลวัดอ่างทอง',
    'กระทุ่มราย',
    'วัดรุ้ง',
    'วัดปลดสัตว์',
    'บ้านน้ำผึ้ง',
    'วัดบ้านอิฐ',
    'วัดอรัญญิกาวาส',
    'วัดไทรย์',
    'วัดราชปักษี',
    'อนุบาลเมืองอ่างทอง(วัดท้องคุ้ง',
    'วัดลิ้นทอง',
    'วัดมหาดไทย',
    'วัดโบสถ์',
    'วัดจันทร์นิรมิตร',
    'วัดท้ายย่าน',
    'วัดไผ่ล้อม',
    'วัดเซิงหวาย',
    'วัดมะขาม',
    'บ้านชะไว',
    'วัดชัยสิทธาราม',
    'วัดนางเล่ว',
    'วัดบางศาลา',
    'วัดมหานาม',
    'วัดโบสถ์',
    'อนุบาลไชโย(วัดสระเกษอาทรสิกขกา',
    'วัดไชยภูมิ',
    'วัดไชโย',
    'วัดละมุด',
    'วัดบ้านป่า',
    'วัดเทวราช',
    'วัดไทรย์นิโครธาราม',
    'วัดเจ้าบุญเกิด',
    'วัดเยื้อง',
    'วัดดอนกระต่ายทอง',
    'วัดวงษ์ภาศน์',
    'วัดกำแพง',
    'วัดปราสาท',
    'วัดลาดเค้า',
    'ไทยรัฐวิทยา 6',
    'วัดท่า',
    'วัดถนน',
    'วัดพิจารณ์โสภณ',
    'วัดศรีมหาโพธิ์',
    'วัดพายทอง',
    'วัดเกตุ',
    'อนุบาลป่าโมก(วัดโบสถ์สายทอง)',
    'วัดเอกราช',
    'วัดคำหยาด',
    'วัดโพธิ์ทอง',
    'วัดโคกพุทรา',
    'วัดจันทราราม',
    'วัดไตรรัตนาราม',
    'วัดทางพระ',
    'วัดท่าโขลง',
    'วัดยางทอง',
    'วัดโพธิ์ราษฎร์',
    'วัดบ้านสร้าง',
    'อนุบาลโพธิ์ทอง',
    'วัดท่าอิฐ',
    'วัดสามประชุม',
    'วัดโบสถ์',
    'วัดข่อย',
    'วัดบุญเกิด',
    'วัดศรีกุญชร',
    'วัดยางช้าย',
    'วัดงิ้วราย',
    'วัดลั่นทม',
    'วัดม่วงคัน',
    'วัดโพธิ์เอน',
    'วัดโพธิ์เกรียบ',
    'วัดสว่าง',
    'วัดสนธิธรรม',
    'วัดจุฬามุนี',
    'วัดยางมณี',
    'วัดสุวรรณราชหงษ์',
    'วัดทองกลาง',
    'วัดท่าสามัคคี',
    'วัดศีลขันธาราม',
    'วัดน้ำอาบ',
    'วัดบุญศิริวิทยาราม',
    'วัดป่ามุนี',
    'วัดท่าตลาด',
    'วัดหัวสะแกตก',
    'วัดรัตนาราม',
    'ชุมชนวัดริ้วหว้า',
    'บ้านดอนกร่าง',
    'วัดวังน้ำเย็น',
    'วัดหมื่นเกลา',
    'วัดหนองยาง',
    'วัดรางฉนวน',
    'วัดจันทร์มณี',
    'วัดบ้านแก',
    'วัดบ้านพราน',
    'บ้านหนองเสือ',
    'วัดศรีบัวทอง',
    'วัดแก้วกระจ่าง',
    'อนุบาลแสวงหา',
    'พวงทองอุปถัมภ์',
    'วัดบ้านเพชร',
    'วัดทองเลื่อน',
    'วัดยาง',
    'ประสิทธิวิทยา',
    'วัดนางชำ',
    'วัดโพธิ์ศรี',
    'วัดตลาดใหม่',
    'วัดโพธิ์',
    'วัดลาดเป็ด',
    'วัดคลองพูล',
    'วัดบางจักร',
    'วัดขุมทอง',
    'วัดราชสกุณา',
    'วัดวันอุทิศ',
    'วัดไผ่วง',
    'วัดต้นทอง',
    'วัดยางมณี',
    'วัดศาลาดิน',
    'วัดน้อย',
    'วัดอบทม',
    'บ้านไผ่หมูขวิด',
    'วัดหลวง',
    'บ้านศาลาดิน',
    'วัดมะนาวหวาน',
    'อนุบาลวัดนางใน(ละเอียดอุปถัมภ์)',
    'ชุมชนวัดวิเศษชัยชาญ',
    'บ้านห้วยคล้า',
    'วัดสิทธาราม',
    'วัดน้ำพุ',
    'วัดสี่ร้อย',
    'วัดทำนบ',
    'วัดหลักแก้ว',
    'วัดลานช้าง',
    'วัดคลองสำโรง',
    'วัดใหม่ทางข้าม',
    'วัดห้วยโรง',
    'วัดห้วยคันแหลน',
    'วัดหัวตะพาน',
    'วัดแปดแก้ว',
    'วัดโบสถ์',
    'วัดท่าชุมนุม',
    'บ้านหนองถ้ำ',
    'วัดมงคลธรรมนิมิต',
    'วัดหนองกร่าง',
    'วัดไผ่แหลม',
    'วัดเกษทอง',
    'วัดสามโก้',
    'อนุบาลสามโก้(บ้านลำสนุ่น)',
    'บ้านดอนตาวง',
    'วัดสามขาว',
    'ปาโมกข์วิทยาภูมิ',
    'แสวงหาวิทยาคม',
    'วิเศษไชยชาญ(ตันติวิทยาภูมิ)',
    'อ่างทองปัทมโรจน์วิทยาคม',
    'ราชสถิตย์วิทยา',
    'บางเสด็จวิทยาคม',
    'โพธิ์ทองพิทยาคม',
    'วิเศษชัยชาญวิทยาคม',
    'ไผ่วงวิทยา',
    'สตรีอ่างทอง',
    'จำปาหล่อพิทยาคม',
    'โพธิ์ทอง(จินดามณี)',
    'ริ้วหว้าวิทยาคม',
    'สามโก้วิทยาคม',
    'วัดโป่งน้อย',
    'วัดดงน้อย',
    'วัดใหม่ขุนนวน',
    'วัดบ้านดาบ',
    'วัดโบสถ์',
    'วัดสิริจันทรนิมิตร(ประกิจประชานุกูล)',
    'กองบินโคกกะเทียม',
    'วัดทุ่งสิงห์โต',
    'ศูนย์การบินทหารบกอุปถัมภ์',
    'โรงพยาบาลอานันทมหิดล',
    'ชุบศร(ร.๓๑ พัน ๑ อุปถัมภ์)',
    'วัดสระมะเกลือ',
    'วัดดงสวอง',
    'วัดหนองบัวขาว',
    'บ้านโคกกะเทียม(ราษฎร์ดำริบำรุง)',
    'วัดคอกกระบือ "จอมประชาสรรค์"',
    'วัดหนองปลิง',
    'ซอย 3 สาย 4 ซ้าย',
    'บ้านห้วยส้ม',
    'ซอย 7 สาย 4 ซ้าย',
    'ซอยพิเศษสาย 4 ซ้าย',
    'บ้านโคกตูม',
    'โคกลำพานวิทยา',
    'วัดธรรมเจดีย์',
    'วัดโพธิ์ลอย',
    'วัดชีแวะ',
    'บ้านไทรย้อย',
    'วัดดอนโพธิ์',
    'วัดใดใหญ่',
    'วัดใดยาว',
    'วัดหลวงสุวรรณาราม',
    'วัดโคกโพธิกุญชร',
    'วัดยาง ณ รังสี',
    'วัดตะลุง',
    'วัดบางพุทโธ',
    'วัดถนนแค',
    'สาริมานอุปถัมภ์',
    'อนุบาลจังหวัดทหารบกลพบุรี',
    'อนุบาลลพบุรี',
    'เมืองใหม่(ชลอราษฎร์รังสฤษฏ์)',
    'วัดตองปุ',
    'บ้านหนองแขม',
    'วัดท่าแค',
    'วัดป่ากล้วย',
    'ค่ายพิบูลสงคราม',
    'วัดหลวงท้ายตลาด',
    'วัดตะเคียน',
    'วัดท่าข้าม',
    'วัดเขาหนีบ',
    'ไทยรัฐวิทยา 1',
    'บ้านหนองถ้ำ',
    'วัดซับเสือแมบ',
    'พิบูลสงเคราะห์1',
    'วัดนิคมสามัคคีชัย',
    'วัดใหม่จำปาทอง',
    'ซอย 5 สาย 2 ซ้าย',
    'วัดหนองแก้ว',
    'วัดโพธิ์ระหัต',
    'วัดสิงห์ทอง "บุญมีประชานุเคราะห์"',
    'วัดอัมพวัน',
    'วัดทองแท่งนิสยาราม',
    'วัดข่อยกลาง',
    'วัดข่อยใต้',
    'พลร่มอนุสรณ์ มิตรภาพที่ ๕๐',
    'วัดเทพกุญชรวราราม',
    'บ้านสามเรือน(วันครู ๒๕๐๐)',
    'วัดพรหมาสตร์',
    'วัดโพธิ์เก้าต้น',
    'วัดบ่อเงิน',
    'วัดสำราญ',
    'วัดโคกหม้อ',
    'วัดไผ่แตร',
    'วัดคุ้งนาบุญ',
    'วัดวังหัวแหวน',
    'บ้านชอนอุดม',
    'บ้านหนองชนะชัย',
    'บ้านชอนบอน',
    'บ้านคลองเกตุ',
    'บ้านโคกสะอาด',
    'บ้านหัวดง',
    'บ้านเขาดิน',
    'บ้านหนองบัว',
    'วัดหนองพิมาน',
    'โคกสำโรง',
    'วัดรัตนาราม',
    'บ้านเนินจันทร์',
    'บ้านดงมะรุม',
    'บ้านวังไผ่',
    'บ้านตะกุดหว้า',
    'บ้านถลุงเหล็ก',
    'บ้านหนองปล้อง',
    'อนุบาลบ้านเพนียด',
    'บ้านนกเขาเปล้า',
    'บ้านวังขอนขว้าง',
    'วัดดงแก้ว',
    'บ้านวังจั่น',
    'บ้านโป่งยอ',
    'บ้านวังเพลิง',
    'บ้านมะม่วงเจ็ดต้น',
    'บ้านเขาเตียน มิตรภาพที่ 134',
    'บ้านพุม่วง',
    'บ้านทุ่งทอง',
    'วัดสะพานคง',
    'วัดสะแกราบ',
    'บ้านหนองสำโรง',
    'วัดหนองแขม',
    'บ้านหนองไผ่',
    'บ้านท่าม่วง',
    'บ้านหลุมข้าว',
    'บ้านพรมทิน',
    'วัดหนองหอย',
    'บ้านเขาสะพานนาค',
    'บ้านเขาทับควาย',
    'วัดห้วยโป่ง',
    'วัดสัมพันธมิตร',
    'วัดหนองคู',
    'สมอคอนวิทยาคาร',
    'วัดถ้ำตะโก',
    'วัดเขาสมอคอน',
    'วัดคลองเม่า',
    'วัดรัมภาราม',
    'วัดเกตุ',
    'ท่าวุ้ง',
    'วัดปากคลองบางคู้',
    'อนุบาลท่าวุ้ง',
    'วัดโพธิ์ศรี(สังวรราษฎร์นุสรณ์)',
    'วัดหนองมน',
    'บ้านบางงา(รัฐประชาสรรค์)',
    'บ้านบางลี่',
    'สิงหฤกษ์ประสิทธิ์',
    'วัดหนองปลาดุก',
    'วัดหนองหลวง',
    'บ้านม่วงอยู่ประยงค์ ณ บ้านเบิก',
    'วัดโพธิ์ตรุ',
    'วัดเกาะวิมุตตาราม',
    'วัดบ้านลาด',
    'วัดสนามไชย',
    'บ้านดอนทอง',
    'วัดมุจลินท์',
    'วัดลาดสาลี่',
    'วัดยวด',
    'วัดหัวสำโรง',
    'วัดมะค่า',
    'วัดคงคาราม',
    'วัดโพธิ์เกษตร',
    'บ้านดงกลาง',
    'อนุบาลบ้านหมี่',
    'วัดดงพลับ',
    'วัดโป่งแค',
    'บ้านกระเบากลัก',
    'บ้านดอนดึง',
    'วัดบางกะพี้(พสกอำรุง)',
    'วัดวังใต้',
    'วัดเทพอำไพ',
    'วัดธรรมิการาม',
    'วัดคุ้งท่าเลา',
    'บ้านกล้วย "พวงราษฎร์บำรุง"',
    'วัดท่าช้าง',
    'วัดท้องคุ้ง',
    'บ้านเกริ่นกฐิน',
    'วัดบ้านทราย',
    'บ้านหนองนางาม',
    'บ้านหัวบึง',
    'วัดน้ำจั้น',
    'วัดหนองน้ำทิพย์',
    'วัดสระตาแวว',
    'วัดบ้านหมี่ใหญ่',
    'วัดทุ่งทะเลหญ้า',
    'วัดมหาสอน "ไคยฤทธิ์วิทยานุสรณ์"',
    'วัดบางพึ่ง',
    'วัดห้วยแก้ว',
    'พิบูลปัทมาคม',
    'วัดเขาวงกฎ',
    'บ้านเขาสาริกา',
    'วัดสายห้วยแก้ว',
    'วัดหนองกระเบื้อง',
    'บ้านไผ่ใหญ่',
    'วัดหนองโพธิ์',
    'วัดท้ายลาด',
    'วัดหนองคู',
    'วัดหนองกระเบียน',
    'วัดพานิชธรรมิการาม',
    'บ้านโคกสุข',
    'วัดบ้านลาด',
    'วัดสระกระเบี้อง',
    'บ้านสระเตย',
    'วัดเนินยาว',
    'วัดเนินยาว(สาขาวัดบ้านคลอง)',
    'บ้านน้ำบ่า',
    'บ้านหนองแก',
    'วัดหนองเมือง',
    'วัดห้วยกรวด',
    'ชุมชนตำบลหินปัก',
    'โคกสำโรงวิทยา',
    'พิบูลวิทยาลัย',
    'โคกกะเทียมวิทยาลัย',
    'จุฬาภรณราชวิทยาลัย ลพบุรี',
    'ดงตาลวิทยา',
    'พระนารายณ์',
    'บ้านข่อยวิทยา',
    'โคกตูมวิทยา',
    'ลพบุรีปัญญานุกูล',
    'ท่าวุ้งวิทยาคาร',
    'บ้านเบิกวิทยาคม',
    'บ้านหมี่วิทยา',
    'ปิยะบุตร์',
    'บ้านชีวิทยา',
    'โสตศึกษาปานเลิศจังหวัดลพบุรี',
    'วัดโคกสลุง',
    'วัดหนองตามิ่ง',
    'ไทยรัฐวิทยา 100(บ้านคันนาหิน)',
    'บ้านเขาขวาง',
    'บ้านห้วยสะอาด',
    'วัดราษฎร์สามัคคี',
    'วัดดำรงบุล',
    'วัดมณีศรีโสภณ',
    'ช่องสาริกา',
    'บ้านหนองโพธิ์',
    'ซอย 20 สาย 4 ขวา',
    'ซอย 19 สาย 2 ขวา',
    'บ้านดีลัง',
    'พรหมรังษี',
    'ซอย 12 สาย 4 ซ้าย',
    'บ้านน้ำสุด',
    'บ้านคลองสาริกา',
    'บ้านฟ่าวงาม',
    'วัดหนองนา',
    'บ้านห้วยบง',
    'ซอย 17 สาย 2 ซ้าย',
    'ซอย 16 สาย 3 ซ้าย',
    'อนุบาลพัฒนานิคม',
    'ซอย 26 สาย 4 ซ้าย',
    'บ้านมะนาวหวาน',
    'บ้านแก่งน้ำเย็น',
    'ชุมชนบ้านแก่งเสือเต้น',
    'บ้านสหพันธ์อ่างทอง',
    'บ้านสวนมะเดื่อ',
    'บ้านซับโศก',
    'วัดสี่ซับศรีเจริญธรรม',
    'บ้านคลองกลุ่ม',
    'บ้านหนองปีกนก',
    'บ้านเกาะรัง',
    'บ้านหนองบง',
    'บ้านลังกาประชาสรรค์',
    'บ้านหนองปล้อง',
    'บ้านเขาแหลม',
    'บ้านห้วยหิน',
    'บ้านเขายายกะตา',
    'บ้านโกรกรกฟ้า',
    'วัดศิริบรรพต',
    'ชุมชนบ้านชัยบาดาล',
    'บ้านลำโกฎิทอง',
    'บ้านท่ามะกอก',
    'บ้านซับตะเคียน',
    'บ้านหนองโกวิทยา',
    'บ้านเขาตะแคง',
    'บ้านซับกระโดน',
    'บ้านท่าดินดำ',
    'บ้านวังก้านเหลือง',
    'บ้านหนองปลาไหล',
    'บ้านท่ามะนาว',
    'บ้านสันตะลุง',
    'บ้านนาโสม',
    'บ้านซับงูเหลือม',
    'บ้านใหม่ทรัพย์เจริญ',
    'นิคมลำนารายณ์',
    'บ้านไร่พัฒนา',
    'บ้านโค้งรถไฟ',
    'บ้านบัวชุม',
    'บ้านเขาสมโภชน์',
    'บ้านซับเค้าแมว',
    'บ้านคลองไทรพัฒนา',
    'บ้านธงชัยสามัคคี',
    'บ้านซับหินขวาง',
    'บ้านใหม่สามัคคี',
    'บ้านห้วยนา',
    'บ้านม่วงค่อม',
    'วัดสว่างอารมณ์',
    'บ้านมะกอกหวาน',
    'อนุบาลลำนารายณ์',
    'โพธิ์นิมิตร',
    'บ้านซับสมบูรณ์',
    'วัดสุนทรเทพคีรี',
    'บ้านซับไทร',
    'บ้านซับผาสุก',
    'บ้านดงน้อย',
    'บ้านเขาตำบล',
    'วัดโพธิ์งาม',
    'บ้านบ่อน้ำ',
    'บ้านทุ่งตาแก้ว',
    'บ้านห้วยดีเลิศ',
    'บ้านท่ากรวด',
    'บ้านเนินทอง',
    'บ้านโป่งสวองคีรีวรรณ์',
    'บ้านทรัพย์เจริญ',
    'บ้านซับจำปา',
    'บ้านทะเลวังวัด',
    'บ้านบ่อคู่',
    'บ้านท่าตะโก',
    'บ้านท่าหลวง',
    'อนุบาลท่าหลวง',
    'บ้านหนองประดู่',
    'บ้านหนองหัวช้าง',
    'บ้านหนองน้ำใส',
    'บ้านหนองกระสังข์',
    'บ้านเนินสวอง',
    'ทรัพย์ราษฎร์บำรุง',
    'บ้านทุ่งดินแดง',
    'บ้านหัวลำ',
    'บ้านหนองจาน',
    'บ้านทุ่งท่าช้าง',
    'วัดซับตะกั่ว',
    'บ้านด่านจันทร์',
    'บ้านดงหลุ่ม',
    'บ้านเขาหมูมัน',
    'บ้านดงน้อย',
    'บ้านห้วยเขว้า',
    'บ้านวังแขม',
    'บ้านคลอง',
    'บ้านมหาโพธิ',
    'อนุบาลสระโบสถ์',
    'บ้านห้วยใหญ่',
    'อนุบาลโคกเจริญ',
    'บ้านทะเลทอง',
    'บ้านดินแดง',
    'บ้านโคกแสมสาร',
    'บ้านเขาราบ',
    'บ้านวังตาอินทร์',
    'บ้านสระเพลง',
    'บ้านยางราก',
    'บ้านพุกะชัด',
    'บ้านห้วยสาราม',
    'บ้านลำโป่งเพชร',
    'บ้านบ่อตะเคียนทอง',
    'บ้านแหลมชนแดน',
    'บ้านหนองมะค่า',
    'บ้านกุดตาเพชร',
    'บ้านราษฎร์บำรุง',
    'หมู่บ้านป่าไม้ซับลังกา',
    'บ้านเหวตาบัว',
    'บ้านปรางค์น้อย',
    'บ้านเขารวก',
    'บ้านวังทอง',
    'บ้านศรีเมือง',
    'บ้านด่านไทยล้อม',
    'บ้านเขาดินทอง',
    'บ้านท่าเยี่ยม',
    'บ้านลำสนธิ',
    'บ้านหนองเกตุ',
    'อนุบาลอำเภอลำสนธิ (บ้านหนองรี)',
    'ชุมชนวัดจงโก มิตรภาพที่ ๑๕๗',
    'บ้านหนองขาม',
    'บ้านชอนสมบูรณ์',
    'บ้านหนองไทร',
    'บ้านหนองกลาง',
    'ชุมชนวัดคีรีนาครัตนาราม',
    'บ้านป่าเขว้า',
    'บ้านหนองสำราญ',
    'บ้านโคกกลาง',
    'บ้านบ่อดินสอพอง',
    'บ้านดงดินแดง',
    'บ้านหนองเสมา',
    'บ่อทองลพบุรี',
    'บ้านใหม่โสพิมพ์',
    'บ้านหนองไก่ห้าว',
    'บ้านไร่ทรัพย์เจริญ',
    'บ้านหนองตะแบก',
    'บ้านยางโทน',
    'อนุบาลวัดหนองม่วง',
    'พัฒนานิคม',
    'ชัยบาดาลวิทยา',
    'โคกสลุงวิทยา',
    'ชัยบาดาลพิทยาคม',
    'ขุนรามวิทยา',
    'เฉลิมพระเกียรติสมเด็จพระศรีนครินทร์ ลพบุรี',
    'ท่าหลวงวิทยาคม',
    'สระโบสถ์วิทยาคาร',
    'โคกเจริญวิทยา',
    'ยางรากวิทยา',
    'หนองรีวิทยา',
    'หนองม่วงวิทยา',
    'ราชประชานุเคราะห์ 33',
    'วัดจักรสีห์',
    'ชุมชนวัดพระนอนจักรสีห์มิตรภาพที่ 133',
    'วัดแจ้งพรหมนคร',
    'วัดศรีสาคร',
    'วัดคลองสามแยก',
    'วัดกระดังงา',
    'วัดประโชติการาม',
    'วัดสะอาดราษฎร์บำรุง',
    'อนุบาลสิงห์บุรี',
    'วัดพรหมสาคร',
    'วัดโคกพระ',
    'วัดราษฎร์ประสิทธิ์',
    'วัดสังฆราชาวาส',
    'วัดเสาธงทอง',
    'วัดหัวว่าว',
    'บ้านบางสำราญ',
    'วัดตึกราชา',
    'วัดพระปรางค์มุนี',
    'วัดศรัทธาภิรม',
    'วัดข่อย',
    'วัดตะโหนด',
    'วัดโพธิ์ชัย',
    'อนุบาลเมืองสิงห์บุรี',
    'ชุมชนวัดพระปรางค์วิริยวิทยา',
    'วัดโพธิ์รัตนาราม',
    'วัดโพธิ์หอม',
    'วัดบ้านจ่า (เอี่ยมโหมดอนุสรณ์)',
    'วัดกลางชูศรีเจริญสุข',
    'วัดชะอมสามัคคีธรรม',
    'บ้านทุ่งกลับ',
    'วัดห้วย',
    'วัดน้ำผึ้ง',
    'วัดวังขรณ์',
    'วัดชันสูตร',
    'วัดแหลมคาง',
    'วัดดาวเรือง',
    'เรืองเดชประชานุเคราะห์',
    'วัดตลาดโพธิ์',
    'อนุบาลบางระจัน',
    'วัดคีม',
    'วัดดอนเจดีย์',
    'บ้านทุ่งว้า',
    'วัดสามัคคีธรรม',
    'วัดประสิทธิ์คุณากร',
    'วัดน้อยนางหงษ์',
    'ชุมชนบ้านไม้ดัด',
    'วัดตะโกรวม',
    'วัดวังกะจับ',
    'ชุมชนวัดกลางท่าข้าม',
    'วัดโพธิ์ศรี',
    'วัดสาธุการาม',
    'วัดขุนสงฆ์',
    'ค่ายบางระจัน',
    'วัดประดับ',
    'อนุบาลค่ายบางระจัน',
    'บ้านหนองลีวิทยาคม',
    'วัดบ้านกลับ',
    'วัดโพธิ์ทะเลสามัคคี',
    'วัดโพธิ์สังฆาราม',
    'ชุมชนวัดม่วง',
    'วัดสิงห์',
    'ไทยรัฐวิทยา 56 (บ้านหนองกระทุ่ม)',
    'อนุบาลพรหมบุรี',
    'วัดโพธิ์เอน',
    'วัดโภคาภิวัฒน์',
    'วัดหลวง',
    'วัดกลางธนรินทร์',
    'วัดเสาธงทอง',
    'บ้านเก่า',
    'วัดเก้าชั่ง',
    'วัดโคปูน',
    'ชุมชนวัดตราชู',
    'วัดอัมพวัน',
    'วัดชีปะขาว',
    'วัดเตย',
    'วัดประสาท',
    'ชุมชนวัดเทพมงคล',
    'ชุมชนวัดเทพมงคล สาขาวัดป่าหวาย',
    'วัดพรหมเทพาวาส',
    'วัดถอนสมอ',
    'วัดโบสถ์',
    'อนุบาลท่าช้าง',
    'วัดพิกุลทอง',
    'ชุมชนวัดเสาธงหิน',
    'วัดโสภา',
    'วัดวิหารขาว',
    'วัดกระโจม',
    'วัดไผ่ขาด',
    'วัดเพิ่มประสิทธิผล',
    'วัดน้อย',
    'วัดบางปูน',
    'วัดระนาม',
    'วัดราษฎร์ศรัทธาทำ',
    'วัดกลาง',
    'วัดคลองโพธิ์ศรี',
    'วัดเชียงราก',
    'วัดเซ่าสิงห์',
    'ชุมชนวัดดงยาง',
    'วัดไผ่ดำ (มิตรภาพที่ 183)',
    'วัดล่องกะเบา',
    'ชุมชนวัดพระนอน',
    'วัดยาง',
    'วัดสุทธาวาส',
    'บ้านหัวดง',
    'วัดท่าอิฐ',
    'วัดกำแพง',
    'วัดกำแพง',
    'วัดตุ้มหู',
    'วัดโฆสิทธาราม',
    'วัดทอง',
    'วัดศรีสำราญ',
    'วัดดอกไม้',
    'วัดปลาไหล',
    'วัดเสือข้าม',
    'วัดสว่างอารมณ์',
    'วัดกระทุ่มปี่',
    'วัดบ้านลำ',
    'วัดสองพี่น้อง',
    'บ้านคูเมือง',
    'วัดหนองสุ่ม',
    'วัดการ้อง',
    'วัดเฉลิมมาศ',
    'อนุบาลอินทร์บุรี(วัดโพธิ์ศรี)',
    'วัดประศุก',
    'วัดโบสถ์',
    'ท่าช้างวิทยาคาร',
    'หัวไผ่วิทยาคม',
    'บางระจันวิทยา',
    'ค่ายบางระจันวิทยาคม',
    'พรหมบุรีรัชดาภิเษก',
    'อินทร์บุรี',
    'สิงห์บุรี',
    'ศรีศักดิ์สุวรรณวิทยา',
    'บ้านแป้งวิทยา',
    'ทองเอนวิทยา',
    'สิงหพาหุ(ประสานมิตรอุปถัมภ์)',
    'ศรีวินิตวิทยาคม',
    'วัดหลวงพ่อขาว',
    'วัดส่องคบ่(ท้ายเมืองอนุสรณ์)',
    'วัดฝาง',
    'วัดไผ่โพธิ์ทอง',
    'วัดสะพาน(สำเภาผดุงวิทย์)',
    'วัดงิ้ว่(เรือไทยสงเคราะห์)',
    'วัดอัมพวัน',
    'บ้านท่าไม้',
    'วัดธรรมามูล',
    'ชุมชนวัดดักคะนน',
    'วัดบ้านกลำ',
    'วัดสระเนินพระราม',
    'บ้านหนองแค',
    'วัดใหม่ศรัทธาราษฎร์',
    'ชุมชนวัดวังเคียน',
    'วัดนางลือ',
    'วัดศรีวิชัย',
    'พระยาตาก',
    'อนุบาลชัยนาท',
    'ลัดดาประชาสรรค์',
    'อนุบาลเมืองชัยนาท',
    'วัดโรงวัว',
    'วัดหนองเต่าดำ',
    'วัดแหลมหว้า',
    'วัดดอนรังนก',
    'วัดหนองพังนาค',
    'วัดเนินถ่าน',
    'รัฐเขื่อนพลเทพ่อุปถัมภ์',
    'วัดใหม่วงเดือน',
    'วัดพิกุลงาม',
    'ชุมชนวัดศรีมณีวรรณ(ธรรมศิริอุปถัมภ์)',
    'อนุบาลมโนรมย์',
    'วัดสะพานหิน(คุรุราษฎร์ศึกษา)',
    'วัดใหญ่(นากประสุตประชานุสรณ์)',
    'ไทยรัฐวิทยา57(บ้านท่าฉนวน)',
    'วัดหัวยาง(รัฐราษฎร์นุเคราะห์)',
    'บ้านหางน้ำหนองแขม(โออุปถัมภ์)',
    'บ้านไร่พัฒนา(ราษฎร์บูรณะวิทยา)',
    'บ้านเขาแหลม',
    'บ้านหัวถนน',
    'บ้านหนองมะขาม(สวัสดิ์ศานุสรณ์)',
    'วัดหัวหว้า',
    'วัดคลองกลาง(พวงเกิดราษฎร์รังสรรค์)',
    'วัดโคกแจง(ประชานุกุลวิทยาสำนักงานสลากกินแบ่งสมทบสร',
    'บ้านเนินไผ่(ไพศาลชลประชาสรรค์)',
    'วัดธรรมขันธ์(ประชาสงเคราะห์)',
    'ชุมชนบ้านหางน้ำสาคร(รัฐราษฎร์ร่วมจิตร)',
    'วัดหางแขยง(สถิตราษฎร์บำรุง)',
    'บ้านดอนแตง',
    'คงรักษ์ประชานุเคราะห์',
    'วัดอู่ตะเภา(สถิตราษฎร์นุกูล)',
    'วัดหนองตาตน',
    'วัดบ่อแร่(วิจิตราษฎร์บำรุง)',
    'วัดหนองจิก',
    'วัดคลองบุญ',
    'วัดหนองน้อย',
    'อนุบาลวัดสิงห์',
    'สำราญราษฎร์บำรุง',
    'วัดวังหมัน',
    'บ้านท่าข้ามวังน้ำ',
    'บ้านสามัคคีธรรมวิทยา',
    'บ้านหนองขุ่นมิตรภาพที่136',
    'บ้านทุ่งกว้าง',
    'วัดโคกสุก',
    'วัดดอนตาล',
    'วัดดอนตูมกมลาวาส',
    'วัดหนองบัว(รัฐปทุมราษฎร์อุปถัมภ์)',
    'วัดนมโฑ(สำนักงานสลากกินแบ่งสมทบสร้าง338)',
    'วัดเขาแก้ว',
    'บ้านโป่งแค',
    'ชุมชนวัดโคกเข็ม(พุทธสรานุสรณ์)',
    'วัดบ้านหนอง',
    'บางไก่เถื่อน(ตันติวิสิษฐประชานุกูล)',
    'วัดอินทาราม(เสนาณรงค์อุปถัมภ์3)',
    'เขื่อนเจ้าพระยา',
    'ชุมชนวัดสวนลำไย(เสนาณรงค์อุปถัมภ์2)',
    'วัดกรุณา',
    'วัดมะฝ่อ(สรรพวิทยา)',
    'วัดโคกจันทน์',
    'บ้านคลองยาง',
    'วัดตะกู(สมจิตรัชนีอุปถัมภ์)',
    'วัดคงคาราม(คุ้มมั่นพุมมะระประชาสรรค์)',
    'วัดมะปราง',
    'วัดไผ่ล้อม(ทองดีประชานุกูล)',
    'วัดสมอ(วุฒาประชานุเคราะห์)',
    'วัดดอนตะไล้',
    'อนุบาลสรรพยา',
    'วัดโพธิมงคล',
    'วัดกำแพง',
    'วัดหลวงสิริบูรณาราม',
    'วัดยางศรีเจริญ',
    'วัดศรีมงคล(สถิตมงคลราษฎร์อุปถัมภ์)',
    'วัดโพธิ์ประสิทธิ์',
    'วัดหาดอาษา',
    'บ้านทุ่งกระถิน',
    'ชุมชนวัดโคกดอกไม้',
    'วัดเทพรัตนวนาราม',
    'เทพรัตน์',
    'วัดอารีทวีวนาราม',
    'วัดหนองแขม',
    'วัดโพธิ์งาม',
    'วัดสนามชัย',
    'วัดสังฆาราม(ปลื้มประชาสงเคราะห์)',
    'วัดมะเห-ยงคณ์(ประชาเนรมิต)',
    'วัดท่ากระแส',
    'วัดโพธิ์ทอง',
    'วัดหัวเด่น',
    'วัดโฆสิตาราม',
    'วัดหอระฆัง',
    'วัดสกุณาราม(ประสิทธิ์ชัยประชาสรรค์)',
    'วัดท่าสมอ(สำนักงานสลากกินแบ่งสงเคราะห์258)',
    'วัดสระไม้แดง',
    'วัดกำแพง',
    'วัดหัวตะพาน',
    'วัดพระแก้ว',
    'อนุบาลสรรคบุรี',
    'วัดใหม่สรรคบุรีวิทยา(สำนักงานสลากกินแบ่งสงเคราะห์)',
    'วัดโพธาราม',
    'วัดพร้าว',
    'วัดโบสถ์ราษฎร์บำรุง',
    'วัดท่า',
    'วัดธรรมิกาวาส',
    'วัดจั่นเจริญศรี',
    'วัดโบสถ์',
    'วัดบำเพ็ญบุญ',
    'วัดจันทน์',
    'วัดคลองงิ้ว',
    'วัดดอนโพธิ์ศรี',
    'ชุมชนวัดมาติการาม',
    'วัดราษฎร์ศรัทธาราม',
    'วัดเด่นใหญ่',
    'บ้านสระแก้ว',
    'บ้านหนองแจง',
    'วัดประชุมธรรม(สัจจะญาณ)',
    'วัดวิจิตรังสิตาราม',
    'ชุมชนวัดพิชัยนาวาส(รัฐราษฎร์ประดิษฐ์วิทยา)',
    'วัดบ้านใหม่',
    'ไกรราษฎร์วิทยา',
    'บ้านวังเดือนห้า',
    'บ้านรางจิก',
    'วัดพรหมวิหาร',
    'บ้านไพรนกยูง(วันชัยประชาสรรค์)',
    'บ้านหนองอ้ายสาม',
    'วัดคลองธรรม',
    'บ้านดอนกะโดน',
    'ท่าบ้านหลวง(รัฐรังสรรค์วิทยา)',
    'วัดวิจิตรรังสรรค์',
    'วัดสะตือสิงห์(ประดิษฐ์ราษฎร์อุปถัมภ์)',
    'วัดถ้ำเข้',
    'วัดวงเดือน',
    'วัดคลองเกษม',
    'วัดท่าโบสถ์(สำนักงานสลากกินแบ่งสงเคราะห์343)',
    'วัดศรีเจริญธรรม',
    'วัดอรัญญวาสี',
    'บ้านชัฏฝาง',
    'ดอนสีนวน(สำนักงานสลากกินแบ่งสงเคราะห์523',
    'วัดหนองแซง',
    'วัดสระดู่',
    'วัดทองนพคุณ',
    'บ้านหมื่นเทพ',
    'วัดท่าแก้ว',
    'ชุมชนคลองจันทน์',
    'วัดโคกหมู',
    'บ้านหนองต่อ',
    'อนุบาลหันคา(วัดท่ากฤษณาสุชัยประชาสรรค์)',
    'วัดสวนอัมพวัน',
    'วัดศรีสโมสร',
    'บ้านวังตะเคียน(ขยันการนาวีราษฎร์อุทิศ)',
    'บ้านวังหัวเรือ',
    'วัดวังน้ำขาว',
    'บ้านหนองหวาย',
    'อนุบาลหนองมะโมง',
    'บ้านน้ำพุ',
    'บ้านสะพานหิน(ประชาสามัคคี)',
    'บ้านโพธิ์ทองชัยประสิทธิ์(จำรองฯ',
    'บ้านหนองตะขบ',
    'บ้านดอนใหญ่',
    'วัดเขาดิน(วันครู่2502)',
    'บ้านเขาเกล็ด',
    'บ้านบ่อยายส้ม(แก้วประชาสรรค์)',
    'บ้านเก่า',
    'บ้านทุ่งโพธิ์',
    'บ้านกะบกเตี้ย',
    'บ้านกลาง(วิโรจน์สามัคคีประชาสรรค์)',
    'บ้านเขาราวเทียนทอง(ประดิษฐ์ราษฎร์อุปถัมภ์)',
    'อนุบาลเนินขาม',
    'บ้านวังคอไห(สงฆ์ประชาชนูทิศ)',
    'บ้านหนองยาง',
    'บ้านสุขเดือนห้า',
    'บ้านหนองเด่น',
    'ชัยนาทพิทยาคม',
    'ชัยนาทพิทยาคม 2',
    'ศึกษาสงเคราะห์ชัยนาท',
    'อุลิตไพบูลย์ชนูปถัมภ์',
    'สาครพิทยาคม',
    'วัดสิงห์',
    'สรรพยาวิทยา',
    'บุญนาคพิทยาคม',
    'ชยานุกิจพิทยาคม',
    'คุรุประชาสรรค์',
    'ห้วยกรดวิทยา',
    'หันคาราษฎร์รังสฤษดิ์',
    'หันคาพิทยาคม',
    'ศรีสโมสรวิทยา',
    'เนินขามรัฐประชานุเคราะห์',
    'วัดกุดนกเปล้า',
    'วัดทุ่งสาริกา(ที-สาโรจน์อุปถัมภ์)',
    'วัดโนนสภาราม',
    'วัดถนนเหล็ก',
    'วัดป่าสัก',
    'วัดหนองบัว',
    'วัดห้วยลี่',
    'วัดบ้านกล้วย',
    'บ้านปากข้าวสาร(บริษัทข้าวไทยจำกัดสงเคราะ',
    'วัดสุวรรณคีรี',
    'อนุบาลสระบุรี',
    'อนุบาลศาลาแดง',
    'ชุมชนวัดหนองโนใต้',
    'วัดป๊อกแป๊ก',
    'วัดนาร่อง',
    'บ้านป่าไม้พระฉาย',
    'วัดเขาจำปา(เทียนราษฎร์อุทิศ)',
    'วัดโคกหนามแท่ง',
    'วัดพระพุทธฉาย',
    'วัดหนองยาวใต้',
    'วัดหนองเขื่อนช้าง',
    'วัดหนองยาวสูง',
    'วัดไก่เส่า(พันธุ์พิทยาคาร)',
    'วัดบ้านใหม่',
    'วัดเขาดิน',
    'วัดหนองกะธาตุ',
    'วัดหนองสองห้อง',
    'วัดอู่ตะเภา',
    'วัดหนองกบ(ไทยกล้าประชานุกูล)',
    'วัดหนองทางบุญ',
    'อนุบาลหนองแซงศีลวุฒาจารย์',
    'วัดหนองกีบม้า',
    'วัดหนองสีดา',
    'วัดหนองหัวโพ',
    'วัดโคกใหญ่',
    'วัดหนองนางปุ๋ย',
    'วัดโพธิ์ทอง',
    'โคกมะขามจั่นเสือสามัคคี',
    'ชุมชนบ้านตลาดน้อย(ประชานุกูล)',
    'วัดสารภี',
    'วัดสุนทรเทพมุนีมิตรภาพที่ 188',
    'บ้านครัว(ซิเมนต์ไทยสงเคราะห์)',
    'วัดมหาโลก(มหาโลกวิทยาคาร)',
    'วัดม่วงน้อย',
    'อนุบาลบ้านหมอ"พัฒนราษฎร์"',
    'วัดหนองพันเรือ(สำนักงานสลากกินแบ่งฯ)',
    'วัดมะขามเรียง',
    'วัดโคกงาม',
    'วัดบ่อพระอินทร์',
    'วัดปัญจาภิรมย์',
    'วัดสร่างโศก(อบศรีประชาอุปถัมภ์)',
    'วัดราษฎร์บำรุง',
    'วัดหนองคล้า(สำนักงานสลกกินแบ่งฯ)',
    'วัดโคกเสลา',
    'บ้านหนองบัว(ประชาสงเคราะห์)',
    'หรเทพ(รุ่งเรืองประชาสามัคคี)',
    'วัดโคกโพธิ์(โอภาสราษฎร์สามัคคี)',
    'วัดดอนทอง(ราษฎร์ร่วมจิต)',
    'อนุบาลดอนพุด(พิพัฒน์ดวงราษฎร์บำรุง)',
    'วัดหนองมน(ลอยเคลือบประชานุกูล)',
    'วัดช้าง',
    'วัดบ้านรี',
    'วัดสว่างอารมณ์',
    'บ้านหลังสวนประชาสามัคคี',
    'ชุมชนบ้านกลับ(สราญราษฎร์)',
    'วัดหัวถนน',
    'ชุมชนหนองโดน(ประมาณราษฎร์บริบาล)',
    'อนุบาลหนองโดน (สนิทประชากร)',
    'อนุบาลวัดพระพุทธบาท',
    'พระพุทธบาท',
    'บ้านเขาพลัด',
    'วัดเขาวง',
    'บ้านซับชะอม',
    'ธารเกษม',
    'บ้านพุซาง',
    'วัดตรีบุญญาราม',
    'นิคมสร้างตนเองจังหวัดสระบุรี (พิบูลสงเคราะห์ 2)',
    'บ้านซับครก',
    'บ้านสระลำใย',
    'วัดนายาว(สำนักงานสลากินแบ่งฯ)',
    'ท้ายพิกุล(สำนักงานสลากกินแบ่งสงเคราะห์ฯ)',
    'ประชาสงเคราะห์รังสรรค์',
    'วัดหนองคณฑี(พลานุกูล)',
    'บ้านพุกร่าง(พลอุปถัมภ์)',
    'บ้านเขาเลี้ยว',
    'บ้านหนองใหญ่',
    'นิคมสงเคราะห์ 2',
    'บ้านธารทองแดง',
    'วัดตาลเสี้ยน(สีมาราษฎร์)',
    'วัดส้มป่อย',
    'วัดหนองจิก',
    'วัดบ้านโพธิ์ (มิตรภาพที่ 132)',
    'วัดศรีจอมทอง',
    'วัดหนองสุทธะ',
    'วัดต้นตาล(ต้นตาลประชาสฤษฎิ์)',
    'วัดท่าช้าง(ท่าช้างประกฤติศึกษา)',
    'วัดบ้านยาง',
    'บ้านสันประดู่(คำประชานุสรณ์)',
    'วัดพระยาทด(สุชลิตวิทยา)',
    'วัดโพธิ์(นาควิทยานุสรณ์)',
    'วัดตะเฆ่(ม่วงงามนุเคราะห์)',
    'วัดบ้านโคก(มงคลประชาสรรค์)',
    'วัดจันทบุรี(พิมพ์ประชานุเคราะห์)',
    'วัดบ้านชุ้ง(อิสระวิทยา)',
    'วัดสมุหประดิษฐ์(วันครู 2501)',
    'วัดเจ้าฟ้า(ยอดวิทยานุกูล)',
    'อนุบาลเสาไห้',
    'บ้านเขาดินใต้',
    'บ้านม่วงฝ้าย',
    'บ้านหนองกะเบา',
    'วัดบำเพ็ญพรต(ประมาณราษฎร์วิทยาฯ)',
    'วัดห้วยหวาย',
    'วัดมงคล',
    'วัดลาดเขาปูน',
    'วัดสุนันทาราม',
    'วัดแพะโคก',
    'วัดท่าวัว',
    'บ้านห้วยหินขาวมิตรภาพที่ 213',
    'วัดพุแค',
    'วัดวังเลน',
    'วัดหนองหว้า',
    'วัดใหม่สามัคคีธรรม',
    'หน้าพระลาน (พิบูลสงเคราะห์)',
    'บ้านหนองจาน',
    'บ้านเขารวก (ร่วมมิตรพัฒนา)',
    'บ้านคุ้งเขาเขียว (สำเภาราษฎร์บำรุง)',
    'วัดป่าคา',
    'วัดห้วยบง (บริษัทข้าวไทยจำกัดสงเคราะห์ 3)',
    'วัดสาธุประชาสรรค์',
    'สุธีวิทยา',
    'ดอนพุดวิทยา',
    'สวนกุหลาบวิทยาลัย  สระบุรี',
    'สระบุรีวิทยาคม',
    'หนองโดนวิทยา',
    'พระพุทธบาท(พลานุกูลวิทยา)',
    'บ้านหมอ(พัฒนานุกูล)',
    'หนองแซงวิทยา',
    'เสาไห้(วิมลวิทยานุกูล)',
    'โคกกระท้อนกิตติวุฒิวิทยา',
    'เทพศิรินทร์ พุแค',
    'ชุมชนวัดบำรุงธรรม',
    'วัดกะเหรี่ยงคอม้า',
    'วัดบึงไม้',
    'วัดสุนทริกาวาส',
    'วัดโคกกรุง',
    'วัดชำผักแพว',
    'วัดบ้านดง',
    'บ้านโคกสะอาด',
    'วัดนาบุญ',
    'วัดตาลเดี่ยว',
    'วัดขอนหอม',
    'วัดเตาปูน',
    'นิคมทับกวางสงเคราะห์ 2',
    'บ้านซับบอน',
    'บ้านหนองผักบุ้ง',
    'วัดป่าไผ่',
    'วัดสมุห์พร้อมศิษย์สามัคคีวนาราม',
    'ชุมชนนิคมทับกวางสงเคราะห์ 1',
    'อนุบาลทับกวาง',
    'วัดท่าคล้อ',
    'วัดถ้ำเต่า',
    'วัดหาดสองแคว(สมบูรณ์วิทยาคาร)',
    'วัดท่าสีโพธิ์เหนือ',
    'วัดโป่งมงคล',
    'วัดโป่งก้อนเส้า',
    'วัดท่ามะปราง',
    'วัดบ้านธาตุใต้(สนิทไชยวิทยาคาร)',
    'บ้านป่าวังกวาง',
    'บ้านช่อง',
    'วัดบ่อโศรก',
    'วัดพระพุทธบาทน้อยมิตรภาพที่ 69',
    'วัดสองคอนกลาง',
    'บ้านหนองสองห้อง',
    'วัดเขาลาดวนาราม"พิพัฒน์คีรีเขต"',
    'วัดบุรีการาม',
    'วัดหนองน้ำเขียว(วันดาคุณอุปถัมภ์)',
    'วัดห้วยคงคาวราวาส',
    'วัดศรัทธาเรืองศรี',
    'บ้านผังสามัคคี',
    'บ้านหินซ้อน',
    'วัดกุ่มหัก(อินทโชติประสิทธิ์)',
    'วัดขอนชะโงก(เขียววิมลราษฎร์อุปถัมภ์)',
    'วัดชัยเฉลิมมิตร(ไพศาลวิทยาคม)',
    'วัดสันติวิหาร(ขันธสาประชานุเคราะห์)',
    'ชุมชนวัดไทยงาม',
    'วัดหนองจอกใหญ่',
    'วัดหนองผักชี',
    'วัดหนองสมัคร(มูลนิธิสุขาวดีอุปถัมภ์)',
    'วัดบัวลอย(สามัคคีสงเคราะห์)',
    'สันมะค่าบกน้อยสามัคคี',
    'วัดราษฎร์ศรัทธาธรรม',
    'วัดโพนทอง',
    'วัดโคกกลาง',
    'วัดราษฎร์เจริญ(สรกิจพิจารณ์อุปถัมภ์)',
    'วัดหนองอ่างทอง',
    'อนุบาลหนองแค(วัดใหญ่วันนาเนตรสวัสดิ์อุปถัมภ์)',
    'วัดหนองตาเดี้ยง',
    'วัดหนองตะเฆ่',
    'วัดบ้านไผ่',
    'วัดหนองจิก(บุญมีประชานุกูล)',
    'วัดบ้านจาน',
    'วัดหนองครก',
    'วัดหนองปลากะดี่',
    'วัดร่องแซง(กุลไพศาลวิทยา)',
    'วัดหนองปลาหมอ',
    'บ้านใหม่ทุ่งดินขอ(เรืองฉายอุปถัมภ์)',
    'วัดหนองปลิง',
    'วัดหนองสะเดา(สุนทรพิทยา)',
    'บ้านคลองห้า(ราษฎร์บำรุง)',
    'วัดคลองห้า',
    'วัดราษฎร์บำรุง',
    'วัดลำบัว',
    'วัดหนองโรง',
    'วัดบ้านลาด',
    'วัดห้วยขมิ้น',
    'วัดห้วยทองหลาง',
    'หินกอง(พิบูลอนุสรณ์)',
    'บ้านหนองจิก(บริษัทข้าวไทยจำกัดสงเคราะห์1',
    'วัดห้วยทราย(อดิเรกสารประชานุสรณ์)',
    'ชุมชนวัดคลองไทร(ฉัตรราษฎร์บำรุง)',
    'วัดเขาแก้วมุขดาราม',
    'วัดสมานมิตรมงคล',
    'วัดหนองไทร',
    'วัดคลองยาง',
    'บ้านราษฎร์เจริญ',
    'วัดเขาน้อยจอมสวรรค์',
    'วัดเจริญธรรม',
    'วัดศรีสัจจาวาส',
    'วัดโคกกระต่าย(วารราษฎร์อุปถัมภ์)',
    'วัดบ้านดอน',
    'วัดหนองตาบุญ',
    'ชุมชนวัดวิหารแดง(วรมิตรภัทรายุวราษฎร์ฯ)',
    'วัดเกาะเซิงหวาย',
    'บ้านบางกง',
    'วัดหนองสรวง(ถิรอุทิศ)',
    'วัดบ้านลำ(บ้านลำวิทยาคาร)',
    'วัดสหกรณ์รังสรรค์',
    'วัดหนองโพธิ์',
    'อนุบาลวิหารแดง',
    'บ้านหัวถนน',
    'วัดคลองใหม่',
    'วัดสร้างบุญ',
    'วัดหนองหมูใต้(ไพบูลย์วิทยาคาร)',
    'บ้านซับกระดาน',
    'บ้านซับสนุ่น',
    'บ้านโป่งเกตุ',
    'บ้านซับน้อยใต้สามัคคี',
    'บ้านเขาไม้เกวียน',
    'ไทยรัฐวิทยา 68(วัดเหวลาด)',
    'บ้านหลังเขา',
    'วัดบ้านหมาก',
    'อนุบาลมวกเหล็ก',
    'วัดมวกเหล็กใน',
    'บ้านซับพริก',
    'นิคมสร้างตนเองเลี้ยงโคนม',
    'บ้านหนองตอตะเคียน',
    'บ้านหนองโป่ง',
    'บ้านหนองเอี่ยว',
    'บ้านซับปลากั้ง',
    'บ้านเขานมนาง',
    'บ้านคลองม่วงเหนือ',
    'บ้านซับน้อยเหนือ',
    'บ้านลำพญากลาง',
    'บ้านโป่งไทร',
    'บ้านซับดินดำ',
    'บ้านท่าพลู',
    'บ้านลำสมพุง',
    'บ้านหนองผักหนอก',
    'วัดสวนทองรวมมิตร',
    'บ้านคลองไทร',
    'บ้านหนองมะค่า',
    'วัดปากน้ำ',
    'วัดคำพราน',
    'บ้านท่าฤทธิ์',
    'อนุบาลวังม่วง',
    'วัดสว่างอารมณ์',
    'บ้านป่าลานหินดาด',
    'บ้านโป่งเก้ง',
    'บ้านโป่งตะขบ',
    'วัดน้อยสามัคคีธรรม(ซับกระทิงเหนือ)',
    'วัดวังยาง',
    'วัดสวนมะเดื่อสามัคคี',
    'บ้านหนองกรดสามัคคี',
    'บ้านปึกสำโรง',
    'บ้านมณีโชติสามัคคี',
    'แก่งคอย',
    'หนองแค(สรกิจพิทยา)',
    'ประเทียบวิทยาทาน',
    'สองคอนวิทยาคม',
    'คชสิทธิ์กิตติคุณวิทยาคม',
    'บ้านท่ามะปรางวิทยา',
    'หินกองวิทยาคม',
    'มวกเหล็กวิทยา',
    'ซับน้อยเหนือวิทยาคม',
    'วังม่วงวิทยาคม',
    'วัดบุญญราศรี',
    'บ้านมาบสามเกลียว',
    'วัดดอนดำรงธรรม',
    'วัดท้องคุ้ง',
    'วัดนาเขื่อน',
    'นาป่ามโนรถ',
    'วัดราษฏร์สโมสร',
    'อนุบาลพุทธยาคม(วัดเขาบางทราย)',
    'อนุบาลวัดช่องลม',
    'อนุบาลชลบุรี',
    'อนุบาลวัดอุทยานนที',
    'วัดใหม่เกตุงาม(วิบูลย์ราษฏร์วิทยา)',
    'อนุบาลวัดอรัญญิกาวาส',
    'บ้านสวนอุดมวิทยา',
    'วัดธรรมนิมิตต์',
    'วัดสำนักบก',
    'วัดเสม็ด(วุฒิสุนทรวิทยา)',
    'วัดเตาปูน',
    'วอนนภาศัพท์',
    'วัดบางเป้ง',
    'บ้านแหลมแท่น',
    'อนุบาลวัดกลางดอนเมืองชลบุรี',
    'บ้านหนองตะโก',
    'วัดผาสุการาม',
    'บ้านวังตะโก',
    'บ้านห้วยสาลิกา',
    'อนุบาลวัดอู่ตะเภา',
    'วัดศรีพโลทัย',
    'ชุมชนวัดหนองรี',
    'บ้านช่องมะเฟือง',
    'บ้านหัวโกรก',
    'บ้านชากพุดซา',
    'บ้านห้วยกะปิ',
    'วัดเขาเชิงเทียนเทพาราม',
    'บ้านไร่ใหหลำ',
    'วัดราษฏร์ศรัทธา',
    'วัดตาลล้อม',
    'พระตำหนักมหาราช',
    'บ้านปากคลองโรงนาค',
    'บ้านมาบลำบิด',
    'บ้านโสม',
    'บ้านมาบคล้า',
    'วัดหนองน้ำเขียว',
    'บ้านหมื่นจิต',
    'ชุมชนบ้านหัวกุญแจ',
    'อนุบาลบ้านบึง(อำนาจคุณูปถัมภ์)',
    'บ้านหนองปลาไหล',
    'บ้านมาบกรูด',
    'บ้านห้วยมะไฟ',
    'บ้านเขาแรต',
    'บ้านมาบไผ่',
    'บ้านเกาะไม้แหลม',
    'บ้านหนองเขิน',
    'บ้านหนองชาก(ประโยชน์บุพการีอุทิศ)',
    'บ้านหนองซ้ำซาก',
    'บ้านโป่ง',
    'บ้านเขาดิน',
    'วัดหนองบอนแดง',
    'วัดหนองยาง',
    'บ้านหนองไผ่แก้ว',
    'บ้านป่ายุบ',
    'บ้านหนองปรือ',
    'วัดเขาไผ่(เชี่ยววิทยาคุณ)',
    'บ้านเนินโมก',
    'บ้านป่าแดง(ไชยอุปถัมภ์)',
    'วัดคลองใหญ่',
    'วัดอรุณรังษี',
    'วัดหนองชันจันทนาราม',
    'บ้านตาลดำ',
    'บ้านบึงกระโดน',
    'ชุมชนบ้านอ่างเวียน',
    'บ้านเขาซก',
    'บ้านชากนา',
    'บ้านคลองสิบแปด',
    'บ้านคลองตะเคียน',
    'ชุมชนบ้านคลองพลู',
    'บ้านหนองเสือช้าง',
    'วัดเฉลิมลาภ',
    'บ้านท่าจาม',
    'บ้านห้วยมะระ',
    'อนุบาลหนองใหญ่',
    'บ้านหนองผักหนาม',
    'บ้านห้างสูง(เกตุวัตถาประชานุเคราะห์)',
    'บ้านหนองประดู่',
    'ชลบุรี (สุขบท)',
    'ชลราษฎรอำรุง',
    'ชลกันยานุกูล',
    'ชลกันยานุกูล แสนสุข',
    'บ้านสวน(จั่นอนุสรณ์)',
    'ชลราษฎรอำรุง 2',
    'หนองรีมงคลสุขสวัสดิ์',
    'โสตศึกษาจังหวัดชลบุรี',
    'บ้านบึง(อุตสาหกรรมนุเคราะห์)',
    'บ้านบึง(มนูญวิทยาคาร)',
    'จุฬาภรณราชวิทยาลัย ชลบุรี',
    'คลองกิ่วยิ่งวิทยา',
    'หนองใหญ่ศิริวรวาทวิทยา',
    'วัดเกาะลอย',
    'วัดยุคลราษฏร์สามัคคี',
    'วัดโคกขี้หนอน',
    'วัดวรพรตสังฆาวาส',
    'บ้านบางแสม',
    'วัดบางนาง',
    'บ้านเนินถาวร',
    'บ้านบางหัก',
    'วัดบ้านเก่า',
    'วัดศรีประชาราม',
    'บ้านย่านซื่อ',
    'อนุบาลวัดโคกท่าเจริญ',
    'วัดพานทอง',
    'วัดหนองแช่แว่น',
    'วัดบ้านไร่',
    'วัดบ้านงิ้ว',
    'อนุบาลพานทองวัดหนองกระทุ่ม',
    'ชุมชนวัดหนองตำลึง',
    'บ้านห้วยตากด้าย',
    'วัดหนองกะขะ',
    'วัดห้วยยาง',
    'วัดหนองกาน้ำ',
    'วัดโป่งตามุข',
    'วัดแหลมแค',
    'อนุบาลพนัสศึกษาลัย',
    'วัดโคกเพลาะ',
    'วัดเนินตามาก',
    'วัดพรหมรัตนาราม',
    'บ้านหนองหัวหมู(บำรุงราษฎร์อุปถัมภ์)',
    'วัดนากระรอก',
    'วัดใหม่ท่าโพธิ์',
    'ไทยรัฐวิทยา42',
    'วัดโคกพระศิลาราม',
    'บ้านเนิน',
    'วัดโป่งปากดง(สง่า รุธิระราษฎร์วิทยา)',
    'วัดเนินสัก',
    'บ้านน้ำซับ',
    'บ้านสระนา',
    'บ้านหนองสองห้อง',
    'วัดอัมพวนาราม',
    'วัดนาวังหิน',
    'บ้านเนินหลังเต่า',
    'วัดเซิดสำราญ(บ้านเซิดวิทยาคาร)',
    'วัดบ้านศาลา',
    'วัดกลางคลองหลวง',
    'ชุมชนวัดโบสถ์',
    'วัดหลวงพรหมาวาส',
    'บ้านเขาดินวังตาสี',
    'วัดแก้วศิลาราม',
    'บ้านสระสี่เหลี่ยม',
    'วัดหนองม่วงใหม่',
    'บ้านหนองขยาด',
    'บ้านหนองไผ่',
    'วัดทรงธรรม',
    'บ้านทรายมูล',
    'วัดหนองปรือ(สามัคคีราษฎร์บำรุง)',
    'บ้านหนองข่า',
    'บ้านไร่เสธ์',
    'บ้านหนองไผ่แก้ว',
    'วัดแปลงเกต',
    'บ้านเขาอำนวยสุข',
    'วัดหนองสังข์',
    'วัดหน้าพระธาตุ',
    'วัดบ้านกลาง',
    'ชุมชนบ้านตลาดทุ่งเหียง',
    'วัดทุ่งเหียง',
    'บ้านหนองยาง',
    'วัดชุมแสงศรีวนาราม',
    'บ้านหนองพรหม',
    'วัดป่าแก้ว',
    'บ้านแปลงกระถิน',
    'วัดเขาคีรีรมย์(ฤทธิพรรณราษฎร์อุปถัมภ์)',
    'วัดหัวถนน(สภาจังหวัด2502)',
    'บ้านอ่างกระพงศ์',
    'บ้านธรรมรัตน์',
    'บ้านคลองโค',
    'บ้านขุนชำนาญ',
    'บ้านวังมะเดื่อ',
    'บ้านคลองปริง',
    'บ้านบึงตะกู',
    'บ้านตลาดเนินหิน',
    'บ้านคลองมือไทร (สว่างไสวราษฏร์บำรุง)',
    'บ้านเนินดินแดง',
    'บ้านโปร่งเกตุ',
    'บ้านบ่อกวางทอง',
    'บ้านหนองยายเภา',
    'บ้านหนองเกตุ',
    'บ้านหนองเสม็ด',
    'อนุบาลบ่อทอง',
    'บ้านทับสูง',
    'บ้านหนองใหญ่(คุรุราษฏร์อุปถัมภ์',
    'บ้านทับร้าง',
    'บ้านคลองใหญ่',
    'บ้านคลองยาง',
    'บ้านคลองกุ่ม',
    'สวนป่าคลองตาเพชรบน',
    'บ้านคลองตาเพชร',
    'สวนป่าเขาชะอางค์',
    'บ้านเขาใหญ่',
    'วัดสุวรรณารัญญิกาวาส',
    'บ้านทุ่งน้อย',
    'บ้านคลองโอ่ง',
    'วัดเขาวนาพุทธาราม',
    'บ้านหนองชุมเห็ด',
    'บ้านเนินทุ่ง',
    'บ้านเขาสัตตพรหม',
    'บ้านหนองยายหมาด',
    'บ้านเขาวังแก้ว',
    'บ้านแปลง',
    'อนุบาลเกาะจันทน์',
    'บ้านโค้งประดู่',
    'บ้านเจ็ดเนิน',
    'บ้านชุมนุมปรกฟ้า',
    'บ้านคลอง',
    'วัดท่าบุญมี',
    'บ้านห้วยหวาย',
    'บ้านเกาะโพธิ์(วันครู2500)',
    'บ้านสามแยก',
    'พานทองสภาชนูปถัมภ์',
    'พานทอง',
    'พนัสพิทยาคาร',
    'ทุ่งเหียงพิทยาคม',
    'หัวถนนวิทยา',
    'อุทกวิทยาคม',
    'บ่อทองวงษ์จันทร์วิทยา',
    'เกาะจันทร์พิทยาคาร',
    'บ้านห้วยไข่เน่า',
    'บ้านภูไทร',
    'ชุมชนวัดเขาไม้แก้ว',
    'วัดบุญสัมพันธ์',
    'บ้านโรงหีบ',
    'บ้านตะเคียนเตี้ย',
    'บ้านนาวัง',
    'วัดเวฬุวนาราม',
    'บ้านโป่งสะเก็ต',
    'อนุบาลบางละมุง',
    'วัดสุกรีย์บุญญาราม',
    'บ้านบางละมุง',
    'บ้านทุ่งกราด',
    'วัดโป่ง',
    'วัดสว่างอารมณ์',
    'บ้านมาบประชัน',
    'ชุมชนบ้านหนองปรือ',
    'วัดสุทธาวาส',
    'บ้านเนินพลับหวาน',
    'วัดเขาโพธิ์ทอง',
    'บ้านทุ่งกลม',
    'บ้านสันติคาม',
    'วัดหนองเกตุใหญ่',
    'วัดหนองเกตุน้อย',
    'ห้วยใหญ่',
    'บ้านนอก',
    'บ้านทุ่งคา',
    'บ้านมาบฟักทอง',
    'บ้านทุ่งละหาน',
    'บ้านหนองชากแง้ว',
    'บ้านชากนอก',
    'บ้านบึง',
    'บ้านหุบบอน',
    'บ้านสุรศักดิ์',
    'บ้านเขาคันทรง',
    'บ้านระเวิง',
    'บ้านพันเสด็จนอก',
    'บ้านชากยายจีน',
    'วัดมโนรม',
    'วัดแหลมฉบัง',
    'วัดใหม่เนินพยอม',
    'วัดบ้านนา',
    'บ้านพันเสด็จใน',
    'บ้านบ่อวิน',
    'บ้านเขาหิน',
    'อนุบาลบ้านบางพระ(ฉิ่งนาวิกอนุสรณ์)',
    'โรงเรียนบ้านห้วยกรุ',
    'วัดตโปทาราม',
    'บ้านห้วยกุ่ม',
    'วัดเขาฉลาก',
    'บ้านทางตรง',
    'วัดอัมพวัน',
    'วัดหนองคล้า',
    'บ้านวังค้อ',
    'บ้านหนองปรือ',
    'บ้านบึง',
    'บริษัทไทยกสิกรสงเคราะห์',
    'วัดจุกกะเฌอ',
    'วัดพระประทานพร',
    'วัดนาพร้าว',
    'วัดรังษีสุทธาวาส',
    'วัดพิบูลสัณหธรรม',
    'วัดวังหิน',
    'ชุมชนวัดหนองค้อ',
    'บ้านเขาดิน',
    'บ้านโค้งดารา',
    'บ้านเขาตะแบก',
    'บ้านเนินตอง',
    'วัดหนองขาม(ไตรราษฏร์อำรุง)',
    'บ้านหินวง',
    'วัดหนองจับเต่า',
    'บ้านอำเภอ',
    'วัดนาจอมเทียน',
    'ชุมชนบ้านบางเสร่',
    'เกล็ดแก้ว',
    'บ้านเขาชีจรรย์',
    'จุกเสม็ด',
    'บ้านขลอด',
    'บ้านเขาบายศรี',
    'บ้านกม.ห้า',
    'อนุบาลบ้านเตาถ่าน',
    'บ้านสัตหีบ',
    'วัดเขาคันธมาทน์',
    'ชุมชนบ้านช่องแสมสาร',
    'บางละมุง',
    'โพธิสัมพันธ์พิทยาคาร',
    'ผินแจ่มวิชาสอน',
    'ศรีราชา',
    'ทุ่งศุขลาพิทยา(กรุงไทยอนุเคราะห์)',
    'บึงศรีราชาพิทยาคม',
    'สุรศักดิ์วิทยาคม',
    'สาขาสวนกุหลาบวิทยาลัย',
    'เกาะสีชัง',
    'สัตหีบวิทยาคม',
    'พลูตาหลวงวิทยา',
    'สิงห์สมุทร',
    'วัดแกลงบน',
    'วัดธงหงส์',
    'บ้านสมานมิตร',
    'บ้านตะพุนทอง',
    'บ้านไร่จันดี',
    'บ้านเขายายชุม',
    'สามัคคีราษฎร์บำรุง',
    'วัดท่าเรือ',
    'วัดตะเคียนทอง',
    'วัดบ้านดอน',
    'บ้านหนองจอก',
    'วัดเกาะกลอย',
    'วัดปลวกเกตุ',
    'วัดช้างชนศิริราษฎร์บำรุง',
    'วัดตะพงนอก',
    'วัดยายดา',
    'บ้านเนินเสาธง',
    'ชุมชนวัดทับมา',
    'อนุบาลระยอง',
    'วัดนาตาขวัญ',
    'บ้านชะวึก',
    'บ้านเขาวังม่าน',
    'วัดน้ำคอก',
    'วัดเนินพระ',
    'อนุบาลระยองวัดหนองสนม',
    'วัดกรอกยายชา',
    'วัดโขดหินมิตรภาพที่ 24',
    'ชุมชนวัดบ้านแลง',
    'บ้านตะเกราทอง',
    'วัดเภตราสุขารมย์',
    'วัดในไร่',
    'วัดเขาสำเภาทอง',
    'เกาะแก้วพิสดาร',
    'บ้านหนองแฟบ',
    'บ้านมาบตาพุด',
    'วัดตากวน',
    'บ้านสำนักทอง',
    'บ้านยายจั่น',
    'วัดธรรมสถิต',
    'วัดห้วยโป่ง',
    'บ้านชากลูกหญ้า',
    'วัดมาบชลูด',
    'บ้านพยูน',
    'วัดบ้านฉาง',
    'วัดประชุมมิตรบำรุง',
    'วัดเนินกระปรอก',
    'บ้านเขาห้วยมะหาด',
    'วัดพลา',
    'บ้านคลองทราย',
    'วัดคีรีภาวนาราม',
    'วัดสุวรรณรังสรรค์',
    'บ้านคลองบางไผ่',
    'วัดสระแก้ว',
    'วัดสมบูรณาราม',
    'วัดสำนักกะท้อน',
    'วัดชากหมาก',
    'วัดห้วงหิน',
    'บ้านบึงต้นชัน',
    'วัดชากกอไผ่',
    'บ้านเขาลอย',
    'วัดหนองตะแบก',
    'ชุมชนวัดหนองคอกหมู',
    'วัดบ้านเก่า',
    'วัดตาขัน',
    'บ้านหลังเขา',
    'วัดหนองพะวา',
    'บ้านคลองยายเมือง',
    'บ้านเขาหวาย',
    'บ้านชากมะหาด',
    'วัดหวายกรอง',
    'วัดบ้านค่าย',
    'วัดไผ่ล้อม',
    'บ้านหนองสะพาน',
    'วัดเกาะ',
    'วัดปทุมาวาส',
    'บ้านหินโค่งประชานุเคราะห์',
    'บ้านมาบป่าหวาย',
    'วัดหนองกรับ',
    'บ้านคลองขนุน',
    'บ้านหนองฆ้อ',
    'บ้านท่าเสา',
    'วัดดอนจันทน์',
    'วัดไร่',
    'บ้านหนองละลอก',
    'บ้านตีนเนิน',
    'วัดหนองกระบอก',
    'บ้านมาบตอง',
    'บ้านคลองกรำ',
    'ชุมชนบริษัทน้ำตาลตะวันออก',
    'บ้านหนองบอน',
    'บ้านปลวกแดง',
    'บ้านมาบเตย',
    'บ้านมาบยางพร',
    'บ้านสะพานสี่',
    'บ้านห้วยปราบ',
    'บ้านหนองมะปริง',
    'บ้านแม่น้ำคู้',
    'นิคมสร้างตนเอง จ.ระยอง 8',
    'นิคมสร้างตนเอง จ.ระยอง 10',
    'นิคมสร้างตนเอง จ.ระยอง 13',
    'บ้านวังประดู่',
    'บ้านหมอมุ่ย',
    'บ้านปากแพรก',
    'บ้านคลองน้ำแดง',
    'บ้านเขาคลองซอง',
    'บ้านบึงตาต้า',
    'บ้านหนองไร่',
    'นิคมสร้างตนเอง จ.ระยอง 1',
    'นิคมสร้างตนเอง จ.ระยอง 2',
    'นิคมสร้างตนเอง จ.ระยอง 5',
    'วัดชากผักกูด',
    'นิคมสร้างตนเอง จ.ระยอง 4',
    'นิคมสร้างตนเอง จ.ระยอง 6',
    'นิคมสร้างตนเอง จ.ระยอง 9',
    'บ้านหนองระกำ',
    'นิคมสร้างตนเอง จ.ระยอง 7',
    'นิคมสร้างตนเอง จ.ระยอง 3',
    'วัดมาบข่า',
    'วัดกระเฉท',
    'วัดป่าประดู่',
    'ระยองปัญญานุกูล',
    'ระยองวิทยาคม',
    'บ้านฉางกาญจนกุลวิทยา',
    'เพรักษมาตาวิทยา',
    'มาบตาพุดพันพิทยาคาร',
    'ระยองวิทยาคม นิคมอุตสาหกรรม',
    'ระยองวิทยาคมปากน้ำ',
    'เฉลิมพระเกียรติสมเด็จพระศรีนครินทร์ ระยอง',
    'บ้านค่าย',
    'ปลวกแดงพิทยาคม',
    'นิคมวิทยา',
    'วัดกระแส',
    'บ้านคลองป่าไม้',
    'บ้านเนินไม้หอม',
    'บ้านยางงาม',
    'ชุมชนวัดกลางกร่ำ',
    'วัดชากมะกรูด',
    'วัดสุขไพรวัน',
    'บ้านหนองคุย',
    'บ้านชำสมอ',
    'วัดเขาน้อย',
    'วัดกองดิน',
    'วัดเขาสำรอง',
    'วัดไตรรัตนาราม',
    'วัดคลองปูน',
    'วัดเนินยาง',
    'บ้านเนินสมบูรณ์',
    'วัดเขากะโดน',
    'วัดบุนนาค',
    'บ้านคลองทุเรียน',
    'บ้านมาบเหลาชะโอน',
    'วัดสันติวัน',
    'วัดสมอโพรง',
    'วัดคลองชากพง',
    'วัดพลงไสว',
    'บ้านหนองตะเกรา',
    'วัดโพธิ์ทอง',
    'บ้านดอนสำราญ',
    'วัดพลงช้างเผือก',
    'วัดสารนารถธรรมาราม',
    'วัดมงคลวุฒาวาส',
    'บ้านชุมนุมสูง',
    'บ้านเนินสุขสำรอง',
    'วัดทุ่งควายกิน',
    'วัดเนินเขาดิน',
    'วัดหนองกะพ้อ',
    'บ้านหนองไทร',
    'วัดถนนกะเพรา',
    'วัดจำรุง',
    'วัดเนินทราย',
    'วัดบ้านนา',
    'บ้านทุ่งเค็ด',
    'บ้านท่าลำบิด',
    'บ้านห้วงหิน',
    'ชุมชนวัดตะเคียนงาม',
    'ชุมชนวัดตะเคียนงาม (สาขาวัดสมมติเทพฯ)',
    'วัดท่ากง',
    'วัดเกาะลอย',
    'วัดปากน้ำพังราด',
    'วัดพังราด',
    'บ้านวังหิน',
    'วัดวังหว้า',
    'บ้านเนินหย่อง',
    'บ้านเนินดินแดง',
    'บ้านเจริญสุข',
    'บ้านเขาหินแท่น',
    'บ้านหนองน้ำขุ่น',
    'บ้านสองสลึง',
    'บ้านสองพี่น้อง',
    'บ้านเต้าปูนหาย',
    'บ้านห้วยยาง',
    'วัดคงคาวราราม',
    'บ้านชุมแสง',
    'บ้านหนองม่วง',
    'บ้านบึงตะกาด',
    'บ้านเขาตาอิ๋น',
    'บ้านแก่งหวาย',
    'บ้านคลองไผ่',
    'ไทยรัฐวิทยา 43 (บ้านคลองเขต)',
    'วัดป่ายุบ',
    'บ้านยุบตาเหน่ง',
    'บ้านคลองบางบ่อ',
    'บ้านพลงตาเอี่ยม',
    'บ้านชงโค',
    'ชุมชนบ้านวังจันทน์',
    'บ้านชุมนุมใน',
    'บ้านเขาตลาด',
    'บ้านเขาชะอางคร่อมคลอง',
    'บ้านมะเดื่อ',
    'บ้านชำฆ้อ',
    'บ้านศรีประชา',
    'บ้านเขาช่องลม',
    'บ้านน้ำเป็น',
    'บ้านสามแยกน้ำเป็น',
    'บ้านมาบช้างนอน',
    'บ้านเหมืองแร่',
    'บ้านน้ำใส',
    'บ้านห้วยทับมอญ',
    'บ้านยางเอน',
    'บ้านสีระมัน',
    'บ้านน้ำกร่อย',
    'บ้านยางเอน(สาขาบ้านคลองหวายโป่ง)',
    'แกลง(วิทยสถาวร)',
    'วังจันทร์วิทยา',
    'เขาชะเมาวิทยา',
    'ชำนาญสามัคคีวิทยา',
    'ชำฆ้อพิทยาคม',
    'สุนทรภู่พิทยา',
    'ห้วยยางศึกษา',
    'มกุฎเมืองราชวิทยาลัย',
    'วัดเกาะขวาง',
    'วัดเกาะตะเคียน',
    'วัดเกาะโตนด',
    'วัดเขาน้อย',
    'วัดคมบาง',
    'วัดเนินยาง',
    'วัดทองทั่ว',
    'วัดเนินสูง',
    'วัดจันทนาราม',
    'วัดไผ่ล้อม',
    'บ้านคลองน้ำใส',
    'บ้านแก้ว',
    'วัดโยธานิมิต',
    'วัดสิงห์',
    'วัดพลับ',
    'บ้านท่าแฉลบ',
    'วัดดอนตาล',
    'วัดพลับพลา',
    'วัดโป่งแรด',
    'อนุบาลจันทบุรี',
    'สฤษดิเดช',
    'บ้านชำโสม',
    'วัดแสลง',
    'วัดหนองบัว',
    'วัดเสม็ดงาม',
    'วัดเนินโพธิ์',
    'บ้านศรัทธาตะพง',
    'บ้านท่าปก',
    'บ้านตาเลียว',
    'บ้านวังปลา',
    'บ้านแก่งน้อย',
    'บ้านเขาแก้ววิทยา',
    'บ้านคลองกะพง',
    'บ้านเนินดินแดง',
    'บ้านมาบโอน',
    'วัดบูรพาพิทยาราม',
    'วัดเสม็ดโพธิ์ศรี',
    'วัดโขมง',
    'วัดคลองขุด',
    'วัดหมูดุด',
    'บ้านเจ้าหลาว',
    'บ้านเตาหม้อ',
    'บ้านสังข์ทอง',
    'วัดหนองคัน',
    'วัดแขมหนู',
    'วัดตะกาดเง้า',
    'อนุบาลบ้านหนองคล้า',
    'วัดทุ่งเบญจา',
    'บ้านสะพานเลือก',
    'วัดรำพัน',
    'วัดท่าศาลา',
    'บ้านไร่เก่า',
    'ไทยรัฐวิทยา 14',
    'วัดสามผาน',
    'บ้านช่องกะพัด',
    'วัดแก่งหางแมว',
    'บ้านโป่งวัว',
    'บ้านหนองบัวทอง',
    'บ้านโคกวัด',
    'วัดขุนซ่อง',
    'วัดขุนซ่องสาขาบ้านโป่งเกตุ',
    'วัดเขาวงกต',
    'บ้านหนองเจ๊กสร้อย',
    'บ้านเนินจำปา',
    'บ้านวังอีแอ่น',
    'บ้านบ่อไฟไหม้',
    'บ้านคลองครก',
    'บ้านวังไม้แดง',
    'บ้านซอยสอง',
    'บ้านประแกต',
    'วัดหนองแหวน',
    'วัดโขดหอย',
    'วัดนาซา',
    'บ้านยางระหง',
    'วัดช้างข้าม',
    'วัดหนองไทร',
    'บ้านเขามะปริง',
    'วัดนายายอาม',
    'บ้านต้นกระบก',
    'วัดโพธิ์ลังกามิตรภาพที่ 171',
    'วัดวังเวียน',
    'บ้านห้วงกระแจะ',
    'วัดวังหิน',
    'วัดหนองสีงา',
    'บ้านคลองลาว',
    'วัดสนามไชย',
    'บ้านหนองหงส์',
    'วัดท่าแคลง',
    'เบญจมราชูทิศ',
    'บางกะจะ',
    'ศรียานุสรณ์',
    'เบญจมานุสรณ์',
    'ท่าใหม่(พูลสวัสดิ์ราษฎร์นุกูล)',
    'มัธยมวัดเขาสุกิม',
    'ศรีรัตน์ราษฎร์นุเคราะห์',
    'สะพานเลือกวิทยาคม',
    'แก่งหางแมวพิทยาคาร',
    'นายายอามพิทยาคม',
    'มัธยมท่าแคลง',
    'วัดเกวียนหัก',
    'วัดซึ้งบน',
    'วัดอิมั้ง',
    'บ้านช้างข้าม',
    'วัดตกพรม',
    'วัดสะตอน้อย',
    'บ้านตรอกนอง',
    'วัดตะปอนใหญ่',
    'วัดตะปอนน้อย',
    'บ้านหนองเสม็ด',
    'บ้านบ่อ',
    'มิตรภาพ 20',
    'วัดวังสรรพรส',
    'บ้านหนองระหาน',
    'บ้านทรัพย์สินพูน',
    'บ้านบ่อเวฬุ',
    'บ้านเขาทอง',
    'บ้านบ่อเวฬุ สาขาบ้านโชคดี',
    'วัดเกาะจิก',
    'บ้านท่าขาหย่าง',
    'วัดบางชัน',
    'วัดมาบไผ่',
    'วัดเวฬุวัน',
    'วัดซึ้งล่าง',
    'วัดวันยาวล่าง',
    'บ้านหนองบอน',
    'วัดผักกาด',
    'บ้านคลองใหญ่',
    'บ้านป่าวิไล',
    'บ้านดงจิก',
    'บ้านทุ่งกร่าง',
    'บ้านเนินดินแดง',
    'บ้านแหลม',
    'บ้านปั้นหม้อ',
    'บ้านโป่งน้ำร้อน',
    'บ้านใหม่',
    'บ้านเขาหอม',
    'บ้านทรัพย์ประเมิน',
    'บ้านสามสิบพัฒนา',
    'บ้านคลองตาคง',
    'วัดพังงอน',
    'วัดทับไทร',
    'บ้านทุ่งม่วง',
    'บ้านคลองคต',
    'ราษฎร์พัฒนาสามัคคี',
    'บ้านคลองบอน',
    'บ้านโพธิ์',
    'บ้านแปลง',
    'บ้านจางวาง',
    'บ้านนาสนาดก์',
    'วัดทุ่งเพล',
    'บ้านวังตัก',
    'วัดน้ำรัก',
    'บ้านหนองสลุด',
    'วัดมะทาย',
    'วัดโป่ง',
    'บ้านทัพนคร',
    'บ้านทุ่งบอน',
    'บ้านมะขาม(???????????????)',
    'วัดวังจะอ้าย',
    'วัดขนุน',
    'วัดบ้านอ่าง',
    'บ้านเกาะเปริด',
    'วัดเขาตาหน่วย',
    'บ้านคลองใหญ่',
    'วัดบางกะไชย',
    'บ้านแหลมสิงห์',
    'วัดบางสระเก้า',
    'บ้านคลองกลอย',
    'วัดปากน้ำ',
    'ปากน้ำแหลมสิงห์',
    'วัดท่าหัวแหวน',
    'วัดท่าเขา',
    'พลิ้ว',
    'บ้านน้ำตกพลิ้ว',
    'วัดหนองชิ่ม',
    'บ้านอีมุย',
    'บ้านตามูล',
    'วัดสำโรง',
    'สังวาลย์วิทย์ 6',
    'บ้านประตง(อนุบาลอ.สอยดาว)',
    'บ้านสมเด็จเจ้าพระยาอุปถัมภ์',
    'บ้านทรายทอง',
    'บ้านทับสงฆ์',
    'บ้านโป่งเจริญชัย',
    'บ้านทับช้าง',
    'บ้านไทรงาม',
    'ส.ไทยเสรีอุตสาหกรรม 2',
    'บ้านไผ่ล้อมสามัคคี',
    'บ้านซับตารี',
    'หนองมะค่า',
    'โฆวินฑะ',
    'บ้านกระทิงทอง',
    'บ้านเขาแก้ว',
    'บ้านตาเรือง',
    'บ้านทรัพย์เจริญ',
    'สตรีวิทยาอุปถัมภ์ 2',
    'บ้านสวนส้ม',
    'วัดน้ำขุ่น',
    'วัดคลองพลู',
    'วัดทุ่งกบิล',
    'บ้านจันทเขลม',
    'บ้านชากไทย',
    'บ้านเนินมะหาด',
    'วัดตะเคียนทอง',
    'วัดคลองตะเคียน',
    'บ้านทุ่งตาอิน',
    'วัดพลวง',
    'วัดกะทิง (???????????????)',
    'บ้านคลองน้ำเป็น',
    'ตกพรมวิทยาคาร',
    'บ่อวิทยาคาร',
    'ขลุงรัชดาภิเษก',
    'เครือหวายวิทยาคม',
    'โป่งน้ำร้อนวิทยาคม',
    'หนองตาคงพิทยาคาร',
    'มะขามสรรเสริญ',
    'แหลมสิงห์วิทยาคม(อาทรสังขะวัฒนะอุปถัมภ์)',
    'สอยดาววิทยา',
    'ทุ่งขนานวิทยา',
    'คิชฌกูฏวิทยา',
    'คลองพลูวิทยา',
    'ศึกษาสงเคราะห์จันทบุรี',
    'บ้านนาเกลือ',
    'วัดคิริวิหาร',
    'วัดตะกาง',
    'คลองขวาง',
    'บ้านทุ่งไก่ดัก',
    'วัดวรุณดิตถาราม',
    'ชุมชนวัดท่าพริก (จริยาอุปถัมภ์)',
    'วัดวิเวกเมืองตราด (ศุภกิจวิบูลอุปถัมภ์)',
    'บ้านโพรงตะเฆ่ (จันทร์ประสิทธิ์นุเคราะห์)',
    'บ้านไร่ป่า',
    'อนุบาลตราด',
    'อนุบาลชุมชนวัดหนองบัว (รัตนวงศ์ราษฎร์บำรุง)',
    'บ้านท่าประดู่',
    'บ้านท่าเรือจ้าง(ประชาชนูปถัมภ์)',
    'บ้านเนินตาแมว',
    'บ้านสวนใน',
    'วัดบุปผาราม (คุณวุฒิอุปถัมภ์)',
    'วัดวัง (กิ่ม  บุญอยู่อุทิศ)',
    'วัดแหลมหิน',
    'วัดหนองคันทรง',
    'วัดหนองเสม็ด (อรรถพรสงเคราะห์)',
    'วัดสุวรรณมงคล',
    'วัดไทรทอง',
    'บ้านปากคลองน้ำเชี่ยว',
    'วัดห้วงน้ำขาว',
    'บ้านเปร็ดใน',
    'บ้านฉางเกลือ (รักเมืองไทย 3)',
    'บ้านบางปรง',
    'บ้านปากพีด',
    'วัดคลองขุด',
    'วัดเนินยาง',
    'วัดบางปรือ',
    'วัดแหลมกลัด',
    'บ้านคลองประทุน',
    'วัดสะพานหิน',
    'วัดอ่าวใหญ่',
    'บ้านแหลมพร้าว',
    'ชุมชนวัดอ่าวช่อ',
    'อนุบาลวัดคลองใหญ่',
    'บ้านคลองจาก',
    'บ้านเนินตาล',
    'บ้านตาหนึก',
    'บ้านคลองมะนาว',
    'บ้านไม้รูด(วิสิทธิ์ประชาสรรค์)',
    'บ้านหนองม่วง',
    'วัดห้วงโสม',
    'บ้านโขดทราย',
    'บ้านคลองมะขาม',
    'บ้านหาดเล็ก',
    'ชุมชนบ้านเขาสมิง',
    'ชุมชนบ้านเขาสมิงสาขาวัดลำภูราย',
    'บ้านดงกลาง',
    'วัดสลัก',
    'บ้านคลองศอก(เพิ่มประชาราษฎร์อุปถัมภ์)',
    'บ้านอ่างกะป่อง',
    'วัดท่าโสม(คุณสารราษฎร์บำรุง)',
    'บ้านตามาง (อมฤทธิ์อุปถัมภ์)',
    'บ้านเกษมสุข',
    'บ้านเจียรพัฒนา',
    'บ้านมณฑล',
    'บ้านดอนสูง',
    'วัดดินแดง',
    'วัดเสนาณรงค์',
    'วัดประณีต',
    'วัดฆ้อ',
    'บ้านโป่ง',
    'บ้านหนองบัว',
    'วัดตาพลาย',
    'ชุมชนวัดแสนตุ้ง',
    'วัดท่าหาด',
    'บ้านห้วงพัฒนา',
    'บ้านเนินตะบก',
    'วัดพนมพริก',
    'บ้านหนองแฟบ',
    'วัดช้างทูน',
    'บ้านด่านชุมพล',
    'บ้านปะเดา',
    'บ้านป้องกันตนเองด่านชุมพล',
    'บ้านมะม่วง',
    'บ้านสระใหญ่',
    'ไทยรัฐวิทยา44(บ้านนนทรีย์)',
    'บ้านมะนาว',
    'บ้านคลองแอ่ง',
    'บ้านตางาม',
    'บ้านจัดสรร',
    'บ้านปะอา',
    'ชุมชนบ้านตากแว้ง',
    'ชุมชนวัดบ่อไร่',
    'อนุบาลบ่อพลอยราษฎร์รังสรรค์',
    'อัมพรจินตกานนท์',
    'คีรีศรีสาครวิทยา',
    'บ้านหนองบอน',
    'บ้านคลองใหญ่',
    'บ้านเนินดินแดง',
    'บ้านธรรมชาติล่าง',
    'วัดทองธรรมชาติ',
    'อนุบาลวัดน้ำเชี่ยว',
    'ชุมชนวัดบางกระดาน(สวนราษฎร์บำรุง)',
    'บ้านปากคลอง',
    'บ้านอ่าวตาลคู่',
    'วัดบางปิดบน',
    'วัดบางปิดล่าง(ราษฎร์สงเคราะห์)',
    'วัดแหลมมะขามสาขาบ้านแหลมทองหลาง',
    'วัดแหลมมะขาม(กัลยาณกิตติวิทยา)',
    'ชุมชนแหลมงอบ(นิเทศก์อุปถัมภ์)',
    'บ้านคลองเจ้า',
    'บ้านอ่าวพร้าว',
    'อนุบาลวัดราษฎร์บำรุง',
    'บ้านเกาะหมาก(สถิตย์สุทธิวงษ์)',
    'บ้านคลองพร้าว',
    'อนุบาลเกาะช้าง',
    'วัดคลองสน',
    'บ้านบางเบ้า',
    'วัดวัชคามคชทวีป',
    'วัดสลักเพชร',
    'ตราษตระการคุณ',
    'สตรีประเสริฐศิลป์',
    'ตราดสรรเสริญวิทยาคม',
    'คีรีเวสรัตนูปถัมภ์',
    'เขาน้อยวิทยาคม',
    'อ่าวใหญ่พิทยาคาร',
    'สตรีประเสริฐศิลป์ 2',
    'คลองใหญ่วิทยาคม',
    'เขาสมิงวิทยาคม(จงจินต์รุจิรวงศ์)',
    'สะตอวิทยาคม รัชมังคลาภิเษก',
    'ประณีตวิทยาคม',
    'บ่อไร่วิทยาคม',
    'หนองบอนวิทยาคม',
    'ศึกษาสงเคราะห์ตราด',
    'แหลมงอบวิทยาคม',
    'เกาะกูดวิทยาคม',
    'วัดจุกเฌอ',
    'วัดชนะสงสาร',
    'วัดพรหมสุวรรณ',
    'วัดจรเข้น้อย',
    'สุเหร่าลาดน้ำขาว',
    'สุเหร่าสมอเซ',
    'สุเหร่าคลองใหญ่',
    'บ้านเกาะดอน',
    'ตลาดเปร็ง',
    'แพ่งพิทยาภูมิ',
    'บ้านคลองเจ้า',
    'สุเหร่าหลวงแพ่ง',
    'วัดคู้เกษมสโมสร',
    'วัดไชยธารา',
    'วัดเทพนิมิตร',
    'วัดประตูน้ำท่าไข่',
    'วัดนิโครธาราม',
    'วัดบ้านนา',
    'วัดนาคู',
    'วัดสัมปทวน',
    'บ้านบางแก้ว',
    'วัดเกาะจันทาราม',
    'วัดสมานรัตนาราม',
    'วัดเที่ยงพิมลมุข',
    'วัดราษฎร์ศรัทธาทำ',
    'วัดดอนทอง',
    'บ้านวนท่าแครง',
    'วัดเกาะ',
    'วัดแพรกนกเอี้ยง',
    'บ้านแขวงกลั่น',
    'วัดบางปลานัก',
    'บ้านบางไผ่',
    'วัดทด',
    'วัดโพธาราม',
    'วัดบางปรง',
    'วัดบางพระ',
    'วัดสายชล',
    'วัดนครเนื่องเขต',
    'บ้านวังตะเคียน',
    'วัดหนามแดง',
    'อนุบาลวัดปิตุลาธิราชรังสฤษฎิ์',
    'บ้านคลอง 21',
    'บ้านปลายคลอง 20',
    'วัดโพธิ์เฉลิมรักษ์',
    'วัดราษฎร์บำรุงศักดิ์',
    'สุเหร่าดอนเกาะกา',
    'วัดบึงตาหอม',
    'บ้านดอนเกาะกา',
    'บ้านบึงพระอาจารย์',
    'วัดสุวรรณเตมีย์',
    'สุเหร่าลำชะล่า',
    'สุเหร่าสมอเอก',
    'ตลาดคลอง 16',
    'เฉลิมช่วงวิทยาทาน',
    'วัดรามัญ',
    'สุเหร่าปากคลอง 20',
    'สุเหร่าดอนกลาง',
    'สุเหร่าคู้',
    'ปากคลองบางขนาก',
    'คลองตันคลอง 18',
    'วัดเกตุสโมสร',
    'วัดประจำรัง',
    'วัดประชาบำรุง',
    'ประชาฤกษ์สมบูรณ์',
    'บึงเทพยา',
    'สุเหร่าใหม่ปากคลอง 17',
    'สุเหร่าแคราย',
    'สกุลดีประชาสรรค์',
    'วัดบึงน้ำรักษ์',
    'สุเหร่าคลอง 15',
    'สุเหร่าดารุลนาอีม',
    'สุเหร่าคลองหกวา',
    'สุเหร่าคลอง 14',
    'สิทธิสุนทรอุทิศ',
    'วัดบางสาย',
    'วัดตะพังคลี',
    'วัดนีลราษฎร์ศรัทธาบำรุง',
    'วัดญาณรังษาราม',
    'สะแกโดดประชาสรรค์',
    'วัดบางไทร',
    'วัดไผ่ดำ',
    'วัดหลวงแพ่ง',
    'วัดสว่างอารมณ์',
    'วัดคลองเจ้า',
    'วัดแคราย',
    'วัดบึงทองหลาง',
    'ปากบึงสิงโต',
    'วัดโพธิ์แสงกาญจนราษฎร์',
    'สุเหร่าคลอง 20',
    'สุเหร่าคลอง 19',
    'วัดพุทธอุดมวิหาร',
    'สุเหร่าคลอง 18',
    'บึงสิงโต',
    'วัดคลอง 18',
    'วัดเขาดิน',
    'วัดบางแสม',
    'คลองพานทอง',
    'บ้านท่าข้ามตั้งตรงจิตร 10',
    'จันทร์เจริญ',
    'ประกอบราษฎร์บำรุง',
    'วัดท่าสะอ้าน',
    'วัดลาดยาว',
    'วัดบางเกลือ',
    'วัดล่าง',
    'วัดบน',
    'พระพิมลเสนี',
    'พรหมานุเคราะห์',
    'สามัคคีราษฎร์บำรุง',
    'วัดบางผึ้ง',
    'บ้านบางข้าว',
    'วัดบางวัว',
    'วัดบางสมัคร',
    'วัดพิมพาวาส',
    'แสมขาววิทยาคาร',
    'วัดสองคลอง',
    'บ้านสีล้ง',
    'สกัด 80',
    'คลองขวาง',
    'วัดสุขาราม',
    'สกัด 40',
    'วัดหล่อเจริญราษฎร์วราราม',
    'คลองตาเอี่ยม',
    'วัดสุคันธสีลาราม',
    'วัดคลองสวน',
    'สุเหร่าเกาะไร่',
    'สุเหร่าจรเข้น้อย',
    'วัดพนมพนาวาส',
    'วัดราษฎร์ศรัทธาธรรม',
    'วัดลาดบัว',
    'วัดใหม่ประเวศ',
    'วัดดอนทราย',
    'วัดประศาสน์โสภณ',
    'วัดเทพราช',
    'วัดกลางราษฎร์บำรุง',
    'วัดผาณิตาราม',
    'วัดสนามจันทร์',
    'วัดคลองบ้านโพธิ์',
    'วัดคลองต้นหมัน',
    'วัดพิพิธประสาทสุนทร',
    'วัดกระทุ่ม',
    'จันทร์ประชาสฤษฎิ์',
    'วัดสามกอ',
    'หนองหน้าบ้าน',
    'วัดประชาบำรุงกิจ',
    'วัดแสนภูดาษ',
    'วัดอินทาราม',
    'วัดดอนสีนนท์',
    'วัดหนองกระสังสามัคคี',
    'วัดอรัญญิการาม',
    'วัดศรีมงคล',
    'เบญจมราชรังสฤษฎิ์',
    'ดัดดรุณี',
    'พุทธโสธร',
    'เบญจมราชรังสฤษฎิ์ 3 ชนะสงสารวิทยา',
    'เบญจมราชรังสฤษฎิ์ 2',
    'เบญจมราชรังสฤษฎิ์ 4',
    'เตรียมอุดมศึกษาเปร็งวิสุทธาธิบดี',
    'เบญจมราชรังสฤษฎิ์ 5',
    'บางน้ำเปรี้ยววิทยา',
    'ดอนฉิมพลีพิทยาคม',
    'หมอนทองวิทยา',
    'ไผ่ดำพิทยาคม รัชมังคลาภิเษก',
    'การทำมาหากินวัดโพธิ์เฉลิมรักษ์ ฯ',
    'บางปะกง(บวรวิทยายน)',
    'พุทธิรังสีพิบูล',
    'วิทยาราษฎร์รังสรรค์',
    'ผาณิตวิทยา',
    'ศึกษาพิเศษฉะเชิงเทรา',
    'สุตะโชติประชาสรรค์',
    'ท่าระหัดราษฎร์สฤษฎิ์',
    'วัดบางกระเจ็ด',
    'วัดสามแยก',
    'วัดบางกระดาน',
    'วัดใหม่บางคล้า',
    'วัดกกสับ',
    'วัดปากน้ำ',
    'วัดสาวชะโงก',
    'วัดเสม็ดใต้',
    'วัดสนามช้าง',
    'วัดหัวสวน',
    'บ้านหนองโสน',
    'บ้านปลายคลอง',
    'วัดศรีสุตาราม',
    'วัดเสม็ดเหนือ',
    'วัดลาดบัวขาว',
    'วัดหัวไทร',
    'วัดทางข้ามน้อย',
    'วัดคูมอญ',
    'วัดพงษาราม',
    'บ้านแหลมตะคร้อ',
    'วัดหนองเสือ',
    'วัดชายเคืองวนาราม',
    'บ้านห้วยพลู',
    'วัดนาน้อย',
    'บ้านหนองอีโถน',
    'บ้านไร่ดอน',
    'วัดดอนขี้เหล็ก',
    'บ้านชำขวาง',
    'บ้านปรือวาย',
    'บ้านม่วงโพรง',
    'วัดแหลมเขาจันทร์',
    'บ้านห้วยหิน',
    'บ้านหนองกลางดง',
    'บ้านหนองเหียง',
    'สว่างศรัทธาธรรมสถาน',
    'วัดลำมหาชัย',
    'วัดหนองปรือ',
    'บ้านหนองแสง',
    'วัดโคกหัวข้าว',
    'วัดดอนทอง',
    'วัดท่าลาดเหนือ',
    'วัดหนองเค็ด',
    'วัดธารพูด',
    'วัดหัวกระสังข์',
    'วัดเกาะแก้วสุวรรณาราม',
    'บ้านหนองหว้า',
    'บ้านหนองสองห้อง',
    'บ้านเขาหินซ้อน',
    'ไพบูลย์ประชานุกูล',
    'วัดท่าเกวียน',
    'วัดนาเหล่าบก',
    'วัดแหลมไผ่ศรี',
    'วัดต้นตาล',
    'วัดหนองปาตอง',
    'วัดบึงกระจับ',
    'วัดสระสองตอน',
    'วัดสุวรรณคีรี',
    'วัดบ้านซ่อง',
    'วัดหนองแหน',
    'วัดดงยาง',
    'วัดหนองบัว',
    'วัดเกาะแก้วเวฬุวัน',
    'วัดหินดาษ',
    'วัดสะแกงาม',
    'บ้านท่าโพธิ์',
    'วัดจรเข้ตาย',
    'วัดแสนภุมราวาส',
    'วัดบางคา',
    'วัดไผ่ขวาง',
    'วัดบางมะเฟือง',
    'วัดดอนท่านา',
    'บางพะเนียง',
    'บ้านอ่างทอง',
    'บ้านท่าม่วง',
    'บ้านยางแดง',
    'บ้านหนองยาง',
    'บ้านสระไม้แดง',
    'บ้านห้วยน้ำใส',
    'ไทยรัฐวิทยา 41',
    'บ้านท่าเลียบ',
    'บ้านนา',
    'บ้านโป่งเจริญ',
    'บ้านหินแร่',
    'บ้านทุ่งส่อหงษา',
    'วัดชำป่างาม',
    'บ้านนาโพธิ์',
    'บ้านท่ากระดาน',
    'บ้านกระบกเตี้ย',
    'บ้านสุ่งเจริญ',
    'บ้านโปร่งเกตุ',
    'บ้านโปร่งตาสา',
    'บ้านท่าทองดำ',
    'บ้านคลองอุดม',
    'บ้านคลองยายสร้อย',
    'บ้านหนองใหญ่',
    'บ้านวังคู',
    'บ้านอ่างตะแบก',
    'บ้านโคกตะเคียนงาม',
    'บ้านมาบนาดี',
    'สวนป่าอุปถัมภ์',
    'บ้าน ก.ม.7',
    'บ้านแปลงไผ่-ขุนคลัง',
    'บ้านห้วยหิน',
    'บ้านลาดกระทิง',
    'บ้านท่าซุง',
    'บ้านห้วยน้ำทรัพย์',
    'วัดสวรรค์นิมิต',
    'บ้านเนินไร่',
    'วัดไผ่แก้ว',
    'วัดแปลงยาว',
    'ตลาดบางบ่อ',
    'บ้านคลองสอง',
    'ไม้แก้วประชานุเคราะห์',
    'ไทรทองอุปถัมภ์',
    'วัดวังกะจะ',
    'ทุ่งสะเดาประชาสรรค์',
    'วัดวังเย็น',
    'หนองปรือประชาสรรค์',
    'บ้านหนองปลาไหลราษฎร์บำรุง',
    'วัดหนองไม้แก่น',
    'วัดโกรกแก้ววงพระจันทร์',
    'บ้านหนองน้ำขาวเจริญราษฎร์',
    'บ้านเขาสะท้อน',
    'วัดหัวสำโรง',
    'บ้านหนองสทิต',
    'วัดอ่าวช้างไล่',
    'บ้านกรอกสะแก',
    'บ้านหนองคอก',
    'บ้านห้วยตะปอก',
    'บ้านร่มโพธิ์ทอง',
    'บ้านเทพประทาน',
    'บ้านทุ่งส่าย',
    'บ้านหนองขาหยั่ง',
    'บ้านธรรมรัตน์ใน',
    'บ้านศรีเจริญทอง',
    'บ้านวังหิน',
    'บ้านหนองปลาซิว',
    'บ้านท่ากลอย',
    'วัดทุ่งยายชี',
    'บ้านหนองปรือกันยาง',
    'บ้านท่าคาน',
    'บ้านหนองประโยชน์',
    'บ้านอ่างเตย',
    'วัดเทพพนาราม',
    'สียัดพัฒนา',
    'บ้านอ่างเสือดำ',
    'วัดก้อนแก้ว',
    'ก้อนแก้วราษฎร์บำรุง',
    'โสภณประชาเทวารุทธารักษ์',
    'วัดคลองเขื่อน',
    'วัดบ้านกล้วย',
    'ราษฎรนุกูล',
    'วัดบางตลาด',
    'วัดคุ้งกร่าง',
    'วัดดอนสนาม',
    'วัดบางโรง',
    'วัดสามร่ม',
    'วัดเปี่ยมนิโครธาราม',
    'บางคล้าพิทยาคม',
    'หนองแหนวิทยา',
    'กาญจนาภิเษกวิทยาลัย ฉะเชิงเทรา',
    'พนมสารคาม(พนมอดุลวิทยา)',
    'ราชสาส์นวิทยา',
    'สนามชัยเขต',
    'พนมอดุลวิทยา 2 ชำป่างาม',
    'ไผ่แก้ววิทยา',
    'หนองไม้แก่นวิทยา',
    'แปลงยาวพิทยาคม',
    'มัธยมสิริวัณวรี 3 ฉะเชิงเทรา',
    'ก้อนแก้วพิทยาคม',
    'วัดท้าวอู่ทอง',
    'บ้านหนองชะอม',
    'บ้านแหลมหิน',
    'วัดภักดีสุนทราวาส',
    'ชุมชนบ้านขอนขว้าง',
    'บ้านดงบัง',
    'วัดนิโครธาวาส',
    'ชุมชนวัดหนองจวง',
    'วัดโคกมะกอก',
    'วัดลำดวน',
    'วัดหาดสะแก',
    'วัดเลียบ(หร่ำวิทยานุกูล)',
    'วัดกระแจะ(ธวัชชัยนุสรณ์)',
    'บ้านห้วยเกษียร',
    'วัดเนินไม้หอม',
    'บ่อแร่-ธารเลา',
    'บ้านเนินหอม',
    'วัดเนินสูง',
    'วัดประสาธน์รังสรรค์ สาขาวัดเทพประสิทธยาราม',
    'วัดประสาธน์รังสรรค์',
    'บ้านดงคุย',
    'วัดทุ่งตะลุมพุก',
    'วัดประชาวาส',
    'วัดหัวกรด',
    'วัดสามัคคีภิรมย์',
    'วัดสง่างาม(สุทธิเกียรติวิทยา)',
    'วัดบ้านพระ',
    'บ้านหัวเขา',
    'อนุบาลเมืองปราจีนบุรี',
    'วัดบ้านยาง',
    'บ้านแหลมไผ่',
    'บ้านหนองเต่า',
    'วัดบางคาง',
    'วัดโบสถ์วิทยา',
    'เมืองปราจีนบุรี',
    'อนุบาลปราจีนบุรี',
    'ชุมชนบ้านทุ่งแฝก',
    'วัดโคกป่าแพง',
    'บ้านหนองช้างลง',
    'บ้านชำโสม',
    'บ้านนางเลง',
    'วัดปากแพรก',
    'วัดสระดู่(บู่บำรุงวิทย์)',
    'บ้านโปร่งสะเดา',
    'บ้านเขาไม้แก้ว',
    'บ้านคลองตามั่น',
    'บ้านคลองเจ้าแรง',
    'บ้านนาคลองกลาง',
    'บ้านหนองประดู่',
    'บ้านหนองหัวช้าง',
    'บ้านโนนสะอาด',
    'บ้านหนองบัว',
    'วัดศรีสุวรรณาราม',
    'วัดตันทาราม',
    'บ้านหนองอนามัย',
    'วัดเนาวรัตนาราม',
    'บ้านท่าอุดม',
    'บ้านหนองมันปลา',
    'วัดศรีสวัสดิ์',
    'บ้านหนองไผ่ล้อม',
    'บ้านหนองโดน',
    'วัดปากน้ำ',
    'บ่อทองวิทยา',
    'บ้านกุงประชาสรรค์',
    'บ้านบุเสี้ยว',
    'บ้านพระปรง',
    'วัดวัฒนารังษี',
    'บ้านหนองนาใน',
    'บ้านหาดมะกอก',
    'บ้านหนองสัง(ไลอุทิศ)',
    'อนุบาลกบินทร์บุรี',
    'บรรหารวิทยา',
    'บ้านไผ่',
    'วัดราษฎร์รังษี',
    'บ้านโคกหอม',
    'บ้านโคกสว่าง',
    'บ้านโนนหัวบึง',
    'บ้านหนองปรือ',
    'บ้านตรอกปลาไหล',
    'บ้านซ่ง',
    'วัดย่านรี',
    'บ้านเขาด้วน',
    'ชุมชนบ้านเขาลูกช้าง',
    'บ้านโคกกระท้อน',
    'ไทยรัฐวิทยา 93 (บ้านลาดตะเคียน)',
    'บ้านหนองตลาด',
    'บ้านคลองร่วม',
    'วัดใหม่พรหมสุวรรณ',
    'บ้านหนองศรีวิชัย',
    'วัดวังหวาย',
    'บ้านหนองจิก',
    'วัดรัตนโชติการาม',
    'วัดรัตนชมภู',
    'วัดเกาะแดง',
    'บ้านวังตะเคียน',
    'บ้านหนองคล้า',
    'บ้านแก่ง',
    'วัดจันทรังษีถาวร',
    'วัดเนินสูง',
    'บ้านเขากระแต',
    'บ้านคลองระกำ',
    'บ้านหนองหอย',
    'บ้านวังมะกรูด',
    'บ้านเขาถ้ำ',
    'บ้านไทยเจริญ',
    'บ้านคลองสิบสอง',
    'บ้านโปร่งใหญ่',
    'บ้านใหม่พัฒนา',
    'บ้านคลองหันแดง',
    'บ้านวังกวาง',
    'บ้านวังดินสอ',
    'บ้านเขาปูน',
    'บ้านทด',
    'บ้านโคกลาน',
    'บ้านโคกสั้น',
    'บ้านคลองอุดม',
    'วัดโคกขี้เหล็ก',
    'วัดโคกอุดม',
    'วัดหาดสูง',
    'บ้านเขาดิน(ประชาอุทิศ)',
    'บ้านปราสาท',
    'บ้านวังบัวทอง',
    'บ้านท่าสะตือ',
    'บ้านชงโคสันติสุขวิทยาคาร',
    'บ้านราษฎร์เจริญ',
    'บ้านวังรี',
    'บ้านแก่งดินสอ',
    'บ้านคลองมะไฟ',
    'บ้านอุดมบรรพต',
    'บ้านหินเทิน',
    'บ้านทุ่งแฝก',
    'ชุมชนบ้านทุ่งโพธิ์',
    'บ้านคลองตาหมื่น',
    'สหกรณ์นิคมบ้านคลองปลาดุกลาย',
    'บ้านกระเดียง',
    'อนุบาลนาดี',
    'บ้านบุสูง',
    'บ้านโนนสูง',
    'บ้านโนนแสนสุข',
    'บ้านขุนศรี',
    'บ้านบุพราหมณ์อรุณอนุสรณ์',
    'บ้านบุพราหมณ์',
    'บ้าน ก.ม.80',
    'บ้านทับลาน',
    'บ้านหนองสองห้อง',
    'บ้านหนองตะแบก',
    'ชุมชนบ้านสำพันตา',
    'บ้านหนองแหน',
    'บ้านโคกกระจง',
    'วัดสารวนาราม',
    'วัดกระทุ่มแพ้ว',
    'วัดบางกระเบา',
    'ไผ่แถวอนุสรณ์',
    'บ้านบางขาม',
    'วัดบางเตย',
    'ชุมชนวัดบางแตน',
    'บ้านปากคลองบางกระดาน',
    'บ้านบางปลาร้า (กมลราษฎร์นุกูล)',
    'นิคมพัฒนา',
    'อนุบาลวัดบ้านสร้าง',
    'วัดหัวไผ่',
    'วัดพิกุลวนาราม',
    'บ้านคลองบางไซ',
    'บ้านหนองงูเหลือม',
    'บ้านคลองสารภี',
    'วัดอินทาราม',
    'บ้านบางรุ่งโรจน์',
    'วัดใหม่โพธิ์เย็น',
    'วัดคลองเฆ๋',
    'วัดเทพพิทักษ์บุญญาราม',
    'ประชุมเขตศึกษา',
    'วัดมูลเหล็ก',
    'บ้านคลองสอง (ราษฎร์บำรุง)',
    'วัดไชยมงคล',
    'บ้านด่าน',
    'วัดเกาะลอย',
    'วัดศรีสุทธาวาส',
    'วัดหนองคุ้ม',
    'วัดทุ่งสบก',
    'วัดเนินสูง',
    'วัดใหม่กวางทอง',
    'วัดตะเคียนทอง',
    'วัดคำภู',
    'วัดมะกอกแก้ว',
    'วัดศรีมงคล',
    'วัดดงบัง (ดงบังบำรุงวิทย์)',
    'วัดบ้านโนน',
    'บ้านหอย(เติมเอนจิเนียริ่งอุปถัมภ์)',
    'วัดเกาะมะไฟ',
    'วัดบุฝ้าย',
    'บ้านว่านบ้านด่าน',
    'บ้านโคกสว่าง',
    'วัดขิงกระชาย',
    'อนุบาลประจันตคาม',
    'วัดดงไชยมัน',
    'ชุมชนวัดศรีประจันตคาม',
    'ชุมชนวัดบ้านโง้ง',
    'วัดโพธิ์งาม',
    'วัดพรหมรังษีมิตรภาพที่ 1',
    'บ้านเขานันทา',
    'บ้านคลองแก้มช้ำ',
    'วัดอินทร์ไตรย์',
    'บ้านประเถท(โพธิพิทยาราม)',
    'บ้านทุ่งยาว',
    'วัดหนองแก้ว',
    'วัดโคกเขื่อน',
    'วัดประดิษฐาราม',
    'วัดประสาทรังสฤษฎิ์',
    'วัดราษฎร์เจริญศรัทธาธรรม',
    'บ้านประพาส',
    'บ้านวังทะลุ',
    'บ้านวังขอน',
    'บ้านดงกระทงยาม',
    'บ้านย่านนางวิ่ง',
    'บ้านโป่งไผ่',
    'วัดบุยายใบ',
    'วัดหลังถ้ำวิทยาคาร',
    'บ้านท่าตูม',
    'วัดบุพพาราม',
    'วัดอรัญไพรศรี',
    'อนุบาลศรีมหาโพธิ',
    'บ้านหนองกระทุ่ม',
    'บ้านหนองหอย',
    'บ้านหว้าเอน',
    'วัดสัมพันธ์',
    'วัดระเบาะไผ่',
    'บ้านปรือวายใหญ่',
    'วัดใหม่ประชุมชนมิตรภาพที่ 76',
    'บ้านโป่งกะพ้อ',
    'บ้านมาบเหียง',
    'วัดหนองโพรง',
    'บ้านโคกกระเจียว',
    'วัดนพคุณทอง',
    'วัดหนองหูช้าง',
    'วัดโคกเจริญเนตร',
    'หัวซาวิทยา',
    'ชุมชนบ้านเกาะสมอ(สามัคคีวิทยา)',
    'บ้านหนองปรือน้อย',
    'วัดเนินผาสุก',
    'วัดลิรัญดอน',
    'วัดคู้ลำพัน',
    'วัดหนองเกตุ',
    'บ้านโป่งตะเคียน',
    'วัดสระข่อย',
    'วัดแสงสว่าง',
    'บ้านโคกพนมดี',
    'วัดต้นโพธิ์ศรีมหาโพธิ',
    'อนุบาลศรีมโหสถ',
    'วัดสระมะเขือ',
    'วัดไผ่งาม',
    'ปราจิณราษฎรอำรุง',
    'ปราจีนกัลยาณี',
    'อดุลศาสนกิจศึกษา',
    'มัธยมวัดป่ามะไฟ',
    'ไทยรัฐวิทยา 7',
    'ประชารัฐพัฒนา',
    'กบินทร์วิทยา',
    'ลาดตะเคียนราษฎร์บำรุง',
    'กบินทร์บุรี',
    'วังตะเคียนวิทยาคม',
    'วังดาลวิทยาคม',
    'นนทรีวิทยาคม',
    'เตรียมอุดมศึกษาน้อมเกล้า กบินทร์บุรี',
    'มณีเสวตรอุปถัมภ์',
    'ร่มเกล้า ปราจีนบุรี',
    'ทุ่งใหญ่วิทยาคาร',
    'ชิตใจชื่น',
    'ศรีรักษ์ราษฎร์บำรุง',
    'กระทุ้มแพ้ววิทยา',
    'ประจันตราษฎร์บำรุง',
    'สุวรรณวิทยา',
    'วัดพรหมประสิทธิ์',
    'ศรีมหาโพธิ',
    'กรอกสมบูรณ์วิทยาคม',
    'ศรีมโหสถ',
    'วัดกุดตะเคียน',
    'วัดวังทิพย์พันธาราม',
    'วัดเกาะกระชาย',
    'วัดสมบูรณ์สามัคคี(ปากช่องประชานุกูล)',
    'วัดหนองทองทราย',
    'วัดใหม่บำเพ็ญผล',
    'วัดคลองโพธิ์',
    'วัดสุวรรณศิริ',
    'วัดสันติวัฒนาราม',
    'บ้านคลอง 3 วิทยา(ประชาสามัคคี)',
    'บ้านคลอง 3 (ดรุณศึกษา)',
    'วัดดอนยอ',
    'บ้านชวดบัว',
    'วัดท่าช้าง(แสงปัญญาวิทยาคาร)',
    'ชลนายกสงเคราะห์',
    'วัดเจดีย์ทอง(สุขวิทยาคาร)',
    'วัดโบสถ์การ้อง',
    'อนุบาลเมืองนครนายก(ศรีประชานคร)',
    'วัดท่าทราย',
    'วัดกุฏิการาม(สามัคคีวิทยาคาร)',
    'อนุบาลนครนายก',
    'วัดวังไทร',
    'วัดท่าข่อย(ศาสนกิจโกศล)',
    'วัดวังตูม',
    'วัดโคกลำดวน',
    'วัดวังปลาจีด',
    'วัดเอี่ยมประดิษฐ์',
    'วัดธรรมปัญญา',
    'วัดสุตธรรมาราม',
    'วัดโยธีราษฎร์ศรัทธาราม',
    'วัดเขาน้อย(ศิลาทองวิทยาคาร)',
    'วัดหนองเตยตั้งตรงจิตร 8',
    'วัดสันตยาราม',
    'ชุมชนบ้านคลองเหมือง',
    'บ้านดอนเจริญ',
    'วัดบางปรัง',
    'บ้านกำแพงเศียร',
    'วัดบางหอย',
    'วัดศรีจุฬา',
    'วัดคีรีวัน',
    'วัดสบกเขียว',
    'ชุมชนวัดศรีนาวา(ดิลกชนศรีนาวา)',
    'วัดพราหมณี',
    'สาริกา',
    'วัดตำหนัก(ละเอียดชำนาญอนุศาสน์)',
    'หัวเขาแก้ว',
    'วังดอกไม้',
    'บ้านดงวิทยาคาร',
    'วัดหุบเมย',
    'วัดวังยายฉิม',
    'วัดท่าด่าน',
    'วัดท่าชัย',
    'วันครู 2504',
    'บ้านดงแขวน',
    'อนุบาลอำเภอปากพลี(ชุมชนบ้านเกาะหวาย)',
    'วัดพรหมเพชร',
    'วัดเกาะกา',
    'วัดลำบัวลอย',
    'วัดแขมโค้ง',
    'บ้านโคกสว่าง',
    'วัดท่ามะปราง',
    'วัดนาหินลาด',
    'วัดโพธิ์',
    'วัดเนินหินแร่',
    'บ้านบุ่งเข้',
    'วัดเนินสะอาด',
    'บ้านเขาเพิ่ม',
    'บ้านเขาส่องกล้อง',
    'บ้านเขาหัวนา',
    'วัดโคกสว่าง',
    'บ้านเขาไม้ไผ่',
    'บ้านคลอง 30',
    'วัดโบสถ์เจริญธรรม',
    'ตลาดสี่แยก',
    'วัดทองหลาง(สุขชมวิทยาคาร)',
    'วัดท่าทราย(พิมพานุสร)',
    'วัดเลขธรรมกิตต์(กรีเมธอุทิศ)',
    'วัดอัมพวัน(ทองโสภณวิทยาคาร)',
    'อนุบาลบ้านนา(วัดช้าง)',
    'วัดทองจรรยา',
    'ชุมชนบ้านวังไทร(วังไทรวิทยาคม)',
    'วัดทองย้อย(วุฒิกรประชานุกูล)',
    'ไทยรัฐวิทยา 73 (วัดบ้านพร้าว)',
    'วัดทางกระบือ (เอื้อดีวิทยาคาร)',
    'วัดหนองรี',
    'วัดโพธิ์แก้วเบญจธาราม',
    'วัดศรีสุวรรณ',
    'วัดบ้านพริก',
    'วัดหนองคันจาม',
    'บ้านคลอง 33',
    'บ้านชะวากยาว',
    'บ้านคลอง 31',
    'ชุมชนวัดป่าขะ',
    'วัดหนองเคี่ยม',
    'วัดแหลมไม้ย้อย',
    'วัดพิกุลแก้ว',
    'วัดเกาะพิกุล',
    'บ้านหนองกันเกรา',
    'บ้านเขาดิน(แสงสว่างวิทยา)',
    'วัดกุฎีเตี้ย',
    'วัดสะพาน(ศิษย์สมพงษ์อุทิศ)',
    'ชุมชนวัดอำภาศิริวงศ์',
    'บ้านคลองใหญ่',
    'ชุมชนวัดเชี่ยวโอสถ',
    'บ้านชุมพล(อ้นอารีศึกษาคาร)',
    'วัดราษฎร์ศรัทธาธรรม',
    'บ้านคลอง 14',
    'บ้านทำนบ(เลิศสินอนุสรณ์)',
    'วัดทวีพูลรังสรรค์',
    'บ้านคลอง 1',
    'วัดสันติธรรมราษฎร์บำรุง',
    'วัดประสิทธิเวช',
    'บ้านกลางคลอง 30',
    'วัดบางนางเล็ก',
    'วัดใหม่พงษ์โสภณ',
    'วัดอรุณรังษี',
    'วัดพลอยกระจ่างศรี',
    'วัดปากคลองพระอาจารย์',
    'บ้านเตยใหญ่',
    'วัดจันทร์เรือง',
    'วัดเข็มทอง',
    'บ้านดอนกลาง(กิดารีศึกษาคาร)',
    'บ้านหัวหมอน',
    'บ้านลาดช้าง(ระคนกิจศึกษา)',
    'บ้านช่องตะเคียน',
    'บ้านปากคลอง 17',
    'บ้านคลองหกวา',
    'วัดสุนทรพิชิตาราม',
    'บ้านปากคลอง 31',
    'วัดโพธิ์แทน',
    'วัดอารีราษฎร์',
    'วัดราษฎร์ประดิษฐ์',
    'บ้านคลอง 24',
    'บ้านหนองขุม(พูนแถมอุปถัมภ์)',
    'บ้านคลอง 23',
    'บ้านปลายคลอง 22',
    'บ้านคลอง 22(อาจเอิบจิตรอุปถัมภ์)',
    'อนุบาลองครักษ์(ผดุงองครักษ์ประชา)',
    'วัดสว่างอารมณ์(ศรีวัฒนานุกูล)',
    'เมืองนครนายก',
    'นครนายกวิทยาคม',
    'นวมราชานุสรณ์',
    'ปิยชาติพัฒนา',
    'มัธยมศรีจุฬา',
    'ปากพลีวิทยาคาร',
    'บ้านนา (นายกพิทยากร)',
    'เขาเพิ่มนารีผลวิทยา',
    'เลขธรรมกิตติ์วิทยาคม',
    'องครักษ์',
    'ภัทรพิทยาจารย์',
    'บ้านด่าน(ราษฎรบำรุง)',
    'บ้านลุงพลู',
    'วนาสงเคราะห์',
    'บ้านบะขมิ้น(วัชระอนุสรณ์)',
    'บ้านคลองน้ำใส',
    'บ้านหนองขี้เห็น',
    'สามัคคีประชาสรรค์',
    'วัดใหม่โพธิ์ทอง',
    'บ้านวังจั่น',
    'บ้านท่าเกษม',
    'บ้านคลองอุดมสุข',
    'บ้านหนองนกกระเรียน',
    'บ้านทุ่งพระ',
    'อนุบาลเมืองสระแก้ว',
    'บ้านคลองปลาโด',
    'บ้านคลองศรีเมือง',
    'บ้านคลองผักขม',
    'บ้านโป่งดาวเรือง',
    'บ้านคลองน้ำเขียว',
    'บ้านท่ากะบาก',
    'บ้านหนองเตียน',
    'บ้านกิโลสาม',
    'บ้านป่าระกำ',
    'บ้านแก้ง',
    'บ้านห้วย',
    'บ้านเหล่ากกโก',
    'เขาสิงโต',
    'บ้านแสงจันทร์',
    'บ้านเนินดินแดง',
    'บ้านคลองหมากนัด',
    'บ้านเทศมงคล',
    'บ้านท่าระพา',
    'บ้านเนินสวนอ้อย',
    'บ้านหนองไทร',
    'บ้านเขามะกา',
    'บ้านหนองข่า',
    'บ้านศาลาลำดวน',
    'บ้านหนองผูกเต่า',
    'บ้านท่าช้าง',
    'บ้านคลองสำอางค์',
    'บ้านคลองหมี',
    'บ้านสี่แยก',
    'อนุบาลวัดสระแก้ว',
    'บ้านเนินสะอาด',
    'บ้านหนองกะพ้อ',
    'บ้านลัดกะสัง',
    'บ้านทุ่งหินโคน',
    'บ้านแก่งสีเสียด',
    'ธรรมยานประยุต',
    'บ้านน้ำซับเจริญ',
    'บ้านทุ่งพลวง',
    'บ้านใหม่ถาวร',
    'บ้านเนินผาสุก',
    'บ้านคลองมะละกอ',
    'บ้านเขาข่า',
    'บ้านคลองคันฉอ',
    'บ้านหน้าสถานี',
    'บ้านโคกมะตูม',
    'บ้านคลองบุหรี่',
    'บ้านท่าแยก',
    'บ้านคลองไก่เถื่อน',
    'บ้านเขาตาง้อก',
    'ทับทิมสยาม่05',
    'บ้านคลองหาด',
    'บ้านเขาดิน',
    'บ้านคลองหาดสาขาบ้านเขาช่องแคบ',
    'อนุบาลคลองหาด',
    'บ้านเขาเลื่อม',
    'อนุบาลคลองหาด(สาขาบ้านใหม่ไทยเจริญ)',
    'บ้านวังวน',
    'บ้านวังยาง',
    'บ้านนาดี',
    'บ้านซับถาวร',
    'บ้านพรหมนิมิต',
    'บ้านวังปืน',
    'บ้านซับน้อย',
    'สามัคคีราษฎร์บำรุง',
    'สามัคคีสกุลอรุณวิทยา',
    'บ้านหินกอง',
    'มหาธิคุณวิทยา',
    'บ้านชุมทอง',
    'บ้านห้วยไคร้',
    'วัดคลองตาสูตรสามัคคี',
    'บ้านคลองหินปูน',
    'บ้านหนองสมบูรณ์',
    'บ้านดอนดินแดง',
    'ชุมชนบ้านตาหลังใน',
    'บ้านท่าตาสี',
    'บ้านทัพหลวง',
    'บ้านหนองปรือ',
    'บ้านใหม่สระบุรี',
    'บ้านหนองผักหนาม',
    'บ้านหนองแก',
    'บ้านมหาเจริญ',
    'บ้านพรสวรรค์',
    'บ้านด่านชัยพัฒนา',
    'บ้านซับเจริญ',
    'บ้านเขาตะกรุบพัฒนา',
    'วังจระเข้',
    'บ้านคลองใหญ่',
    'บ้านแก่งสะเดา',
    'องค์การอุตสาหกรรมป่าไม้ที่6',
    'อนุบาลวังน้ำเย็นมิตรภาพที่179',
    'วัดเกศแก้ว',
    'มิตรสัมพันธ์วิทยา',
    'บ้านวังบูรพา',
    'บ้านวังแดง',
    'อนุบาลเขาฉกรรจ์',
    'บ้านนา(สามัคคีวิทยา)',
    'บ้านวังรี',
    'บ้านเขาภูมิ่ง',
    'บ้านพรสวรรค์',
    'บ้านเขาสามสิบ',
    'วัดพวงนิมิต',
    'บ้านคลองสิบสาม',
    'บ้านพระเพลิง',
    'บ้านภูเงิน',
    'บ้านบึงพระราม',
    'บ้านหนองปักหลัก',
    'บ้านใหม่ไทรทอง',
    'บ้านท่าเต้น',
    'บ้านท่าผักชี',
    'บ้านเนินสายฝน (แหลมทอง)',
    'บ้านคลองนางาม',
    'บ้านเขาน้อยสามัคคี',
    'บ้านคลองเจริญ',
    'บ้านหนองกะทะ',
    'บ้านซับมะนาว',
    'บ้านธารนพเก้า',
    'บ้านหนองหว้า',
    'บ้านคลองธรรมชาติ',
    'บ้านเขาดิน',
    'บ้านวังทอง',
    'บ้านเขาแหลม',
    'บ้านคลองยายอินทร์',
    'บ้านคลองอุดม',
    'บ้านวังดารา',
    'วังไผ่',
    'อนุบาลวังสมบูรณ์',
    'บ้านซับสิงโต',
    'บ้านไร่สามศรี',
    'บ้านคลองทราย',
    'บ้านวังใหม่',
    'บ้านซับเกษม',
    'บ้านวังสำลี',
    'บ้านไพรจิตรวิทยา',
    'วังศรีทอง',
    'บ้านไทรทอง',
    'คลองหาดพิทยาคม',
    'วังน้ำเย็นวิทยาคม',
    'สระแก้ว',
    'ไทรเดี่ยววิทยา',
    'วังหลังวิทยาคม',
    'บ้านแก้งวิทยา',
    'ท่าเกษมพิทยา',
    'สิริราชอนุสรณ์',
    'เขาฉกรรจ์วิทยาคม',
    'ทรัพย์สมบูรณ์วิทยาคม',
    'วังสมบูรณ์วิทยาคม',
    'วังไพรวิทยาคม',
    'บ้านโคคลาน',
    'ประเสริฐ-น้อย รุจิรวงศ์ 1',
    'บ้านไทยสามัคคี',
    'บ้านกุดเวียน',
    'บ้านทัพเซียม',
    'บ้านนางามมิตรภาพที่131',
    'กองทัพบกอุทิศบ้านหนองไผ่',
    'บ้านแก้วเพชรพลอย',
    'อนุบาลตาพระยา',
    'บ้านตะโก',
    'บ้านกะสัง',
    'เพชรรัตนราชสุดา',
    'บ้านหนองบัวสันติสุข',
    'บ้านรัตนะ',
    'บ้านทัพไทย',
    'บ้านโคกปราสาท',
    'ทับทิมสยาม่03',
    'บ้านทดวงศ์สมบูรณ์',
    'กองทัพบกอุทิศบ้านใหม่ไทยถาวร',
    'บ้านหนองผักแว่น',
    'วัดหนองติม',
    'บ้านโคกไพล',
    'บ้านเจริญสุข',
    'บ้านมะกอก',
    'บ้านโคกเพร็ก',
    'บ้านโคกทหาร',
    'บ้านแสง',
    'บ้านโคกแจง',
    'บ้านห้วยชัน',
    'ช่องกุ่มวิทยา',
    'ชุมชนบ้านแซร์ออ',
    'บ้านใหม่ทหารบกพัฒนา',
    'บ้านเขาพรมสุวรรณ',
    'บ้านท่าเกวียน',
    'บ้านเขาจาน',
    'บ้านนางาม',
    'บ้านสี่แยก',
    'บ้านคลองมะนาว',
    'บ้านหนองยาง',
    'บ้านโนน',
    'บ้านทับใหม่',
    'บ้านซับใหญ่',
    'วัดสารคุณสโมสร',
    'ประชาเกษตรพัฒนา',
    'วัดห้วยเดื่อ',
    'บ้านโป่งคอม',
    'บ้านหนองหอย',
    'บ้านหนองใหญ่',
    'บ้านทดน้อย',
    'อนุบาลศรีวัฒนาวิทยา',
    'ชุมชนบ้านโนนจิก',
    'บ้านพร้าว',
    'บ้านเนินผาสุก',
    'บ้านหนองแสง',
    'บ้านช่องก่ำ',
    'บ้านโนนผาสุก',
    'บ้านหนองตะเคียนบอน',
    'ร่มเกล้า',
    'ร่มเกล้าสาขาบ้านคลองคันโท',
    'บ้านใหม่ไทยพัฒนา',
    'บ้านวังรี',
    'บ้านหนองน้ำใส',
    'บ้านหนองเรือ',
    'บ้านเนินสะอาด',
    'บ้านซับนกแก้ว',
    'บ้านซับนกแก้ว(สาขาบ้านหนองหัวช้าง)',
    'บ้านบุกะสัง',
    'บ้านหนองหมู',
    'บ้านหนองแวง',
    'บ้านหนองหล่ม',
    'บ้านภักดีแผ่นดิน',
    'บ้านท่าช้าง',
    'บ้านใหม่ศรีจำปาทอง',
    'บ้านโคกสว่าง',
    'วัดพุทธิสาร',
    'ไทยรัฐวิทยา 83 (บ้านห้วยโจด)',
    'คลองยางนุสรณ์',
    'บ้านบ่อนางชิง',
    'บ้านวังยาว',
    'บ้านหนองหมูน้อย',
    'บีกริม',
    'บ้านคลองวัว',
    'บ้านทับยาง',
    'อพป.คลองน้ำใส',
    'บ้านแสนสุข',
    'บ้านเขาน้อย',
    'บ้านกุดหิน',
    'บ้านจัดสรรสามัคคี',
    'บ้านคลองหว้า',
    'สระปทุม',
    'บ้านโคกสะแบง',
    'บ้านท่าข้าม',
    'บ้านหนองแข้',
    'บ้านด่าน',
    'บ้านกุดม่วง',
    'บ้านตุ่น',
    'บ้านไทยสามารถ',
    'อนุบรรพต',
    'อนุบาลศรีอรัญโญทัย',
    'ชุมชนบ้านใหม่หนองไทร',
    'บ้านหนองเทา',
    'บ้านโคก',
    'ส.ไทยเสรีอุตสาหกรรม 3',
    'นิคมสงเคราะห์ 2',
    'ชุมชนบ้านโนนสาวเอ้',
    'กรุงไทย',
    'บ้านคลองยางประชาสรรค์',
    'บ้านหนองปรือ',
    'บ้านทดเจริญ',
    'บ้านกุดแต้',
    'บ้านหนองผักบุ้ง',
    'บ้านฟากห้วย',
    'นิคมสงเคราะห์ 1',
    'เมืองไผ่',
    'บ้านหนองสังข์',
    'บ้านเหล่าอ้อย',
    'บ้านหันทราย',
    'บ้านหนองบัว',
    'บ้านบ่อหลวง',
    'บ้านโคกสูง',
    'บ้านหนองเสม็ด',
    'บ้านละลมติม',
    'บ้านโนนหมากมุ่น',
    'สมาคมไลอ้อน(บ้านอ่างศิลา)',
    'บ้านหนองจาน',
    'สหัสหงษ์มหาคุณ',
    'บ้านหนองแอก',
    'วัดหนองม่วง',
    'อนุบาลธรรมศิริวิทยาคาร',
    'บ้านโคกสามัคคี',
    'ชุมชนบ้านหนองแวง',
    'บ้านหนองเม็ก',
    'บ้านหนองมั่ง',
    'วัฒนานคร',
    'อรัญประเทศ',
    'ทัพราชวิทยา',
    'ซับนกแก้ววิทยา',
    'คลองน้ำใสวิทยาคาร',
    'ร่มเกล้าวัฒนานคร สระแก้ว รัชมังคลาภิเษก',
    'ซับม่วงวิทยา',
    'ตาพระยา',
    'ทัพพระยาพิทยา',
    'ตำบลโคกกรวด(ผดุงรัฐประชานุกูล)',
    'บ้านโป่งแมลงวัน',
    'โคกเพชรสระมโนรา',
    'บ้านหนองเป็ดน้ำ(สวัสดิ์ราษฎร์วิทยา)',
    'บ้านหนองรังกา',
    'สุขานารี 2 (หนองขอน)',
    'บ้านโคกสูง',
    'บ้านลำเชิงไกร',
    'วัดสว่างอารมณ์',
    'บ้านบึงทับช้าง',
    'บ้านกรูดหนองออก',
    'บ้านกล้วยจอหอ',
    'บ้านจอหอ',
    'บ้านระกาย',
    'บ้านไชยมงคล(สืบสินวิทยา)',
    'บ้านหนองพลวงใหญ่',
    'บ้านบุ(ประชารัฐพัฒนา)',
    'เมืองนครราชสีมา',
    'วัดสระแก้ว',
    'สวนหม่อน',
    'สามัคคีรถไฟ',
    'สุขานารี',
    'อนุบาลนครราชสีมา',
    'วัดทุ่งสว่าง',
    'ชลประทานสงเคราะห์',
    'บ้านหนองไผ่ล้อม"พยัคฆเดชบำเพ็ญ"',
    'เคหะประชาสามัคคี',
    'บ้านหลักร้อย',
    'วัดโคกพรมตั้งตรงจิตร7',
    'บ้านหัวทะเล',
    'บ้านโคกไผ่-ขนาย',
    'บึงพญาปราบ',
    'บ้านมะค่า',
    'บ้านลองตอง',
    'บ้านหนองบัว(สหรัฐพิบูลประชาสรรค์)',
    'ตำบลบ้านโพธิ์',
    'บ้านภูเขาลาด',
    'บ้านมะขามเฒ่า(เปรมประชารัฐวิทยา)',
    'บ้านยางน้อยหัวสิบ',
    'ชุมชนบ้านศีรษะละเลิง',
    'บ้านคนชุม',
    'บ้านพลกรัง(เดชาติวงค์คงฤทธิ์อนุสรณ์)',
    'บ้านบึงตะโก',
    'บ้านพะเนา',
    'บ้านดอนกระทิง-ศีรษะช้าง',
    'บ้านพุดซา',
    'บ้านละลมโพธิ์',
    'บึงสาลี',
    'บ้านหนองยารักษ์-ลำโพง(รัฐประชาสรรค์)',
    'บ้านหนองพลวง',
    'เสนานุเคราะห์',
    'บ้านบึงสาร',
    'บ้านทุ่งกระโดน',
    'บ้านสีมุม(ธวัชชัยราษฎร์อนุสรณ์)',
    'บ้านโกรกเดือนห้า',
    'อ่างห้วยยาง',
    'บ้านฝ้ายนาตมส้มป่อย',
    'บ้านหนองกระทุ่ม',
    'บ้านหนองไข่น้ำ',
    'บ้านกระโดน',
    'บ้านหนองปรู',
    'บ้านต่างตา',
    'บ้านหนองตะลุมปุ๊ก',
    'บ้านหนองตาคง',
    'วัดหนองบัวศาลา',
    'อ่างหนองแหนประชาสามัคคี',
    'โยธินนุกูล',
    'ทหารอากาศบำรุง',
    'บ้านทับช้าง',
    'บ้านหนองสมอประชาพัฒนา',
    'ประชาสามัคคี',
    'บ้านมาบมะค่า',
    'บ้านยองแยง',
    'บ้านหนองระเวียง',
    'บ้านหมื่นไวย',
    'ชุมชนบ้านประโดก-โคกไผ่',
    'บ้านดอนขวาง',
    'บ้านพะไล',
    'ไตรคามสามัคคี',
    'บ้านเกรียมโนนสำโรง',
    'บ้านไพ',
    'บ้านหนองพลอง',
    'บ้านขามเฒ่า',
    'บ้านจันอัด',
    'บ้านด่านติง',
    'บ้านเหล่าพิทยาคม',
    'บ้านส้มมิตรภาพที่93',
    'บ้านดอนชมพู(สังฆประชานุเคราะห์)',
    'บ้านเปลาะปลอเมืองที(ประชารัฐสามัคคี)',
    'บ้านศรีสุข(คุรุราษฎร์สามัคคี)',
    'บ้านดอนหวาย(สิงห์คุรุประชาสรรค์)',
    'บ้านโนนมะกอก',
    'บ้านหนองหว้า(ประชาสรรค์)',
    'วัดบ้านด่านคนคบ',
    'บ้านคอนน้อย',
    'บ้านด่านทองหลาง',
    'บ้านโตนด',
    'บ้านด่านเกวียน',
    'อนุบาลตลาดแค',
    'บ้านธารปราสาท',
    'บ้านวังม่วง',
    'บ้านหญ้าคาใต้',
    'บ้านหญ้าคาเหนือ',
    'การไฟฟ้าส่วนภูมิภาคสงเคราะห์3ฯ',
    'ชุมชนบ้านเพชร',
    'บ้านขนายดี(ประชารัฐบำรุง)',
    'บ้านขามชั่งโค',
    'บ้านบิง(สุประชาจูงกลางนุสสรณ์',
    'บ้านสำโรง(คุรุรัฐประชาสรรค์',
    'บ้านมะรุม',
    'บ้านเสลาถั่วแปบ',
    'บ้านหญ้าคา ว.ท.อ.',
    'บ้านหนองนา',
    'บ้านเปราะหอมพัฒนา',
    'บ้านพลสงคราม(แพงรัฐราษฎร์พิทยา)',
    'บ้านหนองม้า',
    'บ้านหนองตะไก้(ประชาพัฒนา)',
    'บ้านมะค่า(คงฤทธิ์บัวสุวรรณฯ)',
    'บ้านเดิ่นหัวบึงวิทยา',
    'บ้านพลจลก',
    'บ้านหนองแจง',
    'บ้านกระเพรา',
    'บ้านคล้าโนนคราม',
    'ดอนท้าววิทยา',
    'บ้านปราสาท(สุวรรณราษฎร์ฯ)',
    'บ้านเมืองที',
    'บ้านหนองอ้อ',
    'บ้านกระถินหนองเครือชุด',
    'บ้านดอนผวา',
    'บ้านสะพาน',
    'บ้านคอหงษ์',
    'บ้านหนองโจด',
    'ไตรมิตรวิทยา',
    'บ้านโนนมันเทศ',
    'บ้านซาด',
    'บ้านดงพลอง(สำนักงานสลากกินแบ่งฯ)',
    'บ้านเพิก(รัฐประชาสามัคคี)',
    'บ้านไพล(ไตรมิตรสามัคคี)',
    'บ้านหลุมข้าว',
    'บ้านใหม่(คุรุราษฎร์พัฒนา)',
    'โพธารามพิทยาคม',
    'กันเกราพิทยาคม',
    'บ้านดอนม่วง',
    'บ้านถนนถั่ว(กองทัพบกประชาสามัคคี)',
    'บ้านหนองจำปา',
    'ราชสีมาวิทยาลัย',
    'สุรนารีวิทยา',
    'บุญวัฒนา',
    'มหิศราธิบดี',
    'โคราชพิทยาคม',
    'สุรธรรมพิทักษ์',
    'สุรนารีวิทยา 2',
    'พุดซาพิทยาคม',
    'ราชสีมาวิทยาลัย2',
    'นครราชสีมาวิทยาลัย',
    'บุญวัฒนา 2',
    'บุญเหลือวิทยานุสรณ์',
    'นครราชสีมาปัญญานุกูล',
    'โนนสูงศรีธานี',
    'ศรีสุขวิทยา',
    'ธารปราสาทเพชรวิทยา',
    'มะค่าวิทยา',
    'โตนดพิทยาคม',
    'ไทรทอง',
    'บ้านดงพลอง',
    'บ้านทรัพย์เจริญ',
    'บ้านพระนารายณ์',
    'บ้านหนองขามน้อย',
    'วัดหนองบัวยอดแก้ว',
    'จักราชราษฎร์สามัคคี',
    'บ้านตะกุดเครือปลอก',
    'บ้านโนนพฤกษ์',
    'บ้านหนองบัวกลาง',
    'บ้านหนองนกกวัก',
    'หนองพลวงพิทยา',
    'บ้านตูมราษฎร์บูรณะ',
    'บ้านทองหลาง',
    'รัฐการุณวิทยา',
    'วัดเหมสูง',
    'บ้านโนนตาพรม',
    'บ้านช่องโค',
    'บ้านละกอ',
    'ลุงถ่อนราษฎร์อุทิศ',
    'อรพิมพ์วิทยา',
    'อรพิมพ์วิทยา(สาขาบ้านหนองบัวขาว)',
    'บ้านโกรกไม้แดง',
    'บ้านโคกสำโรง',
    'วัดหินมงคล',
    'บ้านบุวังหว้า',
    'บ้านหนองไผ่',
    'ขามสงเคราะห์',
    'ชุมชนสว่างวิทยา',
    'ชาติวิทยา',
    'บ้านโนนทะยุง',
    'บ้านโนนพะไล',
    'บ้านหนองขาม',
    'ราษฎร์บำรุง',
    'สามัคคีวิทยา',
    'บ้านหนองแมว',
    'วัดหนองนกคุ่ม',
    'ชุมชนบ้านบุสามัคคีพัฒนา',
    'บ้านโคกพระ',
    'วัดหนองจอก',
    'วัดหนองพลวง',
    'บ้านสวนปอ',
    'บ้านหินโคน',
    'เสมาอุปถัมภ์',
    'บ้านหนองจระเข้หิน',
    'บ้านดอน',
    'วัดพะโค',
    'โชคชัยพรหมบุตรบริหาร',
    'บ้านบิง',
    'บ้านบึงทับปรางค์',
    'บ้านพิชิตคเชนทร์',
    'บ้านด่านเกวียน',
    'บ้านตูม',
    'บ้านหนองบอน',
    'บ้านขี้ตุ่น(แหล่งจะหลุง)',
    'บ้านหนองกก',
    'ชลประทานบ้านกอโจด',
    'บ้านโจด',
    'บ้านท่าเยี่ยมวิทยา',
    'บ้านโนนเพชร',
    'ชุมชนดอนไพล',
    'ชลประทานนาตลิ่งชัน',
    'บ้านบึงพระ',
    'บ้านท่าลาดขาว',
    'บ้านบึงไทย',
    'บ้านท่าอ่าง',
    'บ้านกุดโบสถ์',
    'บ้านดอนพราหมณ์',
    'บ้านหนองเสาเดียว[',
    'บ้านโนนปอแดง',
    'บ้านโค้งกระโดน',
    'บ้านดอนเกตุ',
    'บ้านดะแลง',
    'บ้านปอพราน',
    'บ้านหนองปรึก',
    'บ้านปรางค์',
    'วัดกุดสวาย',
    'ชุมชนพลับพลา',
    'บ้านกอกวิทยา',
    'บ้านคลองกลาง',
    'บ้านท่าตะเคียน',
    'บ้านละลม',
    'บ้านกุดจอกน้อย',
    'บ้านกุดจอกใหญ่',
    'กงรถราษฎร์สามัคคี',
    'บ้านซ่าเลือด',
    'บ้านโนนขี้ตุ่น',
    'บ้านหนองบัวรองสามัคคี',
    'บ้านหัวทำนบ',
    'บ้านจอมศรี',
    'บ้านหนองม่วงหวาน',
    'บ้านหนองม่วงใหญ่',
    'บ้านหัวอ้อทะมวงสามัคคี',
    'บ้านใหม่ศรีวิไลพุทไธจารย์',
    'วัดหนองนา',
    'บ้านนาตะคุ',
    'บ้านตะโก',
    'บ้านหนองไผ่',
    'ชุมชนบ้านทับสวาย',
    'บ้านโนนทอง',
    'บ้านโนนสำราญ',
    'บ้านเมืองพลับพลา',
    'บ้านท่าลี่',
    'บ้านสระแก้ว',
    'บ้านหนองชุมแสง',
    'บ้านหลุ่งตะเคียน',
    'รักชาติประชาบำรุง',
    'บ้านกระทุ่มแท่น',
    'บ้านตะแกรง',
    'บ้านหนองนกเขา',
    'บ้านหนองสาย',
    'บ้านกู่โกสีย์วิทยา',
    'บ้านเมืองรัง',
    'บ้านหนองปรือ',
    'บ้านหนองโพดเพชรพิทยา',
    'บ้านหลุ่งประดู่',
    'บ้านโคกพลวง',
    'บ้านโนนสุวรรณแสงวิทยา',
    'บ้านหนองม่วง',
    'บ้านห้วยแคน',
    'อนุบาลมณีราษฏร์คณาลัย',
    'บ้านห้วยปอ',
    'บ้านสวนหอม',
    'บ้านแสนสุข',
    'บ้านหนองโสน',
    'บ้านหัวสะพาน',
    'บ้านหินดาด',
    'บ้านโคกสะอาด',
    'บ้านตะคร้อ',
    'บ้านสระมะค่า',
    'บ้านหนองพลอง',
    'บ้านซับตะคร้อ',
    'บ้านสระซาง',
    'บ้านใหม่ไทยเจริญ',
    'บ้านใหม่สมบูรณ์',
    'บ้านถนนหัก',
    'บ้านลุงเขว้า',
    'บ้านหนองจาน',
    'บ้านท่าตะแบก',
    'บ้านทุ่งหัวขวาน',
    'บ้านพระ',
    'บ้านสารภี',
    'บ้านยอกขาม',
    'บ้านหนองตะไก้',
    'บ้านสว่างวิทยา',
    'บ้านหนองไทร',
    'บ้านหนองบุนนาก',
    'เทียมนครวิทยา',
    'บ้านหนองไม้ไผ่',
    'บ้านสระมะค่า',
    'บ้านหนองตะลุมปุ๊ก',
    'บ้านหนองกระทุ่ม',
    'บ้านบุกระโทก',
    'บ้านคลองสารเพชร',
    'บ้านหนองลูกควาย',
    'บ้านหนองสะแก',
    'บ้านหนองหัวแรด',
    'บ้านกรูด',
    'สมบูรณ์วัฒนา',
    'เพชรมาตุคลา',
    'พิมานประชาสันต์',
    'ไทรงามเฉลิมพระเกียรติ',
    'อนุบาลท่าช้างเฉลิมพระเกียรติ',
    'โนนตาวิทย์',
    'บ้านกันผม',
    'บ้านพระพุทธ',
    'บ้านด่านพัฒนา',
    'ภูทองวิทยา',
    'ชุมชนวัดรวง',
    'บ้านนาตาวงษ์',
    'วัดสลักได',
    'สมานมิตรวิทยา',
    'อำนวยศิลป์สารกิจ',
    'โคกวังวนวิทยา',
    'บ้านโสง(คุรุประชาสรรค์)',
    'บ้านหนองบัวโคก',
    'บ้านหนองยาง',
    'จักราชวิทยา',
    'คลองเมืองพิทยาคม',
    'หนองขามพิทยาคม',
    'ด่านเกวียนวิทยา',
    'ดอนไพลพิทยาคม',
    'โชคชัยสามัคคี',
    'เมืองพลับพลาพิทยาคม',
    'หินดาดวิทยา',
    'ห้วยแถลงพิทยาคม',
    'หนองบุนนากพิทยาคม',
    'หนองบุนนากประสงค์วิทยา',
    'หนองงูเหลือมพิทยาคม',
    'ท่าช้างราษฏร์บำรุง',
    'หนองยางพิทยาคม',
    'บ้านโกรกสำโรง',
    'บ้านไชยวาล',
    'บ้านดอนแสนสุข',
    'บ้านหนองต้อ',
    'บ้านหนองใหญ่',
    'บ้านใหญ่',
    'บ้านสระผักโพด',
    'บ้านซับก้านเหลือง',
    'บ้านหนองโสน',
    'บ้านหนองหว้าประชาอาสา',
    'บ้านครบุรีนครธรรมโฆสิตวิทยาคาร',
    'บ้านโคกกระชายโนนกุ่ม',
    'บ้านโคกใบบัว',
    'บ้านมาบกราด',
    'บ้านตะกุดโนนระเวียง',
    'บ้านหนองโบสก์',
    'บ้านไผ่',
    'ชุมชนจระเข้หิน(สังฆ์คุรุประชาฯ)',
    'บ้านคลองยาง(มูลบนอุปถัมภ์)',
    'จอมทองวิทยา',
    'บ้านหนองหว้าดอนตะเกียด',
    'บ้านเฉลียง(คุรุราษฎร์บำรุง)',
    'บ้านทุ่งแขวน',
    'บ้านพนาหนองหิน',
    'บ้านหนองรัง',
    'บ้านแชะ',
    'บ้านดอนกรูด',
    'บ้านหนองตะแบก',
    'บ้านเทพนิมิต',
    'บ้านซับสะเดา',
    'บ้านบุตาโฮ-โนนกลาง',
    'ครบุรีวิทยา',
    'บ้านอังโกน-ห้วยทราย',
    'บ้านบุมะค่า(ชาวไร่สามัคคี)',
    'บ้านบุยายแลบ',
    'บ้านมาบตะโกเอน',
    'บ้านหนองสองห้อง',
    'บ้านหนองไทร',
    'บ้านทรัพย์เจริญ',
    'บ้านโปร่งสนวน',
    'บ้านไร่แหลมทอง',
    'บ้านลำเพียก',
    'บ้านหนองแคทราย',
    'บ้านบุหว้าสามัคคี',
    'บ้านหนองหินโคน',
    'บ้านสระว่านพระยา',
    'บ้านหนองเมา',
    'ชุมชนบ้านนารากอรพิมพ์',
    'บ้านโคกสะอาด',
    'บ้านหนองเสือบอง',
    'บ้านกุดโบสถ์',
    'บ้านโคกโจด',
    'บ้านเตาเหล็ก-ลุมปุ๊ก',
    'บ้านท่าเยี่ยม',
    'บ้านสมบัติเจริญ',
    'บ้านหนองสนวน',
    'บ้านหนองแดง',
    'บ้านโนนสมบูรณ์',
    'บ้านบ่อลิงประชาสรรค์',
    'บ้านราษฎร์สามัคคี',
    'บ้านราษฎร์พัฒนา',
    'บ้านลำไซกง',
    'ตะแบกวิทยา',
    'บ้านปางไม้',
    'สามัคคีประชาสรรค์',
    'บ้านโคกไม้งาม',
    'บ้านสระตะเคียน',
    'บ้านสันติสุข',
    'บ้านหนองใหญ่พัฒนา',
    'บ้านดอนแขวนประชานุกูล',
    'บ้านสุขไพบูลย์',
    'บ้านหนองขนาก',
    'บ้านหนองตะเข้-หนองตูม',
    'บ้านหน้ากลองวิทยา',
    'สกัดนาควิทยา',
    'ชุมชนบ้านเสิงสาง',
    'ท้าวสุรนารี(2521)',
    'บ้านสระประทีป',
    'บ้านหนองไผ่ใหญ่',
    'บ้านหนองหลักศิลา',
    'หนองไผ่วิทยา',
    'บ้านประชาสันต์',
    'บ้านหนองหิน',
    'บ้านนางเหริญ',
    'วัดหลุมข้าว',
    'บ้านโคกเกษม(สหรัฐ-ราษฎร์สามัคคี)',
    'วัดศีรษะกระบือ',
    'บ้านหนองล้างช้าง',
    'วัดมกุฎไทยาราม',
    'หนองไผ่ผดุงกิจวิทยา',
    'ม่วงแก้ววิทยาคาร',
    'จตุคามวิทยาคม',
    'บ้านโกรกหว้า',
    'วัดบ้านพร้าว',
    'วัดบ้านส่อง',
    'วัดไม้เสี่ยว',
    'บ้านหนองผักไร',
    'บ้านดอน(สหรัฐ-ราษฎร์อุทิศ)',
    'ชุมชนบ้านตะขบ',
    'บ้านเขาพญาปราบ',
    'บ้านคลองสาริกา',
    'บ้านน้อย',
    'บ้านบุหัวช้าง',
    'บ้านยางกระทุง',
    'ป่าไม้อุทิศ 2',
    'บ้านโคกสำราญ',
    'บ้านวังหิน',
    'บ้านสุขัง(ราษฎร์สามัคคี)',
    'บ้านหนองตาด',
    'บ้านหนองนมนาง',
    'บ้านหัน(คุรุราษฎร์อุทิศ)',
    'ไผ่สีสุก',
    'อาจวิทยาคาร',
    'วังวารีวนราษฎร์วัฒนา',
    'วัดพรหมราช',
    'วัดโพธิ์เมืองสามัคคี',
    'สวนหมากสงเคราะห์',
    'ธงชัยเหนือวิทยา(โคกศิลา)',
    'บ้านพันธ์สงวน',
    'บ้านหนองหญ้าขาว',
    'บ้านอ่างเก็บน้ำห้วยยาง',
    'แหลมรวกบำรุง',
    'วัดโคกสระน้อย',
    'วัดทุ่งจาน',
    'วัดม่วง',
    'วัดหงษ์',
    'บ้านกลาง',
    'บ้านบ่อปลา',
    'รักเมืองไทย 1(มูลนิธิเตชะไพบูลย์)',
    'หนองนกเขียนสามัคคี',
    'บ้านโนนแดง',
    'บ้านหนองกก',
    'บ้านหนองกระทุ่ม',
    'บ้านหลุมเงิน',
    'ปักธงชัยชุณหะวัณวิทยาคาร',
    'บ้านเมืองปักสามัคคี',
    'โพธิ์ทองวิทยา',
    'บ้านดู่(สหราษฎร์วิทยา)',
    'บ้านคลองซับน้อยสามัคคี',
    'บ้านวังตะเคียน',
    'บ้านพัดทะเล',
    'บ้านลำนางแก้ว',
    'บ้านเก่าปอแดง',
    'บ้านขี้เหล็ก',
    'บ้านคลองเตย',
    'บ้านโป่งสามัคคี',
    'วัดโพธินิมิตร',
    'บ้านหนองแฟบ(วุฒิรัฐประชาสรรค์)',
    'สะแกราชวิทยาคม',
    'วัดใหม่สำโรง',
    'หนองประดู่',
    'วัดโพนทราย',
    'หนองชุมแสงศรัทธาคาร',
    'บ้านพุปลาไหล',
    'บ้านหนองโดน',
    'บ้านปลายดาบ',
    'บ้านไทยสามัคคี',
    'บ้านบุไผ่',
    'บ้านคลองกุ่ม',
    'บ้านคลองหินร่อง',
    'บ้านระเริง',
    'บ้านหนองไม้สัก',
    'บ้านไทรงาม',
    'บ้านคลองทุเรียน',
    'บ้านคลองบง',
    'บ้านศาลเจ้าพ่อ',
    'บ้านซับไทรทอง',
    'บ้านน้ำซับ',
    'บ้านบุตะโก',
    'บ้านคลองทราย',
    'เจียรวนนท์อุทิศ 2',
    'บ้านคลองใบพัด',
    'บ้านคลองสะท้อน',
    'บ้านท่าวังไทรสามัคคี',
    'บ้านวังหมี',
    'บ้านโคกสันติสุข',
    'บ้านโนนสาวเอ้',
    'บ้านบุเจ้าคุณ',
    'บ้านซับเต่า',
    'บ้านโนนเหลื่อม',
    'ด่านอุดมวิทยา',
    'บ้านบะใหญ่',
    'บ้านวังน้ำเขียว',
    'บ้านหนองแวง',
    'บ้านหนองโสมง',
    'บ้านห้วยน้ำเค็ม',
    'ครบุรี',
    'จระเข้หินสังฆกิจวิทยา',
    'บ้านใหญ่พิทยาคม',
    'มาบตะโกพิทยาคม',
    'เฉลียงพิทยาคม',
    'อรพิมพ์วิทยา',
    'บ้านใหม่พิทยาคม',
    'โนนสมบูรณ์วิทยา',
    'เสิงสาง',
    'สุขไพบูลย์วิริยะวิทยา',
    'ปักธงชัยประชานิรมิต',
    'สมเด็จพระธีรญาณมุนี',
    'สะแกราชธวัชศึกษา',
    'ลำพระเพลิงพิทยาคม',
    'วังหมีพิทยาคม',
    'วังน้ำเขียวพิทยาคม',
    'บ้านกุดจิก(รัชชุศิริอนุกูล)',
    'บ้านสลักใด',
    'จตุรคามสามัคคี',
    'บ้านโค้งยาง',
    'บ้านกุดหิน',
    'บ้านนาตะโครก',
    'บ้านเมืองเก่า',
    'บ้านห้วยตะคร้อ',
    'บ้านนากลาง',
    'บ้านนาใหญ่(เสถียรราษฎร์รัฐผดุง)',
    'บ้านหนองบอน',
    'โนนค่าวิทยา',
    'บ้านกุดปลาเข็งเหมืองลี่',
    'บ้านกุดหัวช้าง',
    'บ้านโคกหินเหล็กไฟ',
    'บ้านโคกมะกอก(ก.ร.ป.กลางอุปถัมภ์)',
    'วัดกุดเวียน',
    'สันติวิทยา',
    'บ้านห้วยไผ่',
    'บ้านบุตาสง',
    'บ้านวังราง',
    'บ้านวังรางน้อย',
    'บ้านหนองม่วง',
    'บ้านหนองไม้ตาย',
    'บ้านหนองเลา',
    'บ้านมะเกลือเก่า',
    'บ้านไร่โคกสูง',
    'บ้านหนองเบน',
    'ชินวงศ์อุปถัมภ์',
    'บ้านโคกสว่าง',
    'บ้านทุ่งสะแบง',
    'บ้านมะเกลือใหม่',
    'บ้านใหม่สันติ',
    'กิริวัฒนศักดิ์',
    'บ้านบุใหญ่',
    'บ้านสระเพลง',
    'ประชารัฐสามัคคี',
    'ระดมวิทยานุสรณ์',
    'แหลมทองวิทยานุสรณ์',
    'เสมาวิทยา',
    'บ้านส้มกบงาม',
    'บ้านโสกแจ้ง',
    'บ้านแก่นท้าว(อโศกสงเคราะห์)',
    'บ้านทะเล(เสมาประชาสรรค์)',
    'บ้านน้อยกุดคล้า',
    'บ้านโป่ง',
    'บ้านหนองตะไก้',
    'บ้านหนองพยอม',
    'บ้านหนองแวง',
    'บ้านกุดขมิ้น',
    'บ้านสองคร',
    'บ้านปางละกอ',
    'บ้านวังกระสวย',
    'บ้านวังโรงน้อย',
    'บ้านซับใต้',
    'บ้านโนนรัง',
    'บ้านกุดน้อย',
    'บ้านโนนเสลา',
    'บ้านวังกรวด',
    'บ้านหนองสลักได',
    'บ้านใหม่(ประชาอุปถัมภ์)',
    'พรพิทยาคม',
    'เจ.ซี.โคราชอนุสรณ์ 1',
    'นิยมมิตรวิทยาคาร',
    'บ้านซับศรีจันทร์',
    'บ้านมะค่างาม',
    'เที่ยงธรรมวิทยา',
    'บ้านปางโก',
    'ประชาราษฎร์สามัคคี',
    'รามวิทยาประชานุสรณ์',
    'บ้านดอนเมือง',
    'นาหว้าไตรประชาคม',
    'หนองแวงพิทยาคม',
    'บ้านหนองโอง',
    'บ้านหัน(วุฒิวิทยาคาร)',
    'คีรีวัฒนา',
    'บ้านกุดชะนวน"รัฐราษฎร์สวัสดิ์อุทิศ"',
    'บ้านโนนกุ่มมิตรภาพที่210',
    'บ้านหนองขาม',
    'เจียรวนนท์อุทิศ 1',
    'ถนนมิตรภาพ',
    'บ้านคลองตะแบก',
    'บ้านซับตะเคียน',
    'บ้านเลิศสวัสดิ์(ราษฎร์รังสรรค์วิทยา)',
    'บ้านลาดบัวขาว',
    'บ้านหนองบัว',
    'บ้านใหม่สำโรง',
    'หนองน้ำขุ่น',
    'บ้านห้วยลุง',
    'บ้านโนนประดู่',
    'บ้านวังราง',
    'บ้านวังโรงใหญ่สามัคคี',
    'บ้านหนองกระทุ่ม',
    'สันติพัฒนา',
    'บ้านถนนคด(ศรีวิทยาคาร)',
    'บ้านนาดีศรีสะอาด',
    'ไทยวัฒนาประชารัฐ',
    'คลองแจ้งวิทยา',
    'คลองไทรวิทยาสิทธิ์',
    'ทับ6วิทยาคาร',
    'สง่าพัฒนา',
    'หนองน้ำใส',
    'บ้านหนองกก',
    'ห้วยทรายวิทยาคม',
    'บ้านห้วยตะแคงใต้',
    'บ้านห้วยตะแคง',
    'บ้านหนองบัวน้อย',
    'บ้านหนองไม้ตาย',
    'โนนกราดสามัคคี',
    'บ้านซับชุมพล',
    'บ้านโนนสง่า',
    'บ้านหนองไผ่',
    'บ้านหนองหญ้าขาว',
    'บ้านหนองห่าน(ประชาสามัคคี)',
    'บุญบันดาลวิทยานุสรณ์',
    'เบทาโกรวิทยา',
    'วัดกลางดง',
    'วัดมิตรภาพวนาราม',
    'นิคมสร้างตนเองลำตะคอง 5',
    'บ้านหนองตะกู',
    'ขนงพระใต้',
    'บ้านเขาจันทน์หอม',
    'วัดขนงพระเหนือ',
    'บ้านซับเศรษฐี',
    'นิคมสร้างตนเองลำตะคอง 4',
    'บ้านหนองหมาก',
    'วันอาสาพัฒนา 2520',
    'บ้านโป่งวัวแดง',
    'บ้านวังสีสด',
    'บ้านหนองไข่น้ำ(ลำพญากลาง)',
    'บ้านหนองตอ',
    'เสนารัฐวิทยาคาร',
    'บ้านป่าไผ่',
    'บ้านสอยดาว',
    'บ้านสายชนวน',
    'บ้านหนองกะทุ่ม',
    'บ้านปากช่อง(คุรุสามัคคี 1)',
    'บ้านปางแก(สภาประชานุกูล)',
    'บ้านลำทองหลาง',
    'บ้านหนองกะจะ(ธงนิมลอุปถัมภ์)',
    'บ้านหนองตาแก้ว',
    'บ้านหนองมะค่า',
    'บ้านหนองอีเหลอ',
    'เติมแสงไขปากช่องวิทยา',
    'บ้านซับน้ำเย็น',
    'วัดบันไดม้า',
    'บ้านโป่งตาลอง',
    'บ้านหนองคุ้มวิทยา',
    'บ้านหนองจอก',
    'บ้านหนองซ่อมตะเคียนงาม',
    'สระน้ำใสวิทยา',
    'ผ่านศึกสงเคราะห์',
    'บ้านซับใต้',
    'บ้านโนนกระโดน',
    'บ้านโศกน้อย',
    'บ้านหัวป้าง',
    'บ้านเขาแก้ว',
    'บ้านคลองมะค่าหิน',
    'บ้านวังกะทะ',
    'บ้านหนองขวางวิทยา',
    'ประชารัฐวิทยา',
    'บ้านป่ากล้วย',
    'บ้านกุดโง้ง',
    'บ้านซับน้อย',
    'บ้านวังไทร',
    'นิคมสร้างตนเองลำตะคอง 2',
    'บ้านหินดาด',
    'บ้านหินเพิง',
    'บ้านเขาวง',
    'บ้านโป่งกะสัง',
    'บ้านนา(ประสิทธิ์วิทยาคาร)',
    'วัดวชิราลงกรณวราราม',
    'ไตรรัตน์วิทยาคาร',
    'บ้านคลองยาง',
    'บ้านซับหวาย',
    'บ้านท่ามะนาว',
    'บ้านท่าเลื่อนสามัคคี',
    'บ้านหนองสาหร่าย',
    'นิคมชลประทานสงเคราะห์',
    'นิคมสร้างตนเองลำตะคอง 1',
    'บ้านบ่อทอง',
    'บ้านคลองเดื่อ',
    'บ้านคลองเสือ',
    'บ้านท่าช้าง',
    'บ้านปรางคล้า',
    'บ้านหมูสี',
    'บ้านคลองดินดำ',
    'บ้านบุ่งเตย',
    'สูงเนิน',
    'กุดจิกวิทยา',
    'สองครพิทยาคม',
    'วังรางพิทยาคม',
    'สีคิ้ว"สวัสดิ์ผดุงวิทยา"',
    'คลองไผ่วิทยา',
    'ห้วยลึกผดุงวิทยา',
    'สีคิ้วหนองหญ้าขาว',
    'หนองน้ำใสพิทยาคม',
    'สีคิ้ววิทยาคาร',
    'กฤษณาวิทยา',
    'ปากช่อง',
    'พญาเย็นวิทยา',
    'กลางดงปุณณวิทยา',
    'ปากช่อง 2',
    'เขาใหญ่พิทยาคม',
    'ปากช่องพิทยาคม',
    'บ้านถนนหักใหญ่',
    'บ้านไร่(ทองคูณคุรุราษฎร์สามัคคี)',
    'บ้านสำนักพิมาน',
    'บ้านหนองกระเทียมเหนือ',
    'วัดกุดพิมาน',
    'บ้านหาญ',
    'วัดถนนหักน้อยดอนกลอย',
    'บ้านใหม่เจริญสุข',
    'บ้านจั่นโคกรักษ์',
    'ด่านขุนทด',
    'บ้านละเลิงพิมาน',
    'วัดบ้านด่านนอก(เสรีธรรมวิทยา)',
    'บ้านพระมะขามสามัคคี',
    'บ้านหนองบง',
    'บ้านดอนด่านใน',
    'ชุมชนบ้านกุดม่วง',
    'ไชยณรงค์วิทยา',
    'บ้านโกรกลึก',
    'บ้านหนองละมั่ง',
    'บ้านหินลาด',
    'บ้านหินหล่อง',
    'บ้านตะเคียน',
    'บ้านคลองแค',
    'บ้านโนนเต็ง',
    'บ้านโนนเมือง',
    'บ้านกุดโดก',
    'บ้านเก่า-บ้านน้อย',
    'บ้านโนนสะเดา',
    'บ้านมะเริง',
    'บ้านวังโป่ง',
    'บ้านสระพัง',
    'วัดบ้านกลาง',
    'บ้านประดู่งาม',
    'บ้านแปรง',
    'บ้านฝายโบสถ์',
    'บ้านหนองปรือ',
    'บ้านหนองพลวง',
    'บ้านดอน',
    'บ้านโนนสง่า',
    'บ้านโป่งกระสัง',
    'บ้านพันชนะ',
    'บ้านมาบกราด',
    'บ้านหลุง',
    'วัดสระจรเข้',
    'บ้านหนองสาร',
    'บ้านเกาะลอย',
    'บ้านดอนใหญ่หนองโสน',
    'บ้านวังสนวน',
    'บ้านหนองขุยคูเมือง',
    'บ้านหนองหัน',
    'บ้านหนองแหน',
    'บ้านใหม่ชัยพฤกษ์',
    'บ้านใหม่ศรีสุข',
    'วัดหนองกราด(สังวรอุปถัมภ์)',
    'บ้านหนองไทร',
    'บ้านหนองแดง',
    'บ้านหนองสะแก',
    'บ้านโคกสามัคคี',
    'บ้านหนองกระเทียมใต้',
    'วัดกุดน้ำใส',
    'บ้านจะบู',
    'บ้านโนนระเวียง',
    'บ้านเมืองตะโก',
    'บ้านหนองบัวตะเกียด',
    'บ้านห้วย',
    'บ้านสระขี้ตุ่น',
    'บ้านดอนมุกมัน',
    'บ้านโค้งตะคร้อ',
    'บ้านหนองบัวละคร',
    'บ้านเขื่อน(ท่าพระประชาสรรค์)',
    'บ้านหินเพิง',
    'บ้านหนองใหญ่',
    'บ้านห้วยบง',
    'บ้านซับพลู',
    'บ้านซับยาง',
    'บ้านป่ารังงาม',
    'บ้านศิลาร่วมสามัคคี',
    'บ้านหนองกราดน้อย',
    'บ้านน้อยพัฒนา',
    'บ้านปราสาท',
    'บ้านห้วยจรเข้',
    'บ้านกุดนางทอหูก',
    'บ้านค่ายทะยิง',
    'บ้านทุ่งสว่าง',
    'บ้านหินดาด',
    'บ้านท่าขี้เหล็ก',
    'บัวปุ่นสันเทียะอนุสรณ์',
    'บ้านกำปัง',
    'บ้านจาน',
    'บ้านนา',
    'บ้านอ้อไพล',
    'ศรีชลสินธุ์',
    'บ้านกะพี้',
    'บ้านค้างพลู',
    'วัดหนองราง',
    'บ้านซินหนองเขวา',
    'บ้านโนนพุทรา',
    'ชุมชนบ้านด่านจาก',
    'บ้านตะคร้อ',
    'บ้านหอกลองกระสัง',
    'บ้านถนนโพธิ์',
    'บ้านดอนสระจันทร์',
    'โนนไทย',
    'บ้านด่านกรงกราง',
    'บ้านโนนหวาย',
    'วัดสระจรเข้',
    'ศิริบ้านไร่',
    'บ้านใหม่โคกพรม',
    'บ้านกระดาน(รัฐประชาสรรค์)',
    'บ้านบัลลังก์สะอาด',
    'บ้านโพธิ์ตาสี',
    'บ้านเมืองเก่า',
    'บ้านสระตะเฆ่',
    'บ้านหนองแจง',
    'บึงคำคู',
    'เทพคงคา',
    'บ้านหนองกลางดอน',
    'บุ่งสะอาดวัฒนา',
    'บ้านวัง(สายอนุกูล)',
    'ดอนน้ำใสวิทยา',
    'บ้านหนองดุม',
    'บ้านมะเกลือโกรกกระสัง',
    'บ้านมะค่าดอนทะยิง',
    'ชุมชนบ้านโคกสวาย',
    'บ้านโคกหนองไผ่',
    'บ้านบุสายออ(บุญเขตบริหาร)',
    'บ้านสระตอง',
    'บ้านหนองกราด',
    'บ้านทรงธรรม',
    'บ้านนารายณ์',
    'บ้านสำโรง',
    'บ้านหนองประดู่',
    'อนุบาลขามสะแกแสง',
    'บ้านคู(ประชาอุปถัมภ์)',
    'บ้านหญ้าคาโนนแจง',
    'บ้านหนองจาน',
    'บ้านหนุก',
    'บ้านชีวึก(สังฆประชาสรรค์)',
    'บ้านโนนผักชี',
    'บึงพัฒนาประชาร่วมจิต',
    'บ้านหนองกระทุ่มสระอโนดาต',
    'บ้านหนองโพธิ์',
    'บ้านครึมม่วง',
    'บ้านงิ้ว',
    'บ้านโนนเมือง(เยี่ยมประชาสรรค์)',
    'บ้านสระแจง',
    'บ้านดอนพะงาด(รัฐราษฎร์วิทยา)',
    'ชุมชนบ้านหนองไข่น้ำ',
    'บ้านหนองไผ่',
    'บ้านบุตะโก',
    'บ้านหนองโบสถ์',
    'บ้านคูเมือง',
    'บ้านเมืองนาท',
    'บ้านเสมา',
    'บ้านหนองโพธิ์นามาบ',
    'บ้านห้วยโนนพฤกษ์',
    'บ้านเหนือทัพรั้ง',
    'บ้านโคกคูขาด',
    'บ้านโนนมะเกลือ',
    'ชุมชนหนองหัวฟาน',
    'บ้านโจด(ท้าวสุรนารีอนุสรณ์4)',
    'บ้านหนองปรือ',
    'บ้านขามทะเลสอ',
    'บ้านโคกแฝก',
    'รัฐราษฎร์รังสรรค์',
    'บ้านโนนตาล',
    'บ้านบึงอ้อ(ปุ่นอุปถัมภ์)',
    'บ้านบุระไหว',
    'บ้านกุ่มพะยาหนองกระทุ่มวิทยา',
    'บ้านหนองกระโดนดอนมะเกลือสามัคคี',
    'บ้านโป่งแดงน้ำฉ่าสามัคคี',
    'บ้านโนน',
    'บ้านโกรกกระหาด',
    'บ้านโคกแขวน',
    'บ้านโตนดพูนผลวิทยา',
    'บ้านพันดุง',
    'บ้านหนองสะแก',
    'บ้านหนองสรวง',
    'บ้านหนองกก',
    'บ้านหนองตะครอง',
    'บ้านบึงปรือ',
    'บ้านห้วยน้ำเค็ม',
    'บ้านคลองขามป้อม',
    'บ้านกุดสระแก้ว',
    'บ้านวังยายทอง',
    'บ้านทำนบเทวดา',
    'บ้านสำนักตะคร้อ',
    'บ้านหนองโพธิ์',
    'บ้านหนองบัวหลวง',
    'บ้านโคกกระบือ',
    'บ้านโนนทอง',
    'บ้านวังสมบูรณ์',
    'บ้านหนองแวง',
    'บ้านหิงห้อย',
    'บ้านใหม่เจริญผล',
    'บ้านสะพานหิน',
    'กุดตาดำ(คงฤทธิ์คุรุราษฎร์สามัคคี)',
    'บ้านกุดไผ่',
    'บ้านดอนขวาง',
    'บ้านตะเคียนงาม',
    'บ้านโนนงิ้ว',
    'บ้านหนองยาง(อนุศาสนานุสรณ์)',
    'บ้านห้วยหุงเกลือ',
    'บ้านห้วยสามขา',
    'บ้านป่าเพกา',
    'บ้านหัวถนน',
    'พังเทียม(คุรุสามัคคี 2)',
    'บ้านโกรกช้างน้อย',
    'บ้านหนองกกตลุกผักไร',
    'บ้านทองหลาง',
    'บ้านโพนไพล',
    'ชายพะเนาหนองโพธิ์',
    'บ้านกุดจิกหนองแห้ว',
    'บ้านมาบกราด',
    'บ้านเจริญผล',
    'ถนนสุรนารายณ์(คุรุรัฐประชาสรรค์)',
    'สระพระขมาดไพร',
    'บ้านตลุกชั่งโค',
    'บ้านถนนหัก',
    'บ้านหนองกก',
    'บ้านหนองหอย',
    'บ้านบุเขว้า',
    'บ้านทำนบพัฒนา',
    'มัธยมด่านขุนทด',
    'หนองกราดวัฒนา',
    'ปราสาทวิทยาคม',
    'มัธยมหลวงพ่อคูณ ปริสุทโธ',
    'หนองบัวละครวิทยา',
    'วังโป่งพิทยาคม',
    'มัธยมประดู่วัฒนา',
    'โนนไทยคุรุอุปถัมป์',
    'โนนไทยคุรุอุปถัมป์ 2',
    'ขามสะแกแสง',
    'ขามทะเลสอวิทยา',
    'เทพารักษ์ราชวิทยาคม',
    'มัธยมบึงปรือ',
    'พระทองคำวิทยา',
    'ทัพรั้งพิทยาคม',
    'บุญเหลือวิทยานุสรณ์ 2',
    'บ้านขาม(สำนักงานสลากกินแบ่งสมทบ)',
    'บ้านมะค่า',
    'บ้านตะหนอด',
    'ปริยัติไพศาล',
    'บ้านโคกเสี่ยว',
    'บ้านน้อยหนองหนาด',
    'บ้านปอบิด',
    'บ้านเมืองคง',
    'บ้านหนองหว้า',
    'วัดกู่สามัคคี',
    'บ้านไทรโยง',
    'บ้านสี่เหลี่ยม',
    'บ้านดอนใหญ่',
    'บ้านศรีนิมิต',
    'บ้านหนองแขม',
    'บ้านโคกตะพาบ',
    'บ้านตะโกโคก',
    'บ้านตะโกทุ่ง(คุรุราษฎร์รังสรรค์)',
    'บ้านตาจั่น',
    'บ้านตาชู',
    'บ้านหนองบัวทุ่ง',
    'ชุมชนบ้านวัด',
    'บ้านกระถิน',
    'บ้านคอนเมือง',
    'บ้านตะคร้อ(มิตรภาพ4)',
    'บ้านหนองบัวสามัคคี',
    'บ้านหนองพรานปาน',
    'บ้านทองหลาง',
    'บ้านหนองบัวกระจาย',
    'บ้านตลุกสาหร่าย',
    'บ้านโนนเต็ง',
    'วัดบ้านหนองกก',
    'บ้านประคำ',
    'บ้านปรางค์',
    'บ้านหนองสำโรง',
    'บ้านดอนถั่วแปบพัฒนา',
    'บ้านดอนทะแยง',
    'บ้านตะโก',
    'บ้านทัพมะขาม',
    'ชุมชนคงวิทยา',
    'บ้านโคกเพ็ด',
    'บ้านตลุกหินปูน',
    'วันครู 2502',
    'บ้านหนองบง',
    'บ้านตากิ่ม',
    'บ้านหนองสะแก',
    'บ้านหนองหญ้าขาว',
    'วนาประชารัฐบำรุง',
    'ห้วยไหวัฒนา',
    'บ้านหนองบัว',
    'บ้านห้วยทราย',
    'บ้านเก่าค้อ',
    'บ้านโจด',
    'บ้านดอนกลาง',
    'บ้านหนองม่วง',
    'บ้านหมัน',
    'บ้านโคกกระเบี้องสามัคคี',
    'บ้านดอนตะหนิน',
    'บ้านตลุกพลวง',
    'บ้านสระสี่เหลี่ยม',
    'บ้านหนองโจด',
    'บ้านหนองปรือโป่ง',
    'บ้านหนองม่วงช่างพิม',
    'บ้านโนนเพ็ด',
    'ชุมชนบ้านช่อระกา',
    'บ้านขามเวียน',
    'ชุมชนบ้านเหลื่อม',
    'บ้านโต้นสามัคคีวิทยา',
    'บ้านดอนเปล้า',
    'บ้านนาคุรุราษฎร์วัฒนา',
    'บ้านโนนทองหลาง',
    'บ้านวังโพธิ์',
    'บ้านโคกเพ็ด',
    'วัดบ้านงิ้ว',
    'บ้านดอนแร้ง',
    'บ้านอ้อยช้าง',
    'บ้านหนองหว้าเอน',
    'ไทยรัฐวิทยา18',
    'บ้านโคกสว่าง',
    'บ้านโคกสี',
    'บ้านดอนคนทา',
    'บ้านโสกงูเหลือม',
    'บ้านหญ้าคาประชาเนรมิตร',
    'บ้านกู่',
    'บ้านโคกน้อย',
    'บ้านทองหลางน้อย',
    'วัดบ้านกระเบื้อง',
    'บ้านขามหนองแวง',
    'บ้านโคกสะอาด(ประชาราษฎณ์รังสรรค์)',
    'บ้านดอนหันโนนเพ็ด',
    'บ้านตะคร้อโนนทอง',
    'บ้านโนนทองหลาง(ตุ๊ประสิทธิ์ประชาสรรค์)',
    'บ้านหนองนาโคกไผ่ล้อมสามัคคี',
    'วัดบ้านดอนชุมช้าง',
    'บ้านโคกสะอาดสามัคคี',
    'บ้านหันดอนดู่(วิจิตรอุปถัมภ์)',
    'วัดบ้านไร่',
    'วัดบ้านหนองแวง',
    'บ้านโนนกระพี้วิทยา',
    'บ้านหนองเม็ก(สถาพรประชานุเคราะห์)',
    'บ้านดอนฆ่าเสือ',
    'บ้านป่าตองท่าเนินสามัคคี',
    'วัดบ้านหนองตะไก้',
    'บ้านหนองปรือคึมม่วง',
    'บ้านห้วยโจด',
    'วัดบ้านหนองหัวช้าง',
    'บ้านเสมา',
    'บ้านหนองแจ้งน้อย',
    'บ้านนาดอนบก',
    'บ้านบุเสมาทอง',
    'บ้านหญ้าคาสามัคคี',
    'วัดบ้านหนองไข่ผำ',
    'บ้านหนองแจ้งใหญ่',
    'บ้านโนนมะเฟือง',
    'บ้านหนองกระทุ่ม',
    'บ้านหนองพลวง',
    'บ้านกระพี้',
    'บ้านคูขาด',
    'บ้านสระครก',
    'บ้านห้วยยาง',
    'ขามเตี้ยพิทยาคม',
    'บ้านดงบัง',
    'บ้านด่านช้าง',
    'วัดบ้านหนองหญ้าปล้อง',
    'วัดบ้านเก่างิ้ว',
    'บ้านอุทัยทอง',
    'บ้านหนองขามนาดี',
    'บ้านหนองบง',
    'บ้านโนนรัง',
    'บ้านหนองสะเดา',
    'บ้านโนนสำราญ',
    'บ้านหินลาดหนองโดน',
    'บ้านนาแค',
    'บ้านโสกน้ำขุ่น',
    'บ้านกอก',
    'บ้านแก้งสนามนาง',
    'บ้านโนนระเวียง',
    'บ้านหนองโคบาล',
    'บ้านหนองเต่า',
    'บ้านศาลาหนองขอน',
    'บ้านหนองขามน้อย',
    'บ้านมูกมันโนนอุดมสามัคคี',
    'บ้านหนองบัว(ชาลีราษฎร์สามัคคี)',
    'บ้านกุดปลาฉลาดหนองปรือ',
    'บ้านหัวหนองฯ',
    'บ้านสีสุก',
    'บ้านโสกสนวน',
    'บ้านดอนไผ่',
    'บ้านโนนสูง',
    'บ้านโนนดู่',
    'บ้านหนองคอม',
    'บ้านหลุบกุง',
    'บ้านห้วยม่วง',
    'วัดบ้านศาลาดิน',
    'ชุมชนบ้านหนองบัวลาย',
    'บ้านขามป้อม(สังวาลย์อนุสรณ์)',
    'วังโพน',
    'บ้านป่าหวาย',
    'บ้านหนองแสง',
    'บ้านแดงน้อย',
    'บ้านฝาผนัง',
    'บ้านเพ็ดน้อย',
    'บ้านหนองจาน',
    'บ้านหนองผือ',
    'บ้านคึมมะอุ',
    'ชุมชนหนองแวงสุวิทย์',
    'วัดบ้านโนนประดู่',
    'บ้านเสว(ราษฎร์สามัคคี)',
    'เบญจราษฎร์ศึกษา',
    'วัดบ้านมะค่า',
    'บ้านโพนทอง(สุวรรณราษณ์รังสรรค์)',
    'บ้านโนนสัง',
    'วัดบ้านโนนกอก',
    'บ้านแฝก',
    'วัดบ้านสามเมือง',
    'วัดบ้านสีดา',
    'วัดบ้านดอนโก่ย',
    'บ้านหญ้าคา',
    'บ้านหินแห่',
    'วัดบ้านดอนเต็ง',
    'วัดบ้านเมืองสูง',
    'บ้านหนองตาด',
    'บ้านหัวนาคำ',
    'เมืองคง',
    'ปรางค์ทองวิทยา',
    'เทพาลัย',
    'หนองบัวพิทยาคม',
    'บ้านเหลื่อมพิทยาสรรพ์',
    'บัวใหญ่',
    'วัดประชานิมิตร',
    'เกล็ดลิ้นวิทยา',
    'บึงพะไล',
    'แก้งสนามนางพิทยาคม',
    'บัวลาย',
    'หนองหว้าพิทยาสรรค์',
    'สีดาวิทยา',
    'บ้านกระทุ่มราย',
    'บ้านเขว้าวิทยา',
    'บ้านหนองแวง',
    'วัดบ้านดอนตะหนิน',
    'วัดบ้านสี่เหลี่ยม',
    'วัดบ้านหนองเดิ่น',
    'หนองกรดวิทยา',
    'บ้านละเลิงหิน',
    'บ้านหนองแสง',
    'ดอนมัน',
    'ช่องแมวพิทยา',
    'ดอนใหญ่พัฒนา',
    'วัดปลักแรต',
    'สระประดู่',
    'บ้านหนองกก',
    'ดอนอีลุ่ม(ประชารัฐพัฒนา)',
    'บ้านตลาดไทร',
    'บ้านประทาย',
    'วัดบ้านขี้เหล็ก',
    'วัดบ้านทุ่งสว่าง',
    'บ้านโคกล่ามขี้เหล็ก',
    'บ้านฝาผนัง',
    'บ้านวังม่วง',
    'บ้านหญ้าคา',
    'วัดบ้านหนองสะแบง',
    'บ้านโนนสูง',
    'บ้านนางรำ',
    'บ้านหนองห่าง',
    'บ้านหนองบัวนาค',
    'บ้านหนองคอกควาย(คุรุประชานุสรณ์)',
    'รวมมิตรพัฒนา',
    'โนนเพ็ดวิทยา',
    'ชุมชนประทาย',
    'บ้านเตยกระโตน',
    'บ้านหนองโน(คุรุราษฎร์พัฒนา)',
    'บ้านหันเตย',
    'วัดบ้านถนนโพธิ์',
    'วัดบ้านนางิ้ว',
    'บ้านดอนวัว',
    'บ้านดอนสามัคคี',
    'เมืองโดนสำโรง(รัฐประชานุกูล)',
    'วัดบ้านหนองคู',
    'หนองม่วงหนองแวง',
    'บ้านคึมหญ้านาง',
    'บ้านโนนภิบาลโคกกลาง',
    'บ้านหนองพลวงน้อย',
    'วัดบ้านโคกพระ',
    'วัดบ้านโคกสีโนนรัง',
    'วัดบ้านอุ่มจานโนนงิ้ว',
    'บ้านทุ่งมนวิทยาคาร',
    'ไตรราษฎร์สามัคคี',
    'วัดบ้านโคกเพ็ด',
    'วัดบ้านตำแย',
    'บ้านหนองพลวง',
    'วัดบ้านหนองขาม',
    'วัดบ้านหนองคึม',
    'หนองกุงวันชาติ',
    'ดอนกอกเกตสมบูรณ์สามัคคี',
    'คมเพชรวิทยาคาร',
    'บ้านหันห้วยทราย(คุรุสังฆฯ)',
    'สัมพันธมิตรวิทยา',
    'สี่แยกศรีอรัญวิทยา',
    'อัมพวันวิทยา',
    'บ้านกระเบื้อง',
    'บ้านตลาดประดู่',
    'บ้านโนนตะแบก',
    'บ้านโนนพุทรา',
    'ราษฎร์สโมสร',
    'บ้านกระเบื้องใหญ่',
    'บ้านเตยประชาบำรุง',
    'บ้านทับควาย',
    'บ้านชีวาน',
    'บ้านซาด',
    'บ้านดอนเขว้า',
    'บ้านขามใต้',
    'บ้านดงประชานุกูล',
    'บ้านโนนโชงโลง',
    'บ้านมะกอก',
    'บ้านศาลา',
    'บ้านกล้วย',
    'บ้านโนนม่วง',
    'บ้านท่าหลวง',
    'บ้านถนน',
    'จารย์ตำรา',
    'บ้านหวายโนนโพธิ์',
    'มะค่าสามัคคี',
    'นิคมสร้างตนเองพิมาย 1',
    'นิคมสร้างตนเองพิมาย 3',
    'นิคมสร้างตนเองพิมาย 6',
    'บ้านโนนกระเบื้องหนองหัวช้างสามัคคี',
    'บ้านโพธิ์งามหนองหญ้าขาววิทยา',
    'บ้านสะแกงาม',
    'บ้านใหม่ฉมวก',
    'นิคมสร้างตนเองพิมาย 2',
    'กุลโน',
    'ท้าวสุรนารี(ระดมอนุสรณ์)',
    'บ้านม่วงขามประชารักษ์',
    'อนุบาลสุริยาอุทัยพิมาย',
    'ชุมชนบ้านวังหิน',
    'บ้านดอนแซะ',
    'บ้านหนองปรือ(รัฐราษฎร์พัฒนา)',
    'บ้านลุงตามัน(สาขาโนนไม้แดง)',
    'ชุมชนบ้านหนองจิก',
    'บ้านโคกขาม(นาคพัฒนา)',
    'บ้านตะคร้อ',
    'บ้านตะบอง"เจริญราษฎร์อุทิศ"',
    'บ้านลุงตามัน',
    'บ้านหนองขาม',
    'บ้านฉกาจช่องโค',
    'บ้านพุทรา',
    'พิมายสามัคคี 1',
    'นิคมสร้างตนเองพิมาย 5',
    'วัดวังน้ำ',
    'บ้านคล้า',
    'บ้านซึม(ศีลราษฎร์นุเคราะห์)',
    'บ้านท่าแดง',
    'บ้านสัมฤทธิ์',
    'นิคมสร้างตนเองพิมาย 4',
    'หนองบัวลอย',
    'บ้านดอนหวาย',
    'บ้านหนองบัวคำ',
    'บ้านหนองโสน',
    'เพชรหนองขาม',
    'พิกุลทอง',
    'หนองบัวรีราษฎร์อุทิศ',
    'บ้านตลาดไทร',
    'สองห้องแวงน้อยวิทยา',
    'บ้านหนองบัว',
    'บ้านหนองแวงสามัคคี',
    'วัดเจริญราษฎร์บำรุง',
    'วัดโพธิ์ศรีบรรจง',
    'บ้านหนองหว้าประชาสามัคคี',
    'บ้านหนองตาด',
    'ท่าลาด',
    'บ้านพะงาดวิทยา',
    'บ้านลุงประดู่',
    'บ้านหนองตะคลอง',
    'บ้านหนองนกคู่',
    'หนองปรือแก้ว',
    'ไทยรัฐวิทยา 82 (โคกตองเจริญ)',
    'ห้วยบงวิทยา',
    'บ้านโคกหินช้าง',
    'โนนตูม"รัฐราษฎร์รังสรรค์"',
    'บ้านสว่างนาดี',
    'บ้านสาหร่ายวิทยา',
    'บ้านหนองโดนรกฟ้า',
    'บ้านหนองหว้าตาดำ',
    'วัดสองพี่น้อง',
    'หนองปรือแดง(ศิริราษฎร์ปการ)',
    'บ้านโนนยอ',
    'บ้านยางวิทยา',
    'อนุบาลชุมพวงวิทยา',
    'บ้านโคกเพชร',
    'บ้านดงบัง(สว่างวิทยา)',
    'บ้านโนนรัง',
    'บ้านหนองข่าวิทยานุสรณ์',
    'บ้านหนองทุ่มโนนหาดวิทยา',
    'บ้านใหม่พัฒนา',
    'บ้านใหม่ปฎิรูป',
    'บ้านละโว้',
    'บ้านขามขุนร่ม"ประชาสามัคคี"',
    'บ้านดอนลำดวน',
    'บ้านประสุข',
    'วัดโชติการาม',
    'อาสนาราม(เปล่งอุปถัมภ์)',
    'ประโดกสามัคคี',
    'บ้านเมืองไผ่',
    'บ้านหนองหลักสามัคคี',
    'โนนขุยสามัคคี',
    'บ้านโกรกสำโรง',
    'ดอนยาว',
    'บ้านนาดี',
    'บ้านป่าตะแบง',
    'ชุมชนโนนแดง',
    'บ้านจาบ',
    'บ้านดอนตัดเรือ',
    'บ้านหนองบง',
    'วัดบ้านตะเภาหนุน',
    'บ้านหนองม่วง',
    'วัดบ้านโคกหนองแวง',
    'บ้านโนนตาเถร',
    'วัดบ้านทุ่งรี',
    'วัดบ้านหินตั้ง',
    'วังหินประชาสรรค์',
    'วัดบ้านแก',
    'บ้านฝาง',
    'บ้านหนองหญ้าขาว',
    'บ้านสำพะเนียง',
    'คูเตยราษฎร์สามัคคี',
    'บ้านกระเบื้องวิทยาคาร',
    'ท่าเรือวิทยา',
    'บ้านท่าเยี่ยมวิทยายล',
    'จตุคามพิทยาคม',
    'บ้านโนนอุดมนภดลอุปถัมภ์',
    'บ้านโนนปีบ',
    'เมืองจากวิทยา',
    'บ้านโนนตำหนัก',
    'อรพิมวิทยา',
    'บ้านโนนไม้งาม',
    'เมืองยาง"คุรุราษฎร์อุทิศ"',
    'บ้านนางโทพัฒนา',
    'บ้านโนนตาสุดวิทยาสรรค์',
    'บ้านโนนเพชรพิทยาคม',
    'บ้านโนนสมบูรณ์',
    'บ้านนา',
    'บ้านครบุรีศึกษา',
    'บ้านละหานสามัคคี',
    'ชุมชนลำทะเมนชัยศึกษา',
    'บ้านขุยวิทยา',
    'บ้านหนองโป่ง(ราษฎร์บำรุง)',
    'บ้านหนองไผ่พิทยาคม',
    'บ้านดงหลบ',
    'บ้านมาบป่าแดง',
    'บ้านสี่เหลี่ยม',
    'บ้านหนองจาน',
    'วัดจันทนาราม',
    'หนองมะเขือ',
    'บ้านหนองจานใต้',
    'บ้านช่องแมว',
    'บ้านโสกดู่',
    'บ้านหนองดู่',
    'บ้านหนองม่วงวิทยาคาร',
    'บ้านหนองยาง',
    'บ้านหินแร่หนองนกเป็ดวิทยา',
    'หนองอ้อวิทยาคม',
    'บ้านลำทะเมนชัย',
    'บ้านไพล',
    'บ้านหนองแวง',
    'บ้านอ้อประชาสามัคคี',
    'ประทาย',
    'วังไม้แดงพิทยาคม',
    'หันห้วยทรายพิทยาคม',
    'พิมายวิทยา',
    'พิมายดำรงวิทยาคม',
    'นิคมพิมายศึกษา',
    'ชุมพวงศึกษา',
    'สาหร่ายวิทยาคม',
    'ตลาดไทรพิทยาคม',
    'ภู่วิทยา',
    'มิตรภาพวิทยา',
    'กระเบื้องนอกพิทยาคม',
    'เมืองยางศึกษา',
    'ช่องแมววิทยาคม',
    'ลำทะเมนไชยพิทยาคม',
    'บ้านโพธิ์ดอนหวาย',
    'บ้านกระสัง(หรุ่นราษฎร์รังสรรค์)',
    'บ้านหนองม้า',
    'บ้านกลันทาวิทยา',
    'บ้านท้องเรือ',
    'บ้านพระครูน้อย',
    'บ้านหนองปรือ',
    'บ้านกลางเพชร (สุโขวิทยา)',
    'วัดบ้านเสม็ด',
    'ไตรภูมิวิทยา',
    'บ้านโกรกขี้หนู',
    'บ้านหนองตราดน้อย',
    'สามัคคีมีชัยวิทยา',
    'บ้านหนองไผ่ใหญ่',
    'บ้านตราดหนองพลวง',
    'บ้านสำโรงพรหมอนุสรณ์',
    'วัดบ้านถลุงเหล็ก',
    'บ้านโนนศิลา',
    'บ้านหนองผักแว่น',
    'บ้านพะไล',
    'เขตการทางสงเคราะห์5 (ไตรคามสิทธิศิลป์)',
    'อนุบาลบุรีรัมย์',
    'บ้านตะเคียน',
    'บ้านหนองเพชร',
    'วัดบ้านสวายสอ',
    'วัดบ้านรุน',
    'บ้านหนองค่าย',
    'บ้านดอนใหญ่',
    'บ้านปรุบุมะค่า',
    'บ้านโคกระกา',
    'บ้านม่วงวิทยาคาร',
    'อนุบาลเมืองบุรีรีมย์ (บ้านบัว)',
    'วัดโพธิ์ทอง',
    'บ้านหนองโสน',
    'บ้านมะค่า',
    'บ้านง้าง',
    'บ้านตะโก',
    'บ้านหนองมะค่าแต้',
    'บ้านหนองยาง',
    'บ้านสระเกษ',
    'บ้านหนองขวาง',
    'พระครูวิทยา',
    'บ้านม่วง',
    'วัดบ้านเมืองฝาง',
    'วัดบ้านเมืองดู่',
    'บ้านโคกกลาง',
    'วัดบ้านหนองตลุมปุ๊ก',
    'บ้านหนองทะลอก',
    'วัดสว่างบูรพา',
    'บ้านหนองไทร',
    'บ้านสวายจีก',
    'บ้านโคกเปราะ',
    'บ้านพลวง',
    'บ้านฝ้าย',
    'เบญจคามวิทยา',
    'บ้านหนองปรือน้อย',
    'บ้านถาวร',
    'บ้านโคกระกาน้อย',
    'บ้านหนองสองห้อง',
    'บ้านตลาดชัย',
    'บ้านกระเดื่อง',
    'บ้านร่มไทร',
    'บ้านสารภี',
    'ประชาสวัสดิ์วิทยา',
    'วัดบ้านสะแกซำ',
    'บ้านตลาดควาย',
    'บ้านฝังงา',
    'บ้านโนนแดง',
    'สะแกโพรงอนุสรณ์',
    'บ้านหนองมะเขือ',
    'บ้านสวายสอ (ประชาบูรณะ)',
    'บ้านสมสนุก',
    'บ้านหนองไผ่',
    'ชุมชนบ้านหัววัว (ราษฎร์อุทิศ)',
    'บ้านเขากระโดง(ศิลาทองอุปถัมภ์)',
    'บ้านเสม็ดโคกตาล',
    'โสภณวรวัฒน์วิทยา',
    'บ้านโคกหัวช้าง',
    'วัดบ้านเย้ยสะแก',
    'บ้านสำโรงโนนเค็ง',
    'บ้านม่วงโนนสูง',
    'บ้านนากลาง',
    'บ้านมาบสมอ',
    'วัดบ้านบัลลังก์',
    'บ้านหนองตราด',
    'วัดสถานีหนองตาด',
    'ไตรมิตรวิทยาคาร',
    'วิมลวิทยา',
    'บ้านหลักเขต',
    'บ้านโคกเก่า',
    'ประกาศธรรมคุณวิทยา',
    'บ้านโคกสะอาดวิทยาคาร',
    'คงชัยสิทธิ์วิทยา',
    'เสนศิริอนุสรณ์',
    'บ้านโคกวัด',
    'บ้านโคกกลางอนุสรณ์',
    'บ้านกวางงอย "รัฐราษฏร์อำนวยวิทย์"',
    'วัดโกรกประดู่',
    'บ้านปัญจคาม (เรียนติสสวงศ์วิทยา)',
    'วัดโคกล่าม "สิริประภาวิชาคาร"',
    'บ้านลุงม่วง',
    'บ้านสระคูณ "สิริทัศน์ประชาสรรค์"',
    'วัดโนนสำราญ',
    'เรืองทองสามัคคี',
    'บ้านหนองกุง',
    'บ้านว่านเขื่อนค้อวิทยา',
    'บ้านหนองเฒ่ากา',
    'บ้านหนองปลิง',
    'วัดขี้ตุ่น',
    'บ้านสนวน "ศิริมงคลวิทยา"',
    'วัดห้วยหวาย',
    'บ้านตลาดโพธิ์ "ศรีตลาดโพธิ์"',
    'วัดชัยมงคลปราสาทวิทยา',
    'บ้านหนองไฮ',
    'บ้านหนองหญ้าปล้อง',
    'บ้านหนองตาดน้อย',
    'บ้านหนองไทร',
    'บ้านหนองบัว',
    'บ้านหนองม่วง',
    'บ้านบริหารชนบท',
    'บ้านทะเมนชัย',
    'บ้านยาง "คุรุราษฏร์รังสรรค์"',
    'วัดหนองกระทุ่ม',
    'บ้านม่วงหนองบอนสามัคคี',
    'บ้านสระสี่เหลี่ยม',
    'บ้านซำแฮด (นักศึกษาประชานุสรณ์5)',
    'บ้านนาศรีนวล',
    'บ้านตูมหวาน',
    'วัดบ้านบุโพธิ์',
    'บ้านผไทรินทร์',
    'บ้านโคกสูง',
    'บ้านโคกซาด',
    'บ้านหนองหัวช้าง',
    'บ้านโคกสว่าง',
    'รวมมิตรวิทยา',
    'บ้านหนองแวง',
    'บ้านเมืองแฝก',
    'บ้านตูบช้าง',
    'วัดหนองเก้าข่า',
    'วัดหนองครก',
    'วัดหนองสองห้อง',
    'บ้านห้วยมะไฟ',
    'วัดหนองขุนปราบ',
    'อนุบาลลำปลายมาศ',
    'จันทราวาส (คุรุราษฎร์วิทยา)',
    'บ้านแสลงพัน',
    'บ้านสี่เหลี่ยมน้อย',
    'วัดบ้านบุขี้เหล็ก',
    'บ้านโคกใหม่หนองสรวง',
    'บ้านหนองระนาม',
    'บ้านหนองผักโพด',
    'จตุคามสามัคคีวิทยา',
    'วัดบ้านหนองกะทิง',
    'ไตรคามสามัคคีวิทยา',
    'วัดบ้านหนองปลาไหล',
    'บ้านหนองซอแซ',
    'บ้านหนองเมืองต่ำ',
    'บ้านแท่นพระ',
    'บ้านหนองมันปลา "กุศลสามัคคีวิทยา"',
    'บ้านบุตาวงษ์',
    'บ้านหนองโดนประสาทวิทย์',
    'ชุมชนบ้านหนองบัวโคก',
    'บ้านห้วยศาลา',
    'บ้านสี่เหลี่ยมใหญ่',
    'วัดบ้านตะขบ "โนนตะครองโคกใหม่วิทยา"',
    'วัดหัวสะพาน',
    'วัดบ้านหินโคน "คุรุราษฎร์บำรุง"',
    'วัดกะทิง',
    'วัดโคกสะอาด',
    'บ้านโคกสนวน',
    'บ้านช่อผกา',
    'บ้านบุหนองเทา',
    'บ้านสำโรงโคกเพชร',
    'บ้านหัวสะพาน',
    'บ้านโคกสะอาด',
    'อนุบาลชำนิ',
    'บ้านเมืองยาง (เอี่ยมโอภาสประชานุกูล)',
    'บ้านโคกขามโนนสมบูรณ์',
    'บ้านประคอง',
    'บ้านกระโดนกะลันทา',
    'วัดละลวด',
    'บ้านหนองพะอง',
    'บ้านหนองเพิก',
    'บ้านดอนหวาย',
    'วัดหนองปล่อง',
    'บ้านตาเหล็ง',
    'มหิดลอนุสรณ์ (สันติภาพพัฒนา)',
    'บ้านหนองแต้พัฒนา',
    'บ้านดงเย็น',
    'บ้านดงกะทิงมิตรภาพอนุสรณ์',
    'บ้านสำโรงสันติภาพ',
    'สรีพรรณสันติภาพนาฝาย',
    'บ้านโยนช้า (บุญอาจวิทยา)',
    'อนุบาลบ้านด่าน',
    'บ้านตะโคงสามัคคี',
    'โนนสวรรค์วิทยา',
    'วัดบ้านปราสาท',
    'บ้านบุมะขามป้อม',
    'วัดบ้านกะชาย',
    'บ้านตะโกตาเนตร',
    'วัดบ้านกะหาด',
    'วัดบ้านปลัดปุ๊ก',
    'บุรีรัมย์พิทยาคม',
    'ภัทรบพิตร',
    'บัวหลวงวิทยาคม',
    'พระครูพิทยาคม',
    'สองห้องพิทยาคม',
    'กนกศิลป์พิทยาคม',
    'สวายจีกพิทยาคม',
    'หนองตาดพิทยาคม',
    'กลันทาพิทยาคม',
    'ธารทองพิทยาคม',
    'ลำปลายมาศ',
    'ตลาดโพธิ์พิทยาคม',
    'จตุราษฏร์พิทยาคม',
    'เมืองแฝกพิทยาคม',
    'ทะเมนชัยพิทยาคม',
    'ชำนิพิทยาคม',
    'รมย์บุรีพิทยาคม รัชมังคลาภิเษก',
    'อนุบาลกระสัง',
    'วัดบ้านหนองแขม',
    'บ้านตะเคียน',
    'บ้านระโยงใหญ่',
    'บ้านขามตาเบ้า',
    'บ้านถนน',
    'บ้านหนองตระเสก',
    'วัดชุมพลมณีรัตน์',
    'วัดบ้านกันทรารมย์',
    'บ้านระกาเสม็ด',
    'บ้านหนองขอน',
    'บ้านดอนยาว',
    'บ้านโคกยาง',
    'วัดบ้านชุมแสง',
    'บ้านก้านเหลือง',
    'บ้านไม้แดง',
    'บ้านสวายสอ "ไกรปัญญานุเคราะห์"',
    'บ้านไผ่ลวก',
    'วัดปทุมคงคา',
    'บ้านกะนัง (ฤทธิ์ประชาสรรค์)',
    'บ้านตาสะดำ (สุรวุฒิอนุเคราะห์)',
    'บ้านสำโรง (ภิญโญอนุสรณ์)',
    'บ้านระกา',
    'วัดอินทบูรพา',
    'บ้านนารา"กัลยาประชาสรรค์"',
    'บ้านเมืองไผ่',
    'บ้านสวายสอ',
    'วัดธรรมถาวร',
    'บ้านอโณทัย',
    'บ้านลำดวน',
    'วัดบ้านหนองพลวง',
    'บ้านยาง',
    'บ้านแสลงพันกระเจา',
    'บ้านแซวประดู่',
    'บ้านโนนสว่าง',
    'บ้านขามสามัคคี',
    'บ้านศรีภูมิสามัคคี',
    'บ้านโคกสูง',
    'บ้านนาราใหญ่',
    'บ้านตาราม',
    'ชุมชนบ้านสองชั้น',
    'วัดบ้านจอม',
    'บ้านเสม็ด',
    'บ้านหนองรักษ์',
    'บ้านเสม็ดประชาอุปถัมภ์',
    'บ้านโคลด',
    'บ้านสูงเนิน',
    'บ้านทุ่งสว่าง',
    'บ้านบัวถนน',
    'บ้านจะเนียงสามัคคี',
    'บ้านตะครองใต้',
    'วัดบ้านปราสาท',
    'บ้านละลูน',
    'บ้านชำแระ',
    'บ้านหนองเต็ง (จันทศิริอนุสรณ์)',
    'วัดหนองตะครอง',
    'บ้านเสม็ด "สามัคคีราษฎร์วิทยาคาร"',
    'บ้านโนนแดง',
    'บ้านจาน',
    'บ้านหนองเหล็ก',
    'บ้านกุดใหญ่',
    'บ้านห้วยสำราญ',
    'บ้านกุดโคลน',
    'บ้านน้ำอ้อม',
    'บ้านตะครอง',
    'บ้านตาเป้า',
    'บ้านประดู่',
    'บ้านเขาคอก',
    'บ้านโคกกลอย',
    'บ้านสี่เหลี่ยมวิทยา',
    'บ้านหนองตะโก',
    'บ้านหนองเอียน',
    'บ้านละหอกกระสัง',
    'บ้านโนนศิลา',
    'บ้านไพรวัลย์น้อย',
    'บ้านหนองตะขบ',
    'บ้านโคกตูม',
    'บ้านราษฏร์นิยม',
    'บ้านหนองม่วงพัฒนา',
    'บ้านโคกรัง',
    'บ้านโคกมะขาม',
    'บ้านหนองอาแมะ',
    'บ้านโคกตะเคียนสามัคคี',
    'บ้านหนองไผ่',
    'บ้านโคกเพชร (บุญเหลือคุรุราษฎร์บำรุง)',
    'บ้านเกียรติเจริญ',
    'วัดตะลุงเก่า',
    'บ้านโคกกลาง',
    'บ้านโคกย่าง',
    'บ้านเก็ม',
    'บ้านจรเข้มาก',
    'วัดบ้านเมืองต่ำ',
    'บ้านบุ (รัฐราษฎร์รังสฤษฎ์)',
    'บ้านบัว',
    'บ้านหนองร้าน',
    'บ้านกระสังสามัคคี',
    'บ้านชุมแสง',
    'บ้านโคกเห็ด',
    'วัดบ้านตะโกตาพิ',
    'บ้านบาตร (ชัยทัตราษฏร์บำรุง)',
    'บ้านเกษตรสมบูรณ์',
    'วัดบ้านไทร (เสคุรุราษฏร์อนุสรณ์)',
    'วัดบ้านไพบูลย์',
    'บ้านโคกกี่',
    'บ้านโคกสำโรง',
    'บ้านหนองม่วงราษฎร์อุทิศ',
    'อนุบาลประโคนชัย(อำนวยกิจราษฎร์วิทยาคาร)',
    'ประโคนชัยวิทยา',
    'วัดแจ้ง (เศียรคุรุราษฎร์บำรุง)',
    'บ้านศรีถาวร',
    'บ้านหนองม่วง',
    'บ้านหัวตะแบก',
    'วัดบ้านปะทัดบุ',
    'บ้านหนองกระต่าย',
    'บ้านกระสัง',
    'บ้านหินกอง',
    'บ้านเขว้า',
    'บ้านปังกู (คุรุประชานุสรณ์)',
    'บ้านพาชี',
    'บ้านโคกสะอาด',
    'บ้านห้วยปอ',
    'บ้านหนองเก็ม',
    'บ้านกะโดนหนองยาง',
    'บ้านร่มเย็นวิทยา',
    'บ้านบ่อดิน',
    'บ้านไพศาลสามัคคี',
    'บ้านละลมพนู',
    'บ้านสังเคิล',
    'บ้านไทรโยง',
    'บ้านโพธิ์ไทร',
    'บ้านหนองคูณพิณทอง',
    'บ้านละเวี้ย (แปลงคุรุราษฎร์บำรุง)',
    'บ้านจรอกใหญ่',
    'บ้านปราสาทสามัคคี',
    'บ้านหนองนา',
    'บ้านโคกเพชรไสยา',
    'บ้านสี่เหลี่ยม',
    'บ้านสารภี',
    'บ้านหนองบอน',
    'บ้านแสลงโทน',
    'วัดบ้านยาง (ช้อยคุรุราษฏร์บำรุง)',
    'บ้านหนองบอนวิทยา',
    'บ้านห้วยเสลา',
    'บ้านหนองน้ำขุ่น',
    'บ้านโนนเสน่ห์',
    'บ้านหว้า',
    'บ้านเขาดินเหนือ',
    'บ้านศรีสุข',
    'บ้านหลัก',
    'บ้านตรุง',
    'นิคมสร้างตนเอง 4',
    'นิคมสร้างตนเอง 5',
    'นิคมสร้างตนเอง 6',
    'นิคมสร้างตนเอง 8',
    'บ้านนิคมสายโท 8',
    'บ้านสายโท 4 ใต้',
    'นิคมสร้างตนเอง 2',
    'ชุมชนบ้านโนนเจริญ',
    'บ้านถนนโคกใหญ่',
    'บ้านหนองแวง',
    'นิคมสร้างตนเอง 9',
    'อนุบาลบ้านกรวด',
    'บ้านตาอี',
    'บ้านโคกยาง',
    'บ้านเขาดินใต้',
    'บ้านยาง',
    'บ้านหนองตะเคียน',
    'บ้านโคกเบง',
    'บ้านตัวอย่าง',
    'นิคมสร้างตนเอง 3',
    'นิคมสร้างตนเอง 7',
    'บ้านบึงเจริญ',
    'บ้านสายโท 1',
    'นิคมสร้างตนเอง 1',
    'บ้านโคกระเหย',
    'บ้านปราสาททอง',
    'บ้านละหอกตะแบง',
    'นิคมพัฒนาสายตรี 2',
    'นิคมพัฒนาสายโท 11',
    'บ้านนิคมสายโท 12 เหนือ',
    'กองทัพบกอุปถัมภ์',
    'บ้านนิคมสายโท 12 ใต้',
    'บ้านโคกกะชาย',
    'บ้านหนองไม้งาม 1',
    'บ้านหนองไม้งาม 2',
    'บ้านตะลุมพุก',
    'บ้านสายตรี 9',
    'บ้านสายตรี 16',
    'บ้านหินลาด',
    'บ้านละหานทรายใหม่',
    'บ้านละหานทรายเก่า',
    'บ้านถนนน้อย',
    'วัดบ้านแสลงคง',
    'บ้านโคกขมิ้น',
    'บ้านตาแก',
    'บ้านเขว้าศึกษา',
    'วัดบ้านจันดุม (คุรุธำรงราษฎร์อนุสรณ์)',
    'บ้านปลัด',
    'บ้านตะแบก',
    'บ้านแพงพวย',
    'บ้านโคกชุม',
    'บ้านปราสาท',
    'วัดบ้านโคกเจริญ',
    'บ้านป่าชัน',
    'บ้านปะทัดบุวิทยา',
    'บ้านมะมัง',
    'อนุบาลสะเดา',
    'บ้านบุญช่วย',
    'วัดพลับพลา',
    'บ้านโคกโพธิ์',
    'วัดสำโรง',
    'บ้านตะโกรี',
    'บ้านเสม็ด',
    'บ้านประดู่',
    'พุทธบารมี',
    'วัดบ้านโคกเหล็ก',
    'บ้านกระสังสามัคคี',
    'บ้านโคกขมิ้นพัฒนา',
    'วัดบ้านตาเสา',
    'บ้านมะขามทานตะวัน',
    'บ้านเกต "สุตาประชาอุปถัมภ์"',
    'บ้านตะโกบำรุง',
    'บ้านโชคกราด',
    'วัดบ้านเมืองโพธิ์',
    'บ้านจบก',
    'วัดบ้านสวายจีกน้อย',
    'วัดบ้านสนวน',
    'วัดบ้านใหม่',
    'บ้านเพชรประชาสามัคคี',
    'อนุบาลห้วยราช',
    'บ้านดวน',
    'กระสังพิทยาคม',
    'ลำดวนพิทยาคม',
    'ชุมแสงพิทยาคม',
    'สูงเนินพิทยาคม',
    'ประโคนชัยพิทยาคม',
    'บ้านบุวิทยาสรรค์',
    'ไพศาลพิทยาคม',
    'เมืองตลุงพิทยาสรรพ์',
    'แสลงโทนพิทยาคม',
    'บ้านกรวดวิทยาคาร',
    'โนนเจริญพิทยาคม',
    'พลับพลาชัยพิทยาคม',
    'ห้วยราชพิทยาคม',
    'วัดก้านเหลือง',
    'บ้านโคกยางหนองถนน',
    'บ้านหนองหญ้าวัว',
    'บ้านหนองปรือ',
    'บ้านเสลา',
    'บ้านทุ่งโพธิ์',
    'บ้านโคกมะค่า',
    'บ้านถนนหัก (เพียรประจงวิทยา)',
    'บ้านสวายสอ',
    'ชุมชนบ้านกระเดื่อง (โสภณประชานุกูล)',
    'บ้านหนองม่วง',
    'บ้านตลาดแย้',
    'บ้านหนองหว้าขุนอัดวิทยา',
    'บ้านเสลาโสรง',
    'บ้านโคกยางหนองตาสี',
    'บ้านแท่นบัลลังก์',
    'บ้านชุมแสง(บุญคุรุราษฎร์บำรุง)',
    'บ้านทุ่งใหญ่',
    'บ้านโคกหลวงพ่อ (พินิจศิลป์ประชานุกูล)',
    'บ้านหนองเสม็ด',
    'บ้านหนองรี "มิตรภาพที่ 225"',
    'บ้านหนองงิ้ว หนองไทร',
    'อนุบาลนางรอง(สังขกฤษณ์อนุสรณ์)',
    'บ้านนางรอง (ยุทธกาจราษฎร์วิทยาคาร)',
    'บ้านแพงพวย (สังฆคุรุราษฎร์บำรุง)',
    'บ้านคอกควาย (สันติภาพคุรุราษฎร์พัฒนา)',
    'บ้านสิงห์ (ราษฎร์อนุสรณ์)',
    'บ้านหนองกง',
    'บ้านหนองยาง',
    'บ้านไร่โคกหนองปรือ (ประสมทรัพย์อนุสรณ์)',
    'บ้านหนองตะลุมปุ๊ก',
    'วัดน้ำไหล',
    'บ้านสะเดา (สามัคคีวิทยา)',
    'บ้านหนองแช่ไม้',
    'บ้านหนองกก (ทรงรวมมิตร)',
    'บ้านสองพี่น้อง',
    'บ้านหนองพลวง',
    'บ้านบุตาเวสน์',
    'ชุมชนวัดบ้านหนองกง (ประจวบไตรมิตรวิทยา)',
    'บ้านหนองบัว',
    'บ้านหนองไทร (ปัญจคามคุรุสรรค์)',
    'บ้านนิคมเขต',
    'บ้านตะกุดตาสา (ลองตองประชานุกูล)',
    'บ้านหนองกราด',
    'บ้านสระประดู่หนองมะค่า',
    'บ้านหนองโบสถ์',
    'บ้านหนองทองลิ่ม',
    'บ้านก้านเหลือง',
    'บ้านหนองยายพิมพ์ (ไตรคามประชาสรรค์)',
    'บ้านบุตาสุ่มหนองโจด',
    'บ้านจาน',
    'บ้านโคกตะโก',
    'บ้านระนามพลวง (สมานราษฎร์วิทยาคาร)',
    'บ้านหนองโสน',
    'บ้านโคกสูง (สมานสามัคคี)',
    'บ้านถนน (ประสมทรัพย์ประชานุกูล 1)',
    'บ้านวังกระโดน',
    'บ้านหนองถั่วแปบ',
    'บ้านโคกสว่าง',
    'บ้านโคกลอย',
    'บ้านตะกรุมทอง',
    'บ้านโนนพะไล',
    'บ้านสะเดาหวาน',
    'บ้านหนองหว้า',
    'บ้านสระตะเคียน',
    'บ้านโคกสูงคูขาด',
    'มิตรภาพโนนสมบูรณ์',
    'ชุมชนบ้านดอนอะราง',
    'บ้านโคกสะอาด',
    'บ้านโนนขี้เหล็ก',
    'บ้านคูขาดน้อย',
    'บ้านหนองกราด',
    'บ้านโคกกระพี้หนองนกเขาสามัคคี',
    'บ้านลุงขี้หนู',
    'บ้านเพชรเจริญพัฒนา',
    'อนุบาลหนองกี่',
    'บ้านหนองกี่สุขสำราญ',
    'บ้านหนองหัวหมู',
    'บ้านสระขุด',
    'ไตรคาม',
    'บ้านบุกระสัง',
    'บ้านหนองมัน',
    'บ้านสระสะแก',
    'บ้านสระขาม',
    'บ้านโคกกระชาย',
    'บ้านหนองม่วง',
    'บ้านหนองไผ่',
    'บ้านนาจาน',
    'จตุคามราษฎร์วิทยา',
    'วัดเย้ยปราสาท',
    'บ้านขามน้อย',
    'บ้านหนองตาดำ',
    'บ้านหนองกี่ประชาสามัคคี',
    'บ้านหนองหมี',
    'ชุมชนบ้านโคกว่าน',
    'บ้านสมจิต',
    'บ้านหนองถนน',
    'บ้านโคกขาม',
    'บ้านหนองโบสถ์',
    'บ้านหนองมดแดง (ราษฎร์ประสิทธิ์วิทยาคม)',
    'บ้านเทพพยัคฆ์พัฒนา',
    'ชุมชนวัดชุมพร',
    'บ้านบุ',
    'บ้านโคกตาพรม',
    'บ้านหนองยาง',
    'บ้านสว่างพัฒนา',
    'บ้านปรือพวงสำราญ',
    'บ้านละหานทราย (คุรุราษฎร์บำรุงวิทยา)',
    'บ้านหนองละหานทราย',
    'อนุบาลโคกใหม่ละหานทราย',
    'บ้านโคกไม้แดง',
    'บ้านโคกเฟือง',
    'บ้านสันติสุข',
    'บ้านน้อยหนองหว้า',
    'วัดกัลยาณธรรมาราม',
    'บ้านตากอง',
    'บ้านหนองกราด',
    'บ้านหนองตะครอง',
    'บ้านปากช่อง',
    'บ้านหนองแวง',
    'บ้านหงอนไก่ (โคกสง่า)',
    'บ้านหนองตาเยา',
    'บ้านหนองหว้า',
    'บ้านบาระแนะ',
    'บ้านราษฎร์รักแดน',
    'บ้านแท่นทัพไทย',
    'บ้านผไทรวมพล',
    'บ้านเทพพัฒนา',
    'บ้านปลื้มพัฒนา',
    'บ้านหินโคน',
    'บ้านโคกเขาพัฒนา',
    'ดงใหญ่พัฒนา (ดงใหญ่ 4)',
    'บ้านโคกลอย',
    'บ้านไทยเจริญ',
    'บ้านโคกสมบูรณ์',
    'บ้านกองพระทราย',
    'บ้านโคกงิ้ว',
    'บ้านทุ่งไผ่',
    'อนุบาลปะคำ',
    'บ้านหนองน้ำขุ่น',
    'ชุมชนบ้านดอนนางงาม',
    'บ้านสุขสำราญ',
    'บ้านโคกไม้แดงหัวกระสัง',
    'บ้านหูทำนบ',
    'บ้านหนองต้อ',
    'บ้านไทยสามัคคี',
    'บ้านหนองบัวลี',
    'วัดบ้านหนองตราดใหญ่',
    'บ้านขามเสม็ดบำรุง',
    'บ้านชุมแสง',
    'บ้านหนองตะคร้อ',
    'บ้านโคกปราสาท',
    'บ้านหนองย่างหมู',
    'บ้านฝ้าย',
    'บ้านหนองกวางทอง',
    'อนุบาลหนองหงส์',
    'เบญจมาศกุญชร',
    'บ้านหนองโคลน',
    'บ้านเสาเดียวอนุสรณ์',
    'บ้านหนองเต่า',
    'บ้านนายาวสามัคคี',
    'บ้านหนองกก',
    'บ้านไทรออ',
    'วัดโนนศรีคูณ',
    'บ้านห้วยก้อม',
    'บ้านกระเบื้อง',
    'บ้านศรีม่วงแดง',
    'บ้านโนนสูงน้อย',
    'บ้านศรีภูมิ',
    'ชุมชนวัดโคกกลาง "สระมะค่าวิทยา"',
    'วัดห้วยหิน',
    'บ้านหนองเพชร',
    'บ้านโนนงิ้วตลาดใหม่สามัคคี',
    'บ้านโพธิ์ทอง',
    'บ้านสำราญ',
    'บ้านโคกโสนทองวิทยา',
    'บ้านโกรกแก้ว',
    'วราวัฒนา',
    'บ้านดงบังซับสมบูรณ์',
    'บ้านม่วงงาม',
    'บ้านดอนสมบูรณ์',
    'บ้านทุ่งจังหัน',
    'บ้านขลุงไผ่',
    'บ้านอุบลสามัคคี',
    'อนุบาลโนนสุวรรณ',
    'อนุบาลโนนดินแดง',
    'บ้านทรัพย์สมบูรณ์',
    'บ้านซับคะนิง',
    'บ้านคลองหิน',
    'บ้านหนองบอน',
    'บ้านป่าไม้สหกรณ์',
    'บ้านลำนางรอง',
    'บ้านหนองเสม็ด',
    'บ้านคลองโป่ง',
    'บ้านน้อยสะแกกวน',
    'บ้านส้มป่อย',
    'บ้านหนองกกตะแบงสามัคคี',
    'บ้านเจริญสุข (กรป.กลางอุปถัมภ์)',
    'บ้านหนองสะแก (ราษฎร์บำรุง)',
    'บ้านสี่เหลี่ยม',
    'บ้านพูนสุข (ประชานุกูล)',
    'บ้านดอนไม้ไฟ',
    'บ้านบัวตะเคียน',
    'วัดอัมภาราม (เทิ่งอนุสรณ์)',
    'บ้านถาวร',
    'ไทยรัฐวิทยา 26',
    'บ้านโคกสำราญหินลาดพัฒนา',
    'บ้านยายคำ',
    'วัดสุขสำราญ',
    'อนุบาลเฉลิมพระเกียรติจังหวัดบุรีรัมย์',
    'บ้านโนนศิลา',
    'บ้านโนนทอง',
    'บ้านเขว้า',
    'บ้านโคกยาง',
    'ไทยรัฐวิทยา 90',
    'บ้านตะโก "วันครู 2501"',
    'นางรอง',
    'สิงหวิทยาคม',
    'นางรองพิทยาคม',
    'ทุ่งแสงทองพิทยาคม',
    'พิมพ์รัฐประชาสรรค์',
    'เหลืองพนาวิทยาคม',
    'ถนนหักพิทยาคม',
    'ศึกษาสงเคราะห์นางรอง',
    'หนองกี่พิทยาคม',
    'เมืองโพธิ์ชัยพิทยาคม',
    'ร่วมจิตต์วิทยา',
    'ละหานทรายวิทยา',
    'ตาจงพิทยาสรรค์',
    'ละหานทรายรัชดาภิเษก',
    'ไทยเจริญวิทยา',
    'ปะคำพิทยาคม',
    'หนองหงส์พิทยาคม',
    'ห้วยหินพิทยาคม',
    'โนนสุวรรณพิทยาคม',
    'สามัคคีพิทยาคม',
    'ร่มเกล้า',
    'พนมรุ้ง',
    'บ้านกอกโคกวิทยา',
    'บ้านโคกใหญ่',
    'บ้านจิก',
    'บ้านโสกแต้',
    'อนุบาลคูเมือง',
    'บ้านคูบอน',
    'บ้านหนองติ้ว',
    'บ้านปะคำดง',
    'บ้านโนนเจริญ',
    'วัดบ้านสำราญราษฎร์',
    'บ้านหนองดุม',
    'บ้านหนองตาด',
    'บ้านหนองบัว',
    'บ้านตูม',
    'บ้านหนองกระทุ่ม',
    'บ้านปะคำสำโรง',
    'บ้านสวายสอ',
    'บ้านโนนกลาง',
    'บ้านโนนเพกา',
    'บ้านแพ',
    'บ้านหนองนางดำ',
    'วัดบ้านเบาน้อย',
    'บ้านวังปลัด',
    'บ้านหนองขุนพรม',
    'บ้านหนองบัวแดง',
    'บ้านดงย่อ',
    'บ้านเขว้า',
    'บ้านโจด',
    'วัดบ้านปะเคียบ',
    'บ้านสระขี้ตุ่น',
    'บ้านพรสำราญ',
    'วัดบัวทอง',
    'บ้านโนนยานาง',
    'บ้านหนองขวาง',
    'ไตรคามศรีอนุสรณ์',
    'บ้านสระปะคำ',
    'บ้านสระบัว',
    'บ้านดงเค็ง',
    'บ้านหนองหว้า',
    'ชุมชนบ้านหนองขมาร',
    'บ้านหนองไทร',
    'บ้านคูบัว',
    'วัดบ้านโนนมาลัย',
    'บ้านโศกนาคท่าม่วง',
    'บ้านหนองดุมหัวช้าง (ราษฎร์พัฒนา)',
    'บ้านหนองรัก',
    'บ้านตาหล่ำ',
    'บ้านสาวเอ้',
    'บ้านหินเหล็กไฟ',
    'วัดโพธิ์',
    'บ้านบุ่งเบา',
    'บ้านข่อย',
    'บ้านเป้า (สุทธิสารอุทิศ)',
    'วัดสุวรรณาราม',
    'บ้านคูณสระแก้ว (วิชานนท์ประชานุกูล)',
    'บ้านเพียแก้ว',
    'วัดเทพรังสรรค์',
    'บ้านดอนตูม',
    'บ้านยางนกคู่',
    'บ้านแคน',
    'วัดสระบัว',
    'วัดธรรมประสิทธิ์',
    'ตงศิริราษฏร์อนุสรณ์',
    'บ้านแดง (สหราษฏร์วิทยา)',
    'วัดท่าเยี่ยม',
    'อนุบาลพุทไธสง(โอกาสประชานุสรณ์',
    'วัดหงษ์',
    'วัดวงษ์วารี',
    'บ้านซาดศึกษา',
    'วัดวรดิษฐ์',
    'ชุมชนบ้านหายโศก',
    'บ้านดอน',
    'บ้านกระสัง',
    'บ้านหนองปุน',
    'บ้านหนองยาง-หนองจาน',
    'บ้านหญ้าคา',
    'บ้านตามา',
    'บ้านชุมแสง',
    'บ้านโคกสิงห์',
    'บ้านสระกุด',
    'บ้านเสม็ด',
    'บ้านหนองหัวควาย',
    'ชุมชนบ้านดอนมนต์',
    'บ้านนาลาว',
    'บ้านละกอ',
    'บ้านกุดน้ำขุ่น',
    'บ้านท่าเรือ',
    'บ้านท่าม่วง',
    'บ้านสำโรงพิมาน',
    'บ้านโนนยาง',
    'บ้านโนนธาตุ',
    'บ้านหนองครก',
    'บ้านทุ่งวังสาขาบ้านโคกอะโตด',
    'บ้านทุ่งวัง',
    'บ้านหนองแวง',
    'บ้านสระกอไทร',
    'บ้านดงยายเภา',
    'อนุบาลสตึก(ประชาอนุสรณ์)',
    'บ้านหนองม่วง',
    'บ้านหัวช้าง',
    'บ้านโคกก่อง',
    'บ้านหนองบัวเจ้าป่า',
    'บ้านเมืองแก',
    'บ้านหนองเชือก',
    'บ้านหนองปลาแดก',
    'บ้านหนองนกเกรียน',
    'บ้านหนองดุม',
    'บ้านร่อนทอง',
    'บ้านเสม็ด "สถาปัตย์อนุสรณ์"',
    'บ้านขาม',
    'บ้านปรือเกียน',
    'บ้านหนองน้ำขุ่น',
    'บ้านจะหลวย',
    'บ้านกระทุ่ม',
    'บ้านโคกสุพรรณ',
    'บ้านสตึก',
    'บ้านหนองเกาะ (คุรุราษฏร์บำรุง)',
    'บ้านยางน้ำใส',
    'บ้านคูขาด',
    'บ้านตาโหงก',
    'บ้านหนองแคน',
    'บ้านพงแขม',
    'บ้านปลัดมุม',
    'บ้านปากช่องสนามชัย',
    'ชุมชนบ้านสะแก',
    'บ้านโนนค้อ',
    'วัดบ้านพลับ',
    'บ้านโคกเมือง',
    'บ้านโนนสมบูรณ์',
    'บ้านหนองใหญ่',
    'บ้านหนองกับ',
    'บ้านหนองไผ่',
    'วัดบ้านสวายตางวน',
    'วัดฤาษีสถิต',
    'บ้านโคกกุง',
    'วัดโพธิ์ชัย',
    'วัดสิริมงคล',
    'ชุมชนบ้านนาโพธิ์ (ต.ม.ธ.ก.รุ่น 7 อนุสรณ์)',
    'วัดบุปผาราม',
    'บ้านโศกกะฐิน',
    'วัดท่าเรียบ',
    'วัดพนมวัน',
    'บ้านคู (คุรุศิษย์วิทยาคาร)',
    'อมรสิริสามัคคี',
    'วัดสระทอง',
    'วัดทรงสุวรรณ',
    'วัดธาตุ',
    'บ้านโนนตาล (ศิลาคุรุราษฎร์พัฒนา)',
    'สามัคคีเทพอำนวย',
    'ชุมชนบ้านนาแพง',
    'วัดชัยสมพร',
    'สามัคคีคุรุราษฏร์บำรุง',
    'บ้านหัวขัว (เบญจคุรุราษฏร์อุทิศ)',
    'วัดสมณาวาส',
    'บ้านโนนไฮ',
    'บ้านหนองสองห้อง (เทพผาประชานุกูล)',
    'บ้านหนองเรือ',
    'วัดสุคันธารมย์',
    'วัดวนาสันต์ (โศกนากสามัคคี)',
    'วัดอิสาณ',
    'วัดชายอรัญ',
    'บ้านกอกดอนพยอม',
    'บ้านทองหลาง',
    'วัดพลสุวรรณ',
    'วัดเทพประดิษฐ์',
    'วัดเทพรังษี',
    'วัดสระจันทร์',
    'บ้านกม.ศูนย์',
    'อนุบาลบ้านใหม่ไชยพจน์(ฉลาดราษฎร์บำรุง)',
    'บ้านหนองไผ่',
    'วัดไพรงาม',
    'วัดศรีสุนทร',
    'จุฬางกูรวิทยา',
    'วัดหลักศิลา',
    'อนุบาลแคนดง',
    'บ้านหนองการะโก',
    'วัดราษฏร์สามัคคี',
    'บ้านยางทะเล',
    'บ้านโนนกลาง',
    'บ้านกระทุ่มเครือ',
    'บ้านดงพลอง',
    'บ้านป่าหนาม',
    'บ้านกระทุ่มจานสามัคคี',
    'บ้านขามพิมาย',
    'บ้านสระบัว',
    'บ้านซาด',
    'บ้านหนองสรวง',
    'บ้านงิ้ว',
    'บ้านตะแบงสามัคคี',
    'บ้านปอแดง',
    'บ้านขี้เหล็ก',
    'บ้านหนองหญ้าคา',
    'บ้านโคกสว่าง',
    'บ้านหัวฝาย',
    'คูเมืองวิทยาคม',
    'ตูมใหญ่วิทยา',
    'มัธยมพรสำราญ',
    'หนองขมารวิทยาคม',
    'พุทไธสง',
    'เมืองแกพิทยาคม',
    'จุฬาภรณราชวิทยาลัย บุรีรัมย์',
    'สะแกพิทยาคม',
    'สมเสม็ดวิทยา',
    'ร่อนทองพิทยาคม',
    'สตึก',
    'นาโพธิ์พิทยาคม',
    'อุดมอักษรพิทยาคม',
    'กู่สวนแตงพิทยาคม',
    'แคนดงพิทยาคม',
    'ดงพลองพิทยาคม',
    'บ้านขนาด',
    'บ้านกาเกาะ(แหลมราษฎร์นุเคราะห์)',
    'บ้านโคกอารักษ์',
    'บ้านขามระกา',
    'บ้านแกใหญ่',
    'บ้านแกน้อย',
    'บ้านนาเกา',
    'บ้านตาเตน',
    'บ้านจักจรูก',
    'บ้านตระแบกน้อย',
    'บ้านคอโค',
    'บ้านลำชี(วุฒิวงษ์ราษฎร์สงเคราะห์)',
    'บ้านดงมัน',
    'ราชวิถี(ประสาทราษฎร์บำรุง)',
    'พรหมประสาทราษฎร์นุกูล',
    'บ้านกะทม(ร่วนวิทยา)',
    'บ้านนาสม(วิไลราษฎร์ศึกษา)',
    'บ้านสกร็อม(ประสาทศีลสามัคคี)',
    'บ้านหนองเต่า',
    'อุปัชฌาย์พวนอุทิศ',
    'บ้านใหม่',
    'บ้านตระแสง',
    'บ้านโคกเพชร',
    'รุ่งรัฐวิทยา',
    'บ้านตั้งใจ',
    'บ้านระกาสังแก',
    'บ้านปะตะเมาะ',
    'บ้านหนองกระดาน',
    'บ้านระไซร์(เด่นพัฒนา)',
    'บ้านปราสาทตราดมิตรภาพที่58',
    'บ้านขยอง',
    'เสกวิทยาคม',
    'บ้านอังกัญเสกแอ(ประดิษฐ์ราษฎร์สามัคคี)',
    'บ้านตาอ็อง',
    'บ้านจันรม',
    'บ้านแจรน(ปอเกีย-พลินอุทิศ 1)',
    'บ้านใต้ฆ้องโนนจิก',
    'บ้านกาเกาะ',
    'บ้านโคกยาง',
    'บ้านเขวา(เทพอนุสรณ์)',
    'บ้านระเภาว์',
    'บ้านกาเกาะระโยง',
    'บ้านสำโรง-โคกเพชร',
    'บ้านท่าสว่าง',
    'บ้านส้มป่อย',
    'บ้านตาเพชร',
    'บ้านอำปึล(ปอเกียพลินอุทิศ 3)',
    'บ้านโคกปราสาท',
    'บ้านคาบ',
    'บ้านเทนมีย์',
    'บ้านระหาร',
    'บ้านไถงตรง',
    'บ้านตะตึงไถง',
    'บ้านโคกมะเมียน(ธรรมสุชาติอุทิศ)',
    'บ้านหนองกง',
    'บ้านพันธุลี',
    'บ้านโคกกะเพอ',
    'หนองโตง"สุรวิทยาคม"',
    'บ้านโคกพระ',
    'บ้านประทัดบุอาลอ',
    'บ้านตำปูง',
    'บ้านโดนออง',
    'บ้านสำโรงนาดี',
    'บ้านระไซร์สองชั้น',
    'บ้านโคกวัด',
    'บ้านลุมพุก',
    'บ้านนาเสือก',
    'บ้านละหุ่งหนองกก',
    'บ้านโดนโอก',
    'บ้านนาบัว',
    'บ้านนาสาม(เกียรติประชา)',
    'บ้านกะทม(คุณรสนิมิต)',
    'เมืองสุรินทร์',
    'อนุบาลสุรินทร์',
    'วัดประสพ',
    'บ้านบุฤาษี',
    'บ้านอังกัญ - โคกบรรเลง',
    'บ้านรัตนะ',
    'บ้านกู่(ราษฎร์สามัคคี)',
    'บ้านระกา(สนง.สลากกินแบ่งสงเคราะห์ 74)',
    'บ้านหัวตะพาน',
    'บ้านเพี้ยราม',
    'บ้านหนองกัว',
    'บ้านตาเปาว์',
    'บ้านทะนงชัย',
    'บ้านบุทม',
    'บ้านประปืด',
    'เมืองที',
    'บ้านโคกสำโรง',
    'บ้านราม',
    'บ้านบรมสุข',
    'บ้านเล้าข้าว',
    'บ้านกันแสง',
    'บ้านสลักได',
    'บ้านเสม็ด(ประชารัฐนุเคราะห์)',
    'บ้านตระแบก',
    'บ้านตะบัล',
    'บ้านโคกเมือง',
    'บ้านสวาย',
    'บ้านปอยเดิน(อินทรศึกษา)',
    'บ้านสำโรง',
    'บ้านทัพกระบือ',
    'บ้านแสลงพันธ์',
    'บ้านประสิทธิ์ธนู(ปอเกีย-พลินอุทิศ 2)',
    'บ้านอาโพน(สหมิตรสงเคราะห์)',
    'บ้านโสน(พิทยศึกษา)',
    'บ้านแสลงพันบุณเยิง',
    'บ้านกันโจรง',
    'บ้านเวียย',
    'บ้านอันโนง',
    'บ้านกระทุ่ม',
    'บ้านขามราษฎร์วิทยาคม',
    'บ้านจอมพระ',
    'ไตรมิตรวิทยาสรรค์',
    'ทวีคามวิทยา',
    'บ้านโคกเพร็ดโนนจำปา',
    'บ้านหนองขวาว',
    'บ้านผางอุดมสมบูรณ์วิทยา',
    'บ้านชุมแสง',
    'บ้านจบก',
    'บ้านบุอาไร',
    'บ้านผือ',
    'บ้านม่วง',
    'สามัคคีวิทยา',
    'บ้านอาทึก',
    'บ้านขามศึกษาคาร',
    'บ้านบุแกรงตะเคียน(ปานอุทิศวิทยา)',
    'บ้านว่าน',
    'บ้านอาคตหนองขอนประชาวิทย์',
    'บ้านอาแวะ',
    'บ้านขมิ้น(เรืองราษฎร์รังสรรค์)',
    'บ้านขามเดื่อราษฎร์สงเคราะห์',
    'บ้านดินแดง',
    'บ้านระวี',
    'บ้านกรูดหนองซำ',
    'บ้านดงเค็ง',
    'บ้านทวารไพร',
    'บ้านทุ่งนาค',
    'บ้านเมืองลีง',
    'บ้านหนองเหล็ก',
    'บ้านตากวน',
    'ลุ่มระวีวิทยา',
    'บ้านหนองพลวง',
    'บ้านหนองเหล็กราษฎร์วิทยา',
    'บ้านโนนงิ้ว',
    'บ้านวารไพรศรี',
    'บ้านสำโรง',
    'บ้านหนองกับ',
    'บ้านหนองสนิท',
    'บ้านกุดหวาย',
    'บ้านโคกสนวน',
    'บ้านสมบูรณ์(ปัญจวิทยา)',
    'บ้านดงถาวร',
    'บ้านกระแสร์อุดมท่าโคราช',
    'บ้านกุดหวาย(สาขาบ้านแก่นเมธี)',
    'บ้านไพรษรสำโรง',
    'บ้านขวาวใหญ่',
    'บ้านคาละแมะ',
    'บ้านหนองจอก',
    'บ้านบึงขวาง',
    'บ้านหนองขาม',
    'บ้านจารพัต',
    'บ้านเปรียง',
    'บ้านกันจารย์',
    'บ้านพม่า',
    'บ้านพันษี',
    'บ้านช่างปี่',
    'บ้านตะคร้อ',
    'บ้านกระโดนค้อ',
    'บ้านหัวแรตนาโพธิ์',
    'บ้านโคกลาวหนองเหล็ก',
    'บ้านโคกอาโพน',
    'บ้านหนองผือโนนแคน',
    'บ้านข่า',
    'บ้านตรมไพร',
    'จตุรมิตรวิทยา',
    'บ้านตะกุย(ค้ำคูณบำรุง)',
    'บ้านตะเคียน',
    'บึงวิทยาคาร',
    'บ้านกาเจาะหนองไผ่',
    'บ้านหัวแรต',
    'บ้านม่วงทรัพย์โกฎิ์',
    'บ้านตรึม"ตรึมวิทยานุเคราะห์"',
    'บ้านหนองหิน',
    'บ้านบ่อน้ำใส',
    'บ้านแตล',
    'บ้านประทุนอายอง',
    'บ้านสวาย',
    'บ้านนาท่มหนองผักบุ้ง',
    'บ้านหนองท่ม',
    'บ้านโนนถ่อน',
    'บ้านสังแก',
    'บ้านอาวุธ',
    'บ้านดู่อาราง',
    'บ้านนารุ่ง',
    'บ้านผักไหม',
    'บ้านก้านเหลือง',
    'บ้านทุ่งราม',
    'บ้านทุ่งรูง',
    'บ้านสว่างโนนแดง',
    'บ้านศรีตะวัน',
    'บ้านกะลัน',
    'บ้านอนันต์(ชูศรีราษฎร์สามัคคี)',
    'บ้านกางของ',
    'บ้านระเวียง',
    'บ้านยางเตี้ย',
    'บ้านระแงง(ศรีขรภูมิวิทยาคม)',
    'บ้านหนองขวาว',
    'บ้านขะเนก',
    'บ้านแกสระใหญ่',
    'บ้านกรวดนาฮัง',
    'บ้านหนองขาม',
    'บ้านสะโน',
    'บ้านหนองบัว',
    'บ้านหนองจิก',
    'บ้านหนองกุง',
    'บ้านหนองขนาด',
    'บ้านโพธิ์(โพธิ์ศรีวิทยา)',
    'บ้านลุมพุกหนองกุง',
    'บ้านโคกลำดวน',
    'บ้านห้วยโนนเจริญ',
    'บ้านปะนอยไถง',
    'บ้านหนองเหล็ก',
    'บ้านจารย์',
    'บ้านกระดาน',
    'บ้านภูมิสตึง',
    'บ้านโชกเหนือ',
    'บ้านตระเปียงเตีย',
    'บ้านหนองคู',
    'อมรินทร์ราษฎร์วิทยา',
    'ดรุณวิทยากร',
    'บ้านตรำดม',
    'บ้านระไซร์',
    'บ้านหนองยาง',
    'ศิริราษฎร์วิทยา',
    'บ้านตาเมาะ',
    'บ้านจรวย',
    'บ้านลำดวน',
    'วัดทักษิณวารี',
    'บ้านอู่โลก',
    'บ้านตะเคียน',
    'บ้านโชกใต้',
    'บ้านโคกเกาะ',
    'บ้านมะลูจรุง',
    'บ้านกระออม',
    'บ้านหนองเหล็ก',
    'บ้านหนองอีเลิง',
    'บ้านเกาะแก้ว',
    'บ้านโนนสวรรค์',
    'บ้านตะมะหนองกระจาน',
    'บ้านตางมาง',
    'บ้านขนวน',
    'บ้านดู่',
    'บ้านหนองพญา',
    'บ้านศรีราชา',
    'บ้านหนองดุม',
    'บ้านสะโน',
    'บ้านกุง',
    'บ้านตะเคียน',
    'บ้านสำโรงทาบ(บริคุตต์วิทยาคุณ)',
    'การุญวิทยา',
    'บ้านสังแก',
    'บ้านจังเกา',
    'บ้านจังเอิดหนองแมว',
    'บ้านหนองคู',
    'บ้านหนองแคน',
    'บ้านหนองแล้ง',
    'บ้านหนองหว้า',
    'บ้านโนนลี',
    'บ้านขอนแก่น',
    'บ้านค้อ',
    'บ้านดู่โศก',
    'บ้านหนองฮะ',
    'บ้านกะเลา - ศรีพัฒนา',
    'บ้านกุดงิ้วศิริพัฒนา',
    'หมื่นศรีประชาสรรค์',
    'บ้านหมื่นศรีน้อย',
    'อนุบาลเขวาสินรินทร์',
    'บ้านนาโพธิ์',
    'บ้านสดอ',
    'บ้านนาตังตระแบก(กาญจนูปถัมภ์)',
    'บ้านตากูก',
    'บ้านโชค',
    'บ้านอำปึล(ดาวรุ่งรัฐวิทยา)',
    'บ้านตากแดด',
    'บ้านภูดินหนองตะครอง',
    'บ้านบึง-เบาะอุ่น',
    'บ้านกันเตรีย็ง',
    'บ้านพระปืด',
    'บ้านตลาดตาโหมก',
    'บ้านโคกกรวด',
    'บ้านบึง(สนง.สลากกินแบ่งสมทบสร้าง 2531)',
    'บ้านกันตรง',
    'บ้านสนวนนางแก้ว',
    'บ้านบุญโลก',
    'บ้านแสรออ',
    'บ้านสามโค',
    'บ้านฉันเพล',
    'บ้านพะเนา',
    'สุรวิทยาคาร',
    'สิรินธร',
    'สวายวิทยาคาร',
    'นาดีวิทยา',
    'ศรีไผทสมันต์',
    'รามวิทยา รัชมังคลาภิเษก',
    'สุรินทร์ภักดี',
    'พญารามวิทยา',
    'สุรินทร์ราชมงคล',
    'โชคเพชรพิทยา',
    'สุรินทร์พิทยาคม',
    'วีรวัฒน์โยธิน',
    'มหิธรวิทยา',
    'นาบัววิทยา',
    'ศรีรามประชาสรรค์',
    'ตั้งใจวิทยาคม',
    'ท่าสว่างวิทยา',
    'แสงทรัพย์ประชาวิทยาคาร',
    'เทนมีย์มิตรประชา',
    'จอมพระประชาสรรค์',
    'เมืองลีงวิทยา',
    'บุแกรงวิทยาคม',
    'หนองสนิทวิทยา',
    'ศีขรภูมิพิสัย',
    'แตลศิริวิทยา',
    'ห้วยจริงวิทยา',
    'กุดไผทประชาสรรค์',
    'จารย์วิทยาคาร',
    'มัธยมจารพัตวิทยา',
    'หนองแวงพิทยาคม',
    'ขวาวใหญ่วิทยา',
    'วังข่าพัฒนา',
    'ยางวิทยาคาร',
    'สุรพินท์พิทยา',
    'พนาสนวิทยา',
    'ศรีสุขวิทยา',
    'สำโรงทาบวิทยาคม',
    'สินรินทร์วิทยา',
    'แร่วิทยา',
    'บึงนครประชาสรรค์',
    'บ้านกระเบื้องใหญ่',
    'บ้านหนองพิมาน',
    'บ้านหัวนาคำ',
    'บ้านทิพย์เนตร',
    'บ้านขุนหาร',
    'อนุบาลชุมพลบุรี',
    'บ้านชุมพลบุรี',
    'บ้านตึกชุม',
    'บ้านโคกสูง',
    'บ้านระหาร',
    'บ้านแคนดำ',
    'บ้านสวนหม่อน',
    'โพนทองพิทยาคม',
    'บ้านดู่นาหนองไผ่',
    'บ้านกระเบื้อง',
    'บ้านขวาวโค้ง',
    'บ้านกระสัง',
    'บ้านยางบ่อภิรมย์',
    'บ้านทิพย์นวด',
    'บ้านไพรขลา(ไพรขลาราษฎร์พิทยากูล)',
    'บ้านโพนม่วง',
    'บ้านขามโพนทัน',
    'บ้านโนนตาลหนามแท่ง',
    'บ้านโนนสมบูรณ์',
    'ชุมชนบ้านซาด',
    'บ้านโนนสัง',
    'บ้านกะทะ',
    'บ้านยางบ่ออี',
    'บ้านบุตาโสม',
    'บ้านเมืองไผ่กระท่ม',
    'บ้านโพธิ์ห้วย',
    'บ้านขี้เหล็กโนนจาน',
    'บ้านงิ้ว',
    'บ้านยะวึก(ผจงราษฎร์วิทยาคาร)',
    'สามัคคีศึกษา',
    'บ้านแสนสุข',
    'บ้านยางชุม',
    'บ้านสำโรง',
    'สนวนโคกเม็ก',
    'บ้านเบงท่าลาด',
    'บ้านศรีณรงค์',
    'บ้านสายสนอง',
    'บ้านม่วงน้อย',
    'สระขุดดงสำราญวิทยา',
    'บ้านตั้งใจ',
    'บ้านอ้อตลิ่งชัน',
    'บ้านตลุง',
    'บ้านหนองเรือ',
    'บ้านขุนไชยทอง',
    'บ้านยางขามเฒ่า',
    'บ้านกระโพ (กระโพราษฎร์วิทยาคาร)',
    'บ้านตระมูง',
    'บ้านบอน',
    'บ้านอาคุณ',
    'บ้านโคกกุง',
    'บ้านศาลา',
    'บ้านหนองอีดำ',
    'บ้านภูดิน',
    'บ้านหนองบึง',
    'บ้านโนนโพ',
    'บ้านตาทิตย์',
    'บ้านบัว',
    'บ้านตากลาง',
    'ท่าตูม(สนิทราษฎร์วิทยาคม)',
    'บ้านตูม(อนุกูลราษฎร์พัฒนา)',
    'บ้านปราสาท',
    'บ้านลุงปุง',
    'บ้านโพนทา',
    'บ้านบัลลังก์พงสวาย',
    'วัดบ้านน้ำคำ(สนง.สลากฯ)',
    'บ้านหมากมี่',
    'บ้านเอือด',
    'บ้านตาฮะ',
    'บ้านปอหมัน',
    'บ้านตานบ',
    'บ้านโนนระเวียง',
    'บ้านน้ำอ้อม',
    'บ้านเฉนียง',
    'บ้านปรีง',
    'บ้านบะ',
    'บ้านโนนสูง',
    'บ้านบัวโคก',
    'บ้านธรรมษา',
    'บ้านจันทร์งาม',
    'บ้านหนองแสง',
    'บ้านพิงพวย',
    'บ้านโสมน',
    'ไตรคามสามัคคี',
    'บ้านพรมเทพ(พรมเทพราษฎร์บำรุง)',
    'บ้านยางกระจับ',
    'บ้านเหล่า',
    'บ้านชายทุ่ง',
    'บ้านหัวพี',
    'บ้านสาโรช',
    'บ้านโพนครก',
    'บ้านขี้เหล็ก',
    'บ้านโพนขวาว',
    'บ้านสะเอิง',
    'บ้านสำโรง',
    'บ้านกุดมะโน',
    'บ้านท่าศิลา',
    'บ้านหนองตาด',
    'บ้านหนองยาง(ธนาคารกรุงเทพ10)',
    'บ้านเมืองแก',
    'บ้านหนองคูน้อย',
    'บ้านหนองแวง',
    'บ้านโพนงอย',
    'บ้านหนองบัวมิตรภาพที่ 85',
    'บ้านม่วงมูล',
    'บ้านไกลเสนียด',
    'บ้านหนองไม้ถี่',
    'บ้านทุ่งโก',
    'บ้านบุผาง',
    'บ้านหนองคูใหญ่',
    'บ้านนานวล',
    'บ้านบอน',
    'บ้านลำเพิญ',
    'บ้านดงเปือย(มูลศึกษาวิทยา)',
    'บ้านดงเค็ง',
    'บ้านแก"แกศึกษาวิทยา"',
    'บ้านหนองบัวน้อย',
    'บ้านม่วงบุญมี',
    'บ้านหาญฮี',
    'บ้านบึง',
    'บ้านหนองหิน',
    'บ้านหนองตอ-บัวเสียว',
    'บ้านโนนทราย',
    'บ้านดอนแรด',
    'บ้านจาน',
    'บ้านทับน้อย',
    'บ้านทับโพธิ์วิทยา',
    'บ้านแต้',
    'บ้านม่วงหมาก',
    'บ้านน้ำคำ',
    'บ้านเหล่าม่วง',
    'บ้านโนนจำปา',
    'บ้านธาตุ',
    'บ้านน้ำสร้าง-นางเภา',
    'บ้านอาจญา',
    'บ้านน้ำเขียว',
    'บ้านหนองคู',
    'ไทยรัฐวิทยา 15 บ้านโกส้ม',
    'บ้านเบิด',
    'ไตรคามวิทยา',
    'บ้านหนองกระทุงตากแดด',
    'บ้านแข้ด่อน',
    'บ้านหนองผือ',
    'บ้านนาวอง',
    'บ้านช่อง',
    'บ้านคอนสวรรค์',
    'บ้านสี่เหลี่ยม',
    'บ้านนาอุดม',
    'บ้านไผ่ (วันครู 2503)',
    'บ้านยาง',
    'บ้านนางเข็ม',
    'บ้านขะยูง',
    'บ้านผือน้อย',
    'รัตนวิทยาคม',
    'อนุบาลรัตนบุรี',
    'บ้านผือ(ประชาพัฒนา)',
    'บ้านหนองกา"ประชารัฐพิทยา"',
    'บ้านโคกล่าม',
    'บ้านนาตุ่น(สหราษฎร์วิทยา)',
    'บ้านหนองบัวทอง',
    'ปทุมมาศวิทยา',
    'บ้านหนองบัวบาน',
    'บ้านขาม',
    'บ้านสร้างบก',
    'บ้านดินแดง(สีลาประชารัฐ)',
    'บ้านดู่',
    'บ้านแคน(คุรุราษฎร์ส่งเสริม)',
    'บ้านนายม(นิยมศึกษาวิทยา)',
    'บ้านแคนน้อย',
    'บ้านโนนจาน',
    'บ้านหนองขุนศรี',
    'บ้านสร้างแก้ว',
    'บ้านนานวน',
    'บ้านหัวนา',
    'บ้านตาเพชร',
    'บ้านหนองบัวแดง',
    'บ้านโนนเปือย',
    'บ้านโพนโก',
    'บ้านเป้า',
    'บ้านโพนดวน',
    'บ้านทัพไทย',
    'สนมศึกษาคาร',
    'บ้านนาศรีสุข',
    'วัดบึงบ้านสนม',
    'บ้านโสกแดง',
    'บ้านสำโรง',
    'บ้านหนองระฆัง',
    'บ้านนาดี',
    'บ้านสะทืด',
    'บ้านหนองอียอ',
    'บ้านอาเลา',
    'บ้านโนนเซียง',
    'ผาแดงวิทยา',
    'บ้านกาพระ',
    'บ้านหนองครก',
    'บ้านหัวงัว"แท่นศิลาวิทยา"',
    'บ้านคำผง',
    'บ้านเซียงซิน - โนนดู่',
    'บ้านม่วงหนองตาด',
    'บ้านอาพืด',
    'บ้านโนน"นิยมศาสตร์ศึกษา"',
    'บ้านขี้เหล็ก',
    'บ้านหนองบัวงาม',
    'บ้านผักไหม',
    'บ้านหนองไม้งาม',
    'บ้านม่วงหวาน',
    'บ้านค้อ',
    'บ้านก้านเหลือง',
    'บ้านระเวียง"รัตนกิจวิทยา"',
    'บ้านซาต(มงคลวิทยา)',
    'บ้านหนองหว้า',
    'บ้านหนองเทพ',
    'บ้านอีโสด',
    'บ้านผำ',
    'บ้านสำโรง',
    'บ้านหนองแวง',
    'บ้านขุมดิน',
    'บ้านหนองหลวง',
    'โนนนารายณ์วิทยา',
    'ชุมพลวิทยาสรรค์',
    'เมืองบัววิทยา',
    'ลำพลับพลาวิทยาคาร',
    'ท่าตูมประชาเสริมวิทย์',
    'ลานทรายพิทยาคม',
    'เมืองแกพิทยาสรรค์',
    'พรมเทพพิทยาคม',
    'ช้างบุญวิทยา',
    'ทุ่งกุลาพิทยาคม',
    'โนนแท่นพิทยาคม',
    'ศรีปทุมวิทยาคม',
    'รัตนบุรี',
    'ดอนแรดวิทยา',
    'เบิดพิทยาสรรค์',
    'ธาตุศรีนคร',
    'แกศึกษาพัฒนา',
    'ทับโพธิ์พัฒนวิทย์',
    'หนองอียอวิทยา',
    'หนองขุนศรีวิทยา',
    'สนมวิทยาคาร',
    'นารายณ์คำผงวิทยา',
    'โนนเทพ',
    'ประดู่แก้วประชาสรรค์',
    'ปราสาทศึกษาคาร',
    'บ้านยาง(สารกิจราษฎร์วิทยา)',
    'บ้านตาเสาะ',
    'บ้านสีโค',
    'ปราสาท',
    'กันทราราม(นุ่นศึกษาคาร)',
    'บ้านทำนบ',
    'อมรินทราวารี',
    'บ้านจบก',
    'บ้านโคกทม',
    'บ้านสวายซอ',
    'บ้านกะดาด',
    'บ้านโคลด',
    'บ้านจรูกแขวะ',
    'บ้านกูน',
    'บ้านรันเดง',
    'บ้านตายัวะ',
    'บ้านตาวร',
    'บ้านกาบกระบือ',
    'นิคมสร้างตนเองปราสาท3',
    'รัฐประชาสามัคคี',
    'บ้านโพธิ์กอง',
    'บ้านเชื้อเพลิง',
    'บ้านโชคนาสาม',
    'บ้านเจ้าคุณ',
    'บ้านมะเมียง',
    'มหาราช 4',
    'บ้านถนนหัก(สุวรรณราษฎร์บำรุง)',
    'บ้านจีกแดก',
    'บ้านหนองปรือ',
    'บ้านนาครอง',
    'บ้านตานี',
    'บ้านโคกจำเริญ',
    'บ้านตาเตียว',
    'บ้านสวาย',
    'บ้านลำพุก',
    'บ้านโชค',
    'บ้านตาเบา',
    'บ้านตะคร้อ',
    'บ้านลำดวน(เมยราษฎร์อุปถัมภ์)',
    'บ้านทมอ(เดื่อราษฎร์บำรุง)',
    'บ้านโคกบุ(บอนประชารัฐบำรุง)',
    'บ้านเจริญสุข',
    'บ้านหนองหรี่',
    'บ้านกำไสจาน',
    'บ้านสะพานหัน',
    'บ้านทุ่งมน(ริมราษฎร์นุสรณ์)',
    'บ้านลำดวนพัฒนา',
    'วันเจริญสามัคคี',
    'บ้านไทร',
    'บ้านอำปึลกง',
    'บ้านพลวง(พรหมบำรุงราษฎร์)',
    'เจริญราษฎร์วิทยา',
    'บ้านพนม',
    'บ้านบุอันโนง',
    'บ้านทนง(รัฐราษฎร์วิทยา)',
    'บ้านปราสาท',
    'บ้านคลอง',
    'นิคมสร้างตนเองปราสาท 1',
    'นิคมสร้างตนเองปราสาท',
    'บ้านละลมระไซ',
    'บ้านบักดอก',
    'บ้านโคกเพชร(เภดประชารัฐบำรุง)',
    'บ้านสองสะโกม',
    'ไพลศึกษาคาร',
    'สุวรรณาคารสงเคราะห์',
    'บ้านสมุด',
    'บ้านหนองยาว',
    'บ้านหนองใหญ่',
    'บ้านปลัด',
    'บ้านสำโรง(บางมด 2514)',
    'บ้านก็วล',
    'บ้านเสกกอง',
    'บ้านลังโกม',
    'บ้านกระวัน',
    'กาบเชิงมิตรภาพที่ 190',
    'บ้านปราสาทเบง',
    'บ้านบักจรัง',
    'บ้านโคกกลางสามัคคี',
    'หวลถวิลวิทยา',
    'บ้านคูตัน',
    'บ้านโคกสะอาด',
    'บ้านหนองโยโคกปืด',
    'บ้านราวนคร',
    'บ้านโคกตะเคียน',
    'บ้านจารย์',
    'เกษตรอีสานสามัคคี',
    'บ้านโนนสวรรค์',
    'บ้านถนนชัย',
    'บ้านโนนทอง',
    'บ้านกู่',
    'บ้านโพนทอง',
    'แสงทองสมบูรณ์วิทยา',
    'บ้านโจรก',
    'บ้านด่าน',
    'บ้านเกษตรถาวร',
    'เรืองเจริญพัฒนา',
    'บ้านสกล',
    'บ้านตะเคียน',
    'นิคมสร้างตนเองปราสาท2',
    'บ้านสระทอง',
    'ประชาสามัคคี',
    'บ้านแนงมุด',
    'บ้านกระเทียม(ราษฎร์วิทยาคม)',
    'บ้านแยงมิตรภาพที่ 146',
    'บ้านโนนสบาย',
    'บ้านถนน',
    'บ้านโคกรัมย์',
    'สหราษฎร์วิทยา',
    'บ้านขอนแตก',
    'บ้านอาวอก',
    'บ้านโตงน้อย',
    'บ้านตอกตรา',
    'หนองโสนวิทยา',
    'สุขพรหมมีศรัทธาญาติ',
    'บ้านศรีมงคล',
    'ดมวิทยาคาร',
    'บ้านสนบ',
    'บ้านเถกิง',
    'บ้านกะปู',
    'บ้านตาคง',
    'จตุคามวิทยา',
    'บ้านห้วยปูน',
    'บ้านสนวน',
    'บ้านขนาดมอญ',
    'บ้านตาแตรวทัพดัด',
    'บ้านปวงตึก',
    'บ้านห้วยสิงห์',
    'บ้านภูมินิยมพัฒนา',
    'สตรีวิทยาสมาคม',
    'บ้านคะนา',
    'บ้านทัพทัน',
    'บ้านเลิศอรุณ',
    'บ้านเสรียง',
    'บ้านร่มเย็น',
    'บ้านลำหาด',
    'บ้านโนนเจริญ',
    'บ้านวังปลัด',
    'บ้านตาพราม',
    'บ้านศาลา',
    'บ้านชำเบง',
    'บ้านลันแต้',
    'บ้านกะเลงเวกทับทิมสยาม',
    'บ้านโคกไทร',
    'บ้านโพธิ์',
    'บ้านอังกอล',
    'บ้านจารย์',
    'บ้านไทยสมบูรณ์(กรป.กลางอุทิศ)',
    'บ้านโพนชาย',
    'บ้านชำสมิง',
    'บ้านศาลาสามัคคี',
    'บ้านพรหมสะอาด',
    'ราษฎร์พัฒนา',
    'บ้านหนองขี้เหล็ก',
    'บ้านพระแก้ว',
    'บ้านกระสัง',
    'บ้านแสนกาง',
    'สหมิตรวิทยา',
    'บ้านสะกาด',
    'บ้านตาโมม',
    'บ้านจังเอิล',
    'บ้านหลัก',
    'จัดสรรที่ดินสงเคราะห์',
    'สังขะวิทยาคม',
    'บ้านสังขะ',
    'บ้านศรีนวล',
    'บ้านโดง',
    'บ้านโชค',
    'บ้านจรัส',
    'กลาโหมราชเสนา 2',
    'บ้านโอทะลัน',
    'บ้านตาวัง',
    'บ้านหนองเหล็ก',
    'บ้านนา',
    'บ้านจบก',
    'บ้านหนองโจงโลง',
    'บ้านบัวขุนจง(กรป.กลางอุปถัมภ์)',
    'บ้านออดราษฎร์สามัคคี',
    'บ้านบัวเชด',
    'บ้านระมาดค้อ',
    'บ้านโนนสังข์',
    'บ้านสะเดา',
    'บ้านสน',
    'บ้านสำเภาลูน',
    'บ้านไทยเดิม',
    'บ้านโคกสมอง',
    'บ้านสวาท',
    'บ้านอาโพน',
    'บ้านสะแร',
    'บ้านโคกกลาง',
    'บ้านสระแก้ว',
    'บ้านโคกโบสถ์',
    'บ้านโคกกรม',
    'บ้านตาลวก',
    'บ้านศรีสวาย',
    'บ้านจีกแดก',
    'บ้านละเอาะ',
    'บ้านพนมดิน',
    'บ้านตาเมียง',
    'บ้านหนองคันนา',
    'บ้านหนองจูบ',
    'บ้านอำปึล',
    'บ้านรุน',
    'นิคมสร้างตนเองปราสาท 4',
    'บ้านไทยสันติสุข',
    'บ้านไทยนิยมพัฒนา',
    'บ้านอุโลก',
    'บ้านเกาะแจนแวน',
    'บ้านแดง',
    'บ้านตะโนน',
    'บ้านสำโรง',
    'บ้านพะเนาว์',
    'ศรีณรงค์วิทยาคาร',
    'บ้านละมงค์',
    'รัฐราษฎร์พัฒนา',
    'บ้านมหาชัย',
    'บ้านโสน',
    'บ้านณรงค์',
    'บ้านตรวจ',
    'บ้านฉลีก',
    'บ้านเตาแดก',
    'บ้านโนนทอง',
    'บ้านเกาะตรวจ',
    'บ้านหนองหว้า',
    'บ้านน้อย',
    'ไพลอำนวยวิทย์',
    'บ้านกล้วย',
    'พระจันทร์ศรีสุข',
    'บ้านขยูงทองยางภิรมย์',
    'วิทยาราษฎร์นุกูล',
    'บ้านหนองแวง',
    'บ้านกุง',
    'บ้านคูขาด',
    'บ้านตราด',
    'ประสาทวิทยาคาร',
    'โคกยางวิทยา',
    'ตานีวิทยา',
    'ไทรแก้ววิทยา',
    'ทุ่งมนวิทยาคาร',
    'เชื้อเพลิงวิทยา',
    'ตาเบาวิทยา',
    'กาบเชิงวิทยา',
    'แนงมุดวิทยา',
    'โคกตะเคียนวิทยา',
    'ปราสาทเบงวิทยา',
    'สังขะ',
    'กระเทียมวิทยา',
    'ตาคงวิทยา รัชมังคลาภิเษก',
    'ขนาดมอญพิทยาคม',
    'มัธยมทับทิมสยาม 04',
    'พระแก้ววิทยา',
    'เทพอุดมวิทยา',
    'บัวเชดวิทยา',
    'มัธยมศรีลำเภาลูน',
    'พนมดงรักวิทยา',
    'ศรีณรงค์พิทยาลัย',
    'โสตศึกษาจังหวัดสุรินทร์',
    'บ้านคูซอด',
    'บ้านเวาะ',
    'บ้านเปือย',
    'บ้านจานหนองคู',
    'บ้านดงบัง(คุรุราษฎร์พัฒนา)',
    'บ้านฮ่องขาดหนองตะเคียน',
    'บ้านแดงเหล่ายอด',
    'บ้านหนองแคน(คุรุราษฎร์สามัคคี)',
    'บ้านแทง',
    'หนองหว้าป่าสะแบง',
    'บ้านซำโพธิ์',
    'บ้านตะดอบ',
    'บ้านเปือยนาสูง',
    'บ้านหนองแคนหนองเทา',
    'บ้านหนองม่วงหนองแวง',
    'บ้านหางว่าวโนนบัว',
    'บ้านฮ่องแข้ดำ',
    'บ้านทุ่ม',
    'บ้านขมิ้น',
    'บ้านเสือบอง',
    'บ้านโนนแกด',
    'บ้านน้ำคำ',
    'บ้านหนองโนวิทยา',
    'บ้านง้อหนองแสง',
    'บ้านหนองคำ',
    'บ้านโนนหุ่งหนองหว้าน้ำท่วม',
    'บ้านหนองตะมะพันทาโนนกอง(ธนาคารกรุงเทพ 2 )',
    'บ้านโพธิ์โนนจานอีลอก',
    'มิ่งเมือง',
    'บ้านบก',
    'บ้านโพนข่า(วินิตวิทยาคาร)',
    'บ้านกุดโง้ง(สิริราษฎร์บำรุง)',
    'บ้านหนองแวงโพนเขวา',
    'บ้านหนองเข็งเหล่าแค',
    'บ้านบ้านดอนสั้น',
    'บ้านโพนค้อ',
    'อนุบาลวัดพระโต',
    'บ้านหญ้าปล้อง',
    'บ้านโนนสำนักมิตรภาพที่ 121',
    'บ้านโนนแย้',
    'บ้านเอกสร้างเรือง',
    'มหาราช 3',
    'บ้านโนนอีปังโพนวัว',
    'บ้านหนองหว้าเข้ากรรม',
    'บ้านหนองแก้วสำโรง',
    'บ้านโนนดั่ง(คุรุราษฎร์รังสรรค์)',
    'บ้านหนองครก',
    'บ้านหนองโพธิ์',
    'หนองสาดโนนเจริญ',
    'บ้านหนองสวง',
    'บ้านดอนกลาง',
    'บ้านหนองยาง',
    'อนุบาลศรีสะเกษ',
    'บ้านหนองไผ่',
    'บ้านเพียนาม',
    'บ้านหอยสะเดาพัฒนา',
    'บ้านก่อโนนหล่อง',
    'บ้านโพนแดง',
    'บ้านแก',
    'จินดาวิทยาคาร 3',
    'บ้านหมากเขียบ',
    'บ้านก้านเหลือง',
    'นิคมสร้างตนเองห้วยคล้า1',
    'บ้านยางกุดนาคำ',
    'บ้านยางเครือ',
    'บ้านกุดเมืองฮาม(ศิริราษฎร์สงเคราะห์)',
    'บ้านแก้ง',
    'บ้านคอนกาม',
    'บ้านค้อเมืองแสน',
    'บ้านโนนคูณ',
    'บ้านหอย-โนนดู่',
    'บ้านโนนติ้ว',
    'บ้านบอน',
    'บ้านจอมวิทยา',
    'บ้านโนน(รัฐประชาสงเคราะห์)',
    'อนุบาลยางชุมน้อย(หน่วยคุรุราษฎร์รังสรรค์)',
    'บ้านยางชุมใหญ่',
    'บ้านยางชุมใหญ่สาขาบ้านดอนโก',
    'บ้านลิ้นฟ้า',
    'บ้านดินดำ',
    'บ้านผักขะ',
    'บ้านหนองอีกว่าง',
    'บ้านอีต้อม',
    'บ้านมะกรูด',
    'บ้านคำเนียม(คำเนียมวิทยาคาร)',
    'บ้านหนองทามใหญ่',
    'บ้านจาน',
    'บ้านหนองหญ้าปล้อง',
    'บ้านอาลัย',
    'บ้านบูรพา',
    'บ้านพะแนงวิทยา',
    'บ้านผักบุ้ง',
    'บ้านคล้อ มิตรภาพที่ 226',
    'บ้านหนองถ่ม(คุรุราษฎร์อนุสรณ์)',
    'บ้านดู่',
    'บ้านดูนสิม',
    'บ้านหนองมะแซว',
    'อนุบาลกันทรารมย์',
    'บ้านหมัด(คุรุรัฐสงเคราะห์)',
    'บ้านทาม',
    'บ้านผึ้ง(มธุลีย์ประชาสรรค์)',
    'บ้านเจี่ย',
    'บ้านโนนผึ้ง',
    'บ้านหนองหวาย',
    'บ้านโนนสัง(โนนสังวิทยาคาร)',
    'บ้านขี้เหล็ก(อสพป.40)',
    'บ้านหนองบัวไชยวาน',
    'บ้านหนองโอง',
    'บ้านพันลำ',
    'บ้านแวงโนนเปือยหนองแสง',
    'บ้านเทิน',
    'บ้านบัวน้อยโนนปอ',
    'บ้านขาม',
    'บ้านจิกกะลา',
    'บ้านบกบ่าง',
    'บ้านเกาะ',
    'บ้านนาดี',
    'บ้านโนนสะอาดอีตู้',
    'บ้านผักแพว(คุรุราษฎร์รังสฤษฎ์)',
    'บ้านเมืองน้อยหนองมุข',
    'บ้านหนองน้ำเต้าโนนสวน',
    'บ้านหนองเทา',
    'บ้านโคก',
    'บ้านบ้านยาง',
    'บ้านหนองกี่',
    'บ้านกล้วย',
    'บ้านสร้างเหล่าโนนแดง',
    'บ้านหนองกก',
    'บ้านโพธิ์ลังกา',
    'บ้านละทาย',
    'บ้านกอก',
    'บ้านเขวา',
    'บ้านเหม้าหนองเรือ',
    'บ้านยางน้อยสามัคคี',
    'บ้านกอกหัวนา',
    'บ้านเปือย',
    'บ้านหนองแก้ว(ราษฎร์ผดุงวิทยา)',
    'บ้านหนองบัวท่าช้าง',
    'บ้านโพนทราย โนนเรือ',
    'บ้านสีถาน',
    'บ้านขามป้อม',
    'บ้านหนามแท่ง',
    'บ้านหนองแวง(โสวรรณีวิทยาคม)',
    'บ้านแสงใหญ่(คุรุราษฎร์บำรุง)',
    'บ้านหนองดุมหนองม่วง',
    'บ้านลือชัย',
    'บ้านทุ่งพาย',
    'บ้านหนองถ่ม',
    'บ้านหนองหัวช้าง',
    'บ้านบกขี้ยาง',
    'บ้านเลิงแฝก',
    'บ้านทุ่งมั่ง',
    'บ้านหนองไฮ',
    'บ้านอีปาด',
    'อนุบาลโนนคูณ',
    'บ้านโนนคูณ',
    'หนองจิกเหล่าเชือก',
    'บ้านหนองสามขา',
    'บ้านหนองมะเกลือ',
    'บ้านเท่อเล่อ',
    'บ้านร่องเก้า',
    'บ้านบกแดงผักขะย่า',
    'บ้านดินดำเหล่าเสนใต้',
    'บ้านโปร่ง',
    'บ้านหัวเหล่า',
    'บ้านโนนสว่าง',
    'บ้านโพนงาม',
    'บ้านผักขย่าใหญ่',
    'บ้านหนองปลาเข็ง-กอไหล่',
    'บ้านปลาข่อ',
    'บ้านโพธิ์(โพธิ์ทองวิทยา)',
    'บ้านหนองแวงสาขาบ้านโนนรัง',
    'บ้านหนองแวง',
    'บ้านโนนดู่',
    'บ้านม่วงเป',
    'บ้านโคกสะอาด',
    'บ้านหนองหิน',
    'บ้านโนนสมบูรณ์',
    'บ้านหนองกุงสนามชัย',
    'บ้านก้อนเส้า',
    'บ้านหนองสนม',
    'บ้านเวาะวิทยาคม',
    'บ้านหยอด',
    'บ้านนาม่องนาเมือง',
    'บ้านหนองตลาด',
    'บ้านเหล่าฝ้าย',
    'บ้านโนนหนองสิม',
    'อนุบาลน้ำเกลี้ยง',
    'บ้านโนนสว่าง',
    'บ้านหนองแวง',
    'บ้านสะพุง',
    'บ้านคูบ',
    'บ้านสะเต็ง',
    'บ้านสวนกล้วย',
    'บ้านยางน้อยตองปิด',
    'บ้านกะวัน',
    'บ้านลุมพุกคูวงศ์',
    'บ้านบัวระรมย์',
    'บ้านลุมภู',
    'บ้านหนองนาเวียง',
    'บ้านหนองบาง',
    'บ้านน้ำเกลี้ยง',
    'บ้านรุ่ง',
    'บ้านหนองพะแนง',
    'บ้านสบาย',
    'บ้านโนนงาม',
    'บ้านหนองแลงระไง',
    'บ้านทุ่งสว่าง',
    'บ้านขี้เหล็ก',
    'บ้านละเอาะ',
    'บ้านแวด',
    'บ้านดวนใหญ่(ลีราษฎร์วัฒนา)',
    'บ้านโนนดู่',
    'บ้านหนองนาโพธิ์',
    'บ้านสว่าง',
    'บ้านเจ้าทุ่ง',
    'บ้านโนนสำโรงหนองหมากแซว',
    'บ้านลิงไอ',
    'บ้านหนองกันจอ',
    'บ้านดงยาง',
    'บ้านหนองคูโนนแก้ว',
    'บ้านธาตุ(สหพัฒนานุสรณ์)',
    'นิคมสร้างตนเองห้วยคล้า 2',
    'นิคมสร้างตนเองห้วยคล้า 3',
    'บ้านหนองบัว',
    'บ้านสร้างสะแบง',
    'อนุบาลวังหิน',
    'บ้านกะเอิน',
    'บ้านโพนดวน',
    'บ้านหัววัวหนองนารี',
    'บ้านขุมคำ(สำนักงานสลากกินแบ่งสงเคราะห์ 673)',
    'บ้านโนนสายหนองหว้า',
    'บ้านไทยบวกแต้บวกเตย',
    'บ้านโพนยาง',
    'บ้านทุ่งน้อย',
    'บ้านเห็นอ้ม',
    'บ้านหนองตาเชียง',
    'บ้านทุ่ง',
    'บ้านสร้างบาก',
    'บ้านสะมัด',
    'บ้านหนองคู',
    'บ้านหนองสังข์',
    'บ้านตีกา',
    'บ้านหนองทุ่ม',
    'บ้านตำแยหนองเม็ก',
    'บ้านกระแชง(คุรุราษฎร์นุกูล)',
    'บ้านกระถุน',
    'บ้านขนวนจานสามัคคี',
    'บ้านอะลางหัวขัว',
    'บ้านโนนเพ็ก',
    'บ้านสร้างหว้า',
    'บ้านค้อยางปอ(อสพป.35)',
    'บ้านโพธิ์ศรี(คุรุราษฎร์อินทรสุภาผล)',
    'บ้านโคกเพ็ก',
    'อนุบาลพยุห์',
    'บ้านคูเมือง',
    'บ้านหนองรัง',
    'บ้านหนองหว้าทับทัย',
    'บ้านหนองทุ่ม',
    'บ้านหนองม่วงหนองแต้',
    'บ้านเปือย',
    'บ้านสำโรง',
    'บ้านกระหวัน',
    'บ้านร่องสะอาด',
    'บ้านป่าไร่',
    'บ้านโนนสว่าง(อสพป.17)',
    'บ้านหนองเตย',
    'บ้านเสมอใจ',
    'บ้านหนองค้า',
    'บ้านหนองออ',
    'ศรีสะเกษวิทยาลัย',
    'สตรีสิริเกศ',
    'คูซอดประชาสรรค์',
    'ไกรภักดีวิทยาคม',
    'สิริเกศน้อมเกล้า',
    'น้ำคำวิทยา',
    'เฉลิมพระเกียรติสมเด็จพระศรีนครินทร์ ศรีสะเกษ',
    'มัธยมโพนค้อ',
    'ศรีสะเกษวิทยาลัย 2  ห้วยคล้า',
    'ราชประชานุเคราะห์ 29',
    'ยางชุมน้อยพิทยาคม',
    'ลิ้นฟ้าพิทยาคม',
    'กันทรารมณ์',
    'ละทายวิทยา',
    'บัวน้อยวิทยา',
    'ผักแพววิทยา',
    'แวงแก้ววิทยา',
    'กระดุมทองวิทยา',
    'ประสานมิตรวิทยา',
    'หนองถ่มวิทยา',
    'โนนค้อวิทยาคม',
    'โคกสะอาดวิทยาคม',
    'บกวิทยาคม',
    'เขื่อนช้างวิทยาคาร',
    'น้ำเกลี้ยงวิทยา',
    'บัวเจริญวิทยา',
    'นครศรีลำดวนวิทยา',
    'หนองทุ่มศรีสำราญวิทยา',
    'โพธิ์ธาตุประชาสรรค์',
    'โนนเพ็กวิทยาคม',
    'พยุห์วิทยา',
    'บ้านเชือก',
    'บ้านจิก',
    'บ้านมะฟัก',
    'บ้านหนองบ่อ',
    'บ้านด่าน(รัฐราษฎร์พัฒนา)',
    'บ้านดงแดง',
    'บ้านดู่ค้อ',
    'บ้านกระเดาอุ่มแสง',
    'บ้านครั่ง',
    'บ้านกอย',
    'บ้านท่าบ่อ',
    'ส้มป่อย(ส้มป่อยวิทยาเสริม)',
    'บ้านดอนไม้งาม',
    'บ้านโก',
    'บ้านบึงหมอก',
    'บ้านบัวหุ่ง(รัฐราษฎร์สามัคคี)',
    'บ้านหนองกก',
    'บ้านอีหนา(รัฐประชานุกูล)',
    'บ้านห้วย',
    'บ้านหัวขัวนาแปะหนองอึ่ง',
    'บ้านโนนตุ่น',
    'บ้านไผ่',
    'บ้านเมี่ยงแคนดวน',
    'บ้านคูสระ',
    'เมืองคง(คงคาวิทยา)',
    'อนุบาลราษีไศล',
    'บ้านบากเรือ',
    'บ้านแสงเมืองแคน',
    'บ้านยาง',
    'บ้านท่า',
    'บ้านสร้างปี่',
    'บ้านด่านนอกดง',
    'จินดาวิทยาคาร 2(บ้านหนองศาลา)',
    'บ้านหนองสวง(อสพป.2)',
    'บ้านดอนงูเหลือม',
    'บ้านปลาขาว',
    'บ้านมะยาง',
    'บ้านหนองแค',
    'บ้านเพียมาต(รัฐราษฎร์พิทยาคาร)',
    'บ้านตัง',
    'บ้านเห็ดผึ้งหนองก่าม',
    'บ้านแสนแก้วหนองคูไซ',
    'บ้านหนองหมี(ประชาวิทยาคาร)',
    'บ้านโต่งโต้น',
    'บ้านฮ่องข่า',
    'บ้านโกทา',
    'บ้านเปือยขาม',
    'บ้านหนองโง้ง',
    'บ้านหว้าน(รัฐราษฎร์นุกูล)',
    'บ้านน้ำอ้อมน้อย',
    'บ้านก้านเหลือง(คุรุราษฎร์วัฒนา)',
    'บ้านขนวนสีแก้ว',
    'บ้านอ้อมแก้ว',
    'บ้านหนองเหล็กธาตุน้อย',
    'บ้านกงพาน',
    'บ้านหนองคู (อ.ส.พ.ป.23)',
    'อนุบาลอุทุมพรพิสัย',
    'สระกำแพงวิทยาคม',
    'บ้านขะยูง(ขยูงศิลป์ศึกษา)',
    'บ้านกลาง(ประชาคมวิทยา)',
    'บ้านหนองยาง',
    'บ้านโพนเมือง',
    'บ้านแข้',
    'บ้านโนนกลาง',
    'บ้านอะลาง',
    'บ้านโนนสูง',
    'บ้านแขมโพนทอง',
    'บ้านโนนสายขมิ้นหนองแมว',
    'บ้านโคก',
    'บ้านโปร่งสามัคคี',
    'บ้านเหงี่ยงกระจ๊ะ',
    'บ้านโนนน้อย',
    'บ้านโคกหล่าม',
    'บ้านหนองลุง',
    'บ้านหนองเหล็ก',
    'บ้านยางบอน(อำพานุสรณ์)',
    'บ้านทุ่งสว่าง(ราษฎร์พัฒนา)',
    'บ้านแต้"ประชาบำรุง"',
    'บ้านกุง',
    'บ้านโนนแดงโนนม่วง',
    'บ้านทุ่งไชย(พัฒนานุสรณ์)',
    'บ้านขวาว',
    'บ้านปะหละ',
    'บ้านปะอาว',
    'บ้านพลับ',
    'บ้านโนนใหญ่(คุรุราษฎร์พัฒนา)',
    'บ้านบอนใหญ่',
    'บ้านสะเดา',
    'บ้านผึ้ง(รัฐราษฎร์บำรุง)',
    'บ้านค้อโนนเพ็ก',
    'บ้านขี้เหล็ก',
    'บ้านสระภู',
    'บ้านค้อกำแพง',
    'บ้านหนองหัวหมู',
    'บ้านสำโรง(วิทยาราษฎร์)',
    'บ้านส้มป่อยศรีวิศรมิตรภาพที่ 161',
    'บ้านยางเอือด',
    'บ้านตลาดพัฒนา',
    'บ้านหนองห้าง',
    'บ้านพงพรต',
    'บ้านฟ้าผ่า',
    'บ้านสำโรงน้อยหนองบัว',
    'บ้านก่อ(ราษฎร์นุกูล)',
    'บ้านนาโนน',
    'บ้านโนนเค็ง',
    'บ้านหัวช้าง(ราษฎร์บำรุง)',
    'บ้านหนองสำโรงน้อย',
    'บ้านหอย',
    'บ้านโนนดู่ (รัฐราษฎร์อุปถัมภ์)',
    'บ้านอีหล่ำ(ราษฎร์นุสรณ์)',
    'บ้านโคกกลาง(ประชารัฐพัฒนา)',
    'บ้านหมากยาง',
    'บ้านม่วง',
    'บ้านค้อ',
    'ชุมชนบ้านหนองคู',
    'บ้านหาด(อสพป.13)',
    'บ้านโนนลาน',
    'อนุบาลบึงบูรพ์',
    'บ้านกล้วยกว้าง',
    'บ้านบุยาว',
    'บ้านไพรพะยอม',
    'บ้านพอกหนองแข้',
    'บ้านไฮใหญ่',
    'บ้านพะวร',
    'บ้านนานวนหนองแคน',
    'บ้านพงสิม',
    'บ้านจานแสนไชย',
    'บ้านหนองอาคูณ',
    'บ้านปราสาท',
    'บ้านขะยุง',
    'บ้านหว้า',
    'บ้านปะโด๊ะ',
    'บ้านอีสร้อย',
    'บ้านนาทุ่ง',
    'บ้านตาทอง',
    'บ้านกระสังข์',
    'บ้านกระเต็ล',
    'บ้านเมืองหลวง',
    'บ้านจังเอิน',
    'บ้านหนองน้ำขุ่น',
    'บ้านโทะ',
    'บ้านหนองสะมอน',
    'บ้านเมืองน้อย',
    'บ้านหนองสิม',
    'บ้านสร้างเรือ',
    'อนุบาลห้วยทับทัน',
    'บ้านตาโกน',
    'บ้านหนองปลาคูณ',
    'บ้านห่อง',
    'บ้านแต้ชะบา ( อสพป.38 )',
    'บ้านเมืองจันทร์',
    'บ้านเก็บงา(ประชารัฐพัฒนา)',
    'บ้านหนองแคน',
    'บ้านโคก',
    'บ้านทุ่ม',
    'อนุบาลเมืองจันทร์(บ้านปลาซิว)',
    'บ้านหนองดุมหนองแคนน้อย',
    'บ้านเขวา',
    'บ้านโดด',
    'อนุบาลโพธิ์ศรีสุวรรณ',
    'บ้านหนองเหล็กโนนหนองหว้า',
    'บ้านหนองหว้า',
    'บ้านหนองหงอก',
    'บ้านผือ',
    'บ้านหนองแปน',
    'บ้านตลาดหนองเรือ',
    'บ้านเสียว',
    'หนองผือ',
    'บ้านสามขา',
    'บ้านจาน',
    'บ้านหนองม้า',
    'บ้านโซงเลง',
    'บ้านอีเซ(คุรุราษฎร์วิทยา)',
    'เบญจคามวิทยาประชาสรรค์',
    'อนุบาลศิลาลาด',
    'บ้านแต้มะหลี่',
    'บ้านกุงขาม',
    'บ้านเกิ้ง(อสพป.37)',
    'บ้านเมืองเก่าซ่ง(รัฐราษฎร์รังสรรค์)',
    'บ้านโจดม่วง',
    'บ้านดอนทัพควายครั่งน้อยโนนชาด',
    'บ้านคลีกลิ้ง',
    'บ้านโพธิ์',
    'บ้านโจดนาห่อมเดื่อ',
    'บ้านโนนเหมือดแอ่บัวหล่น',
    'บ้านหนองบัวดง(รัฐราษฎร์วิทยาคม)',
    'บ้านหลักด่านหนองจอก',
    'บ้านดงเค็งตู้',
    'บ้านทัพส่วยหนองพอก',
    'ส้มป่อยพิทยาคม',
    'ไผ่งามพิทยาคม',
    'จิกสังข์ทองวิทยา',
    'ราษีไศล',
    'กำแพง',
    'ประชาพัฒนศึกษา',
    'ด่านอุดมศึกษา',
    'ทุ่งไชยพิทยา รัชมังคลาภิเษก',
    'ทุ่งสิมวิทยาคม',
    'เมืองแคนวิทยาคม',
    'จตุรภูมิพิทยาคาร',
    'หวายคำวิทยา',
    'โคกหล่ามวิทยา',
    'เบญจประชาสรรค์',
    'บึงบูรพ์',
    'ห้วยทับทันวิทยาคม',
    'ผักไหมวิทยานุกูล',
    'วรคุณอุปถัมภ์',
    'เมืองจันทร์วิทยาคม',
    'ไตรมิตร',
    'โพธิ์ศรีสุวรรณวิทยาคม',
    'คลีกลิ้งพัฒนาทร',
    'ศิลาลาดวิทยา',
    'บ้านกฤษณา',
    'บ้านสวัสดี',
    'บ้านป่าใต้',
    'บ้านกันทรารมย์',
    'บ้านโคกสูง',
    'บ้านระกา',
    'บ้านโคกเพชร',
    'บ้านเปี่ยมตะลวก',
    'บ้านภูมิศาลา',
    'บ้านโนนสำราญ',
    'บ้านเสลา',
    'บ้านปะอุง',
    'บ้านเค็ง',
    'บ้านตาสุด',
    'บ้านจะกง',
    'บ้านใจดี',
    'บ้านทะลอก',
    'บ้านพะเยียวตาสุ(อสพป.37)',
    'บ้านอังกุล',
    'บ้านกระโพธิ์ช่างหม้อ',
    'บ้านตรางสวาย',
    'บ้านดองกำเม็ด',
    'บ้านกันจาน',
    'บ้านตะเคียนช่างเหล็ก',
    'บ้านกะกำ',
    'บ้านบัวบก',
    'บ้านตาอุด',
    'บ้านปราสาทกวางขาว',
    'นิคมสร้างตนเองปรือใหญ่',
    'นิคม 2(ต.ช.ด.สงเคราะห์)',
    'บ้านปราสาท',
    'บ้านหนองสะแกสน',
    'บ้านบ่อทอง',
    'บ้านสกุล',
    'บ้านคลองเพชรสวาย',
    'บ้านกันโทรกประชาสรรค์',
    'บ้านปรือคัน',
    'บ้านปรือใหญ่',
    'บ้านมะขาม',
    'บ้านหลัก',
    'บ้านโนนสมบูรณ์(อสพป.25)',
    'ทับทิมสยาม 06',
    'บ้านทุ่งศักดิ์',
    'บ้านหนองกาด',
    'บ้านจันลม',
    'บ้านโนนใหม่(ประชาอุปถัมภ์)',
    'บ้านเคาะ',
    'บ้านละเบิกตาฮีง',
    'บ้านคล้อโคกกว้าง',
    'ศรีสะอาดวิทยาคม',
    'บ้านตะเคียนบังอีง',
    'บ้านใหม่',
    'บ้านสะเดาใหญ่',
    'บ้านติม(พันธ์พิทยาคม)',
    'บ้านเขวิก',
    'บ้านโพง',
    'บ้านสระบานสามัคคี',
    'บ้านสำโรงตาเจ็น',
    'บ้านศาลาประปุน',
    'บ้านโนนดู่',
    'บ้านเริงรมย์',
    'บ้านโสน',
    'บ้านอาวอย',
    'บ้านขนุน(วันธรรมศาสตร์ศรีสะเกษ 2515)',
    'บ้านสวาย',
    'บ้านหนองคล้า',
    'บ้านคำเผีอ',
    'บ้านสนามสามัคคี โรตารี่ 2',
    'นิคม 3(กรมประชาสงเคราะห์)',
    'นิคม 4(กรมประชาสงเคราะห์)',
    'บ้านตรอย',
    'บ้านตาดม',
    'บ้านสมบูรณ์',
    'บ้านกันแตสระรุน',
    'บ้านแขว',
    'บ้านคะนาสามัคคี',
    'บ้านยางชุมภูมิตำรวจ',
    'บ้านแทรง',
    'บ้านนาก๊อก',
    'อนุบาลศรีประชานุกูล',
    'ขุขันธ์วิทยา',
    'บ้านชำแระกลาง',
    'บ้านบก(ประชาสามัคคี)',
    'บ้านสะอาง(ประชาสามัคคี)',
    'วัดเขียน',
    'บ้านเรียม',
    'บ้านคลองสุด(ประชาอุทิศ)',
    'บ้านหัวเสือ',
    'บ้านสวงษ์พัฒนศึกษา',
    'บ้านห้วยสระภูมิ',
    'บ้านดองดึง',
    'บ้านดินแดง',
    'บ้านสร้างใหญ่',
    'หนองอารีพิทยา',
    'บ้านกันตรวจ',
    'บ้านหนองระเยียว',
    'บ้านตาเจา',
    'บ้านแดง',
    'บ้านคูสี่แจ',
    'บ้านกันตรุม',
    'บ้านปราสาทเยอ',
    'บ้านหนองพัง',
    'วัดบ้านประอาง(ประสาธน์คุรุราษฎร์พัฒนา)',
    'วัดบ้านประอางสาขาบ้านเตรี๊ยะ',
    'บ้านติ้ว',
    'บ้านมะขามภูมิ',
    'บ้านทุ่ม(อสพป.3)',
    'บ้านกระแมด',
    'บ้านโพง',
    'อนุบาลไพรบึง',
    'บ้านสวาย-สนวน',
    'บ้านคอกหนองไพร',
    'บ้านตราด',
    'บ้านพราน',
    'บ้านผือ',
    'บ้านสำโรงพลัน',
    'บ้านไม้แก่น',
    'บ้านปุดเนียม',
    'บ้านหัวช้าง',
    'บ้านหัวช้างสาขาบ้านฮ่องสามัคคี',
    'บ้านชำแระ',
    'บ้านตาจวน',
    'บ้านไทร',
    'บ้านสะเดาน้อย',
    'บ้านโป่ง',
    'บ้านสลักได',
    'บ้านพะแวะ',
    'บ้านหนองอิไทย',
    'บ้านอาลัย',
    'บ้านโพนปลัด',
    'บ้านเขวา',
    'บ้านกู่',
    'บ้านไฮน้อย',
    'บ้านกะดึ',
    'บ้านพอก',
    'บ้านเกาะกระโพธิ์',
    'บ้านหนองบัวตาคง',
    'บ้านสามขา',
    'บ้านหว้า',
    'บ้านขามหนองครอง',
    'บ้านหนองคูอาวอย',
    'บ้านดู่',
    'บ้านหนองแวง',
    'บ้านตะเภา',
    'บ้านตูม',
    'บ้านขี้นาค',
    'บ้านบึงกระโพธิ์',
    'อนุบาลปรางค์กู่',
    'บ้านสนาย',
    'บ้านโนนดั่ง',
    'บ้านขามฆ้อง',
    'บ้านโป่ง',
    'บ้านไฮ',
    'บ้านโพธิ์สามัคคี',
    'บ้านเหล็ก',
    'บ้านนาวา',
    'บ้านกอกหวาน',
    'บ้านบัลลังก์',
    'บ้านสมอ',
    'บ้านหนองเพดาน',
    'บ้านกระต่ายด่อน',
    'บ้านดอนหลี่',
    'บ้านกุดปราสาท',
    'บ้านสวายสนิท',
    'บ้านขามทับขอน',
    'บ้านท่าคอยนาง',
    'บ้านสำโรง',
    'บ้านตาเปียง',
    'บ้านไฮเลิง',
    'บ้านหว้าน',
    'บ้านขอนแต้',
    'บ้านหนองเชียงทูน',
    'บ้านกำแมด',
    'บ้านศาลา',
    'บ้านหนองระนาม',
    'บ้านมัดกา',
    'บ้านหนองคูขาม',
    'บ้านบ่อ',
    'บ้านโคกตาล',
    'บ้านลุมพุกคลองแก้ว',
    'บ้านศาลา',
    'บ้านเรือทองคลองคำ',
    'บ้านนาตราว',
    'บ้านจำปานวง',
    'บ้านแซรสะโบว',
    'บ้านตาโสม',
    'บ้านตะเคียนราม',
    'วนาสวรรค์',
    'บ้านไพรพัฒนา',
    'บ้านแซรไปร',
    'บ้านละลม',
    'บ้านธาตุพิทยาคม',
    'บ้านขะยูง(โนนเจริญศึกษา)',
    'บ้านพรหมเจริญ',
    'บ้านไทยถาวรโคกแดงประสานมิตร',
    'บ้านทำนบ',
    'บ้านห้วยตามอญ',
    'บ้านพนมชัย',
    'อนุบาลภูสิงห์',
    'บ้านโคกใหญ่',
    'บ้านตาเม็ง(อสพป.9)',
    'บ้านนกยูง(อสพป.30)',
    'บ้านทุ่งหลวง',
    'เคียงศิริบ้านโพธิ์ทอง',
    'ปรางค์กู่',
    'ขุขันธ์',
    'ไพรบึงวิทยาคม',
    'โนนปูนวิทยาคม',
    'ปรือใหญ่วิทยบัลลังก์',
    'ลมศักดิ์วิทยาคม',
    'กุญชรศิรวิทย์',
    'โนนกระสังวิทยาคม',
    'ศรีตระกูลวิทยา',
    'ตูมวิทยา',
    'สวงษ์วิทยาคม',
    'หนองคูวิทยา',
    'สะเดาใหญ่ประชาสรรค์',
    'สวายพิทยาคม',
    'พอกพิทยาคม รัชมังคลาภิเษก',
    'ละลมวิทยา',
    'ภูสิงห์ประชาเสริมวิทย์',
    'บ้านไฮ(วันครู 2503)',
    'บ้านเขวา (ราษฎรพัฒนา)',
    'บ้านโนนสูง',
    'บ้านระโยง',
    'บ้านจำนรรจ์',
    'บ้านโนนจิก',
    'บ้านกระแชงใหญ่',
    'บ้านชำเบ็ง',
    'บ้านหนองทา',
    'บ้านกุดเสลาหนองขวาง',
    'บ้านนารังกา',
    'บ้านกันจาน',
    'บ้านซะวาซอ',
    'บ้านโนนคูณ',
    'บ้านโนนงาม',
    'บ้านสามแยก-หินกอง',
    'บ้านหนองรุงพระทะเล',
    'บ้านขนุน',
    'บ้านตาเครือ',
    'บ้านโตนด',
    'บ้านนาขนวน',
    'บ้านหนองหัวช้าง',
    'บ้านโนนสมบูรณ์',
    'บ้านจานทองกวาววิทยา',
    'บ้านตาลอย-หนองคัน',
    'บ้านชำโพธิ์ตาเกษ(ประจวบสงเคราะห์)',
    'มหาราช 2 (ภูคำ)',
    'บ้านไร่เจริญ',
    'บ้านชำแจงแมง',
    'ชนะใช้กิจการ',
    'บ้านชำม่วง',
    'บ้านตระกาศขอนแก่น',
    'บ้านโคกวิทยา',
    'บ้านทุ่งกอกหนองเกาะ',
    'บ้านม่วง(รามคำแหงอนุสรณ์)',
    'บ้านศรีอุดม-ซำตารมย์',
    'บ้านร่องตาซุน',
    'บ้านกะมอล(กรป.กลางประชาอุทิศ)',
    'บ้านทุ่งใหญ่(คุรุราษฎร์บำรุง)',
    'บ้านโคก(อสพป.32)',
    'บ้านปะทาย',
    'บ้านสว่าง',
    'อนุบาลดำรงราชานุสรณ์',
    'บ้านขะยูง',
    'บ้านขนา',
    'บ้านโนนน้ำอ้อม',
    'บ้านโนนสำราญ',
    'บ้านท่าสว่าง',
    'บ้านจันทร์หอม-ตาเสก',
    'บ้านรุ่งอรุณ(กองทัพบกอุปถัมภ์)',
    'บ้านท่าสว่างสาขาบ้านกระหวัน',
    'บ้านบึงมะลู',
    'บ้านน้ำขวบ-โนนดู่(กองทัพบกอุปถัมภ์)',
    'บ้านตาแท่น',
    'บ้านโนนเยาะ-โนนศิริ',
    'บ้านโนนแสนคำหนองศาลา(กรป.กลางราษฎร์พัฒนา',
    'บ้านหนองตลาด-โนนเปือย',
    'บ้านนาไพรงาม',
    'บ้านภูเงิน(อินทรสุขศรีราษฎร์สามัคคี)',
    'บ้านหนองกระทิง',
    'บ้านหิน',
    'บ้านท่าพระตระกาศ',
    'บ้านซำผักแว่น-นาซำ',
    'บ้านโศกขามป้อม',
    'บ้านด่าน',
    'บ้านผือ',
    'บ้านหนองเดียงน้อย-โนนใหญ่',
    'บ้านจานเลียว',
    'บ้านน้ำเย็น(กองทัพบกอุปถัมภ์)',
    'บ้านโนนเปือย',
    'บ้านรุง',
    'เกษตรประชาตาทวด',
    'บ้านโดนอาว',
    'บ้านโคกเจริญ',
    'บ้านละลายมีชัย',
    'บ้านทุ่งยาวคำโปรย',
    'บ้านสามเส้า',
    'บ้านเดียง(พลีศึกษา)',
    'บ้านเดียงตะวันตก',
    'บ้านขะยูงสาขาหนองแก',
    'บ้านหนองหิน(อสพป.15)',
    'บ้านจำนันสายเจริญ',
    'บ้นสวนกล้วย',
    'บ้านทุ่งขนวน',
    'บ้านโนนเรือ',
    'บ้านโนนแสนสุข',
    'บ้านกันตม(คุรุราษฎร์สามัคคี)',
    'บ้านสังเม็ก',
    'บ้านไหล่ดุมตาเหมา',
    'บ้านทุ่งโพธิ์',
    'บ้านโนนคำแก้ว',
    'บ้านหนองตึก-ห้วยน้ำใส',
    'บ้านภูมิซรอล',
    'ไทยรัฐวิทยา 27',
    'บ้านพรทิพย์',
    'บ้านหนองเม็กพิทยา',
    'บ้านเสาธงชัย',
    'บ้านหนองหญ้าลาด',
    'บ้านแก',
    'บ้านกระบี่',
    'บ้านจะเนียว',
    'บ้านกระหวัน',
    'บัานกันจด',
    'บ้านเดื่อ',
    'บ้านกระเบาตะหลุงวิทยา',
    'บ้านระหาร',
    'หมู่บ้านป่าไม้ห้วยจันทน์',
    'บ้านตาเอก',
    'บ้านตานวน',
    'บ้านกันทรอม',
    'บ้านกันทรอมน้อย',
    'บ้านจองกอ',
    'บ้านขุนหาญ',
    'บ้านดู่',
    'บ้านโคกระเวียง',
    'บ้านดาน',
    'บ้านดานสาขาบ้านจะกุด',
    'บ้านโนนสูง',
    'บ้านกระเจา',
    'บ้านหนองบัว',
    'บ้านกระเบา',
    'บ้านบักดอง',
    'บ้านสำโรงเกียรติ',
    'บ้านหนองบัวเรณ',
    'บ้านตาเส็ด',
    'บ้านตาปรก',
    'องค์การอุตสาหกรรมป่าไม้8',
    'บ้านภูดินพัฒนา',
    'หมู่บ้านทับทิมสยาม07',
    'บ้านสำโรงเกียรติสาขาบ้านสันติสุข',
    'บ้านซำตาโตง',
    'บ้านหนองเก่า',
    'บ้านโนนแฝก',
    'บ้านม่วงแยก',
    'บ้านทุ่งเลน',
    'บ้านพราน(ประชานุเคราะห์)',
    'บ้านซำขี้เหล็ก',
    'บ้านอาราง',
    'บ้านโนนสะอาด',
    'บ้านดอนข่า',
    'บ้านพยอม',
    'บ้านซำ',
    'บ้านสดำ',
    'บ้านหนองคู',
    'บ้านหนองขนาน',
    'บ้านโพธิ์กระสังข์',
    'บ้านโพธิ์กระมัล',
    'บ้านกระมัลพัฒนา',
    'บ้านตาหมื่น',
    'บ้านหนองใหญ่',
    'บ้านซำเขียน',
    'บ้านกราม',
    'บ้านพอก(บำรุงวิทยา)',
    'บ้านปุน',
    'บ้านหนองจิก',
    'บ้านกุดนาแก้ว',
    'บ้านภูทอง',
    'บ้านโนนอ้อ',
    'บ้านกระทิง',
    'บ้านศิวาลัย',
    'บ้านสิริขุนหาญ',
    'อนุบาลขุนหาญ (สิ)',
    'บ้านกันตรวจห้วย',
    'บ้านหนองผือ',
    'บ้านตูม',
    'บ้านหนองสังข์',
    'บ้านพิงพวย(เสียงราษฎร์พัฒนา)',
    'บ้านศิลาทอง',
    'บ้านบกห้วยโนน',
    'บ้านตระกวน',
    'บ้านศรีแก้ว',
    'อนุบาลศรีรัตนะ',
    'บ้านตระกาจ',
    'บ้านตาแบน',
    'บ้านหนองใหญ่-ตาไทย',
    'บ้านโนนแกสาขาบ้านตาเหมา',
    'โชติพันธุ์วิทยาสามัคคี',
    'บ้านหนองรุง',
    'บ้านโนนแก',
    'บ้านปุน',
    'บ้านสลับ',
    'บ้านขนาด',
    'บ้านตายู(อสพป.32)',
    'บ้านหนองบัวทอง',
    'บ้านทุ่งสว่าง',
    'บ้านจอก(ประชาสามัคคี)',
    'บ้านสะพุง',
    'บ้านหนองปิงโปง',
    'บ้านจานบัว',
    'บ้านเสื่องข้าว',
    'บ้านกระหวัน',
    'บ้านท่าคล้อ',
    'บ้านหนองบักโทน',
    'บ้านหนองคับคา',
    'บ้านโนนสำโรง',
    'บ้านหนองยาว',
    'บ้านคำสะอาด',
    'บ้านคำกลาง',
    'บ้านดอนเขียว',
    'อนุบาลเบญจลักษ์',
    'บ้านแดง(รัฐราษฎร์พัฒนา)',
    'บ้านโนนไหล่-หนองเลิง',
    'บ้านหนองงูเหลือม(คุรุราษฎร์รัฐกิจโกศล)',
    'บ้านหนองนกเขียน',
    'บ้านเขวาธะนัง',
    'บ้านโนนจักจั่น',
    'บ้านโนนจักจั่นสาขาบ้านห้วยเสียว',
    'บ้านเพ็กสาขาบ้านหนองปลาซิว',
    'บ้านหนองหว้า',
    'บ้านโนนคำตื้อ(โรตารี่1)',
    'บ้านเพ็ก',
    'บ้านหนองบัวใหญ่',
    'บ้านหนองฮาง',
    'บ้านไผ่หนองแคน',
    'บ้านสร้างเม็ก',
    'บ้านกุดผักหนามคำน้อย',
    'บ้านหนองตอโพนสูงนาคำ',
    'ตระกาศประชาสามัคคี',
    'กันทรลักษณ์วิทยาคม',
    'ขุนหาญวิทยาสรรค์',
    'กันทรลักษ์วิทยา',
    'ไพรธรรมคุณวิทยา',
    'กุดเสลาวิทยาคม',
    'พรานวิบูลวิทยา',
    'บึงมะลูวิทยา',
    'สายธารวิทยา',
    'กันทรอมวิทยาคม',
    'นาแก้ววิทยา',
    'กระแชงวิทยา',
    'มัธยมบักดองวิทยา',
    'ร่มโพธิ์วิทยา',
    'ดงรักวิทยา',
    'โพธิ์วงษ์วิทยา',
    'ภูมิซรอลวิทยา',
    'ศรีรัตนวิทยา',
    'ศรีแก้วพิทยา',
    'เบญจลักษ์พิทยา',
    'หนองหว้าประชาสรรค์',
    'บ้านหมากมี่',
    'บ้านกระโสบ(เพียรประสิทธิ์วิทยา)',
    'บ้านเค็ง',
    'บ้านโนนบ่อหวายดินดำ',
    'บ้านนาใต้',
    'บ้านนาคำ',
    'บ้านค้อกุดลาด',
    'บ้านปากน้ำ',
    'บ้านผาแก้ว',
    'ประชาสามัคคี',
    'บ้านหนองไผ่',
    'บ้านแคกลางหงษ์',
    'บ้านก้านเหลือง',
    'บ้านหนองปลาปาก',
    'บ้านด้ามพร้า',
    'บ้านนาดูน',
    'บ้านขามใหญ่',
    'บ้านหนองหว้า',
    'บ้านหัวคำ',
    'บ้านหนองตอแก้ว',
    'บ้านขี้เหล็ก',
    'บ้านสว่าง',
    'บ้านหนองแต้',
    'บ้านทุ่งขุนน้อยหนองจานวิทยา',
    'บ้านท่าบ่อ',
    'บ้านคูเดื่อ',
    'บ้านทัพไทย',
    'บ้านหนองแก',
    'วัดท่าวังหิน',
    'สามัคคีวิทยาคาร',
    'เมืองอุบล',
    'อุบลวิทยาคม',
    'มูลนิธิวัดศรีอุบลรัตนาราม ในพระอุปถัมภ์ทูลกระหม่อม',
    'อนุบาลอุบลราชธานี',
    'ปทุมวิทยากร',
    'บ้านปากห้วยวังนอง',
    'บ้านหนองสร้างหมากแข้ง',
    'บ้านปะอาว',
    'บ้านข่าโคม',
    'บ้านนาเมือง',
    'บ้านนามึน',
    'วัดบ้านบาก',
    'กิตติญาณอุปถัมภ์',
    'บ้านปลาดุก',
    'บ้านยางลุ่ม',
    'กองบินอุบลสงเคราะห์',
    'บ้านทุ่งขุนใหญ่',
    'บ้านดอนชี',
    'บ้านเชือก(ทวีปัญญา)',
    'บ้านหนองไหล(พุธเพิ่มวัฒนราษฎร์)',
    'บ้านหนองช้าง',
    'บ้านหนองไฮวิทยา',
    'ชุมชนบ้านหนองบ่อ(วิจิตรราษฎร์สามัคคี)',
    'บ้านมะเขือ',
    'บ้านท่าสนามชัย',
    'บ้านจานตะโนน',
    'บ้านโพนงาม(พูนเพิ่มวิทยาคาร)',
    'บ้านดงบัง(อาทรราษฎร์บำรุง)',
    'บ้านหนองก่านคำไผ่',
    'บ้านหนองมุก',
    'บ้านหนองยาง',
    'ชุมชนบ้านหัวเรือ',
    'บ้านหนองจำนัก',
    'บ้านอ้น',
    'บ้านสำราญ',
    'บ้านกลางใหญ่',
    'ประชานุเคราะห์วิทยา',
    'บ้านไผ่',
    'บ้านยางน้อย(พรหมพิทยา)',
    'บ้านพับ',
    'บ้านก่อ',
    'บ้านเอ้',
    'บ้านโนนโพธิ์',
    'บ้านดงยาง',
    'บ้านท่าลาด',
    'บ้านนามน',
    'บ้านโนนใหญ่(โอภาสพิทยาคาร)',
    'เขื่องใน(เจริญราษฎร์)',
    'บ้านกุดกะเสียน',
    'ไทยรัฐวิทยา 28 (บ้านจานเขื่องนามั่ง)',
    'บ้านนาดูน',
    'บ้านค้อทอง(อำไพพิทยาคาร)',
    'บ้านนาโพธิ์',
    'บ้านหนองขุ่น',
    'บ้านหัวทุ่งประชาสรรค์',
    'ชุมชนกลางแก้งส้มป่อย',
    'บ้านชีทวน',
    'บ้านหวาง',
    'บ้านหนองฮีหนองแคน',
    'บ้านหนองโนหนองดูน',
    'บ้านท่าศาลา',
    'บ้านแดงหม้อ',
    'บ้านทุ่ง',
    'บ้านบุตร',
    'บ้านโนนจานหนองแสง',
    'บ้านท่าค้อ',
    'บ้านท่าไห(ไหทอง)',
    'บ้านทัน',
    'บ้านดินดำคำไฮ',
    'บ้านธาตุน้อย(สิงห์ประชาวิทยาคาร)',
    'บ้านนาคำใหญ่(ราษฎร์บริบาล)',
    'บ้านแสงน้อย',
    'บ้านป่าข่า',
    'บ้านโนนรัง',
    'บ้านนาผาย',
    'บ้านแก้งซาว',
    'บ้านกอก',
    'บ้านนาแก้วประชาสรรค์',
    'บ้านชาติสามัคคี',
    'บ้านไทยโพนทราย',
    'บ้านทุ่งใหญ่',
    'บ้านโพนทอง(แสงราษฎร์สามัคคี)',
    'บ้านโนนดู่',
    'บ้านก่อแสนสำราญสามัคคี',
    'บ้านโพนสิม(เสมาพิทยา)',
    'บ้านผักแว่น(ผักแว่นวิทยาคาร)',
    'บ้านยางขี้นก',
    'บ้านคำสมอ(ศรีศึกษา)',
    'บ้านศรีสุข',
    'บ้านหนองห้างหนองกวาง',
    'บ้านอีต้อม',
    'บ้านเค็งนาดี',
    'บ้านดู่น้อย',
    'ชุมชนสร้างถ่อสามัคคี',
    'บ้านปลาฝา',
    'บ้านดอนเชียงโท',
    'บ้านศรีบัว',
    'บ้านหนองเซือม',
    'บ้านหนองหล่มหนองเซือมใต้',
    'บ้านขามป้อม',
    'บ้านกุดตากล้า',
    'สหธาตุสามัคคี(บำเพ็ญพิทยาคาร)',
    'บ้านคำหมีหนองข่า',
    'บ้านโพนเมือง(สุกวิทยา)',
    'บ้านหนองเหล่า(ราษฎร์บำรุงวิทยาคาร)',
    'บ้านคูขาด(ศรีวิทยาคาร)',
    'บ้านป่าก่อ',
    'บ้านท่าวารี',
    'บ้านแขม(ธรรมเสนานุสรณ์)',
    'บ้านวังอ้อ(ยอดสังข์วิทยาคาร)',
    'บ้านวังถ้ำ',
    'บ้านเสียม(เสียมทองวิทยาคาร)',
    'บ้านศรีศึกษา',
    'บ้านดุมใหญ่ดงยาง',
    'บ้านบัวยาง',
    'ดุมใหญ่บ้านหนองแสง',
    'บ้านท่าลาด',
    'บ้านเตย',
    'บ้านหนองเค็ม',
    'บ้านโนนขวาวนายูง',
    'ชุมชนบ้านน้ำคำแดง(มนต์ชัยเวทย์วิวรณ์)',
    'บ้านน้ำคำน้อย',
    'บ้านคำเกิ่งหนองจิก',
    'บ้านผาสุก',
    'บ้านหนองคูทรายมูล',
    'บ้านทุ่งมณี(คุรุราษฎร์นุกูล)',
    'บ้านนาเลิง',
    'บ้านหนองบัวดอนส้มป่อย',
    'บ้านผือ',
    'บ้านโนนรังใหญ่',
    'ประชาสามัคคี',
    'บ้านหนองมะทอ',
    'บ้านไผ่ใหญ่',
    'บ้านแสงไผ่',
    'บ้านโพนแพง',
    'บ้านเป็ดฟากทุ่ง',
    'บ้านผึ้ง(สามัคคี)',
    'บ้านสมบูรณ์(เบญจานุเคราะห์)',
    'ม่วงสามสิบ(อำนวยปัญญา)',
    'บ้านเทพา',
    'บ้านเหล่าข้าวดอนก่อ',
    'บ้านหนองขอน(ประชารัฐนุเคราะห์)',
    'บ้านตำแย',
    'บ้านหนองดูนโนนค้อหนองหัวลิง',
    'บ้านโนนรังน้อย',
    'ชุมชนบ้านหนองขุ่น',
    'บ้านยางโยภาพ',
    'บ้านน้ำอ้อมผักระย่า',
    'บ้านหนองผำ',
    'บ้านสองห้อง(คุรุราษฎร์สามัคคี)',
    'บ้านหนองแฝกยางเครือ',
    'บ้านยางสักกระโพหลุ่ม',
    'บ้านยางเทิง',
    'บ้านนาดีทุ่งเจริญ',
    'บ้านแคน(ประชาสามัคคี)',
    'บ้านหนองบัวแดง',
    'บ้านก่อ',
    'บ้านบก',
    'บ้านหนองไข่นก',
    'บ้านพระโรจน์(ชนูปถัมภ์)',
    'บ้านหนองช้าง(ประชาสามัคคี)',
    'บ้านหนองแสงหนองเม็ก',
    'บ้านไพบูลย์',
    'บ้านสร้างมิ่ง',
    'บ้านสวนงัว(ราษฎร์ใจดี)',
    'บ้านหนองเมืองน้อย',
    'ราษฎร์ร่วมแรงรัฐ(กำจัดอุปถัมภ์)',
    'บ้านนาขาม',
    'บ้านหนองหล่มหนองเหล่า',
    'บ้านโพนเมืองมะทัน',
    'บ้านวังมนเดือยไก่',
    'บ้านดอนแดง',
    'บ้านทุ่งใต้',
    'บ้านเหล่าบาก',
    'บ้านนาไร่ใหญ่',
    'บ้านหนองฮาง',
    'บ้านดอนประทาย',
    'บ้านเศรษฐี',
    'บ้านหนองหลัก',
    'บ้านสงยางดอนไม้คูณ',
    'บ้านโนนชาติยูง',
    'บ้านขมิ้น',
    'บ้านคำไฮใหญ่',
    'บ้านหนองแล้ง',
    'บ้านหนองหิน',
    'ดอนมดแดง(บ้านดงบัง)',
    'บ้านกุดกั่ว',
    'บ้านยาง',
    'บ้านวังพระวังไฮ',
    'บ้านแคน(สมเด็จอุปถัมภ์)',
    'บ้านท่าเมือง',
    'บ้านยางกะเดา',
    'บ้านสว่าง',
    'บ้านโอด',
    'บ้านยางกะเดา(สาขาบ้านนาคำภูฮี)',
    'บ้านกระบูน',
    'บ้านท่าศิลา',
    'บ้านเหล่าแดง',
    'บ้านค้อ',
    'บ้านท่าลาด',
    'บ้านแพง',
    'บ้านจิก',
    'บ้านแสง',
    'บ้านหนองเป็ด',
    'ชุมชนโพนเมืองวิทยา',
    'บ้านขามน้อย',
    'บ้านคำไหล',
    'บ้านหาด',
    'บ้านหนองมะแซว',
    'บ้านสร้างถ่อ',
    'บ้านธรรมละ',
    'บ้านรังแร้ง',
    'บ้านดอนกลอย',
    'บ้านหนองบก',
    'บ้านดอนกลาง',
    'บ้านแต้เก่า',
    'บ้านคำไฮน้อย',
    'บ้านนาขมิ้น',
    'บ้านดูน',
    'บ้านแต้ใหม่',
    'บ้านเหล่าคำ',
    'บ้านนาไผ่',
    'บ้านเหล่าเสือโก้ก',
    'บ้านเป้า',
    'บ้านเหล่าแค',
    'เบ็ญจะมะมหาราช',
    'เขื่องในพิทยาคาร',
    'นารีนุกูล',
    'สหธาตุศึกษา',
    'ปทุมพิทยาคม',
    'ชีทวนวิทยาสามัคคี',
    'นาคำวิทยา',
    'ศรีปทุมพิทยาคาร',
    'บ้านไทยวิทยาคม',
    'หนองบ่อสามัคคีวิทยา',
    'เสียมทองพิทยาคม',
    'หนองขอนวิทยา',
    'พระวอพระตาวิทยานุสรณ์',
    'เก่าขามวิทยา',
    'นารีนุกูล 2',
    'ประทุมวรราชวิทยา',
    'อุบลปัญญานุกูล',
    'ราชประชานุเคราะห์ 32',
    'ม่วงสามสิบอัมพวันวิทยา',
    'ศรีน้ำคำศึกษา',
    'ไผ่ใหญ่ศึกษา',
    'ดงยางวิทยาคม',
    'ดอนมดแดงวิทยาคม',
    'หกสิบพรรษาวิทยาคม อุบลราชธานี',
    'บ้านนาขนัน',
    'บ้านแก้งเหนือ',
    'บ้านนาแมด',
    'บ้านนาตาลเหนือ',
    'ชุมชนบ้านขามป้อม',
    'บ้านเตย(คุรุราษฎร์วิทยา)',
    'บ้านทรายพูล',
    'บ้านนาสนาม',
    'บ้านไทรย้อย',
    'บ้านอูบมุงแก้งเกลี้ยง',
    'เขมราฐ',
    'บ้านเหนือเขมราฐ',
    'บ้านหนองหลวง',
    'บ้านบุ่งซวยห้วยยาง',
    'ชุมชนบ้านเจียด',
    'บ้านนานวน',
    'บ้านนาแวง',
    'บ้านบุ่งขี้เหล็ก',
    'บ้านโบกม่วง',
    'บ้านลาดหญ้าคา',
    'บ้านนาเมือง',
    'บ้านนายูง',
    'บ้านโนนสูง',
    'บ้านป่าติ้ว',
    'บ้านหนองนกทา',
    'ตชด.ทองพูนพิทยา',
    'บ้านหนองผือ',
    'บ้านพนมดี',
    'บ้านแก้งหลักด่าน',
    'บ้านโคกสว่าง',
    'บ้านบาก',
    'บ้านบึงหอม',
    'บ้านคำหาด',
    'บ้านนาหว้าเหนือ',
    'บ้านป่าข่า',
    'ตชด.ป่งคอม',
    'บ้านม่วงเฒ่า',
    'บ้านห้วยยาง',
    'บ้านอีเติ่ง',
    'บ้านนาชุมใต้',
    'บ้านกระเดียน',
    'บ้านเวียง(ตุอุปถัมภ์)',
    'บ้านนาหว้า',
    'บ้านกุดยาลวน',
    'บ้านคึม',
    'บ้านกาจับ',
    'บ้านกุศกร',
    'บ้านลาดสมดี',
    'บ้านศรีสุข',
    'บ้านหนองเอาะ',
    'บ้านคำสมิง',
    'บ้านเกษม',
    'เกษมบ้านนาคำ',
    'บ้านกึ่งพุทธกาล',
    'บ้านขามเปี้ย',
    'ขามเปี้ยบ้านศรีสุข',
    'บ้านดอนตะมุน',
    'บ้านนาตาหมุด',
    'บ้านบ่อหิน',
    'บ้านดอนหมูวิทยา',
    'ชุมชนบ้านท่าบ่อแบง',
    'บ้านนาคิแลน',
    'บ้านดอนทับช้าง',
    'ขุหลุ(ประชาวิทยาคาร)',
    'บ้านดอนใหญ่',
    'บ้านคอนสาย',
    'บ้านสร้างโพน',
    'บ้านโป่งน้อย',
    'บ้านนาคำวิทยา',
    'บ้านดอนม่วง(แก้วประชาสรรค์)',
    'บ้านม่วงเดียด',
    'บ้านฮี',
    'บ้านแพง',
    'บ้านดอนหมากมาย(ราษฎร์สามัคคีวิทยา)',
    'บ้านคำไหล',
    'บ้านแอมเจริญ',
    'บ้านโคกน้อย',
    'บ้านโคกใหญ่',
    'บ้านโคกจาน',
    'บ้านเสาธงใหญ่',
    'บ้านนาเดื่อ',
    'ชุมชนบ้านเซเป็ด',
    'ตระการพืชผล',
    'บ้านตระการ',
    'บ้านโพนเมือง',
    'บ้านเอ็นอ้า',
    'บ้านดอนหมู',
    'บ้านตากแดด',
    'บ้านไผ่ล้อม',
    'บ้านถ้ำแข้(สาขาบ๋าหอย)',
    'บ้านถ้ำแข้',
    'บ้านคำแคน',
    'บ้านห้วยที',
    'บ้านโนนสูง',
    'บ้านนาส้มมอ',
    'บ้านเทวัญ(คุรุราษฎร์นุกูล)',
    'บ้านแก้งอะฮวน',
    'บ้านท่าหลวง',
    'บ้านดอนจิก',
    'บ้านโหมน',
    'บ้านนาพิน',
    'บ้านนาไฮ(มิตรภาพที่ 145)',
    'บ้านหนองแดง',
    'บ้านสมบูรณ์',
    'บ้านน้ำคำ',
    'น้ำคำพิทยา',
    'บ้านนาห้วยแดง',
    'บ้านร่องข่า',
    'บ้านโนนกุง',
    'บ้านแดง(บุญเสริมอุทิศ)',
    'บ้านเล้า',
    'บ้านทม',
    'บ้านโนนสำราญ',
    'บ้านนางิ้ว(ประชาสามัคคี)',
    'บ้านสมดี',
    'บ้านเป้า(รัฐราษฎร์บำรุง)',
    'บ้านพอก',
    'ชุมชนบ้านสะพือ',
    'บ้านนางิ้ว',
    'บ้านสงยาง',
    'บ้านเขือง',
    'บ้านแหลไหล่',
    'บ้านหนองเต่า',
    'บ้านนาดอกไม้',
    'บ้านดอน',
    'บ้านห้วยฝ้าย',
    'บ้านดอนก่อ',
    'บ้านนาจ่าย',
    'บ้านไหล่ทุ่ง',
    'บ้านไหล่สูง',
    'บ้านดอนงัว',
    'บ้านดงไม้งาม',
    'บ้านคำข่า',
    'บ้านกาบิน',
    'บ้านตุ',
    'บ้านป่าข่า',
    'บ้านฟ้าห่วน',
    'บ้านหนองกุงดอนตาเขสามัคคี',
    'บ้านแก่งเค็ง',
    'บ้านตาดโตน',
    'บ้านขุมคำ',
    'บ้านหนองบั่ว',
    'บ้านน้ำเกลี้ยง',
    'บ้านตาดแต้',
    'บ้านโนนหอม',
    'ชุมชนบ้านข้าวปุ้น(ศาสนานุเคราะห์)',
    'บ้านโคกเลาะ(มิตรภาพที่ 159)',
    'บ้านโคกก่องวังนอง',
    'ชุมชนบ้านโนนสวาง',
    'บ้านหนองหว้า',
    'บ้านดงบัง',
    'บ้านคำแม่มุ่ย',
    'บ้านกุดกลอย',
    'บ้านแก้งลิง',
    'บ้านไชยชนะ',
    'บ้านแข็งขยัน',
    'บ้านบกหนองทันน้ำ',
    'บ้านโนนหินแร่',
    'บ้านแสนอุดม',
    'บ้านศรีสมบูรณ์แหลมทอง',
    'บ้านหนองลุมพุก',
    'บ้านศรีเชียงใหม่',
    'บ้านพะไล',
    'บ้านเลาะ',
    'บ้านดอนเย็นใต้',
    'เสาวนิต(บ้านวังแฮ)',
    'บ้านโพธิ์ไทร',
    'บ้านคำมณี',
    'บ้านแก้งใต้',
    'บ้านดอนกะทอด',
    'บ้านม่วงใหญ่',
    'บ้านนานางวาน',
    'บ้านโปร่ง',
    'บ้านดอนชาด',
    'บ้านนาพะเนียงออ',
    'บ้านกะเตียด',
    'บ้านสองคอน',
    'บ้านจอมปลวกสูงสร้างกุง',
    'บ้านกะลึง',
    'บ้านโสกชัน',
    'บ้านไพรสวรรค์',
    'บ้านหินห่อม',
    'บ้านสารภี(สาขาหนองผักแว่น)',
    'บ้านคำกลาง',
    'บ้านสารภี',
    'บ้านพะลอง',
    'สมาคมจักรยานสมัครเล่น',
    'บุญจิราธร',
    'บ้านสำโรง',
    'บ้านนาหว้าใต้',
    'บ้านนาขาม',
    'ผอบ ณ นคร 1',
    'บ้านปากห้วยม่วง',
    'บ้านดงตาหวัง',
    'บ้านหนองฟานยืน',
    'บ้านโป่งเป้า',
    'บ้านโป่งเป้าสาขาบ้านคำจ้าว',
    'บ้านค้อ',
    'บ้านนาชุม',
    'บ้านไทรงาม',
    'บ้านกองโพน',
    'บ้านไหล่ธาตุ',
    'บ้านเหมือดแอ่แก้งไฮ',
    'บ้านคันพะลาน',
    'บ้านลุมพุก',
    'บ้านนาตาลใต้',
    'บ้านนากลาง',
    'บ้านนาม่วง',
    'บ้านนาทราย',
    'บ้านปากแซง',
    'บ้านโพนแพง',
    'บ้านนาหินโหง่น',
    'บ้านพะลาน',
    'บ้านบก',
    'บ้านพังเคน',
    'บ้านโนนตูม',
    'บ้านโนนขุมคำ',
    'บ้านด่านหม่วน',
    'บ้านด่านฮัง',
    'บ้านหนองเต่า',
    'มัธยมตระการพืชผล',
    'เขมราฐพิทยาคม',
    'สะพือวิทยาคาร',
    'โนนสวางประชาสรรค์',
    'แก้งเหนือพิทยาคม',
    'โนนกุงวิทยาคม',
    'ขุมคำวิทยาคาร',
    'เกษมสีมาวิทยาคาร',
    'นาสะไมพิทยาคม',
    'กุดข้าวปุ้นวิทยา',
    'โพธิ์ไทรพิทยาคาร',
    'เหล่างามพิทยาคม',
    'พังเคนพิทยา',
    'พะลานวิทยาคม',
    'บ้านแก้งกอก',
    'บ้านนาแค',
    'บ้านสัน',
    'บ้านนาห้วยแดงดงสำโรง',
    'บ้านห้วยหมาก',
    'บ้านคำไหล',
    'บ้านจุการ',
    'บ้านขัวแคน',
    'บ้านหนองเชือก',
    'บ้านดอนน้ำคำ',
    'บ้านหนองสองห้อง',
    'บ้านโนนหนองบัว',
    'ชุมชนบ้านดอนใหญ่บูรพา',
    'บ้านไร่',
    'บ้านหินโงม',
    'บ้านตุงลุงเหนือ',
    'บ้านโนนกุง',
    'บ้านฟ้าห่วน',
    'บ้านคำเตย',
    'บ้านศรีเมืองใหม่',
    'บ้านนาเอือด',
    'บ้านหนองห้าง',
    'บ้านนาเลิน',
    'บ้านคำหมาไน',
    'บ้านสวาสดิ์',
    'บ้านนาโป่งโพน',
    'บ้านป่ากุงน้อย',
    'บ้านป่ากุงใหญ่',
    'บ้านหนองผักแพว',
    'บ้านลาดควาย',
    'บ้านหนองหอย',
    'บ้านจันทัย',
    'บ้านโนนม่วงโนนจิก',
    'บ้านหนองขุ่น',
    'บ้านดอนโพธิ์',
    'บ้านภูหล่น',
    'บ้านหนองไผ่',
    'บ้านคำบง',
    'บ้านสระคำ',
    'บ้านสงยาง',
    'บ้านโนน',
    'บ้านโป่ง',
    'บ้านสิม',
    'บ้านหนามแท่ง',
    'บ้านชาด',
    'บ้านนาทอย',
    'บ้านนาคอ',
    'บ้านสร้างถ่อ',
    'บ้านหุ่งหลวง',
    'บ้านโหง่นขาม',
    'บ้านดงนา',
    'บ้านงิ้ว',
    'บ้านเอือดใหญ่',
    'บ้านกุดก่วยโนนเจริญ',
    'บ้านด่าน',
    'บ้านห้วยหมากใต้',
    'บ้านเวินบึก',
    'บ้านห้วยสะคาม',
    'บ้านตุงลุง',
    'บ้านหัวเห่ว',
    'บ้านคันท่าเกวียน',
    'บ้านซะซอม',
    'บ้านนาโพธิ์กลาง',
    'บ้านนาโพธิ์ใต้',
    'บ้านทุ่งนาเมือง',
    'บ้านหนองผือ',
    'บ้านหนองแสง-วังอ่าง',
    'บ้านกุดเรือคำ',
    'บ้านดงบาก',
    'บ้านดงมะไฟ',
    'บ้านหนองแสงใหญ่',
    'ชุมชนบ้านวังสะแบง',
    'บ้านตามุย',
    'บ้านกุ่ม',
    'ชุมชนห้วยไผ่',
    'บ้านท่าล้ง',
    'บ้านห้วยยาง',
    'บ้านดงดิบ',
    'บ้านนาบัว',
    'บ้านโพนแพง',
    'บ้านดงแถบ',
    'บ้านบะไห',
    'บ้านกุดชมภู',
    'บ้านแก่งเจริญ',
    'บ้านคำเม็กห้วยไผ่',
    'บ้านโชคอำนวย',
    'บ้านโนนม่วง',
    'บ้านยอดดอนชี',
    'บ้านหินลาดแสนตอ',
    'ชุมชนบ้านดอนจิก(จรรยาราษฎร์)',
    'บ้านแก่งยาง',
    'บ้านไทรงาม',
    'บ้านนาดี',
    'บ้านม่วงฮี',
    'บ้านนาเจริญ',
    'บ้านบุ่งคำ',
    'บ้านภูมะหรี่',
    'บ้านบุ่งคำสาขาดอนใหญ่',
    'บ้านหนองไผ่',
    'บ้านห้วยแดง',
    'บ้านทรายมูล(จันทน์รัตนานุเคราะห์)',
    'บ้านท่าเสียวคันลึม',
    'บ้านชาด',
    'บ้านนาโพธิ์',
    'บ้านชาดฮี(ธรรมทินโนปถัมภ์)',
    'บ้านเลิงบาก',
    'บ้านหนองไฮ(เมฆวิทยาคาร)',
    'บ้านโนนจันทร์',
    'บ้านโดมประดิษฐ์',
    'บ้านนกเต็น',
    'บ้านราษฎร์เจริญ',
    'บ้านโนนกาหลง',
    'บ้านแก่งดูกใส',
    'บ้านดอนโด่โคกเลียบ',
    'บ้านโนนคูณ',
    'บ้านนาชุม',
    'บ้านหนองคูณ',
    'บ้านแขมเหนือ',
    'บ้านแขมใต้',
    'บ้านสนามชัย(พิมพ์ประชานุกูล)',
    'บ้านวังแคน',
    'บ้านท่าช้าง',
    'บ้านปากโดม',
    'บ้านถิ่นสำราญ',
    'บ้านสร้างแก้ว',
    'บ้านนางาม(ผลแก้วอุปถัมภ์)',
    'บ้านเดื่อสะพานโดม',
    'บ้านโพธิ์ศรี',
    'บ้านนาหว้า',
    'บ้านโนนข่า',
    'บ้านหนองโพธิ์',
    'บ้านสะพือท่าค้อ',
    'บ้านสะพือใต้',
    'บ้านวังพอก',
    'บ้านนาแก',
    'บ้านนาจาน',
    'บ้านคำผ่าน',
    'ชุมชนบ้านระเว',
    'บ้านไร่ใต้',
    'บ้านผักหย่า',
    'บ้านแก่งกอก',
    'บ้านโนนค้อลุคุ',
    'บ้านโนนสุข',
    'บ้านหนองสองห้อง',
    'บ้านหนองบัวฮี',
    'บ้านโนนยานาง',
    'บ้านดอนงัว',
    'บ้านบัวทอง',
    'บ้านหนองกอก',
    'บ้านบัวแดง',
    'บ้านดูกอึ่ง',
    'พิบูลมังสาหาร(วิภาคย์วิทยากร)',
    'บ้านเม็ก',
    'บ้านอ่างหินน้อย',
    'บ้านโนนสว่าง',
    'บ้านดอนก่อ',
    'บ้านอ่างหิน(เพียรพิทยาคาร)',
    'บ้านคำหว้า(วิบูลราษฎร์สามัคคี)',
    'บ้านนาทม',
    'บ้านไฮหย่อง',
    'บ้านห้วยดู่',
    'ชุมชนบ้านเชียงแก้ว',
    'บ้านจิกลุ่ม',
    'บ้านจิกเทิง(วินัยสารประชานุกูล)',
    'บ้านนาห้วยแคน',
    'บ้านม่วงโคน',
    'บ้านแก่งกบ',
    'บ้านดอนตะลี',
    'บ้านดอนพันชาด',
    'บ้านนามน(สามัคคีวิทยาคาร)',
    'บ้านนาคาย',
    'บ้านดอนขวาง',
    'ไมตรีจิตไทย-อเมริกัน',
    'บ้านคำหนามแท่ง',
    'บ้านหนองโน',
    'บ้านสำโรง',
    'บ้านหนองกุง',
    'บ้านหนองกุงน้อย',
    'บ้านคำเตย',
    'บ้านคันไร่(อาภากโรคุรุราษฎร์)',
    'บ้านโคกเที่ยง',
    'บ้านดอนชี',
    'บ้านปากบุ่ง',
    'บ้านห้วยน้ำใส',
    'บ้านคำเขื่อนแก้ว',
    'นิคมสร้างตนเอง 5',
    'นิคมสร้างตนเอง 6',
    'บ้านหนองชาด',
    'หมู่บ้านตัวอย่าง(ป่าดงหินกอง)',
    'บ้านห้วยไฮ',
    'บ้านคันเปือย',
    'บ้านลาดวารี',
    'บ้านสุวรรณวารี',
    'บ้านเหล่าอินทร์แปลง',
    'องค์การอุตสาหกรรมป่าไม้ 17',
    'บ้านทุ่งหนองบัว',
    'บ้านหนองเม็ก',
    'บ้านหินสูง',
    'นิคมสร้างตนเอง 2',
    'นิคมสร้างตนเอง 3',
    'ไทยรัฐวิทยา 87 (นิคมสร้างตนเอง 1)',
    'บ้านแหลมทอง',
    'บ้านโนนก่อ',
    'บ้านป่าเลา',
    'บ้านพลาญชัย',
    'บ้านห้วยเดื่อ',
    'บ้านเหล่าคำ',
    'บ้านบากชุม',
    'บ้านคำก้อม',
    'บ้านโนนจิก',
    'บ้านฝางคำสามัคคี',
    'หนามแท่งพิทยาคม รัชมังคลาภิเษก',
    'พิบูลมังสาหาร',
    'ศรีเมืองวิทยาคาร',
    'โขงเจียมวิทยาคม',
    'ไร่ใต้ประชาคม',
    'เอือดใหญ่พิทยา',
    'อ่างศิลา',
    'หนองบัวฮีวิทยาคม',
    'โนนกลางวิทยาคม',
    'ตาลสุมพัฒนา',
    'เชียงแก้วพิทยาคม',
    'สิรินธรวิทยานุสรณ์',
    'ช่องเม็กวิทยา',
    'บ้านแฮหนามแท่ง',
    'บ้านคำขวาง',
    'บ้านเกษตรพัฒนา',
    'บ้านท่าข้องเหล็ก',
    'บ้านคำนางรวย',
    'บ้านโพธิ์มูลเพียเภ้า',
    'บ้านศีรษะกระบือ(อิสระคุรุราษฎร์)',
    'วัดบ้านค้อหวาง',
    'บ้านโนนจิก',
    'บ้านดอนผอุง',
    'บ้านคูเมือง(อ่อนอนุเคราะห์)',
    'บ้านท่าลาด',
    'บ้านโนนทรายน้อย',
    'บ้านถ่อน',
    'บ้านโนนเกษม',
    'บ้านบัววัด',
    'บ้านดอนกลาง(ราษฎร์สงเคราะห์)',
    'บ้านหนองสองห้อง',
    'บ้านจั่น',
    'บ้านโนนโหนน(อ่อนอุปการ)',
    'บ้านบก(มั่นสามัคคีพิทยาคาร)',
    'บ้านหนองคู(โสภณประชานุกูล)',
    'บ้านคอนสาย',
    'บ้านนาโหนนน้อย',
    'บ้านท่างอย',
    'บ้านวังยาง',
    'บ้านโนนบอน(คำหล้าประชานุเคราะห์)',
    'บ้านบุ่งหวาย',
    'บ้านดอนชาด',
    'บ้านห่องชัน',
    'บ้านกุดระงุม',
    'บ้านยางวังกางฮุง',
    'บ้านโนนแดง',
    'บ้านหนองฝาง(บัวค้ำวิทยา)',
    'บ้านโพธิ์',
    'บ้านแมดคำลือชา(ราษฎร์สามัคคี)',
    'บ้านโนนชาดผดุงวิทย์',
    'ชุมชนบ้านศรีไค(ศรีวัฒนาวิทยาคาร)',
    'บ้านหาดสวนยา',
    'บ้านท่าบ้งมั่ง(ทหารอากาศอเมริกันสงเคราะห์)',
    'ชุมชนบ้านกุดเป่ง',
    'บ้านกุดปลาขาว',
    'บ้านหนองตาปู่',
    'บ้านหนองบัวแดนเกษม',
    'บ้านบาก',
    'บ้านสระสมิง',
    'บ้านแสนคำ(นาชุมสามัคคี)',
    'ประชารัฐสามัคคี(ธนาคารกรุงเทพ๑๓)',
    'บ้านโนนยาง',
    'บ้านคำเจริญ',
    'บ้านก่อ(ก่อวิทยาคาร)',
    'บ้านธาตุ',
    'บ้านปากกุดหวาย',
    'บ้านทางสาย',
    'บ้านขัวไม้แก่น',
    'บ้านคูสว่าง',
    'บ้านกุดชุม',
    'บ้านนาสะแบง',
    'บ้านหนองกินเพล',
    'บ้านห้วยขะยุง(คุรุพานิชวิทยาคาร)',
    'บ้านน้ำเที่ยง',
    'บ้านนางาม',
    'บ้านโนนแคน',
    'บ้านขามป้อม',
    'บ้านโปร่ง',
    'บ้านสร้างแก้ว',
    'บ้านทรายทอง',
    'บ้านหินแห่',
    'บ้านกะแอกน้อย',
    'บ้านสำโรงน้อย',
    'บ้านคำสว่าง',
    'บ้านค้อน้อย',
    'บ้านอาเลา',
    'บ้านผำ',
    'บ้านโคกก่อง',
    'บ้านแกนาคำ',
    'บ้านโนนสมบูรณ์',
    'บ้านหนองจิกนาเรือง',
    'บ้านหนองศิลา(พรชุมชนวิทยา)',
    'บ้านโนนสูงดอนพิกุล',
    'บ้านวังคก',
    'บ้านโคกสว่าง',
    'บ้านคุ้ม',
    'บ้านกะแอก',
    'บ้านนาดี',
    'บ้านสระดอกเกษ',
    'บ้านหนองไฮ(ประชานุกูลวิทยา)',
    'บ้านหนองหิน',
    'บ้านหนองมัง',
    'บ้านโนนสัง',
    'บ้านโคกสว่าง(บุญคุรุราษฎร์วิทยา)',
    'บ้านดอนม่วง',
    'บ้านโพนเมือง',
    'บ้านเปือย',
    'บ้านสว่าง(สว่างวิทยาคาร)',
    'บ้านโพนงาม',
    'บ้านหนองผือโปร่งคำ',
    'บ้านบุ่ง',
    'บ้านห่องขอน',
    'บ้านบอน',
    'บ้านค้อบอน',
    'บ้านห่องยูง',
    'บ้านหนองหัวงัว',
    'บ้านหนองเทา',
    'บ้านหนองสองห้อง',
    'บ้านสำโรง(คุรุประชาสามัคคี)',
    'บ้านผับแล้ง',
    'บ้านหว้าน',
    'บ้านน้ำเกลี้ยง',
    'บ้านนาแก',
    'บ้านนานวล',
    'บ้านสร้างโหง่น',
    'บ้านหนองบัว',
    'บ้านขี้เหล็ก',
    'บ้านดอนยู',
    'บ้านนาดี',
    'บ้านนาดู่',
    'บ้านหนองแปน',
    'บ้านนาจาน',
    'บ้านนาประชุม',
    'ชุมชนบ้านนาเยีย',
    'บ้านแก้งยาง',
    'บ้านนาเรือง',
    'บ้านโนนงาม',
    'บ้านหนองกระบือ',
    'บ้านม่วงคำ',
    'บ้านหินลาด',
    'บ้านโคกสมบูรณ์',
    'บ้านผึ้งโดม',
    'บ้านแก่งโดม',
    'บ้านคำหมากอ่อน',
    'บ้านโคกสมบูรณ์นอก',
    'บ้านสะพานโดม',
    'บ้านท่าช้าง(เขตการทางนครราชสีมาสงเคราะห์2)',
    'บ้านคำโพธิ์โคกก่อง',
    'บ้านบัวท่า',
    'บ้านบัวเทิง',
    'บ้านบุ่งมะแลง',
    'บ้านป่าข่า',
    'บ้านสุขสมบูรณ์',
    'บ้านห้วยนกเปล้า(คุ้ยคุรุราษฎร์วิทยา)',
    'บ้านคำข่า',
    'บ้านทุ่งเพียง',
    'บ้านคำสมิง',
    'บ้านสำโรงคุรุราษฎร์วิทยาคาร(สาขาบ้านหนองแวง)',
    'บ้านสำโรงคุรุราษฎร์วิทยา',
    'บ้านแพง',
    'บ้านสว่าง',
    'นาเยียศึกษา รัชมังคลาภิเษก',
    'สำโรงวิทยาคาร',
    'สว่างวีระวงศ์',
    'ลือคำหาญวารินชำราบ',
    'ห้วยขะยุงวิทยา',
    'โคกสว่างคุ้มวิทยานุสรณ์',
    'วารินชำราบ',
    'วิจิตราพิทยา',
    'บ้านกลาง',
    'บ้านหมากมาย',
    'บ้านเม็กใหญ่',
    'ดำรงสินสงเคราะห์',
    'บ้านบัวเทียม',
    'บ้านโนนสวรรค์',
    'บ้านสุขสมบูรณ์',
    'บ้านนาทุ่ง',
    'บ้านอุดมสุข',
    'บ้านดอนม่วย',
    'บ้านกุดประทาย',
    'บ้านโนนกอย',
    'บ้านโนนขาม',
    'บ้านแสนสุข',
    'บ้านหนองคู',
    'บ้านนาอุดม',
    'บ้านคำนาแซง',
    'บ้านแก้ง',
    'บ้านไฮตาก',
    'บ้านประหูต',
    'บ้านห้วยสำราญ',
    'บ้านยาง',
    'บ้านนาประดู่',
    'บ้านนานวล',
    'บ้านนาสมบูรณ์',
    'บ้านคำครั่ง',
    'บ้านป่าตาว',
    'บ้านโนนแคน',
    'บ้านโนนกาเร็น',
    'บ้านโพธิ์ไทร',
    'บ้านคำสำราญ',
    'บ้านโนนแก้ง',
    'บ้านสองคอน',
    'บ้านโพนดวน',
    'บ้านท่าโพธิ์ศรี',
    'บ้านวารีอุดม',
    'บ้านโพธิ์สง่า',
    'บ้านโนนหลี่',
    'บ้านม่วงนาดี',
    'บ้านทุ่งเทิง',
    'บ้านสว่างโนนทอง',
    'บ้านโนน',
    'บ้านบัวเจริญ',
    'บ้านหม้อทอง',
    'บ้านเตย',
    'บ้านดอนกลาง',
    'บ้านหนองไฮ',
    'บ้านหนองแต้',
    'บ้านนากระแซง',
    'พิชัยศึกษา',
    'บ้านโนนสวาง',
    'บ้านหนองเงินฮ้อย',
    'บ้านห่องเตย',
    'บ้านห่องคำ',
    'บ้านโชคชัย',
    'บ้านหนองบัวแดง',
    'บ้านโนนจิก',
    'บ้านนาเจริญ',
    'บ้านกระเบื้อง',
    'บ้านม่วงโนนกระแต',
    'บ้านนาห้วยแคน',
    'บ้านนาส่วง',
    'บ้านเสาเล้า',
    'บ้านหนองบัว',
    'บ้านโนนสมบูรณ์',
    'บ้านสวนสวรรค์',
    'อุดมสามัคคี',
    'บ้านนาแก',
    'บ้านบัวงาม',
    'บ้านหนองสนม',
    'บ้านโนนแฝก',
    'ประชานุเคราะห์ศึกษา',
    'บ้านนาเลิง',
    'บ้านบัวทอง',
    'บ้านราษฎร์สามัคคี',
    'บ้านโนนค้อ',
    'บ้านป่าหวาย',
    'บ้านป่าโมง',
    'นิคมสร้างตนเองลำโดมใหญ่',
    'ชุมชนบ้านหนองยาว',
    'บ้านโนนสว่าง',
    'บ้านหนองแสง',
    'เมืองเดช',
    'เวตวันวิทยา',
    'นาคสมุทรสงเคราะห์',
    'บ้านแขมเจริญ',
    'ประชาสามัคคี',
    'เมืองเดชบ้านสมสะอาด',
    'ดำรงสินอุทิศ',
    'บ้านหนองสำราญ',
    'สมสะอาดสวนฝ้าย',
    'บ้านหนองบัวหลวง',
    'บ้านโนนสนาม',
    'บ้านท่าหลวงนาคำ',
    'นาจะหลวย(กรป.กลางอุปถัมภ์)',
    'บ้านแก้งขี้เหล็ก',
    'บ้านโคกใหญ่',
    'ผอบ ณ นคร 2',
    'สมเด็จพระราชชนนี(บ้านแก่งเรือง)',
    'บ้านดงขวางคำโทน',
    'บ้านท่าก่อ',
    'บ้านป่าพอก',
    'บ้านกวางดีด',
    'บ้านโคกเทียม',
    'บ้านโนนคูณแสนสุข',
    'บ้านโนนบก',
    'บ้านป่าก้าว',
    'บ้านตบหู',
    'บ้านโนนว่าน',
    'บ้านคำอุดม',
    'บ้านดอนยาว',
    'บ้านคำบอน',
    'ชุมชนบ้านโนนแดง',
    'บ้านทุ่งเงินโนนเจริญ',
    'บ้านโคกน้อย',
    'บ้านตูม(ราษฎร์บูรณะ)',
    'บ้านม่วง',
    'บ้านฝั่งเพ',
    'บ้านคุ้มแสนชะนี',
    'บ้านบุ่งคำ',
    'บ้านแก้งขอ',
    'บ้านทุ่งเพียง',
    'บ้านโสกแสง',
    'บ้านคำม่วง',
    'บ้านดอนชัยชนะ',
    'บ้านทับไฮ',
    'บ้านโนนสว่าง',
    'บ้านป่าไร่',
    'บ้านโพนแอวขัน',
    'โนนสูงโนนโฮมวิทยา',
    'บ้านคำกลาง',
    'บ้านเก่าขาม',
    'บ้านหนองทัพ',
    'บ้านโซง',
    'บ้านดวน',
    'บ้านค้อ',
    'บ้านตาโม',
    'บ้านหนองเทา',
    'บ้านหนองขอน',
    'บ้านแข้ด่อน',
    'ตชด.บ้านค้อ',
    'บ้านโนนสูง(ต.โดมประดิษฐ์)',
    'บ้านกุดเชียงมน',
    'บ้านจันลา',
    'บ้านแปดอุ้ม',
    'บ้านตายอย',
    'บ้านโพนทอง',
    'หนองโพดวิทยา',
    'บ้านแก้งโตน',
    'ชุมชนบ้านบุเปือย',
    'บ้านเปือย',
    'ตชด.เกษตรสมบูรณ์',
    'รัชมังคลาภิเษกบ้านโนนป่าเลา',
    'บ้านยางกลาง',
    'บ้านหนองคู',
    'ชุมชนบ้านปลาขาว',
    'บ้านยางใหญ่',
    'บ้านยางใหญ่(สาขาบ้านสวายน้อย)',
    'บ้านโนนสวาง',
    'บ้านดงกระชู',
    'บ้านห้วยแก้ว',
    'น้ำยืน',
    'บ้านนาสามัคคี',
    'บ้านซำหวาย',
    'เจริญศึกษา',
    'บ้านดงเมย',
    'บ้านคอแลน',
    'บ้านขอนแป้น',
    'บ้านหนองกบ',
    'บ้านป่าแขมหนองเรือ',
    'บ้านห้วยทราย',
    'บ้านแก้งยาง',
    'บ้านโนนสำราญ',
    'บ้านหนองบัว',
    'บ้านโนนเขื่องจงเจริญ',
    'ชุมชนบ้านนาโพธิ์',
    'บ้านโนนหมากเดือย',
    'บ้านหนองยู',
    'บ้านโนนค้อ',
    'บ้านโนนสว่าง',
    'บ้านโนนสวรรค์',
    'บ้านหัวแข้',
    'บ้านห้วยปอ',
    'บุณฑริก',
    'บ้านโนนบากมิตรภาพที่ 83',
    'บ้านดอนโจด',
    'บ้านนาแคน',
    'บ้านหาดทรายคูณ',
    'บ้านโนนกาหลงน้อย',
    'บ้านดอนงัว',
    'บ้านแมด',
    'บ้านป่าสน',
    'บ้านหนองแสง',
    'บ้านโนนน้อย',
    'บ้านโนนสูง',
    'บ้านป่าเตี้ย',
    'บ้านหลักป้ายประชานุเคราะห์',
    'บ้านโนนหุ่ง',
    'บ้านนาโดม',
    'บ้านอุดมชาติ',
    'บ้านโนนเลียง',
    'บ้านกรุงเจริญ',
    'บ้านสร้างม่วงส้มเสี้ยว',
    'บ้านโนนจิก',
    'ชุมชนบ้านหนองสะโน',
    'บ้านหนองม่วง',
    'บ้านสมพรรัตน์',
    'บ้านแก้งสว่าง',
    'บ้านบก',
    'บ้านเก่ากลาง',
    'บ้านหนองแปน',
    'บ้านสร้างหอม',
    'บ้านคำบาก',
    'บ้านแก้งสมบูรณ์',
    'บ้านห้วยข่า',
    'บ้านหนองเม็ก',
    'บ้านหนองแปก',
    'บ้านซำงู',
    'บ้านกุดเรือ',
    'บ้านทุ่งช้าง',
    'บ้านหนองหว้ากุดไก่แก้ว',
    'บ้านโคกชำแระ',
    'แก้งบ้านหนองสนม',
    'บ้านคำกลางวังม่วง',
    'บ้านนาเกษม',
    'บ้านโนนใหญ่',
    'บ้านหนองถาวร',
    'บ้านนาห่อม',
    'บ้านโนนจานหนองสีขา',
    'บ้านน้ำขุ่นคำนกเปล้า',
    'บ้านหนองบัวอารี',
    'บ้านหนองอ้ม',
    'บ้านทองสวัสดิ์',
    'บ้านโนนรังหนองบัวดง',
    'บ้านเบ็ญจ์โนนดู่',
    'บ้านห่องปอ',
    'บ้านหนองขี้เห็น',
    'บ้านตาโอง',
    'บ้านโนนสวรรค์',
    'บ้านหนองดินดำ',
    'บ้านขี้เหล็ก',
    'บ้านห้วยเสลา',
    'บ้านหนองหัวลิง',
    'บ้านโนนยาง',
    'บ้านห้วยยาง',
    'ศิษย์เก่าวังหลังวัฒนา(บ้านโคกสะอาด)',
    'บ้านตาเกา',
    'บ้านหมากแหน่ง',
    'บ้านน้ำขุ่น',
    'บ้านซำสะกวยน้อย',
    'บ้านแสนถาวร',
    'บ้านดอนโมกข์',
    'บ้านโนนสว่าง',
    'บ้านไพบูลย์',
    'บ้านหนองดุม',
    'บ้านหนองโด',
    'อิสานเศรษฐกิจ',
    'เดชอุดม',
    'นาจะหลวย',
    'นาส่วงวิทยา',
    'ดงสว่างวิทยา',
    'บัวงามวิทยา',
    'นากระแซงศึกษา',
    'ท่าโพธิ์ศรีพิทยา',
    'ทุ่งเทิงยิ่งวัฒนา',
    'น้ำยืนวิทยา',
    'โดมประดิษฐ์วิทยา',
    'บุณฑริกวิทยาคาร',
    'นาโพธิ์วิทยา',
    'ห้วยข่าพิทยาคม',
    'ทุ่งศรีอุดม',
    'น้ำขุ่นวิทยา',
    'บ้านเชือกน้อย',
    'บ้านหนองบั่ว',
    'บ้านขั้นไดใหญ่',
    'บ้านคำฮี',
    'บ้านหนองบั่ว (สาขาบ้านโพนขวาว)',
    'บ้านสะแนน(สามัคคีวิทยาคม)',
    'บ้านขุมเงิน',
    'บ้านหนองบัว',
    'บ้านคุยตับเต่า',
    'บ้านหนองหอย',
    'บ้านกุดกง',
    'บ้านดอนแก้ว',
    'บ้านเขื่องคำ',
    'บ้านกว้างท่าเยี่ยม',
    'บ้านค้อเหนือ',
    'บ้านดอนกลอย',
    'บ้านคำน้ำสร้าง',
    'บ้านดอนยางกล้วยสำโรง',
    'บ้านท่าเยี่ยม',
    'บ้านแจ้งน้อย',
    'บ้านผือเหมือดม่วงพัฒนา',
    'บ้านสามเพียแสนจำปา',
    'บ้านพลับหนองคำ',
    'บ้านดุ่ทุ่งคำบอน',
    'บ้านเชือก',
    'บ้านนาคำ',
    'ไทยรัฐวิทยา 63 (ชุมชนบ้านคำแดง)',
    'บ้านเดิด',
    'บ้านบาก',
    'บ้านน้ำโผ่',
    'บ้านใหม่ชุมพร',
    'บ้านตาดทอง',
    'บ้านสะเดา (คุรุประชาวิทยาคาร)',
    'บ้านหนองแฝก',
    'บ้านดอนมะยาง กม.3',
    'อนุบาลยโสธร',
    'บ้านทุ่งแต้',
    'บ้านคำเม็ก',
    'บ้านคำน้ำเกี้ยง',
    'บ้านหนองเป้า',
    'บ้านหนองไม้ตาย',
    'บ้านทุ่งนางโอก(อ่อนอำนวยศิลป์)',
    'บ้านนาดีดอนจาน',
    'บ้านนาสะไมย์',
    'บ้านผือฮีนาลานาจาน',
    'บ้านชาดศาลา',
    'บ้านสมสะอาดหนองแวง',
    'บ้านกุดจอกห้องพอกขี้เหล็ก',
    'บ้านน้ำคำใหญ่',
    'บ้านตับเต่า',
    'บ้านน้ำคำน้อย',
    'บ้านห้องข่าหนองเสือตาย',
    'บ้านสำราญ',
    'บ้านหนองนางตุ้มบากหัวคำ',
    'บ้านสว่างเชียงหวาง',
    'บ้านกุดระหวี่',
    'บ้านสิงห์',
    'บ้านหนองขอนโพนสิม',
    'บ้านหนองไร่',
    'หนองแสง',
    'บ้านโนนทรายงาม',
    'ชุมชนบ้านหนองคู',
    'บ้านดงบัง',
    'บ้านยางเดี่ยวหนองถ่ม',
    'บ้านโนนค้อ',
    'บ้านโต่งโต้น',
    'บ้านนาสีนวล',
    'บ้านดอนกลองหนองไฮ',
    'บ้านหนองสรวง',
    'บ้านหนองเรือ',
    'บ้านหนองทองหลางโนนกุง',
    'บ้านหนองบกโนนสวาท',
    'บ้านหนองหิน',
    'บ้านหนองเป็ด',
    'บ้านหนองหงอก',
    'บ้านกุดกุง',
    'บ้านโนนม่วง',
    'บ้านเหล่ามะเขียว',
    'บ้านทรายงาม',
    'บ้านนาโพธิ์',
    'ชุมชนบ้านกู่จาน',
    'บ้านนาเวียง',
    'บ้านหนองเสียว',
    'ประชาสงเคราะห์',
    'บ้านแคนน้อยหนองเลิง',
    'บ้านกลางนา',
    'บ้านนาถ่ม',
    'บ้านบกน้อย',
    'ชุมชนดงแคนใหญ่',
    'บ้านผักบุ้ง',
    'บ้านดงเจริญ',
    'บ้านคำแหลม',
    'บ้านแหล่งหนู',
    'บ้านทุ่งมน',
    'โซงเหล่าโป่วิทยา',
    'บ้านมะพริกดู่โพนสิม',
    'บ้านนาแก',
    'บ้านนาหลู่',
    'บ้านหนองเทา',
    'บ้านปลาอีด',
    'บ้านหัวขัว',
    'บ้านนาห่อม',
    'บ้านนาคำ',
    'บ้านน้ำเกลี้ยง',
    'บ้านสำโรง',
    'บ้านโพนทัน',
    'บ้านย่อ',
    'บ้านคำม่วง',
    'บ้านสว่าง',
    'บ้านโคกป่าจิก',
    'อนุบาลลุมพุก(วันครู2503)',
    'คำเขื่อนแก้ว',
    'วุฒิศักดิ์คุรุราษฏร์อุปถัมภ์',
    'บ้านโคกกลาง(วรนาทประชาสรรค์)',
    'บ้านโนนหนองแฝก',
    'บ้านแหล่งแป้น',
    'บ้านเหล่าฝ้าย',
    'บ้านดอนขะยอม',
    'บ้านสงเปือย',
    'บ้านบุ่งหวาย',
    'บ้านกุดตากล้า',
    'บ้านกุดเป่ง',
    'บ้านเหล่าไฮ',
    'บ้านเหล่าหุ่ง',
    'บ้านคุ้ม',
    'บ้านขาม',
    'บ้านคูเมือง',
    'บ้านสำโรง',
    'บ้านพลไว',
    'บ้านราชมุนี',
    'บ้านเลียบ',
    'บ้านยางกลาง',
    'บ้านแดง',
    'บ้านท่าช้าง',
    'ชุมชนบ้านดอนผึ้ง',
    'บ้านปอแดง',
    'บ้านบากเรือ',
    'บ้านดงยาง',
    'บ้านยางเครือ',
    'บ้านดงจงอาง',
    'บ้านยางน้อย',
    'ชุมชนบ้านบึงแกชัยชนะบูรพา',
    'บ้านผือฮี',
    'บ้านหัวดอน',
    'บ้านเปือย',
    'บ้านเวินชัย(คุรุราษฏร์บำรุง)',
    'บ้านโนนกอย',
    'บ้านแดงประชาสรรค์',
    'บ้านพระเสาร์(สามัคคีวิทยาคม)',
    'บ้านหัวดง',
    'บ้านโนนยาง',
    'บ้านขาทรายมะเขือสามัคคี',
    'ฟ้าหยาดราษฏร์นิยม',
    'มหาชนะชัย',
    'บ้านเหมือดป่าตอง',
    'บ้านโนนธาตุ(พิศาลราษฏร์พัฒนา)',
    'บ้านม่วง(คุรุราษฏร์วิชาคม)',
    'บ้านท่าสมอ',
    'บ้านบัวขาว',
    'บ้านหนามแท่ง',
    'บ้านเหล่าใหญ่',
    'บ้านเหมือดขาว',
    'บ้านสร้างแป้น',
    'บ้านสงยาง',
    'บ้านบ่อบึง',
    'บ้านดงขวาง',
    'บ้านซำ',
    'บ้านหัวเมือง',
    'บ้านนาดี',
    'บ้านหนองยาง',
    'บ้านหนองตุ',
    'คูสองชั้น',
    'กุดพันเขียว',
    'บ้านติ้ว',
    'บ้านกุดน้ำใสจานบกน้อย',
    'บ้านสังข์(ศรีเมืองราษฎร์สามัคคี)',
    'บ้านตูม',
    'บ้านแจนแลน',
    'สหประชาสรรค์',
    'บ้านผิผ่วน',
    'อนุบาลค้อวัง',
    'บ้านเหล่าน้อย(ศรีคุรุราษฎร์วิทยา)',
    'บ้านดงมะหรี่',
    'บ้านหมากมาย',
    'บ้านน้ำอ้อม',
    'บ้านโพนแบง',
    'วัดบ้านเปาะ',
    'บ้านบากใหญ่หนองปอหนองซำ',
    'บ้านศิริพัฒนา',
    'บ้านดวนบากน้อย',
    'บ้านจานทุ่ง',
    'บ้านฟ้าห่วน',
    'ยโสธรพิทยาคม',
    'คำเขื่อนแก้วชนูปถัมภ์',
    'สอนแก้วว่องไววิทยา',
    'สมเด็จพระญาณสังวร',
    'กู่จานวิทยาคม',
    'ยโสธรพิทยาสรรค์',
    'โพนทันเจริญวิทย์',
    'สิงห์สามัคคีวิทยา',
    'ศิลาทองพิทยาสรรค์',
    'ราชประชานุเคราะห์ 28',
    'มหาชนะชัยวิทยาคม',
    'ตระกูลประเทืองวิทยาคม',
    'ค้อวังวิทยาคม',
    'บ้านดงมะไฟ',
    'บ้านคำครตา',
    'บ้านโคกก่องดอนนกชุม(อ่างประชานุกูล)',
    'บ้านกุดกว้าง',
    'บ้านคำแขนศอก',
    'บ้านโคกกลางหนองแตะแหละ',
    'บ้านหนองลาดควาย',
    'บ้านสีสุก',
    'บ้านดู่ลาด',
    'บ้านโสกผักหวาน',
    'บ้านเสาเล้า',
    'บ้านทรายมูล(ทรายมูลประชาราษฏร์)',
    'ชุมชนบ้านโคกยาว',
    'บ้านนาโป่ง',
    'บ้านเหล่าเมย',
    'บ้านหนองไก่ขาว',
    'บ้านดอนหวาย',
    'บ้านนาเวียง',
    'บ้านโป่ง(คำประชานุกูล)',
    'บ้านหนองแวง',
    'บ้านบะคอม',
    'บ้านสร้างช้างไผ่หนองไร่มิตรภาพที่ 143',
    'บ้านโคกกลาง',
    'บ้านนาเรียง',
    'ชุมชนบ้านกำแมด',
    'บ้านกุดหิน',
    'บ้านหนองตาไก้',
    'บ้านหนองเหี่ย',
    'บ้านโนนยาง',
    'บ้านหัวงัว(อุสุภราชวิทยาคาร)',
    'บ้านกุดชุม (คุรุราษฎร์บำรุง)',
    'บ้านหนองเมืองกลาง',
    'บ้านสองคร',
    'บ้านเอราวัณ',
    'บ้านหนองบอน',
    'บ้านหนองเรือ',
    'บ้านหนองแก',
    'บ้านกุดชุมพัฒนา(ศรีวิสารรังสรรค์)',
    'บ้านเหล่าตำแย',
    'บ้านคำน้ำสร้าง',
    'บ้านเที่ยงนาเรียง',
    'บ้านดงสำราญ',
    'บ้านภูถ้ำพระพรเจริญ',
    'บ้านนาโส่',
    'บ้านโสกท่าลาดพัฒนา',
    'บ้านกุดแดง',
    'บ้านนาซึม',
    'ชุมชนประชาสรรค์',
    'ชุมชนบ้านโนนเปือย(มหาพิมพ์สงเคราะห์)',
    'บ้านผักกะย่า',
    'บ้านหวายกุดสว่าง',
    'บ้านสุขเกษม',
    'บ้านสุขเกษม(สาขาบ้านคำม่วงไข่)',
    'บ้านโพนงาม',
    'บ้านแดงหนองแซง',
    'บ้านคำเลา',
    'บ้านม่วง',
    'บ้านสร้างแต้',
    'บ้านนาทม',
    'บ้านโคกศรี',
    'บ้านหนองหว้า',
    'บ้านหนองหมี',
    'บ้านกุดไกรสร',
    'บ้านคำบอน',
    'บ้านโพนดินแดง',
    'บ้านหนองแหน',
    'บ้านคำผักหนาม',
    'บ้านโนนประทาย',
    'บ้านคำก้าว',
    'บ้านโนนใหญ่',
    'บ้านหัวนา',
    'บ้านหนองบัวบาน',
    'บ้านโสกน้ำขาว',
    'บ้านห้วยแก้งหนองศาลา',
    'บ้านนิคม',
    'บ้านหนองบาก',
    'บ้านเชียงเครือคำสร้างบ่อ',
    'บ้านโคกพระเจ้าโนนรัง',
    'บ้านม่วงไข่',
    'บ้านกระจาย',
    'บ้านคำเกิด',
    'บ้านหนองสำโรง',
    'บ้านโคกสะอาดใหม่สามัคคี',
    'บ้านนาดี(คุรุประชารัฐ)',
    'บ้านดงสวาง',
    'บ้านหนองแข้',
    'บ้านหนองชุม',
    'บ้านโคกกลาง',
    'บ้านโคกสุวรรณ',
    'บ้านโคกนาโก',
    'บ้านคำกลาง',
    'บ้านเซซ่งพัฒนา',
    'บ้านท่าลาดโนนตูม',
    'ชุมชนบ้านเชียงเพ็ง',
    'บ้านเซโนนม่วง',
    'บ้านกลางสระเกษ',
    'อนุบาลป่าติ้ว(โพธาภินันท์นุกูล)',
    'บ้านหนองแสง',
    'บ้านโพธิญาณ',
    'บ้านโพธิ์ไทรโพธิ์ศรี',
    'บ้านป่าตอง',
    'ชุมชนบ้านศรีฐาน',
    'บ้านเตาไห',
    'บ้านกุดสำโรง',
    'บ้านกุดแข้ด่อนหนองบัว',
    'บ้านกุดเชียงหมี',
    'บ้านกุดขุ่นสวาสดิ์',
    'บ้านห้วยยาง',
    'บ้านแก้งนาคำ',
    'บ้านกุดมะฮงดงสวาง',
    'บ้านนาโพธิ์อ่างสร้างหิน',
    'บ้านดอนม่วงป่าโมง',
    'บ้านมันปลา',
    'บ้านกุดแห่',
    'บ้านสมสะอาด',
    'บ้านคำเกิ่ง',
    'บ้านกุดคอก่านโคกสำราญ',
    'บ้านหินสิ่ว',
    'บ้านหนองยางห้วยสะแบก',
    'บ้านดอนมะซ่อม',
    'บ้านน้อมเกล้า',
    'บ้านหนองแคนน้อย',
    'บ้านโคกก่อง',
    'บ้านช่องเม็ก',
    'บ้านหนองเลิงคำ',
    'บ้านบุ่งค้า',
    'บ้านป่าขี้ยาง',
    'ชุมชนบ้านนากอกหนองบก',
    'บ้านโคกเจริญ',
    'บ้านหนองแคน',
    'บ้านศรีแก้ว',
    'บ้านโคกใหญ่',
    'บ้านศรีสว่าง',
    'บ้านแสนสำราญ',
    'บ้านสร้างมิ่งมิตรภาพที่ 191',
    'บ้านโนนหาด',
    'บ้านโคกกอกหินเหล็กไฟ',
    'บ้านกุดเสถียร',
    'บ้านคึมยาว',
    'บ้านท่าลาด',
    'บ้านหนองโสน',
    'บ้านสวาท',
    'บ้านดอนฮี',
    'บ้านม่วงกาซัง',
    'บ้านกุดโจด',
    'บ้านนาจาน',
    'บ้านพรหมนิยม',
    'บ้านแยประชาสรรค์',
    'บ้านคึมชาดตาดบกศรีสมพร',
    'อนุบาลเลิงนกทา',
    'บ้านด่าน',
    'บ้านโป่งหนองสิม',
    'บ้านโคกสะอาดเกษตรพัฒนา',
    'บ้านสามัคคี',
    'บ้านหวาย',
    'บ้านโคกสูงอ่างสมบูรณ์',
    'บ้านนาเซแสงวิไล',
    'นาจานเจริญวิทย์',
    'บ้านป่าชาด',
    'บ้านหนองบึง',
    'บ้านท่าศิลา',
    'บ้านโสกสาน',
    'ชุมชนห้องแซงวิทยา',
    'บ้านห้องแซง',
    'บ้านดงยาง(คุรุราษฏร์บำรุง)',
    'บ้านห้องคลองร่องคำ',
    'บ้านโพง',
    'บ้านโนนแดง',
    'บ้านคำเตย',
    'บ้านหนองซ่งแย้',
    'บ้านดงสวรรค์',
    'บ้านหนองลุมพุก',
    'บ้านหนองสนม',
    'บ้านพืชคาม',
    'บ้านหนองสองห้องทรัพย์เจริญ',
    'บ้านคำไผ่',
    'บ้านพอกโนนหนองผือคำไผ่ใต้',
    'บ้านโสกน้ำใส',
    'บ้านหนองนกเขียน',
    'บ้านป่าตาว',
    'บ้านไทยเจริญ',
    'บ้านสามแยกภูกอย',
    'บ้านห้วยยาง(คุรุประชาสรรค์)',
    'บ้านไทยเจริญ(สาขาบ้านเทพเจริญ)',
    'บ้านน้ำคำ',
    'บ้านคำสร้างช้าง',
    'บ้านโชคชัยพร',
    'บ้านนาเฮืองนางช้างเฒ่า',
    'บ้านเหล่าหันทราย',
    'บ้านโนนแดงประชานุเคราะห์',
    'บ้านส้มผ่อ',
    'บ้านนางาม',
    'เลิงนกทา',
    'ทรายมูลวิทยา',
    'กุดชุมวิทยาคม',
    'ป่าติ้ววิทยา',
    'ศรีฐานกระจายศึกษา',
    'ห้องแซงวิทยาคม',
    'ดงมะไฟพิทยาคม',
    'กำแมดขันติธรรมวิทยาคม',
    'บุ่งค้าวิทยาคม',
    'โพนงามพิทยาคาร',
    'หนองแหนพัฒนวิทยาคม',
    'ศรีแก้วประชาสรรค์',
    'เมืองกลางประชานุกูล',
    'นาโปร่งประชาสรรค์',
    'คำเตยวิทยา',
    'น้ำคำวิทยาคม',
    'บ้านผือวิทยายน',
    'สระไข่น้ำขุนีวิทยา',
    'บ้านหนองไผ่ล้อม',
    'บ้านหนองเขื่อง',
    'กุดตุ้มสามัคคีวิทยา',
    'บ้านหนองคอนไทย(คุรุประชาสรรค์)',
    'โนนโพธิ์บะก่ามวิทยา',
    'โสกตลับราษฎร์อุทิศ',
    'ยางบ่ารัฐประชามิตร',
    'โคกสูงราษฎร์สามัคคี',
    'บ้านห้วยตะแคง',
    'บ้านห้วยยาง',
    'โนนหว้านไพล',
    'วัดพุทโธวาท',
    'หนองสระสำราญราษฎร์',
    'บ้านหนองบัวขาว',
    'หนองปลาโดผดุงราษฎร์',
    'โป่งเกตรัฐราษฎร์นุเคราะห์',
    'บ้านท่าหินโงม สาขาบ้านวังคำแคน',
    'บ้านท่าหินโงม',
    'บ้านห้วยหมากแดง',
    'บ้านวังโพน',
    'บ้านโนนพระคำ',
    'บ้านคร้อห้วยชัน(แย้มมงคลสินธุ์)',
    'บ้านตาดโตน(คุรุประชาสามัคคี)',
    'บ้านนาฝาย',
    'บ้านช่อระกา',
    'บ้านกุดขมิ้น',
    'บ้านหนองหญ้าปล้อง',
    'บ้านหนองแวง',
    'นาเสียวศึกษาคาร',
    'บ้านนาวัง',
    'บ้านซับรวงไทร',
    'ไทรงามผดุงวิทย์',
    'บ้านนาสีนวล',
    'บ้านแจ้งน้อย',
    'กะพี้ดอนขวางวิทยา',
    'โนนสำราญวิทยา',
    'ฉิมพลีวิทยา',
    'บ้านโสกหว้า',
    'เมืองน้อยราษฎร์สงเคราะห์',
    'บ้านหนองหลอดมิตรภาพที่ 48',
    'บ้านขี้เหล็กใหญ่',
    'สุนทรวัฒนา',
    'อนุบาลชัยภูมิ',
    'ชุมพลสวรรค์วิทยา',
    'โค้งน้ำตับวิทยาคาร',
    'วังก้านเหลืองดรุณกิจ',
    'กุดเวียนวิทย์ผดุง',
    'บ้านหมื่นแผ้ว',
    'บ้านเล่าวิทยาคาร',
    'บ้านโพธิ์น้อยกุดสวง',
    'บ้านโพธิ์ใหญ่',
    'บ้านโนนทัน',
    'บ้านเสี้ยวใหญ่',
    'บ้านเสี้ยวน้อย',
    'ชุมชนบ้านบุ่งคล้าวิทยา',
    'บ้านขวาน้อย',
    'บ้านหัวนา',
    'บ้านสัมพันธ์',
    'บ้านหนองฉิม',
    'บ้านโนนแดง',
    'บ้านกุดโง้ง',
    'บ้านซับพระไวย์',
    'บ้านหนองหญ้ารังกา',
    'บ้านซับม่วง',
    'โพนทองพิทยา',
    'บ้านนางเม้ง',
    'บ้านโนนกอก',
    'วัดสว่างอารมณ์',
    'บ้านลาดใหญ่',
    'ดงบังราษฎร์นุเคราะห์',
    'บ้านโนนหว้านไพล(คุรุราษฎร์สงเคราะห์)',
    'บ้านโนนคูณ',
    'บ้านกุดเวียน',
    'บ้านกุดละลม',
    'หนองนาแซงวิทยา',
    'บ้านหนองโง้ง',
    'บ้านโนนสมบูรณ์',
    'บ้านดอนหัน',
    'บ้านหนองไผ่พัฒนา',
    'โนนหญ้าคาหนองสระพิทยาคม',
    'ไตรราษฎร์สามัคคีวิทยา',
    'บ้านหนองแหนดอนกู่',
    'ห้วยต้อนวิทยา',
    'บ้านโป่งคองเหนือ',
    'บ้านโป่งคองใต้',
    'บ้านชีลองกลาง',
    'บ้านชีลองใต้',
    'สามพันตาราษฎร์อุทิศ',
    'ไทยรัฐวิทยา 19 (บ้านคำน้อย)',
    'บ้านชีลองเหนือ',
    'บ้านม่วงเงาะ',
    'บ้านห้วยบงเหนือ',
    'บ้านหินกอง',
    'บ้านห้วยหว้า',
    'บ้านห้วยบงใต้',
    'บ้านสงแคน',
    'บ้านกุดตะเคียน',
    'บ้านหินลาด',
    'บ้านวังกำแพง',
    'บ้านหนองกระทุ่ม',
    'ยางนาดีราษฎร์ดำรง',
    'บ้านหนองอ้อ',
    'บ้านหลุบโพธิ์',
    'บ้านกุดไผ่',
    'บ้านห้วยหนองจันทิ',
    'บ้านโนนขี้ตุ่น',
    'บ้านกุดยาง',
    'บ้านกุดยาง สาขาบ้านตาด',
    'บ้านโนนเปลือย',
    'โคกน้อยวิทยา',
    'บ้านกุดหูลิง',
    'บ้านโนนแดง สาขาบ้านโนนเสาเล้า',
    'บ้านโนนแดง',
    'บ้านต้อน',
    'อนุบาลบ้านเขว้า',
    'บ้านเขว้าศึกษา',
    'บ้านโนนตาด',
    'บ้านกุดฉนวน',
    'บ้านคลองไผ่งาม',
    'บ้านโนนหมาว้อ',
    'บ้านหนองโสมง',
    'บ้านหว้าเฒ่า',
    'บ้านซับปลากั้ง',
    'บ้านแก้งยาว',
    'บ้านนาอินทร์แต่ง',
    'บ้านห้วยน้ำคำ',
    'บ้านไร่ป่าไม้แดงวิทยา',
    'ท่าแกวิทยากร',
    'โนนน้อยแผ่นดินทอง',
    'บ้านป่ายาง',
    'บ้านค่ายหางเรียง',
    'บ้านวังปลาฝา',
    'บ้านวังกุ่ม',
    'ท่าแก้งบำรุงศิลป์',
    'บ้านโนนโพธิ์',
    'บ้านห้วยหวาย',
    'ชุมชนบ้านจอก(คอนสวรรค์วิทยากร)',
    'บ้านคอนสวรรค์',
    'บ้านนาเสียวหนองชาติ',
    'บ้านโนนโพธิ์',
    'บ้านหนองทอน',
    'บ้านหนองโก',
    'สหราษฎร์นุเคราะห์',
    'บ้านหนองโนน้อย',
    'บ้านโคกมั่งงอย(คุรุประชาบำเพ็ญ)',
    'บ้านโสกหาด',
    'บ้านหนองแดง(คุรุราษฎร์บำเพ็ญ)',
    'บ้านหนองหว้าหนองหญ้าขาว',
    'บ้านโคกก่องดอนทองวิทยา',
    'บ้านโนนสะอาด(ศิริคุณราษฎร์วิทยา)',
    'บ้านโนนพันชาติ',
    'บ้านดอนหันวิทยา',
    'บ้านลำชี',
    'ชุมชนบ้านโสก',
    'บ้านหนองบัวเพวัง(คุรุราษฎร์รังสรรค์)',
    'บ้านโปร่งคลอง(ประชาวิทยาคาร)',
    'ดอนหันราษฎร์อุปถัมภ์',
    'บ้านโปร่งโก',
    'บ้านยางหวาย',
    'บ้านท่าฉาง',
    'โนนเขวาประสิทธิ์ศิลป์',
    'บ้านนาโจด',
    'หนองบัวบานเย็นวิทยา',
    'บ้านนามน',
    'บ้านสำราญ',
    'บ้านนาฮี',
    'บ้านหนองปลาปึ่งหนองเบ็น',
    'บ้านโนนแต้(คุรุราษฎร์วิทยา)',
    'บ้านฝาย',
    'หนองตาไก้วิทยากร',
    'บ้านหนองขาม(ทองงามวิทยา)',
    'ห้วยไร่วิทยานุกูล',
    'บ้านห้วยยาง',
    'บ้านหลุบเพ็ก',
    'บ้านโสกมูลนาค',
    'บ้านหนองไฮโคกสง่า',
    'บ้านดงเย็นมิตรภาพที่ 167',
    'บ้านนาคานหักประชานุสรณ์',
    'บ้านกุดชุมแสง',
    'บ้านนาทุ่งใหญ่',
    'บ้านหนองหอย',
    'บ้านหนองลุมพุก',
    'บ้านหนองสะแก',
    'บ้านโชคอำนวย',
    'คุรุประชานุกูล',
    'บ้านห้วยกนทา',
    'บ้านปากห้วยเดื่อ',
    'บ้านท่าข่อย',
    'หนองหอยปังบำเหน็จวิทยา',
    'บ้านหัวนาคำ',
    'บ้านโนนลาน',
    'บ้านโนนถาวร',
    'บ้านหนองไห',
    'บ้านบ่อทอง',
    'บ้านภูนกเขียน',
    'ราษฎร์บำรุง',
    'ศรีสว่างสามัคคี',
    'บ้านโนนสะอาด',
    'บ้านหนองเป็ด(เกษตรศาสตร์อนุสรณ์)',
    'บ้านโนนศรีทอง',
    'บ้านห้วยข่าเฒ่า',
    'บ้านโนนเหม่า',
    'บ้านโหล่นสามัคคี',
    'บ้านห้วยกุ่ม',
    'บ้านนางแดด',
    'บ้านโนนศรีสง่า',
    'บ้านคลองเจริญ',
    'บ้านคลองเตยโนนพยอมประชาสรรค์',
    'บ้านโนนศรีสง่า สาขาบ้านวังตอตั้ง',
    'บ้านโคกกรวดหนองพวง',
    'บ้านนางแดดเหนือ',
    'บ้านห้วยหัน',
    'บ้านโนนตูม',
    'บ้านโนนตูม สาขาบ้านทุ่งสว่าง',
    'บ้านทรัพย์เจริญ',
    'บ้านท่าวังย่างควาย',
    'บ้านทรัพย์เจริญ สาขาบ้านหลักแดน',
    'บ้านหนองไฮเหนือ',
    'บ้านลาดเหนือ',
    'บ้านโสกรัง',
    'บ้านหนองนกออกประชาสรรค์',
    'บ้านราษฎร์ดำเนิน',
    'บ้านหนองไฮใต้',
    'ชุมชนบ้านหนองบัวแดง',
    'บ้านลาดใต้',
    'หนองเบ็นเจริญวิทย์',
    'บ้านหนองแห้วพัฒนา',
    'ชุมชนบ้านหนองแวง',
    'บ้านห้วยหว้า',
    'บ้านนาเจริญ',
    'บ้านนารี',
    'บ้านหนองปล้อง',
    'บ้านห้วยไผ่ใต้',
    'บ้านสะพุงเหนือ',
    'บ้านโคกสง่า',
    'บ้านห้วยม่วง',
    'บ้านหนองกุงบำรุง',
    'บ้านเจาทอง',
    'บ้านโนนม่วง',
    'บ้านลาดชุมพล',
    'บ้านลาดไทรงาม',
    'บ้านนาอุดม',
    'บ้านพัฒนาสามัคคี',
    'บ้านหนองหญ้าปล้อง',
    'บ้านแจ้งตราด',
    'บ้านคลองจันลา',
    'บ้านนาระยะพัฒนา',
    'บ้านวังใหญ่',
    'บ้านห้วยร่วม',
    'สหประชาสรร',
    'บ้านซับชมภู',
    'บ้านเจาเหนือ',
    'บ้านปรางค์มะค่า',
    'แหลมทองผดุงวิทย์',
    'บ้านโนนผักหวาน',
    'บ้านหินเหิบคุรุประชาสรรค์',
    'บ้านสามแยกดงสวรรค์',
    'บ้านวังตะกู',
    'บ้านซับสีทอง',
    'บ้านลาดผักหนาม',
    'บ้านห้วยผักหนาม',
    'บ้านหนองฉนวนวิทยาคม',
    'บ้านหนองแวง',
    'ชัยภูมิภักดีชุมพล',
    'สตรีชัยภูมิ',
    'เมืองพญาแลวิทยา',
    'กุดตุ้มวิทยา',
    'บ้านค่ายวิทยา',
    'ห้วยต้อนพิทยาคม',
    'หนองไผ่วิทยานุสรณ์',
    'กาญจนาภิเษกวิทยาลัย ชัยภูมิ',
    'ภูพระวิทยาคม',
    'สตรีชัยภูมิ 2',
    'ชีลองวิทยา',
    'ศึกษาพิเศษชัยภูมิ',
    'บ้านเขว้าวิทยายน',
    'ภูแลนคาวิทยายน',
    'ลุ่มลำชีนิรมิตรวิทยา',
    'คอนสวรรค์',
    'สามหมอวิทยา',
    'โนนสะอาดวิทยา',
    'ปู่ด้วงศึกษาลัย',
    'หนองบัวแดงวิทยา',
    'นางแดดวังชมภูวิทยา รัชมังคลาภิเษก',
    'คูเมืองวิทยา',
    'ภักดีชุมพลวิทยา',
    'เจียงทองพิทยาคม',
    'ศาลาสามัคคี',
    'บ้านระหัด',
    'บ้านขามป้อม',
    'บ้านกุดเลาะ(คุรุประชานุสรณ์)',
    'บ้านโนนตุ่น(คุรุประชาวิทยาคาร)',
    'บ้านสระห้วยยางอุปถัมภ์',
    'บ้านเมืองกลางวิทยา',
    'บ้านโสกเชือกนกเจ่าพัฒนา',
    'บ้านโนนเห็ดไคล',
    'บุปผาราม',
    'บ้านโนนเขวา',
    'บ้านโนนทอง',
    'บ้านโนนหนองไฮ',
    'บ้านบุ่งสิบสี่ราษฎร์บำรุง',
    'บ้านกลาง',
    'บ้านแก้งตาดไซ',
    'บ้านหนองบัว',
    'บ้านนาสีดา',
    'บ้านหลักแดนห้วยโป่งพัฒนา',
    'บ้านเดื่อดอนกลาง(ประชาสงเคราะห์)',
    'บ้านหัวโสก',
    'บึงมะนาววิทยา',
    'บ้านจมื่น',
    'บ้านหนองแต้ราษฎร์สามัคคีวิทยา',
    'บ้านโนนงิ้ว',
    'บ้านเขวา',
    'บ้านบัว(รัฐราษฎร์วิทยา)',
    'บ้านหามแหโนนสังข์สามัคคี',
    'บ้านเป้า(สำราญไชยวิทยา)',
    'บ้านหญ้านาง(ประชาวิทยาคาร)',
    'บ้านพรมห้วยข่าวิทยา',
    'บ้านโนนชาด',
    'บ้านหญ้านาง(ประชาวิทยาคาร) สาขาบ้านโนนสังข์',
    'ชุมชนบ้านเมืองเก่า',
    'บ้านท่าขามไร่เดื่อสามัคคี',
    'บ้านหนองแต้',
    'เกษตรศิลปวิทยา',
    'ชุมชนบ้านหัน',
    'บ้านดอน- นกเอี้ยงเก่า',
    'บ้านโนนหินลาด- กุดกว้าง',
    'บ้านนาไฮ(ประชาอุปถัมภ์)',
    'บ้านตลาดอุดมวิทย์',
    'สาลิกาวิทยา',
    'บ้านโสกคลอง',
    'สระโพนทอง',
    'บ้านโพธิ์(คุรุราษฎร์ประสิทธิ์)',
    'บ้านหนองคูวิทยาคม',
    'บ้านส้มกบ(รัฐราษฎร์บำรุง)',
    'บ้านพีพวย',
    'บ้านโจดพิทยาคาร',
    'บ้านหนองข่า(คุรุประชาวิทยา)',
    'บ้านฉนวน',
    'บ้านท่าคร้อ',
    'บ้านนาสมบูรณ์',
    'บ้านห้วยยางดำ',
    'บ้านหนองแดงกุดจิก',
    'บ้านหนองโพนงาม',
    'บ้านหนองไผ่',
    'บ้านสารจอด',
    'บ้านวังม่วง',
    'บ้านโคกสง่า',
    'บ้านทิกแล้ง',
    'บ้านทิกแล้ง สาขาห้วยหินลับวิทยาสรรค์',
    'บ้านสันติสุข (กฟผ.อุปถัมภ์)',
    'บ้านสารจอดเก่า',
    'บ้านดอนหัน',
    'ชุมชนบ้านบัวพักเกวียน',
    'บ้านสวนอ้อย',
    'บ้านหนองปอแดง',
    'บ้านกวางโจนโนนทอง',
    'คุรุราษฎร์วิทยา',
    'บ้านดอนจำปา',
    'บ้านหนองกุง',
    'บ้านนาล้อม',
    'กุดยม(คุรุราษฎร์ไตรมิตร)',
    'บ้านเรือ',
    'บ้านโนนสาทร',
    'ม่วงราษฎร์สามัคคี',
    'บ้านภูดิน(มิตรผลอุปถัมภ์)',
    'บ้านกุดโคลน',
    'บ้านหนองไรไก่',
    'บ้านกุดแดง',
    'บ้านโนนสาวเอ้',
    'บ้านกุดจอก',
    'บ้านหนองปลา',
    'บ้านธาตุ',
    'บ้านหนองกุงใหม่',
    'บ้านฝายพญานาค',
    'บ้านตาดรินทอง',
    'บ้านหนองคัน',
    'บ้านแก้ง(คุรุวิทยานุสรณ์)',
    'บ้านลาด(ประชาราษฎร์บำรุง)',
    'บ้านนาหัวแรด',
    'บ้านโนนหินแร่',
    'บ้านดอนเตาเหล็ก',
    'บ้านโคกสะอาดแก้งวิทยา',
    'บ้านนาหนองทุ่ม',
    'บ้านหนองแซง',
    'บ้านโป่งดินดำ',
    'บ้านหนองดินดำ (คุรุราษฎร์ผดุงวิทย์)',
    'บ้านฉนวน',
    'บ้านห้างสูง',
    'บ้านดอนไก่เถื่อน',
    'บ้านกุดหมากเห็บ',
    'ฉิมพลีมา',
    'บ้านดงเมย',
    'เพชรราษฎร์บำรุง',
    'เพชรราษฎร์สามัคคี',
    'บ้านหนองงูเหลือม',
    'บ้านโป่งโพธิ์',
    'บ้านกุดขอนแก่น',
    'บ้านเมืองคง',
    'บ้านหนองขาม',
    'บ้านโนนดินแดง',
    'บ้านโนนสลวย',
    'บ้านหนองสองห้อง',
    'ภูมิวิทยา',
    'บ้านแข้ (เจริญราษฎร์วิทยา)',
    'บ้านหว้าทอง',
    'ชุมชนบ้านหนองเซียงซา',
    'บ้านโนนงาม',
    'บ้านโนนทรายคำ',
    'บ้านพรมใต้พิทยาคาร',
    'บ้านกุดหัวช้าง',
    'บ้านหนองบัวพรม',
    'หนองเบนประภากร',
    'บ้านหนองคอนไทย',
    'บ้านมูลกระบือ',
    'บ้านสีปลาด',
    'บ้านโนนข่า',
    'บ้านหัวหนอง',
    'โนนเสลาประสาทวิทย์ สาขาประชาสามัคคี',
    'บ้านหนองหญ้าข้าวนก',
    'หนองเมยสามัคคี',
    'บ้านแดงสว่าง',
    'โนนเสลาประสาทวิทย์',
    'เนรมิตศึกษา',
    'บ้านโนนดินจี่',
    'บ้านหนองแวง',
    'บ้านห้วยพลวง',
    'บ้านหนองเม็ก',
    'บ้านหินลาดพัฒนา',
    'บ้านดอนดู่',
    'บ้านดอนขิงแคงเหล่าน้อย',
    'บ้านหนองแฝก',
    'บ้านวังหิน',
    'ชัยชุมพลวิทยา',
    'บ้านสะเดาหนองไผ่ สาขาเสาเล้า',
    'ชุมชนแท่นประจัน',
    'บ้านมอญ',
    'หัวนานคร',
    'บ้านโนนคูณ',
    'บ้านนาดี',
    'บ้านหนองผักหลอด',
    'บ้านหนองบัว',
    'บ้านข่าเชียงพิน',
    'บ้านสะเดาหนองไผ่',
    'ไตรมิตรพิทยา',
    'บ้านถนนกลาง',
    'บ้านนายม',
    'บ้านดอนหันนาสวรรค์',
    'บ้านหินลาด',
    'โคกสะอาดศึกษา',
    'บ้านเหมือดแอ่หนองโดน',
    'บ้านหลุบค่าย',
    'สามสวนวิทยา',
    'บ้านโจดโนนข่า',
    'บ้านหนองคู',
    'บ้านหนองเรือ',
    'หนองมะเกลือบำรุงวิทย์',
    'บ้านดอนเค็ง',
    'บ้านหนองดินดำ',
    'บ้านโนนสะอาด',
    'บ้านหนองพีพ่วน',
    'บ้านด่านเจริญท่ากอกวิทยาสรรค์',
    'บ้านเก่าวิทยานุกูล',
    'บ้านหนองม่วงบำรุง',
    'บ้านเซียมป่าหม้อ',
    'บ้านโคกกุง',
    'บ้านตลุกหิน',
    'บ้านหนองแวง',
    'บ้านโปร่งสังข์',
    'ชุมชนบ้านแก้งคร้อหนองไผ่',
    'ศรีแก้งคร้อ',
    'บ้านหนองแสงวิทยา',
    'บ้านหนองไฮ',
    'บ้านสระแต้',
    'บ้านยางคำวิทยา',
    'บ้านท่ามะไฟหวาน',
    'บ้านกุดโง้งดงสวรรค์',
    'บ้านท่าทางเกวียน',
    'บ้านทุ่งคำหลวง',
    'บ้านนกเขาทอง',
    'บ้านโคกล่าม',
    'บ้านโคกงาม',
    'บ้านดงพอง',
    'บ้านนาหนองทุ่ม',
    'บ้านนาแก',
    'บ้านหนองรวก',
    'บ้านหนองไผ่ล้อม',
    'บ้านโคกสูง-วังศิลา',
    'หว้านไพรสามัคคี',
    'กุดเก่าน้อยบำรุงวิทย์',
    'บ้านดงขีสะเดาหวาน',
    'บ้านโนนสะอาด',
    'บ้านแก้ง-โนนส้มกบ',
    'บ้านหนองขาม',
    'บ้านหนองมะเขือหนองตานา',
    'บ้านหนองแห้วปรางค์กู่',
    'บ้านโนนงิ้ว',
    'บ้านหนองช้างเอก',
    'บ้านซำมูลนาก',
    'บ้านหนองสามขา',
    'บ้านโปร่ง',
    'บ้านโนนคูณ',
    'บ้านหนองแกหนองโน',
    'บ้านหนองสังข์',
    'บ้านหนองพอก',
    'บ้านหนองศาลาป่าชาด',
    'หนองแต้วรวิทย์',
    'โคกม่วงศึกษา',
    'บ้านหนองหญ้าปล้อง',
    'บ้านก่าน',
    'บ้านหนองปลามันหนองไร่',
    'โนนศิลาเหล่าเกษตร',
    'บ้านเหล่ากาดย่า',
    'บ้านภูดินหินกอง',
    'บ้านเลิงทุ่ม',
    'บ้านโคกไพรวัน',
    'บ้านโสกหว้าโนนหอม',
    'บ้านหลุบคา',
    'บ้านคอนสาร',
    'บ้านน้ำพุปางวัว',
    'บ้านเซินใต้',
    'บ้านคลองบอน',
    'บ้านฝายดินสอ',
    'บ้านดงกลาง',
    'บ้านดงใต้',
    'บ้านหนองตากล้า',
    'บ้านกุดแคน',
    'บ้านท่าศาลา',
    'บ้านโคกนกทา',
    'บ้านดงบัง',
    'บ้านม่วง',
    'บ้านนาบัว',
    'บ้านหินรอยเมย',
    'บ้านนาเกาะ',
    'บ้านน้ำอุ่น',
    'บ้านนาวงเดือน',
    'บ้านน้ำพุหินลาด',
    'บ้านโนนหัวนา',
    'บ้านทุ่งพระ',
    'บ้านห้วยแก้ว',
    'บ้านห้วยไห',
    'บ้านท่าเริงรมย์',
    'บ้านห้วยทรายทอง',
    'ออป.19 (ส่วนป่าคอนสาร)',
    'บ้านทุ่งลุยลาย(กฟผ.อุปถัมภ์)',
    'บ้านน้ำทิพย์',
    'บ้านหนองหญ้าโก้ง',
    'บ้านหนองเชียงรอด',
    'บ้านโนนโจด',
    'บ้านโนนสง่า',
    'บ้านโนนคูณ',
    'บ้านโสกมะตูม',
    'บ้านโจดกลาง',
    'บ้านดอนอุดม',
    'ชุมชนห้วยยาง',
    'บ้านหนองหล่มคลองเตย',
    'บ้านโนนเพิ่ม',
    'บ้านป่าว่าน',
    'บ้านป่าว่าน สาขามอตาเจ๊ก',
    'บ้านปากช่องผาเบียด',
    'บ้านเป้าวิทยา',
    'โนนกอกวิทยา',
    'บ้านหันวิทยา',
    'บ้านเดื่อวิทยาคม',
    'เกษตรสมบูรณ์วิทยาคม',
    'ภูเขียว',
    'กวางโจนศึกษา',
    'หนองคอนไทยวิทยาคม',
    'พระธาตุหนองสามหมื่น',
    'โคกสะอาดวิทยา',
    'เพชรวิทยาคาร',
    'สระพังวิทยาคม',
    'บ้านแท่นวิทยา',
    'แก้งคร้อวิทยา',
    'หนองสังข์วิทยายน',
    'นาหนองทุ่มวิทยา',
    'ท่ามะไฟหวาน',
    'มัธยมบ้านแก้งวิทยา',
    'มัธยมหนองศาลา',
    'หนองขามวิทยา',
    'คอนสารวิทยาคม',
    'โนนคูณวิทยาคาร รัชมังคลาภิเษก',
    'ห้วยยางวิทยาคม',
    'บ้านวังเสมา(พินิจอนุสรณ์)',
    'บ้านกุดน้ำใส(3 พระครูอนุสรณ์)',
    'บ้านตลาด(คำนายครุราษฎร์รังสรรค์)',
    'บ้านร่วมมิตร',
    'บ้านท่าแตง',
    'บ้านเดื่อ',
    'บ้านสำโรงโคก',
    'จัตุรัสวิทยานุกูล',
    'บ้านมะเกลือโนนทอง',
    'บ้านสำโรงทุ่ง',
    'บ้านหลุบงิ้ว',
    'บ้านสระสี่เหลี่ยม',
    'บ้านทุ่งสว่างวัฒนา',
    'บ้านขามราษฎร์นุกูล',
    'บ้านโนนคร้อ',
    'บ้านใหม่นาดี',
    'บ้านนา',
    'บ้านหนองลูกช้าง',
    'บ้านงิ้ว',
    'บ้านละหาน(อภิรักษ์วิทยา)',
    'บ้านห้วยยาง',
    'บ้านหนองหญ้าข้าวนก',
    'บ้านดอนละนาม',
    'บ้านโนนจาน(เนตรขันธ์ราษฎร์บำรุง)',
    'กันกงบ้านลี่สามัคคี',
    'บ้านโคกแพงพวย(บัวประชาสรรค์)',
    'บ้านส้มป่อย',
    'บ้านโนนเชือก',
    'บ้านหนองม่วง',
    'บ้านห้วยบง',
    'บ้านห้วย',
    'บ้านหนองโดน',
    'บ้านยางเครือ',
    'บ้านร้านหญ้า',
    'บ้านพลับ(มีราษฎร์บำรุง)',
    'บ้านโนนฝาย',
    'ชุมชนหนองบัวโคก(ทองวิทยานุกูล)',
    'บ้านหนองลุมพุก',
    'บ้านโสกรวกหนองซึก',
    'บ้านหนองโสน',
    'บ้านทามจาน',
    'ชุมชนบัวบานสามัคคี',
    'บ้านโคกโต่งโต้น',
    'บ้านหลักศิลา',
    'หนองบัวใหญ่(เขตการทางนครราชสีมาสงเคราะห์ 4 )',
    'บ้านจาน(นิเทศอุปถัมภ์)',
    'บ้านหนองไผ่ล้อม',
    'บ้านหนองบัวรอง(อินทรผลบำรุง)',
    'บ้านเกาะมะนาว',
    'จอมแก้ววิทยา',
    'โคกเพชรวิทยาคาร',
    'โคกสำราญราษฎร์อำนวย',
    'บ้านโคกคึม(สง่าประชาสรรค์)',
    'บ้านซับยาง',
    'บ้านหนองตะครอง',
    'ปากจาบวิทยา',
    'บ้านโคกสะอาด',
    'บ้านโคกหินตั้งศึกษาศิลป์',
    'บ้านโปร่งมีชัย',
    'บ้านกุดตาลาด',
    'โคกเริงรมย์ศิลปาคาร',
    'บ้านปะโคตะกอ(ประชาสามัคคี)',
    'บ้านหนองตะไก้',
    'ชุมชนชวนวิทยา',
    'บ้านเสลาประชารัฐ',
    'หินตั้งพิทยากร',
    'บ้านห้วยทราย',
    'บ้านกุดแคนสามัคคี',
    'บ้านหัวทะเล"ผ่องประชาสรรค์"',
    'บ้านตาล',
    'วังกะอาม',
    'บ้านหนองอีหล่อ',
    'บ้านโคกสว่าง(คุรุประชานุกูล)',
    'บ้านกลอยสามัคคี',
    'หนองกกสามัคคี',
    'ชุมชนบ้านเพชร(วันครู 2500)',
    'ชุมชนบ้านหนองแวง(คุรุราษฎร์อุปถัมภ์)',
    'บ้านหนองผักแว่น',
    'บ้านทองคำพิงวิทยา',
    'บ้านโคกแฝกดอนทะยิง',
    'หัวสระวิทยา',
    'เขาดินพิทยารักษ์',
    'บ้านกุ่ม(คุรุประชาสามัคคี)',
    'บ้านหนองดง',
    'หนองประดู่วิทยา',
    'บ้านโคกแสว',
    'บ้านละหานค่าย',
    'บ้านตะลอมไผ่',
    'บ้านหนองจาน',
    'บ้านโคกสะอาด',
    'บ้านหนองม่วง',
    'โป่งขุนเพชร',
    'บ้านดอนกอก',
    'บ้านดอนกอก สาขาบ้านห้วยทับนาย',
    'บ้านวังตะเฆ่',
    'บ้านวังกะทะ',
    'บ้านหัวสะพาน',
    'บ้านวังอ้ายจีด',
    'บ้านสำนักตูมกา',
    'บ้านท่าโป่ง',
    'บ้านสำนักตูมกา สาขาบ้านหลังสัน',
    'บ้านวังกะทะ สาขาบ้านวังอุดม',
    'บ้านสำนักตูมกา สาขาบ้านปากดง',
    'บ้านท่าศิลา',
    'บ้านหนองตานา',
    'บ้านโสกปลาดุก',
    'ป่าไม้อุทิศ 3 (บ้านท่าช้าง)',
    'ชุมชนบ้านหนองบัวระเหว',
    'บ้านหัวหนอง',
    'บ้านปากค่ายช่องแมว',
    'บ้านหนองโจด',
    'ชุมชนบ้านห้วยแย้',
    'บ้านหนองกองแก้ว',
    'บ้านห้วยยางดี',
    'บ้านห้วยไฮ',
    'บ้านแจ้งใหญ่',
    'บ้านใหม่สามัคคี',
    'บ้านนาประชาสัมพันธ์ สาขาโคกอนุ',
    'บ้านน้ำลาด',
    'บ้านนาประชาสัมพันธ์',
    'บ้านวังตาท้าว',
    'บ้านเสลี่ยงทอง',
    'บ้านหัวสะพาน',
    'บ้านนาประชาสัมพันธ์ สาขาห้วยน้อย',
    'หินฝนวิทยาคม',
    'บ้านโคกสะอาด',
    'ออป.20 (สวนป่าเทพสถิต)',
    'บ้านวังอ้ายคง',
    'วังใหม่พัฒนา',
    'บ้านวังตาเทพ',
    'วังโพธิ์สว่างศิลป์',
    'บ้านไร่พัฒนา',
    'บ้านโคกกระเบื้องไห',
    'บ้านหนองใหญ่',
    'บ้านโนนสำราญ',
    'บ้านโนนสวรรค์',
    'บ้านเทพพนาธรรมนิติอุปถัมภ์',
    'บ้านโป่งนกพิทยา สาขาบ้านคลองผักหวาน',
    'บ้านสะพานหิน',
    'บ้านโป่งนกพิทยา',
    'บ้านโป่งขุนเพชรพัฒนา',
    'บ้านสะพานยาว',
    'บ้านศิลาทอง',
    'บ้านบุ่งเวียนวิทยา',
    'บ้านไทรงาม',
    'ซับมงคลวิทยา',
    'บ้านวังมน',
    'ซับถาวรพัฒนา',
    'บ้านโคกไค',
    'บ้านยางเกี่ยวแฝก',
    'บ้านซับหมี',
    'บ้านห้วยเกตุ',
    'บ้านยางเตี้ยโคกรัง สาขาบ้านหนองหินสามัคคี',
    'อนุบาลเทพสถิต',
    'บ้านดงลาน',
    'บ้านช่องสำราญ',
    'บ้านซับไทร',
    'บ้านยางเตี้ยโคกรัง',
    'บ้านประดู่งาม',
    'ชุมชนบ้านห้วยยายจิ๋ว',
    'บ้านหนองจะบก',
    'บ้านโนนมะกอกงาม',
    'บ้านวังตาลาดสมบูรณ์',
    'บ้านวังตาลาดสมบูรณ์ สาขาวังบายศรี',
    'บ้านวังคมคาย',
    'ร่วมราษฎร์วิทยานุกูล',
    'บ้านหนองดินดำ',
    'บ้านขี้เหล็ก',
    'บ้านหนองขาม',
    'บ้านหนองแขม',
    'บ้านโกรกกุลา',
    'บ้านสามหลักพัฒนา',
    'ตาเนินราษฎร์วิทยาคาร',
    'บุ่งหวายโกรกตาแป้นสามัคคี',
    'รังงามวิทยา',
    'บ้านหนองตอ',
    'ดอนเปล้าศึกษา',
    'บ้านโสกคร้อ',
    'บ้านหนองผักชี',
    'บ้านบะเสียว',
    'บ้านหนองฉิม(สิงห์จันทร์บำรุง)',
    'ซับใหญ่วิทยาคม',
    'บ้านซับเจริญสุข',
    'บ้านวังพง',
    'บ้านบุฉนวน',
    'บ้านโนนสะอาด',
    'บ้านตลุกคูณพัฒนา',
    'บ้านบุฉนวน สาขาหนองใหญ่',
    'บ้านเขื่อนลั่น',
    'บ้านหนองนกเขียน',
    'บ้านซับใหม่',
    'บ้านวังกุง',
    'บ้านวังขอนสัก สาขาซับสายออประชาอุปถัมภ์',
    'บ้านท่ากูบ',
    'บ้านโป่งเกต',
    'บ้านวังขอนสัก',
    'บ้านท่าชวน',
    'จัตุรัสวิทยาคาร',
    'โนนคร้อวิทยา',
    'หนองบัวบานวิทยา',
    'ละหานเจริญวิทยา',
    'กุดน้ำใสพิทยาคม',
    'บำเหน็จณรงค์วิทยาคม',
    'บางอำพันธ์วิทยาคม',
    'เพชรพิทยาสรรค์',
    'เริงรมย์วิทยาคม',
    'หนองบัวระเหววิทยาคาร',
    'ห้วยแย้วิทยา',
    'เทพสถิตวิทยา',
    'นายางกลักพิทยาคม',
    'โป่งนกประชาสามัคคี',
    'ตรีประชาพัฒนศึกษา',
    'เนินสง่าวิทยา',
    'มัธยมชัยมงคลรังงาม',
    'บ้านกุดปลาดุก',
    'บ้านวังแคน',
    'บ้านนาสีนวน',
    'บ้านป่าติ้ว',
    'บ้านนาเมือง',
    'โคกศรีหนองคลอง',
    'บ้านสามัคคีพัฒนา',
    'บ้านไก่คำ',
    'บ่อบุโปโล',
    'บ้านไร่สมบูรณ์',
    'บ้านโคกค่าย',
    'บ้านคึมใหญ่',
    'บ้านภูเขาขาม',
    'บ้านดอนไร่',
    'บ้านหนองเม็ก',
    'บ้านนาคำ',
    'บ้านหัวภู',
    'บ้านดอนเมย',
    'บ้านหนองปลิง',
    'บ้านก่อ(รามคำแหงอนุสรณ์ 2)',
    'หนองนาเทิงโนนผึ้ง',
    'บ้านนาจิกหนองทุ่งมนต์หนองชาด',
    'บ้านเชือก',
    'นาแต้โคกสำราญ',
    'บ้านกุดน้ำกิน',
    'บ้านเหล่าหนาด',
    'นาผือโคกกอก',
    'บ้านโคกสวาสดิ์',
    'บ้านโคกสูง',
    'บ้านนาเรือง',
    'บ้านนายม',
    'บ้านนาเยีย',
    'หนองแห่โนนสมบูรณ์',
    'บ้านนาดอกไม้',
    'บ้านโคกช้างฮ้าย',
    'บ้านนาโพธิ์',
    'บ้านดงสวาง',
    'ตำรวจตระเวนชายแดนบ้านหนองมะแซว',
    'นาวังกุดตากล้า',
    'บ้านนาคำสามัคคี',
    'บ้านนาหมอม้า',
    'บ้านหนองเรือ',
    'บ้านแก้งกฐิน',
    'บ้านยางคำ',
    'บ้านดงบังพัฒนา',
    'บ้านคำสร้างบ่อ',
    'บ้านดอนดู่',
    'ชุมชนบ้านน้ำปลีก',
    'บ้านโนนโพธิ์',
    'โพธิ์ศิลานาหว้า',
    'นาห้วยยางสองคอน',
    'ก้าวหน้าสามัคคีคำงูเหลือม',
    'บ้านโนนหนามแท่ง',
    'โสกโดนคำไหลคำเตย',
    'บ้านทับเมย',
    'บ้านคำกลาง',
    'บ้านเจริญสามัคคี',
    'พุทธอุทยาน',
    'มิ่งมงคล',
    'บ้านดอนแดง',
    'บ้านโนนจาน',
    'เมืองอำนาจเจริญ',
    'ดอนหวายตาใกล้',
    'อนุบาลอำนาจเจริญ',
    'ไทยรัฐวิทยา 91(โคกจักจั่น)',
    'บ้านทุ่งสว่าง',
    'ปลาค้าวหนองน้ำเที่ยง',
    'บ้านถ่อนใหญ่',
    'บ้านก่อนาดี',
    'บ้านโคกก่องนคร',
    'บ้านสร้างนกทา',
    'บ้านโนนดู่',
    'คำมะโค้งหนองแฝก',
    'ขามนาเพียงโคกยาว',
    'บ้านบาก',
    'พัฒนาสามัคคี',
    'บ้านดงสีโท',
    'คำมะเบื่อแสงเพชร',
    'บ้านโคกสว่าง',
    'ห้วยไร่หาดทรายมูล',
    'ภักดีเจริญหัวนาโคกช้างมะนาย',
    'บ้านหนองแสง',
    'บ้านคำปอแก้วคำไหล',
    'เหล่าพรวนป่ากุงโนนสวาง',
    'บ้านหนองบัวสงยาง',
    'บ้านคำแก้ว',
    'บ้านคำเดือย',
    'บ้านโคกจักจั่น',
    'ดงแสนแก้วดงสำราญ',
    'บ้านเหล่ายางชุมสามัคคี',
    'นาเจริญหนองแดง',
    'บ้านโคกก่ง',
    'บ้านบุ่งเขียว',
    'บ้านสายนาดงอุปถัมภ์',
    'บ้านหินกองสามัคคี',
    'บ้านพุทธรักษา',
    'บ้านนางาม',
    'บ้านโสกกระแต้',
    'ชุมชนโคกสารวิทยา',
    'บ้านคันสูง',
    'บ้านหินขัน',
    'บ้านโนนกุง',
    'อนุบาลชานุมาน',
    'บ้านนาสีดา',
    'นิคมชานุมานสงเคราะห์ 1',
    'นิคมชานุมานสงเคราะห์ 2',
    'นิคมชานุมานสงเคราะห์ 3',
    'บ้านหินสิ่ว',
    'บ้านหนองแมงดา',
    'บ้านห้วยทม',
    'สามแยกผดุงวิทย์',
    'บ้านคำโพน',
    'บ้านนิคม',
    'บ้านโค้งอร่าม',
    'บ้านหินเกิ้ง',
    'บ้านหินกอง(บ้านนามน)',
    'บ้านสามัคคี',
    'บ้านโคกพระวินัยดี',
    'สามแยกชมภูโคกเจริญยางเครือพัฒนา',
    'บ้านนาป่าแซง(ประชาสามัคคี)',
    'บ้านคำย่านาง',
    'อนุบาลปทุมราชวงศา',
    'บ้านหนองแสง',
    'บ้านห้วยงูเหลือม',
    'บ้านทวีผล',
    'บ้านโนนสำราญ',
    'บ้านโนนงาม',
    'ม่วงโป้หนองน้ำขุ่น',
    'หนองลุมพุกหนองยางสามัคคี',
    'บ้านชูชาติ',
    'บ้านลือนาคำ',
    'บ้านแสนสุข',
    'บ้านโสกใหญ่',
    'บ้านเกษมสุข',
    'บ้านฤกษ์อุดมอู่สะเภา',
    'บ้านสงยางนาตากล้า',
    'หนองข่าป่าหวาย',
    'บ้านหนองไฮ',
    'บ้านตาด',
    'บ้านนาผาง(วิบูลราษฎร์สามัคคี)',
    'บ้านหนองสะโน',
    'บ้านคำน้อย',
    'บ้านสว่างใต้',
    'บ้านดอนดู่',
    'บ้านดอนชาด',
    'บ้านห้วย(คุรุประชาคาร)',
    'บ้านสร้อย(ประชารัฐรังสรรค์)',
    'บ้านจานลาน',
    'บ้านโนนหวางโนนกุง',
    'บ้านผึ้ง',
    'บ้านดอนแดงนาโนน',
    'บ้านศรีคุณ',
    'อุ่มยางหนองดั่ง',
    'นาแวงหนองหัวลิง',
    'บ้านม่วงสวาสดิ์',
    'อนุบาลพนา(อุดมวิทยากร)',
    'บ้านพนานต์(สามัคคีวิทยา)',
    'บ้านตำแย(ประชาสงเคราะห์)',
    'บ้านนาสะแบง',
    'ชุมชนบ้านเสารีก',
    'บ้านอีเก้ง(ประชารัฐนุกูล)',
    'ไม้กลอนดอนหวาย',
    'บ้านโพนเมือง',
    'โนนทุ่งดอนชาด',
    'บ้านหัวดอนขามสามัคคี',
    'โนนสูงโคกกลาง',
    'บ้านถ่อนดอนม่วง',
    'บ้านนายูง',
    'บ้านจิก',
    'บ้านโคกกลางเหนือ',
    'บ้านโนนสูง',
    'บ้านนาเวียง',
    'บ้านห่องเตย',
    'บ้านโพนทอง',
    'บ้านโป่งหิน',
    'บ้านหนองโนสวนโคก',
    'บ้านนาหนองใหญ่',
    'บ้านศรีราชา',
    'ชุมชนบ้านไร่สีสุก',
    'บ้านหนองคล้า',
    'บ้านบก',
    'ชุมชนบ้านนาไร่ใหญ่',
    'บ้านคึมข่า',
    'บ้านนาสะอาด',
    'อนุบาลเสนางคนิคม(หนองทับม้า)',
    'บ้านหนองสามสี',
    'บ้านสมสะอาดเนินกุง',
    'บ้านโนนสมบูรณ์',
    'บ้านหนองไฮ',
    'บ้านหนองมะเสี่ยงโนนม่วง',
    'บ้านนาอุดม',
    'บ้านดอนหมู',
    'บ้านหนองแคน',
    'ชุมชนบ้านคำพระ',
    'โคกชาดกลางท่าโพธิ์',
    'บ้านท่ายางชุม',
    'บ้านโนนหนามแท่ง',
    'บ้านโนนเมือง',
    'บ้านดู่พัฒนา',
    'บ้านเป้า',
    'หัวดงหนองคลอง',
    'เค็งใหญ่หนองงูเหลือม',
    'บ้านชาด',
    'ชุมชนบ้านจิกดู่',
    'บ้านโพนขวาว',
    'บ้านโนนผักหวาน',
    'บ้านหนองเทา',
    'หนองยอดู่ในวิทยา',
    'บ้านขุมเหล็ก',
    'คุรุประชาสรรค์',
    'บ้านชะแงะ',
    'บ้านโนนค้อทุ่ง',
    'บ้านโพนเมืองน้อย',
    'บ้านเหล่าขวาว',
    'บ้านเวียงหลวง',
    'อนุบาลหัวตะพาน(รัตนวารี)',
    'ประชารัฐวิทยา',
    'บ้านหนองแสง',
    'บ้านเสียว',
    'บ้านคำข่า',
    'บ้านคำน้อย',
    'บ้านนาคู(คุรุราษฎร์อุปถัมภ์)',
    'บ้านโนนแคน',
    'หนองไหลคึมน้อยวิทยา',
    'บ้านสร้างถ่อ',
    'บ้านใหม่พัฒนา',
    'บ้านโคกเลาะ',
    'บ้านหนองแก้ว',
    'บ้านโนนค้อ',
    'บ้านบ่อชะเนง',
    'บ้านคำเจริญ',
    'บ้านดอนว่าน',
    'บ้านหนองขอน',
    'บ้านโสกท่าวังหิน',
    'บ้านหัวตะพาน',
    'บ้านกุงชัย',
    'บ้านดงบังโนนจาน',
    'บ้านเหล่าฝ้าย',
    'บ้านหนองยาง',
    'บ้านดอนชีหนองมะยอด',
    'บ้านจอก',
    'บ้านดงมะยางหนองนกหอ',
    'บ้านแยหนองแคน',
    'บ้านโคกกลาง',
    'บ้านกุดสิม',
    'ม่วงหนองแดงหนองกะเลา',
    'ชุมชนเปือยหัวดง',
    'บ้านน้ำท่วม',
    'ประชาสามัคคี',
    'บ้านเหล่าฝ้ายศาลาสามัคคี',
    'บ้านน้ำซับ',
    'บ้านนาดี',
    'บ้านเหล่าเลิง',
    'บ้านไร่ขี',
    'บ้านนาอุดม',
    'บ้านฟ้าห่วน',
    'บ้านหนองไผ่',
    'บ้านหนองหิ้ง',
    'อนุบาลลืออำนาจ',
    'บ้านยางช้า',
    'อำนาจเจริญ',
    'ชานุมานวิทยาคม',
    'ปทุมราชวงศา',
    'ลือวิทยาคม',
    'คึมใหญ่วิทยา',
    'คำเขื่อนแก้ววิทยาคม',
    'ปลาค้าววิทยานุสรณ์',
    'น้ำปลีกศึกษา',
    'นาวังวิทยา',
    'นาจิกพิทยาคม',
    'นายมวิทยาคาร',
    'สร้างนกทาวิทยาคม',
    'อำนาจเจริญ 2',
    'ศึกษาสงเคราะห์อำนาจเจริญ',
    'พนาศึกษา',
    'ศรีคูณวิทยบัลลังก์',
    'เสนางคนิคม',
    'นาเวียงจุลดิศวิทยา',
    'หัวตะพานวิทยาคม',
    'ศรีเจริญศึกษา',
    'จิกดู่วิทยา',
    'ลืออำนาจวิทยาคม',
    'มัธยมแมด',
    'โพธิ์ศรีสะอาดวิทยา',
    'บ้านโนนอุดม',
    'ยางหลวงพิทยาคม',
    'ทุ่งโปร่งประชาสรรค์',
    'บ้านโนนสว่าง',
    'บ้านกุดจิก',
    'บ้านห้วยข่าโนนสมบูรณ์',
    'บ้านหนองกุง',
    'ชุมชนนาคำไฮวิทยา',
    'บ้านกุดฉิม',
    'บ้านนามะเฟือง',
    'บ้านทรายงาม',
    'บ้านโนนสมบูรณ์พัฒนา',
    'บ้านหนองศาลาโนนสว่าง',
    'บ้านกุดเต่า',
    'บ้านนาอ่าง',
    'บ้านโนนขมิ้น',
    'บ้านห้วยโจด',
    'บ้านโนนคูณ',
    'บ้านนาเลิง',
    'บ้านห้วยไร่',
    'ร่มเกล้า',
    'บ้านโนนทัน',
    'บ้านอ่างบูรพา',
    'บ้านภูพานคำ',
    'บ้านหินคูณ',
    'บ้านข้องโป้สาขาบ้านค้อ',
    'บ้านโคกกุง',
    'บ้านข้องโป้สาขาบ้านโนนหวาย',
    'บ้านดอนนาดี',
    'บ้านข้องโป้',
    'ชุมชนบ้านขามธาตุวิทยา',
    'บ้านห้วยค้อ',
    'บ้านโคกม่วย',
    'บ้านพร้าว',
    'บ้านบกโนนเรียง',
    'บ้านศรีสุขนาล้อม',
    'บ้านบกฯสาขาบ้านพันดอน',
    'บ้านโนนสูงใหม่วังทอง',
    'บ้านบุ่งบก',
    'ป่าไม้งามโนนนาดีประชานุกูล',
    'บ้านโคกกลาง',
    'บ้านโคกน้ำเกี้ยง',
    'บ้านลาด',
    'บ้านห้วยหามต่าง',
    'บ้านห้วยลึก',
    'บ้านตำแย',
    'บ้านคึมชาดห้วยบง',
    'บ้านนาวังเวิน',
    'บ้านลำภู',
    'อนุบาลหนองบัวลำภู',
    'พิศาลวิทยา',
    'หนองบัววิทยายน',
    'บ้านวังหมื่น',
    'บ้านวังน้ำขาวชนูปถัมภ์',
    'บ้านสุขเกษม',
    'บ้านห้วยทราย',
    'บ้านภูพานทอง',
    'บ้านเสาเล้าโคกแก่นช้าง',
    'บ้านหนองผือราษฎร์บำรุง',
    'ไทยรัฐวิทยา 81(บ้านหนองภัยศูนย์)',
    'บ้านดอนยานาง',
    'บ้านกองแป่มหนองสวรรค์',
    'บ้านหินลับศิลามงคล',
    'บ้านหนองบัวโซม',
    'บ้านโคกกลางโนนงาม',
    'บ้านหนองปลาขาว',
    'บ้านหนองหว้าวิทยาสรรค์',
    'บ้านเพ็กเฟื้อย',
    'บ้านหนองผำโคกสวรรค์',
    'หัวนาศึกษาวิทย์',
    'บ้านข่าดอนเข็ม',
    'บ้านดินทรายอ่อน',
    'บ้านโนนสวรรค์หนองแก',
    'บ้านดอนหัน',
    'บ้านโนนม่วง',
    'บ้านหนองแสงนาล้อม',
    'ชุมชนบ้านกุดดู่',
    'บ้านหนองแวง',
    'บ้านหนองเม็ก',
    'บ้านโสกช้าง',
    'บ้านหัวขัว',
    'นิคมสงเคราะห์ 2',
    'บ้านกุดคอเมย',
    'โคกม่วงประชาสรรค์',
    'บ้านตาดไฮ',
    'บ้านข่าน้อย',
    'บ้านหนองปิง',
    'บ้านวังมน',
    'บ้านวังมนสาขาชัยมงคล',
    'บ้านโคกใหญ่',
    'บ้านหนองตานา',
    'บ้านหนองสะแบง',
    'บ้านโนนปอแดง',
    'นิคมสงเคราะห์วิทยา',
    'บ้านโคกสะอาด',
    'บ้านดงบาก',
    'บ้านโสกแดง',
    'นิคมสงเคราะห์ 4',
    'บ้านโนนตาล',
    'บ้านโนนสว่าง',
    'บ้านโนนเมือง',
    'บ้านห้วยบง',
    'บ้านโคกม่วงชุม',
    'บ้านโนนสงเปลือย',
    'โนนสังวิทยาสรรค์',
    'บ้านโคกกลาง',
    'บ้านโสกจาน',
    'บ้านฝายหิน',
    'บ้านหนองแวงงิ้วตาก',
    'บ้านท่าศิลา',
    'บ้านกุดแห่',
    'บ้านค้อ',
    'บ้านหนองทุ่ม',
    'บ้านหนองนกเขียน',
    'บ้านกุดฉิม',
    'บ้านหนองเล้าข้าว',
    'นิคมโคกม่วงศรีสมพร',
    'บ้านหินสิ่ว',
    'บ้านถิ่น',
    'บ้านโนนสูงหนองสวรรค์',
    'บ้านกุดกวางสร้อย',
    'บ้านโสกก้านเหลือง',
    'บ้านโสกแคนหนองหญ้าปล้อง',
    'ปรางค์กู่',
    'นิคมวัฒนา 6',
    'บ้านหนองบัวเงิน',
    'บ้านหนองแวงป่งสัง',
    'นิคมสงเคราะห์ 3',
    'บ้านหนองเหมือดแอ่',
    'บ้านท่าลาด',
    'หนองกุงจารย์ผางวิทยา',
    'บ้านโคกป่ากุง',
    'บ้านหนองลุมพุก',
    'บ้านหนองกุงคำไฮ',
    'บ้านกุดสะเทียนใหม่ศรีทอง',
    'บ้านกุดแท่น',
    'บ้านวังคูณ',
    'บ้านนาทม',
    'บ้านนาชุมแสง',
    'บ้านโนนเสถียร',
    'บ้านทรายมูล',
    'บ้านหนองอุ',
    'บ้านหนองขามท่างาม',
    'บ้านดอนข่า',
    'บ้านหนองขามท่างามสาขาบ้านกุดหัวแฮด',
    'บ้านหนองอุสาขาเหล่านาดี',
    'บ้านห้วยบ่อทอง',
    'ชุมชนบ้านนากอก',
    'บ้านโคกม่วงทองวิทยา',
    'บ้านกุดดุกสุขเกษม',
    'บ้านโคกล่าม',
    'บ้านโนนงาม',
    'บ้านโคกสนั่น',
    'บ้านหนองผือท่าปากเป่ง',
    'บ้านแกท่าวารี',
    'บ้านโนนสำราญสมสนุก',
    'บ้านวังไฮ',
    'บ้านโนนข่า(พอง)',
    'นาหนองทุ่มตาลเดี่ยวสระแก้ววิทยา',
    'โนนสงวนสิบเก้าโปร่งวิทยา',
    'บ้านโนนหว้าทอง',
    'บ้านตะเคียนทอง',
    'บ้านฝายหิน',
    'บ้านมอเหนือ',
    'บ้านโนนสะอาด',
    'โนนคูณวิทยา',
    'บ้านทุ่งโพธิ์นาอุดม',
    'บ้านโต่งโต้น',
    'บ้านวังโปร่ง',
    'บ้านวังโพนคึมน้ำเกลี้ยง',
    'หินตลาดศรีสง่าวิทยา',
    'หินตลาดศรีสง่าวิทยาสาขาศรีภูทอง',
    'บ้านวังแคน',
    'บ้านห้วยบง(ธนาคารกรุงเทพฯ 19)',
    'บ้านหนองม่วง',
    'บ้านห้วยไผ่',
    'บ้านมอใต้',
    'เมืองใหม่วิทยา',
    'บ้านหนองทุ่งมน',
    'บ้านโนนสว่าง',
    'บ้านยางหล่อโนนสวนกล้วย',
    'บ้านดอนเกล็ด',
    'บ้านป่าคา',
    'บ้านสันป่าพลวง',
    'บ้านโนนสมบูรณ์',
    'บ้านศรีบุญเรือง',
    'บ้านห้วยฮวกจอมทองนาฝาย',
    'บ้านโคกสูงโคกสวรรค์',
    'บ้านหนองบัวน้อย',
    'บ้านหนองแตง',
    'บ้านห้วยหว้าวังทอง',
    'บ้านนาแพง',
    'บ้านหนองกุงแก้ว',
    'บ้านโนนสะอาดราษฎร์อุปภัมถ์',
    'บ้านคอกวัว',
    'บ้านผาสุก',
    'บ้านหนองคังคา',
    'บ้านนาสำราญรุ่งเรือง',
    'บ้านผาเสด็จ',
    'บ้านผาสุกสาขาผาสวรรค์',
    'หนองแกสระแก้ววิทยา',
    'บ้านหนองโกโนนประดู่วิทยา',
    'บ้านสำราญสุข',
    'บ้านกุดหานสามัคคี',
    'ดอนปอวิทยา',
    'บ้านนาทับควาย',
    'บ้านบุ่งแก้ว',
    'บ้านหนองโน',
    'บ้านสร้างเสี่ยน',
    'กุดจิกวิทยา',
    'โนนข่าประชาสรรค์',
    'บ้านโปร่งแจ้ง',
    'บ้านห้วยกวางทอง',
    'บ้านนาดี"คุรุราษฎร์บำรุง"',
    'หนองบัวพิทยาคาร',
    'บ้านขามพิทยาคม',
    'หนองสวรรค์วิทยาคาร',
    'เฉลิมพระเกียรติสมเด็จพระศรีนครินทร์ หนองบัวลำภู',
    'โนนสังวิทยาคาร',
    'โนนเมืองวิทยาคาร',
    'กุดดู่พิทยาคม',
    'หนองเรือพิทยาคม',
    'ยางหล่อวิทยาคาร',
    'กุงแก้ววิทยาคาร',
    'นากอกวิทยาคาร',
    'กุดสะเทียนวิทยาคาร',
    'ศรีบุญเรืองวิทยาคาร',
    'บ้านกุดดินจี่',
    'บ้านหม่านศรีทองวิทยา',
    'หนองแต้ศึกษาประชาสรรค์',
    'บ้านอาบช้าง',
    'บ้านโนนสว่างบวรวิทยา',
    'บ้านหนองทุ่มโนนสูงวิทยา',
    'กุดแห่วิทยา',
    'บ้านป่าแดงงาม',
    'บ้านสนามชัย',
    'บ้านนาสมใจ',
    'บ้านยางชุม',
    'บ้านเก่ากลอยกุดกระสู้',
    'บ้านหนองแวงคำ',
    'บ้านหนองแสง',
    'บ้านขอบเหล็ก',
    'บ้านห้วยหาน',
    'บ้านดงสวรรค์วิทยา',
    'บ้านอุดมศรีวิไลวิทยา',
    'บ้านโป่งแคศรีถาวร',
    'บ้านหนองเอี่ยน',
    'บ้านหนองด่าน',
    'บ้านหนองบัวคำแสน',
    'บ้านกกค้อกกโพธิ์',
    'นครชัยประชาสรรค์',
    'บ้านโนนไหมโนนศิลา',
    'บ้านโนนเมือง',
    'บ้านภูพระโนนผักหวาน',
    'บ้านภูพระโนนผักหวานสาขามอเลี้ยว',
    'บ้านโนนม่วง',
    'บ้านแสงดาวโนนธาตุ',
    'บ้านก่าน',
    'บ้านเอื้องโนนไร่โนนสาวิทยา',
    'บ้านโนนงาม',
    'บ้านซำเสี้ยว',
    'บ้านโนนตาล',
    'บ้านนาหนองทุ่ม',
    'บ้านฝั่งแดง',
    'บ้านโนนสวรรค์ชัยมงคล',
    'บ้านท่าอุทัย',
    'บ้านโนนสวาทหนองไพบูลย์',
    'บ้านหนองกุงศรีโพธิ์ศรีสมพร',
    'บ้านห้วยนาหลวง',
    'หนองด้วงวังประทุม',
    'บ้านซำขอนแก่น',
    'บ้านโนนป่าหว้านเชียงฮาย',
    'ยูงทองวิทยา',
    'บ้านกุดผึ้ง',
    'บ้านนาไก่นาคำน้อยวิทยา',
    'โนนสง่าราษฎร์บำรุง',
    'บ้านดงมะไฟ',
    'บ้านนาไร่',
    'บ้านวังหินซา',
    'บ้านผาซ่อนโชคชัย',
    'บ้านกุดฮู',
    'บ้านสัมป่อยค่ายเมืองแสน',
    'ชุมชนหนองบัวน้อยโนนชาติหนองเห็นวิทยา',
    'บ้านนาด่าน',
    'บ้านเซินราษฎร์เกษมศรี',
    'บ้านโนนงาม',
    'บ้านนาดีค่ายสว่างวิทยา',
    'บ้านวิจิตรพัฒนา',
    'บ้านโนนสำราญ',
    'บ้านหินฮาวน้ำกงวิทยทสาขาบ้านซำภูทอง',
    'บ้านนาตาแหลวดงยางวิทยา',
    'บ้านโนนสมบูรณ์',
    'บ้านนาสี',
    'บ้านหินฮาวน้ำกงวิทยา',
    'บ้านโคกทุ่งน้อย',
    'โนนอุดมศึกษา',
    'โนนปอแดงวิทยา',
    'บ้านต่างแคน',
    'บ้านดงบังมวลชนรังสรรค์',
    'โนนมะค่าพรมนุสรณ์ราษฎร์บำรุง',
    'บ้านนาโมง',
    'บ้านต้อง',
    'บ้านโคกนกสาริกา',
    'บ้านคลองเจริญ',
    'บ้านบุญทัน',
    'บ้านหนองเหลือง',
    'สุวรรณคูหาวิทยาคาร',
    'นาโกทรายทองวิทยาคม',
    'ชุมชนโปร่งวังม่วง',
    'บ้านภูเขาวง',
    'เทพคีรีพิทยาคม',
    'บ้านนาซำจวง',
    'บ้านนาส้มโฮง',
    'บ้านหนองค้อ',
    'บ้านผาเวียง',
    'บ้านนาแก',
    'บ้านโคกนาเหล่า',
    'บ้านโคกกะทอ',
    'บ้านนาสมนึก',
    'บ้านหินตั้งบังพระจันทร์',
    'เกษตรนาสมหวังสามัคคี',
    'บ้านนากลาง',
    'บ้านวังสำราญ',
    'บ้านผาวัง',
    'บ้านโคกสง่า',
    'บ้านถ้ำช้างอินทร์แปลง',
    'บ้านไทยนิยม',
    'บ้านนาเจริญ',
    'บ้านนาสุรินทร์',
    'บ้านโคกหนองบัว',
    'บ้านโคกสะอาดหนองหัวช้าง',
    'บ้านวังปลาป้อม',
    'สุวรรณคูหาพิทยาสรรค์',
    'นาวังศึกษาวิช',
    'คำแสนวิทยาสรรค์',
    'ดงมะไฟวิทยาคม',
    'นาแกวิทยา',
    'กุดดินจี่พิทยาคม',
    'ฝั่งแดงวิทยาสรรค์',
    'ภูซางใหญ่วิทยาคม',
    'ชุมชนบ้านพรหมนิมิต',
    'บ้านโคกแปะ',
    'บ้านโคกสีวิทยาเสริม',
    'บ้านเลิง',
    'บ้านหนองไหลหนองบัวทอง',
    'บ้านยางหย่อง',
    'บ้านหนองเต่าบึงเรือใหญ่',
    'บ้านหนองหัววัว',
    'บ้านดอนช้าง',
    'บ้านดอนหญ้านาง',
    'บ้านป่าสังข์หนองฮี',
    'บ้านป่าเหลื่อม',
    'บ้านหัวบึงหัวสระ',
    'บ้านดอนแดงดอนน้อยวิทยา',
    'บ้านดอนหันวิทยาสาร',
    'บ้านโนนเขวา',
    'บ้านโนนตุ่นประชาบำรุง',
    'บ้านสว่างมรรคา',
    'บ้านหลุบหญ้าคา',
    'บ้านหนองหญ้าแพรกท่าแร่',
    'บ้านเหล่านกชุมวิทยาสรรค์',
    'บ้านแดงใหญ่',
    'บ้านป่าชาดคุรุประชาบำรุง',
    'บ้านหนองกอยสิทธิราษฎร์บำรุง',
    'บ้านหนองหลุบ',
    'ชุมชนบ้านท่าพระ',
    'บ้านท่าพระเนาว์',
    'บ้านสวนมอนไคร่นุ่นวังหิน',
    'บ้านหนองโข่ย(ประชารัฐบำรุง)',
    'บ้านหนองบัวดีหมี(คุรุสามัคคี3)',
    'บ้านหนองแวงบวรวิทย์',
    'บ้านห้วยเตยพัฒนา',
    'บ้านคำบอน',
    'บ้านโนนท่อนวิทยา',
    'บ้านบึงแก',
    'บ้านหนองค้ากลางฮุง',
    'บ้านหนองเบ็ญ',
    'สนามบิน',
    'อนุบาลขอนแก่น',
    'บ้านโคกสีโคกเปี้ย',
    'บ้านค้อท่อนน้อย',
    'บ้านโนนเรือง',
    'บ้านโนนลาน',
    'บ้านหินลาดวังตอ',
    'บ้านหนองกุงน้อย',
    'บ้านหนองปอ',
    'บ้านซำจานเนินทอง',
    'บ้านตอกแป้น',
    'บ้านเหล่าเกวียนหัก',
    'บ้านม่วง',
    'บ้านหนองกุงคุรุประชาสรรค์',
    'บ้านกุดนางทุย',
    'บ้านแดงน้อย',
    'บ้านทุ่ม',
    'บ้านคำไฮหัวทุ่งประชาบำรุง',
    'บ้านเป็ด',
    'บ้านหนองขามประชาบำรุง',
    'บ้านหนองโจด',
    'บ้านกอก',
    'บ้านโคกฟันโปง',
    'บ้านทองหลาง',
    'บ้านหว้าเหล่าโพนทอง',
    'บ้านหนองคู',
    'บ้านหนองทุ่ม',
    'บ้านเหล่านาดี',
    'บ้านเหล่านางาม',
    'บ้านโคกสูงวิทยาคม',
    'พงษ์ภิญโญ 1',
    'บ้านดอนดู่คุรุราษฎร์วิทยา',
    'บ้านบึงฉิม',
    'บ้านบึงเนียมบึงใคร่นุ่น',
    'บ้านบึงสวางค์คุยโพธิ์',
    'บ้านสงเปือยฮ่องเดื่อ',
    'บ้านดงพอง',
    'บ้านผือ(สวัสดิ์ราษฎร์วิทยา)',
    'พระคือหนองโพธิ์วิทยา',
    'บ้านเลิงเปือย',
    'อานันทนวิทยา',
    'บ้านหนองแสงโคกน้อย',
    'บ้านหนองไฮโพธิ์ชัย',
    'บ้านกุดกว้างประชาสรรค์',
    'บ้านสะอาด',
    'บ้านดอนบม',
    'บ้านโนนตุ่นสามัคคีศึกษา',
    'บ้านโกทา',
    'บ้านดอนยาง',
    'บ้านเต่านอ',
    'บ้านโนนม่วง',
    'บ้านศิลา',
    'บ้านหนองกุงวิทยาคาร',
    'บ้านหนองหิน',
    'บ้านห้วยชัน',
    'บ้านโคกล่าม',
    'บ้านโนนกู่',
    'บ้านม่วงโป้',
    'บ้านสาวะถี',
    'บ้านหนองตาไก้หนองเม็ก',
    'บ้านงิ้ว',
    'บ้านโนนรังวิทยาคาร',
    'บ้านเพี้ยฟานโนนตุ่น',
    'บ้านลาดนาเพียง',
    'บ้านหนองปิง',
    'บ้านหินขาว',
    'บ้านไก่นา',
    'บ้านอัมพวัน',
    'บ้านนาเพียง',
    'บ้านโคกนางามปลาเซียม',
    'บ้านโนนแต้',
    'บ้านน้ำเกี้ยงโนนสว่าง',
    'ไทยรัฐวิทยา84(บ้านสำราญเพี้ยฟาน)',
    'บ้านหนองบัวน้อย',
    'หนองตูมหนองงูเหลือม',
    'บ้านเหมือดแอ่คุรุราษฎร์อุทิศ',
    'บ้านโคกท่า',
    'บ้านดอนธาตุท่าฉางท่าพระทราย',
    'บ้านโคกกว้าง',
    'โคกงามวิทยาคาร',
    'บ้านคำหญ้าแดง',
    'บ้านโนนค้อ',
    'โนนฆ้องวิทยาคาร',
    'พงษ์ภิญโญ 2',
    'บ้านหินตั้งหนองอีเลิง',
    'หินฮาวคุรุประชาสรรค์',
    'ห้วยหว้าวิทยาคม',
    'ชุมชนบ้านฝาง',
    'บ้านแก่นเท่า',
    'โคกใหญ่ประชารัฐวิทยา',
    'ดอนหันประชารัฐศึกษา',
    'สระแก้วราษฎร์บำรุง',
    'ไตรมิตรวิทยาคาร',
    'เขื่อนกระพี้ศึกษา',
    'บ้านค้อ',
    'บ้านแดงราษฎร์สามัคคี',
    'หนองชาดพิทยาคม',
    'หนองแวงคุรุราษฎร์รังสรรค์',
    'บ้านกระเดื่อง',
    'แก่นเท่าพัฒนศึกษา',
    'บ้านคำหัวช้างโนนตุ่นป่ามะนาว',
    'หินกองวิทยา',
    'บ้านหนองคลอง',
    'บ้านป่าหวาย',
    'สว่างศิริพัฒนา',
    'บ้านโสกแต้',
    'บ้านหนองเซียงซุยโนนสะอาด',
    'บ้านบะยาว',
    'บ้านโสกม่วงดอนดู่',
    'ชุมชนบ้านหนองบัว',
    'บ้านนาฝายนาโพธิ์',
    'บ้านขามป้อมสำนักงานสลากกินแบ่งสงเคราะห์86',
    'บ้านชาด',
    'บ้านบ่อแก',
    'บ้านหนองทุ่งมน',
    'ชุมชนบ้านโต้นศรีพิมลวิทยา',
    'บ้านโจดใหญ่',
    'บ้านดงกลาง',
    'บ้านดงเก่า',
    'พระบุบ้านหันราษฎร์ประสาท',
    'บ้านโจดศรีวิชัย',
    'ชุมชนบ้านพระยืน',
    'บ้านแก่นประดู่',
    'บ้านนาล้อม',
    'บ้านโนนบ่อ',
    'บ้านป่าส่าน',
    'บ้านป่าหม้อหนองคู',
    'บ้านหินเหิบศิลาทิพย์',
    'บ้านหนองหญ้าข้าวนก',
    'หนองโพธิ์ประชานุกูล',
    'บ้านหนองแวงหนองจิกโนนตุ่น',
    'พงษ์ภิญโญ 2 สาขาบ้านหนองสำโรง',
    'บ้านวังโพน',
    'ขอนแก่นวิทยายน',
    'กัลยาณวัตร',
    'แก่นนครวิทยาลัย',
    'ท่าพระวิทยายน',
    'สาวะถีพิทยาสรรพ์',
    'โคกสีพิทยาสรรพ์',
    'นครขอนแก่น',
    'เทพศิรินทร์ ขอนแก่น',
    'ขอนแก่นวิทยาลัย',
    'สีหราชเดโชชัย',
    'ขอนแก่นพัฒนศึกษา',
    'ขอนแก่นวิทยายน 2',
    'พิศาลปุณณวิทยา',
    'ขอนแก่นวิทยายน 3',
    'แก่นนครวิทยาลัย 2',
    'ขามแก่นนคร',
    'โสตศึกษาจังหวัดขอนแก่น',
    'ศรีสังวาลย์ขอนแก่น',
    'ฝางวิทยายน',
    'ศรีเสมาวิทยาเสริม',
    'ป่าหวายวิทยายน',
    'ประชารัฐวิทยาเสริม',
    'พระยืนวิทยาคาร',
    'ประเสริฐแก้วอุทิศ',
    'ไตรคามประชาสรรค์',
    'บ้านแคนเหนือ',
    'บ้านโคกโก',
    'บ้านดอนหมากพริกลิ้นฟ้า',
    'บ้านหนองเจ้าเมือง',
    'บ้านไผ่ประถมศึกษา',
    'บ้านหนองลุมพุก',
    'วัดจันทร์ประสิทธิ์',
    'บ้านศิลานาโพธิ์',
    'บ้านโสกตลิ่งโสกจาน',
    'บ้านทางพาดหนองแวงโอง',
    'บ้านหนองแวงไร่',
    'บ้านเก่าน้อย',
    'บ้านทุ่งมน',
    'เบญจมิตรวิทยาคม',
    'บ้านเป้า',
    'บ้านหนองผือกอบง',
    'บ้านเกิ้ง',
    'ชุมชนบ้านไผ่ยิ่งยงอุทิศ',
    'บ้านไผ่แสงทองประชาสรรค์',
    'บ้านขามเรียน',
    'บ้านชีกกค้อ',
    'บ้านดู่ใหญ่',
    'บ้านเมืองเพีย',
    'บ้านละว้า',
    'บ้านหนองนางขวัญ',
    'บ้านผักหวานโนนสวาง',
    'บ้านสร้างแป้นดอนนาแพง',
    'บ้านกุดเชือก',
    'บ้านขามป้อม',
    'บ้านลาน',
    'บ้านสร้างเอี่ยน',
    'บ้านหนองตับเต่า',
    'บ้านหนองดู่ดอนเปือยสาขาบ้านดอนเปือย',
    'บ้านป่าปอ',
    'บ้านป่าปอสาขาบ้านหนองข่าลิ้น',
    'บ้านเสือเฒ่า',
    'บ้านหนองดู่ดอนเปือย',
    'บ้านภูเหล็ก',
    'บ้านหนองหญ้าปล้อง',
    'บ้านหนองรูแข้',
    'ไตรมิตรประชาบำรุง',
    'เก่านาโนมีวิทยา',
    'จตุรมิตรบ้านไผ่',
    'หนองตอกเกี้ยหัวนาประชาสรรค์',
    'บ้านหนองน้ำใส',
    'บ้านหนองร้านหญ้าโนนข่า',
    'บ้านหัวหนองนาวัวโนนงิ้ว',
    'บ้านหินลาดนาโนหนองกุง',
    'บ้านหินตั้งโคกก่อง',
    'บ้านสว่าง',
    'บ้านป่างิ้วหนองฮี',
    'บ้านโกน้อย',
    'บ้านวังผือขามป้อม',
    'บ้านส้มป่อย',
    'บ้านหนองนกเขียนนาเสถียร',
    'บ้านหินฮาวลำวังชู',
    'ชุมชนบ้านหัวขัว',
    'ไตรคามประชาพัฒนา',
    'บ้านโนนสว่าง',
    'บ้านวังม่วง',
    'บ้านโสกนาค',
    'บ้านห้วยโป่ง',
    'บ้านห้วยแร่',
    'บ้านวังหินเก่าค้อ',
    'บ้านนาเสียวโคกสว่าง',
    'บ้านสระแก้วประชาสรรค์',
    'ชุมชนบ้านหันมัญจาคีรี',
    'บ้านโจด',
    'บ้านหนองขามบ่อหิน',
    'บ้านหนองโนประชานุเคราะห์',
    'บ้านหนองม่วงหลุบคา',
    'บ้านขุมดิน',
    'บ้านเขวา',
    'บ้านบัว',
    'ห้วยยางมัญจาคีรี',
    'บ้านหัน(สลากกินแบ่ง 5)',
    'กุดขอนแก่นท่าเกษม',
    'บ้านคำแคน',
    'บ้านคำโซ่',
    'บ้านคำน้อย',
    'บ้านคำปากดาว',
    'บ้านน้อยกลางคำแคนเหนือ',
    'บ้านป่าดู่',
    'บ้านห้วยแล้ง',
    'บ้านหนองขามวิทยา',
    'บ้านท่าศาลาประชารังสรรค์',
    'บ้านท่าสวรรค์',
    'บ้านโนนคูณรัฐประชาสรรค์',
    'บ้านโนนเค็ง',
    'บ้านโนนตุ่นศึกษา',
    'บ้านไส้ไก่',
    'บ้านหัวนา',
    'บ้านหนองต่อโคกหนองโจด',
    'บ้านหนองบัวเย็น',
    'เหล่าใหญ่วิทยา',
    'สวัสดี',
    'หัวห้วยหัวฝาย',
    'บัวเหลือง',
    'บ้านขามป้อมดอนยูง',
    'บ้านนาข่า',
    'บัวเหลือง(สาขาบ้านหนองบัว)',
    'บ้านโคกสูงพิทยาคาร',
    'แก่นเท่าโสกน้ำขุ่น',
    'บ้านแจ้งทัพม้า',
    'บ้านนางาม',
    'บ้านเสาเล้าหินแตก',
    'บ้านหนองสองห้อง',
    'บ้านขามป้อมราษฎร์นุกูล',
    'บ้านนาฮี',
    'บ้านโนนคุต',
    'โนนสวางวิทยาสรรค์',
    'โนนสวางวิทยาสรรค์(สาขาห้วยหินเกิ้ง)',
    'โพนเพ็กพัฒนวิทยา',
    'หนองหญ้าปล้องวิทยา',
    'หนองไฮแจ้งบูรณวิทยา',
    'บ้านห้วยฮวก',
    'บ้านกอกป่าผุวิทยา',
    'บ้านขามคุรุราษฎร์อุปถัมภ์',
    'ไตรมิตรศึกษา',
    'โนนเขวาประชารักษ์',
    'บ้านมูลตุ่น',
    'บ้านสวนหม่อน',
    'บ้านเหล่ากกหุ่งสว่าง',
    'บ้านวังแคน',
    'บ้านแจ้ง',
    'บ้านนาจานบ่อแก้ววิทยาคาร',
    'บ้านโนนสำนัก',
    'บ้านหนองแปน',
    'บ้านหนองหัวช้างดอนพันชาด',
    'บ้านหนองไห',
    'บ้านกุดเพียขอมนาผาย',
    'บ้านกุดเพียขอมเหนือ',
    'บ้านนาดอกไม้ขามเบี้ยดอนข่า',
    'บ้านหญ้าเครือเสาเล้าร่องดูก',
    'ชุมชนบ้านชนบท',
    'บ้านท่านางเลื่อน',
    'โนนพะยอมพิทยไพศาล',
    'บ้านหนองเต่า',
    'บ้านห้วยไร่',
    'บ้านห้วยอึ่ง',
    'ร่มเย็นประชาสรรค์',
    'บ้านแท่น',
    'บ้านโนนแดงน้อย',
    'สระแก้วโคกกลาง',
    'บ้านหัวนากลาง',
    'บ้านหันแฮด',
    'มาบตากล้าร่มเย็น',
    'บ้านหนองไฮหนองแวง',
    'บ้านปอแดง',
    'บ้านโคกพระหนองยายเกลี้ยง',
    'บ้านโนนข่า',
    'บ้านวังแสง',
    'บ้านหูลิงโนนศิลา',
    'บ้านห้วยไผ่โนนคำมี',
    'บ้านห้วยยางประชาสรรค์',
    'ชุมชนโนนแสนสุข',
    'บ้านดอนดู่คุรุราษฎร์บำรุง',
    'บ้านท่าม่วง',
    'บ้านวังเวินกุดหล่ม',
    'บ้านโซ่งเหล่านาดี',
    'บ้านทุ่มห้วย',
    'บ้านห้วยแกดอนหันหนองหญ้าม้า',
    'บ้านหนองสะแบงหนองโน',
    'บ้านเหล่าเหนือ',
    'บ้านหัวฝายโนนสะอาดวิทยา',
    'บ้านซับแดง',
    'บ้านซับบอน',
    'บ้านนาจาน',
    'บ้านนาตับเต่า',
    'หนองหญ้าปล้องโนนสว่าง',
    'บ้านหินตั้ง',
    'บ้านโสกนาดี',
    'บ้านแก้งค้อ',
    'บ้านนาแพงสงแดง',
    'บ้านโนนงาม',
    'บ้านโนนทองวิทยาคม',
    'บ้านโนนสาวเอ้',
    'บ้านดอนหมู',
    'โนนคูณอัมพวัน',
    'ชุมชนบ้านโคก',
    'บ้านหนองทุ่มวิทยา',
    'อุดมคงคาคีรีเขต',
    'บ้านโนนกระยอมกุดลอบ',
    'บ้านโพธิ์ไชย',
    'สามหมอโนนทัน',
    'บ้านมูลนาค',
    'บ้านหนองหญ้ารังกา',
    'บ้านโคกสำราญ',
    'บ้านดงหนองเกี่ยว',
    'บ้านดอนปอแดง',
    'บ้านโนนพันชาติ',
    'บ้านป่าแดงแจ้งกระหนวน',
    'บ้านป่าม่วง',
    'บ้านเล็บเงือก',
    'บ้านหนองมะเขือ',
    'บ้านหนองหัวช้าง',
    'บ้านโนนทัน',
    'บ้านหนองกระหนวนหนองเม็ก',
    'บ้านหนองขามสมบูรณ์',
    'บ้านหนองผักตบ',
    'ชุมชนบ้านแฮด',
    'ประชาพัฒนาบ้านแฮด',
    'บ้านวังหว้า',
    'บ้านหนองไฮขามเปี้ย',
    'บ้านขอนสักรัฐราษฎร์บำรุง',
    'ขามป้อมประชานุกูล',
    'บ้านห้วยม่วงคุรุราษฎร์รังสรรค์',
    'หนองแซงวิทยาคม',
    'บ้านหนองเต่า',
    'บ้านไผ่',
    'บ้านลานวิทยาคม',
    'บ้านไผ่ศึกษา',
    'บ้านไผ่พิทยาคม',
    'เปือยน้อยศึกษา',
    'กู่ทองพิทยาลัย',
    'มัญจาศึกษา',
    'คำแคนวิทยาคม',
    'โคกนางามพิทยาสรรพ์',
    'มัธยมโพนเพ็ก',
    'เหล่าใหญ่นาข่าวิทยาคม',
    'ท่าศาลาประชานุสรณ์',
    'ชนบทศึกษา',
    'แท่นศิลาทิพย์ศึกษา',
    'ประชารัฐพัฒนาการ',
    'โคกโพธิ์ไชยศึกษา',
    'ซับสมบูรณ์พิทยาลัย',
    'บ้านแฮดศึกษา',
    'หนองขามพิทยาคม',
    'บ้านเก่างิ้วหนองบั่ว(คุรุราษฎร์ประสิทธิ์)',
    'บ้านคึมชาตหนองผงนาดี',
    'บ้านหนองม่วงใหญ่หนองตานา',
    'บ้านคูขาด(สถิตอุปถัมภ์)',
    'บ้านหนองบะ',
    'บ้านหนองไผ่วิทยา',
    'บ้านหนองแวงมน',
    'บ้านโจดหนองแกหนองสิม',
    'บ้านเตาเหล็ก',
    'บ้านวังขอนพาดหนองแวง',
    'บ้านหนองเรือ',
    'บ้านหนองหว้าสามัคคี',
    'บ้านโนนข่าประชานุเคราะห์',
    'บ้านโนนเมืองโคกกุง',
    'บ้านบ่อตะครองโนนฝ้ายสามัคคี',
    'บ้านหัวนาหนองแวง',
    'มิตรภาพ 2',
    'บ้านห้วยโจด',
    'บ้านทุ่งแค',
    'บ้านเพ็กใหญ่และสระบัว(คุรุราษฏร์สามัคคี)',
    'บ้านยานาง',
    'บ้านหนองสะแบงสามัคคี',
    'บ้านโคกกลางประชาสรรค์',
    'บ้านโนนหอม',
    'ชุมชนบ้านชาด',
    'บ้านทับบา',
    'บ้านหญ้าคาท่าหลวง',
    'บ้านหันใหญ่',
    'บ้านหนองห้าง',
    'บ้านทุ่งน้อยหนองโจดหนองหอย',
    'บ้านลอมคอม',
    'บ้านหนองอรุณ',
    'วัดอัมพร(คุรุราษฎร์รังสรรค์)',
    'บ้านโคกล่ามกลางตำแย(ไพจิตรอุปถัมภ์)',
    'บ้านเพ็กน้อย',
    'บ้านป่าเป้า',
    'บ้านโสกนกเต็น',
    'บ้านหนองบัวทวิคามบำรุง',
    'บ้านหนองแวงนอก',
    'บ้านป่าพร้าว-บูรณะ',
    'หนองมะเขือซองแมววิทยา',
    'บ้านโคกไม้งาม',
    'บ้านโนนตะโก',
    'ชุมชนบ้านห้วยค้อมิตรภาพที่206',
    'บ้านฝาผนังวังจาน',
    'บ้านหนองแวงนางเบ้า',
    'บ้านฮ่องหอยห้วยว่านหอมประชาสรรค์',
    'หนองม่วงน้อยนานิคมสามัคคี',
    'ไตรคามราษฎร์บำรุง',
    'บ้านโคกล่ามโนนกอกสามัคคี',
    'บ้านโนนชาดวิทยา',
    'บ้านหนองแปน',
    'บ้านหนองแวงโสกพระ',
    'บ้านหนองหญ้าปล้อง',
    'บ้านหันน้อย',
    'ผ่านบางอนุสรณ์',
    'หนองฉันเพลฮ่องแซงประชาสรร',
    'สหราษฎร์อนุกูล',
    'บ้านหนองทุ่ม',
    'บ้านคอนฉิม',
    'บ้านดงบังคุรุราษฎร์อุปถัมภ์',
    'บ้านดอนโจด',
    'บ้านโนนใหญ่',
    'ป่าแดงคุรุราษฎร์บำรุง',
    'บ้านโนนข่าวิทยา',
    'โนนทองวิทยา',
    'บ้านป่าไม้งาม',
    'บ้านรัตนะ',
    'บ้านรัตนะสาขาชีวังเวิน',
    'บ้านหนองแซงแสงอรุณ',
    'บ้านกุดหมากเห็บ',
    'นาโพธิ์ชีวิทยา',
    'บ้านโนนสะอาดประชาสรรค์',
    'วังบงน้อยวิทยา',
    'บ้านบะแค',
    'ชุมชนบ้านแวงใหญ่',
    'ชุมชนบ้านดอนหัน',
    'บ้านโจดใหญ่',
    'บ้านถลุงเหล็ก',
    'บ้านโนนจันทึก',
    'บ้านโสกไผ่',
    'บ้านใหม่นาเพียง',
    'บ้านโสกไผ่สาขาโสกเหลื่อม',
    'บ้านหนองแดง',
    'บ้านหนองโกบวรวิทยา',
    'วัดจุมพล(วรพรตอุปถัมภ์)',
    'บ้านบะแหบหญ้าคา',
    'บ้านตลาดหนองแก',
    'บ้านโคกใหญ่หนองสองห้อง',
    'บ้านโนนศาลา',
    'บ้านโสกกระหนวน',
    'บ้านโสกน้ำขาว',
    'บ้านทางขวาง',
    'บ้านโคกสง่า',
    'บ้านหันโสกเชือก',
    'บ้านหนองม่วงโนนงิ้ว',
    'บ้านหนองหญ้าปล้องพัฒนา',
    'บ้านท่านางแนว',
    'บ้านโนนเขวาประชาศึกษา',
    'บ้านโพนงาม',
    'บ้านหนองบัวเลิง',
    'บ้านหนองแวงท่าวัด',
    'ป่าไม้อุทิศ1(บ้านลาด)',
    'บ้านรวงโนนไทโนนโจด',
    'บ้านหนองโกน้อย',
    'บ้านหนองดู่',
    'บ้านหญ้าคา',
    'บ้านหนองสะแบงศรีชุมพร',
    'บ้านห้วยทรายทุ่งมน',
    'บ้านละหานนา',
    'บ้านโนนทองหลางโคกสูง',
    'บ้านป่าเป้งน้ำซับ',
    'บ้านป่าเป้งสาขาหนองหญ้าขาว',
    'ชุมชนบ้านโคกสี',
    'บ้านกุดรู',
    'บ้านแวงน้อย',
    'บ้านหนองแขมอีโล',
    'บ้านดอนหันนาจาน',
    'บ้านโนนศิลา',
    'บ้านคึมชาด',
    'บ้านชาดศรีสมบูรณ์',
    'บ้านปอแดง',
    'บ้านหนองกุงโนนทัน',
    'บ้านหนองแวงตอตั้ง',
    'บ้านศุภชัย',
    'บ้านดงเค็ง',
    'โนนชาดหนองช้างน้ำ',
    'หนองพลวงมิตรประชา',
    'บ้านโกรก',
    'บ้านหนองหญ้าขาว',
    'บ้านกระหนวนดอนดั่ง',
    'บ้านโนนสว่าง',
    'บ้านหินแร่',
    'บ้านหนองวัดป่า',
    'บ้านหนองแวงน้อย',
    'บ้านดอนดู่',
    'บ้านโนนท่อน',
    'บ้านโนนสะอาด',
    'บ้านหนองบัวลอง',
    'บ้านหนองไผ่น้อย',
    'บ้านหนองแวงยาว',
    'บ้านหนองแสง',
    'บ้านกุดหว้า',
    'บ้านหนองบัวแดง',
    'บ้านหนองตะครอง',
    'บ้านหนองเปล่ง',
    'จตุคามวิทยา',
    'บ้านตะกั่วป่า',
    'บ้านหนองคูบัว',
    'บ้านกอกโนนแต้',
    'บ้านโคกสูงประชาสรรค์',
    'บ้านชาดหนองเหล็ก',
    'บ้านโนนธาตุ',
    'บ้านสว่างวิทยา',
    'บ้านสำราญ',
    'บ้านวังหิน',
    'บ้านหลักด่าน',
    'บ้านหนองคลองหนองทุ่ม',
    'บ้านหนองยาง',
    'บ้านหนองเสี้ยว',
    'บ้านสำโรง',
    'บ้านหินลาดนาดี',
    'บ้านหนองกาว',
    'บ้านหนองคลองดอนแดง',
    'บ้านเหล่านกชุม',
    'บ้านขุมปูน',
    'บ้านสวองหนองไผ่ล้อม',
    'บ้านหนองกุงสว่าง',
    'ชุมชนหนองเม็ก',
    'บ้านโคกสูงวิทยา',
    'บ้านท่าหนองหว้า',
    'บ้านโนนขามป้อม',
    'บ้านโนนเหลื่อมหัวขัว',
    'บ้านวังคูณ',
    'บ้านหนองไทร',
    'บ้านหนองบัวแดงวิทยา',
    'ชุมชนหนองสองห้องคุรุราษฎร์รังสรรค์',
    'บ้านโนนกราด',
    'บ้านโนนรังหนองบอน',
    'บ้านเปาะ',
    'บ้านเมย',
    'บ้านขาม',
    'บ้านหนองสองห้อง',
    'ไตรมิตรวิทยา',
    'บ้านกุดหอยกาบ',
    'บ้านตะคร้อหนองหญ้าขาว',
    'บ้านหันโจดหนองกุง',
    'บ้านแฝก',
    'บ้านดอนชาด',
    'บ้านโนนแดง',
    'บ้านหนองทุ่มหนองหว้า',
    'บ้านขอนสักตอประดู่',
    'บ้านหลุบคา',
    'บ้านเหล่าโนนคูณ',
    'เบญจคามสามัคคี',
    'บ้านวังยาวสามัคคี',
    'บ้านหันศิลางาม',
    'บ้านหนองขี้เห็น',
    'ไตรมิตรพัฒนศึกษา',
    'โพธิ์ทองหนองน้ำขุ่นเหนือ',
    'บ้านหนองไฮประชาสรรค์',
    'บ้านห้วยแคน',
    'ไตรราษฎร์สามัคคี',
    'บ้านเปือยใหญ่ชาติหนองบ่อ',
    'บ้านผักหวาน',
    'บ้านหนองปลาหมอศรีสง่า',
    'หนองกุงห้วยเสียววิทยา',
    'พล',
    'เมืองพลพิทยาคม',
    'โสนนกเต็นประชาอุปถัมภ์',
    'โนนข่าวิทยา',
    'พลพัฒนศึกษา',
    'แวงใหญ่วิทยาคม',
    'โนนสะอาดวิทยาคาร',
    'แวงน้อยศึกษา',
    'ท่านางแนววิทยายน',
    'ก้านเหลืองวิทยาคม',
    'หนองสองห้องวิทยา',
    'ไตรคามวิทยา',
    'เบญจมิตรวิทยา',
    'ศรีหนองกาววิทยา',
    'รังสรรค์วิทยาคม',
    'สันติพัฒนกิจวิทยา',
    'บ้านกุดกว้าง',
    'บ้านกุดน้ำใส',
    'บ้านฟากพอง',
    'บ้านหนองบัวน้อยห้วยโจดโนนขามแป',
    'บ้านหนองอ้อน้อย',
    'อุดมพัฒน์ฟินิคซฯอุปถัมภ์',
    'บ้านโคกสง่านางาม',
    'บ้านจำปาหัวบึง',
    'บ้านทรายมูล',
    'บ้านหนองหว้าสุขใจ',
    'บ้านหนองแสง',
    'บ้านท่าโพธิ์ท่ามะเดื่อน้อย',
    'บ้านท่ามะเดื่อ',
    'บ้านกุดพังเครือ',
    'บ้านท่ากระเสริม',
    'น้ำพองประชานุกูล',
    'น้ำพองภูริพัฒน์มิตรภาพที่ 94',
    'บ้านด้วงหนองหญ้ารังกา',
    'บ้านห้วยไผ่หนองโน',
    'โคกสูงกุดน้ำใส',
    'บ้านคำมืด',
    'บ้านหนองหารจาง',
    'บ้านห้วยเสือเต้น',
    'บ้านนาฝายเหนือ',
    'บ้านโนนแดงโคกเล้า',
    'บ้านเพี้ยฟานโนนสวรรค์',
    'บ้านหนองกุงขี้ควง',
    'บ้านหนองแวง',
    'บ้านโคกใหญ่',
    'บ้านคำจั่น(ปาสานะเตนุสรณ์)',
    'คำบอนวิทยา',
    'บ้านดงเย็น',
    'กู่สว่างสามัคคี',
    'ชุมชนบ้านบัวใหญ่',
    'บ้านคอกคีแสนตอ',
    'บ้านคำม่วมดงเย็น',
    'บ้านโนนหัวช้าง',
    'พระธาตุขามแก่น',
    'บ้านโนนเชือก',
    'บ้านสองคอนศิริคุรุราษฎร์',
    'บ้านสำโรง',
    'บ้านเหล่าใหญ่ประชานุกูล',
    'บ้านหัวดง',
    'บ้านหินลาด',
    'บ้านหัวบึง',
    'บ้านกุดพังทุย',
    'บ้านดงเรือง',
    'บ้านนาขาม',
    'บ้านโสกม่วง',
    'บ้านสระกุด',
    'บ้านห้วยหินลาด',
    'คำใหญ่ปันน้ำใจ',
    'บ้านคำแก่นคูณ',
    'บ้านโนนพยอม',
    'บ้านโนนศรีสวัสดิ์',
    'บ้านม่วงหวาน',
    'บ้านวังเกิ้ง',
    'บ้านวังชัย',
    'บ้านสร้างแซ่งวังถั่ว',
    'บ้านเสียวโคกกลาง',
    'บ้านหนองอ้อโคกสว่าง',
    'บ้านคำบง',
    'บ้านดงมัน',
    'บ้านนาศรีดงเค็ง',
    'บ้านสะอาดหนองเรือ',
    'บ้านบึงกลาง',
    'ชุมชนหนองกุงวิทยา',
    'น้ำพอง',
    'บ้านโสกแสง',
    'บ้านนาคู',
    'บ้านนาเรียง',
    'อนุบาลอุบลรัตน์',
    'บ้านโนนจิก',
    'บ้านโนนสวรรค์เก่าข่า',
    'บ้านพระบาทท่าเรือ',
    'บ้านภูคำเบ้า',
    'บ้านทรัพย์สมบูรณ์โนนอินทร์แปลง',
    'โคกสูงใหญ่วิทยา',
    'บ้านคำบอนคุรุราษฎร์บำรุง',
    'บ้านโคกสว่าง',
    'บ้านทุ่งโป่ง',
    'บ้านหนองผือ',
    'ห้วยยางวิทยาสรรพ์',
    'บ้านกุดกระหนวน',
    'นาคำพิทยาสรรพ์',
    'บ้านหนองขามพิทยาคม',
    'บ้านทรัพย์ภูพานคำปลายหลายหนองผักแว่น',
    'บ้านกุดเชียงมี',
    'บ้านขุนด่าน',
    'บ้านดง(บึงไทรวิทยาคาร)',
    'บ้านบ่อ',
    'บ้านหนองแต้',
    'บ้านห้วยทรายศึกษา',
    'บ้านโคกกลางหนองไหลพิทยาสรรพ์',
    'บ้านโนนศิลาราศรี',
    'บ้านศาลาดินห้วยเตย',
    'บ้านศรีสุขสำราญ',
    'บ้านหนองแสงน้อย',
    'ดูนสาด',
    'บ้านนามูล',
    'บ้านหนองซา',
    'ชุมชนน้ำอ้อม',
    'บ้านหนองแวงเป่ง',
    'บ้านฝางวิทยา',
    'บ้านหนองแวงคูป่าชาติ',
    'บ้านคำมืดวิทยา',
    'บ้านทุ่งใหญ่',
    'บ้านผักหนาม',
    'บ้านวังโพน',
    'บ้านเวียงแก้ว',
    'บ้านหนองกุงใหญ่',
    'บ้านหนองโอง',
    'ชุมชนกระนวน',
    'บ้านคำไฮผักแว่น',
    'บ้านบะแต้',
    'ศรีสมบูรณ์วิทยา',
    'บ้านหนองโก',
    'บ้านห้วยเชือก',
    'ไทยรัฐวิทยา34(บ้านกุดโง้ง)',
    'บ้านโคกกลาง',
    'บ้านนาเลาะ',
    'บ้านป่าติ้ว',
    'หนองโนวิทยาคาร',
    'ชุมชนบ้านห้วยโจดโคกสะอาด',
    'บ้านนาฝาย',
    'บ้านโนนสังป่ารัง',
    'บ้านโสกเสี้ยวแสนสุข',
    'ชุมชนบ้านหัวยโจดโคกสะอาดสาขาบ้านหนองแสงวิทยา',
    'ศิลาโป่งคำ',
    'บ้านน้ำสามวัง',
    'บ้านห้วยยาง',
    'บ้านโคกกลางวิทยา',
    'บ้านโนนสมบูรณ์',
    'บ้านท่าลาดคำตานา',
    'บ้านคำครึ่ง',
    'บ้านหัวนาคำ',
    'บ้านคำนางปุ่ม',
    'บ้านนาค้อ',
    'บ้านป่าหวายนั่งราษฎร์บำรุง',
    'สองห้องประชาบำรุง',
    'อนุบาลเขาสวนกวาง',
    'บ้านโนนสว่างหนองตะนา',
    'บ้านคำม่วง',
    'บ้านดงเรืองประชาสรรพ์',
    'บ้านทางพาดเขาสวนกวาง',
    'บ้านนาอ่างทอง',
    'บ้านโนนสง่า',
    'บ้านโนนทองห้วยบาก',
    'บ้านดงบังคุรุราษฎร์บำรุง',
    'ทุ่งบ่อวิทยา',
    'บ้านหนองคู',
    'ศรีเมืองแอม',
    'บ้านโนนน้ำผึ้ง',
    'บ้านคำแคนคำเจริญ',
    'บ้านโคกสูงสะอาด',
    'หนองแวงเรือประชาศึกษา',
    'บ้านห้วยยางสะอาด',
    'บ้านคำสมบูรณ์',
    'บ้านแสงสว่าง',
    'บ้านนาง้อง',
    'บ้านโนนสวรรค์ประชารัฐรังสรรค์',
    'บ้านขามป้อมดงเย็น',
    'บ้านนางิ้วนาโพธิ์',
    'ชุมชนโนนสมบูรณ์',
    'บ้านทุ่งโพธิ์ชัย',
    'บ้านโนนหัวช้าง',
    'บ้านคำป่าก่อ',
    'บ้านหัวฝายประชานุกูล',
    'บ้านโคกกลางนาล้อม',
    'บ้านป่าเปือย',
    'บ้านกระนวน',
    'บ้านคำแมด',
    'บ้านหนองบัวคำมูล',
    'บ้านบ่อใหญ่',
    'บ้านหม้อ',
    'คูคำพิทยาสรรพ์',
    'บ้านโคกใหม่นายม',
    'บ้านโคกใหม่นายมสาขาบ้านดงซำ',
    'บ้านโนน',
    'บ้านแห้ว',
    'บ้านห้วยเตย',
    'บ้านหนองกุงดิ่ง',
    'บ้านหลุบเลา',
    'บ้านสว่าง',
    'น้ำพองศึกษา',
    'ดงมันพิทยาคม',
    'บัวใหญ่พิทยาคม',
    'น้ำพองพัฒนศึกษา รัชมังคลาภิเษก',
    'พังทุยพัฒนศึกษา',
    'เปรมติณสูลานนท์',
    'ม่วงหวานพัฒนศึกษา',
    'พระธาตุขามแก่นพิทยาลัย',
    'โคกสูงประชาสรรพ์',
    'ลำน้ำพอง',
    'บึงไทรพิทยาคม',
    'บัวแก้วพิทยาคม',
    'พูวัดพิทยาคม',
    'อุบลรัตน์พิทยาคม',
    'ศรีกระนวนวิทยาคม',
    'ยางคำพิทยาคม',
    'หนองโนประชาสรรค์',
    'ทุ่งใหญ่รัตนศึกษา',
    'เขาสวนกวางวิทยานุกูล',
    'นางิ้ววิทยาสรรค์',
    'ดงบังวิทยายน',
    'ซำสูงพิทยาคม',
    'โนนโพธิ์ศรีวิทยาคม',
    'บ้านหนองไผ่ดุสิตประชาสรรค์',
    'บ้านสำราญหินลาด',
    'บ้านหนองตาไก้ตลาดหนองแก',
    'บ้านโคกสูงวิทยานุกูล',
    'บ้านโนนดู่',
    'บ้านโคกกลางหนองแปน',
    'โนนฟันเรือบะยาววิทยา',
    'บ้านกุดกว้าง',
    'บ้านขนวนนคร',
    'ชุมชนบ้านโพนสว่างดอนโมง',
    'บ้านหนองแปน',
    'บ้านหนองหอย',
    'บ้านหัวนาจระเข้',
    'บ้านหัวบึง(ทูสอนประชาสรรค์)',
    'บ้านกุดแคน',
    'บ้านกุดเลา',
    'บ้านโนนทอง',
    'บ้านฝางน้อย',
    'บ้านภูมูลเบ้า',
    'บ้านสระพังข่า',
    'บ้านหนองเขื่อนช้าง',
    'บ้านหนองนกเขียนประชาอุปถัมภ์',
    'บ้านห้วยอรุณหินลาด',
    'บ้านห้วยทรายทอง',
    'บ้านกุดฉิมพิทยาคาร',
    'บ้านนาเปือย',
    'บ้านโนนทันวิทยา',
    'บ้านโนนสะอาดศึกษา',
    'บ้านร่องสมอ',
    'บ้านหว้า',
    'บ้านห้วยม่วง',
    'บ้านโคกกลางโนนคูณ',
    'บ้านดอนหันโนนหินแห่',
    'บ้านโนนจั่นห้วยแสงพรสวรรค์',
    'โนนสะอาดพิทยา',
    'บ้านหนองลุมพุกโนนสว่าง',
    'บ้านโนนหนองแวง',
    'บ้านหนองสระ',
    'บ้านกง(ประชานุกูล)',
    'บ้านหนองเม็กหนองทุ่ม',
    'บ้านหนองผือราษฎร์ประสิทธิ์',
    'บ้านดอนกอก',
    'โพธิ์ตากวิทยา',
    'หนองแสงวิทยาสรรค์',
    'บ้านหาด',
    'บ้านเหมือดแอ่ภูเม็ง',
    'บ้านสว่างดอนช้าง',
    'บ้านหนองกุงมนศึกษา',
    'บ้านหนองโนประชาสรรค์',
    'บ้านหนองแสงสามัคคี',
    'บ้านนาหว้า',
    'บ้านป่าเสี้ยว',
    'บ้านเม็ง',
    'บ้านหนองแวงภูเขา',
    'บ้านหนองหว้าประชารัฐ',
    'บ้านดอนแขม',
    'บ้านดอนหันสระบัว',
    'บ้านนางิ้ว',
    'บ้านยางคำ',
    'ชุมชนบ้านหนองเรือ',
    'บ้านท่าศาลาคุรุราษฎร์วิทยา',
    'บ้านฟ้าเหลื่อม',
    'หนองกุงประชารัฐบำรุง',
    'หนองไฮประชารัฐ',
    'บ้านเหล่าประชานุเคราะห์',
    'บ้านสะอาด',
    'บ้านขามป้อม',
    'โคกม่วงศึกษา',
    'บ้านโป่งเอียด',
    'บ้านนาสีนวน',
    'บ้านสัมพันธ์',
    'เหมือดแอ่หนองบัว',
    'ชุมชนชุมแพ',
    'บ้านชุมแพ',
    'บ้านหนองบัวโนนเมือง',
    'ชุมแพชนูปถัมภ์',
    'บ้านแห่(ประชานุเคราะห์)',
    'โคกสูงสว่าง(ราษฎร์บูรณะ)',
    'บ้านโนนทองหลาง',
    'บ้านไชยสอ',
    'บ้านไผ่กุดหิน',
    'บ้านหนองสังข์',
    'บ้านหนองใส',
    'บ้านหนองหว้าโพนศิลา',
    'บ้านนาเพียง',
    'โนนงามศึกษา',
    'บ้านหนองผือจำเริญพัฒนา',
    'บ้านโนนโพธิ์',
    'บ้านหนองไฮ',
    'บ้านอาจสามารถ',
    'นาหนองทุ่มวิทยาคม',
    'บ้านโนนโก',
    'บ้านวังยาว',
    'บ้านห้วยอีเปาะ',
    'บ้านโนนโกสาขาบ้านซำผักหนาม',
    'ชุมชนโนนหันวันครู',
    'บ้านโนนบุรุษ',
    'บ้านโนนลาน',
    'บ้านหนองม่วง',
    'บ้านร่องแซง',
    'บ้านโนนชาติ',
    'บ้านหอย"รัฐราษฎร์รังสรรค์"',
    'บ้านโคกสูง',
    'บ้านโนนอุดม',
    'ก้องอุดมวิทยาคาร',
    'บ้านนาคำน้อย',
    'บ้านโป่งแห้ง',
    'ฝายหินหนองทุ่มวิทยาสรรค์',
    'วังหินลาดวังเจริญ',
    'บ้านหนองไผ่นาดี',
    'บ้านธาตุ',
    'หนองกุงพิทยาคม',
    'บ้านหนองเขียดมิตรภาพ 205',
    'บ้านหนองหนามแท่ง',
    'โคกสูงสำราญ',
    'บ้านหนองคะเน',
    'บ้านโนนแหลมทอง',
    'หนองขามวิทยาคาร',
    'บ้านโนนสะอาดห้วยบั้งทิง',
    'วังหูกวางราษฎร์ผดุงวิทย์',
    'บ้านยอดห้วย',
    'กุดเข้ห้วยบง',
    'หนองไผ่พิทยาคม',
    'บ้านหนองเสาเล้า',
    'หนองหว้าวิทยา',
    'บ้านสุขสมบูรณ์',
    'บ้านหนองโพงโพด',
    'บ้านหนองศาลา',
    'บ้านสารจอดร่องกลอง',
    'หนองกระบือวิทยาคม',
    'บ้านซำยาง',
    'บ้านซำจำปา',
    'บ้านวังขอนแดงหนองหญ้าปล้อง',
    'อ่างทองวิทยาคม',
    'บ้านนาอุดม',
    'โนนสำราญหนองหญ้าขาว',
    'แสงบัวทอง',
    'บ้านหนองไฮ-วังขอนยม',
    'ชุมชนบ้านสี่แยกโนนหัวนา',
    'ท่ากุญชร',
    'บ้านนาจานนาเจริญ',
    'บ้านผาน้ำเที่ยง',
    'บ้านบริบูรณ์',
    'บ้านพงษ์',
    'บ้านผาขาม',
    'บ้านท่าช้างน้อยโนนสวัสดิ์',
    'บ้านสวนสวรรค์',
    'บ้านโสกจานนาดี',
    'บ้านใหม่โสกส้มกบ',
    'บ้านโนนงาม',
    'ภูห่านศึกษา',
    'บ้านทุ่งเชือก',
    'บ้านหนองปลาซิว',
    'ชุมชนบ้านวังเพิ่ม',
    'บ้านโนนหว้านไฟประชาสรรค์',
    'บ้านบุ่งเม่นหนองดู่',
    'บ้านปากห้วยฝาง',
    'บ้านโสกหาดสวรรค์',
    'ชุมชนบ้านวังเพิ่มสาขาบ้านห้วยทรายขาว',
    'บ้านท่าช้าง',
    'บ้านโคกกุงสาขาบ้านหินลาดทุ่งโพธิ์',
    'บ้านขมิ้นโนนหัวนา',
    'บ้านโคกกุง',
    'บ้านโคกม่วง',
    'บ้านโคกไม้งาม(ส่งเสริมวิทยา)',
    'บ้านศรีสุข',
    'บ้านโสกรัง กุดบัว วังทรายขาว',
    'บ้านป่าน',
    'อนุบาลสีชมพู',
    'บ้านหนองตาไก้พิทยา',
    'บ้านห้วยสายหนัง',
    'บ้านโนนทองหลางประชาสรรค์',
    'บ้านโคกป่ากุง',
    'บ้านสันติสุข',
    'บ้านหนองโก',
    'โสกเต็นวิทยาคม',
    'บ้านหนองแดง',
    'บ้านหนองทุ่มประชาสรรค์',
    'บ้านกุดขอนแก่น',
    'บ้านไคร่นุ่น',
    'บ้านค้อ',
    'บ้านโนนสำราญ',
    'บ้านโป่งแดง',
    'บ้านเลิงแสง',
    'หนองโพนนาหม่อ',
    'จตุรคามรังสรรค์',
    'บ้านนาแพงคุรุรัฐสามัคคี',
    'บ้านกุดดุกวิทยา',
    'ดินดำวังชัยวิทยา',
    'บ้านหนองทุ่มโนนสวรรค์',
    'บ้านกุดแคน',
    'บ้านโคกสงเปือยดอนดู่วิทยา',
    'บ้านทุ่งชมพู',
    'บ้านหนองพลวง',
    'บ้านห้วยขี้หนูดอนเพิ่มวิทยา',
    'บ้านพระบาทโนนคูณ',
    'บ้านกุดน้ำใส',
    'บ้านนาชุมแสง',
    'บ้านโนนรังวิทยาสรรค์',
    'บ้านหนองย่างแลนหนองทุ่ม',
    'บ้านหนองลุมพุกบุ่งแสง',
    'บ้านหัวฝาย',
    'โนนอุดมสะอาดวิทยา',
    'บ้านคำใหญ่',
    'บ้านนาหว้า',
    'บ้านโนนกระเดา',
    'โนนสมบูรณ์ประชาสรรค์',
    'โสกห้างศึกษา',
    'บ้านหนองโน',
    'บ้านห้วยชัน',
    'ภูเวียงวิทยายน',
    'บ้านโคกสูงวิทยา',
    'บ้านแดง',
    'โพธิ์ทองหนองเตาปูนวิทยา',
    'บ้านโคกสง่า',
    'บ้านเรือ',
    'สวนกล้วยห้วยชันวิทยาคาร',
    'บ้านนาก้านเหลือง',
    'บ้านโพนเพ็กพิทยา',
    'อนุบาลภูเวียง',
    'บ้านหนองขาม',
    'บ้านเมืองเก่า',
    'บ้านหินร่อง',
    'บ้านหนองนาคำประชานุเคราะห์',
    'บ้านหนองบัว',
    'ไตรคามวิทยาสรรค์',
    'บ้านสงเปือย',
    'บ้านอ่างศิลา',
    'บ้านโคกไร่',
    'บ้านถ้ำแข้',
    'หนองกุงเซินหนองโนพัฒนา',
    'หนองกระแหล่งกระเดาวิทยา',
    'บ้านโนนศิลาโนนม่วงวิทยา',
    'บ้านหนองกุงธนสารโศภณ',
    'บ้านโคกไม้งาม',
    'บ้านโคกสหกรณ์',
    'บ้านวังขอนแดง',
    'บ้านโนนสวรรค์วิทยา',
    'บ้านหัน',
    'บ้านโคกกลางวิทยา',
    'หนองผักแว่นโป่งสังข์วิทยา',
    'บ้านห้วยบง',
    'บ้านพระบาท',
    'บ้านหว้าทอง',
    'นาฝายวิทยา',
    'บ้านสะแกเครือ',
    'บ้านสองคอน',
    'บ้านโนนคอม',
    'บ้านป่ากล้วย',
    'บ้านสว่างโนนสูง',
    'บ้านโนนสว่างท่ากระบือ',
    'บ้านนาน้ำซำ',
    'หนองแห้ววังมนศึกษา',
    'บ้านวังสวาบ',
    'บ้านเขาวง',
    'บ้านทรัพย์สมบูรณ์',
    'ชีพอนุสรณ์',
    'บ้านซำภูทอง',
    'นาหม่อโนนลานประชาสรรค์',
    'บ้านกุดธาตุ',
    'บ้านโคกนาฝาย',
    'บ้านนาดี',
    'บ้านสะอาด',
    'บ้านหัวภู',
    'หนองแวงวิทยา',
    'บ้านขนวน',
    'บ้านศาลาดิน',
    'บ้านหนองกุงคชสาร',
    'บ้านหนองพู่',
    'บ้านคึมชาติประชาสรรค์',
    'บ้านหนองหญ้าปล้องหนองหว้า',
    'บ้านหนองหอย',
    'บ้านหนองนาคำ',
    'หนองเรือวิทยา',
    'จระเข้วิทยายน',
    'มัธยมตลาดใหญ่วิทยา',
    'ชุมแพศึกษา',
    'โนนหันวิทยายน',
    'จตุรมิตรวิทยาคาร',
    'ขัวเรียงศึกษา',
    'ชุมแพพิทยาคม',
    'หนองเสาเล้าวิทยาคาร',
    'ชุมแพวิทยายน',
    'มัธยมหนองเขียด',
    'สีชมพูศึกษา',
    'นาจานศึกษา',
    'ผาขามวิทยายน',
    'หนองตาไก้ศึกษา',
    'ซำยางวิทยายน',
    'ภูเวียงวิทยาคม',
    'กุดขอนแก่นวิทยาคม',
    'เวียงวงกตวิทยาคม',
    'เวียงนครวิทยาคม',
    'ภูผาม่าน',
    'หนองนาคำวิทยาคม',
    'บ้านท่าตูมดงสระพัง',
    'มิตรภาพ 6',
    'บ้านโนนยาง',
    'บ้านขมิ้นบ่อโคลน',
    'บ้านดอนหวาย',
    'บ้านยางบึง',
    'นิคมสร้างตนเองเชียงพิณ 3',
    'นิคมสร้างตนเองเชียงพิณ 5',
    'บ้านนาสมบูรณ์',
    'บ้านโคกสะอาดศรีบูรพา',
    'บ้านหนองสวรรค์ (ไชยเชียงพิณ)',
    'บ้านหนองโอนหนองฮาง',
    'บ้านจำปาวิทยา',
    'บ้านเชียงพิณ',
    'บ้านนาคลอง',
    'บ้านเชียงยืน',
    'บ้านอีหลุ่ง',
    'บ้านจำปา',
    'บ้านป่อง',
    'บ้านหนองน้ำเค็ม',
    'บ้านโนนสวรรค์',
    'บ้านหนองหลอด',
    'บ้านหนองตอ',
    'บ่อน้อยประชาสรรค์',
    'บ้านนาเยีย',
    'บ้านดงหนองโพธิ์',
    'บ้านเชียงพัง',
    'บ้านนากว้าง',
    'บ้านนาทาม',
    'บ้านหมากตูมดอนยานาง',
    'ไทยรัฐวิทยา 92 (ชุมชนนาข่า)',
    'ถ่อนใหญ่ถ่อนน้อย',
    'บ้านเหล่าดอนแตง',
    'บ้านนาคำหลวง',
    'บ้านงอย',
    'ประชาสามัคคี',
    'บ้านหนองหัวหมู',
    'บ้านศรีเชียงใหม่',
    'กุดลิงง้อหนองแก',
    'บ้านนาแอง',
    'ชุมชนนิคมสร้างตนเองเชียงพิณ 2',
    'บ้านนิคมพัฒนา',
    'บ้านหนองขุ่นเหล่าหลักวิทยา',
    'บ้านปากดงส่งเสริมธรรม',
    'บ้านโนนบุญมี',
    'ชุมชนโนนสูง',
    'บ้านหนองโสกดาว',
    'บ้านข้าวสาร',
    'บ้านชัยพรมิตรภาพที่ 67',
    'บ้านเม่น',
    'บ้านขาว',
    'บ้านดู่',
    'บ้านหัวบึง',
    'บ้านพรานเหมือน',
    'บ้านดงเค็ง (นําวัฒนาอุปถัมภ์)',
    'บ้านคำกลิ้ง',
    'บ้านหนองตูม',
    'บ้านจั่นศรีวิไล',
    'บ้านเลี่ยมพิลึก',
    'ค่ายประจักษ์ศิลปาคม',
    'บ้านตาด',
    'บ้านกกสะทอน',
    'บ้านโนนเดื่อ',
    'บ้านสุขสมบูรณ์',
    'บ้านศรีบุญเรือง',
    'เจ ซี บ้านอินทร์แปลง',
    'ผ่านศึกสงเคราะห์',
    'บ้านหลุบหวายป่าขาม',
    'ไทยสมุทรมิตรภาพที่ 187',
    'บ้านเลื่อม',
    'บ้านถ่อน',
    'บ้านนาทราย',
    'ชุมชนสามพร้าว',
    'บ้านหนองบุนาหล่ำ',
    'บ้านแมด',
    'บ้านหนองบั่ว',
    'บ้านหนองคอนแสน',
    'บ้านดอนหาด',
    'บ้านนาหยาด',
    'บ้านอีเลี่ยน',
    'บ้านเซประชาอุทิศ',
    'บ้านโก่ย',
    'บ้านจำปาประชานุเคราะห์',
    'บ้านหนองนาคำ',
    'บ้านหนองหว้าหนองไผ่',
    'บ้านหนองแก',
    'บ้านถ่อนน้อยหนองไผ่คำ',
    'บ้านพรสวรรค์',
    'บ้านหนองขาม(ค่ายเสนีย์อุปถัมภ์)',
    'บ้านดงอุดม',
    'บ้านหนองใส',
    'บ้านเก่าน้อย',
    'บ้านแม่นนท์',
    'บ้านหนองไผ่',
    'บ้านหนองนาเจริญ',
    'บ้านหนองบัวเงิน',
    'บ้านหนองตะไก้',
    'บ้านห้วยสำราญ',
    'บ้านโสกน้ำขาว',
    'บ้านศรีสว่าง',
    'หนองไฮวิทยา',
    'บ้านโนนสะอาดผาสุข',
    'บ้านดงมะกรูด',
    'บ้านโคกลาด',
    'บ้านเดื่อ',
    'บ้านช้าง',
    'บ้านหนองเหล็ก',
    'บ้านหนองตุ',
    'บ้านม่วงสว่างสามัคคี',
    'อนุบาลอุดรธานี',
    'อนุบาลหนองหิน',
    'บ้านหมากแข้ง',
    'บ้านดอนอุดมโนนยาง(เนยอุปถัมภ์)',
    'บ้านหนองบัว',
    'หนองสำโรงวิทยา',
    'ชุมชนหมูม่นวิทยาสรรค์',
    'ชุมชนกุดหมากไฟ',
    'บ้านโคกล่าม',
    'บ้านหนองแวงชุมพล',
    'บ้านหนองแซงสร้อย',
    'บ้านหนองแวงเดิด',
    'บ้านน้ำพ่น',
    'บ้านเลา',
    'รัฐประชา 509',
    'บ้านโนนหวาย',
    'บ้านหนองเม็กห้วยทราย',
    'บ้านโคกหนองแซง',
    'บ้านเสาเล้า',
    'บ้านหนองบัวบาน',
    'นิคมสร้างตนเองเชียงพิณ 1',
    'สิริปุณโณ',
    'บ้านหนองอ้อน้อย',
    'บ้านโคกก่องหนองแวงยาว',
    'อนุบาลหนองวัวซอ',
    'บ้านหนองอ้อ',
    'บ้านโนนสำราญ',
    'บ้านหนองแสง',
    'บ้านหนองแวงฮี',
    'บ้านโคกศรีแก้ว',
    'บ้านหมากหญ้า',
    'บ้านผาสิงห์',
    'บ้านดงบัง',
    'บ้านโนนชัยศรี',
    'บ้านหนองบัวเงิน',
    'บ้านอูบมุง',
    'บ้านห้วยไร่',
    'บ้านโคกผักหอม',
    'บ้านนาสีนวล',
    'บ้านโคกกลาง',
    'บ้านดอนจันทร์โพนสวรรค์',
    'บ้านจอมศรี',
    'บ้านข่าทุ่งม่วงโนนศรีสมบูรณ์',
    'บ้านหนองบ่อ',
    'บ้านสะอาดนาพัง',
    'บ้านจอมตาลโนนดู่โนนสำราญ',
    'บ้านดงยางนารายณ์',
    'บ้านเชียงหวาง "คุรุประชาสรรค์"',
    'บ้านนาดี',
    'บ้านสร้างหลวงสร้างคำ',
    'บ้านดงใหญ่',
    'บ้านดอนข่า',
    'บ้านโพนเลา',
    'บ้านด่าน',
    'บ้านโนนสวาง',
    'บ้านคอนเลียบ',
    'บ้านนาพัง',
    'บ้านเตาไห',
    'บ้านหม้อ',
    'บ้านคอนสวรรค์สินเจริญ',
    'บ้านนาบัว',
    'บ้านยางซอง',
    'บ้านนาทราย',
    'บ้านดอนกลอย',
    'บ้านท่าหนาดพัฒนา',
    'บ้านนาพู่',
    'บ้านป่าก้าวดอนแดง',
    'บ้านหลวงหัวสวย',
    'บ้านศรีบุญเรือง',
    'บ้านกิ่วดงมะไฟ',
    'บ้านหนองนกเขียนโพนทัน',
    'ชุมชนบ้านธาตุ',
    'บ้านสังซาวังน้ำขาว',
    'บ้านถิ่น (สุขาวิทยา)',
    'นิคมสร้างตนเอง',
    'บ้านนาคอมนาดอกไม้',
    'บ้านดอนแก้ว',
    'บ้านยามกาใหญ่',
    'บ้านนาพูนทรัพย์',
    'บ้านหมูม่นโพนสว่าง',
    'บ้านยามกาโนนคำ',
    'บ้านเหล่าดอนเงิน',
    'บ้านหนองผง',
    'บ้านยาง (คุรุราษฎร์วิทยา)',
    'บ้านโคกสว่าง',
    '*บ้านยาง(คุรุราษฎร์วิทยา) สาขาบ้านนาพระ',
    'บ้านหนองกุง',
    'บ้านดงขันทอง',
    'บ้านดงศรีสําราญ',
    'บ้านนาส่อนโพนทัน',
    'บ้านใหม่',
    'บ้านศรีสุวรรณโพนสว่าง',
    'บ้านโพนงามหนองตุ',
    'อนุบาลเพ็ญประขานุกูล',
    'บ้านโนนรัง',
    'บ้านทอนดอนยาว',
    'บ้านสร้างแป้น',
    'บ้านหว้าน',
    'สุมเส้าวิทยา',
    'บ้านดงปอ',
    'บ้านหนองนาไฮโนนสะอาด',
    'บ้านแพงศรี',
    'บ้านห้วยวังโตน',
    'บ้านหนองบัวบาน',
    'บ้านหนองแสนตอโนนสร้างคำ',
    'บ้านดงยาง',
    'บ้านเชียงดา',
    'บ้านแมดวิทยาคม',
    'บ้านนาหว้า(สันติราษฎร์พิทยาคม)',
    'บ้านไชยฟอง',
    'บ้านนาสะอาด',
    'บ้านดอนเดื่อ',
    'บ้านดอนบาก',
    'โคกโพธิ์วิทยา',
    'บ้านนาน้ำชุ่ม',
    'บ้านยวด',
    'บ้านชาด',
    'บ้านนามั่ง',
    'บ้านน้ำเที่ยง',
    'บ้านตลิ่งชัน-สร้างแก้ว',
    'บ้านศรีชุมชื่นบุญชิตวิทยา',
    'บ้านหินโงม',
    'บ้านโนนนกหอ(เพ็งแสนวิทยา)',
    'บ้านดงผักเทียม',
    'อนุบาลสร้างคอม',
    'บ้านท่าเสียว',
    'บ้านโนนชาดวรุบลวิทยา',
    'บ้านโคกสว่าง',
    'อุดรพิทยานุกูล',
    'สตรีราชินูทิศ',
    'ประจักษ์ศิลปาคาร',
    'นิคมสงเคราะห์วิทยา',
    'อุดรธรรมานุสรณ์',
    'โนนสูงพิทยาคาร',
    'อุดรพิชัยรักษ์พิทยา',
    'อุดรธานีพิทยาคม',
    'มัธยมสิริวัณวรี 1 อุดรธานี',
    'สามพร้าววิทยา',
    'ธาตุโพนทองวิทยาคม',
    'อุดรพัฒนาการ',
    'อุดรพิทยานุกูล 2',
    'อุดรพัฒนศึกษา',
    'เชียงพังพัฒนวิช',
    'ราชินูทิศ 2',
    'หนองวัวซอพิทยาคม',
    'หนองยางชุมพิทยาคม',
    'แสงอร่ามพิทยาคม',
    'เพ็ญพิทยาคม',
    'สุมเส้าพิทยาคาร',
    'จอมศรีพิทยาคาร',
    'เชียงหวางพิทยาคาร',
    'สร้างคอมวิทยา',
    'ยิ่งยวดพิทยานุกูล',
    'บ้านปอ',
    'บ้านดงเมือง(ดงเมืองวิทยา)',
    'บ้านท่าเปลือย',
    'บ้านโพธิ์สง่า(มังกรสามเสนอนุสรณ์)',
    'บ้านดอนแก้ว',
    'บ้านเชียงแหว',
    'บ้านเซียบ',
    'บ้านหมากบ้า',
    'บ้านสวนหม่อน',
    'บ้านเดียม',
    'บ้านนกขะบา',
    'บ้านเหล่าแชแลหนองแวง',
    'บ้านเมืองพรึก',
    'บ้านเหล่ากกโพธิ์กกเค็ง',
    'บ้านเหล่าหมากบ้าเชียงสม',
    'บ้านยางหล่อโนนน้ำย้อย',
    'บ้านดอนเงิน',
    'บ้านห้วยกองสี',
    'บ้านโนนสิมมา',
    'บ้านโนนจำปา',
    'บ้านตูม',
    'บ้านดอนสวรรค์',
    'บ้านท่าลี่ศรีสุข',
    'บ้านท่าแร่',
    'บ้านเหล่าหมากจันทน์',
    'บ้านนาเพ็ญ',
    'บ้านกุดยาง',
    'บ้านโนนสวรรค์ราษฎร์',
    'บ้านปะโค',
    'บ้านบุ่งหมากลาน',
    'บ้านหัวขัวเหมือดแอ่',
    'บ้านหนองหญ้ารังกาห้วยขุมปูน',
    'บ้านดงน้อย',
    'บ้านเกิ้งน้อย',
    'บ้านหนองแวงโนนขี้เหล็ก',
    'บ้านนาดีสร้างบง',
    'บ้านสี่แจ',
    'บ้านดงเรือง',
    'ชุมชนน้ำฆ้อง(กุมภวาปีวิทยา)',
    'บ้านกงพาน',
    'บ้านพันดอน',
    'บ้านคำเจริญ',
    'บ้านดงแคน',
    'ชุมชนวัดป่าทรงธรรม',
    'บ้านวาปี',
    'บ้านผือ',
    'บ้านหนองศรีเจริญ-หนองผึ้ง',
    'บ้านสวนมอนคำ',
    'บ้านโนนเห็นวังชัย',
    'บ้านคำไผ่',
    'บ้านหินเหลิ่ง',
    'บ้านหินฮาวโนนงาม',
    'บ้านนาแบก',
    'บ้านท่าหนองเทา',
    'บ้านท่าม่วงเวียงคํา',
    'บ้านสีออสว่างราษฎร์สามัคคี',
    'บ้านโนนเขวา',
    'บ้านหนองกวาง',
    'บ้านหนองแดง',
    'บ้านเหล่ากล้วยโนนสมพร',
    'บ้านโคกศรีสำราญวังหน้าผา',
    'บ้านสงเปลือยดงสามสิบ',
    'บ้านเสอเพลอ',
    'บ้านทองอินทร์สวนมอญ',
    'บ้านท่าสัง',
    'บ้านโนนมะข่า',
    'บ้านโนนหนองม่วง',
    'บ้านกุดจิก',
    'บ้านหนองหว้า',
    'บ้านหนองแวง',
    'บ้านห้วยบง',
    'บ้านห้วยเกิ้ง"วัฒนเสรีราษฏร์บำรุง"',
    'บ้านโคกผักหวาน',
    'บ้านโสกคูณ',
    'บ้านโคกกลางหนองแวงใหญ่',
    'บ้านคำเต้าแก้วหินลาด',
    'บ้านหนองแสง',
    'บ้านตาดโตน',
    'บ้านโคกสง่า',
    'บ้านม่วงดง',
    'บ้านหาดสถาพร',
    'ชุมชนทมป่าข่า',
    'บ้านทมนางาม',
    'บ้านค้อน้อย',
    'อนุบาลโนนสะอาด',
    'บ้านหัวฝาย',
    'บ้านม่วงเฒ่า',
    'บ้านโสกรังโนนสว่าง',
    'บ้านโนนหยาดน้ำเกลี้ยง',
    'บ้านหนองจาน',
    'ชุมชนบ้านบุ่งแก้ว',
    'บ้านโนนสำราญ',
    'บ้านกุดขนวน',
    'บ้านโคกล่าม',
    'บ้านกระเบื้องโนนทิง',
    'บ้านหนองโก',
    'บ้านบะยาว',
    'บ้านโพธิ์ชัยดอกคำ',
    'บ้านโพธิ์ศรีสําราญ',
    'บ้านเสาเล้าผักชีศรีสวัสดิ์',
    'บ้านห้วยแสงอรุณวิทยา',
    'บ้านหนองกุง',
    'บ้านนาเหล่า',
    'บ้านโนนทองคำเจริญ',
    'บ้านโนนจำปา',
    'หมู่บ้านป่าไม้',
    'ชุมชนจำปี',
    'บ้านโนนสูง',
    'บ้านโคกน้อย',
    'บ้านโนนสงเปลือย',
    'บ้านดูนเลา',
    'บ้านโนนม่วง',
    'บ้านคำปลากั้ง',
    'บ้านกุดนาค้อ',
    'บ้านตาดราษฎร์สมบูรณ์',
    'บ้านป่าหวาย',
    'บ้านนายูง',
    'บ้านโคกหนองแวง',
    'บ้านหนองกุง',
    'บ้านคำดอกไม้',
    'บ้านห้วยผึ้ง',
    'บ้านสามขา',
    'บ้านสมดี',
    'บ้านโปร่ง',
    '* บ้านโปร่ง สาขาบ้านโคกข่า',
    'บ้านห้วยวังปลา',
    'อนุบาลศรีธาตุ',
    'บ้านโคกศรี',
    'บ้านกอก',
    'บ้านหนองนกเขียน',
    'บ้านคำม่วง',
    'บ้านโคกก่อง',
    'บ้านป่าไร่',
    'คำเมยวิทยาคม',
    'คำกุงประชานุกูล',
    'บ้านโคกกลาง',
    'บ้านคำบอน',
    'คำค้อพิทยศึกษา',
    'สามัคคีวิทยาคาร',
    'บ้านหนองท่มคำเจริญ',
    'ป่าหวายวิทยาสรรพ์',
    'บ้านโนนอำนวย',
    'บ้านท่าลาด',
    '*บ้านท่าลาด สาขาแหลมทอง',
    'บ้านนาแก',
    'บะยาวพัฒนาศึกษา',
    'บ้านนาตาดนาโปร่ง',
    'บ้านโคกเล้า',
    'บ้านนานกชุมนาชุมพร',
    'บ้านโคกสว่าง',
    'บ้านคำยาง',
    'ผาสุกประชานุกูล',
    'บ้านดงกลาง',
    'บ้านคำน้อย',
    'บ้านคำจวง',
    'ดานใหญ่พิทยาคาร',
    'บ้านผาทอง',
    'คำไฮพิทยาคม',
    'บ้านวังทอง',
    'สหราษฎร์วิทยานุสรณ์',
    'อนุบาลวังสามหมอ',
    'หาญใจพิทยาคม',
    'บ้านไทยสมพร',
    'บ้านหนองกุงทับม้า',
    'บ้านโนนสวาง',
    'หนองไผ่พรเจริญ',
    'บ้านโคกสะอาด',
    'บ้านคำโคกสูง',
    'เลิงถ่อนโนนสมบูรณ์',
    'บ้านหนองหญ้าไซ',
    'อนุบาลหนองแสง',
    'บ้านสามเหลี่ยม',
    'บ้านหนองประเสริฐ',
    'บ้านหนองหว้า',
    'บ้านโนนเชียงค้ำ',
    'โรงเรียนทับกุงประชานุกูล',
    'บ้านหนองบัว',
    'บ้านนาดีโคกกลาง',
    'บ้านนาฝาย',
    'บ้านโคกศรีห้วยยาง',
    'บ้านทับไฮ',
    'บ้านท่าสี',
    'บ้านท่ายม',
    'บ้านแสงสว่าง',
    'บ้านโคกสว่าง',
    'บ้านคำบอน',
    'บ้านหนองแสง',
    'อนุบาลประจักษ์ศิลปาคม',
    'บ้านนาม่วง',
    'บ้านโนนสา',
    'บ้านโนนแสวง',
    'บ้านโคกสีวังแสง',
    'บ้านหนองลุมพุกหญ้าม้า',
    'บ้านเชียงกรม',
    'บ้านโนนทรายฟอง',
    'บ้านหนองแวงเหนือ',
    'บ้านห้วยสามพาด(สหราษฎร์พัฒนา)',
    'บ้านสะอาดนามูล',
    'บ้านโนนสมบูรณ์',
    'บ้านน้ำเที่ยงลานเตหนองเม็ก',
    'บ้านอุ่มจาน',
    'บ้านโพนทอง',
    'บ้านเมืองปัง',
    'กุมภวาปี',
    'โนนสะอาดพิทยาสรรค์',
    'ศรีธาตุพิทยาคม',
    'ห้วยเกิ้งพิทยาคาร',
    'ทมนางามวิทยาคม',
    'ตาดทองพิทยาคม',
    'กุงเจริญพิทยาคม',
    'สีออศิลปศาสตร์',
    'พันดอนวิทยา',
    'แชแลพิทยานุสรณ์',
    'หนองแดงวิทโยดม',
    'เสอเพลอพิทยาคม',
    'กุมภวาป์พิทยาสรรค์',
    'วังสามหมอวิทยาคาร',
    'นานกชุมวิทยาคม',
    'คำยางพิทยา',
    'หนองแสงวิทยศึกษา',
    'ประจักษ์ศิลปาคม',
    'สามพาดพิทยาคาร',
    'บ้านกั้งโนนสะอาด',
    'บ้านหนองกล้า',
    'บ้านหนองเต่าดอนหายโศก',
    'บ้านบ่อคํา',
    'บ้านนาอุดม',
    'บ้านดูนประชารัฐ',
    'บ้านคำอ้อ',
    'บ้านเชียง(ประชาเชียงเชิด)',
    'บ้านปูลู',
    'บ้านดงเย็น',
    'บ้านหันน้อย',
    'ธาตุดอนตูม',
    'บ้านยา',
    'บ้านหนองผือ',
    'บ้านป่าก้าว',
    'บ้านกะพี้',
    'บ้านหนองกุงดอนบาก',
    'บ้านหนองกอบง',
    'บ้านนาฮัง',
    'บ้านผักตบประชานุกูล',
    'บ้านหนองบัว',
    'บ้านพังงู',
    'บ้านขาวัว',
    'บ้านพังซ่อน',
    'บ้านเรืองชัย',
    'บ้านโคกถาวร',
    'บ้านม่วง',
    'บ้านนาดี',
    'บ้านดอนกลางตาลเดี่ยว',
    'บ้านหนองหญ้ารังกา',
    'บ้านนางิ้ว',
    'บ้านหนองสองแห้วประชาสรรค์',
    'บ้านศาลา',
    'บ้านโคกสว่างนาดี',
    'บ้านสะคาม',
    'บ้านโคกทุ่งยั้ง',
    'บ้านดอนยางเดี่ยว',
    'บ้านนิคมหนองตาล',
    'บ้านโพนงาม',
    'บ้านดงคำ',
    'บ้านวังฮาง',
    'บ้านนาเยีย',
    'บ้านสร้อยพร้าว',
    'บ้านหงษาวดี',
    'บ้านโคกมุ่นเหล่าสวรรค์',
    'บ้านดอนนางคำ',
    'บ้านสะแบง',
    'บ้านโนนนาสร้าง',
    'บ้านบ่อปัทม์',
    'บ้านดงวังพัง',
    'บ้านหนองสะหนาย',
    'บ้านหนองบัวแดง',
    'บ้านหนองไผ่พิทยาคม',
    'บ้านดงบากโนนสวรรค์',
    'บ้านเพ็กคำบากหายโศก',
    'ชุมชนบ้านหนองเม็ก',
    'บ้านหนองลาดหนองแปนคำผักกูด',
    'บ้านหนองนกทา',
    'บ้านโคกสูง',
    'บ้านหนองตาใกล้',
    'บ้านต้ายสวรรค์ดงหว้าน',
    'บ้านเม็กดงเรือง',
    'บ้านต้องหนองสระปลา',
    'บ้านดงบังหนองเขื่อน',
    'บ้านหนองบัวน้อย',
    'บ้านม่วงประชาบํารุง',
    'บ้านยาง (อาสาพัฒนา 3)',
    'บ้านหนองงิ้ว',
    'บ้านดงโพนยอ',
    'บ้านหนองบ่อ (พอลพิทยาประชานุสรณ์)',
    'อนุบาลหนองหานวิทยายน',
    'บ้านหนองหาน (วันครู 2502)',
    'บ้านเชียงงาม',
    'บ้านโสกหมู',
    'อนุบาลทุ่งฝน',
    'บ้านท่าช่วง',
    'ชุมชนกุดค้า',
    'บ้านโนนสะอาด',
    'ธาตุน้อยก่อสำราญ',
    'บ้านโพนสูง',
    'บ้านทุ่งใหญ่',
    'ชุมชนคำตานาหนองกุง',
    'บ้านคำเจริญ',
    'บุญมีศรีสว่าง',
    'บ้านโนนสมบูรณ์',
    'บ้านโพธิ์เหล่าวิชา',
    'บ้านนาชุมแสง',
    'บ้านช้าง',
    'บ้านนาทม',
    'บ้านหนองแวงตาด',
    'บ้านคำเลาะ',
    'บ้านคำน้ำทิพย์',
    'บ้านคำบอน',
    'บ้านวังชมภู',
    'บ้านดงพัฒนา',
    'สยามกลการ 3',
    'อนุบาลไชยวาน',
    'เพียปู่หนองเรือ',
    'ร่มเกล้า 2',
    'บ้านหนองแซง',
    'หนองแวงวิทยา',
    'บ้านคำยาง',
    'บ้านโพนสูงโนนสวรรค์',
    'บ้านคำม่วง',
    'บ้านห้วยยาง',
    'บ้านป่าก้าว',
    'บ้านโนนสมบูรณ์',
    'บ้านหนองอิอู',
    'บ้านหนองหลัก',
    'ชุมชนสะงวย',
    'บ้านนาปู-นากลาง',
    'บ้านหัวหนองยาง',
    'ชุมชนดงเย็น',
    'บ้านป่าเป้า',
    'บ้านโนนหอม',
    'บ้านดงแสนสุข',
    'บ้านโนนชัยศิลป์',
    'บ้านถ่อนนาลับ',
    'บ้านหนองลาด',
    'บ้านโพธิ์ท่าเมือง',
    'บ้านทุ่งใหญ่โพธิ์ชัย',
    'บ้านนามั่ง',
    'บ้านนาคำ',
    'บ้านหัวดงยาง',
    'บ้านดอนขี้เหล็ก',
    'บ้านนาดี',
    'บ้านดงวัฒนา',
    'บ้านคำบอนโนนสมโภชน์',
    'บ้านคำสง่าประชาสรรค์',
    'บ้านโนนงามอุดมวิทย์',
    'บ้านน้ำผึ้งประชาสรรค์',
    'บ้านวังคางฮูง',
    'บ้านท่าบ่อยาง',
    'บ้านนาคำวัง',
    'บ้านนาไหม',
    'บ้านกุดดู่อุดมวิทย์',
    'บ้านเมืองนาซำ',
    'บ้านดงหวาย',
    'บ้านตูม',
    'บ้านทรายมูล',
    'บ้านเหล่าอุดม',
    'บ้านดงยาง',
    'บ้านจันทน์',
    'บ้านหนองกาโนนสิมมา',
    'บ้านนาสีนวล',
    'บ้านโนนศรีทอง',
    'บ้านชัย',
    'บ้านดงไพรวัลย์',
    'บ้านกล้วย',
    'บ้านโนนสะอาด',
    'บ้านทุ่ง',
    'ชุมชนบ้านดุง',
    'บ้านนาโฮง',
    'บ้านหนองไฮโนนสำราญ',
    'บ้านปอพาน',
    'บ้านห้วยปลาโดศรีสามารถ',
    'บ้านหนองสองห้อง',
    'บ้านถ่อนคำหวด',
    'บ้านคำภูเงิน',
    'บ้านสมวิไล',
    'บ้านงิ้วมีชัย',
    'บ้านตาดโนนทองหลาง',
    'บ้านโคกกลาง',
    'บ้านม่วง',
    'บ้านเมืองไพร',
    'บ้านศรีเมือง',
    'บ้านหนองสว่าง',
    'กมลศิลป์สามัคคี',
    'บ้านนาจาน',
    'บ้านฝาง',
    'บ้านดงค้าพัฒนา',
    'บ้านปากดง',
    'กำแมดคำเจริญ',
    'บ้านวังทอง',
    'บ้านเหล่าหลวง',
    'ศรีขวัญเมือง',
    'พัฒนาศรีผดุงอุดมวิทย์',
    'อนุบาลศรีสุทโธ',
    'บ้านโคกคำไหล',
    'บ้านอ้อมกอ',
    'บ้านดงดารา',
    'บ้านโนนสมบูรณ์',
    'บ้านไผ่ล้อมโนนสมบัติ',
    'บ้านนายม',
    'บ้านดอนกลอย',
    'บ้านไทย',
    'บ้านหว้านใหญ่ประชาคม',
    'บ้านหนองบัวสะอาด',
    'บ้านดงยางน้อย',
    'บ้านดอนม่วง',
    'บ้านหนองไผ ่(วัดเวฬุวันอุปถัมภ์)',
    'บ้านนาทรายน้ำรอด',
    'บ้านนานกหงส์เสรี',
    'บ้านถ่อนนาเพลิน',
    'บ้านไชยวานโนนลือชัย',
    'อนุบาลพิบูลย์รักษ์',
    'บ้านโพธิ์',
    'บ้านดงยางพรพิบูลย์',
    'บ้านหนองผักแว่นดอนเขือง(วัดบูรพารามอุถัมภ์)',
    'บ้านค้อน้อย',
    'บ้านม่วงคอนสาย',
    'บ้านดอนสวรรค์',
    'บ้านโนนสะอาด',
    'บ้านหนองช้างคาว',
    'บ้านโนนถั่วดิน',
    'บ้านคำแคนแก่นคูณ',
    'บ้านไพจาน',
    'บ้านค้อดอนแคน',
    'บ้านโยธา',
    'บ้านวังแข้',
    'บ้านโนนทองอินทร์',
    'บ้านหัวหนอง',
    'บ้านยางเหล่าสวนกล้วย',
    'อนุบาลกู่แก้ว',
    'บ้านซำป่าหัน',
    'บ้านซำป่ารัง',
    'หนองหานวิทยา',
    'ทุ่งฝนวิทยาคาร',
    'ไชยวานวิทยา',
    'ทุ่งฝนพัฒนศึกษา',
    'โพนสูงพัฒนศึกษา',
    'พังงูพิทยาคม',
    'บ้านเชียงวิทยา',
    'บ้านดุงวิทยา',
    'ดงเย็นพิทยาคาร',
    'นาไหมพิทยาคม รัชมังคลาภิเษก',
    'ทรัพย์อุดมวิทยา',
    'ชัยนาคำวิทยา',
    'โนนสะอาดชุมแสงวิทยา',
    'พิบูลย์รักษ์พิทยา',
    'นาทรายวิทยาคม',
    'กู่แก้ววิทยา',
    'บ้านดงบัง',
    'บ้านดงธาตุ',
    'บ้านดงหวาย',
    'บ้านหนองโนไชยวาน',
    'บ้านกุดจับ',
    'บ้านโคกวิชัย',
    'บ้านลำภู',
    'บ้านขอนยูง',
    'บ้านหนองฆ้องนาสีนวล',
    'บ้านหนองกุงหนองเจริญ',
    'บ้านห้วยเชียงหนองอีเบ้า',
    'บ้านถิ่น',
    'บ้านนาแคทุ่งขวาง',
    'ชุมชนบ้านสร้างแป้น',
    'บ้านโนนทอง',
    'ชุมชนหนองแปนหินโงม',
    'บ้านเชียงเพ็ง',
    'ชุมชนบ้านทุ่งตาลเลียน',
    'บ้านเหล่าตำแย',
    'บ้านโนนม่วง',
    'บ้านโคกสว่าง',
    'บ้านหนองสร้างไพร',
    'ชุมชนบ้านโพธิ์โนนแดงวิทยา',
    'บ้านกานต์สามัคคี',
    'บ้านหันเทาผักกาดย่า',
    'ปะโคสามัคคีวิทยา',
    'โนนสูงวิทยา',
    'บ้านหัวขัว',
    'บ้านข่า',
    'บ้านโสกแกคำเจริญ',
    'บ้านดงน้อย',
    'อนุบาลบ้านเพียมิตรภาพที่ 138',
    'นิคมสร้างตนเองห้วยหลวง 2',
    'โนนธาตุนาอ้ายใหญ่วิทยา',
    'นิคมสร้างตนเองห้วยหลวง 1',
    'บ้านหนองทุ่ม',
    'บ้านโนนฐานะ',
    'บ้านสร้างก่อ',
    'บ้านผักบุ้ง',
    'กลางใหญ่นิโรธรังสีอุปถัมภ์',
    'บ้านเม็ก',
    'บ้านข้าวสาร',
    'บ้านหินตั้ง',
    'บ้านเขือน้ำ',
    'บ้านเทื่อม',
    'บ้านหลุมข้าว',
    'บ้านค้อ (เขือน้ำ)',
    'บ้านนาสี',
    'บ้านโนนสะอาด',
    'บ้านนางิ้ว',
    'บ้านคำด้วง',
    'บ้านสระคลองพัฒนา',
    'อุดรวัฒนานุสรณ์',
    'บ้านตาดน้ำพุ',
    'บ้านลาดหอคำ',
    'บ้านนาหลวง',
    'ราษฎร์จุฬาสามัคคี',
    'บ้านดงเย็นพัฒนา',
    'คำบงเจริญสุข',
    'บ้านดงหมูชัยเจริญ',
    'บ้านนาล้อม',
    'บ้านโนนสมบูรณ์',
    'กลางน้อยเหล่ามะแงว',
    'บ้านวังสวย',
    'บ้านโนนสว่าง',
    'บ้านขัวล้อ',
    'บ้านแดง',
    'บ้านจำปาโมง',
    'บ้านม่วง',
    'บ้านนาอ่าง',
    'บ้านลาน',
    'ดอนตาลดงบังวิทยา',
    'บ้านนาเตย',
    'บ้านนาไฮ',
    'บ้านคูดงประชาสรรค์',
    'บ้านโพน',
    'บ้านโนนทอง',
    'บ้านโนนดู่',
    'บ้านค้อ (ธัญญวิทยาคาร)',
    'บ้านหนองกอง',
    'บ้านนาคูณ',
    'บ้านนารายณ์',
    'อนุบาลบ้านผือ',
    'ชุมชนบ้านแวง',
    'บริบาลภูมิเขตต์',
    'บ้านภูดิน',
    'บ้านติ้ว',
    'บ้านหนองกบ',
    'เมืองพานบ้านโก่มวิทยา',
    'บ้านใหม่',
    'จอมศรีโคกก่องวิทยา',
    'บ้านกาลึม',
    'บ้านนางาม',
    'ชุมชนสงเคราะห์',
    'บ้านหนองแวง',
    'บ้านแหลมทองพัฒนา',
    'บ้านหนองบอนเวียงชัย',
    'บ้านสะคุวิทยา',
    'บ้านยางโกน',
    'บ้านโพธิ์',
    'บ้านหนองหัวคู',
    'บ้านกุดเม็ก',
    'บ้านดงหวายดงขวาง',
    'บ้านหนองนกเขียน',
    'หนองกุงวังแสง',
    'บ้านโคกสีแก้ว',
    'บ้านหายโศก',
    'บ้านธาตุประชานุกูล',
    'บ้านนาคำ',
    'บ้านโนนอุดม',
    'บ้านเจริญสุข',
    'บ้านนางัว',
    'บ้านดงพัฒนาดงต้องประชารัฐ',
    'อนุบาลชุมชนน้ำโสม',
    'บ้านโคกน้อย',
    'บ้านนาเมืองไทย',
    'บ้านโนนสมบูรณ์',
    'บ้านเทพประทาน',
    'บ้านหยวก',
    'บ้านจำปาทอง',
    'บ้านน้ำขุ่นโนนผางาม',
    'บ้านทุ่งทอง',
    'บ้านท่าลี่',
    'บ้านน้ำปู่',
    'บ้านท่าโสม',
    'บ้านนาจานโนนนาโพธิ์',
    'บ้านน้ำทรง',
    'บ้านสามัคคี',
    'บ้านผากลางนา',
    'บ้านโชคเจริญ',
    'บ้านสวัสดี',
    'บ้านโสมเยี่ยม',
    'บ้านหัวช้าง',
    'เตชะไพบูลย์ (2)',
    'บ้านนาเก็น',
    'บ้านสาครพัฒนา',
    'บ้านหนองแวง',
    'บ้านน้ำซึม',
    'บ้านโคกสะอาด',
    'บ้านวังบง',
    'บ้านวังแข้',
    'บ้านเพิ่ม(อณุโรอุปถัมย์)',
    'ชายแดนประชาสรรค์',
    'บ้านนาแค',
    'ชุมพลนาคลัง',
    'บ้านนาต้องนาสมนึก',
    'บ้านนายูง',
    'บ้านสว่างปากราง',
    'บ้านวังเลาโสมสวรรค์',
    'บ้านเชียงดีกุดเชือม',
    'บ้านท่าโปงทอง',
    'บ้านปากเจียงโนนทอง',
    'บ้านนาตูม',
    'บ้านนาคำน้อย',
    'บ้านนาคำ',
    'อนุบาลบ้านก้องวิทยา',
    'กุดจับประชาสรรค์',
    'บ้านผือพิทยาสรรค์',
    'น้ำโสมพิทยาคม',
    'ภูพานวิทยา',
    'จำปาโมงวิทยาคาร',
    'น้ำซึมพิทยาคม',
    'สามัคคีพิทยาคาร',
    'เชียงเพ็งวิทยา',
    'หนองหัวคูปวงประชานุเคราะห์',
    'ภูพระบาทวิทยา',
    'ยูงทองพิทยาคม',
    'บ้านท่าเปิบ',
    'บ้านโป่งป่าติ้ว',
    'บ้านไผ่โทน',
    'บ้านห้วยเหล็ก',
    'บ้านตูบโกบ',
    'บ้านลาดค้อ',
    'บ้านกำพี้',
    'บ้านกกดู่',
    'บ้านห้วยหวาย',
    'บ้านกกทอง',
    'บ้านห้วยกระทิง',
    'บ้านสวนกล้วย',
    'เมืองเลย',
    'บ้านหนองผักก้าม',
    'อนุบาลเลย',
    'บ้านภูบ่อบิด',
    'บ้านก้างปลา',
    'บ้านนาบอน',
    'บ้านท่าข้าม',
    'บ้านหัวฝาย',
    'บ้านห้วยทราย',
    'บ้านโป่ง',
    'บ้านนาแขม',
    'บ้านวังยาว',
    'บ้านนาดินดำ',
    'บ้านหนองหญ้าไซ',
    'บ้านห้วยม่วง',
    'บ้านหนองนาทราย',
    'บ้านทรัพย์มงคล',
    'บ้านหนองผำ',
    'บ้านนาโป่ง',
    'บ้านขอนแก่นหนองบอน',
    'บ้านติ้วน้อย',
    'บ้านบง',
    'บ้านห้วยโตก',
    'บ้านกกชุมแสง',
    'บ้านป่าข้าวหลาม',
    'บ้านโพน',
    'ชุมชนบ้านนาอ้อ',
    'บ้านท่ามะนาว',
    'บ้านแหล่งควาย',
    'บ้านไร่ทาม',
    'บ้านขอนแดง',
    'บ้านติดต่อ',
    'บ้านฟากนา',
    'บ้านเพียซำพุวิทยา',
    'บ้านสูบ',
    'บ้านนาน้ำมัน',
    'บ้านวังแคน',
    'บ้านหนองดอกบัว',
    'บ้านสะอาดลายเหนือ',
    'มโนบุเรศรบำรุงการ',
    'บ้านห้วยลวงไซ',
    'บ้านห้วยสีเสียด',
    'บ้านส้าน',
    'บ้านปากภู',
    'บ้านน้ำภู',
    'ชุมชนบ้านกำเนิดเพชร',
    'บ้านนาโคก',
    'บ้านปากหมาก',
    'บ้านท่าบุ่ง',
    'บ้านวังโป่งท่าวังแคน',
    'บ้านนาม่วง',
    'บ้านโพนป่าแดง',
    'บ้านเสี้ยว',
    'บ้านน้ำคิว',
    'บ้านภูสวรรค์',
    'บ้านกอไร่ใหญ่',
    'บ้านท่าสวรรค์',
    'บ้านน้ำสวยภักดี',
    'ชุมชนท่าสะอาด',
    'บ้านราษฎร์ประดิษฐ์',
    'ชุมชนบ้านนาด้วง',
    'บ้านแก้วเมธี',
    'บ้านนาดอกคำ',
    'บ้านน้ำสวยห้วยปลาดุก',
    'บ้านโพนสว่าง',
    'บ้านห้วยตาด',
    'ชุมชนบ้านโสกวิทยาคาร',
    'บ้านนาป่าหนาด',
    'บ้านตาดซ้อ',
    'บ้านท่าบม',
    'บ้านนาเบน',
    'บ้านวังอาบช้างมิตรภาพที่ 141',
    'บ้านนาสีสลากกินแบ่งสงเคราะห์ที่ 59',
    'บ้านหินตั้ง',
    'บ้านแสนสำราญ',
    'บ้านคกมาด',
    'บ้านโนนสว่าง',
    'บ้านเชียงคาน"ปทุมมาสงเคราะห์"',
    'บ้านเชียงคาน"วิจิตรวิทยา"',
    'บ้านน้อย',
    'บ้านสงเปือย',
    'บ้านผากลางดง',
    'ไทยรัฐวิทยา96(ชุมชนบ้านธาตุ)',
    'บ้านธาตุวิทยา',
    'บ้านห้วยหินซาน้ำอ้อมพรหมมานุสรณ์',
    'บ้านห้วยพอด',
    'บ้านผาพอด',
    'ชุมชนบ้านนาบอน',
    'บ้านใหม่ตาแสง',
    'บ้านโพน',
    'บ้านใหม่',
    'บ้านแก่งมี้',
    'บ้านนาซ่าว',
    'บ้านผาแบ่น',
    'ห้วยซวกคกเลาใต้',
    'บ้านอุมุง',
    'บ้านบุฮม',
    'บ้านคกเลา',
    'บ้านนาจาน',
    'บ้านน้ำพร',
    'บ้านคกงิ้ว',
    'บ้านกลาง',
    'บ้านท่าดีหมี',
    'ภิรมย์ภักดีบ้านห้วยสีดา',
    'บ้านหาดทรายขาวผามุม',
    'บ้านสาระแพ',
    'บ้านหัวแก่ง',
    'บ้านแก่งปลาปก',
    'บ้านห้วยอาลัย',
    'บ้านชมน้อย',
    'ร่มเกล้าบ้านชมเจริญ',
    'ชุมชนบ้านเชียงกลม',
    'บ้านปางคอม',
    'บ้านโพนทอง',
    'บ้านคอนสา',
    'บ้านห้วยนา',
    'บ้านห้วยผักกูด',
    'จักรพงษ์บ้านห้วยเทียน',
    'ชุมชนบ้านปากชม',
    'บ้านนาค้อ',
    'บ้านหาดเบี้ย',
    'บ้านคกไผ่',
    'บ้านห้วยบ่อซืน',
    'บ้านวังผา',
    'บ้านเลิง',
    'บ้านสงาว',
    'บ้านปากเนียม',
    'บ้านปากปัด',
    'บ้านห้วยพิชัย',
    'บ้านห้วยหินขาว',
    'บ้านหาดคัมภีร์',
    'บ้านปากมั่งห้วยทับช้าง',
    'บ้านนาโม้',
    'บ้านคกเว้า',
    'บ้านห้วยขอบห้วยเหียม',
    'บ้านกกจาน',
    'บ้านน้ำหมัน',
    'บ้านห้วยมุ่น',
    'เย็นศิระบ้านหมากแข้ง',
    'บ้านตูบค้อ',
    'บ้านน้ำเย็น',
    'บ้านแก่งครก',
    'บ้านนาหว้าน้อย',
    'บ้านโคกงาม',
    'บ้านห้วยตาด',
    'บ้านนาเจียง',
    'บ้านหนองหลวง',
    'บ้านชั่งสี่',
    'บ้านหนองสนุ่น',
    'บ้านนาหว้า',
    'บ้านศาลาน้อย',
    'ด่านซ้าย',
    'ชุมชนบ้านด่านซ้าย',
    'บ้านนาทุ่ม',
    'บ้านน้ำพุ',
    'บ้านห้วยปลาฝา',
    'บ้านแก่งม่วง',
    'บ้านแก่วตาว',
    'บ้านนาหมูม่น',
    'บ้านนาดี',
    'บ้านหัวนาแหลม',
    'บ้านโพนหนอง',
    'บ้านหนองผือ',
    'บ้านปากหมัน',
    'บ้านนาทอง',
    'บ้านนาข่า',
    'บ้านเครือคู้',
    'บ้านปากโป่ง',
    'บ้านห้วยน้ำเมย',
    'บ้านโป่งชี',
    'บ้านโป่ง',
    'บุญลักษณ์อุปถัมภ์',
    'บ้านทุ่งน้ำใส',
    'บ้านทุ่งเทิง',
    'บ้านน้ำพุง',
    'บ้านปางคอม',
    'บ้านด่านดู่',
    'บ้านโพนสูง',
    'บ้านหนองอุมลัว',
    'บ้านกกเหี่ยน',
    'บ้านห้วยทอง',
    'บ้านนาลานข้าว',
    'บ้านหัวฝาย',
    'บ้านกกโพธิ์วังกำ',
    'บ้านวังยาว',
    'บ้านกกสะตี',
    'บ้านผึ้ง',
    'บ้านปากแดง',
    'บ้านวังเวิน',
    'บ้านวังบอน',
    'บ้านตาดเสี้ยว',
    'บ้านทับกี่',
    'บ้านหินแลบ',
    'บ้านวังเป่ง',
    'บ้านป่าสะแข',
    'บ้านห้วยไผ่',
    'บ้านถ้ำพระ',
    'บ้านแก่วตาว',
    'บ้านป่าม่วง',
    'บ้านห้วยลาด',
    'ชุมชนบ้านนาพึง',
    'บ้านกลาง',
    'บ้านนามาลา',
    'บ้านโคก',
    'บ้านซำทอง',
    'บ้านหนองสิม',
    'บ้านหนองหวาย',
    'บ้านหัวด่านนาปูน',
    'บ้านนาแห้ว',
    'บ้านเหมืองแพร่',
    'บ้านแสงภา',
    'บ้านบ่อเหมืองน้อย',
    'บ้านห้วยน้ำผัก',
    'บ้านนาเจริญ',
    'บ้านนาลึ่ง',
    'บ้านนาผักก้าม',
    'บ้านเหล่ากอหก',
    'บ้านแก่งม่วง',
    'บ้านท่าศาลา',
    'บ้านท่อน',
    'บ้านบง',
    'บ้านปลาบ่า',
    'บ้านสองคอน',
    'บ้านกลาง',
    'บ้านหินสอ',
    'บ้านโป่งกวาง',
    'บ้านแก่ง',
    'บ้านร่องจิก',
    'บ้านนาขามป้อม',
    'บ้านนาคูณ',
    'บ้านกกโพธิ์แสนเอี้ยม',
    'บ้านแก่งแล่น',
    'บ้านไฮตาก',
    'บ้านแก่งเกลี้ยง',
    'บ้านลาดค่าง',
    'บ้านห้วยติ้ว',
    'บ้านห้วยผักเน่า',
    'บ้านโคนผง',
    'บ้านห้วยลาด',
    'บ้านถ้ำมูล',
    'บ้านหนองแซง',
    'บ้านโนนสมบูรณ์',
    'บ้านสานตม มิตรภาพที่ 101',
    'บ้านแก่งไฮ',
    'ชุมชนภูเรือ',
    'บ้านหนองบง',
    'บ้านโคกใหญ่',
    'บ้านร่องไผ่',
    'บ้านบวกอ่าง',
    'บ้านห้วยไค้',
    'บ้านยาง',
    'บ้านวังขาม',
    'บ้านชลประทาน',
    'บ้านน้ำกระโทม',
    'บ้านท่าลี่',
    'บ้านห้วยด้าย',
    'บ้านน้ำแคม',
    'บ้านปากยาง',
    'บ้านแก่งม่วง',
    'บ้านห้วยเดื่อ',
    'บ้านน้ำมี',
    'บ้านหนองบง',
    'บ้านวังเป่ง',
    'บ้านเมี่ยง',
    'ชุมชนบ้านปากห้วย',
    'บ้านขอนแก่น',
    'บ้านหนองผือ',
    'บ้านหาดพระ',
    'บ้านน้ำพาน',
    'บ้านหนองปกติ',
    'บ้านห้วยคัง',
    'บ้านนากระเซ็ง',
    'ชุมชนบ้านอาฮี',
    'เลยพิทยาคม',
    'เลยอนุกูลวิทยา',
    'นาอ้อวิทยา',
    'น้ำสวยพิทยาสรรพ์',
    'นาด้วงวิทยา',
    'เชียงคาน',
    'เขาแก้ววิทยาสรรพ์',
    'ธาตุพิทยาคม',
    'จุฬาภรณราชวิทยาลัย เลย',
    'ปากชมวิทยา',
    'เชียงกลมวิทยา',
    'คอนสาวิทยา',
    'วังโพนงามวิทยา',
    'พระแก้วอาสาวิทยา',
    'ศรีสองรักษ์วิทยา',
    'นาแห้ววิทยา',
    'ภูเรือวิทยา',
    'ท่าลี่วิทยา',
    'บ้านนาซำแซง',
    'บ้านห้วยผุก',
    'บ้านนาหลวงยางเดี่ยว',
    'บ้านขอนแก่น',
    'บ้านน้ำทบ',
    'บ้านแก่งหินกกสะทอน',
    'บ้านขอนยาง',
    'บ้านป่าบง',
    'บ้านโคกขมิ้น',
    'บ้านหินเกิ้ง',
    'บ้านลาด',
    'บ้านกุดลัน',
    'บ้านโคกสว่างป่าไม้งาม',
    'บ้านซำนกจิบ',
    'ชุมชนบ้านทรายขาว',
    'บ้านนาวัว',
    'บ้านโพนงาม',
    'บ้านบุ่งค้อ',
    'บ้านกกซ้อ',
    'บ้านเหว่อฟากห้วยกุดตอเรือ',
    'บ้านโพนทอง',
    'บ้านน้ำค้อ',
    'บ้านบุ่งตาข่าย',
    'บ้านกุดโง้งวังเดื่อ',
    'บ้านปากปวน',
    'บ้านท่าทิศเฮืองกกเกลือ',
    'บ้านวังแท่น',
    'บ้านผาน้อย',
    'บ้านหนองขาม',
    'บ้านโคกแฝก',
    'บ้านกุดแก',
    'บ้านโนน',
    'บ้านโนนกกจาน',
    'บ้านโคกมน',
    'บ้านนาแก',
    'บ้านกกเกลี้ยง',
    'บ้านนาอีเลิศ',
    'ชุมชนวังสะพุง',
    'บ้านวังสะพุง',
    'บ้านน้ำอ้อมเล้า',
    'บ้านบุ่งไสล่',
    'บ้านห้วยทรายคำ',
    'บ้านโนนสว่าง',
    'บ้านดงน้อยโคกหนองแก',
    'บ้านเล้า',
    'บ้านกกบก',
    'บ้านน้ำจันทร์',
    'เจียรวนนท์อุทิศ 4',
    'บ้านหมากแข้งกกกอก',
    'บ้านหนองงิ้ว',
    'บ้านวังไหโนนสมบูรณ์',
    'บ้านหนองนอ',
    'บ้านหนองปาดฟาน',
    'บ้านศรีอุบลพัฒนา',
    'บ้านบึงสวรรค์',
    'บ้านนาดอกไม้ มิตรภาพที่ 120',
    'บ้านเหมืองแบ่ง',
    'บ้านวังกกเดื่อ',
    'ชุมชนบ้านผานกเค้า',
    'บ้านห้วยส้มใต้',
    'บ้านช่องฝาง',
    'บ้านศรีรักษา',
    'บ้านวังลาน',
    'บ้านห้วยหมาก',
    'บ้านผาสามยอด',
    'บ้านซำไฮ',
    'บ้านปวนพุ',
    'บ้านเหล่าใหญ่',
    'บ้านหนองหมากแก้ว',
    'บ้านผาหวาย',
    'บ้านสวนห้อมผางาม',
    'บ้านสงป่าเปลือย',
    'ภูกระดึง',
    'บ้านทุ่งใหญ่',
    'บ้านหนองตูม',
    'ไทยรัฐวิทยา 49 (บ้านวงเวียน)',
    'บ้านนายาง',
    'บ้านห้วยคะมะ',
    'บ้านร่องป่าไผ่',
    'บ้านตาดข่า',
    'บ้านไร่พวย มิตรภาพที่ 18',
    'บ้านโคกใหญ่',
    'บ้านห้วยไผ่เหนือ',
    'บ้านห้วยเป้าเหนือ',
    'บ้านห้วยเดื่อน้อย',
    'บ้านภูทองหลัก 160',
    'ชุมชนบ้านหนองหิน',
    'บ้านห้วยไผ่ใต้',
    'บ้านห้วยเดื่อโคกสว่าง',
    'บ้านไร่ศรีอุบล',
    'บ้านนาแปน',
    'บ้านนาโก',
    'บ้านนาน้อย',
    'บ้านพองหนีบ',
    'บ้านอีเลิศ',
    'บ้านวังยาง',
    'บ้านห้วยไผ่',
    'บ้านซำบ่าง',
    'บ้านผาฆ้อง',
    'บ้านห้วยส้ม',
    'บ้านแก่งศรีภูมิ',
    'บ้านฟากเลย',
    'บ้านนาดินดำ',
    'บ้านสวนปอ',
    'บ้านหนองบัว',
    'บ้านหนองเขียด',
    'บ้านหนองอีเก้ง',
    'บ้านนาฝายนาโพธิ์',
    'บ้านไร่สุขสันต์สาขาห้วยส้มป่อย',
    'บ้านไร่สุขสันต์',
    'บ้านโคกหนองแห้ว',
    'บ้านเลยวังไสย์',
    'เลยตาดโนนพัฒนา',
    'ชุมชนบ้านหนองคัน',
    'บ้านอุ่ม',
    'บ้านนามูลตุ่น',
    'บ้านห้วยสีเสียด',
    'บ้านทรัพย์เจริญ',
    'วังน้ำพุ',
    'บ้านพวยเด้ง',
    'บ้านหนองตานา',
    'บ้านนาตาดสมสะอาด',
    'บ้านหนองแต้หนองโก',
    'บ้านโนนสมบูรณ์',
    'บ้านหัวขัว',
    'บ้านนาเหมือดแอ่',
    'บ้านท่าช้างคล้องหนองอีเปี้ย',
    'บ้านโนนภูทอง',
    'บ้านโนนปอแดง',
    'บ้านนาล้อม',
    'บ้านโนนกกข่า',
    'บ้านห้วยไคร้',
    'บ้านซำกกค้อ',
    'บ้านศรีวิไลท่าสวนยา',
    'บ้านซำไคร้',
    'บ้านโนนป่าซาง',
    'บ้านห้วยกาบเปลือย',
    'บ้านห้วยเดื่อเหนือ',
    'บ้านห้วยยาง',
    'บ้านโนนสว่างโสกนกไก่นา',
    'บ้านภูป่าไผ่',
    'เจียรวนนท์อุทิศ 3',
    'บ้านซำพร้าว',
    'บ้านเพิ่ม',
    'บ้านโคกผักหวาน',
    'บ้านหัวฝาย',
    'บ้านแสนสุข',
    'บ้านผาขาว',
    'บ้านดงน้อย',
    'บ้านซำบุ่น',
    'บ้านนาอ่างคำ',
    'บ้านผาสามยอด',
    'บ้านผาสะนา',
    'บ้านนาอุดม',
    'บ้านซำม่วง',
    'บ้านห้วยป่าน',
    'บ้านหนองตูม',
    'บ้านโนนสวรรค์',
    'บ้านหนองใหญ่',
    'บ้านเอราวัณ',
    'บ้านผาซำบอน',
    'ผานาง-ผาเกิ้ง',
    'บ้านโคกสง่า',
    'บ้านเหล่าใหญ่',
    'บ้านวังม่วง',
    'บ้านโป่งศรีโทน',
    'บ้านวังเลาหัวฝาย',
    'ศึกษาสงเคราะห์เลย',
    'ศรีสงครามวิทยา',
    'ภูกระดึงวิทยาคม',
    'ภูหลวงวิทยา',
    'สันติวิทยาสรรพ์',
    'เอราวัณวิทยาคม',
    'เหมืองแบ่งวิทยาคม',
    'ผาอินทร์แปลงวิทยา',
    'วังทรายขาววิทยา',
    'ผาสามยอดวิทยาคม',
    'เขาหลวงวิทยา',
    'เซไลวิทยาคม',
    'ผาน้อยวิทยาคม',
    'เลยสว่างวิทยาคม',
    'หนองหินวิทยาคม',
    'บ้านเมืองหมี "พรหมวิทยา"',
    'บ้านพร้าวเหนือ',
    'ราชประชานุเคราะห์ 14',
    'นาฮีนุเคราะห์',
    'ค่ายบกหวานโพนตาลวิทยา',
    'บ้านโคกสำราญ',
    'บ้านคำโป้งเป้งโคตรธิสารวิทยา',
    'บ้านหนองหมื่น',
    'บ้านเดื่อใต้',
    'บ้านโคกป่าฝาง',
    'บ้านหนองบ่อ',
    'ชุมชนบ้านปะโค',
    'พระธาตุบังพวน',
    'ราษฎร์นุเคราะห์',
    'บ้านเชือกบ้านหม้วยวิทยา',
    'โคกคำทุ่งสว่างวิทยา',
    'บ้านบ่อแปบหนองหญ้าม้าวิทยา',
    'บ้านหนองผือ',
    'บ้านโคกก่อง',
    'มะเขือดอนหมูโพธิ์ทองวิทยา',
    'บ้านหนาด "คุรุราษฎร์อุทิศ"',
    'อนุบาลอรุณรังษี',
    'เนินพระเนาว์วิทยา',
    'จันทราราม',
    'หนองแดงสร้างประทายพัฒนา',
    'มีชัยวิทยา',
    'บ้านโคกแมงเงา',
    'จอมมณี',
    'บ้านหนองแจ้ง',
    'บ้านเมืองบาง',
    'บ้านนาคลอง',
    'บ้านวังยาง',
    'หาดคำบอนวัฒนา',
    'บ้านเบิดวิทยา',
    'เหล่าฝ้ายผดุงวิทย์',
    'บ้านเวียงคุก',
    'ครุราษฎร์รังสรรค์',
    'โพนสว่างป่าตองวิทยา',
    'บ้านนาอ่างสร้างอ่าง',
    'ดงเว้นดงเจริญวิทยา',
    'ดาวเรืองสมสะอาด',
    'บ้านสร้างพอก',
    'บ้านสวยหลง',
    'บ้านบงวิทยา',
    'บ้านจอมแจ้ง',
    'หัวหาดวิทยา',
    'บ้านหมากก่องฝายแตก',
    'สีกายวิทยาคม',
    'อนุบาลหนองคาย',
    'บ้านคุยนางขาว',
    'ธนากรสงเคราะห์',
    'บ้านหนองเดิ่น',
    'อักษรอดุลวิทยา',
    'บ้านพร้าวใต้',
    'หินโงมวิทยา',
    'บ้านดงเจริญ',
    'บ้านกองนาง',
    'บ้านปากมาง',
    'บ้านหงส์ทองสามขา',
    'บ้านป่าสัก',
    'บ้านนาบง',
    'บ้านโคกคอน',
    'บ้านหนองผือ',
    'บ้านโพนพระ',
    'โกมลวิทยาคาร',
    'อนุบาลดารณีท่าบ่อ',
    'บ้านนาช้างน้ำ',
    'บ้านกวดโคกสว่าง',
    'ร่วมมิตรวิทยา',
    'บ้านท่าสำราญ',
    'บ้านอุ่มเย็น',
    'บ้านหนองแวง',
    'บ้านน้ำโมง',
    'บ้านทุ่มฝาง',
    'บ้านเดื่อ',
    'บ้านน้อย',
    'บ้านเทวีดอนขม',
    'โพนศิลางามวิทยา',
    'ชุมชนบ้านถ่อน',
    'บ้านว่าน',
    'บ้านแสนสุข',
    'บ้านเป้า',
    'คุรุศิษย์วิทยา',
    'ชุมชนบ้านโพนสา',
    'บ้านท่ามะเฟือง',
    'ราษฎร์สามัคคี',
    'บ้านนาน้ำพาย',
    'บ้านหนองนาง',
    'บ้านนาดง',
    'บ้านดงนาคำ',
    'ชุมชนบ้านหม้อ',
    'บ้านท่ากฐิน',
    'บ้านหม้อเหนือ(หลวงปู่เหรียญ อุปถัมภ์)',
    'พัฒนาคำแก้ว',
    'บ้านป่าสัก',
    'บ้านวังน้ำมอก',
    'พระพุทธบาทเทสรังสีวิทยา',
    'บ้านถิ่นทอนเหนือ',
    'บ้านศรีเชียงใหม่',
    'อนุบาลศรีเชียงใหม่',
    'สำนักงานสลากกินแบ่งที่ 75',
    'บ้านโนนสง่า',
    'บ้านหัวทราย',
    'บ้านพานพร้าว',
    'ดอนก่อโนนสวรรค์',
    'บ้านไทยสามัคคี',
    'หนองปลาปากจำปาทอง',
    'บ้านขุมคำ',
    'บ้านนาโพธิ์',
    'บ้านดงบัง',
    'บ้านเสียว',
    'ชุมชนประชาสงเคราะห์ 4',
    'บ้านสังกะลีนาขาม',
    'บ้านโสกกล้า',
    'บ้านสังกะลีนาขาม สาขา หนองแสง',
    'บ้านนางิ้ว',
    'บ้านซำเจียง-ดงป่าเปลือย',
    'บ้านเทา-นาบอน',
    'บ้านเทพประทับ',
    'บ้านตาดเสริม',
    'บ้านม่วง',
    'บ้านหนอง',
    'บ้านวังมน',
    'บ้านผาตั้ง',
    'บ้านห้วยไซงัว',
    'บ้านดงต้อง',
    'บ้านปากโสม',
    'บ้านนาโคก',
    'บ้านม่วงน้ำไพร',
    'บ้านแก้งใหม่',
    'บ้านฟ้าประทาน',
    'อนุบาลสังคม',
    'อนุบาลบ้านคอกช้าง',
    'บ้านเหมือดแอ่',
    'สมสะอาดดงมุขวิทยา',
    'บ้านนากอ',
    'บ้านฝางวิทยา',
    'บ้านเดื่อทุ่งสวรรค์',
    'บ้านหมากหุ่งขี้เหล็ก',
    'โนนดู่โพนหวายประชาสรรค์',
    'บ้านโนนแดง',
    'บ้านหนองบัววิทยา',
    'บ้านสมสนุก',
    'ฉันทนาวัณรถ',
    'บ้านตอแก',
    'สระใครไชยานุเคราะห์',
    'บ้านหนองบัวเงิน',
    'บ้านน้ำสวยมิตรภาพที่ 19',
    'บ้านโพนสวรรค์',
    'บ้านศูนย์กลาง',
    'บ้านน้ำทอน',
    'อาโอยาม่า 2',
    'อาโอยาม่า 2 สาขาบ้านห้วยหมากพริก',
    'บ้านดอนขนุนพัฒนา',
    'บ้านไร่',
    'บ้านโพนทอง',
    'อนุบาลดอนไผ่',
    'บ้านโพธิ์ตาก',
    'บ้านสาวแล',
    'ปทุมเทพวิทยาคาร',
    'หินโงมพิทยาคม',
    'กวนวันวิทยา',
    'เวียงคำวิทยาคาร',
    'หนองคายวิทยาคาร',
    'พระธาตุบังพวนวิทยา',
    'ค่ายบกหวานวิทยา',
    'ท่าบ่อ',
    'โคกคอนวิทยาคม',
    'ถ่อนวิทยา',
    'ท่าบ่อพิทยาคม',
    'หนองนางพิทยาคม',
    'เดื่อวิทยาคาร',
    'พานพร้าว',
    'พระพุทธบาทวิทยาคม',
    'วรลาโภนุสรณ์',
    'สังคมวิทยา',
    'วังม่วงพิทยาคม',
    'น้ำสวยวิทยา',
    'ฝางพิทยาคม',
    'โพธิ์ตากพิทยาคม',
    'บ้านกุดบง',
    'บ้านใหม่',
    'บ้านนิคมดงบัง',
    'ชุมชนบ้านโนน-โนนสวรรค์',
    'บ้านโนนฤาษี',
    'บ้านหนองกุ้ง',
    'บ้านกลุ่มพัฒนา',
    'บ้านนิคมดงบัง สาขาบ้านนามะเฮียว',
    'อนุบาลจุมพลโพนพิสัย',
    'บ้านเวินร่องถ่อนสามัคคี',
    'หมู่บ้านตัวอย่าง',
    'บ้านห้วยเปลวเงือก',
    'บ้านนาตาล',
    'บ้านนาเพียงใหญ่',
    'บ้านปักหมู',
    'บ้านนาเพียงน้อย',
    'ยูเนสโกสัมมนา',
    'ชุมชนบ้านชุมช้าง',
    'บ้านบัว',
    'บ้านโนนหนามแท่ง',
    'บ้านโพนทัน',
    'ทุ่งธาตุท่าลี่ดอนกลางสามัคคี',
    'บ้านเซิม',
    'บ้านท่าหนองพันทา',
    'บ้านโคกกลาง',
    'บ้านคำตอยูง',
    'บ้านหนองแคน-ดอนสนุก',
    'บ้านหนองแหวน',
    'บ้านร่องโนร่องแกมคำบอน',
    'บ้านทุ่งหลวง',
    'บ้านนาตาลเหนือ',
    'บ้านแป้น',
    'บ้านกุดแกลบ',
    'บ้านหนองหอย',
    'บ้านก่องขันธ์',
    'บ้านดงสระพัง',
    'บ้านต้อนเหนือ',
    'บ้านนาหนัง',
    'บ้านโคกกลาง สาขา คำอินแปลง',
    'บ้านหนองแอก',
    'บ้านผือ',
    'บ้านคำปะกั้ง สาขาบ้านดงอ่าง',
    'บ้านคำจำปา',
    'บ้านคำเจริญ',
    'บ้านโพธิ์',
    'บ้านคำปะกั้ง',
    'บ้านปัก',
    'บ้านดงกำพี้',
    'ชุมชนบ้านปากสวย',
    'บ้านวัดหลวง',
    'บ้านโคกถ่อนวิทยา',
    'บ้านสร้างนางขาว',
    'บ้านเหล่าโพธิ์ศรี',
    'บ้านหาดสั่ง',
    'บ้านหนองอั้ว',
    'บ้านเหล่าต่างคำ',
    'บ้านเชียงอาด',
    'บ้านนาเมย',
    'บ้านโคกหัวภู',
    'บ้านห้วยน้ำเย็น',
    'บ้านคำแก้ว',
    'บ้านหนองโดดอนเสียด',
    'บ้านโนนสะแบง',
    'บ้านนางวงสุ่มห้วยเนียม',
    'บ้านคำเตาะเลาะโนนเค็ง',
    'บ้านคำแวง',
    'บ้านห้วยทราย',
    'บ้านท่าสวาท',
    'บ้านโคกสะอาด',
    'อนุบาลโซ่พิสัย',
    'บ้านหนองตะไก้โนนมันปลา',
    'บ้านไทรงามโนนภูดิน',
    'สังวาลย์วิทย์ 1',
    'บ้านถ้ำเจริญ',
    'บ้านศรีนาวา',
    'บ้านโคกกระแช',
    'บ้านโนนสวาง',
    'บ้านหนองแวง',
    'ชุมชนบ้านตูม',
    'บ้านนาเหว่อโนนอุดม',
    'บ้านท่าคำรวม',
    'บ้านสามหนอง',
    'บ้านโคกกลาง สาขาสามแยกศรีชมภู',
    'บ้านหนองแวงประชาสรรค์',
    'บ้านคำไผ่',
    'บ้านโคกกลาง',
    'บ้านแสงอรุณ',
    'บ้านศรีชมภู',
    'บ้านนาขาม',
    'บ้านโนนประเสริฐ',
    'บ้านห้วยลึก',
    'บ้านโนนแก้วโพนทอง',
    'บ้านป่าไร่โนนม่วง',
    'บ้านหนองพันทา',
    'บ้านเหล่าทองหลาง',
    'บ้านคลองทิพย์',
    'บ้านซำบอน',
    'บ้านต้าย',
    'บ้านนากั้ง',
    'บ้านหนองบัว',
    'บ้านนาดง',
    'บ้านโสกบง',
    'บ้านไร่',
    'บ้านดาลบังบด',
    'บ้านโนนเสถียร',
    'บ้านพรสวรรค์',
    'อนุบาลปากคาด สาขาโนนชัยศรี',
    'บ้านห้วยก้านเหลือง',
    'อนุบาลปากคาด',
    'บ้านห้วยไม้ซอด',
    'สมาคมสื่อมวลชนกีฬาสมัครเล่นและดารา',
    'ชุมชนสมสนุก',
    'บ้านศรีสว่างพัฒนา',
    'บ้านบะยาวราษฎร์นุเคราะห์',
    'สิริภัทรวิทยา',
    'ชุมชนบ้านหนองยองวิทยา',
    'บ้านโนนยาง-ดอนเม็ก',
    'บ้านนาดี',
    'เฝ้าไร่วิทยา',
    'บ้านจับไม้',
    'รสลินคัคณางค์',
    'บ้านนาฮำ',
    'บ้านหนองยาง',
    'อนุบาลหนองควาย',
    'ประชาบำรุง',
    'บ้านโพนทอง',
    'บ้านท่าคำบง',
    'บ้านหนองวัวชุม',
    'บ้านกุดแคนโนนมันปลา',
    'บ้านหนองจอกโนนสมบูรณ์',
    'บ้านหนองบัวเงิน',
    'ป่าไม้อุทิศ 8',
    'บ้านหนองหลวง',
    'บ้านแบง',
    'เวทีราษฎร์บำรุง',
    'บ้านโนนสะอาด',
    'บ้านคำโคนสว่าง',
    'สุทธสิริโสภา',
    'บ้านนาทับไฮ',
    'บ้านนาคำมูลชมภูพร',
    'บ้านนาคำมูลชมภูพร สาขา ชมภูพร',
    'รุจีจินตกานนท์',
    'บ้านหนองเค็ม',
    'บ้านพระบาทนาหงส์ สาขา โนนภูทอง',
    'บ้านดอนเหมือด',
    'บ้านต้อน',
    'บ้านพระบาทนาหงส์',
    'บ้านนาสิงห์',
    'บ้านนายาง',
    'บ้านสามัคคีชัย',
    'บ้านโปร่งสำราญ',
    'เตชะไพบูลย์ 1',
    'บ้านดงดาล',
    'อนุบาลนิคมเปงจาน',
    'บ้านหนองคอน',
    'บ้านโพนแพง',
    'สนธิราษฎร์บำรุง',
    'บ้านหนองแก้ว',
    'บ้านน้ำเป',
    'บ้านตาลชุม',
    'นิคมเจริญชัย',
    'บ้านนาชุมช้าง',
    'บ้านโนนสวรรค์',
    'ชุมพลโพนพิสัย',
    'เซิมพิทยาคม',
    'ปากสวยพิทยาคม',
    'นาหนังพัฒนศึกษา',
    'ร่มธรรมานุสรณ์',
    'กุดบงพิทยาคาร',
    'ราชประชานุเคราะห์ ๒๗ จังหวัดหนองคาย',
    'โซ่พิสัยพิทยาคม',
    'ศรีชมภูวิทยา',
    'โนนคำพิทยาคม',
    'โพนทองประชาสรรค์',
    'ปากคาดพิทยาคม',
    'หนองยองพิทยาคม รัชมังคลาภิเษก',
    'สมสนุกพิทยาคม',
    'วังหลวงพิทยาสรรพ์',
    'นาดีพิทยาคม',
    'ประชาบดีพิทยาคม',
    'พระบาทนาสิงห์พิทยาคม',
    'เลิดสิน',
    'บ้านชุมภูทอง',
    'บ้านนาเจริญวิทยา',
    'บ้านดอนแก้ว',
    'บ้านโคกก่องมิตรภาพที่ 86',
    'บ้านโนนสา',
    'บ้านนาคำ',
    'บ้านห้วยดอกไม้',
    'บ้านท่าอินทร์แปลง',
    'บ้านไร่สุขสันต์',
    'บ้านโนนวังเยี่ยม',
    'บ้านห้วยเซือมเหนือ',
    'บ้านไคสี',
    'บ้านคำหมื่น"ต.ช.ด.อนุสรณ์"',
    'แก่งอาฮง',
    'บ้านนาต้อง',
    'ประสานมิตรวิทยา',
    'บ้านคำภู',
    'บ้านหนองยาว',
    'บ้านห้วยเชือมใต้',
    'บ้านหนองตอ',
    'บ้านนาสวรรค์',
    'บ้านโคกสวาง',
    'บ้านโนนจำปา',
    'ชุมชนบ้านโนนสมบูรณ์',
    'บ้านทองสาย',
    'บ้านกำแพงเพชร',
    'บ้านบึงกาฬ',
    'บ้านท่าไคร้',
    'บ้านท่าโพธิ์',
    'บ้านแสนสำราญ',
    'บ้านโป่งเปือย',
    'บ้านห้วยสามยอดเทวกุล',
    'บ้านเหล่าถาวร',
    'บ้านหนองนาแซง',
    'อนุบาลวิศิษฐ์อำนวยศิลป์',
    'บ้านนาป่าน',
    'บ้านพันลำ "เจริญวิทยา"',
    'บ้านโนนสว่าง',
    'บ้านดอนปอ',
    'บ้านหนองหมู',
    'บ้านนาเจริญ',
    'บ้านคลองเค็ม',
    'บ้านสรรเสริญ',
    'บ้านสุขสำราญ',
    'หอคำพิทยาสรรค์',
    'ธเนตรวิทยา',
    'บ้านสะง้อ',
    'บ้านหนองเข็ง',
    'บ้านโนนยาง',
    'บ้านโคกสะอาด',
    'บ้านดอนหญ้านาง',
    'บ้านโคกสว่าง',
    'สันติสุขเจริญราษฎร์',
    'บ้านปรารถนาดี',
    'บ้านใหม่ศรีชมภู',
    'ชุมชนบ้านโคกอุดม',
    'อนุบาลพรเจริญ',
    'บ้านโคกนิยม',
    'บ้านวังยาว',
    'บ้านสร้างคำ',
    'บ้านดงเสียด',
    'บ้านโนน',
    'บ้านหนองผักแว่น',
    'บ้านหนองบัวน้อย',
    'บ้านนาคำนาใน',
    'บ้านเหล่าใหญ่',
    'บ้านโคกหนองลาด',
    'บ้านท่าศรีชมชื่น',
    'บ้านโนนสวรรค์',
    'บ้านนาซาว',
    'บ้านหนองหัวช้าง',
    'ชุมชนบ้านซาง',
    'บ้านท่าสำราญ',
    'บ้านซ่อมกอก',
    'บ้านโคกบริการสันกำแพง',
    'บ้านห้วยผักขะ สาขา ไทยเสรี',
    'บ้านห้วยผักขะ',
    'บ้านนางัวสายปัญญา',
    'บ้านโนนเหมือดแอ่',
    'บ้านห้วยเรือ',
    'บ้านทรัพย์วังทอง',
    'บ้านห้วยคอม',
    'อนุบาลเซกา',
    'บ้านโนนสูงสุขสมบูรณ์',
    'บ้านท่ากกแดง',
    'ชุมชนบ้านหนองหิ้ง',
    'บ้านหนองบัวแดง',
    'บ้านโพธิ์น้อย',
    'บ้านท่ากกต้อง',
    'บ้านใหม่สามัคคี',
    'บ้านหัวแฮต',
    'บ้านตาลเดี่ยว',
    'บ้านยางชุม',
    'บ้านท่าสะอาด',
    'บ้านคำบอน',
    'บ้านดงกะพุงหนองนาแซง',
    'บ้านเหล่าคาม',
    'บ้านทุ่งทรายจก',
    'บ้านดอนเสียด',
    'บ้านศิริพัฒน์',
    'บ้านต้องโคกกระแซ สาขาคำชมภู',
    'บ้านต้องโคกกระแซ',
    'บ้านโนนยางคำ',
    'บ้านท่าเชียงเครือ',
    'บ้านหนองชัยวาน',
    'บ้านท่าสวรรค์',
    'บ้านหนองแก่งทราย',
    'ท่าไร่วิทยา',
    'บ้านป่งไฮราษฎร์สามัคคี',
    'บ้านโสกก่ามนาตาไก้',
    'บ้านดงสว่าง',
    'บ้านบ่อพนา',
    'บ้านโนนสำราญ-ยางเรียน',
    'หนองทุ่มวิทยา',
    'บ้านโคกก่อง',
    'บ้านเหล่าหนองยาง',
    'บ้านกุดสิม',
    'บ้านหนองจิก',
    'บ้านดงโทน',
    'บ้านดงบัง',
    'บ้านท่าสีไค',
    'บ้านหนองฮูฝอยลม',
    'บ้านท่าไร่ไทยเจริญ',
    'บ้านท่าดอกคำ',
    'บ้านเหล่าหมากผาง',
    'โนนสาสวรรค์',
    'หนองแสงประชาสรรค์',
    'บึงของหลงวิทยา',
    'บ้านโสกโพธิ์โสกพอก',
    'บ้านนาขาม',
    'บ้านคำสมบูรณ์',
    'บ้านบัวโคก',
    'บ้านดงชมภู',
    'อนุบาลสาสนสิทธิ์อำนวย',
    'บ้านนาอ่าง',
    'หนองสิมโนนสวรรค์',
    'บ้านชุมภูพรโสกก่าม',
    'บ้านโคกสำราญพรสวรรค์',
    'บ้านห้วยลึก',
    'บ้านโนนม่วง',
    'บ้านนาสะแบง',
    'บ้านหนองบัวงาม',
    'บ้านหนองจิก',
    'บ้านนาทราย',
    'บ้านนาสิงห์',
    'บ้านนาแสง',
    'บ้านนาคำแคน',
    'กัลยาณิวัฒนา 2',
    'บ้านคำไชยวาล',
    'บ้านนาสิงห์ สาขา อู่คำ',
    'บ้านหนองจันทน์',
    'อนุบาลศรีวิไล',
    'บ้านหาดแฮ่',
    'ประวิตร (หนองบ่อ)',
    'ไทยรัฐวิทยา 51',
    'บ้านท่าส้มโฮง',
    'อนุบาลบุ่งคล้า',
    'บ้านขามเปี้ย',
    'บ้านนาจาน',
    'บ้านดอนแพง',
    'บ้านห้วยเล็บมือ',
    'บ้านหนองเดิ่นทุ่ง',
    'บ้านหนองเดิ่นท่า',
    'บึงกาฬ',
    'หนองเข็งวิทยาคม',
    'โพธิ์ทองวิทยานุสรณ์',
    'นาสวรรค์พิทยาคม',
    'พรเจริญวิทยา',
    'หนองหัวช้างวิทยา',
    'ศรีสำราญวิทยาคม',
    'เซกา',
    'หนองหิ้งพิทยา',
    'โสกก่ามวิทยา',
    'เหล่าคามพิทยาคม รัชมังคลาภิเษก',
    'ประชานิมิตพิทยานุกูล',
    'เจ็ดสีวิทยาคาร',
    'บึงโขงหลงวิทยาคม',
    'ท่าดอกคำวิทยาคม',
    'ศรีวิไลวิทยา',
    'ภูทอกวิทยา',
    'บุ่งคล้านคร',
    'บ้านเกิ้ง',
    'บ้านท่าปะทายโนนตูม',
    'บ้านโขงกุดหวาย',
    'ชุมชนบ้านหนองจิกท่าแร่',
    'วันครู 2502',
    'บ้านดอนตูมดอนโด',
    'บ้านเม่นใหญ่',
    'บ้านหนองใหญ่',
    'บ้านเขวา "รัฐประชาวิทยากร"',
    'บ้านหันเชียงเหียน',
    'บ้านเอียด "สังฆราษฎร์รังสรรค์"',
    'บ้านติ้วสันติสุข',
    'บ้านหนองตื่น',
    'บ้านหมี่เหล่าน้อย',
    'บ้านสงเปลือยโพธิ์สระแก้ว',
    'บ้านดอนดู่',
    'นิคมบ้านหัน',
    'บ้านสมศรี',
    'บ้านหนองโจดสวนมอน',
    'บ้านหัวช้างหนองแสง',
    'บ้านโคกก่อ',
    'บ้านภูดิน',
    'บ้านหนองหิน',
    'บ้านดอนหว่านหัวหนอง',
    'บ้านหนองหล่ม',
    'บ้านเหล่าหนาด',
    'หลักเมืองมหาสารคาม',
    'อนุบาลมหาสารคาม',
    'บ้านท่าตูมดอนเรือ',
    'บ้านหนองข่าหนองคูโปโล',
    'บ้านหนองโดนอ้อยช้าง',
    'บ้านอุปราช',
    'บ้านหนองกุงเต่า',
    'บ้านบ่อน้อยหนองงัวสว่างวิทย์',
    'บ้านท่าสองคอน',
    'บ้านดงเค็งดอนหัน',
    'บ้านหนองเขื่อนช้าง',
    'บ้านหินลาด',
    'บ้านหนองค้อสวนอ้อย',
    'บ้านหนองบัวโนนมี้',
    'บ้านหนองคูณน้ำจั้น',
    'บ้านโคกบัวค้อ',
    'ชุมชนบ้านลาด',
    'บ้านกุดซุย',
    'บ้านเลิงบ่อ',
    'บ้านหนองหวาย',
    'บ้านวังไผ่ป่าจั่น',
    'บ้านม่วงโพธิ์ศรี',
    'บ้านบุ่งคล้าท่างาม',
    'บ้านร่วมใจ 1',
    'บ้านหนองแวง',
    'บ้านดงน้อย',
    'บ้านหนองโพด',
    'บ้านเก่าน้อย',
    'บ้านนานกเขียน',
    'บ้านกุดแคน',
    'บ้านหนองโนอีดำ',
    'บ้านหนองคู',
    'บ้านโคกศรี',
    'บ้านหนองปลิง',
    'บ้านจำนัก',
    'บ้านนาแพงดอนไฮ',
    'บ้านโคกล่ามหมากหญ้าท่าสำราญ',
    'บ้านโดท่างาม',
    'อนุบาลแกดำ',
    'บ้านเหล่าจั่นนาภู',
    'บ้านหนองเจริญ',
    'บ้านหัวขัวโพธิ์ศรี',
    'ชุมชนบ้านหมากค่า',
    'บ้านโสกแดง',
    'บ้านโนนศรีภิบาล',
    'บ้านโคกลิ่น',
    'บ้านนาคูณ',
    'บ้านเสือกินวัวโคกน้อย',
    'ชุมชนมิตรภาพ',
    'บ้านป่าข่างโนนลาน',
    'บ้านหนองขุ่น',
    'บ้านหนองบัว',
    'บ้านดอนก่อกกเหลี่ยม',
    'บ้านขอนแก่นตะคุ',
    'บ้านวังแสง',
    'บ้านโคกกลาง',
    'บ้านโพนสวาง',
    'บ้านโพนละออม',
    'บ้านโคกไร่',
    'บ้านแก้งขิงแคง',
    'บ้านหนองกุงวันดีประชาสรรค์',
    'บ้านหัวขัว',
    'บ้านหนองสระพังโนนสะอาด',
    'บ้านโนนราษีโคกล่าม',
    'บ้านวังโพน',
    'บ้านเหล่าพ่อหา',
    'บ้านโคกกลาง',
    'บ้านหนองเขื่อน',
    'บ้านวังกุง',
    'บ้านเขื่อน (เขื่อนศึกษาคาร)',
    'บ้านโนนเนาว์',
    'บ้านวังจาน',
    'บ้านดอนกลางนุกูลวิทย์',
    'บ้านทิพโสต',
    'บ้านแพง',
    'บ้านม่วงน้อย',
    'บ้านหนองโก',
    'บ้านโนนเมืองรุ่งเรืองวิทย์',
    'บ้านดอนจำปาดอนสวรรค์',
    'บ้านม่วงใหญ่ดอนน้อยวิทยา',
    'ชุมชนโพนงามโพนสวาง',
    'บ้านโนนเมืองประชาสรรค์',
    'บ้านหนองบัวเรียน',
    'บ้านปลาเดิดปลาปัดสำโรง',
    'บ้านสว่างยางท่าแจ้ง',
    'บ้านผักหนอก',
    'บ้านโนนนกหอ',
    'บ้านยางน้อย',
    'บ้านป่าเป้า',
    'บ้านยางใหญ่',
    'บ้านเลิงบัว',
    'บ้านเลิงใต้',
    'บ้านหนองหอย',
    'บ้านน้ำจ้อย',
    'บ้านบะหลวงหนองแวง',
    'บ้านวังยาววิทยายน',
    'บ้านหินแห่เสริมศิลป์',
    'บ้านหญ้าขาว',
    'บ้านเหล่าโพธิ์',
    'บ้านหนองกุงวิทยากร',
    'บ้านแห่เหนือ',
    'บ้านท่าเดื่อ',
    'บ้านหนองบอนหัวหนองเหล่ายาว',
    'บ้านโนนสัง',
    'บ้านหนองปลาเข็ง',
    'บ้านหินแห่โนนเมืองน้อย',
    'บ้านคุยแพง',
    'บ้านคุยเชือก',
    'บ้านกอกหนองผือ',
    'บ้านโนนสูงวังขอนจิก',
    'บ้านนาล้อมโคกสว่าง',
    'บ้านหมากมายโพธิ์ทอง',
    'บ้านหนองเหล็ก',
    'บ้านหนองแวงสวนกล้วย',
    'บ้านโนนสะอาดหนองหญ้าม้าโคกสี',
    'บ้านทันดู่เหนือ',
    'บ้านคุยโพธิ์',
    'บ้านแก่งโกสุมท่างาม',
    'ศรีโกสุมวิทยามิตรภาพที่ 209',
    'บ้านสังข์ทอง',
    'บ้านดอนกลอยหนองยาง',
    'บ้านโชคชัย',
    'บ้านศรีสุข',
    'บ้านเขวา',
    'บ้านแท่นโนนหนองคู',
    'บ้านยางสินไชย หนองหาด',
    'บ้านเหล่าหนองแคน',
    'บ้านห้วยม่วง',
    'บ้านแห่บริหารวิทย์',
    'บ้านดอนสันติ',
    'บ้านโพนทอง',
    'บ้านโนนคัดเค้าคุยกอก',
    'บ้านป่าปอ',
    'บ้านน้ำใสม่วงวิทยา',
    'บ้านลิ้นฟ้า',
    'บ้านขี้เหล็กดงเค็งวิทยา',
    'บ้านไส้จ่อ',
    'บ้านขามเฒ่า',
    'บ้านเปลือยดง',
    'บ้านยางสามัคคี',
    'บ้านวังบัวสามัคคีวิทยา',
    'บ้านปอแดง',
    'บ้านดอนหน่อง',
    'บ้านขามเรียง',
    'บ้านมะกอก',
    'บ้านหนองขามหัวหนองสามัคคี',
    'บ้านดอนนา',
    'บ้านเขวาใหญ่',
    'บ้านขีศรีสง่าโนนเสียว',
    'บ้านเชือกคุยเพ็ก',
    'บ้านขี้เหล็กบุ่งเบาสามัคคี',
    'บ้านโพธิ์เงิน',
    'บ้านส้มโฮง',
    'บ้านดอนดู่',
    'บ้านคันธาร์',
    'บ้านโพนงาม',
    'บ้านสมศรีมะแปบประชาบำรุง',
    'อนุบาลกันทรวิชัย',
    'บ้านคอกม้า',
    'บ้านเหล่า',
    'บ้านดอนเวียงจันทน์',
    'บ้านใคร่นุ่น',
    'บ้านท่าขอนยาง',
    'บ้านหัวขัว',
    'บ้านนาสีนวน',
    'บ้านหนองอุ่ม',
    'บ้านเขวาโดนแหย่งสามัคคี',
    'บ้านหวาย',
    'บ้านตำแยหนองคู',
    'บ้านแสนสุข',
    'บ้านโพธิ์มีหนองเม็กโนนสะอาด',
    'บ้านทัน',
    'บ้านใคร่นุ่นราษฏร์บำรุง',
    'ชุมชนบ้านมะค่า',
    'บ้านไผ่น้ำเที่ยง',
    'บ้านโนนตาลกุดเวียนหนองหญ้าม้า',
    'บ้านเปลือยน้ำสามัคคี',
    'บ้านยางห่างโนนทัน',
    'บ้านโขงใหญ่',
    'บ้านน้ำเที่ยงดอนอิจันทร์',
    'บ้านหนองหว้าหนองแคนดอนบาก',
    'บ้านนาดีศรีสุข',
    'บ้านอิตื้อดอนหวายขมิ้น',
    'ชุมชนบ้านลาด',
    'บ้านขามเปี้ย',
    'บ้านกู่ทอง',
    'บ้านเมืองเพ็ง',
    'บ้านโจดบัวบาน',
    'บ้านหนองมันปลา',
    'บ้านหนองชาด',
    'บ้านหนองบุญชู',
    'บ้านโนนงิ้ว',
    'ศิริราษฎร์หมากหญ้า',
    'บ้านหนองมะเม้า',
    'บ้านหนองแวง',
    'บ้านเชียงยืน',
    'บ้านสร้างแก้ว',
    'บ้านโคกสูง',
    'บ้านแฝกโนนสำราญ',
    'บ้านค้อ',
    'บ้านสะอาดดอนเงิน',
    'บ้านผำ',
    'จินดาอารมณ์',
    'บ้านแบก (แบกสมบูรณ์วิทย์)',
    'บ้านทัพม้าดอนหันแวงวิทยา',
    'บ้านนาทอง',
    'บ้านหนองซอน',
    'บ้านสีดาสระแก้ว',
    'บ้านหนองไห',
    'บ้านโพน',
    'บ้านโนนศรีสวัสดิ์โนนสวรรค์',
    'บ้านเสือเฒ่าหนองเรือวิทยา',
    'บ้านจานโนนสูง',
    'บ้านท่ากระเสริม',
    'บ้านโคกข่า',
    'บ้านเข็งมิตรภาพที่ 139',
    'บ้านหนองคู (หนองคูวิทยาคาร)',
    'บ้านขีหนองจิก',
    'ไทยรัฐวิทยา 17 (บ้านมะโม)',
    'บ้านหนองเดิ่น',
    'บ้านเหล่าหนองบัวดอนโพธิ์',
    'บ้านหนองล่าม',
    'บ้านกุดรัง',
    'บ้านกุดเม็ก',
    'บ้านหัวช้าง',
    'บ้านหนองคลองหัวขัว',
    'บ้านหนองแหน',
    'บ้านไพศาล (ลี้ลอยอุทิศ)',
    'บ้านนาโพธิ์',
    'บ้านหนองโดน',
    'บ้านโนนสมบูรณ์',
    'บ้านหัวนาคำโนนสมบัติ',
    'บ้านโนนสะอาด',
    'บ้านเลิงแฝกบัวแก้ว',
    'บ้านบ่อแก',
    'บ้านโสกคลอง',
    'บ้านหนองบอน',
    'บ้านโสกกาว',
    'บ้านศรีอรุณ',
    'บ้านห้วยแคนโนนสูง',
    'บ้านหนองแวงสหคามวิทย์',
    'บ้านจอมทอง',
    'บ้านหนองแคน',
    'บ้านสำโรงหัวนาโนนจันทร์หอม',
    'บ้านหนองแสง',
    'ชุมชนบ้านกุดปลาดุก',
    'บ้านโนนทัน',
    'ดอนสวรรค์หนองกุงใต้วิทยา',
    'บ้านหนองนาไร่เดียว',
    'บ้านโคกกลางน้ำจั้นจอมศรี',
    'อนุบาลชื่นชม',
    'บ้านผือ',
    'บ้านหนองกุง',
    'บ้านหลุบแซง',
    'บ้านกุดจอก',
    'บ้านกระบาก (กระบากวิทยาคาร)',
    'บ้านผักแว่น',
    'บ้านส้มกบ',
    'สารคามพิทยาคม',
    'ผดุงนารี',
    'มหาวิชานุกูล',
    'โคกก่อพิทยาคม',
    'มหาชัยพิทยาคาร',
    'เกิ้งวิทยานุกูล',
    'แกดำวิทยาคาร',
    'มิตรภาพ',
    'โกสุมวิทยาสรรค์',
    'เขวาไร่ศึกษา',
    'เขื่อนพิทยาสรรค์',
    'โพนงามพิทยานุกูล',
    'วังยาวศึกษาวิทย์',
    'หนองเหล็กศึกษา',
    'หนองบัวปิยนิมิตร',
    'เขวาใหญ่พิทยาสรรค์',
    'มะค่าพิทยาคม',
    'นาสีนวนพิทยาสรรค์',
    'ท่าขอนยางพิทยาคม',
    'ศรีสุขพิทยาคม',
    'กันทรวิชัย',
    'เชียงยืนพิทยาคม',
    'กู่ทองพิทยาคม',
    'ดอนเงินพิทยาคาร',
    'กุดรังประชาสรรค์',
    'นาโพธิ์พิทยาสรรพ์',
    'เลิงแฝกประชาบำรุง',
    'ชื่นชมพิทยาคาร',
    'บ้านกำพี้เหล่ากาเจริญศิลป์',
    'บ้านสองห้องฮ่องน้อย',
    'บ้านเหล่ายาว',
    'บ้านเป้า',
    'บ้านแดงน้อย',
    'บ้านฮ่องไผ่',
    'บ้านหนองขามแสบง',
    'บ้านดอนงัว',
    'บ้านเหล่าตามา',
    'บ้านโคกกุง',
    'บ้านหนองทุ่มศรีโพธิ์ทอง',
    'บ้านป่ากุงหนา',
    'บ้านศาลา',
    'บ้านโนนทองโนนสะอาด',
    'บ้านดอนพยอม',
    'บ้านโนนแดงมะขามหวาน',
    'บ้านหัวหนอง',
    'บ้านหัวขัว',
    'บ้านห้วยทราย',
    'บ้านหนองบัวโนนสว่าง',
    'บ้านโนนสวรรค์',
    'บ้านโคกกลางบ่อหลุบ',
    'บ้านแก่นเท่า',
    'บ้านโนนราษีฝางวิทยา',
    'บ้านเหล่าผักหวาน',
    'บ้านบรบือ',
    'อนุบาลบรบือ',
    'บ้านโคกกลาง',
    'วัดนาดีวราราม',
    'บ้านโคกล่าม',
    'บ้านเหล่าโง้ง',
    'บ้านบ่อใหญ่',
    'บ้านหนองหว้าโนนทอง',
    'บ้านเก่าใหม่',
    'บ้านหนองหญ้าปล้อง',
    'ชุมชนบ้านดงเค็ง',
    'บ้านบัวมาศ',
    'บ้านหนองแวงหนองตุ',
    'บ้านยาง',
    'บ้านบกพร้าว',
    'บ้านแดง',
    'บ้านหัวหนอง (สังฆวิทยา)',
    'บ้านดอนก่อ',
    'บ้านโนนเกษตร',
    'บ้านโนนสำราญ',
    'บ้านวังไฮ',
    'บ้านโนนทอง',
    'บ้านวังปลาโด',
    'บ้านเปลือยหนองตูบ',
    'บ้านโคกกลาง',
    'บ้านหนองโก',
    'บ้านหนองคูขาด',
    'บ้านฝายป่าบัว',
    'บ้านโสกภารา',
    'บ้านไพรวัลย์ปอแดง',
    'บ้านหนองจิก',
    'บ้านดอนบม',
    'บ้านหนองแก',
    'บ้านซองแมว',
    'บ้านหัวนา',
    'บ้านม่วงขี้เหล็กยานาง',
    'บ้านโนนม่วง',
    'บ้านดงมัน',
    'บ้านหนองโดนหอกลอง',
    'บ้านพงโพด',
    'บ้านหนองสิม',
    'บ้านห้วยหลาว',
    'บ้านเขวาไร่',
    'บ้านหนองบัวแดง',
    'บ้านหนองแสง',
    'บ้านหัวหนองคู',
    'อนุบาลนาเชือกมหามงคล',
    'บ้านห้วยหิน',
    'บ้านหัวช้าง',
    'บ้านห้วยทราย',
    'บ้านหนองม่วง',
    'บ้านปอพานหนองโน',
    'บ้านหนองสระ',
    'บ้านเหล่าค้อ',
    'บ้านโนนแร่',
    'บ้านปลาขาว',
    'บ้านตำแยโนนยาง',
    'บ้านป่าตองหนองงู',
    'บ้านกระต่ายโนนสวัสดิ์',
    'บ้านโนนทอง',
    'บ้านขี้เหล็กขี้ตุ่นหลุบเปลือย',
    'บ้านดงม่วงหัวดงหนองบาก',
    'บ้านโพนทราย',
    'บ้านเหล่ากว้าง',
    'ชุมชนบ้านสำโรง',
    'บ้านชุมแสงโนนเหลี่ยม',
    'บ้านป่าแดง',
    'บ้านอีโต้',
    'บ้านหนองแดง',
    'บ้านหนองกุง',
    'บ้านวังหินโนนสว่าง',
    'บ้านหนองแดงสหมิตร',
    'บ้านโคกก่องโคกสูงหนองแต้',
    'บ้านเก่าใหญโนนสะอาด',
    'บ้านหนองโพธิ์',
    'บ้านคึมบง',
    'บ้านหนองบึง',
    'บ้านกุดน้ำใส',
    'บ้านหนองเม็ก',
    'บ้านหมากหม้อโนนเกษตร',
    'ชุมชนบ้านหนองเลา',
    'บ้านหนองแฮ',
    'บ้านหนองปอ',
    'บ้านตลาดม่วง',
    'บ้านหัวเข่าแตก',
    'บ้านนางเลิ้ง',
    'บ้านหัวสระ',
    'บ้านหัวหมู',
    'บ้านหนองสนมดอนติ้ว',
    'บ้านหนองผือ',
    'ชุมชนนาสีนวล',
    'บ้านเหล่าน้อย',
    'บ้านเตาบ่า',
    'บ้านน้ำสร้างหนองบะ',
    'อนุบาลพยัคฆภูมิพิสัย',
    'บ้านหนองห้าง',
    'บ้านดง',
    'บ้านเขวาทุ่ง',
    'บ้านมะชมโนนสง่า',
    'บ้านดงบากหนองตาเต็น',
    'บ้านหารฮี',
    'บ้านเม็กน้อยหนองไผ่',
    'บ้านสำโรงโคกจันทร์หอมวิทยา',
    'บ้านแก่นท้าว',
    'บ้านเม็กดำ',
    'บ้านเหล่า',
    'บ้านตาลอก',
    'บ้านเมืองเตา',
    'บ้านหนองฮี',
    'บ้านโนนบ่อ',
    'บ้านโนนจาน',
    'บ้านหนองกกหนองยาว',
    'บ้านเมืองเสือ',
    'บ้านหนองแคน',
    'บ้านทัพป่าจิก',
    'บ้านค่ายนุ่นโนนแคน',
    'บ้านดงเย็น',
    'บ้านโนนม่วงท่าพลับพลา',
    'บ้านสระบาก',
    'บ้านสระแคน',
    'พยัคฆภูมิพิสัย',
    'บ้านหนองแก',
    'บ้านมะโบ่',
    'บ้านจอมพะลาน',
    'บ้านหนองระเวียง',
    'บ้านโนนแคน',
    'บ้านดอนหมี',
    'บ้านหนองจานบุลาน',
    'บ้านนาค่าย',
    'บ้านขี้เหล็ก',
    'บ้านโคกสะอาด',
    'บ้านหนองนาใน',
    'บ้านดงหัวช้าง',
    'บ้านหนองบัวกาเหรียญ',
    'บ้านขามเรียน',
    'บ้านหนองหว้าเฒ่า',
    'บ้านโคกล่ามวิทยา',
    'บ้านโนนสูงดอนหลี่',
    'บ้านหนองบัวคูสองห้อง',
    'บ้านหนองบัวแก้ว',
    'บ้านหนองแสง',
    'บ้านเก่าน้อย',
    'บ้านขามป้อม',
    'บ้านหนองแวงบกไผ่ล้อมวิทยา',
    'บ้านโนนจาน',
    'บ้านโนน',
    'บ้านแคน',
    'บ้านตำแย(ประชานุเคราะห์)',
    'บ้านแวงชัยโคกหนองโจด',
    'บ้านโคกใหญ่วิทยา',
    'บ้านเขวาค้อโคกกลาง',
    'บ้านแวงเหล่าเก่าน้อย',
    'บ้านดู่',
    'บ้านโคกสีทองหลาง',
    'บ้านโคกแปะ',
    'บ้านแวงนคร (สังฆวิทยา)',
    'บ้านโคกไร่',
    'บ้านหนองหว้า',
    'ชุมชนบ้านงัวบา',
    'บ้านสระแก้วหนองคู',
    'บ้านดงใหญ่',
    'บ้านก่อ',
    'บ้านดอนหัน',
    'บ้านนาข่า',
    'บ้านโคกสูงหนองเสียวหนองขี',
    'บ้านโคกเต่า',
    'บ้านดอนแดงน้ำเกลี้ยง',
    'บ้านวังจานโนนสำราญ',
    'บ้านตลาดโนนโพธิ์',
    'บ้านหวาย',
    'บ้านมะแซวหนองโง้ง',
    'บ้านหนองไผ่',
    'บ้านหนองคลอง',
    'บ้านเปลือยดอนมันน้ำ',
    'บ้านหนองแต้',
    'บ้านประแหย่ง',
    'บ้านหนองเหล่า',
    'บ้านหนองข่า',
    'บ้านแก่นเท่า',
    'ราชประชานุเคราะห์ 18',
    'บ้านตำแย',
    'บ้านโพธิ์ชัย',
    'บ้านเสือโก้ก',
    'บ้านไก่นา',
    'บ้านสนาม',
    'บ้านหนองผือ',
    'บ้านดงน้อย',
    'บ้านหนองกุง',
    'บ้านโนนเขวาหนองแสง',
    'ชุมชนบ้านหนองทุ่ม',
    'บ้านหนองตาไก้เผือกใต้วิทยา',
    'บ้านกระยอมหนองเดิ่น',
    'ชุมชนบ้านโพธิ์สองห้องวิทยา',
    'ราชประชานุเคราะห์ 17',
    'บ้านโนนท่อน',
    'บ้านโสกยาง',
    'อนุบาลวาปีปทุม',
    'เมืองวาปีปทุม',
    'บ้านหนองคูไชยหนองขาม',
    'บ้านจอกขวาง',
    'บ้านหัวงัว',
    'บ้านหนองแสน',
    'บ้านดู่หนองโก',
    'บ้านกุดนาดีโนนลาน',
    'บ้านโคกใหญ่',
    'บ้านนาเลา',
    'บ้านหนองคูม่วง',
    'บ้านหนองไฮ',
    'บ้านหนองบัวกุดอ้อ',
    'ราชประชานุเคราะห์ 16',
    'บ้านนาฝายเหล่าหุ่ง',
    'บ้านชาดฝางหัวเรือ',
    'บ้านหนองแกวิทยา',
    'บ้านยางอิไลดอนก่อ',
    'บ้านสระบัว',
    'กู่สันตรัตน์',
    'บ้านโคกยาว',
    'บ้านแดงโพงคำแก้ว',
    'บ้านนาฝาย',
    'บ้านหนองบัวน้อย',
    'ชุมชนบ้านดงบัง',
    'บ้านหนองแคนหัวฝาย',
    'บ้านดงยาง',
    'บ้านหลุบควัน',
    'บ้านเครือซูด',
    'บ้านหนองผง',
    'อนุบาลนครจัมปาศรี',
    'บ้านหนองโนใต้',
    'บ้านหนองกลางโคก',
    'บ้านโกทา',
    'บ้านหนองจิก',
    'บ้านหนองโนทับม้า',
    'บ้านหนองแต้น้อย',
    'บ้านโพนทอง',
    'บ้านร่วมใจ 2',
    'บ้านเหล่าจั่น',
    'บ้านหนองป้าน',
    'บ้านหนองไผ่ด้ามขวาน',
    'บ้านหนองบัวคู',
    'บ้านโนนเห็ดไค',
    'บ้านหนองหิน',
    'บ้านโคกเพิ่มโคกกลาง',
    'บ้านหัวดง',
    'บ้านดอนดู่วังบอน',
    'จตุคามประชาสรรค์',
    'บ้านตาพวนสร้างแซ่ง',
    'อนุบาลดงเมืองน้อย',
    'บ้านสว่าง',
    'บ้านศาลา',
    'บ้านนาภู',
    'บ้านเหล่าหมากคำ',
    'บ้านหัวช้างโคกม่วง',
    'บ้านโนนรัง',
    'บ้านหนองจิก',
    'บ้านแวงยางกุดตะเข้',
    'บ้านหนองหน่อง',
    'ชุมชนบ้านยางสีสุราช',
    'บ้านหนองแปน',
    'บ้านแวงดงหนองยาง',
    'บ้านนกเหาะ',
    'บ้านบุ่งง้าว',
    'บ้านเปล่งโนนกระยอม',
    'บ้านโดน',
    'บ้านดงแคน',
    'บ้านหนองบัวสันตุ',
    'บ้านหนองแวง',
    'บ้านหนองรูแข้',
    'บรบือ',
    'เหล่ายาววิทยาคาร',
    'โนนแดงวิทยาคม',
    'หนองโกวิชาประสิทธิ์พิทยาคม',
    'ยางวิทยาคม',
    'โนนราษีวิทยา',
    'หนองม่วงวิทยาคาร',
    'บรบือวิทยาคาร',
    'นาเชือกพิทยาสรรค์',
    'ปอพานพิทยาคม รัชมังคลาภิเษก',
    'หนองโพธิ์วิทยาคม',
    'พยัคฆภูมิวิทยาคาร',
    'เมืองเตาวิทยาคม',
    'เวียงสะอาดพิทยาคม',
    'วาปีปทุม',
    'นาข่าวิทยาคม',
    'เสือโก้กวิทยาสรรค์',
    'ดงใหญ่วิทยาคม รัชมังคลาภิเษก',
    'งัวบาวิทยาคม',
    'ประชาพัฒนา',
    'หัวเรือพิทยาคม',
    'ขามป้อมวิทยาคม',
    'นาดูนประชาสรรพ์',
    'ดงบังพิสัยนวการนุสรณ์',
    'มัธยมดงยาง',
    'นาภูพิทยาคม',
    'มัธยมยางสีสุราช',
    'บ้านขอนแก่น(นิกรราษฎร์ศรัทธาคาร)',
    'บ้านป่าบากเหล่าเรือ(คุรุราษฎร์สามัคคี)',
    'บ้านแคนสามัคคี',
    'ธรรมจารีนิวาส',
    'บ้านหนองพานแยบัวห้าว',
    'บ้านหนองโทนหนองเขวา(คุรุราษฎร์สามัคคี)',
    'บ้านเปลือยน้อย(คุรุรัฐประชาสามัคคี)',
    'บ้านหนองจิกโคกสูง',
    'บ้านเหล่าใหญ่(คุรุศิษย์ประชานุสรณ์)',
    'บ้านดงลาน(คุรุรัฐประชาสรรค์)',
    'บ้านกอกนายูง(คุรุราษฎร์บำรุง)',
    'บ้านนาโพธิ์(สหพันธ์พิทยาภรณ์)',
    'บ้านหัวโนนตาล(สหราษฎร์วิทยาสรรค์)',
    'สหสามัคคีวิทยา',
    'บ้านป่ายาง',
    'บ้านหนองผือโพนศรี',
    'บ้านโนนสั้นหนองแคสามัคคี',
    'บ้านโนนรัง',
    'บ้านค้อ',
    'อนุบาลร้อยเอ็ด',
    'เมืองร้อยเอ็ด',
    'วัดโนนสำราญ(ประชาสามัคคี)',
    'บ้านสงเปลือย(ประชาราษฎร์บำรุง)',
    'บ้านเมืองทอง(วีระประชานุสรณ์)',
    'บ้านเปลือย(สามัคคีราษฎร์ประสิทธิ์)',
    'บ้านเปลือยนอก',
    'บ้านแก่นทราย(คุรุราษฎร์ผดุงวิทย์)',
    'บ้านรอบเมือง(คุรุรัฐประชานุสรณ์)',
    'บ้านโนนสั้นหนองแคสามัคคีสาขาบ้านเขวา',
    'ไตรคามสามัคคี(บ้านขี้เหล็ก)',
    'บ้านดงเค็ง(คุรุราษฎร์ประชาสามัคคี)',
    'บ้านสังข์สงยาง(คุรุราษฎร์วิทยา)',
    'บ้านแมตวิทยาคาร',
    'โสภโณประชาสรรค์',
    'บ้านดงยางโคกพิลา(ประชาอุปถัมภ์)',
    'บ้านโคกสง่า(มิตรภาพคุรุราษฎร์นุสรณ์)',
    'บ้านเหล่าขาม(มูลสารศึกษาสงเคราะห์)',
    'ชุมชนบ้านสีแก้ว',
    'บ้านปอภาร(ปอภารราษฎร์บำรุง)',
    'บ้านหนองตาไก้หนองตุ',
    'บ้านคานหักโนนใหญ่(ศิริเกตุประชาวิทย์)',
    'บ้านดงสวองหนองสูบ',
    'บ้านเปลือยสีแก้ว',
    'บ้านซ้งวิทยาคม',
    'บ้านเล้าวิทยาคาร',
    'บ้านหนองแวง',
    'บ้านแวงวิทยา',
    'บ้านหนองยูง',
    'บ้านขี้เหล็ก',
    'บ้านโนนแท่น',
    'บ้านหนองนาสร้าง(ศิริราษฎร์บูรณะ)',
    'บ้านหนองตากร้า(ศรีไพรไตรคามพิทยา)',
    'บ้านหนองผักแว่น',
    'บ้านแดงโนนสว่าง',
    'บ้านสามแยกโพธิ์ชัย',
    'บ้านโนนเมือง(เทอดเกียรติวีระชน)',
    'ชุมชนบ้านโคกล่าม',
    'บ้านหนองคูบอน',
    'บ้านหนองผือน้อย(สันติราษฎร์บำรุง)',
    'บ้านเหล่ายูง',
    'บ้านดงกลาง',
    'รัฐประชาวิทยาคาร',
    'บ้านหัวนาคำ',
    'บ้านดงแดงหนองเพียขันธ์',
    'บ้านเหล่าจั่นหนองทุ่ม',
    'บ้านงูเหลือม',
    'บ้านกอกวิทยาคาร',
    'พรหมยานุสรณ์',
    'บ้านดู่น้อย',
    'บ้านป่าแดง',
    'บ้านดงยาง',
    'บ้านหนองชาด',
    'บ้านป่าแดงสาขาบ้านหนองตอน้อย',
    'รัฐทวิคาม',
    'บ้านขุมดินสว่างประชานุกูล',
    'บ้านยางเครือ',
    'บ้านน้ำใสเที่ยมแข้',
    'ป่าสังข์ป่าม่วงวิทยา',
    'บ้านร่องคำ',
    'บ้านโคกมอน',
    'บ้านหนองบัวเลิง',
    'บ้านไม้ล่าว(คุรุราษฎร์พัฒนา)',
    'ชุมชนบ้านเมืองหงส์',
    'บ้านโคกกลาง',
    'รัตนประชานุสรณ์',
    'ลิ้นฟ้าวิทยาคาร',
    'บ้านโนนสำราญ',
    'บ้านป่าดวน',
    'บ้านหนองแกหนองโล',
    'บ้านอีโคตร(สามัคคีราษฎร์บำรุง)',
    'สหคามวิทยาคาร',
    'บ้านดอนแคน',
    'บ้านหนองหิน',
    'บ้านอุ่มจาน',
    'บ้านหนองแอก',
    'บ้านหนองแวงหนองหัวคน',
    'บ้านข่า',
    'บ้านหนองผือ',
    'ไตรมิตรวิทยา',
    'บ้านอ้น',
    'หนองตอวิทยา',
    'จตุรพักตรพิมาน',
    'ราชสารสุธีอนุสรณ์',
    'บ้านหนองบั่ว',
    'บ้านอีง่อง',
    'บ้านสวนมอญ',
    'บ้านค้อกุดจอก',
    'บ้านเขวาทุ่ง',
    'บ้านหนองช้าง',
    'บ้านหนองต่าย',
    'บ้านงิ้ว(คุรุปถัมภ์)',
    'บ้านธวัชบุรี',
    'บ้านหนองดู่',
    'บ้านหัวโนน',
    'บ้านดู่',
    'บ้านคางฮุง',
    'บ้านหนองบัวดอนไผ่',
    'บ้านนิเวศน์',
    'ชุมชนบ้านประตูชัย',
    'บ้านไผ่',
    'บ้านขาม',
    'บ้านสังข์',
    'บ้านสองพี่น้องวิทยาคาร',
    'ไพศาลวิทยาคม',
    'บ้านดู่ไพศาล',
    'บ้านเพ็กท่าบ่อวิทยา',
    'บ้านดงบ้านนาประชาสรรค์',
    'ชุมชนบ้านยางกู่',
    'บ้านมะอึ',
    'บ้านหนองเบิด',
    'บ้านเมืองน้อย',
    'บ้านมะยาง',
    'บ้านราชธานี',
    'บ้านอีหมุน',
    'บ้านหนองโสน',
    'บ้านหนองเต่า',
    'เวฬุวันวิทยา',
    'บ้านตาอุด',
    'บ้านหนองสาหร่าย',
    'บ้านชาดมะเหลื่อม',
    'บ้านไก่ป่า',
    'บ้านดอนชัย',
    'ขี้เหล็กเขียวไพรวัลย์วิทยา',
    'บ้านนาคำ',
    'บ้านฝั่งแดง',
    'บ้านตรีคาม',
    'บ้านป่าสุ่ม',
    'บ้านจันทร์สว่าง',
    'บ้านอุ่มเม้า',
    'บ้านบากหนองแดง',
    'บ้านเหล่าใหญ่(สมานราษฎร์ศรัทธาลัย)',
    'บ้านโพธิ์ทอง(ยุทธศิลป์สงเคราะห์)',
    'โพธิ์สัยสว่างวิทย์',
    'บ้านโพนทอง',
    'หนองแสงโนนสมบูรณ์',
    'บ้านโสกเชือก(คุรุราษฎร์อำนวย)',
    'อดุลวิหารกิจ(คุรุราษฎร์วิทยา)',
    'บ้านหนองสองห้อง(คุรุราษฎร์บำรุง)',
    'ชุมชนบ้านเหล่าล้อ(พิมพ์คุรุราษฎร์บำรุง)',
    'บ้านศรีสมเด็จ',
    'บ้านโคกข่าหนองโก',
    'บ้านป่าแหนหนองไร่',
    'บ้านกล้วยวิทยา',
    'บ้านเหล่ากุด',
    'บ้านสวนจิก',
    'บ้านหนองแวงยาว',
    'ไทยรัฐวิทยา 101 เฉลิมพระเกียรติ',
    'หนองคูโคกขุมดิน',
    'บ้านหนองใหญ่',
    'โนนสีดาวิทยา',
    'ทรายทองวิทยา',
    'เมืองจังหาร',
    'บ้านแคน',
    'บ้านเหล่างิ้ว(เหล่างิ้ววิทยานุกูล)',
    'บ้านแพง(จัตุคามสำราญวิทยา)',
    'บ้านดงสิงห์(ดำรงราษฎร์สามัคคี)',
    'บ้านเปลือยตาลคุรุวิทยาประชานุกูล',
    'บ้านหนองตอ(คุรุราษฎร์ศึกษาคาร)',
    'บ้านหนองเข็ง',
    'บ้านป่านหนองอ้อ',
    'ดินดำบัวรองวิทยา',
    'บ้านคุยค้อ(บัญฑิตประชาสรรค์)',
    'บ้านเลิงคา',
    'บ้านขมิ้นจานทุ่ง',
    'ดงยางสะแบง',
    'บ้านพยอม',
    'ดอนแคนดอนหวายสามัคคี',
    'บ้านม่วงน้ำ(ประชาศิษย์วิทยา)',
    'บ้านปาฝา(คุรุประชานุสรณ์)',
    'บ้านเหล่ากล้วย',
    'โนนเชียงบังหาดหนองแค',
    'บ้านผักแว่นอนามัย(คุรุศิษยานุกูล)',
    'บ้านดงเครือวัลย์',
    'บ้านคางฮุงบากไร่นุ่นโนนศึกษา',
    'บ้านแจ้ง(คุรุประชาสรรค์)',
    'บ้านม่วงท่าลาด',
    'หัวนางามวิทยา',
    'ไตรคามวิทยา',
    'แซงแหลมดินแดงโนนสวรรค์(ธรรมศาสตร์ร้อยเอ็ด)',
    'บ้านบึงโดนรัฐราษฎร์อนุสรณ์',
    'เขวาชีรัฐประชาสรรค์',
    'บ้านโนนข่า',
    'บ้านหวายหลึม(โพธิ์ศรีราษฎร์วิทยา)',
    'บ้านเขือง',
    'บ้านดอนพยอมน้อย',
    'บ้านพระเจ้า(ศรีมหาธาตุ)',
    'บ้านป่าสุ่มหนองปิงวิทยาคาร',
    'บ้านดงบัง(พิกุลศึกษาคาร)',
    'บ้านคุยชนวนไผ่อ้อม',
    'บ้านแมด',
    'บ้านดอนแดง',
    'บ้านบุ่งค้าแก่งข่า',
    'บ้านวังปากบุ่ง',
    'บ้านดอนวิเวก(วิเวกวิทยาคาร)',
    'บ้านดอนสำราญ',
    'วังยาวเจริญวิทย์',
    'บ้านหนองแก่ง',
    'บ้านเกษมสุข',
    'บ้านพลับพลา',
    'บ้านมีชัย',
    'บ้านหมูม้น',
    'บ้านธาตุประทับ',
    'ท่าโพธิ์ผักก้ามวิทยา',
    'บ้านหวายน้อย',
    'บ้านจาน',
    'บ้านหนาด',
    'บ้านยางใต้',
    'บ้านอีโก่ม',
    'บ้านเทอดไทย',
    'บึงงามพัฒนา',
    'ประชาราษฏร์รังสรรค์',
    'บ้านนางาม',
    'บ้านโนนราษี',
    'วังทองวิทยาคาร',
    'บ้านดอนแก้ว',
    'บ้านขว้างใหญ่',
    'บ้านขว้างท่าสะแบง',
    'บ้านหวายหลึม',
    'บ้านซองแมว',
    'บ้านบัวหลวง(สหราษฎร์บำรุง)',
    'บ้านห้วยสนุกสะพานทอง',
    'ธวัชบุรีวิทยาคม',
    'จตุรพักตรพิมานรัชดาภิเษก',
    'โคกล่ามพิทยาคม',
    'ธงธานี',
    'สตรีศึกษา',
    'ดู่น้อยประชาสรรค์',
    'พลาญชัยพิทยาคม',
    'ศรีธวัชวิทยาลัย',
    'ขัติยะวงษา',
    'สตรีศึกษา 2',
    'ศึกษาพิเศษร้อยเอ็ด',
    'ศรีสมเด็จพิมพ์พัฒนาวิทยา',
    'เฉลิมพระเกียรติสมเด็จพระศรีนครินทร์ร้อยเอ็ด',
    'ม่วงลาดวิทยาคาร',
    'จังหารฐิตวิริยาประชาสรรค์',
    'เชียงขัวญพิทยาคม',
    'พลับพลาวิทยาคม',
    'เทอดไทยวิทยาคม',
    'ร้อยเอ็ดวิทยาลัย',
    'เมืองน้อยวิทยาคม',
    'ศึกษาสงเคราะห์ธวัชบุรี',
    'บ้านหนองอ่างดอกรัก',
    'บ้านฮ่องทราย',
    'บ้านไทรทอง',
    'บ้านหนองพระบาง',
    'บ้านหนองสระหงส์',
    'บ้านกู่กาสิงห์',
    'จตุรคามพัฒนา',
    'บ้านสะแบง',
    'บ้านหนองส้าวโพนงอย',
    'บ้านโพนโพธิ์หว้างาม',
    'เมืองเกษตรวิสัย',
    'วัดธาตุ(ธรรมธาดากิจจานุสรณ์)',
    'บ้านป่ายาง',
    'บ้านหนองแวง',
    'บ้านดงครั่งน้อย',
    'ทุ่งกุลาประชารัฐ',
    'บ้านโพนแท่น',
    'บ้านแสนสี',
    'วัดแจ่มอารมณ์',
    'บ้านโพนเงินโพนทอง',
    'บ้านดงครั่งใหญ่',
    'บ้านนกเหาะ',
    'บ้านโพนทัน',
    'บ้านโพนหิน',
    'บ้านหนองไผ่ลุ่ม',
    'บ้านเขวาหรดี',
    'บ้านเขวาตะคลอง',
    'ชีโนวาทธำรง',
    'บ้านโนนจาน',
    'บ้านยางจ้อง',
    'บ้านน้ำอ้อม',
    'ไตรคามสามัคคี',
    'บ้านผำโนนสว่าง',
    'คุรุราษฎร์สามัคคี',
    'บ้านเขวาใหญ่',
    'บ้านหนองกุง',
    'บ้านหนองเปลือยตาแสง',
    'บ้านหัวดอนชาด',
    'บ้านตลาดไชย',
    'บ้านฝาง',
    'เมืองบัววิทยาคาร',
    'บ้านหัวดงกำแพง',
    'บ้านสำราญนิวาสสามัคคี',
    'วัดบ้านโพนเงิน',
    'บ้านขี้เหล็กซ้ายดอนแตง',
    'สิงห์โคกดงมันเหล่างามวิทยาคม',
    'บ้านจานทุ่งโนนสังข์',
    'บ้านหัวหนองตาด',
    'บ้านหัวหนองแวง(สามัคคีราษฎร์บำรุง)',
    'บ้านหนองช้าง',
    'บ้านเหนือ',
    'บ้านสิงห์ไคล',
    'บ้านอี่เม้ง',
    'ชุมชนบ้านหนองผึ้ง(ประพันธ์คุรุราษฎร์อุทิศ)',
    'บ้านโพนสะอาด',
    'บ้านหนองสังข์',
    'ชุมชนบ้านอุ่มเม่าสิงห์ไคล',
    'บ้านคุยผงดงน้อย',
    'บ้านดอนดู่เหล่าเสือ',
    'บ้านขี้เหล็ก',
    'บ้านพิลา',
    'บ้านนาแค',
    'บ้านโพนทอง',
    'บ้านขามป้อม',
    'บ้านม่วง',
    'บ้านค้อแสนสี',
    'บ้านตาจ่อยหนองสระ',
    'บ้านหนองคูโคกเพ็ก',
    'บ้านโพนงาม',
    'บ้านโนนสะอาด',
    'บ้านขนวน(คุรุประชาสรรค์)',
    'บ้านโนนจาน',
    'บ้านโนนสวรรค์',
    'บ้านดูน(บักตู้ โนนตาแสง)',
    'บ้านน้ำคำ',
    'บ้านหัวช้าง',
    'บ้านส้มโฮง',
    'บ้านฮ่องแฮ่',
    'บ้านหนองขาม',
    'บ้านดงช้าง',
    'บ้านบัวแดง(ปทุมรัตต์วิทยาคาร)',
    'บ้านจานเหนือ',
    'ชุมชนบ้านโคกทม(พิศิษฐ์พิทยาคาร)',
    'บ้านหนองบึง',
    'บ้านจานใต้',
    'บ้านโคกก่อง',
    'บ้านโพธิ์น้อย',
    'บ้านโพนสูง',
    'บ้านสระบัว',
    'บ้านหนองมะเขือ',
    'บ้านเขวาโคกสวายโดด',
    'บ้านเขวาทุ่ง',
    'บ้านภูดิน',
    'บ้านบัวขาว',
    'บ้านหนองหญ้ารังกา',
    'ชุมชนบ้านหนองแคน(นนทาภิบาล)',
    'บ้านดู่',
    'บ้านสวนปอ',
    'บ้านสำราญ',
    'บ้านโคกกลาง',
    'ประสิทธาราม',
    'บ้านดงมัน',
    'บ้านกุดน้ำใส(ผดุงวิทยาคาร)',
    'บ้านคูฟ้า',
    'ค้อใหญ่ชัยยุทธวิทยา',
    'บ้านคำไฮ(เจริญวิทยาคาร)',
    'บ้านนางเตี้ยไศลทอง',
    'บ้านโปง(ประชาสามัคคี)',
    'บ้านดอนคำ',
    'บ้านโนนศิลา',
    'โนนไทยวิทยาคาร',
    'ไตรราษฎร์วิทยา',
    'บ้านขุมเงิน(ราษฎร์สามัคคีวิทยา)',
    'บ้านคำแดง',
    'บ้านหนองฟ้า',
    'บ้านชานุวรรณ(ประชาวิทยาคาร)',
    'บ้านพนัสดงสว่าง',
    'บ้านชาติ',
    'บ้านโนนม่วง',
    'บ้านโนนจิกหนองคู',
    'บ้านคำพระ',
    'บ้านนานวลราษฎร์นิยม',
    'บ้านพนมไพร',
    'บ้านดอนเจริญ',
    'บ้านชะโดเหล่าบึงแก้ว',
    'ดอนเสาโฮง',
    'ปัญจคามประชาสรรค์',
    'บ้านหนองศรีทองสว่างวิทย์',
    'บ้านดอนบ่อดอนแฮด',
    'บ้านหนองเหล็ก',
    'บ้านบัวงาม',
    'บ้านโพธิ์ใหญ่',
    'ท่าโพธิ์ท่าไฮหนองแวงประชาสรรค์',
    'บ้านหนองกุงวิทยา',
    'บ้านโนนชัยศรีหนองแคน',
    'บ้านหัวหนองกุดหลด',
    'บ้านสมสะอาด',
    'บ้านวารีสวัสดิ์วิทยา',
    'บ้านดอนดู่',
    'บ้านสระแก้ว',
    'บ้านโนนทรายสระทอง',
    'บ้านโพธิ์น้อย',
    'บ้านนาชม(นาชมพิทยาสิทธิ์)',
    'บ้านหงษ์ทอง(แสนสุขวิทยา)',
    'บ้านดอนแดง',
    'บ้านท่าลาดวารีวิทยา',
    'บ้านท่าเสียว(กาญจนาอุปถัมภ์)',
    'ทุ่งหมื่นทุ่งแสนพิทยาคม',
    'บ้านโพธิ์งาม(สุขะวรรณะอุปถัมภ์)',
    'บ้านหัวนา',
    'บ้านหนองทัพไทย',
    'เมืองจำปาขัน',
    'บ่อพันขันวิทยา',
    'บ้านปลาค้าว',
    'บ้านหญ้าหน่อง',
    'บ้านหนองบั่ววิทยา',
    'บ้านแดง',
    'บ้านเปลือย(คุรุสรรค์วิทยา)',
    'บ้านค้อโนนสว่าง',
    'บ้านยางเลิง',
    'บ้านหมอตา',
    'บ้านดอกไม้',
    'บ้านโคก(ทองคุรุราษฎร์พัฒนา)',
    'บ้านภูงา',
    'บ้านจานเตย',
    'บ้านฮ่องสังข์',
    'บ้านสาหร่าย',
    'บ้านดอนแคน',
    'บ้านโนนตาด',
    'เมืองทุ่งวิทยา',
    'ทุ่งศรีเมืองวิทยา',
    'บ้านโพนขวาว',
    'หว่านท่าสมอวิทยา',
    'บ้านตาหยวก(ประชาสงเคราะห์)',
    'บ้านร้านหญ้า',
    'บ้านโพนเดื่อ',
    'บ้านสระโพนทอง',
    'บ้านคูดินทราย',
    'บ้านตาแหลวโนนหมากแงว',
    'บ้านเม็กหางเหย',
    'บ้านนา',
    'บ้านงูเหลือม',
    'ดงหัวเรือป่าม่วงวิทยา',
    'บ้านโพนยานาง',
    'บ้านเหว่อดงสวนผึ้ง(สุขนาคธรรม)',
    'หนองบัวหนองพังคีคุรุประชาสรรค์',
    'ไตรราษฎร์คุรุสามัคคี',
    'น้ำคำใหญ่วิทยา',
    'บ้านผักเผ็ดจ้อก้อ',
    'บ้านหนองยางหนองไผ่',
    'บ้านเปลือยน้อย',
    'บ้านเหล่าติ้ว',
    'บ้านมหิงษาราม',
    'ห้วยหินลาด',
    'บ้านยางเครือด่านน้อย',
    'เมืองอารัมย์',
    'บ้านสนาม',
    'บ้านดอนยาง',
    'บ้านโพนครกน้อย',
    'เมืองสุวรรณภูมิ',
    'ดอนแฮดวิทยา',
    'โสภาพพิทยาภรณ์',
    'บ้านแวง',
    'บ้านน้ำคำน้อยวิทยา',
    'กระดิ่งทอง',
    'บ้านหนองตอกแป้นหนองมั่งวิทยา',
    'เมืองใหม่สุวรรณภูมิ',
    'บ้านเหม้าวิทยา',
    'บ้านหนองแวง',
    'จตุคามวิทยา',
    'บ้านขี้เหล็ก',
    'บ้านหนองแล้ง',
    'บ้านน้ำคำน้อย',
    'บ้านลำโกน(คุรุรัฐราษฎร์พัฒนา)',
    'บ้านหัวช้าง',
    'บ้านห้างหว้า(คุรุรัฐประชาสรรค์)',
    'บ้านโคกสำโรง',
    'บ้านหัวโทนมิตรภาพที่ 165',
    'บ้านตากแดดสุธรรมประชาสรรค์',
    'บ้านสองชั้น',
    'บ้านส้มโฮง',
    'บ้านโพนดวน',
    'เบญจคามวิทยา',
    'บ้านโพนพอุง',
    'บ้านหินกอง',
    'บ้านเล้าข้าว',
    'บ้านขัดเค้า',
    'บ้านตังหมอง',
    'บ้านหนองอีเข็ม',
    'บ้านหัวนา',
    'บ้านกกกุงคุรุราษฎร์พัฒนา',
    'บ้านเหล่าหัวภู',
    'บัวสูงบัวเงินวิทยา',
    'บ้านหนองยาง',
    'บ้านสูงยาง',
    'บ้านคูเมือง',
    'บ้านป่าดวนพังหาด',
    'ดงเกลือวิทยา',
    'บ้านเมืองสรวง',
    'ชุมชนบ้านผำ',
    'บ้านเหล่าฮก',
    'บ้านป่ายาง',
    'บ้านหนองผือโนนค้อ',
    'บ้านข่อย',
    'บ้านหนองหิน',
    'บ้านหนองหินน้อย',
    'บ้านดอนโมงสัมพันธ์ปลาคูณ',
    'บ้านโพนทอง',
    'บ้านหนองพลับ',
    'บ้านโนนสูง',
    'บ้านโพนทราย',
    'บ้านหนองบัวดอนต้อน',
    'บ้านหนองสรวง',
    'บ้านหนองยาง',
    'บ้านดอนหม่วย',
    'ยางคำวิทยา',
    'บ้านเหล่าข้าว',
    'บ้านหนองฝั่งแดง',
    'บ้านสว่าง',
    'บ้านดอนขาม',
    'บ้านสะแบงตาก',
    'บ้านโพนดวนสาวเอ้',
    'บ้านดงหมากไฟ',
    'บ้านหัวดง',
    'สามขาพิทยาคม',
    'ชุมชนบ้านหมากยาง',
    'บ้านขี้เหล็ก',
    'บ้านยางเฌอ(วีรชนอุทิศ)',
    'บ้านจิก(หจก.วิบูลย์พาณิชย์อนุเคราะห์)',
    'บ้านเหล่าง้าวโนนค้อประชาสรรค์',
    'บ้านหัวนา',
    'บ้านกุดเขียว',
    'บ้านชีโหล่น',
    'บ้านแจ้ง',
    'บ้านท่าค้อ',
    'บ้านโนนยางรัฐพัฒนา',
    'บ้านสำโรง',
    'บ้านหว่านไฟ',
    'บ้านลิ้นฟ้า',
    'บ้านดู่',
    'บ้านดงเมืองจอก',
    'บ้านธาตุสามัคคี',
    'บ้านโนน',
    'บ้านกอก',
    'บ้านดงบัง',
    'บ้านเหล่าบากสามัคคี',
    'ชุมชนบ้านโพนเมือง',
    'บ้านน้ำคำ',
    'บ้านดงสว่าง',
    'บ้านแวง',
    'บ้านหนองฮาง',
    'บ้านเป้า',
    'บ้านเมืองคลอง',
    'หนองขามวิทยา',
    'บ้านขุมเงิน',
    'บ้านหนองบัว',
    'บ้านหนองเรือ',
    'บ้านแขม',
    'บ้านมะกอก',
    'บ้านทองหลาง',
    'บ้านหนองยาง',
    'บ้านกระจายเหล่าสูง',
    'บ้านหนองหัวคน',
    'สีสวาดเล็บขาว',
    'บ้านหนองหมื่นถ่าน',
    'บ้านหัวดอนเปลือยพัฒนา',
    'บ้านห้วยแก้วห้วยสำราญ',
    'บ้านโนนแฮด',
    'บ้านหน่อมฟ้าเลื่อมวิทยา',
    'บ้านจ้อก้อ',
    'บ้านกลอยโนนสมบูรณ์',
    'บ้านโหรา',
    'บ้านหนองหว้า',
    'บ้านหนองนาหล้า',
    'บ้านหนองย่างงัว',
    'บ้านน้ำเงิน',
    'บ้านแคน(วันครู 2503)',
    'บ้านศาลา',
    'เมืองอาจสามารถ',
    'บ้านส่องอนามัย',
    'บ้านเมืองคลองสาขาบ้านดอนงัว',
    'บ้านสำราญ',
    'บ้านร่องคำหงษ์ทองสามัคคี',
    'บ้านบุ่งเบา',
    'บ้านหนองจาน',
    'บ้านดูกอึ่งศรีเจริญ(ศรีปัญญาวิทยาคม)',
    'บ้านหนองแห้วหนองบัวทอง',
    'บ้านธาตุจอมศรี',
    'บ้านวารีสีสุก',
    'โรงเรียนบ้านหนองไศล',
    'บ้านขมิ้น',
    'บ้านดงเย็น',
    'บ้านกอกแก้ว',
    'บ้านเด่นราษฎร์',
    'บ้านหนองคูณ',
    'สาวแหวิทยา',
    'บ้านดอนแดง',
    'ชุมชนบ้านหนองฮี(ถาวรวิทยาคม)',
    'บ้านขวาว',
    'ดอนกลอยวิทยาคาร',
    'พนมไพรวิทยาคาร',
    'จันทรุเบกษาอนุสรณ์',
    'ปทุมรัตต์พิทยาคม',
    'น้ำใสวรวิทย์',
    'หนองผึ้งวิทยาคาร',
    'โพนสูงประชาสรรค์',
    'โพธิ์ทองพิทยาคม',
    'ทุ่งกุลาประชานุสรณ์',
    'กู่กาสิงห์ประชาสรรค์',
    'เหล่าหลวงประชานุสรณ์',
    'สุวรรณภูมิพิทยไพศาล',
    'หัวโทนวิทยา',
    'สุวรรณภูมิ',
    'ช้างเผือกวิทยาคม',
    'ทุ่งหลวงพลับพลาไชย',
    'หินกองวิทยาคาร',
    'เมืองสรวงวิทยา',
    'สูงยางวิทยาประชาสรรค์',
    'ทรายทองวิทยา',
    'สามขาวิทยา',
    'อาจสามารถวิทยา',
    'หนองหมื่นถ่านวิทยา',
    'บ้านขี้เหล็กพิทยาคม',
    'โพนเมืองประชารัฐ',
    'หนองฮีเจริญวิทย์',
    'ดูกอึ่งประชาสามัคคี',
    'บ้านหนองขอนแก่นสาขาบ้านสี่แยก',
    'บ้านบัวสาขาบ้านดงมัน',
    'บ้านปากช่อง',
    'บ้านบัว',
    'บ้านราษฎร์ดำเนิน',
    'บ้านชุมแสงชัย',
    'บ้านคำนาดี',
    'บ้านหนองขอนแก่น',
    'บ้านโคกกกม่วง',
    'บ้านพรหมจรรย์',
    'บ้านดงกลาง',
    'บ้านสองห้องหนองดงหนองหิน',
    'บ้านแฝก',
    'บ้านหนองตอ',
    'นาแพงวิทยาคาร',
    'บ้านบึงอุดม',
    'บ้านหนองม่วง',
    'บ้านสะอาดเจริญ',
    'ถนนชัยสามัคคี',
    'บ้านงิ้วหว่านศึกษาคาร',
    'บ้านดอนหวาย',
    'บ้านโนนไชยศรี',
    'บ้านโคกสมบูรณ์',
    'น้ำคำสมศรี',
    'บ้านหนองขี้ม้า',
    'บ้านนาศรีนวล',
    'บ้านพรมสวรรค์',
    'บ้านนาสีใส',
    'บ้านหนองหิ่งหาย',
    'บ้านโพนทอง',
    'บ้านนาทม',
    'บ้านหนองไฮ',
    'บ้านนาอุ่ม',
    'บ้านบะเค(ราษฎร์ผดุงวิทยาคาร)',
    'บ้านโพธิ์ศรีหัวงัว',
    'บ้านป้องสร้างบุ',
    'บ้านหนองโก',
    'บ้านวังยาว',
    'บ้านวังม่วย',
    'บ้านกุดก่วง',
    'บ้านเหล่าน้อย',
    'บ้านหนองแสงทุ่ง',
    'บ้านหนองแสงท่า',
    'อนุบาลโพนทอง',
    'พนาลัยวิทยาเสริม',
    'บ้านกุดสระ',
    'บ้านหนองนกเป็ด',
    'บ้านโนนแก้ว',
    'โนนเพชรพัฒนา',
    'ชุมชนบ้านสว่าง',
    'บ้านท่าสำราญ',
    'คำพระโคกก่งวิทยา',
    'บ้านคำอุปราช',
    'บ้านโนนลาด',
    'ไทยรัฐวิทยา 36(บ้านบะตากา)',
    'บ้านหนองแวงแห่',
    'บ้านกุดแห่',
    'โคกล่ามวิทยาคาร',
    'บ้านดงดิบ',
    'บ้านหนองใหญ่ทับครัว',
    'บ้านโนนโพธิ์',
    'บ้านหนองม่วงสาขาบะยาวมลชัยศรี',
    'สีดาประชาสรรค์',
    'บ้านดอนชาด',
    'บ้านอุ่มเม่า',
    'จุมจังนาคำชลประทานวิทยา',
    'เมืองโพธิ์ชัย',
    'บ้านโคกกุง',
    'บ้านจัมปา',
    'บ้านหนองบัว',
    'บ้านหนองแวงใหญ่',
    'บ้านคำพอุง(ประชาราษฎร์บำรุง)',
    'บ้านภูเขาทอง',
    'บ้านโนนใหญ่',
    'บ้านโคกใหญ่',
    'บ้านนางาม',
    'ชุมชนเชียงใหม่พัฒนา',
    'บ้านดอนโองท่าวารีวิทยาสรรค์',
    'บ้านดอนเรือ',
    'บ้านดงยาง',
    'บ้านหนองอึ่งโปโล',
    'ชุมชนบัวคำ',
    'บ้านโคกหนองบัว',
    'บ้านหนองนกทา',
    'บ้านหนองนกทา(สาขาบ้านหนองขุมเงิน)',
    'บ้านโพธิ์ศรี',
    'บ้านปากบุ่ง',
    'บ้านดอนควาย',
    'บ้านท่าเสียว',
    'บ้านสะอาด(สะอาดวิทยาคาร)',
    'บ้านหนองตาไก้',
    'บ้านหนองแสงสว่างวิทย์',
    'บ้านหนองอิโล',
    'บ้านนาเลาสาขาบ้านอวยศรี',
    'บ้านอัคคะ(อัคควิทยาคาร)',
    'บ้านนาเลา',
    'บ้านโพนเฒ่า',
    'บ้านน้ำเกลี้ยง',
    'บ้านคำโพนสูง',
    'ไตรมิตรวิทยา',
    'บ้านวังใหญ่',
    'บ้านกุดเต่าวิทยา',
    'บ้านบะยาวบุ่งโง้งสามัคคี',
    'สามพานประชาสรรค์',
    'โคกสว่างหาญไพรวัลย์',
    'บ้านแก้ง',
    'บ้านคูเมืองสามัคคีวิทยา',
    'ท่าสีดาวิทยา',
    'ทรายทองเฉลิมวิทย์',
    'บ้านหนองแข้ดง',
    'ป่าไม้อุทิศ 11',
    'หนองแวงบึงงาม',
    'ชุมชนบ้านโคกกลางมิตรภาพที่ 84',
    'เหล่าขุมมันท่าสะอาด',
    'ชุมชนบ้านโคกกลางสาขาห้วยค้อ-คำเม็ก',
    'ชุมชนหนองแสงโนนสว่าง',
    'บ้านหนองคำ',
    'หนองไฮภูเขาทอง',
    'บ้านโนนสมบูรณ์',
    'บ้านโนนปลาเข็ง',
    'บ้านเหล่าโพนงาม',
    'บ้านกุดขุ่น',
    'เมืองหนองพอก',
    'บ้านหนองโน',
    'บ้านหนองขุ่นวิทยาคาร',
    'บ้านกกตาลดงบังวิทยา',
    'บ้านโพนงาม',
    'บ้านหนองบก',
    'บ้านโคกนาคำ',
    'ศรีสวัสดิ์',
    'หนองพอกพัฒนาประชานุสรณ์',
    'บ้านปลาโด',
    'บ้านกลางเสลภูมิ',
    'เสลภูมิสามัคคี',
    'บ้านท่าไคร้',
    'บ้านเหล่าแขมดงกลาง',
    'บ้านดงหวาย',
    'บ้านท่าสี',
    'บ้านโคกกุงดอนกอก',
    'บ้านหนองเม็ก',
    'บ้านขวัญเมือง',
    'บ้านดงประเสริฐ',
    'ชุมชนบ้านขวาว',
    'บ้านโนนยาง',
    'บ้านสะทอนโนนแพงประชาสรรค์',
    'บ้านหันหน่องสามัคคี',
    'บ้านทรายมูล',
    'บ้านนากระตึบ',
    'บ้านหนองสิม',
    'บ้านท่าม่วง',
    'บ้านกุดแข้โนนโพธิ์วิทยา',
    'บ้านน้ำเที่ยงนาเรียง',
    'บ้านสำโรงหนองจอก',
    'บ้านพันขาง',
    'บ้านโพธิ์ชัน',
    'บ้านกุดเรือ',
    'บ้านโพธิ์ตาก',
    'บ้านไค่นุ่นหนองบัวบานวิทยา',
    'บ้านบุ่งเบ้าห้วยเจริญ',
    'บ้านนาแซง(นาแซงราษฎร์บำรุง)',
    'บ้านขว้างวิทยากร',
    'บ้านป่าขี',
    'บ้านนาเมือง',
    'บ้านทรายขาวหนองแป้นประชาสรรค์',
    'บ้านโจด',
    'บ้านหนองหงษ์',
    'บ้านหนองหว้านาคำพัฒนา',
    'บ้านยาง',
    'บ้านลาด',
    'บ้านผักกาดหญ้าโรตารี่อุทิศ',
    'ไชยวานนาเลิงคุ้งสะอาด',
    'บ้านแห่',
    'บ้านหัวคู',
    'บ้านบ่อแกนาเลา',
    'บ้านโนนสว่าง',
    'บ้านน้ำจั้นน้อย',
    'บ้านโนนสวรรค์',
    'บ้านสะอาดนาดี',
    'บ้านนาแพง',
    'บ้านหนองผักแว่น',
    'บ้านนาโพธิ์',
    'บ้านหนองฟ้า',
    'บ้านหนองกุง',
    'บ้านบาโคหนองแคน',
    'ชุมชนนาทมโคกก่อง',
    'บ้านหวาย(หวายวิทยานุกูล)',
    'บ้านหนองตุ',
    'บ้านมะหรี่(ศิริวิทยาคาร)',
    'บ้านโนนคำน้ำจั้นใหญ่',
    'บ้านดอนน้อยท่าแห่ตาลม',
    'บ้านเมืองไพร(เมืองไพรวิทยาคาร)',
    'บ้านบากหนองเทา',
    'บ้านหนองขามบ่อแก้ววิทยา',
    'บ้านคอกควาย(ประชาอำนวย)',
    'วังหลวง',
    'บ้านท่าเยี่ยม',
    'บ้านโพธิ์งาม',
    'บ้านโสกเตย',
    'บ้านนาวี',
    'บ้านหนองไผ่หนองหว้า',
    'บ้านสีเสียด',
    'หนองหลวงประชาบำรุง',
    'บ้านหนองสำราญหนองเรือ',
    'บ้านดงแจ้งโนนสูง',
    'บ้านหนองจอก',
    'บ้านวังเข',
    'บ้านท่าลาดหนองผักตบ',
    'ชุมชนบ้านชมสะอาด',
    'บ้านหนองโน',
    'บ้านหนองเดิ่น',
    'บ้านโคกสี',
    'บ้านห้วยทราย',
    'ชุมพรนาเจริญ',
    'บ้านโป่งประชาพัฒน์',
    'คำนางตุ้มโนนสวรรค์',
    'ชุมชนบ้านบุ่งเลิศ',
    'บ้านหนองเมย',
    'บ้านหนองสองห้อง',
    'โนนชัยศรีวิทยา',
    'เชียงใหม่ประชานุสรณ์',
    'วังหลวงวิทยาคม',
    'เมยวดีพิทยาคม',
    'โพนทองพัฒนาวิทยา',
    'โพธิ์ชัยชนูปถัมภ์',
    'หนองพอกวิทยา',
    'เสลภูมิพิทยาคม',
    'โพนทองวิทยายน',
    'ขวาววิทยาคาร',
    'ม่วงมิตรวิทยาคม',
    'โพธิ์แก้วประชาสรรค์',
    'เสลภูมิ',
    'โพธิ์ทองวิทยาคาร',
    'ท่าม่วงวิทยาคม',
    'คำนาดีพิทยาคม',
    'โพธิ์ศรีสว่างวิทยา',
    'ผาน้ำทิพย์วิทยา',
    'ห้วยตูมวิทยาคาร',
    'กลางหมื่นสงเคราะห์',
    'หนองหัวช้าง',
    'ทุ่งศรีเมืองประชาวิทย์',
    'วัดชัยสุนทร',
    'มิตรภาพที่ 32',
    'อนุบาลกาฬสินธุ์',
    'กาฬสินธุ์พิทยาสัย',
    'ขมิ้นราษฎร์สงเคราะห์',
    'นาโกวิทยา',
    'หนองแวงประชานุกูล',
    'คำปลาฝาโนนชัย',
    'บ้านเหล่าค้อ',
    'แกเปะราษฎร์นิยม',
    'เชียงเครือวิทยาคม',
    'ชุมชนนาจารย์วิทยา',
    'แก่งนาขามสามัคคี',
    'คำโพนคำม่วงวิทยา',
    'องค์การอุตสาหกรรมป่าไม้ 7 (สวนป่าสมเด็จ)',
    'บ้านโจดศิริราษฎร์บำรุง',
    'บึงวิชัยสงเคราะห์',
    'หนองทุ่มสงเคราะห์',
    'ท่าไคร้สามัคคี',
    'บ้านคำเม็ก',
    'บ้านโคกกว้าง',
    'โคกล่ามวิทยา',
    'หนองโพนวิทยายน',
    'หนองโพนวิทยายน(สาขาบ้านหนองไผ่ราษฎร์บำรุง)',
    'หนองกุงวิทยาคม',
    'หนองบัวราษฎร์นิยม',
    'โคกน้ำเกลี้ยงวิทยาคม',
    'หามแหโพนทองวิทยาคม',
    'นาขามร่วมราษฎร์บำรุง',
    'โคกนางามสามัคคี',
    'บ้านสว่าง',
    'เหล่าหลวงวิทยาคาร',
    'ค่ายลูกเสือนิคมลำปาว',
    'สะอาดสมศรีวิทยา',
    'บ้านห้วยแสง',
    'ภูปอวิทยา',
    'หนองแวงเหนือ',
    'บ้านโนนสวรรค์ประชาสรรพ์ภูปอ',
    'สะอาดประชาสรรพ์',
    'ปลาเค้าวิทยาคาร',
    'บ้านหนองม่วง',
    'ชุมชนหนองสอวิทยาคาร',
    'โนนสะอาดราษฎร์สามัคคี',
    'ท่าแสงวิทยายน',
    'ดงเมืองวิทยาคาร',
    'ฝายแตกวิทยาคม',
    'กุดอ้อประชานุสรณ์',
    'วังเกาะเหล็ก',
    'โคกคอนวิทยานุกูล',
    'บ้านนาคอกควาย',
    'คำไผ่สงเคราะห์',
    'ท่าลำดวนประชาบำรุง',
    'คอนเรียบอนุกูล',
    'เตาไหราษฎร์ผดุง',
    'บ้านดอนสนวน',
    'โคกกลางราษฎร์พัฒนา',
    'กุดอ้อสุขสวัสดิ์วิทยา',
    'บ้านหลุบอินทรราษฎร์บำรุง',
    'ดงสวางวิทยายน',
    'ดงน้อยสงเคราะห์',
    'บ้านโหมนสงเคราะห์',
    'เหล่าสูงวิทยา',
    'กลางดงราษฎร์อุปถัมภ์',
    'บ้านหนาดสงเคราะห์',
    'บ้านต้อนวิทยาคาร',
    'บ้านเหล็กกกกอกสามัคคี',
    'พิพัฒน์ราษฎร์บำรุง',
    'ปากน้ำราษฎร์บำรุง',
    'น้ำจั้นราษฎร์บำรุง',
    'โปโลเล้าสามัคคีวิทยา',
    'ดอนยูงราษฎร์ประดิษฐ์วิทยาคาร',
    'ข้าวหลามชัยศรีดอนยางวิทยา',
    'สงยางสงเปลือยวิทยาคม',
    'หลักเมืองกมลาไสย',
    'ด่านเหนือหนองไผ่สงเคราะห์(สาขาราษฎร์สำราญ)',
    'นามลวิทยาคาร',
    'คำโพนทองราษฎร์นิยม',
    'เก่าน้อยโนนรังวิทยาเสริม',
    'ท่าเพลิงเหมือดแอ่ท่าใหม่หนองมะเกลือวิทยา',
    'ท่ากลางแจ้งจมวิทยา',
    'หนองบัวโจดดงลิงวิทยา',
    'ชุมชนสีถานดำรงวิทย์',
    'โคกเมยประชาพัฒนา',
    'โคกล่ามผดุงวิทย์',
    'โนนเมืองวิทยาคาร',
    'บ้านแวงประชารัฐบำรุง',
    'ดอนหวายราษฎร์บำรุง',
    'หนองตุวิทยา',
    'สะอาดโนนงามวิทยา',
    'บ้านบ่อโนนมะค้ำวิทยาคาร',
    'บ้านแกหัวแฮดส้มโฮงวิทยา',
    'สงยางสงเปลือยวิทยาคม(สาขาสงเปลือยสามัคคี)',
    'ชุมชนโพนงามประสาทศิลป์',
    'โนนไฮหนองอีกุ้มวิทยา',
    'ธนบุรีวิทยายน',
    'ด่านเหนือหนองไผ่สงเคราะห์',
    'เสมาสามัคคี',
    'บ้านโนนโพธิ์ศรี',
    'หนองแปนโนนสูงนาเชือกราษฎร์บำรุง',
    'นาบึงราษฎร์สงเคราะห์',
    'บึงไฮโนนสวางวิทยา',
    'บ้านลาดวิทยาเสริม',
    'บึงสว่างวิทยาคม',
    'หัวขวาวิทยา',
    'กุดลิงวิทยาคม',
    'สองห้องราษฎร์บูรณะ',
    'ชุมชนนาเรียงวิทยาคม',
    'เหล่าเขืองโนนเสียววิทยา',
    'สำเริงวิทยา',
    'สามัคคีสำราญวิทย์',
    'ดงพะยอมหนองคูสามัคคีวิทยา',
    'ด่านใต้วิทยา',
    'เหล่าอ้อยบำรุงวิทย์',
    'ชุมชนนามะเขือวิทยา',
    'บ้านโคกเจริญ',
    'บ้านคำน้ำแซบ',
    'โพนสวางพิทยาคม',
    'โพนสวางพิทยาคม(สาขาโพนคำ)',
    'โป่งเชือกศึกษาสถาน',
    'ชุมชนนามะเขือวิทยา(สาขาหนองบัวลอย)',
    'บ้านศรีสมบูรณ์',
    'ชุมชนแก้งคำวิทยา',
    'นิคมลำปาววิทยา',
    'คำแคนวิทยา',
    'คำประถมนิคมสงเคราะห์',
    'ชุมชนน้ำเกลี้ยงกล่อมวิทยา',
    'โนนป่างิ้ววิจิตวิทยา',
    'วังสวาทนิคมสงเคราะห์',
    'สหัสขันธ์วิทยาคม',
    'โนนศิลาไกรฤกษ์ราษฎร์อำนวย',
    'บ้านม่วงวิทยาคม',
    'ม่วงคำราษฎร์สามัคคี',
    'โนนเมืองประชานุเคราะห์',
    'บ้านแกวิทยายน',
    'โคกก่องวิทยา',
    'คำคาราษฎร์บำรุง',
    'ดงไร่ราษฎร์พัฒนา',
    'บ้านคำเชียงวัน',
    'นามนราษฎร์นุเคราะห์',
    'บ้านโคกใส',
    'บ้านโนนแหลมทอง(สาขาคอนผึ้ง)',
    'บ้านโนนแหลมทอง',
    'คำชมภูท่าเรือสมบูรณ์วิทย์',
    'ดงน้อยวิทยา',
    'โนนปลาขาวราษฎร์บูรณะ',
    'นาคูณวิทยา',
    'คำลือชาศรีสวัสดิ์วิทยา',
    'ถ้ำปลาวิทยายน',
    'สิงห์สะอาด',
    'ห้วยเตยพรประสิทธิ์วิทยา',
    'นาสีนวลโสกทรายวิทยา',
    'ท่านาจานวิทยา',
    'คำแคนราษฎร์วิทยา',
    'บ้านโนนสะอาด',
    'บ้านหนองกุงกลาง',
    'บ้านหนองกุงใหญ่',
    'โคกกลางวิทยา',
    'หนองแซงวิทยา',
    'บ้านท่างาม',
    'บ้านท่างาม สาขาบ้านดานเม็ก',
    'บ้านหนองกุงน้อย',
    'ท่าช้างรัฐประชาพัฒนา',
    'บ้านกุดแห่',
    'คำหุ่งราษฎร์บำรุง',
    'หนองแสงราษฎร์พัฒนา',
    'โคกสำราญวิทยา',
    'เขื่อนภูมิพล',
    'สามชัยอุดมวิทย์',
    'บ้านหนองช้าง',
    'ค้อจารย์วิทยา',
    'บ้านจานวิทยา',
    'บ้านห้วยยาง',
    'แก้งนางราษฎร์บำรุง',
    'ดงพยุงสงเคราะห์',
    'ดงพยุงสงเคราะห์(สาขาบ้านโนนกกจิกประชาสรรพ์)',
    'หัวคูประชาอุทิศ',
    'กุดครองวิทยาคาร',
    'ด่านแต้ราษฎร์สงเคราะห์',
    'หนองแคนวิทยา',
    'หนองแวงแสน',
    'นาจำปา',
    'หนองโพนโนนสมบูรณ์',
    'บ้านม่วงวิทยายน',
    'บ้านนาวิทยาคม',
    'สายป่าแดงวิทยา(สาขาบ้านภูเงิน)',
    'หนองพอกวิทยายน',
    'สะอาดไชยศรี',
    'สายป่าแดงวิทยา',
    'หนองสอพิทยาคม',
    'ขมิ้นพิทยาสรรพ์',
    'กาฬสินธุ์พิทยาสรรพ์',
    'กมลาไสย',
    'อนุกูลนารี',
    'ฟ้าแดดสูงยางวิทยาคาร',
    'ดอนไทรงามพิทยาคม',
    'เมืองกาฬสินธุ์',
    'เหล่านาแกวิทยานุสรณ์',
    'โคกศรีเมือง',
    'ธัญญาพัฒนวิทย์',
    'แก่งนาจารย์พิทยาคม',
    'เมืองเหนือวิทยาคม',
    'ร่องคำ',
    'สหัสขันธ์ศึกษา',
    'นามะเขือพัฒนศึกษา',
    'โนนศิลาพิทยาคม',
    'สามชัย',
    'ดอนจานวิทยาคม',
    'จำปาหลวงวิทยาคม',
    'นากุงวิทยาเสริม',
    'บ้านหนองแสง',
    'หนองกุงราษฎร์วิทยา',
    'หนองบัวหน่วยอำนวยวิทย์',
    'บ้านดงอัคคะ',
    'บ้านขามวิทยาคม',
    'บ้านหนองแวงฮี',
    'บ้านหนองบัว',
    'บ้านโคกศรี',
    'บ้านสาวิทยาสรรพ์',
    'ชุมชนบ้านป่าแดง',
    'บ้านหนองขาม',
    'ดอนยานางศึกษา',
    'นาเชือกวิทยาสรรพ์',
    'โนนภักดีประชานุเคราะห์',
    'ท่าเรือวิทยา',
    'นาแกราษฎร์อำนวย',
    'หนองกาวคุรุราษฎร์บำรุง',
    'ปอแดงวิทยา',
    'นาดีหลุมข้าววิทยา',
    'บ้านขมิ้นพัฒนวิทย์',
    'นางามวิทยาคม',
    'หนองไผ่รัฐบำรุง',
    'สร้างมิ่งประสิทธิ์ผล',
    'โนนสูงวิทยา',
    'บ้านแดงเจริญศิลป์',
    'คำแมดพิทยาสรรพ์',
    'เชียงสาศิลปสถาน',
    'ชุมชนบ้านตูมวิทยาคาร',
    'เชียงงามวิทยาคาร',
    'โพธิ์ศรีโปร่งแควิทยาเสริม',
    'โคกก่องราษฎร์นุกูล',
    'บ้านเว่อวิทยานุกูล',
    'โคกคำวิทยา',
    'ฮ่องฮีวิทยา',
    'พินิจราษฎร์บำรุง',
    'บ้านยางตลาดใหม่(ผดุงราษฎร์วิทยา)',
    'ชุมชนดอนยูงวิทยายน',
    'หัวงัววิทยาคาร',
    'บ้านหนองเสือ',
    'ห้วยเตยวิทยา',
    'บ้านหนองตอกแป้นวิทยา',
    'คำไฮวิทยา',
    'หนองอิเฒ่าวิทยา',
    'หลักด่านวิทยา',
    'บ้านดงน้อย',
    'บ้านดอนกลอย(พิลาศอุปถัมภ์)',
    'วัดบ้านกุดสังข์',
    'บ้านเสียววิทยาสรรพ์',
    'หนองไม้พลวงวิทยาคม',
    'บ้านลาดสระบัว',
    'บ้านโคกคันจ้องหนองแต้สามัคคี',
    'หัวนาคำจรูญศิลป์',
    'ดงบังวิทยา',
    'โคกค่ายโคกใหญ่วิทยา',
    'บ้านหนองหัวช้าง',
    'โพนสิมอนุเคราะห์',
    'วัดบ้านดอนกลาง',
    'ดอนขีวิทยา(สาขาบ้านสร้างหิน)',
    'ยางคำวิทยา',
    'บ้านแกวิทยาคม',
    'บ้านหนองแวงบ่อแก้ว',
    'ดอนขีวิทยา',
    'อุ่มเม่าวิทยา',
    'โคกศรีวิทยายน',
    'บ้านคำม่วง',
    'ชุมชนกุดโดนวิทยาคม',
    'หนองแวงประชาสรรพ์',
    'หนองแวงม่วง',
    'นาค้อวิทยาคม',
    'หนองปะโอประชาอุทิศ',
    'คุรุชนประสิทธิ์ศิลป์',
    'คำมันปลาผดุงวิทย์',
    'คำเหมือดแก้วบำเพ็ญวิทยา',
    'ป่าหวายศึกษา',
    'คำใหญ่วิทยา',
    'ชุมชนสะอาดผดุงศิลป์',
    'บ้านชัยศรี',
    'โนนเตาไหหนองแก',
    'บ้านทรายทองวิทยาคม',
    'หนองกุง',
    'โนนสะอาดราษฎร์อำนวย',
    'หนองแสงวิทยา',
    'ยางเนียมพัฒนา',
    'หนองบัววิทยาเสริม',
    'นาสีนวลอุดมเวศม์',
    'เดชอุดมพิทยาคม',
    'พิมูลวิทยา',
    'บ้านหาดทรายมูล',
    'หน่อคำประชานุเคราะห์',
    'เนินลาดวิทยา',
    'ห้วยเม็กราษฎร์นุกูล',
    'บ้านกุดท่าลือ',
    'หัวดงวิทยา',
    'โคกกลางเหนือพิทยาสรรพ์',
    'ห้วยมะทอโคกล่ามวิทยา',
    'หนองโนวิทยาคม',
    'หัวหินราษฎร์บำรุง',
    'ชุมชนกุงเก่าราษฎร์ประสิทธิ์',
    'สร้างแก้วรัฐราษฎร์บำรุง',
    'ขอนแก่นวิทยาเสริม',
    'บ้านคำแคน',
    'กุดจิกวิทยาคาร',
    'บ้านโคกกลาง',
    'บ้านแสนสุข',
    'ดงสวรรค์อุดมมิตร(สาขาบ้านดงจันทร์)',
    'ดงบังอำนวยวิทย์',
    'ดงสมบูรณ์ประชารัฐ',
    'ดงสวรรค์อุดมมิตร',
    'มิตรมวลชน 5 (บ้านไทยเจริญ)',
    'นาตาลวิทยาคม',
    'ท่าคันโทวิทยายน',
    'ท่าเมืองสำราญวิทย์',
    'บ้านเกิ้ง(สลากกินแบ่งสงเคราะห์-170)',
    'บ้านหนองแซง',
    'บ้านโนนอำนวย',
    'ยางอู้มวิทยาคาร',
    'คำบอนวิทยาสรรพ์',
    'บ้านชัยศรีสุข',
    'ประชารัฐศึกษา',
    'โคกเครือวิทยา',
    'หนองโนวิทยา',
    'บ้านนางาม',
    'ห้วยยางดงวิทยา',
    'สายปัญญาสมาคม',
    'ภูฮังวิทยาคาร',
    'สำราญ - ประภาศรี',
    'ไทรทองวิทยาคาร',
    'คำขามวิทยา',
    'สว่างกิจวิทยา',
    'คำถาวรเจริญวิทย์',
    'เสริมเสาเล้าวิทยา',
    'หนองมันปลาวิทยา',
    'หนองแฝกหนองหว้าวิทยา',
    'บ้านหนองไผ่',
    'บ้านโนนสมบูรณ์วิทยา',
    'หนองแฝกหนองหว้าวิทยา(สาขาคำน้อย)',
    'โคกเจริญวิทยา',
    'หนองหอไตรราษฎร์บำรุง',
    'หนองกุงศรีวิทยาคม',
    'ชุมชนสามัคคีราษฎร์บำรุง',
    'สะอาดนาดีศิลาวิทย์',
    'คำไฮวิทยา',
    'สระแก้ววิทยานุกูล',
    'ไชยวารวิทยาคม',
    'พรมลีศรีสว่าง',
    'หนองบัวคุรุรัฐประชาสรรพ์',
    'ท่อนสังข์วิทยา',
    'ไชยศรีเรืองวิทย์',
    'พรมลีศรีสว่าง(สาขาพรมลี)',
    'นาบงวิทยา',
    'หนองสวงวิทยาคม',
    'คำโองวิทยา',
    'นาอวนวิทยาสิทธิ์',
    'หนองแข้วิทยา',
    'สว่างศรีโคกคึม',
    'หนองบัวชุม',
    'ชุมชนหนองหินวิทยาคาร',
    'หนองใหญ่วิทยา',
    'บ้านหนองบัวแดง',
    'จินดาสินธวานนท์',
    'นามูลสมบูรณ์วิทยา',
    'บ้านหนองชุมแสง',
    'เหล่ากลางร่วมวงศ์วิทยา',
    'กุดฆ้องชัยวิทยา',
    'บ้านน้อยดอนข่าประชานุเคราะห์',
    'โคกประสิทธิ์วิทยา',
    'โนนชัยประชาสรรค์',
    'หนองเม็กวิทยา',
    'บัวสะอาดส่งเสริม',
    'บ้านชาดวิทยาคาร',
    'ท่าแห่วิทยาคม',
    'ดอนแคนโนนเปลือยวิทยา',
    'โนนเขวาเหล่าใหญ่ราษฎร์ส่งเสริม',
    'บ้านสีสุก',
    'โนนศิลาสว่างวิทย์',
    'หนองคูวังเดือนห้าวิทยา',
    'วังยางวิทยาคาร',
    'ท่าเยี่ยมหนองคล้าประชาส่งเสริม',
    'โนนแดงราษฎร์ประสิทธิ์',
    'ยางตลาดวิทยาคาร',
    'โนนสูงพิทยาคม',
    'ทรายมูลพิทยาคม',
    'ลำปาววิทยาคม',
    'หัวหินวัฒนาลัย',
    'เขาพระนอนวิทยาคม',
    'วังลิ้นฟ้าวิทยาคม',
    'วังมนวิทยาคาร',
    'คำเจริญวิทยาคม',
    'นาเชือกวิทยาคม',
    'คลองขามวิทยาคาร',
    'ห้วยเม็กวิทยาคม',
    'ท่าคันโทวิทยาคาร',
    'ดงกลางพัฒนศึกษา',
    'หนองกุงศรีวิทยาคาร',
    'ดงมูลวิทยาคม',
    'หนองชุมแสงวิทยาคม',
    'มัธยมภูฮังพัฒนวิทย์',
    'หนองบัวไชยวารพิทยาสรรพ์',
    'ฆ้องชัยวิทยาคม',
    'เหล่ากลางวิทยาคม',
    'ไตรรัตนวิทยาคม',
    'นามนราษฎร์สงเคราะห์',
    'บ้านโนนเที่ยง',
    'คำยิ่งหมีมณีเวทย์',
    'สามัคคีวิทยาคม',
    'เย็นสยามวิทยา',
    'ชุมชนยอดแก่งสงเคราะห์',
    'หนองหญ้าปล้อง',
    'บ้านแห่เจริญวิทย์',
    'หนองโพนสูง',
    'สงเปลือยวิทยายน',
    'หัวงัววิทยาคม',
    'หนองแวงหนองน้อยวิทยาคาร',
    'ดงสวางวรวิทย์',
    'หนองบัวใน',
    'ยิ่งสันต์วิทยา',
    'หนองบัวนอก',
    'หนองบัวกลาง',
    'หนองคอนเตรียมราษฎร์บำรุง',
    'ชุมชนหลักเหลี่ยมวิทยาคม',
    'สังคมพัฒนา',
    'กุดค้าวเทพพิทยา',
    'บ้านหนองโง้ง',
    'ศรีกุดหว้าเรืองเวทย์',
    'บ้านวังมน',
    'บ้านขุมขี้ยาง',
    'บ้านห้วยแดง',
    'บ้านโคกโก่ง',
    'นาไคร้ประชาสรรค์',
    'กุดหว้าวิทยา',
    'โคกกลางราษฎร์พิทักษ์',
    'บ้านนาเหนือ',
    'จุมจังอุปกรณ์เจริญเวทย์',
    'หนองจระเข้เรืองศิลป์',
    'หนองฟ้าเลื่อนอำนวยวิทย์',
    'ชุมชนแจนแลนราษฎร์บำรุง',
    'แจนแลนวิทยา',
    'นาโกวิทยาสูง',
    'นาโกพิศาลราษฎร์อุปถัมภ์',
    'โคกหวายราษฎร์สามัคคี',
    'บ้านคำอีหงษ์',
    'บ้านคำป่าหว้าน',
    'นาขามวิทยา',
    'บ้านสุขเจริญ',
    'บอนเขียวราษฎร์บำรุง',
    'บ้านโนนสวรรค์',
    'บุ่งคล้าวิทยาคมมิตรภาพที่ 194',
    'บ้านกกตาล',
    'สามัคคีบัวขาว',
    'บ้านบัวขาว(พร้อมพันธ์อุปถัมภ์)',
    'บ้านดอนอุมรัว',
    'บ้านบัวขาว(วันครู 2500)',
    'หนองหูลิงเจริญเวทย์',
    'สมสะอาดพิทยาสรรพ์',
    'บ้านดงมัน',
    'หนองบัวทองวิทยาเสริม',
    'สามขาราษฎร์บำรุง',
    'บ้านคุย',
    'บ้านบึงทอง',
    'คำโพนทองบริบูรณ์ราษฎร์บำรุง',
    'บ้านแก้งเดื่อ',
    'บ้านห้วยม่วง',
    'หนองสระพังวิทยาคาร',
    'หนองห้างฉวีวิทย์',
    'หนองห้างอำนวยวิทย์',
    'ไทยรัฐวิทยา 35 (คำกั้ง)',
    'มะนาววิทยาเสริม',
    'บ้านกุดฝั่งแดง',
    'บ้านจอมทอง',
    'เหล่าใหญ่วนาสณฑ์ผดุงเวทย์',
    'ดงเหนือประชาสรรค์',
    'บ้านกลาง',
    'เหล่าไฮงามวิทยาสูง',
    'บ้านสวนผึ้ง',
    'บ้านหนองเม็ก',
    'บ้านโนนยาง',
    'กุดปลาค้าวราษฎร์บำรุง',
    'กุดกว้างสวาสดิ์วิทยา',
    'บ้านโพนสวาง',
    'บ้านโนนสำราญ',
    'กุดสิมวิทยาสาร',
    'บ้านดงหมู',
    'กุดตอแก่นราษฎร์วิทยา',
    'ชุมชนกุดบอดวิทยาเสริม',
    'บ้านนาวี',
    'หนองแสงถวิลราษฎร์',
    'บ้านส้มป่อย',
    'บ้านกุดกอก',
    'โคกมะลิวิทยา',
    'บ้านหนองผือ',
    'บ้านโพธิ์ไทร',
    'บ้านโพนนาดี',
    'บ้านโพนแพง',
    'บ้านดินจี่',
    'นักธุรกิจ(บ้านโคกสนาม)',
    'คำม่วงจรัสวิทย์',
    'ทุ่งคลองวิทยา',
    'บ้านเก่าเดื่อ',
    'โป่งนกเปล้า',
    'บ้านคำพิมูล',
    'บ้านนาไร่เดียว',
    'บ้านโนนค้อ',
    'นาทันวิทยา',
    'บ้านหนองสระพัง',
    'บ้านโจดนาตาล',
    'บ้านดงสวนพัฒนา',
    'บ้านนาบอน',
    'สะพานหินวิทยาคม',
    'บ้านโนนศรีสวัสดิ์',
    'คำเมยราษฎร์พัฒนา',
    'ทุ่งมนราษฎร์บำรุง',
    'ชุมชนหนองยางวิทยาคม',
    'บ้านหนองม่วง',
    'บ้านโคกเพิ่มพูนพิทยา',
    'สูงเนินวิทยาคม',
    'บ้านใหม่ชัยมงคล',
    'ชุมชนโพนพิทยาคม',
    'หนองซองแมวผดุงวิทย์',
    'แซงบาดาลบ้านบากวิทยา',
    'บ้านบัวสามัคคี',
    'คำเม็กวิทยา',
    'บ้านเหล่าภูพานวิทยา',
    'โพธิ์คำประชาสรรพ์',
    'บ้านบึงบาดาลราษฎร์บำรุง',
    'หนองแห้วคุรุราษฎร์บูรณะ',
    'บ้านกอกวิทยาคม',
    'บ้านแก้งกะอาม',
    'บ้านขมิ้น',
    'หนองบัววิทยาสรรพ์',
    'บ้านขมิ้น (สาขาภูเงิน)',
    'มหาไชยโคกกว้างวิทยา',
    'หนองสนมราษฎร์อำนวย',
    'บ้านคำหม่วย',
    'ดงบังนาแก้ววิทยา',
    'ชุมชนบ้านบอนวิทยา',
    'บ้านสร้างแสน',
    'บ้านค้อราษฎร์บำรุงวิทยา',
    'บ้านหนองป่าอ้อย',
    'หนองแสงวิทยาเสริม',
    'บ้านโนนสะอาด',
    'บ้านคำกุงหนองอิดุม',
    'บ้านสี่แยกสมเด็จ',
    'หนองบัวโดน"วรนารถประชานุกูล"',
    'สมเด็จประชานุเคราะห์',
    'หนองกุงสมเด็จ',
    'หนองแวงวิทยานุกูล',
    'บ้านโนนชาด',
    'บ้านสร้างแก้ว',
    'บ้านหนองผ้าอ้อม',
    'โคกกลางสมเด็จ',
    'บ้านคำไผ่',
    'หนองขามวิทยา',
    'บ้านโนนชาด(สาขาสวนป่าสมเด็จย่อยที่ 2)',
    'บ้านหนองไผ่',
    'บ้านนาทัน',
    'บ้านดงแหลม',
    'ชุมชนหมูม่นวิทยาสรรพ์',
    'แก่งพฤาชัยวิทยา',
    'บ้านบึงโคกสะอาด',
    'คำบงพิทยาคม',
    'คำหมุนผดุงเวทย์',
    'บ้านหนองมะงง',
    'บ้านคำม่วง',
    'บ้านปลาขาว',
    'ค่ายลูกเสือจังหวัดกาฬสินธุ์',
    'บ้านเหล่าสีแก้ว',
    'ไค้นุ่นวิทยาพูน',
    'อุปรีศรีวิทยา',
    'บ้านผึ้ง',
    'นิคมกุฉินารายณ์ หมู่ 6',
    'บ้านห้วยผึ้ง',
    'บ้านหนองแสง',
    'นิคมกุฉินารายณ์ หมู่ 2',
    'บ้านห้วยฝา',
    'บ้านหนองขอนแก่น',
    'นิคมกุฉินารายณ์ หมู่ 3',
    'หนองอีบุตรไพรเวทย์',
    'บ้านนาคู',
    'บ้านวังเวียง',
    'บ้านชาด',
    'นาสีนวลวิทยา',
    'สามเพื่อนพัฒนา',
    'บ้านจาน',
    'โคกนาดี',
    'บ้านหนองห้าง',
    'รัชมังคลาภิเษก(บ้านนายอ)',
    'นางามวิทยา',
    'หินลาดนารายณ์สาร',
    'บ้านบ่อแก้ว',
    'บ้านม่วงกุล',
    'ชุมชนภูแล่นช้างคเชนทร์พิทยาคาร',
    'บ้านพรหมสว่าง',
    'บ้านน้ำปุ้น',
    'บ้านโนนศาลา',
    'บ้านจอมศรี',
    'บ้านนากระเดา',
    'บ้านกุดตาใกล้',
    'บ้านนากุดสิม',
    'บ้านหนองขามป้อม',
    'บัวขาว',
    'เขาวงพิทยาคาร',
    'นามนพิทยาคม',
    'กุฉินารายณ์',
    'พุทโธภาวนาประชาสรรค์',
    'มอสวนขิงพิทยาสรรพ์',
    'จุมจังพลังราษฎร์',
    'กาญจนาภิเษกวิทยาลัย กาฬสินธุ์',
    'สามขาสว่างวิทย์',
    'โนนคำวิทยา',
    'นาไคร้พิทยาสรรพ์',
    'หนองห้างพิทยา',
    'คำม่วง',
    'เนินยางประชาสามัคคี',
    'สมเด็จพิทยาคม',
    'มหาไชยพิทยาคม',
    'แก้วเสด็จพิทยาคม',
    'ประชารัฐพัฒนศึกษา',
    'บัวสามัคคีพิทยาสรรพ์',
    'เมืองสมเด็จ',
    'ผาเสวยรังสรรค์',
    'ห้วยผึ้งพิทยา',
    'นิคมหนองแก้วญาณเวที',
    'บ้านนาคูพัฒนา(กรป.กลางอุปถัมภ์)',
    'บ่อแก้วญาณเวทีพัฒนา',
    'บ้านพานสหราษฎร์บำรุง',
    'บ้านประชาสุขสันติ์',
    'โคกเลาะวิทยาคาร',
    'บ้านโพนบกสหราษฎร์วิทยา',
    'ชุมชนดงมะไฟเจริญศิลป์',
    'ศรีบุญเรืองวิทยาคาร',
    'บ้านโคกก่องสหมิตรวิทยาคาร',
    'บ้านโคกสูง (สหราษฎร์บำรุง)',
    'นายอวัฒนา',
    'งิ้วด่อนราษฎร์สามัคคีบำรุง',
    'บ้านดอนหมูโพธิ์ศรี"ชุมชนพัฒนา"',
    'ดอนเชียงบาลราษฎร์บำรุงศิลป์',
    'ทุ่งมนพิทยาคาร',
    'ชุมชนเชียงเครือราษฎร์รังสรรค์',
    'บ้านป่าหว้าน',
    'บ้านหนองหอย',
    'ศิริราษฎร์วิทยาคาร',
    'บ้านดอนเชียงคูณ',
    'ดงชนเหล่าแมดบำรุงวิทย์',
    'บ้านหนองมะเกลือ',
    'บ้านกุดแข้',
    'บ้านนาแก',
    'บ้านดงขวาง',
    'บ้านดงมะไฟสามัคคีราษฎร์อุทิศ',
    'บ้านโพนก้างปลา',
    'บ้านนากับแก้',
    'บ้านหนองไผ่',
    'บ้านหนองไผ่ สาขาบ้านโพนแดง',
    'บ้านพะโค',
    'ท่าแร่วิทยา',
    'ท่าแร่วิทยา(สาขาบ้านหนองบัวทอง)',
    'อนุบาลสกลนคร',
    'เชิงชุมราษฎร์นุกูล',
    'สกลนคร(วันครู2501)',
    'นาอ้อยคำสะอาด',
    'เมืองสกลนคร(ธาตุนารายณ์เจงเวง)',
    'บ้านกกส้มโฮง',
    'ธาตุนาเวงวิทยา',
    'ชุมชนโนนหอมไผ่ล้อม',
    'ห้วยปลาใยผดุงวิทยา',
    'ผักแพวบำรุงวิทยา',
    'บ้านโพนยางคำ กรป.กลางพัฒนา',
    'บ้านท่าเยี่ยมน้ำพุง',
    'บ้านหนองปลาดุกศรีวิทยา',
    'บ้านพังขว้างวัฒนศิลป์',
    'บ้านดงยอ',
    'บ้านดงขุมข้าว',
    'บ้านโนนสวรรค์สมบูรณ์วิทย์',
    'บ้านพังขว้างเหนือ 909 กรป.กลางอุปถัมภ์',
    'ห้วยทรายวิทยา',
    'บ้านหนองนาเลิศ',
    'บ้านม่วงลายราษฎร์ผดุงศิลป์',
    'บ้านนา(คุรุราษฎรอุทิศวิทยายน)',
    'บ้านดอนแคน',
    'บ้านท่าม่วง',
    'บ้านดอนสัมพันธ์',
    'บ้านหนองลาด(ราษฎร์อุทิศวิทยาคม)',
    'บ้านสายปลาหลาย',
    'บ้านศรีวิชา"ครุราษฎร์อุทิศ"',
    'บ้านพะเนาว์ สาขาบ้านนาขาม',
    'บ้านนาคำวิทยาคาร',
    'บ้านลาดกะเฌอ(คุรุราษฎร์ชูวิทย์)',
    'บ้านพะเนาว์ราษฎร์บำรุง',
    'บ้านบอนสหราษฎร์บำรุง',
    'บ้านหนองแคนสหราษฎร์วิทยา',
    'บ้านหนองสระ',
    'บ้านดอนยาง(สหราษฎร์บำรุงวิทย์)',
    'บ้านหนองปลาน้อย(สุทธะเรืองศิลป์)',
    'บ้านท่าวัด"คุรุราษฎร์บำรุงวิทย์"',
    'บ้านหนองศาลาราษฎร์บำรุง',
    'บ้านนาดอกไม้ผดุงวิทยา',
    'บ้านน้อยจอมศรี',
    'บ้านฮางโฮงประชาอุทิศ',
    'อนุบาลกุสุมาลย์',
    'บ้านอีกุด',
    'ชุมชนนิรมัย',
    'นาเพียงสว่างวิทยานุกูล',
    'บ้านซ่งเต่า',
    'บ้านกุงศรี',
    'บ้านหนองปลาตอง(ประชาวิทยาคาร)',
    'นาเพียงสว่างวิทยานุกูล (สาขาศรีคงคำ)',
    'บ้านนาเพียงเก่าสนธิราษฎร์สามัคคี',
    'บ้านโคกม่วง',
    'บ้านนาโพธิ์คุรุราษฎร์พัฒนา',
    'บ้านบอนสหราษฎร์อุทิศ',
    'บ้านม่วงวิทยา',
    'บ้านนาดี',
    'ไพศาลวิทยา',
    'บ้านโพนแพง',
    'บ้านกุดฮู',
    'บ้านห้วยกอกหนองเค็ม',
    'บ้านกุดสะกอย',
    'บ้านอุ่มจาน',
    'บ้านโคกสะอาด',
    'แก้งคำประชาสามัคคี',
    'ชุมชนบ่อแสนพันมิตรภาพที่211',
    'บ้านสนามบิน',
    'บ้านหนองบัวสร้างวิทยาคาร',
    'บ้านงิ้วศิริราษฎร์บำรุง',
    'บ้านจันทร์เพ็ญ',
    'บ้านห้วยหวด',
    'บ้านกวนบุ่น',
    'บ้านโคกกลาง',
    'นางอยโพนปลาโหล',
    'อนุบลเต่างอย',
    'บ้านนาอ่างม่วงคำ',
    'บ้านนาตาลคำข่า',
    'บ้านหนองบึงทวาย',
    'บ้านดงหลวง',
    'บ้านด่านม่วงคำ',
    'บ้านลาดค้อ',
    'บ้านป่าปอสหพัฒนศึกษา',
    'บ้านหนองกอมป่าขาว',
    'บ้านม่วงไข่ประชาราษฎร์สงเคราะห์',
    'บ้านนามนประชาสามัคคี',
    'บ้านห้วยหีบวิทยาธาร',
    'บ้านนาสีนวล',
    'บ้านห้วยแคน',
    'บ้านหนองแข้โนนมาลา',
    'บ้านตองโขบ',
    'อนุบาลโคกศรีสุพรรณ',
    'บ้านแมดนาท่ม',
    'บ้านโพนทองประชาอุทิศ',
    'บ้านไร่นาดี',
    'บ้านห้วยยาง',
    'บ้านดงหนองเหียน',
    'บ้านเหล่าโพนค้อเหล่าราษฎร์วิทยา',
    'บ้านโนนกุง',
    'บ้านเชียงสือราษฎร์อำนวย',
    'นาแก้วพิทยาคม',
    'บ้านกลางนาเดื่อ',
    'บ้านหนองกระบอกราษฎร์อุทิศวิทยา',
    'หนองผือเทพนิมิต',
    'บ้านโคกแก้ว',
    'บ้านนาเดื่อ',
    'บ้านนาตงสหราษฎร์อุทิศ',
    'บ้านป่าผาง',
    'บ้านปุ่งสหราษฎร์บำรุง',
    'โพนงามโคกวิทยาคาร',
    'บ้านแป้นสามัคคีราษฎร์บำรุง',
    'ท่าศาลาราษฎร์วิทยา',
    'บ้านน้ำพุคุรุราษฎร์สงเคราะห์',
    'โพนบกผดุงศาสตร์',
    'อนุบาลโพนนาแก้ว',
    'บ้านโพนวัฒนาวิทยา',
    'บ้านวังปลาเซือม',
    'บ้านนาจาน',
    'บ้านกกปลาซิวนาโด่',
    'บ้านหนองครองราษฎร์ประสงค์',
    'บ้านโคกภู',
    'เขื่อนน้ำพุง (สาขาท่าเจริญ)',
    'บ้านบ่อเดือนห้า',
    'อนุบาลภูพาน',
    'บ้านยางโล้นเจริญราษฎรไพบูลย์',
    'เขื่อนน้ำพุง',
    'บ้านสร้างแก้วราษฎร์พัฒนา',
    'ชุมชนบ้านสร้างค้อ',
    'บ้านชมภูพาน',
    'บ้านสะพานสาม',
    'บ้านใหม่พัฒนา',
    'บ้านต้อนราษฎร์ดำรงวิทย์',
    'น้ำพุงจัดระเบียบวิทยา',
    'บ้านฮ่องสิมประชาสรร',
    'บ้านหลุบเลา',
    'สกลราชวิทยานุกูล',
    'ดงมะไฟวิทยา',
    'สกลนครพัฒนศึกษา',
    'ธาตุนารายณ์วิทยา',
    'ท่าแร่ศึกษา',
    'สกลราชวิทยานุกูล 2',
    'เฉลิมพระเกียรติพระบาทสมเด็จพระเจ้าอยู่หัวทรงครองรา',
    'กุสุมาลย์วิทยาคม',
    'โพธิแสนวิทยา',
    'เต่างอยพัฒนศึกษา',
    'ร่มเกล้า',
    'ด่านม่วงคำพิทยาคม',
    'โพนพิทยาคม',
    'คำเพิ่มพิทยา',
    'สาขาคำเพิ่มพิทยา',
    'กุดบากราษฎร์บำรุง',
    'บ้านกุดแฮดวัฒนราษฎร์ผดุง',
    'บ้านบัวคุรุราษฎร์สามัคคี',
    'ชุมชนบ้านกุดไห',
    'บ้านกลาง "ผดุงราษฎร์วิทยา"',
    'บ้านค้อน้อย',
    'บ้านค้อใหญ่',
    'บ้านงิ้ว',
    'บ้านนาม่องดงนิมิตวิทยา',
    'บ้านกุดน้ำใส',
    'บ้านหนองค้าดารกานนท์วิทยา',
    'บ้านนาขาม',
    'บ้านโพนงามคุรุราษฎร์วิทยา',
    'บ้านหนองสะไนราษฎร์รังสฤษฏ์',
    'บ้านเชิงดอย',
    'บ้านดอนม่วย',
    'บ้านหนองเดิ่นดอนขาว',
    'บ้านช้างมิ่ง',
    'บ้านหนองโดกนาคำโนนธาตุ',
    'บ้านอุ่มไผ่',
    'บ้านตาลเลียน',
    'บ้านเชิงชุม',
    'บ้านโนนพอก',
    'บ้านกุดน้ำขุ่น',
    'บ้านนาขามผดุงวิทยา',
    'บ้านถ่อน',
    'บ้านห้วยบุ่นนาทัน',
    'บ้านหนองผือ',
    'บ้านนาเลา',
    'บ้านอูนดง',
    'บ้านนาใน',
    'บ้านผักคำภู',
    'บ้านโนนเรือตอเรือ',
    'ไทยรัฐวิทยา 62 (บ้านนาหัวบ่อบัวหนองเม็ก)',
    'บ้านกลางหนองดินดำ',
    'บ้านนาสาวนาน',
    'ชุมชนบ้านนาดีหนองไผ่',
    'บ้านภูเพ็ก',
    'บ้านบะฮี',
    'บ้านท่าสองคอน',
    'บ้านนาตากาง',
    'บ้านสร้างหินวิทยานุเคราะห์',
    'บ้านบะหัวเมย',
    'อนุบาลพรรณานิคม',
    'วัดสุทธิมงคล',
    'บ้านม่วงไข่บ้านเม็ก(โพธิ์ชัยวิทยา)',
    'บ้านบะทองนาหัวช้าง',
    'พรรณาวิทยา(สาขาบ้านหนองทุ่ม)',
    'ชุมชนบดมาดพอกน้อย',
    'บ้านคางฮุง',
    'บ้านสมสะอาด',
    'บ้านพอกใหญดอนต้นม่วง',
    'บ้านสูงเนินสามัคคี',
    'บ้านทิดไทย',
    'บ้านคำแหว',
    'บ้านไร่บ้านไฮ่',
    'บ้านคำข่า',
    'บ้านโนนอุดม',
    'บ้านโคก',
    'บ้านเสาขวัญกุดก้อม',
    'บ้านหินแตก',
    'บ้านวังยาง(วังยางวิทยานุกูล)',
    'บ้านบึง',
    'บ้านหนองหวาย',
    'บ้านเปือย',
    'บ้านขมิ้น',
    'บ้านบัว (สระพังวิทยา)',
    'บ้านสว่าง',
    'บ้านดอนกอย',
    'บ้านโนนทรายคำ',
    'บ้านนาถ่อน',
    'บ้านต้นผึ้ง',
    'บ้านโพนสวาง',
    'บ้านนาล้อม',
    'บ้านขัวขอนแคน',
    'บ้านดอนดู่',
    'บ้านโนนขมิ้น',
    'บ้านโคกสามัคคี',
    'บ้านพังโคน(จำปาสามัคคีวิทยา)',
    'บ้านนาเหมือง',
    'บ้านดอนตาลโนนสูง',
    'บ้านหนองหญ้าปล้อง',
    'บ้านสร้างขุ่ย',
    'ชุมชนดงม่วงไข่',
    'บ้านผ้าขาวโพนแพง',
    'บ้านม่วงคำ',
    'บ้านดอนหวาย',
    'บ้านนาแยง',
    'บ้านหนองบัว',
    'บ้านแร่',
    'บ้านสมสะอาด',
    'บ้านหนองแคนโคกสะอาด',
    'บ้านหนองไฮ',
    'บ้านหนองบัว(สาขานาเชือก)',
    'ชุมชนบ้านฝั่งแดง',
    'บ้านท่าลาด',
    'บ้านสงเปลือย',
    'บ้านไฮหย่อง (ภูเงินประชานุกูล)',
    'บ้านอุ่มเหม้า',
    'บ้านดงสวรรค์หนองนกกด',
    'บ้านค้อเขียวพิทยาภูมิ',
    'บ้านดงบัง',
    'บ้านโคกตาดทอง',
    'บ้านดอนส้มโฮงวิทยาคม',
    'บ้านทุ่งเชือก',
    'บ้านหนองกุง',
    'บ้านหนองแปน',
    'บ้านคำบ่อ',
    'บ้านคำบิดโคกโพนยาง',
    'บ้านตาดภูวง',
    'บ้านดงคำโพธิ์',
    'บ้านดอนยานาง',
    'บ้านโคกศาลา',
    'บ้านผักตบ',
    'บ้านไฮ่ปลาโหล',
    'บ้านนาบ่อ',
    'บ้านงิ้วพังฮอ',
    'บ้านโพนไผ่',
    'บ้านห้วยบาง',
    'บ้านเหล่าโพนทองขอนขว้าง',
    'บ้านหนองแวง',
    'วาริชภูมิพิทยาคาร',
    'บ้านธาตุกุดพร้าว',
    'บ้านกุดตะกาบ',
    'บ้านจำปาศิริราษฎร์',
    'บ้านหนองลาดวิทยาคาร',
    'บ้านดอนยาวประชากรอุปการ',
    'บ้านห้วยเหล็กไฟ',
    'บ้านดงสว่าง',
    'บ้านหนองเบญจ',
    'บ้านสุวรรณคาม',
    'บ้านอูนโคก',
    'บ้านหนองบัวบาน',
    'บ้านโคกมะนาว',
    'บ้านหนองผักเทียม',
    'ชุมชนบ้านหนองปลิง',
    'บ้านค้อใต้',
    'บ้านขาม(ขามเขตวิทยาคาร)',
    'บ้านหนองแคหนองโข่ย(อำนวยศิลป์)',
    'ชุมพลศึกษา',
    'คำเจริญวิทยา',
    'บ้านกุดจิกนาสมบูรณ์',
    'บ้านนาดินจี่ (รวมธรรมราษฎร์บำรุง)',
    'บ้านยางคำ',
    'บ้านคำสะอาด',
    'บ้านปลวกธาตุโสภาวิทยา',
    'บ้านหนองหมากแซว',
    'บ้านดงจันทูหนองไผ่(ราษฎร์บำรุง)',
    'บ้านโคกสี(จตุรภูมิพิทยา)',
    'บ้านโคกคอนดอนม่วย',
    'บ้านบึงโน',
    'บ้านหนองไผ่',
    'บ้านท่าสะอาด(สวรรค์คงคา)',
    'บ้านตาล',
    'บ้านตาลโกน',
    'บ้านเล่า(ประชานุเคราะห์)',
    'บ้านหนองดินดำ',
    'บ้านตาลเนิ้ง (โพธิราชอำนวย)',
    'บ้านหนองหลักช้าง',
    'บ้านนาเตียง',
    'บ้านหนองหวาย',
    'ทรายมูลหนองกุงทรายศรีพิทยา',
    'บ้านเตาไหสร้างแก้ว',
    'บ้านม้า',
    'บ้านดอนยานาง',
    'บ้านคำเม็ก(พัฒนะบำรุง)',
    'บ้านหนองหว้า',
    'บ้านหินโงมโนนสร้างไพ',
    'บ้านโคกหลวง',
    'บ้านคันชา',
    'บ้านหนองหอย',
    'บ้านธาตุ',
    'บ้านบงใต้โนนรัง',
    'บ้านยางงาม(สาครเขตอุดม)',
    'บ้านบ่อร้าง (ผลานิวรรต)',
    'บ้านนาดอกไม้',
    'บ้านโคกหนองกุง',
    'บ้านคำนาดี',
    'ชุมชนบงเหนือ',
    'บ้านดอนหัน',
    'บ้านขาว',
    'บ้านบาก',
    'บ้านจำปานาถ่อน',
    'บ้านโคกสวัสดี',
    'บ้านโนนสะอาด',
    'บ้านหนองกุง',
    'บ้านต้ายนาคูณวิทยา',
    'บ้านดอนม่วงไข่',
    'บ้านคำไชยวาน',
    'บ้านนาทม',
    'บ้านคำสะแนน',
    'บ้านหนามแท่ง',
    'บ้านถ่อน "คุรุราษฎร์สามัคคี"',
    'บ้านหนองบัวแพ',
    'บ้านคำเจริญราษฎร์บำรุง',
    'บ้านหนองตาล',
    'บ้านทุ่งปลากัด',
    'บ้านหนองหอยคัน',
    'บ้านโมน(เกาะแก้ววิทยา)',
    'บ้านพันนา',
    'บ้านถ่อน ต.พันนา',
    'บ้านคำตานา (อรัญวาสวิทยา)',
    'บ้านโพนสูง',
    'บ้านค้อโพนสวางยางชุม',
    'บ้านดอนเชียงยืน',
    'บ้านคำชนดงต้อง',
    'บ้านนางัว',
    'บ้านโคกสีไค',
    'ชุมชนบ้านสร้างดู่ดอนเขือง',
    'บ้านแวง',
    'บ้านโนนเสาขวัญ',
    'บ้านโพนงาม',
    'บ้านสร้างแป้น',
    'บ้านหนองชาด',
    'บ้านง่อนหนองพะเนาว์(มิตรภาพที่',
    'บ้านดงสวรรค์(คุรุราษฎร์รังสรรค์)',
    'อนุบาลสว่างแดนดิน',
    'บ้านหนองทุ่ม(พุทธานุเคราะห์)',
    'บ้านเปือยทานตะวันพิทยาสรรพ์',
    'บ้านหวาย',
    'หนองหลวงวิทยานุกูล',
    'บ้านโคกดินแดง',
    'บ้านโคกสำราญ',
    'บ้านชุมชัยหนองย่างชิ้น',
    'บ้านชัยชนะ',
    'ชุมชนบ้านท่าศิลา',
    'บ้านภูตะคาม',
    'บ้านสีสุกห้วยโมง',
    'บ้านคำก้าว',
    'บ้านท่าวารี',
    'บ้านปทุมวาปี',
    'บ้านหนองแวง (ประชาราษฎร์อำนวย)',
    'บ้านโพนสวาง',
    'บ้านบ่อแกใหญ่(มิตรภาพที่ 36)',
    'อภัยดำรงธรรม',
    'บ้านหนองม่วง',
    'บ้านโคกสะอาด',
    'จงกลกิตติขจรวิทยา',
    'บ้านดงแสนตอ(ผดุงราษฎร์วิทยาคาร)',
    'บ้านกุดแสง(ธาราประชาบำรุง)',
    'บ้านไทยเจริญ',
    'อุดมสังวรวิทยา',
    'ชุมชนส่องดาว',
    'หนองใสพรเจริญวิทยา',
    'บ้านทันสมัย',
    'บ้านหนองกุง',
    'พังโคนวิทยาคม',
    'กุดบากพัฒนาศึกษา',
    'พรรณาวุฒาจารย์',
    'บ้านบัวราษฎร์บำรุง',
    'ลำปลาหางวิทยา',
    'ร่มไทรวิทยา',
    'เทพสวัสดิ์วิทยา',
    'ภูริทัตต์วิทยา',
    'บะฮีวิทยาคม',
    'ช้างมิ่งพิทยานุกูล',
    'ศึกษาสงเคราะห์สกลนคร',
    'มัธยมวาริชภูมิ',
    'ธรรมบวรวิทยา',
    'วาริชวิทยา',
    'นิคมน้ำอูนเจริญวิทยา',
    'เตรียมอุดมศึกษา ภาคตะวันออกเฉียงเหนือ',
    'แวงพิทยาคม',
    'หนองหลวงศึกษา',
    'โคกสีวิทยาสรรค์',
    'บงเหนือวิทยาคม',
    'สว่างแดนดิน',
    'ส่องดาววิทยาคม',
    'บ้านกุดเรือคำ',
    'บ้านจำปาดง',
    'บ้านห้วยน้ำเที่ยง',
    'บ้านขัวก่าย',
    'บ้านขาม',
    'บ้านวังเวิน',
    'บ้านส้งเปือย',
    'บ้านหนองนาแซงโนนงิ้ว',
    'บ้านโคกถาวร',
    'บ้านคอนศรีบะสะแบง',
    'บ้านนาโพธิ์',
    'บ้านคำเจริญสามแยกราษฎร์บำรุง',
    'บ้านคูสะคาม',
    'บ้านหนองปลาหมัดบัวงาม',
    'บ้านโคกแสง',
    'บ้านเดื่อศรีคันไชย',
    'บ้านขุนภูมิ',
    'บ้านยางคำ',
    'บ้านโนนแพง',
    'ชุมชนบ้านโพธิ์ตาก',
    'บ้านวังหว้า',
    'บ้านนาอวนน้อย',
    'บ้านโคกกลาง',
    'บ้านโคกสว่าง',
    'บ้านท่าศรีไคล',
    'บ้านธาตุตาลเดี่ยว',
    'บ้านหนองตากวย',
    'บ้านท่าเยี่ยมโนนคอม',
    'บ้านหนองฮาง',
    'บ้านโนนแต้',
    'บ้านกุดจิก',
    'บ้านหนองขุ่น',
    'บ้านหนองม่วง',
    'บ้านดอนมุยหนองแฝก',
    'บ้านกุดเรือ',
    'บ้านหนองแสงคำ',
    'บ้านนาซอ',
    'บ้านหนองนาหารสมสนุกวิทยา',
    'บ้านดอนยานาง',
    'บ้านหินเหิบ',
    'บ้านสุวรรณคีรี',
    'บ้านห้วยโทงดอนสวรรค์',
    'บ้านโพธิ์ชัยแหลมทอง',
    'บ้านปลาหลาย',
    'บ้านนาง่ามเล้า',
    'บ้านวานรนิวาส(ราษฎร์บำรุง)',
    'บ้านวังบง',
    'ชุมชนขัวสูงสวรรค์',
    'บ้านโนนอุดม',
    'บ้านดอนแดง',
    'บ้านแก้ง',
    'บ้านโคกสะอาด',
    'บ้านศรีวิชัย',
    'บ้านห้วยแสง',
    'บ้านห้วยหินลาด',
    'บ้านห้วยหิน',
    'บ้านโนนทับช้าง',
    'บ้านวังเยี่ยม',
    'บ้านโพนสวาง',
    'บ้านหนองแวง',
    'บ้านโนนเจริญศิลป์',
    'บ้านนาบัว',
    'บ้านโคกก่องหนองแวง',
    'บ้านนาจาร',
    'บ้านตาดโตน',
    'บ้านบะป่าคา',
    'บ้านน้ำบุ้น',
    'บ้านเชียงเพ็ง',
    'บ้านทุ่งโพธิ์',
    'บ้านหนองสนม',
    'บ้านโพนแพง',
    'บ้านโนนชนะสังคม',
    'บ้านนาคอย',
    'บ้านก่อ',
    'บ้านบะนกทา',
    'บ้านนายม',
    'บ้านวังโพน',
    'บ้านอินทร์แปลง',
    'บ้านโคกก่องคูสะคาม',
    'บ้านคำตากล้า',
    'บ้านหนองพอกน้อย',
    'บ้านหนองเหมือดเมี่ยง',
    'บ้านผาศักดิ์ทรัพย์เจริญ',
    'บ้านแสนสุขสามัคคี',
    'บ้านสันติสุขสระแก้ว',
    'ชุมชนพอกใหญ่ไทยสมบูรณ์',
    'บ้านหนองแสง',
    'บ้านดอนคำ',
    'บ้านนาแต้',
    'แพดพิทยารัตน์',
    'บ้านตาด',
    'บ้านกุดจาน',
    'บ้านดงบัง',
    'บ้านดงอีด่อย',
    'บ้านหนองบัวสิม',
    'บ้านนาถ่อน',
    'บ้านเพีย',
    'บ้านท่างาม',
    'บ้านดงห้วยเปลือย',
    'นาข่าวิทยา',
    'บ้านหนองลาด',
    'บ้านหนองท่มท่ากระดัน',
    'บ้านซ่อมดู่',
    'บ้านขี้เหล็กเหล่าสมบูรณ์',
    'บ้านด่านสมบูรณ์',
    'ขาวสง่าเจริญวิทย์',
    'บ้านดงหม้อทอง',
    'บ้านคำยาง',
    'บ้านดงเหนือ',
    'บ้านดอนแดงคำอ้อ',
    'บ้านท่าช้างเจริญไพศาล',
    'บ้านคำปลาฝา',
    'บ้านหนองแอก',
    'บ้านน้ำจั้น',
    'บ้านโนนไทย',
    'บ้านสามแยกพิทักษ์',
    'บ้านโนนสะอาดนาเหมือด',
    'ชุมชนนาเต่าสุขสวรรค์(ประชาราษฎ',
    'บ้านพุทธรักษา',
    'บ้านคำภูทอง',
    'สร้างแก้วอ่างคำค้ออำนวยวิทย์',
    'บ้านลึมบอง',
    'บ่อแก้วดงมะไฟมิตรภาพที่ 81',
    'บ้านหว้าน',
    'บ้านสรศรี',
    'บ้านจาร',
    'บ้านดงยาง',
    'อนุบาลบ้านม่วง',
    'บ้านนาดอกไม้',
    'ชุมชนบ้านมาย',
    'นาจานกล้วยน้อย',
    'บ้านหนองบ่อ',
    'บ้านโพธิ์ชัย',
    'บ้านบ่อแดง',
    'บ้านหนองกวั่ง',
    'บ้านคำลอดพื้น',
    'บ้านนาดูนนาดี',
    'บ้านโนนแสบง',
    'ชุมชนบ้านห้วยหลัว',
    'บ้านห้วยทราย',
    'บ้านเหล่าผักใส่',
    'บ้านท่าก้อน',
    'บ้านดอนแดง',
    'บ้านท่าควาย',
    'บ้านนาหวาย',
    'บ้านโพนก่อ',
    'บ้านโนนสวรรค์',
    'บ้านสงเปือย',
    'บ้านนากะทาด',
    'บ้านนาฮี',
    'บ้านบะหว้า',
    'บ้านบะยาว',
    'บ้านวังม่วง',
    'บ้านเม่นน้อย',
    'บ้านดงสาร',
    'บ้านเสาวัด',
    'บ้านท่าแร่',
    'บ้านดงเสียว',
    'บ้านดงมะกุ้ม',
    'ชุมชนบ้านโพนงาม',
    'ชุมชนบ้านกลางนาโน',
    'บ้านแพงน้อย',
    'บ้านคึม',
    'บ้านเซือม',
    'บ้านแพงใหญ่',
    'บ้านข้าวแป้ง',
    'บ้านกุดจอกน้อย',
    'บ้านโคกไม้ล้ม',
    'บ้านวาใหญ่',
    'บ้านวาน้อย',
    'บ้านหนองบัวแดง',
    'บ้านกุดจอก',
    'บ้านหนองสามขา',
    'บ้านถ้ำเต่า',
    'บ้านนาดอกไม้',
    'บ้านหนองผือ',
    'บ้านดอนทอย',
    'บ้านนายอ',
    'บ้านหนองตาไก้',
    'บ้านดอนแดงเม่นใหญ่',
    'บ้านอากาศ',
    'บ้านนาเมือง',
    'บ้านโคกศิลา',
    'บ้านหนองทุ่มหนองโจด',
    'บ้านคำบอน',
    'อนุบาลเจริญศิลป์',
    'บ้านทุ่งคำ',
    'บ้านสร้างฟาก',
    'บ้านนาดี',
    'บ้านกุดนาขาม(เชิดชูวิทยาสาร)',
    'บ้านแกดำ',
    'บ้านหนองฮังแหลว',
    'บ้านทุ่งแก',
    'บ้านโสก',
    'บ้านดอนชัยวิทยา',
    'บ้านทุ่งมนธาตุวิทยา',
    'บ้านหนองจาน',
    'บ้านหนองน้อย',
    'บ้านหนองแสง',
    'บ้านเหล่าบ้านถ่อน',
    'บ้านหนองแวงน้อย',
    'บ้านดงบาก',
    'บ้านโพนบกหนองผือ',
    'บ้านดงบัง(คุรุราษฎร์อุทิศ)',
    'บ้านคำเม็ก',
    'บ้านนาสีนวล',
    'เจริญพัฒนา',
    'บ้านดงสง่า',
    'บ้านหนองแปน',
    'มัธยมวานรนิวาส',
    'คำตากล้าราชประชาสงเคราะห์',
    'กุดเรือคำพิทยาคาร',
    'ศึกษาประชาสามัคคี',
    'ธาตุทองอำนวยวิทย์',
    'ภูดินแดงวิทยา',
    'หนองแวงวิทยา',
    'เดื่อศรีไพรวัลย์',
    'บ้านม่วงพิทยาคม',
    'โพธิ์ชัยทองพิทยาคม',
    'คำยางพิทยาคม',
    'อากาศอำนวยศึกษา',
    'โพนงามศึกษา',
    'ท่าสงครามวิทยา',
    'เจริญศิลป์ศึกษา(โพธิ์คำอนุสรณ์)',
    'บ้านกุรุคุ',
    'บ้านหนองบัวกุรุคุ',
    'บ้านหนองหญ้าไซ',
    'รอดโพธิ์ทองวิทยาสรรค์',
    'บ้านไทยสามัคคี',
    'บ้านนาป่ง',
    'บ้านกล้วย',
    'บ้านชะโนต',
    'บ้านชะโงมนาโดน',
    'บ้านขามเฒ่า',
    'บ้านคำเตย',
    'บ้านโพนค้อโพนสวรรค์',
    'บ้านหนองยาว',
    'บ้านหนองดินแดง',
    'บ้านดอนแดง',
    'บ้านวังไฮหนองกุง',
    'บ้านทุ่งมน',
    'บ้านโพนป่าหว้าน',
    'บ้านนาคอกควาย',
    'บ้านกกไฮ',
    'บ้านดงหมู',
    'บ้านนาหลวง',
    'บ้านท่าค้อ(ชุณห์ประสาทศิลป์)',
    'บ้านหนองจันทน์',
    'บ้านนาทราย',
    'บ้านนามูลฮิ้น',
    'บ้านดอนยานาง',
    'บ้านนาคำโนนขาม',
    'บ้านนาราชควาย',
    'บ้านเนินสะอาด',
    'บ้านนาราชควายน้อย',
    'บ้านสร้างหิน',
    'บ้านหนองบัวนาราชควาย',
    'อนุบาลนครพนม',
    'บ้านน้อยใต้',
    'สุนทรวิจิตร',
    'บ้านดงยอ',
    'บ้านกลางน้อย',
    'บ้านดงติ้ว',
    'บ้านกลาง',
    'บ้านดงต้อง',
    'ชุมชนนามนวิทยาคาร',
    'บ้านผึ้ง',
    'บ้านวังกระแส',
    'บ้านสุขเจริญ',
    'บ้านดงสว่าง',
    'บ้านหนองปลาดุก',
    'บ้านดอนม่วง',
    'บ้านหนองบัวหน้าฐานบิน',
    'บ้านโพธิ์ตาก',
    'บ้านสุขเกษม',
    'บ้านหนองค้าโคกกุง',
    'บ้านโคกก่อง',
    'บ้านคำสว่าง',
    'บ้านหนองแซง',
    'บ้านโชคอํานวย',
    'บ้านวังตามัว',
    'เมืองนครพนม',
    'บ้านหนองญาติ',
    'บ้านดงโชค',
    'บ้านคำพอก',
    'บ้านเหล่าภูมี-คำธาตุ',
    'ชุนชนอาจสามารถ',
    'บ้านสําราญ',
    'บ้านไผ่ล้อม',
    'บ้านคำเกิ้ม',
    'บ้านนาหัวบ่อ',
    'บ้านกุตาไก้',
    'บ้านวังโพธิ์',
    'บ้านดอนดู่',
    'บ้านโพนทา',
    'บ้านนางาม',
    'บ้านนาดอกไม้',
    'บ้านโคกสว่าง',
    'บ้านวังกะเบา',
    'บ้านนาสีนวล',
    'บ้านศรีธน',
    'บ้านสว่างสำราญ',
    'บ้านโคกสูง',
    'บ้านม่วง',
    'บ้านนกเหาะ',
    'บ้านหนองบัวคำ',
    'โพนสวางนางิ้ววิทยา',
    'บ้านนามะเขือ',
    'บ้านหนองแสง',
    'ปลาปากราษฎร์บำรุง',
    'บ้านนาขาม',
    'บ้านวังยาง',
    'บ้านกอก',
    'บ้านหนองบัว',
    'บ้านโคกกลาง',
    'บ้านใหม่วังเซือม',
    'บ้านปลาปากน้อย',
    'บ้านวังสิม',
    'บ้านห้วยไหล่',
    'บ้านถาวรนาอุดม',
    'บ้านมหาชัย',
    'บ้านทันสมัย',
    'บ้านหนองบัวราษฎร์รัฐบำรุง',
    'บ้านวังม่วง',
    'บ้านดอนกลาง',
    'บ้านหนองหมากแก้ว',
    'บ้านตับเต่าหนองเทาสามัคคี',
    'บ้านนาเชือก',
    'ชุมชนหนองฮีสามัคคี',
    'บ้านนาสะเดา',
    'บ้านโพนทัน',
    'บ้านจรุกเตย',
    'บ้านกุงโกน',
    'บ้านหนองกกคูณ',
    'บ้านสร้างแห่',
    'บ้านกุดฉิม',
    'บ้านดอนนางหงส์(หงส์ทองฯ)',
    'ดอนนางหงส์สงเคราะห์',
    'บ้านหนองสะโน',
    'บ้านนาแกบึงเหล็ก',
    'บ้านโคกสว่าง(เรืองศิษย์วิทยาฯ)',
    'วัดพระธาตุพนม "พนมวิทยาคาร"',
    'บ้านธาตุพนม (พิทักษ์วิทยา)',
    'บ้านหัวดอน(สำนักงานสลากฯ)',
    'บ้านหัวบึงทุ่ง(เขตการทางฯ)',
    'บ้านดอนแดง',
    'นาถ่อนวิทยานุกูล',
    'นาถ่อนท่าวิทยาคาร',
    'บ้านดงยอ',
    'บ้านโพนบกหัวขัววิทยาคาร',
    'บ้านดงก้อม',
    'บ้านดงป่ายูง',
    'บ้านหนองหญ้าม้า',
    'บ้านขอนขว้าง',
    'บ้านขอนกองโพนทอง',
    'บ้านนาหนาด',
    'บ้านคำชะอี',
    'บ้านดอนข้าวหลาม',
    'บ้านน้ำก่ำ(ล้ำประชาอุปถัมภ์)',
    'บ้านน้ำก่ำ (สิทธิผลนุกูล)',
    'บ้านนาคำ',
    'บ้านคับพวง',
    'บ้านทรายมูลคำผักแพว',
    'บ้านทู้(สามัคคีวิทยา)',
    'บ้านแก่งโพธิ์',
    'บ้านต้อง(ศรีบัวบานวิทยาคาร)',
    'บ้านฝั่งแดง',
    'บ้านโปร่งหนองเปงใหม่จำปา',
    'พระกลางวิทยาคาร',
    'มรุกขนคร',
    'บ้านนาทาม',
    'ประชามิตรวิทยา',
    'บ้านหนองกุดแคน',
    'บ้านโพนแพง',
    'บ้านห้วยทราย',
    'บ้านตาลกุดยางเดี่ยวโนนสมบูรณ์',
    'บ้านแสนพันหมันหย่อน',
    'บ้านดงขวาง',
    'อุ่มเหม้าวิทยาคาร',
    'บ้านดอนสวรรค์',
    'บ้านโสกแมว',
    'บ้านชาติพัฒนาสันติสุข',
    'บ้านโคกหินแฮ่',
    'บ้านหนองแซง',
    'บ้านนาโดนใหม่',
    'บ้านนาบัว',
    'บ้านนาม่วง',
    'บ้านหนองกุง',
    'บ้านท่าลาด',
    'บ้านวังยาง',
    'บ้านสร้างเม็ก',
    'ลาดสว่างวิทยา',
    'บ้านดอนขาว',
    'บ้านนาขาม',
    'บ้านเหิบ',
    'ชายแดนอนุสรณ์',
    'บ้านนายอ',
    'นางามวิทยาคาร',
    'บ้านสร้างแป้น',
    'บ้านดอนกกโพธิ์ค่ายเสรีวิทยา',
    'บ้านนายอน้อย',
    'บ้านโนนสวรรค์',
    'บ้านเหล่าสำราญ',
    'บ้านดงบาก',
    'บ้านหนองลาดควาย',
    'โพนทองวิทยาคาร',
    'บ้านดงมะเอก',
    'บ้านคำผาสุก',
    'เรณูวิทยาคาร',
    'บ้านโนนสังข์',
    'บ้านโพนสาวเอ้',
    'บ้านนาบั่ว',
    'บ้านโคกกลาง',
    'บ้านบ่อสะอาด',
    'บ้านนาโดนเก่า',
    'บ้านทุ่งสว่าง',
    'ชุมชนบ้านหนองย่างซี้น',
    'บ้านหัวขัวใต้',
    'บ้านนาดี',
    'บ้านดอนโทน',
    'ศรีโพนทองวิทยา',
    'ดอนเหลืองทองวิทยา',
    'บ้านจำปา',
    'ไทยรัฐวิทยา 50 (บ้านคำพี้)',
    'นาผือบอนราษฎร์นุกูล',
    'บ้านกลางมูลอ้น',
    'นาแกผดุงราชกิจเจริญ',
    'บ้านโคกกลางแก้งน้อยฯ',
    'บ้านต้นแหน(ราษฎร์ต้นแหนฯ)',
    'บ้านนาป่งคอง',
    'บ้านนาคู่',
    'บ้านแขนนาง',
    'บ้านน้ำบ่อ',
    'บ้านนาฉันทะญาณวีโรอนุสรณ์',
    'บ้านนาเลียง',
    'บ้านหนองคอง',
    'บ้านนามนดงติ้ววิทยา',
    'นาฮุมอุดมสวรรค์สามัคคี',
    'บ้านนาแกน้อย กรป.กลางฯ',
    'บ้านแก้ง',
    'บ้านโพธิ์ศรี',
    'บ้านนาโสก',
    'บ้านนาทุ่งมั่ง',
    'บ้านหนองยอ',
    'บ้านหนองหอยคำอ้อม',
    'บ้านบ่อดอกซ้อน',
    'บ้านพระซองเหนือ',
    'บ้านนาเหนือ',
    'บ้านดงอินำ',
    'พระซองวิทยาคาร',
    'ธารน้ำใจ',
    'ชุมชนบ้านพิมาน',
    'บ้านพิมานท่า',
    'บ้านดงน้อยพัฒนา',
    'บ้านจอมมณี',
    'บ้านปากบัง',
    'ชุมชนบ้านพุ่มแก',
    'บ้านหนองหอยใหญ่',
    'บ้านนางเลิศ',
    'บ้านหนองกุง (ฮินดูสมาช)',
    'บ้านตับเต่าโนนจันทร์',
    'บ้านโพนดู่โคกสวัสดิ์',
    'บ้านสีชมพูมิตรภาพที่ 164',
    'บ้านหนองหญ้าปล้องขัวสูง',
    'บ้านเหล่าทุ่ง',
    'บ้านดงดู่งาม',
    'บ้านหนองบ่อ',
    'บ้านดินดำหมากเฟือง',
    'บ้านนากุงยางคำ',
    'บ้านหนองห้าง',
    'บ้านคำเม็ก',
    'บ้านหนองกุง',
    'หนองสังข์ราษฎร์บำรุง',
    'บ้านหนองผักแว่น',
    'บ้านโคกสีมิตรภาพที่ 87',
    'บ้านโนนหอม',
    'บ้านยอดชาดวิทยา',
    'บ้านผักขะย่านาคอยโนนแดง',
    'บ้านหนองบึง',
    'บ้านหนองแคน',
    'บ้านวังยาง',
    'บ้านนาขามส้มป่อย',
    'บ้านหนองนางด่อน',
    'บ้านหนองสะโน',
    'บ้านสามแยกหัวภูธร',
    'บ้านหนองโพธิ์วังโน',
    'บ้านโพนสวาง',
    'ปิยะมหาราชาลัย',
    'คำเตยอุปถัมภ์',
    'นครพนมวิทยาคม',
    'ศรีโคตรบูรณ์',
    'บ้านผึ้งวิทยาคม',
    'ศรีบัวบานวิทยาคม',
    'วังกระแสวิทยาคม',
    'ปลาปากวิทยา',
    'กุตาไก้วิทยาคม',
    'มัธยมพัชรกิติยาภา 1 นครพนม',
    'ธรรมโฆษิตวิทยา',
    'โคกสว่างประชาสรรค์',
    'มหาชัยวิทยาคม',
    'ธาตุพนม',
    'อุ่มเหม้าประชาสรรค์',
    'โพนแพงพิทยาคม',
    'กุดฉิมวิทยาคม',
    'วัดบึงเหล็ก',
    'นาถ่อนพัฒนา',
    'เรณูนครวิทยานุกูล',
    'ดงดาวแจ้งพัฒนศึกษา',
    'นาแกพิทยาคม',
    'พระซองสามัคคีวิทยา',
    'หนองบ่อวิทยานุกูล',
    'ธรรมากรวิทยานุกูล',
    'นาแกสามัคคีวิทยา',
    'วังยางวิทยาคม',
    'หนองโพธิ์พิทยาคม',
    'ชุมชนบ้านไชยบุรี',
    'บ้านตาลปากน้ำ',
    'บ้านนาเพียง',
    'บ้านนาหนองบก',
    'บ้านวังโพธิ์',
    'บ้านหาดกวน',
    'บ้านแก้วปัดโป่ง',
    'บ้านดอนดู่',
    'ชุมชนบ้านคำพอกท่าดอกแก้ว',
    'บ้านท่าจำปา',
    'บ้านห้วยพระ',
    'บ้านดอนแดง',
    'บ้านคำเตย',
    'บ้านดอนติ้ว',
    'บ้านนาดอกไม้',
    'บ้านท่าแต้ปุ่งแก',
    'บ้านท่าอุเทน',
    'บ้านน้อยหัวบึง',
    'อุเทนวิทยาคาร',
    'บ้านนาผักปอด',
    'บ้านโพน',
    'บ้านธาตุ',
    'บ้านน้อยทวย',
    'บ้านตาล',
    'บ้านพนอม',
    'บ้านดง',
    'บ้านหนองสาหร่าย',
    'บ้านเหล่าหนาด',
    'บ้านเหล่าศรีโหนโห่',
    'บ้านพะทาย',
    'บ้านนาข่าท่า',
    'บ้านโคกสว่าง',
    'บ้านบุ่ง',
    'บ้านโพนแดง',
    'ไตรราษฎร์วิทยาคาร',
    'บ้านรามราช',
    'บ้านแพงสะพัง',
    'บ้านโพนก่อ',
    'บ้านม่วงนาสีดา',
    'บ้านเวินพระบาท',
    'บ้านเชียงยืน',
    'บ้านปากทวย',
    'บ้านหนองเทา',
    'บ้านท่าหนามแก้วสวนกล้วย',
    'บ้านนาเข',
    'บ้านโคกสายทอง',
    'บ้านนางัว',
    'บ้านพืชผล',
    'บ้านนาเรียงทุ่งเจริญ',
    'บ้านแพงโคก',
    'บ้านดอนบาก',
    'บ้านแพงวิทยา',
    'บ้านนายาง',
    'บ้านท่าลาดราษฎร์สามัคคี',
    'บ้านหัวหาด',
    'ชุมชนไผ่ล้อม',
    'บ้านนาโพธิ์',
    'บ้านโพธิ์ไทรทองวิทยา',
    'บ้านโพนทอง',
    'บ้านนาดีหัวภู',
    'บ้านโคกยาว',
    'บ้านม่วงชี',
    'บ้านดอนสะฝาง',
    'บ้านโคกพะธาย',
    'บ้านป่าหว้าน',
    'บ้านหนองแวง',
    'บ้านนาพระชัย',
    'บ้านนาข่าคำพอก',
    'บ้านไชยศรี',
    'บ้านคำนกกก',
    'บ้านดอนกลาง',
    'บ้านนากระแต้',
    'บ้านดงหนองบัว',
    'บ้านดอนสมอ',
    'บ้านดอนมะจ่าง',
    'บ้านดอนแดง',
    'บ้านนาเพียง',
    'บ้านท่าบ่อ(บ่อศรีรัตนอำนวย)',
    'บ้านนาคํา',
    'บ้านโพนก่อ (ราษฏร์อนุสรณ์)',
    'บ้านคําสว่าง',
    'บ้านอุ่มไผ่',
    'บ้านภูกระแต',
    'บ้านโพนงาม',
    'บ้านเหล่า',
    'บ้านอีอูด',
    'บ้านนาเดื่อ',
    'บ้านนาชุม',
    'บ้านจอมศรี',
    'บ้านนาอินทร์ นาโอ นาคอยพัฒนา',
    'บ้านข่า',
    'บ้านขามเปี้ย',
    'ดอนถ่อนโคกกลางวิทยา',
    'ชุมชนเอื้องก่อนาดี',
    'บ้านหนองผือ',
    'บ้านเซียงเซา',
    'บ้านดงขวาง',
    'บ้านนาจาน',
    'ประชาสนธิ์นุสรณ์',
    'บ้านนาโพธิ์',
    'บ้านโพนสว่าง',
    'บ้านโพนขาว',
    'บ้านปากอูน (ปากอูนผดุงวิทย์)',
    'บ้านยางงอย',
    'บ้านหนองบาท้าว',
    'บ้านปฏิรูป',
    'บ้านแค',
    'บ้านปากยาม(สุนทรกัลยาคาร)',
    'บ้านศรีเวินชัย',
    'ชุมชนบ้านสามผง',
    'บ้านนาหนองหวาย',
    'บ้านดงน้อย',
    'บ้านคําไฮ',
    'บ้านหาดแพง (หาดแพงวิทยา)',
    'บ้านเสียวสงคราม',
    'บ้านอ้วน',
    'ราษฎร์สามัคคี',
    'บ้านนาคูณน้อยหนองหัวงัว',
    'ชุมชนประสานมิตร',
    'บ้านนากะทืม',
    'ชุมชนนางัว',
    'บ้านนาคอย',
    'บ้านอูนนา',
    'บ้านโนนกุง',
    'บ้านสามัคคี',
    'บ้านกุดน้ำใส',
    'บ้านนาหว้า',
    'บ้านตาล',
    'บ้านนาพระ',
    'บ้านอูนยางคำ',
    'บ้านหนองบัว',
    'บ้านดอนแดง',
    'บ้านเสียววิทยา',
    'บ้านดอนพะธาย',
    'บ้านโคกสะอาด',
    'บ้านเหล่าพัฒนา',
    'บ้านหนองดุด',
    'บ้านดอนศาลา',
    'บ้านนาน้อย',
    'บ้านดอนปอหนองโอง',
    'บ้านนาขมิ้น',
    'บ้านดงวิทยาคาร',
    'บ้านขามเตี้ยใหญ่',
    'บ้านขามเตี้ยน้อย',
    'บ้านดอนยางทุ่งน้อย',
    'บ้านอ้อวังหมากเห็บ',
    'บ้านนาใน',
    'บ้านนาผักหมนาหมากแงว',
    'บ้านนาล้อมกุดตะกล้า',
    'บ้านนาหัวบ่อ',
    'บ้านหนองผักตบ',
    'บ้านโพนตูมหนองแสง',
    'บ้านค้อ',
    'บ้านขว้างคลีชูชาติ',
    'บ้านหนองท่ม',
    'บ้านงิ้วสร้างแก้ว',
    'บ้านห้วยไหสมบูรณ์',
    'บ้านนาคำ',
    'บ้านนาเต่า',
    'บ้านต้าย',
    'บ้านโพนจาน',
    'บ้านบงคํา',
    'บ้านโพนบก',
    'บ้านโพนเพ็ก',
    'บ้านเหล่าบะดาโนนอุดม',
    'บ้านโพนสวรรค์',
    'บ้านเสาเล้า',
    'บ้านหนองนางเลิง',
    'บ้านท่าเรือ',
    'บ้านหนองหญ้าปล้อง',
    'บ้านหมูม้น',
    'บ้านโคกศรี',
    'บ้านคำแม่นาง',
    'บ้านดอนเตย',
    'บ้านท่าพันโฮง',
    'บ้านเหล่าส้มป่อย',
    'บ้านนาทม',
    'บ้านดอนแดงดอนดู่',
    'บ้านพันห่าวดอนดู่',
    'บ้านนาโดโพธิ์ศรี',
    'บ้านหนองซน',
    'บ้านชัยมงคล',
    'บ้านนานอ',
    'บ้านนาดีวิทยา',
    'บ้านเทพนิมิตร',
    'อุเทนพัฒนา',
    'ไชยบุรีวิทยาคม',
    'พะทายพิทยาคม',
    'รามราชพิทยาคม',
    'ท่าจำปาวิทยา',
    'เชียงยืนวิทยา',
    'บ้านแพงพิทยาคม',
    'ลังกาพิทยาคม',
    'สหราษฎร์รังสฤษดิ์',
    'สามผงวิทยาคม',
    'สนธิราษฎร์วิทยา',
    'นาคำราษฎร์รังสรรค์',
    'นาเดื่อพิทยาคม',
    'บ้านข่าพิทยาคม',
    'นาหว้าพิทยาคม(ธาตุประสิทธิ์ประชานุเคราะห์)',
    'นางัวราษฎร์รังสรรค์',
    'ดอนเสียวแดงพิทยาคม',
    'อุดมพัฒนศึกษา',
    'โพนสวรรค์ราษฎร์พัฒนา',
    'ค้อวิทยาคม',
    'นาทมวิทยา',
    'หนองซนพิทยาคม',
    'หนองแวงวิทยานุกูล',
    'บ้านกุดแข้',
    'บ้านกุดแข้ใต้',
    'บ้านดงยาง2',
    'บ้านสามขามิตรภาพ 3',
    'บ้านแก้ง1',
    'บ้านคำป่าหลาย',
    'บ้านนาคำน้อย2',
    'บ้านนาตะแบง1',
    'บ้านนาสองห้อง',
    'บ้านนาเสือหลาย',
    'บ้านคำอาฮวน',
    'บ้านคำเขือง',
    'บ้านคำเม็ก',
    'บ้านโค้งสำราญ',
    'บ้านดงมัน',
    'บ้านโนนสะอาด1',
    'บ้านพรานอ้น',
    'บ้านเหล่าคราม',
    'บ้านเหมืองบ่า',
    'บ้านนาดี2',
    'บ้านสงเปือยเหนือ',
    'บ้านห้วยยาง',
    'บ้านไร่',
    'บ้านดงมอน',
    'บ้านดงเย็น',
    'บ้านคำบง2',
    'บ้านโคกขามเลียน',
    'นราธิป-พร้อยสุพิณบ้านโคกตะแบง',
    'บ้านป่งโพน',
    'บ้านโพนสวาง',
    'บ้านสามขัว',
    'บ้านหนองแคนนาจาน',
    'บ้านบุ่งอุทัย',
    'บ้านส้มป่อย',
    'บ้านท่าไค้',
    'บ้านโนนศรี',
    'บ้านป่งเปือย',
    'บ้านนาโด่',
    'บ้านแก้งนาบอน',
    'บ้านนาโสก',
    'บ้านหนองน้ำเต้า',
    'บ้านนาหัวภู',
    'บ้านเหล่าป่าเป้ด',
    'บ้านคำผักหนอกสงเปือย',
    'บ้านดอนม่วย',
    'บ้านหนองแอก',
    'ชุมชนบางทรายใหญ่',
    'บ้านหนองหอยป่าหวาย',
    'บ้านป่าหวาย',
    'บ้านพังคอง',
    'บ้านสงเปือย',
    'บ้านหนองบัว',
    'บ้านหนองแวง',
    'บ้านโคก1',
    'บ้านคำผึ้ง',
    'บ้านจอมมณีใต้',
    'บ้านนาคำหนองแล้ง',
    'บ้านโนนตูม',
    'บ้านผึ่งแดด',
    'บ้านหนองไผ่',
    'บ้านนาถ่อน',
    'บ้านนาโสกน้อย',
    'ชุมชนโพนทราย',
    'บ้านม่วงหัก',
    'บ้านหนองหญ้าไซย์',
    'บ้านคำฮี',
    'บ้านแก่นเต่า',
    'คำสายทองวิทยา',
    'บ้านกุดโง้ง',
    'บ้านดานคำ',
    'บ้านนาคำน้อย3',
    'เมืองใหม่',
    'บ้านศูนย์ไหม',
    'บ้านนาโปน้อย',
    'มุกดาลัย',
    'บ้านนาโปใหญ่-โคกสุวรรณ',
    'อนุบาลมุกดาหาร',
    'บ้านศรีบุญเรือง',
    'บ้านคำแฮด',
    'บ้านนาหลวง2',
    'บ้านบะ',
    'บ้านป่าแดง',
    'บ้านหนองสระพัง',
    'บ้านอุ่มไผ่',
    'ชุมชนบ้านหนองแวงน้อย',
    'บ้านคำบง1',
    'บ้านคำพอก2',
    'บำรุงพงศ์อุปถัมภ์',
    'บ้านห้วยกอก1',
    'เตรียมทหารรุ่นที่13อนุสรณ์(บ้านด่านยาว)',
    'บ้านนากอก',
    'บ้านนาสองเหมือง',
    'บ้านน้ำเที่ยง2',
    'บ้านนาอุดม',
    'บ้านขอนแก่น',
    'บ้านคำไหล',
    'คณะเทศบาลนครกรุงเทพ 3',
    'บ้านป่าเตย',
    'บ้านโนนเกษม',
    'บ้านภูแผงม้า',
    'ชุมชนบ้านม่วงไข่',
    'บ้านคำสร้อย',
    'บ้านด่านมน',
    'บ้านคำนางโอก',
    'บ้านหนองนกเขียน',
    'บ้านนิคมร่มเกล้า',
    'ป่งแดงวิทยาคม',
    'บ้านเหล่าหลวงเตาถ่าน',
    'บ้านโนนสะอาด2',
    'บ้านหนองข่า',
    'บ้านหนองแวงใหญ่',
    'ชุมชนดอนตาล',
    'บ้านนาม่วง',
    'บ้านโพนสว่าง',
    'บ้านห้วยกอก2',
    'บ้านนาสะเม็ง',
    'บ้านโคกพัฒนา',
    'บ้านนาสะโน',
    'บ้านนาหว้า',
    'บ้านหนองกระยัง',
    'บ้านภูผาหอมพัฒนา',
    'บ้านแก้ง2',
    'บ้านคำดู่',
    'บ้านดง',
    'บ้านนาคำน้อย1',
    'บ้านภูวง',
    'บ้านภูล้อม',
    'บ้านหนองบอน',
    'บ้านบาก2',
    'บ้านนายาง',
    'บ้านป่าไร่',
    'บ้านห้วยทราย2',
    'บ้านหนองเม็ก',
    'บ้านนาทาม',
    'บ้านนาป่ง',
    'บ้านนามน',
    'บ้านโนนสวาท',
    'ชุมชนโพธิ์ไทร',
    'บ้านนาโพธิ์',
    'บ้านโคกหนองหล่ม',
    'บ้านเหล่าหมี',
    'บ้านโคกสว่าง1',
    'บ้านเหล่าแขมทอง',
    'บ้านท่าห้วยคำ',
    'บ้านนายอ',
    'บ้านป่าพยอม',
    'สยามกลการ4',
    'บ้านกกตูม',
    'บ้านแก้งนาง',
    'บ้านขัวสูง',
    'บ้านนาหินกอง',
    'บ้านปากช่อง',
    'สยามกลการ 5',
    'บ้านสานแว้',
    'หมู่บ้านป่าไม้',
    'บ้านชะโนด 2',
    'บ้านหนองยาง',
    'บ้านย้อมพัฒนา',
    'บ้านโสก',
    'บ้านเปียด',
    'บ้านโพนแดง',
    'บ้านดงหลวง',
    'บ้านติ้ว',
    'บ้านฝั่งแดง',
    'บ้านมะนาว',
    'บ้านหนองคอง',
    'พระราชทานบ้านหนองหมู',
    'บ้านโพนไฮ',
    'บ้านก้านเหลืองดง',
    'ร่มเกล้า',
    'ชุมชนบ้านหนองบัว',
    'บ้านน้ำบ่อดง',
    'บ้านเหล่าดง',
    'บ้านหนองหนาว',
    'ชุมชนบ้านคำชะอี',
    'บ้านกกไฮโนนน้ำคำ',
    'บ้านแก้งช้างเนียม',
    'บ้านนาปุ่ง',
    'บ้านโนนสว่าง1',
    'บ้านหนองกะปาด',
    'บ้านห้วยทราย1',
    'บ้านกลาง',
    'คำบกราษฎร์นุกูล',
    'บ้านบาก1',
    'บ้านห้วยลำโมง',
    'บ้านน้ำเที่ยงวันครู 2501',
    'บ้านหนองเอี่ยนดง"ราษฎร์สงเคราะห์"',
    'ห้วยตาเปอะ',
    'บ้านค้อ',
    'บ้านโคก2',
    'บ้านดงยาง1',
    'ไทยรัฐวิทยา11(บ้านแข้)',
    'บ้านโนนสังข์ศรี',
    'บ้านซ่ง',
    'บ้านม่วง',
    'บ้านแมด',
    'บ้านเหล่า',
    'บ้านหนองสระพังทอง',
    'บ้านดอนป่าแคน',
    'บ้านตูมหวาน',
    'บ้านแฝก',
    'บ้านโพนงาม',
    'บ้านนาหลวง1',
    'บ้านหนองบง',
    'บ้านหนองเอี่ยน',
    'บ้านโคกสว่าง2',
    'บ้านหนองไฮ',
    'บ้านเหล่าสร้างถ่อ',
    'เมืองพาลุกากรภูมิ',
    'บ้านชะโนด 1',
    'สมเด็จพระศรีนครินทราบรมราชชนนี84พรรษา',
    'บ้านขามป้อม',
    'บ้านทรายทอง',
    'ชุมชนบ้านบางทรายน้อย',
    'บ้านสองคอน',
    'บ้านป่งขาม',
    'บ้านนาขามป้อมวิทยาคม',
    'บ้านนาดีโคกสวาท',
    'บ้านหว้านใหญ่',
    'หนองผือดอนม่วง',
    'บ้านหว้านน้อย',
    'บ้านนาแพงโคกน้ำสร้าง',
    'บ้านโนนสว่าง 2',
    'บ้านโนนยาง',
    'บ้านคำพอก 1',
    'บ้านงิ้ว',
    'บ้านวังนอง',
    'บ้านหนองโอใหญ่',
    'บ้านคำพี้',
    'บ้านเป้าป่าแสด',
    'บ้านภู',
    'บ้านวังไฮ',
    'บ้านนาตะแบง 2',
    'บ้านบุ่ง',
    'บ้านโคกกลาง',
    'บ้านแวง',
    'บ้านโคกหินกอง',
    'บ้านหลุบปึ้ง',
    'บ้านเหล่าน้อย',
    'บ้านนาหนองแคน',
    'บ้านคันแท',
    'ชุมชนเมืองหนองสูง',
    'มุกดาหาร',
    'นาโสกวิทยาคาร',
    'ผึ่งแดดวิทยาคาร',
    'นวมินทราชูทิศ อีสาน',
    'คำป่าหลายสรรพวิทย์',
    'ดงเย็นวิทยาคม',
    'นาวาราชกิจพิทยานุสรณ์',
    'มุกดาวิทยานุกูล',
    'จุฬาภรณราชวิทยาลัย มุกดาหาร',
    'ดงมอนวิทยาคม',
    'เมืองมุกวิทยาคม',
    'คำสร้อยพิทยาสรรค์',
    'อุดมวิทย์',
    'ร่มเกล้าพิทยาสรรค์',
    'แวงใหญ่พิทยาสรรค์',
    'โชคชัยวิทยา',
    'ดอนตาลวิทยา',
    'โพธิ์ไทรวิทยา',
    'ผาเทิบวิทยา',
    'ดงหลวงวิทยา',
    'กกตูมประชาสรรค์ รัชมังคลาภิเษก',
    'คำชะอีวิทยาคาร',
    'คำบกวิทยาคาร',
    'ชัยปัญญาวิทยานุสรณ์',
    'คำชะอีพิทยาคม',
    'เหล่าประชาอุทิศ',
    'หว้านใหญ่วิทยา',
    'หนองสูงสามัคคีวิทยา',
    'พลังราษฎร์พิทยาสรรพ์',
    'หนองแวงวิทยาคม',
    'ศรีเนห์รู',
    'วัดช่างเคี่ยน',
    'วัดเจ็ดยอด',
    'วัดข่วงสิงห์',
    'วัดป่าตัน',
    'บ้านท่อเมืองลัง',
    'คำเที่ยงอนุสสรณ์',
    'ท่าศาลา',
    'วัดดอนจั่น',
    'วัดป่าแดด',
    'วัดวังสิงห์คำ',
    'พุทธิโศภน',
    'อนุบาลเชียงใหม่',
    'วัดขะจาว',
    'ศิริมังคลาจารย์',
    'บ้านแม่เหียะสามัคคี',
    'บ้านดอนปิน',
    'ชุมชนวัดท่าเดื่อ',
    'วัดป่าข่อยใต้',
    'วัดร้องอ้อ',
    'บ้านท่าหลุกสันทราย',
    'เจ้าพ่อหลวงอุปภัมภ์ 1',
    'สังวาลย์วิทยา',
    'เชิงดอยสุเทพ',
    'วัดสวนดอก',
    'บ้านโป่งน้อย',
    'บ้านห้วยทราย',
    'วัดหนองป่าครั่ง',
    'ชุมชนบ้านบวกครกน้อย',
    'วัดเสาหิน',
    'วัดเมืองสาตร',
    'บ้านแม่ดอกแดง',
    'บ้านเชิงดอย',
    'บ้านร้องขี้เหล็ก',
    'บ้านบ่อหิน',
    'บ้านแม่ก๊ะ',
    'บ้านแม่จ้อง',
    'บ้านแม่ตอน',
    'บ้านปางไฮ',
    'บ้านป่าไม้แดง',
    'บ้านป่าป้อง',
    'บ้านปางแดง',
    'บ้านปางน้ำถุ',
    'บ้านแม่หวาน',
    'บ้านโป่งกุ่ม',
    'บ้านป่าดู่',
    'บ้านแม่คือ',
    'บ้านสันต้นแหน',
    'บ้านป่าบง',
    'บ้านตลาดขี้เหล็ก',
    'บ้านป่าไผ่',
    'บ้านแม่โป่ง',
    'บ้านแม่ฮ้อยเงิน',
    'บ้านม่วงโตน',
    'บ้านสันทราย',
    'ชลประทานผาแตก',
    'บ้านลวงเหนือ',
    'บ้านป่าสักงาม',
    'บ้านป่างิ้ว',
    'บ้านสันต้นม่วงใต้',
    'บ้านสันต้นดู่',
    'บ้านป่าขุย',
    'บ้านท่ารั้ว',
    'บ้านป่าเสร้า',
    'บ้านป่าเหมือด',
    'บ้านสันต้นม่วงเหนือ',
    'วัดดอยยาว',
    'วัดป่าเปา',
    'วัดดงขี้เหล็ก',
    'บ้านดอนปิน',
    'บ้านแช่ช้าง',
    'บ้านหนองโค้ง',
    'บ้านบ่อสร้าง',
    'วัดสันมะฮกฟ้า',
    'วัดสันป่าค่า',
    'วัดทรายมูล',
    'บ้านบวกค้าง',
    'วัดป่าตาล',
    'วัดย่าปายร้อยพร้อมสามัคคี',
    'บ้านกอสะเลียม',
    'บ้านร้องก่องข้าว',
    'วัดช่างเพี้ยน',
    'วัดบ้านโป่ง',
    'วัดสันมะแปบ',
    'บ้านปงป่าเอื้อง',
    'วัดบ้านใหม่',
    'วัดร้องวัวแดง',
    'วัดน้ำจำ',
    'บ้านมอญ',
    'วัดสันกลางเหนือ',
    'วัดสันกลางใต้',
    'สันกำแพงคันธาอนุสรณ์',
    'บ้านสันกำแพง',
    'วัดบ้านน้อย',
    'วัดสันโค้ง',
    'บ้านแม่ปูคา',
    'วัดหนองแสะ',
    'วัดล้านตอง',
    'วัดบ้านหม้อ',
    'บ้านป่าตึงดอยซิว',
    'วัดป่าตึง',
    'วัดบ้านโห้ง',
    'วัดริมออน',
    'วัดแม่ผาแหน',
    'บ้านแม่ตะไคร้',
    'บ้านห้วยบง',
    'บ้านห้วยบง สาขาบ้านห้วยยาบ',
    'หมู่บ้านสหกรณ์ 5',
    'หมู่บ้านสหกรณ์ 6',
    'หมู่บ้านสหกรณ์ 4',
    'หมู่บ้านสหกรณ์ 1',
    'หมู่บ้านสหกรณ์ 2',
    'หมู่บ้านสหกรณ์ 3',
    'วัดบ้านม่อน',
    'วัดห้วยทราย',
    'วัดดอนชัย',
    'วัดแม่กำปอง',
    'บ้านแม่เตาดิน',
    'วัดห้วยแก้ว',
    'บ้านจำปี',
    'บ้านออนกลาง',
    'วัดเปาสามขา สาขาบ้านวาก',
    'วัดเปาสามขา',
    'บ้านออนหลวย สาขาบ้านหัวฝาย',
    'บ้านออนหลวย',
    'วัดหนองหอย',
    'ยุพราชวิทยาลัย',
    'วัฒโนทัยพายัพ',
    'หอพระ',
    'กาวิละวิทยาลัย',
    'โสตศึกษาอนุสารสุนทร',
    'สอนคนตาบอดภาคเหนือ',
    'กาวิละอนุกูล',
    'ดอยสะเก็ดวิทยาคม',
    'สันกำแพง',
    'ออนเหนือ',
    'บ้านเมืองกื้ด',
    'บ้านแม่ตะมาน',
    'บ้านต้นขาม',
    'บ้านบวกหมื้อ',
    'สันป่าสักวิทยา',
    'วัดทรายมูล',
    'ร่ำเปิงวิทยา',
    'บ้านแม่ขะจาน',
    'บ้านปางเปา',
    'วัดห้วยไร่',
    'ชุมชนวัดช่อแล',
    'บ้านสันป่าสัก',
    'บ้านใหม่',
    'บ้านปางไม้แดง',
    'บ้านช้างใน',
    'วัดช่อแลสาขาบ้านดง',
    'บ้านเป้าวิทยาคาร',
    'บ้านแม่โจ้',
    'บ้านแม่เลิม',
    'บ้านก๊างหงส์',
    'บ้านป่ายางหนาด',
    'วัดปางมะกล้วย',
    'บ้านผาเด็ง',
    'ป่าแป๋วิทยา',
    'บ้านแม่เลา',
    'บ้านปางลัน',
    'บ้านแม่ไคร้',
    'บ้านแม่แสะ',
    'บ้านแม่แมม',
    'บ้านท่าผา',
    'บ้านก๋ายน้อย',
    'วัดบ้านเหล่า',
    'วัดบ้านเหล่า สาขาบ้านม่อนเงาะ',
    'วัดแม่กะ',
    'ภูดินวิทยา',
    'วัดทุ่งหลวง',
    'บ้านแม่นาป้าก',
    'วัดนาเม็ง',
    'วัดป่าเลา',
    'หัวฝายวิทยาคาร',
    'บ้านผึ้ง',
    'วัดแม่หอพระ',
    'บ้านปางฮ่าง',
    'สบเปิงวิทยา',
    'วัดท่าข้าม',
    'บ้านผาหมอน',
    'วัดท่าข้าม สาขาม้งชุ่มเย็น',
    'บ้านหนองบัวหลวง',
    'บ้านปางฮ่างสาขาผาแตก',
    'สันป่ายางวิทยาคาร',
    'สันป่าตึงวิทยาคาร',
    'บ้านหนองก๋าย',
    'บ้านเอียก',
    'ป่าบงห้วยฮ่าง',
    'สันมหาพนวิทยา',
    'หนองหล่มวิทยาคาร',
    'บ้านปางกว้าง',
    'ป่าจี้วังแดงวิทยา',
    'ม่วงคำประชานุกูล',
    'บ้านหางดง',
    'บ้านปง',
    'บ้านเด่น',
    'บ้านทับเดื่อ',
    'วัดหนองออน',
    'บ้านสันป่าตอง',
    'บ้านปางห้วยตาด',
    'บ้านขี้เหล็กน้อย',
    'บ้านสันคะยอม',
    'บ้านน้ำริน',
    'บ้านซาง',
    'บ้านต้นขาม',
    'ชุมชนบ้านขี้เหล็กหลวง',
    'บ้านศาลา',
    'บ้านดอนแก้ว',
    'บ้านพระนอน',
    'บ้านบวกจั่น',
    'บ้านปงไคร้',
    'เจ้าพ่อหลวงอุปถัมภ์ 7',
    'บ้านโป่งแยงใน',
    'บ้านโป่งแยงนอก',
    'บ้านกองแหะ',
    'บ้านทุ่งโป่ง',
    'บ้านแม่แมะ',
    'บ้านปางไฮ',
    'บ้านป่าม่วง',
    'เจ้าพ่อหลวงอุปถัมภ์ 8',
    'เจ้าแม่หลวงอุปถัมภ์ 2',
    'บ้านแม่สา',
    'วัดทรายมูล',
    'บ้านริมใต้',
    'บ้านขอนตาล',
    'มูลนิธิมหาราช 5',
    'บ้านนาหืก',
    'บ้านสะลวงใน',
    'บ้านสะลวงนอก',
    'บ้านกาดฮาว',
    'บ้านเมืองก๊ะ',
    'วัดสุวรรณาวาส',
    'บ้านหนองอาบช้าง',
    'บ้านป่าติ้ว',
    'บ้านหนองปลามัน',
    'วัดวาลุการาม',
    'บ้านวังป้อง',
    'บ้านน้ำต้น',
    'บ้านต้นผึ้ง',
    'บ้านดอนตัน',
    'บ้านแมตะละ สาขาบ้านห้วยเต่า',
    'บ้านห้วยน้ำจาง',
    'บ้านแม่ขะปู',
    'บ้านแม่ยางห้า',
    'บ้านบ่อแก้ว',
    'บ้านแม่โต๋',
    'บ้านแม่ยางห้า สาขาบ้านเด่นฮ่อม',
    'วัดอมลอง',
    'ชุมชนบ้านแม่สาบสาขาวัดแม่ขาน',
    'วัดงิ้วเฒ่า',
    'วัดปางเติม',
    'วัดปางเติมสาขาบ้านขุนสาบ',
    'บ้านนากู่',
    'บ้านปางขุม',
    'บ้านแม่แว',
    'วัดยั้งเมิน',
    'บ้านอังคาย',
    'บ้านกิ่วเสือ',
    'บ้านแม่ตะละ',
    'บ้านแม่ลานคำ สาขาบ้านป่าคานอก',
    'ทรายทองราษฎร์อุทิศ',
    'บ้านแม่ลานคำ',
    'ชุมชนบ้านแม่สาบ',
    'วัดน้ำริน',
    'ชุมชนบ้านดง',
    'วัดแม่เลย',
    'วัดแม่แพะ',
    'วัดโป่งกวาว',
    'บ้านป่าลาน',
    'วัดบ้านป้อก',
    'บ้านเขื่อนผาก',
    'บ้านห้วยบง',
    'ชุมชนสหกรณ์นิคมวิทยา',
    'บ้านป่าอ้อ',
    'สหกรณ์ดำริ',
    'สามัคคีวิทยา',
    'ชุมชนบ้านโป่ง',
    'บ้านป่าฮิ้น',
    'พร้าวบูรพา',
    'บ้านต้นรุง',
    'บ้านป่าตุ้ม',
    'บ้านป่าไหน่',
    'บ้านสันยาว',
    'บ้านสันกลาง',
    'ประชาสามัคคีวิทยา',
    'ประดู่วิทยา',
    'บ้านแม่ปั๋ง',
    'บ้านแม่ปั๋ง สาขาบ้านขุนปั๋ง',
    'บ้านแม่เหียะ',
    'บ้านขุนแจ๋',
    'บ้านแจ่งกู่เรือง',
    'บ้านสันปง',
    'บ้านแม่ปาคี',
    'บ้านหนองปิด',
    'บ้านท่ามะเกี๋ยง',
    'บ้านหลวง',
    'บ้านนาบุญโหล่งขอด',
    'บ้านเมืองขอน',
    'บ้านป่าเหมือด',
    'วัดแม่แก้ดน้อย',
    'บ้านศรีบุญเรือง',
    'บ้านโปง',
    'บ้านหัวฝาย',
    'บ้านหนองมะจับ',
    'บ้านโป่ง',
    'บ้านร่มหลวง',
    'บ้านศรีงาม',
    'บ้านแม่แต',
    'บ้านโป่ง สาขาบ้านห้วยแก้ว',
    'บ้านเจดีย์แม่ครัว',
    'บ้านแม่แฝก',
    'บ้านแม่ย่อย',
    'บ้านสันคะยอม',
    'วัดสันทรายมูล',
    'สันทรายหลวง',
    'บ้านหลักปัน',
    'บ้านป่าก้าง',
    'บ้านสันพระเนตร',
    'บ้านสันศรี',
    'บ้านหนองไคร้',
    'บ้านท่าเกวียน',
    'บ้านห้วยเกี๋ยง',
    'บ้านแม่โจ้',
    'บ้านป่าบง',
    'บ้านหนองหาร',
    'บ้านบวกเปา',
    'บ้านดงเจริญชัย',
    'ชลประทานเขื่อนแม่กวงจิราธิวัฒน์อุปถัมภ์',
    'แม่แตง',
    'แม่หอพระวิทยาคม',
    'สันป่ายางวิทยาคม',
    'แม่ริมวิทยาคม',
    'นวมินทราชูทิศ พายัพ',
    'ศึกษาสงเคราะห์เชียงใหม่',
    'สะเมิงพิทยาคม',
    'พร้าววิทยาคม',
    'สันทรายวิทยาคม',
    'ศรีสังวาลย์เชียงใหม่',
    'บ้านถ้ำ',
    'บ้านเชียงดาว',
    'บ้านทุ่งหลุก',
    'บ้านทุ่งหลุก สาขาปางแดง',
    'บ้านทุ่งละคร',
    'บ้านดอน',
    'บ้านม่วงฆ้อง',
    'ชุมชนบ้านวังจ๊อม',
    'บ้านห้วยทรายขาว',
    'พัฒนาต้นน้ำขุนคอง',
    'บ้านห้วยเป้า',
    'บ้านแม่กอนใน',
    'บ้านทุ่งข้าวพวง',
    'บ้านออน',
    'บ้านปางเฟือง',
    'บ้านแม่ป๋าม',
    'บ้านปางมะเยา',
    'มิตรมวลชนเชียงใหม่',
    'บ้านห้วยจะค่าน ตชด.อนุสรณ์',
    'บ้านเมืองคอง',
    'บ้านวังมะริว',
    'บ้านหนองบัว',
    'ดอยสามหมื่น',
    'ชุมชนบ้านเมืองงาย',
    'บ้านใหม่',
    'บ้านแม่ข้อน',
    'บ้านเมืองนะ',
    'บ้านอรุโณทัย',
    'แกน้อยศึกษา',
    'บ้านหนองเขียว',
    'โครงการหลวงแกน้อย',
    'บ้านรินหลวง',
    'บ้านนาหวาย',
    'บ้านโละป่าหาญ',
    'บ้านโป่งอาง',
    'เบญจม 2 บ้านน้ำรู',
    'บ้านแม่แมะ',
    'บ้านสันป่าเกี๊ยะ',
    'บ้านสบคาบ สาขาแก่งปันเต๊า',
    'บ้านแม่แมะ สาขาป่าโหล',
    'บ้านป่าบง',
    'วัดจอมคีรี',
    'บ้านแม่นะ',
    'บ้านสบคาบ',
    'บ้านแม่อ้อใน',
    'วัดปางมะโอ',
    'บ้านต้นผึ้ง',
    'บ้านหัวฝาย',
    'บ้านป่าบง',
    'บ้านโป่งน้ำร้อน',
    'บ้านหัวนา',
    'บ้านม่วงชุม',
    'บ้านลาน',
    'บ้านม่วงชุม สาขาบ้านสวนชา',
    'บ้านหัวนา สาขาบ้านห้วยหมากเลี่ยม',
    'บ้านขอบด้ง',
    'บ้านม่อนปิ่น',
    'บ้านเวียงหวาย',
    'บ้านหนองบัว',
    'บ้านสันม่วง',
    'บ้านแม่ข่า',
    'บ้านหล่ายฝาง',
    'บ้านดงป่าลัน',
    'บ้านสันต้นเปา',
    'บ้านแม่คะ สาขาบ้านหนองบัวคำ',
    'บ้านปางปอย',
    'บ้านโป่งนก สาขาบ้านแม่ป่าไผ่',
    'บ้านโป่งนก',
    'บ้านห้วยไคร้',
    'บ้านแม่คะ',
    'บ้านเหมืองแร่',
    'บ้านทุ่งหลุก',
    'บ้านยาง',
    'เจ้าแม่หลวงอุปถัมภ์ 1',
    'บ้านห้วยห้อม',
    'บ้านหนองขวาง',
    'บ้านหลวง',
    'บ้านคุ้ม',
    'วิรุณเทพ',
    'บ้านแม่งอนกลาง',
    'บ้านแม่งอนขี้เหล็ก',
    'บ้านล้องอ้อ',
    'บ้านสันป่าแดง',
    'บ้านปางสัก',
    'บ้านแม่สูนน้อย',
    'บ้านต้นส้าน',
    'บ้านหนองยาว',
    'ชุมชนบ้านแม่สูนหลวง',
    'บ้านห้วยเฮี่ยน',
    'บ้านแม่ใจ',
    'บ้านห้วยบอน',
    'บ้านโป่งถืบ',
    'บ้านหนองตุ้ม',
    'บ้านศรีดอนชัย',
    'บ้านเวียงฝาง',
    'บ้านสันทรายคองน้อย',
    'บ้านท่าสะแล',
    'บ้านห้วยงูกลาง สาขาเจ้าพ่อหลวงอุปถัมภ์',
    'บ้านสันต้นดู่',
    'บ้านห้วยงูกลาง',
    'วัดนันทาราม',
    'บ้านสันทราย',
    'วัดศรีบุญเรือง',
    'วัดห้วยน้ำเย็น',
    'ไทยทนุบ้านสันต้นดู่',
    'บ้านท่ามะแกง',
    'บ้านห้วยปู',
    'บ้านท่าตอน',
    'ต.ช.ด. ไลออนส์มหาจักร 8',
    'เจ้าพ่อหลวงอุปถัมภ์ 9',
    'บ้านห้วยศาลา',
    'โชติคุณเกษมบ้านเมืองงาม',
    'กมล -เรียม สุโกศล (บ้านผาใต้)',
    'บ้านสุขฤทัย',
    'ต.ช.ด.ศึกษานารีอนุสรณ์ 3',
    'บ้านหลวง',
    'บ้านป่าก๊อ',
    'บ้านป่าแดง',
    'บ้านหลวง สาขาห้วยมะเฟือง',
    'บ้านป่าก๊อ สาขาโละป่าไคร้',
    'ไทยรัฐวิทยา 12 (บ้านเอก)',
    'บ้านปางต้นเดื่อ',
    'บ้านสันป่าเหียว',
    'บ้านป่าก๊อ สาขาบ้านห้วยม่วง',
    'บ้านหนองขี้นกยาง',
    'บ้านห้วยคอกหมู สาขาหัวแม่เมืองน้อย',
    'ทหารม้าอนุสรณ์ บ้านวังดิน',
    'ศึกษานารีอนุสรณ์ 1 บ้านแม่เมืองน้อย',
    'ศึกษานารีอนุสรณ์ 2',
    'บ้านห้วยคอกหมู',
    'ชุมชนบ้านคาย',
    'ประพันธ์ - อารีย์ หงษ์สกุล(บ้านห้วยหลวง)',
    'บ้านห้วยม่วง',
    'บ้านฮ่างต่ำ',
    'บ้านฮ่องห้า',
    'บ้านแม่สาว',
    'บ้านหนองเต็ง',
    'บ้านโป่งไฮ',
    'บ้านห้วยป่าซาง',
    'บ้านดง',
    'ชุมชนบ้านแม่ฮ่าง',
    'บ้านสันป่าข่า',
    'บ้านดอนชัย',
    'บ้านดอนชัยสาขาบ้านตาดหมอก',
    'บ้านโละ',
    'บ้านสันปอธง',
    'บ้านทรายแดง',
    'บ้านสันต้นหมื้อ',
    'บ้านเปียงหลวง',
    'บ้านจอง',
    'บ้านห้วยไคร้',
    'รัฐราษฎร์อุปถัมภ์',
    'บ้านแม่แพม',
    'บ้านแม่หาด',
    'บ้านกองลม',
    'บ้านเวียงแหง',
    'บ้านนามน',
    'บ้านปางป๋อ',
    'บ้านม่วงป๊อก',
    'บ้านปงตำ',
    'บ้านห้วยบง',
    'วัดมหิงสาวาส',
    'วัดอรัญญวาสี',
    'วัดบ้านท่า',
    'บ้านแม่ทะลบ',
    'วัดป่าแดง',
    'บ้านป่างิ้ว',
    'บ้านห้วยต้นตอง',
    'บ้านหัวฝาย สาขาบ้านศรีค้ำ',
    'บ้านแม่ขิ',
    'ชุมชนวัดศรีดงเย็น',
    'บ้านอ่าย',
    'บ้านร้องธาร',
    'บ้านถ้ำตับเตา',
    'บ้านป่าซาง',
    'บ้านหัวฝาย',
    'บ้านสินชัย',
    'บ้านผาแดง',
    'บ้านต้นโชค',
    'บ้านปง',
    'สันติวนา',
    'บ้านห้วยไผ่',
    'บ้านใหม่หนองบัว',
    'เชียงดาววิทยาคม',
    'อรุโณทัยวิทยาคม',
    'ศึกษาสงเคราะห์เชียงดาว',
    'ฝางชนูปถัมภ์',
    'แม่อายวิทยาคม',
    'ราชประชานุเคราะห์ 30',
    'เวียงแหงวิทยาคม',
    'ไชยปราการ',
    'ต้นแหนหลวงประสิทธิ์วิทยา',
    'วัดทุ่งหลุก',
    'ดอนชัยวิทยาคาร',
    'วัดช่างกระดาษ',
    'ป่าลานประสิทธิ์วิทยา',
    'บ้านแม่กุ้งหลวง',
    'วัดรัตนาราม',
    'วัดสว่างอารมณ์',
    'บ้านหัวริน',
    'วัดน้ำบ่อหลวง',
    'วัดศรีอุดม',
    'วัดท้องฝาย',
    'บ้านทุ่งเสี้ยว (นวรัฐ)',
    'วัดท่ากาน',
    'วัดสามหลัง',
    'วัดท่าโป่ง',
    'กิ่วแลน้อยประสิทธิ์วิทยา',
    'บ้านเปียง',
    'บ้านกวน',
    'ทุ่งฟ้าบดราษฎร์บำรุง',
    'บ้านร่องน้ำ',
    'วัดดงป่างิ้ว',
    'บ้านม่วงพี่น้อง',
    'วัดโรงวัว',
    'วัดหนองครอบ',
    'วัดอินทะวิชัย',
    'วัดกู่คำ',
    'ชุมชนกิ่วแลหลวง',
    'สันป่าตอง(สุวรรณราษฎร์วิทยาคาร)',
    'สันป่าตอง',
    'วัดอุเม็ง',
    'บ้านห้วยส้ม',
    'วัดบุปผาราม',
    'บ้านต้นแก้ว',
    'วัดขุนคง',
    'บ้านท่าขุนคง',
    'บ้านแสนตอ',
    'บ้านน้ำแพร่',
    'บ้านปง',
    'วัดคีรีเขต',
    'วัดประชาเกษม',
    'วัดช่างคำ',
    'วัดจอมทอง',
    'บ้านไร่',
    'บ้านท้าวบุญเรือง',
    'บ้านวังศรี',
    'บ้านป่าตาล',
    'บ้านสันผักหวาน',
    'บ้านต้นงิ้ว',
    'วัดศรีสุพรรณ์',
    'บ้านสันทราย',
    'บ้านต้นเกว๋น',
    'บ้านสันป่าสัก',
    'บ้านตองกาย',
    'บ้านฟ่อน',
    'วัดอินทราพิบูลย์',
    'วัดพระเจ้าเหลื้อม',
    'บ้านหนองตอง',
    'วัดป่าลาน',
    'บ้านทรายมูล',
    'บ้านดง',
    'วัดศรีล้อม',
    'บ้านหารแก้ว',
    'บ้านกวน',
    'บ้านปากเหมือง',
    'วัดพญาชมภู',
    'วัดท่าต้นกวาว',
    'วัดแม่สะลาบ',
    'วัดนันทาราม',
    'วัดตำหนัก',
    'วัดบวกครกเหนือ',
    'ชุมชนวัดศรีคำชมภู',
    'วัดเทพาราม',
    'วัดเวฬุวัน',
    'วัดศรีโพธาราม',
    'บ้านสันทราย',
    'วัดศรีดอนชัย',
    'ชุมชนวัดปากกอง',
    'วัดสันดอนมูล',
    'วัดพระนอนหนองผึ้ง',
    'วัดกองทราย',
    'เวียงเศรษฐีวิทยา',
    'วัดดอนเปา',
    'บ้านเหล่าป่าฝาง',
    'บ้านใหม่สวรรค์',
    'วัดทุ่งศาลา',
    'วัดห้วยแก้ว',
    'วัดดอนแก้ว',
    'บ้านพันตน',
    'วัดมะกับตองหลวง',
    'ไตรมิตรวิทยา',
    'วัดสารภีวิทยาคาร',
    'บ้านกาด',
    'บ้านใหม่ปางเติม',
    'วัดศิริชัยนิมิตร',
    'บ้านทุ่งหลวง',
    'บ้านหนองเต่า',
    'แม่วินสามัคคี',
    'วัดวังผาปูน',
    'บ้านห้วยตอง',
    'หลวงพัฒนาบ้านขุนวาง',
    'พัฒนาต้นน้ำที่ 5 ขุนวาง',
    'แม่วินสามัคคี สาขาขุนป๋วย',
    'บ้านห้วยข้าวลีบ',
    'บ้านเหล่าเป้า',
    'บ้านเจริญสามัคคี',
    'บ้านดอยหล่อ',
    'วัดวังขามป้อม',
    'บ้านดงป่าหวาย',
    'กรป.กลางอุปถัมภ์',
    'วัดดอนชื่น',
    'วัดหนองหลั้ว',
    'วัดศรีดอนชัย',
    'บ้านห้วยน้ำขาว',
    'บ้านสามหลัง',
    'วัดสองแคว',
    'บ้านหัวข่วง',
    'บ้านแม่ขาน',
    'บ้านใหม่หนองหอย',
    'สันป่าตองวิทยาคม',
    'น้ำบ่อหลวงวิทยาคม',
    'หางดงรัฐราษฎร์อุปถัมภ์',
    'สารภีพิทยาคม',
    'บ้านกาดวิทยาคม',
    'สองแคววิทยาคม',
    'สันติสุข',
    'บ้านม่อนหิน',
    'บ้านแปะ',
    'บ้านห้วยทราย',
    'บ้านข่วงเปาใต้',
    'บ้านท่าข้าม',
    'บ้านบนนา',
    'บ้านขุนแปะ',
    'บ้านสบแจ่มฝั่งซ้าย',
    'บ้านขุนแปะ สาขาแม่จร',
    'บ้านสบแปะ',
    'วัดวังดิน',
    'ชุมชนบ้านข่วงเปาเหนือ',
    'บ้านปากทางท่าลี่',
    'นิยมไพรผาหมอนอินทนนท์',
    'บ้านเมืองอาง',
    'บ้านน้ำตกแม่กลาง',
    'บ้านเมืองกลาง',
    'วัดพระบาท',
    'บ้านน้ำลัด',
    'วัดพระบาท สาขาแม่ปอน',
    'บ้านขุนยะสาขาแม่ยะน้อย',
    'ชุมชนศรีจอมทอง',
    'บ้านห้วยส้มป่อย',
    'บ้านแม่เตี๊ยะ',
    'บ้านห้วยน้ำดิบ',
    'บ้านขุนแตะ',
    'บ้านขุนยะ',
    'บ้านขุนกลาง',
    'บ้านวังน้ำหยาด',
    'บ้านแม่สอย',
    'บ้านโรงวัว',
    'บ้านหนองคัน',
    'บ้านห้วยม่วงฝั่งซ้าย',
    'บ้านใหม่สารภี',
    'ป่ากล้วยพัฒนา',
    'บ้านห้วยสะแพด',
    'วัดพุทธนิมิตร',
    'บ้านหาดนาค',
    'บ้านห้วยโจ้',
    'ไทยรัฐวิทยา 79 (บ้านหนองอาบช้าง)',
    'บ้านห้วยม่วง',
    'บ้านโมคคัลลาน',
    'บ้านท่าหลุก',
    'บ้านทุ่งหมากหนุ่ม',
    'บ้านทุ่งปูน',
    'บ้านสบเตี๊ยะ',
    'บ้านกองแขก',
    'บ้านอมเม็ง',
    'ออป.13',
    'บ้านโม่งหลวง',
    'บ้านอมขูด',
    'บ้านกองแขก สาขาบ้านแม่หลุ',
    'บ้านโม่งหลวง สาขาบ้านแม่อมลานใน',
    'บ้านแจ่มหลวง',
    'บ้านห้วยยา',
    'บ้านขุนแม่รวม',
    'บ้านทุ่งยาว',
    'บ้านต่อเรือ',
    'ชุมชนบ้านช่างเคิ่ง',
    'บ้านห้วยริน',
    'บ้านแม่ปาน',
    'อินทนนท์วิทยา',
    'บ้านเนินวิทยา',
    'บ้านต้นตาล',
    'บ้านบนนา',
    'ชุมชนบ้านพร้าวหนุ่ม',
    'บ้านสามสบ',
    'บ้านผานัง',
    'บ้านนาเรือน',
    'บ้านป่าแดด',
    'บ้านยางหลวง',
    'บ้านทัพ',
    'สามัคคีสันม่วง',
    'บ้านจันทร์',
    'บ้านจันทร์ สาขาแจ่มน้อย',
    'บ้านอมเม็งสาขาสบลอง',
    'บ้านสองธาร',
    'บ้านทุ่งแก',
    'บ้านปางหินฝน',
    'บ้านพุย',
    'บ้านแม่ตูม',
    'บ้านเฮาะวิทยา',
    'บ้านแปะ',
    'บ้านแม่แฮใต้',
    'บ้านแม่หงานหลวง',
    'บ้านพุย สาขาพุยใต้',
    'บ้านปางหินฝนสาขาบ้านดินขาว',
    'บ้านแม่แดดน้อย',
    'บ้านห้วยปู',
    'บ้านแม่ตะละเหนือ',
    'บ้านห้วยปูสาขาแม่ตะละใต้',
    'บ้านแม่แดดน้อย สาขาบ้านดงสามหมื่น',
    'บ้านแม่แฮเหนือ สาขาแม่แจ๊ะ',
    'บ้านแม่มุ',
    'บ้านแม่นาจร',
    'บ้านแม่มะลอ',
    'บ้านแม่วาก',
    'บ้านสบวาก',
    'บ้านสบวาก สาขาแม่จอนหลวง',
    'บ้านแม่แฮเหนือ',
    'บ้านสบแม่รวม',
    'บ้านห้วยผา',
    'บ้านแม่ซา',
    'บ้านแม่หอย',
    'บ้านแม่เอาะ',
    'บ้านแม่ศึก',
    'บ้านขุนแม่นาย',
    'บ้านผาละปิ',
    'บ้านปางอุ๋ง',
    'บ้านขุนแม่หยอด',
    'บ้านปางอุ๋ง สาขาแม่สะต๊อป',
    'บ้านปางเกี๊ยะ',
    'บ้านนากลาง',
    'บ้านนาฮ่อง',
    'บ้านนากลาง สาขาห้วยผักกูด',
    'บ้านหลังท่อ',
    'บ้านนาคอเรือ',
    'บ้านเด่นวิทยา',
    'บ้านห้วยฝาง',
    'บ้านแม่งูด',
    'บ้านตีนตก',
    'บ้านแม่โถ',
    'บ้านแม่อมลอง',
    'บ้านดอกแดง',
    'บ้านกองลอย',
    'บ้านทุ่ง',
    'บ้านบ่อสลี',
    'บ้านทุ่ง สาขาทุ่งยง',
    'บ้านบ่อสลี สาขาแม่แวน',
    'บ้านบ่อหลวง',
    'บ้านวังกอง',
    'บ้านขุน',
    'บ้านนาฟ่อน',
    'บ้านแม่ลาย',
    'บ้านกิ่วลม',
    'บ้านแม่สะนาม',
    'บ้านแม่ลายเหนือ',
    'บ้านพุย',
    'บ้านเตียนอาง',
    'บ้านตาลใต้',
    'บ้านตาลเหนือ',
    'บ้านทุ่งโป่ง',
    'บ้านผาแตน',
    'บ้านโค้งงาม',
    'บ้านดอยคำ',
    'บ้านแม่ลอง',
    'ชุมชนบ้านท่าข้าม',
    'บ้านหางดง',
    'ชุมชนบ้านแม่ทัง',
    'บ้านกองหิน',
    'บ้านวังลุง',
    'บ้านแควมะกอก',
    'บ้านทรายแก้ว',
    'บ้านดงดำ',
    'บ้านสันติสุข',
    'บ้านไร่ สาขาทุ่งคอกช้าง',
    'บ้านถิ่นสำราญ',
    'บ้านผาจุก',
    'บ้านไร่',
    'ชุมชนบ้านดอยเต่า',
    'บ้านฉิมพลี',
    'บ้านวังหลวง',
    'บ้านแปลง 5',
    'ศูนย์อพยพแปลง 8',
    'บ้านหนองบัวคำ',
    'บ้านน้อยห้วยรินวิทยา',
    'บ้านโท้งวิทยา',
    'บ้านหนองผักบุ้ง',
    'บ้านบงตัน',
    'บ้านแอ่นใหม่',
    'บ้านแอ่นจัดสรร',
    'บ้านวังหม้อ',
    'บ้านเกาะหลวง',
    'บ้านโปง',
    'บ้านงิ้วสูง',
    'บ้านโปงทุ่ง',
    'บ้านกองวะ',
    'บ้านแม่บวน',
    'บ้านแม่ตูบ',
    'บ้านแปลง 2',
    'ชุมชนศูนย์อพยพแปลง 4',
    'บ้านนาเกียน',
    'บ้านห่างหลวง',
    'บ้านใบหนา',
    'บ้านอูตูม',
    'บ้านผีปานเหนือ',
    'บ้านแม่สะเต',
    'บ้านแม่โขง',
    'บ้านมูเซอ',
    'บ้านห้วยปูลิง',
    'บ้านห้วยไม้หก',
    'ตรีมิตรวิทยา',
    'บ้านห้วยน้ำขาว',
    'บ้านแม่ตื่น',
    'บ้านขุนแม่ตื่นน้อย',
    'บ้านห้วยหล่อดูก',
    'ชุมชนบ้านใหม่ สาขาบ้านแม่เทย',
    'บ้านซิแบร',
    'ชุมชนบ้านใหม่',
    'ทุ่งต้นงิ้ว',
    'บ้านยางเปียง',
    'บ้านยางครก',
    'บ้านนาไคร้',
    'บ้านสบลาน',
    'บ้านหลวง',
    'บ้านแม่ลานหลวง',
    'บ้านแม่ลานหลวง สาขาบ้านดอยซาง',
    'บ้านห้วยโค้ง',
    'บ้านขุนตื่น',
    'บ้านแม่หลองน้อย',
    'บ้านผาปูน',
    'ชุมชนบ้านอมก๋อย สาขาบ้านสบอมแฮด',
    'บ้านทุ่งจำเริง',
    'บ้านยางแก้ว',
    'บ้านตุงลอย',
    'บ้านตุงติง',
    'บ้านแม่ต๋อม',
    'บ้านแม่อ่างขาง',
    'ชุมชนบ้านอมก๋อย',
    'บ้านยางเปา',
    'จอมทอง',
    'แม่แจ่ม',
    'ราชประชานุเคราะห์ 31',
    'ฮอดพิทยาคม',
    'ดอยเต่าวิทยาคม',
    'อมก๋อยวิทยาคม',
    'แม่ตื่นวิทยาคม',
    'วัดป่ายาง',
    'วัดสันต้นธง',
    'วัดพันตาเกิน',
    'บ้านศรีย้อย',
    'บ้านสันมะนะ',
    'วัดจักรคำภิมุข',
    'อนุบาลลำพูน',
    'วัดขี้เหล็ก',
    'วัดศรีชุม',
    'บ้านประตูโขง',
    'วัดร่องส้าว',
    'ตำบลบ้านแป้น',
    'บ้านแป้น',
    'บ้านป่าซางน้อย',
    'วัดบ้านม่วง',
    'วัดสันมะกรูด',
    'วัดชัยชนะ',
    'วัดล่ามช้าง',
    'บ้านประตูป่า',
    'บ้านศรีสุพรรณ',
    'วัดหนองท่า',
    'วัดสันป่าสัก',
    'วัดห้วยม้าโก้ง',
    'วัดน้ำพุ',
    'วัดหนองซิว',
    'วัดสันหลวง',
    'บ้านหนองบัว',
    'วัดศรีดอนตัน',
    'วัดกิ่วมื่น',
    'บ้านสันป่าเหียง',
    'วัดสันคะยอม',
    'วัดสะแล่ง',
    'บ้านหนองหอย',
    'บ้านมะเขือแจ้',
    'วัดเหมืองกวัก',
    'บ้านฮ่องกอม่วง',
    'วัดบ้านแจ่ม',
    'วัดศรีบังวัน',
    'ตำบลริมปิง',
    'บ้านป่าแก',
    'วัดสบปะ',
    'บ้านเวียงยอง',
    'วัดศรีบุญชู-วังไฮ',
    'วัดป่าแดด',
    'บ้านจำขี้มด',
    'วัดศรีบัวบาน',
    'วัดหนองหล่ม',
    'วัดทุ่งยาว',
    'บ้านจำบอน',
    'บ้านขัวแคร่',
    'วัดบ้านม้า',
    'บ้านหนองช้างคืน',
    'วัดหัวฝาย',
    'วัดบ่อโจง',
    'บ้านหนองหนาม',
    'วัดบ้านขว้าง',
    'วัดกอข่อย',
    'บ้านต้นผึ้ง',
    'วัดเหมืองง่า',
    'บ้านศรีบุญยืน-วังทอง',
    'วัดบ้านหลุก',
    'บ้านป่าขาม',
    'วัดกู่เส้า',
    'ชุมชนบ้านเหมืองจี้',
    'บ้านห้วยส้ม',
    'วัดป่าตึงเชตวัน',
    'วัดต้นโชค',
    'บ้านอุโมงค์',
    'อนุบาลเมืองลำพูน',
    'บ้านป่าเส้า',
    'เชตวันหนองหมู',
    'วัดฮ่องกอก',
    'วัดชัยสถาน',
    'บ้านหนองยางไคล สาขาท้องฝาย',
    'บ้านหมื่นข้าว',
    'บ้านแพะยันต์ดอยแช่',
    'วัดทากาศ',
    'บ้านดอยคำ',
    'บ้านดอยคำ สาขาแม่ขนาด',
    'บ้านป่าเลา',
    'บ้านป่าเลา สาขาผาด่าน',
    'บ้านป่าเลา สาขาห้องเรียนบ้านแม่สะแงะ',
    'บ้านศรีป้าน',
    'บ้านดอยครั่ง',
    'บ้านสัน',
    'บ้านสบเมย',
    'บ้านแม่เมย',
    'บ้านแม่เมย สาขาแม่เหล็ก',
    'บ้านทาขุมเงิน',
    'บ้านสวนหลวง',
    'บ้านทาทุ่งหลวง',
    'บ้านเหมืองลึก',
    'บ้านหนองยางไคล',
    'บ้านทากู่',
    'บ้านทาปลาดุก',
    'บ้านทาทุ่งไผ่',
    'บ้านปงอ้อ',
    'บ้านทาสองท่า',
    'บ้านทาป่าสัก สาขาบ้านขุนตาล',
    'บ้านทาชมภู',
    'บ้านทาป่าสัก',
    'บ้านทาป่าเปา',
    'บ้านศรีทรายมูล',
    'บ้านปงแม่ลอบ สาขาห้องเรียนบ้านห้วยเหี้ยะ',
    'บ้านปงแม่ลอบ สาขาห้วยฮ่อม',
    'บ้านปงแม่ลอบ สาขาขุนก๋อง',
    'บ้านปงแม่ลอบ',
    'บ้านจำตาเหิน',
    'บ้านหล่ายทา',
    'บ้านศาลาแม่ทา',
    'วัดเกาะสบชัย',
    'บ้านร้องเรือ',
    'บ้านดงสารภี',
    'วัดอรัญญาราม',
    'บ้านกอลุง',
    'บ้านผาตั้ง',
    'บ้านดอยแก้ว',
    'บ้านหนองบัว',
    'บ้านหนองเกิด',
    'บ้านร่องห้า',
    'บ้านร่องช้าง',
    'บ้านห้วยไฟ',
    'บ้านน้ำย้อย',
    'บ้านแม่อาว',
    'บ้านโป่งรู',
    'บ้านโทกน้ำกัด',
    'วัดนครเจดีย์',
    'วัดหนองสมณะ',
    'บ้านป่ารกฟ้า',
    'บ้านเหล่าป่าก๋อย',
    'วัดน้ำดิบ',
    'บ้านห้วยอ้อ',
    'บ้านไร่ดง',
    'วัดปากล้อง',
    'วัดวังกู่',
    'บ้านหนองผ้าขาว',
    'วัดท่าตุ้ม',
    'วัดเจดีย์สามยอด',
    'วัดบ้านเรือน',
    'วัดศรีชุม',
    'บ้านหนองดู่',
    'วัดบ้านเหล่า',
    'วัดบ้านก้อง',
    'วัดท่าต้นงิ้ว',
    'บ้านฉางข้าวน้อย',
    'บ้านล้องหนองหอย',
    'บ้านป่าซาง',
    'บ้านสะปุ๋ง',
    'วัดบ้านไร่',
    'บ้านป่าตาล',
    'วัดบ้านหวาย',
    'บ้านกิ่วมื่น',
    'วัดช้างค้ำ',
    'บ้านมะกอก',
    'บ้านกองงาม',
    'วัดบ้านดอน',
    'บ้านหนองเงือก',
    'วัดป่าบุก',
    'บ้านดอนตอง',
    'วัดบ้านธิ',
    'วัดสันทราย',
    'วัดป่าสัก',
    'วัดป่าตาล',
    'บ้านดอยเวียง',
    'วัดศรีดอนชัย',
    'วัดห้วยยาบ',
    'บ้านห้วยไซ',
    'วัดสันพระเจ้าแดง',
    'วัดป่าตึง-ห้วยยาบ',
    'วัดสันพระเจ้าแดง สาขาบ้านแม่หาด',
    'บ้านแป้นพิทยาคม',
    'ราชประชานุเคราะห์ 26',
    'จักรคำคณาทร',
    'แม่ทาวิทยาคม',
    'ป่าซาง',
    'ป่าตาลบ้านธิพิทยา',
    'ส่วนบุญโญปถัมภ์ ลำพูน',
    'ทาขุมเงินวิทยาคาร',
    'อุโมงค์วิทยาคม',
    'น้ำดิบวิทยาคม',
    'วชิรป่าซาง',
    'บ้านใหม่สบปุง',
    'บ้านนางแตน',
    'บ้านท่าผา',
    'บ้านศาลาสามัคคี',
    'บ้านจอมปิง',
    'สบต๋ำวิทยา (แปงภักดีอุปถัมภ์)',
    'บ้านป่าแข(อุดมศรัทธานุกูล)',
    'บ้านนาแก้วประชารัฐ',
    'บ้านสองแควใต้',
    'บ้านสองแควเหนือ',
    'วัดหาดปู่ด้าย',
    'วัดนาแก้ว',
    'บ้านนาแส่ง-แม่ไฮ',
    'ชุมชนวัดพระธาตุลำปางหลวง',
    'วัดนางเหลียว',
    'บ้านจู้ด(วิชัยสามัคคีราษฎร์)',
    'วัดป่าเหียง',
    'บ้านแม่หลง-สบจาง',
    'วังพร้าววิทยา',
    'บ้านสาด-ผึ้งนาเกลือ',
    'ศาลาวิทยา',
    'อนุบาลเกาะคา',
    'บ้านดอนธรรม',
    'บ้านทุ่งขาม',
    'บ้านโป่งร้อน-สันป่าสัก',
    'บ้านดอนธรรมสาขาต้นผึ้ง',
    'วัดมะกอก-นาบัว',
    'บ้านไหล่หิน',
    'บ้านแม่ฮวก',
    'บ้านเข้าซ้อน',
    'วัดสาแล',
    'ชุมชนบ้านมั่ว',
    'บ้านแม่กึ๊ด',
    'อนุบาลเสริมงาม',
    'บ้านกิ่วห้วยเบิก',
    'บ้านโป่งน้ำร้อน',
    'บ้านทุ่งต๋ำ',
    'วัดนาเอี้ยง',
    'ศรีลังกาวิทยา',
    'บ้านเหล่ายาว',
    'สันโป่งวิทยา',
    'บ้านปงหลวง',
    'บ้านปงแพ่ง',
    'บ้านปงหัวทุ่ง',
    'เสริมขวาวิทยา',
    'บ้านแม่ผึ้ง',
    'บ้านแม่เลียง',
    'บ้านแม่เลียงพัฒนา',
    'บ้านแม่ต๋ำ',
    'นาไผ่วิทยา',
    'วัดสบแม่ทำ',
    'ไทยรัฐวิทยา 47(วัดนาเดา)',
    'บ้านท่าโป่ง',
    'บ้านนาจะลา',
    'บ้านท่า',
    'วังหินวิทยา',
    'บ้านป่าตาล',
    'บ้านสองแคว',
    'ห้วยแก้ววิทยา',
    'บ้านสันหลวง',
    'บ้านนาโป่ง',
    'บ้านนาเบี้ย',
    'นาบ้านไร่วิทยา',
    'บ้านแม่เตี๊ยะ',
    'นาบ้านไร่วิทยาสาขาบ้านแม่แก่ง',
    'แม่ถอดวิทยา',
    'บ้านแม่เติน',
    'บ้านสบเติน',
    'บ้านแม่ปะ',
    'เถิน-ท่าผา',
    'บ้านท่ามะเกว๋น',
    'ปางกุ่มวิทยา',
    'บ้านหัวน้ำ',
    'แม่มอกวิทยา',
    'บ้านกุ่มเนิ้งใต้',
    'บ้านสะพานหิน',
    'บ้านน้ำดิบ',
    'บ้านแม่วะเด่นชัย',
    'แม่วะวิทยา',
    'บ้านท่าช้าง',
    'บ้านอุมลอง',
    'บ้านเวียง',
    'อนุบาลเถิน (ท่านางอุปถัมภ์)',
    'ล้อมแรดวิทยา',
    'บ้านเหล่า',
    'บ้านดอนไชย',
    'บ้านหนองเตา',
    'แสลมวิทยา',
    'บ้านสะเลียมหวาน',
    'บ้านปางอ้า',
    'บ้านท่าเกวียน',
    'แม่พุหอรบวิทยา',
    'บ้านท่าเวียง',
    'ผาปังวิทยา',
    'บ้านแม่ตั๋ง',
    'แม่เชียงรายวิทยา',
    'ชุมชนบ้านแม่ปุ',
    'ต้นธงวิทยา',
    'ห้วยขี้นกวิทยา',
    'อนุบาลแม่พริก (สุนันทสังฆประสิทธิ์)',
    'บ้านแม่พริกบน',
    'บ้านแม่เชียงรายลุ่ม',
    'บ้านท่าด่าน',
    'บ้านดอนไฟ',
    'บ้านเอียก-นาบง',
    'บ้านใหม่',
    'บ้านนากวาง',
    'อนุบาลแม่ทะ (ชุมชนบ้านน้ำโท้ง-บ้านฮ่อมอุปถัมภ์)',
    'บ้านหนองถ้อย',
    'บ้านหลวง',
    'บ้านหลุก',
    'นาครัวประชาสามัคคี',
    'บ้านน้ำโจ้',
    'บ้านป่าจ้ำ',
    'บ้านแม่ปุง',
    'บ้านม่อนแสนศรี',
    'บ้านฮ่องห้า',
    'ทองทิพย์วิทยา',
    'บ้านหนอง',
    'กิ่วประชาวิทยา',
    'บ้านนาต๋ม',
    'บ้านนากว้าว(กิ่ว)',
    'บ้านแม่อิบ',
    'บ้านบอม',
    'บ้านแม่ไทย',
    'บ้านปง',
    'ป่าตันวิทยา',
    'บ้านนาคต',
    'บ้านแม่ทะ',
    'ชุมชนบ้านท่าแหน',
    'บ้านน้ำโทก-หัวดง',
    'บ้านปางมะโอ',
    'บ้านทุ่งตอน',
    'บ้านนาดู่',
    'บ้านนาดง',
    'บ้านสันป่าเปา',
    'บ้านอ้อวิทยา',
    'บ้านสันดอนแก้ว',
    'บ้านแม่ทาน',
    'บ้านแม่วะ',
    'บ้านก้อมหนองมุง',
    'บ้านหัวเสือ',
    'ห้วยมะเกลือวิทยา',
    'บ้านดอนมูล',
    'วัดบ้านทุ่ง',
    'บ้านสามขา',
    'บ้านนายาบ',
    'บ้านผาแมว',
    'บ้านแก่น',
    'นายางวิทยา',
    'บ้านหนองวัวแดง',
    'ไตรมิตรสามัคคี',
    'บ้านปงกา',
    'บ้านแม่กัวะ',
    'บ้านวังยาว',
    'บ้านอ้อ',
    'บ้านวัฒนา',
    'อนุบาลสบปราบ',
    'บ้านหล่าย',
    'ชุมชนบ้านทุ่ง',
    'บ้านหล่ายฮ่องปุ๊',
    'บ้านวังพร้าว-สบเรียง',
    'ชุมชนบ้านจัว',
    'บ้านเด่น',
    'บ้านสมัย',
    'บ้านน้ำหลง',
    'บ้านอุมลอง',
    'สบปราบพิทยาคม',
    'เกาะคาวิทยาคม',
    'เสริมงามวิทยาคม',
    'แม่พริกวิทยา',
    'แม่ทะประชาสามัคคี',
    'ไหล่หินวิทยา',
    'เวียงมอกวิทยา',
    'แม่ทะพัฒนศึกษา',
    'เถินวิทยา',
    'อนุบาลแจ้ห่ม',
    'บ้านสบฟ้า',
    'บ้านม่วงงาม',
    'บ้านฮ่องลี่',
    'บ้านหนองนาว',
    'บ้านทุ่งผึ้ง',
    'บ้านหัวฝาย',
    'แจ้คอนวิทยา',
    'บ้านทุ่งฮ้าง',
    'บ้านช่อฟ้า',
    'บ้านช่อฟ้าสาขาบ้านแม่จอกฟ้า',
    'ชุมชนบ้านสา',
    'บ้านแป้น',
    'บ้านสาแพะ',
    'บ้านแม่ตา',
    'บ้านแม่ตาใน',
    'บ้านแม่ตาสามัคคี',
    'บ้านปงดอน',
    'บ้านไฮ',
    'บ้านเปียงใจ',
    'บ้านเลาสู',
    'ไผ่งามวิทยา',
    'บ้านนางาม',
    'บ้านแม่เปิน',
    'บ้านนาไหม้',
    'ผาช่อวิทยา',
    'บ้านใหม่สามัคคี',
    'บ้านศรีบุญเรือง',
    'บ้านหนองกอก',
    'บ้านแม่สุก',
    'ทุ่งคาวิทยา',
    'บ้านปงคอบ',
    'บ้านสวนดอกคำ',
    'บ้านสันมะเกลือ',
    'บ้านใหม่เหล่ายาว',
    'บ้านใหม่ผ้าขาว',
    'ทุ่งฮั้ววิทยา',
    'บ้านทุ่งปี้',
    'บ้านวังมน',
    'บ้านแม่ม่า',
    'บ้านดอนแก้ว',
    'บ้านวังใหม่',
    'บ้านทุ่งฝูง',
    'บ้านห้วยก้อด',
    'บ้านป่าคา',
    'ร่องเคาะวิทยา',
    'บ้านแม่สง',
    'บ้านแม่หีด',
    'วังแก้ววิทยา',
    'บ้านแม่สุขใน',
    'บ้านป่าแขม',
    'บ้านหัวทุ่ง',
    'บ้านแม่สุขวังเหนือ',
    'บ้านวังโป่ง',
    'บ้านแม่พริก',
    'บ้านแม่โป่ง',
    'บ้านก่อ',
    'บ้านทุ่งฮี',
    'ปงวังวิทยา',
    'บ้านปงถ้ำ',
    'บ้านแม่เย็น',
    'บ้านเมืองตึง',
    'วังทองวิทยา',
    'อนุบาลวังเหนือ',
    'ชุมชนบ้านใหม่',
    'บ้านทัพป่าเส้า',
    'บ้านป่าเหมี้ยง',
    'บ้านทุ่ง',
    'บ้านใหม่พัฒนา',
    'บ้านสบลี',
    'แจ้ซ้อนวิทยา',
    'บ้านหลวงแจ้ซ้อน',
    'บ้านแม่แจ๋ม',
    'บ้านทุ่งจี้',
    'บ้านทุ่งปง',
    'บ้านเฮี้ย',
    'ปลายนาวิทยา',
    'บ้านถ้ำ',
    'อนุบาลเมืองปาน (บ้านทุ่งกว๋าว)',
    'บ้านจ๋ง',
    'บ้านทุ่งข่วง',
    'บ้านทุ่งส้าน',
    'บ้านหนอง',
    'บ้านแม่กองปิน',
    'บ้านขอวิทยา',
    'บ้านทุ่งสะแกง',
    'บ้านป่าเหว',
    'บ้านปางดะ',
    'บ้านม่วง',
    'บ้านดอนแก้ว',
    'บ้านดอนไชย',
    'บ้านน้ำจำ',
    'บ้านแพะ',
    'บ้านทุ่งโป่ง',
    'บ้านต้นงุ้น',
    'บ้านขาม',
    'บ้านกล้วย',
    'บ้านหัวเมือง',
    'บ้านไร่',
    'เมืองปานวิทยา',
    'วังเหนือวิทยา',
    'ทุ่งกว๋าววิทยาคม',
    'เมืองปานพัฒนวิทย์',
    'ทุ่งอุดมวิทยา',
    'แจ้ห่มวิทยา',
    'บ้านเหล่าป่าสา',
    'บ้านขุนฝาง',
    'บ้านบ่อพระ',
    'บ้านหาดเสือเต้น',
    'บ้านคุ้งตะเภา',
    'ป่าขนุนเจริญวิทยา',
    'ป่ากล้วยเสริมวิทยา',
    'บ้านหัวหาด',
    'วัดศรีธาราม',
    'ชุมชนเมืองปากฝาง',
    'บ้านงิ้วงาม',
    'บ้านน้ำไผ่',
    'มิตรภาพ 39(ส.ท.ถนอม อุประอนุสรณ์)',
    'บ้านวังถ้ำ',
    'บ้านห้วยฉลอง',
    'วัดอรัญญิการาม',
    'วัดดอย',
    'ม่อนดินแดงวิทยาคม',
    'พิชัยดาบหัก 1',
    'อนุบาลอุตรดิตถ์',
    'น้ำริดราษฎร์บำรุง',
    'ชายเขาวิทยา',
    'บ้านไผ่ล้อมสามัคคี',
    'วัดคุ้งวารี',
    'บ้านป่าเซ่า(ประชานุสรณ์)',
    'บ้านเด่นด่าน',
    'สามัคยาราม',
    'วัดคุ้งยาง',
    'ปางต้นผึ้ง',
    'ราษฎร์อุปถัมภ์สาขาหนองน้ำเขียว',
    'ราษฎร์อุปถัมภ์',
    'บ้านน้ำไคร้',
    'ชุมชนด่านวิทยา',
    'วัดแม่เฉย',
    'ราษฎร์อํานวย',
    'ประชาชนอุทิศ',
    'ราษฎร์ดําริ',
    'หนองกลาย(ราษฎร์อุทิศวิทยา)',
    'สวนหลวงสาธิตสปจ.อุตรดิตถ์',
    'วัดทุ่งเศรษฐี',
    'วัดวังยาง',
    'หนองบัววิทยา',
    'วัดผาจักร',
    'วัดคลองนาพง สาขาวัดผาจุก',
    'วัดคลองนาพง',
    'วัดคลองนาพง สาขาม่อนหินขาว',
    'วัดพระฝาง',
    'บ้านวังโป่ง',
    'บ้านป่าถ่อน',
    'วัดวังกะพี้(วิเชียรประชานุกูล)',
    'วัดท่าทอง(นิมมานโกวิทพิทยา)',
    'บ้านหนองป่าไร่',
    'บ้านวังดิน',
    'สามัคคีวิทยา',
    'วัดสิงห์ศรีสว่าง',
    'วัดดอยแก้ว',
    'บ้านทับใหม่(ราษฎร์บำรุง)',
    'วัดวังหมู(นิมมานโกวิท)',
    'บ้านซ่านสามัคคี',
    'ไผ่ใหญ่(เกษตรกรอุปถัมภ์)',
    'บ้านดงช้างดี',
    'นาน้อยวิทยา',
    'วัดช่องลม',
    'บ้านหาดงิ้ว',
    'บ้านวังแดง',
    'บ้านแหลมทอง(ราษฎร์สามัคคี)',
    'บ้านแหลมถ่อนสามัคคี',
    'ศรีอุทิศ',
    'บ้านข่อยสูง',
    'บ้านน้ำอ่าง(สนง.สลากกินแบ่งสงเคราะห์ที่ 163)',
    'บ้านไชยมงคล',
    'บ้านเหล่า',
    'บ้านชําทอง',
    'ร้องประดู่(หนุนเภา-ประชาสงเคราะห์)',
    'หมู่สี่พัฒนา',
    'พงสะตือ(รัฐ-ประชาสรรค์)',
    'ชุมชนบ้านแก่ง(ไกรสรพงษ์สงเคราะห์)',
    'หมู่ห้าสามัคคี',
    'บ้านใหม่(เยาวชนประชาสงเคราะห์)',
    'วัดบ้านใหม่',
    'บ้านวังแดง(หมู่สองสามัคคี)',
    'บ้านหัวดาน',
    'บ้านวังแดง(สหจิตวิทยาคาร)',
    'ชุมชนบ้านวังหิน',
    'บ้านท่าอวน',
    'บ้านเด่นสําโรง',
    'บ้านบึงพาด(นิคมราษฎร์สงเคราะห์)',
    'บ้านหาดสองแคว',
    'บ้านคลองกล้วย',
    'บ้านปากคลอง',
    'บ้านกองโค',
    'บ้านท่าเดื่อ',
    'บ้านป่าแต้ว',
    'บ้านบึงท่ายวน',
    'บ้านป่ากะพี้',
    'วัดดอกไม้',
    'บ้านหาดทับยา',
    'บ้านท่าสัก',
    'บ้านชําสอง',
    'บ้านชําหนึ่ง',
    'บ้านชําตก',
    'บ้านท่าสัก สาขาบ้านเต่าไหใต้',
    'บ้านเต่าไหเหนือ',
    'บ้านหนองกวาง',
    'บ้านนายาง',
    'บ้านหลักร้อย',
    'บ้านนาอิซาง',
    'บ้านนาคะนึง',
    'แหลมนกแก้ว',
    'บ้านฟากบึง',
    'บ้านนาอิน',
    'บ้านหน้าพระธาตุ',
    'บ้านคลองละวาน',
    'บ้านหัวค่าย',
    'บ้านคลองกะชี',
    'บ้านในเมือง',
    'ชุมชนบ้านโคน',
    'บ้านคลองเรียงงาม',
    'บ้านคลองกะพั้ว',
    'บ้านดารา',
    'วัดบ้านเกาะ',
    'บ้านวังสําโม',
    'บ้านวังสะโม',
    'บ้านโรงหม้อมิตรภาพที่ 75',
    'บ้านท้ายน้ำ',
    'วัดโรงม้า',
    'บ้านดอนโพ',
    'วัดพญาปันแดน',
    'ชุมชนบ้านดง',
    'บ้านขอม',
    'บ้านท่ามะปราง',
    'บ้านทุ่งป่ากระถิน',
    'บ้านคลองละมุง',
    'วัดวังผักรุง',
    'บ้านดินแดง',
    'วัดไร่อ้อย',
    'ไร่อ้อย',
    'วัดน้ำใส',
    'วัดนาทะเล',
    'วัดห้องสูง',
    'บ้านชําผักหนาม',
    'บ้านห้วยทราย',
    'ด่านแม่คํามัน',
    'ฤาษีพหลวิทยา',
    'ศิลปศาสตร์วิทยา',
    'บ้านหัวดุม',
    'อนุบาลไทยรัฐวิทยา 5(วัดตลิ่งต่ำ)',
    'บ้านห้วยขมิ้น',
    'ชุมชนวัดบรมธาตุ',
    'บ้านเนินซ่าน',
    'นานกกก',
    'วัดดงสระแก้ว',
    'วัดโพธิ์ทอง',
    'วัดสว่าง',
    'วัดไผ่ล้อม',
    'ชุมชนไผ่ล้อมวิทยา',
    'วัดเจดีย์',
    'วัดท้องลับแล(ธรรมเนตรโศภณ)',
    'หนองนาเกลือวิทยา',
    'วัดดอนสัก',
    'วัดใหม่',
    'บ้านห้วยใต้',
    'ชุมชนหัวดง',
    'ดอนแก้ว',
    'บ้านน้ำพี้มิตรภาพที่ 214',
    'บ้านห้วยปลาดุก',
    'บ้านหนองหิน',
    'บ้านแสนขัน',
    'บ้านแพะ',
    'บ้านน้ำลอก(รัฐราษฎร์รังสรรค์)',
    'บ้านปางหมิ่น',
    'บ้านท่าช้าง',
    'บ้านวังเบน(ภูธรอุปถัมภ์)',
    'เลอเดชประชานุสรณ์',
    'บ้านนาลับแลง',
    'บ้านไร่ห้วยพี้',
    'บ้านนาป่าคาย',
    'บ้านวังปรากฏ',
    'บ้านเขาสัก',
    'บ้านน้ำหมีใหญ่',
    'วังโป่งดํารงวิทย์',
    'บ้านร้องลึก',
    'บ้านปางวุ้น',
    'บ้านผักขวง',
    'อุตรดิตถ์',
    'อุตรดิตถ์วิทยา',
    'ตรอนตรีสินธุ์',
    'อุตรดิตถ์ดรุณี',
    'บ้านโคนพิทยา',
    'ดาราพิทยาคม',
    'ทุ่งกะโล่วิทยา',
    'แสนตอวิทยา',
    'วังกะพี้พิทยาคม',
    'น้ำริดวิทยา',
    'พิชัย',
    'ลับแลศรีวิทยา',
    'ด่านแม่คำมันพิทยาคม',
    'ลับแลพิทยาคม',
    'ทองแสนขันวิทยา',
    'ท่าปลาอนุสรณ์ 2',
    'บ้านสีเสียดบํารุง',
    'นิคมสร้างตนเองลำน้ำน่านสงเคราะห์ 5',
    'คุรุประชาสรรค์ศึกษา',
    'จริมอนุสรณ์ 2',
    'ท่าแฝกอนุสรณ์ 4',
    'บ้านกิ่วเคียน',
    'ท่าปลาอนุสรณ์ 1',
    'นิคมสร้างตนเองลำน้ำน่านสงเคราะห์ 4',
    'น้ำสิงห์(สนง.สลากกินแบ่งสงเคราะห์ที่78)',
    'สิงห์คอมพิทยา',
    'สว่างวิทย์',
    'บ้านท่าเรือ',
    'ป่ากั้งวิทยา',
    'บ้านห้วยผึ้ง',
    'ไผ่งามวิทยา',
    'บ้านงอมมด',
    'บ้านงอมถ้ำ',
    'ราชประชานุเคราะห์ 13',
    'น้ำพร้าสามัคคี',
    'บ้านห้วยต้าวิทยา',
    'บ้านน้ำหมัน',
    'บ้านวังหัวดอย',
    'บ้านนาต้นโพธิ์',
    'บ้านน้ำลี',
    'บ้านน้ำต๊ะ',
    'บ้านทรายงามวิทยา',
    'จริมอนุสรณ์1',
    'เจริญราฎร์อุปถัมภ์',
    'บ้านห้วยสีเสียด',
    'บ้านย่านดู่',
    'บ้านซําบ้อ',
    'บ้านปากห้วยฉลอง',
    'ผาเต่าพัฒนา',
    'ชุมชนผาเลือดวิทยาคาร',
    'อนุบาลท่าปลา(ชุมชนร่วมจิต)',
    'ประชานิคมวิทยา',
    'นิคมสร้างตนเองลําน้ำน่านสงเคราะห์ 3',
    'ท่าแฝกอนุสรณ์ 1',
    'นิคมสร้างตนเองลําน้ำน่านสงเคราะห์ 2',
    'นิคมสร้างตนเองลำน้ำน่านสงเคราะห์ 1',
    'ท่าแฝกอนุสรณ์ 2',
    'นิคมสร้างตนเองลําน้ำน่านสงเคราะห์ 6',
    'บ้านต้นม่วง',
    'บ้านเด่นเหล็ก',
    'บ้านท่าโพธิ์',
    'บ้านปางเกลือ',
    'บ้านวังผาชัน',
    'ชุมชนบ้านนากล่ำ',
    'บ้านห้วยแมง',
    'บ้านน้ำไคร้',
    'บ้านนาผักฮาด',
    'บ้านนาผักฮาดสาขาบ้านห้วยตาด',
    'บ้านห้วยคอม',
    'บ้านห้วยเนียม',
    'บ้านต้นขนุน',
    'บ้านห้วยคอมสาขาบ้านเพีย',
    'บ้านห้วยเดื่อ',
    'บ้านห้วยคอมสาขาไทยเสรีอุตสาหกรรม',
    'บ้านม่วง',
    'บ้านทรายขาว',
    'บ้านหนองแห้ว',
    'บ้านม่วงสาขาบ้านชำบุ่น',
    'ชุมชนไกรลาสฯสาขาวัดศรีสะอาดโพธิ์ชัย',
    'บ้านใหม่',
    'ชุมชนไกรลาสฯสาขาบ้านนากวางกองทัพบกอุทิศ',
    'บ้านปากปาด',
    'สมพงษ์วิทยา',
    'ชุมชนไกรลาสวิทยาคม',
    'บ้านห้วยไคร้',
    'บ้านห้วยหูด',
    'บ้านห้วยมุ่น',
    'บ้านห้วยโป่ง',
    'บ้านโป่งปุ้น',
    'บ้านโป่งพาน',
    'บ้านห้วยมุ่นสาขากุญชรวิทยา',
    'ชุมชนวัดมหาธาตุ',
    'วัดวังอ้อ',
    'บ้านห้วยลึก',
    'บ้านนาไพร',
    'บ้านห้วยก้านเหลือง',
    'วัดโพธิ์ชัย',
    'บ้านนาหน่ำ',
    'วัดปากไพร',
    'บ้านไร่ตีนตก',
    'วัดวังกอง',
    'วัดโพนดู่',
    'อนุบาลฟากท่า(วัดวังขวัญวันครู 2503)',
    'บ้านห้วยสูน',
    'วัดหนองหิน',
    'บ้านชําบอน',
    'บ้านนาแซง',
    'บ้านเดิ่น',
    'บ้านนาไร่เดียว',
    'บ้านสุมข้าม',
    'บ้านม่วงชุม',
    'วัดนาขุม',
    'บ้านนาบัว',
    'บ้านปางคอม',
    'บ้านบ่อเบี้ย',
    'บ้านเด่นชาติ',
    'บ้านห้วยยาง',
    'บ้านน้ำแพ',
    'อนุบาลบ้านโคก',
    'บ้านห้วยครั่ง',
    'วัดจอมแจ้ง',
    'บ้านห้วยยศ',
    'วัดวังสัมพันธ์',
    'ชด.ศึกษาสงเคราะห์ 1',
    'บ้านน้ำลัดสามัคคี',
    'ฟากท่าวิทยา',
    'ท่าปลาประชาอุทิศ',
    'บ้านโคกวิทยาคม',
    'น้ำปาดชนูปถัมภ์',
    'บ้านหัวฝาย(ประชาสารสงเคราะห์)',
    'บ้านดอนดี(ประชานุกูล)',
    'วัดกาญจนาราม(ประชาชนูทิศ)',
    'บ้านพันเชิง(วงศ์หล้าราษฎร์บำรุง)',
    'วัดต้นไคร้',
    'บ้านใน(อินทราษฎร์บำรุง)',
    'บ้านนาตอง(นาตองสามัคคี)',
    'บ้านท่าขวัญ(ราษฎร์บำรุง)',
    'รัฐราษฎร์บำรุง',
    'บ้านนาแหลม(กองแก้วราษฎร์อุปถัมภ์)',
    'บ้านทุ่งโฮ้ง(อภิวังวิทยาลัย)',
    'อนุบาลแพร่',
    'บ้านแต(กวีรัตน์ราษฎร์บำรุง)',
    'บ้านเหล่า(พงษ์เกีรยติประชาสรรค์)',
    'บ้านนาจักร(จักรราษฎร์บำรุง)',
    'บ้านน้ำชำ(วิชัยชนานุเคราะห์)',
    'วัดเมธังกราวาส(เทศรัฐราษฎร์นุกูล)',
    'บ้านในเวียง',
    'บ้านถิ่น(ถิ่นวิทยาคาร)',
    'บ้านโป่งศรี',
    'บ้านแม่ลัว(วิริยะราษฎร์สงเคราะห์)',
    'บ้านน้ำกลาย',
    'บ้านหนองแขม(หนองแขมประชาสรรค์)',
    'บ้านป่าแดง(วันรัตตวิทยา)',
    'บ้านน้ำชำ(ประชาราษฎร์สามัคคี)',
    'บ้านสุพรรณ(คุรุราษฎร์วิทยา)',
    'วัดมณีวรรณ(ประชารัฐวิทยาคาร)',
    'วัดน้ำโค้ง(คำหล้าราษฎร์นุกูล)',
    'วัดศรีภูมิ(ศึกษาประชานุสรณ์)',
    'บ้านวังช้าง(ธีรราษฎร์รังสฤษฏ์)',
    'บ้านต้นม่วง(ธรรมบาลวิทยา)',
    'บ้านสันป่าสัก',
    'บ้านนันทาราม(นันทราษฎร์วิทยา)',
    'วัดบุญเจริญ(รัฐราษฎร์รังสรรค์)',
    'บ้านแม่หล่ายกาซ้อง(ลือราษฎร์อุปถัมภ์)',
    'บ้านแม่หล่าย(ประชานุสรณ์)',
    'บ้านร่องฟอง',
    'บ้านเวียงตั้ง(คำวรรธประชานุกูล)',
    'บ้านปากห้วย',
    'บ้านวังหงส์(วังหงส์วิทยาคาร)',
    'บ้านสวนเขื่อน(ราษฎร์บูรณวิทยา)',
    'บ้านทุ่งเหนือ(ราษฎร์สามัคคี)',
    'บ้านแม่แคม',
    'บ้านนาคูหา',
    'บ้านดง(ธรรมาราษฎร์นุสรณ์)',
    'บ้านปง(ป้อมประชานุกูล)',
    'วัดทุ่งล้อม(ทองประชานุเคราะห์)',
    'บ้านศรีสิทธิ์(ใจมาอนุเคราะห์)',
    'บ้านห้วยม้า(สุนทรนิวาส)',
    'บ้านกาซ้อง',
    'ชุมชนบ้านเหมืองหม้อ(เหมืองหม้อสามัคคี)',
    'บ้านปทุม(ถาวรราษฎร์ศึกษา)',
    'วัดเหมืองค่า(ประชารัฐอุปถัมภ์)',
    'บ้านผารางวังหม้อ(มิตรประชา)',
    'ชุมชนบ้านทุ่งศรี(ราษฎร์รังสฤษฎ์)',
    'บ้านน้ำเลา(น้ำเลาวิทยาคาร)',
    'บ้านน้ำเลาเหนือ(เหรียญราษฎร์อนุสรณ์)',
    'บ้านบุญแจ่ม(ประชาสามัคคี)',
    'บ้านไผ่ย้อย',
    'บ้านห้วยฮ่อม',
    'บ้านอ้อยวิทยาคาร',
    'บ้านเวียง(เวียงราษฎร์บำรุง)',
    'บ้านป่ากล้วย',
    'บ้านห้วยอ้อย',
    'บ้านบุญเริง(ประชานุสรณ์)',
    'บ้านไผ่โทน(นวลราษฎร์วิทยา)',
    'บ้านปางยาว(รัฐราษฎร์ผดุงวิทย์)',
    'บ้านแม่กระทิง',
    'บ้านวังปึ้ง',
    'บ้านทุ่งคัวะ',
    'บ้านแม่ทราย(คุรุราษฎร์เจริญวิทย์)',
    'บ้านแม่ยางกาด(มิตรภาพที่ 13)',
    'บ้านโทน(ราษฎร์อุทิศ)',
    'บ้านหนองเจริญ(ประชาพัฒนา)',
    'บ้านแม่ยางตาล(กิตติประชานุกูล)',
    'บ้านดอนชุม(พิริยเทพวงศ์อนุสรณ์)',
    'บ้านแม่ยางร้อง(แม่ยางประชานุเคราะห์)',
    'บ้านแม่ยางยวง(ลูนราษฎร์สงเคราะห์)',
    'บ้านแม่ยางเปี้ยว(แก้วพิสณฑ์ประชานุกูล)',
    'บ้านแม่ยางโพธิ์(โพธิ์ราษฎร์รังสรรค์)',
    'บ้านวังโป่ง(ประชาบำรุง)',
    'บ้านร้องกวาง(จันทิมาคม)',
    'บ้านดอนมูล',
    'ชุมชนบ้านร้องเข็ม(ร้องเข็มวิทยาคาร)',
    'บ้านใหม่จัดสรร',
    'บ้านน้ำโค้ง(ประชานิมิตร)',
    'บ้านห้วยโรงนอก',
    'บ้านแดนชุมพล(ประชานุกูล)',
    'บ้านโทกค่า(สหราษฎร์บำรุง)',
    'บ้านเตาปูน(พิไชยาประชานุกูล)',
    'บ้านนาไร่เดียว(ประชาบำรุง)',
    'บ้านแม่แรม',
    'บ้านป่าแดง(รัฐราษฎร์รังสรรค์)',
    'วัดร่องเย็น(สารแก้วประชาสงเคราะห์)',
    'ไทยรัฐวิทยา 31(ทุ่งน้าวพุทธิมาศึกษากร)',
    'บ้านวังดิน(วังดินราษฎร์บำรุง)',
    'บ้านหนองสุวรรณ(ราษฎร์สงเคราะห์)',
    'ชุมชนบ้านกลาง(สนิทวิทยาการ)',
    'บ้านเทพ(เทพสุนทรินทร์)',
    'ธาตุพระลอ(รามราษฎร์บูรณะ)',
    'บ้านคุ้ม(ประสารราษฎร์วิทยา)',
    'ชุมชนบ้านหนุนเหนือ(แสงศรีวิทยา)',
    'บ้านหนุนใต้(หนุนราษฎร์บำรุง)',
    'บ้านศรีมูลเรือง(วัลลภราษฎร์รังสรรค์)',
    'บ้านลอง(ฟองจันทร์ราษฎร์อุปถัมภ์)',
    'บ้านแม่พร้าว',
    'บ้านท่าวะ(ราษฎร์บำรุง)',
    'บ้านห้วยโป่ง(ประชานุเคราะห์)',
    'บ้านดอนชัย(ประชาอุทิศ)',
    'บ้านนาฝาย(ประชาสงเคราะห์)',
    'บ้านนาหลวง(ประชาพัฒนา)',
    'บ้านป่าเลา(ประชานุสรณ์)',
    'ดอนแก้ววิทยาคาร',
    'บ้านลู(คำภีร์ราษฎร์บำรุง)',
    'วัดห้วยหม้าย(หมายประชากร)',
    'บ้านห้วยขอน(ญาณศรัทธาสามัคคี)',
    'บ้านดอนแก้ว(อาสาฯ)',
    'บ้านห้วยกาน(รัฐราษฎร์อุปถัมภ์)',
    'บ้านต้นหนุน(หนุนราษฎร์สามัคคี)',
    'บ้านสันปู่สี(ราษฎร์วิทยากุล)',
    'บ้านหนองเสี้ยว',
    'บ้านวังฟ่อน(สิทธิราษฎร์บำรุง)',
    'บ้านดงเจริญ(วงค์ประชานุกูล)',
    'บ้านหัวเมือง(แหวนวังประชาบำรุง)',
    'วัดแม่ทะ(ดำรงประชานุเคราะห์)',
    'แม่คำมีตำหนักธรรม(ตำหนักธรรมวิทยาคาร)',
    'บ้านสะเลียม(ปัญญาราษฎร์รังสรรค์)',
    'บ้านทุ่งแค้ว (คันธวงศ์วิทยาคาร)',
    'บ้านหนองน้ำรัด (พรหมาประชาสงเคาระห์)',
    'บ้านย่านยาว(ย่านยาวประชาสรรค์)',
    'บ้านแม่คำมี (รัตนปัญญา)',
    'บ้านวังหลวง',
    'บ้านหนองม่วงไข่',
    'นารีรัตน์จังหวัดแพร่',
    'พิริยาลัยจังหวัดแพร่',
    'เมืองแพร่',
    'ถิ่นโอภาสวิทยา',
    'ท่าข้ามวิทยาคม',
    'ห้วยม้าวิทยาคม',
    'แพร่ปัญญานุกูล',
    'ร้องกวางอนุสรณ์',
    'ราชประชานุเคราะห์ 25',
    'สองพิทยาคม',
    'เวียงเทพวิทยา',
    'ม่วงไข่พิทยาคม',
    'พัฒนาประชาอุปถัมภ์',
    'บ้านน้ำริน(คุรุราษฎร์รังสรร)',
    'บ้านผามอก(ประชานุเคราะห์)',
    'บ้านปง(วัฒนาวิทยาคาร)',
    'อิมอรัญ',
    'บ้านศรีดอนไชย(ประชานุกูล)',
    'บ้านปากจอกวิทยา',
    'บ้านวังเลียง',
    'บ้านอ้ายลิ่ม(ลิ่มราษฎร์สามัคคี)',
    'บ้านผาจั๊บ(คุรุราษฎร์อนุกูล)',
    'ประชารัฐวิทยาคาร',
    'บ้านแม่รัง(จัดสรรราษฎร์สงเคราะห์)',
    'บ้านค้างตะนะวิทยา',
    'บ้านนาตุ้ม(คุรุราษฎร์วิทยานุสรณ์)',
    'บ้านแม่หลู้(รัฐราษฎร์รังสรรค์)',
    'บ้านบ่อ',
    'บ้านผาคันวิทยา',
    'อนุสรณ์รัฐประชา',
    'บ้านห้วยแม่ต้า(ราษฎร์รัฐพัฒนา)',
    'ชุมชนบ้านปิน(ปินประชาราษฎร์)',
    'บ้านทุ่งตากล้า(คุรุราษฎร์บำรุงวิทย์)',
    'บ้านปากกาง(ประชาราษฎร์รังสฤษฎ์)',
    'บ้านไฮสร้อย(คุรุราษฎร์บำรุง)',
    'บ้านท่าเดื่อ',
    'บ้านแม่ปาน(แม่ปานประชาสงเคราะห์)',
    'บ้านแก่งหลวง(รถไฟราษฎร์บพิธ)',
    'บ้านเหล่า(รัฐราษฎร์บำรุง)',
    'บ้านแหลง(คุรุประชาสามัคคี)',
    'บ้านม่อน(ปัญญาฉลาดราษฎร์นุกูล)',
    'บ้านแป้น(รัฐราษฎร์วิทยา)',
    'รักเมืองไทย 2',
    'บ้านแม่จองไฟ(รัฐราษฎร์พัฒนา)',
    'บ้านดอนมูล(สุวรรณสาราษฎร์บำรุง)',
    'บ้านนาหลวง(สลากกินแบ่งสงเคราะห์)',
    'บ้านนาแก',
    'บ้านแม่ลาน(ราษฎร์อุทิศ)',
    'ชุมชนจรูญลองรัตนาคาร',
    'บ้านห้วยอ้อ(ห้วยอ้อวิทยาคม)',
    'บ้านนาอุ่นน่อง(ประชารัฐวิทยาคาร)',
    'บ้านหัวทุ่ง(สามัคคีวิทยาคาร)',
    'บ้านเค็ม(รัฐประสาทราษฎร์บำรุง)',
    'บ้านแม่จอก(ราษฎร์บำรุงวิทยา)',
    'บ้านไผ่ล้อม(สำนักงานสลากกินแบ่งสงเคราะห์ 46',
    'ดอนมูลวิทยาคาร',
    'บ้านดอนแท่น(ประชานุสรณ์)',
    'บ้านหัวดง',
    'บ้านค่างาม(อัตตราษฎร์บำรุง)',
    'บ้านร่องแค(ป่าผึ้งสามัคคี)',
    'บ้านร่องแดง(ชัยมาประชารัฐอุปถัมภ์)',
    'บ้านร่องเสี้ยว(รัฐราษฎร์ผดุงวิทย์)',
    'ดอนแก้ว(รัฐประชาสรรค์)',
    'ชุมชนบ้านบวกโป่ง(พรหมราษฎร์อุปถัมภ์)',
    'บ้านกวาง(ลือราษฎร์วัฒนา)',
    'วัดมงคลถาวร(มงคลวิทยาสรรค์)',
    'บ้านกาศ(วัฒนาประชานุกูล)',
    'บ้านม่วง(ม่วงเกษมวิทยาคาร)',
    'บ้านปงท่าข้าม(ปงประชานุกูล)',
    'บ้านปงหัวหาด(หัวหาดราษฎร์บำรุง)',
    'บ้านน้ำฮอก(รัฐราษฎร์วิทยา)',
    'บ้านเหล่า(พิชัยศิริราษฎร์รังสรรค์)',
    'วัดสิทธิวิมล(สุวรรณราษฎร์ปัญญา)',
    'วัดพระหลวง(อภิชัยราษฎร์สงเคราะห์)',
    'บ้านปงพร้าว(ประชารัฐวิทยา)',
    'บ้านแม่สายดอนทัน(คำแวงประชาอุทิศ)',
    'บ้านร่องกาศ(ศรีวิทยาคาร)',
    'บ้านร่องกาศใต้(รัฐราษฎร์บูรณะ)',
    'วัดตอนิมิตร(สิทธิราษฎร์รังสรรค์)',
    'บ้านเขื่อนสุนทร(รัฐประชาวิทยา)',
    'บ้านผาสุก(ราษฎร์บำรุง)',
    'บ้านเวียงทอง(เวียงทองวิทยาคาร)',
    'บ้านวังวน(ราษฎร์วิทยา)',
    'บ้านสบสาย(จันทร์พัฒน์ประชาสรรค์)',
    'สูงเม่นวิทยาคาร',
    'วัดสร่างโศก',
    'วัดโชคเกษม(เกษมราษฎร์บำรุง)',
    'บ้านดอนชัย(กันทราษฎร์วิทยาคาร)',
    'บ้านทุ่งเจริญ(เครือราษฎร์สงเคราะห์)',
    'ช่องลมจันทิมาประชาพิทักษ์',
    'วัดศรีดอก(ประชาพัฒนา)',
    'ชุมชนบ้านปากพวก',
    'บ้านห้วยกูด',
    'เด่นทัพชัย',
    'เด่นไชยประชานุกูล',
    'บ้านน้ำโค้ง',
    'บ้านบ่อแก้ว',
    'บ้านป่าไผ่',
    'บ้านปางเคาะ',
    'บ้านไทรย้อย',
    'บ้านปากปาน',
    'บ้านเด่นชุมพล',
    'ชุมชนบ้านปงป่าหวาย',
    'บ้านแม่ยุ้น',
    'บ้านสวนหลวง',
    'บ้านแม่จั๊วะ',
    'บ้านดงสุระ',
    'บ้านห้วยไร่',
    'บ้านแม่พวก',
    'บ้านห้วยลากปืน',
    'บ้านน้ำแรม',
    'บ้านแม่แปง',
    'บ้านนาปลากั้ง',
    'บ้านวังลึก',
    'บ้านเปาปมดงยาง(มิตรภาพที่ 155)',
    'บ้านไร่หลวง',
    'บ้านนาพูน',
    'บ้านป่าสัก',
    'บ้านแม่กระต๋อม',
    'บ้านสองแคว',
    'บ้านค้างใจ',
    'บ้านแม่สิน',
    'บ้านสบป้าก',
    'บ้านสลก',
    'บ้านแช่ฟ้า',
    'บ้านนาฮ่าง',
    'บ้านแม่บงเหนือ',
    'ชุมชนบ้านแม่ป้าก',
    'บ้านแม่จอก',
    'บ้านแม่บงใต้',
    'บ้านวังขอนป่าไผ่',
    'บ้านวังกวาง',
    'บ้านค้างปินใจ',
    'บ้านค้างคำปัน',
    'บ้านขุนห้วย',
    'บ้านแม่พุง',
    'บ้านปางมะโอ',
    'สวนป่าแม่สรอย',
    'บ้านแม่แฮด',
    'บ้านแม่ตื้ด',
    'บ้านป่าคาป่าม่วง',
    'บ้านปางไฮ',
    'บ้านวังชิ้น',
    'บ้านวังเบอะ',
    'บ้านวังแฟน',
    'บ้านป่ายุบ',
    'บ้านหาดรั่ว',
    'บ้านใหม่(สมบัติทวีประชาสรรค์)',
    'บ้านนาใหม่',
    'บ้านแพะทุ่งเจริญ',
    'บ้านแม่ขมิง',
    'บ้านแม่ขมวก',
    'บ้านม่วงคำ',
    'ชุมชนบ้านแม่หละป่าป๋วย',
    'ลองวิทยา',
    'สูงเม่นชนูปถัมภ์',
    'บ้านกาศประชานุเคราะห์',
    'วิไลเกียรติอุปถัมภ์',
    'วังชิ้นวิทยา',
    'สรอยเสรีวิทยา',
    'บ้านนาผา',
    'บ้านน้ำครกใหม่',
    'บ้านดอนน้ำครก',
    'บ้านธงหลวง',
    'บ้านน้ำครกเก่า',
    'บ้านไชยสถาน',
    'บ้านศรีเกิด',
    'ชุมชนบ้านดอนมูล',
    'บ้านดู่ใต้',
    'บ้านธงน้อย',
    'บ้านถืมตอง',
    'บ้านฝายแก',
    'บ้านซาวหลวง',
    'บ้านนวราษฎร์',
    'บ้านนาซาว',
    'ดอนสะไมย์วิทยา',
    'บ้านดอน(ศรีเสริมกสิกร)',
    'ราชานุบาล',
    'บ้านวังหมอ',
    'บ้านสะละภูเวียง',
    'บ้านห้วยยื่น',
    'บ้านสันติภาพ',
    'บ้านทรายทอง',
    'บ้านน้ำงาว',
    'บ้านผาขวาง',
    'บ้านผาตูบ',
    'บ้านผาสิงห์',
    'บ้านศรีนาป่าน',
    'บ้านห้วยมอญ',
    'บ้านเรือง',
    'บ้านดอนเฟือง',
    'บ้านม่วงเจริญราษฎร์',
    'บ้านบ่อสวก',
    'บ้านป่าคา',
    'บ้านต้าม',
    'บ้านเชียงยืน',
    'บ้านน้ำโค้ง',
    'บ้านห้วยมอญสาขาใหม่ในฝัน',
    'บ้านสะเนียน',
    'บ้านสองแคว',
    'บ้านปางเป๋ย',
    'บ้านกาใส',
    'บ้านวังตาว',
    'บ้านห้วยปุก',
    'บ้านห้วยเฮือ',
    'บ้านห้วยเฮือสาขาห้วยระพี',
    'บ้านห้วยละเบ้ายา',
    'บ้านน้ำปาย',
    'บ้านป่าสัก',
    'บ้านน้ำปูน',
    'บ้านน้ำพาง',
    'มิตรมวลชน 3',
    'บ้านร่มเกล้า',
    'บ้านกิ่วน้ำสาขาน้ำตวง',
    'บ้านตอง',
    'บ้านก้อ',
    'บ้านแคว้ง',
    'บ้านหนองแดง',
    'บ้านพรหม',
    'ชุมชนบ้านนาคา',
    'บ้านกิ่วน้ำ',
    'อนุบาลแม่จริม',
    'บ้านบวกแรด',
    'บ้านห้วยบง',
    'บ้านพี้ใต้',
    'บ้านพี้เหนือ',
    'บ้านเป้า',
    'บ้านนาวี',
    'บ้านดู่',
    'บ้านฟ้า',
    'บ้านวังยาว',
    'รัฐราษฎร์นุเคราะห์',
    'บ้านดอนหล่ายทุ่ง',
    'ชุมชนบ้านหลวง',
    'บ้านดอย',
    'บ้านน้ำหิน',
    'บ้านเชียงของ',
    'บ้านห้วยเลา',
    'บ้านนาหล่าย',
    'บ้านบุ้ง',
    'บ้านนาราบ',
    'ชุมชนบ้านนาหลวง',
    'บ้านพืชเจริญ',
    'บ้านน้ำพุ',
    'บ้านเปา',
    'ชุมชนบ้านอ้อย',
    'บ้านนาไค้',
    'บ้านทัพม่าน',
    'บ้านใหม่',
    'บ้านหัวเมือง',
    'บ้านน้ำหก',
    'บ้านหนองบัว',
    'บ้านหนองห้ามิตรภาพที่ 125',
    'บ้านศาลา',
    'บ้านสถาน',
    'บ้านนา',
    'บ้านน้ำลัด',
    'บ้านไร่น้ำหิน',
    'บ้านสันทะ',
    'บ้านเชตวัน',
    'บ้านห้วยส้ม',
    'บ้านห้วยจอย',
    'บ้านส้าน',
    'ประกิตเวชศักดิ์',
    'ศรีเวียงสาวิทยาคาร',
    'ไทยรัฐวิทยา 45',
    'บ้านดอนไชย',
    'บ้านไชยสถาน',
    'บ้านหนองนก',
    'บ้านขึ่งงามมงคล',
    'บ้านท่าลี่',
    'บ้านหลับมืนพรวน',
    'จอมจันทร์วิทยาคาร',
    'บ้านปางสา',
    'บ้านนาเคียน',
    'บ้านครกคำ',
    'บ้านนากอก',
    'บ้านป่าสัก',
    'ตาลชุมศึกษาลัย',
    'ทุ่งศรีทอง',
    'ศรีนาชื่น',
    'บ้านนาเหลืองไชยราม',
    'ริมฝั่งน่านวิทยา',
    'บ้านนาเหลืองใน',
    'ชุมชนบ้านน้ำปั้ว',
    'บ้านห้วยแก้วมิตรภาพที่ 51',
    'บ้านวังม่วง',
    'บ้านสาลี่',
    'บ้านน้ำมวบ',
    'บ้านแม่ขะนิง',
    'ภูเค็งพัฒนา',
    'ไตรธารวิทยาสาขาห้วยไฟ',
    'ไตรธารวิทยา',
    'บ้านหัวนา',
    'บ้านเอิน',
    'บ้านวัวแดง',
    'บ้านสาคร',
    'บ้านท่าเลอ',
    'บ้านจะเข้ภูหอม',
    'บ้านไพรอุดม',
    'บ้านฮากฮาน',
    'บ้านบ่อหอย',
    'บ้านสะเลียม',
    'บ้านห้วยหลอด',
    'บ้านป่าหุ่ง',
    'บ้านนาก้า',
    'บ้านผาเวียง',
    'บ้านสาลีก',
    'ชุมชนบ้านใหม่',
    'หาดทรายทองวิทยาคาร',
    'บ้านส้านนาหนองใหม่',
    'วัดท่าข้าม(สิริพรหมคุณ วิรุฬห์ศีลาจาร อุปถัมภ์',
    'บ้านนาสา',
    'บ้านไหล่น่าน',
    'บ้านบุญเรือง',
    'ริมฝั่งว้าวิทยา',
    'บ้านชมพู',
    'บ้านม่วงเนิ้ง',
    'บ้านฝั่งหมิ่น',
    'บ้านห้วยน้ำอุ่น',
    'บ้านพะเยา',
    'บ้านอ่ายนาผา',
    'บ้านนาไลย',
    'บ้านปงสนุก',
    'ชุมชนบ้านนาทะนุง',
    'บ้านค้างอ้อย',
    'บ้านน้ำเลา',
    'บ้านห้วยนาย',
    'บ้านน้ำเลาสาขาปากนาย',
    'บ้านคำเรือง',
    'บ้านนาบอน',
    'ชุมชนบ้านบ่อแก้ว',
    'บ้านนายาง',
    'บ้านปิงใน',
    'บ้านปิงหลวง',
    'บ้านน้ำแพะ',
    'บ้านน้ำลี',
    'บ้านน้ำเคิม',
    'บ้านนาคา',
    'บ้านป่าซาง',
    'บ้านน้ำอูน',
    'บ้านโป่งคำ',
    'บ้านดู่พงษ์',
    'บ้านสบยาง',
    'บ้านป่าอ้อย',
    'บ้านป่าแลวหลวงวิทยา',
    'บ้านป่าแดด',
    'บ้านศรีบุญเรือง',
    'บ้านห้วยแฮ้ว',
    'บ้านศรีนาม่าน',
    'ราษฎร์รัฐพัฒนา',
    'บ้านดอนไพรวัลย์',
    'บ้านปางช้าง',
    'บ้านหัวนา',
    'บ้านหนองรังมิตรภาพที่ 107',
    'บ้านม่วงใหม่',
    'บ้านน้ำลัดสบแก่น',
    'บ้านก้อด',
    'บ้านน้ำเกี๋ยน',
    'บ้านนาเหลืองม่วงขวา',
    'บ้านน้ำแก่นกลาง',
    'บ้านน้ำแก่นเหนือ',
    'บ้านทุ่งน้อย',
    'บ้านฝายแก้ว',
    'บ้านห้วยคำ',
    'บ้านห้วยไฮ',
    'บ้านป่าสัก',
    'บ้านท่าล้อ',
    'บ้านบุปผาราม',
    'บ้านหัวเวียงเหนือ',
    'บ้านม่วงตึ๊ด',
    'วัดพระธาตุแช่แห้ง',
    'บ้านเมืองจัง',
    'บ้านหาดเค็ด',
    'ศรีสวัสดิ์วิทยาคาร',
    'สตรีศรีน่าน',
    'บ่อสวกวิทยาคาร',
    'น่านประชาอุทิศ',
    'แม่จริม',
    'บ้านหลวง',
    'นาน้อย',
    'สา',
    'ตาลชุมพิทยาคม',
    'สาธุกิจประชาสรรค์ รัชมังคลาภิเษก',
    'ยาบหัวนา',
    'ศึกษาสงเคราะห์น่าน',
    'นาหมื่นพิทยาคม',
    'เมืองลีประชาสามัคคี',
    'สันติสุขพิทยาคม',
    'น่านนคร',
    'ศรีนครน่าน',
    'น่านปัญญานุกูล',
    'บ้านทุ่งกวาง',
    'บ้านพาน',
    'บ้านดอนมูล (แงง)',
    'บ้านนาก้อ',
    'บ้านปงสนุก',
    'บ้านนาวงศ์',
    'บ้านเสี้ยว',
    'บ้านแดนพนา',
    'บ้านหนาด',
    'ศรีสระวงศ์',
    'บ้านห้วยท่าง',
    'บ้านป่าลาน',
    'บ้านปรางค์',
    'บ้านร้อง',
    'บ้านขอน',
    'บ้านป่าหัด',
    'บ้านป่ากลาง(มิตรภาพที่ 166)',
    'สหราษฎร์บำรุง',
    'บ้านปางยาง',
    'ภูคาประชาสรรค์',
    'บ้านผาเวียง',
    'บ้านพนาสวรรค์',
    'บ้านเต๋ย',
    'วรนคร',
    'จอมแจ้งวิทยาคาร',
    'ชุมชนศิลาเพชร',
    'บ้านดอนมูล (ศิลาเพชร)',
    'ชุมชนศิลาแลง',
    'ไตรประชาวิทยา',
    'บ้านสกาดใต้',
    'สกาดพัฒนา',
    'บ้านนาฝาง',
    'บ้านส้าน',
    'จตุราษฎร์ศึกษา',
    'บ้านไร่',
    'บ้านทุ่งเฮ้า',
    'บ้านน้ำยาว',
    'บ้านน้ำยาวสาขาห้วยหลักลาย',
    'บ้านยู้',
    'บ้านน้ำฮาว',
    'บ้านนาฝ่า',
    'บ้านถ่อน',
    'บ้านสบหนอง',
    'บ้านสบสาย',
    'ตาลชุมมิตรภาพที่ 186',
    'บ้านปง',
    'บ้านน้ำป้าก',
    'บ้านวังว้า',
    'ริม-ป่าคาประชานุเคราะห์',
    'บ้านท่าวังผา',
    'ชุมชนไตรคามราษฎร์บำรุง',
    'บ้านฝายมูล',
    'บ้านหนองบัว',
    'บ้านสบขุ่น',
    'บ้านนาหนุน 2',
    'บ้านสบเป็ด',
    'บ้านน้ำโมงปางสา',
    'บ้านน้ำพุร้อน',
    'บ้านสันเจริญ',
    'บ้านน้ำลักใต้',
    'บ้านแหน',
    'บ้านน้ำแป่ง',
    'บ้านก๋ง',
    'บ้านเสี้ยว',
    'บ้านพร้าว',
    'ไตรราษฎร์วิทยา',
    'บ้านเชียงแล นาทราย(คุรุราษฎร์รังสฤษฎ์)',
    'บ้านท่าค้ำ',
    'ชุมชนบ้านดอนตัน',
    'บ้านม่วง',
    'บ้านคัวะ',
    'พัฒนานิคมวิทยา',
    'บ้านดอยติ้ว',
    'บ้านนาหนุน 1',
    'แสนทองวิทยา',
    'บ้านห้วยสะแตง',
    'บ้านน้ำลาด',
    'ไทยรัฐวิทยา98(บ้านงอบ)',
    'มณีพฤกษ์',
    'ชุมชนบ้านทุ่งช้าง',
    'บ้านน้ำสอดสาขาน้ำเพาะ',
    'บ้านปางแก',
    'บ้านแพะกลาง',
    'บ้านน้ำพิ',
    'บ้านปอน',
    'บ้านน้ำเลียง',
    'บ้านเวียงสอง',
    'บ้านน้ำสอด',
    'ชุมชนบ้านเฟือยลุง',
    'บ้านสบกอน',
    'ชุมชนบ้านเจดีย์',
    'ไตรราษฎร์สามัคคี',
    'บ้านกอก',
    'ไลออนส์บ้านหนองปลา',
    'บ้านหนอง',
    'บ้านชี',
    'บ้านดอนแท่น',
    'บ้านใหม่',
    'บ้านหนองผุก',
    'ชุมชนรัชดาภิเษกมิตรภาพที่ 115',
    'บ้านส้อเด่นพัฒนา',
    'บ้านดอนสบเปือ',
    'ไตรคามวิทยา',
    'บ้านน้ำอ้อ',
    'บ้านเกวต',
    'บ้านน้ำคา',
    'ไตรมิตรวิทยา',
    'บ้านผาน้ำย้อย',
    'ธาราบรรพต',
    'บ้านดอนแก้ว',
    'บ้านพร้าวกลาง',
    'บ้านสร้อยพร้าว',
    'บ้านตึ๊ด',
    'พระพุทธบาทวิทยา',
    'บ้านเหล่าอ้อ',
    'บ้านสะปัน',
    'บ้านสว้า',
    'บ้านนาขวาง',
    'บ้านบ่อหลวง',
    'บ้านนาคอก',
    'บ้านผักเฮือก',
    'บ้านยอดดอยวัฒนา',
    'ยอดดอยวัฒนาสาขาน้ำหมาว',
    'บ้านนาคอกสาขานาบง',
    'บ้านบ่อหลวงสาขาห้วยโป่ง',
    'บ้านบ่อหยวก',
    'บ้านขุนน้ำน่าน',
    'บ้านห่างทางหลวง',
    'บ้านสบมาง',
    'บ้านนากอก',
    'บ้านปางกอม',
    'บรรณโศภิษฐ์',
    'บ้านห้วยแกลบ',
    'บ้านสองแคว',
    'บ้านปางปุก',
    'สมาคมพยาบาลไทย',
    'บ้านน้ำโมง',
    'บ้านผาสิงห์',
    'บ้านยอด',
    'บ้านสะเกิน',
    'บ้านผาหลัก',
    'บ้านปางส้าน',
    'บ้านเปียงซ้อ',
    'บ้านห้วยฟอง',
    'บ้านด่าน',
    'บ้านด่านสาขาบวกหญ้า',
    'บ้านกิ่วจันทร์(เสรินทวัฒน์อุปถัมภ์)',
    'บ้านปางหก',
    'บ้านห้วยโก๋น',
    'บ้านน้ำช้างพัฒนา',
    'บ้านน้ำรีพัฒนา',
    'บ้านสบปืน',
    'บ้านห้วยทรายขาว',
    'หม่อมเจ้าเจริญใจจิตรพงศ์',
    'ปัว',
    'ศรัทธาศิลาเพชรรังสรรค์',
    'เมืองแงง',
    'มัธยมป่ากลาง',
    'ท่าวังผาพิทยาคม',
    'สารธรรมวิทยาคาร',
    'หนองบัวพิทยาคม',
    'เมืองยมวิทยาคาร',
    'ทุ่งช้าง',
    'เชียงกลาง(ประชาพัฒนา)',
    'พระธาตุพิทยาคม',
    'บ่อเกลือ',
    'ไตรเขตประชาสามัคคี รัชมังคลาภิเษก',
    'มัธยมพระราชทานเฉลิมพระเกียรติ',
    'บ้านจำป่าหวาย',
    'บ้านดาวเรือง(ธนาคารกรุงเทพ17)',
    'บ้านต๊ำพระแล สาขาบ้านต๊ำกลาง',
    'ต๊ำ(อินโต-ประชานุเคราะห์)',
    'บ้านป่าแดงบุญนาค',
    'บ้านเจดีย์งาม',
    'บ้านดอกบัว (บุญมานุเคราะห์)',
    'บ้านศาลา',
    'บ้านต๋อม',
    'บ้านร่องไผ่',
    'บ้านสันป่าม่วง',
    'บ้านร่องห้า',
    'บ้านต๊ำพระแล',
    'บ้านต๊ำใน',
    'ชุมชนบ้านตุ้มท่า',
    'บ้านต๊ำเหล่า',
    'บ้านดอกบัวฯ สาขาบ้านห้วยหม้อ',
    'บ้านห้วยลึก',
    'บ้านดอกบัว(ราษฏร์บำรุง)',
    'บ้านสันกว๊านทุ่งกิ่ว',
    'บ้านตุ่น',
    'บ้านสันเวียงใหม่',
    'บ้านสันบัวบก',
    'บ้านสาง',
    'บ้านโป่ง',
    'บ้านใหม่',
    'บ้านบัว',
    'บ้านแม่กา',
    'บ้านแม่กา สาขาบ้านแม่ต๋ำน้อย',
    'บ้านห้วยเคียน',
    'บ้านแม่ต๋ำบุญโยง',
    'อนุบาลเมืองพะเยา(บ้านโทกหวาก)',
    'บ้านโซ้',
    'บ้านร่องคำ',
    'บ้านสันป่าสัก',
    'บ้านแม่นาเรือ',
    'บ้านไร่',
    'บ้านแม่นาเรือใต้',
    'บ้านภูเงิน',
    'บ้านห้วยบง',
    'บ้านป่าคา',
    'บ้านป่าคา สาขาบ้านสันต้นหวีด',
    'บ้านแม่ปืม',
    'บ้านต๊ำดอนมูล',
    'บ้านต๊ำม่อน',
    'บ้านร่องไฮ',
    'ชุมชนบ้านแม่ใส',
    'บ้านบ่อแฮ้วสันช้างหิน',
    'บ้านสันป่าถ่อน',
    'อนุบาลพะเยา',
    'บ้านสันปูเลย',
    'บ้านต๋อมดง',
    'บ้านผาช้างมูบ',
    'บ้านผาช้างมูบ สาขาบ้านแม่ต๋อมใน',
    'บ้านหนองหล่ม',
    'บ้านแม่พริก',
    'บ้านใหม่',
    'บ้านดง',
    'บ้านห้วยดอกเข็ม',
    'บ้านสันโค้ง',
    'บ้านจำไก่',
    'บ้านห้วยทรายเลื่อน',
    'บ้านดอกคำใต้',
    'ไทยรัฐวิทยา 46(ดอกคำใต้)',
    'บ้านถ้ำประชาบำรุง',
    'บ้านถ้ำประชานุเคราะห์',
    'บ้านปางงุ้น',
    'บ้านปางงุ้น สาขาบ้านร่วมจิต',
    'บ้านทุ่งหลวง',
    'บ้านสันจกปก',
    'บ้านค่า',
    'บ้านค่าบน',
    'บ้านร่องจว้า',
    'อนุบาลดอกคำใต้(ชุมชนบ้านสันช้างหิน',
    'บ้านสันป่าหนาด',
    'บ้านบุญเรือง(อินทะวงศานุเคราะห์)',
    'บ้านปาง',
    'บ้านทุ่งกาไชย',
    'บ้านโพธิ์ทอง',
    'บ้านปิน',
    'บ้านปินเหนือ',
    'บ้านวังขอนแดง',
    'ชุมชนบ้านห้วยลาน',
    'บ้านป่ากล้วยแข้',
    'บ้านทุ่งต้นศรี',
    'บ้านเนินสมบูรณ์',
    'เจริญใจ สาขาบ้านปางปูเลาะ',
    'บ้านสันต้นม่วง',
    'บ้านสันต้นม่วง สาขาบ้านสันสลี',
    'บ้านป่าแฝกเหนือ',
    'บ้านดงอินตา',
    'บ้านหนองบัว',
    'บ้านดงบุญนาค',
    'บ้านดงบุญนาค สาขาบ้านสันกำแพง',
    'บ้านเหล่า',
    'บ้านไร่อ้อย',
    'บ้านห้วยเจริญราษฏร์',
    'ชุมชนบ้านป่าแฝกสามัคคี',
    'บ้านหนองสระ',
    'บ้านหนองสระ สาขาบ้านแม่เย็น',
    'บ้านป่าตึง',
    'บ้านแม่ใจ',
    'ชุมชนบ้านแม่สุก',
    'บ้านแม่จว้า',
    'บ้านแม่จว้าใต้',
    'บ้านสันขวาง',
    'เจริญใจ สาขาบ้านผาแดง',
    'อนุบาลแม่ใจ(บ้านศรีถ้อย)',
    'บ้านป่าสักสามัคคี',
    'บ้านทุ่งป่าข่า',
    'เจริญใจ',
    'บ้านป่าแฝกใต้',
    'บ้านเจน(เจนจันทรานุกูล)',
    'อนุบาลภูกามยาว',
    'บ้านแม่อิง',
    'บ้านกว้านเหนือ',
    'บ้านร่องปอ',
    'บ้านร้อง',
    'อนุบาลภูกามยาว สาขาพระธาตุภูปอ',
    'บ้านสันป่างิ้ว',
    'บ้านสันต้นผึ้ง',
    'บ้านอิงโค้ง',
    'บ้านห้วยแก้ว',
    'บ้านม่วงคำ',
    'บ้านกาดถี',
    'บ้านสันต้นแหน',
    'บ้านหนองลาว',
    'บ้านห้วยทรายขาว',
    'พะเยาพิทยาคม',
    'ฟากกว๊านวิทยาคม',
    'พะเยาประสาธน์วิทย์',
    'ดอกคำใต้วิทยาคม',
    'ถ้ำปินวิทยาคม',
    'งำเมืองวิทยาคม',
    'แม่ใจวิทยาคม',
    'ดงเจนวิทยาคม',
    'เฉลิมพระเกียรติสมเด็จพระศรีนครินทร์ พะเยา',
    'บ้านสร้อยศรี',
    'บ้านดอนมูล',
    'บ้านจุน',
    'บ้านห้วยกั้ง',
    'บ้านร่องแมด',
    'บ้านสันหลวง',
    'บ้านห้วยไคร้',
    'ชุมชนบ้านห้วยงิ้ว',
    'บ้านแม่ทะลาย',
    'บ้านธาตุขิงแกง',
    'บ้านแม่วังช้าง',
    'บ้านร่องย้าง',
    'บ้านเวียงลอ',
    'บ้านศรีเมืองชุม',
    'บ้านน้ำจุน',
    'บ้านปางป้อม',
    'บ้านพวงพยอม',
    'บ้านศรีจอมแจ้ง',
    'บ้านสักลอ',
    'บ้านสักทุ่ง',
    'อนุบาลจุน(บ้านบัวสถาน)',
    'ชุมชนบ้านทุ่ง',
    'บ้านกิ่วแก้ว',
    'บ้านปงสนุก',
    'บ้านยางขาม',
    'บ้านหัวขัว',
    'บ้านฝั่งหมิ่น',
    'บ้านวังเค็มใหม่',
    'บ้านดอนลาว',
    'บ้านปัว',
    'บ้านร่องค้อม',
    'บ้านปัวศรีพรม',
    'บ้านฝายกวาง',
    'บ้านแม่ต๋ำ',
    'บ้านแวน',
    'บ้านสันปูเลย',
    'บ้านปางมดแดง',
    'ชุมชนบ้านเชียงบาน',
    'บ้านทุ่งมอก',
    'บ้านปางวัว',
    'บ้านผาฮาว',
    'บ้านหัวทุ่ง',
    'บ้านร่องส้าน',
    'บ้านสบสา',
    'บ้านโจ้โก้',
    'บ้านโจ้โก้ สาขาบ้านห้วยสา',
    'บ้านใหม่ร่มเย็น',
    'บ้านปางถ้ำ',
    'บ้านทุ่งเย็น',
    'บ้านสบทุ"ศีรีราษฏร์สงเคราะห์',
    'บ้านถ้ำผาลาด',
    'บ้านต้นผึ้ง',
    'บ้านคะแนง',
    'บ้านน้ำมิน',
    'บ้านแฮะ',
    'บ้านไชยพรม',
    'บ้านปี้',
    'บ้านพระนั่งดิน',
    'บ้านหย่วน(เชียงคำนาคโรวาท)',
    'อนุบาลเชียงคำ(วัดพระธาตุสบแวน)',
    'อนุบาลเชียงคำฯ สาขาบ้านแดนเมือง',
    'บ้านแวนโค้ง',
    'บ้านทุ่งหล่ม',
    'บ้านหนองบัวเงิน',
    'บ้านจำบอน',
    'บ้านผาลาด',
    'ชัยชุมภู',
    'บ้านไชยสถาน',
    'บ้านท่าม่าน',
    'ชุมชนบ้านหลวง',
    'บ้านหล่ายทุ่ง',
    'บ้านปงสนุก',
    'บ้านป่าแขม',
    'บ้านบ่อเบี้ย',
    'อนุบาลเชียงม่วน',
    'บ้านสระสาขาห้วยก้างปลา',
    'บ้านท่าฟ้าใต้',
    'บ้านนาบัว',
    'บ้านทุ่งมอก',
    'บ้านสระ',
    'บ้านทุ่งหนอง',
    'บ้านท่าฟ้าเหนือ',
    'บ้านวังบง',
    'บ้านใหม่น้ำเงิน',
    'ชุมชนบ้านดอนไชย',
    'บ้านควร',
    'บ้านเลี้ยว',
    'บ้านแบ่ง',
    'บ้านนาอ้อม สาขาบ้านน้ำแป้ง',
    'บ้านนาอ้อม สาขาบ้านห้วยขุ่น',
    'บ้านปัว',
    'บ้านปางผักหม',
    'บ้านสีพรม',
    'บ้านป่าคา',
    'บ้านใหม่พัฒนา',
    'บ้านควรดง',
    'บ้านดอนแก้ว',
    'บ้านดอนเงิน',
    'บ้านหลวง',
    'บ้านดอนไชยป่าแขม',
    'บ้านหล่ายฝายแก้ว',
    'บ้านสันกลางนาดอ',
    'บ้านหนองบัว',
    'บ้านแฮะ',
    'บ้านทุ่งแต',
    'บ้านห้วยแม่แดง สาขาบ้านห้วยคอก',
    'บ้านหนองท่าควาย',
    'อนุบาลปง',
    'บ้านบุญยืน',
    'บ้านห้วยแม่แดง',
    'บ้านหมุ้น',
    'บ้านดู่',
    'ชุมชนบ้านบอน',
    'บ้านหนุน',
    'บ้านม่วง',
    'บ้านบุญเรือง',
    'บ้านห้วยสิงห์',
    'ชุมชนบ้านบอน สาขาบ้านค่าไพบูลย์',
    'ราชานุเคราะห์',
    'บ้านใหม่ปางค่า(ภูลังกาอนุสรณ์)',
    'บ้านแม่ทาย',
    'บ้านสันกลางนาดอสาขาบ้านตุ่นราษฎร์อุทิศ',
    'บ้านใหม่ปางค่าสาขาบ้านห้วยกอก',
    'บ้านใหม่ปางค่าสาขาบ้านห้วยเฟือ',
    'ราชานุเคราะห์ สาขาบ้านน้ำต้ม',
    'บ้านขุนกำลัง',
    'บ้านสันติสุข',
    'อนุบาลปง  สาขาบ้านหนองขวาง',
    'บ้านนาอ้อม',
    'บ้านสบขาม',
    'บ้านผาตั้ง',
    'บ้านน้ำปุก',
    'บ้านดอนมูล',
    'บ้านสะแล่ง',
    'บ้านร้องเชียงแรง',
    'บ้านน้ำเปื๋อย',
    'บ้านดอนไชย',
    'บ้านก๊อน้อย',
    'บ้านทุ่งกล้วย',
    'บ้านก๊อหลวง',
    'บ้านแกใหม่นิคม',
    'บ้านสา',
    'บ้านทุ่งติ้ว  สาขาบ้านนาหนุน',
    'บ้านป่าสัก  สาขาบ้านม่วงชุม',
    'ชุมชนบ้านหนองเลา',
    'บ้านป่าสัก',
    'บ้านสถาน',
    'บ้านทุ่งติ้ว  สาขาบ้านผาลาดทุ่งติ้ว',
    'บ้านทุ่งติ้ว',
    'บ้านแก',
    'บ้านสบบง',
    'บ้านปง',
    'บ้านฮวก',
    'อนุบาลภูซาง(บ้านดอนตัน)',
    'บ้านปงใหม่',
    'จุนวิทยาคม',
    'พญาลอวิทยาคม',
    'ราชประชานุเคราะห์ 24',
    'เชียงคำวิทยาคม',
    'ฝายกวางวิทยาคม',
    'เชียงม่วนวิทยาคม',
    'ปงรัชดาภิเษก',
    'ปงพัฒนาวิทยาคม',
    'ขุนควรวิทยาคม',
    'ภูซางวิทยาคม',
    'บ้านโป่งเกลือ',
    'บ้านโป่งช้าง',
    'ดอยลานพิทยา',
    'บ้านโล๊ะป่าตุ้ม',
    'บ้านทรายงาม',
    'บ้านจำหวาย',
    'บ้านจำบอน',
    'บ้านผาเสริฐ',
    'บ้านโป่งนาคำ',
    'บ้านดอยฮาง',
    'บ้านโป่งน้ำร้อน',
    'บ้านท่าสาย',
    'บ้านหัวดอย',
    'ห้วยพลูพิทยา',
    'บ้านแม่ข้าวต้มท่าสุด',
    'บ้านถ้ำผาตอง',
    'บ้านบ่อทอง',
    'บ้านป่ารวก(คุรุราษฎร์สงเคราะห์)',
    'อนุบาลนางแล(บ้านทุ่ง)',
    'บ้านนางแลใน',
    'บ้านนางแล',
    'บ้านสันต้นขาม',
    'บ้านโป่งพระบาท',
    'บ้านโป่งน้ำตก',
    'บ้านดู่(สหราษฎร์พัฒนาคาร)',
    'บ้านขัวแคร่',
    'บ้านป่าสักไก่',
    'บ้านริมลาว',
    'บ้านป่าอ้อดอนชัย',
    'บ้านห้วยทราย',
    'บ้านปุยคำ',
    'บ้านสันกลาง',
    'บ้านหนองหม้อ',
    'บ้านแม่กรณ์',
    'บ้านปางริมกรณ์',
    'บ้านปางคึก',
    'บ้านปางลาว',
    'บ้านเวียงกลาง',
    'ชุมชนบ้านแม่ข้าวต้มหลวง',
    'ห้วยเจริญวิทยา',
    'บ้านหนองบัวแดง',
    'บ้านโล๊ะป่าห้า',
    'บ้านห้วยทรายขาว',
    'บ้านรวมมิตร',
    'บ้านผาขวางพัฒนา',
    'แม่ยาววิทยา',
    'บ้านทุ่งหลวง',
    'บ้านริมกก',
    'บ้านห้วยขม',
    'อนุบาลหัวฝาย',
    'อนุบาลฮ่องลี่(ค่ายเม็งรายมหาราชอุปถัมภ์)',
    'บ้านป่ายางมน',
    'บ้านป่ายางหลวง',
    'บ้านน้ำลัด',
    'บ้านใหม่',
    'บ้านเวียงกือนา',
    'บ้านฟาร์มสัมพันธ์กิจ',
    'อนุบาลเชียงราย',
    'บ้านสันโค้ง(เชียงรายจรูญราษฎร์)',
    'อนุบาลริมกก(บ้านสันต้นเปา)',
    'อนุบาลเมืองเชียงราย(สันทรายราษฎร์ดรุณานุเคราะห์)',
    'บ้านห้วยชมภู',
    'บ้านปางขอน',
    'บ้านผาลั้ง',
    'บ้านจะคือ',
    'บ้านห้วยแม่เลี่ยม',
    'แม่มอญวิทยา',
    'บ้านห้วยแม่ซ้าย',
    'บ้านป่าก๊อ',
    'บ้านหัวดง',
    'บ้านดงป่าเหมี้ยง',
    'บ้านป่าแหย่ง',
    'บ้านโป่งฮึ้ง',
    'บ้านร่องปลาขาว',
    'อนุบาลห้วยสัก',
    'บ้านร่องเผียว',
    'ร่องเบ้อวิทยา',
    'บ้านดอน',
    'บ้านชัยพฤกษ์',
    'บ้านช่องลม',
    'บ้านจอเจริญ',
    'บ้านดอยงาม',
    'บ้านสมานมิตร',
    'บ้านทุ่งยั้งหัวฝายวิทยา',
    'บ้านร่องห้า',
    'บ้านดงมะตื๋นเนินสยามวิทยา',
    'บ้านป่าบง',
    'บ้านหนองบัวผาบ่ม',
    'บ้านดอนมูล',
    'บ้านเมืองชุม',
    'เวียงแก้ววิทยา',
    'บ้านศรีเวียง',
    'บ้านร่องบัวลอย',
    'บ้านเวียงชัย',
    'อนุบาลเวียงชัย',
    'บ้านปง',
    'บ้านเวียงเดิม',
    'บ้านค่ายเจริญ',
    'อนุบาลดงมหาวัน',
    'บ้านปงเคียน',
    'บ้านร่องหวาย',
    'บ้านโป่ง',
    'บ้านน้ำตกพัฒนา(นพค.อุปถัมภ์)',
    'บ้านทุ่งก่อ(ใจประชานุเคราะห์)',
    'อนุบาลเวียงเชียงรุ้ง',
    'เวียงเชียงรุ้งวิทยา',
    'บ้านป่าซาง',
    'บ้านห้วยหมากเอียก',
    'บ้านห้วยห้างป่าสา',
    'บ้านห้วยขี้เหล็ก',
    'บ้านป่าซางเหนือ',
    'บ้านกกน้อยวิทยา',
    'สามัคคีวิทยาคม',
    'ดำรงราษฎร์สงเคราะห์',
    'เม็งรายมหาราชวิทยาคม',
    'ห้วยสักวิทยาคม',
    'เมืองเชียงราย',
    'ดอนชัยวิทยาคม',
    'จุฬาภรณราชวิทยาลัย เชียงราย',
    'สามัคคีวิทยาคม 2',
    'ดอนศิลาผางามวิทยาคม',
    'เวียงชัยวิทยาคม',
    'เวียงเชียงรุ้งวิทยาคม',
    'เจริญเมืองวิทยา',
    'บ้านเจริญเมือง',
    'บ้านป่าตึง',
    'บ้านป่าบง',
    'บ้านสิบสอง',
    'บ้านจำคาวตอง',
    'สันหนองควาย',
    'บ้านสันช้างตาย',
    'บ้านท่าฮ่อ',
    'ร่องธารวิทยา',
    'บ้านหนองตุ้ม',
    'บ้านหนองผักจิก',
    'บ้านโป่งแดง',
    'บ้านห้วยทรายขาว',
    'บ้านสันป่ง',
    'บ้านร่องคตสันปูเลย',
    'ทานตะวันวิทยา(ห้วยบง-ดงเจริญ)',
    'บ้านดงเวียง',
    'บ้านดงขนุน',
    'บ้านสันมะแฟน',
    'ธารทองวิทยา(ป่ารวก)',
    'ชุมชนบ้านทุ่งป่าแดด',
    'บ้านป่าหัดป่าแขม',
    'บ้านห้วยประสิทธิ์',
    'บ้านป่าหุ่ง',
    'บ้านศาลา',
    'บ้านปางเกาะทราย',
    'ริมวัง ๑',
    'ริมวัง ๒',
    'บ้านสันผักแค',
    'บ้านสันมะเหม้า',
    'บ้านม่วงคำ',
    'บ้านสันต้นผึ้ง',
    'บ้านฝั่งตื้น',
    'บ้านหนองบัว',
    'บ้านป่าส้าน',
    'บ้านดอนตัน',
    'บ้านเก่า',
    'อนุบาลพาน(ป่ากว๋าวมิตรภาพที่ ๖๘)',
    'พานพสกสวัสดิ์',
    'ปูแกง(อินทราราษฎร์อุปถัมภ์)',
    'บ้านป่าสัก',
    'บ้านแม่เย็น',
    'บ้านดง',
    'บ้านสันต้นแหน',
    'บ้านจำผักกูด',
    'โป่งทะลายใหม่เจริญ',
    'บ้านแม่แก้วเด่นชัย',
    'บ้านแม่แก้วเหนือ',
    'บ้านแม่แก้ว',
    'บ้านแม่แก้วใต้',
    'บ้านปอเรียง',
    'บ้านแม่อ้อนอก',
    'บ้านแม่อ้อ',
    'เวียงห้าว',
    'บ้านท่าดอกแก้ว',
    'บ้านทุ่งสามเหลี่ยม',
    'บ้านแม่คาวหลวง',
    'สันกลางวิทยา',
    'บ้านถ้ำ',
    'บ้านป่าต้าก',
    'บ้านเหมืองง่า',
    'บ้านไร่อ้อย',
    'บ้านสันติวัน',
    'บ้านสันกอเหียง',
    'บ้านใหม่ท่าต้นเกี๋ยง',
    'ชุมชนสันมะเค็ด',
    'บ้านกว๋าวโท้ง',
    'บ้านสันต้นดู่',
    'บ้านกล้วย',
    'บ้านห้วยเครือบ้า',
    'บ้านป่าแดงงาม',
    'บ้านหนองฮ่างสันหลวง',
    'บ้านป่าคา',
    'ป่าแดงวิทยา',
    'บ้านห้วยลึกผาลาดสามัคคี',
    'บ้านป่าแงะ',
    'บ้านป่าเส้า',
    'บ้านใหม่พัฒนา',
    'สามัคคีราษฏร์รังสรรค์',
    'บ้านใหม่ใต้',
    'อนุบาลป่าแดด',
    'บ้านสักพัฒนา',
    'บ้านแม่พุง',
    'โรงช้างวิทยา',
    'บ้านโป่ง',
    'อนุบาลศรีโพธิ์เงิน',
    'บ้านวังผา',
    'ชุมชนบ้านสันมะค่า',
    'บ้านป่าตึงงาม',
    'บ้านร้องบง',
    'เจดีย์หลวงพิทยา',
    'บ้านห้วยหมอเฒ่า',
    'บ้านสันก้างปลา',
    'บ้านแม่ตาแมว',
    'บ้านดินดำ',
    'บ้านแม่ผักแหละ',
    'บ้านป่าถ่อน(วิทยาสมิทธิ์)',
    'บ้านแม่ต๋ำ',
    'บ้านท่าก๊อพลับพลา',
    'บ้านปางหลวง',
    'บ้านปางหก',
    'บ้านแม่น้ำขุ่น',
    'บ้านห้วยมะแกง',
    'บ้านห้วยสะลักวิทยา',
    'บ้านป่าแดด(เวทยาสมิทธิ์)',
    'ชุมชนบ้านโป่ง',
    'บ้านดอนสลี',
    'บ้านหัวฝาย',
    'บ้านห้วยหญ้าไซ',
    'บ้านแม่ตาช้าง',
    'บ้านปางอ้อย',
    'บ้านป่าเมี่ยงแม่พริก',
    'บ้านแม่พริก',
    'ชุมชนบ้านสันจำปา',
    'บ้านทุ่งฟ้าผ่า',
    'บ้านโป่งปูเฟือง',
    'บ้านห้วยส้าน',
    'บ้านหนองผำ',
    'บ้านป่าบงท้าวแก่นจันทร์',
    'อนุบาลแม่สรวย',
    'บ้านสันกลาง (ราษฎร์พัฒนา)',
    'บ้านวาวี',
    'บ้านมังกาล่า',
    'บ้านเลาลี',
    'บ้านปางกิ่ว(ศุภสิทธิ์มหาคุณ)',
    'บ้านแม่โมงเย้า',
    'ดอยเวียงวิทยา',
    'บ้านโป่งกลางน้ำ',
    'บ้านขุนสรวย',
    'บ้านห้วยไคร้',
    'บ้านห้วยน้ำเย็น',
    'บ้านห้วยกล้า',
    'บ้านผาแดงหลวง',
    'บ้านทุ่งพร้าว(เพ็กกี้ฮิทค็อค)',
    'บ้านแสนเจริญ',
    'บ้านดอยช้าง',
    'บ้านห้วยมะซาง',
    'บ้านห้วยขี้เหล็ก',
    'ศรีถ้อยสุนทรราษฎร์วิทยา',
    'บ้านห้วยเฮี้ย',
    'บ้านแม่ยางมิ้น',
    'เวียงผาวิทยา',
    'บ้านสัน',
    'บ้านลังกา',
    'บ้านโป่งเทวี',
    'บ้านป่าตึง',
    'บ้านห้วยหินลาดใน',
    'บ้านทุ่งขจี',
    'บ้านป่าสัก',
    'บ้านฮ่างต่ำ',
    'ป่างิ้ววิทยา',
    'บ้านหม้อ',
    'บ้านสันกู่ทอง',
    'ไทยรัฐวิทยา 32(บ้านสาขันหอม)',
    'อนุบาลแม่ขะจาน',
    'บ้านป่าแงะ',
    'บ้านทุ่งยาว',
    'บ้านปางมะกาด',
    'บ้านห้วยโป่ง',
    'บ้านห้วยม่วง',
    'บ้านปางอ่ายห้วยชมภู',
    'โป่งน้ำร้อนวิทยา',
    'บ้านเมืองน้อย(นิมมานเหมินทานุสรณ์)',
    'บ้านขุนลาว',
    'บ้านแม่เจดีย์',
    'บ้านโฮ่ง',
    'อนุบาลเวียงป่าเป้า',
    'ชุมชนบ้านกู่(ป่าเป้าประชานุเคราะห์)',
    'บ้านหนองยาว',
    'ดอยเวียงผาพิทยา',
    'บ้านแม่ปูนหลวง',
    'บ้านแม่ปูนหลวงสาขาสามกุลา',
    'เวียงกาหลงวิทยา',
    'บ้านป่าจั่น',
    'บ้านสันมะเค็ดสันขี้เหล็กหัวฝายพัฒนา',
    'บ้านทุ่งม่าน',
    'บ้านดงป่าส้าน',
    'อนุบาลสันสลี(เด่นศาลา)',
    'บ้านแม่ตะละ',
    'บ้านปางมะขามป้อม',
    'บ้านโป่งนก',
    'บ้านโป่งเหนือ',
    'บ้านทุ่งห้า',
    'บ้านสันสลี',
    'อนุบาลจอมหมอกแก้ว',
    'บ้านท่ามะโอ',
    'ห้วยส้านยาววิทยาสาขาใหม่จัดสรร',
    'บ้านดงมะดะ',
    'อนุบาลแม่ลาว',
    'บ้านผาบ่อง(คุรุราษฎร์สามัคคี)',
    'ห้วยส้านยาววิทยา',
    'บ้านหนองเก้าห้อง',
    'บ้านต้นง้าว',
    'อนุบาลบัวสลี',
    'บ้านต้นยาง',
    'บ้านโป่งมอญ',
    'บ้านแม่ผง',
    'ชุมชนบ้านป่าก่อดำ',
    'บ้านห้วยส้านพลับพลา',
    'โป่งแพร่วิทยา',
    'พานพิทยาคม',
    'พานพิเศษพิทยา',
    'นครวิทยาคม',
    'แม่อ้อวิทยาคม',
    'ดอยงามวิทยาคม',
    'ป่าแดดวิทยาคม',
    'แม่สรวยวิทยาคม',
    'วาวีวิทยาคม',
    'เวียงป่าเป้าวิทยาคม',
    'แม่เจดีย์วิทยาคม',
    'แม่ลาววิทยาคม',
    'อนุบาลจอมสวรรค์(บ้านบ่อก๊าง)',
    'บ้านขี้เหล็ก',
    'บ้านห้วยน้ำราก(ไตรราษฎร์บำรุง)',
    'บ้านใหม่(ประกอบราษฎร์วิทยานุกูล)',
    'ไทยรัฐวิทยา 86 (บ้านป่าสักหลวง)',
    'บ้านหนองครก',
    'บ้านแม่คำฝั่งหมิ่น',
    'บ้านป่าบง(หล้าราษฎร์วิทยา)',
    'อนุบาลจันจว้าใต้(บ้านกิ่วพร้าว)',
    'บ้านท่าข้าวเปลือก',
    'บ้านทับกุมารทอง',
    'ผ่านศึกสงเคราะห์ ๒',
    'บ้านแม่หะป่าไร่ศรียางชุม',
    'บ้านแม่แพง',
    'บ้านแม่ลัว',
    'บ้านแม่ลาก',
    'บ้านแม่คี-หนองอ้อ',
    'บ้านแม่สลองใน',
    'นิคมสร้างตนเองสงเคราะห์ชาวเขา',
    'บ้านใหม่สามัคคี',
    'อนุบาลป่าซาง',
    'บ้านรวมใจ',
    'บ้านป่าบงแม่จัน',
    'บ้านสันโค้ง',
    'บ้านโป่งน้ำร้อน',
    'ประชารัฐพัฒนา',
    'บ้านห้วยมะหินฝน',
    'บ้านปางสา',
    'บ้านสันติสุข',
    'บ้านป่าตึง',
    'บ้านแม่คำ(บ้านแม่คำประชานุเคราะห์)',
    'บ้านหนองแหย่ง',
    'บ้านแม่คำสบเปิน',
    'บ้านสันธาตุ',
    'บ้านร่องก๊อ-ม่วงคำ',
    'บ้านป่าเปา-ป่าซางน้อย',
    'ธรรมจาริกอุปถัมภ์ 1',
    'บ้านแม่จัน(เชียงแสนประชานุสาสน์)',
    'บ้านหนองแว่น',
    'บ้านสันกอง',
    'อนุบาลแม่ไร่(บ้านป่ากว๋าว)',
    'บ้านเวียงสา',
    'สันสลีหลวง-สันนายาว',
    'อนุบาลศรีค้ำ',
    'สันทราย(พรหมณีวิทยา)',
    'บ้านแหลว',
    'ป่าตึงพิทยานุกูล',
    'บ้านแซว',
    'บ้านสวนดอกท่าขันทองสามัคคี',
    'บ้านห้วยกว๊าน',
    'บ้านแม่แอบวิทยา',
    'บ้านศรีกองงาม',
    'บ้านป่าไร่หลวงวิทยา(ตชด.ช่างกลปทุมวันอนุสรณ์ 9)',
    'อนุบาลเชียงแสน(ปางหมอปวงชัยบานราษฎร์ศึกษา)',
    'ชุมชนบ้านป่าสักน้อย',
    'บ้านแม่คำ(ธรรมาประชาสรรค์)',
    'บ้านสันมะเค็ด',
    'บ้านปงของ',
    'ชุมชนบ้านป่าคาแม่เงิน',
    'บ้านแม่คำ',
    'บ้านไร่',
    'บ้านสันต้นเปา',
    'บ้านดอยจัน',
    'บ้านร่องบง',
    'บ้านทุ่งฟ้าฮ่าม',
    'บ้านสันธาตุ',
    'บ้านสบคำ',
    'ราชประชานุเคราะห์ 15(เวียงเก่าแสนภูวิทยาประสาท)',
    'บ้านสบรวก',
    'บ้านวังลาว',
    'บ้านห้วยเกี๋ยง',
    'บ้านแม่มะ',
    'บ้านเวียงแก้ว',
    'บ้านสันต้นธง',
    'บ้านป่าก๋อย',
    'บ้านสันสลี',
    'บ้านด้าย',
    'บ้านศรีดอนมูล',
    'บ้านหนองปลาสะเด็ด',
    'บ้านดอยสะโง๊ะ',
    'บ้านป่าแดง',
    'ศรีป่าแดง',
    'บ้านม่วงคำ',
    'บ้านปางห้า',
    'บ้านสันนา',
    'บ้านสันบุญเรือง',
    'บ้านสันหลวง',
    'บ้านเหมืองแดงน้อย',
    'บ้านสันทรายน้อย',
    'บ้านด้ายเทพกาญจนา',
    'บ้านถ้ำ',
    'บ้านโป่ง',
    'บ้านถ้ำ ตชด.',
    'บ้านถ้ำปลา',
    'บ้านผาฮี้',
    'บ้านน้ำจำ',
    'บ้านจ้อง',
    'บ้านป่าแฝ - หนองอ้อ',
    'บ้านสันทราย(ราษฎร์สามัคคี)',
    'ชุมชนบ้านไม้ลุงขน มิตรภาพที่ 169',
    'บ้านสันทราย',
    'บ้านสันมะนะ',
    'บ้านป่ายาง',
    'บ้านเหมืองแดง',
    'เจ้าพ่อหลวงอุปถัมภ์ ๕',
    'บ้านเวียงพาน',
    'บ้านป่าเหมือด',
    'อนุบาลแม่สาย(สายศิลปศาสตร์)',
    'บ้านสันถนน',
    'บ้านสันธาตุ',
    'บ้านทุ่งเกลี้ยง',
    'บ้านสันโค้ง',
    'บ้านห้วยไคร้',
    'บ้านสันยาวสันลิดไม้',
    'บ้านฮ่องแฮ่',
    'บ้านเทอดไทย',
    'บ้านพญาไพร',
    'บ้านแแม่หม้อ',
    'พญาไพรไตรมิตร',
    'บ้านจะตี',
    'รัฐราษฎร์วิทยา',
    'สามัคคีพัฒนา',
    'บ้านปางมะหัน',
    'บ้านห้วยอื้น',
    'บ้านผาจี',
    'บ้านขาแหย่งพัฒนา',
    'บ้านป่าซางนาเงิน',
    'บ้านห้วยไร่สามัคคี',
    'อนุบาลแม่ฟ้าหลวง',
    'บ้านสันติคีรี',
    'บ้านใหม่สันติ',
    'บ้านกลาง',
    'บ้านพนาสวรรค์',
    'ราษฎร์พัฒนา',
    'บ้านแม่เต๋อ',
    'บ้านมนตรีวิทยา',
    'ชุมชนศึกษาบ้านแม่สแลป',
    'บ้านผาเดื่อ',
    'ดอยแสนใจ(ตชด.อนุสรณ์)',
    'บ้านห้วยผึ้ง',
    'อนุบาลโชคชัย(บ้านแม่เลียบแม่บง)',
    'บ้านทุ่งกวาง',
    'บ้านดอย',
    'บ้านดอนงาม',
    'บ้านขุนแม่บง(ตชด.)',
    'อนุบาลดอยหลวง',
    'บ้านป่าลัน',
    'บ้านห้วยไร่',
    'ผ่านศึกสงเคราะห์ ๑',
    'บ้านไทรทอง',
    'บ้านหนองป่าก่อ',
    'บ้านใหม่พัฒนา',
    'บ้านป่าซางงาม',
    'บ้านเทอดไทย สาขาห้วยหก',
    'บ้านเทอดไทย สาขาห้วยหยวกป่าโซ',
    'บ้านไร่สาขาธารทอง',
    'จันจว้าวิทยาคม',
    'ศึกษาสงเคราะห์แม่จัน',
    'แม่จันวิทยาคม',
    'เชียงแสนวิทยาคม',
    'บ้านแซววิทยาคม',
    'แม่สายประสิทธิ์ศาสตร์',
    'วัดถ้ำปลาวิทยาคม',
    'สันติคีรีวิทยาคม',
    'ดอยหลวง รัชมังคลาภิเษก',
    'บ้านศรีลานนา',
    'บ้านหลวง',
    'บ้านครึ่ง',
    'บ้านครึ่งใต้',
    'บ้านส้าน',
    'บ้านตองม่วงชุม',
    'บ้านหก',
    'บ้านบุญเรือง',
    'บ้านต้นปล้อง',
    'บ้านเมืองกาญจน์',
    'บ้านสองพี่น้อง',
    'ริมโขงวิทยา',
    'บ้านหัวเวียง(โกศัลย์วิทย์)',
    'อนุบาลเชียงของ',
    'อนุบาลทุ่งทรายพัฒนา',
    'บ้านทุ่งนาน้อย',
    'บ้านดอนมหาวัน',
    'บ้านห้วยเม็ง(ประสาทวิทย์)',
    'บ้านปากอิง',
    'บ้านหวาย',
    'ชุมชนบ้านศรีดอนชัย',
    'บ้านลุงท่าเจริญ',
    'บ้านทุ่งซางดงหลวง',
    'บ้านเขียะ',
    'บ้านทุ่งงิ้ว(ประชาสงเคราะห์2)',
    'บ้านเชียงคาน(ราษฎร์ประสานมิตร)',
    'บ้านทุ่งอ่าง',
    'บ้านน้ำม้า',
    'อนุบาลสถาน',
    'บ้านแก่นวิทยา',
    'บ้านเกี๋ยง',
    'บ้านห้วยซ้อ',
    'บ้านเวียงหมอก',
    'บ้านโนนสมบูรณ์',
    'ศรีสว่าง',
    'สักสันเชียงใหม่วิทยาคม',
    'บ้านงิ้วใหม่',
    'บ้านงิ้วป่าไผ่(ราษฎร์นุกูล)',
    'บ้านตุ้มเหนือ',
    'บ้านห้วยตุ้ม(ตุ้มใต้ตระการคุณ)',
    'บ้านขอนซุง',
    'บ้านเชียงเคี่ยน(คณาราษฎร์บำรุง)',
    'บ้านสันชุม(สหราษฎร์บำรุ)',
    'บ้านก๊อ',
    'บ้านสันปูเลย',
    'บ้านขุนห้วยไคร้',
    'บ้านร่มโพธิ์ไทย',
    'บ้านร่มโพธิ์ไทยสาขาบ้านขุนต้า',
    'บ้านร่มโพธิ์ไทยสาขาห้องเรียนบ้านร่มฟ้าไทย',
    'บ้านร่มโพธิ์ไทยสาขาห้องเรียนบ้านพิทักษ์ไทย',
    'บ้านรักแผ่นดิน',
    'บ้านแผ่นดินทอง',
    'บ้านราษฏ์ภักดี',
    'อนุบาลตับเต่า(ไคร้สามัคคีวิทยา)',
    'บ้านต้นเขือง',
    'บ้านตีนเป็ด',
    'บ้านปางค่า',
    'บ้านรักถิ่นไทย',
    'บ้านดอนดินแดง',
    'บ้านป่ามื่น',
    'อนุบาลปล้อง',
    'จำไฮบ้านเหล่า',
    'บ้านปล้องใต้',
    'บ้านปล้องส้าน',
    'บ้านเกี๋ยง(คุรุราษฎร์วิทยา)',
    'บ้านแม่ลอยไร่',
    'บ้านเกี๋ยงใต้',
    'วัดพระเกิดคงคาราม',
    'อนุบาลเทิง',
    'บ้านใหม่',
    'บ้านร่องแช่',
    'บ้านทุ่งโห้ง',
    'บ้านห้วยไคร้',
    'บ้านป่าตึงงาม',
    'บ้านป่ารวก',
    'ศรีดอนไชยวิทยา',
    'บ้านสันทรายมูล',
    'สันทรายงามวิทยา',
    'บ้านหนองบัว',
    'บ้านปี๊',
    'บ้านสันป่าบง',
    'บ้านเอียน(สหราษฎร์บำรุง)',
    'บ้านบุญนาค',
    'บ้านดอนแยง',
    'บ้านป่าจี้',
    'บ้านท่าข้าม',
    'บ้านดอนไชย',
    'บ้านดอนไชยสาขาบ้านผาลาด',
    'อนุบาลหนองแรด',
    'บ้านม่อนป่ายาง',
    'อนุบาลตาดควัน',
    'บ้านใหม่สุขสันต์',
    'บ้านใหม่สุขสันต์สาขาบ้านมูเซอร์',
    'บ้านใหม่โชคชัย',
    'บ้านศรีสะอาด',
    'บ้านเวียงหวาย',
    'บ้านหนองบัวคำ',
    'บ้านหนองเสา',
    'บ้านสันป่าสัก',
    'สันสะลีกวิทยา',
    'บ้านป่าซาง',
    'บ้านแม่ต๋ำน้อย',
    'บ้านป่าม่วง',
    'บ้านแม่ต๋ำกลาง',
    'บ้านบ่อแสง',
    'บ้านทุ่งเจ้า',
    'บ้านแม่เปา',
    'ชุมชนบ้านสบเปา',
    'แม่หลวงอุปถัมภ์ไทยคีรี',
    'บ้านกระแล',
    'บ้านสันหลวงสามัคคี',
    'อนุบาลไม้ยา (ห้วยก้างราษฎร์)',
    'อนุบาลไม้ยา(ห้วยก้างราษฎร์)สาขาบ้านสันสะอาด',
    'บ้านไม้ยามิตรภาพที่168',
    'บ้านร่องกอก',
    'บ้านห้วยเดื่อ',
    'บ้านห้วยก้าง"รัฐประชาสงเคราะห์"',
    'ขุนขวากพิทยา',
    'บ้านท่าข้าม',
    'ปางหัดสหศาสตร์',
    'บ้านโล๊ะ',
    'บ้านผาแล',
    'บ้านห้วยหาน',
    'บ้านห้วยคุ',
    'ปอวิทยา',
    'บ้านหนองเตา',
    'ไตรมิตรวิทยา',
    'บรรพตวิทยา',
    'บ้านทรายทอง',
    'บ้านห้วยลึก',
    'บ้านหลู้',
    'บ้านไทยสมบูรณ์',
    'บ้านม่วงยาย',
    'บ้านแจมป๋อง',
    'อนุบาลเวียงแก่น',
    'บ้านทุ่งคำ',
    'บ้านห้วยเอียน',
    'บ้านห้วยโป่ง',
    'อนุบาลบ้านพระเนตร(แก้วสุวรรณประสิทธิ์)',
    'บ้านห้วยเดื่อ',
    'ชุมชนบ้านต้าตลาด',
    'บ้านต้าหลวง(ต้าประชานุกูล)',
    'บ้านต้านาล้อม',
    'บ้านร่องขุ่นป่าข่า(สุทธานุสรณ์)',
    'บ้านห้วยห้อม(ราษฎร์สามัคคี)',
    'อนุบาลขุนตาล(ป่าตาลเจดีย์)',
    'บ้านป่าตาลใต้(ป่าตาลราษฎร์นุกูล)',
    'บ้านห้วยสัก(ประชานุกูล)',
    'อนุบาลยางฮอม',
    'บ้านพญาพิภัทดิ์',
    'บ้านทุ่งน้ำแพร่ป่าบง',
    'บ้านชมภู',
    'บ้านป่าแดงห้วยหลวง',
    'บ้านร่มโพธิ์ไทยสาขาห้องเรียนบ้านร่มโพธิ์ทอง',
    'เชียงของวิทยาคม',
    'บุญเรืองวิทยาคม',
    'ห้วยซ้อวิทยาคม รัชมังคลาภิเษก',
    'เทิงวิทยาคม',
    'ปล้องวิทยาคม',
    'พญาเม็งราย',
    'แม่ต๋ำวิทยา',
    'ไม้ยาวิทยาคม',
    'เวียงแก่นวิทยาคม',
    'ขุนตาลวิทยาคม',
    'ยางฮอมวิทยาคม',
    'ชุมชนบ้านปางหมู',
    'บ้านกุงไม้สักมิตรภาพที่ 98',
    'บ้านทุ่งกองมู',
    'บ้านสบสอย',
    'บ้านในสอย',
    'บ้านไม้สะเป่',
    'บ้านสบป่อง',
    'บ้านใหม่',
    'บ้านท่าโป่งแดง',
    'บ้านจองคำ',
    'บ้านแม่สะกึด',
    'บ้านห้วยเดื่อ',
    'อนุบาลเมือง (ป่าปุ๊มิตรภาพที่ 29)',
    'บ้านห้วยโป่งกาน',
    'บ้านน้ำเพียงดิน',
    'บ้านห้วยเสือเฒ่า(เจ้ากอแก้วอุปถัมภ์)',
    'บ้านป่าปุ๊ ฯ สาขาบ้านห้วยแก้วล่าง',
    'ชุมชนบ้านผาบ่อง',
    'อนุบาลแม่ฮ่องสอน',
    'บ้านน้ำส่อม',
    'บ้านห้วยช่างคำ ส.บ้านห้วยช่างเหล็ก',
    'ร่มเกล้าปางตอง',
    'บ้านห้วยมะเขือส้ม',
    'บ้านห้วยขาน',
    'บ้านนาป่าแปก',
    'บ้านรักไทย',
    'บ้านห้วยโป่งอ่อน',
    'ชุมชนบ้านหมอกจำแป่',
    'บ้านแม่สะงา',
    'บ้านห้วยฮี้',
    'บ้านหนองขาวกลาง',
    'บ้านห้วยปมฝาด',
    'บ้านห้วยปูลิง',
    'บ้านห้วยปูเลย',
    'บ้านห้วยตอง',
    'บ้านหัวน้ำแม่ฮ่องสอน',
    'บ้านห้วยโป่ง',
    'บ้านยอดดอยวิทยา',
    'บ้านกลาง',
    'บ้านแก่นฟ้า',
    'บ้านป่าลาน',
    'บ้านห้วยช่างคำ',
    'เสรีวิทยา',
    'บ้านไม้ซางหนาม',
    'บ้านแม่จ๋า',
    'บ้านหนองเขียว',
    'บ้านห้วยผาและห้องเรียนบ้านห้วยสลอบ',
    'ไทยรัฐวิทยา 99 (บ้านแม่สุยะ)',
    'บ้านนาปลาจาด',
    'บ้านห้วยผึ้ง',
    'บ้านคาหาน',
    'บ้านห้วยส้าน',
    'ขุนยวม',
    'อนุบาลขุนยวม',
    'บ้านแม่สุริน',
    'บ้านแม่สะเป่ใต้ ส.บ้านแม่สะเป่',
    'บ้านแม่สะเป่ใต้',
    'บ้านห้วยฟาน',
    'ชุมชนบ้านเมืองปอน',
    'บ้านท่าหินส้ม',
    'บ้านหนองแห้ง',
    'บ้านแม่ลาก๊ะ',
    'บ้านแม่โข่จู',
    'รักษ์ไพรวิทยา',
    'บ้านแม่กิ๊',
    'บ้านห้วยส้าน',
    'ห้วยต้นนุ่นวิทยา',
    'บ้านห้วยนา',
    'ตชด.บำรุงที่ 60',
    'ต.ช.ด.บำรุงที่ 60 ส.บ้านนาหัวแหลม',
    'ชุมชนต่อแพวิทยา',
    'บ้านประตูเมือง',
    'บ้านหัวปอน',
    'บ้านแม่หาด',
    'บ้านแม่ออ',
    'บ้านหว่าโน',
    'บ้านแม่แจ๊ะ',
    'บ้านแม่โกปี่',
    'บ้านปางตอง',
    'บ้านแม่อูคอ',
    'บ้านแม่อูคอหลวง',
    'บ้านหัวแม่สุริน',
    'บ้านพัฒนา',
    'บ้านนางิ้ว',
    'บ้านคำสุข',
    'บ้านทุ่งยาว',
    'บ้านทุ่งโป่งมิตรภาพที่ 78',
    'บ้านกุงแกงป่าไม้อุทิศที่ 12',
    'บ้านแพมบก',
    'บ้านสบแพม',
    'บ้านแม่อีแลบ',
    'บ้านมะโนรา',
    'บ้านปางปึงราษฎร์บำรุง',
    'บ้านห้วยเดื่อ',
    'บ้านแม่เหมืองหลวง',
    'บ้านโป่งสา',
    'ปางตองประชาสรรค์',
    'บ้านขุนสาใน',
    'บ้านแกงหอม สาขาบ้านม้ง',
    'บ้านหมากพริก',
    'บ้านเหมืองแร่',
    'บ้านสบสา',
    'บ้านเมืองแปง',
    'บ้านผาสำราญ',
    'บ้านห้วยหมีศรีสวัสดิ์',
    'บ้านแกงหอม',
    'บ้านแม่นาเติง',
    'บ้านนาจลอง',
    'บ้านปางแปก',
    'บ้านไทรงาม',
    'บ้านดอยผีลู',
    'บ้านดอยผีลู สาขาบ้านในของ',
    'บ้านม่วงสร้อย',
    'บ้านหมอแปง',
    'บ้านป่ายาง',
    'บ้านน้ำปลามุง',
    'บ้านแม่ปิง สาขาบ้านหวาย',
    'ชุมชนบ้านแม่ฮี้',
    'บ้านแม่ปิง',
    'อนุบาลปาย (เวียงใต้)',
    'บ้านน้ำฮู',
    'บ้านใหม่สหสัมพันธ์',
    'บ้านเวียงเหนือ',
    'สังวาลย์วิทย์ 3',
    'บ้านเมืองแพม',
    'บ้านถ้ำลอด',
    'บ้านผามอน',
    'บ้านห้วยแห้ง',
    'บ้านวนาหลวง',
    'บ้านนาปู่ป้อม',
    'บ้านนาปู่ป้อม สาขาทุ่งสาแล',
    'บ้านน้ำฮูผาเสื่อ',
    'บ้านส่อแบะ',
    'บ้านปางบอนวัฒนาคาร',
    'บ้านทุ่งหลวงวัฒนาคาร',
    'บ้านแม่ละนา',
    'บ้านจ่าโบ่',
    'บ้านปางคาม',
    'บ้านยาป่าแหน',
    'บ้านผาแดง',
    'บ้านห้วยเฮี๊ยะ',
    'อนุบาลปางมะผ้า',
    'บ้านน้ำริน',
    'ศูนย์ปางมะผ้าในโครงการตามพระราชดำริ',
    'บ้านลุกป่าก๊อ',
    'บ้านกึ้ดสามสิบ',
    'บ้านน้ำฮูผาเสื่อสาขาปางตอง',
    'ห้องสอนศึกษา',
    'ศึกษาสงเคราะห์แม่ฮ่องสอน',
    'ขุนยวมวิทยา',
    'ปายวิทยาคาร',
    'ราชประชานุเคราะห์ 22',
    'ปางมะผ้าพิทยาสรรพ์',
    'ราชประชานุเคราะห์ 34',
    'เขตพื้นที่การศึกษาแม่ฮ่องสอน เขต 1',
    'ดอนชัยวิทยา',
    'บ้านสบหาร',
    'บ้านแพะพิทยา',
    'บ้านห้วยหลวง',
    'บ้านแม่หาร',
    'บ้านแม่ต้อบใต้',
    'บ้านแม่ต้อบเหนือ',
    'บ้านป่าหมาก',
    'บ้านแม่เกาะ',
    'บ้านขุนแม่ต้อบ',
    'อนุบาลแม่สะเรียง( บ้านโป่ง)',
    'อนุบาลแม่สะเรียงฯส.บ้านท่าข้าม',
    'บ้านพะมอลอ',
    'รัตนประทีปวิทยา',
    'บ้านแม่อุมลอง',
    'บ้านแม่อุมป๊อก',
    'บ้านแม่อุมลองหลวง',
    'เจ้าพ่อหลวงอุปถัมภ์ 10',
    'บ้านห้วยฮากไม้ใต้',
    'บ้านห้วยฮากไม้เหนือ',
    'บ้านห้วยเดื่อ',
    'บ้านแม่ปุ๋น',
    'บ้านห้วยงู',
    'บ้านแม่ละ',
    'บ้านอมพาย',
    'บ้านช่างหม้อ',
    'บ้านสันติสุข',
    'บ้านแม่สะลาบ',
    'บ้านจอซิเดอเหนือ',
    'บ้านห้วยแห้ง',
    'บ้านห้วยหมูพิทยา',
    'บ้านแม่ก๋อน',
    'บ้านศรีมูลเมือง',
    'บ้านห้วยห้อม',
    'บ้านท่าตาฝั่ง',
    'บ้านห้วยโผ',
    'บ้านแม่กองคา',
    'บ้านทุ่งแพม',
    'บ้านห้วยทราย',
    'บ้านห้วยสิงห์',
    'บ้านแม่กองแป',
    'ชุมชนบ้านน้ำดิบ',
    'บ้านคะปวง',
    'ไทยรัฐวิทยา 33 (บ้านทุ่งพร้าว)',
    'บ้านแม่สะเรียง',
    'ทองสวัสดิ์วิทยาคาร',
    'บ้านจอมแจ้งมิตรภาพที่ 193',
    'บ้านไร่วิทยา',
    'สังวาลย์วิทยา',
    'บ้านเด่น',
    'บ้านแม่ลิด',
    'บ้านดอยเลี่ยม',
    'บ้านแม่สวรรค์น้อย',
    'บ้านสุดห้วยนา',
    'บ้านแม่ลิด สาขาบ้านแม่สวรรค์หลวง',
    'บ้านห้วยปางผาง',
    'บ้านแม่กะไน',
    'บ้านแม่จ๊าง',
    'บ้านห้วยปลากั้ง',
    'บ้านขุนวง',
    'บ้านดงกู่',
    'บ้านขุนวงเหนือ',
    'บ.แม่จ๊างส.บ้านดงหลวง-ดงน้อย',
    'บ้านโพซอ',
    'บ้านเสาหิน',
    'บ้านสล่าเจียงตอง',
    'บ้านส้มป่อย',
    'บ้านแม่อมลาน',
    'บ้านฟักทอง',
    'บ้านป่าแก่',
    'บ้านแม่ลาผาไหว',
    'บ้านขุนแม่ลา',
    'บ้านห้วยผึ้ง',
    'บ้านแม่กวางใต้',
    'บ้านท่าผาปุ้ม',
    'บ้านแม่เตี๋ย',
    'บ้านห้วยหมากหนุน',
    'บ้านแม่สะกั๊วะ',
    'บ้านห้วยหมากหนุน สาขาบ้านขุนแม่เตี๋ย',
    'บ้านแม่กวางเหนือ',
    'บ้านแม่สะกึ๊ด',
    'บ้านแม่ปอถ่า',
    'บ้านแม่โถ',
    'บ้านแม่โถใต้',
    'บ้านห้วยไม้ซาง',
    'บ้านหัวแม่โถ',
    'บ้านแม่อุมพาย',
    'บ้านแม่จอ',
    'บ้านห้วยผึ้งใหม่',
    'บ้านผาแดงหลวง',
    'บ้านแม่นาจางเหนือ',
    'บ้านแม่ขีด',
    'บ้านแม่นาจาง',
    'บ้านหนองม่วน',
    'บ้านแม่สะแมง',
    'บ้านกอกหลวง',
    'บ้านแม่ฮุ',
    'บ้านพระบาทห้วยผึ้ง',
    'บ้านท่าสองแคว ส.บ้านทุ่งรวงทอง',
    'บ้านแม่แลบ',
    'ลุ่มน้ำวิทยา',
    'บ้านป่าหมากวิทยา',
    'บ้านแม่งะ',
    'บ้านแม่สะปึ๋งใต้',
    'บ้านแม่สะปึ๋ง',
    'อนุบาลบ้านแม่ลาน้อย',
    'บ้านท่าสองแคว',
    'บ้านห้วยไก่ป่า',
    'ชุมชนแม่ลาศึกษา',
    'บ้านแม่สุ',
    'บ้านห้วยกองเป๊าะ',
    'บ้านห้วยกู่ป๊ะ',
    'บ้านหัวลา',
    'บ้านแม่ปาง',
    'บ้านห้วยมะกอก',
    'บ้านกะริคี',
    'บ้านดูลาเปอ',
    'บ้านแม่และ',
    'บ้านดงใหม่',
    'บ้านละอูบ',
    'ศิษย์เก่าพยาบาลศิริราชบ้านดง',
    'บ้านห้วยห้า',
    'บ้านห้วยห้า สาขาบ้านห้วยห้าใหม่',
    'บ้านสาม',
    'บ้านกองก๋อย',
    'บ้านแม่แพ',
    'บ้านแม่แพน้อย',
    'บ้านผาเยอ',
    'บ้านทะโลงเหนือ',
    'บ้านห้วยวอก',
    'บ้านห้วยไก่ป่า',
    'บ้านแม่ลาย',
    'บ้านห้วยกุ้ง',
    'บ้านกองแปใต้',
    'บ้านป่าโปง',
    'บ้านป่าโปง สาขาบ้านกองแปเหนือ',
    'บ้านต้นงิ้ว',
    'บ้านป่าโปง สาขาบ้านห้วยเหี้ยะ',
    'บ้านอุมดาเหนือ สาขาบ้านอุมดาใต้',
    'บ้านแม่ออกเหนือ',
    'บ้านอุมดาเหนือ',
    'ชุมชนบ้านผาผ่า',
    'อนุบาลสบเมย (คอนผึ้ง)',
    'บ้านไหม้',
    'บ้านแม่เกาะวิทยา',
    'บ้านแม่ออก',
    'บ้านแม่สวด',
    'บ้านห้วยม่วง',
    'บ้านแม่หลุย',
    'บ้านนาดอย',
    'บ้านแม่หาด',
    'บ้านแม่แพหลวง',
    'ล่องแพวิทยา',
    'บ้านห้วยม่วง ส.บ้านแม่เลาะ',
    'บ้านอุมโละเหนือ',
    'บ้านแม่เงา สาขาบ้านกองอูม',
    'บ้านแม่เงา',
    'บ้านแม่สามแลบ',
    'บ้านห้วยกองก้าด',
    'บ้านซิวาเดอ',
    'บ้านกอมูเดอ',
    'บ้านกลอเซโล',
    'บ้านแม่แคะ',
    'บ้านแม่ตอละ',
    'บ้านสบเมย',
    'บ้านห้วยกระต่าย',
    'บ้านบุญเลอ',
    'บ้านเครอะบอ',
    'บ้านห้วยน้ำใส',
    'บ้านทิยาเพอ สาขาบ้านห้วยไชยยงค์',
    'บ้านทิยาเพอ',
    'บ้านทีฮือลือ',
    'บ้านแม่คะตวน',
    'บ้านห้วยกองมูล',
    'บ้านห้วยกองมูล สาขาบ้านขุนแม่คะตวน',
    'บ้านห้วยกองมูล สาขาบ้านห้วยทิชะ',
    'บ้านแม่ทะลุ',
    'บ้านปู่แก้ว',
    'บ้านดอยงาม',
    'บ้านเลโคะ',
    'บ้านแม่ลามา',
    'บ้านเวฬุวัน',
    'ล่องแพวิทยา สาขาบ้านอุมโละ',
    'แม่สะเรียง(บริพัตรศึกษา)',
    'สบเมยวิทยาคม',
    'เฉลิมรัชวิทยาคม',
    'แม่ลาน้อยดรุณสิกข์',
    'ราชประชานุเคราะห์ 21',
    'วัดกลางแดด',
    'บ้านบ่อดินสอพอง',
    'วัดเกรียงไกรเหนือ',
    'วัดเกรียงไกร(โพธิ์ประสิทธิ์)',
    'วัดชุมนุมสงฆ์(มงคลประชานุกูล)',
    'วัดสโมสร(จันทร์ฮกหลีอุทิศ)',
    'วัดวังยาง',
    'ชุมชนวัดเกาะหงษ์(นิโรธธรรมรังสรรค์)',
    'วัดตะเคียนเลื่อน(รังสีวุฒากาสอุปถัมภ์)',
    'วัดท่าทอง',
    'วัดสันคู',
    'อนุบาลนครสวรรค์',
    'วัดวังไผ่',
    'วัดศรีสวรรค์สังฆาราม',
    'วัดหัวถนน',
    'วัดท่าดินแดง',
    'วัดท่าล้อ',
    'วัดนิเวศวุฒาราม',
    'วัดยางงาม(ประชาพัฒนา)',
    'บ้านคุ้งวารี',
    'ชุมชนวัดบ้านแก่ง',
    'วัดเกาะแก้ว',
    'วัดบ้านมะเกลือ',
    'บ้านสระงาม',
    'วัดบึงน้ำใส',
    'วัดคลองคาง(แดงประชานุกูล)',
    'บ้านคลองลาดเค้า',
    'ศิริราษฎร์สามัคคี',
    'อนุบาลเมืองนครสวรรค์(เขากบวิวรณ์สุขวิทยา)',
    'วัดพระนอน',
    'วัดบ้านไร่',
    'วัดหัวดง',
    'วัดรังงาม',
    'บ้านเขากะลา',
    'บ้านปากดง(เทียนศรีประสิทธิ์)',
    'วัดบางม่วง',
    'วัดหาดทรายงาม',
    'วัดวังหิน',
    'วัดสันติธรรม',
    'วัดดอนใหญ่',
    'วัดหนองโรง',
    'วัดหนองเขนง',
    'วัดทัพชุมพล',
    'วัดสวรรค์ประชากร',
    'บ้านสามัคคีธรรม',
    'วัดศรีอุทุมพร',
    'บ้านสุวรรณประชาพัฒนา',
    'วัดศรีประชาสรรค์',
    'วัดหนองกระโดน',
    'วัดศรีอัมพวัลย์',
    'วัดวังสวัสดี',
    'ศรีพูลราษฎร์สามัคคี',
    'วัดบริรักษ์ประชาสาร',
    'วัดสมานประชาชน',
    'วัดเนินมะขามงาม',
    'วัดเขามโน',
    'บ้านพรหมเขต',
    'วัดหนองปลิง',
    'วัดสุบรรณาราม',
    'บ้านช่อกระถินพัฒนา',
    'อนุบาลโกรกพระ',
    'วัดนากลาง',
    'บ้านเขาปูน',
    'วัดหนองพรมหน่อ',
    'บ้านเนินศาลา',
    'บ้านคลองม่วง',
    'บ้านเขาถ้ำพระ',
    'วัดเนินกะพี้',
    'วัดบางประมุง',
    'วัดท่าซุด(เจริญศิลป์)',
    'วัดมโนราษฎร์',
    'บ้านโพธิ์งาม',
    'ราษฎร์อุทิศ',
    'บ้านบางมะฝ่อ',
    'บ้านเนินเวียง',
    'วัดบางมะฝ่อ"จันทโชตอนุสรณ์"',
    'วัดยางตาล',
    'สระวิทยา',
    'ชุมชนวัดบ้านหว้า"ประชาประสาทวิทย์"',
    'ภาณุฑัตกรีฑาเวทย์',
    'วัดศาลาแดง',
    'บ้านกระจังงาม',
    'บ้านหาดสูง',
    'ชุมชนวัดเกยไชยเหนือ(นิรภัยประชานุกูล)',
    'วัดท่านา',
    'บ้านต้นโพธิ์(ประชานุกูล)',
    'บ้านเนิน',
    'บ้านท่าจันทน์(ศักดี)',
    'วัดแหลมยาง',
    'วัดฆะมัง',
    'วัดพิกุล',
    'วัดท้บกฤชเหนือ',
    'วัดทับกฤชเหนือสาขาวัดทุ่งแว่น',
    'ชุมชนวัดคลองปลากด',
    'อนุบาลชุมแสง(วัดทับกฤชกลาง)',
    'วัดดอนสนวน',
    'วัดท่าไม้',
    'บ้านวังใหญ่',
    'วัดวังใหญ่',
    'วัดดงกะพี้',
    'บ้านท่ากร่าง',
    'วัดหนองขอน',
    'บ้านท่าเตียน',
    'บ้านบึงหมัน(นิรภัยประชานุเคราะห์)',
    'บ้านประชาสามัคคี',
    'วัดคลองยาง',
    'วัดเนินสะเดา',
    'วัดบ้านลาด',
    'วัดบางเคียน',
    'วัดไผ่สิงห์',
    'วัดคลองเกษมเหนือ',
    'วัดคลองเกษมใต้',
    'วัดโพธิ์หนองยาว',
    'บ้านท่าจันทน์(สำลีประชานุเคราะห์)',
    'วัดแสงรังสรรค์',
    'วัดพันลาน',
    'วัดปากคลองปลากด',
    'วัดบางไซ',
    'วัดหัวกะทุ่ม',
    'วัดหนองกุ่ม',
    'วัดหนองโก',
    'บ้านหนองโพลง',
    'บ้านดงขุย',
    'อนุบาลเก้าเลี้ยว(วัดเก้าเลี้ยว',
    'บ้านแหลมยาง',
    'ชุมชนวัดเขาดินเหนือ',
    'โอสถสภาอุปถัมภ์',
    'วัดมรรครังสฤษดิ์',
    'วัดดงเมือง',
    'บ้านคลองคล้า',
    'สังข์บุญธรรมราษฎร์นุสรณ์',
    'บ้านหนองหัวเรือ',
    'วัดมหาโพธิใต้',
    'ชุมชนวัดมหาโพธิเหนือ',
    'บ้านยางใหญ่',
    'บ้านห้วยรั้ว(เสนาณรงค์อุปถัมภ์)',
    'วัดหนองเต่า',
    'วัดหนองแพงพวย',
    'บ้านทุ่งตาทั่ง(ศิริราษฎร์บำรุง)',
    'ศึกษาศาสตร์',
    'วัดหาดเสลา(ไพศาลศิริราษฎร์อุปถัมภ์)',
    'วัดหัวดงเหนือ(บรรพตานุสรณ์)',
    'วัดหัวดงใต้(บางทองประชาสรรค์)',
    'บ้านเนินพะยอม',
    'วัดหนองกลอย',
    'บ้านหนองเต่า',
    'ย่านคีรี',
    'บ้านห้วยบง',
    'บ้านทรัพย์ไพรวัน',
    'เขาสามยอด',
    'บ้านซับผักกาด',
    'เขาทอง',
    'วัดบ้านบน',
    'อนุบาลพยุหะคีรี(วัดพระปรางค์เหลือง)',
    'บ้านเขาไม้เดน',
    'บ้านสระเศรษฐี',
    'วัดหนองคล่อ',
    'วัดสุวรรณรัตนาราม',
    'วัดคลองบางเดื่อ',
    'วัดใหม่',
    'วัดน้ำทรง',
    'นิคมเขาบ่อแก้ว',
    'บ้านสระบัว',
    'บ้านหนองไม้แดง',
    'เขาสระนางสรง',
    'วัดโป่งสวรรค์',
    'วัดหนองหมู',
    'เนินมะกอก',
    'บ้านประดู่เฒ่า',
    'วัดหัวงิ้ว',
    'พยุหะศึกษาคาร',
    'วัดเขาบ่อพลับ',
    'บ้านดอนกระชาย',
    'วัดยางขาว',
    'วัดท่าโก',
    'วัดหาดสะแก',
    'บ้านย่านมัทรี',
    'สามแยกเจ้าพระยา',
    'ประชาอุทิศ',
    'บ้านใหม่ศึกษา',
    'บ้านธารหวาย',
    'นครสวรรค์',
    'สตรีนครสวรรค์',
    'จิรประวัติวิทยาคม',
    'พระบางวิทยา',
    'นวมินทราชูทิศ มัชฌิม',
    'บึงบอระเพ็ดวิทยา',
    'นครสวรรค์ปัญญานุกูล',
    'โกรกพระ',
    'ทับกฤชพัฒนา',
    'ชุมแสงชนูทิศ',
    'เก้าเลี้ยววิทยา',
    'หัวดงราชพรหมาภรณ์',
    'พยุหะพิทยาคม',
    'เขาทองพิทยาคม',
    'เขากะลาวิทยาคม',
    'บ้านเทพนคร',
    'บ้านไร่',
    'บ้านท่าตะคร้อ',
    'ปราสาทอนุสรณ์',
    'บ้านท่าเสลี่ยง',
    'บ้านโพธิ์พัฒนา',
    'บ้านใหม่สามัคคี',
    'บ้านหงษ์ทอง',
    'บ้านเทียมเจริญ',
    'ยางเลียงราษฎร์พัฒนา',
    'บ้านเขาน้ำเพชร',
    'บ้านปางเรือ(จุฬาสงเคราะห์)',
    'อ่างทองพัฒนา(สนง.สลากกินแบ่งสงเคราะห์)',
    'ประชารัฐพัฒนา',
    'บ้านไตรตรึงษ์',
    'บ้านวังประดา',
    'บ้านดงตาจันทร์',
    'บ้านโนนโก',
    'บ้านวังน้ำขาว',
    'สหวิทยาคม',
    'บ้านทรงธรรม',
    'วัดราษฎร์เจริญพร',
    'บ้านไทรย้อย',
    'บ้านสุวรรณภูมิ',
    'หนองบัวราษฎร์บำรุง',
    'วัดอุทุมพร',
    'บ้านน้ำโท้ง',
    'บ้านโนนสมอ',
    'บ้านบ่อตาโพธิ์',
    'บ้านตลุกงาม',
    'บ้านคลองเรือ',
    'บ้านโขมงหัก',
    'บ้านวังโบสถ์',
    'บ้านสระสิงห์โต',
    'ชุมชนบ้านคณฑี(ประสิทธิ์อุปถัมภ์)',
    'บ้านหนองเต่าสามัคคี',
    'บ้านคลองสีนวล',
    'บ้านธำมรงค์(รองประชาสงเคราะห์)',
    'อนุบาลเมืองกำแพงเพชร(บ้านนครชุม)',
    'วัดพระบรมธาตุ',
    'สาธิต',
    'บ้านทุ่งเศรษฐี',
    'บ้านโนนม่วง',
    'บ้านทุ่งสวน',
    'บ้านศรีไกรลาศ',
    'บ้านปางขนุน',
    'บ้านนาบ่อคำ',
    'บ้านชัยภูมิ',
    'แนะนำราษฎร์บำรุง',
    'ผินสหราษฎร์พัฒนา',
    'บ้านเขาวังเยี่ยม',
    'นิคมสร้างตนเอง 1 (ทุ่งโพธิ์ทะเล)',
    'นิคมสร้างตนเอง 2 (บ่อทอง)',
    'นิคมสร้างตนเอง 3 (แปดอ้อม)',
    'นิคมสร้างตนเอง 4 (บ้านใหม่ศรีอุบล)',
    'นิคมสร้างตนเอง 5 (สระเตย)',
    'นิคมสร้างตนเอง 6 (หนองปล้อง)',
    'วัดคูยาง',
    'วัดอรัญญิกาวาสน์',
    'บ้านเกาะน้ำโจน',
    'บ้านลานหิน',
    'วัดกัลปพฤกษ์',
    'คลองใหญ่วิทยา',
    'บ้านหนองรี',
    'นิยมราษฎร์วิทยา',
    'จำรูญชัยพฤกษ์ราษฎร์วิทยา',
    'บ้านมอสมบัติ',
    'บ้านวังทอง',
    'บ้านหนองหญ้ามุ้ง',
    'บ้านคลองใหญ่ใต้',
    'บ้านโพธิ์สวัสดิ์',
    'อนุบาลกำแพงเพชร',
    'บ้านหนองกรด',
    'บ้านลำมะโกรก',
    'บ้านหนองสะแก',
    'ประชาสันติภาพ(วงศ์สุดใจอุปถัมภ์)',
    'บ้านบ่อสามแสน',
    'ชุมชนบ้านหนองปลิง(นาคอุปถัมภ์)',
    'บ้านน้ำดิบ(ศรีกำแพงอุปถัมภ์)',
    'บ้านคลองห้วยทราย',
    'บ้านเกาะสะบ้า',
    'บ้านท่าเสากระโดง',
    'บ้านทุ่งตาพุก',
    'บ้านดาดทองเจริญ',
    'บ้านหนองใหญ่',
    'บ้านวังตะเคียน',
    'บ้านหนองขาม',
    'อนุบาลไทรงาม',
    'บ้านเนินสำราญราษฎร์พัฒนา',
    'อนุบาลไทรงาม สาขาบ้านหนองไม้แดง',
    'บ้านหนองหัวควาย',
    'บ้านพานทอง',
    'บ้านโนนจั่น',
    'บ้านบ่อแก้ว',
    'ห้วยใหญ่ยางงาม',
    'บ้านทุ่งมหาศาล',
    'บ้านแก้วสุวรรณ',
    'บ้านทุ่งมหาชัย',
    'บ้านป่าถั่ว',
    'บ้านแม่บัว',
    'บ้านหัวยาง',
    'บ้านหนองคล้า',
    'บ้านวังโขน',
    'บ้านจิกคันช้อน',
    'บ้านโนนใหญ่',
    'บ้านเนินกรอย',
    'บ้านใหม่สามัคคี',
    'บ้านหนองไม้กอง',
    'อ่อนวิมลราษฎร์วิทยา',
    'ยั่งยืนราษฎร์วิทยา',
    'บ้านจิกลาด',
    'บ้านหนองแม่แตง (ธรรมศาสตร์อาสา)',
    'บ้านตอรัง',
    'บ้านไผ่ตาสุ่ม',
    'หนองลวกราษฎร์บำรุง',
    'สิริแก้วเจริญ',
    'บ้านคุยป่ายาง',
    'บ้านเขาคีริส',
    'บ้านหนองโสน',
    'บ้านเมืองพาน',
    'บ้านทุ่งรวงทอง',
    'บ้านบึงลูกนก',
    'บ้านบึงพิไกร',
    'บ้านวังเฉลียง',
    'บ้านไร่ดง',
    'บ้านคุยบ้านโอง',
    'บ้านนาป่าแดง',
    'บ้านคุยแขวน',
    'วัดโพธาราม',
    'บ้านหนองตากล้า',
    'บ้านท่าไม้',
    'บ้านวังชะโอน',
    'บ้านเก่า',
    'บ้านวังไม้แดง',
    'บ้านสมอโคน',
    'บ้านน้ำดิบมะพร้าว',
    'บ้านเขาสว่างอารมณ์',
    'ชุมชนบ้านเขาแก้ว',
    'บ้านใหม่เขานิยม',
    'อนุบาลพรานกระต่าย',
    'บ้านพรานกระต่าย',
    'บ้านลานสะเดา',
    'บ้านนานอก',
    'บ้านลานทอง',
    'บ้านลานกระทิง',
    'บ้านคุยป่ารัง',
    'บ้านหนองทราย',
    'บ้านวังมะค่า',
    'บ้านวังตะแบก',
    'บ้านคุยประดู่',
    'หนองแขมประชาสรรค์',
    'บ้านห้วยน้ำใส',
    'บ้านหนองหัววัว',
    'บ้านทุ่งน้ำตก',
    'บ้านบางลาด',
    'บ้านลานไผ่',
    'บ้านคลองห้วยยั้ง',
    'บ้านลานช้างท่าว',
    'บ้านสุขขุม(อนุสรณ์พลยงค์)',
    'บ้านแคทอง',
    'บ้านหนองจิก',
    'บ้านจันทิมา',
    'บ้านช่องลม',
    'บ้านเกศกาสร',
    'บ้านปรือพันไถ',
    'บ้านบึงกระดาน',
    'ไทยรัฐวิทยา 16 (บ้านโนนพลวง)',
    'บ้านอินทรานุสรณ์',
    'บ้านคุยมะม่วง',
    'บ้านเด่นพระ',
    'บ้านบึงทับแรต',
    'บ้านหนองละมั่งทอง',
    'บ้านลานตาบัว',
    'บ้านประชาสุขสันต์',
    'บ้านหร่ายการ้อง',
    'วัฒนราษฎร์ศึกษา',
    'บ้านอาชานุสรณ์',
    'อนุบาลลานกระบือ',
    'บ้านปลักไม้ดำ',
    'บ้านหนองแขม',
    'นิคมสร้างตนเองบางระกำ 5',
    'บ้านหนองปากดง',
    'บ้านประดาเจ็ดรัง',
    'บ้านใหม่สามัคคี',
    'บ้านหนองหลวง',
    'บ้านทรายทอง',
    'บ้านหนองมะเกาะ',
    'บ้านบึงมาลย์',
    'บ้านสุรเดชสามัคคี',
    'บ้านโกสัมพี(วิเศษน้อมมิตรอนุกูล)',
    'บ้านมะเดื่อชุมพร',
    'อนุบาลโกสัมพีนคร(บ้านท่าคูณ)',
    'บ้านไร่ลำปาง',
    'บ้านไทยทวี',
    'บ้านดงซ่อม',
    'บ้านคลองเมือง',
    'บ้านหนองวัวดำ',
    'บ้านโนนสมบูรณ์',
    'บ้านหนองแดน',
    'บ้านหนองแดน สาขาบ้านโละโคะ',
    'บ้านเกาะรากเสียด',
    'บ้านวังชมภู',
    'บ้านท่านา',
    'บ้านใหม่พัฒนา',
    'ยอดประชาสรรค์',
    'บ้านลานดอกไม้',
    'บ้านเกาะพิมูล',
    'กำแพงเพชรพิทยาคม',
    'วชิรปราการวิทยาคม',
    'ทุ่งโพธิ์ทะเลพิทยา',
    'วัชรวิทยา',
    'คณฑีพิทยาคม',
    'นาบ่อคำวิทยาคม',
    'นครไตรตรึงษ์',
    'วังตะเคียนประชานุสรณ์',
    'หนองกองพิทยาคม',
    'เฉลิมพระเกียรติสมเด็จพระศรีนครินทร์ กำแพงเพชร',
    'ไทรงามพิทยาคม',
    'บ่อแก้ววิทยา',
    'พรานกระต่ายพิทยาคม',
    'พิไกรวิทยา',
    'เรืองวิทย์พิทยาคม',
    'ลานกระบือวิทยา',
    'มัธยมพัชรกิติยาภา 2 กำแพงเพชร',
    'ราษฎร์ปรีชาวิทยาคม',
    'โกสัมพีวิทยา',
    'บ้านคลองใหญ่ใต้',
    'รอดนิลวิทยา',
    'บ้านใหม่เจริญสุข สาขาบ้านคลองปิ่นโตประชาสามัคคี',
    'บ้านสุขสำราญ',
    'บ้านคลองพลูประชาสรรค์',
    'อนุบาลคลองลาน',
    'บ้านบึงหล่ม',
    'บ้านมอแดง(สิงห์ทองประชาสรรค์)',
    'บ้านท่าช้าง',
    'บ้านใหม่เจริญสุข',
    'บ้านพรหมมาสามัคคี',
    'บ้านคลองน้ำไหลใต้',
    'บ้านโชคชัยพัฒนา',
    'บ้านทะเลพัฒนา',
    'บ้านท่าข้ามสามัคคี',
    'ชุมชนบ้านคลองลาน',
    'บ้านปากคลองลาน',
    'บ้านใหม่ธงชัย',
    'บ้านแปลงสี่',
    'บ้านคลองเตย',
    'บ้านโป่งน้ำร้อน',
    'บ้านคลองไพร',
    'บ้านคลองสมบูรณ์',
    'บ้านคลองสมบูรณ์ สาขาบ้านคลองสมุย',
    'บ้านคลองมดแดง',
    'บ้านคลองมดแดง สาขาบ้านใหม่ชุมนุมไทร',
    'บ้านคลองมดแดง สาขาป่าคา',
    'บ้านท่ามะเขือ',
    'บ้านเพชรนิยม',
    'บ้านมอสมบูรณ์มิตรภาพที่ 189',
    'บ้านหนองปรือ',
    'บ้านคลองแขยงวิทยา',
    'สักงามประชาสรรค์(เกษตรศาสตร์อนุสรณ์37)',
    'บ้านปางลับแล',
    'บ้านดอนขวาง',
    'บ้านโนนตารอด',
    'บ้านเกาะตาล',
    'ชุมชนบ้านโค้งไผ่',
    'บ้านหัวรัง',
    'บ้านวังโป่งพัฒนา',
    'บ้านสระตาพรม',
    'บ้านดงดำมิตรภาพที่ 88',
    'บ้านโคกเลาะ',
    'บ้านดอนโค้ง',
    'วัดคูหาสวรรค์',
    'วัดคูหาสวรรค์ สาขาบ้านโอ่งทานราษฎร์บำรุง',
    'ประชาราษฎร์สามัคคี',
    'บ้านเปาะสวอง',
    'บ้านใหม่หนองยาง',
    'บ้านบ่อกระบาก',
    'บ้านหนองบอน',
    'บ้านศรีเกษตรพัฒนา',
    'บ้านศรีสมบูรณ์พัฒนา',
    'คีรีวงศ์วัฒนา',
    'บ้านปางมะนาว',
    'อนุบาลปางมะค่า',
    'บ้านพัดโบก',
    'บ้านหนองน้ำแดง',
    'บ้านเขาพริกไทย',
    'บ้านศรีไพศาล',
    'บ้านคลองสะพานช้าง',
    'บ้านส่องตาแล',
    'บ้านโป่งแต้',
    'บ้านวังน้ำพัฒนา',
    'บ้านวังน้ำพัฒนา สาขาบ้านบ่อหิน',
    'บ้านเกาะแก้วอนุสรณ์',
    'อนุบาลขาณุวรลักษบุรี',
    'บ้านหนองกระทุ่ม',
    'บ้านอุดมสามัคคี',
    'บ้านวังพลับ',
    'บ้านหนองตะเคียน',
    'วัดพัฒนราษฎร์บำรุง',
    'บ้านเกาะฝ้าย (ราษฎร์อุทิศวิทยาคาร)',
    'บ้านบึงเสือเต้น',
    'วัดปรีชาราษฎร์บำรุง',
    'บ้านบึงหล่มสามัคคี',
    'บ้านหัวเสลา',
    'บ้านโป่งดู่ประชาอุทิศ',
    'บ้านวังน้ำซึม',
    'บ้านวังตาช่วย',
    'บ้านหนองชุมแสง',
    'บ้านช่องลม',
    'บ้านนาเหนือ',
    'บ้านห้วยแก้วสามัคคีธรรม',
    'บ้านไร่ดอนแตง',
    'บ้านเขาพริกอนุสรณ์',
    'บ้านหนองช้างงาม',
    'บ้านหนองปล้อง',
    'บ้านวังล้อมรำลึก',
    'บ้านวังหามแห',
    'บ้านจิตตมาสพัฒนา',
    'บ้านวังหัวแหวนพัฒนา',
    'บ้านหนองชะแอน',
    'ชุมชนบ้านสลกบาตร(วันครู 2504)',
    'บ้านโนนปอแดง(รัฐพินิจประชาสรรค์)',
    'บ้านรังแถว',
    'วชิรสารศึกษา',
    'วัดสว่างอารมณ์',
    'บ้านหาดชะอม',
    'วัดน้อยวรลักษณ์',
    'วัดหนองเหมือด',
    'ชุมชนประชาสามัคคี',
    'พิบูลวิทยาคาร',
    'บ้านถนนงาม (มิตรภาพที่ 27)',
    'ทุ่งน้อยพัฒนา',
    'หนองปรือประชาสรรค์',
    'ประชารักษ์ศึกษา',
    'บ้านหนองผักหนาม',
    'บ้านท่าพุทรา',
    'วัดคลองเจริญ',
    'บ้านคลองแขยง',
    'บ้านป่าเหียง',
    'บ้านวังบัว',
    'วัดแสงอุทัย',
    'บ้านสามแยก',
    'อนุบาลคลองขลุง',
    'บ้านหนองจอก',
    'บ้านโนนมะกอก',
    'บ้านแม่ลาด',
    'วัดคงคาราม',
    'วัดพรหมประดิษฐ์',
    'วัดพรหมประดิษฐ์ สาขาบ้านท่ามะขาม',
    'บ้านห้วยน้อย',
    'บ้านร้อยไร่',
    'บ้านมาบคล้า',
    'บ้านวังหันน้ำดึง',
    'บ้านทุ่งหันตรา',
    'บ้านวังแขม (สว่างชัยวงษ์)',
    'บ้านไร่ใหม่',
    'บ้านบ่อทอง',
    'บ้านบึงลาด',
    'ชุมชนวังไทรวิทยาคาร',
    'บ้านสามเรือน',
    'อ่างทองราษฎร์วิทยา',
    'บ้านกระโดนเตี้ย',
    'บ้านหนองเม่น',
    'บ้านหนองทองหล่อ',
    'บ้านช้างคับ',
    'บ้านทรัพย์มะนาว',
    'บ้านนิคม',
    'บ้านวังตะล่อม',
    'บ้านวังน้ำ',
    'บ้านคลองยาง',
    'วัดฤกษ์หร่ายสามัคคี',
    'วัดพิกุลทอง',
    'วัดอุเบกขาราม',
    'บ้านหนองโมก',
    'บ้านถาวรวัฒนา',
    'บ้านบึงสำราญ',
    'บ้านหนองไผ่',
    'บ้านศรีอุดมธัญญะ',
    'อนุบาลทรายทองวัฒนา',
    'บ้านวังน้ำแดง',
    'บ้านหนองนกชุม',
    'บ้านถนนน้อย',
    'บ้านชุมนาก',
    'บ้านคลองสุขใจ',
    'บ้านทุ่งทอง',
    'บ้านดงเจริญ',
    'บ้านเพชรมงคล',
    'บ้านไพรสวรรค์',
    'บ้านคลองปลาสร้อย',
    'บ้านปางตาไว',
    'บ้านตากฟ้าพัฒนา',
    'บ้านคลองลึกพัฒนา',
    'บ้านไผ่ยาวสามัคคี',
    'บ้านท่าขึ้น',
    'บ้านโพธิ์ทอง',
    'บ้านหนองหล่ม',
    'บ้านคลองขุด',
    'บ้านท่าขึ้น สาขาบ้านคลองพร้าว',
    'อนุบาลปางศิลาทอง',
    'บ้านหินดาตราษฎร์บำรุง',
    'บ้านหนองหิน',
    'บ้านเขาน้ำอุ่น',
    'บ้านมอเจริญ',
    'บ้านมอเจริญ สาขาบ้านใหม่เชียงราย',
    'บ้านจอมทองพัฒนา',
    'บ้านอุดมทรัพย์',
    'บ้านซับใหญ่ศึกษานารีอนุสรณ์ 5',
    'บ้านกระบวยทอง',
    'บ้านโนนพลวง',
    'บ้านสามขา',
    'บ้านวังเจ้า',
    'บ้านโพธิ์เอน',
    'บ้านทุ่งซ่าน',
    'บ้านหนองคล้าพงษ์ทอง',
    'อนุบาลบึงสามัคคี(บ้านทุ่งสนุ่น)',
    'บ้านดงเย็น',
    'บ้านศรีทองสามัคคี',
    'บ้านชายเคือง',
    'บ้านวังชะโอน',
    'บ้านคอปล้อง',
    'บ้านไผ่งาม',
    'บ้านระหานประชาศึกษา',
    'บ้านบึงสามัคคีกำแพงเขต',
    'คลองลานวิทยา',
    'สักงามวิทยา',
    'คลองลานพัฒนาจินดาศักดิ์',
    'ขาณุวิทยา',
    'โค้งไผ่วิทยา',
    'ปางมะค่าวิทยาคม',
    'สลกบาตรวิทยา',
    'คลองขลุงราษฎร์รังสรรค์',
    'วังแขมวิทยาคม',
    'วังไทรวิทยาคม',
    'ทุ่งทรายวิทยา',
    'ปางศิลาทองศึกษา',
    'ระหานวิทยา',
    'บ้านตลุกกลางทุ่ง',
    'บ้านเนินมะลื่น',
    'บ้านลานมะคร้อ',
    'บ้านคลองสัก',
    'อนุบาลบ้านบ่อไม้หว้า',
    'ชุมชนบ้านป่ามะม่วง',
    'บ้านชะลาด',
    'บ้านลานห้วยเดื่อ',
    'บ้านตลุกป่าตาล',
    'บ้านโป่งแดง',
    'บ้านหนองนกปีกกา',
    'บ้านหนองกระทุ่ม',
    'บ้านโพรงตะเข้',
    'บ้านชะลาดระฆัง',
    'บ้านมูเซอ',
    'บ้านลานสาง',
    'บ้านห้วยเหลือง',
    'บ้านหนองแขม',
    'บ้านห้วยนึ่ง',
    'บ้านวังม่วง',
    'บ้านหนองกระโห้',
    'ชุมชนบ้านไม้งาม',
    'ทรัพย์สมบูรณ์พิทยาคม',
    'บ้านลานเต็ง',
    'บ้านหนองร่ม',
    'บ้านวังประจบ',
    'น้ำดิบพิทยาคม',
    'หนองเสือพิทยาคม',
    'บ้านสะแกเครือ',
    'บ้านเกาะอ้ายด้วน',
    'บ้านน้ำด้วน',
    'ชุมชนบ้านวังหิน',
    'บ้านท่าไม้แดง',
    'บ้านไร่',
    'บ้านหนองบัวใต้',
    'บ้านคลองห้วยทราย',
    'บ้านท่าเล่',
    'เด่นวิทยา',
    'วัดปากห้วยไม้งาม',
    'บ้านหนองบัวเหนือ',
    'ตากสินราชานุสรณ์',
    'อนุบาลตาก',
    'ศรีวิทยา',
    'ชูวิชาราษฎร์',
    'ชุมชนบ้านแม่ยะ',
    'บ้านแม่พะยวบ',
    'สว่างวิทยา',
    'บ้านตากประถมวิทยา',
    'บ้านหนองชะลาบ',
    'วัดพระธาตุน้อย',
    'ชุมชนวัดสันป่าลาน',
    'อนุบาลรอดบำรุง',
    'ประชาพัฒนา',
    'บ้านวังไม้ส้าน',
    'บ้านปากวัง',
    'บ้านฉลอม',
    'บ้านห้วยพลู',
    'บ้านท้องฟ้า',
    'ขุนห้วยตากพัฒนาศึกษา',
    'บ้านใหม่',
    'บ้านดงยาง',
    'บ้านทุ่งกระเชาะ',
    'ยางโองน้ำวิทยาคม',
    'เด่นไม้ซุงวิทยาคม',
    'บ้านห้วยแม่บอน',
    'บ้านแม่สลิด',
    'บ้านสันกลาง',
    'บ้านยางโองนอก',
    'เสริมปัญญา',
    'บ้านเกาะลาน',
    'บ้านน้ำดิบ',
    'บ้านสันป่าป๋วย',
    'บ้านโสมง',
    'บ้านอูมวาบ',
    'บ้านหินลาดนาไฮ',
    'บ้านหนองเชียงคา',
    'ชุมชนบ้านแม่ระวานสองแคว',
    'บ้านท่าไผ่',
    'บ้านคลองไม้แดง',
    'บ้านใหม่สามัคคี',
    'บ้านวังโพ',
    'บ้านดงลาน',
    'หมู่บ้านตัวอย่าง',
    'บ้านนาตาโพ',
    'บ้านวังหวาย',
    'บ้านปากทางเขื่อนภูมิพล',
    'บ้านวังไคร้มิตรภาพที่ 102',
    'อนุบาลบ้านท่าปุย',
    'เขื่อนภูมิพล',
    'ชุมชนชลประทานรังสรรค์',
    'บ้านสามเงา',
    'บ้านป่ายางใต้',
    'บ้านป่ายางตะวันตก',
    'วัดสามเงา',
    'บ้านท่าปุยตก',
    'บ้านสบยม(น้อมมิตรอนุกูล)',
    'อนุบาลวังเจ้า',
    'เด่นวัวราษฎร์รังสรรค์',
    'ดงซ่อมพิทยาคม',
    'บ้านใหม่เสรีธรรม',
    'บ้านหนองปลาไหล',
    'บ้านผาผึ้ง',
    'บ้านตะเคียนด้วน',
    'บ้านวังตำลึง',
    'บ้านนาโบสถ์(น้อมมิตรอนุกูล)',
    'ตชด.ค่ายพระเจ้าตากสงเคราะห์ 2',
    'บ้านประดาง(บุญธรรมจำปาอุปถัมภ์)',
    'บ้านทุ่งกง',
    'บ้านท่าตะคร้อ',
    'ตากพิทยาคม',
    'ผดุงปัญญา',
    'วังหินกิตติวิทยาคม',
    'ถนอมราษฏร์บำรุง',
    'วังประจบวิทยาคม',
    'โสตศึกษาจังหวัดตาก',
    'ศึกษาสงเคราะห์ตาก',
    'บ้านตาก(ประชาวิทยาคาร)',
    'ทุ่งฟ้าวิทยาคม',
    'สามเงาวิทยาคม',
    'ยกกระบัตรวิทยาคม',
    'นาโบสถ์พิทยาคม',
    'วังเจ้าวิทยาคม',
    'บ้านป่าไร่',
    'บ้านแม่ระมาดน้อย',
    'บ้านจกปก',
    'บ้านแม่ระมาดน้อยสาขาบ้านห้วยปูลิง',
    'บ้านพะละ',
    'บ้านแม่ระมาดน้อยสาขาบ้านแพะ',
    'บ้านขะเนจื้อ',
    'กลาโหมราชเสนา',
    'บ้านป่าไร่เหนือ',
    'บ้านขุนห้วยนกแล',
    'บ้านทุ่งมะขามป้อม',
    'บ้านสันป่าไร่',
    'ท่านผู้หญิงพรสม กุลฑลจินดา สาขาบ้านห้วยกระทิง',
    'บ้านวังผา',
    'ชุมชนบ้านแม่จะเราบ้านทุ่ง',
    'บ้านแม่จะเราสองแคว',
    'บ้านห้วยบง',
    'บ้านห้วยโป่ง',
    'บ้านน้ำหอม',
    'บ้านต้นผึ้ง',
    'บ้านแม่ระมาดราษฎร์บำรุง',
    'บ้านห้วยนกแล',
    'บ้านขุนห้วยแม่ท้อ',
    'บ้านสามหมื่น',
    'ท่านผู้หญิงพรสม กุลฑลจินดา',
    'ชุมชนบ้านท่าสองยาง',
    'บ้านแม่ตื่น',
    'บ้านทุ่งถ้ำ',
    'บ้านแม่โพ',
    'บ้านอู่หู่',
    'ชุมชนบ้านแม่ต้านราษฎรบำรุง',
    'บ้านขุนห้วยแม่ต้าน',
    'บ้านแม่วะหลวง',
    'บ้านแม่อมกิ',
    'บ้านแม่สลิดหลวง',
    'บ้านแม่ระเมิง',
    'บ้านแม่ระเมิงสาขาตะพิเดอ',
    'บ้านแม่สละเหนือ',
    'บ้านแม่ออกผารู',
    'บ้านแม่สละ',
    'บ้านห้วยนกกกสาขาบ้านจ่อคี',
    'บ้านห้วยนกกก',
    'บ้านแม่อุสุ',
    'บ้านหนองบัว',
    'บ้านแม่พลู',
    'บ้านปางส้าน',
    'ไทยรัฐวิทยา 61 (บ้านห้วยยะอุ)',
    'บ้านห้วยปลาหลด',
    'บ้านธงชัย',
    'บ้านห้วยม่วง',
    'บ้านท่าอาจ',
    'บ้านวังตะเคียน',
    'บ้านแม่ตาว',
    'บ้านหัวฝาย',
    'บ้านค้างภิบาล',
    'บ้านแม่ตาวใหม่',
    'บ้านพะเด๊ะ',
    'บ้านปูแป้',
    'ญาณวิศิษฎ์',
    'วัดสว่างอารมณ์',
    'บ้านแม่ละเมา',
    'บ้านม่อนหินเหล็กไฟ',
    'บ้านเจดีย์โคะ',
    'บ้านห้วยไม้แป้น',
    'บ้านแม่โกนเกน',
    'บ้านแม่กื้ดหลวงกัญไชยมิตรภาพที่ 182',
    'บ้านแม่กื้ดสามท่า',
    'บ้านแม่กาษา',
    'บ้านน้ำดิบ',
    'ชุมชนบ้านแม่กุน้อย',
    'บ้านแม่กุเหนือ',
    'บ้านแม่กุหลวง',
    'บ้านห้วยผาลาด',
    'บ้านปูเต้อ',
    'บ้านแม่ตาวแพะ',
    'บ้านแม่ตาวใต้',
    'ชุมชนบ้านแม่ตาวกลางมิตรภาพที่ 26',
    'บ้านแม่ปะ',
    'บ้านแม่ปะเหนือ',
    'บ้านแม่ปะใต้',
    'บ้านห้วยกะโหลก',
    'บ้านห้วยหินฝน',
    'แม่สอด',
    'ป่าไม้อุทิศ 4',
    'บ้านร่มเกล้า 2',
    'บ้านร่มเกล้า 3',
    'บ้านร่มเกล้า 4',
    'บ้านป่าคาใหม่',
    'รวมไทยพัฒนา 3',
    'ไทยราษฎร์คีรี',
    'รวมไทยพัฒนา 6',
    'อรุณเมธา',
    'บ้านช่องแคบ',
    'บ้านขุนห้วยช่องแคบ',
    'บ้านทีกะเป่อ',
    'ชุมชนบ้านพบพระ',
    'บ้านหมื่นฤาชัย',
    'บ้านห้วยน้ำนัก',
    'รวมไทยพัฒนา 1',
    'รวมไทยพัฒนา 2',
    'รวมไทยพัฒนา 4',
    'รวมไทยพัฒนา 5',
    'บ้านวาเล่ย์',
    'บ้านมอเกอ',
    'ตำรวจตระเวนชายแดนบ้านแม่ออกฮู',
    'บ้านยะพอ',
    'บ้านแม่กลองใหม่',
    'บ้านแม่กลองเก่า',
    'บ้านกล้อทอ',
    'บ้านนุเซะโปล้',
    'ท่านผู้หญิงวิไล อมาตยกุล (บ้านเปิงเคลิ่ง)',
    'บ้านปะหละทะ',
    'บ้านปรอผาโด้',
    'บ้านไม้กะพง',
    'บ้านแม่กลองน้อย',
    'ขุนห้วยบ้านรุ่ง',
    'บ้านหนองหลวง',
    'บ้านเซอทะ',
    'สามัคคีวิทยาสาขาบ้านอุ้มผางคี',
    'ชุมชนบ้านอุ้มผาง',
    'สามัคคีวิทยา',
    'บ้านปูเต้อ สาขาบ้านหนองน้ำเขียว',
    'รวมไทยพัฒนา 4 สาขาบ้านพะดี',
    'บ้านปางส้าน สาขาบ้านห้วยพลู',
    'แม่ระมาดวิทยาคม',
    'แม่จะเราวิทยาคม',
    'ท่าสองยางวิทยาคม',
    'สรรพวิทยาคม',
    'แม่ปะวิทยาคม',
    'ด่านแม่ละเมาวิทยาคม',
    'แม่กุวิทยาคม',
    'พบพระวิทยาคม',
    'อุ้มผางวิทยาคม',
    'วัดยางเอน',
    'วัดตาลเตี้ย',
    'วัดคูหาสุวรรณ วันครู2504',
    'บ้านสนามบิน(ประชาศึกษา)',
    'บ้านกล้วย',
    'บ้านขวาง(น้อมประชานุกูล)',
    'บ้านเพชรไฝ',
    'บ้านคลองด่าน',
    'วัดจันทร์(ลอมราษฎร์บำรุง)',
    'บ้านคลองปลายนา(สาขาบ้านไผ่ขวาง)',
    'บ้านเหมืองตามุก',
    'วัดฤทธิ์(คำหมื่นปทุมานุสสรณ์)',
    'บ้านสวนเหนือ(ประชาบำรุง 303)',
    'วัดคุ้งยางใหญ่',
    'บ้านสวนใต้(ครองประชานุกูล)',
    'บ้านคลองปลายนา',
    'วัดวังแดง',
    'บ้านกระชงค์(ประชาอุทิศ)',
    'ศรีมหาโพธิ์(ประชาสรรค์)',
    'บ้านบางคลอง(ราษฎร์อุทิศ)',
    'อนุบาลสุโขทัย',
    'บ้านปากแคว',
    'บ้านบางคลอง(สามัคคีประชาคาร)',
    'วัดปากพระ',
    'วัดลัดทรายมูล',
    'บ้านวังขวาก',
    'บ้านหนองพยอม',
    'บ้านจอมสังข์',
    'บ้านเมืองเก่า(ศรีอินทราทิตย์)',
    'วัดเชตุพน',
    'บ้านมนต์คีรี',
    'บ้านปากคลอง',
    'บ้านวังตะคร้อ(ธนาคารกรุงเทพฯ 5)',
    'บ้านวังวน',
    'อนุบาลเมืองสุโขทัย',
    'บ้านยางซ้าย(พรหมมาประชาสรรค์)',
    'บ้านหรรษา(เจริญประชานุเคราะห์)',
    'วัดวังสวรรค์(ประชาบำรุง)',
    'บ้านวังทองแดง',
    'บ้านสำนัก',
    'บ้านไสยาศน์(ราษฎร์บูรณะ)',
    'วัดหนองตาโชติ',
    'บ้านราวรังงาม',
    'บ้านตลิ่งชัน',
    'บ้านวังหาด',
    'บ้านคลองไผ่งาม',
    'บ้านวังหิน',
    'บ้านลานกระบือ',
    'บ้านด่าน',
    'บ้านวังแดด',
    'บ้านวังโคนไผ่',
    'บ้านหนองจัง',
    'อนุบาลบ้านด่านลานหอย',
    'เชิงคีรี(สุวิชานวรวุฒิ)',
    'บ้านหนองเฒ่า',
    'บ้านวังโคนเปือย',
    'บ้านวังไทร',
    'บ้านปากคลองร่วม',
    'บ้านวังน้ำขาว',
    'บ้านห้วยไคร้',
    'บ้านลานทอง',
    'บ้านวังลึก',
    'บ้านหนองสองตอน',
    'บ้านหนองตม',
    'หนองหญ้าปล้อง',
    'บ้านหนองจิกตีนเนิน',
    'บ้านหนองไม้กอง',
    'บ้านวังตะคร้อ',
    'บ้านโตนด(คีรีมาศวิทยา)',
    'บ้านปากคลองเรือ(โพธิ์โตนด)',
    'บ้านยางแหลม',
    'บ้านเนินประดู่',
    'วัดหนองกก',
    'บ้านวังกร่าง',
    'บ้านยางเมือง',
    'บ้านคลองน้ำเย็น',
    'วัดดุสิดาราม',
    'อนุบาลคีรีมาศ(วัดบึง)',
    'วัดลายมิตรภาพที่ 80',
    'บ้านนาเชิงคีรี',
    'บ้านขุนนาวัง',
    'บ้านโว้งบ่อ',
    'บ้านนาโพธิ์',
    'บ้านน้ำลาด',
    'บ้านเขาทองผางับ',
    'บ้านน้ำพุ',
    'บ้านลานเอื้อง',
    'มหาตมาคานธีนุสรณ์(น้ำตกสายรุ้ง)',
    'บ้านเนินยาง',
    'บ้านนากาหลง',
    'บ้านป้อมประชานุกูล',
    'บ้านนาไผ่ล้อม',
    'บ้านนาสระลอย',
    'บ้านหนองตลับ',
    'ศรีคีรีมาศวิทยา',
    'บ้านสามพวง(สามัคคีพิทยา)',
    'บ้านตะเข้ขานสามัคคีวิทยา',
    'บ้านสามพวง(สาขาบ้านหนองสีดา)',
    'บ้านหนองกระดิ่ง',
    'บ้านลำคลองยาง',
    'บ้านเนินพยอม',
    'บ้านใหม่เจริญผล',
    'บ้านหนองยาง',
    'บ้านหนองบัวเขาดิน',
    'วัดมุจลินทาราม',
    'สามัคคีวิทยา(สหกรณ์อุปถัมภ์)',
    'บ้านบึงหญ้า',
    'วัดปรักรัก',
    'วัดกกแรต',
    'บ้านคลองตะเข้',
    'บ้านประดู่เฒ่า',
    'บ้านกง(ราษฎร์อุทิศ)',
    'วัดสงฆาราม',
    'บ้านบางสนิม',
    'บ้านหนองหญ้าปล้อง',
    'บ้านหนองสามพญา',
    'วัดศรีเมือง',
    'บ้านป่าสัก',
    'วัดดอนสักมิตรภาพที่ 231',
    'วัดคุ้งยาง',
    'บ้านโป่งแค',
    'วัดโบสถ์',
    'บ้านหนองกระทุ่ม',
    'บ้านหนองมะเกลือ',
    'วัดเสาหิน',
    'บ้านวังขวัญ',
    'บ้านหนองบัว',
    'บ้านหนองเสาเถียร',
    'บ้านป่ามะม่วง',
    'บ้านดงเดือย',
    'บ้านโพธิ์หอม',
    'วัดดงยาง',
    'บ้านยางแดน',
    'บ้านดอนสำโรง',
    'บ้านหนองบัว(จิตพินิจประชาสรรค์)',
    'วัดท่าฉนวน',
    'วัดหางตลาด',
    'บ้านน้ำเรื่อง',
    'อนุบาลกงไกรลาศ',
    'บ้านใหม่สุขเกษม',
    'บ้านใหม่โพธิ์ทอง',
    'บ้านข่อยสองนาง',
    'วัดทุ่ง',
    'วัดเต่าทอง',
    'วัดใหม่ไทยบำรุง',
    'บ้านหนองตูม',
    'บ้านเกาะนอก(ประชานุเคราะห์)',
    'สุโขทัยวิทยาคม',
    'บ้านด่านลานหอยวิทยา',
    'คีรีมาศพิทยาคม',
    'กงไกรลาศวิทยา',
    'ตลิ่งชันวิทยานุสรณ์',
    'หนองตูมวิทยา',
    'บ้านสวนวิทยาคม',
    'ไกรในวิทยาคม รัชมังคลาภิเษก',
    'ยางซ้ายพิทยาคม',
    'ลิไทพิทยาคม',
    'อุดมดรุณี',
    'บ้านใหม่เจริญผลพิทยาคม',
    'บ้านดงคู่',
    'บ้านห้วยไคร้',
    'บ้านห้วยติ่ง',
    'บ้านหนองบัว',
    'บ้านศาลาไก่ฟุบ',
    'บ้านลำโชค',
    'บ้านป่าคา',
    'บ้านแม่ท่าแพ',
    'บ้านห้วยหยวก',
    'บ้านแก่ง',
    'บ้านศรีสวรรค์',
    'บ้านปากคะยาง',
    'บ้านตึก',
    'วัดภูนก',
    'บ้านนาต้นจั่น',
    'บ้านหมอนสูง',
    'บ้านดงย่าปา',
    'บ้านแม่คุ',
    'บ้านห้วยตม',
    'บ้านป่างิ้ว(ราษฎร์บำรุง)',
    'บ้านแม่ราก',
    'บ้านทุ่งพล้อ',
    'บ้านดอยไก่เขี่ย',
    'วัดวังค่า',
    'บ้านห้วยสักตะเพียนทอง',
    'บ้านแม่สำ',
    'บ้านท่าโพธิ์',
    'บ้านดอนระเบียง',
    'แม่สานสามัคคี',
    'บ้านปากสาน',
    'บ้านสันหีบ',
    'บ้านแม่สาน',
    'บ้านแม่ฮู้',
    'บ้านปางสา',
    'บ้านโป่งตีนตั่ง',
    'บ้านสุเม่น',
    'บ้านปากสิน(ต้นประชานุเคราะห์)',
    'บ้านแม่เทิน',
    'บ้านร้องตลาด(ประชานุเคราะห์)',
    'บ้านห้วยโป้',
    'บ้านสะท้อ',
    'บ้านผาเวียง',
    'ไชยะวิทยา',
    'บ้านป่ากล้วย',
    'หมอนสูงประชาสรรค์',
    'บ้านวังยายมาก',
    'บ้านท่าชัย',
    'บ้านเมืองเก่า',
    'วัดตลิ่งชัน',
    'หนองช้าง',
    'บ้านพระปรางค์',
    'วัดโบราณหลวง',
    'วัดแสนตอ',
    'บ้านสารจิตร',
    'บ้านดงยาง',
    'บ้านเกาะน้อย',
    'บ้านป่ายาง',
    'หนองอ้อบำเพ็ญ',
    'บ้านหนองอ้อ(ราษฏร์สามัคคี)',
    'วัดเกาะน้อย',
    'บ้านใหม่(ราษฎร์สามัคคี)',
    'หาดเสี้ยววิทยา',
    'อนุบาลศรีสัชนาลัย(บ้านหาดสูง)',
    'บ้านหนองกระดี่(บางประชาสรรค์)',
    'บ้านวงฆ้อง',
    'บ้านหนองยาว',
    'บ้านเกาะตาเลี้ยง(ทองดีประชานุกูล)',
    'วัดโพธาราม(บุญมากพิทยาคาร)',
    'บ้านคลองตาล(กระจ่างจินดา)',
    'บ้านเตว็ดนอก(สร้อยสนประชาสรรค์)',
    'บ้านเตว็ดกลาง',
    'วัดศรีสังวร',
    'บ้านเกาะวงษ์เกียรติ์',
    'บ้านทับผึ้ง',
    'มิตรสัมพันธ์(เพียวอนุสรณ์)',
    'บ้านสันติสุข',
    'บ้านเขาดินไพรวัน',
    'บัานเขาดินไพรวัน(สาขาโซกเปือย)',
    'บ้านวังตามน(น้อยประชาสรรค์)',
    'บ้านนาขุนไกร',
    'บ้านวังพิกุล',
    'บ้านวังสมบูรณ์',
    'บ้านซ่าน',
    'บ้านปากคลองแดน',
    'บ้านนา',
    'บ้านไร่(สำนักงานสลากกินแบ่งสงเคราะห์155)',
    'บ้านดอนจันทร์',
    'บ้านบุ่งสัก',
    'บ้านราวต้นจันทน์',
    'บ้านวังไฟไหม้',
    'บ้านท่ามักกะสัง',
    'บ้านวังทอง',
    'วัดวังทอง',
    'บ้านวังลึก(ยุวนาฎชนูทิศ)',
    'บ้านโคกกะทือ',
    'วัดวังใหญ่',
    'วัดเตว็ดใน',
    'ประชาอุทิศ',
    'บ้านสันติพิทยาคาร',
    'บ้านหนองแหน',
    'วัดเกาะ',
    'เทวัญอำนวยวิทย์',
    'อนุบาลศรีสำโรง',
    'วัดบ้านกรุ',
    'วัดคลองโป่ง(ธรรมภาณบำรุง)',
    'วัดคลองกระจง',
    'วัดกรงทอง',
    'บ้านโป่งมะขาม',
    'บ้านไผ่ตะล่อม',
    'ชุมชนบ้านคลองยาง',
    'บ้านวังแร่',
    'บ้านคลองวังทอง',
    'บ้านท่าทอง',
    'วัดหนองชุมแสง',
    'บ้านหนองป่าตอ',
    'บ้านถนนพระร่วง',
    'บ้านดงไทยวิทยา',
    'บ้านโบสถ์โพธิ์',
    'บ้านเขาทอง',
    'มิตรภาพที่ 38(บ้านหนองเรียง)',
    'บ้านบ่อแปดร้อย',
    'บ้านป่ามะม่วง',
    'บ้านป่าเลา',
    'วัดปากน้ำ',
    'บ้านไม้งาม',
    'สามัคคีวิทยา',
    'ไทยรัฐวิทยา 30 (ศรีสังวร)',
    'บ้านป่ากุมเกาะ',
    'วัดคุ้งยาง',
    'บ้านใหม่โพธิ์งาม',
    'บ้านนาพง',
    'บ้านขอนซุง',
    'บ้านคลองแห้ง',
    'วัดปากคลองช้าง',
    'บ้านวัดโบสถ์',
    'วัดท่าเกษม',
    'วัดหนองโว้ง(อรรถกิจวิทยาคาร)',
    'วัดท่าทอง',
    'วัดท่าช้าง',
    'วัดไทรย้อย',
    'อนุบาลสวรรคโลก(คุ้งวารีวิทยา)',
    'วัดไผ่ล้อม',
    'วัดอัมพวนาราม',
    'บ้านจันทโรภาส',
    'บ้านหนองกลับ',
    'วัดป่าถ่อน',
    'โรตารี่สวรรคโลก 1',
    'ศีกษาเกษตรศิลป์',
    'วัดศิริบูรณาราม',
    'บ้านทุ่งมหาชัย',
    'บ้านหนองแหน',
    'บ้านดงจันทน์',
    'บ้านบึงงาม',
    'บ้านบึงสวย',
    'บ้านน้ำขุม',
    'วัดบ้านคลอง',
    'อนุบาลศรีนคร(ไทยธัญญานุกูล)',
    'วัดบ้านเหมือง',
    'บ้านตาลพร้า',
    'บ้านหนองบัว',
    'บ้านกลางดง',
    'บ้านหัวฝาย',
    'บ้านโป่งฝาง',
    'เชิงผา',
    'บ้านแม่บ่อทอง',
    'บ้านแม่ทุเลาใน',
    'บ้านหนองผักบุ้ง',
    'บ้านบึงบอน',
    'บ้านห้วยเจริญ',
    'บ้านไทยชนะศึก',
    'บ้านสามหลัง(ประชารังสรรค์)',
    'ศรีเสลี่ยมวิทยา',
    'บ้านธารน้ำทิพย์',
    'บ้านวังธาร',
    'บ้านโซกม่วง',
    'อนุบาลทุ่งเสลี่ยม(บ้านเหมืองนา)',
    'บ้านท่าชุม(ประชาอุทิศวิทยาคาร)',
    'บ้านแม่ทุเลา',
    'บ้านคลองสำราญ',
    'บ้านฝั่งหมิ่น',
    'บ้านธารชะอม',
    'บ้านหนองหมื่นชัย',
    'บ้านกมลราษฎร์',
    'บ้านหนองรังสิต',
    'บ้านแสงสว่าง',
    'บ้านท่าวิเศษ',
    'บ้านลานตาเมือง',
    'เมืองเชลียง',
    'ศรีสำโรงชนูปถัมภ์',
    'สวรรค์อนันต์วิทยา',
    'หนองปลาหมอวิทยาคม',
    'บ้านไร่พิทยาคม',
    'วังทองวิทยา',
    'หนองกลับวิทยาคม',
    'บ้านแก่งวิทยา',
    'เมืองด้งวิทยา',
    'ขุนไกรพิทยาคม',
    'สวรรค์อนันต์วิทยา 2',
    'ท่าชัยวิทยา',
    'ศรีนคร',
    'ทุ่งเสลี่ยมชนูปถัมภ์',
    'ชัยมงคลพิทยา',
    'วัดงิ้วงาม',
    'วัดจอมทอง',
    'วัดท่าตะเคียน',
    'บ้านเต็งสำนัก',
    'บ้านเขาฟ้า',
    'บ้านสะอัก(ประชาอุปถัมภ์)',
    'วัดปากห้วย',
    'ชุมชนที่ 14 วัดดอนทอง',
    'บ้านหนองกวางลี้(ประสิทธิ์-ประช',
    'บ้านร้องยุ้งข้าว',
    'บ้านน้ำดำ',
    'บ้านหินลาด',
    'วัดบ้านไร่',
    'วัดจุฬามณี',
    'วัดสว่างอารมณ์',
    'บ้านหนองหัวยาง',
    'วัดศรีรัตนาราม(รัตนราษฎร์สงเคราะห์)',
    'ชุมชน 1 วัดสะกัดน้ำมัน',
    'วัดคุ้งวารี(อำพันอุปถัมภ์)',
    'วัดยางเอน(ประชานุเคราะห์)',
    'จ่าการบุญ',
    'วัดจันทร์ตะวันตก มิตรภาพที่ 73',
    'วัดอรัญญิก',
    'วัดศรีวิสุทธาราม(วิไลราษฎร์อุปถัมภ์)',
    'วัดจันทร์ตะวันออก',
    'นิคมบ้านกร่าง(สามัคคีสงเคราะห)',
    'บ้านแม่ระหัน',
    'บ้านกร่าง(พระขาวชัยสิทธิ์)',
    'วัดธรรมเกษตร(เกษตรานุเคราะห์)',
    'วัดแหลมโพธิ์',
    'วัดยาง(มีมานะวิทยา)',
    'บ้านป่า(บิลอาสาประชาสรรค์)',
    'บ้านดงวิทยา',
    'บ้านบึงกระดาน(เมธาประชานุกูล)',
    'วัดบึงพระ(เหรียญ จั่น อนุสรณ์)',
    'วัดพิกุลทอง',
    'วัดใหม่ราษฎร์ศรัทธาราม',
    'วัดไผ่ค่อม(แพรประชาอุปถัมภ์)',
    'บ้านตูม',
    'วัดแสงดาว(เทพประชานุสรณ์)',
    'ชุมชนที่ 11 วัดสุวรรณประดิษฐ์',
    'บ้านพลายชุมพล',
    'บ้านแหลมซ่าน',
    'บ้านหัววังกร่าง',
    'บ้านทองหลาง(พุ่มประชานุกูล)',
    'ไทยรัฐวิทยา 8(บ้านมะขามสูง)',
    'บ้านหนองหญ้า',
    'วัดปากพิงตะวันออก',
    'วัดปากพิงตะวันตก',
    'บ้านวังยาง',
    'วัดบางทราย(หลวงพิณพลราษฎร์ จู',
    'วัดไผ่หลงราษฎร์เจริญ',
    'วัดเสาหิน',
    'วัดบ้านใหม่',
    'วัดท่าโรงตะวันออก',
    'วัดท่าโรงตะวันตก',
    'วัดอินทรีย์',
    'วัดเนินมะคึก',
    'วัดกรมธรรม์',
    'วัดศรีวนาราม',
    'วัดสมอแข(พิณพลราษฎร์บำรุง)',
    'อนุบาลพิษณุโลก',
    'วัดโพธิญาณ(อ.ท.ผ.อุปถัมภ์)',
    'วัดมหาวนาราม',
    'วัดตาปะขาวหาย',
    'วัดสระโคล่',
    'สะพานที่ 3',
    'วัดคุยม่วง',
    'วัดหนองขานาง',
    'วัดคุยขวาง',
    'วัดบ้านดง',
    'บ้านเรียงกระดก',
    'นิคมบางระกำ 1(รุ้งวิไล)',
    'บ้านหนองแพงพวย',
    'วัดพรหมเกษร',
    'บ้านตะแบกงาม',
    'วัดวังแร่',
    'วัดห้วงกระได',
    'วัดหนองอ้อ',
    'วัดแตน',
    'วัดหนองพะยอม',
    'บ้านห้วยชัน',
    'บ้านบางแก้ว',
    'บ้านย่านใหญ่',
    'วัดโป่งหม้อข้าว',
    'บ้านนาชักหวาย',
    'บ้านท่านางงาม',
    'บ้านกรุงกรัก',
    'นิคมบางระกำ 6(ท่ามะเกลือ)',
    'บ้านคลองน้ำเย็น',
    'รัฐราษฎร์สงเคราะห์',
    'นิคมบางระกำ 4(บางระกำชนูปถัมภ์)',
    'บ้านใหม่เจริญธรรม',
    'นิคมบางระกำ 2(ราษฎร์บำรุง)',
    'บ้านหนองนากวางอั้น',
    'วัดดอนอภัย',
    'บ้านดงยาง',
    'วัดท่าโก',
    'วัดวังเป็ด',
    'วัดแหลมเจดีย์',
    'บ้านคุยยาง',
    'วัดยางแขวนอู่',
    'บ่อวิทยบางระกำ',
    'คลองวัดไร่',
    'บางระกำ',
    'บ้านตะโม่ประชาสรรค์',
    'วัดโพธิ์ทองเจริญผล',
    'บ้านวังดาน',
    'บ้านคุยมะตูม',
    'บ้านบัวจันทร์',
    'บ้านประดาประชาราษฎร์',
    'บ้านคลองเตย',
    'วัดบึงกอก',
    'วัดปรือกระเทียม',
    'บ้านเสวยซุง',
    'บ้านปลักแรด',
    'วัดหล่ายโพธิ์',
    'บ้านหนองแขม',
    'วัดทุ่งชา',
    'วัดดงโคกขาม',
    'บ้านพันเสา',
    'ประดู่มะค่าประชาสรรค์',
    'บ้านหล่ายขานาง',
    'วัดโพธิ์งาม',
    'วัดกรับพวง',
    'วัดกระทุ่มยอดน้ำ',
    'วัดวังอิทก',
    'วัดกลางสุริยวงศ์',
    'นิคมบางระกำ 8(หนองปลวก)',
    'วัดหนองนา',
    'บ้านหนองกรับ',
    'บ้านหนองตะเคียน',
    'บ้านหนองกุลา',
    'วัดบึงบอน',
    'บ้านหนองไผ่',
    'บ้านท่าไม้งาม',
    'บ้านใหม่คลองเจริญ',
    'พิษณุโลกพิทยาคม',
    'เฉลิมขวัญสตรี',
    'จ่านกร้อง',
    'เตรียมอุดมศึกษาภาคเหนือ พิษณุโลก',
    'วังน้ำคู้ศึกษา',
    'ท่าทองพิทยาคม',
    'ดอนทองวิทยา',
    'บ้านกร่างวิทยาคม',
    'พุทธชินราชพิทยา',
    'จุฬาภรณราชวิทยาลัย พิษณุโลก',
    'บางระกำวิทยศึกษา',
    'ชุมแสงสงคราม(อุดรคณารักษ์อุปถัมภ์)',
    'ประชาสงเคราะห์วิทยา',
    'วัดย่านยาว',
    'ราษฎร์เจริญ',
    'ราษฎร์ดำริ',
    'ชุมชน 12 ท่าตาลประชาสฤษฏ์',
    'บ้านวังสาร',
    'บ้านทุ่งน้อย',
    'บ้านบุใหญ่',
    'ประชาสามัคคี',
    'วัดราษฎร์สโมสร',
    'วัดกรุงศรีเจริญ',
    'วัดบึงลำ',
    'วัดแหลมพระธาตุ',
    'ชุมชน 3บ้านเนินกุ่ม(ประชานุกูล)',
    'บ้านท่ายาง',
    'บ้านสระเศรษฐี',
    'บ้านใหม่ไพรมณี มิตรภาพ 118',
    'บ้านหนองกลด',
    'วัดดงหมี(ธรรมรสอุปถัมภ์)',
    'วัดบางกระทุ่ม',
    'ศึกษาลัย',
    'บ้านแม่เทียบ',
    'วัดไพรสุวรรณ',
    'วัดสิริสุทธาวาส',
    'วัดใหม่ราษฎร์ทรงธรรม',
    'วัดโคกสลุด',
    'ประชาสรรค์วิทยา',
    'อภัยสุพรรณภูมิ',
    'บ้านยางโทน',
    'วัดท่านา',
    'วัดท่ามะขาม',
    'บ้านหนองสระพัง',
    'วัดตายม',
    'รัฐราษฎร์บำรุง',
    'วัดสนามคลีตะวันตก',
    'วัดสนามคลีตะวันออก',
    'องค์การอุตสาหกรรมป่าไม้ 2',
    'บ้านโป่งปะ',
    'บ้านเขาน้อย',
    'บ้านปากยาง',
    'บ้านม่วงหอม(สนง.สลากกินแบ่งสงเคราะห์43)',
    'บ้านทุ่งเอี้ยงสามัคคี',
    'ห้วยพลู',
    'บ้านแก่งกุลาสามัคคี',
    'ราษฎร์อุปถัมภ์',
    'วัดไชยนาม',
    'บ้านบึงพร้าว',
    'บ้านดินทอง',
    'บ้านซำเตย(ประกอบนงนุชอุปถัมภ์)',
    'ชุมชน 2 บ้านกกไม้แดง',
    'บ้านเนินสะอาด',
    'วัดท่าหมื่นราม',
    'สามัคคีประชาราษฎร์',
    'บ้านแสงสว่าง',
    'บ้านดงน้อยปลั่งทองคำวิทยา',
    'บ้านหินประกาย',
    'บ้านใหม่ชัยเจริญ',
    'บ้านซำหวาย',
    'บ้านนาพราน',
    'บ้านใหม่พนมทอง',
    'บ้านป่าขนุน',
    'บ้านน้ำริน',
    'บ้านหนองปรือ',
    'บ้านกลาง',
    'บ้านน้ำยาง',
    'ชุมชน 15 บ้านเนินสว่าง',
    'บ้านคลองดู่',
    'บ้านหนองงา',
    'สามัคคีธรรม',
    'วัดสุพรรณพนมทอง',
    'วัดพันชาลี',
    'บ้านเนินไม้แดง',
    'บ้านเนินทองราษฎร์พัฒนา',
    'บ้านวังไม้ตอก',
    'วัดหนองตาเรือง',
    'บ้านแถววังน้ำใสสามัคคี (คุรุราษฎร์บำรุง)',
    'บ้านถ้ำเต่า',
    'บ้านแม่ระกา',
    'บ้านเข็ก',
    'บ้านดงพลวง',
    'วัดดงข่อย',
    'บึงราชนก',
    'คุรุประชาชนูทิศ(วันครู 2504)',
    'บ้านเขาสมอแคลง(ริ้วศิริวัฒน์อุปถัมภ์)',
    'วัดบางสะพานประพันธ์ชนานุสรณ์',
    'บ้านวังพรม',
    'พิณพลราษฎร์ ตั้งตรงจิตร 12',
    'บ้านวังตาดราษฎร์อุทิศ',
    'บ้านแก่งจูงนาง',
    'ป่าไม้อุทิศ 6(ค่ายลูกเสือแก่งซอง)',
    'บ้านวังนกแอ่น',
    'บ้านวังดินสอ',
    'บ้านป่ามะกรูด',
    'บ้านน้ำพรม',
    'บ้านท่าข้าม',
    'บ้านแก่งสฤษดิ์เสนาอุปถัมภ์',
    'บ้านซำตะเคียน(ลส.ชบ.สมุทรปราการอุปถัมภ์)',
    'บ้านตอเรือ',
    'บ้านไผ่ใหญ่(ดำรงค์อุทิศ)',
    'วัดทางลัด',
    'วัดวังพิกุล',
    'บ้านคลองเป็ด',
    'บ้านคลองเมือง',
    'บ้านวังประดู่',
    'บ้านสะเดา',
    'บ้านหนองพระ',
    'บ้านเจริญผล',
    'บ้านคลองแร่',
    'บ้านซำรัง',
    'บ้านชมภู',
    'บ้านน้ำปาด',
    'วัดปลวกง่าม',
    'รักไทยร่มเกล้าอุปถัมภ์',
    'ชุมชนบ้านไทรย้อย',
    'บ้านหนองขมิ้น',
    'บ้านเขาดิน',
    'บ้านพุกระโดน',
    'บ้านโคกวังสาร',
    'บ้านผารังหมี',
    'บ้านเนินมะปราง',
    'เนินดินราษฎร์อุทิศ',
    'บ้านใหม่ทองประเสริฐ',
    'บ้านหนองขอนประชาสรรค์',
    'บ้านหนองไม้ยางดำ',
    'บ้านพัฒนาดงน้อย',
    'บ้านผาท่าพล',
    'บ้านน้อยซุ้มขี้เหล็ก',
    'บ้านลำภาศ',
    'หัวเขาราษฎร์บำรุง',
    'บ้านใหม่สามัคคี',
    'บ้านห้วยบ่อทอง',
    'วัดบ้านมุง',
    'บ้านวังโพรง',
    'บ้านวังขวัญ',
    'บ้านเขาเขียว',
    'บ้านทุ่งยาว',
    'บ้านหนองดู่',
    'บ้านวังดินเหนียว',
    'บ้านวังยาง (ราษฎร์สามัคคี)',
    'บ้านไทรดงยั้ง',
    'เนินกุ่มวิทยา',
    'บางกระทุ่มพิทยาคม',
    'วังทองพิทยาคม',
    'สฤษดิ์เสนา',
    'หนองพระพิทยา',
    'ทรัพย์ไพรวัลย์วิทยาคม',
    'บ้านกลางพิทยาคม',
    'เนินสะอาดวิทยาคม',
    'น้ำรินพิทยาคม',
    'วังพิกุลวิทยศึกษา',
    'เนินมะปรางศึกษาวิทยา',
    'วังโพรงพิทยาคม',
    'ไทรย้อยพิทยาคม',
    'บ้านนาเมือง',
    'บ้านนาฟองแดง',
    'บ้านนาขุมคัน',
    'บ้านนาทุ่งใหญ่',
    'นครไทยวิทยาคม',
    'บ้านหนองน้ำสร้าง',
    'บ้านหนองลาน',
    'ประชาสงเคราะห์พิทยา',
    'ภูขัดรวมไทยพัฒนา',
    'กัลยาณิวัฒนา 1',
    'บ้านนาไก่เขี่ย',
    'บ้านน้ำลอม',
    'บ้านนาคล้าย',
    'บ้านบุ่งผลำ',
    'บ้านน้ำกุ่ม',
    'บ้านนาแฝก',
    'บ้านท่าหินลาด',
    'ชุมชน 8 ราษฎร์อุทิศพิทยา',
    'บ้านหนองแห้ว',
    'บ้านโป่งกะเฌอ',
    'บ้านห้วยตีนตั่ง',
    'บ้านห้วยน้ำไซ (สาขาร่องกล้าวิทยา)',
    'บ้านห้วยน้ำไซ',
    'ชุมชน 17 บ้านนาตาดี',
    'สหภาพแรงงานการไฟฟ้านครหลวง',
    'บ้านป่ารวก',
    'บ้านน้ำเลา',
    'บ้านบ่อโพธิ์',
    'บ้านน้ำคลาด',
    'บ้านหนองกระดาษ',
    'บ้านป่าซ่าน',
    'บ้านพร้าว',
    'บ้านห้วยทรายทอง',
    'บ้านแยง',
    'บ้านเข็กใหญ่',
    'เกษตรสุขราษฎร์บำรุง',
    'บ้านถ้ำพริก',
    'บ้านหนองหิน',
    'บ้านแก่งทุ่ง',
    'บ้านยางโกลน',
    'บ้านบุ่งตารอด',
    'บ้านบุ่งปลาฝา',
    'บ้านกกม่วง',
    'บ้านน้ำพริก',
    'บางยางพัฒนา',
    'บ้านน้ำตาก',
    'บ้านหลังเขา',
    'บ้านหนองกะท้าว',
    'บ้านนาโพธิ์นาจาน',
    'บ้านแก่งหว้าแก่งไฮ',
    'บ้านนาหนอง',
    'วิทยสัมพันธ์',
    'บ้านป่าคาย',
    'บ้านห้วยเฮี้ย',
    'บ้านห้วยกอกพัฒนา',
    'ศิริราษฎร์พัฒนา',
    'บ้านลาดคื้อ',
    'บ้านห้วยทรายเหนือ',
    'บ้านชาติตระการ',
    'บ้านนาเปอะ',
    'บ้านปากรอง',
    'บ้านนาจาน',
    'บ้านท่าสะแก',
    'บ้านห้วยท้องฟาน',
    'บ้านขอนสองสลึง',
    'บ้านนาม่วง',
    'บ้านน้ำจวง',
    'บ้านร่มเกล้า',
    'บ้านนาตอน',
    'บ้านขุนน้ำคับ',
    'บ้านขวดน้ำมัน',
    'วัดบ่อภาค',
    'ชุมชนวัดบ้านดง',
    'บ้านน้ำทองน้อย',
    'บ้านห้วยน้ำปลา',
    'บ้านนาหล่ม',
    'บ้านเนินทอง',
    'บ้านเนินสุวรรณ',
    'บ้านห้วยเหิน',
    'บ้านนาล้อม',
    'บ้านน้ำภาคน้อย',
    'บ้านป่าแดง',
    'บ้านสวนเมี่ยง',
    'บ้านห้วยช้างแทง',
    'บ้านหนองขาหย่าง',
    'บ้านห้วยหมากหล่ำ',
    'บ้านโคกใหญ่',
    'บ้านแก่งบัวคำ',
    'วัดท้องโพลง',
    'วัดทุ่งตาเปรี้ยว',
    'วัดสมอสุวรรณาราม',
    'วัดวังงิ้วงาม',
    'บ้านคลองตกวิทยา',
    'วัดเขาน้อย',
    'บ้านหนองไผ่',
    'ผดุงวิทยา',
    'หนองสะแกประชานุกูล',
    'วัดเซิงหวาย',
    'วัดหนองมะคัง',
    'วัดทับยายเชียง',
    'ทับยายเชียงวิทยา',
    'วัดนาขุม',
    'วัดเมมสุวรรณาราม',
    'วัดท่าช้าง',
    'วัดหนองหม้อแกง',
    'ราษฎร์สามัคคี',
    'ประชาสามัคคี',
    'ราษฎร์บำรุง',
    'พิรามอุทิศ',
    'วัดห้วยดั้ง',
    'วัดสะพานหิน(วงศ์วานประชาสรรค์)',
    'ศึกษากุลบุตร',
    'วัดหางไหล',
    'วัดสนามไชย',
    'มะต้องประชาสรรค์',
    'บ้านป่าสัก (ทศพลอนุสรณ์)',
    'บ้านมะต้อง',
    'วัดมะตูม',
    'ชุมชนดิฎฐอำรุง',
    'วัดวังมะด่าน',
    'วัดธรรมาราม',
    'ราษฎร์ศรัทธาวิทยา',
    'วัดย่านยาวประชานุกูล',
    'วัดท่าสี่ร้อย',
    'วัดวงฆ้อง',
    'วัดหัวเขาสมอคร้า',
    'วัดวังไม้แก่น',
    'วัดวังวน',
    'บ้านคลองทำเนียบ',
    'บ้านหนองห้าง',
    'บ้านบึงธรรมโรง',
    'ศรีภิรมย์พิทยา',
    'วัดคลองมะเกลือ',
    'บ้านบุ่งวิทยา',
    'บ้านบึงวิทยา',
    'วัดหาดใหญ่',
    'วัดคลองตาล(ศรีรัตนประชานุกูล)',
    'ชุมชนวัดย่านขาด(ประชาสงเคราะห์)',
    'บ้านหาดใหญ่',
    'ประชาอุปถัมภ์',
    'วัดหอกลอง',
    'วัดกระบังมังคลาราม (พิศาลประชาสรรค์)',
    'ห้วยปลาไหล',
    'วัดคันโช้ง',
    'บ้านน้ำโจน',
    'บ้านแก่งคันนา',
    'บ้านห้วยเจียง',
    'บ้านหนองกระบาก',
    'วัดทองแท้(จ่างอนุกูลพิทยา)',
    'วัดเหล่าขวัญ(จ่างอนุเคราะห์)',
    'บ้านท่ากระดุน',
    'บ้านหนองมะคัง(จ่างอนุสรณ์)',
    'วัดเสนาสน์',
    'บ้านหนองปลิง',
    'เขาไร่ศรีราชา',
    'ชุมชนบ้านท่างาม',
    'วัดนาขาม',
    'บ้านแก่งเจ็ดแคว',
    'วัดน้ำคบ',
    'แก่งบ้านยางป่าคาย',
    'บ้านน้ำหัก',
    'บ้านคลองช้าง',
    'บ้านเนินมะคึด',
    'บ้านวัดโบสถ์(บำรุงวุฒิวิทยา)',
    'วัดโบสถ์',
    'บ้านชาน',
    'วัดบ้านน้อย',
    'วัดหินลาด',
    'บ้านท่าหนองปากพาน',
    'นครไทย',
    'นาบัววิทยา',
    'นครชุมพิทยา รัชมังคลาภิเษก',
    'นครบางยางพิทยาคม',
    'บ่อโพธิ์วิทยา',
    'ยางโกลนวิทยา',
    'ศรีอินทราทิตย์พิทยาคม',
    'บางกลางท่าวพิทยาคม',
    'ราชประชานุเคราะห์ 23',
    'ชาติตระการวิทยา',
    'สวนเมี่ยงวิทยา',
    'พรหมพิรามวิทยา',
    'วังมะด่านพิทยาคม',
    'ดงประคำพิทยาคม',
    'วัดโบสถ์ศึกษา',
    'คันโช้งพิทยาคม',
    'ชุมชนวัดคลองคะเชนทร์',
    'วัดดงชะพลู',
    'วัดมูลเหล็ก',
    'วัดคลองโนน',
    'บ้านสวนแตง',
    'วัดฆะมัง',
    'วัดดงป่าคำ สาขาบ้านเนินถ่อน',
    'วัดสามขา',
    'วัดดงป่าคำ',
    'วัดดงกลาง',
    'วัดบ้านวังไร่',
    'วัดวังหูทิพย์',
    'วัดเขารูปช้าง',
    'บ้านบึงตะโกน (ธนาคารกรุงเทพ 1)',
    'บึงสีไฟ',
    'วัดวังมะเดื่อ',
    'บ้านท่าฬ่อ(ครุฑวิทยากรณ์)',
    'อนุบาลพิจิตร',
    'อนุบาลเมือง(ท่าหลวงสงเคราะห์)',
    'ชุมชนบ้านวังกลม',
    'วัดบ้านบุ่ง',
    'บ้านไดชุมแสง',
    'วัดหนองไผ่',
    'วัดราชช้างขวัญ',
    'วัดยางคอยเกลือ',
    'บ้านคลองอุดม',
    'บ้านหนองถ้ำ',
    'วัดเนินสมอ(พินิจวิทยา)',
    'บ้านท่ามะไฟ',
    'วัดป่ามะคาบ(สามัคคีพิทยาคาร)',
    'บ้านสระสาลี',
    'วัดดานคุรุราษฎร์อุทิศ',
    'บ้านเมืองเก่า',
    'วัดเขื่อน"พิจิตรอนุสรณ์"',
    'วัดเขื่อน',
    'วัดขนุน',
    'วัดท่าข่อย',
    'บ้านสนามคลี',
    'วัดหาดมูลกระบือ',
    'บ้านวังกระดี่ทอง',
    'วัดหงษ์',
    'วัดโรงช้าง(ธรรมกิตต์วิทยา)',
    'บ้านวังทับยา',
    'บ้านสายคำโห้',
    'บ้านน้อย',
    'บ้านดงหวาย',
    'บ้านดงสว่าง',
    'บ้านเนินยาว',
    'วัดน้ำโจนเหนือ',
    'บ้านหัวดง',
    'วัดเขาพระ',
    'วัดลำชะล่า',
    'วัดหนองนาดำ',
    'อนุบาลวังทรายพูน',
    'บ้านวังแสง',
    'ชุมชนบ้านเนินหัวโล้หนองยาง',
    'บ้านทุ่งโม่ง',
    'วัดตากแดด',
    'บ้านบุ่งมะกรูด-วังพลับ',
    'วัดหนองจิกเภา',
    'วัดหนองปลาหมอ(มิตรภาพ 215)',
    'วัดหนองปล้อง',
    'วัดใหม่สามัคคีธรรม',
    'บ้านพนมกาว',
    'บ้านใหม่เนินสวรรค์',
    'บ้านหนองปลาไหล สาขาบ้านวังคล้าย',
    'บ้านท่ากระดาน',
    'บ้านหนองปลาไหล',
    'บ้านคลองสะแกป่าหวาย',
    'บ้านหนองปลาไหล สาขาบ้านห้วยตัดไม้วังเดื่อ',
    'บ้านยางสามต้น',
    'บ้านหนองพระ',
    'บ้านยางสามต้น สาขาบ้านดงคันแทว',
    'วัดเขาพนมพา',
    'ชุมชนวัดหนองลากฆ้อน',
    'บ้านดงพลับ',
    'บ้านดงเสือเหลือง',
    'บ้านหนองขานาง',
    'วัดบ้านนา',
    'วัดเนินพยอม',
    'บ้านบ่อบิ้งเกลือ',
    'บ้านหนองพง',
    'บ้านห้วยคำตาล',
    'อนุบาลโพธิ์ประทับช้าง(ทุ่งใหญ่)',
    'บ้านทุ่งประพาส',
    'บ้านห้วยน้อย',
    'บ้านเนินขวาง',
    'บ้านเนินพลวง',
    'บ้านหนองสะแก',
    'บ้านหนองขำ',
    'บ้านมาบมะไฟ',
    'บ้านหนองคล้า',
    'วัดใหม่แสงมรกต',
    'วัดบ้านไผ่ท่าโพใต้',
    'วัดบ้านลำนัง',
    'วัดบ้านประดาทอง',
    'วัดบ้านไผ่ท่าโพเหนือ',
    'วัดไผ่รอบ',
    'วัดหนองหลวง',
    'วัดเกาะแก้ว',
    'วัดสระประทุม(มิตรภาพที่ 54)',
    'บ้านหนองหัวปลวก',
    'บ้านหนองไม้แดง',
    'บ้านใดโพธิ์',
    'วัดท่าบัวทอง',
    'บ้านโพธิ์ประทับช้าง',
    'บ้านบึงโพธิ์',
    'วัดท่าพุดซา',
    'บ้านวังกระโดน',
    'ชุมชนวัดวังจิก',
    'วัดราษฎร์บูรณะ',
    'บ้านวังเทโพ',
    'บ้านวังโป่ง',
    'ชุมชนบ้านกำแพงดิน',
    'บ้านท่าแห',
    'วัดวังปลาทู',
    'วัดศรีศรัทธาราม',
    'วัดศรีศรัทธาราม สาขาบ้านหนองบัว',
    'วัดเนินปอ',
    'บ้านมาบแฟบ',
    'บ้านยางห้าหลุม',
    'สระยายชีมิตรภาพที่ 79',
    'บ้านเนินพยอม',
    'บ้านเนินพลวง',
    'บ้านปลายห้วย',
    'บ้านเนินยุ้ง',
    'วัดบ้านใหม่',
    'บ้านรังนก(ประชาพิทักษ์)',
    'วัดจระเข้ผอม',
    'บ้านหนองน้ำเขียว',
    'บ้านโนนไผ่ขุย',
    'วัดบ้านใหม่ สาขาบ้านกระทุ่มน้ำเดือด',
    'บ้านวังลูกช้าง',
    'วัดวังแดง สาขาบ้านสามง่าม',
    'วัดบ้านไร่',
    'วัดวังแดง สาขาวัดสามง่าม(วันครู2501)',
    'วัดวังแดง',
    'อนุบาลสามง่าม',
    'วัดรายชะโด',
    'วัดใหม่ราษฎร์บำรุง',
    'บ้านหนองโสน',
    'บ้านป่าแซง',
    'บ้านมาบกระเปา',
    'บ้านสุขสำราญ',
    'บ้านไผ่ใหญ่',
    'บ้านบึงเฒ่า',
    'บ้านหนองจั่ว',
    'บ้านหนองตะเคียน',
    'บ้านหนองสาหร่าย',
    'บ้านท่าเยี่ยม',
    'บ้านวังอ้อ',
    'บ้านวังทับไทร',
    'บ้านตลุกหิน',
    'บ้านหนองสองห้อง',
    'บ้านสายดินแดง',
    'บ้านท่าพิกุล',
    'บ้านทุ่งสำราญ',
    'อนุบาลสากเหล็ก',
    'บ้านถ้ำคะนอง',
    'บ้านปากดง',
    'บ้านหนองต้นพลวง',
    'บ้านคลองทรายราษฎร์อุทิศ',
    'บ้านคลองทรายใหญ่',
    'วัดบ้านนา',
    'บ้านปลวกสูง',
    'วัดโนนสะเดา',
    'วัดกรดงาม',
    'บ้านหนองหูช้าง',
    'บ้านนิคม',
    'บ้านหนองขาว',
    'บ้านยางตะพาย',
    'บ้านมาบฝาง',
    'บ้านบึงบัว',
    'บ้านยางสุขวัฒน์',
    'บ้านโนนทอง',
    'บ้านหนองริ้น',
    'วัดวังตะขบ',
    'ชุมชนบ้านหนองหญ้าปล้อง',
    'บ้านสระบรเพ็ด',
    'บ้านบัวยาง',
    'วัดหนองหลุม',
    'วัดกลางวงศ์มณี',
    'อนุบาลวชิรบารมี (ต้นประดู่)',
    'วัดโนนป่าแดง',
    'บ้านยางนกกระทุง',
    'พิจิตรพิทยาคม',
    'หัวดงรัฐชนูปถัมภ์',
    'วังกรดพิทยา',
    'เมธีพิทยา',
    'สระหลวงพิทยาคม',
    'พิจิตรปัญญานุกูล',
    'วังทรายพูนวิทยา',
    'สรรเพชญอัฏฐมาพิทยาคม',
    'ดงเสือเหลืองพิทยาคม',
    'สามง่ามชนูปถัมภ์',
    'กำแพงดินพิทยาคม',
    'เนินปอรังนกชนูทิศ',
    'หนองโสนพิทยาคม',
    'สากเหล็กวิทยา',
    'วชิรบารมีพิทยาคม',
    'วังโมกข์พิทยาคม',
    'วัดวังไคร้',
    'วัดคลองแขก',
    'วัดคลองคูณ',
    'วัดต้นชุมแสง',
    'วัดหาดแตงโมจันทรประเสริฐ',
    'วัดหนองคล่อ',
    'วัดใหม่สำราญ',
    'ดงตะขบราษฎร์สงเคราะห์',
    'วัดพฤกษะวันโชติการาม',
    'วัดทับปรู',
    'บ้านบึงประดู่',
    'วัดทับหมัน',
    'วัดไทรโรงโขน',
    'วัดคลองข่อย',
    'บ้านไดกระพังปลา',
    'บ้านสามบึงบาตร',
    'บ้านหนองหวาย',
    'วัดวังสำโรง',
    'วัดบึงน้ำกลัด',
    'ชุมชนวังหลุมวิทยาคาร',
    'วัดสัตตวนาราม',
    'วัดเขารวก',
    'บ้านเนินทราย',
    'วัดวังหว้า',
    'วัดท่าปอ',
    'วัดยางคลี',
    'เขาพนมกาวหนองนกยาง',
    'วัดหนองแก',
    'อนุบาลตะพานหิน(หนองพยอม)',
    'วัดป่าแดง',
    'วัดพรหมประสิทธิ์',
    'วัดโพธิ์ลอย',
    'วัดคลองทองหลาง',
    'บ้านห้วงสลิด',
    'วัดเทวประสาท',
    'วัดธงไทยยาราม',
    'บ้านวังกร่าง"ราชานุสรณ์"',
    'บำรุงราษฎร์วิทยาคม',
    'บ้านวังทองราษฎร์สามัคคี',
    'ชุมชนวัดโพธิ์ทอง"ปกาสิตวิทยา"',
    'วัดวังน้ำเต้า',
    'บางไผ่ราษฎร์สงเคราะห์',
    'วัดท่าหอย',
    'พฤกษวนารามวิทยา',
    'วัดโพธิ์แดน',
    'ศรีประสิทธิ์วิทยา',
    'อนุบาลบางมูลนาก"ราษฎร์อุทิศ"',
    'บางมูลนาก(ราษฎร์อุทิศ)',
    'ชุมชนภูมิวิทยา',
    'วัดลำประดาใต้',
    'วัดไดโสน',
    'วัดลำประดากลาง',
    'วัดห้วยเรียงใต้',
    'วังกรดนุกูลศึกษา',
    'วัดห้วยเรียงกลาง',
    'วัดวังตะกู',
    'วัดมะกอกงอ',
    'วัดลำประดาเหนือ',
    'วัดเขาโค้ง',
    'วังสำโรง"ราษฎร์บำรุงวิทย์"',
    'วัดหนองกอไผ่',
    'บ้านไดปลาดุกราษฎร์ศรัทธาทรงธรรม',
    'อนุบาลอำเภอบางมูลนาก(วัดห้วยเขน)',
    'วัดหอไกร',
    'บำรุงราษฎร์วิทยาคม สาขาบ้านคลองแค',
    'บ้านน้ำผึ้ง"วิจิตรวุฒิกรอุปถัมภ์"',
    'ชุมชนทะนงบ้านไร่',
    'บ้านหนองตะแบก',
    'ชุมชนวัดหนองคล้ายางพะเนียด',
    'วัดบ้านท่าขมิ้น',
    'บ้านทุ่งใหญ่',
    'วัดบ้านห้วยยาว',
    'บ้านวังแดง',
    'บ้านเนินแค',
    'วัดบ้านท่านั่ง',
    'บ้านท่าบัว"รัฐประชานุเคราะห์"',
    'วัดหลวง',
    'วัดทับทิม',
    'บ้านปากน้ำ',
    'บ้านเนินโพธิ์',
    'วัดบ้านท้ายน้ำ',
    'บ้านหนองบัว',
    'วัดบ้านท่ามะไฟ',
    'วัดบ้านหนองดง',
    'วัดบ้านพังน้อย',
    'บ้านบางพล้อ-ยางหลวง',
    'บ้านยี่มุ่ย',
    'ชุมชนบ้านทุ่งน้อย"โสภณวิทยาคาร"',
    'บ้านหนองไผ่-มาบชุมแสง',
    'ชุมชนบ้านบางคลาน(เงินอนุสรณ์)',
    'วัดคงคาราม',
    'บ้านน้อย"ปรึกอุทิศ"',
    'บ้านวังตายศ"ราษฎร์สามัคคี"',
    'อนุบาลโพทะเล"รัฐบำรุง"',
    'วัดโพทะเล',
    'วัดบ้านตาล"ศิริรัฐราษฎร์วิทยาคาร"',
    'บ้านคลองตางาว',
    'วัดขวาง',
    'วัดพร้าว',
    'วัดใดวน',
    'บ้านใหม่ราษฎร์ดำรง',
    'ไทยรัฐวิทยา 60(บ้านเขาตะพานนาก)',
    'วัดสัตตศิลาอาสน์',
    'บ้านเขาโล้น',
    'บ้านคีรีเทพนิมิต',
    'วัดลำประดา',
    'วัดเขาทราย',
    'วัดวังหินเพลิง',
    'วัดวังแดง',
    'บ้านหนองจะปราบ',
    'บ้านเขานกยูง',
    'บ้านเขาพระ',
    'วัดหนองน้ำเต้า',
    'วัดชัยศรี',
    'บุรพรัตน์วิทยาคาร',
    'ชุมชนวัดสายดงยาง',
    'อนุบาลทับคล้อ',
    'วัดป่าเรไร',
    'วัดศรีรัตนวราราม',
    'วัดไดอีเผือก',
    'บ้านคลองห้วยหลัว',
    'วัดน้ำเคือง',
    'ท้ายทุ่งราษฎร์วิทยาคาร',
    'วัดบางเบน',
    'วัดวังกระชัน',
    'สี่แยกเขาดิน',
    'วัดเขาส้าน',
    'บ้านเนินม่วง',
    'บ้านหนองจอก',
    'วัดบ้านบางลายเหนือ',
    'บ้านทุ่งพรหมทอง"ราษฎร์สามัคคี"',
    'วัดบ้านบางลายใต้',
    'วัดบ้านคลองข่อย',
    'บ้านหนองแขม',
    'วัดบ้านบึงนาราง"แก้ววิทยาคาร"',
    'บ้านห้วงปลาไหล"สิงหะวิทยา"',
    'บ้านศรีศรัทธามิตรภาพที่ 176',
    'บ้านทุ่งทอง',
    'วัดบ้านบึงลี',
    'อนุบาลบึงนาราง (ห้วยแก้ว)',
    'บ้านวังพร้าว',
    'บ้านแหลมรัง"ราษฎร์บำรุง"',
    'บ้านบึงทับจั่น"ทศอุปถัมภ์"',
    'บ้านโป่งวัวแดง',
    'วังก้านเหลือง',
    'วัดใหม่ดงเจริญ',
    'บ้านวังกะทะ',
    'อนุบาลดงเจริญ (วังงิ้ว)',
    'วัดวังเรือน',
    'วัดวังหินแรง',
    'วัดหนองง้าว',
    'สำนักขุนเณร(หลวงพ่อเขียนอุทิศ)',
    'บ้านวังบงค์',
    'บ้านไดลึก"ประชานุสรณ์"',
    'วัดใหม่วังหว้า',
    'บ้านห้วยตาดำ',
    'ห้วยพุกวิทยา',
    'วัดหนองสนวน',
    'บ้านไดรัง(ราษฎร์อุทิศ)',
    'วัดหนองสองห้อง',
    'ตะพานหิน',
    'วังสำโรงวังหว้า',
    'ทุ่งโพธิ์พิทยา',
    'บางมูลนากภูมิวิทยาคม',
    'วังตะกูราษฎร์อุทิศ',
    'โพธิธรรมสุวัฒน์',
    'พิบูลธรรมเวทวิทยา',
    'ท่าเสาพิทยาคม',
    'ห้วยยาวพิทยาคม',
    'เขาทรายทับคล้อพิทยา',
    'วันทีสถิตย์พิทยาคม',
    'บางลายพิทยา"จิตตภาวันอุปถัมภ์"',
    'แหลมรังวิทยาคม',
    'โพธิ์ไทรงามวิทยาคม',
    'วังงิ้ววิทยาคม',
    'บ้านชอนไพร',
    'บ้านคลองสำโรง',
    'บ้านคลองขุด',
    'บ้านสักแห้ง',
    'บ้านดงมูลเหล็ก',
    'บ้านโนนสะอาด',
    'บ้านลำป่าสักมูล',
    'บ้านคลองบง',
    'บ้านโนนตะแบก',
    'บ้านท่ากกตาล',
    'บ้านวังโค้ง',
    'บ้านป่าบง',
    'บ้านตะเบาะ',
    'บ้านห้วยไคร้',
    'บ้านเขาขาด',
    'บ้านอมกง',
    'บ้านโพธิ์งาม',
    'บ้านป่าม่วง',
    'บ้านคลองมะนาว',
    'บ้านท่าพล',
    'บ้านวังซอง',
    'บ้านดง',
    'บ้านโพธิ์ทอง',
    'บ้านอมชี',
    'บ้านนางั่วเหนือ',
    'บ้านคลองสาร',
    'บ้านสำนักหมัน',
    'บ้านนางั่ว',
    'บ้านนา',
    'บ้านเฉลียงลับ',
    'บ้านปากน้ำ',
    'บ้านบง',
    'บ้านชอน',
    'บ้านกกไทร',
    'บ้านนาป่า',
    'เฉลียงลับวิทยา',
    'บ้านนายม',
    'บ้านขมวด',
    'บ้านหัวนา',
    'บ้านถ้ำน้ำบัง',
    'บ้านบุฉนวน',
    'ชุมชนบ้านน้ำร้อน',
    'บ้านทุ่งหินปูน',
    'เมืองเพชรบูรณ์',
    'บ้านสะเดียง',
    'บ้านไทรงาม',
    'บ้านหนองนารี',
    'อนุบาลเพชรบูรณ์',
    'บ้านน้ำเลา',
    'บ้านห้วยผักไล',
    'บ้านโคก',
    'บ้านน้ำอ้อม',
    'บ้านกงกะยาง',
    'บ้านทุ่งแค',
    'บ้านวังจาน',
    'บ้านโตก',
    'บ้านโตกใต้',
    'บ้านพี้',
    'บ้านพนานิคม',
    'บ้านป่าแดง',
    'บ้านพลำ',
    'บ้านป่าเลา',
    'บ้านยางลาด',
    'บ้านวังขอนมิตรภาพที่ 137',
    'บ้านยางกุด',
    'บ้านห้วยไร่',
    'บ้านระวิง',
    'บ้านยางหัวลม',
    'บ้าน กม.2',
    'บ้านยาวี-ห้วยโป่ง',
    'บ้านสามแยกวังชมภู',
    'บ้านคลองห้วยนา',
    'บ้านวังทอง',
    'บ้านซับข่อย',
    'บ้านหลุง',
    'บ้านไร่เหนือ',
    'บ้านไร่',
    'บ้านหนองแหวน',
    'บ้านชัยมงคล',
    'บ้านห้วยสะแก',
    'บ้านห้วยนาค',
    'บ้านเนินสง่า',
    'บ้านบุ่งกกเรียง',
    'บ้านห้วยใหญ่',
    'บ้านสะแกงาม',
    'บ้านน้ำเดื่อ',
    'บ้านโป่งหว้า',
    'บ้านห้วยแหน',
    'บ้านน้ำเดื่อใต้',
    'บ้านศาลาลาย',
    'บ้านหนองแดง',
    'บ้านโป่งตาเบ้า',
    'อนุบาลชนแดน',
    'บ้านคลองห้วยนา',
    'บ้านโคกเจริญ',
    'บ้านโคกยาวสาขาบ้านโคกสาร',
    'บ้านซับเจริญ',
    'บ้านทรัพย์พุทรา',
    'บ้านสามแยกป่ามะม่วง',
    'บ้านทรัพย์พุทราสาขาบ้านคลองกลาง',
    'บ้านดงขุย',
    'บ้านหนองระมาน',
    'บ้านบุ่งคล้า',
    'บ้านดงขุยใต้',
    'บ้านดงขุยใต้สาขาบ้านทุ่งโตนด',
    'บ้าน กม.28',
    'บ้าน กม.28สาขาบ้านดงเสือเหลือง',
    'บ้านเขาสัก',
    'บ้านเนินสว่าง',
    'บ้านเขาน้อย',
    'บ้านโป่งนกแก้ว',
    'บ้านหนองกลอย',
    'บ้านท่าข้าม',
    'บ้านกุฏิพระ',
    'บ้านดงแขวน',
    'บ้านโคกสูง',
    'บ้านเขาบ่อทอง',
    'บ้านตะกุดจั่น',
    'บ้านตะกุดเป้า',
    'บ้านวังปลาช่อน',
    'บ้านกล้วย',
    'บ้านเขาชะโงก',
    'บ้านโคกสำราญ',
    'บ้านวังรวก',
    'บ้านเขาคณฑา',
    'อนุบาลชนแดนสาขาบ้านหนองเล้าไก่',
    'บ้านห้วยงาช้าง',
    'บ้านห้วยตูม',
    'บ้านน้ำพุ',
    'บ้านลาดน้อย',
    'บ้านหนองตาด',
    'บ้านน้ำลัด',
    'บ้านกกจั่น',
    'บ้านโป่งเจ็ดหัว',
    'บ้านผาทอง',
    'บ้านซับสีทอง',
    'บ้านลาดแค',
    'บ้านโคกยาว',
    'บ้านซับม่วง',
    'ธาราคีรี',
    'บ้านโคกหนองจอก',
    'ลูกจันทน์ปิยะอุย',
    'บ้านหนองใหญ่',
    'บ้านถ้ำแก้ว',
    'บ้านดงลาน',
    'สายสมร',
    'บ้านคลองปลาหมอ',
    'บ้านซับขลุง',
    'บ้านวังไทรทอง',
    'บ้านคลองน้ำคัน',
    'บ้านซับเปิบ',
    'บ้านเชิงชาย',
    'ชุมชนบ้านวังกระดาษเงิน',
    'บ้านวังขอน',
    'บ้านวังชะนาง',
    'บ้านดงหลง',
    'บ้านด่านช้าง',
    'อนุบาลวังโป่ง',
    'บ้านไร่ฝาย',
    'บ้านโนนตูม',
    'บ้านทางข้าม',
    'บ้านวังแช่กลอย',
    'บ้านวังศาล',
    'บ้านดงลึก',
    'บ้านวังหิน',
    'บ้านใหม่วังตะเคียน',
    'บ้านวังใหญ่',
    'บ้านวังพลับ',
    'บ้านน้ำอ้อม',
    'บ้านวังหินซอง',
    'เพชรพิทยาคม',
    'วิทยานุกูลนารี',
    'วังชมภูวิทยาคม',
    'สวนกุหลาบวิทยาลัย เพชรบูรณ์',
    'เนินพิทยาคม',
    'เพชรบูรณ์วิทยา',
    'พัชรพิทยาคม',
    'ศึกษาสงเคราะห์เพชรบูรณ์',
    'โสตศึกษาจังหวัดเพชรบูรณ์',
    'ชนแดนวิทยาคม',
    'ดงขุยวิทยาคม',
    'วังโป่งพิทยาคม',
    'วังโป่งศึกษา',
    'วัดดอนตลุง(ราษฎร์ศรัทธาทาน)',
    'วัดเกาะลอย(ชุณหะวัต ฯ)',
    'วัดห้วยตะแคง(พรประชานุกูล)',
    'บ้านเขาสวนหลวง',
    'วัดโสดาประดิษฐาราม',
    'วัดศาลเจ้า(เรือง เล็บครุฑ)',
    'วัดเกตุน้อย',
    'วัดคุ้งกระถิน(ราษฎร์สามัคคึอุทิศ)',
    'วัดเหนือวน',
    'จันทคามวิทยา',
    'วัดแคทราย(บุญประชาสรรค์)',
    'ชุมชนวัดคูบัว',
    'วัดบ้านโพธิ์',
    'วัดหนามพุงดอ',
    'วัดบางลี่(วุฒิพันธุ์วิทยา)',
    'วัดเจติยาราม(บัณฑิตประชาพานิช)',
    'วัดห้วยหมู(อมรธรรมรัตราษฎร์บำรุง)',
    'วัดเขางูสันติธรรม',
    'วัดอรัญญิกาวาส(เจริญธรรมวิทยา)',
    'วัดดอนแจง(ไสวราษฎร์วิทยาคาร)',
    'วัดใหม่นครบาล',
    'วัดดอนตะโก(วังประชารัฐอุปถัมภ์)',
    'วัดทุ่งหญ้าคมบาง',
    'วัดนาหนอง(วิธานราษฎร์อนุกูล)',
    'วัดบ้านกล้วย(เฟื้อราษฎร์บำรุง)',
    'บ้านหนองนางแพรว',
    'บ้านชัฎเจริญ',
    'วัดน้ำพุ',
    'วัดอัมพวัน',
    'วัดพเนินพลู',
    'วัดไผ่ล้อม(เจริญราษฎร์วิทยาคม)',
    'วัดใหม่ราษฎร์บำรุงเจริญธรรม',
    'วัดท้ายเมือง',
    'วัดพิกุลทอง(แปลกประชาคาร)',
    'วัดโพธิ์ราษฏร์ศรัทธาธรรม',
    'วัดบางกระ',
    'วัดประเสริฐราษฎร์รังสรรค์วิทยา',
    'บ้านเขากรวด',
    'อนุบาลราชบุรี',
    'วัดเขาวัง(แสง ช่วงสุวนิช)',
    'วัดท่าโขลง(พยุงประชานุกูล)',
    'วัดเขาถ้ำกุญชร',
    'วัดห้วยไผ่',
    'วัดอรุณรัตนคีรี',
    'วัดห้วยปลาดุก(อนันตกูลอุปถัมภ์)',
    'บ้านทุ่งน้อย(มิตรภาพที่ 12)',
    'วัดหนองกระทุ่ม(สังฆรักษ์ราษฎร์บำรุง)',
    'วัดหนองตาหลวง',
    'วัดโพธิ์ดก',
    'วัดใหญ่อ่างทอง(เทียนประชาสรรค์)',
    'อนุบาลเมืองราชบุรี',
    'บ้านหนองกระทุ่ม',
    'มหาราช 7',
    'บ้านพุม่วงราษฎรบำรุง',
    'บ้านหนองปากชัฎ',
    'บ้านรางเฆ่',
    'บ้านพุตะเคียน',
    'บ้านทุ่งแฝก',
    'บ้านสันดอน',
    'บ้านทุ่งแจง',
    'บ้านวังปลา',
    'บ้านจอมบึง(วาปีพร้อมประชาศึกษา)',
    'วันครู 2503 (บ้านหนองบัว)',
    'อนุบาลจอมบึง',
    'บ้านรางม่วง',
    'บ้านตลาดควาย(ประชานุกูล)',
    'บ้านหนองขนาก',
    'วัดด่านทับตะโก',
    'บ้านห้วยท่าช้าง',
    'บ้านทุ่งกระถิน',
    'บ้านหนองสีนวล',
    'บ้านหุบพริก',
    'บ้านโกรกสิงขร',
    'บ้านหนองไก่เถื่อน',
    'บ้านหนองศาลเจ้า',
    'วัดป่าเขาสัมมะงา',
    'บ้านหนองปรือ',
    'บ้านเบิกไพร',
    'วัดสูงเนินราษฎรบำรุง',
    'บ้านหนองตาเนิด',
    'บ้านหนองไผ่',
    'บ้านพุแค',
    'วัดเขาปิ่นทอง',
    'วัดปากช่อง',
    'วัดนาสมอ',
    'บ้านห้วยยาง',
    'วัดชัฎใหญ่',
    'ชุมชนวัดรางบัว(แหลมราษฎร์บำรุง)',
    'บ้านหนองแร้ง(แหลมสุขประชานุกูล)',
    'วัดหนองบัวค่าย',
    'บ้านหนองนกกะเรียน',
    'รุจิรพัฒน์',
    'บ้านท่ามะขาม',
    'บ้านสวนผึ้ง',
    'บ้านบ่อหวี (กลุ่มนักข่าวหญิงสอง)',
    'บ้านมะขามเอน',
    'วัดรางเสน่ห์นครจันทร์',
    'วัดเขาไก่แจ้',
    'สมนึกวิทิศวรการ',
    'วัดเขากลิ้ง(รุ่งเจริญธรรม)',
    'บ้านชัฎหนองหมี',
    'บ้านหนองขาม',
    'บ้านหนองหม้อข้าว(ศิริทวีอุปถัมภ์)',
    'บ้านกล้วย',
    'บ้านทุ่งศาลา',
    'อนุบาลสวนผึ้ง',
    'ธรรมศาสตร์ - จุฬา 2',
    'บ้านห้วยผาก',
    'สินแร่สยาม',
    'บ้านตะโกล่าง',
    'ชุมชนบ้านบ่อ',
    'วัดนาขุนแสน(แกละประชานุกูล)',
    'ชุมชนบ้านบ่อ (สาขาวังส้มป่อย)',
    'วัดดอนทราย(ไชยประชาสรรค์)',
    'บ้านเขาถ้ำ',
    'บ้านเขาอีส้าน',
    'บ้านหนองบัวหิ่ง(สนิทราษฎร์บำรุง)',
    'บ้านไพรสะเดา',
    'บ้านเขาถ่าน',
    'ชุมชนวัดทุ่งหลวง',
    'บ้านพุคาย',
    'บ้านหนองน้ำใส',
    'วัดสันติการาม',
    'บ้านเขาพระเอก',
    'บ้านหนองไผ่',
    'วัดสนามสุทธาวาส(พรหมประชานุกูล)',
    'บ้านหนองวัวดำ',
    'วัดวิมลมรรคาราม',
    'วัดหนองบัว',
    'บ้านบ่อตะคร้อ',
    'อนุบาลปากท่อ',
    'วัดปากท่อ(ปากท่อวิทยาคาร)',
    'วัดโคกพระเจริญ',
    'ไทยรัฐวิทยา 64(บ้านหนองเกตุ)',
    'วัดป่าไก่(ส่วนประชานุกูล)',
    'บ้านหินสี สาขาบ้านลานคา',
    'บ้านห้วยศาลา',
    'บ้านแม่ประจัน',
    'บ้านท่ายาง',
    'บ้านหินสี',
    'บ้านยางคู่',
    'สีวะรา',
    'บ้านไทรงาม',
    'วัดสว่างอารมณ์',
    'บ้านมณีลอย',
    'วัดวังมะนาว',
    'วัดยางงาม',
    'วัดหัวป่า',
    'วัดวันดาว',
    'วัดหนองกระทุ่ม',
    'บ้านเขาช้าง',
    'วัดราษฎร์สมานฉันท์',
    'วัดโพธิศรี',
    'วัดพิบูลวนาราม',
    'บ้านหนองลังกา',
    'บ้านหัวเขาจีน',
    'บ้านพุเกตุ',
    'บ้านห้วยยางโทน',
    'บ้านโป่ง',
    'บ้านอ่างหิน',
    'บ้านนาคอก มิตรภาพที่ 104',
    'วัดเวียงทุน',
    'วัดเกาะศาลพระ',
    'วัดแจ้งเจริญ',
    'บ้านปากสระ',
    'จรูญศรีวิทยาประชานุกูล',
    'อนุบาลวัดเพลง(หรีดศรัทธาฯ)',
    'วัดศรัทธาราษฎร์',
    'บ้านลำพระ',
    'อนุบาลบ้านคา',
    'บ้านบึง(สันติมโนประชาสรรค์)',
    'บ้านโป่งเจ็ด',
    'ประชาพัฒนาวิทย์',
    'บ้านเก่าต้นมะค่า',
    'บ้านโป่งกระทิงบน',
    'บ้านโป่งกระทิงล่าง',
    'โรตารี่ 1 บ้านพุน้ำร้อน (สาขาบ้านพุหิน)',
    'บ้านพุบอนบน',
    'บ้านร่องเจริญ',
    'น้ำตกห้วยสวนพลู',
    'โรตารี่ 1 บ้านพุน้ำร้อน',
    'บ้านหนองโก',
    'วัดหนองพันจันทร์',
    'วัดยอดสุวรรณคีรี',
    'บ้านหนองจอก(จิตติ-บุญศรี)',
    'วัดช่องลาภ(โชคลาภประชาพัฒนา)',
    'สวนผึ้งวิทยา',
    'ปากท่อพิทยาคม',
    'เบญจมราชูทิศ',
    'คุรุราษฎร์รังสฤษฏ์',
    'ด่านทับตะโกราษฎร์อุปถัมภ์',
    'วัดสันติการามวิทยา',
    'ราชโบริกานุเคราะห์',
    'แคทรายวิทยา',
    'บรมราชินีนาถราชวิทยาลัย',
    'รัฐราษฎร์อุปถัมภ์',
    'โสภณศิริราษฎร์',
    'บ้านคาวิทยา',
    'บ้านดอนฟักทอง',
    'วัดสอนประดิษฐ์วราราม',
    'วัดพิทักษ์เทพาวาส(พิทักษ์รัฐประชาสรรค์)',
    'อนุบาลดำเนินสะดวก(วัดโคกบำรุงฯ',
    'วัดสนามไชย',
    'บ้านหนองสลิด',
    'ชุมชนวัดดอนคลัง มิตรภาพที่ 178',
    'บ้านดอนไผ่',
    'วัดเวฬุวนาราม',
    'ชุมชนวัดราษฎร์เจริญธรรม',
    'วัดโชติทายการาม',
    'วัดปรกเจริญ',
    'บ้านรางสีหมอก',
    'วัดอมรญาติสมาคม(อมรวิทยาคาร)',
    'วัดบัวงาม',
    'วัดตาลเรียง(เชยประชานุกูล)',
    'วัดบ้านไร่',
    'วัดชาวเหนือ',
    'ชุมชนวัดประสาทสิทธิ์',
    'วัดท่าเรือ',
    'บ้านหนองไก่แก้ว',
    'วัดสิดาราม',
    'วัดเนกขัมมาราม',
    'วัดอุบลวรรณา',
    'วัดหลักหกรัตนาราม',
    'วัดใหม่สี่หมื่น',
    'วัดคูหาสวรรค์',
    'วันมหาราช(ผาณิตพิเชฐวงศ์)',
    'วัดหนองประทุน',
    'ชุมชนบ้านห้วยกระบอก(สามัคคีวิทยา)',
    'วัดหนองเสือ(ประชาอุทิศ)',
    'วัดอ้ออีเขียว(รัฐราษฎร์มิตรผลอนุกูล)',
    'วัดหนองกลางด่าน',
    'วัดไผ่สามเกาะ(ปัญญาประชาสามัคคี)',
    'วัดสัมมาราม',
    'วัดเขาขลุง',
    'บ้านดอนไม้ลาย',
    'บ้านหนองไก่ขัน',
    'วัดสระสี่มุม',
    'วัดเขาแจง',
    'บ้านโป่งยอ(จำเริญ โฆสิตกุล)',
    'วัดเจริญธรรม(รัฐประชานุเคราะห์)',
    'บ้านห้วยยาง',
    'วัดตาล',
    'วัดโพธิ์บัลลังก์',
    'วัดลำพยอม',
    'วัดอริยวงศาราม(หนองน้ำขาว)',
    'ชุมชนวัดท่าผา',
    'วัดโกสินารายน์',
    'วัดดอนเสลา (ปาน พูน รัฐ ราษฎร์บำรุง)',
    'วัดยางหัก',
    'วัดบ้านฆ้องน้อย',
    'วัดหัวหิน',
    'วัดใหญ่(บุญเอี่ยมอนุเคราะห์)',
    'วัดตาผา',
    'วัดอัมพวนาราม',
    'บ้านหนองคา',
    'วัดม่วง',
    'วัดบัวงาม',
    'อนุบาลบ้านโป่ง(วัดปลักแรด)',
    'วัดหุบกระทิง',
    'วัดบางพัง',
    'วัดโคกหม้อ(ประชาสามัคคี)',
    'วัดโพธิ์รัตนาราม',
    'วัดรางวาลย์',
    'วัดลาดบัวขาว',
    'วัดหนองหญ้าปล้อง',
    'ค่ายลูกเสือบ้านโป่ง',
    'วัดหนองกบ(ขุนทองประชานุเคราะห์)',
    'วัดหนองปลาดุก',
    'วัดบึงกระจับ(รัฐประชาตันติธนานนท์)',
    'วัดหนองหิน(วิเชียรประชานุกูล)',
    'วัดสระตะโก(หันประชาสามัคคี)',
    'ชุมชนวัดหนองปลาหมอ',
    'วัดมาบแค',
    'วัดจันทาราม(ตั้งตรงจิตร 5)',
    'วัดหนองอ้อ(ธรรมธรใยประชาสามัคคี)',
    'วัดหัวโป่ง มิตรภาพที่ 224',
    'บ้านลาดใหญ่',
    'วัดตากแดด(รอดประชานุกูล)',
    'วัดดอนคา(ชอบธรรมประชาสรรค์)',
    'วัดดอนพรม',
    'วัดลำน้ำ(กัลยาณราษฎร์บำรุง)',
    'วัดดอนใหญ่(ทรัพย์ประชาสรรค์)',
    'วัดตาลเตี้ย',
    'วัดดอนสาลี',
    'อนุบาลบางแพ(ชุมชนวัดบางแพใต้ฯ)',
    'วัดท่าราบ(วันชัยประชานุกูล)',
    'วัดเหนือ',
    'วัดดอนเซ่ง',
    'บ้านกุ่ม',
    'วัดสามัคคีธรรม',
    'ชุมชนวัดใหญ่โพหัก',
    'ชุมชนวัดกลางวังเย็น(เริ่มประชานุกูล)',
    'วัดหนองม่วง',
    'วัดหลวง',
    'วัดหนองเอี่ยน',
    'วัดทำนบ',
    'วัดบ้านใหม่',
    'วัดแก้ว(รัตนบินวิทยาคาร)',
    'วัดแหลมทอง',
    'วัดหัวโพ',
    'วัดดอนมะขามเทศ(กิติยานุกูล)',
    'วัดเขาชะงุ้ม',
    'วัดเขาส้ม',
    'วัดหนองมะค่า',
    'บ้านพุลุ้ง(กรป.กลางอุปถัมภ์)',
    'วัดระฆังทอง',
    'บ้านเขาแหลม',
    'วัดจอมปราสาท(อุดมวิทยา)',
    'วัดมณีโชติ(เทียมประชานุเคราะห์)',
    'วัดคงคาราม(อินทราชผดุงศิลป)',
    'ธรรมาธิปไตย',
    'วัดบ้านหม้อ(ประชารังษี)',
    'บ้านเกาะโพธิ์งาม',
    'บ้านหาดสำราญ มิตรภาพที่207',
    'ชุมชนวัดเจ็ดเสมียน',
    'วัดสนามชัย',
    'วัดตึกหิรัญราษฎร์(มหาธีรราชอนุสรณ์)',
    'บ้านหนองสองห้อง',
    'วัดหนองกลางดง',
    'วัดดอนกระเบื้อง(ดอนกระเบื้องราษฎร์บำรุง)',
    'วัดดอนทราย',
    'วัดท่ามะขาม(ท่ามะขามประชาอุทิศ)',
    'วัดบางลาน',
    'วัดช่องพราน',
    'วัดโคกทอง',
    'วัดเขาพระ',
    'วัดท่าหลวงพล(ถาวรวิทยา)',
    'วัดชัยรัตน์(ปัญญาประชานุกูล)',
    'ชุมชนวัดเฉลิมอาสน์',
    'บ้านเนินม่วง(ประชาบำรุง)',
    'บ้านหนองตาพุด',
    'วัดแก้วฟ้า',
    'วัดนางแก้ว',
    'วัดสมถะ(สมถวิทยาคาร)',
    'วัดศรีประชุมชน',
    'วัดบางโตนด',
    'วัดบ่อมะกรูด',
    'วัดบ้านฆ้อง',
    'วัดดีบอน',
    'วัดพระศรีอารย์',
    'วัดหุบมะกล่ำ',
    'วัดโบสถ์(อินรัฐราษฎร์บำรุง)',
    'ชุมชนวัดบ้านเลือก(โพธาภิรมย์)',
    'ชุมชนวัดกำแพงใต้(กำแพงสังฆรักษ์วิทยา)',
    'วัดหนองอ้อ',
    'อนุบาลโพธาราม ฯ',
    'วัดบางกะโด(สามัคคีวิทยาคาร)',
    'ชุมชนวัดหนองโพ',
    'วัดขนอน',
    'วัดสร้อยฟ้า',
    'บ้านหนองกวาง',
    'บ้านหนองใยบัว',
    'บ้านหนองครึม',
    'บ้านหนองกระทุ่ม',
    'สายธรรมจันทร์',
    'โพธาวัฒนาเสนี',
    'ประสาทรัฐประชากิจ',
    'หนองปลาหมอพิทยาคม',
    'โพหัก(วงศ์สมบูรณ์ราษฎร์อุปถัมภ์)',
    'หนองโพวิทยา',
    'ดอนคลังวิทยา',
    'ช่องพรานวิทยา',
    'เนกขัมวิทยา',
    'วัดดอนตูม',
    'ท่ามะขามวิทยา',
    'บางแพปฐมพิทยา',
    'กรับใหญ่ว่องกุศลกิจพิทยาคม',
    'รัตนราษฎร์บำรุง',
    'บ้านเขาตก"ไตรเดชวิทยา"',
    'บ้านแก่งหลวง',
    'วัดถ้ำมังกรทอง',
    'บ้านท่าอ้อ',
    'วัดท่าน้ำตื้น',
    'ท่าพะเนียดกุญชร',
    'บ้านหนองสองตอน',
    'บ้านหนองปลวก',
    'บ้านเกาะแก้ว(การไฟฟ้าอุปถัมภ์)',
    'บ้านทับศิลา',
    'บ้านช่องสะเดา(การไฟฟ้าฝ่ายผลิตอุปถัมภ์)',
    'บ้านช่องกระทิง',
    'บ้านหัวหิน',
    'วัดบ้านยาง',
    'วัดท่ามะขาม',
    'พัฒน์พงศ์',
    'บ้านห้วยน้ำขาว',
    'หนองบ้านเก่าสามัคคี',
    'บ้านตะเคียนงาม',
    'บ้านทุ่งศาลา',
    'วัดท่าโป๊ะ',
    'บ้านห้วยน้ำขาว สาขาพุน้ำร้อน',
    'วัดบ้านเก่า',
    'วัดพุน้อย',
    'ตชด.บ้านประตูด่าน',
    'อนุบาลวัดไชยชุมพลชนะสงคราม',
    'อนุบาลกาญจนบุรี',
    'บ้านหัวนา',
    'บ้านวังสารภี',
    'วัดลาดหญ้า"ลาดหญ้าวิทยา"',
    'วัดกาญจนบุรีเก่า"อุดมราษฎร์วิทยา"',
    'บ้านท่าหวี',
    'บ้านจันอุย',
    'อาทรสังขะวัฒนะ 3',
    'บ้านหนองแก(สามัคคีวิทยา)',
    'บ้านทุ่งนานางหรอก',
    'บ้านหนองสามพราน',
    'บ้านท่ามะนาว',
    'บ้านวังด้ง',
    'บ้านท่าทุ่ม',
    'บ้านท่าโป่ง',
    'บ้านหนองหอย',
    'วัดเขาน้อย',
    'บ้านนากาญจน์',
    'บ้านดงยาง',
    'บ้านวังตะเคียน',
    'บ้านหนองกลางพง',
    'ลุ่มโป่งเสี้ยว',
    'บ้านวังเย็น',
    'วัดศรีอุปลาราม',
    'บ้านหัวเขาพุประดู่',
    'สมาคมนักเรียนเก่าราชินี',
    'บ้านหนองบัว',
    'บ้านพุเลียบ',
    'บ้านทุ่งนาคราช',
    'บ้านเขาปูน',
    'บ้านวังปลาหมู',
    'บ้านวังลาน',
    'วัดไทยวิวัฒนาราม',
    'บ้านถ้ำ',
    'บ้านดอนคราม',
    'วัดหนองตะโก',
    'บ้านห้วยน้ำโจน',
    'วัดท่าตะคร้อ',
    'วัดศรีโลหะราษฎร์บํารุง',
    'อนุบาลท่าม่วง',
    'วัดท่าล้อ',
    'เขาดินวิทยาคาร',
    'บ้านห้วยนาคราช',
    'วัดศรีสุวรรณาราม',
    'วัดกร่างทองราษฎร์บูรณะ',
    'บ้านหนองมงคล',
    'บ้านหนองน้ำขุ่น',
    'บ้านใหม่(จีนาภักดิ์วิทยา)',
    'บ้านสระเศรษฐี',
    'วัดลําสำรอง',
    'บ้านรางสะเดา',
    'ตลาดสํารอง',
    'วัดหนองพังตรุ',
    'บ้านห้วยไร่',
    'บ้านหนองเป็ด',
    'วัดม่วงชุม',
    'วัดชุกพี้',
    'วัดขุนไทยธาราราม',
    'บ้านรางสาลี่',
    'วัดสระกลอยสามัคคีธรรม',
    'บ้านมะกอกหมู่(ปุญสิริวิทยา)',
    'วัดวังขนายทายิการาม',
    'บ้านหนองตาบ่ง',
    'วัดวังศาลา',
    'วัดหนองเสือ',
    'บ้านหนองสะแก',
    'วัดโพธิ์ศรีสุขาราม',
    'บ้านรางจิก',
    'วัดอินทาราม"โกวิทอินทราทร"',
    'บ้านห้วยตลุง',
    'วัดหนองตะครอง',
    'บ้านโกรกตารอด',
    'ดิศกุล',
    'บ้านหนองอีเห็น',
    'บ้านหนองสองห้อง',
    'วัดยางเกาะ',
    'บ้านกลอนโด',
    'บ้านดอนสว่าง',
    'วัดหนองบัว',
    'บ้านแหลมทอง',
    'บ้านทุ่งมะขามเฒ่า',
    'วัดจรเข้เผือก',
    'บ้านท่ามะเฟือง',
    'บ้านท่าเสด็จ',
    'วัดถ้ำอ่างหิน',
    'บ้านหนองหญ้าปล้อง',
    'บ้านหนองกวาง',
    'บ้านทุ่งยาว',
    'บ้านหนองโสน',
    'บ้านไทรทอง',
    'บ้านโป่งโก',
    'บ้านท่ามะไฟ',
    'บ้านเนินไพร',
    'บ้านหนองหิน',
    'บ้านหนองปล้อง',
    'บ้านท่าแย้',
    'ช่างกลอนุสรณ์ 3 (บ้านหนองหวาย)',
    'บ้านท่าพุ',
    'อนุบาลด่านมะขามเตี้ย',
    'บ้านหนองผู้เฒ่า',
    'บ้านหนองไผ่',
    'บ้านหนองปากดง',
    'บ้านหินแด้น',
    'เทพศิรินทร์ลาดหญ้ากาญจนบุรี',
    'ท่าม่วงราษฎร์บำรุง',
    'หนองขาวโกวิทพิทยาคม',
    'พังตรุราษฎร์รังสรรค์',
    'ด่านมะขามเตี้ยวิทยาคม',
    'วิสุทธรังษี',
    'กาญจนานุเคราะห์',
    'เทพมงคลรังษี',
    'หนองตากยาตั้งวิริยะราษฎร์บำรุง',
    'เฉลิมพระเกียรติสมเด็จพระศรีนครินทร์ กาญจนบุรี',
    'บ้านเก่าวิทยา',
    'โสตศึกษาจังหวัดกาญจนบุรี',
    'บ้านหนองหว้า',
    'บ้านช่องด่าน',
    'บ้านวังใหญ่',
    'บ้านหนองเข้',
    'บ้านเขาแดง',
    'บ้านสามยอด',
    'บ้านหนองเตียน',
    'บ้านหนองโพธิ์',
    'บ้านบึงหัวแหวน',
    'อนุบาลบ่อพลอย',
    'บ้านหนองย่างช้าง',
    'บ้านท่าว้า',
    'บ้านหนองกร่าง',
    'บ้านหลังเขา',
    'บ้านพุรวก',
    'บ้านหนองหวาย',
    'บ้านเสาหงส์',
    'บ้านวังด้ง',
    'บ้านพุพรหม',
    'วังเขาแก้ว(หงำอุปถัมภ์)',
    'บ้านหนองกระทุ่ม',
    'บ้านหนองกุ่ม',
    'วัดทุ่งมะสัง(มิตรภาพที่ 9)',
    'บ้านรางขาม',
    'บ้านหนองรี',
    'บ้านหนองแกใน',
    'บ้านลําอีซู',
    'บ้านลําอีซู สาขาบ้านหนองสําโรง',
    'ชุมชนบ้านหลุมรัง',
    'บ้านหนองหมู',
    'ไทยรัฐวิทยา 21 (บ้านลําเหย)',
    'บ้านยางสูง',
    'บ้านไร่เจริญ',
    'วัดเขาสามสิบหาบ',
    'บ้านหนองหิน',
    'บ้านเขาช่อง',
    'วัดเขาใหญ่',
    'บ้านชายธูป',
    'วัดเขาตะพั้น',
    'อนุบาลท่ามะกา(วัดลูกแกประชาชนูทิศ)',
    'วัดดอนขมิ้น',
    'วัดทุ่งมะกรูด',
    'บ้านทุ่งประทุน',
    'วัดดอนชะเอม',
    'บ้านบึงวิทยา',
    'บ้านหนองตาแพ่ง',
    'บ้านรางกระต่าย"พิริยะประชาวิทยาคาร"',
    'บ้านหนองตาคง',
    'วัดตะคร้ำเอน',
    'วัดสำนักคร้อ',
    'บ้านหนองลาน',
    'บ้านท่ามะกา',
    'วัดคร้อพนัน',
    'วัดกระต่ายเต้น',
    'วัดท่าเรือ"อุตสาหะวิทยาคาร"',
    'วัดใหม่เจริญผล',
    'บ้านท่าพะเนียง',
    'ดอนสามง่ามผิว หงสวีณะอุปถัมภ์',
    'วัดหนองพันท้าว(พรประชาวิทยาคาร)',
    'เกียรติวัธนเวคิน 1 (วัดปากบาง)',
    'วัดดงสัก"หมั่นวิทยาคาร"',
    'วัดพระแท่นดงรัง',
    'บ้านไร่ร่วมวิทยาคาร',
    'บ้านดอนรัก',
    'บ้านหนองกรด',
    'วัดห้วยตะเคียน',
    'วัดหนองโรง',
    'วัดเขาสะพายแร้ง',
    'วัดสนามแย้',
    'บ้านดอนเขว้า',
    'วัดหนองพลับ',
    'วัดท่ากระทุ่ม',
    'วัดแสนตอ(วรวัตตวิทยาคาร)',
    'วัดหนองลาน',
    'บ้านจันทร์ลาดวิทยา',
    'วัดดาปานนิมิต มิตรภาพที่ 142',
    'วัดหนองไม้แก่น',
    'วัดหวายเหนียว"ปุญสิริวิทยา"',
    'บ้านหนองซ่อนผึ้งผดุงวิทย์',
    'ประชาวิทยาคาร',
    'บ้านแจงงาม',
    'บ้านดอนตาลเสี้ยน',
    'บ้านอุโลกสี่หมื่น',
    'วัดดอนเจดีย์',
    'วัดบ้านน้อย',
    'วัดสาลวนาราม',
    'บ้านบ่อหว้า',
    'บ้านหนองโพธิ์',
    'บ้านดอนมะขาม',
    'วัดทุ่งสมอ',
    'บ้านดอนสระ',
    'วัดบ้านทวน',
    'วัดพังตรุ',
    'บ้านบ่อระแหง',
    'บ้านโป่งกูป',
    'บ้านรางสมอ',
    'บ้านกระเจา',
    'วัดเบญพาด',
    'อนุบาลพนมทวน',
    'บ้านโคราช',
    'บ้านดอนเตาอิฐ',
    'บ้านหนองจอก',
    'บ้านตลาดเขต มิตรภาพที่ 105',
    'วัดห้วยสะพาน',
    'วัดนาพระยา',
    'บ้านรางยอม',
    'บ้านห้วยด้วน',
    'บ้านวังรัก',
    'บ้านหลุมหิน',
    'บ้านหนองขุย',
    'บ้านสระลุมพุก',
    'เกียรติวัธนเวคิน 2 (วัดปลักเขว้า)',
    'บ้านน้ำลาด',
    'บ้านทุ่งกระบ่ำ',
    'หมู่บ้านป่าไม้',
    'ราษฎร์บํารุงธรรม',
    'บ้านหนองหวาย',
    'บ้านหนองอําเภอจีน',
    'อนุบาลเลาขวัญ',
    'บ้านหนองแสลบ',
    'บ้านพุบอน',
    'บ้านเขานางสางหัว',
    'บ้านหนองผือ',
    'บ้านรางพยอม',
    'บ้านหนองนกแก้ว',
    'บ้านช่องกลิ้งช่องกรด',
    'บ้านหนองเค็ด',
    'บ้านหนองไก่เหลือง',
    'บ้านหนองประดู่',
    'บ้านน้ำโจน',
    'บ้านตลุงเหนือ',
    'บ้านโป่งไหม',
    'บ้านหนองปรือ',
    'บ้านหนองตาก้าย',
    'บ้านหนองปลิง',
    'บ้านพรหมณี',
    'ประชาพัฒนา',
    'บ้านหนองใหญ่',
    'บ้านบะลังกา',
    'บ้านหนองกะหนาก',
    'บ้านตรอกสะเดา',
    'บ้านหนองมะสัง',
    'บ้านกรับใหญ่',
    'บ้านชุมนุมพระ',
    'ชุมชนบ้านหนองฝ้าย',
    'เขาวงพระจันทร์',
    'บ้านหนองงูเห่า',
    'บ้านหนองไผ่ล้อม',
    'วัดใหม่ภูมิเจริญ',
    'บ้านสระเตยพัฒนา',
    'บ้านหนองไก่ต่อ',
    'บ้านหนองม่วง',
    'บ้านหนองโสน',
    'บ้านน้ำคลุ้ง',
    'บ้านหนองผักแว่น',
    'บ้านเขาหินตั้ง',
    'ป่าไม้อุทิศ 15(บ้านม่วงเฒ่า)',
    'บ้านเขาแหลม',
    'อนุบาลหนองปรือ',
    'บ้านหนองขอน',
    'บ้านทุ่งโป่ง',
    'เสรี-สมใจ',
    'บ้านหนองใหญ่',
    'วัดหนองไม้เอื้อย',
    'บ้านเขามุสิ',
    'บ้านหนองสาหร่าย',
    'บ้านหนองตาเดช',
    'บ้านพยอมงาม',
    'บ้านหนองขอนเทพพนม',
    'บ้านห้วยหวาย',
    'บ้านหนองปลาไหล',
    'บ้านหนองแกประชาสรรค์',
    'วัดดอนแสลบ',
    'วัดหนองปลิง',
    'วัดเขารักษ์',
    'บ้านตลุง',
    'บ้านพนมนาง',
    'บ้านหนองนางเลิ้ง',
    'บ้านวังไผ่',
    'บ้านหนองตายอด',
    'บ้านอ่างหิน',
    'บ้านนาใหม่',
    'บ้านไผ่สี',
    'บ้านห้วยยาง',
    'บ้านสระจันทอง',
    'บ้านสระลงเรือ',
    'บ้านหนองเจริญสุข',
    'เมตตาจิตต',
    'บ้านห้วยลึก',
    'บ้านเขาศาลา',
    'บ้านทุ่งมังกะหร่า',
    'บ้านทัพพระยา',
    'บ้านเขากรวด',
    'อนุบาลห้วยกระเจา',
    'บ้านซ่อง',
    'บ้านไพรงาม',
    'บ่อพลอยรัชดาภิเษก',
    'ท่ามะกาวิทยาคม',
    'พนมทวนชนูปถัมภ์',
    'หนองรีประชานิมิต',
    'พระแท่นดงรังวิทยาคาร',
    'พนมทวนพิทยาคม(สว่างเคลิ้มสุคนธ์)',
    'นิวิฐราษฎร์อุปถัมภ์',
    'ท่ามะกาปุญสิริวิทยา',
    'ท่าเรือพิทยาคม',
    'ศึกษาสงเคราะห์พนมทวน',
    'เลาขวัญราษฎร์บำรุง',
    'หนองปรือพิทยาคม',
    'ประชามงคล',
    'ห้วยกระเจาพิทยาคม',
    'บ้านวังใหญ่',
    'บ้านพุม่วง-พุพง',
    'บ้านหาดงิ้ว',
    'บ้านพุเตย',
    'อนุบาลไทรโยค',
    'บ้านพุองกะ',
    'พุทธวิมุติวิทยา',
    'บ้านช่องแคบ',
    'บ้านทุ่งก้างย่าง',
    'บ้านแก่งประลอม',
    'ไทรโยคใหญ่',
    'วัดใหม่ดงสัก',
    'บ้านท่าทุ่งนา',
    'บ้านแก่งจอ',
    'บ้านถ้ำดาวดึงส์',
    'บ้านบ้องตี้',
    'ตชด.บ้านบ้องตี้ล่าง',
    'บ้านเขาสามชั้น',
    'หลุงกัง',
    'สามัคคีธรรมานุสรณ์',
    'บ้านหนองขอน',
    'บ้านวังโพธิ์',
    'บ้านลุ่มผึ้ง',
    'บ้านแก่งระเบิด',
    'บ้านเขาช้าง',
    'บ้านเขาพัง',
    'บ้านสารวัตร',
    'บ้านวังกระแจะ',
    'สมาคมไทย-ออสเตรเลียน',
    'บ้านท่ามะเดื่อ',
    'บ้านหินดาด',
    'บ้านยางโทน',
    'บ้านทุ่งเรือโกลน',
    'บ้านหนองปลาซิว',
    'บ้านห้วยกะทะทอง',
    'บ้านพุปลู',
    'บ้านท่าตาเสือ',
    'บ้านวังสิงห์',
    'บ้านหนองปรือ',
    'วัดปากกิเลน',
    'บ้านน้ำพุ',
    'บ้านสามหลัง',
    'บ้านด่านแม่แฉลบ',
    'บ้านดงเสลา',
    'บ้านโป่งหวาย',
    'บ้านพุน้ำเปรี้ยว',
    'บ้านท่าสนุ่น',
    'บ้านบนเขาแก่งเรียง',
    'ชุมชนบ้านท่ากระดาน',
    'บ้านหม่องกระแทะ',
    'บ้านเจ้าเณร',
    'บ้านแก่งแคบ',
    'วัดถ้ำองจุ',
    'บ้านองหลุ',
    'บ้านปากนาสวน',
    'บ้านองสิต',
    'บ้านนาสวน',
    'บ้านน้ำมุด',
    'บ้านต้นมะพร้าว',
    'อนุบาลศรีสวัสดิ์',
    'บ้านทุ่งนา',
    'บ้านห้วยเสือ',
    'วัดป่าถ้ำภูเตย',
    'บ้านเกริงกระเวีย',
    'บ้านทุ่งเสือโทน',
    'บ้านเหมืองสองท่อ',
    'อนุบาลทองผาภูมิ',
    'วัดปรังกาสี',
    'บ้านจันเดย์',
    'บ้านเสาหงษ์',
    'บ้านอูล่อง',
    'บ้านหินแหลม',
    'บ้านเหมืองแร่อีต่อง',
    'บ้านกุยแหย่',
    'บ้านลิ่นถิ่น',
    'บ้านนามกุย',
    'บ้านหนองเจริญ',
    'บ้านดินโส',
    'บ้านป่าไม้สะพานลาว',
    'บ้านท่ามะเดื่อ',
    'สมาคมป่าไม้แห่งประเทศไทยอุทิศ',
    'บ้านไร่ป้า',
    'บ้านไร่',
    'บ้านประจําไม้',
    'บ้านห้วยเขย่ง',
    'บ้านปากลําปิล็อก',
    'คุรุสภา',
    'บ้านวังผาตาด',
    'บ้านดงโคร่ง',
    'วัดหินดาด',
    'บ้านหินดาด',
    'บ้านท่าดินแดง',
    'บ้านยางขาว',
    'บ้านหินตั้ง',
    'บ้านกองม่องทะ',
    'บ้านซองกาเรีย สาขาพระเจดีย์สามองค์',
    'บ้านซองกาเรีย',
    'อนุบาลสังขละบุรี (บ้านวังกะ)',
    'วัดวังก์วิเวการาม',
    'บ้านห้วยมาลัย',
    'บ้านใหม่พัฒนา',
    'บ้านห้วยกบ',
    'ไทรโยคมณีกาญจน์วิทยา',
    'ศรีสวัสดิ์พิทยาคม',
    'ทองผาภูมิวิทยา',
    'อุดมสิทธิศึกษา',
    'ไทรโยคน้อยวิทยา',
    'ร่มเกล้า กาญจนบุรี',
    'สมเด็จพระปิยมหาราชรมณียเขต',
    'วัดโคกโคเฒ่า',
    'วัดหน่อสุวรรณ',
    'วัดนิเวศน์ธรรมาราม',
    'วัดวังพระนอน',
    'วัดพันตำลึง',
    'วัดอู่ยา',
    'วัดดอนตาล',
    'วัดไผ่เกาะโพธิ์งาม',
    'วัดดอนโพธิ์ทอง',
    'วัดไตรรัตนาราม',
    'วัดอุทุมพราราม',
    'วัดใหม่รัตนเจดีย์',
    'วัดลาดตาล',
    'วัดดอนเจดีย์ราษฎร์บูรณะ',
    'วัดสกุณปักษี',
    'วัดสามทอง',
    'วัดสุวรรณาคี',
    'เมืองสุพรรณบุรี',
    'วัดอินทร์ศรัทธาราษฎร์',
    'วัดสระประทุม',
    'วัดแก้ว',
    'วัดหัวไผ่',
    'สุพรรณภูมิ',
    'อนุบาลสุพรรณบุรี',
    'วัดมเหยงคณ์',
    'วัดโพคอย',
    'วัดคันทด',
    'บ้านบางกุ้ง',
    'วัดบ้านโพธิ์ตะวันตก',
    'วัดศีรษะเกษ',
    'วัดประชุมชน',
    'วัดโพธิ์ท่าทราย',
    'วัดบ้านโพธิ์ตะวันออก',
    'วัดไผ่ขวาง',
    'วัดดอนกลาง',
    'บ้านรางกะทุ่ม',
    'วัดสว่างอารมณ์',
    'วัดพิหารแดง',
    'วัดวรจันทร์',
    'วัดวังกุ่ม',
    'วัดพร้าว',
    'อนุบาลวัดป่าเลไลยก์',
    'วัดกุฎีทอง',
    'วัดราษฎร์ศรัทธาธรรม',
    'วัดพระธาตุ',
    'วัดลาดกระจับ',
    'วัดจำปี',
    'วัดหนองโสน',
    'บ้านหนองขาม',
    'วัดสำนักตะฆ่า',
    'บ้านดอนโพ',
    'บ้านกิโลแปด',
    'บรรหารแจ่มใสวิทยา 4',
    'บ้านท่าเสด็จ',
    'วัดเขาดิน',
    'บ้านหลักเมตร',
    'บ้านหนองปรือ',
    'วัดสามัคคีธรรม',
    'บ้านหนองบอน',
    'วัดไผ่ลูกนก',
    'บ้านไผ่แปลกแม่',
    'วัดสวนแตง',
    'วัดสังฆจายเถร',
    'กฤษณา',
    'วัดราษฎร์บูรณะ',
    'วัดโคกโพธิ์',
    'วัดบางเลน',
    'วัดกลาง',
    'วัดปากคลองกุ่ม',
    'บรรหารแจ่มใสวิทยา 2',
    'วัดขวาง',
    'วัดลานคา',
    'วัดจระเข้ใหญ่',
    'วัดดารา',
    'วัดชีปะขาว',
    'บางปลาม้า',
    'วัดศาลาท่าทราย',
    'วัดลำบัว',
    'บ้านโพธิ์ศรี',
    'อนุบาลวัดสวนหงส์',
    'วัดบ้านหมี่',
    'วัดบ้านด่าน',
    'วัดเสาธง',
    'วัดโพธิ์ศรี',
    'บางแม่หม้าย',
    'วัดบางใหญ่',
    'บ้านรางทอง',
    'วัดดอนกระเบื้อง',
    'วัดศุขเกษม',
    'วัดป่าพฤกษ์',
    'วัดช่องลม',
    'วัดราษฎร์บำรุง',
    'วัดโบสถ์',
    'วัดสุขเกษม',
    'วัดตปะโยคาราม',
    'วัดตะลุ่ม',
    'วัดไผ่มุ้ง',
    'วัดวังน้ำเย็น',
    'วัดโพธิ์ตะควน',
    'วัดสังโฆสิตาราม',
    'วัดดาว',
    'วัดดอนตาจีน',
    'วัดคูบัว',
    'วัดดอนขาด',
    'วัดดอนไข่เต่า',
    'วัดไผ่เดี่ยว',
    'วัดบางจิก',
    'วัดสาลี',
    'วัดลาดน้ำขาว',
    'วัดบึงคา',
    'วัดเสาธงทอง',
    'วัดทรงกระเทียม',
    'วัดองครักษ์',
    'วัดคลองโมง',
    'วัดสามัคคีธรรม',
    'วัดห้วยสุวรรณาราม',
    'วัดดอนสุทธาวาส',
    'วัดโพธิ์นฤมิตร',
    'วัดสามจุ่น',
    'บ้านหนองเพียน',
    'วัดบ้านกล้วย',
    'วัดปู่เจ้า',
    'วัดดอนบุบผาราม',
    'วัดบ้านกร่าง',
    'บ้านคลองชะอม',
    'บ้านหนองสรวง',
    'วัดป่าพระเจ้า',
    'บ้านรางหางม้า',
    'วัดเทพสุธาวาส',
    'วัดเสาธงทอง',
    'วัดวังพลับเหนือ',
    'วัดวังพลับใต้',
    'วัดเถรพลาย',
    'วัดปลายนา',
    'บ้านห้วยเจริญ',
    'วัดพังม่วง',
    'วัดบรรไดทอง',
    'วัดไก่เตี้ย',
    'บ้านหนองจิก',
    'วัดสัปรสเทศ',
    'วัดเกาะ',
    'บ้านตลาดวังหว้า',
    'บ้านคลองชะโด',
    'วัดดงขี้เหล็ก',
    'วัดลาดปลาเค้า',
    'วัดม่วงเจริญผล',
    'อนุบาลศรีประจันต์',
    'วัดยาง',
    'ตลิ่งชันวิทยา',
    'กรรณสูตศึกษาลัย',
    'สงวนหญิง',
    'หรรษาสุจิตต์วิทยา 2',
    'สรวงสุทธาวิทยา',
    'วังหว้าราษฏร์สามัคคี',
    'กาญจนาภิเษกวิทยาลัย สุพรรณบุรี',
    'ศรีประจันต์(เมธีประมุข)',
    'บางปลาม้า(สูงสุมารผดุงวิทย์)',
    'สวนแตงวิทยา',
    'บางแม่หม้ายรัฐราษฎร์รังสฤษดิ์',
    'อนุบาลพระบรมราชานุสรณ์ดอนเจดีย์',
    'บ้านนเรศ',
    'วัดสระศรีเจริญ',
    'วัดราษฎร์บำรุง',
    'วัดหนองหลอด',
    'บ้านทะเลบก',
    'บ้านสระหลวง',
    'บ้านบ่อสำราญ',
    'บ้านหนองแขม',
    'บ้านหนองสานแตร',
    'วัดธัญญวารี',
    'วัดชีธาราม',
    'บ้านหนองจิกรากข่า',
    'บ้านดงกะเชา',
    'วัดหนองแจง',
    'บ้านหนองฝ้าย',
    'บ้านสระกระโจม',
    'บ้านยมเบือ',
    'บ้านหนองสลัดได',
    'บุญศรีสุวรรณราษฎร์สามัคคี',
    'บ้านหัวเขา',
    'บ้านดอนกลาง',
    'บ้านห้วยม้าลอย',
    'บ้านหนองขุม',
    'บ้านโคกหม้อ',
    'วัดสระด่าน',
    'วัดบ้านกรวด',
    'วัดท่ากุ่ม',
    'วัดศรีเฉลิมเขต',
    'วัดดอนมะนาว',
    'วัดสองพี่น้อง',
    'บ้านโคกงูเห่า',
    'บ้านบัวขาว',
    'บ้านหนองวัลย์เปรียง',
    'วัดทุ่งคอก',
    'วัดทุ่งเข็น',
    'วัดเทพพิทักษ์',
    'บ้านโป่ง',
    'วัดเนินพระปรางค์',
    'วัดใหม่นพรัตน์',
    'บ้านหัววัง',
    'บ้านหนองเฝ้า',
    'บ้านหนองกระดี่',
    'บ้านดอนตำลึง',
    'วัดหัวกลับ',
    'วัดทับกระดาน',
    'วัดเวฬุวัน',
    'วัดหนองพันเทา',
    'วัดบางสาม',
    'วัดสำเภาทอง',
    'บ้านโคก 7 ลูก',
    'วัดใหม่บำรุงธรรม',
    'ตลาดขุนขยัน',
    'วัดโบสถ์ดอนลำแพน',
    'สองพี่น้อง',
    'วัดไชนาราษฎร์',
    'วัดท่าข้าม',
    'บ้านไผ่ตาโม้',
    'วัดไผ่โรงวัว',
    'อนุบาลวัดดงตาล',
    'วัดท่าจัด',
    'วัดใหม่พิบูลย์ผล',
    'วัดทองประดิษฐ์',
    'วัดรางบัวทอง (สุวรรณวิทยา)',
    'วัดบางสะแก (วิทยารังสรรค์)',
    'วัดสว่างอารมณ์',
    'วัดลาดประทุมทอง',
    'อาทรสังขะวัฒนะ 1 วัดลาดบัว',
    'วัดย่านซื่อ',
    'วัดไผ่ขาด',
    'วัดรางกร่าง',
    'วัดใหม่เพชรรัตน์',
    'บ้านหนองโพธิ์',
    'บ้านสะพังกร่าง',
    'อาทรสังขะวัฒนะ 2 วัดวังตะกู',
    'บ้านหนองกระทู้',
    'บ้านหนองจิก',
    'วัดพรสวรรค์',
    'วัดเขาพนมนาง',
    'วัดดอนสงวน',
    'วัดหัวโพธิ์',
    'บ้านประทุนทอง',
    'วัดท่าไชย',
    'วัดบางบอน',
    'วัดยางยี่แส',
    'วัดสามัคคีธรรม',
    'วัดจันทราวาส',
    'บ้านจรเข้สามพัน',
    'บ้านเขาชานหมาก',
    'วัดปทุมวนาราม',
    'บ้านรางโพธิ์',
    'วัดโคกยายเกตุ',
    'พานิชชีวอุปถัมภ์',
    'วัดโภคาราม',
    'วัดคีรีรัตนาราม',
    'วัดสระกร่างเจริญธรรม',
    'บ้านเขากำแพง',
    'วัดห้วยคู้สามัคคี',
    'บ้านโป่งพรานอินทร์',
    'วัดดอนมะเกลือ (นันทราษฎร์รังสรรค์)',
    'วัดจำปา',
    'วัดโพธิ์เขียว',
    'วัดกกม่วง',
    'บ้านสระบัวทอง',
    'บ้านทุ่งดินดำ',
    'วัดศรีสร้อยเพชร',
    'บ้านหัวทำนบ',
    'วัดกลางบ้านดอน',
    'วัดยางสว่างอารมณ์',
    'วัดเขาดีสลัก',
    'วัดคีรีเจริญผล',
    'วัดโพธิ์ทองเจริญ',
    'พลับพลาไชย',
    'บ้านหนองกุฏิ',
    'วัดโบสถ์วิทยาคาร',
    'วัดใหม่สิทธาวาส',
    'บ้านดอนพุทรา',
    'วัดสระพังลาน',
    'วัดดอนสุโข',
    'วัดนันทวัน',
    'หรรษาสุจิตต์วิทยา',
    'วัดบ่อคู่',
    'วัดสระยายโสม',
    'วัดใหม่ปทุมสูตร',
    'วัดคลองตัน',
    'วัดหนองยายทรัพย์',
    'บ้านห้วยหิน',
    'วัดหนองหลุม',
    'บ้านหนองแหน',
    'วัดโคกสำโรง',
    'วัดคอกวัว',
    'วัดคณฑี',
    'บ้านหนองโอ่ง',
    'วัดปทุมสราวาส',
    'วัดเขาพระ',
    'วัดหนองตาสาม',
    'วัดช่องลม',
    'อนุบาลบ้านท่าพระยาจักร',
    'บ่อสุพรรณวิทยา',
    'สองพี่น้องวิทยา',
    'อู่ทองศึกษาลัย',
    'สระยายโสมวิทยา',
    'ดอนคาวิทยา',
    'บรรหารแจ่มใสวิทยา 5',
    'หนองวัลย์เปรียงวิทยา',
    'สระกระโจมโสภณพิทยา',
    'บรรหารแจ่มใสวิทยา 1',
    'อู่ทอง',
    'บางลี่วิทยา',
    'วัดกาบบัว',
    'วัดปากดงท่าศาล',
    'วัดกร่างสามยอด',
    'วัดท่าช้าง',
    'วัดกำมะเชียร',
    'วัดเขาพระ',
    'วัดวังกุลา',
    'วัดหนองกรด',
    'บ้านหนองตาแก้ว',
    'วัดสามเอก',
    'วัดเดิมบาง',
    'วัดท่าเตียน',
    'บ้านแหลมสะแก',
    'วัดคูเมือง',
    'วัดวุ้งสุทธาวาส',
    'วัดกุ่มโคก',
    'วัดขุยโพธิ์',
    'บ้านเขาดิน',
    'วัดนางบวช',
    'วัดวังสำเภาล่ม',
    'วัดบ่อกรุ',
    'วัดทุ่งกฐิน',
    'บ้านลาด',
    'วัดท่าทอง',
    'วัดประชุมสงฆ์',
    'วัดปากน้ำ',
    'พัฒนาปากน้ำ',
    'วัดป่าสะแก',
    'วัดยางนอน',
    'วัดดงพิกุล',
    'วัดท่ามะนาว',
    'วัดหนองกระทุ่ม',
    'บ้านหนองนา',
    'บ้านหนองอิงพิง',
    'บ้านหนองหิน',
    'วัดฉวาก',
    'วัดไทร',
    'บ้านปากดง',
    'วัดหัวเขา',
    'วัดน้ำพุ',
    'วัดดอนตาล',
    'วัดหัวนา',
    'วัดอู่ตะเภา',
    'บ้านพุน้ำร้อน',
    'บ้านโป่งคอม',
    'บ้านหนองผือ',
    'สวนป่าองค์พระ',
    'บ้านวังน้ำเขียว',
    'วัดดอนประดู่',
    'อนุบาลด่านช้าง',
    'วัดด่านช้าง',
    'บ้านทุ่งนาตาปิ่น',
    'วัดหนองเปาะ',
    'บ้านทับกระดาษ',
    'บ้านหนองกระดี่',
    'นิคมสร้างตนเองกระเสียว 1',
    'บ้านรังงาม',
    'วัดวังคัน',
    'วัดทับผึ้งน้อย',
    'บ้านทับละคร',
    'บ้านละว้าวังควาย',
    'บ้านวังยาว',
    'บ้านห้วยหินดำ',
    'บ้านกล้วย',
    'บ้านหนองอุโลก',
    'ไทยรัฐวิทยา 20 (บ้านหนองมะค่าโมง)',
    'บ้านใหม่กิโล 8',
    'วัดราษฎร์ศรัทธาธรรม',
    'บ้านสระบัวก่ำ',
    'นิคมสร้างตนเองกระเสียว 2',
    'บ้านหนองยาว',
    'วัดกกตาด',
    'บ้านพุบ่องป่าขี',
    'วัดกกเต็น',
    'บ้านกกเชียง',
    'บ้านดงเสลา',
    'บ้านทุ่งมะกอก',
    'บ้านไผ่สีทอง',
    'วัดคอกช้าง',
    'วัดท่ามะกรูด',
    'วัดโคกหม้อ',
    'บ้านหนองบัวทอง',
    'วัดบ้านสระ',
    'วัดใหม่สระพลอย',
    'วัดลาดสิงห์',
    'วัดวังหว้า',
    'วัดวังหิน',
    'บ้านหนองแขม',
    'วัดบางขวาก',
    'บ้านทุ่งใหญ่',
    'วัดทุ่งแฝก',
    'วัดนางพิมพ์',
    'วัดวังจิก',
    'วัดสามชุก',
    'วัดคลองขอม',
    'อนุบาลสมเด็จพระวันรัต',
    'วัดบ้านทึง',
    'วัดโป่งแดง',
    'วัดหนองโรง',
    'วัดหนองผักนาก',
    'วัดหนองไผ่',
    'วัดหนองสังข์ทอง',
    'วัดเนินมหาเชษฐ',
    'วัดดอนไร่',
    'วัดหนองสะเดา',
    'วัดสุวรรณตะไล',
    'บ้านดงเชือก',
    'บ้านสระเตย',
    'บ้านแจงงาม',
    'วัดโคกพระ',
    'บ้านทัพตาแทน',
    'บ้านทัพหลวง',
    'บ้านหนองกระถิน',
    'บ้านทุ่งหนองแก้ว',
    'บ้านราษฎร์บำรุง',
    'บ้านหนองแหน',
    'บ้านหนองขาม',
    'บ้านหนองห้าง',
    'บ้านหนองจิกยาว',
    'บ้านใหม่ไร่อ้อย',
    'วัดลำพันบอง',
    'วัดสามัคคีธรรม',
    'บ้านหนองราชวัตร',
    'วัดหนองทราย',
    'บ้านมาบพะยอม',
    'บ้านหนองสำโรง',
    'อนุบาลหนองหญ้าไซ(วัดหนองหลวง)',
    'วัดบัลลังก์',
    'วัดดอนสำโรง',
    'บ้านหนองโก',
    'บรรหารแจ่มใสวิทยา 3',
    'บ่อกรุวิทยา',
    'ทุ่งแฝกพิทยาคม',
    'ทุ่งคลีโคกช้างวิทยา',
    'บรรหารแจ่มใสวิทยา 6',
    'บรรหารแจ่มใสวิทยา 7',
    'หนองหญ้าไซวิทยา',
    'ธรรมโชติศึกษาลัย',
    'สามชุกรัตนโภคาราม',
    'ด่านช้างวิทยา',
    'วัดดอนยายหอม',
    'วัดดอนขนาก',
    'วัดตาก้อง',
    'บ้านมาบแค',
    'วัดเกาะวังไทร',
    'วัดทัพหลวง',
    'วัดม่วงตารศ',
    'วัดทุ่งรี',
    'บ้านทุ่งน้อย',
    'บ้านลำท่าโพ',
    'วัดธรรมศาลา',
    'บ้านต้นสำโรง',
    'บ้านนาสร้าง',
    'บ้านบ่อพลับ',
    'หลวงพ่อแช่มวัดตาก้องอนุสรณ์',
    'วัดดอนเสาเกียด',
    'วัดบางแขม',
    'วัดลาดปลาเค้า',
    'บ้านคลองยาง',
    'วัดบ้านยาง',
    'บ้านหนองกะโดน',
    'บ้านคอวัง',
    'วัดพระปฐมเจดีย์',
    'อนุบาลนครปฐม',
    'วัดไผ่ล้อม',
    'วัดพระประโทณเจดีย์',
    'วัดหว้าเอน',
    'วัดโพรงมะเดื่อ',
    'วัดหุบรัก',
    'วัดศรีวิสารวาจา',
    'วัดพะเนียงแตก',
    'บ้านลำพยา',
    'บ้านหนองหิน',
    'วัดวังตะกู',
    'วัดวังเย็น',
    'บ้านไร่ต้นสำโรง',
    'บ้านหนองขาหยั่ง',
    'วัดใหม่ดอนทราย',
    'วัดสระกะเทียม',
    'วัดลาดหญ้าแพรก',
    'วัดรางปลาหมอ',
    'วัดใหม่ห้วยลึก',
    'วัดสามควายเผือก',
    'บ้านรางมะเดื่อ',
    'บ้านหนองงูเหลือม',
    'บ้านหนองแก',
    'วัดหนองดินแดง',
    'วัดหนองเสือ',
    'บ้านทุ่งหัวพรหม',
    'วัดทัพยายท้าว',
    'บ้านหนองปากโลง',
    'วัดโพธิ์งาม',
    'วัดทะเลบก',
    'ประถมฐานบินกำแพงแสน',
    'อนุบาลกำแพงแสน',
    'บ้านทุ่งขี้อ้าย',
    'วัดทุ่งกระพังโหม',
    'วัดบ่อน้ำจืด',
    'บ้านดอนทอง',
    'วัดสระพัง',
    'วัดหนองปลาไหล',
    'อินทรศักดิ์ศึกษาลัย(บ้านยาง)',
    'บ้านห้วยขวาง',
    'บ้านรางอีเม้ย',
    'บ้านอ้อกระทิง',
    'วัดสองห้อง',
    'วัดลาดหญ้าไทร',
    'บ้านสระน้ำส้ม',
    'บ้านหลักเมตร',
    'วัดปลักไม้ลาย',
    'บ้านหนองขาม',
    'บ้านบ่อน้ำพุ',
    'เมืองเก่ากำแพงแสน',
    'วัดนิยมธรรมวราราม',
    'บ้านหนองไม้งาม',
    'วัดหนองศาลา',
    'บ้านหนองกร่าง',
    'บ้านห้วยรางเกตุ',
    'บ้านห้วยปลากด',
    'วัดหนองจิก',
    'วัดศาลาตึกสิทธิชัยวิศาล',
    'บ้านอ้อกระทุง',
    'วัดห้วยผักชี',
    'วัดประชาราษฎร์บำรุง',
    'บ้านห้วยด้วน',
    'บ้านไผ่โน้ม',
    'บ้านดอนซาก',
    'บ้านคลองตัน',
    'วัดวังน้ำเขียว',
    'บ้านหนองพงนก',
    'วัดสระสี่มุม',
    'บ้านสามัคคี',
    'วัดราษฎร์วราราม',
    'บ้านหนองพงเล็ก',
    'วัดดอนเตาอิฐ',
    'บ้านหนองเขมร',
    'วัดหนองกระทุ่ม',
    'วัดห้วยม่วง',
    'วัดปทุมทองสุทธาราม',
    'วัดท่าเสา',
    'บ้านบัวแดง',
    'วัดกำแพงแสน',
    'บ้านหนองโสน',
    'วัดหนองโพธิ์',
    'บ้านปากหว้า',
    'บ้านหนองบอน',
    'ละเอียดอุปถัมภ์',
    'บ้านหัวถนน',
    'วัดสระสี่เหลี่ยม',
    'วัดสุขวราราม',
    'วัดบ้านหลวง',
    'วัดหนองกระพี้',
    'วัดลำลูกบัว',
    'บ้านแหลมกะเจา',
    'วัดลำเหย',
    'วัดทุ่งสีหลง',
    'บ้านใหม่',
    'บ้านรางมูก',
    'วัดแหลมมะเกลือ',
    'วัดตะโกสูง',
    'บ้านดอนกลาง',
    'วัดสามง่าม',
    'วัดทุ่งผักกูด',
    'วัดกงลาด',
    'วัดห้วยพระ',
    'บ้านแจงงาม',
    'วัดเลาเต่า',
    'วัดทุ่งพิชัย',
    'บ้านสามแก้ว',
    'พระปฐมวิทยาลัย',
    'ราชินีบูรณะ',
    'พระปฐมวิทยาลัย2หลวงพ่อเงินอนุสรณ์',
    'ศรีวิชัยวิทยา',
    'สระกะเทียมวิทยาคม',
    'วัดห้วยจรเข้วิทยาคม',
    'โพรงมะเดื่อวิทยาคม',
    'สิรินธรราชวิทยาลัย',
    'กำแพงแสนวิทยา',
    'มัธยมฐานบินกำแพงแสน',
    'ศาลาตึกวิทยา',
    'คงทองวิทยา',
    'บ้านหลวงวิทยา',
    'วัดประชานาถ',
    'วัดสว่างอารมณ์',
    'วัดโคกพระเจดีย์',
    'วัดงิ้วราย',
    'บ้านคลองบางกระจัน',
    'วัดไทร',
    'คลองทางหลวง',
    'วัดท่าตำหนัก',
    'วัดน้อย',
    'วัดไทยาวาส',
    'วัดกลางบางแก้ว',
    'วัดตุ๊กตา',
    'วัดบางพระ',
    'วัดกลาง',
    'วัดบ่อตะกั่ว',
    'วัดห้วยตะโก',
    'วัดลานตากฟ้า',
    'วัดพุทธธรรมรังษี',
    'วัดสัมปทวน',
    'วัดละมุด',
    'บ้านลานแหลม',
    'วัดใหม่สุคนธาราม',
    'วัดสำโรง',
    'วัดศรีมหาโพธิ',
    'วัดศีรษะทอง',
    'วัดเสถียรรัตนาราม',
    'วัดกลางครูเวียง',
    'วัดกกตาล',
    'วัดห้วยพลู',
    'บ้านห้วยพลู',
    'วัดท้องไทร',
    'บ้านห้วยกรด',
    'วัดโคกเขมา',
    'วัดทุ่งน้อย',
    'วัดบางภาษี',
    'บ้านคลองนกกระทุง',
    'วัดลานคา',
    'วัดดอนยอ',
    'วัดลาดสะแก',
    'วัดไผ่สามตำลึง',
    'วัดโพธิ์',
    'วัดไผ่จรเข้',
    'วัดเกษตราราม',
    'วัดพระมอพิสัย',
    'บ้านนราภิรมย์',
    'วัดสว่างอารมณ์',
    'วัดนราภิรมย์',
    'วัดนิลเพชร',
    'วัดบัวปากท่า',
    'บ้านหนองปรงกาญจนา',
    'วัดบอนใหญ่',
    'ตลาดเจริญสุข',
    'วัดบัวหวั่น',
    'วัดบางไผ่นารถ',
    'วัดผาสุการาม',
    'วัดบางปลา',
    'วัดเกาะแรต',
    'ตลาดเกาะแรต',
    'วัดรางกำหยาด',
    'ตลาดรางกระทุ่ม',
    'บ้านรางกระทุ่ม',
    'บ้านรางปลาหมอ',
    'บ้านคลองพระมอพิสัย',
    'วัดบึงลาดสวาย',
    'วัดสุขวัฒนาราม',
    'ไทยรัฐวิทยา 4',
    'บ้านหนองปรง',
    'วัดเกษมสุริยัมนาจ',
    'บ้านประตูน้ำพระพิมล',
    'บ้านบางเลน',
    'บ้านไผ่คอกวัว',
    'วัดบางหลวง',
    'วัดบางน้อยใน',
    'วัดราษฎร์สามัคคี',
    'วัดลัฏฐิวนาราม',
    'บ้านลาดหลวง',
    'บ้านหนองมะม่วง',
    'บ้านไผ่ล้อม',
    'วัดไผ่หูช้าง',
    'วัดลำพญา',
    'วัดเวฬุวนาราม',
    'วัดศิลามูล',
    'บ้านไผ่หลวง',
    'บ้านกระทุ่มล้ม',
    'บ้านเพลินวัฒนา',
    'วัดปรีดาราม',
    'บ้านคลองจินดา',
    'บ้านพาดหมอน',
    'วัดวังน้ำขาว',
    'บ้านฉาง',
    'วัดบางช้างเหนือ',
    'บ้านดอนทอง',
    'บ้านคลองใหม่',
    'วัดราษฏร์ศรัทธาราม',
    'หลวงพ่อแช่มอุปถัมภ์',
    'บ้านตากแดด',
    'วัดจินดาราม',
    'วัดทรงคนอง',
    'วัดท่าข้าม',
    'บ้านท่าตลาด',
    'วัดดอนหวาย',
    'คลองบางกระทึก',
    'บ้านหัวอ่าว',
    'วัดเชิงเลน',
    'วัดบางช้างใต้',
    'บ้านบางม่วง',
    'บ้านบางเตย',
    'วัดเทียนดัด',
    'บ้านบางประแดง',
    'วัดสรรเพชญ',
    'วัดเดชานุสรณ์',
    'วัดไร่ขิง',
    'วัดท่าพูด',
    'บ้านสามพราน',
    'บ้านดงเกตุ',
    'วัดหอมเกร็ด',
    'บ้านหอมเกร็ด',
    'วัดอ้อมใหญ่',
    'บ้านคลองลัดอ้อมใหญ่',
    'บุณยศรีสวัสดิ์',
    'บ้านคลองสว่างอารมณ์',
    'วัดมะเกลือ',
    'บ้านคลองโยง',
    'บ้านคลองมหาสวัสดิ์',
    'วัดสาลวัน',
    'วัดสุวรรณาราม',
    'พระตำหนักสวนกุหลาบ มหามงคล',
    'งิ้วรายบุญมีรังสฤษดิ์',
    'ภัทรญาณวิทยา',
    'ปรกแก้ววิทยา',
    'พลอยจาตุรจินดา',
    'แหลมบัววิทยา',
    'เพิ่มวิทยา',
    'โสตศึกษาจังหวัดนครปฐม',
    'บางเลนวิทยา',
    'บางหลวงวิทยา',
    'สถาพรวิทยา',
    'บัวปากท่าวิทยา',
    'สามพรานวิทยา',
    'ภ.ป.ร.ราชวิทยาลัย',
    'วัดไร่ขิงวิทยา',
    'ปรีดารามวิทยาคม',
    'รัตนโกสินทร์สมโภชบวรนิเวศศาลายา',
    'กาญจนาภิเษกวิทยาลัย นครปฐม',
    'วัดกาหลง(พึ่งสุนทรอุทิศ)',
    'วัดนาขวาง',
    'บ้านชายทะเลกาหลง',
    'วัดราษฎร์รังสรรค์',
    'วัดโสภณาราม(ปลั่งร่วมราษฎร์บํารุง)',
    'วัดโคกขาม(นรสิงห์อนุสรณ์)',
    'บ้านสันดาบ',
    'บ้านชายทะเลโคกขามมิตรภาพที่ 95',
    'สหกรณ์นิคมเกลือ',
    'วัดชัยมงคล',
    'วัดน่วมกานนท์',
    'วัดปากบ่อ(เหลืองราษฎร์บํารุง)',
    'วัดชีผ้าขาว',
    'หลวงแพทย์โกศลอุปถัมภ์',
    'วัดใหญ่จอมปราสาท',
    'บ้านท่าทราย',
    'วัดศรีเมือง',
    'บ้านยกกระบัตร',
    'วัดคลองครุ',
    'บ้านนาโคก(นาเกลือสัมพันธ์)',
    'วัดปัจจันตาราม',
    'วัดราษฎร์ธรรมาราม',
    'วัดบางปิ้ง(บริษัทเกลือไทยสงเคราะห์)',
    'บ้านบางปิ้ง',
    'เทพนรรัตน์',
    'วัดศรีวนาราม',
    'วัดเจริญสุขาราม',
    'บ้านชายทะเลบางกระเจ้า',
    'วัดบางกระเจ้า',
    'วัดวิสุทธาราม',
    'บ้านคลองซื่อ',
    'เมืองสมุทรสาคร',
    'วัดบางพลี',
    'วัดบางนํ้าวน',
    'วัดเกตุมดีศรีวราราม',
    'บ้านหนองหาดใหญ่',
    'บ้านบางนํ้าจืด',
    'วัดโพธิ์แจ้',
    'วัดสามัคคีศรัทธาราม',
    'วัดศรีสุทธาราม',
    'วัดบางหญ้าแพรก',
    'สมุทรมณีรัตน์',
    'วัดบางปลา',
    'บ้านอ้อมโรงหีบ',
    'วัดพันธุวงษ์',
    'วัดศิริมงคล',
    'วัดเกาะ(สําลีราษฎร์อุปถัมภ์)',
    'วัดใหญ่บ้านบ่อ(บ้านบ่อราษฎร์บํารุง)',
    'วัดใต้บ้านบ่อ',
    'วัดบางขุด',
    'วัดกระซ้าขาว',
    'สหกรณ์กสิกรรมชายทะเล',
    'บ้านโคก',
    'วัดบ้านไร่(ประชานุกูล)',
    'เอกชัย',
    'วัดป้อมวิเชียรโชติการาม',
    'อนุบาลสมุทรสาคร',
    'บ้านคลองกระทุ่มแบน',
    'วัดราษฎร์บํารุง',
    'บ้านแคราย(เกษตรพันธุ์พิทยาคาร)',
    'วัดสุวรรณรัตนาราม',
    'บ้านดอนไก่ดี',
    'วัดศรีสําราญราษฎร์บํารุง',
    'บ้านปล่องเหลี่ยม',
    'วัดนางสาว',
    'บ้านท้องคุ้ง',
    'บ้านปลายคลองน้อย',
    'วัดท่าเสา',
    'วัดอ่างทอง',
    'วัดท่ากระบือ(ท่ากระบือพิทยาคาร)',
    'วัดบางยาง(บางยางพิทยาคาร)',
    'บ้านสวนหลวง',
    'วัดใหม่หนองพะอง',
    'วัดหนองนกไข่',
    'วังนกไข่',
    'บ้านคลองแค',
    'วัดอ้อมน้อย(มิตรครูราษฎร์รังสรรค์)',
    'วัดคลองตันราษฎร์บํารุง',
    'บ้านคลองตัน',
    'บ้านรางสายบัว',
    'วัดเจ็ดริ้ว',
    'วัดธรรมเจดีย์ศรีพิพัฒน์',
    'ไทยรัฐวิทยา 9 (วัดใหม่ราษฎร์นุกูล)',
    'วัดยกกระบัตร',
    'วัดหลักสี่ราษฎร์สโมสร',
    'ทํานบแพ้ว',
    'วัดโรงเข้',
    'หลวงสินธุ์ราษฎร์รังสฤษฏ์',
    'วัดธรรมโชติ',
    'บ้านโรงเข้',
    'วัดสวนส้ม(ชื่นสินอนุสรณ์)',
    'บ้านดําเนินสะดวก(กิมลี้มณีฉาย)',
    'บ้านคลองสําโรง',
    'วัดดอนโฆสิตาราม',
    'วัดหนองบัว',
    'บ้านดอนไผ่(อุดม-สอางค์อุ่นสุวรรณ)',
    'วัดหนองสองห้อง',
    'บ้านกลางมิตรภาพที่ 63',
    'วัดกระโจมทอง',
    'วัดหลักสองราษฎร์บํารุง',
    'อนุบาลบ้านแพ้ว(วันครู2500)',
    'บ้านวังจรเข้',
    'วัดราษฎร์ศรัทธากะยาราม',
    'บ้านเจริญสุข',
    'วัดสุนทรสถิต(สามัคคีวิทยาคม)',
    'วัดธัญญารามราษฎร์บํารุง',
    'บ้านคลองหลวง',
    'สมุทรสาครวิทยาลัย',
    'สมุทรสาครบูรณะ',
    'เฉลิมพระเกียรติสมเด็จพระศรีนครินทร์ สมุทสาคร',
    'พันท้ายนรสิงห์วิทยา',
    'สมุทรสาครวุฒิชัย',
    'กระทุ่มแบน (วิเศษสมุทคุณ)',
    'อ้อมน้อยโสภณชนูปถัมภ์',
    'กุศลวิทยา',
    'วัดธรรมจริยาภิรมย์',
    'หลักสองส่งเสริมวิทยา',
    'วัดหลักสี่พิพัฒน์ราษฎร์อุปถัมภ์',
    'วัดดาวโด่ง(เอิบวิทยาคม)',
    'วัดคลองโคน',
    'วัดธรรมประสิทธิ์',
    'วัดท้ายหาด(พลอยเจียเส็ง)',
    'บ้านคลองบางกก',
    'วัดธรรมาวุธาราม(ประชารัฐสมุทรการอุปถัมภ์)',
    'วัดลาดเป้ง',
    'ไทยรัฐวิทยา 70(บ้านบางแก้ว)',
    'บ้านบางบ่อ',
    'วัดศรีสุวรรณคงคาราม(ราษฎร์สงเคราะห์)',
    'วัดบางขันแตก(บำเพ็ญอยู่รังสฤษดิ์)',
    'วัดธรรมสถิติ์วราราม(ธรรมวิถีสามัคคีราษฎร์อุทิศ)',
    'วัดนางพิมพ์(สมกิตติราษฎร์บำรุง)',
    'วัดศรัทธาธรรม',
    'วัดบางจะเกร็ง(ประโชตประชานุกูล)',
    'บ้านฉู่ฉี่',
    'วัดคู้สนามจันทร์(สามัคคีราษฎร์รังสรรค์)',
    'วัดจันทร์เจริญสุข',
    'วัดช่องลม(ธรรมโชติ)',
    'วัดโรงธรรม(มิตรภาพที่ 70)',
    'วัดปากลัด(ผลาคารบำรุงวิทย์)',
    'วัดคู้สนามจันทร์สาขาวัดศรีศรัทธาธรรม',
    'เมืองสมุทรสงคราม',
    'อนุบาลสมุทรสงคราม',
    'บ้านลาดใหญ่สามัคคี',
    'วัดน้อยแสงจันทร์(จันทร์ศิริวิทยาคาร)',
    'วัดบางประจันต์(หวังเจริญราษฎร์นุกูล)',
    'วัดสวนแก้ว',
    'บ้านเขตเมือง',
    'บ้านตะวันจาก',
    'บ้านลาดใหญ่',
    'วัดปากสมุทรสาขาบ้านคลองสองร่อง',
    'วัดปากสมุทร(ไวยวุฒิชนานุสรณ์)',
    'อนุบาลบางคนที(วัดเกาะใหญ่เลี้ยงล้อมอนุกูล)',
    'วัดปากง่าม(ผ่องสะอาดราษฎร์นุกูล)',
    'วัดบางใหญ่(ประภัสสรประชานุกูล)',
    'วัดบางน้อย(แจ่มประชานุกูล)',
    'บ้านรางห้าตำลึง(สายบุญเรืองราษฎร์รังสรรค์)',
    'ศาลจ้าวอาม้า',
    'วัดดอนมะโนรา(รังสิยานุกูล)',
    'วัดตะโหนดราย(พรหมสวัสดิ์สาทร)',
    'วัดตรีจินดาวัฒนาราม(เฉลิมรัฐราษฎร์นุกูล)',
    'วัดบางคนทีใน(วามโกประชานุกูล)',
    'วัดเจริญสุขาราม(วัฒนานุวรรัตน์)',
    'วัดบางพลับ(สุนทรานุกูล)',
    'วัดแก่นจันทน์(จันทร์ เอียง ทับทิมราษฎร์บำรุง)',
    'คลองตาจ่า(คงจีบอุทิศ)',
    'ศาลแม่อากาศ(โรจน์มณี)',
    'วัดบางสะแก(ล้อมประชานุกูล)',
    'วัดปราโมทย์(พิศสะอาดราษฎร์อำรุง)',
    'บ้านยายแพง(พิศผ่องประชานุสรณ์)',
    'วัดเกตการามสาขาวัดบางกล้วย',
    'วัดเกตการาม(พรหมสุวัฒน์วิทยาคาร)',
    'วัดปากน้ำ(อมรวิมลจันทร์)',
    'วัดเทพประสิทธิ์คณาวาส(ฉ่ำบุญรอดชนูทิศ)',
    'วัดมณีสรรค์(ไสวประชานุสรณ์)',
    'วัดบางแคกลาง(ไพลประชานุกูล)',
    'วัดปรกสุธรรมาราม(สามัคคีพิทยาคาร)',
    'วัดสาธุชนาราม(สินประชานุกูล)',
    'วัดจุฬามณี(มาลยานุสาวรีย์)',
    'วัดช้างเผือก(วัฒนคชาราษฎร์ศึกษา)',
    'อนุบาลอัมพวา(ไวยประชานุกูล)',
    'บ้านบางนางลี่(กมลราษฎร์สามัคคี)',
    'วัดประชาโฆสิตาราม(สมบูรณ์ตาบอุทิศวิทยา)',
    'วัดสี่แยกราษฎร์บำรุง',
    'วัดอมรวดี(อมรวิทยาคาร)',
    'วัดโคกเกตุ(เสริมสมบูรณ์วงศ์)',
    'บ้านคลองสมบูรณ์',
    'บ้านแพรกหนามแดง',
    'บ้านต้นลำแพน',
    'บ้านดอนจั่น(วาสน์เจริญเรือนอุทิศ)',
    'วัดเขายี่สาร(ประมุขเวชกิจ)',
    'จตุวัฎมหาราชานุสรณ์',
    'วัดช่องลมวรรณาราม(สมุทรประชานุกูล)',
    'วัดประดู่(ประดู่ดิลกศิลป์)',
    'วัดบางนางลี่ใหญ่(บริษัทเกลือไทยสงเคราะห์)',
    'วัดวรภูมิ(วรไวทยาวาส)',
    'วัดเสด็จ(สุทธิวิทยานุสรณ์)',
    'วัดแก้วเจริญ(เฟื้อบำรุง)',
    'ศรัทธาสมุทร',
    'ถาวรานุกูล',
    'ท้ายหาด',
    'เมธีชุณหะวัณวิทยาลัย',
    'สกลวิสุทธิ',
    'อัมพวันวิทยาลัย',
    'วัดบางกะพ้อม(คงลาภยิ่งประชานุสรณ์)',
    'เทพสุวรรณชาญวิทยา',
    'วัดแก้วเจริญอำนวยวิทย์',
    'อนุบาลเพชรบุรี',
    'วัดนาค (แบ่งอุทิศวิทยาคาร)',
    'บ้านพี่เลี้ยง (อิ่มอุทิศ)',
    'วัดพระรูป (จันทศรีสงเคราะห์)',
    'วัดหนองไม้เหลือง (ธรรมโชติประชาธร)',
    'วัดถิ่นปุรา (เดชสมบูรณ์)',
    'บ้านดอนยาง',
    'บ้านหนองมะขาม',
    'วัดหนองควง (ปริยัติพุทธสรราษฎร์นุเคราะห์)',
    'วัดอินทาราม',
    'วัดนาพรม (มนมหาวิริยาคาร)',
    'บ้านดอนมะขามช้าง (ใจราษฎร์อุปถัมภ์)',
    'วัดไสกระดาน',
    'วัดทองนพคุณ (เจริญราษฎร์วิทยาคาร)',
    'วัดเจริญศรีมณีผล (สุวรรณไวประชานุกูล)',
    'วัดโพธิ์ทัยมณี (ศิริพรรณพิทยา)',
    'วัดบันไดทอง (วิเศษศึกษา)',
    'วัดขุนตรา',
    'วัดดอนไก่เตี้ย',
    'วัดโพพระใน (รุ่งรังสฤษฎ์)',
    'บ้านบางพรม (คุรุราษฎร์บำรุง)',
    'วัดเพรียง (วิเทศราษฎร์วัฒนา)',
    'บ้านบ่อหวาย',
    'บ้านไร่ดอน (อินทโชติวิริยานุสรณ์)',
    'วัดโรงเข้ (พุฒประชาสรรค์)',
    'วังตะโก (ช้องประชาพิทักษ์)',
    'วัดเวียงคอย (สว่างแสงวิทยา)',
    'วัดใหม่ท่าศิริ (สุพรรณวิทยาคาร)',
    'วัดลาดโพธิ์ (รวมราษฎร์วิทยาคาร)',
    'วัดหนองหว้า (เสธิยสาธร)',
    'บ้านบ่อโพง',
    'บ้านดอนนาลุ่ม',
    'บ้านโตนดน้อย',
    'บ้านบ่อขม',
    'บ้านหนองพลับ (อินทจิตบำรุง)',
    'บ้านดอนยี่กรอก',
    'วัดสิงห์ (ขจิตพุทธสรศึกษา)',
    'วัดเขมาภิรัติการาม (เกษมราษฎร์บำรุง)',
    'บ้านบางกุฬา',
    'หาดเจ้าสำราญ (พรหมานุกูล)',
    'วัดบางทะลุ (สุนทรธรรมานุสรณ์)',
    'บ้านเขาย้อย',
    'วัดท้ายตลาด (วิวัฒน์เกษนราประชานุกูล)',
    'วัดยาง (เย็นประชาสรรค์)',
    'บ้านวัง',
    'วัดดอนทราย (สัมฤทธิ์ราษฎร์นุเคราะห์)',
    'วัดเทพประชุมนิมิตร (สาครราษฎร์บำรุง)',
    'วัดโพธิ์ (ชื่นศรีผดุง)',
    'วัดกุฎิ (ชุ่มประชารังสรรค์)',
    'วัดหนองส้ม',
    'บ้านสระพัง',
    'บ้านหนองชุมพล',
    'บ้านหนองประดู่',
    'ไทยรัฐวิทยา 13 (บ้านพุม่วง)',
    'วัดทรงธรรม',
    'บ้านเนินรัก',
    'วัดมณีเลื่อน',
    'บ้านคีรีวงศ์',
    'วัดหนองปรง (บุญมานุสรณ์)',
    'วัดเขาสมอระบัง',
    'วัดหนองปลาไหล (วชิรราษฎร์วิทยา)',
    'วัดเวฬุวนาราม',
    'วัดกุญชรวชิราราม (พัฒนาวิเทศประชาสรรค์)',
    'วัดห้วยโรง (หนึ่งนฤมิตรพิทยาคาร)',
    'บ้านท่าตะคร้อมิตรภาพที่ 192 (อนุสรณ์แด่พลเอกกฤษณ์',
    'บ้านหนองรี',
    'บ้านสามเรือน',
    'บ้านพุพลู',
    'บ้านยางน้ำกลัดใต้',
    'บ้านปากรัตน์',
    'บ้านท่าเสลา',
    'บ้านลิ้นช้าง',
    'บ้านยางน้ำกลัดเหนือ',
    'บ้านจะโปรง',
    'บ้านหนองไผ่',
    'บ้านไทรงาม',
    'บ้านอ่างศิลา',
    'บ้านหนองหญ้าปล้อง (อุดมวนา)',
    'บ้านท่าแร้ง (สหราษฎร์)',
    'บ้านคลองมอญ',
    'วัดไทรทอง (สาครราษฎร์สงเคราะห์)',
    'วัดกุฎิ (นันทวิเทศประชาสรรค์)',
    'บ้านเหมืองไทร',
    'บ้านบางแก้ว (สำนักงานสลากกินแบ่งสงเคราะห์ 44)',
    'วัดราษฎร์ศรัทธา',
    'วัดบางขุนไทร (ผดุงวิทยา)',
    'วัดดอนผิงแดด (ผลสิริราษฎร์รังสรรค์)',
    'วัดปากคลอง (ศุทยาลัยอุทิศ)',
    'วัดเขาตะเครา (ประสพประชาสรรค์)',
    'บ้านทุ่งเฟื้อ (เอมสะอาดอำนวย)',
    'วัดบางลำภู',
    'บ้านบางหอ (โสภณเชาวราษฎร์รังสรรค์)',
    'วัดคุ้งตำหนัก',
    'บ้านสามแพรก',
    'วัดลัด (บางตะบูนวิทยาคาร)',
    'วัดเกาะแก้ว',
    'วัดปากอ่าว (ญาณสาครวิทยาคาร)',
    'วัดต้นสน(บุญมีโชติวิทยา)',
    'บ้านแหลม',
    'วัดในกลาง',
    'วัดอุตมิงค์ (จันทสิริราษฎร์รังสรรค์)',
    'วัดลักษณาราม (สมุทรราษฎร์วิทยาคาร)',
    'มิตรภาพที่ 34',
    'วัดนอกปากทะเล',
    'บ้านดอนมะขาม',
    'วัดสมุทรโคดม',
    'วัดสมุทรธาราม',
    'พรหมานุสรณ์จังหวัดเพชรบุรี',
    'เบญจมเทพอุทิศจังหวัดเพชรบุรี',
    'บางจานวิทยา',
    'วัดจันทราวาส(ศุขประสารราษฎร์)',
    'คงคาราม',
    'ดอนยางวิทยา',
    'โยธินบูรณะเพชรบุรี',
    'เขาย้อยวิทยา',
    'หนองหญ้าปล้องวิทยา',
    'วชิรธรรมโศภิต',
    'บางตะบูนวิทยา',
    'บ้านแหลมวิทยา',
    'นิคมสร้างตนเองเขื่อนเพชร (พิบูลย์สงเคราะห์)',
    'บ้านหุบกะพง',
    'บ้านหนองยาว (ราษฎร์สามัคคีรังสรรค์)',
    'ชาวไร่',
    'บ้านร่องระกำ',
    'บ้านหนองเผาถ่าน (ประชาประสิทธิ์)',
    'บ้านดอนขุนห้วย',
    'บ้านนายาง (วิเทศปริยัติราษฎร์รังสรรค์)',
    'บ้านดอน',
    'บ้านเนินทราย',
    'บ้านบางเกตุ',
    'บ้านบางเก่า',
    'วัดโตนดหลวง (สุขประสิทธิ์วิทยา)',
    'บ้านทุ่งขาม',
    'บ้านโป่งแย้',
    'บ้านไร่ใหม่พัฒนา',
    'บ้านรางจิก',
    'บ้านหนองเขื่อน',
    'วัดช้างแทงกระจาด (จันทร์จำนง)',
    'บ้านดอนมะกอก',
    'บ้านอ่างหิน',
    'บ้านบ่อไร่ (ร่วมจิตอนุกูล)',
    'วัดหนองศาลา (ธรรมกรประสาท)',
    'บ้านบ่อหลวง',
    'บ้านพุหวาย',
    'บ้านหนองขาม',
    'บ้านท่ามะริด',
    'บ้านทุ่งโป่ง',
    'บ้านโพรงเข้',
    'บ้านยางชุม',
    'บ้านสารเห็ด',
    'บ้านหนองเขาอ่อน',
    'บ้านเขากระปุก',
    'บ้านโป่งเกตุ',
    'บ้านหนองคอไก่',
    'บ้านหนองตาฉาว',
    'บ้านเขาอ่างแก้ว',
    'บ้านหนองโรง',
    'บ้านสระพระ',
    'บ้านหนองขานาง',
    'เขื่อนเพชร (ชลประทานสงเคราะห์)',
    'วัดเขื่อนเพชร (วชิรเวทประชาสรรค์)',
    'วัดท่าขาม (สะเทื้อนราษฎร์อุปถัมภ์)',
    'วัดท่าคอย (สกุณอุปถัมภ์)',
    'บ้านท่าไม้รวก',
    'บ้านท่าลาว (พัชรศาสน์ราษฎร์นุเคราะห์)',
    'บ้านหนองชุมแสง (พัชรศาสน์ราษฎร์นุเคราะห์)',
    'บ้านหนองเตียน',
    'วัดพระพุทธบาทเขาลูกช้าง (พิพิธพัฒนานุเคราะห์)',
    'บ้านท่ากระเทียม',
    'บ้านท่ายาง (ประชาสรรค์)',
    'บ้านหนองแขม',
    'วัดเขากระจิว มิตรภาพที่ 103',
    'บ้านหนองบ้วย',
    'วัดไสค้าน',
    'สหกรณ์บำรุงวิทย์',
    'บ้านหนองน้ำถ่าย',
    'บ้านห้วยตะแกะ',
    'วัดเขาปากช่อง',
    'วัดท่าเหว',
    'บ้านในดง',
    'บ้านดอนเตาอิฐ',
    'วัดตาลกง (ธรรมศรียาทร)',
    'วัดมาบปลาเค้า (ธรรมโชติผดุง)',
    'บ้านท่าโล้',
    'วัดวังไคร้',
    'บ้านท่าหัวลบ',
    'บ้านแม่ประจันต์',
    'บ้านหนองเกตุ',
    'บ้านหันตะเภา',
    'วัดหนองบัว',
    'วัดหนองจอก (ศรีสรรค์พานิช)',
    'วัดศาลาเขื่อน (อุภัยเพชร)',
    'วัดอินจำปา (มงคลคุรุราษฎร์)',
    'วัดม่วงงาม',
    'วัดถ้ำรงค์ (ผ่านผดุง)',
    'วัดโพธิ์กรุ (ผินผดุง)',
    'วัดเขาทะโมน (เรือนวงศ์ทองศรี)',
    'วัดหาดทราย (เรือนมั่นนิรมิต)',
    'วัดท่าศาลาราม (เรือนบุญนาคาลัย)',
    'วัดจันทาราม (เติมไลราษฎร์อนุเคราะห์)',
    'บ้านดงห้วยหลวง',
    'วัดลาดศรัทธาราม (สำนักงานสลากกินแบ่งสงเคราะห์ 128)',
    'วัดกุ่ม (เรือนพูนพิทยา)',
    'วัดหนองกาทอง (ม่วงนิรมิตร)',
    'วัดดอนกอก (ล้วนประชาสรรค์)',
    'บ้านแหลมทอง',
    'บ้านไร่โคก (อินทร์ประชาสรรค์)',
    'บ้านไร่ถิ่นน้อย (อาจอำรุง)',
    'บ้านหนองไก่เถื่อน',
    'วัดบ่อบุญ (จารุราษฎร์บำรุง)',
    'วัดวังบัว (ศรสายราษฎร์บำรุง)',
    'บ้านลาดโพธิ์ (สุทธิราษฎร์รังสรรค์)',
    'วัดห้วยเสือ (มงคลประชาสรรค์)',
    'วัดท่าไชยศิริ (เหลื่อมวิทยาคาร)',
    'วัดหว้า',
    'วัดช่อม่วง',
    'บ้านซ่อง (ประชากรบำรุง)',
    'บ้านโป่งสลอด',
    'วัดโพธิ์ลอย (เรือนวงศ์ผดุง)',
    'บ้านหนองโสน (รัฐราษฎร์บำรุง)',
    'บ้านพุตุม',
    'บ้านหนองจอก',
    'วัดหนองแก (กล่อมวิทยาคาร)',
    'ชลประทานแก่งกระจาน',
    'บ้านวังนางนวล',
    'บ้านตะเคียนงาม',
    'บ้านน้ำทรัพย์',
    'บ้านต้นเกด',
    'บ้านแม่คะเมย',
    'บ้านพุเข็ม',
    'บ้านเขากลิ้ง',
    'บ้านหนองหงษ์',
    'บ้านท่าเรือ (ประสาทอนุสรณ์)',
    'บ้านป่าเด็ง',
    'บ้านมะขามโพรง',
    'บ้านห้วยกวางจริง',
    'บ้านหนองมะกอก',
    'บ้านหนองสะแก',
    'บ้านซ่อง',
    'บ้านทุ่งเคล็ด',
    'อ.ฮ.ลิงค์ สาขาบ้านท่าลิงลม',
    'บ้านห้วยปลาดุก',
    'อ.ฮ.ลิงค์ (ในพระอุปถัมภ์สมเด็จพระศรีนครินทราบรมราช',
    'บ้านสองพี่น้อง (สายปัญญาสมาคมในพระฯ)',
    'บ้านหนองปืนแตก',
    'บ้านด่านโง',
    'บ้านห้วยไผ่',
    'บ้านพุสวรรค์',
    'โตนดหลวงวิทยา',
    'ห้วยทรายประชาสรรค์',
    'จุฬาภรณราชวิทยาลัย เพชรบุรี',
    'ชะอำคุณหญิงเนื่องบุรี',
    'ศึกษาสงเคราะห์เพชรบุรี',
    'หนองจอกวิทยา',
    'หนองชุมแสงวิทยา',
    'ท่ายางวิทยา',
    'บ้านลาดวิทยา',
    'แก่งกระจานวิทยา',
    'ป่าเด็งวิทยา',
    'เพชรบุรีปัญญานุกูล',
    'บ้านหนองกก',
    'บ้านทุ่งเคล็ด',
    'ด่านสิงขร',
    'อุดมราชภักดี',
    'บ้านทางหวาย',
    'บ้านหนองหิน',
    'บ้านคลองวาฬ',
    'บ้านหว้ากอ (มิตรภาพที่72)',
    'บ้านห้วยใหญ่',
    'เศรษฐพานิชวิทยา',
    'พัฒนาประชานุเคราะห์',
    'บ้านห้วยน้ำพุ',
    'ประชารังสรรค์',
    'วัดปากคลองเกลียว',
    'บ้านท่าฝาง',
    'วัดบ่อนอก(ทองวิทยา)',
    'บ้านคอกช้าง',
    'บ้านหนองปุหลก',
    'บ้านยุบพริก',
    'บ้านคลองชายธง',
    'ยุวสมาคมประจวบคีรีขันธ์อนุสรณ์',
    'อนุบาลเมืองประจวบคีรีขันธ์(สละชีพ)',
    'อนุบาลประจวบคีรีขันธ์',
    'ประชาสงเคราะห์อุปถัมภ์',
    'บ้านบึง',
    'บ้านคั่นกระได',
    'บ้านอ่าวน้อย',
    'บ้านหนองเสือ',
    'บ้านหนองยายเอม',
    'เนินแก้ววิทยาคาร',
    'บ้านย่านซื่อ',
    'ไทยรัฐวิทยา 65 (วังไทรติ่ง)',
    'สามัคคีร่วมจิต',
    'วังมะเดื่อ',
    'ประชาบูรณะวิทยา',
    'หาดสนุกราษฏร์บำรุง',
    'บ้านพุตะแบก',
    'วัดทุ่งกลาง',
    'บ้านดอนใจดี',
    'บ้านมะเดื่อทอง',
    'วัดนาหูกวาง',
    'วัดนาล้อม',
    'บ้านหนองพิกุล',
    'วัดทุ่งประดู่',
    'อนุบาลทับสะแก',
    'บ้านดอนทราย',
    'ประชาราษฏร์บำรุง',
    'สมาคมเลขานุการสตรี 2',
    'บ้านเหมืองแร่',
    'บ้านทุ่งตาแก้ว',
    'อ่างทองพัฒนา',
    'บ้านไร่ใน',
    'บ้านหินเทิน',
    'บ้านเนินดินแดง',
    'ประชาพิทักษ์',
    'บ้านห้วยยางมิตรภาพที่ 35',
    'บ้านทุ่งยาว',
    'บ้านอ่างทอง',
    'บ้านสีดางาม',
    'บ้านวังยาง',
    'วัดหนองหอย',
    'บ้านหนองมะค่า',
    'บ้านห้วยทรายขาว',
    'บ้านดอนทอง',
    'วัดนาผักขวง',
    'บ้านถ้ำเขาน้อย',
    'บ้านดอนสง่า',
    'อนุบาลบางสะพาน',
    'ไตรกิตทัตตานนท์',
    'บ้านดงไม้งาม',
    'บ้านมรสวบ',
    'บ้านเขามัน',
    'สมาคมเลขานุการสตรี 1',
    'บ้านหนองจันทร์',
    'บ้านยางเขา',
    'บ้านหนองหญ้าปล้อง',
    'บ้านวังยาว',
    'บ้านทองมงคล',
    'บ้านราษฏร์ประสงค์',
    'บ้านธรรมรัตน์',
    'บ้านในล็อค',
    'วัดธงชัยธรรมจักร',
    'วัดดอนยาง',
    'บ้านหนองมงคล',
    'บ้านดอนสูง',
    'บ้านหนองระแวง',
    'วัดถ้ำคีรีวงศ์',
    'บ้านหินกอง',
    'บ้านชะม่วง',
    'บ้านสวนหลวง',
    'บ้านห้วยแก้ว',
    'บางสะพาน',
    'บ้านท่าขาม',
    'บ้านดอนสำราญ',
    'บ้านอ่าวยาง',
    'บ้านคลองลอย',
    'บ้านวังน้ำเขียว',
    'ธนาคารออมสิน',
    'บ้านทุ่งเชือก',
    'บ้านดอนสำนัก',
    'มูลนิธิศึกษา',
    'บ้านไร่บนสามัคคี',
    'บ้านบางเจริญ',
    'บ้านทองอินทร์',
    'บ้านช้างเผือก',
    'บ้านไชยราช',
    'อนุบาลบางสะพานน้อย',
    'บ้านบางเบิด',
    'บ้านฝั่งแดง',
    'วัดกำมะเสน',
    'บ้านห้วยสัก',
    'บ้านทุ่งสีเสียด',
    'บ้านทรายทอง',
    'บ้านหนองฆ้อง',
    'บ้านทุ่งไทร',
    'บ้านบางสะพานน้อย',
    'วัดดอนตะเคียน',
    'บ้านปากคลอง(ราษฏร์อุปกรณ์)',
    'บ้านทุ่งกระโตน',
    'บ้านหนองห้วยฝาด',
    'วัดเขาราษฎร์บำรุง',
    'บ้านปากแพรก',
    'ประจวบวิทยาลัย',
    'อ่าวน้อยวิทยานิคม',
    'หว้ากอวิทยาลัย',
    'ทับสะแกวิทยา',
    'ห้วยยางวิทยา',
    'บางสะพานวิทยา',
    'ธงชัยวิทยา',
    'ชัยเกษมวิทยา',
    'โสตศึกษาเทพรัตน์',
    'บางสะพานน้อยวิทยาคม',
    'บ้านหนองกระทิง',
    'วัดกุยบุรี',
    'อนุบาลกุยบุรี(วัดวังยาว)',
    'บ้านบน',
    'บ้านหนองตาเสือ',
    'บ้านหนองหมู',
    'บ้านหนองเตาปูน',
    'บ้านหนองบัว',
    'บ้านปากเหมือง',
    'บ้านเขาแดง',
    'วัดทุ่งน้อย',
    'วัดดอนยายหนู',
    'บ้านดอนบ่อกุ่ม',
    'บ้านดอนมะขาม',
    'สามแยกป่าถล่ม',
    'บ้านป่าถล่ม',
    'บ้านสามกระทาย',
    'บ้านดอนกลาง',
    'บ้านสำโหรง',
    'บ้านรวมไทย',
    'บ้านโป่งกะสัง',
    'บ้านหนองเกด',
    'บ้านยางชุม',
    'ดีมากาเร็ดฟุลเลอร์',
    'บ้านไร่บน',
    'บ้านหาดขาม',
    'ชลประทานปราณบุรีสาขาบ้านวังปลา',
    'บ้านเขาน้อย',
    'ค่ายธนะรัชต์',
    'บ้านตำหรุ(วิงประชาสงเคราะห์',
    'บ้านปลายน้ำ',
    'บ้านหนองกา',
    'วัดนาห้วย',
    'อนุบาลปราณบุรี(บ้านเมืองเก่า)',
    'บ้านปากน้ำปราณ',
    'บ้านหนองบัว',
    'บ้านปรือน้อย',
    'บ้านวังก์พง(รัฐราษฏร์บำรุง)มิตรภาพที่158',
    'บ้านหนองหอย',
    'บ้านวังยาว',
    'บ้านหนองตาแต้ม',
    'บ้านหนองตาเมือง',
    'ชลประทานปราณบุรี',
    'บ้านห้วยพลับ',
    'บ้านวังวน(ชูชีพ-สนิทพลอยน้อยอุปถัมภ์)',
    'บ้านหนองยิงหมี',
    'บ้านหนองตาเย็น',
    'บ้านห้วยมงคล',
    'บ้านวังข่อย',
    'บ้านหนองไผ่',
    'บ้านหนองพรานพุก',
    'บ้านทุ่งยาว',
    'บ้านทับใต้',
    'บ้านห้วยไคร้',
    'บ้านเนินพยอมสาขาบึงนคร',
    'บ้านเนินพยอม',
    'บ้านหนองพลับ',
    'บ้านหนองกระทุ่ม',
    'โรตารี่กรุงเทพ',
    'บ้านคอกช้างพัฒนา',
    'บ้านห้วยไทรงาม',
    'วไลย',
    'ละเมาะ',
    'อานันท์',
    'บ้านหนองซอ',
    'บ้านหนองคร้า',
    'อนุบาลหัวหิน(บ้านหนองขอน)',
    'บ้านวังโบสถ์',
    'บ้านหนองเหียง',
    'บ้านหนองตะเภา',
    'บ้านไร่เก่าสามร้อยยอด',
    'บ้านสามร้อยยอด (บุญรักอุทิศ)',
    'บ้านตาลเจ็ดยอด',
    'บ้านหนองน้ำกลัด',
    'บ้านไร่ใหม่',
    'บ้านหนองจิก',
    'อนุบาลสามร้อยยอด(บ้านหนองแก)',
    'บ้านเกาะไผ่',
    'บ้านสิบสองหุ้น',
    'บ้านหนองไทร',
    'บ้านนาวัลเปรียง',
    'บ้านศาลาลัย',
    'บ้านหนองเป่าปี่',
    'บ้านเนินกรวด',
    'บ้านหนองขาม',
    'บ้านลาดวิถี',
    'บ้านหนองคาง',
    'บ้านเกาะนาน้อย',
    'บ้านหนองหญ้าปล้อง',
    'บ้านเขาโป่ง',
    'วัดเกษตรกันทราราม',
    'บ้านบางปู',
    'บ้านใหม่',
    'บ้านพุน้อย',
    'บ้านพุใหญ่',
    'บ้านหนองข้าวเหนียว',
    'บ้านหัวตาลแถว',
    'กุยบุรีวิทยา',
    'ยางชุมวิทยา',
    'เมืองปราณบุรี',
    'ปากน้ำปราณวิทยา',
    'เตรียมอุดมศึกษาพัฒนาการ ปราณบุรี',
    'รัชตวิทยาคม',
    'หัวหิน',
    'หัวหินวิทยาคม',
    'หนองพลับวิทยา',
    'สามร้อยยอดวิทยาคม',
    'วัดชัน',
    'บ้านย่านซื่อ',
    'วัดสวนพล',
    'วัดบ้านตาล',
    'อนุบาลนครศรีธรรมราช"ณ นครอุทิศ"',
    'วัดหนองบัว',
    'วัดน้ำรอบ',
    'วัดศาลาไพ',
    'วัดป่ายาง',
    'วัดเทพธิดาราม',
    'วัดมหาชัยวนาราม',
    'วัดป่าไหม้',
    'วัดท่างาม',
    'บ้านปากพญา',
    'บ้านปากน้ำปากพญา',
    'บ้านถนนใหญ่',
    'วัดนาวง',
    'ชุมชนวัดหมน',
    'วัดสว่างอารมณ์',
    'วัดจังหูน',
    'วัดพังสิงห์',
    'บ้านหนองหนอน',
    'บ้านบางเตย',
    'วัดโดน',
    'วัดนารีประดิษฐ์',
    'บ้านบางหลวง',
    'วัดโพธาราม',
    'บ้านบางกระบือ',
    'วัดราษฎร์เจริญ',
    'วัดพระมงกุฎ',
    'บ้านนาเคียน',
    'บ้านคลองดิน',
    'บ้านทวดเหนือ',
    'วัดน้ำสรง',
    'วัดดอนยาง',
    'วัดหญ้า',
    'วัดพระมหาธาตุ',
    'บ้านเนิน',
    'วัดบางใหญ่',
    'วัดมุจลินทราวาส',
    'วัดตรีเอการาม',
    'วัดมัชฌิมภูมิ',
    'วัดสระแก้ว',
    'บ้านเกาะโพธิ์แจ้ง',
    'ชุมชนบ้านบางจาก',
    'วัดบางตะพาน',
    'วัดท่าสะท้อน',
    'ราชประชานุเคราะห์ 5',
    'บ้านคันธง',
    'วัดนางพระยา',
    'วัดหัวนอน',
    'วัดท่านคร',
    'วัดมุขธารา',
    'วัดท่าแพ',
    'ราชประชานุเคราะห์ 4',
    'วัดไพศาลสถิต',
    'บ้านห้วยไทร',
    'บ้านปากน้ำเก่า',
    'วัดวิสุทธิยาราม',
    'วัดศรีมงคล',
    'บ้านชะเอียน',
    'บ้านปากน้ำปากพูน',
    'วัดท่าม่วง',
    'วัดโบสถ์',
    'วัดมะม่วงทอง',
    'วัดหัวอิฐ',
    'บ้านทวดทอง',
    'วัดโพธิ์ทอง',
    'วัดวนาราม',
    'วัดโพธิ์เสด็จ',
    'วัดทุ่งแย้',
    'วัดมะม่วงสองต้น',
    'วัดราษฎร์ประดิษฐ์',
    'ไทยรัฐวิทยา 74 (ชุมชนบ้านคีรีวง)',
    'วัดสมอ',
    'วัดวังไทร',
    'วัดโคกโพธิ์สถิตย์',
    'วัดจันทร์',
    'ราษฎร์บำรุง',
    'บ้านสำนักใหม่',
    'วัดสอ',
    'บ้านคดศอก',
    'วัดใหม่ทอน',
    'บ้านตลาด',
    'วัดไทรงาม',
    'วัดคีรีกันทร์',
    'บ้านร่อน',
    'บ้านสันยูง',
    'ชุมชนลานสกา',
    'วัดปะ',
    'บ้านมะม่วงทอง',
    'วัดดินดอน',
    'วัดน้ำรอบ',
    'วัดวังหงส์',
    'วัดเจดีย์',
    'วัดแพร่',
    'วัดไสมะนาว',
    'บ้านห้วยยูง',
    'วัดท้ายสำเภา',
    'บ้านไสใหญ่',
    'วัดเชิงแตระ',
    'วัดราษฏร์เจริญวราราม',
    'วัดพระพรหม',
    'บ้านห้วยระย้า',
    'วัดห้วยพระ',
    'วัดท่าช้าง',
    'วัดมะม่วงตลอด',
    'วัดกัด',
    'วัดคันนาราม',
    'วัดหนองแตน',
    'วัดพระเพรง',
    'วัดสระไคร',
    'วัดป่าหวาย',
    'บ้านอ่าวตะเคียน',
    'บ้านปลายคลอง',
    'วัดบางหว้า',
    'วัดโคกกะถิน',
    'บ้านหนองไม้แก่น',
    'วัดดอนตรอ',
    'วัดสระเพลง',
    'บ้านปากช่อง',
    'ราษฎร์บำรุง',
    'วัดทางพูน',
    'วัดทางพูน สาขาบ้านทุ่งทับควาย',
    'วัดพังยอม',
    'วัดทุ่งเฟื้อ',
    'บ้านบางไทร',
    'บ้านบางนกวัก',
    'บ้านทุ่งสร้าน',
    'รักบี้จุฬา-ธรรมศาสตร์',
    'เบญจมราชูทิศ',
    'กัลยาณีศรีธรรมราช',
    'ปากพูน',
    'เมืองนครศรีธรรมราช',
    'โยธินบำรุง',
    'ตรีนิมิตรวิทยา',
    'ท่านครญาณวโรภาสอุทิศ',
    'จุฬาภรณราชวิทยาลัย นครศรีธรรมราช',
    'ลานสกาประชาสรรค์',
    'ขุนทะเลวิทยาคม',
    'เฉลิมราชประชาอุทิศ',
    'เตรียมอุดมศึกษาภาคใต้',
    'พระพรหมพิทยานุสรณ์',
    'ทางพูนวิทยาคาร',
    'เฉลิมพระเกียรติสมเด็จพระศรีนครินทร์ นครศรีธรรมราช',
    'บ้านคลองขัน',
    'วัดเพ็ญญาติ',
    'วัดกะเปียด',
    'บ้านปากน้ำ',
    'วัดควนสูง',
    'บ้านทุ่งกรวด',
    'วัดวังม่วง',
    'ฉวาง',
    'วัดมะเฟือง',
    'วัดโคกเมรุ',
    'บ้านนาเส',
    'วัดนาเขลียง',
    'บ้านโคกมะขาม',
    'บ้านไสโคกเกาะ',
    'บ้านควนสวรรค์',
    'วัดควนยูง',
    'บ้านห้วยทรายขาว',
    'วัดศิลา',
    'บ้านกันละ',
    'วัดไม้เรียง',
    'วัดหาดสูง',
    'บ้านหาดทรายแก้ว',
    'บ้านโคกยาง',
    'บ้านเศลาใต้',
    'วัดมะปรางงาม',
    'บ้านป่าพาด',
    'บ้านทอนวังปราง',
    'บ้านสวนอาย',
    'บ้านปลายคลองเพรง',
    'วัดโคกหาด',
    'บ้านควน',
    'วัดเพ็ญมิตร',
    'วัดสามัคคีนุกูล',
    'วัดควนสะตอ',
    'บ้านคลองสาย',
    'บ้านห้วยปริก',
    'ชุมชนวัดสุวรรณาราม',
    'ชุมชนวัดกะทูนเหนือ มิตรภาพที่ 218',
    'วัดปากจัง',
    'วัดนางเอื้อย',
    'บ้านห้วยกลาง',
    'วัดหน้าเขา',
    'จุฬาภรณ์พิชญาคาร',
    'บ้านปากระแนะ',
    'วัดในไร่',
    'ชุมชนบ้านปากเสียว',
    'บ้านวังวัว',
    'บ้านคุ้งวังวัว',
    'วัดมังคลาราม',
    'วัดโบราณาราม',
    'บ้านนาเหนือ',
    'บ้านเหนือคลอง',
    'วัดทุ่งนาใหม่',
    'วัดยางค้อม',
    'ราชประชานุเคราะห์ 6',
    'บ้านพูน',
    'วัดกะโสม',
    'บ้านคลองตูก',
    'สหกรณ์นิคมกสิกรรมทุ่งสง',
    'บ้านเขาตาว',
    'บ้านสามัคคีธรรม',
    'บ้านไสส้าน',
    'วัดนิคมคีรี',
    'บ้านวังเต่า',
    'บ้านหนองปลิง',
    'วัดเขาโร',
    'บ้านนาพรุ',
    'บ้านหนองท่อม',
    'วัดทะเลมิตรภาพที่ 151',
    'ชุมชนบ้านไทรห้อง',
    'บ้านหน้าเขา',
    'วัดวังหีบ',
    'บ้านหนองหว้า',
    'วัดเขากลาย',
    'บ้านนาตาแย้ม',
    'มหาราช 3',
    'บ้านจำปา',
    'บ้านน้ำพุ',
    'วัดถ้ำใหญ่',
    'บ้านวังยวน',
    'วัดควนชม',
    'วัดก้างปลา',
    'บ้านชายคลอง',
    'วัดธรรมเผด็จ',
    'บ้านเกาะปราง',
    'บ้านบนควน',
    'บ้านเกาะยวน',
    'ทุ่งควายพัฒนศึกษา',
    'บ้านวังยาว',
    'บ้านบ่อมอง',
    'วัดวังขรี',
    'วัดทุ่งส้าน',
    'บ้านคอกช้าง',
    'ชุมชนวัดสำโรง',
    'วัดศิลาราย',
    'วัดราษฎร์ประดิษฐ์',
    'บ้านถ้ำตลอด',
    'บ้านน้ำตก',
    'บ้านวังธน',
    'วัดคงคาเจริญ',
    'บ้านนาเกิดผล',
    'บ้านคลองขุด',
    'บ้านนาป่าประชาสรรค์',
    'วัดมณีเจริญมิตรภาพที่ 227',
    'บ้านไสโป๊ะ',
    'วัดเกาะสระ',
    'บ้านหนองยาง',
    'วัดหนองดี',
    'บ้านไสยูงปัก',
    'ชุมชนบ้านสี่แยก',
    'บ้านคลองโอม',
    'วัดสุวรรณคีรี',
    'บ้านนาโพธิ์',
    'ชุมชนวัดอัมพวัน',
    'บ้านนาบอน',
    'วัดเทวสิทธิ์',
    'บ้านคลองจัง',
    'องค์การสวนยาง 2',
    'องค์การสวนยาง 3',
    'หมู่บ้านป่าไม้',
    'บ้านทะเลสองห้อง',
    'บ้านบางปรน',
    'บ้านบ่อปลา',
    'บ้านควนประชาสรรค์',
    'วัดเสม็ดจวน',
    'วัดคงคาเลียบ',
    'ราชเวชพิศาล',
    'วัดควนสระบัว',
    'บ้านนาท่อม',
    'บ้านพอโกบ',
    'บ้านบางตะเภา',
    'บ้านกรุงหยันใต้',
    'วัดท่ายาง',
    'ชุมชนบ้านหน้าเขา',
    'บ้านโคกวัด',
    'บ้านวังหิน',
    'วัดควนยูง',
    'วัดภูเขาหลัก',
    'บ้านนาใหญ่',
    'วัดคลังสินธาราม',
    'วัดใหม่',
    'บ้านห้วยรื่น',
    'วัดขนาน',
    'บ้านหนองใหญ่',
    'บ้านก่องาม',
    'บ้านบางรูป',
    'วัดวิสุทธิวงศ์',
    'วัดประดิษฐาราม',
    'บ้านทุ่งกรวด',
    'บ้านไร่มุสลิม',
    'วัดควน',
    'บ้านไสใหญ่',
    'บ้านสระนางมโนราห์',
    'บ้านควนลำภู',
    'บ้านหนองคล้า',
    'ราษฎร์ประชาอุทิศ',
    'บ้านควนอวดพัน',
    'คล่องหมื่นเพชรอนุสรณ์วชิราลงกรณ์อุปถัมภ์',
    'วัดลำนาว',
    'บ้านปากแพรก',
    'โสตทัศนศึกษาจุฬาลงกรณ์มหาวิทยาลัย',
    'บ้านควนประ',
    'บ้านคลองเสาเหนือ',
    'บ้านไสเตาอ้อย',
    'บ้านนาตำเสา',
    'บ้านนิคมวังหิน',
    'สมสรร',
    'บ้านเขาวง',
    'เจริญรัชต์ภาคย์',
    'สังวาลย์วิท 7',
    'ตำรวจตระเวนชายแดนช่างกลปทุมวันอนุสรณ์ 10',
    'บ้านไสยาสน์',
    'บ้านสวน',
    'วัดวังหิน',
    'บ้านหนองเจ',
    'ไทยรัฐวิทยา 38(บ้านน้ำร้อน)',
    'บ้านแพรกกลาง',
    'บ้านเกาะขวัญ',
    'วัดสวนพิกุล',
    'บ้านพรุวง',
    'พรรณราชลเขต',
    'บ้านทุ่งโพธิ์งาม',
    'วัดวังรีบุญเลิศ',
    'บ้านวังตลับ',
    'บ้านนาพา',
    'วัดปากท่าซอง',
    'บ้านปลายเส',
    'วัดควนกอ',
    'บ้านปลายรา',
    'วัดควนส้าน',
    'บ้านคลองงา',
    'บ้านนา',
    'วัดมะนาวหวาน',
    'บ้านหน้าเหมน',
    'บ้านกุยเหนือ',
    'บ้านคลองกุย',
    'บ้านจันดี',
    'ชุมชนบ้านนาวา',
    'องค์การสวนยาง 1',
    'บ้านคลองปีก',
    'บ้านไทรงาม',
    'บ้านคลองปีกเหนือ',
    'วัดสวนขัน',
    'บ้านหนองเตย',
    'วัดจันดี',
    'วัดหลักช้าง',
    'บ้านควนตม',
    'บ้านนาปราน',
    'ฉวางรัชดาภิเษก',
    'ไม้เรียงประชาสรรค์',
    'ละอายพิทยานุสรณ์',
    'พิปูนสังฆรักษ์ประชาอุทิศ',
    'นางเอื้อยวิทยา',
    'ทุ่งสง',
    'สตรีทุ่งสง',
    'ทุ่งสงวิทยา',
    'ทุ่งสงสหประชาสรรค์',
    'ก้างปลาวิทยาคม',
    'ราชประชานุเคราะห์ 19',
    'นครศรีธรรมราชปัญญานุกูล',
    'นาบอน',
    'ทุ่งใหญ่วิทยาคม',
    'กรุงหยันวิทยาคาร',
    'ทุ่งใหญ่เฉลิมราชอนุสรณ์ รัชมังคลาภิเษก',
    'เสม็ดจวนวิทยาคม',
    'ทุ่งสังพิทยาคม',
    'บางขันวิทยา',
    'วังหินวิทยาคม',
    'ประสาธน์ราษฎร์บำรุง',
    'ช้างกลางประชานุกูล',
    'โสตศึกษาจังหวัดนครศรีธรรมราช',
    'วัดท้ายทะเล',
    'ชุมชนวัดท่าลิพง',
    'บ้านนา',
    'บ้านเกาะทวด',
    'วัดบางยิ่ว',
    'บ้านศาลาตะเคียน',
    'วัดทายิการาม',
    'บ้านดอนโรง มิตรภาพที่ 196',
    'วัดแดง',
    'วัดพระบาท',
    'วัดทองพูน',
    'ชุมชนวัดเขาแก้ววิเชียร',
    'บ้านบางพระ',
    'วัดดอนรักษา',
    'บ้านปากเชียร',
    'วัดเทพนิมิต',
    'บ้านยางกาญจน์',
    'วัดสระแก้ว',
    'วัดพระหอม',
    'วัดคลองขยัน',
    'วัดชัยสุวรรณ',
    'วัดบ้านเนิน',
    'วัดทวยเทพ',
    'บ้านหัวปอ',
    'วัดบางทองคำ',
    'บ้านท่าขนาน',
    'บ้านบางด้วน',
    'บ้านน้ำบ่อ',
    'บ้านเนินธัมมัง',
    'บ้านคงคาล้อม',
    'บ้านทุ่งขวัญแก้ว',
    'วัดบ่อล้อ',
    'วัดสระโพธิ์',
    'บ้านบางปรง',
    'บ้านดอนโตนด',
    'บ้านช่องขาด',
    'วัดคงคาวดี',
    'วัดทาบทอง',
    'วัดสระเกษ',
    'ชุมชนควนดินแดง ม.173',
    'บ้านทุ่งใหญ่',
    'บ้านลานนา',
    'วัดไม้เสียบ',
    'บ้านตรอกแค',
    'บ้านขอนหาด',
    'บ้านใสถิน',
    'บ้านดอนตาสังข์',
    'วัดควนใส',
    'บ้านทุ่งโชน',
    'ชุมชนวัดเขาลำปะ',
    'เขาพระทอง',
    'บ้านทุ่งไม้ไผ่',
    'บ้านควนหนองหงส์',
    'บ้านควนมุด',
    'บ้านทุ่งโป๊ะ',
    'วัดกาโห่ใต้',
    'ตชด.บ้านท่าข้าม',
    'บ้านควนชิง',
    'บ้านทุ่งใคร',
    'วัดควนยาว',
    'วัดควนป้อม',
    'วัดควนเคร็ง',
    'วัดปากควน',
    'วัดท่าสะท้อน',
    'บ้านปากบางกลม',
    'วัดจิกพนม',
    'วัดรักขิตวัน',
    'บ้านศาลาทวดทอง',
    'บ้านชะอวด',
    'บ้านห้วยโส',
    'วัดควนสมบูรณ์',
    'วัดห้วยแหยงราษฎร์ภูเก็ตอุทิศ',
    'วัดดอนมะปราง',
    'วัดท่าเสม็ด',
    'บ้านหนองหิน',
    'บ้านบางน้อย',
    'บ้านนางหลง',
    'วัดโคกทราง',
    'วัดหนองจิก',
    'บ้านพรุบัว',
    'วัดควนเถียะ',
    'บ้านตูล',
    'บ้านกุมแป',
    'บ้านควนเงิน',
    'วัดวังกลม',
    'บ้านหนองบัว',
    'บ้านท่าไทร',
    'บ้านหนองนนทรี',
    'บ้านควนมิตร',
    'บ้านวังหอน',
    'ชุมชนวัดบางบูชา',
    'วัดเกาะจาก',
    'วัดโบสถ์',
    'บ้านเกาะน้อย',
    'บ้านบางตะลุมพอ',
    'บ้านนำทรัพย์',
    'วัดขนาบนาก',
    'วัดโคกมะม่วง',
    'วัดตรงบน',
    'วัดฝ่าพระบาทราษฎร์บำรุง',
    'วัดบ้านงาม',
    'วัดสุขุม',
    'วัดสามแพรก',
    'วัดมหิสสาราม',
    'วัดศรีสุวรรณาราม',
    'บ้านบางเนียน',
    'บ้านแสงวิมาน',
    'วัดบางมะขาม',
    'บ้านเกาะนางโดย',
    'บ้านเขาน้อย',
    'วัดปิยาราม',
    'วัดกัลยานฤมิต',
    'บ้านราษฏร์สงวน',
    'วัดคลองน้อย',
    'บ้านปากคลอง',
    'บ้านบางลึก',
    'วัดชมพูประดิษฐ์',
    'วัดปากเนตร',
    'ชุมชนวัดมัชฌิมเขตชลาราม',
    'บ้านบางมูลนาก',
    'บ้านปลายคลอง',
    'บ้านเกาะทัง',
    'วัดสระ',
    'วัดหงส์แก้ว',
    'วัดกาญจนาราม',
    'วัดบางไทร',
    'วัดบางศาลา',
    'บ้านบางแรด',
    'วัดแจ้ง',
    'บ้านหมาก',
    'วัดท่าพญา',
    'วัดปากบางท่าพญา',
    'วัดเทพสิทธาราม',
    'วัดบ้านใหม่บน',
    'วัดบางคุระ',
    'วัดรัตนาราม',
    'บ้านท้องโกงกาง',
    'วัดบางทวด',
    'วัดบางฉนาก',
    'บ้านบางวัง',
    'วัดพิบูลยาราม มิตรภาพที่ 232',
    'บ้านโก้งโค้ง',
    'บ้านชายทะเล',
    'วัดปากแพรก',
    'วัดบางพระ',
    'วัดบางด้วน',
    'วัดหอยกัน',
    'วัดอัฒฑศาสนาราม',
    'บ้านหัวลำพู',
    'วัดเหมก',
    'วัดป่าระกำ',
    'วัดป่าระกำเหนือ',
    'บ้านศรีสมบูรณ์',
    'วัดสองพี่น้อง',
    'วัดปากตรง',
    'วัดแหลมตะลุมพุก',
    'ราชประชานุเคราะห์',
    'วัดเทพมงคล',
    'วัดควนเกย',
    'บ้านหนองมาก',
    'วัดวัวหลุง',
    'วัดคันธมาลี',
    'วัดทุ่งหล่อ',
    'บ้านเกยเชน',
    'บ้านทุ่งเลน',
    'วัดปลายสระ',
    'สุนทราภิบาล',
    'วัดสุวรรณโฆษิต',
    'บ้านกลอง',
    'บ้านด่าน',
    'วัดมัชฌิมภูผา',
    'วัดเขาน้อย',
    'บ้านหนองกก',
    'บ้านม่วงงาม',
    'วัดพิศาลนฤมิต',
    'วัดร่อนนา',
    'วัดเถลิงกิตติยาราม',
    'บ้านห้วยไม้แก่น',
    'วัดเทพนมเชือด',
    'ชุมชนวัดสุวรรณรังษี',
    'วัดเนกขัมมาราม',
    'ร่อนพิบูลย์',
    'วัดพระอานนท์',
    'วัดหนา',
    'ราชประชานุเคราะห์ 7',
    'วัดโคกคราม',
    'บ้านป่าแชง',
    'บ้านท่าเจริญ',
    'วัดธงทอง',
    'บ้านควนรุย',
    'วัดถลุงทอง',
    'บ้านปลายราง',
    'บ้านห้วยหาร',
    'ชุมชนบ้านพุดหง',
    'วัดถ้ำเขาแดง',
    'วัดสามัคยาราม',
    'วัดธาราวง',
    'บ้านท่าไทร',
    'ชุมชนวัดเกาะเพชร',
    'บ้านบางโหนด',
    'บ้านเกาะสุด',
    'วัดบูรณาวาส',
    'บ้านลากชาย',
    'วัดบางตะพาน',
    'วัดสว่างอารมณ์',
    'วัดควนชะลิก',
    'วัดปากเหมือง',
    'บ้านหัวไทร',
    'วัดดอนผาสุก',
    'วัดโคกยาง',
    'วัดอู่แก้ว',
    'บ้านโคกทราย',
    'ห้วยน้ำเย็น มิตรภาพ112',
    'วัดโคกพิกุล',
    'วัดหัวค่าย (พิศาลอุปถัมภ์)',
    'วัดบ้านด่าน',
    'วัดท่าเสริม',
    'ชุมชนพิบูลสงคราม',
    'บ้านสระนอก',
    'บ้านปากพรุ',
    'วัดนพคุณ',
    'วัดบ่อโพง',
    'วัดบ้านราม',
    'วัดรามแก้ว',
    'วัดปากระวะ',
    'วัดหน้าสตน',
    'วัดฉิมหลา',
    'บ้านหน้าศาล',
    'วัดอิมอญ',
    'วัดหัวลำภู',
    'บ้านศาลาแก้ว',
    'บ้านหนองบอน',
    'บ้านลำคลอง',
    'วัดทะเลปัง',
    'หัวไทร(เรือนประชาบาล)',
    'วัดท้ายโนต',
    'บ้านท่าเตียน',
    'วัดแหลม',
    'วัดโคกสูง',
    'บ้านลำหัก',
    'วัดสมควร',
    'บ้านดอนทราย',
    'บ้านอายเลา',
    'บ้านไสหินตั้ง',
    'วัดแร่ราษฎร์สุทัศน์',
    'วัดทุ่งโพธิ์',
    'บ้านเหนือคลอง',
    'ทัศนาวลัย',
    'วัดนาหมอบุญ',
    'บ้านทุ่งบก',
    'บ้านคอกวัว',
    'วัดชะอวด',
    'บ้านไสขาม',
    'บ้านชายควน',
    'บ้านควนโตน',
    'วัดสำนักขัน',
    'บ้านวังใส',
    'บ้านสำนักไม้เรียบ',
    'วัดวังฆ้อง',
    'บ้านคอพรุ',
    'เชียรใหญ่',
    'วิเชียรประชาสรรค์',
    'เชียรใหญ่สามัคคีวิทยา',
    'ธัญญาวดีศึกษา',
    'ชะอวดวิทยาคาร',
    'ขอนหาดประชาสรรค์',
    'ชะอวดเคร่งธรรมวิทยา',
    'เกาะขันธ์ประชาภิบาล',
    'ชะอวด',
    'ปากพนัง',
    'โศภนคณาภรณ์',
    'อินทร์ธานีวิทยาคม',
    'สตรีปากพนัง',
    'ร่อนพิบูลย์เกียรติวสุนธราภิวัฒก์',
    'คีรีราษฎร์พัฒนา',
    'ควนเกยสุทธิวิทยา',
    'เสาธงวิทยา',
    'ตระพังพิทยาคม',
    'หัวไทรบำรุงราษฎร์',
    'แหลมราษฎร์บำรุง',
    'นพคุณประชาสรรค์',
    'ทรายขาววิทยา',
    'เขาพังไกร',
    'สามตำบลวิทยาประชาสรรค์',
    'ชุมชนวัดทอนหงส์',
    'บ้านในเขียว 1',
    'บ้านในเขียว 2',
    'บ้านวังลุง',
    'วัดใหม่',
    'บ้านชุมขลิง',
    'วัดหลวงครู',
    'วัดกำแพงถม',
    'วัดโยธาธรรม',
    'วัดนากัน',
    'วัดท้าวโทะ',
    'ชุมชนวัดอินทคีรี',
    'วัดเขาขุนพนม',
    'วัดพรหมโลก',
    'วัดเขาปูน',
    'บ้านคลองแคว',
    'วัดโทเอก',
    'วัดป่ากิ่ว',
    'วัดสากเหล็ก',
    'วัดดอนใคร',
    'บ้านบ่อกรูด',
    'บ้านพังปริง',
    'วัดเขาพนมไตรรัตน์',
    'บ้านปากดวด',
    'วัดจันพอ',
    'วัดหญ้าปล้อง',
    'บ้านฉาง',
    'บ้านวังหิน',
    'บ้านหน้าเขาวัด',
    'บ้านหนองหว้า',
    'บ้านปากเจา',
    'วัดปลักปลา',
    'วัดจันทาราม',
    'บ้านสำนักม่วง',
    'วัดป่า(ท่าขึ้น)',
    'วัดหมาย',
    'วัดเทวดาราม',
    'วัดยางงาม',
    'วัดทางขึ้น',
    'วัดพระเลียบ',
    'วัดประดู่หอม',
    'วัดนางตรา',
    'ปทุมานุกูล',
    'วัดชลธาราม',
    'ราชประชานุเคราะห์ 8',
    'บ้านสระบัว',
    'บ้านท่าสูง',
    'บ้านในถุ้ง',
    'ท่าศาลา',
    'ชุมชนใหม่',
    'บ้านสะพานหัน',
    'วัดพระอาสน์',
    'วัดโคกเหล็ก',
    'วัดสระประดิษฐ์',
    'บ้านสองแพรก มิตรภาพที่ 92',
    'วัดสโมสร',
    'วัดโมคลาน',
    'วัดยางทอง',
    'บ้านทุ่งเกราะ',
    'วัดป่า(โมคลาน)',
    'บ้านประชาอารี',
    'วัดชัยธารามประดิษฐ์',
    'วัดสวนหมาก',
    'วัดน้ำตก',
    'ชุมชนวัดสระแก้ว',
    'บ้านเราะ(อุทัยประชาสรรค์)',
    'บ้านชุมโลง',
    'บ้านอินทนิน',
    'บ้านหนับเภา',
    'วัดนากุน',
    'บ้านทุ่งชน',
    'วัดเขาน้อย',
    'บ้านทุ่งขันหมาก',
    'บ้านสำนักเนียน',
    'บ้านเขาใหญ่',
    'วัดสุธรรมาราม',
    'วัดคงคาเลียบ',
    'วัดกลาง',
    'บ้านน้ำขาว',
    'วัดชนาราม',
    'บ้านต้นจันทน์',
    'บ้านปลายทอน',
    'วัดถ้ำเทียนถวาย',
    'ชุมชนวัดปัณณาราม',
    'บ้านเขาฝ้าย',
    'บ้านเขาตาสัก',
    'บ้านคลองตีนเป็ด',
    'วัดสุชน',
    'บ้านสุวรรณเขต',
    'บ้านห้วยทรายทอง',
    'วัดธารน้ำฉา',
    'ชุมชนวัดเทพราช',
    'บ้านเขายวนเฒ่า',
    'บ้านเผียน',
    'บ้านน้ำฉา',
    'วัดชนสังขรณพิจิตร',
    'วัดศิลาชลเขต',
    'บ้านเขาทราย',
    'บ้านไสพลู',
    'บ้านไสเหรียง',
    'วัดประทุมทายการาม',
    'บ้านบางฉาง',
    'วัดจอมทอง',
    'วัดเขาพนมไตย',
    'บ้านช่องเขาหมาก',
    'บ้านยางโพรง',
    'บ้านน้ำร้อน',
    'บ้านทุ่งหัวนา',
    'บ้านปราบราษฏร์อุทิศ',
    'วัดสโมสรสันนิบาต',
    'บ้านทุ่งครั้ง',
    'วัดขรัวช่วย',
    'วัดปากด่าน',
    'วัดคงคาวดี',
    'วัดสวนศิขรบรรพต',
    'ชุมชนบ้านบางโหนด',
    'วัดเขา (วันครู2501)',
    'วัดกระดังงา',
    'บ้านเปร็ต',
    'บ้านในเพลา',
    'บ้านวัดใน',
    'บ้านท่าน้อย',
    'ชุมชนวัดบางคู',
    'บ้านเขาวังทอง',
    'วัดเจดีย์หลวง',
    'บ้านเขาหัวช้าง',
    'บ้านคลองเหลง',
    'บ้านคลองวัง',
    'บ้านควนทอง(ประชาอุทิศ)',
    'บ้านแขวงเภา',
    'วัดคีรีวง',
    'บ้านท่าจันทน์',
    'บ้านน้ำโฉ',
    'บ้านท่าม่วง',
    'บ้านปากลง',
    'วัดเปียน',
    'บ้านพิตำ',
    'บ้านนบ',
    'วัดเขาน้อย',
    'วัดคงคา',
    'บ้านขุนทะเล',
    'วัดโรงเหล็ก',
    'บ้านโรงเหล็ก',
    'บ้านพังหรัน',
    'วัดนาเหรง',
    'บ้านในตูล',
    'เคียงศิริ',
    'วัดวังเลา',
    'พรหมคีรีพิทยาคม',
    'บ้านเกาะวิทยา',
    'ท่าศาลาประสิทธิ์ศึกษา',
    'โมคลานประชาสรรค์',
    'สระแก้วรัตนวิทย์',
    'สิชลคุณาธารวิทยา',
    'ฉลองรัฐราษฎร์อุทิศ',
    'สิชลประชาสรรค์',
    'เทพราชพิทยาสรรค์',
    'ขนอมพิทยา',
    'ท้องเนียนคณาภิบาล',
    'คงคาประชารักษ์',
    'นบพิตำวิทยา',
    'บ้านกระบี่น้อย',
    'บ้านเขาตั้ง',
    'บ้านทุ่งพะยอม',
    'บ้านนานอก',
    'บ้านน้ำจาน',
    'วัดโพธิ์เรียง',
    'บ้านหว่างคลอง',
    'บ้านเขาคราม(ศรีจันทร์อุทิศ)',
    'บ้านทุ่ง',
    'ทุ่งต้นปีก',
    'บ้านหนองจิก',
    'บ้านไหนหนัง',
    'บ้านเขาเทียมป่า',
    'บ้านคลองทราย',
    'บ้านเขาทอง',
    'บ้านบางขนุน',
    'บ้านเกาะกลาง',
    'บ้านคลองกำ',
    'บ้านคลองประสงค์',
    'วัดห้วยโต้',
    'บ้านในช่องมิตรภาพที่123',
    'บ้านทับปริก',
    'บ้านคลองใหญ่',
    'อนุบาลกระบี่',
    'อุตรกิจ',
    'บ้านหนองกก',
    'บ้านอ่าวน้ำเมา',
    'บ้านแหลมโพธิ์',
    'บ้านไสไทย',
    'บ้านหนองทะเล',
    'บ้านคลองม่วง',
    'บ้านดินแดงน้อย',
    'ชุมชนบ้านเขากลม',
    'บ้านคลองแห้ง',
    'บ้านช่องพลี',
    'บ้านอ่าวนาง',
    'บ้านแหลมตง',
    'บ้านเกาะพีพี',
    'บ้านช้างตาย',
    'บ้านกอตง',
    'บ้านควนม่วง',
    'วัดภูมิบรรพต',
    'บ้านควนม่วงสาขาทับพรุ',
    'บ้านหนองน้ำแดง',
    'บ้านทับพล',
    'บ้านเทพพนม',
    'บ้านห้วยเสียด',
    'บ้านห้วยพลู',
    'บ้านเขาพนม',
    'บ้านโคกหาร',
    'บ้านหินลูกช้าง',
    'บ้านโคกกลาง',
    'วัดพรุเตียว',
    'บ้านบางเหรียง',
    'บ้านทุ่งปรือ',
    'บ้านห้วยสารสาขาบ้านช่องฉันทนา',
    'บ้านมะม่วงเอน',
    'บ้านห้วยสาร',
    'บ้านควนกลาง',
    'บ้านโคกคา',
    'บ้านถ้ำโกบ',
    'บ้านห้วยน้ำแก้ว',
    'บ้านเขาดิน',
    'บ้านร่าหมาด',
    'บ้านร่าปู',
    'บ้านคลองย่าหนัด',
    'บ้านลิกี',
    'บ้านปากคลอง',
    'บ้านนาทุ่งกลาง',
    'บ้านโล๊ะใหญ่',
    'บ้านทุ่ง',
    'บ้านคลองโตนด',
    'บ้านหลังสอด',
    'บ้านคลองหิน',
    'วัดเกาะลันตา',
    'บ้านเจ๊ะหลี',
    'บ้านคลองนิน',
    'บ้านเกาะปอ',
    'บ้านสังกาอู้',
    'บ้านเขาฝาก',
    'บ้านหลังโสด',
    'บ้านคลองยาง',
    'บ้านโคกยูง',
    'บ้านทุ่งหยีเพ็ง',
    'บ้านศาลาด่าน',
    'บ้านพระแอะ',
    'คลองท่อมมิตรภาพที่160',
    'อนุบาลคลองท่อม',
    'ไทยรัฐวิทยา53(บ้านคลองขนาน)',
    'บ้านเหนือ',
    'บ้านแชงเปิง',
    'บ้านคลองท่อมเหนือ',
    'บ้านบางคราม',
    'บ้านบางเตียว',
    'บ้านท่ามะพร้าว',
    'บ้านหินเพิง',
    'บ้านคลองปิ้ง',
    'บ้านดินนา',
    'บ้านหินเพิงสาขาพรุใหญ่',
    'คลองพน',
    'บ้านทุ่งครก',
    'บ้านคลองไคร',
    'บ้านคลองแรด',
    'บ้านห้วยลึก',
    'บ้านห้วยพลูหนัง',
    'บ้านพรุพี',
    'บ้านบ่อมะม่วง',
    'บ้านทรายขาว',
    'บ้านทุ่งคา',
    'บ้านนางรอง',
    'บ้านช่องเสียด',
    'บ้านทุ่งนุ้ย',
    'บ้านเขาสามหน่วย',
    'บ้านพรุดินนา',
    'บ้านทับไทร',
    'บ้านควน',
    'บ้านคลองชะมวง',
    'บ้านเพหลา',
    'บ้านพรุเตย',
    'บ้านพรุเตียว',
    'บ้านบางใหญ่',
    'บ้านท่าประดู่',
    'บ้านควนใต้',
    'บ้านน้ำร้อน',
    'บ้านห้วยน้ำขาว',
    'บ้านทุ่งเสม็ด',
    'บ้านทุ่งสูง',
    'บ้านเขาล่อม',
    'วัดไพรสณฑ์',
    'บ้านเขาแก้ว',
    'บ้านคลองยา(เจริญราษฎร์สามัคคี)',
    'สหกรณ์นิคมอ่าวลึก2',
    'สหกรณ์นิคมอ่าวลึก3',
    'บ้านช่องไม้ดำ',
    'บ้านป่างาม',
    'บ้านศาลาพระม่วง',
    'บ้านคลองหิน',
    'วัดบางโทง',
    'ชุมชนวัดนาเหนือ',
    'บ้านบางเจริญ',
    'บ้านหนองหลุมพอ',
    'บ้านกลางมิตรภาพที่44',
    'บ้านเขางาม',
    'บ้านหินราว',
    'บ้านคลองแรด',
    'วัดสถิตโพธาราม',
    'บ้านแหลมสัก',
    'บ้านอ่าวน้ำ',
    'อ่าวลึก',
    'บ้านในควน',
    'บ้านถ้ำเสือ',
    'บ้านห้วยปริศนา',
    'บ้านอ่าวลึกน้อย',
    'บ้านควนโอ',
    'บ้านบากัน',
    'ชุมชนบ้านอ่าวลึกเหนือ',
    'บ้านทุ่ง',
    'สหกรณ์นิคมอ่าวลึก1',
    'บ้านถ้ำเพชร',
    'บ้านในยวน',
    'บ้านคลองปัญญา',
    'บ้านนาเทา',
    'วัดช่องแบก',
    'วิทยาประชาคม',
    'วัดเขาต่อ',
    'โคกยอ',
    'วัดบ้านนา',
    'บ้านบางโสก',
    'บ้านบางหอย',
    'สหกรณ์ประชาอุทิศ',
    'วัดบางเหลียว',
    'บ้านบกเก้าห้อง',
    'ชุมชนวัดหาดถั่ว',
    'วัดนทีมุขาราม',
    'บ้านบางเหียน',
    'บ้านปากหยา',
    'บ้านคลองพระยา',
    'บ้านศรีพระยาราษฎร์บำรุง',
    'บ้านคลองพวนประทีปบำรุง',
    'บ้านทะเลหอยราษฎร์อุทิศ',
    'บ้านทะเลหอยราษฎร์อุทิศสาขาบ้านควนเขียว',
    'บ้านดินแดง',
    'บ้านควนแดง',
    'บ้านเขาดินประชาพัฒน์',
    'บ้านเสม็ดจวน',
    'บ้านหนองจูด',
    'บ้านสะพานพน',
    'บ้านเกาะไทร',
    'บ้านย่านอุดม',
    'บ้านลำทับ',
    'บ้านนาพรุ',
    'บ้านไร่พัฒนา',
    'บ้านเกาะจำ',
    'บ้านเกาะปู',
    'บ้านเกาะศรีบอยา',
    'บ้านเกาะฮั่ง',
    'บ้านคลองเตาะ',
    'บ้านติงไหร',
    'บ้านควน',
    'บ้านคลองขนาน',
    'บ้านคลองหมาก',
    'บ้านคลองหวายเล็ก',
    'บ้านแหลมกรวด',
    'บ้านท่ายาง',
    'บ้านควนต่อ',
    'บ้านคลองเขม้า',
    'บ้านนาวง',
    'ราชประชานุเคราะห์2',
    'บ้านควนเกาะจันทร์',
    'บ้านควนนกหว้า',
    'วัดโคกยาง',
    'บ้านคลองรั้ว',
    'บ้านคลองยวน',
    'บ้านตลิ่งชัน',
    'บ้านปากหรา',
    'บ้านหาดยาว',
    'บ้านทุ่งสาคร',
    'บ้านเกาะไทร',
    'บ้านทุ่งประสาน',
    'วัดธรรมาวุธสรณาราม',
    'บ้านห้วยยูง',
    'วัดห้วยคราม',
    'บ้านห้วยมัด',
    'บ้านนาปง',
    'บ้านในทับ',
    'บ้านบกห้อง',
    'บ้านท่านุ่น',
    'บ้านนาออก',
    'ราชประชานุเคราะห์1',
    'บ้านไร่ใหญ่',
    'บ้านไสโป๊ะ',
    'ลันตาราชประชาอุทิศ',
    'คลองยางประชานุสรณ์',
    'คลองท่อมราษฎร์รังสรรค์',
    'คลองพนสฤษดิ์พิทยา',
    'พนมเบญจา',
    'อ่าวลึกประชาสรรค์',
    'อำมาตย์พานิชนุกูล',
    'เมืองกระบี่',
    'สินปุนคุณวิชญ์',
    'คลองหินพิทยาคม',
    'หนองทะเลวิทยา',
    'เขาดินประชานุกูล',
    'กาญจนาภิเษกวิทยาลัย กระบี่',
    'ราชประชานุเคราะห์ 37',
    'ปลายพระยาวิทยาคม',
    'ลำทับประชานุเคราะห์',
    'เหนือคลองประชาบำรุง',
    'เกาะปันหยี',
    'เกาะหมากน้อย',
    'เกาะไม้ไผ่',
    'บ้านตากแดด',
    'อนุบาลพังงา',
    'บ้านทุ่งเจดีย์',
    'บ้านบางกัน',
    'วัดปัจจันตคาม',
    'บ้านบางม่า',
    'วัดอรัญญิกาวาส',
    'บ้านปริง (วันครู 2500)',
    'เมืองพังงา',
    'วัดชนาธิการาม',
    'บ้านเขาเฒ่า',
    'บ้านบางเตย',
    'เกาะเคี่ยม',
    'วัดบางเสียด',
    'บ้านกลาง',
    'วัดมงคลสถิต',
    'บ้านป่ากอ',
    'วัดสองแพรก',
    'เกาะยาว',
    'บ้านน้ำจืด',
    'บ้านริมทะเล',
    'บ้านท่าเขา',
    'บ้านช่องหลาด',
    'บ้านคลองเหีย',
    'บ้านคลองบอน',
    'บ้านย่าหมี',
    'อ่าวมะม่วง',
    'บ้านท่าเรือ',
    'บ้านพรุใน',
    'อ่าวกะพ้อ',
    'บ้านคลองดินเหนียว',
    'บ้านกะปง',
    'กะปง',
    'บ้านปากถัก',
    'บ้านปากพู่ (รองหงส์ทองรัฐรังสรรค์)',
    'บ้านรมณีย์',
    'บ้านท่าหัน',
    'บ้านปากคลอง',
    'บ้านสะพานเสือ',
    'บ้านบางแก้ว',
    'บ้านช้างเชื่อ',
    'วัดนารายณิการาม',
    'บ้านกระโสม (ผดุงนิคมวิทยา)',
    'วัดสุวรรณคูหา',
    'บ้านหินสามก้อน',
    'บ้านเขาเปาะ',
    'บ้านกะไหล',
    'วัดศรีรัตนาราม',
    'บ้านป่ายาง',
    'บ้านบางนุ',
    'บ้านสามช่อง',
    'บ้านเกาะกลาง',
    'บ้านคลองเคียน',
    'บ้านคลองไส',
    'บ้านอ่าวมะขาม',
    'บ้านหินร่ม',
    'บ้านติเตะ',
    'บ้านเจ้าขรัว',
    'บ้านย่านสะบ้า',
    'วัดไตรมารคสถิตตั้งตรงจิตร 11 ฯ',
    'บ้านดอน',
    'บ้านอ่าวจิก',
    'บ้านเขาปิหลาย',
    'บ้านท่านุ่น',
    'บ้านท่าปากแหว่ง',
    'วัดราษฎร์สโมสร',
    'บ้านลำวะ',
    'วัดตรัยรัตนากร',
    'วัดดิตถาราม',
    'บ้านบางหลาม',
    'บ้านบากัน',
    'บ้านบางจัน',
    'บ้านควน',
    'บ้านทองหลาง',
    'บ้านแหลมหิน',
    'บ้านในหยง',
    'วัดนากลางมิตรภาพที่ 163',
    'บุญสูงอุปถัมภ์',
    'บ้านนอกนา',
    'บ้านบางเนียง',
    'วัดคมนียเขต',
    'บ้านปากวีปมิตรภาพที่ 124',
    'บ้านทุ่งใหญ่',
    'บ้านบางกรัก',
    'วัดควรนิยม',
    'บ้านดอกแดง',
    'บ้านในปิ',
    'บ้านบางใหญ่',
    'บ้านบางด้ง',
    'บ้านบางนายสี',
    'บ้านท่าจูด',
    'บ้านโคกยาง',
    'บ้านบางม่วง',
    'บ้านบางมรวน',
    'บ้านน้ำเค็ม',
    'ราชประชานุเคราะห์ 35',
    'เกียรติประชา',
    'บ้านห้วยทรัพย์มิตรภาพที่ 61',
    'บ้านสวนใหม่',
    'บ้านเตรียม',
    'บ้านบางหว้า',
    'บ้านหินลาด',
    'บ้านกลางประชาสรรค์',
    'คุระบุรี',
    'บ้านบางติบ',
    'บ้านทุ่งละออง',
    'บ้านบางวัน',
    'บ้านตำหนัง',
    'บ้านบางครั่ง',
    'บ้านคุรอด',
    'บ้านทับช้าง',
    'บ้านทุ่งรัก',
    'บ้านคุระ',
    'โชคอำนวย',
    'บ้านโคกเจริญ',
    'วัดโคกสวย',
    'บ้านทุ่งต่อเรือ',
    'บ้านถ้ำทองหลาง',
    'บ้านในวัง',
    'บ้านทุ่งไทรงาม',
    'วัดนิโครธาราม',
    'ทับปุด',
    'บ้านคอกช้าง',
    'วัดราษฎร์อุปถัมภ์',
    'ไทยรัฐวิทยา 54 (วัดไสเสียด)',
    'บ้านบ่อแสน',
    'บ้านท่าสนุก',
    'วัดนิโครธคุณากร',
    'วัดศรัทธาราม',
    'บ้านพอแดง',
    'วัดเหมืองประชาราม',
    'วัดปัตติการาม',
    'บ้านลำปี (พิบูลสงครามอุปถัมภ์)',
    'บ้านเขาน้อย',
    'ชาวไทยใหม่(โดมทักษิณอนุสรณ์)',
    'วัดประชุมศึกษา',
    'วัดอินทนิน',
    'บ้านฝ่ายท่า',
    'บ้านนาใน',
    'บ้านเกาะนก',
    'บ้านควนแรด',
    'นิคมสร้างตนเอง 1',
    'บ้านท่าแตง',
    'วัดประชาธิการาม',
    'บ้านห้วยทราย',
    'วัดลุมพินี',
    'บ้านบางคลี',
    'บ้านนาแฝก',
    'บ้านบ่อดาน',
    'บ้านในไร่',
    'วัดสุวรรณาวาส',
    'บ้านทุ่งดอน',
    'บ้านใหญ่',
    'บ้านบางทอง',
    'บ้านท่าดินแดง',
    'พระราชทานทับละมุ',
    'บ้านลำแก่น',
    'นิคมสร้างตนเอง 2',
    'บ้านคลองห้าง',
    'วัดช้างนอน',
    'บ้านบกปุย',
    'เกาะยาววิทยา',
    'กะปงพิทยาคม',
    'ตะกั่วทุ่งงานทวีวิทยาคม',
    'คลองเคียนรัฐราษฎร์รังสรรค์',
    'ทุ่งโพธิ์วิทยา',
    'ดีบุกพังงาวิทยายน',
    'ตะกั่วป่า(เสนานุกูล)',
    'สตรีพังงา',
    'ตะกั่วป่าคีรีเขต',
    'คุระบุรีพิทยาคม',
    'ทับปุดวิทยา',
    'ท้ายเหมืองวิทยา',
    'ทุ่งมะพร้าววิทยา',
    'บ้านกะตะ',
    'วัดสุวรรณคีรีเขต',
    'บ้านบางคู',
    'บ้านสะปำ',
    'เกาะมะพร้าว',
    'บ้านฉลอง',
    'วัดลัฎฐิวนาราม',
    'บ้านตลาดเหนือ',
    'อนุบาลภูเก็ต',
    'พิบูลสวัสดี',
    'บ้านกู้กู',
    'เกาะสิเหร่',
    'บ้านทุ่งคา',
    'เกาะโหลน',
    'เมืองภูเก็ต',
    'วัดสว่างอารมณ์',
    'วิชิตสงคราม',
    'บ้านนาบอน',
    'บ้านอ่าวน้ำบ่อ',
    'วัดเทพนิมิตร',
    'แหลมพันวา',
    'ราชประชานุเคราะห์ 36 จังหวัดภูเก็ต',
    'ไทยรัฐวิทยา 29',
    'บ้านบางทอง',
    'บ้านเก็ตโฮ่',
    'บ้านไม้เรียบ',
    'บ้านกะทู้',
    'บ้านกะหลิม',
    'วัดสุวรรณคีรีวงก์',
    'บ้านไสน้ำเย็น',
    'บ้านบางเทา',
    'บ้านเชิงทะเล',
    'บ้านแหลมทราย',
    'บ้านพรุจำปา',
    'บ้านป่าครองชีพ',
    'วัดเมืองใหม่',
    'ถลางพระนางสร้าง',
    'วัดเทพกระษัตรี',
    'บ้านโคกวัดใหม่',
    'บ้านอ่าวปอ',
    'สิทธิ์สุนทรบำรุง',
    'บ้านป่าคลอก',
    'บ้านพารา',
    'บ้านบางโรง',
    'เกาะนาคา',
    'บ้านไม้ขาว',
    'หงษ์หยกบำรุง',
    'บ้านหมากปรก',
    'บ้านคอเอน',
    'ท่าฉัตรไชย',
    'บ้านม่าหนิก',
    'วัดศรีสุนทร',
    'บ้านท่าเรือ',
    'บ้านลิพอน',
    'วัดมงคลวราราม',
    'บ้านสาคู',
    'บ้านในทอน',
    'ภูเก็ตวิทยาลัย',
    'สตรีภูเก็ต',
    'เฉลิมพระเกียรติสมเด็จพระศรีนครินทร์ ภูเก็ต',
    'กะทู้วิทยา',
    'เมืองถลาง',
    'เชิงทะเลวิทยาคม (จุติ-ก้องอนุสรณ์)',
    'วีรสตรีอนุสรณ์',
    'ภูเก็ตปัญญานุกูล',
    'นิคมสร้างตนเอง',
    'บ้านควนยูง',
    'บ้านซอย 2',
    'บ้านซอย 10',
    'บ้านดอนเกลี้ยง',
    'วัดนทีคมเขต',
    'วัดแหลมทอง',
    'วัดบุญบันเทิง',
    'บ้านทอนหญ้าปล้อง',
    'วัดโพธิ์นิมิต',
    'วัดท่าทอง',
    'บ้านโพหวาย',
    'บ้านสันติสุข',
    'วัดประสิทธาราม',
    'บ้านคลองสุข',
    'วัดชลธาร',
    'วัดบางกล้วย',
    'วัดบางใบไม้',
    'วัดโชติการาม',
    'บ้านสุชน',
    'วัดกลางใหม่',
    'บ้านบางใหญ่',
    'บ้านท่าเพชร',
    'อนุบาลสุราษฎร์ธานี',
    'ชุมชนวัดสุนทรนิวาส',
    'วัดสมหวัง',
    'บ้านบางชุมโถ',
    'บ้านบ่อน้ำร้อน',
    'บ้านแม่โมกข์',
    'บ้านกำสนราษฎร์อุทิศ',
    'วัดเขานางเภา',
    'บ้านไสใน',
    'วัดกาญจนาราม',
    'บ้านคลองสระ',
    'บ้านม่วงลีบ',
    'วัดคงคาล้อม',
    'บ้านมะม่วงหวาน',
    'บ้านไสตอ',
    'วัดพุฒ',
    'บ้านสะพานกฐิน',
    'บ้านหัวหมากล่าง',
    'บ้านห้วยโศก',
    'บ้านควนราชา',
    'บ้านหัวหมากบน',
    'บ้านห้วยด่าน',
    'วัดกงตาก',
    'บ้านไสขาม',
    'วัดปากคู',
    'วัดวชิรประดิษฐ์',
    'วัดแสงประดิษฐ์',
    'วัดนิกรประสาท',
    'บ้านดอนสน',
    'วัดเขาพระนิ่ม',
    'บ้านท่าโพธิ์',
    'วัดดอนยาง',
    'วัดประสพ',
    'บ้านบางสำโรง',
    'วัดท่าไทร(ดิตถานุเคราะห์)',
    'บ้านศิลางาม',
    'วัดบ้านใน',
    'วัดอุทยาราม',
    'ไทยรัฐวิทยา 88 (บ้านคลองควน)',
    'บ้านกงหนิง',
    'บ้านคลองนามิตรภาพที่ 201',
    'บ้านดอนหลวง',
    'บ้านวังทองสามัคคี',
    'บ้านทับท้อน',
    'บ้านบ่อโฉลก',
    'วัดเขาแก้ว',
    'บ้านวังหวาย',
    'บ้านควนนิมิต',
    'บ้านคลองกรูด',
    'วัดวังไทร',
    'บ้านคีรีรอบ',
    'บ้านกำสนประชาสรรค์',
    'บ้านท่าเสาเภา',
    'บ้านหนองเปล',
    'บ้านปากกะแดะ',
    'วัดพ่วง',
    'วัดชลคราม',
    'วัดนทีวัฒนาราม',
    'วัดนอก',
    'บ้านศรีชัยคราม',
    'วัดท้องอ่าว',
    'ชุมชนวัดวิสุทธิชลาราม',
    'บ้านเกาะแรต',
    'บ้านปากดอนสัก',
    'วัดเขาสุวรรณประดิษฐ์',
    'บ้านเขาแค',
    'บ้านน้ำฉา',
    'บ้านห้วยเสียด',
    'วัดสิงขร',
    'บ้านเกาะนกเภา',
    'ชุมชนบ้านนางกำ',
    'บ้านใหม่สามัคคี',
    'วัดคีรีวง',
    'บ้านคอกช้าง',
    'บ้านดอนเสาธง',
    'บ้านเขาพระอินทร์',
    'บ้านคลองคราม',
    'องค์การอุตสาหกรรมป่าไม้ 9',
    'บ้านดินแดงสามัคคี',
    'วัดสระเกศ',
    'วัดคีรีมาส',
    'วัดคีรีวงการาม',
    'วัดสันติวราราม',
    'วัดสว่างอารมณ์',
    'บ้านหาดงาม',
    'วัดบุณฑริการาม',
    'บ้านบ่อผุด',
    'บ้านบางรักษ์',
    'บ้านปลายแหลม',
    'บ้านหน้าค่าย',
    'วัดละไม',
    'วัดภูเขาทอง',
    'บ้านแหลมหอย',
    'บ้านดอนธูป',
    'วัดนาราเจริญสุข',
    'วัดสมุทราราม',
    'วัดกลาง',
    'วัดคุณาราม',
    'วัดประเดิม',
    'วัดแจ้ง',
    'บ้านเกาะพลวย',
    'บ้านอ่างทอง',
    'บ้านเกาะเต่า',
    'บ้านโฉลกหลำ',
    'บ้านมะเดื่อหวาน',
    'บ้านศรีธนู',
    'วัดสมัยคงคา',
    'วัดราษฎร์เจริญ',
    'บ้านท้องนายปาน',
    'บ้านหาดริ้น',
    'ชุมชนบ้านใต้',
    'สุราษฎร์ธานี',
    'สุราษฎร์พิทยา',
    'เมืองสุราษฎร์ธานี',
    'ประดู่พิทยาคม',
    'มัธยมพัชรกิติยาภา ๓ สุราษฎร์ธานี',
    'สุราษฎร์ธานี 2',
    'สอนคนตาบอดภาคใต้ จังหวัดสุราษฎร์ธานี',
    'ศึกษาสงเคราะห์สุราษฎร์ธานี',
    'กาญจนดิษฐ์',
    'กาญจนดิษฐ์วิทยาคม',
    'ท่าอุแทพิทยา',
    'ท่าเฟืองวิทยา',
    'ดอนสักผดุงวิทย์',
    'ปากแพรกวิทยาคม',
    'เกาะสมุย',
    'ทีปราษฎร์พิทยา',
    'เกาะพะงันศึกษา',
    'วัดชยาราม',
    'สารภีอุทิศ',
    'บ้านตะกรบ',
    'บ้านห้วยพุน',
    'วัดวิชิตธารามราม',
    'วัดไตรรัตนากร',
    'บ้านนาแค',
    'บ้านหนองมน',
    'บ้านนา',
    'บ้านห้วยตาหมิง',
    'บ้านพรุยายชี',
    'วัดวิโรจนาราม',
    'บ้านคลองไม้แดง',
    'บ้านลุ่มชุมแสง',
    'บ้านห้วยไผ่',
    'วัดเขาพนมแบก',
    'วัดเดิมเจ้า',
    'วัดเววน',
    'บ้านเหนือน้ำ',
    'บ้านแหลมทราย',
    'วัดพุมเรียง',
    'วัดโพธาราม(โพธิพิทยากร)',
    'บ้านทือ',
    'บ้านทุ่งนางเภา',
    'วัดสุทธาวาส',
    'วัดจาย(สุทธบูรราษฎร์)',
    'วัดธารน้ำไหล',
    'วัดรัตนาราม',
    'บ้านไทรงาม',
    'บ้านสามสัก',
    'วัดพระบรมธาตุไชยา',
    'บ้านกลาง',
    'บ้านคลองพา',
    'บ้านชายท่า',
    'บ้านมะม่วงงาม',
    'บ้านท่าไท',
    'บ้านขวัญพัฒน์',
    'บ้านควนสูง',
    'บ้านคันธุลี',
    'วัดสังขประดิษฐ์',
    'บ้านหนองสะบ้า',
    'บ้านแหลมดิน',
    'ตลาดหนองหวาย',
    'บ้านปากน้ำท่ากระจาย',
    'บ้านหนองปรือ',
    'วัดชัยธาราวาส',
    'วัดสุมังคลาราม',
    'บ้านเกาะมุกด์',
    'บ้านดอนมะกอก',
    'บ้านศรีพนม',
    'วัดมหาถูปาราม',
    'วัดมัชฌิมาราม',
    'บ้านอู่ตะเภา',
    'วัดกาฬสินธุ์',
    'บ้านคลองรอก',
    'บ้านคลองสงค์',
    'ต.ช.ด.ราษฎร์พิทักษ์',
    'วัดอัมพาวาส',
    'วัดขจรบำรุง',
    'บ้านดินก้อง',
    'บ้านทุ่งพลับ',
    'บ้านทุ่งเสียน',
    'บ้านหนองเหรียง',
    'วัดศรีสุวรรณ',
    'บ้านตะเคียนทอง',
    'บ้านปากทำเรียง',
    'วัดโกศาวาส',
    'วัดอินทราวาส',
    'บ้านต้นมะพร้าวมิตรภาพที่ 91',
    'บ้านบ่อน้ำผุด',
    'วัดถ้ำสิงขร',
    'บ้านดอนสุวรรณ',
    'บ้านย่านยาว',
    'บ้านหินดาน',
    'บ้านปากโตน',
    'บ้านโตนยาง',
    'บ้านทรายแก้ว',
    'บ้านท่าขนอน',
    'คีรีรัฐนิคม',
    'วัดน้ำหัก',
    'บ้านบางขนุน',
    'มหาราชบ้านแสงอรุณ',
    'บ้านน้ำราด',
    'บ้านทำเนียบ',
    'วัดนิลาราม',
    'บ้านยาง',
    'บ้านท่ากระดาน',
    'บ้านปากหาร',
    'บ้านเชี่ยวหมวง',
    'บ้านวังพลาย',
    'บ้านท่าไคร',
    'บ้านบางพระ',
    'บ้านมะเลาะ',
    'บ้านย่านมะปราง',
    'บ้านเขาเทพพิทักษ์',
    'บ้านเชี่ยวหลาน',
    'บ้านพัฒนา',
    'บ้านพัฒนา 2',
    'บ้านวังขุม',
    'ตาขุน',
    'บ้านช่องไม้งาม',
    'บ้านเชี่ยวขวาน',
    'วัดพรุศรี',
    'วัดพะแสง',
    'บ้านตาขุน',
    'บ้านปากน้ำ',
    'บ้านบางปรุ',
    'บ้านหญ้าปล้อง',
    'วัดถ้ำวราราม',
    'วัดสองพี่น้อง',
    'บ้านเขานาใน',
    'บ้านจำปาทอง',
    'บ้านถ้ำผึ้ง',
    'บ้านป่าตง',
    'วัดปากตรัง',
    'บ้านคลองชะอุ่น',
    'บ้านต้นยวน',
    'บ้านบางหิน',
    'บ้านแสนสุข',
    'บ้านอรุโณทัย',
    'บ้านบางสาน',
    'บ้านลูกเดือน',
    'วัดพนม',
    'บ้านใหญ่',
    'วัดธัญญาราม',
    'บ้านเบญจา',
    'บ้านบางโหว่',
    'บ้านช่องม้าเหลียว',
    'บ้านพังกาญจน์',
    'บ้านควนรา',
    'วัดน้ำพุ',
    'วัดบางน้ำจืด',
    'บ้านควนสุวรรณ',
    'ชุมชนบ้านโตนดสูง',
    'บ้านท่าแซะ',
    'วัดนันทาราม',
    'วัดอัมพาราม',
    'วัดพระพรหม',
    'วัดบางปอ',
    'บ้านคลองวัว',
    'วัดบ่อมะปริง',
    'วัดอินทาราม',
    'วัดจันทาราม',
    'บ้านคชาธาร',
    'บ้านหน้าซึง',
    'วัดบางคราม',
    'วัดประตูใหญ่',
    'วัดท่า',
    'บ้านแม่แขก',
    'บ้านสหกรณ์',
    'วัดประชาวงศาราม',
    'บ้านท่านาง',
    'วัดสันติคีรีรมย์',
    'วัดราษฎร์บำรุง',
    'บ้านราษฎร์ประดิษฐ์',
    'บ้านปลายคลอง',
    'บ้านบนไร่',
    'บ้านห้วยทรายขาว',
    'เจริญเวชวิทยา',
    'วัดท่าตลิ่งชัน',
    'บ้านท่าตะเภา',
    'วัดตรณาราม',
    'บ้านไทรงาม',
    'บ้านนาใหญ่',
    'วัดทุ่งเซียด',
    'วัดนาคาวาส',
    'วัดสุวรรณโกฏิ',
    'บ้านหนองจอก',
    'บ้านนาค้อ',
    'บ้านบ่อกรัง',
    'วัดน้ำรอบ',
    'บ้านดอนมะตูม',
    'บ้านราษฎร์ประสานจิต',
    'วัดเกษมบำรุง',
    'วัดบางงอน',
    'วัดรัษฎาราม',
    'บ้านแท่นแก้ว',
    'วัดจันทร์ประดิษฐาราม',
    'วัดบางมะเดื่อ',
    'บ้านท่าม่วง',
    'บ้านบางประชาภิบาล',
    'บ้านศรีปทุมวัลย์',
    'วัดสระพัง',
    'บ้านทุ่งโพธิ์',
    'วัดแหลมไผ่',
    'บ้านชุมแสง',
    'วัดห้วยกรวด',
    'บ้านคลองราง',
    'วัดตรีธาราราม',
    'วัดบางพลา',
    'วัดเขาศรีวิชัย',
    'วัดสามัคคีธรรมาราม',
    'บ้านอ่างทอง',
    'วัดหนองไทร',
    'บ้านยางงาม',
    'วัดหัวเตย',
    'บ้านเชี่ยวมะปราง',
    'บ้านโพธิ์พนา',
    'บ้านวังผักแว่น',
    'วัดโพธิ์น้อย',
    'บ้านเชี่ยวเฟือง',
    'บ้านท่านหญิงวิภา',
    'วัดอรัญญาราม',
    'สหกรณ์นิคม',
    'บ้านท่าใหม่',
    'บ้านเคี่ยมเพาะ',
    'ท่าฉางวิทยาคาร',
    'ไชยาวิทยา',
    'ท่าชนะ',
    'คีรีรัฐวิทยาคม',
    'บ้านตาขุนวิทยา',
    'รัชชประภาวิทยาคม',
    'เสวียดวิทยา รัชมังคลาภิเษก',
    'เขาพนมแบกศึกษา',
    'มัธยมบ้านทำเนียบ',
    'กาญจนาภิเษกวิทยาลัย สุราษฎร์ธานี',
    'พุนพินพิทยาคม',
    'น้ำรอบวิทยา',
    'ท่าสะท้อนวิทยา',
    'บางเดือนสถิตย์พิทยาคม',
    'ตะกุกใต้ศึกษา',
    'มัธยมวิภาวดี',
    'บ้านคลองปราบ',
    'บ้านหนองม่วง',
    'บ้านควนพรุพี',
    'บ้านควนมหาชัย',
    'บ้านควนเนียง',
    'วัดควนศรี',
    'ควนสุบรรณ',
    'บ้านโกงเหลง',
    'วัดไทรโพธิ์งาม',
    'บ้านทุ่งตำเสา',
    'บ้านนาควน',
    'บ้านยูงงาม',
    'วัดท่าเจริญ',
    'บ้านทุ่งในไร่',
    'บ้านไร่เหนือ',
    'วัดวิเวการาม',
    'บ้านห้วยตอ',
    'บ้านไสดง',
    'บ้านห้วยชัน',
    'บ้านขุนราษฎร์',
    'บ้านควนกองเมือง',
    'บ้านนายาง',
    'นาสาร',
    'วัดน้ำพุ',
    'บ้านยางอุง',
    'บ้านดอนทราย',
    'บ้านควนใหม่',
    'บ้านช่องช้าง',
    'บ้านห้วยล่วง',
    'ราชประชานุเคราะห์ 12',
    'บ้านกอบแกบ',
    'บ้านเหมืองทวด',
    'บ้านวังหิน',
    'บ้านเขาน้อย',
    'บ้านคลองหาเหนือ',
    'บ้านปลายน้ำ',
    'ทรัพย์ทวี',
    'วัดควนท่าแร่',
    'วัดปัฏนาราม',
    'วัดอินทการาม',
    'บ้านปากด่าน',
    'บ้านคลองยา',
    'บ้านหนองหญ้าปล้อง',
    'บ้านหนองเรียน',
    'บ้านดอนงาม',
    'บ้านทรายทอง',
    'บ้านธารอารี',
    'บ้านนา',
    'บ้านห้วยห้าง',
    'บ้านห้วยใหญ่',
    'บ้านสวยศรี',
    'บ้านเกาะแก้ว',
    'บ้านเขาตอก',
    'บ้านคลองจัน',
    'บ้านทุ่งจูด',
    'บ้านสี่แยกสามัคคี',
    'บ้านบางประสามัคคี',
    'วัดดอนพยอม',
    'วัดเพ็งประดิษฐาราม',
    'ศึกษาประชาคม',
    'สุธรรมเจริญมิตรภาพที่ 130',
    'จิตประชาราษฎร์',
    'บ้านในปราบราษฎร์ประสงค์',
    'ไทยรัฐวิทยา 22',
    'บ้านเขารักษ์',
    'บ้านเขาสามยอด',
    'บ้านทับใหม่',
    'บ้านหน้าเขา',
    'บ้านไร่ยาว',
    'บ้านบางดี',
    'บ้านพรหมรังสิต',
    'บ้านสี่แยกคลองศิลา',
    'สมบูรณ์ประชาสรรค์',
    'บ้านคลองโร',
    'บ้านควนคีรีวงศ์',
    'บ้านควนสามัคคี',
    'บ้านบางใหญ่สามัคคี',
    'บ้านห้วยมุด',
    'มิตรประชาราษฎร์',
    'วัดอรัญคามวารี',
    'บ้านเขานิพันธ์',
    'บ้านเขาปูน',
    'บ้านคลองลำพลา',
    'บ้านควนกลาง',
    'บ้านควนเจริญ',
    'บ้านกลาง',
    'บ้านคลองกา',
    'บ้านควนสูง',
    'บ้านใต้',
    'บ้านปากกา',
    'บ้านปากหาน',
    'บ้านห้วยทรายทอง',
    'วัดคลองฉนวน',
    'บ้านยูงงาม',
    'บ้านไสยง',
    'บ้านปากลัด',
    'บ้านปากสาย',
    'บ้านลานเข้',
    'บ้านวังใหญ่',
    'บ้านหานเพชร',
    'วัดโสภณประชาราม',
    'บ้านโคกมะพร้าว',
    'วัดทุ่งหลวง',
    'มหาราช 2',
    'บ้านนาชุมเห็ด',
    'บ้านห้วยมะนาว',
    'บ้านในไร่',
    'บ้านพรุแชง',
    'บ้านส้องเหนือ',
    'วัดบ้านส้อง',
    'สามัคคีอนุสรณ์',
    'บ้านส้อง',
    'บ้านหัวสะพานมิตรภาพที่ 217',
    'บ้านหนองโสน',
    'วัดคลองตาล',
    'วัดเวียงสระ',
    'บ้านควนบก',
    'บ้านไทรห้อง',
    'นิคมสร้างตนเองพระราชประสงค์ 1',
    'นิคมสร้างตนเองพระราชประสงค์ 3',
    'บ้านควนสว่างวัฒนา',
    'บ้านบางน้ำเย็น',
    'บ้านไสท้อน',
    'บ้านควนยอ',
    'วัดสามพัน',
    'บ้านไสดง',
    'บ้านประตูพลิก',
    'บ้านราษฎร์พัฒนา',
    'บ้านหมาก',
    'วัดบางพา',
    'บ้านเกาะน้อย',
    'บ้านคลองโหยน',
    'วัดบางสวรรค์',
    'ปัญญาประชาอุทิศ',
    'อุดมมิตรพัฒนา',
    'บ้านหน้าเขา',
    'บ้านห้วยแห้ง',
    'บ้านบางใหญ่',
    'องค์การอุตสาหกรรมป่าไม้ 18',
    'บ้านสาคูมิตรภาพที่ 195',
    'บ้านทุ่งหญ้าแดง',
    'บ้านบางเหรียง',
    'บ้านบางใหญ่สินปุน',
    'บ้านบางรูป',
    'วัดบางกำยาน',
    'หมู่บ้านป่าไม้พระราชประสงค์ 1',
    'บ้านเมรัย',
    'ประชาอุทิศ',
    'บ้านควนนิยม',
    'บ้านควนมหาชัย',
    'บ้านโคกมะม่วง',
    'บ้านยูงทอง',
    'บ้านไสขรบ',
    'บ้านบางหยด',
    'บ้านปลายสาย',
    'บ้านย่านดินแดง',
    'บ้านบ่อพระ',
    'บ้านควนพัฒนาประชาสรรค์',
    'บ้านบางหอย',
    'บ้านปลายศอก',
    'บ้านปลายคลอง',
    'บ้านคลองพังกลาง',
    'บ้านยวนปลา',
    'บ้านบางปาน',
    'วัดสองแพรก',
    'บ้านควนสินชัย',
    'บ้านควนสระ',
    'วัดสมัยสุวรรณ',
    'บ้านเสด็จพิทยาคม',
    'บ้านนาสาร',
    'บ้านนาวิทยาคม',
    'เคียนซาพิทยาคม',
    'เวียงสระ',
    'พระแสงวิทยา',
    'พรุพีพิทยาคม',
    'พ่วงพรมครวิทยา',
    'คลองฉนวนวิทยา',
    'บางสวรรค์วิทยาคม',
    'ควนสุบรรณวิทยา',
    'ท่าชีวิทยาคม',
    'ชัยบุรีพิทยา',
    'บ้านเกาะพยาม',
    'บ้านเกาะช้าง',
    'อนุบาลระนอง',
    'ชาติเฉลิม',
    'บ้านน้ำตก',
    'บ้านหินดาด',
    'บ้านทรายแดง',
    'บ้านบางสีกิ้ม',
    'บ้านบางนอน',
    'เอกศิลป์ราษฎร์พัฒนา',
    'ระนองพัฒนามิตรภาพที่ 60',
    'บ้านบางริ้น',
    'บ้านบางกลาง',
    'บ้านทุ่งหงาว',
    'บ้านเกาะสินไห',
    'บ้านเขานางหงส์',
    'บ้านเกาะเหลา',
    'บ้านปากน้ำ',
    'บ้านหินช้าง',
    'บ้านนกงาง',
    'บ้านราชกรูด',
    'บ้านขจัดภัย',
    'บ้านคลองของ',
    'บ้านละออง',
    'บ้านท่าฉาง',
    'บ้านหาดทรายดำ',
    'บ้านหาดทรายดำ (สาขาบ้านหน้านอก)',
    'วัดหาดส้มแป้น',
    'บ้านเขาฝาชี',
    'ทับไชยาพัฒนา',
    'วัดช่องลม',
    'ทุ่งตาพลวิทยา',
    'บ้านบางนา',
    'บ้านคอกช้าง',
    'ระวิราษฎร์บำรุง',
    'ทอนเสียดวิทยาคาร',
    'บ้านบางขุนแพ่ง',
    'บ้านละอุ่นใต้',
    'บ้านปากแพรก',
    'ประชาอุทิศ',
    'ชนม์พัฒนา',
    'บ้านบางปรุ',
    'บ้านห้วยเสียด',
    'บ้านด่าน',
    'บ้านเชี่ยวเหลียง',
    'บ้านชนะบำรุง',
    'บ้านบางหิน',
    'บ้านชาคลี',
    'บ้านทองหลาง',
    'บ้านนา',
    'บ้านบางเบน',
    'บ้านสำนัก',
    'นิคมสงเคราะห์',
    'บ้าน จ.ป.ร.',
    'บ้านหินวัว',
    'บ้านทุ่งมะพร้าว',
    'บ้านน้ำขาว',
    'บ้านรังแตน',
    'บ้านดอนกลาง',
    'บ้านปลายคลอง',
    'บ้านบกกราย',
    'บ้านบางกุ้ง',
    'กระบุรี',
    'บ้านน้ำจืดน้อย',
    'บ้านน้ำแดง',
    'บ้านบางสองรา',
    'บ้านทับจาก',
    'วัดสุวรรณคีรี',
    'บ้านหนองจิก',
    'บ้านหาดจิก',
    'บ้านคลองเงิน',
    'บ้านปลายคลองวัน',
    'บ้านสวัสดิ์',
    'บ้านสองพี่น้อง',
    'มัชฌิมวิทยา',
    'ทับหลีสุริยวงศ์',
    'บ้านลำเลียง',
    'บ้านสองแพรก',
    'ไทยรัฐวิทยา 97 (บ้านบางบอน)',
    'บ้านภูเขาทอง',
    'บ้านทะเลนอก',
    'บ้านกำพวน',
    'บ้านสุขสำราญ',
    'บ้านบางมัน',
    'บ้านแหลมนาว',
    'ไทยรัฐวิทยา 67 (บ้านนาพรุ)',
    'บ้านควนไทรงาม',
    'บ้านบางกล้วยนอก',
    'พิชัยรัตนาคาร',
    'สตรีระนอง',
    'ราชประชานุเคราะห์ 38',
    'ละอุ่นวิทยาคาร',
    'กะเปอร์วิทยา',
    'กระบุรีวิทยา',
    'ปากจั่นวิทยา',
    'สุขสำราญ',
    'วัดขุนกระทิง',
    'วัดหาดทรายแก้ว',
    'บ้านในห้วย',
    'ชุมชนบ้านถ้ำสิงห์',
    'บ้านควนจำปา',
    'อนุบาลชุมพร',
    'วัดดอนทรายแก้ว',
    'วัดพิชัยยาราม',
    'บ้านคอสน',
    'บ้านบางคอย',
    'บ้านบางหลง',
    'บ้านช่องไทรราษฎร์พัฒนา',
    'บ้านทุ่งหงษ์',
    'บ้านเขาบ่อ',
    'วัดเขาดิน',
    'วัดดอนมะม่วง',
    'วัดทุ่งคา',
    'อนุบาลเมืองชุมพร(วัดสุบรรณนิมิตร)',
    'วัดหูรอ',
    'บ้านสามเสียม',
    'ชุมชนบ้านนาชะอัง',
    'วัดนาทุ่ง',
    'บ้านหนองเนียน',
    'วัดบางลึก',
    'บ้านหนองส้ม',
    'บ้านศาลาลอย',
    'บ้านดอนรัก',
    'วัดคูขุด',
    'บ้านดอนไทรงาม',
    'บ้านคอเตี้ย',
    'วัดดอนรวบ',
    'บ้านเขาวง',
    'บ้านท่ามะปริง',
    'วัดหัวกรูด',
    'บ้านปากน้ำ (เผดิมจีนนาวาสงเคราะห์)',
    'บ้านหัวถนน',
    'บ้านหาดภราดรภาพ',
    'เมืองชุมพรบ้านเขาถล่ม',
    'วัดดอนเมือง',
    'วัดวังไผ่',
    'ศรีอุทัยธรรมพัฒนา',
    'ไทยรัฐวิทยา 66(บ้านนาเนียน)',
    'วัดน้อมถวาย',
    'บ้านนาแซะ',
    'บ้านท่าไม้ลาย',
    'วัดแหลมลำภู',
    'วัดเชิงกระ',
    'บ้านคลองสูบ',
    'บ้านเขาชันโต๊ะ',
    'บ้านเขายาวราษฎร์พัฒนา',
    'บ้านหาดทรายรี',
    'บ้านทุ่งมะขาม',
    'บ้านเขาน้อย',
    'ชุมชนวัดหาดพันไกร',
    'บ้านหนองจันทน์หอม',
    'บ้านใหม่พัฒนา',
    'ประชานิคม 2',
    'บ้านท่าลานทอง',
    'ชุมชนบ้านคุริง (มิตรภาพที่ 25)',
    'บ้านแก่งเพกา',
    'บ้านหาดหงส์',
    'ประชานิคม 3',
    'บ้านเขาพาง',
    'บ้านหนองเรือ',
    'บ้านหัวว่าว',
    'บ้านหาดส้มแป้น',
    'อนุบาลท่าแซะ(บ้านท่าแซะ)',
    'ชุมชนประชานิคม',
    'บ้านทุ่งบิ่น',
    'ไทยรัฐวิทยา 77(บ้านเนินสันติ)',
    'วัดควนมณี',
    'วัดวังครก',
    'ประชาสันติ',
    'บ้านละมุ',
    'บ้านดอนเคี่ยม',
    'บ้านรับร่อ',
    'บ้านหาดใน',
    'บ้านบางไม้แก้วประชาสามัคคี',
    'บ้านเหมืองทอง',
    'บ้านคันธทรัพย์',
    'บ้านเนินทอง',
    'บ้านสวนทรัพย์',
    'บ้านพรุตะเคียน',
    'บ้านใหม่สมบูรณ์',
    'บ้านงาช้าง',
    'ประชาพัฒนา',
    'บ้านร้านตัดผม',
    'บ้านทรายขาว',
    'บ้านธรรมเจริญ',
    'บ้านตาหงษ์',
    'บ้านกลาง',
    'บ้านบึงลัด',
    'ประชานิคม 4',
    'บ้านยายไท',
    'บ้านหินแก้ว',
    'บ้านน้ำเย็น',
    'บ้านจันทึง',
    'บ้านทรายแก้ว',
    'บ้านหนองผาก',
    'ไทยรัฐวิทยา 76 (บ้านพละ)',
    'บ้านเขาเลี้ยว',
    'บ้านไชยราช',
    'บ้านวังช้าง',
    'บ้านหินกบ',
    'บ้านบางจาก',
    'บ้านชุมโค',
    'สหกรณ์ประชานุกูล',
    'บ้านดอนทราย',
    'บ้านทรัพย์สมบูรณ์',
    'บ้านบ่ออิฐ',
    'บ้านทุ่งเรี้ย',
    'ชุมชนมาบอำมฤต',
    'วัดดอนยาง',
    'บ้านห้วยรากไม้',
    'ชุมชนบ้านทะเลทรัพย์',
    'บ้านคอกม้า',
    'อนุบาลปะทิว(บางสนพิพิธราษฏร์บำรุง)',
    'บ้านดอนตะเคียน',
    'วัดดอนกุฎี',
    'บ้านน้ำพุ',
    'บ้านถ้ำธง',
    'บ้านบนไร่',
    'บ้านปากคลอง',
    'วัดบางแหวน',
    'บ้านบ่อสำโรง',
    'ไทยรัฐวิทยา 78 (วัดสามัคคีชัย)',
    'อนุบาลบ้านเนินสำลี',
    'วัดปากด่าน',
    'บ้านสะพลี(บางกอกโพสก์ศึกษา)',
    'บ้านทุ่งวัวแล่น',
    'ศรียาภัย',
    'สอาดเผดิมวิทยา',
    'ปากน้ำชุมพรวิทยา',
    'ทุ่งคาพิทยาคาร',
    'เมืองชุมพรวิทยา',
    'ท่าแซะรัชดาภิเษก',
    'ท่าข้ามวิทยา',
    'หงษ์เจริญวิทยาคม',
    'ราชประชานุเคราะห์ 20',
    'ปะทิววิทยา',
    'มาบอำมฤตวิทยา',
    'วัดสุวรรณคีรี',
    'วัดถ้ำเขาเงิน',
    'วัดราษฎร์บูรณาราม',
    'บ้านดอนนน',
    'วัดดอนวาส',
    'บ้านในเหมือง',
    'บ้านหนองปลา',
    'วัดนพคุณ',
    'บ้านดอนตาผล',
    'บ้านควนตะวันออก',
    'บ้านสะพานยูง',
    'วัดอัมพาวาส',
    'วัดราษฎ์อรุณ',
    'วัดชลธีนิมิตร',
    'บ้านบางหยี',
    'บ้านท้องครก',
    'บ้านน้ำลอด',
    'บ้านหนองไก่ปิ้ง',
    'วัดประสาทนิกร',
    'บ้านปากน้ำหลังสวน',
    'วัดนาทิการาม',
    'วัดราษฎร์บำรุง',
    'บ้านห้วยหลอด',
    'บ้านน้ำตก',
    'วัดชลธารวดี',
    'บ้านคลองกก',
    'บ้านหนองโพธิ์ มิตรภาพที่ 129',
    'บ้านทับวัง',
    'สหกรณ์นิคมอุปถัมภ์',
    'วัดสว่างมนัส',
    'วัดดอนชัย',
    'บ้านคลองระ',
    'ชุมชนวัดขันเงิน',
    'วัดสมุหเขตตาราม',
    'บ้านเขาแงน',
    'ชุมชนวัดหาดสำราญ',
    'บ้านในกริม',
    'ประชาบุษยวิทย์',
    'บ้านสวนจันทน์',
    'วัดวาลุการาม',
    'วัดวิเวการาม',
    'ประชาสงวน',
    'สหกรณ์พัฒนา',
    'บ้านทับใหม่',
    'วัดโสมสิริวัฒนาราม',
    'วัดสุวรรณนิมิต',
    'ชุมชนบ้านเขาหลาง',
    'บ้านทรายทอง',
    'บ้านแหลมสันติ',
    'ประชาเอื้ออารี',
    'สามัคคีวัฒนา',
    'บ้านคลองสง',
    'วัดปิยะวัฒนาราม',
    'วัดปากน้ำละแม',
    'บ้านดอนแค',
    'บ้านดวด',
    'บ้านเขาตากุน',
    'บ้านสวนสมบูรณ์',
    'บ้านท่าแพราษฎร์พัฒนา',
    'บ้านคลองเหนก',
    'บ้านพังเหา',
    'บ้านปังหวาน',
    'บ้านต่อตั้ง',
    'บ้านปากทรง',
    'พัฒนศึกษา',
    'บ้านพระรักษ์ราษฎร์บำรุง',
    'บ้านโหมงราษฎร์พัฒนา',
    'อนุบาลพะโต๊ะ',
    'บ้านตรัง (จันทสิงห์อุทิศ)',
    'บ้านในหยาน',
    'บ้านเขาตะเภาทอง',
    'บ้านปากเลข',
    'บ้านในหุบ',
    'บ้านเขาค่าย',
    'บ้านทุ่งคาโตนด',
    'บ้านเขาทะลุ',
    'บ้านห้วยกลาง',
    'บ้านไทรล่า',
    'บ้านเขาล้าน',
    'บ้านน้ำฉา',
    'บ้านคู',
    'วัดจันทราวาส',
    'บ้านนาเหรี่ยง',
    'วัดบรรพตวารีเขต',
    'วัดถ้ำเขาล้าน',
    'บ้านเล็บกะรอก',
    'วัดท้องตม',
    'บ้านหาดทรายรี',
    'บ้านหนองปลา',
    'วัดท่าหิน (ดรุณศึกษา)',
    'วัดชุมแสง',
    'บ้านน้ำลอดน้อย',
    'บ้านคลองน้อย',
    'บ้านห้วยชัน',
    'วัดธัญญาราม',
    'ชุมชนสวี',
    'อนุบาลสวี(บ้านนาโพธิ์)',
    'บ้านไทยพัฒนา',
    'ห้วยทรายขาว',
    'บ้านเขาน้อยสามัคคี',
    'บ้านยางงาม',
    'วัดนาสัก',
    'บ้านแก่งกระทั่ง',
    'บ้านห้วยใหญ่',
    'วัดหนองบัว',
    'วัดเชิงคีรี',
    'วัดควน',
    'บ้านทุ่งเขาสะบ้า',
    'ค่ายลูกเสืออำเภอสวี',
    'บ้านแพรกแห้ง',
    'วัดแหลมปอ',
    'วัดพะงุ้น',
    'บ้านห้วยทับทอง',
    'บ้านวังปลา',
    'วัดผุสดีภูผาราม',
    'บ้านเขาวงกรด',
    'บ้านทับช้าง',
    'บ้านแหลมยางนา',
    'บ้านสามแยกจำปา',
    'วัดเทพนิมิตวนาราม',
    'ชุมชนวัดท่าสุธาราม',
    'วัดท่าทอง',
    'บ้านห้วยมุด',
    'ราชประชานุเคราะห์ 3',
    'วัดมุจลินทาราม',
    'ชุมชนวัดธรรมถาวร',
    'บ้านอ่าวมะม่วง',
    'วัดชลธีพฤกษาราม',
    'สวนศรีวิทยา',
    'ปากน้ำหลังสวนวิทยา',
    'ชลธารวิทยา',
    'เมืองหลังสวน',
    'ละแมวิทยา',
    'พะโต๊ะวิทยา',
    'สวีวิทยา',
    'นาสักวิทยา',
    'ด่านสวีวิทยา',
    'เขาทะลุพิทยาคม',
    'ครนพิทยาคม',
    'ทุ่งตะโกวิทยา',
    'วัดสามกอง',
    'ชุมชนบ้านด่าน',
    'บ้านบ่ออิฐ',
    'วัดท้ายยอ',
    'วัดแหลมพ้อ',
    'วัดเกาะถํ้า',
    'วัดแช่มอุทิศ',
    'วัดทุ่งหวังใน',
    'บ้านยางงาม (ศุภรธรรมนิวิฏฐ์)',
    'วัดอ่างทอง',
    'วัดทรายขาว',
    'วิเชียรชม',
    'อนุบาลสงขลา',
    'บ้านบางดาน',
    'อนุบาลบ้านท่าสะอ้าน)',
    'บ้านน้ำกระจาย',
    'บ้านดอนขี้เหล็ก',
    'บ้านกลาง',
    'วัดกลาง',
    'วัดกระดังงา',
    'บ้านท่าคุระ',
    'วัดโคกโพธิ์',
    'ชุมชนวัดคลองรีมิตรภาพที่220',
    'วัดประดู่หอม',
    'วัดธรรมประดิษฐ์',
    'วัดศรีไชย',
    'วัดคูขุด',
    'วัดแหลมวัง',
    'ในเมือง',
    'บ้านจะทิ้งพระ',
    'บ้านพังเภา',
    'วัดนางเหล้า',
    'สมเด็จเจ้าพะโคะ',
    'วัดดีหลวง',
    'บ้านชะแม',
    'วัดโพธิ์กลาง',
    'วัดท่าหิน',
    'วัดห้วยลาด',
    'วัดผาสุกาวาส',
    'บ้านวัดใหม่',
    'วัดบ่อแดง',
    'ชุมชนบ้านบ่อประดู่',
    'วัดจันทน์',
    'วัดพังกก',
    'วัดประเจียก',
    'วัดสนามไชย',
    'ราชประชานุเคราะห์ 11',
    'วัดหัวคุ้ง',
    'บ้านมาบหัวทีง',
    'วัดทุ่งสงวน',
    'วัดบางหรอด',
    'บ้านแม่ใหญ่',
    'วัดเกษตรชลธี',
    'บ้านดอนแบก',
    'บ้านมาบบัว',
    'วัดศาลาหลวงบน',
    'ท่าบอนมิตรภาพที่ 90',
    'บ้านรับแพรก',
    'วัดคลองเป็ด',
    'วัดศาลาหลวงล่าง',
    'บ้านบ่อตรุ',
    'วัดเจดีย์งาม',
    'วัดสีหยัง',
    'วัดประดู่',
    'วัดคูวา',
    'ชุมชนวัดบ้านขาว',
    'วัดหัวป่า',
    'วัดหัววัง',
    'วัดผักกูด',
    'บ้านขี้นาก',
    'วัดปากแตระ',
    'วัดตะพังหม้อ',
    'วัดหน้าเมือง',
    'วัดพังยาง',
    'วัดสามี',
    'ไทยรัฐวิทยา 37 (วัดหัวถิน)',
    'วัดระโนด',
    'วัดหัวเค็ด',
    'วัดบ้านใหม่',
    'วัดมหาการ',
    'บ้านระโนต (ธัญเจริญ)',
    'บ้านมหาการ - เฉียงพง',
    'วัดจาก',
    'วัดเถรแก้ว',
    'วัดใหญ่',
    'วัดแจ้ง',
    'บ้านหน้าทอง',
    'วัดพังตรี',
    'ชุมชนวัดสามบ่อ',
    'วัดสน',
    'วัดโตนดด้วน',
    'วัดทุ่งบัว',
    'วัดแหลมบ่อท่อ',
    'วัดอ่าวบัว',
    'บ้านแหลมหาด',
    'ชุมชนวัดเชิงแส',
    'บ้านรัดปูน',
    'บ้านโคกแห้ว',
    'วัดกาหรํา',
    'วัดโรง (ขาวประชาสรรค์)',
    'วัดแม่เปียะ',
    'บ้านต้นปริง',
    'บ้านนาทองสุก',
    'วัดโพธาราม',
    'บ้านควนจง',
    'วัดทุ่งโตนด',
    'วัดทุ่งฆ้อโฆสิตาราม',
    'วัดนาหม่อม',
    'วัดพรหมประดิษฐ์',
    'วัดเนินพิจิตร',
    'วัดชะแล้',
    'วัดตาหลวงคง',
    'วัดเปรมศรัทธา',
    'วัดเลียบ',
    'วัดบ่อสระวิทยาทาน',
    'วัดดีหลวงนอก',
    'วัดประตูไชย',
    'วัดบ้านพร้าว',
    'วัดทํานบตางหน',
    'บ้านคลองท่าแตง',
    'บ้านท่าแคง',
    'วัดบางเขียด',
    'บ้านแหลมจาก (รักเมืองไทย 27)',
    'วัดบ่อทรายเจริญธรรม',
    'บ้านบางไหน',
    'วัดป่าขาด',
    'วัดสว่างอารมณ์',
    'บ้านม่วงงาม',
    'วัดมะขามคลาน',
    'วัดปะโอ',
    'บ้านม่วงพุ่ม',
    'วัดประตูเขียน',
    'วัดป่าขวาง',
    'วัดห้วยพุด',
    'วัดหนองหอย',
    'วัดขนุน',
    'บ้านขนุน',
    'วัดวาส',
    'วัดสถิตย์ชลธาร',
    'วัดธรรมโฆษณ์',
    'วัดโลกา',
    'วัดบ่อปาบ',
    'บ้านเขาแดง',
    'วัดบ่อทรัพย์',
    'บ้านหัวเขา',
    'มหาวชิราวุธ จังหวัดสงขลา',
    'สทิงพระวิทยา',
    'สทิงพระชนูปถัมภ์',
    'วรนารีเฉลิม จังหวัดสงขลา',
    'นวมินทราชูทิศ ทักษิณ',
    'เกาะแต้วพิทยาสรรค์',
    'สงขลาพัฒนาปัญญา',
    'ตะเครียะวิทยาคม',
    'ระโนด',
    'สามบ่อวิทยา',
    'คลองแดนวิทยา',
    'ระโนดวิทยา',
    'กระแสสินธุ์วิทยา',
    'ธรรมโฆสิต',
    'สงขลาวิทยาคม',
    'ชะแล้นิมิตวิทยา',
    'บ้านเขารักเกียรติ',
    'บ้านห้วยโอน',
    'บ้านควนนา',
    'บ้านคลองต่อ',
    'บ้านห้วยสมบูรณ์',
    'บ้านม่วง',
    'วัดทุ่งคา',
    'บ้านชายคลอง',
    'บ้านปลายละหาน',
    'บ้านทุ่งคมบาง',
    'ชุมชนบ้านนาสีทอง',
    'บ้านคลองกั่ว',
    'บ้านเขาพระ',
    'บ้านคลองเขาล้อน',
    'บ้านควนดินแดง',
    'บ้านนาลึก',
    'วัดไทรใหญ่',
    'ชุมชนบ้านโคกค่าย',
    'บ้านไสท้อน',
    'วัดจังโหลน',
    'บ้านควนขัน',
    'บ้านกองอิฐ',
    'สํานักสงฆ์ศรีวิชัย',
    'วัดเจริญภูผา',
    'วัดคูหาใน',
    'บ้านทุ่งมะขาม',
    'บ้านกําแพงเพชร',
    'บ้านลานควาย',
    'บ้านท่ามะปราง',
    'บ้านเนินนิมิต',
    'บ้านพรุพ้อ',
    'บ้านควนสะตอ',
    'นิคมสร้างตนเองรัตภูมิ',
    'วัดรัตนวราราม',
    'บ้านทุ่งนํ้า',
    'บ้านหนองนายขุ้ย',
    'บ้านเกาะหมี',
    'บ้านท่าไทร',
    'วัดคลองแห',
    'วัดท่าแซ',
    'วัดควนลังมิตรภาพที่ 11',
    'บ้านวังหรัง',
    'บ้านบางแฟบ',
    'บ้านบึงพิชัย',
    'บ้านหน้าควนลัง',
    'วัดม่วงค่อม',
    'เสนาณรงค์วิทยา',
    'บ้านคลองหวะ (ทวีรัตน์ราษฎร์บํารุง)',
    'บ้านคลองเปล',
    'บ้านใต้',
    'วัดชลธารประสิทธิ์',
    'วัดดอน',
    'บ้านเกาะนก',
    'บ้านควน',
    'วัดบางลึก',
    'บ้านฉลุง',
    'วัดเจริญราษฏร์',
    'วัดเขากลอย',
    'วัดแม่เตย',
    'วัดท่าข้าม',
    'วัดหินเกลี้ยง',
    'บ้านวังพา',
    'วัดหูแร่',
    'บ้านทุ่งตําเสา',
    'รักเมืองไทย 6 (โตนงาช้าง)',
    'บ้านนาแสน',
    'บ้านทุ่งเลียบ',
    'บ้านท่าหมอไชย',
    'บ้านหินผุด',
    'วัดพรุเตาะ',
    'บ้านทุ่งใหญ่',
    'บ้านทุ่งงาย',
    'วัดศีรษะคีรี',
    'ท่าจีนอุดมวิทยา',
    'วัดท่านางหอม',
    'ชุมชนบ้านน้ำน้อย',
    'วัดเทพชุมนุม',
    'บ้านโปะหมอ',
    'บ้านไร่',
    'บ้านคลองปอม',
    'ทุ่งปรือพิทยาคม',
    'วัดทุ่งลุงมิตรภาพที่ 198',
    'บ้านทุ่งจัง',
    'วัดควนเนียง',
    'วัดโคกสมานคุณ',
    'วัดโพธิธรรมาราม',
    'บ้านควนโส',
    'บ้านบ่อหว้า',
    'บ้านกรอบ',
    'วัดปากจ่า',
    'วัดบางทีง',
    'บ้านบางเหรียง',
    'บ้านคลองคล้า',
    'บ้านโคกเมือง',
    'บ้านเกาะใหญ่',
    'บ้านคลองช้าง',
    'บ้านเกาะนํ้ารอบ',
    'บ้านหน้าควน',
    'วัดคงคาวดี',
    'บ้านควนเนียง',
    'บ้านยางงาม',
    'บ้านควนเนียงใน',
    'บ้านหนองปลิง',
    'บ้านหัวไทร',
    'วัดท่าหยี',
    'บ้านหัวปาบ',
    'บ้านห้วยลึก',
    'บ้านโคกเมา',
    'บ้านคลองนกกระทุง',
    'วัดเนินพิชัย',
    'บ้านดินลาน',
    'บ้านป่ายาง',
    'บ้านยางงาม',
    'วัดท่าช้าง',
    'บ้านบางกลํ่า',
    'วัดท่าเมรุ',
    'วัดบางหยี',
    'บ้านยวนยาง',
    'บ้านหาร',
    'บ้านหนองม่วง',
    'วัดนารังนก',
    'บ้านแม่ทอม',
    'วัดเลียบ',
    'บ้านต้นส้าน',
    'บ้านควนกบ',
    'บ้านคลองหอยโข่ง',
    'บ้านหน้าวัดโพธิ์',
    'บ้านเก่าร้าง',
    'วัดโคกม่วง',
    'บ้านปลักคล้า',
    'วัดโคกเหรียง',
    'บ้านโคกพยอม',
    'วัดบางศาลา',
    'วัดปรางแก้ว',
    'รัตภูมิวิทยา',
    'หาดใหญ่วิทยาลัย',
    'หาดใหญ่วิทยาลัยสมบูรณ์กุลกันยา',
    'มัธยมสิริวัณวรี 2 สงขลา',
    'หาดใหญ่รัฐประชาสรรค์',
    'คูเต่าวิทยา',
    'หาดใหญ่พิทยาคม',
    'หาดใหญ่วิทยาลัย 2',
    'หาดใหญ่เจริญราษฎร์พิทยา',
    'พะตงประธานคีรีวัฒน์',
    'โสตศึกษาจังหวัดสงขลา',
    'ควนเนียงวิทยา',
    'ปากจ่าวิทยา',
    'บางกล่ำวิทยา รัชมังคลาภิเษก',
    'รัตนพลวิทยา',
    'วัดขุนตัดหวาย',
    'บ้านทุ่งนาเคียน',
    'ชุมชนวัดควนมีด',
    'วัดช่องเขา',
    'บ้านควนหัวช้าง',
    'บ้านคูศักดิ์สิทธิ์',
    'วัดนาปรือ',
    'บ้านโหนด',
    'บ้านทุ่งครก',
    'บ้านแค',
    'บ้านคูนายสังข์',
    'บ้านเขาจันทร์',
    'ชุมชนบ้านจะโหนงมิตรภาพที่ 222',
    'ชุมชนบ้านทางควาย',
    'บ้านปากช่อง',
    'บ้านป่างาม',
    'บ้านตลิ่งชัน',
    'บ้านนนท์',
    'บ้านสุเหร่า',
    'บ้านช้างคลอด',
    'บ้านท่าหมาก',
    'บ้านสะพานหัก',
    'กระจายสุทธิธรรมโมอนุสรณ์ ฯ',
    'เหมืองควนกรด',
    'บ้านแพร้ว',
    'ชุมชนบ้านนาทับ',
    'บ้านท่าคลอง',
    'บ้านปากบางนาทับ',
    'บ้านม้างอน',
    'วัดประจ่า',
    'บ้านเกาะทาก',
    'บ้านควนขี้แรด',
    'บ้านหว้าหลัง',
    'บ้านนาหว้า',
    'บ้านควนไม้ไผ่',
    'บ้านคลองแงะ',
    'ชุมชนวัดน้ำขาว',
    'บ้านนํ้าเค็ม',
    'บ้านนา',
    'บ้านโคกม้า',
    'บ้านลางา',
    'วัดบ้านไร่',
    'บ้านป่าชิง',
    'วัดทุ่งพระ',
    'บ้านเลียบ',
    'บ้านบ่อโชน',
    'บ้านปากบาง',
    'บ้านสะกอม',
    'ชุมชนสะพานไม้แก่น',
    'บ้านทรายขาว',
    'วัดเกษมรัตน์',
    'บ้านเก่า',
    'อายุรกิจโกศล',
    'วัดลําพดจินดาราม',
    'บ้านลำชิง',
    'บ้านโต้นนท์',
    'ชุมชนวัดปลักชะเมา',
    'บ้านวังบวบ',
    'วัดวังไทร',
    'บ้านกะทิง',
    'วัดบ้านลุ่ม',
    'วัดท่าประดู่',
    'วัดทุ่งข่า',
    'บ้านพอบิด',
    'บ้านลําลอง',
    'บ้านนาทวี',
    'บ้านทุ่งนํ้าขาว',
    'บ้านเคลียง',
    'บ้านวังใหญ่ปลายรํา',
    'วัดนาหมอศรี',
    'บ้านโมย',
    'ทองอยู่นุตกูล',
    'บ้านพรุหวา',
    'บ้านประกอบ',
    'บ้านคลองกวางเขาวัง',
    'บ้านสม็อง',
    'บ้านป่าเร็ด',
    'บ้านนาปรัง',
    'ชุมชนบ้านสะท้อน (วันครู 2503)',
    'บ้านสะพานเคียน',
    'บ้านป็อง',
    'ประสิทธิ์ทวีสิน 2',
    'บ้านพรุตู',
    'บ้านบ่อนํ้าส้ม',
    'วัดนิคมประสาท(บุญชอบ สาครินทร์)มิตรภาพที่ 149',
    'บ้านโคกพยอม',
    'บ้านกรงอิตํา',
    'บ้านใหม่',
    'บ้านควนหรัน',
    'บ้านนาจวก',
    'บ้านกระอาน',
    'บ้านโคกกอ',
    'ชุมชนนิคมสร้างตนเองเทพา',
    'นิคมสร้างตนเองเทพา 2',
    'นิคมสร้างตนเองเทพา 3',
    'บ้านหนองสาหร่าย',
    'บ้านท่าม่วง',
    'บ้านพรุชิง',
    'บ้านเทพา',
    'วัดเทพาไพโรจน์',
    'บ้านพระพุทธ',
    'บ้านป่ากอ',
    'บ้านป่าโอน',
    'นิคมสร้างตนเองเทพา 4',
    'บ้านคลองขุด',
    'บ้านตาแปด',
    'บ้านควนตีหมุน',
    'บ้านตูหยง',
    'บ้านบ่อเตย',
    'บ้านเกาะแลหนัง',
    'บ้านคลองประดู่',
    'บ้านแม่ที',
    'บ้านลำไพล',
    'บ้านลำเปา',
    'บ้านทุ่งโพธิ์',
    'บ้านท่าไทร',
    'วัดปริก',
    'บ้านควนเจดีย์',
    'ลําไพลราษฎร์อุทิศ',
    'บ้านควนหมาก',
    'บ้านวังใหญ่',
    'วัดคลองยอ',
    'บ้านม่วงถํ้า',
    'วัดคงคาสวัสดิ์',
    'บ้านแซะ',
    'บ้านเขาน้อย',
    'บ้านพรุหลุมพี',
    'บ้านปากบางสะกอม',
    'บ้านสวรรค์',
    'บ้านทุ่งไพล',
    'บ้านถํ้าตลอด',
    'มหิดล',
    'บ้านทัพหลวง',
    'บ้านห้วยเต่า',
    'บ้านคูหา',
    'บ้านล่องควน',
    'บ้านทับยาง',
    'บ้านคอลอมุดอ',
    'บ้านมุนี',
    'บ้านนาจะแหน',
    'บ้านโคกตก',
    'บ้านโคกสิเหรง',
    'บ้านฉลุง',
    'บ้านตะเคียนทอง',
    'บ้านเมาะลาแต',
    'บ้านสุโสะ',
    'บ้านนาม่วง',
    'ชุมชนบ้านนากัน',
    'บ้านห้วยบอน',
    'ธรรมศาสตร์ - จุฬา 1',
    'บ้านบาโหย',
    'บ้านท่า',
    'อุทยานอุทิศ',
    'บ้านควนหรัน',
    'บ้านเก่า',
    'สะบ้าย้อย',
    'บ้านทําเนียบ',
    'บ้านบ่อทอง',
    'บ้านไร่',
    'บ้านปลักบ่อ',
    'วัดเขามีเกียรติ',
    'บ้านสํานักหว้า',
    'ไทยรัฐวิทยา 80 (ชุมชนบ้านวังปริง)',
    'บ้านท่าโพธิ์',
    'วัดสองพี่น้อง',
    'บ่อเกดวิจิตรวิทยา',
    'ชาติตระการโกศล',
    'วัดยางทอง (รัตนประชานุกูล)',
    'บ้านปริกใต้',
    'บ้านตะเคียนเภา',
    'บ้านปริก',
    'บ้านใหม่',
    'บ้านควนเสม็ด',
    'บ้านหัวถนน',
    'บ้านยางเกาะ',
    'สังวาลย์วิท 2',
    'ต.ช.ด.ทุ่งไม้ด้วน 1',
    'บ้านท่าข่อย',
    'บ้านไร่',
    'ชุมชนบ้านปาดัง',
    'ต.ช.ด.สมาคมไทยผลิตยา 2',
    'บ้านพังลา',
    'บ้านระตะ',
    'บ้านคลองแงะ',
    'วัดม่วงก็อง',
    'วัดศรีวิเทศสังฆาราม',
    'บ้านพรุเตียว',
    'บ้านสํานักขาม',
    'บ้านหน่ำฮั้ว',
    'บ้านทับโกบ',
    'บ้านนํ้าลัด',
    'บ้านม่วง',
    'วัดห้วยคู',
    'บ้านเกาะค่าง',
    'บ้านควนตานี',
    'เทพา',
    'จะนะวิทยา',
    'นาทวีวิทยาคม',
    'จะนะชนูปถัมภ์',
    'ทับช้างวิทยาคม',
    'เทพพิทยาภาณุมาศ',
    'เขาแดงกุศลวิทยา',
    'กอบกุลวิทยาคม',
    'ปาดังติณสูลานนท์',
    'ไม้แก่นประชาอุทิศ',
    'จะโหนงพิทยาคม',
    'สะบ้าย้อยวิทยา',
    'สะเดา (ขรรค์ชัยกัมพลานนท์อนุสรณ์)',
    'บ้านเกตรี',
    'บ้านวังพะเนียด',
    'บ้านเกาะสาหร่าย',
    'บ้านตันหยงกลิง',
    'บ้านตันหยงอุมา',
    'บ้านเกาะอาดัง',
    'บ้านเกาะยะระโตดนุ้ย',
    'วัดหน้าเมือง',
    'บ้านท่าจีน',
    'บ้านนาแค',
    'บ้านคลองขุด',
    'บ้านเขาจีน',
    'บ้านไทรงาม',
    'บ้านท่าหิน',
    'บ้านนาลาน',
    'บ้านควนขัน',
    'บ้านเนินสูง',
    'บ้านใหม่',
    'ไทยรัฐวิทยา 40',
    'บ้านเจ๊ะบิลัง',
    'บ้านฉลุง',
    'บ้านกาลันยีตัน',
    'บ้านบันนังปุเลา',
    'บ้านปาเต๊ะ',
    'บ้านทุ่ง',
    'บ้านโคกประดู่',
    'บ้านกุบังจามัง',
    'อนุบาลเมืองสตูล',
    'บ้านตันหยงโป',
    'บ้านหาดทรายยาว',
    'บ้านตำมะลังเหนือ',
    'บ้านตำมะลังใต้',
    'บ้านกาลันบาตู',
    'บ้านกาลูบี',
    'บ้านกาเนะ',
    'บ้านทุ่งวิมาน',
    'บ้านควน',
    'บ้านเกาะยาว',
    'บ้านตันหยงกาโบย',
    'อนุบาลสตูล',
    'อนุบาลควนโดน',
    'บ้านดูสน',
    'บ้านบูเกตยามู',
    'บ้านถ้ำทะลุ',
    'บ้านควนพระ',
    'บ้านควนสตอ',
    'บ้านกุบังปะโหลด',
    'บ้านทุ่งพัฒนา',
    'บ้านทางงอ',
    'บ้านปันจอร์',
    'บ้านย่านซื่อ มิตรภาพ 147',
    'บ้านวังประจัน',
    'บ้านทุ่งมะปรัง',
    'อนุบาลควนกาหลง',
    'นิคมซอย 10',
    'บ้านเหนือคลอง',
    'บ้านเขาใคร',
    'บ้านควนล่อน',
    'บ้านห้วยน้ำดำ',
    'บ้านกลุ่ม 5 ประชารัฐ',
    'นิคมพัฒนาภาคใต้ 1',
    'บ้านน้ำหรา',
    'บ้านโตนปาหนัน',
    'บ้านหัวกาหมิง',
    'บ้านทุ่งนุ้ย มิตรภาพที่ 49',
    'บ้านค่ายรวมมิตร',
    'บ้านทุ่งตำเสา ชูสินอุปถัมภ์',
    'บ้านน้ำร้อน',
    'นิคมพัฒนาภาคใต้ 2',
    'นิคมพัฒนาภาคใต้ 3',
    'บ้านอุได',
    'นิคมพัฒนาผัง 120',
    'นิคมพัฒนาผัง 42',
    'บ้านท่าแพ',
    'อนุบาลท่าแพ',
    'บ้านสายควน',
    'บ้านท่าน้ำเค็มใต้',
    'บ้านนาแก้ว',
    'บ้านแป-ระใต้',
    'บ้านปลักหว้า',
    'บ้านวังปริง',
    'บ้านไร่',
    'บ้านสวนเทศ',
    'บ้านแป-ระเหนือ',
    'บ้านควนเก',
    'บ้านสาคร',
    'บ้านสาครเหนือ',
    'บ้านทางยาง',
    'บ้านทุ่งริ้น',
    'สมาคมเลขานุการสตรี 3',
    'บ้านคลองสองปาก',
    'อนุบาลละงู',
    'บ้านไสใหญ่',
    'บ้านปิใหญ่',
    'บ้านโกตา',
    'บ้านปากปิง',
    'บ้านป่าฝาง',
    'บ้านท่าแลหลา',
    'บ้านทุ่งเสม็ด มิตรภาพที่ 114',
    'บ้านอุไร',
    'บ้านตูแตหรำ',
    'บ้านควนฟ้าแลบ',
    'บ้านบ่อหิน',
    'บ้านดาหลำ',
    'บ้านนาข่า',
    'บ้านหาญ',
    'บ้านนางแก้ว',
    'บ้านทุ่งไหม้',
    'บ้านหนองหอยโข่ง',
    'บ้านวังสายทอง',
    'บ้านหัวควน',
    'บ้านตะโละใส',
    'บ้านปากบารา',
    'บ้านบ่อเจ็ดลูก',
    'บ้านท่ายาง',
    'บ้านท่าพยอม',
    'บ้านเกาะบูโหลน',
    'บ้านโคกพะยอม',
    'บ้านลาหงา',
    'บ้านนาพญา',
    'บ้านห้วยมะพร้าว',
    'บ้านเกาะยวน',
    'บ้านทุ่ง',
    'บ้านคลองน้ำเค็ม',
    'บ้านห้วยไทร',
    'บ้านปากละงู',
    'บ้านในเมือง',
    'บ้านปากบาง',
    'บ้านท่าชะมวง',
    'บ้านบากันโต๊ะทิด',
    'บ้านสุไหงมูโซ๊ะ',
    'บ้านบุโบย',
    'บ้านสนกลาง',
    'บ้านตันหยงละไน้',
    'บ้านกาแบง',
    'บ้านขอนคลาน',
    'บ้านราไว',
    'บ้านทุ่งบุหลัง',
    'บ้านทุ่งสะโบ๊ะ',
    'บ้านมะหงัง',
    'อนุบาลทุ่งหว้า',
    'วัดชมพูนิมิต',
    'บ้านคีรีวง',
    'บ้านคีรีวง สาขาบ้านราวปลา',
    'บ้านนาทอน',
    'บ้านท่าข้ามควาย',
    'บ้านวังตง',
    'บ้านท่าศิลา',
    'บ้านบารายี',
    'บ้านช่องไทร',
    'บ้านป่าแก่บ่อหิน',
    'วรรธนะสาร',
    'บ้านทุ่งดินลุ่ม',
    'อนุบาลมะนัง',
    'นิคมพัฒนาผัง 6',
    'นิคมพัฒนาผัง 20',
    'บ้านวังพระเคียน',
    'ผังปาล์ม 1',
    'ผังปาล์ม 2',
    'ผังปาล์ม 4',
    'ผังปาล์ม 7',
    'บ้านมะนัง',
    'บ้านป่าพน',
    'ผังปาล์ม 3',
    'จุฬาภรณราชวิทยาลัย สตูล',
    'ท่าแพผดุงวิทย์',
    'ควนกาหลงวิทยาคม (นิคมวัฒนา)',
    'กำแพงวิทยา',
    'ทุ่งหว้าวรวิทย์',
    'ปาล์มพัฒนวิทย์',
    'สตูลวิทยา',
    'ควนโดนวิทยา',
    'สาครพิทยาคาร',
    'ละงูพิทยาคม',
    'ท่าศิลาบำรุงราษฎร์',
    'พิมานพิทยาสรรค์',
    'บ้านควนปริง',
    'บ้านนาป้อ',
    'วัดควนวิเศษ สาขาวัดโคกมะม่วง',
    'วัดนิคมประทีป',
    'วัดสาริการาม',
    'มิตรภาพที่ 31',
    'วัดควนวิเศษ',
    'อนุบาลตรัง',
    'วัดไทรงาม',
    'วัดไพรสนฑ์',
    'ไทยรัฐวิทยา 39',
    'ไทยรัฐวิทยา 39 สาขาบ้านห้วยเหรียง',
    'ไทยรัฐวิทยา 39 สาขาบ้านทำเล',
    'ไทยรัฐวิทยา 39 สาขาบ้านเกาะเคี่ยม',
    'บ้านควนสระแก้ว',
    'วัดศรีสุวรรณาราม',
    'วัดนานอน',
    'วัดนานอน สาขาบ้านยางงาม',
    'วัดนานอน สาขาบ้านทุ่งโดนทอง',
    'วัดนานอน สาขาบ้านพิกุลลอย',
    'วัดไทรงามส.วัดราษฎร์รื่นรมย์',
    'วัดไทรงามส.บ้านทุ่งปง',
    'บ้านบางยาง',
    'บ้านโคกมะม่วง',
    'บ้านคลองเต็ง',
    'วัดสันติวัน',
    'บ้านเกาะปราง',
    'วัดทุ่งหินผุด',
    'บ้านโคกชะแง้',
    'วัดโคกพิกุล สาขาวัดหัวถนน',
    'บ้านหนองเรี้ย',
    'ต้นบากราษฎร์บำรุง',
    'วัดขุนสิทธิ์',
    'เพาะปัญญา',
    'เพาะปัญญาสาขาบ้านมาบมะขาม',
    'วัดโคกพิกุล',
    'วัดน้ำผุด',
    'บ้านห้วยเร็จ',
    'บ้านปากห้วย',
    'บ้านไร่พรุ',
    'บ้านทุ่งนา',
    'บ้านเขาหลัก',
    'บ้านด่าน',
    'วัดแจ้ง',
    'วัดเกาะมะม่วง',
    'วัดนิคมประทีป สาขาวัดควน',
    'บ้านห้วยไทร',
    'วัดโพธาราม',
    'บ้านคลองขุด',
    'วัดเชี่ยวชาญกิจ',
    'บ้านปากไพ',
    'บ้านกลางนา',
    'บ้านโพรงจระเข้',
    'วัดโหละคล้า',
    'บ้านทุ่งเกาะญวน สาขาบ้านควนโต๊ะทอง',
    'วัดอัมพวัน',
    'บ้านคลองทรายขาว',
    'บ้านทุ่งนา',
    'บ้านหนองชวด',
    'บ้านควนโพธิ์สาขาวัดพิกุลทอง',
    'บ้านนาโตง',
    'บ้านนานิน',
    'บ้านคลองปะเหลียน',
    'บ้านท่าบันได',
    'บ้านควนอินทนินงาม',
    'บ้านควนเคี่ยม',
    'บ้านควนยาง',
    'บ้านควนยวน',
    'บ้านห้วยไม้แก่น',
    'บ้านทุ่งค่าย',
    'บ้านเขาไม้แก้ว',
    'บ้านยูงงาม',
    'วัดหนองเป็ด',
    'บ้านคลองทรายขาว',
    'วัดควนนิมิตศิลา',
    'บ้านไทรงาม',
    'บ้านลำพิกุล',
    'บ้านทอนพลา',
    'บ้านทุ่งศาลา',
    'บ้านห้วยด้วน',
    'บ้านในควน',
    'บ้านทุ่งหนองแห้ง',
    'วัดโพรงจระเข้',
    'บ้านทุ่งหนองแห้ง สาขาวัดช่องหาย',
    'บ้านย่านตาขาว สาขาบ้านพรุโต๊ะปุก',
    'บ้านโคกทราย',
    'บ้านย่านตาขาว',
    'บ้านนายายหม่อม',
    'บ้านควนโพธิ์',
    'บ้านย่านตาขาว สาขาบ้านโหละหาร',
    'วัดชลวาปีวิหาร',
    'วัดชลวาปีวิหาร สาขาวัดประชาบำรุง',
    'บ้านปากคลอง',
    'บ้านทุ่งเกาะญวน',
    'หาดทรายทอง',
    'บ้านแหลม',
    'บ้านสามแยก',
    'บ้านทอนหาน',
    'บ้านท่าข้าม',
    'บ้านหยงสตาร์',
    'ทุ่งรวงทอง',
    'วัดท่าพญา',
    'บ้านทุ่งยาว',
    'บ้านหนองหว้า',
    'บ้านโคกแต้ว',
    'บ้านทุ่งมะขามป้อม',
    'บ้านยวนโปะ',
    'บ้านป่าแก่',
    'บ้านบางด้วน',
    'วัดควนวิไล',
    'บ้านหนองยายแม็ม',
    'บ้านหนองโต๊ะ',
    'บ้านห้วยม่วง',
    'บ้านหินคอกควาย',
    'บ้านหัวควน',
    'บ้านวังศิลา',
    'บ้านควนไม้ดำ',
    'บ้านวังเจริญ',
    'บ้านลำปลอกเหนือ',
    'บ้านหาดเลา',
    'บ้านปะเหลียน',
    'บ้านลำแคลง',
    'บ้านเขาติง',
    'บ้านลิพัง',
    'บ้านทุ่งปาหนัน',
    'บ้านสุโสะ',
    'บ้านท่าคลอง',
    'ทุ่งไทรทอง',
    'บ้านท่าเทศ',
    'หนองผักฉีด',
    'บ้านหนองเจ็ดบาท',
    'บ้านแหลมสอม',
    'บ้านนาทุ่ง',
    'วัดไทรทอง',
    'วัดนางประหลาด',
    'บ้านไร่หลวง',
    'บ้านช่อง',
    'วัดช่องเปี่ยมราษฎร์',
    'บ้านห้วยลึก',
    'วัดธรรมาราม',
    'บ้านเกาะปุด',
    'วัดควนสีนวล',
    'บ้านหนองไทร',
    'วัดจอมไตร',
    'ไทรงาม',
    'บ้านควนสวรรค์',
    'บ้านคลองลำปริง',
    'บ้านหนองยวน',
    'บ้านทุ่งส้มป่อย',
    'วัดมงคลสถาน',
    'บ้านตะเสะ',
    'บ้านนาทะเล',
    'บ้านโคกรัก',
    'บ้านทุ่งกอ',
    'บ้านบ้าหวี',
    'บ้านบกหัก',
    'หาดสำราญ',
    'วัดหนองสมาน',
    'บ้านปากปรน',
    'วัดปากปรน',
    'วิเชียรมาตุ',
    'สภาราชินี',
    'น้ำผุด',
    'จุฬาภรณราชวิทยาลัย ตรัง',
    'สภาราชินี 2',
    'วิเชียรมาตุ 3',
    'ย่านตาขาวรัฐชนูปถัมภ์',
    'ทุ่งหนองแห้งประชาสรรค์',
    'วิเชียรมาตุ 2',
    'ตรังรังสฤษฎ์',
    'ปะเหลียนผดุงศิษย์',
    'คันธพิทยาคาร',
    'ทุ่งยาวผดุงศิษย์',
    'สวัสดิ์รัตนาภิมุข',
    'นาโยงวิทยาคม',
    'หาดสำราญวิทยาคม',
    'บ้านแตะหรำ',
    'บ้านกันตังใต้',
    'บ้านเกาะเคี่ยม',
    'บ้านจุปะ',
    'บ้านหาดยาว',
    'บ้านบ้านบาตูปูเต๊ะ สาขาบ้านหลังเขา',
    'บ้านมดตะนอย',
    'บ้านเกาะมุกด์',
    'บ้านเกาะลิบง',
    'บ้านบาตูปูเต๊ะ',
    'บ้านคลองชีล้อม',
    'บ้านหนองเสม็ด',
    'บ้านคลองลุ',
    'บ้านบางเตา',
    'บ้านนาเหนือ',
    'วัดควนธานี',
    'บ้านบางหมากน้อย',
    'บ้านโคกยาง',
    'บ้านโคกยาง สาขาบ้านหนองยาว',
    'บ้านน้ำฉา',
    'บ้านท่าโต๊ะเมฆ',
    'บ้านพระม่วง',
    'บ้านนาเกลือ',
    'บ้านท่าส้ม สาขาบ้านหัวหิน',
    'บ้านท่าส้ม',
    'บ้านท่าส้ม สาขาบ้านควนแคง',
    'บ้านท่าปาบ',
    'บ้านท่าปาบ สาขาบ้านเคี่ยมงาม',
    'บ้านบางเป้า',
    'บ้านป่าเตียว',
    'วัดสีหราษฎร์ศรัทธา',
    'บ้านควนตุ้งกู',
    'บ้านน้ำราบ',
    'บ้านบางสัก สาขาบ้านทุ่งค่าย',
    'บ้านบางสัก',
    'บ้านพรุใหญ่',
    'บ้านบางหมาก',
    'บ้านตะเคียนหลบฟ้า',
    'บ้านไร่ใหญ่',
    'บ้านทุ่งอิฐ',
    'บ้านย่านซื่อ',
    'บ้านย่านซื่อ สาขาบ้านโคกทราย',
    'วัดวารีวง',
    'บ้านแหลม',
    'บ้านปาเต',
    'วัดเจริญร่มเมือง',
    'บ้านโตน',
    'บ้านพรุเตย',
    'กมลศรี',
    'บ้านสายควน',
    'บ้านแหลมไทร',
    'บ้านแหลมมะขาม',
    'บ้านทุ่งขี้เหล็ก',
    'บ้านเขาไม้แก้ว',
    'บ้านบางค้างคาว',
    'บ้านนาเมืองเพชร สาขาตรังคสมบัติ 1',
    'บ้านกลิ้งกลอง',
    'บ้านนาเมืองเพชร',
    'บ้านห้วยไทร',
    'บ้านนาเมืองเพชร สาขาวัดเขาแก้ว',
    'บ้านไร่ออก',
    'บ้านบ่อหิน',
    'บ้านดุหุน',
    'บ้านไสต้นวา',
    'บ้านหัวหิน',
    'บ้านพรุจูด',
    'หาดปากเมง',
    'บ้านฉางหลาง',
    'วัดไม้ฝาด',
    'บ้านผมเด็น',
    'บ้านห้วยต่อ',
    'วัดคีรีวิหาร สาขาบ้านนาพระยา',
    'วัดคีรีวิหาร',
    'บ้านหนองปรือ',
    'บ้านเขากอบ',
    'บ้านเขากอบ สาขาบ้านเขาหัวแหวน',
    'บ้านเขากอบ สาขาบ้านนาท่อม',
    'บ้านหนองยูง',
    'วัดกาญจน์บริรักษ์',
    'วัดเขา สาขาบ้านไทรงาม',
    'บ้านควนตัง',
    'บ้านวังสมบูรณ์',
    'ท่างิ้ว (ต.ช.ด.อุปถัมภ์)',
    'บ้านไสมะม่วง',
    'วัดเขา',
    'บ้านทุ่งต่อ สาขาบ้านลานกระทิง',
    'บ้านทุ่งต่อ',
    'บ้านทุ่งต่อ สาขาบ้านท่าประดู่',
    'บ้านหนองหมอ สาขาบ้านสะพานยูง',
    'บ้านหนองหมอ',
    'บ้านป่ากอ',
    'บ้านไสบ่อ',
    'บ้านป่ากอ สาขาวัดเขาปินะ',
    'บ้านเกาะเต่า',
    'บ้านทุ่งศาลา',
    'บ้านควนพญา',
    'วัดนาวง',
    'บ้านเหนือคลอง',
    'วัดบางดี สาขาบ้านพรุจูด',
    'บ้านทุ่งยางงาม สาขาบ้านทุ่งวางาม',
    'วัดบางดี สาขาบ้านควนงาช้าง',
    'บ้านควนอารี',
    'บ้านทุ่งยางงาม',
    'วัดบางดี',
    'บ้านซา',
    'บ้านน้ำพราย',
    'วัดควนไทร',
    'วัดควนไทร สาขาบ้านทุ่งล้อม',
    'วัดปากแจ่ม',
    'บ้านท่ามะปราง',
    'บ้านคลองคุ้ย',
    'บ้านลำแพะ',
    'บ้านลำภูรา',
    'วัดสิทธิโชค',
    'บ้านวังลำ',
    'บ้านเขาพรุเสม็ด',
    'วัดไตรสามัคคี',
    'บ้านป่ายาง',
    'วัดศรีรัตนาราม',
    'บ้านห้วยน้ำเย็น',
    'บ้านหนองสองพี่น้อง',
    'บ้านควนเลียบ',
    'บ้านวังหลาม',
    'บ้านหนองราโพ',
    'วัดห้วยนาง (วันครู2501)',
    'ห้วยยอด (กลึงวิทยาคาร)',
    'วัดเขาวิเศษ',
    'บ้านเขาโอน',
    'บ้านคลองโตน',
    'บ้านหนองคล้า',
    'บ้านหน้าเขา',
    'บ้านหลังเขา',
    'บ้านทุ่งยาง',
    'วัดทุ่งใหญ่',
    'บ้านจิจิก',
    'สวนป่าประชาอุปถัมภ์',
    'วัดน้ำฉ่า',
    'บ้านบางพระ',
    'บ้านช่องลม',
    'บ้านช่องหาร',
    'บ้านต้นปรง',
    'บ้านไชยภักดี',
    'บ้านบางคราม',
    'บ้านในปง',
    'บ้านหนองชุมแสง',
    'บ้านทอนเหรียน',
    'บ้านลำช้าง',
    'บ้านโพธิ์น้อย',
    'วัดโคกเลียบ',
    'วัดเขาพระ',
    'บ้านควนหนองกก',
    'บ้านทุ่งสมอ',
    'บ้านหนองมวง',
    'วัดควนเมา',
    'บ้านไร่ใหญ่',
    'บ้านหนองบัว',
    'วัดถ้ำพระพุทธ',
    'บ้านหนองศรีจันทร์',
    'บ้านต้นไทร',
    'บ้านควนหนองยาง',
    'บ้านหนองบัวน้อย',
    'บ้านคลองมวน',
    'บ้านไทรบ่วง',
    'กันตังพิทยากร',
    'กันตังรัษฎาศึกษา',
    'สิเกาประชาผดุงวิทย์',
    'ห้วยยอด',
    'สามัคคีศึกษา',
    'ห้วยนางราษฎร์บำรุง',
    'ลำภูราเรืองวิทย์',
    'ในเตาพิทยาคม',
    'บางดีวิทยาคม',
    'วังวิเศษ',
    'รัษฎานุประดิษฐ์อนุสรณ์',
    'รัษฎา',
    'วัดโคกชะงาย(ติสสโรอำนวย)',
    'วัดนางลาด',
    'วัดควนแร่',
    'วัดบ้านสวน',
    'บ้านควนกุฎ',
    'วัดประดู่ทอง',
    'บ้านท่ามิหรำ',
    'อนุบาลพัทลุง',
    'วัดธาราสถิตย์',
    'วัดปากสระ',
    'วัดแจ้ง(ปัญญาฐิปิตประชาสรรค์)',
    'บ้านมะกอกใต้ (มุขประชาอุปถัมภ์)',
    'วัดท่าสำเภาใต้',
    'วัดท่าสำเภาเหนือ',
    'วัดสุวรรณวิหารน้อย',
    'บ้านควนคง',
    'วัดตำนาน (วรพัฒนประชาสรรค์)',
    'บ้านทุ่งลาน',
    'วัดประจิมทิศาราม',
    'บ้านท่าแค วันครู 2500',
    'บ้านไสถั่ว',
    'วัดอภยาราม',
    'วัดโพธาวาส (ภิรมอุปถัมภ์)',
    'วัดควนอินทร์นิมิตร',
    'วัดอัมพวนาราม(มิตรภาพที่89)',
    'วัดนาท่อม',
    'วัดโคกแย้ม',
    'บ้านนาโหนด',
    'วัดหัวหมอน',
    'วัดบ่วงช้าง',
    'บ้านต้นไทร',
    'วัดควนกรวด',
    'บ้านแหลมยาง',
    'บ้านปรางหมู่(ศรีวิทย์ศึกษา)',
    'วัดเขาแดง',
    'วัดควนถบ',
    'อนุบาลเมืองพัทลุง',
    'วัดกลาง',
    'วัดโอ่(ปิปผลิกประชาสรรค์)',
    'บ้านยางยายขลุย',
    'บ้านเตาปูน',
    'วัดปากประ',
    'วัดวิหารเบิก (กาญจนานุกูล)',
    'วัดโพเด็ด (พุ่มพูนพิทยา)',
    'บ้านพน',
    'วัดเขาวงก์',
    'สามัคคีอนุสรณ์',
    'บ้านนาทุ่งโพธิ์',
    'บ้านป่าแก่',
    'บ้านพูด กรป.กลาง',
    'บ้านคู',
    'บ้านควนประกอบ',
    'บ้านหน้าวัง',
    'อนุบาลกงหรา',
    'บ้านทอนตรน',
    'บ้านวังปริง',
    'บ้านต้นประดู่',
    'วัดควนขี้แรด',
    'วัดพังกิ่ง',
    'วัดหวัง',
    'อนุบาลเขาชัยสน',
    'วัดหัวเขาชัยสน',
    'บ้านลานช้าง(มิตรภาพ 45)',
    'บ้านควนโคกยา',
    'บ้านเทพราช',
    'บ้านท่านางพรหม (ธนาคารกุรงเทพ 8)',
    'บ้านท่าลาด',
    'บ้านนาหยา',
    'วัดท่านางพรหม',
    'วัดชุมประดิษฐ์',
    'วัดควนสามโพธิ์',
    'บ้านไสนายขัน',
    'วัดโพธิยาราม',
    'บ้านควนหมอทอง',
    'บ้านเกาะทองสม',
    'บ้านควนยวน',
    'วัดท่าควาย',
    'บ้านโคกม่วง (ดำประชาอุทิศ)',
    'วัดบางแก้ว',
    'วัดแหลมจองถนน',
    'วัดแตระ (ปาลานุเคราะห์)',
    'วัดหานโพธิ์',
    'บ้านคลองขุด',
    'ไทยรัฐวิทยา 23 (วัดโคกโหนด)',
    'วัดสะทัง',
    'วัดควนโก(ไพศาลประชาอุปถัมภ์)',
    'บ้านแหลมดิน (หัสนันท์อุปถัมภ์)',
    'บ้านคลองใหญ่',
    'บ้านท่าเชียด',
    'บ้านพรุนายขาว',
    'บ้านทุ่งหนองสิบบาท',
    'วัดตะโหมด (หมุนคณานุสรณ์)',
    'บ้านหัวช้าง',
    'วัดโหล๊ะจันกระ',
    'บ้านควนอินนอโม',
    'บ้านด่านโลด',
    'บ้านร่มโพธิ์ไทร',
    'บ้านแม่ขรี(สวิงประชาสรรค์)',
    'วัดปลักปอม',
    'บ้านควนขนุน',
    'บ้านโคกวา',
    'บ้านควนพลี',
    'วัดพิกุลทอง',
    'วัดไทรโกบ',
    'บ้านหยีในสามัคคี',
    'บ้านควนดินแดง',
    'บ้านใสอ้อย',
    'วัดหรังแคบ',
    'วัดป่าตอ',
    'อนุบาลควนขนุน',
    'บ้านโคกโดน',
    'วัดทะเลน้อย',
    'วัดประดู่หอม (สุขประชาสรรค์)',
    'บ้านหัวป่าเขียว',
    'วัดโคกศักดิ์ (เซี้ยนประชารัฐ)',
    'บ้านขันหมู่',
    'วัดเกาะยาง',
    'วัดลานแซะ',
    'วัดโงกน้ำ',
    'วัดพังดาน',
    'วัดเขาโพรกเพลง',
    'วัดควนปันตาราม',
    'วัดสุนทราวาส',
    'บ้านปากสระ',
    'บ้านสํานักกอ',
    'บ้านใสหลวง',
    'ชุมชนบ้านควนปริง',
    'วัดทุ่งขึงหนัง',
    'ทุ่งยางเปล',
    'บ้านระหว่างควน',
    'บ้านเขาพนม',
    'บ้านธรรมเถียร',
    'บ้านควนพนางตุง (สินประชา)',
    'วัดไทรงาม',
    'บ้านชายคลอง',
    'บ้านท่าช้าง',
    'วัดเขาป้าเจ้',
    'วัดควนแพรกหา',
    'วัดเขาอ้อ',
    'วัดดอนศาลา',
    'วัดเขาทอง',
    'วัดบ้านสวน',
    'วัดประดู่เรียง',
    'วัดแหลมโตนด',
    'บ้านเตง',
    'วัดสุภาษิตาราม',
    'วัดแหลมดินสอ',
    'บ้านท่าเนียน',
    'บ้านเกาะนางคำ',
    'บ้านเกาะนางคำเหนือ',
    'บ้านเกาะหมาก',
    'บ้านเกาะเสือ',
    'บ้านปากบางนาคราช',
    'บ้านเกาะโคบ',
    'บ้านท่าวา',
    'บ้านช่องฟืน',
    'วัดบ้านแหลมกรวด (อินทรประดิษฐ์)',
    'บ้านโคกทราย',
    'วัดไทรพอน',
    'วัดควนเผยอ',
    'บ้านดอนประดู่',
    'วัดหัวควน',
    'บ้านโพธิ์ (ชุมคณานุสรณ์)',
    'อนุบาลปากพะยูน',
    'ปากพะยูน',
    'วัดโรจนาราม',
    'วัดฝาละมี',
    'บ้านควนพระสาครินทร์',
    'บ้านแหลม',
    'วัดบางขวน',
    'บ้านบางมวง',
    'วัดพระเกิด',
    'วัดควนนางพิมพ์',
    'บ้านม่วงทวน',
    'บ้านหารเทา',
    'สำนักสงฆ์ห้วยเรือ',
    'บ้านทะเลเหมียง',
    'บ้านควนนกหว้า',
    'บ้านเขาปู่',
    'บ้านเหรียงงาม',
    'วัดใสประดู่',
    'ชุณหะวัณ',
    'วัดโพรงงู',
    'บ้านไทรทอง',
    'อนุบาลศรีบรรพต',
    'บ้านโหล๊ะเร็ด',
    'วัดตะแพน',
    'บ้านสวนโหนด',
    'บ้านหัสคุณ',
    'วัดควนเพ็ง',
    'บ้านควนแหวง',
    'วัดโคกตะเคียน',
    'วัดพรุพ้อ',
    'บ้านควนหินแท่น',
    'บ้านทุ่งคลองควาย',
    'บ้านโหล๊ะหาร',
    'บ้านยางขาคีม',
    'บ้านทุ่งนารี',
    'บ้านห้วยทราย (มิตรภาพที่ 150)',
    'วัดป่าบอนต่ำ',
    'อนุบาลป่าบอน',
    'วัดท่าดินแดง',
    'บ้านน้ำตก',
    'วัดควนเคี่ยม',
    'บ้านหนองธง',
    'มิตรมวลชน 1',
    'บ้านเหมืองตะกั่ว',
    'บ้านโคกสัก',
    'วัดลอน',
    'บ้านต้นสน',
    'วัดรัตนวราราม',
    'วัดสังฆวราราม',
    'วัดปัณณาราม',
    'บ้านหูแร่',
    'อนุบาลบางแก้ว',
    'บ้านหาดไข่เต่า',
    'วัดนาปะขอ',
    'วัดโตนด',
    'วัดนาหม่อม (เชนวิทยา)',
    'บ้านปากพล',
    'ประชารัฐบำรุง 2',
    'วัดคลองใหญ่',
    'บ้านห้วยกรวด',
    'บ้านปากเหมือง',
    'บ้านไสเลียบ',
    'บ้านโหล๊ะท่อม (ราษฎร์พิพัฒน์)',
    'บ้านเนินทราย',
    'บ้านศาลาน้ำ',
    'บ้านบ่อทราย',
    'บ้านตลิ่งชัน',
    'อนุบาลป่าพะยอม',
    'บ้านไสกุน',
    'บ้านลานข่อย',
    'บ้านทุ่งชุมพล',
    'บ้านห้วยน้ำดำ',
    'บ้านห้วยศรีเกษร',
    'บ้านถ้ำลา',
    'บ้านขัน',
    'อนุบาลศรีนครินทร์',
    'วัดทุ่งยาว (ศิริราษฎร์สามัคคี)',
    'บ้านควนดินสอ (กรป.กลางอุปถัมภ์)',
    'วัดบ้านนา',
    'วัดลำใน',
    'บ้านนาวง',
    'บ้านโตน',
    'วัดเกษตรนิคม',
    'บ้านโหละหนุน',
    'บ้านอ่างทอง',
    'วัดร่มเมือง',
    'พัทลุง',
    'พัทลุงพิทยาคม',
    'พรหมพินิตชัยบุรี',
    'ประภัสสรรังสิต',
    'วชิรธรรมสถิต',
    'สตรีพัทลุง',
    'ศึกษาสงเคราะห์พัทลุง',
    'กงหราพิชากร',
    'ชะรัดชนูปถัมภ์',
    'หานโพธิ์พิทยาคม',
    'ประชาบำรุง',
    'ตะโหมด',
    'ควนขนุน',
    'พนางตุง',
    'ดอนศาลานำวิทยา',
    'ปัญญาวุธ',
    'ควนพระสาครินทร์',
    'ปากพะยูนพิทยาคาร',
    'หารเทารังสีประชาสรรค์',
    'ศรีบรรพตพิทยาคม',
    'ตะแพนพิทยา',
    'ป่าบอนพิทยาคม',
    'บางแก้วพิทยาคม',
    'ป่าพะยอมพิทยาคม',
    'นิคมควนขนุนวิทยา',
    'เขาชัยสน',
    'นาขยาดวิทยาคาร',
    'อุดมวิทยายน',
    'มัธยมเกาะหมาก',
    'บ้านกาแลบือซา',
    'ชุมชนบ้านกะมิยอ',
    'บ้านสระมาลา',
    'บ้านคลองมานิง',
    'ชุมชนบ้านตะลุโบะ',
    'บ้านจะรังบองอ',
    'บ้านตันหยงลุโละ',
    'ชุมชนบ้านกรือเซะ',
    'เมืองปัตตานี',
    'บ้านกูวิง',
    'ชุมชนบ้านยูโย',
    'บ้านบานา',
    'บ้านจือโระ',
    'บ้านปาเระ',
    'ชุมชนบ้านสะนิง',
    'บ้านลดา',
    'บ้านกาฮง',
    'บ้านปะกาลิมาปุโระ',
    'บ้านกือยา',
    'บ้านกอแลบีเละ',
    'บ้านปะกาฮะรัง',
    'บ้านบาราเฮาะ',
    'ชุมชนบ้านปูยุด',
    'บ้านรามง',
    'บ้านรูสะมิแล',
    'อนุบาลปัตตานี',
    'บ้านบางปลาหมอ',
    'บ้านสะบารัง',
    'วัดควน',
    'วัดมหิงษาราม',
    'บ้านมะรวด',
    'บ้านดอน (นุ้ยนิธยาคาร)',
    'บ้านราวอ',
    'บ้านคลอง',
    'บ้านทุ่ง',
    'บ้านท่าข้าม',
    'บ้านท่าน้ำ',
    'บ้านท่าน้ำตะวันออก',
    'บ้านท่าน้ำตะวันตก',
    'บ้านบางมะรวด',
    'บ้านท่าชะเมา',
    'บ้านบ่ออิฐ',
    'วัดศรีสุดาจันทร์',
    'บ้านกลาง',
    'บ้านนอก',
    'บ้านปุลามาวอ',
    'บ้านหัวคลอง',
    'บ้านน้ำบ่อ',
    'บ้านเตราะหัก',
    'บ้านท่าสู',
    'บ้านบางหมู',
    'บ้านคลองต่ำ',
    'บ้านปะนาเระ (รัฐอุทิศ)',
    'บ้านคาโต',
    'บ้านพ่อมิ่ง',
    'บ้านเคียน',
    'บ้านปายอ',
    'บ้านโตะบาลา',
    'บ้านละอาร์',
    'บ้านแซะโมะ',
    'บ้านลานช้าง',
    'บ้านเจาะกือแย',
    'บ้านตะบิ้ง',
    'บ้านกะลาพอ',
    'บ้าน?ือดัง',
    'บ้านชะเมาสามต้น',
    'บ้านบาโงมูลง',
    'บ้านพอเหมาะ',
    'ชุมชนบ้านทุ่งคล้า',
    'ชุมชนบ้านบางเก่า',
    'บ้านป่าทุ่ง',
    'บ้านบาโงยือริง',
    'บ้านบือเระ',
    'บ้านบน',
    'บ้านบาเลาะ',
    'บ้านทุ่งเค็จ',
    'บ้านจ่ากอง',
    'ชุมชนวัดถัมภาวาส',
    'บ้านจะเฆ่',
    'วัดโบกขรณี',
    'ชุมชนบ้านแป้น',
    'บ้านละหาร',
    'บ้านกาหงษ์',
    'บ้านทุ่งกินนร',
    'ชุมชนบ้านมะนังดาลำ',
    'บ้านป่าม่วง',
    'ชุมชนบ้านละหารมิตรภาพที่ 113',
    'บ้านช่องแมว',
    'บ้านวังไชย',
    'บ้านป่าไหม้',
    'บ้านละเวง',
    'บ้านรังมดแดง',
    'บ้านตะโละไกรทอง',
    'บ้านท่าช้าง',
    'วัดสารวัน',
    'บ้านโคกนิบง',
    'บ้านปลักแตน',
    'วัดโชติรส',
    'บ้านกระจูด',
    'บ้านมะปริง',
    'บ้านจะรัง',
    'บ้านตูเวาะ',
    'บ้านดูซงปาแย',
    'บ้านตะโละ',
    'ชุมชนวัดป่าศรี',
    'ชุมชนวัดปิยาราม',
    'ชุมชนบ้านปาตา',
    'บ้านท่ากุน',
    'บ้านท่าด่าน',
    'บ้านท่าพง',
    'บ้านยือริง',
    'บ้านตันหยงดาลอ',
    'บ้านตือระ',
    'บ้านเฑียรยา',
    'บ้านฝาง',
    'บ้านบากง',
    'บ้านใหม่',
    'บ้านตาลีอายร์',
    'ชุมชนบ้านตาแกะ',
    'บ้านบางปู',
    'บ้านบือเจาะ',
    'บ้านจาแบปะมิตรภาพที่ 197',
    'บ้านโต๊ะตีเต',
    'บ้านบาโลย',
    'บ้านตาหมน',
    'บ้านปุลากง',
    'บ้านตันหยง',
    'บ้านดาลอ',
    'บ้านชะเอาะ',
    'ยะหริ่ง',
    'บ้านยามูเฉลิม',
    'บ้านมูหลง',
    'ชุมชนบ้านสาบัน',
    'บ้านหนองแรต',
    'บ้านแบรอ',
    'บ้านลางา',
    'บ้านบูดี',
    'บ้านตะโละสะมีแล',
    'บ้านดาโต๊ะ',
    'บ้านวังกะพ้อ "เพียรอนุสรณ์"',
    'บ้านกะรุบี',
    'บ้านบาโงยือแบ็ง',
    'บ้านอุแตบือราแง',
    'บ้านตะโละดารามัน',
    'บ้านบีติง',
    'บ้านบือแต',
    'บ้านปล่องหอย',
    'บ้านมือลอ (มะอาแดอุทิศ)',
    'บ้านมะกอ',
    'บ้านมะแนดาแล',
    'บ้านโลทู',
    'เบญจมราชูทิศ จังหวัดปัตตานี',
    'เดชะปัตตนยานุกูล',
    'ท่าข้ามวิทยาคาร',
    'วุฒิชัยวิทยา',
    'สายบุรี(แจ้งประชาคาร)',
    'ไม้แก่นกิตติวิทย์',
    'สุวรรณไพบูลย์',
    'วังกะพ้อพิทยาคม',
    'บ้านควนโนรี',
    'บ้านตุปะ',
    'บ้านป่าไร่',
    'วัดภมรคติวัน',
    'บ้านโคกโพธิ์',
    'บ้านสามยอด',
    'บ้านท่าคลอง',
    'บ้านทุ่งยาว',
    'บ้านโคกต้นสะตอ',
    'วัดบันลือคชาวาส',
    'บ้านควนลังงา',
    'วัดทรายขาว',
    'นิคมสร้างตนเองโคกโพธิ์',
    'บ้านท่าเรือ',
    'บ้านควนแตน',
    'สมเด็จหลวงพ่อทวดวัดช้างให้',
    'วัดอรัญวาสิการาม',
    'บ้านเกาะตา',
    'บ้านนาเกตุ',
    'บ้านคลองช้าง',
    'บ้านควนลาแม',
    'บ้านบาโงฆาดิง',
    'บ้านชะเมา',
    'วัดนาประดู่',
    'บ้านนาประดู่',
    'บ้านควนประ',
    'บ้านยางแดง',
    'วัดนิคมสถิต',
    'วัดสุนทรวารี',
    'บ้านล้อแตก',
    'วัดธนาภิมุข',
    'บ้านคลองหิน',
    'บ้านโผงโผง',
    'บ้านบาเงง',
    'บ้านป่าบอน',
    'บ้านนาค้อเหนือ',
    'บ้านนาค้อกลาง',
    'บ้านดอนเค็ด',
    'บ้านนาค้อใต้',
    'วัดมะกรูด',
    'บ้านเกาะเปาะ',
    'บ้านใหม่',
    'บ้านคอลอตันหยง',
    'บ้านกาแลกุมิ',
    'บ้านตลาดนัดต้นสะท้อน',
    'บ้านโคกโตนด',
    'บ้านดอนรัก',
    'บ้านปะกาจินอ',
    'บ้านเปียะ',
    'บ้านบาโงกาเซาะ',
    'บ้านโคกหมัก',
    'วัดมุจลินทวาปีวิหาร',
    'ประชาอุทิศบ้านโคกม่วง',
    'บ้านปะกาลือสง',
    'บ้านปากบางตาวา',
    'บ้านปรัง',
    'บ้านเกาะหม้อแกง',
    'บ้านท่ายามู',
    'บ้านบางราพา',
    'บ้านท่ากำชำ',
    'บ้านตันหยงเปาว์',
    'วัดสุวรรณากร',
    'บ้านควนดิน',
    'บ้านปะแดลางา',
    'บ้านไผ่มัน',
    'กองทัพบกอุทิศบ้านดอนยาง',
    'บ้านควนคูหา',
    'บ้านบางทัน',
    'บ้านดอนนา',
    'รัชดาภิเษก',
    'บ้านตลาดนัดคลองขุด',
    'บ้านสายหมอ',
    'บ้านแคนา',
    'วัดสถิตย์ชลธาร',
    'บ้านกาหยี',
    'พัฒนศึกษา',
    'บ้านค่าย',
    'บ้านยาบี',
    'บ้านน้ำดำ',
    'บ้านคลองช้าง',
    'บ้านมะพร้าวต้นเดียว',
    'บ้านทุ่งโพธิ์',
    'บ้านกระเสาะ',
    'บ้านลางสาด',
    'บ้านกระหวะ',
    'บ้านราวอ',
    'บ้านแขนท้าว',
    'บ้านเกาะจัน',
    'บ้านคลองช้าง',
    'บ้านตรัง',
    'บ้านเขาวัง',
    'บ้านดูวา',
    'บ้านถนน',
    'บ้านมะหุด',
    'บ้านควนหยี',
    'บ้านบูดน',
    'บ้านบาโง',
    'บ้านปานัน',
    'มายอ (สถิตย์ภูผา)',
    'บ้านปาลัส',
    'บ้านด่าน',
    'บ้านบาละแต',
    'ชุมชนบ้านเมืองยอน',
    'บ้านตะบิงตีงี',
    'บ้านน้ำใส',
    'บ้านเจาะบาแน',
    'บ้านบูเกะกุง',
    'บ้านสะกำ',
    'บ้านกูบังบาเดาะ',
    'บ้านกูวิง',
    'บ้านสมาหอ',
    'บ้านบาตะกูโบ',
    'บ้านแลแวะ',
    'บ้านตะโละแมะนา',
    'บ้านน้ำดำ',
    'บ้านปากู',
    'พระราชประสงค์บ้านทรายขาว',
    'ชุมชนบ้านปาแดปาลัส',
    'บ้านมะนังยง',
    'บ้านเขาดิน',
    'บ้านพิเทน(วันครู 2502)',
    'บ้านโต๊ะชูด',
    'บ้านตือเบาะ',
    'บ้านบือจะ',
    'ตลาดปรีกี',
    'บ้านกระโด',
    'บ้านสายชล',
    'บ้านละหารยามู',
    'บ้านสายชล (สาขากูแบสีรา)',
    'บ้านโคกขี้เหล็ก',
    'บ้านเขาตูม',
    'บ้านอินทนิล',
    'ชุมชนบ้านต้นสน',
    'บ้านบินยา',
    'บ้านกาแลสะนอ',
    'วัดโคกหญ้าคา',
    'บ้านบราโอ (สาขากูนิง)',
    'บ้านบูโกะ',
    'บ้านบราโอ',
    'ชุมชนบ้านบือแนปีแน',
    'บ้านประจัน',
    'ตลาดนัดบาซาเอ',
    'ไทยรัฐวิทยา 52 (ชุมชนบ้านต้นไพ)',
    'บ้านต้นแซะ',
    'ตลาดนัดต้นมะขาม',
    'ชุมชนวัดอัมพวนาราม',
    'ชุมชนบ้านพงสตา',
    'บ้านต้นทุเรียน',
    'บ้านกรือเซะ',
    'บ้านยะรัง',
    'บ้านระแว้ง',
    'ต้นพิกุล',
    'วัดเกาะหวาย',
    'บ้านศาลาสอง',
    'บ้านสิเดะ',
    'บ้านอีบุ๊',
    'บ้านบากง',
    'บ้านอาโห',
    'ชุมชนบ้านสะนอ',
    'บ้านคางา',
    'บ้านต้นโตนด',
    'บ้านวังกว้าง',
    'บ้านควนแปลงงู',
    'บ้านม่วงเตี้ย',
    'บ้านปลักปรือ',
    'บ้านคูระ',
    'บ้านตันหยง',
    'วัดบุพนิมิต',
    'บ้านโคกเหรียง',
    'วัดป่าสวย',
    'บ้านคลองทราย',
    'ราชมุนีรังสฤษฏ์',
    'โพธิ์คีรีราชศึกษา',
    'ปทุมคงคาอนุสรณ์',
    'ยาบีบรรณวิทย์',
    'ศิริราษฏร์สามัคคี',
    'ทุ่งยางแดงพิทยาคม',
    'ประตูโพธิ์วิทยา',
    'สะนอพิทยาคม',
    'แม่ลานวิทยา',
    'วัดกําแพง',
    'บ้านยาบี',
    'บ้านไร่พญา',
    'บ้านกาแนะ',
    'บ้านกูแบสาลอ',
    'บ้านโคกศิลา',
    'บ้านบางมะนาว',
    'บ้านค่าย',
    'บ้านปูลากาป๊ะ',
    'บ้านเขาตันหยง(มิตรภาพที่ 153)',
    'บ้านหัวเขา',
    'ไทยรัฐวิทยา10(บ้านใหม่)',
    'บ้านโคกสยา',
    'บ้านเปล',
    'วัดพนาสณฑ์',
    'บ้านโคกเคียน',
    'บ้านโคกพยอม',
    'บ้านบือราเปะ',
    'บ้านฮูแตทูวอ',
    'บ้านทอน',
    'บ้านทรายขาว',
    'อนุบาลนราธิวาส',
    'เมืองนราธิวาส',
    'บ้านทุ่งโต๊ะดัง',
    'บ้านจืองา',
    'บ้านตะโละแน็ง',
    'บ้านยารอ',
    'บ้านโคกตีเต',
    'บ้านโคกสุมุ',
    'บ้านแคนา',
    'บ้านโต๊ะนอ',
    'บ้านจูดแดง',
    'บ้านโคกแมแน',
    'บ้านจะแลเกาะ',
    'บ้านมะนังกาหยี',
    'บ้านสุไหงบาลา',
    'วัดโคกโก',
    'บ้านรามา',
    'บ้านทําเนียบ',
    'บ้านปลักปลา',
    'วัดลําภู',
    'บ้านยะลูตง(ประชุมวิทยาสาร)',
    'บ้านกาเยาะมาตี',
    'บ้านยือลอ',
    'บ้านแป๊ะบุญ',
    'บ้านบือเจ๊าะ',
    'น้ำตกปาโจ (วันครู 2501)',
    'บ้านปะลุกานากอ',
    'บ้านดูกู',
    'บ้านบือเระ',
    'บ้านบูเกะสูดอ',
    'บ้านคลอแระ',
    'บ้านตันหยง',
    'บ้านฮูแตยือลอ',
    'บ้านกะลูแป',
    'บ้านอีโยะ',
    'บ้านอาตะบือเระ',
    'บ้านบือแนปีแย',
    'บ้านปะลุกาสาเมาะ',
    'วัดเชิงเขา(แดงอุทิศ)',
    'บ้านมะยูง',
    'บ้านบาตูฯ มิตรภาพที่ 66',
    'บ้านปะลุกาแปเราะ',
    'บ้านตะโละบาลอ',
    'บ้านสุไหงบาตู',
    'ราชประชานุเคราะห์ 9',
    'บ้านปูซูตีฆอ',
    'ราชประชานุเคราะห์ 10',
    'บ้านต้นตาล',
    'บ้านแยะ',
    'บ้านนากอ(มะดากะอุทิศ)',
    'บ้านลุโบะปาเระ',
    'บ้านจอเบาะ',
    'บ้านตะโละมีญอ',
    'บ้านบูเกะบากง',
    'บ้านบลูกาสนอ',
    'บ้านกูยิ',
    'บ้านยี่งอ',
    'บ้านกูเล็ง',
    'บ้านบูเกะกอตอ',
    'บ้านกาบุ๊',
    'บ้านกาเด็ง',
    'บ้านศาลาลูกไก่',
    'บ้านทุ่งคา',
    'บ้านปูตะ',
    'บ้านกูแว',
    'บ้านลูโบะบายะ',
    'บ้านปาลอบาต๊ะ',
    'บ้านกําปงปีแซ',
    'บ้านลูโบะดาโต๊ะ',
    'บ้านกาแร',
    'บ้านกาลิซา',
    'บ้านกาหนั๊วะ',
    'บ้านฮูลู',
    'บ้านกาเด็ง',
    'บ้านสะโล',
    'บ้านลูโบ๊ะกาเยาะ',
    'บ้านบาโง',
    'บ้านสิโป',
    'บ้านกูจิงลือปะ',
    'บ้านทํานบ',
    'ระแงะ',
    'บ้านเขาพระ',
    'วัดร่อน',
    'บ้านบือนังกือเปาะ',
    'บ้านบ่อทอง',
    'บ้านกอแนะเหนือ',
    'วัดตันติการามมิตรภาพที่ 109',
    'บ้านตะโละ',
    'บ้านแกแม',
    'บ้านป่าไผ่',
    'บ้านตันหยงลิมอ',
    'บ้านลูโบ๊ะดีแย',
    'ชุมชนสหพัฒนา',
    'บ้านบองอ',
    'บ้านบาโงกูโบ',
    'บ้านสาเมาะ',
    'บ้านลาไม',
    'บ้านปาเซ',
    'ประชาบํารุง',
    'บ้านบละแต',
    'บ้านลูโบ๊ะบาตู',
    'บ้านเจ๊ะเก',
    'บ้านเขาน้อย',
    'บ้านบาโงสะโต',
    'บ้านตราแดะ',
    'ไอยรานุสรณ์',
    'บ้านบระเอ็ง',
    'บ้านเขาแก้ว',
    'บ้านมะรือโบตก',
    'บ้านบาโงระนะ',
    'บ้านนิบง',
    'บ้านบาโงปะแต',
    'บ้านสายน้ำทิพย์',
    'บ้านสวนพลู',
    'บ้านธรรมเจริญ',
    'บ้านบลูกาฮีเล',
    'ประชาพัฒนา',
    'บ้านพนาทักษิณ',
    'บ้านบาตง',
    'วัดราษฎร์สโมสร(ไพโรจน์วิทยา)',
    'บ้านนาดา',
    'บ้านรือเสาะ',
    'บ้านยาแลเบาะ',
    'บ้านบากง',
    'บ้านนาโอน',
    'บ้านบูกิ๊ตยือแร',
    'บ้านบลูกา',
    'บ้านยะบะ(อุปการวิทยา)',
    'บ้านกาโดะ',
    'บ้านลาเมาะ',
    'บ้านลาเมาะฮีเล',
    'บ้านสะแนะ',
    'บ้านสุเป๊ะ',
    'บ้านซือเลาะ',
    'บ้านลูโบะบูโละ',
    'บ้านอีนอ',
    'บ้านกูยิ',
    'บ้านบริจ๊ะ',
    'บ้านบือแรง',
    'ชุมชนสัมพันธ์บ้านลาโละ',
    'บ้านมะนังปันยัง',
    'บ้านบาโงปูโละ',
    'บ้านละหาน',
    'บ้านดือแย',
    'บ้านสาวอ',
    'บ้านจือแร',
    'บ้านมือและห์',
    'บ้านตายา',
    'บ้านยือลาแป',
    'บ้านดือแยหะยี',
    'บ้านบือเล็งใต้',
    'ประชาวิทยรังสรรค์',
    'บ้านป่าไผ่',
    'คีรีราษฎร์รังสฤษดิ์',
    'บ้านลาเวง',
    'บ้านตามุง',
    'สว่างวัฒนา',
    'บ้านดาฮง',
    'บ้านสาคร',
    'บ้านกลูบี',
    'ชุมชนบ้านซากอ',
    'บ้านตะมะยูง',
    'สามัคคีวิทยา',
    'บ้านตอหลัง',
    'บ้านบือแนนากอ',
    'บ้านลูโบ๊ะบาตู',
    'บ้านไอร์แยง',
    'บ้านปาหนัน',
    'บ้านคอลอกาเว',
    'นราธิวาส',
    'นราสิกขาลัย',
    'เฉลิมพระเกียรติ์ กรมหลวงนราธิวาสราชนครินทร์ บางปอป',
    'บาเจาะ',
    'ร่มเกล้า',
    'ตันหยงมัส',
    'รือเสาะชนูปถัมภ์',
    'เรียงราษฏร์อุปถัมภ์',
    'ศรีวารินทร์',
    'บ้านปูยู',
    'บ้านศรีพงัน',
    'บ้านตะเหลี่ยง',
    'บ้านเกาะสะท้อน',
    'บ้านปะลุกา',
    'บ้านสะหริ่ง',
    'บ้านโคกมือบามิตรภาพที่ 223',
    'บ้านปลักปลา',
    'วัดชลธาราสิงเห(เสาร์ศึกษาคาร)',
    'บ้านตาบา',
    'บ้านหัวคลอง',
    'บ้านแฆแบ๊ะ',
    'บ้านปะดะดอ',
    'วัดเกษตรธิการาม',
    'บ้านโคกงู',
    'บ้านบางขุนทอง',
    'วัดฉัททันต์สนาน',
    'วัดโคกมะม่วง',
    'วัดพระพุทธ',
    'วัดสิทธิสารประดิษฐ์',
    'บ้านกูบู',
    'บ้านสะปอม',
    'บ้านโคกกระดูกหมู',
    'บ้านไพรวัน',
    'วัดเกาะสวาด',
    'วัดทรายขาว',
    'บ้านตอหลัง',
    'วัดโคกมะเฟือง',
    'บ้านปูลาเจ๊ะมูดอ',
    'บ้านคลองตัน',
    'บ้านศาลาใหม่',
    'บ้านสามแยก',
    'บ้านบาโง',
    'บ้านไม้ฝาด',
    'บ้านศาลาอูมา',
    'บ้านตอออ',
    'บ้านจะมาแกะ',
    'บ้านบางขุด',
    'บ้านตําเสาพัฒนา',
    'นิคมสร้างตนเองแว้ง',
    'บ้านแม่ดง',
    'บ้านตอแล',
    'บ้านยะหอ',
    'บ้านบูเกะตา',
    'นิคมสร้างตนเองแว้งสายโท2',
    'บ้านนูโร๊ะ',
    'บ้านเจ๊ะยอ',
    'เทพประทาน(บ้านเจ๊ะเด็ง)',
    'เทพประทานไทยยืนยง',
    'บ้านฆอเลาะทูวอ',
    'บ้านกูวา',
    'บ้านกรือซอ',
    'บ้านเจ๊ะเหม',
    'บ้านแว้ง',
    'บ้านแอแว',
    'บ้านตือมายู',
    'เพลินพิศ',
    'บ้านแขยง',
    'นิคมพัฒนา2',
    'บ้านน้ำใส',
    'รักไทย',
    'บ้านภูเขาทอง',
    'สุคิริน',
    'นิคมพัฒนา 4',
    'นิคมพัฒนา10',
    'บ้านน้ำทุเรียน',
    'บ้านบาลูกายาอิง',
    'บ้านร่วมใจ',
    'นิคมพัฒนา5',
    'นิคมพัฒนา6',
    'บ้านจุฬาภรณ์ 12',
    'นิคมพัฒนา7',
    'นิคมพัฒนา9',
    'บ้านซรายอ',
    'บ้านตือระ มิตรภาพที่ 172',
    'บ้านลูโบ๊ะซามา',
    'บ้านกวาลอซีรา',
    'บ้านมือบา',
    'บ้านลาแล',
    'บ้านโต๊ะเวาะ',
    'บ้านกูแบอีแก',
    'บ้านมูโนะ',
    'บ้านปาดังยอ',
    'บ้านลูโบะลือซง',
    'บ้านปูโป๊ะ',
    'บ้านสุไหงโก-ลก',
    'บ้านกาวะ',
    'บ้านสายะ',
    'ไทยรัฐวิทยา 89 (บ้านโคกสยา)',
    'บ้านบาโงฮูมอ',
    'บ้านเปาะเจ๊ะเต็ง',
    'บ้านไอบาตู',
    'บ้านบือราแง',
    'บ้านโคกโก',
    'บ้านโผลง',
    'บ้านโต๊ะเด็ง',
    'อิสลามบํารุง',
    'บ้านปะลุรู',
    'บ้านปอเนาะ',
    'บ้านละหาน',
    'บ้านโคกตา',
    'บ้านตาเซะใต้',
    'บ้านกูวา',
    'บ้านดอเหะ',
    'บ้านบาโงกือเต',
    'บ้านตะโละบูเก๊ะ',
    'บ้านสากอ',
    'บ้านไม้แก่น',
    'บ้านกลูบี',
    'บ้านจือแร',
    'บ้านบอเกาะ',
    'บ้านบาโงมาแย',
    'บ้านตาเซะเหนือ',
    'วัดโบราณสถิตย์',
    'วัดประดิษฐ์บุปผา',
    'บ้านตลิ่งสูง',
    'ราชภักดี',
    'วัดประชุมชลธารา',
    'บ้านจะแนะ',
    'บ้านตือกอ',
    'บ้านละหาร',
    'บ้านปารี',
    'บ้านไอกรอส',
    'บ้านยะออ',
    'บ้านน้ำวน',
    'พิทักษ์วิทยากุมุง',
    'ร่วมจิตประชา',
    'บ้านไอร์โซ',
    'บ้านน้ำหอม',
    'บ้านรือเปาะ',
    'บ้านกาแย',
    'บ้านดุซงยอ',
    'บ้านแมะแซ',
    'บ้านบูเก๊ะบือแต',
    'บ้านริแง',
    'ผดุงมาตร',
    'บ้านยานิง',
    'บ้านเจาะไอร้อง',
    'บ้านโคก',
    'บ้านบาโงดุดุง',
    'บ้านลูโบ๊ะเยาะ',
    'บ้านบูกิต',
    'บ้านไอสะเตียร์',
    'บ้านบาตาปาเซ',
    'บ้านบำรุงวิทย์',
    'บ้านเจาะเกาะ',
    'บ้านบูเก๊ะตาโมงมิตรภาพที่ 128',
    'บ้านมะรือโบออกมิตรภาพที่ 199',
    'บ้านจูโวะ',
    'ราชพัฒนา',
    'ราชประสงค์',
    'บ้านตาโงะ',
    'ตากใบ',
    'เวียงสุวรรณวิทยาคม',
    'ศึกษาสงเคราะห์นราธิวาส',
    'สุคิรินวิทยา',
    'สุไหงโกลก',
    'ธัญธารวิทยา',
    'มัธยมสุไหงปาดี',
    'บูกิตประชาอุปถัมภ์',
    'สตรีวรนาถ',
    'สุพมาศพิทยาคม',
    'แสนยานุกรวิทยา',
    'สวนเด็ก',
    'ราชินี',
    'ภารตวิทยาลัย',
    'พิมานวิทย์',
    'อนุบาลกรแก้ว',
    'อนุบาลธนสมบูรณ์อำนวยสงคราม',
    'อัมพวันศึกษา',
    'อักษรเจริญ',
    'อนุบาลธรรมภิรักษ์เทเวศร์',
    'อนุบาลศรีสัปดาห์',
    'อนุบาลสาริน',
    'ศรีดรุณ',
    'ช่างอากาศอำรุง',
    'วชิราวุธวิทยาลัย',
    'อนุบาลเลิศนุวัฒน์',
    'อนุบาลยุววิทย์',
    'เซนต์คาเบรียล',
    'กันตะบุตร',
    'คอนเซ็ปชัญ',
    'สรรพาวุธบำรุง',
    'เซนต์ฟรังซีสซาเวียร์คอนแวนต์',
    'ราชินีบน',
    'กองทัพบกอุปถัมภ์สื่อสารสงเคราะห์',
    'โยนออฟอาร์ค แผนกอนุบาล',
    'พันธะศึกษาพิทยา',
    'พันธะศึกษา',
    'พันธะวัฒนา',
    'เยาวชนศึกษา',
    'โยนออฟอาร์ค',
    'จิตรลดา',
    'เซนต์เทเรซา',
    'วิทยปัญญา',
    'อนุบาลธนามาศ',
    'หนองจอกพิทยานุสสรณ์มัธยม',
    'สุเหร่าคู้วิทยา',
    'อัสสัมชัญศึกษา',
    'อัสสัมชัญ',
    'อนุบาลเปล่งประสิทธิ์',
    'อนุบาลจิดาภา',
    'อนุบาลชวนชื่น',
    'อัสสัมชัญคอนแวนต์',
    'อาทรศึกษา',
    'สัจจพิทยา',
    'กรุงเทพคริสเตียนวิทยาลัย',
    'เยนเฮส์เม็มโมเรียล',
    'อนุบาลเสริมมิตร',
    'อัสสัมชัญคอนแวนต์ สีลม',
    'เซนต์โยเซฟคอนเวนต์',
    'เลิศอุบลอนุบาล',
    'อนุบาลชัยพฤกษ์',
    'ภูมิไพโรจน์พิทยา',
    'อนุบาลนิเวศน์วารินทร์',
    'นิเวศน์วารินทร์',
    'อนุบาลจารุวรรณ',
    'อนุบาลรัตนฉัตร',
    'สุขฤทัย',
    'อนุบาลปราโมชพัฒนา',
    'ปราโมชวิทยารามอินทรา',
    'ปราโมชวิทยาทาน',
    'กองทัพบกอุปถัมภ์เพชราวุธวิทยา ในพระอุปถัมภ์ ฯ',
    'เอี่ยมพานิชวิทยา',
    'อุทัยวิทยา',
    'อนุบาลปริญญา',
    'อนุบาลพิมลทิพย์',
    'อมาตยกุล',
    'อนุบาลวัดไตรรัตนาราม',
    'นันทนิตย์',
    'นราธิปพิทยา',
    'ณัฏฐวุฒิวิทยา',
    'อนุบาลชุติมา',
    'อนุบาลกาญจนาพร',
    'เพ็ญสมิทธ์',
    'หัวหมากวิทยานุสรณ์',
    'อนุบาลเด็กสากล',
    'พระมารดานิจจานุเคราะห์',
    'เด็กสากล',
    'ถนอมบุตร',
    'อนุบาลกรสุภา',
    'วัฒนานนท์วิทยา',
    'อนุบาลหัวหมาก',
    'อนุบาลภัทรบุตร์',
    'อนุบาลทับแก้ว',
    'อนุบาลศรีนวล',
    'อนุบาลบ้านทิพย์',
    'อนุบาลใจทิพ',
    'อนุบาลวิมลทิพย์',
    'อนุบาลวรรณทิพย์ ชุมชนคลองจั่น',
    'อนุบาลสุดนรี',
    'สีตบุตรบำรุง',
    'ประเสริฐธรรมวิทยา',
    'อนุบาลสุนี',
    'อนุบาลแสงประเสริฐ',
    'อนุบาลจุไรรัตน์',
    'มาแตร์เดอีวิทยาลัย',
    'สุนีพิทยา',
    'พระมหาไถ่ศึกษา',
    'สตรีศรีบำรุง',
    'สตรีจุลนาค',
    'สมานศึกษาวิทยา',
    'บำรุงอิสลามวิทยา',
    'อนุบาลบ้านบาตร',
    'อนุบาลสวนดรุณ',
    'อนุบาลหนูน้อย',
    'อนุบาลพัฒนศิริ',
    'อนุบาลปรานี',
    'วัดวชิรธรรมสาธิต',
    'สมถวิล',
    'นวลวรรณศึกษา',
    'ใจรักอนุบาล',
    'เลียงแสงทองอนุบาล',
    'บพิธวิทยา',
    'มูลนิธิวัดราษฎร์ศรัทธาธรรมสงเคราะห์',
    'สมาหารศึกษา',
    'สหะพาณิชย์แผนกสามัญปุณณวิถี',
    'อนุบาลวัฒนาสาธิต',
    'นารีนิรมล',
    'อนุบาลปาณยา',
    'พิพัฒนา',
    'มีนบุรีศึกษา',
    'อนุบาลดรุณตะวัน',
    'อนุบาลจิตรเกษม',
    'นาดาวิทยาทาน',
    'เทพอักษร',
    'พรสุดามีนบุรี',
    'สุขเนตร',
    'มีนประสาทวิทยา',
    'สุดใจวิทยา',
    'อนุบาลพร้อม',
    'ยุพลพัฒน์วัฒนา',
    'อนุบาลวังทอง',
    'อรรถญาสาธิต',
    'ยอดดวงใจ',
    'อนุบาลเคหะร่มเกล้า',
    'ปัญญาภรณ์',
    'ศึกษาพัฒนา',
    'มาเรียลัย',
    'เชิดเจิมศิลป์',
    'สารสาสน์วิเทศร่มเกล้า',
    'ประเวศพิทยาคาร',
    'อนุบาลมณีรัตน์',
    'อนุบาลบ้านสานฝัน',
    'สารสาสน์เอกตรา',
    'อนุบาลมลิวัลย์',
    'อนุบาลบ้านทรงไทย',
    'อนุบาลมงคลวรรณ',
    'สารสาสน์พิทยา',
    'ทวีวัฒนา',
    'วรมงคล',
    'อนุบาลพระแม่แห่งยุติธรรม',
    'เซนต์ยอแซฟยานนาวา',
    'พระแม่มารีสาธุประดิษฐ์',
    'สารสาสน์พัฒนา',
    'กุหลาบวิทยา',
    'อนุบาลพิณทิพย์',
    'เรวดี',
    'อนุบาลบ้านแก้ว',
    'อนุบาลเพ็ญสันต์',
    'อนุบาลรัศมี',
    'อนุบาลดรุณา',
    'สวนบัว',
    'รุจิเสรีวิทยา',
    'ธำรงวิทย์',
    'อนุบาลเบญจมาศ',
    'สัตย์สงวนวิทยา',
    'อนุบาลสายทอง',
    'สตรีกุหลาบธนบุรี',
    'แสงอรุณ',
    'อำนวยศิษย์ศึกษา',
    'อำนวยวงศ์วิทยา',
    'อนุบาลวรรณบูรณ์ศึกษา',
    'อนุบาลซางตาครู้ส',
    'อนุบาลแสงอรุณ',
    'อนุบาลแสงอรุณ ธนบุรี',
    'อนุบาลพัชรินทร์พร',
    'อนุบาลอำไพวานิช',
    'อนุบาลบัวเทพ',
    'สหบำรุงวิทยา',
    'ฤดีศึกษา',
    'น้ำชลชนานุกูล',
    'สหนิยมวิทยา',
    'เกื้อวิทยา',
    'จรวยพรวิทยา',
    'ซางตาครู้สศึกษา',
    'ดรุณวิทย์วิทยา',
    'พรประสาทวิทยา',
    'มังคละศึกษา',
    'มณีวิทยา',
    'มนตรีวิทยา',
    'วัฒนะศึกษา',
    'ศิริรักษ์วิทยา',
    'ซางตาครู้สคอนแวนท์',
    'สายประสิทธิ์วิทยา',
    'อนุบาลรุจิโรจน์',
    'เสสะเวชวิทยา',
    'ชัยพิทยพัฒน์ มูลนิธิชัยพัฒนา (จารุวัฒนานุกูล ท่าพระ)',
    'แสงมณี',
    'ประสาทวิทยาอนุชน แผนกอนุบาล',
    'กรุงธนวิทยาภวัน',
    'ประสาทวิทยาอนุชน',
    'ชาตศึกษา',
    'อนุบาลวรพิม',
    'สุโรจน์วิทยา',
    'สิริเทพ',
    'บางกอกทวิวิทย์',
    'อนุบาลบุณยรักษ์',
    'อนุบาลสีชมพู',
    'อนุบาลวัฒนานิเวศน์',
    'ศิริพรรณวิทยา',
    'อนุบาลธนชา',
    'อนุบาลประนันทนิจ',
    'อนุบาลทรงวิทย์',
    'อนุบาลลีนา',
    'สมฤทัย',
    'ดลวิทยา',
    'อนุบาลกุลมาศวิทยา',
    'ชาญวิทย์',
    'อนุบาลโชคชัย',
    'สตรีวุฑฒิศึกษา',
    'อนุบาลนันทิยา',
    'อนุบาลเชาวน์ดี',
    'อนุบาลธีรชาต',
    'อนุบาลบ้านไทย',
    'อนุบาลสุวาวรรณ',
    'พัฒนาวิทยา',
    'บำรุงวิชา',
    'ภาษานุสรณ์ ธนบุรี',
    'อนุบาลศันสนีย์',
    'จันทรวิทยา',
    'ศิริธนศึกษา',
    'จิตตเมตต์',
    'อนุบาลเตรียมศึกษา',
    'อนุบาลสุนทรเกตุ',
    'อยู่เย็นวิทยา',
    'อนุบาลนกน้อย',
    'กุศลศึกษา',
    'อนุบาลกุลบุตรวัฒนโรจน์',
    'วรรัตน์ศึกษา',
    'ปิยะวิทยา ตลิ่งชัน',
    'ปิยะมิตร',
    'นิลประพันธ์',
    'อนุบาลอภิรักษ์ชัยพฤกษ์',
    'อนุบาลดรุณวัฒนา',
    'อนุบาลมีลำใย',
    'อนุบาลจันทยานนท์',
    'อนุบาลฐนิตา',
    'อนุบาลธรรมภิรักษ์ธนบุรี',
    'อนุบาลจิรภา',
    'อนุชนบางกอกน้อย',
    'บุณยาจารย์',
    'อนุบาลสุนันท์ศึกษา',
    'นฤมลทิน ธนบุรี',
    'ดรุณวัฒนา',
    'ดิลกศึกษา',
    'เทพกาญจนา',
    'บำรุงวิทยาธนบุรี',
    'มงคลวิจิตรวิทยา',
    'ศุภวรรณ',
    'สุรวิทยา',
    'สุภัทรา',
    'อานันท์วิทยา',
    'อนุบาลเสริมวิทย์',
    'อนุบาลทิพารัตน์',
    'อนุบาลนพรัตน์',
    'สุพิชญา',
    'อนุบาลคุ้มเกล้า',
    'รุ่งอรุณ',
    'อนุบาลอภิสรา',
    'ศิรินุสรณ์วิทยา',
    'จารุวัฒนานุกูล',
    'สมิทธิพงษ์',
    'อนุบาลสุพิชญา',
    'อนุบาลวันเฉลิม',
    'วรรณสว่างจิต',
    'อนุบาลธารพระพร',
    'สุจิณณ์วดี',
    'เอกประสิทธิ์ศึกษา',
    'อนุบาลบ้านสวน',
    'อนุบาลธีรานุรักษ์',
    'สุธรรมศึกษา',
    'ตรีมิตรวิทยา',
    'อนุบาลดารวี',
    'ฐานปัญญา',
    'เปี่ยมสุวรรณวิทยา',
    'สุภาคมวิทยา',
    'ดำรงเรืองวิทย์',
    'ทองพูน',
    'นีรนาทวิทยา',
    'ชาญกิจวิทยา',
    'เผดิมศึกษา',
    'วิบูลย์เสรีวิทยา',
    'ศิลปบำรุง',
    'สุภาคมศึกษา',
    'สถาพรศึกษา',
    'อนุบาลนัยนา',
    'ศึกษาสมบูรณ์อนุสรณ์',
    'อนุบาลยุวมิตร',
    'เลิศหล้า',
    'อนุบาลเสริมปัญญาพุดตาน',
    'กรพิทักษ์ศึกษา',
    'อนุบาลยูรดา',
    'อนุบาลนครธน',
    'วีรสุนทร',
    'จันทศิริวิทยา',
    'อนุบาลก้องหล้า',
    'เมตตาวิทยา',
    'อนุบาลเยาวมิตร',
    'สารสาสน์สุขสวัสดิ์',
    'อนุบาลสมฤดี',
    'อนุบาลสุพัชชา',
    'อนุบาลบริบูรณ์ศึกษา',
    'อนุบาลปัญญาศักดิ์',
    'ปัญญาศักดิ์',
    'อนุบาลรังสิมา',
    'อนุบาลบูรณะศึกษา',
    'สมรรถภาพวิทยา',
    'โรจนนิมิตวิทยา',
    'เยาวลักษณ์วิทยาธนบุรี',
    'ขจรโรจน์วิทยา แผนกอนุบาล',
    'ขจรโรจน์วิทยา',
    'อนุบาลแสงอารีย์',
    'อุดมศาสตร์พัฒนา',
    'รัตนกุลอนุบาล',
    'อนุบาลบุษกร',
    'อนุบาลคุณแม่',
    'อนุบาลฤชากร',
    'สตรีบูรณวิทย์ แผนกอนุบาล',
    'อนุบาลมูลนิธิมัสยิดดารุลอิหซาน',
    'ทิวไผ่งาม',
    'อนุบาลแสงทองอนุสรณ์',
    'บางอ้อศึกษา',
    'ธรรมภิรักษ์',
    'สตรีบูรณวิทย์',
    'อนุบาลจินกาญจณ์ดา',
    'บูรณวิทย์',
    'ประมุขวิทยา',
    'พัฒนวิทย์',
    'พิมลวิทย์',
    'ศานติวิทยา',
    'ศิริมงคลศึกษา',
    'ศิริมงคลศึกษา แผนกอนุบาล',
    'ศรีอุลัย',
    'เขมะสิริอนุสสรณ์',
    'จำนงค์วิทยา',
    'อนุบาลสัตย์สงวนอนุสรณ์',
    'อำนวยพิทยา',
    'กอบวิทยา',
    'ปัญจทรัพย์',
    'อนุบาลเอกลักษณ์',
    'นิธิปริญญา',
    'อนุบาลปานตา',
    'อนุบาลสมใจ',
    'สามัคคีบำรุงวิทยา',
    'พร้อมพรรณวิทยา',
    'อนุบาลรังษีวิทย์',
    'แม่พระฟาติมา',
    'ประไพพัฒน์',
    'จำนงค์พิทยา',
    'อนุบาลจิราวิทย์',
    'อนุบาลสายสุดา',
    'อนุบาล ณ ฤดี',
    'ชินวร',
    'อนุบาลแก้วฤทัย',
    'โสมาภา',
    'อนุบาลรังรอง',
    'เบญจมินทร์',
    'อนุบาลพุทธชาด',
    'อนุบาลศรีนครพัฒนา',
    'อนุบาลอินทรารักษ์',
    'ปัฐวิกรณ์วิทยา',
    'ภัสสรานุบาล',
    'อนุบาลชินวร',
    'อนุบาลมารีนิรมล',
    'อนุบาลลูกหนู',
    'สายอักษร',
    'ยุวทูตศึกษา',
    'อนุบาลเครือคล้าย',
    'อนุบาลเกษมสุข',
    'ศุภวิทย์',
    'อัสสัมชัญแผนกประถม',
    'อนุบาลอนันตศานติศึกษา',
    'อนุบาลอารีวิทย์',
    'พระแม่มารีสาทร',
    'โกศลภัทรวิทย์',
    'อนุบาลจันทร์เจ้า',
    'อนุบาลแสงอรุณพระนคร',
    'ศิลปวัฒนา',
    'อนุบาลแสงเรืองศึกษา',
    'เธียรประสิทธิ์ศาสตร์',
    'อนุบาลพิรินทร',
    'เซนต์ไมเกิ้ล',
    'นิพัทธ์วิทยา',
    'ตรอกจันทน์วิทยา',
    'เซนต์หลุยส์ศึกษา',
    'เบญจวรรณศึกษา',
    'นวพัฒน์วิทยา',
    'ชูธรรมานุสรณ์',
    'อนุบาลบวรพรรณ',
    'เทพสัมฤทธิ์วิทยา',
    'ประชาราษฎร์พิทยา',
    'ปิ่นวัฒนา',
    'อนุบาลจุฬาศิริ',
    'พิริยะโยธิน',
    'วีระพิทยา',
    'อนุบาลแสงโสม',
    'กุลวรรณศึกษา',
    'อนุบาลศักดิ์สงวนวิทยา',
    'สมศรีรื่นศึกษา',
    'อนุบาลเจริญจรัมพรวิทยา',
    'รัตนวิทยาบางซื่อ',
    'ผะดุงศิษย์พิทยา',
    'ซินไตท้ง',
    'อนุบาลเศรษฐบุตร',
    'เพชรรัชต์ในพระอุปถัมภ์ฯ',
    'มาลากฤตยวิทยา',
    'สตรีวรนาถบางเขน',
    'สมิทธิโชติ',
    'ทับทอง',
    'อนุบาลไสวตาแตง',
    'แย้มสอาด',
    'อนุบาลเซนต์จอห์น',
    'อนุบาลสุดารักษ์ บางเขน',
    'อนุบาลพุทธรักษา',
    'อนุบาลหทัยสยาม',
    'พีระยา-นาวิน ในพระอุปถัมภ์ฯ',
    'ดาราทร',
    'อนุบาลแย้มสอาด',
    'อรรถมิตร',
    'อนุบาลโชติมา',
    'อนุบาลชนานันท์',
    'อนุบาลปรางทิพย์',
    'อนุบาลอาภาภิรมย์',
    'ธัชรินทร์วิทยาบางเขน',
    'เซนต์จอห์น',
    'จินดานุกุล',
    'แสงโสม',
    'เปรมฤดีศึกษา',
    'อนุบาลสุพิศมร',
    'อัสสละฟียะฮ์ วิทยาลัย',
    'เสริมปัญญาอนุบาล',
    'เสริมปัญญา',
    'วาสุเทวี',
    'รักษาราชวิตร',
    'เพาะปัญญา',
    'อนุบาลจิตตานนท์',
    'อนุบาลพรประสิทธิ์ ประดู่ 1',
    'อนุบาลเล็กประยูร',
    'อนุบาลเพาะปัญญา',
    'ศุภกรณ์วิทยา',
    'อนุบาลเปล่งประสิทธิ์ศรีนครินทร์',
    'นราทร',
    'อนุบาลมารียา',
    'อนุบาลศรินทิพย์',
    'อนุบาลจินดารัตน์',
    'วังเด็กพัฒนาและพัฒน์บริหารธุรกิจ',
    'วัฒนศิลป์วิทยาลัย',
    'อนุบาลดิสกล',
    'กฤตศิลป์วิทยา',
    'กรุงเทพพิทยา',
    'อนุบาลจินดาพร',
    'ไทยประสิทธิ์ศาสตร์',
    'อนุบาลบ้านรัก',
    'เจริญวุฒิวิทยา',
    'ดรุณาลัยสุขุมวิท',
    'ศรีวิกรม์',
    'วิศาลวิทย์',
    'วรรณวิทย์',
    'พระหฤทัยคอนแวนต์',
    'อนุบาลอธิพร',
    'อนุบาลเทพสนิท',
    'อนุบาลกุ๊กไก่',
    'อนุบาลสี่พี่น้อง',
    'วชิรสิน',
    'พระหฤทัยพัฒนเวศม์',
    'สามัคคีสงเคราะห์',
    'อนุบาลศรีวัฒนา',
    'สวนรัฐวิทยา',
    'ปิยะจิตวิทยา',
    'จินดาพงศ์',
    'อนุบาลสุวัฒนา',
    'อนุบาลศรีเสริมวิทย์',
    'อนุบาลใจปราณี',
    'อโศกวิทย์อ่อนนุช',
    'อนุบาลสายทิพย์',
    'อนุบาลบ้านริมคลอง',
    'อนุบาลวิมลโรจน์',
    'ภักดีวิทยา',
    'ประสานวิทย์วัฒนา',
    'อโศกวิทย์อ่อนนุช แผนกอนุบาล',
    'อนุบาลเทพสุดา',
    'ธนบุรีศึกษา',
    'บุญเสริมวิทยาธนบุรี',
    'เลิศพัฒนาศึกษา',
    'จินดามณี',
    'สิริอักษร ธนบุรี',
    'แม่พระประจักษ์',
    'อนุบาลศิริสุข (ศิริสุขวิทยา)',
    'อนุบาลสิริอักษร',
    'สมบุญวิทย์',
    'จินดาศึกษา',
    'อนุบาลบ้านยิ้ม',
    'อนุบาลเสริมสุธี',
    'อนุบาลประเสริฐศิลป์',
    'อนุบาลพรพิมพ์',
    'อนุบาลรัศมีสุข',
    'อนุบาลดอนเมืองบริรักษ์',
    'อนุบาลธรรมาภรณ์',
    'อนุบาลบริบูรณ์ศิลป์',
    'อนุบาลเกษร',
    'กรุงเทพวิเทศศึกษา',
    'อนุบาลธนินทร',
    'อนุบาลบ้านพลอยเพลิน',
    'ผ่องเพ็ญวิทยาดอนเมือง',
    'บริบูรณ์ศิลป์ศึกษา',
    'เจริญวิทยศึกษา',
    'พระหฤทัยดอนเมือง',
    'สมาคมสตรีไทย',
    'อนุบาลหมีน้อย',
    'เซนต์ดอมินิก',
    'โกวิทธำรง',
    'ปานะพันธุ์พญาไท',
    'สัมมาชีวศิลป',
    'ประเทืองวิทย์อุรุพงษ์',
    'อำนวยศิลป์',
    'อนุบาลอิ่มเอม',
    'สวนมิสกวัน',
    'อนุบาลตันติเมธ',
    'อนุบาลทองอินทร์',
    'อนุบาลจารุณี',
    'อนุบาลพันธ์พิศวัฒนา',
    'บ้านครัววิทยา',
    'อนุบาลโชคชัยลาดพร้าว',
    'อนุบาลบ้านแสนรัก',
    'ปิยะพงษ์วิทยา',
    'อนุบาลทองฤทัย',
    'อนุบาลดวงปัญญา',
    'สมิตสันต์',
    'อนุบาลเฉลิมขวัญ',
    'สันติสุขวิทยา',
    'โชคชัย',
    'กัลยวิทย์',
    'อนุบาลสังสิทธิ',
    'อนุบาลวรวรรณา',
    'อนุบาลฌัชฐาลักษม์',
    'อนุบาลเยี่ยมนุช',
    'เทพเสนานุสรณ์',
    'ฤทธิไกรศึกษา',
    'บุญฤดี',
    'แสงหิรัญ',
    'อนุบาลจุฑาภรณ์',
    'อนุบาลดารารัตน์',
    'อนุบาลมิตรเด็ก',
    'ทอสี',
    'แอ๊ดเวนตีสเอกมัย',
    'อนุบาลใจดี',
    'คริสต์ธรรมศึกษา',
    'วัฒนาวิทยาลัย',
    'เกษมพิทยา',
    'พระแม่มารีพระโขนง',
    'อำนวยศิลป์ธนบุรี',
    'ประทุมนุสรณ์',
    'ภูมิสุขศึกษา',
    'พิชญ์ชนก',
    'ภาษานุสรณ์บางแค',
    'มณีวัฒนา',
    'สารสาสน์ธนบุรี',
    'อนุบาลพิชญ์ชนก',
    'อนุบาลยุวภัทร',
    'สากลศึกษา บางแค',
    'อนุบาลเปี่ยมปัญญา',
    'อภิจิตอนุสรณ์',
    'กสิณธรอนุสรณ์',
    'อัสสัมชัญธนบุรี',
    'อนุบาลธีระนิตย์',
    'อนุบาลอนุบัณฑิต',
    'อนุบาลเด่นหล้า',
    'อนุบาลลำเพยพรรณ',
    'อนุบาลเศรษฐราณี',
    'กสิณธรวิทยา',
    'กองทัพบกอุปถัมภ์ไกรอำนวยวิทยา',
    'อนุบาลเยาวบุตร',
    'อนุบาลอรณิชชา',
    'อนุบาลสว่างวรรณ',
    'อนุบาลน้องหนู',
    'อนุบาลทอฝัน',
    'ชาลีสมุทร',
    'เมืองทองนคร',
    'อนุบาลดวงจิต',
    'ไผทอุดมศึกษา',
    'เจริญผลวิทยา',
    'อนุบาลฉัตรเฉลิม',
    'สารินันทน์ศึกษา',
    'อนุบาลเข็มทอง',
    'ระเบียบศึกษา',
    'ศิริวัฒน์วิทยา',
    'อนุบาลปิยะพัฒนา',
    'อนุบาลฤทธิยะวรรณาลัย',
    'อนุบาลปิยะวิทย์',
    'เอกอนุบาล',
    'ประเทืองทิพย์วิทยา',
    'อนุบาลอมรรัตน์',
    'อนุบาลบ้านอัมรินทร์',
    'อนุบาลมะลิวรรณวิทยา',
    'วรนาถวิทยา',
    'ทรงวิทย์ศึกษา',
    'อนุบาลสวนสยาม',
    'อนุบาลราชเดช',
    'ฉัตรวิทยา',
    'อนุบาลนิรมล',
    'อนุบาลทัศนีวรรณ',
    'อนุบาลเฟื่องฟ้า',
    'อนุบาลธิดารัก',
    'อนุบาลภวภูตานนท์',
    'อนุบาลวันทา',
    'อนุบาลยุคลธร',
    'นรรัตน์รังสฤษดิ์',
    'วิทยานนท์',
    'อนุบาลสัมมากร',
    'ศิริเพ็ญ',
    'โสมาภานุสสรณ์',
    'สองภาษาลาดพร้าว',
    'ถนอมพิศวิทยา',
    'บางกอกศึกษา',
    'เศรษฐบุตรอุปถัมภ์',
    'อุดมศึกษา',
    'กานดา',
    'ปฐมพัชร',
    'ศึกษากิจศึกษา',
    'แต่งตั้งวิทยา',
    'แสนสนุกไตรทักษะ',
    'อนุบาลราชพงษา',
    'อนุบาลจิตต์นัฐดา',
    'อนุบาลรัชนีบูล',
    'ศิริกุลพิทยา',
    'อนุบาลบ้านครู',
    'อนุบาลศรีวรางค์',
    'อนุบาลสมศกุลนา',
    'นีรชาศึกษา',
    'อนุบาลวรรณฤดี',
    'ขุมทองวิทยา',
    'โสมาภาพัฒนา',
    'มาคิดวิทยา',
    'สรรพาวุธวิทยา',
    'มเหสักข์พิทยา',
    'อนุบาลเจริญพงศ์',
    'ลาซาล',
    'อรรถวิทย์',
    'อนุบาลศรีศึกษา',
    'รุ่งเรืองวิทยา',
    'อนุบาลนวลทอง',
    'อนุบาลอุดมสุข',
    'สุทธิบงกช',
    'ประภามนตรี',
    'อนุบาลพัสวี',
    'เปรมสันต์',
    'โชติกาญจน์',
    'อนุบาลบ้านพลอยภูมิ',
    'อนุบาลจารุเวช',
    'อนุบาลสร้างสรรค์',
    'อนุบาลพลอยแก้ว',
    'บูรณะศึกษา',
    'อนุบาลปานตะวัน',
    'บูรณะศึกษา แผนกอนุบาลทุ่งครุ',
    'อนุบาลบ้านโดม',
    'สารสาสน์ประชาอุทิศพิทยาคาร',
    'อนุบาลบัณฑิตวิทย์',
    'อนุบาลทานตะวัน',
    'สิบสองวิทยา',
    'อนุบาลเสริมมิตรบางขุนเทียน',
    'สารสาสน์วิเทศบางบอน',
    'อนุบาลดุสิต',
    'เลิศหล้าถนนกาญจนาภิเษก',
    'ปัญญาศักดิ์บางบอน',
    'ดวงวิภา',
    'อนุบาลเอกชัย',
    'วิชัยวิทยา',
    'ดุสิตวิทยา',
    'ตั้งตรงจิตรพณิชยการ',
    'พณิชยการตั้งตรงจิตร',
    'พระนครพณิชยการ',
    'โยนออฟอาร์คบริหารธุรกิจ',
    'พณิชยการสุโขทัย',
    'วิมลพณิชยการ ศรีย่าน',
    'อาชีวศิลปศึกษา',
    'พณิชยการสามเสน',
    'กองทัพบกอุปถัมภ์ ช่างกล ขส.ทบ.',
    'อาชีวศิลป์สถาปัตย์กรุงเทพ',
    'ไทยบริหารธุรกิจและพณิชยการ',
    'วิบูลย์บริหารธุรกิจ รามอินทรา',
    'เทคโนโลยีบริหารธุรกิจ',
    'เทคโนโลยีบางกะปิ',
    'เทคโนโลยีช่างอุตสาหกรรมกรุงเทพ',
    'สหะพาณิชย์บริหารธุรกิจ',
    'ศรีวัฒนาบริหารธุรกิจและเทคโนโลยีนานาชาติ',
    'เทคโนโลยีกรุงเทพ',
    'พณิชยการมีนบุรี',
    'พณิชยการเอเซีย',
    'สาธุประดิษฐ์พณิชยการ',
    'สายประสิทธิ์พณิชยการ',
    'วิบูลย์บริหารธุรกิจ',
    'พณิชยการราชดำเนิน ธนบุรี',
    'พณิชยการราชดำเนิน',
    'เทคโนโลยีสยาม(ช่างกลสยาม)',
    'พณิชยการกรุงเทพ',
    'มิตรพลพณิชยการ',
    'กุลสิริเทคโนโลยีและบริหารธุรกิจ',
    'พณิชยการจรัลสนิทวงศ์',
    'จรัลสนิทวงศ์บริหารธุรกิจ',
    'กรุงเทพวิจิตรศิลป์และพณิชยการ',
    'วิทยาการจัดการเพชรเกษม',
    'วีรสุนทรบริหารธุรกิจ',
    'เทคโนโลยีหมู่บ้านครู',
    'สารสาสน์โปลีเทคนิค',
    'พณิชยการสยาม',
    'เทคนิคพระรามหก',
    'ไทยอาชีวศึกษา',
    'พระรามหกเทคโนโลยี',
    'พณิชยการบางอ้อ',
    'เทคนิควิทยา ปวส.',
    'เทคนิควิทยา',
    'พณิชยการจำนงค์',
    'เทคนิคพณิชยการจำนงค์',
    'พระนครบริหารธุรกิจ',
    'อัสสัมชัญพาณิชยการ',
    'สารสาสน์บริหารธุรกิจกนกอนุสรณ์',
    'กิตติบริหารธุรกิจ',
    'เทคนิควิมลบริหารธุรกิจ',
    'ผะดุงศิษย์พิทยาธุรกิจและพณิชยการ',
    'เทคนิคกรุงเทพพณิชยการ',
    'ไทยวิจิตรศิลป',
    'วราธิปบริหารธุรกิจ',
    'มหานครพณิชยการ',
    'สันติราษฎร์บริหารธุรกิจ',
    'เซนต์จอห์นโปลีเทคนิค',
    'เซนต์จอห์นเทคโนโลยี',
    'พณิชยการธัชรินทร์',
    'ช่างฝีมือปัญจวิทยา',
    'ไทยวิจิตรศิลปอาชีวะ',
    'เอกวิทย์อ่อนนุชบริหารธุรกิจ',
    'ศรีวิกรม์บริหารธุรกิจ',
    'เทคนิคพณิชยการเจ้าพระยา',
    'ดุสิตพณิชยการภาคบ่าย',
    'กรุงเทพการบัญชีวิทยาลัย',
    'ดุสิตพณิชยการ',
    'ดอนบอสโก',
    'สยามธุรกิจพณิชยการ',
    'เทคนิคบริหารธุรกิจกรุงเทพ',
    'เกษมโปลีเทคนิค',
    'เทคโนโลยีกรุงธน',
    'เทคนิคพณิชยการธนบุรี',
    'พณิชยการภาษานุสรณ์บางแค',
    'บุษยรัตน์บริหารธุรกิจ',
    'เทคนิครัตนโกสินทร์',
    'พณิชยการรัตนโกสินทร์',
    'สยามบริหารธุรกิจ',
    'เทคโนโลยีดุสิต',
    'ทักษิณาบริหารธุรกิจ',
    'มีนบุรีโปลีเทคนิค',
    'นิดาบริหารธุรกิจและวิศวกรรม',
    'อรรถวิทย์พณิชยการ',
    'เทคโนโลยีปิ่นมณฑล',
    'โพลีเทคนิคพณิชย์กรุงเทพ',
    'ปัญญาวุฒิกร',
    'สอนคนตาบอดกรุงเทพ',
    'กงลี้จงซัน',
    'สัมมาสิกขาสันติอโศก',
    'มูลนิธิสตรีไทยมุสลิม',
    'มูลนิธิวัดปากบ่อ',
    'ปัญญาวิทยา',
    'อนุบาลวัดธาตุทอง',
    'ราชวัตรวิทยา',
    'หนองจอกกงลิบฮัวเคียว',
    'ยุหมินพัฒนา',
    'คริสต์ธรรมวิทยา',
    'กว่างเจ้า',
    'พร้อมมิตรพิทยา',
    'เจริญศึกษา',
    'ประสาทวุฒิ',
    'ตรีวิทยา',
    'เผยอิง',
    'ผดุงกิจวิทยา',
    'จันทรวิชา',
    'ชาญเวทย์ศึกษา',
    'จิ้นเตอะ',
    'โกศลวิทยา',
    'พระโขนงวิทยา',
    'ซินหยกฮั้วกงฮัก',
    'คลองเตยวิทยา',
    'สิงฟ้า',
    'บางกอกวิทยา (มูลนิธิ)',
    'อิสลามสันติชน',
    'ดรุณพัฒน์',
    'อนุบาลเปล่งประสิทธิ์สายลม',
    'อนุบาลหงส์ทอง',
    'อนุบาลเด่นสยาม',
    'เทคโนโลยียานยนต์',
    'อนุบาลวณลักษณ์',
    'อนุบาลวลีรัตน์',
    'คุ้มเกล้า สุวินทวงศ์',
    'พรพิมพ์ พระราม 2',
    'อนุบาลโรจน์จิราภา',
    'สากลพัฒนา',
    'ศิริสุขวิทยา',
    'อนุบาลแสนสนุกไตรทักษะ บางมด',
    'อนันตา',
    'อนุบาลวัดไผ่เขียว',
    'อนุบาลบ้านวาดฝัน',
    'อนุบาลปษิตา',
    'อนุบาลกิตติ',
    'อนุบาลสิริกาญจน์รัต',
    'อัลกุรรออ์วิทยา (ประเวศ)',
    'สวนสันติ',
    'อนุบาลอนงค์นาฏ',
    'เจริญพัฒนาบริหารธุรกิจ',
    'อนุบาลภูภิงค์',
    'ปริยากร',
    'ปรัชชาธร',
    'ธุรกิจการอาหารไทยและนานาชาติ',
    'เพลินพัฒนา',
    'อนุบาลบัณฑิตวิทย์หนองแขม',
    'ปาณยา พัฒนาการ',
    'อนุบาลบ้านไทยวัฒนา',
    'อนุบาลปาลินา',
    'เอกบูรพา วิเทศศึกษา',
    'อนุบาลช้างเผือก',
    'อนุบาลแช่มเสริม',
    'ปัญจทรัพย์ มีนบุรี',
    'อนุบาลบ้านไมตรีจิต',
    'สารสาสน์วิเทศธนบุรี',
    'สารสาสน์วิเทศนิมิตใหม่',
    'ทินกรพิทยานุสรณ์',
    'เด็กสากลนิมิตใหม่',
    'อนุบาลบ้านหวาน',
    'ธนินทรวิทยา',
    'เลิศหล้า ถนนเกษตร-นวมินทร์',
    'ภูมิสมิทธ์',
    'อนุบาลช้างมงคล',
    'อนุบาลบวรโชติ',
    'อนุบาลกฤตยา',
    'บ้านของเล่น',
    'สารสาสน์วิเทศสายไหม',
    'อนุบาลทอฟ้า',
    'อนุบาลธรรมพิทักษ์',
    'อิกเราะสามัญศึกษา',
    'เทคโนโลยีธนบุรีบริหารธุรกิจ',
    'อนุบาลนิธิกานต์',
    'อนุบาลธนสมบูรณ์ดุสิต',
    'อนุบาลหทัยนิรมล',
    'อนุบาลกาญจนาภา',
    'อนุบาลศิวภรณ์',
    'ผ่องสุวรรณวิทยา สายไหม',
    'สาธิตพัฒนา',
    'ศศิภา',
    'อนุบาลสนุกคิด',
    'ปิยพัฒน์',
    'ปรีดาวิทยา',
    'ปัญโญทัย',
    'อนุบาลธนาทิพย์',
    'อนุบาลสิริวิทย์',
    'ราชานุชิต',
    'ศาสนวิทยา',
    'ศาสนบริหารธุรกิจ',
    'อันวารุ้ลอิสลาม',
    'สอนศาสนาศาสนูปถัมภ์',
    'สุมานัน',
    'สุขเจริญผลแพรกษา',
    'อนุบาลมิตรอุดม',
    'อนุบาลกิติมา',
    'อนุบาลบ้านหนู',
    'อนุบาลพัฒนาสุข',
    'อนุบาลจันทนา',
    'อนุบาลเทพารักษ์',
    'อนุบาลดีพร้อม',
    'อนุบาลจิตอาทร',
    'ซิกข์วิทยาลัย',
    'อู่ทิพย์',
    'อัสสัมชัญสมุทรปราการ',
    'อนุบาลบ้านไทรงาม',
    'อนุบาลเคหะบ้านใหม่',
    'อนุบาลตันติภักดิ์',
    'ล้วนพฤกษา',
    'อนุบาลสวนแพรกษา',
    'อนุบาลอนงค์เวท',
    'ดิษลี',
    'ทอรัก',
    'อนุบาลเทพดรุณ',
    'อนุบาลรักษ์ดรุณ',
    'ศึกษาสรรค์',
    'เฉลิมวิทยา',
    'ฉัตรสุดา',
    'ดรุณรัตน์',
    'เฉลิมไฉไลวิทยา',
    'ชวลิตวิทยา',
    'สิริศึกษา',
    'เซนต์ราฟาแอล',
    'ศรีวิทยาปากน้ำ',
    'จตุพรวิทยา',
    'เสงี่ยมพิทยานุกุล',
    'นพคุณวิทยา',
    'ทรงวิทยา',
    'อนุสาส์นวิทยา',
    'อนุบาลบุษยมาส',
    'มนตเสรี',
    'ประภามนตรี 2',
    'เซนต์โยเซฟ บางนา',
    'ปราณีเนาวบุตร',
    'บุรารักษ์',
    'อนุบาลสวนปากน้ำ',
    'อนุบาลนนนที',
    'อุ่นอารีวิทยา',
    'อนุบาลสุวิชา',
    'อนุสาสน์วิทยาสมุทรปราการ',
    'อนุบาลจตุรมุก',
    'เซนต์โยเซฟทิพวัล',
    'สุขเจริญผล',
    'สิริวุฒิวิทยา',
    'อนุบาลธิติพร',
    'อนุบาลเพชรงาม',
    'สามัคคีวิทยา',
    'สุดใจวิทยา',
    'อนุบาลภาสินี',
    'สาธิตบางนา',
    'อนุบาลนันทวรรณ (นันทวรรณทวิภาษ)',
    'อนุบาลอังสนา',
    'บางพลีพัฒนศึกษาลัย',
    'อนุบาลมงคลทิพย์',
    'ไพรีขยาด',
    'สันต์เสริมวิทย์',
    'ศรีดรุณ',
    'ทรงวิทยาเทพารักษ์',
    'กาญจนวิทยา',
    'ภรวิชญ์',
    'อนุบาลบุษยมาส ปู่เจ้าฯ',
    'สายอนุสรณ์',
    'ศิริวิทยา',
    'อนุบาลมารดา',
    'อนุบาลเซ็นดาณี',
    'อนุบาลเจริญวัย',
    'สันติดรุณ',
    'ปัญจนะวิทย์',
    'อนุบาลบัวขาว',
    'อนุบาลบ้านพุทธรักษา',
    'ราชประชาสมาสัย ในพระบรมราชูปถัมภ์',
    'สำราญวิทยา',
    'อาษาวิทยา',
    'ประชานาถ',
    'อำนวยวิทย์',
    'ณัฎฐเวศม์',
    'เยาวนารีคริสเตียน',
    'อนุบาลเทพกร',
    'สารสาสน์สมุทรสาร',
    'สารสาสน์วิเทศศึกษา',
    'เทพกรศึกษา',
    'นพคุณศึกษา',
    'ยินดีวิทย์',
    'อนุบาลเกวลิน',
    'อนุบาลวชิรา',
    'อนุบาลสวนดรุณ',
    'อนุบาลยินดีวิทย์',
    'อนุบาลพัตรียารม',
    'เกวลินวิทยา',
    'ชำนิเทคโนโลยี',
    'เทคโนโลยีเปรมฤทัย',
    'เกริกวิทยาลัย',
    'เทคโนโลยีบริหารธุรกิจสมุทรปราการ',
    'ไทย-ไต้หวันเทคโนโลยี',
    'ป้วยฮั้ว',
    'อนุบาลสาริน',
    'อนุบาลสีสวาท',
    'อนุบาลพนาสนธิ์',
    'อนุบาลทรายทอง',
    'อนุบาลอิศรานล',
    'อนุบาลผึ้งน้อย',
    'สาธิตคริสเตียนวิทยา',
    'อนุบาลเพื่อนเด็ก',
    'ศรีรัตโนภาศ',
    'อนุบาลยุวัจฉรา',
    'อนุบาลวัฒนผล',
    'อนุบาลนิตา',
    'อนุบาลจุฬสมา',
    'อนุบาลดวงใจ',
    'อนุบาลอัมพรรัตน์',
    'อนุบาลบรรลือทรัพย์',
    'อนุบาลมณียา',
    'อนุบาลสุรีย์ลักษณ์',
    'อนุบาลสันติวัน',
    'อนุบาลบ้านนนท์',
    'อนุบาลเรวดี นนทบุรี',
    'พลับพลาศิริวิทยา',
    'เขมพิทยา',
    'อนุบาลวัชราภา',
    'ธัมมสิริศึกษา',
    'การัญศึกษา',
    'นนทบุรีคริสเตียนวิทยา',
    'อุดมศึกษา',
    'รุ่งโรจน์ศึกษา',
    'เล็กโกเมศอนุสรณ์',
    'พิทักษ์นครานุสรณ์',
    'สันติวัน',
    'เทพประทานพร',
    'สมานพิชากร',
    'วรรัตน์ศึกษา นนทบุรี',
    'ประสาทวิทยานนทบุรี',
    'ศึกษาบัณฑิต',
    'อนุบาลมณีรัตน์',
    'รัตนบัณฑิตวิทยา',
    'สุรีย์ลักษณ์ 2',
    'ชาญรัตน์วิทยา',
    'อนุบาลกุลนันท์',
    'เปรมประชาวัฒนา',
    'พิศาลธรรมพาทีอนุสรณ์',
    'ดรุณวิทย์ศึกษา',
    'พระแม่สกลสงเคราะห์',
    'นันทนวรวิทย์',
    'สารสาสน์วิเทศบางบัวทอง',
    'เทพพิทักษ์',
    'มารีย์วิทยาบางบัวทอง',
    'อนุบาลสวนเด็กบางแพรก',
    'อนุบาลพิชาดา',
    'ศิริมงคลศึกษา',
    'ประถมพฤกษา',
    'สากลศึกษา บางบัวทอง',
    'อนุบาลบ้านพฤกษา',
    'ผกาภรณ์ศึกษา',
    'ปิยะฉัตร',
    'จิรดา',
    'กสิณธร',
    'เทพพิมานพิทยา',
    'อนุบาลจัตุพร',
    'บ้านรัตนาธิเบศร์',
    'ธนพร',
    'พึงรำลึก',
    'อนุบาลประคองสุข',
    'อนุบาลเลิศวินิต',
    'อนุบาลจุติพร',
    'ดารานุกูล',
    'จิตร์จำรูญวิทยา',
    'กุมุทมาส',
    'อนุบาลระพีพรรณ',
    'ฤกษบุตร์(สอิ้งอนุสรณ์)',
    'อัมพรไพศาล',
    'วรรณทิพย์วิทยา',
    'เซนต์ฟรังซีสเซเวียร์',
    'พระหฤทัยนนทบุรี',
    'พิชญศึกษา',
    'วัฒนพฤกษา',
    'ธรรมมิสลาม ท่าอิฐ',
    'ชลประทานวิทยา',
    'มูลนิธิปากเกร็ดวิทยา',
    'พงษ์สวัสดิ์พณิชยการ',
    'พณิชยการนนทบุรี',
    'ปัญญาภิวัฒน์เทคโนธุรกิจ',
    'พัฒนวิทย์บริหารธุรกิจ',
    'วานิชพณิชยการ',
    'ดุสิตพณิชยการนนทบุรี',
    'พณิชยการบางบัวทอง',
    'วัฒนพฤกษาบริหารธุรกิจ',
    'ศรีสังวาลย์',
    'บางบัวทองราษฎร์บำรุง',
    'ญามีอุ้ลควาน',
    'ท่าอิฐศึกษา',
    'อนุบาลธัญญาภรณ์',
    'ประชาบดี',
    'อนุบาลวัยใส',
    'อนุบาลโชตะนันท์',
    'อนุบาลอันวิดา',
    'เอเชียเอกตรา',
    'สยามบริหารธุรกิจ นนทบุรี',
    'วรนาถวิทยา',
    'เด่นหล้า พระราม 5',
    'รอบีตีบริหารธุรกิจ',
    'บ้านใหม่วัฒนา',
    'สารินันทน์',
    'บ้านพลอย',
    'อนุบาลอูนากูล',
    'อนุบาลเมืองเอก',
    'พิทักษ์นครินทร์',
    'อนุบาลชวนชื่น',
    'พระแม่มารีอุปถัมภ์',
    'เมืองเอกศึกษา',
    'อินทุสุตศึกษา',
    'อนุบาลดวงจิต 2',
    'อนุบาลดวงพร',
    'อุดมศึกษา รังสิต',
    'อนุบาลไทยธานี',
    'อนุบาลนวนคร',
    'ยิ่งยศอนุสรณ์',
    'ทวิวิชช์',
    'อนุบาลแห่งมหาวิทยาลัยธรรมศาสตร์',
    'อนุบาลรังสิต',
    'รังสฤษฎิ์สองภาษา',
    'อนุบาลธัญวิทยา',
    'รัตนโกสินทร์-รังสิต',
    'อนุบาลศิริเสนา',
    'แก้วสว่างวิทยา',
    'อุดมวิทยา',
    'ทิพพากรวิทยาการ',
    'อนุบาลธราดล',
    'อนุบาลฟ้าสิรินทร์',
    'วนิษา',
    'กานต์บดี',
    'บรรจบรักษ์',
    'อนุบาลรสสุคนธ์อากาศ',
    'พิทยนันท์ศึกษา',
    'อนุบาลบ้านหนูดี',
    'บริบูรณ์ศิลป์รังสิต',
    'อนุบาลปิยะรัตน์',
    'อนุบาลบ้านวังทอง',
    'สองภาษาวัชรพล',
    'ธัญวิทย์',
    'พุทธารักษ์',
    'กันตวรรณ',
    'ประชุมวิทยา',
    'สีวลี',
    'อนุบาลประสานมิตร',
    'เตรียมบัณฑิต',
    'อนุบาลประทีป',
    'เฟื่องฟ้าวิทยา',
    'อนุบาลฟ้าใส',
    'อนุบาลบ้านวนารักษ์',
    'อนุบาลลัดดาวรรณ',
    'แย้มสอาดรังสิต',
    'สวนอักษร',
    'ศรีจิตรา',
    'พระวิสุทธิวงศ์',
    'ศิริกานต์',
    'ผ่องสุวรรณวิทยา',
    'วงศ์ไพฑูรย์วิทยา',
    'พัฒนาวิทยา',
    'ไกรลาศศึกษา',
    'ระเบียบวิทยา',
    'อนุบาลวิภารัตน์',
    'เทคโนโลยีแหลมทอง',
    'เทคโนโลยีปทุมธานี',
    'กาญจนาภิเษกสมโภช ในพระราชูปถัมภ์ฯ',
    'มูลนิธิการศึกษาประชาเจริญ',
    'ประคองศิลป์',
    'รังสิตวิทยา',
    'มูลนิธิธัญวิทยา',
    'อนุบาลหมีน้อยแพนด้า',
    'อนุบาลผลิใบ',
    'จุฑารัตน์วิทยา',
    'สารสาสน์วิเทศรังสิต',
    'อนุบาลสุรัชนันท์',
    'อนุบาลปาริชาต',
    'อนุบาลนภสร',
    'สีวลี 2',
    'อนุบาลศุภสร',
    'สาธิตแห่งมหาวิทยาลัยรังสิต',
    'โชคชัยรังสิต',
    'อนุบาลทินโชติ',
    'อนุบาลเด่นปทุม',
    'อนุบาลนพวงศ์วิทยา',
    'คอมมูนิต้า อินคอนโทร',
    'อนุบาลปทุมวิทยา',
    'อิสลามศรีอยุธยามูลนิธิ',
    'ยอแซฟอยุธยา',
    'อรุณประเสริฐ',
    'จิระศาสตร์วิทยา',
    'วัดธรรมมิกราชวิทยา',
    'ศิริเสนาวิทยา',
    'สุนทรวิทยา',
    'แสงทวีปวิทยา',
    'แสนโกศิกนุสรณ์',
    'อนุบาลศูนย์ศิลปาชีพ บางไทรฯ',
    'ไตรอรุณศึกษา',
    'ไตรราชวิทยา',
    'เทพประสิทธิ์วิทยา',
    'เซนต์เทเรซาแสงทอง',
    'รอตเสวกวิทยา',
    'อนุบาลเปรมฤทัย',
    'อนุบาลเสนา',
    'อนุบาลผดุงเสนา',
    'เซนต์จอห์น บัปติสต์',
    'ราษฎร์บำรุงศิลป์',
    'มัธยมผดุงวิทยา',
    'บริหารธุรกิจอยุธยา',
    'เทคนิคพณิชยการอยุธยา',
    'เทคโนโลยีอยุธยา',
    'ผดุงอาชีวะเสนา',
    'ไทยอโยธยาบริหารธุรกิจ',
    'ประเสริฐวิทยาทาน',
    'อนุบาลบางปะอิน',
    'ธรรมิกราชวิทยา ๒',
    'ประชาศึกษา',
    'เสริมมิตรวิทยา',
    'ประสาทวิทย์',
    'อนุบาลสาธิตา',
    'อนุบาลในฝันพระนครศรีอยุธยา',
    'เซนต์แมรี',
    'อนุบาลรัตนนันท์',
    'อนุบาลบ้านดรุณ',
    'ปัณณวิชญ์',
    'เอกอโยธยา',
    'สนามชัยสิทธินุสรณ์',
    'อนุบาลวงค์สวัสดิ์วิทยา',
    'วรดิตถ์วิทยาประสูทน์',
    'วัดสระแก้ว(รุ่งโรจน์ธนกุลอุปถัมภ์)',
    'อนุบาลพูนสุข',
    'สนิทวิทยา',
    'อมราวิทยาภูมิ',
    'ราษฎร์นิยมวิทยา',
    'วิสัยทัศน์เทคโนโลยีอ่างทอง',
    'อนุบาลบรรจงรัตน์',
    'อัสสัมชัญคอนแวนต์ ลพบุรี',
    'กองทัพบกอุปถัมภ์ค่ายนารายณ์ศึกษา',
    'วินิตศึกษา ในพระราชูปถัมภ์ฯ',
    'กองทัพบกอุปถัมภ์สองเหล่าสร้าง',
    'บำรุงศึกษา',
    'โกษานุสรณ์',
    'บัณฑิตศึกษา',
    'พัฒนปัญญา',
    'อนุบาลเย็นศิระ',
    'วรกิจพิทยา',
    'จินดารัตน์',
    'เมืองละโว้วิทยา',
    'วรพิทยา',
    'บรรจงรัตน์',
    'วรนาถวิทยาลพบุรี',
    'กำจรวิทย์',
    'เพ็ญพัฒนา',
    'ทศธรรมศึกษา',
    'จารึกล้อมวิทยา',
    'ศรีแก้วอนุกูล',
    'นารายณ์วิทยา',
    'สามัคคีวิทยา',
    'อารีสวัสดิ์',
    'สัตยาไส',
    'อัสสัมชัญคอนแวนต์ลำนารายณ์',
    'รุ่งนิวัติวิทยา',
    'ดอนทองราษฎร์วิทยา',
    'รัตนศึกษา',
    'พระวรสาร',
    'สุเทพวิทยาลัย',
    'สร้างพัฒนา',
    'อนุบาลธรรมรักษ์',
    'ทองทาบพิทยา',
    'แก้วประทานพรวิทยา',
    'วิไลวิทยา',
    'ยงศ์สุรีศึกษา',
    'เอเชียบริหารธุรกิจเทคโนโลยีลพบุรี',
    'เทคนิคพณิชยการลพบุรี',
    'เทคโนโลยีละโว้',
    'โปลีเทคนิคลพบุรี',
    'บริหารธุรกิจละโว้',
    'เทคโนโลยีลพบุรี',
    'พณิชยการช่างเทคนิคลำนารายณ์',
    'การศึกษาคนตาบอดและคนตาบอดพิการซ้ำซ้อน',
    'แพทย์แผนไทยธรรมรักษ์',
    'งามมีศรีพัฒนา',
    'นารายณ์วิทยา ท่าหลวง',
    'แสงเทียน',
    'อินทโมลีประทาน',
    'โพธิรัตน์วรานุสรณ์',
    'วิจิตรศึกษา',
    'เจริญสอนวิทยา',
    'อุดมทรัพย์',
    'พระกุมารเยซู สิงห์บุรี',
    'นาคประดิษฐ์วิทยา',
    'ใจเพียรวิทยานุสรณ์',
    'อุดมศิลป์(โพธิลังการ์มูลนิธิ)',
    'สามัคคีวิทยา',
    'ปราสาทวิทยา',
    'บริหารธุรกิจสิงห์บุรี',
    'พณิชยการสิงห์บุรี',
    'ศรีอุดมวิทยา',
    'สิงห์อุดมวิทยา',
    'วรรณรัตน์วิทยา',
    'ตันติวัฒน์',
    'วัฒนาชัยนาท',
    'อักษรประสิทธิ์',
    'ถาวรวิทยา',
    'อนุบาลณัฐชา',
    'อนุบาลเกียรติคุณ',
    'อนุบาลกิตติกร',
    'พณิชยการชัยนาท',
    'อนุบาลสุธิชา',
    'บำรุงวิทยา',
    'อนุบาลกิตติกร',
    'อนุบาลจันทรอัมพร สระบุรี',
    'อนุบาลปิยะพร',
    'ราษฎร์ศึกษา',
    'กองทัพบกอุปถัมภ์ อดิศรศึกษา',
    'อนุบาลยุววิทยา',
    'ไพฑูรย์วิทยา',
    'แสงวิทยา',
    'อนุบาลวิชชากร',
    'กลั่นวิทยา',
    'บำรุงปัญญา',
    'มัธยมอนุกูลวิทยา',
    'วัฒนานุกูลวิทยา',
    'เกตุพิชัยวิทยา',
    'รอดณรงค์ศึกษา',
    'พีระวิทยา',
    'จำรัสวิทยา',
    'โปลีเทคนิคสระบุรี',
    'เทคโนโลยีชื่นชมไทย-เยอรมันสระบุรี',
    'อาชีวเกษตรสงเคราะห์สระบุรี',
    'เทคโนโลยีบริหารสากล (ไอ-แมท)',
    'พิฉาย "รวมมิตตศึกษา"',
    'จินเต๊อะแก่งคอย',
    'อนุบาลบุญเกื้อวิทยา',
    'พูลเจริญวิทยา',
    'อนุบาลสุทธิวรรณท์',
    'อนุบาลรัศมี',
    'นารานุบาล',
    'สุตา',
    'พงศ์สิริวิทยา',
    'เพชรพิทยาคม',
    'ประภัสสรวิทยา',
    'เจริญสุขวิทยา',
    'สถาพรพิทยา',
    'สามมุกคริสเตียนวิทยา',
    'อนุบาลสุนันทา',
    'อนุบาลบูรณิจฉ์',
    'อนุบาลภักดีพรรณ',
    'อนุบาลร่มไม้',
    'อนุบาลสมัครมิตรวัฒนา',
    'สุอารีวิทยา',
    'กล่อมปฐมวัย',
    'เมืองชลพิทยา',
    'กาญจนะวิทยา',
    'ธารทิพย์วิทยา',
    'สหะพาณิชย์',
    'ปรีชานุศาสน์',
    'อนุบาลรินทอง',
    'ศุทธรัตน์บ้านสวน',
    'อนุบาลสว่างวิทย์',
    'เมรี่อิมมาคุเลตคอนแวนต์',
    'เกษมวิทย์',
    'แสนสุขศึกษา',
    'สุขุมวิทยา',
    'เฉลิมศรี',
    'ศุทธรัตน์',
    'กุลศิริศาสน์',
    'บุญประทีปวิทยาคาร',
    'กุญแจคริสเตียนวิทยา',
    'สมคิดจิตต์วิทยา',
    'จิรพลวิทยา',
    'วัฒนดรุณวิทย์',
    'พิทยรังสี',
    'อนุบาลทนาพรวิทยา',
    'มารีวิทย์',
    'ตันตรารักษ์',
    'วุฒิโชติ',
    'พัทธยาอรุโณทัย',
    'ทนาพรวิทยา',
    'จุฬเทพ',
    'ศรีสุวิช',
    'อักษรพัทยา',
    'อนุบาลเพ็ญศิริ',
    'อนุบาลกาญจนา',
    'อนุบาลบุญพึ่ง',
    'บ้านรถไฟ',
    'อักษรศึกษา',
    'ผู้รู้ ญส.ส. 80',
    'บูรพาพัฒนศาสตร์',
    'สาธิตอุดมศึกษา',
    'สว่างบริบูรณ์วิทยา',
    'อนุบาลมารีวิทย์',
    'อักษรเทพประสิทธิ์',
    'อนุบาลประชาสงเคราะห์',
    'เพลินจิตวิทยา',
    'ประชาสงเคราะห์',
    'อนุบาลพรประสงค์',
    'วัฒนานุศาสน์',
    'อนุบาลบุญญโชติ',
    'เฉลิมชัยศึกษา',
    'บุญจิตวิทยา',
    'บรรณพันธ์ศึกษา',
    'ดาราสมุทร',
    'รัตนชัยศึกษา',
    'ศรีราชาเอกชน',
    'อนุบาลวรางค์ลักษณ์',
    'มัธยมสัมมาชีวศิลปบางพระ',
    'อนุบาลประเสริฐสุข',
    'อนุบาลนิสารัตน์',
    'บางพระวิทยา',
    'อัสสัมชัญศรีราชา',
    'ดวงมณี',
    'เซนต์ปอลคอนแวนต์',
    'อนุบาลศรีมณี',
    'อนุบาลภาษาศาสตร์',
    'มารีวิทย์สัตหีบ',
    'ณัฏฐเวศม์',
    'บำรุงศิษย์ศึกษา',
    'จุฬเทพบางเสร่',
    'อนุบาลอธิฎฐาน',
    'นาวิกโยธินบูรณะ',
    'สัตหีบ',
    'ธัมมสิริศึกษาสัตหีบ',
    'ราษฎร์ประดิษฐ์วิทยา',
    'ศรีมณีวิทยา',
    'นันทนวิทย์',
    'ดาราสมุทรบริหารธุรกิจ',
    'ชลพินิจพณิชยการ',
    'เทคนิคบริหารธุรกิจกรุงเทพชลบุรี',
    'ไทยเบญจบริหารธุรกิจชลบุรี',
    'เมืองชลพณิชยการ',
    'เทคโนโลยีชลบุรี',
    'ชลบุรีบริหารธุรกิจและเทคโนโลยี',
    'วิศวกรรมเทคโนโลยีบริหารธุรกิจ',
    'พณิชยการชลบุรี',
    'พัทยาบริหารธุรกิจ',
    'อักษรเทคโนโลยีพัทยา',
    'ไทยอิโตะเทคโนโลยี (พัทยา)',
    'พัทยาพณิชยการ',
    'เทคโนโลยีภาคตะวันออก (อีเทค)',
    'วิศวกรรมแหลมฉบัง',
    'เทคโนโลยีแหลมฉบัง',
    'เทคโนโลยีศรีราชา',
    'เทคนิคพณิชยการสัตหีบ',
    'จอมเทียนบริหารธุรกิจ',
    'สอนคนตาบอดพระมหาไถ่พัทยา',
    'อนุบาลโสตพัฒนา',
    'ธรรมวาที',
    'วุฒิวิทยา',
    'บูรพาวิทยา ชลบุรี',
    'เฮงฮั้ว',
    'จิ้นฮั้ว',
    'บุญญวิทยาคาร',
    'อนุบาลบ้านต้นกล้า',
    'อนุบาลสุเฌอ',
    'อนุบาลผกาทิพย์',
    'ชลบุรีเรืองกิตติ์พณิชยการ',
    'อนุบาลฟีนิกซ์',
    'อนุบาลบ้านทรายทอง',
    'อนุบาลศรีอุดมศุขพิทยา',
    'บางละมุงอินเตอร์ - เทค',
    'อนุบาลเรณู เนสเซอร์รี่',
    'อัสสัมชัญระยอง',
    'ตรีวิทย์ศึกษา',
    'อนุบาลภักดีพรรณระยอง',
    'อนุบาลศรอารีย์',
    'อนุบาลปิยะอาทร',
    'มณีวรรณวิทยา',
    'เซนต์โยเซฟระยอง',
    'ศิริพร',
    'บวรรัตนศาสตร์',
    'อนันต์ศึกษา',
    'สมคิดวิทยา',
    'อารีย์วัฒนา',
    'วุฒินันท์',
    'พัฒนเวชศึกษา',
    'สัจจศึกษา',
    'อุดมวิทยานุกูล',
    'อนุบาลชุมชนพัฒนา',
    'รุ่งนภาพิทยา',
    'สายสัมพันธ์',
    'อนุบาลหลานรัก',
    'จินดาวัฒน์',
    'อนุบาลเวชประสิทธิ์',
    'อนุบาลร่มโพธิ์',
    'อรวินวิทยา',
    'ระยองพาณิชยการ',
    'เทคโนโลยีทีพีไอ',
    'โปลีเทคนิคระยอง',
    'พัฒนเวชบริหารธุรกิจ',
    'เทคโนโลยีระยองเฉลิมพระเกียรติ',
    'กวงฮั้ว',
    'อนุบาลรักภาษา',
    'สองภาษาระยอง',
    'วิบูลวิทยา',
    'อนุบาลฝนทิพย์',
    'อนุบาลทิพวัลย์',
    'นานาชาติสวนระยอง',
    'ลาซาลจันทบุรี (มารดาพิทักษ์)',
    'บุญสมอนุบาล',
    'อนุบาลกัลยา',
    'อนุบาลประศาสน์วิทยา',
    'อนุบาลมารีนิรมล',
    'บุญสมวิทยา',
    'สตรีมารดาพิทักษ์',
    'อำนวยวิทย์พัฒน์',
    'อำนวยวิทย์',
    'ศรีหฤทัย',
    'อำนวยศึกษา',
    'ประดิษฐ์ศึกษา',
    'ยอแซฟวิทยา',
    'สุดแสวงวิทยา',
    'ปะตงวิทยา',
    'ไทยโปลีเทคนิคจันทบุรี',
    'วิศวกรรมบริหารธุรกิจภาคตะวันออก',
    'ตังเอ็ง',
    'ตากสินศึกษา',
    'เตรียมศึกษาสอยดาว',
    'เตรียมศึกษานายายอาม',
    'ลัมแบรต์พิชญาลัย',
    'วิริยาลัยโป่งน้ำร้อน',
    'ตากสินทิพย์เทคโนโลยี จันทบุรี',
    'กิตติวิทยา',
    'สุนันทาวิทยา',
    'มารดานุสรณ์',
    'นคราศึกษา',
    'อนุบาลสว่างไสว',
    'อนุบาลจุติพร',
    'ไสวนันทวิทย์',
    'ปิยบุตร',
    'สันติภาพ',
    'ดาราสมุทร',
    'เซนต์แอนโทนี',
    'เซนต์หลุยส์',
    'เทพประสิทธิ์วิทยา',
    'ศรีวรการ',
    'ปัญจพิทยาคาร"สนั่นพิชิตกุลอนุสรณ์"',
    'ดาราจรัส',
    'สุตะบำรุงพิทยาคาร',
    'วัฒนาลัย',
    'ศรีวิทยา',
    'อนุบาลขวัญตา',
    'มารดานฤมล',
    'อนุบาลจันทรา',
    'จุฑาทิพย์',
    'อนันต์วิทยา',
    'พนมอิทธิศึกษา',
    'ธีราวิทยา',
    'อนุบาลศรีวรการแปลงยาว',
    'เทคโนโลยีศรีวรการ',
    'เทคโนโลยีฉะเชิงเทรา',
    'เทคโนโลยียานยนต์โตโยต้า',
    'พณิชยการฉะเชิงเทรา',
    'บางปะกงบริหารธุรกิจ',
    'สมาคมสงเคราะห์วิทยา',
    'มัธยมพระราชทานบ้านนายาว',
    'รุ่งโรจน์วิทยา',
    'บางน้ำเปรี้ยวกงลิ',
    'อิสลามวิทยาบึงน้ำรักษ์',
    'สุธีวรราษฎร์รังสฤษฎ์',
    'โชติพิทยากร',
    'มารีวิทยา',
    'ฉัตรลิตราศึกษา',
    'หทัยชาติ',
    'กองทัพบกอุปถัมภ์จักรพงษ์พิทยาลัย',
    'มารีวิทยากบินทร์บุรี',
    'กบินทร์จริยาคม',
    'สายมิตรศึกษา',
    'กชนันท์อนุบาล',
    'รักษาทรัพย์พิทยากร',
    'อนุบาลจรันวิทยา',
    'อนุบาลอุดมวิทย์',
    'ผะดุงศิษย์พิทยาประจันตคาม',
    'มัธยมวัดใหม่กรงทอง',
    'มารีวิทยาศรีมโหสถ',
    'เทวรักษ์',
    'พณิชยการปราจีนบุรี',
    'สายมิตรเทคโนโลยี',
    'เทคนิคพณิชยการกบินทร์บุรี',
    'เทคโนโลยีนครกบินทร์',
    'จงหมิน',
    'เจียหมิน',
    'รักษาทรัพย์พิทยากร (นาดี)',
    'นายกวัฒนากรวัดพราหมณี',
    'อนุบาลคุณากร',
    'อนุบาลจุฬาทิพพ์',
    'นายกวัฒนากรวัดอุดมธานี',
    'ถาวรนิมิต',
    'นายกวัฒนากร วัดเกาะหวาย',
    'นักบุญยอแซฟอุปถัมภ์ หนองรี',
    'นายกวัฒนากรบ้านนา',
    'มาลาสวรรค์พิทยา',
    'สุวรรณประสิทธิ์',
    'อนุบาลคริสตสงเคราะห์',
    'คริสตสงเคราะห์',
    'เทคโนโลยีวีรพัฒน์',
    'เทคโนโลยีพณิชยการนครนายก',
    'เหลียนหัว',
    'ผดุงอิสลาม',
    'ดาราสมุทร สระแก้ว',
    'อนุบาลบ้านจันทร์',
    'อนุบาลจิตละมุน',
    'ชุมชนพัฒนา',
    'สุภวิทย์',
    'อนุบาลวัฒนานคร',
    'พวงคราม',
    'พรพงษ์กุล',
    'สำเร็จวิทยา',
    'เทคโนโลยีสระแก้ว',
    'ศรีตาพระยา',
    'ไฮเทค-เทคโนโลยี',
    'อนุบาลบ้านคุณหนู',
    'อนุบาลดวงแขวิทยา',
    'อนุบาลบพิตร',
    'อนุบาลพร้อมวิทย์',
    'อนุบาลทิพย์ปัญญา',
    'อนุบาลกองบัญชาการช่วยรบที่ 2',
    'อนุบาลชัยศิริ',
    'อนุบาลบวรนครราชสีมา',
    'อนุบาล ก ไก่',
    'อนุบาลวัดสุทธจินดา',
    'อนุบาลวัดสะแก',
    'มารีรักษ์',
    'รวมมิตรวิทยา',
    'โคราชวิทยา',
    'อนุบาลวัดทุ่งสว่าง',
    'มารีย์วิทยา',
    'ไชยมนัสศึกษา',
    'อนุบาลหลานย่าโม',
    'สายมิตรจอหอ 8',
    'สายทิพย์อนุบาล',
    'สุรนารายณ์วิทยา',
    'อัสสัมชัญนครราชสีมา',
    'อนุบาลเยาวนารถ',
    'อุบลรัตน์',
    'อนุบาลอินทรา',
    'เกียรติคุณวิทยา',
    'สมบูรณ์วิทยานุกูล',
    'อนุบาลค่ายสุรนารี',
    'อนุบาลแสนยวิทยาภูมิ',
    'โพธิ์พรรณวิทยา',
    'เหรียญทองวิทยา',
    'แสนยวิทยาภูมิ',
    'อนุบาลสกุลจิตร',
    'อนุบาลมีมั่งคั่ง',
    'อนุบาลสมปอง',
    'สายมิตรโชคชัย 3',
    'อนุบาลเอราวัณ',
    'อนุบาลแสนสนุก',
    'อนุบาลบัณฑิตน้อย',
    'คมพิสิฐวิทยา',
    'พงษ์ศิริวิทยา',
    'สายมิตรโนนไทย 6',
    'พิทักษ์ภูเบนทร์ 7',
    'พิทักษ์ภูเบนทร์',
    'อนุบาลจรัสพงษ์',
    'พิทักษ์ภูเบนทร์ 2',
    'มัธยมพุทธเกษตรวัดโนนเมือง',
    'บัวใหญ่วิทยา',
    'อมรศิลป์',
    'ดงเค็งศึกษา',
    'อนุบาลจินดา',
    'อุดมวิทยานุสรณ์',
    'อนุศาสน์วิทยา',
    'มารีย์ธงชัย',
    'สายมิตรปักธงชัย 7',
    'อนุบาลแก้วตา',
    'อนุบาลบำรุงการ',
    'พิทักษ์ภูเบนทร์ 3',
    'อนุบาลพิมาย',
    'เทเรซาอุปถัมภ์',
    'อนุบาลอนันต',
    'สุนันทาวิทยา',
    'พิสุทธิ์วิทยาคม 1',
    'อนุบาลจุรีพันธ์',
    'คูณวรวิชญ์',
    'มงคลกุลวิทยา',
    'เซนต์โยเซฟศึกษา',
    'ประสารวิทยา',
    'สกุลศึกษา',
    'บุญเจริญวิทยา',
    'กีรติศึกษา',
    'อนุบาลธารทิพย์',
    'วัฒนวิทยา',
    'ถนอมศิษย์วิทยา',
    'สายสัมพันธ์',
    'ประชานุสรณ์',
    'แสงสุริยาวิทยา',
    'รุ่งอรุณวิทยา',
    'เรืองศรีวิทยา',
    'สัญลักษณ์วิทยา',
    'พฤกษาวิทยา',
    'ณัฐญาศึกษา',
    'สายมิตรหนองบุญมาก 11',
    'อนุบาลณัฐญา',
    'อนุบาลแพรวไพลิน',
    'สายมิตรวังน้ำเขียว 5',
    'เทคโนโลยีอาชีวศึกษา',
    'เทคโนโลยีอาชีวศึกษานครราชสีมา',
    'เทคโนโลยีชนะพลขันธ์ นครราชสีมา',
    'อัสสัมชัญพาณิชยการนครราชสีมา',
    'เมรี่เทคโนโลยี',
    'มารีย์บริหารธุรกิจ',
    'เทคโนโลยีช่างกลพณิชยการนครราชสีมา',
    'บัวใหญ่เทคโนโลยีพณิชยการ',
    'กุสุมภ์เทคโนโลยี',
    'พณิชยการปากช่อง',
    'สายมิตรเทคโนโคราช 10',
    'การศึกษาคนตาบอดนครราชสีมา',
    'มูลนิธิรุ่งนภานครราชสีมา',
    'เก้งเต็ก',
    'มารดาดรุณีรักษ์',
    'จุ้ยฮั้ววิทยา',
    'วานิชวิทยา',
    'เทคโนโลยีบริหารธุรกิจรักไทย คง',
    'เจ้าคุณปริยัติมุนีวิทยา',
    'อนุบาลประภารัตน์',
    'กมลวิชญ์ศึกษา',
    'อนุบาลลูกขวัญ',
    'อนุบาลอัจฉรา',
    'อนุบาลบ้านผู้การ',
    'ปลูกปัญญา',
    'อนุบาลธีรา',
    'อนุบาลเตรียมพัฒน์',
    'มารีย์อนุสรณ์',
    'มารีย์พิทักษ์',
    'กมลลักษณ์',
    'นิภาศิริ',
    'อนุบาลกัลยาประสิทธิ์',
    'เอื้อแววมณี',
    'ลำปลายมาศพัฒนา',
    'อนุบาลทศพร',
    'อนุบาลสายสัมพันธ์',
    'มารดาวนารักษ์',
    'เบญจเทคโน',
    'รัตนเทคโนโลยี',
    'รุ่งนภาอนุสรณ์ศาสตร์',
    'มูลนิธิรุ่งนภา',
    'ฮั่วเคี้ยว',
    'บำรุงวิทยา',
    'อนุบาลชนิดา',
    'นอร์ทอิสเทิร์นวิทยาลัย',
    'นอร์ทอิสเทิร์นบริหาธุรกิจ',
    'เทคโนโลยีบริหารธุรกิจรักไทย ละหานทราย',
    'เทคโนโลยีบริหารธุรกิจรักไทย พุทไธสง',
    'อนุบาลแสนสุข',
    'อนุบาลรัตนะ',
    'อนุบาลศรทิพย์',
    'ทศพรวิทยา',
    'อนุบาลทรัพย์ปัญญา',
    'วรเวทวิทยา',
    'เทคโนโลยีรักไทย',
    'อนุบาลภรภัทร',
    'บุรีรัมย์บริหารธุรกิจ',
    'อิสาณโกศลศึกษา',
    'อนุบาลรัตนศึกษา',
    'พระกุมารสุรินทร์',
    'สุรินทรศึกษา',
    'อนุบาลทองอุ่น',
    'เบญจเทคโน สุรินทร์',
    'เทคโนโลยีสุรินทรศึกษา',
    'วาณิชย์นุกูล',
    'แงงกวง',
    'อนุบาลกมลนิตย์',
    'สุวรรณวิจิตรวิทยา',
    'มานิตวิทยาคม',
    'วัดมหาพุทธาราม',
    'วัดหลวงวิทยา',
    'มารีย์วิทยา',
    'มารดาทรงธรรม',
    'อรจันทร์วิทยา',
    'มารีย์อุปถัมภ์',
    'ขุขันธ์ราษฎร์บำรุง',
    'วัดไพรบึงวิทยา',
    'อุทุมพรวิทยา',
    'ศรีเจริญเทพมงคล',
    'ศรีสะเกษบริหารธุรกิจและบริหารธุรกิจ',
    'เทคโนโลยีศรีสะเกษ',
    'เทคโนโลยีศรีสะเกษเฉลิมหล้า',
    'รวมสินวิทยา',
    'สัมมาสิกขาศรีษะอโศก',
    'เคียวนำ',
    'การกุศลวัดบ้านโดด',
    'อาชีวศึกษาจุลมณีกันทรารมย์',
    'เกศเกล้าวิทยา',
    'เทคโนโลยีพณิชยการราษีไศล',
    'สว่างคูณวิทยา',
    'อาชีวศึกษาจุลมณีอุทุมพรพิสัย',
    'อนุบาลน้ำเพชร',
    'เทคโนโลยีบริหารธุรกิจรักไทย ขุขันธ์',
    'วัดโพธิ์น้อย',
    'วัดจันทาราม',
    'ศรีสะเกษบริหารธุรกิจกันทรลักษ์',
    'สัมมาอาชีวสิกขาศรีษะอโศก',
    'อนุบาลเทพปทุมทิพย์',
    'ศรีทองวิไลวิทยา',
    'วัดไร่น้อย',
    'อนุบาลน้ำทิพย์ 2',
    'อนุบาลน้ำทิพย์',
    'อนุบาลกิตติคุณ',
    'แอ๊ดเวนตีสอุบล',
    'อนุบาลบ้านเด็ก',
    'เยาวเรศศึกษา',
    'อเนกวิทยา',
    'มารีย์นิรมล',
    'มหาชนะวิทยา',
    'อัสสัมชัญอุบลราชธานี',
    'พระกุมารอุบล',
    'อุบลวิทยากร',
    'อาเวมารีอา',
    'สมเด็จ',
    'อนุบาลจงกลนี',
    'อนุบาลตระการทิพย์',
    'ศักดาวิทยา',
    'อนุบาลพรรณนิดาทิพย์',
    'วัดเวตวันวิทยาราม',
    'อนุบาลวรรณวิไล',
    'วรรณวิไล',
    'อนุบาลเปรมชนก',
    'อนุบาลเดชอุดม',
    'อนุบาลเพชรบุปผา',
    'อนุบาลน้องหญิง',
    'ปุญญสารวัดศรีโพธิ์ชัย',
    'บ้านพิณโท',
    'เบญจธัญพิทยา',
    'อนุบาลเกษมวันวิทยา',
    'พิมพ์ประภาสงเคราะห์',
    'เทพพิทักษ์พิทยา',
    'สงเคราะห์ศึกษา',
    'อนุบาลงามจิต',
    'บ้านเด็กวารินชำราบ',
    'ประสานวิทยาคม',
    'สัมมาสิกขาราชธานีอโศก',
    'มัธยมพิบูลวิทยาลัย',
    'ดำรงวิทยา',
    'โปลีเทคนิคภาคตะวันออกเฉียงเหนือ',
    'เทคโนโลยีอาชีวศึกษาอุบลราชธานี',
    'เทคโนโลยีราชพฤกษ์',
    'นิคม 2 อนุบาล',
    'ฮั้วเฉียวกงฮัก อุบลฯ',
    'อนุบาลอัมพรบัวขาว',
    'ยุวทูตศึกษา 2',
    'ฮั้วเฉียวอุบลราชธานี 2',
    'อนุบาลพุทธเมตตา',
    'ทวีวัฒน์ศึกษา',
    'นันตาศึกษา',
    'พรเมตตาคริสเตียน',
    'พุทธศาสตร์ศึกษาบ้านดอนโด่',
    'วัดสวายน้อยวิทยา',
    'เทคโนโลยีบริหารธุรกิจรักไทย สาขาบุณฑริก',
    'เทคโนโลยีบริหารธุรกิจรักไทย เขมราฐ',
    'เทคโนโลยีบริหารธุรกิจรักไทย น้ำยืน',
    'เทคโนโลยีบริหารธุรกิจรักไทย สิรินธร',
    'อนุบาลศูนย์พลาญข่อย',
    'เวฬุวันวิทยา',
    'ศูนย์พลาญข่อย 2 บ้านด่านเม่น',
    'ปลาขาววิทยานุสรณ์',
    'วัดนาเยียวิทยา',
    'ประสิทธิ์ศึกษาสงเคราะห์',
    'วัดคูเดื่อวิทยาคม',
    'อนุบาลลูกรัก',
    'อนุบาลแสงสุรีย์',
    'อนุบาลวรัญญู',
    'อนุบาลอินทร์-โฮม',
    'ศรีธรรมวิทยา',
    'สันติธรรมวิทยาคม',
    'ยโสธรวิทยาคาร',
    'มูลนิธิรุ่งนภายโสธร',
    'อนุบาลการกุศลวัดประชาชุมพล',
    'อนุบาลแก้วปัญญาอุปถัมภ์',
    'อนุบาลแก้วปัญญาอุปถัมภ์ มหาชนะชัย',
    'อนุบาลบ้านเด็กยศวดี',
    'มารีย์นุเคราะห์',
    'ซ่งแย้ทิพยา',
    'ยโสธรพณิชยการเทคโนโลยี',
    'สาธิตอนุบาลพรเพชร',
    'ยโสธรบริหารธุรกิจ',
    'พรเพชรบริหารธุรกิจภาคตะวันออกเฉียงเหนือ',
    'เทคโนโลยีบริหารธุรกิจกุดชุม',
    'เทคโนโลยีบริหารธุรกิจคำเขื่อนแก้ว',
    'ไพเราะวิทยา(อนุบาล)',
    'อนุบาลบ้านเด็กดี',
    'มารีย์อุปถัมภ์ชัยภูมิ',
    'สาคริชวิทยา',
    'อนุบาลศักดิ์ปทุม',
    'สวนวิทยา',
    'อนุบาลบ้านสิทธิวงศ์',
    'อนุบาลเรืองชาญ',
    'อนุบาลหนองบัวแดง',
    'อนุบาลสรรพกิจโกศล',
    'ศรีเทพบาล',
    'มะนะศึกษา',
    'อนุบาลไพโรจน์วิทยา',
    'มงคลศึกษา',
    'อนุบาลรัตโนดม',
    'อนุบาลบ้านทับทิม',
    'อนุบาลอโนมา',
    'วรนุชวิทยา',
    'พุฒิชัยวิทยา',
    'สัมมาสิกขาหินผาฟ้าน้ำ',
    'อนุบาลสำราญยงค์',
    'อนุบาลณัฐพันธ์ 1',
    'ชัยภูมิบริหารธุรกิจ',
    'เทคโนโลยีบริหารธุรกิจศิขรินทร์',
    'เทคโนโลยีภูเขียวบริหารธุรกิจ',
    'อนุบาลบ้านรักเด็ก',
    'อนุบาลหลวงภักดี',
    'อนุบาลโชคนวกุล(เกษตรสมบูรณ์)',
    'คอนสารเทคโนโลยี',
    'อนุบาลบ้านเด็กดี 2',
    'อนุบาลนพเก้า',
    'เจริญวิทยา',
    'มูลนิธิสุดีวิทยาวัดไชยาติการาม',
    'วิสุทธิวงศ์',
    'โปลีเทคอำนาจเจริญ',
    'เทคนิคบริหารธุรกิจอำนาจเจริญ',
    'เอกวรรณพณิชยการอำนาจเจริญ',
    'เต็มฤทัยศึกษา',
    'อนุบาลธันยธรณ์',
    'อนุบาลสมบูรณ์วิทย์',
    'อนุบาลหนองบัวลำภูคริสเตียน',
    'พิชญบัณฑิต',
    'อนุบาลกาบแก้วบัวบาน',
    'อนุบาลสุดา',
    'จริยานุสรณ์',
    'เฉลิมจักรศึกษา',
    'เทคโนโลยีพิชญบัณฑิต',
    'อนุบาลภูบดินทร์',
    'เทคโนโลยีบริหารธุรกิจรักไทย',
    'อนุบาลอินนัดดา',
    'ขอนแก่นคริสเตียน',
    'ขอนแก่นวิเทศศึกษา',
    'กัลยาณีวิทยา',
    'อนุบาลสามพี่น้อง',
    'อนุบาลณัฐสุดา',
    'อนุบาลสาธิตพงษ์ภิญโญ',
    'อนุบาลดวงดาว',
    'ญาณไพศาลวิทยา',
    'พัฒนาเด็ก',
    'อนุบาลเบญจพร',
    'วรรณรัตน์ศึกษา',
    'พระกุมารเยซูวิทยาขอนแก่น',
    'ไพบูลย์วิทยา',
    'มหาไถ่ศึกษาภาคตะวีนออกเฉียงเหนือ',
    'พิมพ์ใจวิทย์',
    'มหาไถ่ศึกษาบ้านน้อยสามเหลี่ยม',
    'อนุบาลบุญพร้อง',
    'การกุศลวัดหนองแวง',
    'อนุบาลค่ายศรีพัชรินทร',
    'อนุบาลปิ่นทิพย์',
    'อนุบาลวรานุช',
    'อนุบาลราตรี',
    'อนุบาลอุปบุตร',
    'อนุบาลเพชรรัตน์',
    'อนุบาลสวนสน',
    'อนุบาลเคหะชุมชนขอนแก่น',
    'อนุบาลภาคตะวันออกเฉียงเหนือ',
    'อนุบาลเพียงนภา',
    'อนุบาลพิมานเด็ก',
    'อนุบาลกุลศิริ',
    'อนุบาลรัตนมาลี',
    'อนุบาลแก้วกรุณา',
    'มหาไถ่ศึกษาขอนแก่น',
    'มณีอนุสรณ์ศึกษา',
    'สุรัสวดี',
    'อนุบาลพรทิพย์',
    'อนุบาลราษฎร์อุทิศศึกษา',
    'อนุบาลเรือนแก้ว',
    'อนุบาลโชคมณี',
    'การกุศลวัดร่มประดู่',
    'อนุบาลอภิรดี',
    'อนุบาลนันทาพร',
    'อนุบาลสุดรัก',
    'ชุมแพวิทยา',
    'อนุบาลชุมแพ',
    'อนุบาลณัฐพันธุ์ 2',
    'อนุบาลเดือนฉาย',
    'อนุบาลบ้านรักเรียน',
    'อนุบาลอินทร',
    'อนุบาลศุภมน',
    'อนุบาลบ้านสวน',
    'อนุบาลราษฎร์อำนวยศิลป์',
    'มนตรีศึกษา',
    'อนุบาลพูนสวัสดิ์',
    'อนุบาลขวัญรัตน์',
    'คงฤทธิ์ศึกษาการ',
    'อนุบาลศิวิไลย์',
    'พัฒนาการศึกษา',
    'อนุบาลนวลน้อย',
    'สุดารัตน์วิทยาคม',
    'ประถมปรีดาภรณ์',
    'นวลน้อยพิทยา',
    'กรุณาศึกษา',
    'เอี่ยมไพศาล',
    'มูลนิธิบ้านไผ่วิทยา',
    'อนุบาลมหาไถ่ศึกษาบ้านไผ่',
    'มหาไถ่ศึกษาบ้านไผ่',
    'กระแสพัฒนา',
    'อนุบาลไพศาลวิทย์',
    'มหาไถ่ศึกษาเมืองพล',
    'อำไพพิทยา',
    'อมตวิทยา',
    'อนุบาลนันทพร 2',
    'อนุบาลพัฒนาเด็กศรีวิสุทธิ์',
    'พิชาภรณ์',
    'อนุบาลมัญจาวิทยา',
    'อนุบาลมัญจาคริสเตียน',
    'อนุบาลศักดิ์สุภา',
    'อนุบาลทันวิทยา',
    'กฤชานันท์เขาสวนกวาง',
    'อนุบาลโชคนวกุล (ภูผาม่าน)',
    'อนุบาลอรุณโรจน์',
    'อนุบาลธิดารัตน์',
    'มหาไถ่ศึกษาโนนสมบูรณ์',
    'อนุบาลสุรีพร',
    'เทคโนโลยีธุรกิจอาชีวะ',
    'พณิชยการขอนแก่น',
    'ธุรกิจอาชีวะ',
    'ขอนแก่นเทคโนโลยีพณิชยการ',
    'เทคโนโลยีภาคตะวันออกเฉียงเหนือ',
    'เทคโนโลยีขอนแก่น',
    'ขอนแก่นบริหารธุรกิจ',
    'พงษ์ภิญโญเทคโนโลยี',
    'ชุมแพบริหารธุรกิจเทคโนโลยี',
    'เทคโนโลยีพลพณิชยการ',
    'อมตวิทยา',
    'การศึกษาคนตาบอด ขอนแก่น',
    'การศึกษาพิเศษมารีย์นิรมล',
    'ฮั่วเคี้ยววิทยาลัย',
    'ซู่เอ็ง',
    'เต็กก่าดรุณธรรม',
    'อนุบาลจิรภา',
    'อนุบาลเพชรลดา',
    'วัดศรีจันทร์',
    'อนุบาลศรีซำสูง',
    'อนุบาลตั้งใจ',
    'อนุบาลวรวิมล',
    'อินทรวิทยา',
    'อนุบาลถนอมจิตร',
    'อนุบาลบุตรธิดา',
    'อนุบาลกุลฤดีแวงน้อย',
    'อนุบาลสมสวัสดิ์',
    'อนุบาลลดาวัลย์',
    'ไพบูลย์วิทยาเต่านอ',
    'การกุศลวัดโพธิ์ทอง',
    'อนุบาลพรชิดา',
    'เอี่ยมอินทร',
    'อนุบาลลูกหลวง',
    'อนุบาลเพื่อนเด็ก',
    'สหกิจวิทยา',
    'พันธกิจวิทยา',
    'อุดรคริสเตียนวิทยา',
    'อินทร์อุดมวิทยา',
    'กระจ่างวิทย์',
    'ทรงธรรมวิทยา',
    'อนุบาลอาภาพัชร',
    'อนุบาลวิมานทิพย์',
    'เซนต์เมรี',
    'คุณากรณ์',
    'มารีย์พิทักษ์',
    'อนุบาลลักษณ์สุข',
    'อนุบาลค่ายประจักษ์ศิลปาคม',
    'ดอนบอสโกวิทยา',
    'เทคโนโลยีอีสานเหนือ 2 กุมภวาปี',
    'อนุบาลนันทวดี',
    'ภูมิพิชญ',
    'มหาไถ่ศึกษากุมภวาปี',
    'มัธยมน้ำตาลอนุสรณ์',
    'เทพปัญญา',
    'อนุบาลกฤชานันท์',
    'อนุบาลวิฆเนศวร์',
    'กมลาลักษณ์',
    'อนุบาลวรสุเมธาวิทย์',
    'วิสุทธิวงศ์โพนสูง',
    'อนุบาลมหาไถ่ศึกษาบ้านดุง',
    'พิชญวิทย์',
    'อนุบาลศศิกานต์',
    'อนุบาลญาริดา',
    'อนุบาลศศิกานต์น้ำโสม',
    'อนุบาลทิพากานต์',
    'โปลีเทคนิคอุดรธานี',
    'เทคนิคพณิชยการสันตพล',
    'ช่างกลอุดรธานี',
    'อุดรพณิชยการ-ช่างกล',
    'เทคโนโลยีราชธานี',
    'เทคโนโลยีอีสานเหนือ',
    'เทคโนโลยีภูมิพิชญ',
    'เทคโนโลยีพณิชยการหนองหาน',
    'เทคโนโลยีวังสามหมอ',
    'เทคนิคพิชญบัณฑิต',
    'เทคโนโลยีพณิชยการบ้านดุง',
    'เทคโนโลยีน้ำโสม',
    'อุดรวิทยา',
    'อุดรคริสเตียนศึกษา',
    'อนุบาลวาสนา',
    'อนุบาลมหาไถ่ศึกษาอุดรธานี',
    'อนุบาลมหาไถ่ศึกษาโพนสูง',
    'อนุบาลขวัญประชา',
    'สาธิตอนุบาล  มหาวิทยาลัยราชภัฏอุดรธานี',
    'เทคโนโลยีภูมิบัณฑิต',
    'บริหารธุรกิจเทคโนโลยีอุดรธานี',
    'อนุบาลปรมินทร์',
    'อนุบาลสันติธรรม',
    'อนุบาลชื่นจิต',
    'อนุบาลพระศรีสงคราม',
    'แสงจันทร์พัฒนา',
    'อนุบาลอรพัฒน์',
    'มหาไถ่ศึกษาเลย',
    'มหาไถ่ศึกษาท่าบ่ม',
    'ประจันตวิทยา',
    'วรราชวิทยา',
    'อนุบาลประจันตะ',
    'อนุบาลภูกระดึง',
    'อนุบาลณัฐพันธุ์ 3',
    'เทคโนโลยีเลยบริหารธุรกิจ',
    'หทัยคริสเตียน',
    'วิถีพุทธภูกระดึงวิทยา',
    'มูลมังหลวงปู่ชอบ ฐานสโม',
    'การกุศลหลวงปู่ศรีจันทร์ วัณณาโภ',
    'เช็นต์จอห์นท่าบม',
    'อนุบาลศุภกรผาขาว',
    'ราชตั้งใจวิทยา',
    'นิคมศึกษา แผนกอนุบาล',
    'อนุบาลนิจจานุเคราะห์',
    'เซนต์ปอลหนองคาย',
    'อนุบาลโสมนัส',
    'อนุบาลศิริกุล',
    'อนุบาลดาวส่องแสง',
    'อนุบาลสาริกา',
    'อนุบาลน้ำผึ้ง',
    'โรซารีโอวิทยา',
    'อนุบาลดวงรัตน์',
    'โพธิเสนวิทยา',
    'อนุบาลจินลดา',
    'มหาไถ่ศึกษาบึงกาฬ',
    'อนุบาลกมลวรรณ',
    'อนุบาลภัทรมน',
    'สันติรักษ์',
    'อนุบาลรัชดา',
    'อนุบาลปัญญารัตน์',
    'อนุบาลหัสดี',
    'อนุบาลเสาวลักษณ์',
    'อนุบาลจินตนา',
    'อนุบาลฉัตรพงษ์',
    'อนุบาลบึงโขงหลง',
    'อนุบาลเฝ้าไร่',
    'อนุบาลอรวรรณ',
    'เทคโนโลยีเอเซียหนองคาย',
    'เจมส์บริหารธุรกิจ',
    'หนองคายบริหารธุรกิจ',
    'เทคนิคบริหารธุรกิจพรเจริญ',
    'เทคโนโลยีโพนพิสัย',
    'เอ็นเทคอินเตอร์เนชั่นแนลเทคโนโลยีหนองคาย',
    'ฮั่วเคียวกงฮัก',
    'อนุบาลบึงกาฬพิทักษ์',
    'อนุบาลบ้านบัณฑิตน้อย',
    'อนุบาลจุฬารัตน์',
    'เทคโนโลยีบริหารธุรกิจรักไทย บึงกาฬ',
    'เทคโนโลยีบริหารธุรกิจรักไทย สาขาท่าบ่อ',
    'เทคโนโลยีบริหารธุรกิจรักไทย โพนพิสัย',
    'การกุศลวัดธรรมาธิปไตย',
    'อนุบาลชโลบล',
    'การกุศลของวัดในพระพุทธศาสนาวัดท่าสะอาด',
    'อนุบาลนวรัตน์',
    'อนุบาลกิติยา',
    'พระกุมารมหาสารคาม',
    'อนุบาลสกุลรัตน์',
    'อนุบาลลูกรัก',
    'อนุบาลอภิสิทธิปัญญา',
    'อนุบาลพิสมัย',
    'มัธยมวัดกลางโกสุม',
    'อนุบาลสายรุ้ง',
    'มัธยมชาญวิทยา',
    'อนุบาลจอนวิทยกุล',
    'อนุบาลเกียรติขจร',
    'จันทนภาศึกษา',
    'อนุบาลมาลีรัตน์',
    'อนุบาลนาเชือก',
    'อนุบาลเอี่ยมสุข',
    'พระกุมารศึกษา',
    'รุ่งอรุณวิทย์',
    'วัดกลางโกสุม',
    'จอนวิทยกุล',
    'พัฒนศึกษา',
    'อนุบาลพุทธธิดา',
    'อนุบาลพรเทพ',
    'วัดป่านาเชือก',
    'อนุบาลอุ่นรัก',
    'ช.วิทยา',
    'อนุบาลไพโรจน์',
    'พระกุมารร้อยเอ็ด',
    'ไพโรจน์วิชชาลัย',
    'อนุบาลสมประสงค์',
    'ศรีศิลปวิทยาลัย',
    'อนุบาลเอกกุมาร',
    'อนุบาลเทพอำนวย',
    'กมลวิทยา',
    'อนุบาลชีวะรัตน์',
    'อนุบาลทองสตา',
    'อนุบาลบุญทวีวัฒนาวิทยา',
    'อนุบาลเครือวัลย์',
    'อนุบาลภัทรดล',
    'อนุบาลตรีภพ',
    'บัณฑิตวิทยา',
    'เสลภูมิวิทยาสันต์',
    'สหมิตรพิทยา',
    'ศรีอรุณวิทย์เสลภูมิ',
    'เจริญศึกษา',
    'เชื้อจรูญวิทย์',
    'อนุบาลค่ายสมเด็จพระพุทธยอดฟ้าจุฬาโลกมหาราช',
    'การกุศลวัดบ้านก่อ',
    'พรเพชรโปลีเทคนิคร้อยเอ็ด',
    'ร้อยเอ็ดเทคโนโลยี',
    'พณิชยการพลาญชัยร้อยเอ็ด',
    'ศรีสมเด็จพณิชยการ',
    'ธีรภาดาเทคโนโลยี',
    'การศึกษาคนตาบอดร้อยเอ็ด',
    'วัดศรีทองนพคุณ',
    'การกุศลวัดหนองนกเอี้ยง',
    'การกุศลวัดกลาง',
    'การกุศลวัดบ้านยางเครือ',
    'อนุบาลอุดมพันธ์',
    'ภัทรดล',
    'อนุบาลเกาะแก้ว',
    'สาธิตศิลปะกมลวิชชสลัย',
    'การกุศลวัดธาตุ',
    'อนุบาลเด็กดี',
    'เทคโนโลยีบริหารธุรกิจรักไทย สุวรรณภูมิ',
    'อนุบาลเมืองสรวง',
    'อนุบาลศรีภูมิวิทยารักษ์',
    'อนุบาลสุนันทา',
    'สตรีชาญยุทธวิทยา',
    'เซนต์ยอแซฟกาฬสินธุ์',
    'อนุบาลรัตนา',
    'อนุบาลแสงตะวัน',
    'อนุบาลทุ่งมน',
    'อนุบาลสาธิตศึกษา',
    'ชาญยุทธวิทยา',
    'อนุบาลขวัญฤทัย',
    'อนุบาลบานเย็น',
    'อนุบาลปิยวรรณ',
    'อนุบาลบุญมีวิทยา',
    'แสงนิลทวนทองวิทยา',
    'เซนต์ยอแซฟ กุฉินารายณ์',
    'สมฤทัย',
    'เขาวงวิทยา',
    'อนุบาลสมฤทัย',
    'เพิ่มพูนวิทยาคมกาฬสินธุ์',
    'อนุบาลชุลีพร',
    'รวมบัณฑิต',
    'รวมบัณฑิตศึกษา',
    'ไตรมิตรวิทยาเสริม',
    'อนุบาลวราภรณ์',
    'อนุบาลพรวิรุฬห์',
    'อนุบาลพรประดิษฐ์',
    'อนุบาลธษพลศึกษา',
    'อนุบาลอุดมพร',
    'คุรุมิตรประสิทธิ์ศิลป์',
    'อนุบาลสรรพเลิศ',
    'อนุบาลเปรมจิต',
    'บัณฑิตศึกษา',
    'อนุบาลพรบิดา',
    'พณิชยการกาฬสินธุ์ ภาคพิเศษ',
    'พณิชยการกาฬสินธุ์',
    'เทคโนโลยีกมลาไสย',
    'ไทยเทคโนโลยีสารสนเทศเอเชีย',
    'ไท-เทค กาฬสินธุ์',
    'เพิ่มพูนพณิชยการเทคโนโลยีกาฬสินธุ์',
    'สมเด็จเทคโนโลยีพณิชยการ',
    'อนุบาลสุปรีดี',
    'เพชรผ่องพันธุ์วิทยา',
    'อนุบาลการะเกด',
    'บุญมีวิทยา',
    'ธนาศิริวัฒนศึกษา',
    'อนุบาลณัฐริกา',
    'เอกปัญญา',
    'บานเย็นศึกษา',
    'กิตติรวี',
    'อนุบาลมูลเสนา',
    'สกลราษฎร์อนุบาล',
    'เซนต์โยเซฟท่าแร่',
    'นิรมลวิทยา',
    'อนุบาลร่วมฤทัย',
    'เซนต์ยอแซฟสกลนคร',
    'อนุบาลประภัสสรสกลนคร',
    'อนุบาลจิตธิดา',
    'อนุบาลนวลมณี',
    'อนุบาลภาวิกา',
    'อนุบาลภนิตา',
    'อนุบาลจินดาพร',
    'เมธาศึกษา',
    'อนุบาลอุดมจิตวิทยา',
    'มารีย์พิทักษ์พังโคน',
    'อนุบาลปิยะธิดา',
    'อนุบาลวาริชภูมิ',
    'อนุบาลเพ็ญจันทร์',
    'อนุบาลโกญจนาท',
    'อนุบาลร่มเกษ',
    'อนุบาลเพ็ญจันทร์บ้านม่วง',
    'อนุบาลแสงอรุณ',
    'อนุบาลสุวรรณเทน',
    'มารีย์พิทักษ์สว่างแดนดิน',
    'อนุบาลสิริพร',
    'อนุบาลจรูญลักษณ์',
    'อนุบาลรพีพร',
    'อนุบาลชนิกานต์',
    'อนุบาลเจริญวัฒนา',
    'เทคโนโลยีภาคตะวันออกเฉียงเหนือสกลนคร',
    'เทคนิคช่างกลสกลนคร',
    'โปลีเทคนิคพุทธรักษา',
    'เทคนิคภูพานบริหารธุรกิจ',
    'สกลนครธุรกิจบัณฑิต',
    'เทคโนโลยีพณิชยการสกลนคร',
    'เทคโนโลยีบริหารธุรกิจพรรณา',
    'พังโคนพณิชยการเทคโนโลยี',
    'วีเทค',
    'เทคโนโลยีพณิชยการบ้านม่วง',
    'เทคนิคธุรกิจสว่างแดนดิน',
    'เทคโนโลยีสว่างแดนดิน',
    'สกลเทคนิค',
    'การกุศลวัดบูรพา',
    'อนุบาลปานชีวา',
    'อนุบาลสุรียาพร',
    'อนุบาลอังสนา',
    'อนุบาลเก่งดีมีสุข',
    'อนุบาลภัทรวรรณ',
    'อนุบาลสรัลพร',
    'ศรีสกลกิจการกุศลวัดศรีสุมังคล์',
    'อนุบาลนิคมน้ำอูน',
    'กิจเจริญวิทยา',
    'อนุบาลนิลวรรณ',
    'สันตยานันท์',
    'อนุบาลมีสุข',
    'อนุบาลวรรณรัตน์',
    'อนุบาลเชษฐานงนุช',
    'อนุบาลธาตุพนม',
    'อนุบาลเรณูนคร',
    'เซนต์ยอแซฟนาแก',
    'เซนต์ยอแซฟศรีสงคราม',
    'อนุบาลแสงอรุณนาหว้า',
    'อินเตอร์เทคโนโลยีแห่งเอเซีย',
    'คฤหบริหารธุรกิจ',
    'อัสสัมชัญเทคนิคนครพนม',
    'เทคโนโลยีธาตุพนม',
    'เทคนิคพาณิชยการนาแก',
    'เทคโนโลยีศรีสงคราม',
    'ตงเจี่ย',
    'อนุบาลดวงแก้วนครพนม',
    'อนุบาลบรรดิษฐน้อย',
    'อนุบาลบ้านเด็ก',
    'อนุบาลสุพร',
    'เซนต์ยอแซฟมุกดาหาร',
    'ท่านพระสารีบุตร',
    'เทคโนโลยีมุกดาหาร',
    'เทคโนโลยีพณิชยการดงหลวง',
    'อนุบาลคำชะอี',
    'เทคโนโลยีบริหารธุรกิจรักไทย  มุกดาหาร',
    'พิงครัตน์',
    'อนุบาลพรรณี',
    'อนุบาลร่มเย็น',
    'อนุบาลอักษรธรรม',
    'อนุบาลสวนน้อย',
    'สันติศึกษา',
    'ไมตรีจิตคริสเตียนเชียงใหม่',
    'อนุบาลดรุณนิมิตวิทยา',
    'อนุบาลอัมพรพรรณ',
    'อนุบาลมั่นศิลป์',
    'อนุบาลมยุรี',
    'อนุบาลโพธิรักษ์',
    'อนุบาลโชติวิทย์',
    'อนุบาลลานนา',
    'วิชัยวิทยา',
    'วชิรวิทย์',
    'อนุบาลบ้านหมอ',
    'อนุบาลสภาพร',
    'อนุบาลบ้านคุณแม่',
    'อนุบาลเวียงพิงค์',
    'อนุบาลดวงแก้ว',
    'เชียงใหม่คณาวิทย์',
    'วารีเชียงใหม่',
    'ปรินส์รอยแยลส์วิทยาลัย',
    'ขาลสุวรรณอนุสรณ์',
    'สิริมังคลานุสรณ์',
    'ดาราวิทยาลัย',
    'มงฟอร์ตวิทยาลัย แผนกมัธยม',
    'เรยีนาเชลีวิทยาลัย',
    'พระหฤทัยเชียงใหม่',
    'วรเชษฐอนุสสรณ์',
    'อนุบาลพิมานเด็ก',
    'โกวิทธำรงเชียงใหม่',
    'อนุบาลวารี',
    'เมตตาศึกษา',
    'ธรรมราชศึกษา',
    'ไชยโรจน์วิทยา',
    'เชียงใหม่คริสเตียน',
    'อนุบาลอุบลลักษณ์',
    'มงฟอร์ตวิทยาลัย แผนกประถม',
    'เทพบดินทร์วิทยา',
    'อนุบาลดรุณนิมิต',
    'อนุบาลดรุณรักษ์',
    'อนุบาลสวนเด็กเชียงใหม่',
    'อนุบาลบ้านเด็ก',
    'อนุบาลสายดรุณ',
    'จิตราวิทยา',
    'อนุศึกษา',
    'สุทธิวงศ์ดำรงวิทย์',
    'ณัทชวิทย์',
    'ไชยจีนะวิทยา',
    'อนุบาลสุทธิวงศ์',
    'เมืองเด็กวิทยา',
    'อนุบาลเฉลิมพล',
    'ศีลรวี',
    'อนุบาลปัญญา',
    'อนุบาลเรวดี',
    'เจ้าฟ้าอุบลรัตน์',
    'อนุบาลจันทโชติ',
    'อนุบาลสุมาลี',
    'อนุบาลวิลาวัลย์',
    'อนุบาลรุ่งเรือง',
    'เชียงใหม่มัธยม',
    'อนุบาลอำไพ',
    'เมฆขจรเชียงใหม่',
    'อนุบาลรัตนา',
    'อนุบาลจิตโรภาส',
    'ดรุณเกษม',
    'ธีระวัธน์บำเพ็ญ',
    'อนุบาลเจริญวัย',
    'รัตนาเอื้อวิทยา',
    'อนุบาลสมฤทธิ์',
    'อนุบาลสายอักษร',
    'สายอักษร',
    'รังษีวิทยา',
    'ปราณีวิทยา',
    'อนุบาลรัตนกาญจน์',
    'อนุบาลพรชัย',
    'เวียงพร้าววิทยา',
    'อนุบาลวิมานทิพย์',
    'อนุบาลอมรศิริ',
    'ศรัญย์ปัญญา',
    'พัฒนวิทย์ศึกษา',
    'อนุบาลเปรมฤดี',
    'อนุบาลไผ่แก้ว',
    'วัยพัฒนาวิทยา',
    'อนุบาลร่มแก้ว',
    'สวนเด็กสันกำแพง',
    'อนุบาลสิทธิโรจน์',
    'อนุบาลแม่บัวคลี่',
    'อนุบาลวัยพัฒนา',
    'อนุบาลเนตรระหงษ์',
    'อนุบาลวัยดรุณ',
    'อนุบาลสันทราย (ตรวจสอบชื่อ)',
    'อนุบาลวีรยา',
    'พงษ์พิกุลเชียงใหม่',
    'อนุบาลขวัญดรุณ',
    'อนุบาลวรศิลป์',
    'เยาวลักษณ์วิทยาหางดง',
    'ลักษณ์พงศ์วิทยา',
    'อนุบาลบ้านบัว',
    'อนุบาลพัฒนชัย',
    'ศรัญย์ปัญญาแผนกอนุบาล',
    'อนุบาลเพลินพิศ',
    'อนุบาลเรือนคำ',
    'อนุบาลช่องฟ้าซินเซิงวาณิชบำรุง',
    'วชิราลัย',
    'อนุบาลดวงกมล',
    'อนุบาลสุรีย์วรรณ',
    'อนุบาลพิมมาลา',
    'วรานีกูล',
    'อนุบาลสุปราณี',
    'ช่องฟ้าซินเซิงวาณิชบำรุง',
    'สืบนทีธรรม',
    'อนุบาลณิชดรุณ',
    'อนุบาลฉัตรพฤกษ์',
    'อนุบาลเอื้อวิทยา',
    'อนุบาลร่มฉัตร',
    'พณิชยการลานนาเชียงใหม่',
    'เชียงใหม่บริหารธุรกิจนานาชาติ',
    'เทคโนโลยีเอเซีย',
    'ไทยวิจิตรศิลป เชียงใหม่',
    'ศรีธนาพณิชยการเทคโนโลยี เชียงใหม่',
    'เมโทรเทคโนโลยี',
    'พณิชยการนครพิงค์',
    'โปลีเทคนิคลานนา',
    'เชียงใหม่เทคโนโลยี',
    'พณิชยการเชียงใหม่',
    'พิงค์พระนครพาณิชยการและเทคโนโลยี',
    'พายัพเทคโนโลยีและบริหารธุรกิจ',
    'เทคโนโลยีไทย-ญี่ปุ่น-เยอรมัน',
    'สหมิตรวิทยา',
    'ฉือจี้เชียงใหม่',
    'อนุบาลบ้านปลาดาว',
    'บ้านคุณแม่',
    'อนุบาลชฎาพร',
    'อนุบาลปัญญาราษฎร์',
    'เพื่อชีวิตเชียงใหม่',
    'อนุบาลแม่บัวไหล',
    'ฉัตรพฤกษ์วิทยา',
    'วชิรวิทย์ ฝ่ายมัธยม',
    'อนุบาลจงลักษณ์',
    'อนุบาลสวนแก้ว',
    'ธนาเทคโนโลยี เชียงใหม่',
    'อนุบาลเต็มสิริวิทยา',
    'อนุบาลสวนองุ่น',
    'อนุบาลทัธนินทร',
    'อนุบาลพงศ์อาภา',
    'พิงคณาทร',
    'อนุบาลวรรณภา',
    'นันทชาติเกรดสคูล',
    'เฉลิมพระเกียรติ ๔๘ พรรษา',
    'มงคลวิทยา',
    'อนุบาลใบบุญ',
    'เลาหจิตรวิทยา',
    'อนุบาลสาธิตวิทยา',
    'ใบบุญลำพูน',
    'อรพินวิทยา',
    'อนุบาลคริสเตียน',
    'รพีเลิศวิทยา',
    'ธนรัตน์วิทยา',
    'อนุบาลดรุณพัฒนา',
    'ดรุณพัฒนา',
    'เมธีวุฒิกร',
    'อนุบาลบ้านทา',
    'จิรพิทยา',
    'บ้านโฮ่งวิทยา',
    'อนุบาลลี้',
    'อนุบาลอรุณรัตน์',
    'อัยยสิริ',
    'ใบบุญป่าซาง',
    'ธรรมสาธิตศึกษา',
    'อรุณโรจน์ศึกษา',
    'อรุณโรจน์',
    'อนุบาลกุลภัทร',
    'อนุบาลละอองทิพย์',
    'อนุบาลสุทธิวงศ์ศึกษา',
    'เทคโนโลยีหมู่บ้านครู ภาคเหนือ',
    'วิสุทธิ์วิทยากร',
    'พินิจวิทยา',
    'เคนเน็ตแม็คเคนซี',
    'วิชชานารี',
    'ผดุงวิทย์',
    'ลำปางวิทยา',
    'อัสสัมชัญลำปาง',
    'ไตรภพวิทยา',
    'อนุบาลเบญจมาศ',
    'อนุบาลทิวฟ้า',
    'อรุโณทัย',
    'อนุบาลศิริมาศ',
    'มัธยมวิทยา',
    'อนุบาล กฟผ.แม่เมาะ',
    'เพ็ญจิตตพงษ์',
    'อ้อมอารีพิทยา',
    'ณัฐบรรจงวิทยา',
    'อนุบาลเพ็ญจิตตพงษ์',
    'ดอนไชยวิทยา',
    'ไชยชุมพลศึกษา',
    'ภินิตย์อัฐพิทยาลำปาง',
    'อักษรศิลป์',
    'ศักดิ์สุนันท์วิทยา',
    'อนุบาลผดุงรัตน์',
    'สมฤทัยวิทยา',
    'ลำปางเทคโนโลยี',
    'ลำปางพาณิชยการและเทคโนโลยี',
    'เถินเทคโนโลยีพณิชยการ',
    'การศึกษาคนตาบอดลำปาง',
    'ศรีสุนทรสีลวิสุทธิ์',
    'ประชาวิทย์',
    'อนุบาลพินิจวิทยา',
    'สุนทรศึกษา',
    'อนุบาลคริสเตียนอุตรดิตถ์',
    'เปี่ยมเมธีวิทยาคาร',
    'บัณฑิตวิทยา',
    'เจริญวิทยา',
    'ภราดานุสรณ์',
    'พิศาลศึกษาวิทยา',
    'หวัวเฉียว',
    'จรัสศิลป์',
    'อุตรดิตถ์เทคโนโลยี',
    'สหคริสเตียน',
    'เซนต์แมรี่อุตรดิตถ์',
    'เทพพิทักษ์วิทยา',
    'อนุบาลเพ็ญรัต',
    'มารดาอุปถัมภ์',
    'อนุบาลพรกมล',
    'อนุบาลเลิศศิริ',
    'อนุบาลน้ำทอง',
    'เจริญราษฎร์',
    'อนุบาลร้องกวาง',
    'ศรีดวงแก้ว',
    'ระเบียบพิทยา',
    'อนุบาลผ่องศรี',
    'อนุบาลเทพกมล',
    'เทพนารี',
    'อนุพงศ์พัชรินทร์',
    'เทพนารี แผนกประถมศึกษา',
    'อนุบาลภาวนา',
    'อนุบาลธนารัตน์',
    'อนุบาลอำนวยพร',
    'พณิชยการลานนาและเทคโนโลยีแพร่',
    'บริหารธุรกิจพณิชยการแพร่',
    'เจริญศิลป์',
    'สอนคนตาบอดสัตติจินตนา จังหวัดแพร่',
    'แพร่คณาวิทย์',
    'อนุบาลธนาวิทย์',
    'นันทบุรีวิทยา',
    'อนุบาลอยู่วิทยา',
    'น่านคริสเตียนศึกษา',
    'พระคุณานุบาลแม่จริม',
    'อนุบาลจริยปัญญา',
    'อนุบาลอิมมานูเอล',
    'อนุบาลปัวอินเตอร์คิดส์',
    'อนุบาลสุวรรณรัตน์',
    'อนุบาลสุภาพวิทยา',
    'อนุบาลนิธากร',
    'อนุบาลกุลหทัย',
    'พรรณทิพย์วิทยา',
    'อนุบาลทะกอง',
    'อนุบาลนันทรัตน์',
    'อิมมานูเอลเชียงกลาง',
    'อนุบาลประภาพรรณ',
    'ซินจง',
    'นิธิวิทย์',
    'พินิตประสาธน์',
    'อนุบาลดรุณวิทยา',
    'บุญสิษฐ์วิทยา',
    'วัฒนาวิทยา',
    'ไชยพันธ์พงษ์พิทยา',
    'อนุบาลบ้านสวนเด็ก',
    'จุนวิทยา',
    'สิรภัทรศึกษา',
    'สาธิตวิทยาเชียงคำ',
    'อนุบาลคุณากรวิทยา',
    'ปิยมิตรวิทยา',
    'วัดบุญเกิดวิทยาแสงส่องหล้า',
    'อนุบาลศรีชัยวิทยา',
    'นฤมลวิทยากร',
    'ไชยพันธ์พงษ์เทคโนโลยี',
    'บริหารธุรกิจพะเยา',
    'ศรีวิราชพณิชยการเทคโนโลยี',
    'ประชาบำรุง',
    'อนุบาลสุนทรารม',
    'ศรีวิราชวิทยา',
    'อนุบาลจำเนียรวิทยา',
    'ปิยมาตย์',
    'อนุบาลหฤทัยวิทยา',
    'อนุบาลปรียาพร',
    'เชียงรายวิทยาคม',
    'ภักดีศึกษา',
    'อนุบาลร่วมใจ',
    'อนุบาลฉัตรมงคล',
    'อนุบาลอำไพ',
    'อนุบาลพรรณี',
    'มารีย์รักษ์เชียงราย',
    'กองทัพบกอุปถัมภ์บริบูรณ์ธนวัฒน์',
    'บัณฑิตศึกษา',
    'สันติวิทยา',
    'คริสเตียนไพศาลศาสตร์',
    'อนุบาลเด็กก้าวหน้า',
    'อนุบาลหวานใจ',
    'อนุบาลศุภลักษณ์',
    'อนุบาลลูกรักเชียงของ',
    'คริสเตียนแสงประทีป',
    'กิตติคุณเทิงวิทยา',
    'เด็กดีพิทยาคม',
    'อนุบาลศิริวัฒนา',
    'อนุบาลคำนวณ',
    'ศิริมาตย์เทวี',
    'อนุบาลอรุณี',
    'อนุบาลวัดเหมืองง่า (ฤทธิไกรอุปถัมภ์)',
    'อนุบาลคริสเตียนสันกอง',
    'อนุบาลสุชาดา',
    'บ้านคุณหนู',
    'อนุบาลอำพร',
    'อนุบาลสุภาพรรณ',
    'บุญพิทักษ์',
    'มานิตวิทยา',
    'อนุบาลนิมิตใหม่',
    'ดรุณราษฎร์วิทยา',
    'พรพิกุลพิทยา',
    'อนุบาลเฟื่องฟ้า',
    'อนุบาลสวนเด็กแม่สรวย',
    'อนุบาลเรืองปัญญา',
    'นุชนาถอนุสรณ์',
    'ความหวังเวียงป่าเป้า',
    'อนุบาลพญาเม็งราย',
    'อนุบาลกฤษณา',
    'เทคโนโลยีกรุงธนเชียงราย',
    'พณิชยการเชียงราย',
    'โปลิเทคนิคเชียงราย',
    'อาชีวศึกษาแม่สาย',
    'สหศาสตร์ศึกษา',
    'ธารทิพย์',
    'วัฒน์ศึกษา',
    'อนุบาลลูกรัก',
    'เอกทวีวิทย์',
    'วัดจอเจริญสุขุมวาท',
    'มารีย์รักษ์เชียงราย แผนกประถม',
    'วิรุณบริหารธุรกิจและเทคโนโลยีเชียงราย',
    'เชียงแสน อาคาเดมี',
    'อนุบาลบ้านคุณหนู',
    'เฟื่องฟ้าวิทยา',
    'ชัยศิษย์บริหารธุรกิจและเทคโนโลยี',
    'พระกุมารเยซูเชียงของ',
    'อนุบาลสุขเจริญพิทยา',
    'อนุบาลจรัสศรี',
    'อนุบาลบรรณวิทย์',
    'อนุบาลสิริอัมพร',
    'อนุบาลธารทิพย์',
    'อนุบาลกฤษณพรรณ',
    'พุทธเกษตร',
    'ลาซาลโชติรวีนครสวรรค์',
    'วิสุทธิศึกษา',
    'อนุบาลพัดทอง',
    'อนุบาลนพคุณนครสวรรค์',
    'สหวิทยาศึกษา',
    'แสงสวรรค์',
    'อนุบาลสมบูรณ์',
    'ประชานุเคราะห์',
    'ยุวบัณฑิตวิทยา',
    'ดรุณศึกษา',
    'วิชาวดี',
    'อนุศาสน์สวรรค์',
    'มารีย์วิทยา',
    'เซนต์โยเซฟนครสวรรค์',
    'จำลองวิทย์',
    'โพฒิสารศึกษา',
    'อนุชนวัฒนา',
    'อนุบาลปราณีวิทยา',
    'โกรกพระพิทยาคม',
    'วันทาศิริศึกษา',
    'ขวัญชัยทับกฤช',
    'สหวิทยานุสรณ์',
    'สหสาธิต',
    'พรหมอนุสรณ์',
    'เยาวชนพัฒนา',
    'ปวีณาวิทยา',
    'อุตสาหะวิทยา',
    'อนุบาลศรีวิชัยรัตน์',
    'อำนวยวิทย์',
    'จันทนานุสรณ์',
    'จันทร์ฉายวิทยา',
    'ปรียาโชติ',
    'วรนารถพิทยา',
    'พินิจอนุสรณ์',
    'ประดับวิทย์',
    'เสรีวิทยา',
    'สามมิตร',
    'กิจจานุศาสตร์',
    'นิมิตศึกษา',
    'วัดโคกเดื่อวิทยา',
    'จิตศรัทธาวิทยานุสรณ์',
    'ศรีอุทิศวิทยา',
    'พยุหะวิทยา',
    'อินทารามวิทยา',
    'บูรพาศึกษา',
    'อนุบาลเสริมสวัสดิ์วิทยา',
    'รังสรรค์วิทยา',
    'พิพัฒน์ศึกษา',
    'พณิชยการนครสวรรค์',
    'เทคโนโลยีภาคเหนือ',
    'สหนครสวรรค์พณิชยการ',
    'พณิชยการวิริยาลัยนครสวรรค์',
    'บริหารธุรกิจนครสวรรค์',
    'เทคโนโลยีนครสวรรค์',
    'เทคโนธุรกิจวิริยาลัยนครสวรรค์',
    'พาณิชสงเคราะห์',
    'เคี่ยวมิ้นเซียะเซี่ยว',
    'จงซันเซียะเซียว',
    'วัดท่าพระเจริญพรตวิทยา',
    'สัมมาสิกขาศาลีอโศก',
    'อนุบาลยุวพัฒน์ นครสวรรค์',
    'อนุศิษย์วิทยา 4',
    'อนุศิษย์วิทยา 3',
    'พิทักษ์ศิษย์วิทยา',
    'อุทัยวิทยาลัย',
    'อนุบาลรจนา',
    'ทองประสาทเวทย์',
    'อนุบาลสีฟ้า',
    'จตุพร',
    'อนุบาลรัศมี',
    'พระสุธรรมยานเถระวิทยา',
    'เชษฐศิลป์บริหารธุรกิจและเทคโนโลยี',
    'สีฟ้าพัฒนวิทย์',
    'อนุบาลศรีลานสัก',
    'จงสวัสดิ์เจริญวิทยา',
    'วัฒนศิริวิทยา',
    'เพ็ชระศึกษา',
    'วรนาถวิทยากำแพงเพชร',
    'กาญจนะศึกษา',
    'เจริญสุขอุดมวิทยา',
    'องอาจวิทยา',
    'อนุบาลวาริน',
    'อนุบาลอุทิศศึกษา',
    'อนุบาลจุฬารัตน์',
    'ทรัพย์สถิตย์วิทยาคาร',
    'อนุบาลภู่ขจร',
    'อนุบาลไอรฎา',
    'อนุบาลสมฤดี',
    'ภู่ขจร',
    'ภักดีพณิชยการและเทคโนโลยี',
    'วรรณวัฒน์ศึกษา',
    'วชิรนิติโสภณ',
    'สุมนเมธี',
    'อุทิศศึกษา',
    'ศรีวรลักษณ์',
    'อนุบาลจิตรวีศึกษา',
    'รังษีวิทยา',
    'อนุบาลฑิฆัมพร',
    'วัชรสหศึกษา',
    'อนุบาลธัญรดา',
    'สหะราษฎร์ศึกษา',
    'มิตรอารีวิทยาสิน',
    'เทพผดุงศิษย์ศึกษา',
    'อนุบาลมิ่งขวัญ',
    'ดรุณานุกูล',
    'อนุบาลธรรมรัตน์',
    'ดรุณานุกูลหัวถนน',
    'อนุบาลดรุณานุกูลลานกระบือ',
    'อนุบาลธัญญาดา',
    'ปัทมดรุณวิทย์',
    'เซนต์ฟรังซีสเซเวียร์มัธโนทัย',
    'อนุบาลสีห์อักษร',
    'กรุงเทพการบัญชีวิทยาลัยลุ่มน้ำปิง',
    'ตากกวง',
    'ธรรมราชานุสรณ์',
    'เซนต์โยเซฟแม่ระมาด',
    'อิสลามศึกษา',
    'ราษฎร์วิทยา',
    'ภัทรวิทยา',
    'ศิริวรรณศึกษา',
    'อนุบาลราษฎร์อุทิศ',
    'อนุบาลการุณย์',
    'พลูหลวงวิทยา',
    'อนุกูลวิทยา (วัดดอนแก้ว)',
    'อนุบาลเพ็ชระ',
    'คริสเตียนแม่หละโกร',
    'ธรรมวัตรวิทยา (วัดดอนมูล)',
    'อนุบาลสายธรรม',
    'สายธรรม',
    'อนุบาลการกุศลวัดตระพังทอง',
    'พณิชยการสุโขทัยธานี',
    'กวางตง',
    'ชุมพรพิทยา',
    'อนุบาลสมประสงค์',
    'ศรีสำโรงวิทยา',
    'พุ่ยฮัว',
    'พระหฤทัยสวรรคโลก',
    'อนุบาลเบญจมาศ',
    'ป้วยมิ้ง',
    'กวางวา',
    'ผดุงราษฎร์',
    'เซนต์นิโกลาส',
    'อภิชาตบุตรวิทยา',
    'อนุบาลโรจนวิทย์',
    'ไทยกล้าวิทยา',
    'อนุบาลประชาราษฎร์',
    'อนุบาลวังจันทน์',
    'อนุบาลโรจนวิทย์ป้อมเพชร',
    'โรจนวิทย์มาลาเบี่ยง',
    'ธีรธาดา พิษณุโลก',
    'ธีรธาดาปฐมวัยและภาษาต่างประเทศพิษณุโลก',
    'เทคนิคพณิชยการพิษณุโลก',
    'บริหารธุรกิจและเทคโนโลยีพิษณุโลก',
    'สิ่นหมิน',
    'ศึกษาวิทย์',
    'อนุบาลจันทราพระกิตติคุณ',
    'จันทราพระกิตติคุณ',
    'อินทุภูติพิทยา',
    'อินทุภูติพิทยา(หนองตม)',
    'ญาณนเรศว์รพิทยาคม',
    'เอ.เจ.เนินมะปราง',
    'อนุบาลธรรมศาสตร์',
    'ราษฎร์บำรุง',
    'พิจิตรอินเตอร์',
    'อนุบาลวชิร',
    'อนุบาลจอมขวัญ',
    'อนุบาลวชิรวิทย์',
    'ยอแซฟพิจิตร',
    'บริหารธุรกิจและเทคโนโลยีพิจิตร',
    'จันทวิทยา',
    'ราษฎร์วิทยา',
    'นรบุตรศึกษา',
    'ดรุณบัณฑิตพิทยา',
    'บัณฑิตศึกษาลัย',
    'เทพประทาน',
    'หัวเฉียว',
    'โถงจื้อบางมูลนาก(สหสงเคราะห์วิทยา)',
    'เยาวชนศึกษา',
    'อนุบาลสุธินี',
    'อนุบาลพุทธชาติ',
    'สิริวัฒนาบริหารธุรกิจและพณิชยการ',
    'ประสิทธิ์วิทยา',
    'โฆษิตวิทยา',
    'สำราญราษฎร์วิทยา',
    'โลมนิลวงศ์อนุสรณ์',
    'เซนต์โยเซฟศรีเพชรบูรณ์',
    'วัชรชัย',
    'กิตติพิทยา',
    'เสรีศึกษา',
    'ดงขุยวิริยานุสรณ์',
    'พิสิษฐ์พิทยา',
    'ผดุงวิทย์',
    'เมตตาศึกษา',
    'พอจิตวิทยา',
    'อนุบาลทิพยาหล่มสัก',
    'แก้วนิมิตวิทยา',
    'เมตตาวิทยา',
    'ฤทัยทิพย์',
    'เมตตาชนูปถัมภ์',
    'หยกฟ้า',
    'บูรณวิทยา',
    'อนุบาลสุภาพพิทยา',
    'ใจจำนงศึกษา',
    'บุญญรักษ์วรานุสรณ์',
    'ปาลพันธุ์วิทยา',
    'ประถมวิทยา',
    'สันติพัฒนา',
    'สัมพันธ์วิทยา',
    'อนุบาลกาญจนาวิทยา',
    'บริหารธุรกิจและเทคโนโลยีบุญญรักษ์',
    'เพชรพินิตศึกษา',
    'ศิริวัฒน์วิทยา',
    'อนุบาลกิลาพร',
    'สหวิทยศึกษา',
    'ศรีปัญญา',
    'วีรสัมพันธ์วิทยา',
    'อนุบาลบังอร',
    'อนุบาลแสงธรรม',
    'แสงธรรมวิทยา',
    'โชติวิทยา',
    'สมอทอดศึกษา',
    'ศิริรัตน์วิทยา',
    'อุดมวิทยา',
    'อนุบาลพร้อมจิต',
    'อนุบาลใจทิพ',
    'ศรีไพรวิทยา',
    'เพชรบูรณ์การ์เด้นฮิลล์',
    'เทคนิคธุรกิจภาคเหนือ',
    'เอื้ออังกูรวิทยา',
    'หล่มสักบริหารธุรกิจเทคโนโลยี',
    'เทคโนโลยีสารสนเทศและบริหารธุรกิจ',
    'วัดซับบอนวิทยา',
    'เทคโนโลยีบริหารธุรกิจเพชรบูรณ์',
    'พิบูลสงครามอุปถัมภ์ ราชบุรี',
    'ดรุณาราชบุรี',
    'นารีวิทยา',
    'สุริยวงศ์',
    'กองทัพบกอุปถัมภ์โยธินวิทยา',
    'อนุบาลภัททณี',
    'กองทัพบกอุปถัมภ์ บูรณวิทยา',
    'ดรุณาราชบุรี แผนกสองภาษา',
    'ราชบุรีบริหารธุรกิจ',
    'ดรุณาราชบุรีพณิชยการ',
    'ดรุณาราชบุรีโปลีเทคนิค',
    'ดำเนินวิทยา',
    'อภิบาลกุลบุตร',
    'พัชรวิทยา',
    'วันทามารีอา ราชบุรี',
    'อนุบาลวัดโชติทายการามสงเคราะห์',
    'ฮั้วเคี้ยวกงลิบไต้ฮึน',
    'อุดมวิทยา',
    'ดุสิตวิทยา',
    'เทพวิทยา',
    'เทพินทร์พิทยา',
    'บ้านโป่งวิทยาคม',
    'วังตาลวิทยา',
    'อนุบาลสุชาดา',
    'นารีวุฒิ',
    'วัดบ้านโป่งสามัคคีคุณูปถัมภ์',
    'สารสิทธิ์พิทยาลัย',
    'ธีรศาสตร์',
    'เพ็ญวิวัฒน์วิทยา',
    'บ้านโป่งบริหารธุรกิจ-เทคโนโลยี',
    'อาชีวะดอนบอสโกบ้านโป่ง',
    'ฮกเฮง',
    'ราษฎร์บำรุงวิทย์',
    'อนุบาลวัดสนามชัย"สุตกิจวิทยา"',
    'มัธยมวัดไทรราษฎร์อุปถัมภ์',
    'วิชัยวิชาคม',
    'แย้มวิทยการ',
    'สมศักดิ์วิทยา',
    'เจี้ยไช้',
    'ธรรมจารินีวิทยา',
    'เรืองวิทย์พระหฤทัย',
    'สิทธิศึกษา',
    'ถาวรวิทยา',
    'ชัยจิตต์วิทยา',
    'อนุบาลเมฆบัณฑิต',
    'หมู่บ้านเด็กมูลนิธิเด็ก',
    'อนุบาลค่ายสุรสีห์',
    'ดรุณากาญจนบุรี',
    'เทคโนโลยีกาญจนบุรี',
    'ดรุณากาญจนบุรีบริหารธุรกิจ',
    'ราษฎร์บำรุง',
    'วินัยนุกูล',
    'ประชาสงเคราะห์',
    'ฐิติวิทยา',
    'อนุชนศึกษา',
    'เขมภัทร์ศึกษา',
    'ศิริวิทยานุบาล',
    'ทิพย์พิทยา',
    'บริหารธุรกิจกาญจนบุรี',
    'บริหารธุรกิจชั้นสูงกาญจนบุรี',
    'วีรศิลป์',
    'อำนวยวิทย์กาญจนบุรี',
    'โชตินันต์',
    'เทคโนโลยีบริหารธุรกิจกาญจนบุรี',
    'ยู่เฉียวเซียะเลี้ยว',
    'อนุบาลเฟื่องฟ้า',
    'สหคริสเตียนศึกษา',
    'สาธิตกาญจนบุรี',
    'เทคโนโลยีสารสนเทศบริหารธุรกิจกาญจนบุรี',
    'บ้านอุ่นรัก',
    'สหวิทย์',
    'อนุบาลยุ้งทอง',
    'ศุภลักษณ์',
    'อนุบาลดวงแก้ว',
    'เอกชนการกุศลวัดสกุณปักษี',
    'สหวิทย์บริหารธุรกิจ',
    'วิทยาศึกษา',
    'อนุบาลเขาพระ',
    'อนุบาลพรประสาทวิทยา',
    'อนุบาลอุทิศวิทยา',
    'ถวิลวิทยา',
    'อนุบาลยอดไม้งาม',
    'ปัญญาประเสริฐ',
    'ปัญญาประเสริฐวิทยา',
    'คี่ตี้',
    'อำนวยเวทย์',
    'ดอนเจดีย์พิทยาคม',
    'อุภัยภาดาวิทยาลัย',
    'แม่พระประจักษ์',
    'อนุบาลเตือนจิตร',
    'ประทีปศึกษา',
    'ศรีรัตน์วิทยา',
    'ปรีดาวิทย์',
    'รัตนศึกษา',
    'สมถวิลอนุบาล',
    'ยอแซฟอุปถัมภ์อู่ทอง',
    'รัตนไพศาล',
    'ภาวนาภิมณฑ์พิทยา',
    'อนุบาลวชิรา',
    'อนุบาลปราณี',
    'บำรุงวิทยา',
    'อนุบาลไผทวิทยา',
    'บอสโกพิทักษ์',
    'อนุบาลเพ็ญศิริ',
    'ธรรมาภิสมัย',
    'อำนวยวิทย์นครปฐม',
    'จันทร์สว่างกูล',
    'อนุบาลสุธีธร',
    'อนุบาลเสริมปัญญา',
    'อนุบาลศิริวรรณ',
    'มัธยมเทคโนโลยีนครปฐม',
    'อนุบาลหอเอกวิทยา',
    'สัมมาสิกขาปฐมอโศก',
    'ช่างสำรวจนครปฐมและเทคโนโลยี',
    'เทคโนโลยีนครปฐม',
    'พัฒนบริหารธุรกิจ',
    'สว่างวิทยา',
    'อนุบาลชโลทร',
    'อนุบาลวัยสะอาด',
    'จารุวรรณวิทยา',
    'สาธิตวิทยา',
    'อนุบาลศิริวิทย์',
    'เดชอนุสรณ์',
    'แสงทองวิทยา',
    'เจริญสุข',
    'เม่งฮั้วกงฮัก',
    'สหบำรุงวิทยา',
    'ราษฎร์บำรุงวิทยา',
    'อนุบาลแสงอรุณ',
    'เจี้ยนหัว',
    'นาคประสิทธิ์',
    'นักบุญเปโตร',
    'สกลวิทยา',
    'ยอแซฟอุปถัมภ์',
    'บวรธนวิทย์',
    'มารีย์อุปถัมภ์',
    'เทพนิมิตวัฒนา',
    'เอกดรุณ',
    'ภัทรบุตร',
    'อนุบาลเอกปฐม',
    'ภัทรนุสรณ์',
    'เสริมสิทธิ์วิทยา',
    'เซนต์โยเซฟอินเตอร์เทคโนโลยี',
    'ภัทรนุสรณ์',
    'อนุบาลพรละมัย',
    'ฟ้าใสวิทยา',
    'เซนต์แอนดรูว์',
    'อนุบาลหมู่บ้านเด็กสานรัก',
    'ศรีทัศน์ดำรงวิทย์',
    'สาครวิทยา',
    'เจริญศิลป์ศึกษา',
    'อันนาลัย',
    'มหาชัยคริสเตียนวิทยา',
    'อนุบาลสมฤดี',
    'อนุบาลน้อมจิตต์',
    'สมฤดีสมุทรสาคร',
    'อนุสรณ์ศุภมาศ',
    'อนุบาลต้นกล้า',
    'ฐานเทคโนโลยี',
    'อนุบาลบ้านทอฝัน',
    'ประชินนุสรณ์',
    'ปฐมพรพิทยา',
    'อนุบาลยุวธัช',
    'จงรักษ์พิทยา',
    'บ้านแพ้ววิทยา',
    'ทานตะวันไตรภาษา',
    'อัตตาภิวัฒน์',
    'สามชัยวิเทศศึกษา',
    'อนุบาลช้างน้อย',
    'ดรุณานุกูล',
    'ประกอบวิทยา',
    'เอื้อวิทยา',
    'กงหลีเจี้ยนหมิน',
    'ดรุณานุเคราะห์',
    'นารีวัฒนา',
    'ถาวรวิทยา',
    'วิริยะวิทยามูลนิธิ',
    'อนุกูลวิทยา',
    'ล้อมรัก',
    'ล้อมรักแม่กลอง',
    'อรุณประดิษฐ์',
    'สุวรรณรังสฤษฎิ์วิทยาลัย',
    'ศึกษาปัญญา',
    'ปริยัติรังสรรค์',
    'อนุบาลเทพศึกษา',
    'อนุบาลศึกษาปัญญา',
    'อนุบาลแสงมิตร',
    'เปรมฤดี',
    'เซนต์โยเซฟ เพชรบุรี',
    'อนุบาลบันไดอิฐ',
    'ศศิมา',
    'อนุบาลโพธิ์ใหญ่พัฒนา',
    'พณิชยการเพชรบุรี',
    'พณิชยการเพชรบุรีบริหารธุรกิจ',
    'ราษฎร์วิทยา',
    'อนุบาลวุฒิสารศึกษา',
    'แก่นจันทร์วิทยา',
    'วัดเขาย้อยไพบูลย์อุปถัมภ์ (แสงส่องหล้า 5)',
    'อิสลามดำรงธรรม',
    'พัฒนศาสน์วิทยาท่าแร้ง',
    'ศิริวิทยา',
    'จันทรศึกษา',
    'พระแม่มารีประจวบคีรีขันธ์',
    'กิตติคุณ',
    'ห้วยหนำ',
    'เกี้ยวกวง',
    'อรุณวิทยา',
    'สหมิตรวิทยาคม',
    'มัธยมนพคุณ',
    'อนุบาลดรุณวัฒนา',
    'สายวิทยา',
    'กองทัพบกอุปถัมภ์รัชตวิทยา',
    'วิเศษวิทยา',
    'ชินนุกูลวิทยา',
    'กำเนิดวิทยา',
    'วิเศษศึกษา',
    'วันทามารีอา',
    'รักษ์วิทยา',
    'เชวงศักดิ์ศึกษา',
    'หัวหินวิทยาลัย',
    'วังไกลกังวล',
    'มัธยมสาธุการวิทยา',
    'ดรุณศึกษา',
    'สมถวิล หัวหิน',
    'อนุบาลดรุณศึกษา',
    'พณิชยการหัวหิน',
    'อุดมศึกษา',
    'วราวัฒน์วิทยา',
    'พัฒนศาสน์วิทยา',
    'บุญประเสริฐ',
    'อนุบาลปรารถนา',
    'เศรษฐรัชต์',
    'แย้มสอาด หัวหิน',
    'ศรีธรรมราชศึกษา',
    'มารีย์พิทักษ์ศึกษา',
    'อนุบาลอกนิษฐ์',
    'จรัสพิชากร',
    'อำมาตย์พิทยานุสรณ์',
    'หนองนกประสาทศิลป์',
    'ชูศิลป์วิทยา',
    'โสภณวิทยา',
    'ชุบธรรมวิทยา',
    'ท้าวราษฎร์สงเคราะห์',
    'ไตรภูมิวิทยา',
    'สตรีจรัสพิชากร',
    'อนุบาลบ้านเด็ก',
    'สหมิตรศึกษา',
    'ตันติวัตร',
    'อนุบาลภิญญรัตน์',
    'ศรีอนุบาล',
    'เกตุชาติศึกษา',
    'ภัทรวิทย์',
    'นรรถยาพิศนุสรณ์',
    'พริ้มพิทยานุสรณ์',
    'อนุบาลปิยะรัตน์',
    'อนุบาลลูกรัก',
    'อนุบาลธนสาร',
    'อนุบาลบ้านพิชญรัตน์',
    'พระมหาธาตุมูลนิธิมัธยม',
    'จรรยานุยุตวิทยา',
    'อนุบาลดำรงค์ศาสน์',
    'อนุบาลวิเชียรรัตน์',
    'อนุบาลบ้านทิพย์รัตน์',
    'อนุบาลสกนวิชญ์',
    'อนุบาลวัดจังหูนสงเคราะห์',
    'วัดกุฎิขันติพลาภิรมสงเคราะห์',
    'อนุบาลแสงทอง',
    'อนุสรณ์พรสวัสดิ์',
    'เทคนิคธุรกิจบัณฑิต',
    'จรัสพิชากรอาชีวศึกษา',
    'นครเทคนิค',
    'นครอาชีวศึกษา',
    'นครพณิชยการ',
    'ศรีนครเทคโนโลยี',
    'โสภณเทคโนโลยี',
    'เทคนิคสถาปัตย์นครบริหารธุรกิจ',
    'พณิชยการคณะพล (เค.ซี.ซี.)',
    'มุสลิมสันติธรรมมูลนิธิ',
    'อัลมูวาห์ฮิดีน',
    'ประทีปศาสน์',
    'มัดร่อซาตุลอุลูมิดดีน',
    'พรหมประสาทวิทย์',
    'อนุบาลกมลทิพย์',
    'รุจิธรรมวิทยา',
    'บ้านเด็กศึกษา',
    'ฉวางสงเคราะห์',
    'อนุบาลวงศ์สวัสดิ์พัฒนา',
    'เจริญมิตร',
    'อักษรวิทย์',
    'ประสานศิลป์ศึกษา',
    'ธนรัตน์อุปถัมภ์',
    'ไพบูลย์วิทยา',
    'อนุบาลบุญญธิดา',
    'เจริญมิตรพณิชยการ',
    'เจริญวิทย์',
    'อนุบาลพิปูนวิทยา',
    'อนุบาลจิรเศรษฐ์',
    'ชะอวดวิทยา',
    'มัธยมศักดิศิลปิน',
    'ชนะวิทยา',
    'อนุบาลพรอุดม',
    'อนุบาลลูกแก้ว',
    'สระแก้ววิทยา',
    'สตรีดำรงเวท',
    'อุบลวิทยา',
    'อนุบาลอินทิรา',
    'อนุบาลวัยวัฒน์',
    'ปัญญาสารธรรมมูลนิธิ',
    'มะฮัดอิสลามอีมานียะฮ์',
    'เจริญวิทยา',
    'รัตนศึกษา',
    'อนุบาลคำแหง',
    'เจริญวัยวิทยา',
    'รัตนศึกษาชนปรีดา',
    'เสริมปัญญา',
    'มนต์หทัย',
    'กาญจนศึกษา',
    'อนุบาลวัดอินทารามสงเคราะห์',
    'พณิชยการทุ่งสง',
    'เทคโนโลยีภาคใต้ (เอส.เทค)',
    'สหมิตรบำรุง',
    'ประชาอุทิศมูลนิธิ',
    'สุขสวัสดิ์วิทยา',
    'วิทยานุเคราะห์',
    'มัธยมบุญคงอุปการ',
    'อนุบาลไกรแก้ว',
    'สุมณฑาศึกษา',
    'อนุบาลพลพันธ์',
    'ภูมิสถิตวิทยา',
    'สหมิตรวิทยา',
    'อนุบาลหลวงครูวิทยา',
    'แพทยกิจพิทยา',
    'อนุบาลพรธิรา',
    'อนุบาลแพทยกิจพิทยา',
    'อนุบาลอภิรัตน์',
    'หลวงครูวิทยา',
    'อนุบาลชุมสุข',
    'เปรมฤทัยศึกษา',
    'อนุบาลกานต์สินี',
    'ปากพนังพณิชยการ',
    'เทคโนโลยีทักษิณพณิชยการ',
    'มัธยมวีรศิลปิน',
    'ทิพย์กองลาศวิทยา',
    'ร่อนพิบูลย์วิทยา',
    'โคกครามพิชากร',
    'ดรุณศึกษา',
    'อนุบาลสกุลดี',
    'นาคคามพิทยาคม',
    'อาชีวศึกษาวีรศิลปิน',
    'พณิชยการศักดิ์ศิลปิน',
    'คุณาธารวิทยา',
    'มัธยมฆังคะทวีศิลป์',
    'ประสาทธรรมวิภัช',
    'อนุบาลเจริญจิต',
    'อนุบาลสิทธิพร',
    'อนุบาลโชติกร',
    'เลิศปัญญาวิทย',
    'เทคโนโลยีพณิชยการสิชล',
    'อุดมปัญญาจารย์',
    'เกียรติขนอมศึกษา',
    'ราษฎร์นิยมวิทยา',
    'ไพศาลศึกษา',
    'อนุบาลเพชรทวี',
    'บำรุงวิทยา',
    'อนุบาลจิตพรรณ',
    'จุฑาวิทย์',
    'พึ่งตนเอง',
    'พิศิษฐ์อนุสรณ์',
    'จันดีพิทยาคม',
    'อนุบาลเพ็ญอาภา',
    'อนุบาลศิริเพชร',
    'จิรวิทย์พิทยา',
    'อนุบาลปัญญาพร',
    'พิมพ์มณี',
    'อนุบาลสิทธาภัทร์',
    'ทุ่งใหญ่พณิชยการ',
    'กาญจนศึกษาพัฒนาการ',
    'อนุบาลสันติสุข',
    'พัฒนศาสน์วิทยา',
    'มีสิทธิ์ศึกษา',
    'อนุบาลรุ่งรัศมี',
    'ประทีปศาสน์อิสมาอีลอนุสรณ์',
    'ประทีปศาสน์พณิชยการ',
    'โภคาพาณิชย์นุกูล มูลนิธิ',
    'อิศรานุสรณ์',
    'อนุบาลหนองทะเล',
    'อนุบาลโภคาพาณิชย์นุกูล มูลนิธิ',
    'สาธิตคำนวณ',
    'เทคนิคพณิชยการนครกระบี่',
    'ประทีปศึกษา',
    'ภูมิสวรรค์วิทยา',
    'เจริญวิชช์',
    'พัฒนศาสน์วิทยา คลองยาง',
    'วิทยาการอิสลาม',
    'อนุบาลรามเดชา',
    'ประกอบธรรมอิสลาม',
    'สันติวิทยา',
    'อนุบาลคณาพร',
    'อนุบาลลำทับ',
    'สังข์ทองวิทยา',
    'อนุบาลโชคชัยเหนือคลอง',
    'อุทยานศึกษากระบี่',
    'ฮีดายาตุดดีนนาวงวิทยา',
    'มุสลิมด้ารุลอุโลมวิทยา',
    'มุสลิมวังทองวิทยา',
    'ทองคำวิทยานุสรณ์',
    'กระบี่โปลีเทคนิคและบริหารธุรกิจ',
    'รุ่งอรุณศึกษา',
    'เอกภาพศาสน์วิชญ์',
    'วุฒิศาสตร์วิทยานุสรณ์',
    'ดวงแก้วพิทยา',
    'อนุบาลมารีรักพังงา',
    'อนุบาลไพรัช',
    'โคกกลอยวิทยา',
    'สุทธินอนุสรณ์',
    'อนุบาลนิลุบล',
    'อนุบาลบ้านเด็ก',
    'สังคมอิสลามวิทยา',
    'ตัรบียะห์อิสลามียะห์',
    'ศิริราษฎร์วิทยา',
    'บุญแสวงวิทยา',
    'ดาริสสลามวิทยา',
    'เยาววิทย์',
    'เพื่อชีวิตพังงา (เบลูก้าอุปถัมภ์)',
    'ขจรเกียรติศึกษา',
    'ดาวรุ่งวิทยา',
    'พุทธมงคลนิมิตร',
    'กาญจนวัฒน์วิทยา',
    'เทพอำนวยวิทยา',
    'ดาราสมุทรภูเก็ต',
    'อนุบาลกนกขวัญ',
    'อนุบาลภูเก็ตกุลธิดา',
    'อนุบาลบุษบง',
    'อนุบาลกาญจนวัฒน์วิทยา',
    'อนุบาลมุสลิมภูเก็ต',
    'วิทยาสาธิต',
    'บริหารธุรกิจภาคใต้',
    'ภูเก็ตเทคโนโลยี',
    'มุสลิมวิทยาภูเก็ต',
    'ภูเก็ตไทยหัว(ประศาสน์วิทยา)',
    'ถลางวิทยา',
    'อนุบาลถลางวิทยา',
    'ขจรเกียรติพัฒนา',
    'มานิตานุเคราะห์',
    'สัมพันธศึกษา',
    'เทพมิตรศึกษา',
    'ยุวศึกษา',
    'ธิดาแม่พระ',
    'เรืองดรุณีวิทยา',
    'ธีราศรมสุราษฎร์',
    'อนุบาลกุลบุตร',
    'อนุบาลหนูน้อย',
    'ประถมตวงวิชช์',
    'อนุบาลเสาวลักษณ์',
    'ศรีมิลินท์อนุสรณ์',
    'อนุบาลจอย',
    'อนุบาลค่ายวิภาวดีรังสิต',
    'อนุบาลบูรณประภา',
    'อนุบาลวิจิตรา',
    'อุ่นรัก 2 ภาษา',
    'สุราษฎร์เทคโนโลยีช่างอุตสาหกรรม',
    'สุราษฎร์เทคโนโลยี',
    'อาชีวะดอนบอสโกสุราษฎร์',
    'อนุบาลขวัญชนก',
    'อนุบาลชุลีกร',
    'อนุบาลขวัญยืน',
    'เซนต์โยเซฟเกาะสมุย',
    'การกุศลวัดละไม',
    'สมุยบริหารธุรกิจ',
    'พุทธนิคม',
    'เพชรผดุงเวียงไชย',
    'อนุบาลกิตติญาณโสภิต',
    'อนุบาลพุทธนิคม',
    'อุปถัมภ์วิทยาพนม',
    'อนุบาลบ้านคุณหนู',
    'พุทธยาศรม',
    'อนุบาลนวพร',
    'อนุบาลกัญจนา',
    'จงฮั้ว',
    'อนุบาลนงรัตน์',
    'อนุบาลเพ็ญพักตร์',
    'อนุบาลบัณฑิตา',
    'อนุบาลพฤษชาติ',
    'เยาวเรศวิทยา',
    'เวียงสระศึกษา',
    'อนุบาลภิญพัฒน์',
    'อนุบาลบ้านเด็ก',
    'อนุบาลบัณฑิต',
    'อนุบาลเพื่อนลูกรัก',
    'อนุบาลดวงใจแม่',
    'ธีราศรม พุนพิน',
    'พันธศึกษา',
    'ปกรณ์พิทยา',
    'ดรุโณทัยพุนพิน',
    'อนุบาลร่วมฤดี',
    'อนุบาลกองบิน 7',
    'อนุบาลวังบอนหอม',
    'เทคโนโลยีศรีวิชัยสุวรรณภูมิ',
    'อนุบาลชนะพร',
    'วมินทร์วิทยา',
    'อนุบาลศรีสุรัตน์',
    'ดารุสสลามวิทยา',
    'อนุบาลอุ่นรักเกาะสมุย',
    'อนุบาลเพียรจรัส',
    'อนุบาลเปี่ยมรัก',
    'พัฒนศาสน์วิทยา',
    'คริสเตียนสุราษฎร์ธานี',
    'อนุบาลบ้านน้องเล็ก',
    'อนุบาลปิยะพัฒน์',
    'อนุบาลอุทัยทิพย์',
    'อนุบาลขวัญแม่',
    'อนุบาลวริศสา',
    'ปัญญาดี',
    'อนุบาลเมธัส',
    'อนุบาลเสริมสุข',
    'สหายวิทย์',
    'ศรีอรุโณทัย',
    'อนุบาลกิจติมาพร',
    'อนุบาลดวงกมล',
    'อนุบาลสุพัชชา',
    'ระนองเทคโนโลยี',
    'หมิงซิน',
    'อนุบาลธรรมรัตน์',
    'ดำรงศาสน์วิทยา',
    'นูโรลฮูดาห์',
    'พัฒนศาสน์วิทยา',
    'นับเงิน',
    'ชุมพรศึกษา',
    'พุทธยาคมศรียาภัย',
    'นิรมลชุมพร',
    'บุญสมบูรณ์ศึกษา',
    'สหศึกษา',
    'ฝวาหมินกงลิ',
    'อนุบาลดรุณรัตน์',
    'อนุบาลปานฤทัย',
    'หลังสวนเทศบาลอุปถัมภ์',
    'อนุบาลเกษมวิทยา',
    'หลังสวนคริสเตียน',
    'ตาแจ้งวังตะกอ',
    'ชุมพรบริหารธุรกิจ',
    'ชุมพรบริหารธุรกิจ',
    'เอื้อพรคริสเตียนมอนเตสซอรี่ ชุมพร',
    'เอื้ออำพน',
    'บุญสมบัติวิทยา',
    'บุญสิทธิ์อนุสรณ์',
    'รักธรรมบริหารธุรกิจ',
    'อนุบาลสุภาภรณ์',
    'อนุบาลบ้านเด็ก',
    'อนุบาลอำนวยสุข',
    'พัฒนศาสน์วิทยาท่าแซะ',
    'กลับเพชรศึกษา',
    'สตรีวชิรานุกูล',
    'วชิรานุกูล',
    'หวังดี',
    'แจ้งวิทยา',
    'ศิริพงศ์วิทยา',
    'จุลสมัย',
    'สุมิตรา',
    'สายพิณ',
    'วชิราโปลีเทคนิค',
    'บริหารธุรกิจวิทยา',
    'เทคโนโลยีพณิชยการสงขลา',
    'สงขลาเทคโนโลยี',
    'บริหารธุรกิจสงขลา',
    'บำรุงศาสน์',
    'อูลูมุดดีน',
    'ดารุลอามาน',
    'สงขลาวิทยามูลนิธิ',
    'สมพงศ์วิทยาจะทิ้งพระ',
    'ศรีวิทยา',
    'สุทธิรักษ์',
    'วิทยาสรรค์ศึกษา',
    'รุ่งโรจน์วิทยา',
    'ตัสดีกียะห์',
    'ศาสนบำรุง (สอลิหุดดีน)',
    'ดรุณวิทยา',
    'สรรเสริญอิสลาม',
    'ดีนูลอิสลาม',
    'กีญามุดดีน',
    'อัลมูตาวัสษิเต๊าะห์',
    'บุสตานุดดีน',
    'จริยธรรมศึกษามูลนิธิ',
    'เจริญธรรมวิทยา',
    'ดำรงค์ศาสน์วิทยา',
    'อิสลามวิทยานุเคราะห์',
    'มอ.วิทยานุสรณ์',
    'สายชล',
    'ศรีนครินทร์วิทยานุเคราะห์',
    'สมบูรณ์ศาสน์วิทยา',
    'ส่งเสริมอิสลาม',
    'อนุบาลเทพา',
    'พัฒนศาสน์วิทยา',
    'อนุบาลรุ่งภิญโญ',
    'รุ่งภิญโญ',
    'อิสลามวิทยามูลนิธิ',
    'ศาสน์สันติธรรม',
    'ราษฎร์บำรุง',
    'กิติคุณ',
    'จิราภรณ์',
    'อนุบาลเจริญรัตน์',
    'สายยิดวิทยามูลนิธิ',
    'ดารุลศาสน์วิทยา',
    'กฤษณาวิทยา',
    'เสนพงศ์',
    'วงศ์วิทย์',
    'สุขเกษมวิทยา',
    'แก้วบุตร-คูสกุล',
    'พัฒนาศาสตร์มูลนิธิ',
    'สังคมอิสลามวิทยา',
    'สามัคคีศาสน์วิทยา',
    'ดารุลมูญาฮีดีนวิทยา',
    'กิตติวิทย์',
    'กอบกาญจน์ศึกษา',
    'พลวิทยา',
    'พัฒนศึกษา',
    'แสงทองวิทยา',
    'ธำรงวัฒนา พัฒนาเยาวชน',
    'ธิดานุเคราะห์',
    'สมานคุณวิทยาทาน',
    'ทิพรัตน์วิทยาคาร',
    'วิริยะเธียรวิทยา',
    'หาดใหญ่อำนวยวิทย์',
    'สหศาสตร์วิทยาคาร',
    'ส่องแสงวิทยา 1997',
    'ส่องแสงพณิชยการ',
    'ศรีนคร',
    'เทพอำนวยหาดใหญ่',
    'กิตติวิทย์บ้านพรุ',
    'ชาตรีวิทยา',
    'สุวรรณวงศ์',
    'สมัยศึกษา',
    'ส่องแสงวิทยา 2000',
    'ทวีรัตน์',
    'บุญเลิศอนุสรณ์',
    'ศรีสว่างวงศ์',
    'วรพัฒน์',
    'อนุบาลบุญเลิศอนุสรณ์',
    'อนุบาลกุลจินต์',
    'สวัสดิ์บวร',
    'พลวิทยาระบบสองภาษา',
    'อนุบาลพัฒนาเยาวชน',
    'แสงอรุณวิทยา',
    'หาดใหญ่อำนวยวิทย์พณิชยการ',
    'ช่างกลภาคใต้เทคโนโลยี',
    'พณิชยการหาดใหญ่',
    'อุดมศึกษาพณิชยการ',
    'หาดใหญ่เทคโนโลยี',
    'หาดใหญ่อำนวยวิทย์บริหารธุรกิจ',
    'หาดใหญ่บริหารธุรกิจนานาชาติ',
    'ท่าใหญ่วิทยา',
    'พัฒนาการศึกษา',
    'ส่งเสริมศาสน์วิทยามูลนิธิ',
    'ส่งเสริมวิทยามูลนิธิ',
    'ธรรมศึกษามูลนิธิ',
    'เจริญศึกษา',
    'บำรุงศึกษา',
    'พะตงวิทยามูลนิธิ',
    'ประชาบำรุงวิทย์',
    'พรหมเจริญวิทยา',
    'อนุบาลดิษฐพร',
    'นุกูลวิทยา',
    'อนุบาลสุพัตรา',
    'พัฒนาวิทยามูลนิธิ',
    'ส่งเสริมศาสนา',
    'บัวหลวง',
    'ศอลาหุดดีน',
    'แสงธรรมวิทยามูลนิธิ',
    'ราษฎร์นิยมวิทยา',
    'มิฟตาฮุดดีน (บ้านพลีใต้)',
    'ชุมชนศึกษาอัล-ฟุรกอน',
    'บ้านป่าระไมวิทยา',
    'ทรัพย์ธานีวิทยา',
    'อนุบาลนครหาดใหญ่ (นะ สุวรรณวงศ์)',
    'การศึกษาคนตาบอดธรรมสากลหาดใหญ่',
    'ป่าเรียนวิทยา',
    'หาดใหญ่วิทยาคาร',
    'สตรีศึกษา',
    'มุสลีมีนศึกษา',
    'จริยศาสน์อิสลาม',
    'ทางธรรมวิทยา',
    'ประทีปศาสตร์อิสลามวิทยา',
    'ศาสนอุปถัมภ์',
    'อนุบาลวลัยพร',
    'เร๊าะห์มานีย๊ะห์',
    'ภักดีศึกษามูลนิธิ',
    'การกุศลในพระพุทธศาสนา วัดในวังพระอารามหลวง',
    'คุณธรรมวิทยา',
    'อิตีซอมวิทยา',
    'ศาสน์พัฒนาวิทยา',
    'อนุบาลทักษิณสยาม',
    'ปิยนุสรณ์ศึกษา',
    'อนุบาลรุ่งทิพย์',
    'ภูริภรณ์ชัยศึกษา',
    'มำบาอูลอูลูม',
    'แสงประทีปวิทยามูลนิธิ',
    'อันซอเรียะห์ อัดดีนียะห์',
    'พัฒนาการศึกษามูลนิธิ',
    'แสงธรรม',
    'จริยธรรมอิสลามมูลนิธิ',
    'สันติศาสตร์ศึกษา',
    'จงหัว',
    'อิสลามวิทยา',
    'ดารุลอูลูม',
    'ดารุลมาอาเรฟ',
    'ธรรมศาสน์วิทยา',
    'เพชรชูศึกษา',
    'อรุณศาสน์วิทยามูลนิธิ',
    'ตัรกี้ยะตุลอุมมะห์',
    'อนุบาลเพชรกาญจน์',
    'สายเพชรศึกษา',
    'อนุบาลอมรรัตน์',
    'มุสลิมศึกษา',
    'อิสลามศึกษาดารุ้ลบิร',
    'อนุบาลมุสลิมสตูล',
    'พรรษนันท์ศึกษา',
    'ศาสนธรรมวิทยา',
    'สตูลศานติศึกษา',
    'สามัคคีอิสลามวิทยา',
    'อนุบาลท่าแพพัฒนา',
    'สุไหงอุเปวิทยา',
    'มัธยมวัดควนวิเศษมูลนิธิ',
    'ตรังคริสเตียนศึกษา',
    'ตรังวิทยา',
    'บูรณะรำลึก',
    'ทับเที่ยงฮั่วเฉียวเซี่ยเซี่ยว',
    'วัฒนาศึกษา',
    'พรศิริกุล',
    'ดรุโณทัย',
    'ทิพยรัตน์วิทยา',
    'โพธิ์วิเชียรชัย',
    'บุญเหลือเกื้อคง',
    'จินตรังษี',
    'นูรุลอิงซาน',
    'เทคนิคพณิชยการสยาม',
    'โปลีเทคนิคตรัง',
    'สหตรังอาชีวะ',
    'แสงธรรมวิทยาตรัง',
    'อนุบาลกันตังศึกษา',
    'วิเศษกาญจน์',
    'ประชาวิทยา',
    'จุ๋งฮัวโชะเซียว',
    'วัดนิกรรังสฤษฎิ์',
    'สว่างราษฎร์วิทยา',
    'อัมพวันวิทยา',
    'ทุ่งยาววิทยา',
    'เมธาวิทยา',
    'บ้านเด็กน้อยตรัง',
    'วิวัฒน์วิทยา',
    'ธาดาอนุสรณ์',
    'อนุบาลรัษฎา',
    'พรศิริกุลปฐมวัย',
    'อิสมาอีลียะห์ราษฎร์อุปถัมภ์',
    'มัธยมพัทลุงวิทยา',
    'วีรนาทศึกษา',
    'อนุบาลอารีนาถ',
    'อนุบาลเสริมปัญญา',
    'อนุบาลเรวดี',
    'อนุบาลช้างน้อย',
    'อนุบาลธิดาเมตตาธรรมพัทลุง',
    'เรวดีพัทลุง',
    'พณิชยการพัทลุง',
    'อนุบาลบ้านเด็ก',
    'พณิชยการพัทลุง',
    'พัทลุงบริหารธุรกิจ',
    'พัทลุงการกุศลมูลนิธิ (จุ่งฮั่ว)',
    'ศาสนวิทยา',
    'ศึกษาธรรมอิสลาม',
    'สันติธรรมวิทยา',
    'อิสลามศาสตร์ศึกษา',
    'พัฒนาวิทยามูลนิธิ',
    'บำรุงอิสลาม',
    'ดารุสสลาม',
    'อิสลามิยะฮ์สหวิทยา',
    'อนุบาลธนะวิทย์',
    'มุสลิมวิทยา(ประชาอุปถัมภ์)',
    'อนุบาลเรวดีควนขนุน',
    'อารียาศึกษา',
    'ศาสนูปถัมภ์ปากพะยูนมูลนิธิ',
    'อนุบาลสุขคณะ',
    'ภักดีอนุสรณ์',
    'อะมาดียะห์',
    'นูรุลอิสลามหนองธง',
    'อนุบาลอุดมศรีวิทยา',
    'อนุบาลเรวดีศึกษา',
    'บ้านนาบอน (อิสลามศึกษา)',
    'อิสลามศาสตร์',
    'อนุบาลสุวรรณภักดี',
    'ประทีปศาสน์วิทยา',
    'บางแก้วอิสลามศึกษา (หนองบ่อ)',
    'ดรุณศาสตร์สหวิทยา',
    'วรคามินอนุสรณ์',
    'แหลมทองอุปถัมภ์',
    'อามานะศักดิ์',
    'เจริญศรีศึกษา',
    'ยุวชิตบริหารธุรกิจปัตตานี',
    'ศานติธรรมวิทยา',
    'ปูยุดประชารักษ์',
    'บำรุงอิสลาม',
    'พัฒนาอิสลาม',
    'สตรีพัฒนศึกษา"หะยีนิเซาะ อาเก็บอุไร-บำเพ็ญ"',
    'วิทยาอิสลาม',
    'ศาสนูปถัมภ์',
    'เตรียมศึกษาวิทยา',
    'จงรักสัตย์วิทยา',
    'จ้องฮั้ว',
    'จิปิภพพิทยา',
    'อนุบาลสว่างวิทย์',
    'บุญมีวิทยา',
    'เพชรชูนุเคราะห์',
    'ซอลีฮียะห์อัดดีนียะห์',
    'มูฮัมมาดียะห์',
    'อิสลามสามัคคี',
    'มูลนิธิอาซิซสถาน',
    'พีระยานาวินคลองหินวิทยา',
    'ลำหยังวิทยามูลนิธิ',
    'จริยอิสลามศึกษาอนุบาล',
    'แสงประทีปวิทยา',
    'บากงพิทยา',
    'ศาสน์สามัคคี',
    'มูลนิธิชุมชนอิสลามศึกษา',
    'อะเดร์รอซะอิสลามียะห์',
    'อัลยามีอะห์อัลอิสลามียะห์',
    'วัฒนธรรมอิสลาม',
    'สมบูรณ์ศาสน์อิสลาม',
    'วิทยาศีล',
    'ดรุณมุสลิมวิทยา',
    'ยุวอิสลามวิทยามูลนิธิ',
    'กูตงวิทยา',
    'สันนิธิอิสลาม',
    'อิสลามดูวา (ศาลาฟี)',
    'อิสลามศาสน์วิทยา',
    'รัศมีสถาปนา',
    'ธรรมพิทยาคาร',
    'อิสลามประชาสงเคราะห์',
    'อิสลามนิติวิทย์',
    'ภักดีวิทยา',
    'อนุบาลอนุสรณ์',
    'ดรุณศาสน์วิทยา',
    'ราษฎร์ประชานุเคราะห์',
    'อรุณศาสน์วิทยา',
    'บำรุงมุสลีมีน',
    'ศาสน์อิสลาม',
    'ลาลอวิทยา',
    'ศาสนศึกษา',
    'สามารถดีวิทยา',
    'สายบุรีอิสลามวิทยา',
    'ฮั่วหนำมูลนิธิ',
    'อัตตัรบียะฮ์อิสลามียะฮ์',
    'มุสลิมพัฒนศาสตร์',
    'ส่งเสริมศาสน์',
    'อนุบาลแสงสันติปอซัน',
    'ประสานวิทยามูลนิธิ',
    'ดารุลบารอกะฮุ',
    'สันติวิทยา',
    'ปอซันพัฒนา',
    'อัลอิสลามียะห์วิทยามูลนิธิ',
    'แสงธรรมศึกษาปัตตานี',
    'พิทักษ์ศาสน์',
    'อะห์มาดีวิทยามูลนิธิ',
    'บ้านดอนวิทยา',
    'ทวีวิทยาอิสลาม',
    'สามัคคีธรรมวิทยา',
    'พัฒนศาสตร์',
    'ปัตตานีดารุสลาม',
    'ปัญญาวิทย์',
    'เตรียมวิทยาอาฆง',
    'ส่งเสริมอิสลามศึกษา',
    'พิทักษ์ศาสน์วิทยา',
    'ดรุณวิทยา',
    'อิสลามพัฒนา',
    'นูรุลอิสลามภูมีวิทยา',
    'อามานะวิทยา',
    'รัชตะวิทยา',
    'ยะลาบำรุงผดุงประชา',
    'รังสีอนุสรณ์',
    'มานะศึกษา',
    'เพ็ญศิริ',
    'อนุบาลศรีบำรุง',
    'อนุบาลประณีต',
    'ศานติธรรม',
    'ผดุงประชาพณิชยการ',
    'เทคนิคบริหารธุรกิจยะลา',
    'ธรรมวิทยามูลนิธิ',
    'สตรีอิสลามวิทยามูลนิธิ',
    'พัฒนาวิทยา',
    'พัฒนาอิสลามวิทยา',
    'อาสาสุลดินวิทยา',
    'มุสลิมศึกษา',
    'อิสลาฮียะห์',
    'อุดมศาสน์วิทยา',
    'ตาร์เบียตุลวาตันมูลนิธิ',
    'ประทีปวิทยา',
    'อิสลามประสานวิทย์',
    'ดารุลอูโลมนิบงบารู',
    'อิสลาฮุดดีนวิทยา',
    'ถนอมศรีศึกษา',
    'ธีรวิทยา',
    'วชิรศึกษา',
    'คอยรียะห์วิทยามูลนิธิ',
    'จงฝามูลนิธิ',
    'ดำรงวิทยา',
    'ผดุงศีลวิทยา',
    'อิสลามบาเจาะวิทยา',
    'สุทธิศาสน์วิทยา',
    'สตรีศาสนูปถัมภ์',
    'สุขสวัสดิ์วิทยา',
    'มุสลิมบำรุง',
    'อนุบาลศรีรามันห์',
    'ดารุลฮูดาห์วิทยา',
    'พัฒนาวิทยากร',
    'ศรีฟารีดาบารูวิทยา',
    'มะฮัดดะวะห์อิสลามียะห์',
    'มะอาหัดอิสลามียะห์',
    'อาลาวียะห์วิทยา',
    'คัมภีร์วิทยา',
    'เจริญศาสน์วิทยา',
    'ประทีปธรรมวิทยา',
    'อนุบาลญัณญาร์',
    'ศาสโนวาทวิทยา',
    'ลุกมานูลฮากีม',
    'พัฒนศาสน์โกตาบารู',
    'ดารุลไอดัมวัลมาซากีน',
    'ผดุงศาสน์วิทยา',
    'มาอาฮัดดารุสสลาม',
    'แสงธรรมวิทยา',
    'ธรรมอิสลามศึกษา',
    'สมบูรณ์ศาสน์',
    'ดาราวิทย์',
    'แสงจริยธรรมวิทยา',
    'บำรุงศาสน์วิทยา',
    'อัลฟัลลาฮ์ อัลอิสลามี',
    'พิมานวิทย์นราธิวาส',
    'ศรสมบูรณ์',
    'สวนสวรรค์วิทยา',
    'อัตตัรกียะห์อิสลามียะห์',
    'อัซซูลามียะห์ตูดีนียะห์',
    'นิรันดรวิทยา',
    'สุกัญศาสน์วิทยา',
    'โต๊ะดามียะห์',
    'นราวิทย์อิสลาม',
    'อิสลามบูรพา',
    'อิสลามบูรณะโต๊ะนอ',
    'พัฒนศาสน์วิทยา',
    'บางนราวิทยา',
    'ราษฎร์วิทยา',
    'บ้านสมถวิล',
    'นูรุดดิน',
    'อิสลามวิทยาทาน',
    'อัดดีนียาตุลอิสลามียะห์',
    'ศิริธรรมวิทยา',
    'เจริญศาสตร์',
    'ดาราศาสน์วิทยา',
    'ธรรมสตัมภ์วิทยา',
    'เจริญวิทยานุสรณ์',
    'อัลมัสโฮร์',
    'ตายุลอิสลาม',
    'พิพัฒน์ทักษิณ',
    'ฮาซานียะห์',
    'สมานมิตรวิทยา',
    'อดุลวิทยา',
    'อัครศาสน์วิทยา',
    'อนุรักษ์อิสลาม',
    'มาฮัดมูฮัมมาดียะห์',
    'ดารุลกุรอานิลการีม',
    'แหลมทองวิทยา',
    'ประทีปวิทยา',
    'ดารุสสาลาม',
    'ดาราวิทยา',
    'เจริญศึกษา',
    'รือเสาะวิทยา',
    'ศรีทักษิณ',
    'ภัทรียาอนุบาล',
    'อนุบาลรือเสาะ',
    'อิบตีดาวิทยา',
    'ดารุลอุลลม',
    'นะห์ฎอตุลสูบาน',
    'ต้นตันหยง',
    'ไอร์เต๊ะวิทยา',
    'อัตเตาฟีกียะห์อิสลามียะห์',
    'วุฒิศาสน์วิทยา',
    'จริยธรรมวิทยา',
    'รอมาเนีย',
    'ผดุงวิทย์',
    'บุณยลาภนฤมิต',
    'รังผึ้ง',
    'อนุบาลบ้านสุชาดา',
    'เกษมทรัพย์',
    'นะห์ฎอฎุลอิสลาฮียะห์',
    'แสงธรรมวิทยา(มูลนิธิ)',
    'ประชานุเคราะห์',
    'อิสลามอนุศาสน์',
    'ดารุลรอห์มาฮ์',
    'สัมพันธ์วิทยา',
    'อัลอิสลามียะห์',
    'บูกิตอิสลามียะห์',
    'สานียาติลอิสลามียะห์',
    'ดารุลฮิกมะห์',
    'ดารุลอามานวิทยา',
    'อัลอีย๊ะวิทยา',
    'จรรยาอิสลาม',
    'ดารุลอิสละห์',
    'ศูนย์บริการการศึกษานอกโรงเรียนเขตพระนคร',
    'ศูนย์ส่งเสริมการศึกษานอกโรงเรียนกลุ่มเป้าหมายพิเศษ',
    'ศูนย์บริการการศึกษานอกโรงเรียนเขตหนองจอก',
    'ศูนย์บริการการศึกษานอกโรงเรียนเขตบางเขน',
    'ศูนย์บริการการศึกษานอกโรงเรียนเขตบางกะปิ',
    'ศูนย์บริการการศึกษานอกโรงเรียนเขตปทุมวัน',
    'ศูนย์บริการการศึกษานอกโรงเรียนเขตป้อมปราบศัตรูพ่าย',
    'ศูนย์บริการการศึกษานอกโรงเรียนเขตพระโขนง',
    'ศูนย์การศึกษานอกโรงเรียนกรุงเทพมหานคร 4',
    'ศูนย์บริการการศึกษานอกโรงเรียนเขตมีนบุรี',
    'ศูนย์บริการการศึกษานอกโรงเรียนเขตลาดกระบัง',
    'ศูนย์บริการการศึกษานอกโรงเรียนเขตยานนาวา',
    'ศูนย์บริการการศึกษานอกโรงเรียนเขตสัมพันธวงศ์',
    'ศูนย์การศึกษานอกโรงเรียนกรุงเทพมหานคร 2',
    'ศูนย์บริการการศึกษานอกโรงเรียนเขตพญาไท',
    'ศูนย์บริการการศึกษานอกโรงเรียนเขตธนบุรี',
    'ศูนย์บริการการศึกษานอกโรงเรียนเขตบางกอกใหญ่',
    'ศูนย์บริการการศึกษานอกโรงเรียนเขตห้วยขวาง',
    'ศูนย์บริการการศึกษานอกโรงเรียนเขตคลองสาน',
    'ศูนย์บริการการศึกษานอกโรงเรียนเขตตลิ่งชัน',
    'ศูนย์การศึกษานอกโรงเรียนกรุงเทพมหานคร 1',
    'ศูนย์บริการการศึกษานอกโรงเรียนเขตบางกอกน้อย',
    'ศูนย์บริการการศึกษานอกโรงเรียนเขตบางขุนเทียน',
    'ศูนย์บริการการศึกษานอกโรงเรียนเขตภาษีเจริญ',
    'ศูนย์บริการการศึกษานอกโรงเรียนเขตหนองแขม',
    'ศูนย์บริการการศึกษานอกโรงเรียนเขตราษฎร์บูรณะ',
    'ศูนย์บริการการศึกษานอกโรงเรียนเขตบางพลัด',
    'ศูนย์บริการการศึกษานอกโรงเรียนเขตดินแดง',
    'ศูนย์บริการการศึกษานอกโรงเรียนเขตบึงกุ่ม',
    'ศูนย์บริการการศึกษานอกโรงเรียนเขตสาทร',
    'ศูนย์บริการการศึกษานอกโรงเรียนเขตบางซื่อ',
    'ศูนย์บริการการศึกษานอกโรงเรียนเขตจตุจักร',
    'ศูนย์บริการการศึกษานอกโรงเรียนเขตบางคอแหลม',
    'ศูนย์บริการการศึกษานอกโรงเรียนเขตประเวศ',
    'ศูนย์การศึกษานอกโรงเรียนกรุงเทพมหานคร 3',
    'ศูนย์บริการการศึกษานอกโรงเรียนเขตคลองเตย',
    'ศูนย์บริการการศึกษานอกโรงเรียนเขตสวนหลวง',
    'ศูนย์บริการการศึกษานอกโรงเรียนเขตจอมทอง',
    'ศูนย์บริการการศึกษานอกโรงเรียนเขตดอนเมือง',
    'ศูนย์บริการการศึกษานอกโรงเรียนเขตราชเทวี',
    'สถาบันการศึกษาทางไกล',
    'ศูนย์บริการการศึกษานอกโรงเรียนเขตลาดพร้าว',
    'ศูนย์บริการการศึกษานอกโรงเรียนเขตวัฒนา',
    'ศูนย์การศึกษานอกโรงเรียนจังหวัดสมุทรปราการ',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอเมืองสมุทรปราการ',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอบางบ่อ',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอบางพลี',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอพระประแดง',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอพระสมุทรเจดีย์',
    'ศูนย์บริการการศึกษานอกโรงเรียนกิ่งอำเภอบางเสาธง',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอเมืองนนทบุรี',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอบางกรวย',
    'ศูนย์การศึกษานอกโรงเรียนจังหวัดนนทบุรี',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอบางใหญ่',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอบางบัวทอง',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอไทรน้อย',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอปากเกร็ด',
    'ศูนย์การศึกษานอกโรงเรียนจังหวัดปทุมธานี',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอเมืองปทุมธานี',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอคลองหลวง',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอธัญบุรี',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอหนองเสือ',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอลาดหลุมแก้ว',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอลำลูกกา',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอสามโคก',
    'ศูนย์การศึกษานอกโรงเรียนจังหวัดพระนครศรีอยุธยา',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอพระนครศรีอยุธยา',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอท่าเรือ',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอนครหลวง',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอบางไทร',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอบางบาล',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอบางปะอิน',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอบางปะหัน',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอผักไห่',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอภาชี',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอลาดบัวหลวง',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอวังน้อย',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอเสนา',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอบางซ้าย',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภออุทัย',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอมหาราช',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอบ้านแพรก',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอเมืองอ่างทอง',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอไชโย',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอป่าโมก',
    'ศูนย์การศึกษานอกโรงเรียนจังหวัดอ่างทอง',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอโพธิ์ทอง',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอแสวงหา',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอวิเศษชัยชาญ',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอสามโก้',
    'ศูนย์การศึกษานอกโรงเรียนจังหวัดลพบุรี',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอเมืองลพบุรี',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอพัฒนานิคม',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอโคกสำโรง',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอชัยบาดาล',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอท่าวุ้ง',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอบ้านหมี่',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอท่าหลวง',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอสระโบสถ์',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอโคกเจริญ',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอลำสนธิ',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอหนองม่วง',
    'ศูนย์การศึกษานอกโรงเรียนจังหวัดสิงห์บุรี',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอเมืองสิงห์บุรี',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอบางระจัน',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอค่ายบางระจัน',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอพรหมบุรี',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอท่าช้าง',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภออินทร์บุรี',
    'ศูนย์การศึกษานอกโรงเรียนจังหวัดชัยนาท',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอเมืองชัยนาท',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอมโนรมย์',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอวัดสิงห์',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอสรรพยา',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอสรรคบุรี',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอหันคา',
    'ศูนย์บริการการศึกษานอกโรงเรียนกิ่งอำเภอหนองมะโมง',
    'ศูนย์การศึกษานอกโรงเรียนจังหวัดสระบุรี',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอเมืองสระบุรี',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอแก่งคอย',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอหนองแค',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอวิหารแดง',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอหนองแซง',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอบ้านหมอ',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอดอนพุด',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอหนองโดน',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอพระพุทธบาท',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอเสาไห้',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอมวกเหล็ก',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอวังม่วง',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอเฉลิมพระเกียรติ',
    'ศูนย์การศึกษานอกโรงเรียนจังหวัดชลบุรี',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอเมืองชลบุรี',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอบ้านบึง',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอหนองใหญ่',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอบางละมุง',
    'ศูนย์ฝึกและพัฒนาอาชีพเกษตรกรรมวัดญาณสังวรารามวรมหาวิหาร อันเนื่องมาจากพระราชดำริ',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอพานทอง',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอพนัสนิคม',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอศรีราชา',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอเกาะสีชัง',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอสัตหีบ',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอบ่อทอง',
    'ศูนย์การศึกษานอกโรงเรียนจังหวัดระยอง',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอเมืองระยอง',
    'ศูนย์การศึกษานอกโรงเรียนภาคตะวันออก',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอบ้านฉาง',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอแกลง',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอวังจันทร์',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอบ้านค่าย',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอปลวกแดง',
    'ศูนย์บริการการศึกษานอกโรงเรียนกิ่งอำเภอเขาชะเมา',
    'ศูนย์บริการการศึกษานอกโรงเรียนกิ่งอำเภอนิคมพัฒนา',
    'ศูนย์การศึกษานอกโรงเรียนจังหวัดจันทบุรี',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอเมืองจันทบุรี',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอขลุง',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอท่าใหม่',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอโป่งน้ำร้อน',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอมะขาม',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอแหลมสิงห์',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอสอยดาว',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอแก่งหางแมว',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอนายายอาม',
    'ศูนย์บริการการศึกษานอกโรงเรียนกิ่งอำเภอเขาคิชฌกูฏ',
    'ศูนย์การศึกษานอกโรงเรียนจังหวัดตราด',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอเมืองตราด',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอคลองใหญ่',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอเขาสมิง',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอบ่อไร่',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอแหลมงอบ',
    'ศูนย์บริการการศึกษานอกโรงเรียนกิ่งอำเภอเกาะกูด',
    'ศูนย์บริการการศึกษานอกโรงเรียนกิ่งอำเภอเกาะช้าง',
    'ศูนย์การศึกษานอกโรงเรียนจังหวัดฉะเชิงเทรา',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอเมืองฉะเชิงเทรา',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอบางคล้า',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอบางน้ำเปรี้ยว',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอบางปะกง',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอบ้านโพธิ์',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอพนมสารคาม',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอราชสาส์น',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอสนามชัยเขต',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอแปลงยาว',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอท่าตะเกียบ',
    'ศูนย์บริการการศึกษานอกโรงเรียนกิ่งอำเภอคลองเขื่อน',
    'ศูนย์การศึกษานอกโรงเรียนจังหวัดปราจีนบุรี',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอเมืองปราจีนบุรี',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอกบินทร์บุรี',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอนาดี',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอบ้านสร้าง',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอประจันตคาม',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอศรีมหาโพธิ',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอศรีมโหสถ',
    'ศูนย์การศึกษานอกโรงเรียนจังหวัดนครนายก',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอเมืองนครนายก',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอปากพลี',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอบ้านนา',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอองครักษ์',
    'ศูนย์การศึกษานอกโรงเรียนจังหวัดสระแก้ว',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอเมืองสระแก้ว',
    'ศูนย์ฝึกและพัฒนาอาชีพราษฎรไทยบริเวณชายแดนจังหวัดสระแก้ว',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอคลองหาด',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอตาพระยา',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอวังน้ำเย็น',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอวัฒนานคร',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภออรัญประเทศ',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอเขาฉกรรจ์',
    'ศูนย์บริการการศึกษานอกโรงเรียนกิ่งอำเภอโคกสูง',
    'ศูนย์การศึกษานอกโรงเรียนจังหวัดนครราชสีมา',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอเมืองนครราชสีมา',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอครบุรี',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอเสิงสาง',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอคง',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอบ้านเหลื่อม',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอจักราช',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอโชคชัย',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอด่านขุนทด',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอโนนไทย',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอโนนสูง',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอขามสะแกแสง',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอบัวใหญ่',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอประทาย',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอปักธงชัย',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอพิมาย',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอห้วยแถลง',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอชุมพวง',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอสูงเนิน',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอขามทะเลสอ',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอสีคิ้ว',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอปากช่อง',
    'สถาบันการศึกษาและพัฒนาต่อเนื่องสิรินธร',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอหนองบุนนาก',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอแก้งสนามนาง',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอโนนแดง',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอวังน้ำเขียว',
    'ศูนย์บริการการศึกษานอกโรงเรียนกิ่งอำเภอเทพารักษ์',
    'ศูนย์บริการการศึกษานอกโรงเรียนกิ่งอำเภอเมืองยาง',
    'ศูนย์บริการการศึกษานอกโรงเรียนกิ่งอำเภอพระทองคำ',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอลำทะเมนชัย',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอเฉลิมพระเกียรติ',
    'ศูนย์การศึกษานอกโรงเรียนจังหวัดบุรีรัมย์',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอเมืองบุรีรัมย์',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอคูเมือง',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอกระสัง',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอนางรอง',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอหนองกี่',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอละหานทราย',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอประโคนชัย',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอบ้านกรวด',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอพุทไธสง',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอลำปลายมาศ',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอสตึก',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอปะคำ',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอนาโพธิ์',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอหนองหงส์',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอพลับพลาชัย',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอห้วยราช',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอโนนสุวรรณ',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอชำนิ',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอบ้านใหม่ไชยพจน์',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอโนนดินแดง',
    'ศูนย์บริการการศึกษานอกโรงเรียนกิ่งอำเภอบ้านด่าน',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอเฉลิมพระเกียรติ',
    'ศูนย์การศึกษานอกโรงเรียนจังหวัดสุรินทร์',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอเมืองสุรินทร์',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอชุมพลบุรี',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอท่าตูม',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอจอมพระ',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอปราสาท',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอกาบเชิง',
    'ศูนย์ฝึกและพัฒนาอาชีพราษฎรไทยบริเวณชายแดนจังหวัดสุรินทร์',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอรัตนบุรี',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอสนม',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอศีขรภูมิ',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอสังขะ',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอลำดวน',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอสำโรงทาบ',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอบัวเชด',
    'ศูนย์บริการการศึกษานอกโรงเรียนกิ่งอำเภอพนมดงรัก',
    'ศูนย์บริการการศึกษานอกโรงเรียนกิ่งอำเภอศรีณรงค์',
    'ศูนย์บริการการศึกษานอกโรงเรียนกิ่งอำเภอเขวาสินรินทร์',
    'ศูนย์การศึกษานอกโรงเรียนจังหวัดศรีสะเกษ',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอเมืองศรีสะเกษ',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอยางชุมน้อย',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอกันทรารมย์',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอกันทรลักษ์',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอขุขันธ์',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอไพรบึง',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอปรางค์กู่',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอขุนหาญ',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอราษีไศล',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภออุทุมพรพิสัย',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอบึงบูรพ์',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอห้วยทับทัน',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอโนนคูณ',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอศรีรัตนะ',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอน้ำเกลี้ยง',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอวังหิน',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอภูสิงห์',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอเมืองจันทร์',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอเบญจลักษ์',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอพยุห์',
    'ศูนย์บริการการศึกษานอกโรงเรียนกิ่งอำเภอโพธิ์ศรีสุวรรณ',
    'ศูนย์การศึกษานอกโรงเรียนจังหวัดอุบลราชธานี',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอเมืองอุบลราชธานี',
    'ศูนย์การศึกษานอกโรงเรียนภาคตะวันออกเฉียงเหนือ',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอศรีเมืองใหม่',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอโขงเจียม',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอเขื่องใน',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอเขมราฐ',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอเดชอุดม',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอนาจะหลวย',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอน้ำยืน',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอบุณฑริก',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอตระการพืชผล',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอกุดข้าวปุ้น',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอม่วงสามสิบ',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอวารินชำราบ',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอพิบูลมังสาหาร',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอตาลสุม',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอโพธิ์ไทร',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอสำโรง',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอดอนมดแดง',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอสิรินธร',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอทุ่งศรีอุดม',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอนาเยีย',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอนาตาล',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอเหล่าเสือโก้ก',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอสว่างวีระวงศ์',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอน้ำขุ่น',
    'ศูนย์การศึกษานอกโรงเรียนจังหวัดยโสธร',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอเมืองยโสธร',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอทรายมูล',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอกุดชุม',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอคำเขื่อนแก้ว',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอป่าติ้ว',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอมหาชนะชัย',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอค้อวัง',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอเลิงนกทา',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอไทยเจริญ',
    'ศูนย์การศึกษานอกโรงเรียนจังหวัดชัยภูมิ',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอเมืองชัยภูมิ',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอบ้านเขว้า',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอคอนสวรรค์',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอเกษตรสมบูรณ์',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอหนองบัวแดง',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอจัตุรัส',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอบำเหน็จณรงค์',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอหนองบัวระเหว',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอเทพสถิต',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอภูเขียว',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอบ้านแท่น',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอแก้งคร้อ',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอคอนสาร',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอภักดีชุมพล',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอเนินสง่า',
    'ศูนย์การศึกษานอกโรงเรียนจังหวัดอำนาจเจริญ',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอเมืองอำนาจเจริญ',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอชานุมาน',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอปทุมราชวงศา',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอพนา',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอเสนางคนิคม',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอหัวตะพาน',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอลืออำนาจ',
    'ศูนย์การศึกษานอกโรงเรียนจังหวัดหนองบัวลำภู',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอเมืองหนองบัวลำภู',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอนากลาง',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอโนนสัง',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอศรีบุญเรือง',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอสุวรรณคูหา',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอนาวัง',
    'ศูนย์การศึกษานอกโรงเรียนจังหวัดขอนแก่น',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอเมืองขอนแก่น',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอบ้านฝาง',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอพระยืน',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอหนองเรือ',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอชุมแพ',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอสีชมพู',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอน้ำพอง',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภออุบลรัตน์',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอกระนวน',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอบ้านไผ่',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอเปือยน้อย',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอพล',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอแวงใหญ่',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอแวงน้อย',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอหนองสองห้อง',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอภูเวียง',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอมัญจาคีรี',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอชนบท',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอเขาสวนกวาง',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอภูผาม่าน',
    'ศูนย์บริการการศึกษานอกโรงเรียนกิ่งอำเภอซำสูง',
    'ศูนย์บริการการศึกษานอกโรงเรียนกิ่งอำเภอโคกโพธิ์ไชย',
    'ศูนย์บริการการศึกษานอกโรงเรียนกิ่งอำเภอหนองนาคำ',
    'ศูนย์บริการการศึกษานอกโรงเรียนกิ่งอำเภอบ้านแฮด',
    'ศูนย์บริการการศึกษานอกโรงเรียนกิ่งอำเภอโนนศิลา',
    'ศูนย์การศึกษานอกโรงเรียนจังหวัดอุดรธานี',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอเมืองอุดรธานี',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอกุดจับ',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอหนองวัวซอ',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอกุมภวาปี',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอโนนสะอาด',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอหนองหาน',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอทุ่งฝน',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอไชยวาน',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอศรีธาตุ',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอวังสามหมอ',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอบ้านดุง',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอบ้านผือ',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอน้ำโสม',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอเพ็ญ',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอสร้างคอม',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอหนองแสง',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอนายูง',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอพิบูลย์รักษ์',
    'ศูนย์บริการการศึกษานอกโรงเรียนกิ่งอำเภอกู่แก้ว',
    'ศูนย์การศึกษานอกโรงเรียนจังหวัดเลย',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอเมืองเลย',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอนาด้วง',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอเชียงคาน',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอปากชม',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอด่านซ้าย',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอนาแห้ว',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอภูเรือ',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอท่าลี่',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอวังสะพุง',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอภูกระดึง',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอภูหลวง',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอผาขาว',
    'ศูนย์บริการการศึกษานอกโรงเรียนกิ่งอำเภอเอราวัณ',
    'ศูนย์การศึกษานอกโรงเรียนจังหวัดหนองคาย',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอเมืองหนองคาย',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอท่าบ่อ',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอบึงกาฬ',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอพรเจริญ',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอโพนพิสัย',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอโซ่พิสัย',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอศรีเชียงใหม่',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอสังคม',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอเซกา',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอปากคาด',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอบึงโขงหลง',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอศรีวิไล',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอบุ่งคล้า',
    'ศูนย์บริการการศึกษานอกโรงเรียนกิ่งอำเภอสระใคร',
    'ศูนย์บริการการศึกษานอกโรงเรียนกิ่งอำเภอเฝ้าไร่',
    'ศูนย์บริการการศึกษานอกโรงเรียนกิ่งอำเภอรัตนวาปี',
    'ศูนย์บริการการศึกษานอกโรงเรียนกิ่งอำเภอโพธิ์ตาก',
    'ศูนย์การศึกษานอกโรงเรียนจังหวัดมหาสารคาม',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอเมืองมหาสารคาม',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอแกดำ',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอโกสุมพิสัย',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอกันทรวิชัย',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอเชียงยืน',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอบรบือ',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอนาเชือก',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอพยัคฆภูมิพิสัย',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอวาปีปทุม',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอนาดูน',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอยางสีสุราช',
    'ศูนย์บริการการศึกษานอกโรงเรียนกิ่งอำเภอกุดรัง',
    'ศูนย์การศึกษานอกโรงเรียนจังหวัดร้อยเอ็ด',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอเมืองร้อยเอ็ด',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอเกษตรวิสัย',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอปทุมรัตต์',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอจตุรพักตรพิมาน',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอธวัชบุรี',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอพนมไพร',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอโพนทอง',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอโพธิ์ชัย',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอหนองพอก',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอเสลภูมิ',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอสุวรรณภูมิ',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอเมืองสรวง',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอโพนทราย',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภออาจสามารถ',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอเมยวดี',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอศรีสมเด็จ',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอจังหาร',
    'ศูนย์บริการการศึกษานอกโรงเรียนกิ่งอำเภอเชียงขวัญ',
    'ศูนย์บริการการศึกษานอกโรงเรียนกิ่งอำเภอหนองฮี',
    'ศูนย์การศึกษานอกโรงเรียนจังหวัดกาฬสินธุ์',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอเมืองกาฬสินธุ์',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอนามน',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอกมลาไสย',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอร่องคำ',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอกุฉินารายณ์',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอเขาวง',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอยางตลาด',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอห้วยเม็ก',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอสหัสขันธ์',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอคำม่วง',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอท่าคันโท',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอหนองกุงศรี',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอสมเด็จ',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอห้วยผึ้ง',
    'ศูนย์บริการการศึกษานอกโรงเรียนกิ่งอำเภอสามชัย',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอนาคู',
    'ศูนย์บริการการศึกษานอกโรงเรียนกิ่งอำเภอดอนจาน',
    'ศูนย์การศึกษานอกโรงเรียนจังหวัดสกลนคร',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอเมืองสกลนคร',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอกุสุมาลย์',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอกุดบาก',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอพรรณานิคม',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอพังโคน',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอวาริชภูมิ',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอนิคมน้ำอูน',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอวานรนิวาส',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอคำตากล้า',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอบ้านม่วง',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภออากาศอำนวย',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอสว่างแดนดิน',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอส่องดาว',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอเต่างอย',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอโคกศรีสุพรรณ',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอเจริญศิลป์',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอโพนนาแก้ว',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอภูพาน',
    'ศูนย์การศึกษานอกโรงเรียนจังหวัดนครพนม',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอเมืองนครพนม',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอปลาปาก',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอท่าอุเทน',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอบ้านแพง',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอธาตุพนม',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอเรณูนคร',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอนาแก',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอศรีสงคราม',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอนาหว้า',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอโพนสวรรค์',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอนาทม',
    'ศูนย์การศึกษานอกโรงเรียนจังหวัดมุกดาหาร',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอเมืองมุกดาหาร',
    'ศูนย์ฝึกและพัฒนาอาชีพราษฎรไทยบริเวณชายแดนมุกดาหาร',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอนิคมคำสร้อย',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอดอนตาล',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอดงหลวง',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอคำชะอี',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอหว้านใหญ่',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอหนองสูง',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอเมืองเชียงใหม่',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอจอมทอง',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอแม่แจ่ม',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอเชียงดาว',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอดอยสะเก็ด',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอแม่แตง',
    'ศูนย์การศึกษานอกโรงเรียนจังหวัดเชียงใหม่',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอแม่ริม',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอสะเมิง',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอฝาง',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอแม่อาย',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอพร้าว',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอสันป่าตอง',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอสันกำแพง',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอสันทราย',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอหางดง',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอฮอด',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอดอยเต่า',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภออมก๋อย',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอสารภี',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอเวียงแหง',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอไชยปราการ',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอแม่วาง',
    'ศูนย์บริการการศึกษานอกโรงเรียนกิ่งอำเภอแม่ออน',
    'ศูนย์บริการการศึกษานอกโรงเรียนกิ่งอำเภอดอยหล่อ',
    'ศูนย์การศึกษานอกโรงเรียนจังหวัดลำพูน',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอเมืองลำพูน',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอแม่ทา',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอบ้านโฮ่ง',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอลี้',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอทุ่งหัวช้าง',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอป่าซาง',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอบ้านธิ',
    'ศูนย์บริการการศึกษานอกโรงเรียนกิ่งอำเภอเวียงหนองล่อง',
    'ศูนย์การศึกษานอกโรงเรียนจังหวัดลำปาง',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอเมืองลำปาง',
    'ศูนย์การศึกษานอกโรงเรียนภาคเหนือ',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอแม่เมาะ',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอเกาะคา',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอเสริมงาม',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภองาว',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอแจ้ห่ม',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอวังเหนือ',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอเถิน',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอแม่พริก',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอแม่ทะ',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอสบปราบ',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอห้างฉัตร',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอเมืองปาน',
    'ศูนย์การศึกษานอกโรงเรียนจังหวัดอุตรดิตถ์',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอเมืองอุตรดิตถ์',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอตรอน',
    'ศูนย์ฝึกและพัฒนาอาชีพราษฎรไทยบริเวณชายแดนอุตรดิตถ์',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอท่าปลา',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอน้ำปาด',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอฟากท่า',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอบ้านโคก',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอพิชัย',
    'ลับแล',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอทองแสนขัน',
    'ศูนย์การศึกษานอกโรงเรียนจังหวัดแพร่',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอเมืองแพร่',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอร้องกวาง',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอลอง',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอสูงเม่น',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอเด่นชัย',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอสอง',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอวังชิ้น',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอหนองม่วงไข่',
    'ศูนย์การศึกษานอกโรงเรียนจังหวัดน่าน',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอเมืองน่าน',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอแม่จริม',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอบ้านหลวง',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอนาน้อย',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอปัว',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอท่าวังผา',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอเวียงสา',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอทุ่งช้าง',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอเชียงกลาง',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอนาหมื่น',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอสันติสุข',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอบ่อเกลือ',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอสองแคว',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอเฉลิมพระเกียรติ',
    'ศูนย์การศึกษานอกโรงเรียนจังหวัดพะเยา',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอเมืองพะเยา',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอจุน',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอเชียงคำ',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอเชียงม่วน',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอดอกคำใต้',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอปง',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอแม่ใจ',
    'ศูนย์บริการการศึกษานอกโรงเรียนกิ่งอำเภอภูซาง',
    'ศูนย์การศึกษานอกโรงเรียนจังหวัดเชียงราย',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอเมืองเชียงราย',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอเวียงชัย',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอเชียงของ',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอเทิง',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอพาน',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอป่าแดด',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอแม่จัน',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอเชียงแสน',
    'ศูนย์ฝึกและพัฒนาอาชีพราษฎรไทยบริเวณชายแดนจังหวัดเชียงราย',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอแม่สาย',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอแม่สรวย',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอเวียงป่าเป้า',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอพญาเม็งราย',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอเวียงแก่น',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอขุนตาล',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอแม่ฟ้าหลวง',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอแม่ลาว',
    'ศูนย์บริการการศึกษานอกโรงเรียนกิ่งอำเภอเวียงเชียงรุ้ง',
    'ศูนย์บริการการศึกษานอกโรงเรียนกิ่งอำเภอดอยหลวง',
    'ศูนย์การศึกษานอกโรงเรียนจังหวัดแม่ฮ่องสอน',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอเมืองแม่ฮ่องสอน',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอขุนยวม',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอปาย',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอแม่สะเรียง',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอแม่ลาน้อย',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอสบเมย',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอปางมะผ้า',
    'ศูนย์การศึกษานอกโรงเรียนจังหวัดนครสวรรค์',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอเมืองนครสวรรค์',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอโกรกพระ',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอชุมแสง',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอหนองบัว',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอบรรพตพิสัย',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอเก้าเลี้ยว',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอตาคลี',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอท่าตะโก',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอไพศาลี',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอพยุหะคีรี',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอลาดยาว',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอตากฟ้า',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอแม่วงก์',
    'ศูนย์บริการการศึกษานอกโรงเรียนกิ่งอำเภอแม่เปิน',
    'ศูนย์การศึกษานอกโรงเรียนจังหวัดอุทัยธานี',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอเมืองอุทัยธานี',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอทัพทัน',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอสว่างอารมณ์',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอหนองฉาง',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอหนองขาหย่าง',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอบ้านไร่',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอลานสัก',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอห้วยคต',
    'ศูนย์การศึกษานอกโรงเรียนจังหวัดกำแพงเพชร',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอเมืองกำแพงเพชร',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอไทรงาม',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอคลองลาน',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอขาณุวรลักษบุรี',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอคลองขลุง',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอพรานกระต่าย',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอลานกระบือ',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอทรายทองวัฒนา',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอปางศิลาทอง',
    'ศูนย์บริการการศึกษานอกโรงเรียนกิ่งอำเภอบึงสามัคคี',
    'ศูนย์บริการการศึกษานอกโรงเรียนกิ่งอำเภอโกสัมพีนคร',
    'ศูนย์การศึกษานอกโรงเรียนจังหวัดตาก',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอเมืองตาก',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอบ้านตาก',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอสามเงา',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอแม่ระมาด',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอท่าสองยาง',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอแม่สอด',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอพบพระ',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภออุ้มผาง',
    'ศูนย์บริการการศึกษานอกโรงเรียนกิ่งอำเภอวังเจ้า',
    'ศูนย์การศึกษานอกโรงเรียนจังหวัดสุโขทัย',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอเมืองสุโขทัย',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอบ้านด่านลานหอย',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอคีรีมาศ',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอกงไกรลาศ',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอศรีสัชนาลัย',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอศรีสำโรง',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอสวรรคโลก',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอศรีนคร',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอทุ่งเสลี่ยม',
    'ศูนย์การศึกษานอกโรงเรียนจังหวัดพิษณุโลก',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอเมืองพิษณุโลก',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอนครไทย',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอชาติตระการ',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอบางระกำ',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอบางกระทุ่ม',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอพรหมพิราม',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอวัดโบสถ์',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอวังทอง',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอเนินมะปราง',
    'ศูนย์การศึกษานอกโรงเรียนจังหวัดพิจิตร',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอเมืองพิจิตร',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอวังทรายพูน',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอโพธิ์ประทับช้าง',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอตะพานหิน',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอบางมูลนาก',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอโพทะเล',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอสามง่าม',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอทับคล้อ',
    'ศูนย์บริการการศึกษานอกโรงเรียนกิ่งอำเภอสากเหล็ก',
    'ศูนย์บริการการศึกษานอกโรงเรียนกิ่งอำเภอบึงนาราง',
    'ศูนย์บริการการศึกษานอกโรงเรียนกิ่งอำเภอดงเจริญ',
    'ศูนย์การศึกษานอกโรงเรียนจังหวัดเพชรบูรณ์',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอเมืองเพชรบูรณ์',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอชนแดน',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอหล่มสัก',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอหล่มเก่า',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอวิเชียรบุรี',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอศรีเทพ',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอหนองไผ่',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอบึงสามพัน',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอน้ำหนาว',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอวังโป่ง',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอเขาค้อ',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอเมืองราชบุรี',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอจอมบึง',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอสวนผึ้ง',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอดำเนินสะดวก',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอบ้านโป่ง',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอบางแพ',
    'ศูนย์การศึกษานอกโรงเรียนจังหวัดราชบุรี',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอโพธาราม',
    'ศูนย์การศึกษานอกโรงเรียนภาคกลาง',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอปากท่อ',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอวัดเพลง',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอเมืองกาญจนบุรี',
    'ศูนย์ฝึกและพัฒนาอาชีพราษฎรไทยบริเวณชายแดนจังหวัดกาญจนบุรี"สามสงฆ์ทรงพระคุณ"',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอไทรโยค',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอบ่อพลอย',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอศรีสวัสดิ์',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอท่ามะกา',
    'ศูนย์การศึกษานอกโรงเรียนจังหวัดกาญจนบุรี',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอท่าม่วง',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอทองผาภูมิ',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอสังขละบุรี',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอพนมทวน',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอเลาขวัญ',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอด่านมะขามเตี้ย',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอหนองปรือ',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอห้วยกระเจา',
    'ศูนย์การศึกษานอกโรงเรียนจังหวัดสุพรรณบุรี',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอเมืองสุพรรณบุรี',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอเดิมบางนางบวช',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอด่านช้าง',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอบางปลาม้า',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอศรีประจันต์',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอดอนเจดีย์',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอสองพี่น้อง',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอสามชุก',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภออู่ทอง',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอหนองหญ้าไซ',
    'ศูนย์การศึกษานอกโรงเรียนจังหวัดนครปฐม',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอเมืองนครปฐม',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอกำแพงแสน',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอนครชัยศรี',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอดอนตูม',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอบางเลน',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอสามพราน',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอพุทธมณฑล',
    'ศูนย์การศึกษานอกโรงเรียนกาญจนาภิเษก',
    'ศูนย์การศึกษานอกโรงเรียนจังหวัดสมุทรสาคร',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอเมืองสมุทรสาคร',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอกระทุ่มแบน',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอบ้านแพ้ว',
    'ศูนย์การศึกษานอกโรงเรียนจังหวัดสมุทรสงคราม',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอเมืองสมุทรสงคราม',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอบางคนที',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภออัมพวา',
    'ศูนย์การศึกษานอกโรงเรียนจังหวัดเพชรบุรี',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอเมืองเพชรบุรี',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอเขาย้อย',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอหนองหญ้าปล้อง',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอชะอำ',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอท่ายาง',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอบ้านลาด',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอบ้านแหลม',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอแก่งกระจาน',
    'ศูนย์การศึกษานอกโรงเรียนจังหวัดประจวบคีรีขันธ์',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอเมืองประจวบคีรีขันธ์',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอกุยบุรี',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอทับสะแก',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอบางสะพาน',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอบางสะพานน้อย',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอปราณบุรี',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอหัวหิน',
    'ศูนย์บริการการศึกษานอกโรงเรียนกิ่งอำเภอสามร้อยยอด',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอเมืองนครศรีธรรมราช',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอพรหมคีรี',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอลานสกา',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอฉวาง',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอพิปูน',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอเชียรใหญ่',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอชะอวด',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอท่าศาลา',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอทุ่งสง',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอนาบอน',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอทุ่งใหญ่',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอปากพนัง',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอร่อนพิบูลย์',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอสิชล',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอขนอม',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอหัวไทร',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอบางขัน',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอถ้ำพรรณรา',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอจุฬาภรณ์',
    'ศูนย์การศึกษานอกโรงเรียนจังหวัดนครศรีธรรมราช',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอพระพรหม',
    'ศูนย์บริการการศึกษานอกโรงเรียนกิ่งอำเภออำเภอนบพิตำ',
    'ศูนย์บริการการศึกษานอกโรงเรียนกิ่งอำเภอช้างกลาง',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอเฉลิมพระเกียรติ',
    'ศูนย์การศึกษานอกโรงเรียนจังหวัดกระบี่',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอเมืองกระบี่',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอเขาพนม',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอเกาะลันตา',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอคลองท่อม',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภออ่าวลึก',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอปลายพระยา',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอลำทับ',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอเหนือคลอง',
    'ศูนย์การศึกษานอกโรงเรียนจังหวัดพังงา',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอเมืองพังงา',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอเกาะยาว',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอกะปง',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอตะกั่วทุ่ง',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอตะกั่วป่า',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอคุระบุรี',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอทับปุด',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอท้ายเหมือง',
    'ศูนย์การศึกษานอกโรงเรียนจังหวัดภูเก็ต',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอเมืองภูเก็ต',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอกะทู้',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอถลาง',
    'ศูนย์การศึกษานอกโรงเรียนจังหวัดสุราษฎร์ธานี',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอเมืองสุราษฏร์ธานี',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอกาญจนดิษฐ์',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอดอนสัก',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอเกาะสมุย',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอเกาะพะงัน',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอไชยา',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอท่าชนะ',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอคีรีรัฐนิคม',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอบ้านตาขุน',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอพนม',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอท่าฉาง',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอบ้านนาสาร',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอบ้านนาเดิม',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอเคียนซา',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอเวียงสระ',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอพระแสง',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอพุนพิน',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอชัยบุรี',
    'ศูนย์บริการการศึกษานอกโรงเรียนกิ่งอำเภอวิภาวดี',
    'ศูนย์การศึกษานอกโรงเรียนจังหวัดระนอง',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอเมืองระนอง',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอละอุ่น',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอกะเปอร์',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอกระบุรี',
    'ศูนย์บริการการศึกษานอกโรงเรียนกิ่งอำเภอสุขสำราญ',
    'ศูนย์การศึกษานอกโรงเรียนจังหวัดชุมพร',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอเมืองชุมพร',
    'ศูนย์ฝึกและพัฒนาอาชีพราษฎรไทยบริเวณชายแดนจังหวัดชุมพร',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอท่าแซะ',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอปะทิว',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอหลังสวน',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอละแม',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอพะโต๊ะ',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอสวี',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอทุ่งตะโก',
    'ศูนย์การศึกษานอกโรงเรียนจังหวัดสงขลา',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอเมืองสงขลา',
    'ศูนย์การศึกษานอกโรงเรียนภาคใต้',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอสทิงพระ',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอจะนะ',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอนาทวี',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอเทพา',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอสะบ้าย้อย',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอระโนด',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอกระแสสินธุ์',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอรัตภูมิ',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอสะเดา',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอหาดใหญ่',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอนาหม่อม',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอควนเนียง',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอบางกล่ำ',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอสิงหนคร',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอคลองหอยโข่ง',
    'ศูนย์การศึกษานอกโรงเรียนจังหวัดสตูล',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอเมืองสตูล',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอควนโดน',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอควนกาหลง',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอท่าแพ',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอละงู',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอทุ่งหว้า',
    'ศูนย์บริการการศึกษานอกโรงเรียนกิ่งอำเภอมะนัง',
    'ศูนย์การศึกษานอกโรงเรียนจังหวัดตรัง',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอเมืองตรัง',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอกันตัง',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอย่านตาขาว',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอปะเหลียน',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอสิเกา',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอห้วยยอด',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอวังวิเศษ',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอนาโยง',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอรัษฎา',
    'ศูนย์บริการการศึกษานอกโรงเรียนกิ่งอำเภอสำราญ',
    'ศูนย์การศึกษานอกโรงเรียนจังหวัดพัทลุง',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอเมืองพัทลุง',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอกงหรา',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอเขาชัยสน',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอตะโหมด',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอควนขนุน',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอปากพะยูน',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอศรีบรรพต',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอป่าบอน',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอบางแก้ว',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอป่าพะยอม',
    'ศูนย์บริการการศึกษานอกโรงเรียนกิ่งอำเภอศรีนครินทร์',
    'ศูนย์การศึกษานอกโรงเรียนจังหวัดปัตตานี',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอเมืองปัตตานี',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอโคกโพธิ์',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอหนองจิก',
    'ศูนย์ฝึกและพัฒนาอาชีพราษฎรไทยบริเวณชายแดนจังหวัดปัตตานี',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอปะนาเระ',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอมายอ',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอทุ่งยางแดง',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอสายบุรี',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอไม้แก่น',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอยะหริ่ง',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอยะรัง',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอกะพ้อ',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอแม่ลาน',
    'ศูนย์การศึกษานอกโรงเรียนจังหวัดยะลา',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอเมืองยะลา',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอเบตง',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอบันนังสตา',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอธารโต',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอยะหา',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอรามัน',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอกาบัง',
    'ศูนย์บริการการศึกษานอกโรงเรียนกิ่งอำเภอกรงปินัง',
    'ศูนย์การศึกษานอกโรงเรียนจังหวัดนราธิวาส',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอเมืองนราธิวาส',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอตากใบ',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอบาเจาะ',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอยี่งอ',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอระแงะ',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอรือเสาะ',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอศรีสาคร',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอแว้ง',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอสุคิริน',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอสุไหงโกลก',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอสุไหงปาดี',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอจะแนะ',
    'ศูนย์บริการการศึกษานอกโรงเรียนอำเภอเจาะไอร้อง',
    'กาญจนาภิเษกวิทยาลัยช่างทองหลวง',
    'วิทยาลัยอาชีวศึกษาเสาวภา',
    'วิทยาลัยเทคนิคดุสิต',
    'วิทยาลัยการอาชีพกาญจนาภิเษกหนองจอก',
    'วิทยาลัยสารพัดช่างสี่พระยา',
    'วิทยาลัยศิลปหัตถกรรมกรุงเทพ',
    'วิทยาลัยสารพัดช่างพระนคร',
    'วิทยาลัยพณิชยการบางนา',
    'วิทยาลัยเทคนิคมีนบุรี',
    'วิทยาลัยเทคนิคกาญจนาภิเษกมหานคร',
    'วิทยาลัยอาชีวศึกษาธนบุรี',
    'วิทยาลัยสารพัดช่างธนบุรี',
    'วิทยาลัยพณิชยการธนบุรี',
    'วิทยาลัยพณิชยการเชตุพน',
    'วิทยาลัยสารพัดช่างนครหลวง',
    'วิทยาลัยการอาชีพนวมินทราชูทิศ',
    'วิทยาลัยอาชีวศึกษาเอี่ยมละออ',
    'วิทยาลัยเทคนิคดอนเมือง',
    'วิทยาลัยพณิชยการอินทราชัย',
    'วิทยาลัยบริหารธุรกิจและการท่องเที่ยวกรุงเทพ',
    'วิทยาลัยเทคนิคราชสิทธาราม',
    'วิทยาลัยเทคนิคสมุทรปราการ',
    'วิทยาลัยสารพัดช่างสมุทรปราการ',
    'วิทยาลัยเทคนิคกาญจนาภิเษกสมุทรปราการ',
    'วิทยาลัยการอาชีพพระสมุทรเจดีย์',
    'วิทยาลัยการอาชีพไทรน้อย',
    'วิทยาลัยเทคนิคปทุมธานี',
    'ศูนย์ฝึกอบรมวิศวกรรมเกษตร',
    'วิทยาลัยเทคนิคธัญบุรี',
    'วิทยาลัยเทคนิคพระนครศรีอยุธยา',
    'วิทยาลัยเทคโนโลยีและอุตสาหกรรมการต่อเรือพระนครศรีอยุธยา',
    'วิทยาลัยเทคนิคอุตสาหกรรมยานยนต์',
    'วิทยาลัยอาชีวศึกษาพระนครศรีอยุธยา',
    'วิทยาลัยเกษตรและเทคโนโลยีศูนย์ศิลปาชีพบางไทร',
    'วิทยาลัยการอาชีพเสนา',
    'วิทยาลัยสารพัดช่างพระนครศรีอยุธยา',
    'วิทยาลัยการอาชีพมหาราช',
    'วิทยาลัยเทคนิคอ่างทอง',
    'วิทยาลัยการอาชีพโพธิ์ทอง',
    'วิทยาลัยการอาชีพวิเศษชัยชาญ',
    'วิทยาลัยเทคนิคลพบุรี',
    'วิทยาลัยอาชีวศึกษาลพบุรี',
    'วิทยาลัยสารพัดช่างลพบุรี',
    'วิทยาลัยเกษตรและเทคโนโลยีลพบุรี',
    'วิทยาลัยเทคนิคลพบุรี2',
    'วิทยาลัยการอาชีพโคกสำโรง',
    'วิทยาลัยการอาชีพชัยบาดาล',
    'วิทยาลัยเทคนิคสิงห์บุรี',
    'วิทยาลัยอาชีวศึกษาสิงห์บุรี',
    'วิทยาลัยเกษตรและเทคโนโลยีสิงห์บุรี',
    'วิทยาลัยเทคนิคสิงห์บุรี2',
    'วิทยาลัยการอาชีพอินทร์บุรี',
    'วิทยาลัยเทคนิคชัยนาท',
    'วิทยาลัยเกษตรและเทคโนโลยีชัยนาท',
    'วิทยาลัยการอาชีพเนินขาม',
    'วิทยาลัยเทคนิคสระบุรี',
    'วิทยาลัยอาชีวศึกษาสระบุรี',
    'วิทยาลัยสารพัดช่างสระบุรี',
    'วิทยาลัยการอาชีพหนองแค',
    'วิทยาลัยเทคนิคท่าหลวงซิเมนต์ไทยอนุสรณ์',
    'วิทยาลัยการอาชีพสระบุรี',
    'วิทยาลัยเทคนิคมวกเหล็ก',
    'วิทยาลัยอาชีวศึกษาชลบุรี',
    'วิทยาลัยสารพัดช่างชลบุรี',
    'วิทยาลัยเทคนิคชลบุรี',
    'วิทยาลัยการอาชีพบางละมุง',
    'วิทยาลัยการอาชีพพานทอง',
    'วิทยาลัยการอาชีพพนัสนิคม',
    'วิทยาลัยเทคนิคสัตหีบ',
    'วิทยาลัยเกษตรและเทคโนโลยีชลบุรี',
    'วิทยาลัยเทคนิคระยอง',
    'วิทยาลัยเทคนิคมาบตาพุด',
    'วิทยาลัยสารพัดช่างระยอง',
    'วิทยาลัยการอาชีพแกลง',
    'วิทยาลัยเทคนิคบ้านค่าย',
    'วิทยาลัยเทคนิคจันทบุรี',
    'วิทยาลัยสารพัดช่างจันทบุรี',
    'วิทยาลัยการอาชีพสอยดาว',
    'วิทยาลัยการอาชีพนายายอาม',
    'วิทยาลัยเทคนิคตราด',
    'วิทยาลัยสารพัดช่างตราด',
    'วิทยาลัยการอาชีพบ่อไร่',
    'วิทยาลัยเทคนิคฉะเชิงเทรา',
    'วิทยาลัยอาชีวศึกษาฉะเชิงเทรา',
    'วิทยาลัยสารพัดช่างฉะเชิงเทรา',
    'วิทยาลัยการอาชีพบางปะกง',
    'วิทยาลัยเทคนิคจุฬาภรณ์',
    'วิทยาลัยเกษตรและเทคโนโลยีฉะเชิงเทรา',
    'วิทยาลัยการอาชีพพนมสารคาม',
    'วิทยาลัยเทคนิคปราจีนบุรี',
    'วิทยาลัยสารพัดช่างปราจีนบุรี',
    'วิทยาลัยการอาชีพกบินทร์บุรี',
    'วิทยาลัยเทคนิคบูรพาปราจีน',
    'วิทยาลัยเทคนิคนครนายก',
    'วิทยาลัยการอาชีพนครนายก',
    'วิทยาลัยการอาชีพองครักษ์',
    'วิทยาลัยการอาชีพวังน้ำเย็น',
    'วิทยาลัยเทคนิคสระแก้ว',
    'วิทยาลัยเกษตรและเทคโนโลยีสระแก้ว',
    'วิทยาลัยเทคนิคนครราชสีมา',
    'วิทยาลัยอาชีวศึกษานครราชสีมา',
    'วิทยาลัยบริหารธุรกิจและการท่องเที่ยวนครราชสีมา',
    'วิทยาลัยสารพัดช่างนครราชสีมา',
    'วิทยาลัยเกษตรและเทคโนโลยีนครราชสีมา',
    'วิทยาลัยเทคนิคสุรนารี',
    'วิทยาลัยเทคนิคหลวงพ่อคูณ ปริสุทฺโธ',
    'วิทยาลัยการอาชีพบัวใหญ่',
    'วิทยาลัยเทคนิคปักธงชัย',
    'วิทยาลัยการอาชีพพิมาย',
    'วิทยาลัยการอาชีพชุมพวง',
    'วิทยาลัยการอาชีพปากช่อง',
    'วิทยาลัยเทคนิคบุรีรัมย์',
    'วิทยาลัยเกษตรและเทคโนโลยีบุรีรัมย์',
    'วิทยาลัยสารพัดช่างบุรีรัมย์',
    'วิทยาลัยเทคนิคคูเมือง',
    'วิทยาลัยการอาชีพนางรอง',
    'วิทยาลัยการอาชีพสตึก',
    'วิทยาลัยเทคนิคสุรินทร์',
    'วิทยาลัยอาชีวศึกษาสุรินทร์',
    'วิทยาลัยสารพัดช่างสุรินทร์',
    'วิทยาลัยการอาชีพท่าตูม',
    'วิทยาลัยการอาชีพปราสาท',
    'วิทยาลัยการอาชีพศีขรภูมิ',
    'วิทยาลัยการอาชีพสังขะ',
    'วิทยาลัยเทคนิคศรีสะเกษ',
    'วิทยาลัยเกษตรและเทคโนโลยีศรีสะเกษ',
    'วิทยาลัยการอาชีพศรีสะเกษ',
    'วิทยาลัยสารพัดช่างศรีสะเกษ',
    'วิทยาลัยการอาชีพกันทรลักษ์',
    'วิทยาลัยการอาชีพขุนหาญ',
    'วิทยาลัยอาชีวศึกษาอุบลราชธานี',
    'วิทยาลัยเกษตรและเทคโนโลยีอุบลราชธานี',
    'วิทยาลัยเทคนิคอุบลราชธานี',
    'วิทยาลัยสารพัดช่างอุบลราชธานี',
    'วิทยาลัยการอาชีพเขมราฐ',
    'วิทยาลัยเทคนิคเดชอุดม',
    'วิทยาลัยการอาชีพตระการพืชผล',
    'วิทยาลัยการอาชีพวารินชำราบ',
    'วิทยาลัยเทคนิคยโสธร',
    'วิทยาลัยเกษตรและเทคโนโลยียโสธร',
    'วิทยาลัยการอาชีพเลิงนกทา',
    'วิทยาลัยเทคนิคชัยภูมิ',
    'วิทยาลัยเกษตรและเทคโนโลยีชัยภูมิ',
    'วิทยาลัยสารพัดช่างชัยภูมิ',
    'วิทยาลัยการอาชีพบำเหน็จณรงค์',
    'วิทยาลัยการอาชีพแก้งคร้อ',
    'วิทยาลัยเทคนิคอำนาจเจริญ',
    'วิทยาลัยการอาชีพหัวตะพาน',
    'วิทยาลัยเทคนิคหนองบัวลำภู',
    'วิทยาลัยการอาชีพศรีบุญเรือง',
    'วิทยาลัยเทคนิคขอนแก่น',
    'วิทยาลัยอาชีวศึกษาขอนแก่น',
    'วิทยาลัยการอาชีพบ้านไผ่',
    'วิทยาลัยสารพัดช่างขอนแก่น',
    'วิทยาลัยการอาชีพชุมแพ',
    'วิทยาลัยเทคนิคน้ำพอง',
    'วิทยาลัยการอาชีพกระนวน',
    'วิทยาลัยการอาชีพพล',
    'วิทยาลัยเกษตรและเทคโนโลยีขอนแก่น',
    'วิทยาลัยการอาชีพขอนแก่น',
    'วิทยาลัยเทคนิคอุดรธานี',
    'วิทยาลัยอาชีวศึกษาอุดรธานี',
    'วิทยาลัยเกษตรและเทคโนโลยีอุดรธานี',
    'วิทยาลัยการอาชีพหนองหาน',
    'วิทยาลัยสารพัดช่างอุดรธานี',
    'วิทยาลัยการอาชีพกุมภวาปี',
    'วิทยาลัยเทคนิคกาญจนาภิเษกอุดรธานี',
    'วิทยาลัยการอาชีพบ้านผือ',
    'วิทยาลัยบริหารธุรกิจและการท่องเที่ยวอุดรธานี',
    'วิทยาลัยเทคนิคเลย',
    'วิทยาลัยอาชีวศึกษาเลย',
    'วิทยาลัยการอาชีพด่านซ้าย',
    'วิทยาลัยการอาชีพวังสะพุง',
    'วิทยาลัยเทคนิคหนองคาย',
    'วิทยาลัยเทคโนโลยีและอุตสาหกรรมการต่อเรือหนองคาย',
    'วิทยาลัยอาชีวศึกษาหนองคาย',
    'วิทยาลัยการอาชีพบึงกาฬ',
    'วิทยาลัยการอาชีพเซกา',
    'วิทยาลัยเทคนิคมหาสารคาม',
    'วิทยาลัยอาชีวศึกษามหาสารคาม',
    'วิทยาลัยเกษตรและเทคโนโลยีมหาสารคาม',
    'วิทยาลัยสารพัดช่างมหาสารคาม',
    'วิทยาลัยการอาชีพพยัคฆภูมิพิสัย',
    'วิทยาลัยการอาชีพวาปีปทุม',
    'วิทยาลัยเทคนิคร้อยเอ็ด',
    'วิทยาลัยอาชีวศึกษาร้อยเอ็ด',
    'วิทยาลัยการอาชีพร้อยเอ็ด',
    'วิทยาลัยการอาชีพเกษตรวิสัย',
    'วิทยาลัยเกษตรและเทคโนโลยีร้อยเอ็ด',
    'วิทยาลัยการอาชีพพนมไพร',
    'วิทยาลัยการอาชีพโพนทอง',
    'วิทยาลัยเทคนิคสุวรรณภูมิ',
    'วิทยาลัยเทคนิคกาฬสินธุ์',
    'วิทยาลัยสารพัดช่างกาฬสินธุ์',
    'วิทยาลัยเทคนิคเขาวง',
    'วิทยาลัยการอาชีพคำม่วง',
    'วิทยาลัยการอาชีพหนองกุงศรี',
    'วิทยาลัยการอาชีพห้วยผึ้ง',
    'วิทยาลัยเทคนิคสกลนคร',
    'วิทยาลัยสารพัดช่างสกลนคร',
    'วิทยาลัยการอาชีพพรรณานิคม',
    'วิทยาลัยการอาชีพสว่างแดนดิน',
    'วิทยาลัยสารพัดช่างนครพนม',
    'วิทยาลัยเทคนิคบ้านแพง',
    'วิทยาลัยการอาชีพนวมินทราชินีมุกดาหาร',
    'วิทยาลัยการอาชีพนิคมคำสร้อย',
    'วิทยาลัยเทคนิคเชียงใหม่',
    'วิทยาลัยอาชีวศึกษาเชียงใหม่',
    'วิทยาลัยสารพัดช่างเชียงใหม่',
    'วิทยาลัยการอาชีพจอมทอง',
    'วิทยาลัยการอาชีพฝาง',
    'วิทยาลัยเกษตรและเทคโนโลยีเชียงใหม่',
    'วิทยาลัยเทคนิคสันกำแพง',
    'วิทยาลัยเทคนิคลำพูน',
    'วิทยาลัยเกษตรและเทคโนโลยีลำพูน',
    'วิทยาลัยการอาชีพบ้านโฮ่ง',
    'วิทยาลัยการอาชีพป่าซาง',
    'วิทยาลัยเทคนิคลำปาง',
    'วิทยาลัยอาชีวศึกษาลำปาง',
    'วิทยาลัยสารพัดช่างลำปาง',
    'วิทยาลัยการอาชีพเกาะคา',
    'วิทยาลัยการอาชีพเถิน',
    'วิทยาลัยเทคนิคอุตรดิตถ์',
    'วิทยาลัยอาชีวศึกษาอุตรดิตถ์',
    'วิทยาลัยสารพัดช่างอุตรดิตถ์',
    'วิทยาลัยการอาชีพพิชัย',
    'วิทยาลัยเทคนิคแพร่',
    'วิทยาลัยอาชีวศึกษาแพร่',
    'วิทยาลัยการอาชีพลอง',
    'วิทยาลัยสารพัดช่างแพร่',
    'วิทยาลัยเกษตรและเทคโนโลยีแพร่',
    'วิทยาลัยการอาชีพสอง',
    'วิทยาลัยเทคนิคน่าน',
    'วิทยาลัยสารพัดช่างน่าน',
    'วิทยาลัยการอาชีพปัว',
    'วิทยาลัยการอาชีพเวียงสา',
    'วิทยาลัยเทคนิคพะเยา',
    'วิทยาลัยเกษตรและเทคโนโลยีพะเยา',
    'วิทยาลัยการอาชีพเชียงคำ',
    'วิทยาลัยการอาชีพดอกคำใต้',
    'วิทยาลัยเทคนิคเชียงราย',
    'วิทยาลัยเทคนิคกาญจนาภิเษกเชียงราย',
    'วิทยาลัยอาชีวศึกษาเชียงราย',
    'วิทยาลัยเกษตรและเทคโนโลยีเชียงราย',
    'วิทยาลัยการอาชีพเชียงราย',
    'วิทยาลัยการอาชีพเวียงเชียงรุ้ง',
    'วิทยาลัยการอาชีพเทิง',
    'วิทยาลัยการอาชีพนวมินทราชินีแม่ฮ่องสอน',
    'วิทยาลัยการอาชีพแม่สะเรียง',
    'วิทยาลัยเทคนิคนครสวรรค์',
    'วิทยาลัยอาชีวศึกษานครสวรรค์',
    'วิทยาลัยเกษตรและเทคโนโลยีนครสวรรค์',
    'วิทยาลัยการอาชีพนครสวรรค์',
    'วิทยาลัยการอาชีพบรรพตพิสัย',
    'วิทยาลัยเทคนิคแม่วงก์',
    'วิทยาลัยเทคนิคอุทัยธานี',
    'วิทยาลัยเกษตรและเทคโนโลยีอุทัยธานี',
    'วิทยาลัยสารพัดช่างอุทัยธานี',
    'วิทยาลัยเทคนิคกำแพงเพชร',
    'วิทยาลัยเกษตรและเทคโนโลยีกำแพงเพชร',
    'วิทยาลัยสารพัดช่างกำแพงเพชร',
    'วิทยาลัยการอาชีพขาณุวรลักษบุรี',
    'วิทยาลัยเกษตรและเทคโนโลยีตาก',
    'วิทยาลัยสารพัดช่างตาก',
    'วิทยาลัยการอาชีพบ้านตาก',
    'วิทยาลัยการอาชีพแม่สอด',
    'วิทยาลัยเทคนิคสุโขทัย',
    'วิทยาลัยอาชีวศึกษาสุโขทัย',
    'วิทยาลัยสารพัดช่างสุโขทัย',
    'วิทยาลัยการอาชีพศรีสัชนาลัย',
    'วิทยาลัยการอาชีพศรีสำโรง',
    'วิทยาลัยเกษตรและเทคโนโลยีสุโขทัย',
    'วิทยาลัยเทคนิคพิษณุโลก',
    'วิทยาลัยเทคนิคสองแคว',
    'วิทยาลัยอาชีวศึกษาพิษณุโลก',
    'วิทยาลัยพณิชยการบึงพระพิษณุโลก',
    'วิทยาลัยสารพัดช่างพิษณุโลก',
    'วิทยาลัยการอาชีพนครไทย',
    'วิทยาลัยเทคนิคพิจิตร',
    'วิทยาลัยสารพัดช่างพิจิตร',
    'วิทยาลัยเกษตรและเทคโนโลยีพิจิตร',
    'วิทยาลัยเทคนิคเพชรบูรณ์',
    'วิทยาลัยเกษตรและเทคโนโลยีเพชรบูรณ์',
    'วิทยาลัยสารพัดช่างเพชรบูรณ์',
    'วิทยาลัยการอาชีพชนแดน',
    'วิทยาลัยการอาชีพวิเชียรบุรี',
    'วิทยาลัยเทคนิคราชบุรี',
    'วิทยาลัยสารพัดช่างราชบุรี',
    'วิทยาลัยเทคนิคราชบุรี2',
    'วิทยาลัยการอาชีพบ้านโป่ง',
    'วิทยาลัยเทคนิคโพธาราม',
    'วิทยาลัยเกษตรและเทคโนโลยีราชบุรี',
    'วิทยาลัยการอาชีพปากท่อ',
    'วิทยาลัยเทคนิคกาญจนบุรี',
    'วิทยาลัยอาชีวศึกษากาญจนบุรี',
    'วิทยาลัยเกษตรและเทคโนโลยีกาญจนบุรี',
    'วิทยาลัยสารพัดช่างกาญจนบุรี',
    'วิทยาลัยการอาชีพกาญจนบุรี',
    'วิทยาลัยการอาชีพพนมทวน',
    'วิทยาลัยเทคนิคสุพรรณบุรี',
    'วิทยาลัยอาชีวศึกษาสุพรรณบุรี',
    'วิทยาลัยสารพัดช่างบรรหาร-แจ่มใส',
    'วิทยาลัยเกษตรและเทคโนโลยีสุพรรณบุรี',
    'วิทยาลัยการอาชีพสองพี่น้อง',
    'วิทยาลัยการอาชีพอู่ทอง',
    'วิทยาลัยเทคนิคนครปฐม',
    'วิทยาลัยอาชีวศึกษานครปฐม',
    'วิทยาลัยสารพัดช่างนครปฐม',
    'วิทยาลัยการอาชีพบางแก้วฟ้า',
    'วิทยาลัยการอาชีพนครปฐม',
    'วิทยาลัยการอาชีพพุทธมณฑล',
    'วิทยาลัยเทคนิคสมุทรสาคร',
    'วิทยาลัยการอาชีพบ้านแพ้ว',
    'วิทยาลัยเทคนิคสมุทรสงคราม',
    'วิทยาลัยสารพัดช่างสมุทรสงคราม',
    'วิทยาลัยการอาชีพอัมพวา',
    'วิทยาลัยเทคนิคเพชรบุรี',
    'วิทยาลัยอาชีวศึกษาเพชรบุรี',
    'วิทยาลัยสารพัดช่างเพชรบุรี',
    'วิทยาลัยการอาชีพเขาย้อย',
    'วิทยาลัยเกษตรและเทคโนโลยีเพชรบุรี',
    'วิทยาลัยการอาชีพบ้านลาด',
    'วิทยาลัยเทคนิคประจวบคีรีขันธ์',
    'วิทยาลัยการอาชีพบางสะพาน',
    'วิทยาลัยการอาชีพปราณบุรี',
    'วิทยาลัยการอาชีพวังไกลกังวล',
    'วิทยาลัยเทคนิคนครศรีธรรมราช',
    'วิทยาลัยอาชีวศึกษานครศรีธรรมราช',
    'วิทยาลัยศิลปหัตถกรรมนครศรีธรรมราช',
    'วิทยาลัยสารพัดช่างนครศรีธรรมราช',
    'วิทยาลัยการอาชีพพรหมคีรี',
    'วิทยาลัยเทคนิคทุ่งสง',
    'วิทยาลัยเทคโนโลยีและอุตสาหกรรมการต่อเรือนครศรีธรรมราช',
    'วิทยาลัยเทคนิคสิชล',
    'วิทยาลัยการอาชีพหัวไทร',
    'วิทยาลัยการอาชีพนครศรีธรรมราช',
    'วิทยาลัยเกษตรและเทคโนโลยีนครศรีธรรมราช',
    'วิทยาลัยเทคนิคกระบี่',
    'วิทยาลัยสารพัดช่างกระบี่',
    'วิทยาลัยการอาชีพคลองท่อม',
    'วิทยาลัยการอาชีพอ่าวลึก',
    'วิทยาลัยเกษตรและเทคโนโลยีกระบี่',
    'วิทยาลัยเทคนิคพังงา',
    'วิทยาลัยเกษตรและเทคโนโลยีพังงา',
    'วิทยาลัยการอาชีพตะกั่วป่า',
    'วิทยาลัยการอาชีพท้ายเหมือง',
    'วิทยาลัยเทคนิคภูเก็ต',
    'วิทยาลัยอาชีวศึกษาภูเก็ต',
    'วิทยาลัยสารพัดช่างภูเก็ต',
    'วิทยาลัยเทคนิคถลาง',
    'วิทยาลัยเทคนิคสุราษฎร์ธานี',
    'วิทยาลัยเทคนิคสุราษฎร์ธานี2',
    'วิทยาลัยอาชีวศึกษาสุราษฎร์ธานี',
    'วิทยาลัยสารพัดช่างสุราษฎร์ธานี',
    'วิทยาลัยการอาชีพไชยา',
    'วิทยาลัยเกษตรและเทคโนโลยีสุราษฎร์ธานี',
    'วิทยาลัยการอาชีพเวียงสระ',
    'วิทยาลัยเทคนิคระนอง',
    'วิทยาลัยเกษตรและเทคโนโลยีระนอง',
    'วิทยาลัยการอาชีพกระบุรี',
    'วิทยาลัยเทคนิคชุมพร',
    'วิทยาลัยสารพัดช่างชุมพร',
    'วิทยาลัยการอาชีพท่าแซะ',
    'วิทยาลัยประมงชุมพรเขตรอุดมศักดิ์',
    'วิทยาลัยการอาชีพหลังสวน',
    'วิทยาลัยเกษตรและเทคโนโลยีชุมพร',
    'วิทยาลัยอาชีวศึกษาสงขลา',
    'วิทยาลัยประมงติณสูลานนท์',
    'วิทยาลัยสารพัดช่างสงขลา',
    'วิทยาลัยการอาชีพสมเด็จเจ้าพะโคะ',
    'วิทยาลัยเทคนิคจะนะ',
    'วิทยาลัยการอาชีพนาทวี',
    'วิทยาลัยเกษตรและเทคโนโลยีสงขลา',
    'วิทยาลัยเทคนิคหาดใหญ่',
    'วิทยาลัยการอาชีพหลวงประธานราษฎร์นิกร',
    'วิทยาลัยเทคนิคสตูล',
    'วิทยาลัยเกษตรและเทคโนโลยีสตูล',
    'วิทยาลัยการอาชีพละงู',
    'วิทยาลัยเทคนิคตรัง',
    'วิทยาลัยเกษตรและเทคโนโลยีตรัง',
    'วิทยาลัยการอาชีพตรัง',
    'วิทยาลัยสารพัดช่างตรัง',
    'วิทยาลัยการอาชีพกันตัง',
    'วิทยาลัยการอาชีพปะเหลียน',
    'วิทยาลัยการอาชีพห้วยยอด',
    'วิทยาลัยเทคนิคพัทลุง',
    'วิทยาลัยเกษตรและเทคโนโลยีพัทลุง',
    'วิทยาลัยสารพัดช่างพัทลุง',
    'วิทยาลัยการอาชีพควนขนุน',
    'วิทยาลัยการอาชีพบางแก้ว',
    'วิทยาลัยเทคนิคพัทลุง2',
    'วิทยาลัยเทคนิคปัตตานี',
    'วิทยาลัยอาชีวศึกษาปัตตานี',
    'วิทยาลัยการอาชีพปัตตานี',
    'วิทยาลัยเทคนิคกาญจนาภิเษกปัตตานี',
    'วิทยาลัยประมงปัตตานี',
    'วิทยาลัยการอาชีพสายบุรี',
    'วิทยาลัยเทคนิคยะลา',
    'วิทยาลัยอาชีวศึกษายะลา',
    'วิทยาลัยสารพัดช่างยะลา',
    'วิทยาลัยการอาชีพเบตง',
    'วิทยาลัยการอาชีพรามัน',
    'วิทยาลัยสารพัดช่างนราธิวาส',
    'วิทยาลัยการอาชีพสุไหงโก-ลก',
    'มหาวิทยาลัยมหาจุฬาลงกรณราชวิทยาลัย วิทยาลัยดองกุก ชอนบอบ',
    'สาธิตมหาวิทยาลัยราชภัฏสวนดุสิต(ละอออุทิศ)',
    'สาธิตมหาวิทยาลัยราชภัฏสวนสุนันทา',
    'สาธิตมหาวิทยาลัยเกษตรศาสตร์บางเขน',
    'สาธิตมหาวิทยาลัยราชภัฏพระนคร',
    'สาธิตมหาวิทยาลัยรามคำแหง(ฝ่ายประถม)',
    'สาธิตมหาวิทยาลัยรามคำแหง(ฝ่ายมัธยม)',
    'สาธิตจุฬาลงกรณ์มหาวิทยาลัย(ฝ่ายประถม)',
    'สาธิตจุฬาลงกรณ์มหาวิทยาลัย(ฝ่ายมัธยม)',
    'สาธิตมหาวิทยาลัยศรีนครินทรวิโรฒปทุมวัน',
    'จุฬาลงกรณ์มหาวิทยาลัย',
    'มหาวิทยาลัยเกษตรศาสตร์',
    'มหาวิทยาลัยธรรมศาสตร์',
    'มหาวิทยาลัยศิลปากร',
    'มหาวิทยาลัยศรีนครินทรวิโรฒ',
    'สถาบันบัณฑิตพัฒนบริหารศาสตร์',
    'สถาบันเทคโนโลยีพระจอมเกล้าพระนครเหนือ',
    'สถาบันเทคโนโลยีพระจอมเกล้าเจ้าคุณทหารลาดกระบัง',
    'มหาวิทยาลัยมหาจุฬาลงกรณราชวิทยาลัย',
    'มหาวิทยาลัยมหามกุฏราชวิทยาลัย',
    'สถาบันเทคโนโลยีปทุมวัน',
    'สาธิตมหาวิทยาลัยราชภัฏบ้านสมเด็จเจ้าพระยา',
    'มหาวิทยาลัยราชภัฏธนบุรี',
    'มหาวิทยาลัยราชภัฏพระนคร',
    'มหาวิทยาลัยราชภัฏสวนดุสิต',
    'มหาวิทยาลัยราชภัฏสวนสุนันทา',
    'มหาวิทยาลัยราชภัฏบ้านสมเด็จเจ้าพระยา',
    'มหาวิทยาลัยราชภัฏจันทรเกษม',
    'สาธิตมหาวิทยาลัยราชภัฏจันทรเกษม',
    'มหาวิทยาลัยเทคโนโลยีราชมงคลกรุงเทพ',
    'มหาวิทยาลัยเทคโนโลยีราชมงคลกรุงเทพ วิทยาเขตเทคนิคกรุงเทพฯ',
    'มหาวิทยาลัยเทคโนโลยีราชมงคลกรุงเทพ  วิทยาเขตบพิตรพิมุข มหาเมฆ',
    'มหาวิทยาลัยเทคโนโลยีราชมงคลกรุงเทพ  วิทยาเขตพระนครใต้',
    'มหาวิทยาลัยเทคโนโลยีราชมงคลตะวันออก  วิทยาเขตจักรพงษภูนารถ',
    'มหาวิทยาลัยเทคโนโลยีราชมงคลตะวันออก  วิทยาเขตอุเทนถวาย',
    'มหาวิทยาลัยเทคโนโลยีราชมงคลพระนคร',
    'มหาวิทยาลัยเทคโนโลยีราชมงคลพระนคร  วิทยาเขตเทเวศร์',
    'มหาวิทยาลัยเทคโนโลยีราชมงคลพระนคร  วิทยาเขตโชติเวช',
    'มหาวิทยาลัยเทคโนโลยีราชมงคลพระนคร  วิทยาเขตพณิชยการพระนคร',
    'มหาวิทยาลัยเทคโนโลยีราชมงคลพระนคร  วิทยาเขตพระนครเหนือ',
    'มหาวิทยาลัยเทคโนโลยีราชมงคลรัตนโกสินทร์  วิทยาเขตเพาะช่าง',
    'มหาวิทยาลัยเทคโนโลยีราชมงคลรัตนโกสินทร์  วิทยาเขตบพิตรพิมุข จักรวรรดิ',
    'สาธิตมหาวิทยาลัยศรีนครินทรวิโรฒประสานมิตร(ฝ่ายประถม)',
    'สาธิตมหาวิทยาลัยศรีนครินทรวิโรฒประสานมิตร(ฝ่ายมัธยม)',
    'มหาวิทยาลัยรามคำแหง',
    'มหาวิทยาลัยเทคโนโลยีพระจอมเกล้าธนบุรี',
    'มหาวิทยาลัยกรุงเทพ',
    'มหาวิทยาลัยศรีปทุม',
    'มหาวิทยาลัยหอการค้าไทย',
    'มหาวิทยาลัยเกริก',
    'มหาวิทยาลัยเซนต์จอห์น',
    'มหาวิทยาลัยเทคโนโลยีมหานคร',
    'มหาวิทยาลัยอัสสัมชัญ',
    'มหาวิทยาลัยธนบุรี',
    'มหาวิทยาลัยธุรกิจบัณฑิตย์',
    'มหาวิทยาลัยเอเชียอาคเนย์',
    'มหาวิทยาลัยรัตนบัณฑิต',
    'มหาวิทยาลัยเกษมบัณฑิต',
    'มหาวิทยาลัยสยาม',
    'วิทยาลัยกรุงเทพธนบุรี',
    'วิทยาลัยเซนต์หลุยส์',
    'วิทยาลัยเซาธ์อีสท์บางกอก',
    'วิทยาลัยดุสิตธานี',
    'วิทยาลัยทองสุข',
    'วิทยาลัยนอร์ทกรุงเทพ',
    'วิทยาลัยมิชชัน',
    'วิทยาลัยรัชต์ภาคย์',
    'วิทยาลัยกรุงเทพสุวรรณภูมิ',
    'วิทยาลัยเทคโนโลยีสยาม',
    'สถาบันบัณฑิตศึกษาจุฬาภรณ์',
    'สถาบันเทคโนโลยีไทย-ญี่ปุ่น',
    'สถาบันอาศรมศิลป์',
    'วิทยาลัยชุมชน',
    'มหาวิทยาลัยหัวเฉียวเฉลิมพระเกียรติ',
    'สถาบันเทคโนโลยีปัญญาภิวัฒน์',
    'มหาวิทยาลัยเทคโนโลยีราชมงคลสุวรรณภูมิ  วิทยาเขตนนทบุรี',
    'มหาวิทยาลัยสุโขทัยธรรมาธิราช',
    'วิทยาลัยราชพฤกษ์',
    'สาธิตมหาวิทยาลัยราชภัฏวไลยอลงกรณ์(ในพระบรมราชูปถัมภ์)',
    'สาธิตมหาวิทยาลัยเทคโนโลยีราชมงคล',
    'มหาวิทยาลัยธรรมศาสตร์ ศูนยรังสิต',
    'มหาวิทยาลัยราชภัฏวไลอลงกรณ์ ในพระบรมราชูปถัมภ์',
    'มหาวิทยาลัยเทคโนโลยีราชมงคลธัญบุรี',
    'มหาวิทยาลัยเทคโนโลยีราชมงคลธัญบุรี วิทยาเขตปทุมธานี',
    'มหาวิทยาลัยกรุงเทพ วิทยาเขตรังสิต',
    'มหาวิทยาลัยรังสิต',
    'มหาวิทยาลัยชินวัตร',
    'มหาวิทยาลัยอีสเทิร์นเอเชีย',
    'มหาวิทยาลัยปทุมธานี',
    'สถาบันเทคโนโลยีแห่งอโยธยา',
    'สาธิตมหาวิทยาลัยราชภัฏพระนครศรีอยุธยา',
    'มหาวิทยาลัยมหามกุฏราชวิทยาลัย วิทยาเขตมหาวชิราลงกรณราชวิทยาลัย',
    'มหาวิทยาลัยราชภัฏพระนครศรีอยุธยา',
    'มหาวิทยาลัยเทคโนโลยีราชมงคลสุวรรณภูมิ',
    'มหาวิทยาลัยเทคโนโลยีราชมงคลสุวรรณภูมิ  วิทยาเขตพระนครศรีอยุธยา วาสุกรี',
    'มหาวิทยาลัยเทคโนโลยีราชมงคลสุวรรณภูมิ  วิทยาเขตพระนครศรีอยุธยา หันตรา',
    'สาธิตมหาวิทยาลัยราชภัฏเทพสตรี',
    'มหาวิทยาลัยเกษตรศาสตร์ วิทยาเขตลพบุรี',
    'มหาวิทยาลัยราชภัฏเทพสตรี',
    'วิทยาลัยมิชชั่น มวกเหล็ก',
    'สาธิตมหาวิทยาลัยบูรพา"พิบูลย์บำเพ็ญ"',
    'มหาวิทยาลัยเกษตรศาสตร์ วิทยาเขตศรีราชา',
    'มหาวิทยาลัยธรรมศาสตร์ ศูนยถาวร-อุษา ประภา(ศูนย์พัทยา)',
    'สถาบันบัณฑิตพัฒนบริหารศาสตร์ วิทยาเขตชลบุรี',
    'มหาวิทยาลัยบูรพา',
    'มหาวิทยาลัยเทคโนโลยีราชมงคลตะวันออก',
    'มหาวิทยาลัยเทคโนโลยีราชมงคลตะวันออก  วิทยาเขตบางพระ จังหวัดชลบุรี',
    'มหาวิทยาลัยเทคโนโลยีราชมงคลตะวันออก  คณะเกษตรศาสตร์บางพระ จังหวัดชลบุรี',
    'มหาวิทยาลัยศรีปทุม วิทยาเขตชลบุรี',
    'มหาวิทยาลัยเอเชียน',
    'วิทยาลัยเฉลิมกาญจนา  วิทยาเขตระยอง',
    'วิทยาลัยเฉลิมกาญจนาระยอง',
    'สาธิตมหาวิทยาลัยราชภัฏรำไพพรรณี',
    'มหาวิทยาลัยบูรพา วิทยาเขตสารสนเทศจันทบุรี',
    'มหาวิทยาลัยราชภัฏรำไพพรรณี',
    'มหาวิทยาลัยเทคโนโลยีราชมงคลตะวันออก  วิทยาเขตจันทบุรี',
    'วิทยาลัยชุมชนตราด',
    'สาธิตมหาวิทยาลัยราชภัฏราชนครินทร์',
    'มหาวิทยาลัยราชภัฏราชนครินทร์',
    'สถาบันเทคโนโลยีพระจอมเกล้าพระนครเหนือ วิทยาเขตปราจีนบุรี',
    'มหาวิทยาลัยศรีนครินทรวิโรฒ องครักษ์',
    'วิทยาลัยเซนต์เทเรซา-อินติ',
    'มหาวิทยาลัยบูรพา วิทยาเขตสารสนเทศสระแก้ว',
    'วิทยาลัยชุมชนสระแก้ว',
    'สาธิตมหาวิทยาลัยราชภัฏนครราชสีมา',
    'สถาบันบัณฑิตพัฒนบริหารศาสตร์ วิทยาเขตนครราชสีมา',
    'มหาวิทยาลัยมหาจุฬาลงกรณราชวิทยาลัย วิทยาเขตนครราชสีมา',
    'มหาวิทยาลัยราชภัฏนครราชสีมา',
    'มหาวิทยาลัยเทคโนโลยีราชมงคลอีสาน',
    'มหาวิทยาลัยเทคโนโลยีราชมงคลอีสาน  วิทยาเขตภาคตะวันออกเฉียงเหนือ จังหวัดนครราชสีมา',
    'มหาวิทยาลัยเทคโนโลยีสุรนารี',
    'มหาวิทยาลัยวงษ์ชวลิตกุล',
    'วิทยาลัยนครราชสีมา',
    'สาธิตมหาวิทยาลัยราชภัฏบุรีรัมย์',
    'มหาวิทยาลัยราชภัฏบุรีรัมย์',
    'วิทยาลัยชุมชนบุรีรัมย์',
    'สาธิตมหาวิทยาลัยราชภัฏสุรินทร์',
    'มหาวิทยาลัยมหาจุฬาลงกรณราชวิทยาลัย วิทยาเขตสุรินทร์',
    'มหาวิทยาลัยราชภัฏสุรินทร์',
    'มหาวิทยาลัยเทคโนโลยีราชมงคลอีสาน  วิทยาเขตภาคสุรินทร์',
    'มหาวิทยาลัยราชภัฏศรีสะเกษ',
    'วิทยาลัยเฉลิมกาญจนา',
    'สาธิตมหาวิทยาลัยราชภัฏอุบลราชธานี(ศูนย์พัฒนาเด็กเล็ก)',
    'สถาบันบัณฑิตพัฒนบริหารศาสตร์ วิทยาเขตอุบลราชธานี',
    'มหาวิทยาลัยอุบลราชธานี',
    'มหาวิทยาลัยมหาจุฬาลงกรณราชวิทยาลัย วิทยาเขตอุบลราชธานี',
    'มหาวิทยาลัยราชภัฏอุบลราชธานี',
    'มหาวิทยาลัยราชธานี',
    'วิทยาลัยโปลีเทคนิคภาคตะวันออกเฉียงเหนือ',
    'วิทยาลัยชุมชนยโสธร',
    'มหาวิทยาลัยราชภัฏชัยภูมิ',
    'วิทยาลัยชุมชนหนองบัวลำภู',
    'สาธิตมหาวิทยาลัยขอนแก่น(มอดินแดง)',
    'สาธิตมหาวิทยาลัยขอนแก่น(ศึกษาศาสตร์)',
    'มหาวิทยาลัยขอนแก่น',
    'มหาวิทยาลัยมหาจุฬาลงกรณราชวิทยาลัย วิทยาเขตขอนแก่น',
    'มหาวิทยาลัยมหามกุฏราชวิทยาลัย วิทยาเขตอีสาน',
    'มหาวิทยาลัยเทคโนโลยีราชมงคลอีสาน  วิทยาเขตภาคขอนแก่น',
    'มหาวิทยาลัยภาคตะวันออกเฉียงเหนือ',
    'วิทยาลัยบัณฑิตบริหารธุรกิจ',
    'วิทยาลัยบัณฑิตเอเชีย',
    'สาธิตมหาวิทยาลัยราชภัฏอุดรธานี',
    'สถาบันบัณฑิตพัฒนบริหารศาสตร์ วิทยาเขตอุดรธานี',
    'มหาวิทยาลัยราชภัฏอุดรธานี',
    'วิทยาลัยสันตพล',
    'วิทยาลัยเทคโนโลยีราชธานีอุดร',
    'สาธิตมหาวิทยาลัยราชภัฏเลย',
    'มหาวิทยาลัยมหาจุฬาลงกรณราชวิทยาลัย วิทยาลัยสงฆ์เลย',
    'มหาวิทยาลัยมหามกุฏราชวิทยาลัย วิทยาเขตศรีล้านช้าง',
    'มหาวิทยาลัยราชภัฏเลย',
    'มหาวิทยาลัยขอนแก่น วิทยาเขตหนองคาย',
    'มหาวิทยาลัยมหาจุฬาลงกรณราชวิทยาลัย วิทยาเขตหนองคาย',
    'สาธิตมหาวิทยาลัยราชภัฏมหาสารคาม',
    'สาธิตมหาวิทยาลัยมหาสารคาม(ฝ่ายประถม)',
    'สาธิตมหาวิทยาลัยมหาสารคาม(ฝ่ายมัธยม)',
    'มหาวิทยาลัยมหาสารคาม',
    'มหาวิทยาลัยราชภัฏมหาสารคาม',
    'มหาวิทยาลัยมหามกุฏราชวิทยาลัย วิทยาเขตร้อยเอ็ด',
    'มหาวิทยาลัยราชภัฏร้อยเอ็ด',
    'มหาวิทยาลัยราชภัฏกาฬสินธุ์',
    'มหาวิทยาลัยเทคโนโลยีราชมงคลอีสาน  วิทยาเขตภาคกาฬสินธุ์',
    'มหาวิทยาลัยเกษตรศาสตร์ วิทยาเขตเฉลิมพระเกียรติสกลนคร',
    'มหาวิทยาลัยราชภัฏสกลนคร',
    'มหาวิทยาลัยเทคโนโลยีราชมงคลอีสาน  วิทยาเขตภาคสกลนคร',
    'มหาวิทยาลัยเทคโนโลยีราชมงคลอีสาน  สถาบันวิจัยและฝึกอบรมการเกษตรสกลนคร',
    'มหาวิทยาลัยนครพนม',
    'มหาวิทยาลัยมหาจุฬาลงกรณราชวิทยาลัย วิทยาลัยสงฆ์นครพนม',
    'มหาวิทยาลัยราชภัฏนครพนม',
    'วิทยาลัยชุมชนมุกดาหาร',
    'สาธิตมหาวิทยาลัยเชียงใหม่',
    'สาธิตมหาวิทยาลัยราชภัฏเชียงใหม่',
    'มหาวิทยาลัยเชียงใหม่',
    'มหาวิทยาลัยแม่โจ้',
    'มหาวิทยาลัยมหาจุฬาลงกรณราชวิทยาลัย วิทยาเขตเชียงใหม่',
    'มหาวิทยาลัยมหามกุฏราชวิทยาลัย วิทยาเขตล้านนา',
    'มหาวิทยาลัยราชภัฏเชียงใหม่',
    'มหาวิทยาลัยเทคโนโลยีราชมงคลล้านนา',
    'มหาวิทยาลัยเทคโนโลยีราชมงคลล้านนา  วิทยาเขตภาคพายัพจังหวัดเชียงใหม่',
    'มหาวิทยาลัยพายัพ',
    'มหาวิทยาลัยนอร์ท-เชียงใหม่',
    'วิทยาลัยฟาร์อีสเทอร์น',
    'สถาบันบัณฑิตพัฒนบริหารศาสตร์ วิทยาเขตลำพูน',
    'มหาวิทยาลัยมหาจุฬาลงกรณราชวิทยาลัย วิทยาลัยสงฆ์ลำพูน',
    'มหาวิทยาลัยธรรมศาสตร์ ศูนย์ลำปาง',
    'มหาวิทยาลัยราชภัฏลำปาง',
    'มหาวิทยาลัยเทคโนโลยีราชมงคลล้านนา  วิทยาเขตลำปาง',
    'มหาวิทยาลัยเทคโนโลยีราชมงคลล้านนา  สถาบันวิจัยและฝึกอบรมการเกษตรลำปาง',
    'มหาวิทยาลัยโยนก',
    'วิทยาลัยอินเตอร์เทคลำปาง',
    'สาธิตมหาวิทยาลัยราชภัฏอุตรดิตถ์',
    'มหาวิทยาลัยราชภัฏอุตรดิตถ์',
    'มหาวิทยาลัยเกษตรศาสตร์ แพร่',
    'มหาวิทยาลัยแม่โจ้ วิทยาเขตเฉลิมพระเกียรติแพร่',
    'มหาวิทยาลัยมหาจุฬาลงกรณราชวิทยาลัย วิทยาเขตแพร่',
    'วิทยาลัยชุมชนแพร่',
    'มหาวิทยาลัยเทคโนโลยีราชมงคลล้านนา  วิทยาเขตน่าน',
    'มหาวิทยาลัยนเรศวร วิทยาเขตสารสนเทศพะเยา',
    'มหาวิทยาลัยมหาจุฬาลงกรณราชวิทยาลัย วิทยาเขตพะเยา',
    'สาธิตมหาวิทยาลัยราชภัฏเชียงราย',
    'สถาบันบัณฑิตพัฒนบริหารศาสตร์ วิทยาเขตเชียงราย',
    'มหาวิทยาลัยราชภัฏเชียงราย',
    'มหาวิทยาลัยเทคโนโลยีราชมงคลล้านนา  วิทยาเขตเชียงราย',
    'มหาวิทยาลัยแม่ฟ้าหลวง',
    'วิทยาลัยเชียงราย',
    'วิทยาลัยชุมชนแม่ฮ่องสอน',
    'สาธิตมหาวิทยาลัยราชภัฏนครสวรรค์',
    'มหาวิทยาลัยมหาจุฬาลงกรณราชวิทยาลัย วิทยาลัยสงฆ์นครสวรรค์',
    'มหาวิทยาลัยราชภัฏนครสวรรค์',
    'มหาวิทยาลัยเจ้าพระยา',
    'มหาวิทยาลัยภาคกลาง',
    'วิทยาลัยชุมชนอุทัยธานี',
    'มหาวิทยาลัยราชภัฏกำแพงเพชร',
    'มหาวิทยาลัยเทคโนโลยีราชมงคลล้านนา  วิทยาเขตตาก',
    'วิทยาลัยลุ่มน้ำปิง',
    'วิทยาลัยชุมชนตาก',
    'สาธิตมหาวิทยาลัยนเรศวร',
    'สาธิตมหาวิทยาลัยราชภัฏพิบูลสงคราม',
    'สถาบันบัณฑิตพัฒนบริหารศาสตร์ วิทยาเขตพิษณุโลก',
    'มหาวิทยาลัยนเรศวร',
    'มหาวิทยาลัยราชภัฏพิบูลสงคราม',
    'มหาวิทยาลัยเทคโนโลยีราชมงคลล้านนา  วิทยาเขตพิษณุโลก',
    'วิทยาลัยพิษณุโลก',
    'วิทยาลัยชุมชนพิจิตร',
    'สาธิตมหาวิทยาลัยราชภัฏเพชรบูรณ์',
    'มหาวิทยาลัยราชภัฏเพชรบูรณ์',
    'สาธิตมหาวิทยาลัยราชภัฏหมู่บ้านจอมบึง(อนุบาลทานตะวัน)',
    'มหาวิทยาลัยเกษตรศาสตร์ ราชบุรี',
    'มหาวิทยาลัยราชภัฏหมู่บ้านจอมบึง',
    'มหาวิทยาลัยเทคโนโลยีพระจอมเกล้าธนบุรี วิทยาเขตราชบุรี',
    'มหาวิทยาลัยมหิดล วิทยาเขตกาญจนบุรี',
    'มหาวิทยาลัยราชภัฏกาญจนบุรี',
    'มหาวิทยาลัยเวสเทิร์น',
    'มหาวิทยาลัยเกษตรศาสตร์ วิทยาเขตสุพรรณบุรี',
    'มหาวิทยาลัยเทคโนโลยีราชมงคลสุวรรณภูมิ  วิทยาเขตสุพรรณบุรี',
    'สาธิตมหาวิทยาลัยศิลปากร',
    'สาธิตมหาวิทยาลัยราชภัฏนครปฐม',
    'สาธิตมหาวิทยาลัยเกษตรศาสตร์วิทยาเขตกำแพงแสน',
    'วิทยาลัยดุริยางศิลป์มหาวิทยาลัยมหิดล',
    'มหาวิทยาลัยเกษตรศาสตร์ วิทยาเขตกำแพงแสน',
    'มหาวิทยาลัยมหิดล',
    'มหาวิทยาลัยศิลปากร วิทยาเขตพระราชวังสนามจันทร์',
    'มหาวิทยาลัยมหาจุฬาลงกรณราชวิทยาลัย วิทยาเขตบาฬิกศึกษาพุทธโฆส',
    'มหาวิทยาลัยมหามกุฏราชวิทยาลัย วิทยาเขตสิรินธรราชวิทยาลัย',
    'มหาวิทยาลัยราชภัฏนครปฐม',
    'มหาวิทยาลัยเทคโนโลยีราชมงคลรัตนโกสินทร์',
    'มหาวิทยาลัยเทคโนโลยีราชมงคลรัตนโกสินทร์  วิทยาเขตศาลายา',
    'มหาวิทยาลัยคริสเตียน',
    'วิทยาลัยแสงธรรม',
    'วิทยาลัยชุมชนสมุทรสาคร',
    'มหาวิทยาลัยศิลปากร วิทยาเขตสารสนเทศเพชรบุรี',
    'มหาวิทยาลัยราชภัฏเพชรบุรี',
    'มหาวิทยาลัยเว็บสเตอร์(ประเทศไทย)',
    'มหาวิทยาลัยนานาชาติแสตมฟอร์ด',
    'มหาวิทยาลัยเทคโนโลยีราชมงคลรัตนโกสินทร์  วิทยาเขตวังไกลกังวล',
    'สาธิตมหาวิทยาลัยราชภัฏนครศรีธรรมราช',
    'มหาวิทยาลัยมหาจุฬาลงกรณราชวิทยาลัย วิทยาเขตนครศรีธรรมราช',
    'มหาวิทยาลัยมหามกุฏราชวิทยาลัย วิทยาเขตศรีธรรมาโศกราช',
    'มหาวิทยาลัยราชภัฏนครศรีธรรมราช',
    'มหาวิทยาลัยเทคโนโลยีราชมงคลศรีวิชัย  วิทยาเขตนครศรีธรรมราช',
    'มหาวิทยาลัยเทคโนโลยีราชมงคลศรีวิชัย  วิทยาเขตศรีวิชัย  จังหวัดนครศรีธรรมราช',
    'มหาวิทยาลัยเทคโนโลยีราชมงคลศรีวิชัย คณะเกษตรศาสตร์นครศรีธรรมราช',
    'มหาวิทยาลัยวลัยลักษณ์',
    'วิทยาลัยเทคโนโลยีภาคใต้',
    'วิทยาลัยศรีโสภณ',
    'มหาวิทยาลัยเกษตรศาสตร์ วิทยาเขตกระบี่',
    'วิทยาลัยชุมชนพังงา',
    'สาธิตมหาวิทยาลัยราชภัฏภูเก็ต',
    'มหาวิทยาลัยสงขลานครินทร์ วิทยาเขตภูเก็ต',
    'สถาบันบัณฑิตพัฒนบริหารศาสตร์ วิทยาเขตภูเก็ต',
    'มหาวิทยาลัยราชภัฏภูเก็ต',
    'มหาวิทยาลัยสงขลานครินทร์ วิทยาเขตสุราษฎร์ธานี',
    'สถาบันบัณฑิตพัฒนบริหารศาสตร์ วิทยาเขตสุราษฎร์ธานี',
    'มหาวิทยาลัยราชภัฏสุราษฏร์ธานี',
    'วิทยาลัยตาปี',
    'วิทยาลัยชุมชนระนอง',
    'มหาวิทยาลัยแม่โจ้ วิทยาเขตชุมพร',
    'สถาบันเทคโนโลยีพระจอมเกล้าเจ้าคุณทหารลาดกระบัง วิทยาเขตชุมพร',
    'มหาวิทยาลัยเทคโนโลยีราชมงคลพระนคร  วิทยาเขตชุมพรเขตรอุดมศักดิ์',
    'สาธิตมหาวิทยาลัยราชภัฏสงขลา',
    'มหาวิทยาลัยสงขลานครินทร์',
    'สถาบันบัณฑิตพัฒนบริหารศาสตร์ วิทยาเขตหาดใหญ่',
    'มหาวิทยาลัยทักษิณ',
    'มหาวิทยาลัยราชภัฏสงขลา',
    'มหาวิทยาลัยเทคโนโลยีราชมงคลศรีวิชัย',
    'มหาวิทยาลัยเทคโนโลยีราชมงคลศรีวิชัย  วิทยาเขตภาคใต้  จังหวัดสงขลา',
    'มหาวิทยาลัยหาดใหญ่',
    'วิทยาลัยพุทธศาสนานานาชาติ',
    'วิทยาลัยชุมชนสตูล',
    'มหาวิทยาลัยสงขลานครินทร์ วิทยาเขตตรัง',
    'มหาวิทยาลัยเทคโนโลยีราชมงคลศรีวิชัย  คณะวิทยาศาสตร์และเทคโนโลยีการประมง จังหวัดตรัง',
    'สาธิตมหาวิทยาลัยสงขลานครินทร์',
    'มหาวิทยาลัยสงขลานครินทร์ วิทยาเขตปัตตานี',
    'วิทยาลัยชุมชนปัตตานี',
    'สาธิตมหาวิทยาลัยราชภัฏยะลา',
    'สถาบันบัณฑิตพัฒนบริหารศาสตร์ วิทยาเขตยะลา',
    'มหาวิทยาลัยราชภัฏยะลา',
    'มหาวิทยาลัยอิสลามยะลา',
    'วิทยาลัยชุมชนยะลา',
    'มหาวิทยาลัยนราธิวาสราชนครินทร์',
    'วิทยาลัยชุมชนนราธิวาส',
    'มหิดลวิทยานุสรณ์',
    'เทศบาล 1 (เยี่ยมเกษสุวรรณ)',
    'เทศบาล 2 (วัดใน)',
    'เทศบาล 3 (คลองตาเค็ด)',
    'เทศบาล 4 (สิทธิไชยอุปถัมภ์)',
    'เทศบาล 5 (วัดกลางวรวิหาร)',
    'เทศบาลวัดแค',
    'เทศบาลวัดทรงธรรม',
    'เทศบาลป้อมแผลงไฟฟ้า',
    'เทศบาล 1',
    'เทศบาล 2',
    'บางเมืองสามแพรก',
    'วัดสัก',
    'วัดประชารังสรรค์',
    'วัดมะเดื่อ',
    'วัดบางบัวทอง',
    'วัดเสาธงหิน',
    'นครนนท์วิทยา 1 วัดท้ายเมือง',
    'นครนนท์วิทยา 2 วัดทินกรนิมิต',
    'นครนนท์วิทยา 3 วัดนครอินทร์',
    'นครนนท์วิทยา 4 วัดบางแพรกเหนือ',
    'นครนนท์วิทยา 5 ทานสัมฤทธิ์',
    'เทศบาลวัดละหาร',
    'วัดสิงห์ (แจ่มชื่นวิทยาคม)',
    'วัดสุนทรธรรมิการาม',
    'เทศบาลเมืองปทุมธานี',
    'เทศบาลท่าโขลง ๑',
    'อนุบาลเทศบาลเมืองสนั่นรักษ์',
    'อนุบาลเทศบาลตำบลบางกะดี',
    'อนุบาลองค์การบริหารส่วนตำบลบึงยี่โถ',
    'ขจรเนติยุทธ',
    'วัดช่างทอง',
    'เทศบาลสรรพสามิตบำรุง',
    'เทศบาลวัดเขียน',
    'เทศบาลวัดตองปุโบราณคณิสสร',
    'เทศบาลวัดศาลาปูน',
    'เทศบาลวัดรัตนชัย',
    'เทศบาลวัดแม่นางปลื้ม',
    'เทศบาลวัดป่าโค',
    'ชุมชนป้อมเพชร',
    'เทศบาลวัดแค',
    'เทศบาลท่าเรือประชานุกูล',
    'เทศบาลวัดกลาง',
    'เสนาบดี',
    'วัดบางนมโค',
    'เทศบาล 1 บ้านท่าหิน',
    'เทศบาล 2 วัดกวิศราราม',
    'เทศบาล 3 วัดคลองสายบัว',
    'เทศบาล 4 วัดเมืองใหม่',
    'เทศบาล 1 บ้านโคกสำโรง',
    'เทศบาล 2 วัดแก้วจันทราราม',
    'อนุบาลเทศบาลตำบลโคกสำโรง',
    'เทศบาลบ้านหมี่',
    'อนุบาลเทศบาลบ้านหมี่',
    'เทศบาล 1 (ซอย 6)',
    'อนุบาลองค์การบริหารส่วนตำบลเขาสามยอด',
    'เทศบาล 1 วัดโพธิ์แก้วนพคุณ',
    'เทศบาล 2 วัดเสฐียรวัฒนดิษฐ์',
    'อนุบาลเทศบาล 3',
    'เทศบาล 4 (รักษ์เมืองสิงห์)',
    'เทศบาล 1 (สหราษฎร์วิทยา)',
    'ชัยนาทพิทยาคม 2',
    'เทศบาลบ้านกล้วย',
    'เทศบาลเขาท่าพระ',
    'เทศบาลวัดหัวยาง',
    'เทศบาลวัดสิงห์สถิตย์',
    'หรเทพ (รุ่งเรืองประชาสามัคคี)',
    'เทศบาล 1 (วัดทองพุ่มพวง)',
    'เทศบาล 2 (วัดศรีบุรีรัตนาราม)',
    'เทศบาล 3 (วัดบ้านอ้อย)',
    'เทศบาล 4 (วัดบำรุงธรรม)',
    'เทศบาล 5 (วัดดาวเรือง)',
    'เทศบาล 6 (วัดเชิงเขา)',
    'เทศบาล 7 (วัดแก่งขนุน)',
    'เทศบาล 8 (วัดเจดีย์งาม)',
    'เทศบาล 9 (วัดเขาคูบา)',
    'เทศบาล 10 (อนุบาลเทศบาล)',
    'เทศบาลบ้านม่วง',
    'เทศบาลพัฒนา',
    'เทศบาลวัดแก่งคอย',
    'เทศบาล 1 นาเริ่งราษฎร์บำรุง',
    'เทศบาล 2 วัดเสนานฤมิตร',
    'เทศบาล 3 วัดสหมิตรมงคล',
    'เทศบาล 4 หนองแคอนุสรณ์',
    'เทศบาล 5 วัดเกาะกลาง',
    'เทศบาลพระพุทธบาท',
    'วิทยาลัยอาชีวศึกษาเทศบาลพระพุทธบาท',
    'องค์การบริหารส่วนจังหวัดชลบุรี 1  (เมืองใหม่)',
    'ชุมชนวัดหนองค้อ',
    'วัดพรหมรัตนาราม',
    'สวนป่าเขาชะอางค์',
    'พลูตาหลวงวิทยา',
    'บ้านเขาซก',
    'เทศบาลวัดกำแพง',
    'เทศบาลวัดโพธิ์',
    'เทศบาลชลราษฎร์นุเคราะห์',
    'เทศบาลวัดเนินสุทธาวาส',
    'เทศบาลอินทปัญญา',
    'เทศบาลบ้านศรีมหาราชา',
    'เทศบาลวัดราษฎร์นิยมธรรม',
    'เทศบาล 1 ศรีกิตติวรรณนุสรณ์',
    'เทศบาล 2 วัดกลางทุมมาวาส',
    'เทศบาล 3 วัดเกาะแก้วนครสวรรค์',
    'เทศบาล 4',
    'เทศบาล 1 (สถาวร)',
    'เทศบาลแหลมฉบัง 2 (มูลนิธิไต้ล้ง - เช็ง พรประภา)',
    'เทศบาลแหลมฉบัง 3',
    'อนุบาลเทศบาลเกาะสีชัง',
    'อนุบาลเทศบาลตำบลบ่อทอง',
    'เมืองพัทยา 1 (เชิญพิศลยบุตรราษฎร์บำเพ็ญ)',
    'เมืองพัทยา 2 (เจริญราษฎร์อุทิศา)',
    'เมืองพัทยา 3 (สว่างฟ้าพฤฒาราม)',
    'เมืองพัทยา 4 (วัดหนองใหญ่)',
    'เมืองพัทยา 5 (บ้านเนินพัทธยาเหนือ)',
    'เมืองพัทยา 6 (วัดธรรมสามัคคี)',
    'เมืองพัทยา 7 (บ้านหนองพังแค)',
    'เมืองพัทยา 8 (พัทธยานุกูล)',
    'เมืองพัทยา 9 (วัดโพธิสัมพันธ์)',
    'เมืองพัทยา 10 (บ้านเกาะล้าน)',
    'เทศบาลวัดปากน้ำ',
    'เทศบาลบ้านปากคลอง',
    'เทศบาลวัดโขดทิมทาราม',
    'เทศบาลวัดลุ่มมหาชัยชุมพล',
    'สาธิตเทศบาลนครระยอง',
    'เทศบาลมาบตาพุด',
    'เทศบาลเมืองจันทบุรี 1',
    'เทศบาลเมืองจันทบุรี ๒',
    'เทศบาลขลุง (บุรวิทยาคาร)',
    'เทศบาลบ้านป่าแดง',
    'เทศบาลวัดไผ่ล้อม',
    'เทศบาลวัดบุญญวาสวิหาร',
    'เทศบาลวัดหนองบัว',
    'กีฬาจังหวัดตราด',
    'เทศบาลชุมชนวิมลวิทยา',
    'วัดสัมปทาน (บางแก้วพุทธิยาคาร)',
    'พนมอดุลวิทยา 2 ชำป่างาม',
    'เทศบาล 1 วัดแหลมใต้',
    'เทศบาล 2  พระยาศรีสุนทรโวหาร',
    'เทศบาล ๑ วัดแจ้ง',
    'เทศบาล ๒ วัดโพธิ์',
    'เทศบาล ๑ บางวัว',
    'ประชารัฐพัฒนา',
    'ประจันตราษฎร์บำรุง (อำเภอ)',
    'มัธยมวัดป่ามะไฟ',
    'ลาดตะเคียนราษฎร์บำรุง',
    'กบินทร์บุรี',
    'วังตะเคียนวิทยาคม',
    'วังดาลวิทยาคม',
    'นนทรีวิทยาคม',
    'เตรียมอุดมศึกษาน้อมเกล้ากบินทร์บุรี',
    'ทุ่งใหญ่วิทยาคาร',
    'ศรีรักษ์ราษฎร์บำรุง',
    'สุวรรณวิทยา',
    'วัดพรหมประสิทธิ์',
    'เทศบาล 1 (ปราจีนราษฎร์รังสฤษฎ์)',
    'เทศบาล 2 วัดหลวงปรีชากูล',
    'เทศบาล ๓ (วัดแก้วพิจิตร)',
    'เทศบาล 4 อุดมวิทย์สมใจ',
    'เทศบาล 5 (บดินทร์เดชา)',
    'เทศบาล 6 วัดศรีมงคล',
    'เทศบาล 1 (กบินทร์ราษฎร์บำรุง)',
    'เทศบาล 2 (วัดใหม่ท่าพาณิชย์)',
    'เทศบาล 1 วัดศรีเมือง',
    'เทศบาล 2 บ้านตลาดเก่า',
    'เทศบาล 3 บ้านต่ำบุญศิริ',
    'ซับนกแก้ววิทยา',
    'บ้านแก้งวิทยา',
    'เทศบาลชนะชัยศรี',
    'เทศบาลบ้านกิโลสอง',
    'บ้านหนองกะพ้อ',
    'บ้านลัดกะสัง',
    'มิตรสัมพันธ์วิทยา',
    'บัวใหญ่',
    'กลางดงปุณณวิทยา',
    'โตนดพิทยาคม',
    'มะค่าวิทยา',
    'นครราชสีมาวิทยาลัย',
    'ดอนไพลพิทยาคม',
    'หนองยางพิทยาคม',
    'คลองเมืองพิทยาคม',
    'หนองขามพิทยาคม',
    'ด่านเกวียนวิทยา',
    'หนองงูเหลือมพิทยาคม',
    'หินดาดวิทยา',
    'หนองบุนนากพิทยาคม',
    'บ้านใหญ่พิทยาคม',
    'มาบตะโกพิทยาคม',
    'เฉลียงพิทยาคม',
    'บ้านใหม่พิทยาคม',
    'ลำพระเพลิงพิทยาคม',
    'สูงเนิน',
    'สองครพิทยาคม',
    'วังรางพิทยาคม',
    'สีคิ้ว "สวัสดิ์ผดุงวิทยา"',
    'สีคิ้ววิทยาคาร',
    'ปากช่องพิทยาคม',
    'ปากช่อง 2',
    'เขาใหญ่พิทยาคม',
    'สีคิ้วหนองหญ้าขาว',
    'วังโป่งพิทยาคม',
    'มัธยมประดู่วัฒนา',
    'บ้านปราสาทวิทยาคม',
    'ทัพรั้งพิทยาคม',
    'พระทองคำวิทยา',
    'เทพารักษ์ราชวิทยาคม',
    'โนนไทยคุรุอุปถัมภ์',
    'หนองบัวพิทยาคม',
    'ปรางค์ทองวิทยา',
    'วังไม้แดงพิทยาคม',
    'ตลาดไทรพิทยาคม',
    'สาหร่ายวิทยาคม',
    'เทศบาล 1 (บูรพาวิทยากร)',
    'เทศบาล 2 (วัดสมอราย)',
    'เทศบาล 3 (ยมราชสามัคคี)',
    'เทศบาล 4 (เพาะชำ)',
    'เทศบาล 5 (วัดป่าจิตตสามัคคี)',
    'กีฬาเทศบาลนครนครราชสีมา',
    'เทศบาล 1 (รัฐราษฎร์สงเคราะห์)',
    'เทศบาล 2 (รัฐราษฎร์บำรุง)',
    'เทศบาล 3 (รัฐราษฎร์พัฒนา)',
    'เทศบาล 1 (บ้านบัวใหญ่)',
    'เทศบาล 2 (สหกรณ์สมทบ)',
    'เทศบาล 3 (บ้านหนองม่วง)',
    'เทศบาล 1 (บ้านหนองสาหร่าย)',
    'อนุบาลเทศบาลตำบลโนนแดง',
    'เทศบาลหนองหัวฟาน',
    'อนุบาลเทศบาลตลาดแค',
    'โยธินนุกูล',
    'หนองขมารวิทยาคม',
    'เทศบาล 1 บุรีราษฎร์ดรุณวิทยา',
    'เทศบาล 2',
    'เทศบาล 3',
    'ทีโอเอวิทยา (เทศบาล 1)',
    'เทศบาลตำบลนาโพธิ์',
    'เทศบาล 1 "สุรินทร์วิทยาคม"',
    'เทศบาล 2 "วิภัชศึกษา"',
    'เทศบาล 3 "เทศบาลอนุสรณ์"',
    'เทศบาลตำบลท่าตูม',
    'กีฬาหมื่นศรีวิทยานุสรณ์',
    'บ้านสิริขุนหาญ',
    'บ้านเพียนาม',
    'ไพรบึงวิทยาคม',
    'กันทรอมวิทยาคม',
    'บ้านเปือยนาสูง',
    'ปราสาท',
    'หนองทุ่งศรีสำราญวิทยา',
    'ผักแพววิทยา',
    'กระดุมทองวิทยา',
    'แวงก้ววิทยา',
    'หนองถ่มวิทยา',
    'โพธิ์ธาตุประชาสรรค์',
    'โคกสะอาดวิทยาคม',
    'บกวิทยาคม',
    'ไตรมิตร',
    'โพธิ์ศรีสุวรรณวิทยาคม',
    'เมืองจันทร์วิทยาคม',
    'จิกสังข์ทองวิทยา',
    'ด่านอุดมศึกษา',
    'เมืองแคนวิทยา',
    'หวายคำวิทยา',
    'ประชาพัฒนศึกษา',
    'กุดเสลาวิทยาคม',
    'ดงรักวิทยา',
    'ตระกาศประชาสามัคคี',
    'สายธารวิทยา',
    'นาแก้ววิทยา',
    'พรานวิบูลวิทยา',
    'โพธิ์วงศ์วิทยา',
    'ร่มโพธิ์วิทยา',
    'ไพรธรรมคุณวิทยา',
    'หนองหว้าประชาสรรค์',
    'ศรีแก้วพิทยา',
    'เทศบาล 1 (วัดเจียงอี)',
    'เทศบาล 2 (รัชมังคลานุสรณ์)',
    'เทศบาล 3 (เฉลิมพระเกียรติฯ)',
    'ชุมชนหนองยางหนองม่วง',
    'บ้านโนนสำนักมิตรภาพที่ 121',
    'บ้านหนองสิม',
    'บ้านหนองมะเกลือ',
    'พิบูลมังสาหาร',
    'แก้งเหนือพิทยาคม',
    'เหล่างามพิทยาคม',
    'โนนกลางวิทยาคม',
    'หนองบัวฮีวิทยาคม',
    'ไร่ใต้ประชาคม',
    'ช่องเม็กวิทยา',
    'ห้วยข่าพิทยา',
    'น้ำขุ่นวิทยา',
    'บัวงามวิทยา',
    'เทศบาลบูรพาอุบล',
    'เทศบาล 2 หนองบัว',
    'สามัคคีวิทยาคาร',
    'เทศบาล 4 อนุบาลพระเจ้าใหญ่องค์ตื้อ',
    'เทศบาลวารินวิชาชาติ',
    'เทศบาลบ้านสุขสำราญ',
    'เทศบาลบ้านหนองตาโผ่นมิตรภาพที่ 5',
    'เทศบาล 1 (บ้านโพธิ์กลาง)',
    'เทศบาล 2 พิบูลวิทยาคาร',
    'เทศบาลแสนสุข',
    'หนองแหนพัฒนวิทยาคม',
    'เทศบาล 1 สุขวิทยากร',
    'เทศบาล 2 สามัคคีวัฒนา',
    'เทศบาล ๓ ห้าธันวาคม',
    'เทศบาลตำบลทรายมูล',
    'กุดชุม (คุรุราษฎร์บำรุง)',
    'บ้านสวาท',
    'บำเหน็จณรงค์วิทยาคม',
    'ภูแลนคาวิทยายน',
    'สตรีชัยภูมิ 2',
    'ห้วยต้อนพิทยาคม',
    'ลุ่มลำชีนิรมิตวิทยา',
    'หนองไผ่วิทยานุสรณ์',
    'กวางโจนศึกษา',
    'บ้านเดื่อวิทยา',
    'มัธยมบ้านแก้งวิทยา',
    'เกษตรสมบูรณ์วิทยาคม',
    'มัธยมหนองศาลา',
    'บ้านหันวิทยา',
    'หนองสังข์วิทยายน',
    'โคกสะอาดวิทยา',
    'สระพังวิทยาคม',
    'ห้วยยางวิทยาคม',
    'โนนกอกวิทยา',
    'หนองขามวิทยา',
    'เพชรวิทยาคาร',
    'บ้านเป้าวิทยา',
    'กุดน้ำใสพิทยาคม',
    'เนินสง่าวิทยา',
    'ตรีประชาพัฒนาศึกษา',
    'โป่งนกประชาสามัคคี',
    'เทศบาล 1 (วิทยานารี)',
    'เทศบาล 2 (เมืองเก่าวิทยา)',
    'เทศบาล 3 (ปรางค์กู่วิทยาคาร)',
    'เทศบาล 4 (อมรสะเพียรชัยอุทิศ)',
    'เทศบาล 1  วัดเทพมงคล',
    'บ้านพนานต์ (สามัคคีวิทยา)',
    'เทศบาล 1 (วัดเกาะแก้ว)',
    'เมืองพลพิทยาคม',
    'บ้านคูขาด (สถิตย์อุปถัมภ์)',
    'บ้านหนองเสี้ยว',
    'แท่นศิลาทิพย์ศึกษา',
    'ศรีเสมาวิทยาเสริม',
    'พิศาลปุณณวิทยา',
    'ซำสูงพิทยาคม',
    'พระธาตุขามแก่นพิทยาลัย',
    'หนองโนประชาสรรค์',
    'พูวัดพิทยาคม',
    'โคกสูงประชาสรรพ์',
    'โนนหันวิทยายน',
    'สีชมพูศึกษา',
    'มัธยมหนองเขียด',
    'เทศบาลบ้านโนนหนองวัด',
    'เทศบาลวัดกลาง',
    'เทศบาลคุ้มหนองคู',
    'เทศบาลสวนสนุก',
    'เทศบาลบ้านหนองใหญ่',
    'เทศบาลบ้านศรีฐาน',
    'เทศบาลบ้านโนนชัย',
    'เทศบาลบ้านสามเหลี่ยม',
    'เทศบาลบ้านหนองแวง',
    'เทศบาลบ้านตูม',
    'เทศบาลบ้านโนนทัน',
    'เทศบาลหนองแวงประชาอุปถัมภ์',
    'เทศบาลศรีเมืองพลประชานุเคราะห์',
    'เทศบาลพลประชานุกูล',
    'เทศบาลบ้านไผ่',
    'เทศบาล 1',
    'อนุบาลเทศบาลตำบลหนองแก',
    'อนุบาลองค์การบริหารส่วนตำบลน้ำพอง',
    'อนุบาลองค์การบริหารส่วนตำบลกุดน้ำใส (วัดศรีมงคล)',
    'บ้านทุ่งเชือก',
    'คำใหญ่ปันน้ำใจ',
    'เชียงเพ็งวิทยา',
    'นาทรายวิทยาคม',
    'ชัยนาคำวิทยา',
    'เทศบาล 1 โพศรี',
    'เทศบาล 2 มุขมนตรี',
    'เทศบาล 3 บ้านเหล่า',
    'เทศบาล 4 วัดโพธิวราราม',
    'เทศบาล 5 สีหรักษ์วิทยา',
    'เทศบาล 6 ไลออนส์อุทิศ',
    'เทศบาล 7 รถไฟสงเคราะห์',
    'เทศบาล 8 (ไทยรัฐวิทยา 72)',
    'เทศบาล 9 มณเฑียรอนุสรณ์',
    'บ้านโสกคูณ',
    'บ้านหนองกอง',
    'บ้านโคกสูง',
    'เหมืองแบ่งวิทยาคม',
    'ชุมชนศรีสะอาด  (เทศบาล 1)',
    'เทศบาล 2 ศรีบุญเรือง',
    'เทศบาล 3 ศรีสว่าง',
    'บ้านภูบ่อบิด',
    'บ้านหนองผักก้าม',
    'อนุบาลเทศบาลวังสะพุง',
    'เทศบาล 1 สว่างวิทยา',
    'เทศบาล 2 ชำนาญอนุเคราะห์',
    'เทศบาล 3 ยุวบูรณ์บำรุง',
    'เทศบาล 4 ฉลองรัตน',
    'เทศบาล 1 (เมืองท่าบ่อ)',
    'อนุบาลเทศบาลตำบลปากคาด',
    'เทศบาลพรเจริญ',
    'งัวบาวิทยาคม',
    'ดอนเงินพิทยาคาร',
    'หนองเหล็กศึกษา',
    'ศรีสุขพิทยาคม',
    'มะค่าพิทยาคาร',
    'เลิงแฝกประชาบำรุง',
    'หนองบัวปิยนิมิตร',
    'ท่าขอนยางพิทยาคม',
    'นาสีนวลพิทยาสรรค์',
    'ขามป้อมพิทยาคม',
    'เสือโก้กวิทยาสรรค์',
    'นาข่าวิทยาคม',
    'หนองโกวิชาประสิทธิ์พิทยาคม',
    'หนองโพธิ์วิทยาคม',
    'หัวเรือพิทยาคม',
    'มัธยมดงยาง',
    'เวียงสะอาดพิทยาคม',
    'เทศบาลสามัคคีวิทยา',
    'เทศบาลโพธิ์ศรี',
    'เทศบาลบ้านค้อ',
    'เทศบาลบ้านแมด',
    'เทศบาลบูรพาพิทยาคาร',
    'เทศบาลศรีสวัสดิ์วิทยา',
    'เทศบาลบ้านส่องนางใย',
    'โพธิ์ทองพิทยาคม',
    'ทุ่งกุลาประชานุสรณ์',
    'เทศบาลวัดเหนือ',
    'เทศบาลชุมชนบ้านหนองหญ้าม้า',
    'เทศบาลวัดบูรพาภิราม',
    'เทศบาลวัดป่าเรไร',
    'เทศบาลวัดราษฎรอุทิศ',
    'เทศบาลวัดเวฬุวัน',
    'เทศบาลวัดสระทอง',
    'เมืองสมเด็จ',
    'นาเชือกวิทยาลัย',
    'ขมิ้นพิทยาสรรค์',
    'ลำปาววิทยาคม',
    'คลองขามวิทยาคาร',
    'ดงมูลวิทยาคม',
    'ทรายมูลพิทยาคม',
    'เนินยางประชาสามัคคี',
    'หนองห้างพิทยา',
    'เทศบาล 1 กาฬสินธุ์พิทยาสิทธิ์',
    'เทศบาล 2 วัดสว่างคงคา',
    'เทศบาล 3 วัดเหนือ',
    'เทศบาล 4 วัดใต้โพธิ์ค้ำ',
    'องค์การบริหารส่วนตำบลโคกสะอาด',
    'ร่มไทรวิทยา',
    'ธาตุทองอำนวยวิทย์',
    'สกลราชวิทยานุกูล 2',
    'ภูดินแดงวิทยา',
    'เดื่อศรีไพรวัลย์',
    'คำยางพิทยาคม',
    'เทศบาล 1 "เชิงชุมประชานุกูล"',
    'เทศบาล 2 "เชิงชุมอนุชนวิทยา"',
    'เทศบาล 3 "ยุติธรรมวิทยา"',
    'เทศบาล 4 "รัฐประชานุเคราะห์"',
    'เทศบาล ๑  (หนองแสง)',
    'เทศบาล ๒  (ถนนบำรุงเมือง)',
    'ชุมชนเทศบาล ๓  (พินิจพิทยานุสรณ์)',
    'เทศบาล ๔ (รัตนโกสินทร์ ๒00 ปี)',
    'เทศบาล ๕ สมพรอภัยโส',
    'อนุบาลเทศบาลเมืองนครพนม',
    'ทีโอเอวิทยา (ท.1)',
    'เทศบาล 2 (วัดนิรมิตร)',
    'แม่อายวิทยาคม',
    'บ้านแม่งอนขี้เหล็ก',
    'เทศบาลวัดท่าสะต๋อย',
    'เทศบาลวัดกู่คำ',
    'ชุมชนเทศบาลวัดศรีดอนไชย',
    'เทศบาลวัดศรีสุพรรณ',
    'เทศบาลดอกเงิน',
    'เทศบาลวัดป่าแพ่ง',
    'เทศบาลวัดศรีปิงเมือง',
    'เทศบาลวัดพวกช้าง',
    'เทศบาลวัดเชียงยืน',
    'เทศบาลวัดหมื่นเงินกอง',
    'อนุบาลองค์การบริหารส่วนตำบลบวกค้าง',
    'อนุบาลสันทรายพัฒนา',
    'นาทรายวิทยาคม',
    'เทศบาลจามเทวี',
    'เทศบาลประตูลี้',
    'ชุมชนสันป่ายางหลวง',
    'เทศบาลสันป่ายางหน่อม',
    'บ้านกลางวิทยานุกูล',
    'วอแก้ววิทยา',
    'เทศบาล 1 (บ้านแสนเมืองมูล)',
    'เทศบาล 3 บุญทวงศ์อนุกูล',
    'เทศบาล 4 (บ้านเชียงราย)',
    'เทศบาล 5 (บ้านศรีบุญเรือง)',
    'เทศบาล 6 (วัดป่ารวก)',
    'เทศบาล 7 (ศิรินาวินวิทยา)',
    'เทศบาลท่าอิฐ',
    'เทศบาลวัดท้ายตลาด',
    'เทศบาลวัดเกษมจิตตาราม',
    'เทศบาลวัดหนองผา',
    'เทศบาลวัดคลองโพธิ์',
    'เทศบาลวัดไผ่ล้อม',
    'เทศบาลศรีพนมมาศพิทยากร',
    'บ้านไผ่ย้อย',
    'พัฒนาประชาอุปถัมภ์',
    'เทศบาลวัดสวรรคนิเวศ',
    'เทศบาลวัดหัวข่วง',
    'เทศบาลวัดชัยมงคล',
    'เทศบาลวัดเหมืองแดง',
    'สาธิตเทศบาลบ้านเชตวัน',
    'อนุบาลเทศบาลตำบลเด่นชัย',
    'เทศบาลตำบลร้องกวาง',
    'ตาลชุมพิทยาคม',
    'จุมปีวนิดาภรณ์ (เทศบาลบ้านภูมินทร์)',
    'ดรุณวิทยา (เทศบาลบ้านสวนตาล)',
    'สามัคคีวิทยาคาร (เทศบาลบ้านพระเนตร)',
    'เทศบาล 1 (พะเยาประชานุกูล)',
    'เทศบาล 2 (แม่ต๋ำดรุณเวทย์)',
    'เทศบาล 3 (หล่ายอิงราษฎร์บำรุง)',
    'เทศบาล 4 (ภูมินทร์ราษฎร์นุกูล)',
    'เทศบาล 5 (แก้วปัญญาอุปถัมภ์)',
    'เทศบาล 6 (รัฐประชาอุทิศ)',
    'องค์การบริหารส่วนจังหวัดเชียงราย',
    'เทศบาล 1 ศรีเกิด',
    'เทศบาล 2 หนองบัว',
    'เทศบาล 3 ศรีทรายมูล',
    'เทศบาล 4 สันป่าก่อ',
    'เทศบาล 5 เด่นห้า',
    'เทศบาล 6  สลากกินแบ่งรัฐบาล',
    'เทศบาล 7  ฝั่งหมิ่น',
    'อนุบาลเทศบาลตำบลป่าก่อคำ',
    'วัดพรหมวิหาร  (เทศบาล 1)',
    'วัดป่ายาง  (เทศบาล 2)',
    'อนุบาลเทศบาลป่าแงะ',
    'เทศบาล 1 ต้นยาง',
    'บ้านใหม่ (ประกอบราษฎร์วิทยานุกูล)',
    'อนุบาลเทศบาลตำบลเวียงเชียงแสน',
    'อนุบาลป่าแดด',
    'บ้านเก่า',
    'อนุบาลตำบลป่าหุ่ง',
    'บ้านดอนตัน',
    'บ้านท่าฮ้อ',
    'บ้านจองคำ',
    'เทศบาลเมืองแม่ฮ่องสอน',
    'เทศบาลวัดไทรใต้',
    'เทศบาลวัดปากน้ำโพใต้',
    'เทศบาลวัดพรหมจริยวาส',
    'เทศบาลวัดวรนาถบรรพต',
    'เทศบาลวัดช่องคีรีสิทธิวราราม',
    'เทศบาลวัดจอมคีรีนาคพรต',
    'เทศบาลวัดสุคตวราราม',
    'เทศบาลวัดไทรเหนือ',
    'เทศบาล 1 (บ้านชุมแสง)',
    'เทศบาล 2 (คลองระนง)',
    'เทศบาล 3 (วัดแสงธรรมสุทธาวาส)',
    'เทศบาลตาคลี',
    'เทศบาลบ้านปากกะบาด',
    'เทศบาลวัดหลวงราชาวาส',
    'เทศบาลวัดอมฤตวารี',
    'ชุมชนเทศบาลวัดมณีสถิตกปิฎฐาราม',
    'เทศบาลวัดธรรมโศภิต',
    'องค์การบริหารส่วนจังหวัดกำแพงเพชร  (กีฬา)',
    'เทศบาล 1 (เกริกกฤตยาอุปถัมภ์)',
    'เทศบาล 2 (วัดทุ่งสวน)',
    'เทศบาล 3 (อินทรัมพรรย์)',
    'เทศบาล 4 (อินทร์-ชุ่มดีสาร)',
    'เทศบาล 1 (คลองสวนหมาก)',
    'เทศบาลรัตนวิทย์',
    'เทศบาลชัยชนะวิทย์',
    'เทศบาลสายประทีป',
    'เทศบาลวัดดอนแก้ว',
    'เทศบาลวัดบุญญาวาส',
    'เทศบาลวัดมณีไพรสนฑ์',
    'เทศบาลวัดชุมพลคีรี',
    'เทศบาลเพชรวิทย์',
    'วัดวังสวรรค์',
    'วัดคลองโป่ง (ธรรมภาณบำรุง)',
    'เทศบาลวัดไชยชุมพล',
    'เทศบาลเมืองสุโขทัย',
    'เทศบาลแป้นจันทร์กระจ่าง',
    'เทศบาลสวรรคโลกประชาสรรค์',
    'เทศบาลเมืองสวรรคโลก',
    'เทศบาลวัดสวรรคาราม',
    'เทศบาลวัดสวัสดิการาม',
    'วัดลายมิตรภาพที่ 80',
    'บ้านหรรษาเจริญประชานุเคราะห์',
    'บ้านวังโคนไผ่',
    'วัดท่าช้าง',
    'ศรีอินทราทิตย์พิทยาคม',
    'บางกลางท่าวพิทยาคม',
    'เทศบาล 1 (วัดน้อย)',
    'เทศบาล 2 (วัดคูหาสวรรค์)',
    'เทศบาล 3 (วัดท่ามะปราง)',
    'เทศบาล 4 (ชุมชนวัดธรรมจักร)',
    'เทศบาล 5 (วัดพันปี)',
    'เทศบาล 1 หนองตมศึกษา (จิ้นตง)',
    'อนุบาลเทศบาลตำบลวัดโบสถ์',
    'พิรามอุทิศ',
    'บางลาย "จิตตภาวันอุปถัมภ์"',
    'เทศบาล 1 บ้านปากทาง',
    'เทศบาล 2 บ้านท่าหลวง',
    'เทศบาล ๓  "๕ ธันวามหาราช"',
    'เทศบาล 1 บางมูลนาก',
    'เทศบาล 2 วัดชัยมงคล',
    'เทศบาลตะพานหินวิทยาคาร',
    'เทศบาล 2',
    'เทศบาล 3 วัดสันติพลาราม',
    'เทศบาล 4',
    'เทศบาลราษฎร์เจริญ',
    'องค์การบริหารส่วนตำบลหัวดง',
    'เทศบาล 1 (บ้านในเมือง)',
    'เทศบาล 2 (วัดภูเขาดิน)',
    'เทศบาล 3 (ชาญวิทยา)',
    'เทศบาลวัดประชุมคงคาราม',
    'เทศบาลบ้านศรีมงคล',
    'เทศบาลบ้านสักงอย',
    'เทศบาล 1 (วัดสัตตนารถปริวัตร)',
    'เทศบาล 2 (วัดช่องลม)',
    'เทศบาล 3 (เทศบาลสงเคราะห์)',
    'เทศบาล 4 (วัดมหาธาตุวรวิหาร)',
    'เทศบาล 5 (พหลโยธินฯ)',
    'เทศบาลวัดไทรอารีรักษ์',
    'เทศบาลวัดโชค',
    'เทศบาล 1 ทรงพลวิทยา',
    'เทศบาล 2 วัดบ้านโป่ง',
    'เทศบาล 3 ประชายินดี',
    'เทศบาลตำบลหลักเมือง',
    'บ้านเก่าวิทยา',
    'เทศบาล 1 (วัดเทวสังฆาราม)',
    'เทศบาล 2 (ประชาภิบาล)',
    'เทศบาล 3 (บ้านบ่อ)',
    'เทศบาล 4 (บ้านชุกกุ่ม)',
    'เทศบาล 5 (กระดาษไทยอนุเคราะห์)',
    'เทศบาล 1 วัดประตูสาร',
    'เทศบาล 2 วัดปราสาททอง',
    'เทศบาล 3 วัดไชนาวาส',
    'เทศบาล 4 วัดศรีบัวบาน',
    'เทศบาล 1 ตลาดบางลี่',
    'เทศบาล 2 อำนวยวิทย์',
    'เทศบาล 3 วัดใหม่อัมพวัน',
    'เทศบาล 4 วัดโพธิ์อ้น',
    'เทศบาล 5 วัดศรีสำราญ',
    'อนุบาลเทศบาลหนองหญ้าไซ',
    'องค์การบริหารส่วนตำบลบางตาเถร',
    'เทศบาล 1 วัดพระงาม',
    'เทศบาล 2 วัดเสนหา',
    'เทศบาล 3 (สระกระเทียม)',
    'เทศบาล 4 (เชาวนปรีชาอุทิศ)',
    'เทศบาล 5 วัดพระปฐมเจดีย์',
    'วิทยาลัยอาชีวศึกษาเทศบาลนครปฐม',
    'กีฬาเทศบาลนครปฐม',
    'มัธยมศึกษาเมืองทวารวดี',
    'เทศบาล 1 วัดลำลูกบัว',
    'วัดห้วยพลู',
    'บ้านปล่องเหลี่ยม',
    'วัดหลักสี่ราษฎร์สโมสร',
    'เทศบาลวัดเจษฎาราม',
    'เทศบาลวัดช่องลม',
    'เทศบาลวัดโกรกกราก',
    'เทศบาลวัดตึกมหาชยาราม',
    'เทศบาลวัดแหลมสุวรรณาราม',
    'เทศบาลบ้านมหาชัย',
    'เทศบาลวัดดอนไก่ดี',
    'เทศบาลศรีบุณยานุสสรณ์',
    'เทศบาลเมืองอ้อมน้อย',
    'อนุบาลเทศบาลอ้อมน้อย',
    'เทศบาลวัดใหญ่ (ราชพงษ์)',
    'เทศบาลแสงวณิชอุปถัมภ์',
    'เทศบาลวัดธรรมนิมิต (พ่อค้าอุทิศ)',
    'เทศบาลวัดประทุมคณาวาส (นิพัทธ์หริณสูตร)',
    'เทศบาลวัดป้อมแก้ว (อัครพงศ์ชนูปถัมภ์)',
    'เทศบาล 1 วัดนางวัง',
    'เทศบาล 2 วัดเกษม',
    'เทศบาล 3 วัดอัมพวันเจติยาราม',
    'เทศบาล 1 วัดแก่นเหล็ก',
    'เทศบาล 2 วัดพระทรง',
    'เทศบาล 3 ชุมชนวัดจันทราวาส',
    'เทศบาล 4 วัดไชยสุรินทร์',
    'เทศบาล 1 บ้านชะอำ',
    'เทศบาล 2 วัดไทรย้อย',
    'เทศบาล 3 วัดเนรัญชรา',
    'เทศบาล 4 บ้านบ่อแขม',
    'เทศบาล 5 บ้านห้วยทรายใต้',
    'เทศบาล 6 บ้านห้วยทรายเหนือ',
    'เทศบาล 7 บ้านหนองตาพด',
    'เทศบาล 8 สวนสนชะอำ',
    'เทศบาล 9 บ้านสามพระยา',
    'รัชตวิทยาคม',
    'เทศบาลวัดธรรมิการาม',
    'เทศบาลบ้านหนองบัว',
    'เทศบาลบ้านค่าย',
    'เทศบาลบ้านหัวหิน',
    'เทศบาลบ้านเขาเต่า',
    'เทศบาลบ้านตะเกียบ',
    'เทศบาลวัดหนองแก',
    'เทศบาลบ้านสมอโพรง',
    'เทศบาลบ้านบ่อฝ้าย',
    'เทศบาลเขาพิทักษ์',
    'บ้านปลายน้ำ',
    'บ้านท่าเรือมิตรภาพที่ 30',
    'บ้านน้ำโฉ',
    'ไม้เรียงประชาสรรค์',
    'วัดสำนักขัน',
    'บ้านสำนักไม้เรียบ',
    'เทศบาลวัดท้าวโคตร',
    'เทศบาลวัดเสมาเมือง',
    'เทศบาลวัดมเหยงคณ์',
    'สาธิตเทศบาลวัดเพชรจริก',
    'เทศบาลวัดใหญ่',
    'เทศบาลวัดศาลามีชัย',
    'เทศบาลวัดเสาธงทอง',
    'เทศบาลวัดศรีทวี',
    'เทศบาลวัดท่าโพธิ์',
    'เทศบาลวัดศรีสมบูรณ์',
    'เทศบาลปากพนัง 1',
    'เทศบาลวัดคงคาสวัสดิ์',
    'เทศบาลปากพนัง 2',
    'เทศบาลวัดรามประดิษฐ์',
    'เทศบาลวัดนาควารี',
    'เทศบาลวัดโคกสะท้อน',
    'เทศบาลวัดท่าแพ',
    'เทศบาลวัดชัยชุมพล',
    'เทศบาลบ้านนาเหนือ',
    'กีฬาเทศบาลเมืองทุ่งสง',
    'รีสอร์ทอนุบาล',
    'เทศบาล 1 บ้านตลาดเก่า',
    'เทศบาล 2 คลองจิหลาด',
    'เทศบาล 3 ท่าแดง',
    'เทศบาล 4 อนุบาลปานุราช',
    'เทศบาลคลองท่อมใต้',
    'เทศบาลตำบลอ่าวลึกใต้',
    'บ้านช่องพลี',
    'เทศบาลบ้านท้ายช้าง',
    'อนุบาลเทศบาลเมืองพังงา',
    'เทศบาลบ้านเสนารังสรรค์',
    'เทศบาลศรีตะกั่วป่า',
    'เทศบาลบ้านย่านยาว',
    'อนุบาลท้ายเหมือง',
    'บ้านตลาดเหนือ (วันครู 2502)',
    'เมืองภูเก็ต',
    'บ้านไม้เรียบ',
    'เทศบาลปลูกปัญญา',
    'เทศบาลเมืองภูเก็ต',
    'เทศบาลบ้านบางเหนียว',
    'เทศบาลวัดขจรรังสรรค์',
    'เทศบาลบ้านสามกอง',
    'พิบูลสวัสดี',
    'อนุบาลเทศบาลตำบลเชิงทะเล',
    'อนุบาลเทศบาลเมืองป่าตอง',
    'อนุบาลเทศบาลกะทู้',
    'ดอนสักผดุงวิทย์',
    'เทศบาล 1 (แตงอ่อนเผดิมวิทยา)',
    'เทศบาล 3 ตลาดล่าง',
    'เทศบาล 4 วัดโพธาวาส',
    'เทศบาล 5',
    'เทศบาล 1 (ห้วยมุด)',
    'เทศบาล 2 (บ้านอู่มาด)',
    'เทศบาล 3 (บ้านคลองหา)',
    'เทศบาล 4 (บ้านทุ่งคาเกรียน)',
    'เทศบาล 5 (วสุนธราภิวัฒก์)',
    'เทศบาลเมืองท่าข้าม 1 (วัดตรณาราม)',
    'เทศบาลเมืองท่าข้าม 2 (เจริญเวช)',
    'เทศบาลเมืองท่าข้าม 3 (บ้านค้อกลาง)',
    'เทศบาลเมืองท่าข้าม 4',
    'เทศบาลตำบลเกาะพะงัน',
    'อนุบาลเทศบาลตำบลกาญจนดิษฐ์',
    'เทศบาลวัดอุปนันทาราม',
    'เทศบาลบ้านเขานิเวศน์',
    'สหกรณ์ประชานุกูล',
    'บ้านทับวัง',
    'เทศบาล 1 บ้านท่าตะเภา',
    'เทศบาล 2 (วัดเกาะแก้ว)',
    'เทศบาลวัดด่านประชากร',
    'เทศบาล 1 วัดต้นสน',
    'เทศบาล 2 วัดโล่ห์สุทธาวาส',
    'เทศบาล 3 วัดชัยมงคล',
    'ชุมชนเทศบาลวัดปาโมกข์',
    'เทศบาลวัดพินิจธรรมสาร',
    'เทศบาลวัดโบสถ์วรดิตถ์',
    'เทศบาลวัดแจ้ง',
    'เทศบาลวัดอัมพวัน',
    'เทศบาลวัดแสนสุข',
    'เทศบาล 1 (ถนนนครนอก)',
    'เทศบาล 2 (อ่อนอุทิศ)',
    'เทศบาล 3 วัดศาลาหัวยาง',
    'เทศบาล 4 (บ้านแหลมทราย)',
    'เทศบาล 5 (วัดหัวป้อมนอก)',
    'เทศบาล 1 (เอ็งเสียงสามัคคี)',
    'เทศบาล 2 (บ้านหาดใหญ่)',
    'เทศบาล 3 (โศภณพิทยาคุณานุสรณ์)',
    'เทศบาล 4 (วัดคลองเรียน)',
    'เทศบาล 5 วัดหาดใหญ่',
    'เทศบาล 1 บ้านสะเดา',
    'เทศบาล 2 บ้านสะเดา',
    'เทศบาล 3 ชุมชนหมู่บ้านตัวอย่าง',
    'เทศบาล 1 (ชุมชนบ้านอุดมทอง)',
    'อนุบาลเทศบาลตำบลคลองแห (ชุมชนทุ่งปาบ)',
    'เทศบาล 1 (วัดสตูลสันตยาราม)',
    'เทศบาล 2 (วัดชนาธิปเฉลิม)',
    'เทศบาล 3 (บ้านหัวทาง)',
    'เทศบาล 4 (บ้านศาลากันตง)',
    'เทศบาล 1 (สังขวิทย์)',
    'เทศบาล 2 (วัดกะพังสุรินทร์)',
    'เทศบาล 3 (บ้านนาตาล่วง)',
    'เทศบาล 4 (วัดมัชฌิมภูมิ)',
    'เทศบาล 5 (วัดควนขัน)',
    'เทศบาล 6 (วัดตันตยาภิรม)',
    'เทศบาล 7 (วัดประสิทธิชัย)',
    'เทศบาลบ้านคลองภาษี',
    'เทศบาลวัดตรังคภูมิพุทธาวาส',
    'เทศบาลห้วยยอดวิทยา',
    'เทศบาลวัดภูผาภิมุข',
    'เทศบาลวัดนิโครธาราม',
    'เทศบาลบ้านคูหาสวรรค์',
    'เทศบาลจุ่งฮั่ว',
    'วัดนางลาด',
    'วัดธาราสถิตย์',
    'บ้านเขาตูม',
    'บ้านตะบิงตีงิ',
    'เทศบาล 1 บ้านจะบังติกอ',
    'เทศบาล 2 วัดตานีนรสโมสร',
    'เทศบาล 3 บ้านปากน้ำ',
    'เทศบาล 4 วัดนพวงศาราม',
    'เทศบาล 5 อาคารสลากกินแบ่งรัฐบาล',
    'เทศบาลบ้านปาตาตีมอ',
    'เทศบาลบ้านกาหยี',
    'เทศบาลบ้านตะลุบัน',
    'เทศบาลบ้านอุเมะ',
    'เทศบาลบ้านบางตาหยอด',
    'เทศบาลบ้านปากน้ำ',
    'ลำพระยาประชานุเคราะห์',
    'เทศบาล 1 (บ้านสะเตง)',
    'เทศบาล 2 (บ้านมลายูบางกอก)',
    'เทศบาล 3 (วัดพุทธภูมิ)',
    'เทศบาล 4 (ธนวิถี)',
    'เทศบาล 5 (บ้านตลาดเก่า)',
    'เทศบาล 6 (วัดเมืองยะลา)',
    'เทศบาล 1 (บ้านกาแป๊ะ)',
    'เทศบาล 2 (บ้านกาแป๊ะกอตอ)',
    'เทศบาล 3 (บ้านกุนุงจนอง)',
    'เทศบาล 4 (บ้านกาแป๊ะฮูลู)',
    'เทศบาล 6',
    'เทศบาล 1 (ถนนภูผาภักดี)',
    'เทศบาล 2 (บ้านบาเละฮิเล)',
    'เทศบาล 3 (บ้านยะกัง)',
    'เทศบาล 4 กำปงตาโก๊ะ',
    'เทศบาล 5 (วัดประชาภิรมย์)',
    'เทศบาล 6 (ถนนโคกเคียน)',
    'เทศบาล 1 (ราษฎรบำรุง)',
    'เทศบาล 2 (บ้านตันหยงมะลิ)',
    'เทศบาล 3 (วิมุกตายนวิทยา)',
    'เทศบาล 4 (บ้านทรายทอง)',
    'วัดมกุฎกษัตริยาราม',
    'วัดตรีทศเทพ',
    'วัดราชนัดดา',
    'วัดมหรรณพ์',
    'วัดสุทัศน์',
    'วัดมหาธาตุ',
    'วัดพระเชตุพน',
    'ราชบพิธ',
    'วัดราชบูรณะ',
    'วัดอินทรวิหาร',
    'วัดใหม่อมตรส',
    'วัดพระพิเรนทร์',
    'วัดสิตาราม',
    'วัดคณิกาผล',
    'วัดดิสานุการาม',
    'วัดบรมนิวาส',
    'สวนหลวง',
    'วัดสระบัว',
    'สวนลุมพินี',
    'ปทุมวัน',
    'วัดดวงแข',
    'ปลูกจิต',
    'วัดจักรวรรดิ์',
    'วัดปทุมคงคา',
    'วัดสัมพันธวงศ์',
    'วัดหัวลำโพง',
    'วัดม่วงแค',
    'วัดมหาพฤฒาราม',
    'วัดสวนพลู',
    'วัดแก้วแจ่มฟ้า',
    'วัดประชาระบือธรรม',
    'วัดสวัสดิวารีสีมาราม',
    'วัดราชผาติการาม',
    'วัดเทวราชกุญชร',
    'วัดสัมณานัมบริหาร',
    'เบญจมบพิตร',
    'สุโขทัย',
    'วัดจันทรสโมสร',
    'วัดธรรมาภิรตาราม',
    'วัดประดู่ธรรมาธิปัตย์',
    'วัดบางโพโอมาวาส',
    'วัดสร้อยทอง',
    'วัดทองสุทธาราม',
    'วัดเลียบราษฎร์บำรุง',
    'วัดประชาศรัทธาธรรม',
    'วัดไผ่ตัน',
    'วัดดิสหงสาราม',
    'วัดพระยายัง',
    'วัดทัศนารุณสุนทริการาม',
    'วัดดอกไม้',
    'วัดช่องลม',
    'วัดช่องนนทรี',
    'วัดปริวาศ',
    'วัดคลองใหม่',
    'วัดคลองภูมิ',
    'วัดดอน',
    'วัดยานนาวา',
    'วัดไผ่เงินโชตนาราม',
    'วัดราชสิงขร',
    'วัดลาดบัวขาว',
    'วัดไทร',
    'วัดจันทร์นอก',
    'วัดบางโคล่นอก',
    'วัดจันทร์ใน',
    'บางจาก (นาคเผื่อนอุปถัมภ์)',
    'วัดราษฎร์ศรัทธาธรรม',
    'วัดธรรมมงคล (หลวงพ่อวิริยังค์อุปถัมภ์)',
    'วัดสะพาน',
    'วัดคลองเตย',
    'ศูนย์รวมน้ำใจ',
    'วัดตะกล่ำ',
    'แก่นทองอุปถัมภ์',
    'วัดกระทุ่มเสือปลา',
    'คลองมะขามเทศ',
    'สุเหร่าจรเข้ขบ',
    'สุเหร่าทางควาย',
    'สุเหร่าทับช้าง',
    'สุเหร่าบึงหนองบอน',
    'สุเหร่าศาลาลอย',
    'สุเหร่าบ้านม้า',
    'งามมานะ (แผน-ทับอุทิศ)',
    'คชเผือกอนุสรณ์',
    'สุวิทย์เสรีอนุสรณ์',
    'อยู่เป็นสุขอนุสรณ์',
    'บ้านคลองบัว',
    'บ้านบัวมล (เจริญราษฎร์อุทิศ)',
    'ประชาภิบาล',
    'วัดไตรรัตนาราม',
    'วัดดอนเมือง (ทหารอากาศอุทิศ)',
    'พหลโยธิน',
    'เปรมประชา',
    'วัดเสมียนนารี',
    'วัดเทวสุนทร',
    'บ้านลาดพร้าว',
    'รัตนโกสินทร์สมโภช',
    'ประชานิเวศน์',
    'ไขศรีปราโมชอนุสรณ์',
    'สุเหร่าคลองจั่น',
    'วัดเทพลีลา',
    'วัดศรีบุญเรือง',
    'ลำสาลี (ราษฎร์บำรุง)',
    'คลองกะจะ',
    'สุเหร่าวังใหญ่',
    'สุเหร่าหัวหมากน้อย',
    'บ้านบางกะปิ',
    'วัดบึงทองหลาง',
    'ลอยสายอนุสรณ์',
    'เทพวิทยา',
    'วัดลาดปลาเค้า',
    'เพชรถนอม (คลองเสือน้อย)',
    'คลองทรงกระเทียม',
    'วัดบางเตย',
    'ประภาสวิทยา',
    'คลองลำเจียก',
    'วัดนวลจันทร์',
    'วัดพิชัย',
    'พิชัยพัฒนา',
    'แย้มจาดวิชชานุสรณ์',
    'วัดประยุรวงค์',
    'วัดขุนจันทร์',
    'วัดบุคคโล',
    'วัดดาวคนอง',
    'วัดกัลยาณมิตร',
    'วัดราชคฤห์',
    'วัดประดิษฐาราม',
    'วัดบางสะแกใน',
    'วัดเวฬุราชิณ',
    'กันตทาราราม',
    'วัดโพธินิมิตร',
    'วัดใหม่ยายนุ้ย',
    'วัดกระจับพินิจ',
    'วัดราชวรินทร์',
    'วัดบางสะแกนอก',
    'วัดใหญ่ศรีสุพรรณ',
    'วัดบางน้ำชน',
    'วัดทองธรรมชาติ',
    'วัดทองนพคุณ',
    'วัดพิชัยญาติ',
    'วัดสุวรรณ',
    'วัดนิมมานรดี',
    'วัดจันทร์ประดิษฐาราม',
    'วัดชัยฉิมพลี',
    'วัดโคนอน',
    'วัดมะพร้าวเตี้ย',
    'วัดโตนด',
    'วัดตะล่อม',
    'วัดกำแพง',
    'วัดทองศาลางาม',
    'วัดวิจิตรการนิมิตร',
    'วัดประดู่ (พ่วงอุทิศ)',
    'บางจาก',
    'หมู่บ้านเกาะโพธิ์',
    'คลองห้วยทราย',
    'แก้วขำทับอุปถัมภ์',
    'คลองพิทยาลงกรณ์',
    'ศาลเจ้า (ห้าวนุกูลวิทยา)',
    'วัดท่าข้าม',
    'วัดบัวผัน',
    'วัดประชาบำรุง',
    'วัดเลา',
    'บางขุนเทียนศึกษา',
    'วัดมงคลวราราม',
    'วัดนางนอง',
    'วัดศาลาครืน',
    'วัดสีสุก',
    'วัดบางประทุนนอก',
    'วัดโพธิ์แก้ว',
    'วัดนาคนิมิตร',
    'วัดบางขุนเทียนนอก',
    'วัดไทร',
    'วัดบางขุนนนท์',
    'วัดวิเศษการ',
    'วัดเจ้าอาม',
    'วัดโพธิ์เรียง',
    'วัดยางสุทธาราม',
    'วัดศรีสุดาราม',
    'วัดบางเสาธง',
    'วัดสุวรรณาราม',
    'วัดพระยาทำ',
    'วัดดงมูลเหล็ก',
    'วัดอัมพวา',
    'วัดสุวรรณคีรี',
    'วัดดุสิตาราม',
    'วัดปฐมบุตรอิศราราม',
    'วัดอาวุธวิกสิตาราม',
    'วัดพระยาศิริไอยสวรรค์',
    'วัดเปาโรหิตย์',
    'วัดเทพากร (เลี่ยมมาตุทิศ)',
    'วัดฉัตรแก้วจงกลณี',
    'วัดวิมุตยาราม',
    'วัดบางพลัด',
    'วัดคฤหบดี',
    'วัดรวก',
    'วัดสามัคคีสุทธาวาส',
    'วัดไก่เตี้ย (สวัสดิ์ประชานุกูล)',
    'วัดชัยพฤกษมาลา',
    'ชุมทางตลิ่งชัน',
    'วัดทอง (อุดมศิลปวิทยาคาร)',
    'วัดมะกอก',
    'วัดโพธิ์ (ราษฏร์ผดุงผล)',
    'วัดอินทราวาส',
    'วัดปากน้ำฝั่งเหนือ',
    'วัดประสาท',
    'วัดเกาะ',
    'วัดพิกุล',
    'วัดกระโจมทอง',
    'วัดรัชฎาธิฐาน',
    'วัดตลิ่งชัน',
    'วัดช่างเหล็ก',
    'วัดราชสิทธาราม',
    'วัดท่าพระ',
    'วัดประดู่ฉิมพลี',
    'วัดใหม่พิเรนทร์',
    'วัดดีดวด',
    'วัดนาคกลาง',
    'คลองสองต้นนุ่น',
    'คลองสาม',
    'วังเล็กวิทยานุสรณ์ (คลองสี่)',
    'บึงขวาง (มหาดไทยอุปถัมภ์)',
    'มีนบุรี',
    'วัดบำเพ็ญเหนือ',
    'วัดแสนสุข',
    'วัดใหม่ลำนกแขวก',
    'ศาลาคู้',
    'สุเหร่าบางชัน',
    'วัดปลูกศรัทธา',
    'วัดบำรุงรื่น',
    'วัดพลมานีย์',
    'วัดสุทธาโภชน์',
    'แสงหิรัญวิทยา',
    'ประสานสามัคคี (บ้านทับยาว)',
    'วัดบึงบัว',
    'วัดทิพพาวาส',
    'ลำพะอง',
    'วัดขุมทอง',
    'ตำบลขุมทอง (ประชาอุทิศ)',
    'ขุมทอง(เพชรทองคำอุปถัมภ์)',
    'วัดปากบึง',
    'สุเหร่าลำนายโส',
    'แดงเป้า (สิงสุขบูรณะ)',
    'เคหะชุมชนลาดกระบัง',
    'วัดหนองจอก (ภักดีนรเศรษฐ)',
    'สุเหร่าหะยีมินา',
    'สุเหร่าบ้านเกาะ',
    'ลำบุหรี่พวง',
    'สุเหร่าศาลาแดง',
    'สุเหร่าใหม่',
    'สามแยกท่าไข่',
    'หลวงแพ่ง (บำรุงรัฐกิจ)',
    'นิลราษฎร์อุปถัมภ์',
    'บ้านเจียรดับ',
    'วัดทรัพย์สโมสร',
    'สุเหร่านาตับ',
    'สุเหร่าอีรั้ว',
    'อิสลามลำไทร',
    'บ้านลำต้นกล้วย',
    'วัดพระยาปลา',
    'วัดแสนเกษม',
    'วัดใหม่เจริญราษฎร์',
    'วัดสีชมพู',
    'ลำเจดีย์',
    'สุเหร่าคลองเก้า',
    'สุเหร่าคลองสิบ',
    'สุเหร่าคลองสิบเอ็ด',
    'สุเหร่าสนามกลางลำ',
    'วัดใหม่กระทุ่มล้ม',
    'ผลลีรุ่งเรือง',
    'วัดราษฎร์บำรุง',
    'คลองสอง',
    'คารีอุปถัมภ์',
    'ศิริวังวิทยาคาร',
    'วัดอู่ตะเภา',
    'ลำผักชี',
    'วัดประเสริฐสุทธาวาส',
    'วัดแจงร้อน',
    'วัดสน',
    'วัดสารอด',
    'วัดหนองแขม',
    'ประชาบำรุง',
    'คลองบางแวก (มนต์ จรัสสิงห์)',
    'วัดศรีนวลธรรมวิมล',
    'พระราม ๙ กาญจนาภิเษก',
    'วัดใหม่ช่องลม',
    'วัดใต้ (ราษฎรนิรมิต)',
    'นาคนาวาอุปถัมภ์',
    'สุเหร่าใหม่',
    'หัวหมาก',
    'คลองกลันตัน',
    'วัดทองใน',
    'วิชากร',
    'วัดหลักสี่ (ทองใบ ทิวารี)',
    'เคหะทุ่งสองห้องวิทยา 2',
    'บางเขน (ไว้สาลีอนุสรณ์)',
    'วัดเกาะสุวรรณาราม',
    'ออเงิน',
    'ฤทธิยะวรรณาลัย',
    'ประชานุกูล',
    'พรพระร่วงประสิทธิ์',
    'วัดราษฎร์นิยมธรรม',
    'ซอยแอนเนกซ์',
    'สุเหร่าทับช้างคลองบน',
    'วัดลาดบัวขาว',
    'สมโภชกรุงอนุสรณ์ (200 ปี)',
    'สุเหร่าลาดบัวขาว',
    'จินดาบำรุง',
    'คันนายาว',
    'สุเหร่าลาดพร้าว',
    'สุเหร่าดอนสะแก',
    'วัดสามัคคีธรรม',
    'กลางคลองสอง',
    'บ้านแบนชะโด',
    'บ้านหนองระแหง (ชมแสงประชานุกูล)',
    'ประชาราษฎร์อุปถัมภ์',
    'วัดบัวแก้ว',
    'วัดแป้นทอง',
    'วัดพระยาสุเรนทร์',
    'วัดลำกระดาน',
    'วัดศรีสุก',
    'วัดสุขใจ',
    'วัดสุทธิสะอาด',
    'สุเหร่าเกาะขุนเณร',
    'สุเหร่าคลองสี่',
    'สุเหร่าคลองหนึ่ง',
    'สุเหร่าสามวา',
    'วัดธาตุทอง',
    'สุเหร่าบางมะเขือ',
    'สุเหร่าสามอิน',
    'วัดภาษี',
    'แจ่มจันทร์',
    'สวัสดีวิทยา',
    'บางแค (เนื่องสังวาลย์อนุสรณ์)',
    'เพชรเกษม (จตุรงค์สงครามอนุสรณ์)',
    'วัดศาลาแดง',
    'บางไผ่ (บ้านนายพันแก้วขาว)',
    'บางเชือกหนัง',
    'วัดพรหมสุวรรณสามัคคี',
    'บางแคเหนือ',
    'วัดม่วง',
    'หมู่บ้านเศรษฐกิจ',
    'วัดบางนาใน',
    'วัดบางนานอก',
    'ผ่องพลอยอนุสรณ์',
    'อำนวยกนกศิริอนุสรณ์',
    'ศรีเอี่ยมอนุสรณ์',
    'บ้านนายผล',
    'บ้านนายสี',
    'วัดบางบอน',
    'พรหมราษฎร์รังสรรค์',
    'สถานีพรมแดน',
    'วัดนินสุขาราม',
    'คลองต้นไทร (สุขล้อมอุทิศ)',
    'คลองทวีวัฒนา',
    'คลองบางพรหม',
    'คลองมหาสวัสดิ์',
    'ตั้งพิรุฬห์ธรรม',
    'นาหลวง',
    'สามัคคีบำรุง',
    'บางมด (ตันเปาว์วิทยาคาร)',
    'ราษฎร์บูรณะ',
    'รางราชพฤกษ์ (นุชมีอุทิศ)',
    'วัดทุ่งครุ',
    'คลองรางจาก',
    'ชูสินทองประดิษฐ์อนุสรณ์',
    'วัดชัยมงคล',
    'วัดปทุมวนาราม',
    'วัดมัชฌันติการาม',
    'กิ่งเพชร',
    'พูนสิน (เพชรสุขอุปถัมภ์)',
    'ชุมชนหมู่บ้านพัฒนา',
    'คลองปักหลัก',
    'มัธยมสุวิทย์เสรีย์อนุสรณ์',
    'ไทยนิยมสงเคราะห์',
    'วัดเวฬุวนาราม (สินทรัพย์อนุสรณ์)',
    'ประชาอุทิศ (จันทาบอนุสรณ์)',
    'บำรุงรวิวรรณวิทยา (ทุ่งสีกัน)',
    'เสนานิคม',
    'มัธยมประชานิเวศน์',
    'มัธยมบ้านบางกะปิ',
    'วัดลาดพร้าว',
    'คลองกุ่ม',
    'วัดเศวตฉัตร',
    'วัดสุทธาราม',
    'วัดทองเพลง',
    'วัดอ่างแก้ว (จีบ ปานขำ)',
    'วัดหัวกระบือ',
    'วัดสะแกงาม',
    'วัดแสมดำ',
    'วัดบางกระดี่',
    'วัดกำแพง',
    'วัดกก',
    'วัดโพธิ์ทอง',
    'วัดยายร่ม',
    'วัดมะลิ',
    'บางยี่ขันวิทยาคม',
    'ฉิมพลี',
    'บ้านเกาะ',
    'วัดทองสัมฤทธิ์',
    'สุเหร่าทรายกองดิน',
    'วัดลาดกระบัง',
    'วัดสังฆราชา',
    'วัดราชโกษา',
    'วัดลานบุญ',
    'หนองจอกพิทยานุสรณ์',
    'วัดสามง่าม',
    'สุเหร่าลำแขก',
    'สังฆประชานุสสรณ์',
    'วัดลำต้อยติ่ง',
    'วัดบางปะกอก',
    'รัตนจีนะอุทิศ',
    'วัดอุดมรังสี',
    'บ้านขุนประเทศ',
    'ประชาราษฎร์บำเพ็ญ',
    'วัดปากบ่อ',
    'มัธยมนาคนาวาอุปถัมภ์',
    'สามเสนนอก',
    'วิชูทิศ',
    'ทุ่งสองห้อง (คุปตัษเฐียรอุทิศ)',
    'เคหะทุ่งสองห้องวิทยา 1',
    'การเคหะท่าทราย',
    'สายไหม',
    'วัดหนองใหญ่',
    'สุเหร่าซีรอ',
    'สามแยกคลองหลอแหล',
    'บางชัน',
    'วัดคู้บอน',
    'สุเหร่าแสนแสบ',
    'วิจิตรวิทยา',
    'สุเหร่าบ้านดอน',
    'วัดบุณยประดิษฐ์',
    'คลองหนองใหญ่',
    'วัดราษฎร์บำรุง',
    'รุ่งเรืองอุปถัมภ์',
    'เพี้ยนพินอนุสรณ์',
    'บ้านนายเหรียญ',
    'พระยามนธาตุราชศรีพิจิตร์',
    'วัดปุรณาวาส',
    'วิทยาลัยพยาบาลเกื้อการุณย์',
    'วิทยาลัยแพทย์ศาสน์ กทม.',
    'ศูนย์ฝึกพาณิชย์นาวี',
    'วิทยาลัยพยาบาลบรมราชชนนี นพรัตน์วชิระ',
    'วิทยาลัยพยาบาลบรมราชชนนี กรุงเทพ',
    'วิทยาลัยพยาบาลบรมราชชนนี จังหวัดนนทบุรี',
    'วิทยาลัยเทคโนโลยีทางการแพทย์และสาธารณสุขกาญจนาภิเษก',
    'วิทยาลัยพยาบาลบรมราชชนนี ชัยนาท',
    'วิทยาลัยพยาบาลบรมราชชนนี สระบุรี',
    'วิทยาลัยพยาบาลบรมราชชนนี พระพุทธบาท',
    'วิทยาลัยการสาธารณสุขสิรินธร จังหวัดชลบุรี',
    'วิทยาลัยพยาบาลบรมราชชนนี ชลบุรี',
    'วิทยาลัยพยาบาลพระปกเกล้า จันทบุรี',
    'วิทยาลัยพยาบาลบรมราชชนนี นครราชสีมา',
    'วิทยาลัยพยาบาลบรมราชชนนี สุรินทร์',
    'วิทยาลัยพยาบาลบรมราชชนนี สรรพสิทธิประสงค์',
    'วิทยาลัยการสาธารณสุขสิรินธร จังหวัดอุบลราชธานี',
    'วิทยาลัยพยาบาลบรมราชชนนี  ขอนแก่น',
    'วิทยาลัยการสาธารณสุขสิรินธร  จังหวัดขอนแก่น',
    'วิทยาลัยพยาบาลบรมราชชนนี อุดรธานี',
    'วิทยาลัยพยาบาลศรีมหาสารคาม',
    'วิทยาลัยพยาบาลบรมราชชนนี  เชียงใหม่',
    'วิทยาลัยพยาบาลบรมราชชนนี นครลำปาง',
    'วิทยาลัยพยาบาลบรมราชชนนี อุตรดิตถ์',
    'วิทยาลัยพยาบาลบรมราชชนนี  พะเยา',
    'วิทยาลัยพยาบาลบรมราชชนนี     สวรรค์ประชารักษ์  นครสวรรค์',
    'วิทยาลัยพยาบาลบรมราชชนนี พุทธชินราช',
    'วิทยาลัยการสาธารณสุขสิรินธร จังหวัดพิษณุโลก',
    'วิทยาลัยพยาบาลบรมราชชนนี ราชบุรี',
    'วิทยาลัยพยาบาลบรมราชชนนี จักรีรัช',
    'วิทยาลัยการสาธารณสุขสิรินธร จังหวัดสุพรรณบุรี',
    'วิทยาลัยพยาบาลบรมราชชนนี สุพรรรณบุรี',
    'วิทยาลัยพยาบาลพระจอมเกล้า เพชรบุรี',
    'วิทยาลัยพยาบาลบรมราชชนนี นครศรีธรรมราช',
    'วิทยาลัยพยาบาลบรมราชชนนี  สุราษฏร์ธานี',
    'วิทยาลัยพยาบาลบรมราชชนนี สงขลา',
    'วิทยาลัยพยาบาลบรมราชชนนี ตรัง',
    'วิทยาลัยการสาธารณสุขสิรินธร จังหวัดตรัง',
    'วิทยาลัยการสาธารณสุขสิรินธร  จังหวัดยะลา',
    'วิทยาลัยพยาบาลบรมราชชนนี  ยะลา',
    'วิทยาลัยนาฎศิลป',
    'สถาบันบัณฑิตพัฒนศิลป์',
    'วิทยาลัยช่างศิลป',
    'วิทยาลัยนาฎศิลปอ่างทอง',
    'วิทยาลัยนาฎศิลปลพบุรี',
    'วิทยาลัยนาฎศิลปจันทบุรี',
    'วิทยาลัยนาฎศิลปนครราชสีมา',
    'วิทยาลัยนาฎศิลปร้อยเอ็ด',
    'วิทยาลัยนาฎศิลปกาฬสินธุ์',
    'วิทยาลัยนาฎศิลปเชียงใหม่',
    'วิทยาลัยนาฎศิลปสุโขทัย',
    'วิทยาลัยช่างศิลปสุพรรณบุรี',
    'วิทยาลัยนาฎศิลปสุพรรณบุรี',
    'วิทยาลัยนาฎศิลปนครศรีธรรมราช',
    'วิทยาลัยช่างศิลปนครศรีธรรมราช',
    'วิทยาลัยนาฎศิลปพัทลุง',
    'วชิรมกุฏ',
    'บาลีสาธิตศึกษา มจร.',
    'บาลีเตรียมอุดมศึกษา',
    'วัดศรีบุญเรือง',
    'มังกรกมลาวาสวิทยาลัย',
    'พระปริยัติธรรมวัดธรรมมงคล',
    'กุศลสมาครวิทยาลัย',
    'วัดกาญจนสิงหาสน์วิทยา',
    'พุทธศาสตร์วิทยา',
    'พระปริยัติธรรมแผนกสามัญศึกษาวัดเวตวันธรรมาวาส',
    'พระปริยัติธรรมวัดพรหมรังษี',
    'พระปริยัติธรรมวัดผ่องพลอยวิริยาราม',
    'เตรียมพุทธศาสตร์',
    'วัดโกเมศรัตนาราม',
    'วัดนิเวศธรรมประวัติ',
    'มหาวชิราลงกรณราชวิทยาลัย',
    'ศรีเมืองทองวิทยา',
    'พระปริยัติธรรม วัดไชโยวรวิหาร',
    'วัดเกาะปริยัติวิทยา',
    'กิตติยารามวิทยา',
    'ขุมทองวิทยา',
    'พระปริยัติธรรมวัดกวิศราราม',
    'พระปริยัติธรรมวัดลำนารายณ์',
    'วัดโบสถ์อินทร์บุรี',
    'พิพิธสุตคุณานุสรณ์',
    'วัดไผ่ดำ แผนกสามัญศึกษา',
    'พระปริยัติธรรมวัดโพธิ์ศรี',
    'พระปริยัติธรรมพลานุสรณ์วิทยา',
    'โพธิ์ทัตปัทมสัมภาวะศึกษา',
    'เครือวัลย์วิทยา',
    'ผาสุกวิทยา',
    'พระปริยัติธรรมแผนกสามัญศึกษาวัดเนินบุญญาราม',
    'มงคลวิทยา',
    'พระปริยัติธรรมอำนวยวิทยา แผนกสามัญศึกษา',
    'พระปริยัติธรรมวัดสุวรรณรังสรรค์',
    'พระปริยัติธรรมวัดโสธรวราราม',
    'วัดจุกเฌอ',
    'พุทธศาสตร์ศึกษา',
    'พระปริยัติธรรมแผนกสามัญศึกษาวัดแจ้ง',
    'พระปริยัติธรรมวัดอุดมธานี',
    'ปริยัติสามัญนครราชสีมา',
    'บูรพาวิทยานุสรณ์',
    'พระปริยัติธรรมวัดบึงกิตติวิทยา',
    'วัดตะกุดเครือปลอก',
    'สัทธรรมวิทยา',
    'ดอนหวายพิทยาคม',
    'ประชานิมิตรวิจัยศึกษา',
    'ศรีปริยัติคุณศึกษา',
    'พระปริยัตธรรมปักธงชัยวิทยา',
    'วิสุทธิพรตพิทยาคม',
    'พระปริยัตธรรมอุทัยวิทยา',
    'ปริยัติสามัญศึกษา อำเภอปากช่อง',
    'ปริยัติธีรวิทยา',
    'พระปริยัติธรรมอิสาณราชวิทยานุสรณ์',
    'พระปริยัติธรรมตลาดชัยวิทยา',
    'วัดท่าสว่างวิทยา',
    'พระปริยัติธรรมเย้ยปราสาทวิทยา',
    'ปริยัติสามัญวัดโพนทองธรรมวิทย์',
    'วัดเวฬุวันวิทยา',
    'บาลีสาธิตศึกษา มจร. วัดอภิสิทธ์',
    'อุทัยทิศวิทยา',
    'วัดสุวรรณาวาส',
    'ปัจจิมเชียงยืน',
    'ปทุมพิทยากร',
    'วัดหัวหนองสังฆประชาสรรค์',
    'วัดศรีเมืองนาเชือก',
    'ปริยัติธรรมทัพป่าจิกวิทยา',
    'พระปริยัติธรรมวัดหัวดงนาค่าย',
    'วาปีคณานุสรณ์วิทยา',
    'หนองแวงวิทยาคม',
    'พระปริยัติธรรมวัดยางสีสุราช',
    'ปริยัติสามัญโพนทองวิทยานุสรณ์',
    'เทพบัณฑิตวิทยาลัย',
    'ธีรปริยัติ',
    'พระปริยัติธรรมแผนกสามัญศึกษาวัดบูรพาภิราม',
    'มงคลญาณปริยัติ',
    'สุนทรธรรมปริยัติ',
    'พระปริยัติธรรมแผนกสามัญศึกษาวัดธาตุ',
    'พระปริยัติธรรมศรีอริยวงศ์',
    'สามัคคีปริยัติธรรม',
    'ปริยัติสารคุณ',
    'วิโรจน์ผดุงศาสน์',
    'ศรีทองนพคุณวิทยา',
    'พระปริยัติธรรมทุ่งศรีเมืองวิทยา',
    'สุพรรณภูมิพิทยา',
    'สุเทพนครวิช',
    'พระปริยัติธรรมสามัญวัดสิมนาโก',
    'พระปริยัตธรรมสามัญวัดกลาง',
    'ชัยมงคลวิทยา',
    'พระปริยัติธรรมวัดปฐมแพงศรี',
    'วัดบุณฑริกกาวาส',
    'สว่างอารมณ์วิทยา',
    'ไชยวารวิทยาเสริม',
    'หนองบัวพัฒนาวิทยา',
    'สมเด็จปริยัติ',
    'พระปริยัติธรรมแผนกสามัญศึกษาวัดแจ้งแสงอรุณ',
    'พระปริยัติธรรมแผนกสามัญศึกษาวัดศรีสุมังคล์',
    'วัดพระธาตุเชิงชุมวรวิทยา',
    'วัดพระธาตุนารายณ์เจงเวง',
    'เหล่าแมดวิทยา',
    'วัดโพธิ์คำ',
    'วัดพระธาตุศรีมงคล',
    'วัดทุ่ง',
    'พระปริยัติธรรมแผนกสามัญศึกษาวัดสว่างภูมิดล',
    'พระปริยัติธรรม แผนกสามัญศึกษา วัดธาตุประสิทธิ์',
    'พระปริยัติธรรมวัดมหาธาตุ',
    'คณิศรธรรมิการาม',
    'วัดพระธาตุพนม',
    'พระปริยัติธรรมวัดธาตุเรณูวิทยา',
    'พระปริยัติธรรมแผนกสามัญศึกษาวัดพระซองวิทยา',
    'วัดศรีสงคราม',
    'พระปริยัติธรรมแผนกสามัญศึกษาวัดศรีชมภู',
    'พระปริยัติธรรมวัดศรีหนาถวิทยา',
    'พระปริยัติธรรมแผนกสามัญศึกษาวัดศรีสว่าง',
    'วัดศรีบุญเรืองวิทยา',
    'สว่างอารมณ์วิทยา',
    'สมเด็จพระพุทธชินวงศ์',
    'ดอยสะเก็ดผดุงศาสน์',
    'วัดสะเมิงวิทยา',
    'เชตุพนศึกษา',
    'บาลีสาธิตศึกษา มจร.',
    'พลราชมวิทยากร',
    'ศรีบุญเรืองปริยัติศึกษา',
    'สามัคคีวิทยาทาน',
    'อภัยอริยศึกษา',
    'แม่แจ่มปริยัติศึกษา',
    'วัดอินทารามวิทยา',
    'พระปริยัติธรรมแม่แตงวิทยา',
    'วัดแม่ริมวิทยา',
    'ฝางธรรมศึกษา',
    'วัดเจดีย์งามปริยัติวิทยา',
    'วัดพระบาทอุดม',
    'พระปริยัติธรรมวัดท่าตอน',
    'เขื่อนผากวิทยา',
    'สันป่าตองศึกษา',
    'วัดวิเวกวนาราม',
    'วัดฉิมพลีวิทยา',
    'เวียงแหงปริยัติศึกษา',
    'พระปริยัตธรรมวัดป่าไม้แดง',
    'วัดโขงขาววิทยา',
    'หางดงสังฆราษฎร์อุปถัมภ์',
    'วัดบ้านขุนเตรียมพุทธศาสตร์',
    'สารภีปริยัติศึกษา',
    'สัพพัญญูวิทยา',
    'พระปริยัติธรรมพระชัยยะวงศาพัฒนา',
    'วัดท่าช้างวิทยา',
    'โสภณวิทยา',
    'หอปริยัติศึกษาวัดพระธาตุหริภุญชัย',
    'อินทะวิทยา',
    'ปริยัติสามัญสามัคคีวิทยา',
    'บ้านโฮ่งหลวง',
    'พระปริยัติธรรมพระเจ้าตนหลวง',
    'วัดพวงคำ',
    'พระปริยัติธรรมแผนกสามัญศึกษาวัดทุ่งหัวช้าง',
    'พรหมจักรสังวร',
    'ปริยัติวัดม่อนจำศีล',
    'วิฑิตธรรมคุณอุปถัมภ์',
    'แม่สุกศึกษา',
    'บ้านใหม่วิทยา',
    'บ้านหม้อศึกษา',
    'แม่พระปริยัติศึกษา',
    'วัดมงคลนิมิตร',
    'พระปริยัติธรรมแผนกสามัญศึกษาวัดเสาหิน',
    'พุทธโกศัยวิทยา',
    'ร้องเข็มวิทยา',
    'เชตวันวิทยา',
    'ร้องแหย่งวิทยาคม',
    'ศรีคีรินทร์วิทยา',
    'วังฟ่อนวิทยา',
    'พระปริยัติธรรมานุสรณ์ วัดฟ้าสวรรค์',
    'เชียงกลางปริยัติศึกษา',
    'นันทบุรีวิทยา',
    'วัดนาราบวิทยา',
    'วัดปรางค์',
    'พระปริยัติธรรมวัดนิโครธาราม',
    'วัดน้ำไคร้นันทชัยศึกษา',
    'พระปริยัติศาสนาภิพัฒน์วัดเมืองราม',
    'วัดบุญยืน',
    'วัดราษฎร์บำรุงวิทยา',
    'ดอนมงคลสันติสุขวิทยา',
    'วัดบ่อหลวงวิทยาธรรม',
    'ราชคฤห์วิทยา',
    'ห้วยข้าวก่ำวิทยา',
    'วัดหย่วนวิทยา',
    'ป่าแขมวิทยา',
    'สุวรรณคณานุสรณ์',
    'พระปริยัติธรรมแผนกสามัญศึกษาวัดปัวดอย',
    'นาปรังวิทยา',
    'พระปริยัติธรรมแผนกสามัญศึกษาวัดโพธาราม',
    'วัดม่วงชุมวิทยา',
    'วัดฝังหมิ่นวิทยา',
    'วัดครึ่งใต้ตรีนิคมวิทยา',
    'พุทธิวงศ์วิทยา',
    'วัดเวียงชัยพิทยา',
    'โสภณจริยธรรมวิทยา',
    'วัดอำมาตย์วิทยา',
    'ชยาภิวัฒน์วิทยา',
    'วัดใหม่ใต้วิทยา',
    'วัดแม่คำวิทยา',
    'วัดหมื่นพุทธวิทยา',
    'วัดไชยสถานวิทยา',
    'วัดวิเชตร์มณี',
    'วัดเจดีย์หลวงวิทยา',
    'วัดสันหนองบัววิทยา',
    'วัดป่าตาลใต้',
    'บุญเรืองวิทยา',
    'คีรีวงศ์วิทยา',
    'บาลีเตรียมอุดมศึกษาวัดโพธาราม',
    'วัดท่าพระเจริญพรต',
    'วัดไทรเหนือวิทยา',
    'วัดหนองปลิงวิทยา',
    'พระปริยัติธรรมแผนกสามัญศึกษาวัดส้มเสี้ยว',
    'วัดวาปีรัตนารามวิทยา',
    'วัดโคกเดื่อวิทยา',
    'ศรีนภเขตวิทยา',
    'วัดมณีสถิตกปิฏฐาราม',
    'อุทานอุทิศธรรมวิทยา',
    'วชิรกุญชรมัธยม',
    'วัดคฤหบดีสงฆ์',
    'วัดหนองโว้งวิทยา',
    'ราชธานีวิทยาลัย',
    'วัดเหมืองนาวิทยา',
    'ธรรมจักรวิทยา',
    'วัดจันทร์วิทยา',
    'สาธิตวิทยาลัยสงฆ์พุทธชินราช',
    'หลวงพ่อเพชรวิทยา',
    'ปริยัติสามัญวัดมหาธาตุ',
    'ปริยัติสามัญวัดไพรสณฑ์ศักดาราม',
    'ปริยัติสามัญวัดสระเกศ',
    'พระปริยัติธรรมแผนกสามัญศึกษาวัดประชานิมิต',
    'วัดปริยัติธรรมแผนกสามัญศึกษาวัดถ้ำสิงโตทอง',
    'วัดสิริกาญจนาราม',
    'โพธิสังวรวิทยา',
    'วัดเสนหา',
    'พระปริยัติธรรมมหาสวัสดิ์วิทยา',
    'พระปริยัติธรรมวัดไสกระดาน',
    'พระปริยัติธรรมแผนกสามัญศึกษา วัดธรรมิการามวรวิหาร',
    'พระปริยัติธรรม สามัญศึกษาวัดสระเรียง',
    'พระบรมธาตุพิทักษ์วิทยา',
    'สาธิตศึกษาวิทยาลัยสงฆ์ภาคทักษิณ',
    'พระปริยัติธรรมวัดมะนาวหวาน',
    'วัดถ้ำทองพรรณรา',
    'พระปริยัติธรรมวัดขนาน',
    'วัดแก้วโกรวาราม',
    'แสงธรรมพิทยาคม',
    'พระปริยัติธรรมวัดกรวด',
    'พระบรมธาตุไชยาวิทยา',
    'พระปริยัติแผนกสามัญศึกษาวัดบางภาวนาราม',
    'พระปริยัติธรรมวัดนาสาร',
    'พระปริยัติธรรม วัดตโปทาราม',
    'พุทธรังษีพิทยาลัย',
    'วัดมัชฌิมาวิสุทธิเทพวิทยา',
    'แสงทองวิทยาธรรม',
    'พุทธวิทยา',
    'หลวงพ่อเดิมประสิทธิ์วิทยา',
    'ถาวรวิทยาลัย',
    'มหาปัญญา',
    'ธรรมจักรวิทยา',
    'สิทธิธรรม',
    'มุจลินท์ปริยัติธรรมสามัญศึกษา',
    'ปริยัติธรรม แผนกสามัญศึกษา วัดเมืองยะลา',
    'บ้านน้ำแดง',
    'บ้านคลองมะลิประเวศน์วิทยา',
    'สิงคโปร์แอร์ไลน์ส',
    'บ้านบ่อชะอม',
    'บ้านหางแมว',
    'บ้านคลองแดง',
    'บ้านท่ากุ่ม',
    'บ้านเขาฉลาด',
    'บ้านนายาว',
    'บ้านนาอิสาน',
    'มัธยมพระราชทานนายาว',
    'การบินไทย',
    'บ้านคลองตะเคียนชัย',
    'บ้านเขาสารภี',
    'ประชารัฐบำรุง 1',
    'บ้านน้ำอ้อม',
    'บ้านทุ่งกบินทร์',
    'วังศรีทอง',
    'บ้านคลองเจริญสุข',
    'บ้านโคกน้อย',
    'พีระยานุเคราะห์ฯ 1',
    'เฉลิมพระเกียรติฯ',
    'พีระยานุเคราะห์ฯ 2',
    'บ้านตาตุม',
    'บ้านตาแตรว',
    'บ้านซำปะโต',
    'บ้านรุน',
    'บ้านโคกแสลง',
    'บ้านหนองใหญ่',
    'บ้านปากลา',
    'บ้านคำสะอาด',
    'บ้านท่าแสนคูณ',
    'บ้านป่าไม้',
    'บ้านแก่งศรีโคตร',
    'ชุมชนป่าหญ้าคา',
    'บ้านตาเอ็ม',
    'บ้านศรีสวัสดิ์',
    'บ้านป่าก่อ',
    'บ้านห้วยฆ้อง',
    'บ้านเทพภูเงิน',
    'บ้านสมประสงค์',
    'บ้านห้วยเวียงงาม',
    'บ้านเมืองทอง',
    'บ้านนาชมภู',
    'บ้านหนองแคน',
    'บ้านห้วยเป้า',
    'ฮิลมาร์พาเบิล',
    'บ้านนานกปีด',
    'เฉลิมราษฎร์บำรุง',
    'บ้านนาปอ',
    'บ้านวังชมภู',
    'บ้านห้วยดอกไม้',
    'บ้านหนองตะไก้',
    'บ้านนาแวง',
    'ค็อกนิสไทยฯ',
    'ชูทิศวิทยา',
    'บ้านหาดทรายเพ',
    'บ้านหนองดู่',
    'คอนราดเฮงเค็ล',
    'ช่างกลปทุมวันอนุสรณ์ 8',
    'บ้านนาสามัคคี',
    'การท่าอากาศยานฯ',
    'บ้านแม่ลอง',
    'บ้านใหม่พัฒนาสันติ',
    'เฉลิมพระเกียรติฯ',
    'บ้านหนองแขม',
    'ไลออนส์มหาจักร 9',
    'รัปปาปอร์ต',
    'อาโอยาม่า',
    'เฮียงไทยธำรงค์',
    'ทุติยะโพธิ์อนุสรณ์',
    'เจ้าพ่อหลวงอุปถัมภ์ 2',
    'บ้านผาแดง',
    'เบญจมะ 1',
    'สโมสรไลออนส์รัตนโกสินทร์',
    'บุญธรรม-บุญพริ้ง',
    'ยอดโพธิ์ทอง 1',
    'พีระยานุเคราะห์ฯ 3',
    'บำรุงที่ 98',
    'เฉลิมฉลอง 100 ปีฯ',
    'ผู้บังคับบัญชาลูกเสือรถไฟมักกะสัน',
    'เบ็ตตี้ดูเมน',
    'เทคนิคดุสิต',
    'อาชีวศึกษาเชียงราย-พะเยา',
    'บ้านดอยล้าน',
    'ชนัตถ์ปิยะอุย',
    'สมถวิลจินตมัย',
    'เจ้าพ่อหลวงอุปถัมภ์ 3',
    'ศรีสมวงศ์',
    'บำรุง ที่ 87',
    'เจ้าพ่อหลวงอุปถัมภ์ 4',
    'บำรุงที่ 112',
    'สังวาลย์วิท 8',
    'บ้านนาโต่',
    'ทอท.เฉลิมพระเกียรติฯ',
    'แม่ลางิ้ว',
    'บ้านตุนฯ',
    'บ้านโตแฮ',
    'บ้านพะกะเช',
    'ชมรมอนุรักษ์พุทธศิลปไทยอนุสรณ์',
    'ชมรมอนุรักษ์พุทธศิลปไทยอนุสรณ์ 2',
    'บ้านแพะ',
    'บ้านแสมใหญ่',
    'บ้านเลอตอ',
    'บ้านทีวะเบยทะ',
    'บ้านวะกะเลโค๊ะ',
    'บ้านเลพะสุคี',
    'จุฬา-ธรรมศาสตร์ 3',
    'เทคนิคอาสา 1',
    'กามาผาโด้',
    'บ้านแม่อมยะ',
    'บ้านแม่ละนา',
    'บ้านพอบือละคี',
    'บ้านแม่หละคี',
    'บ้านคะเนจือทะ',
    'บ้านเลโพเด',
    'บ้านตีนดอย',
    'ท่านผู้หญิงทวี มณีนุตร',
    'บ้านห้วยน้ำขุ่น',
    'บ้านมอเกอร์',
    'บ้านแม่กลองคี',
    'บ้านแม่กลองใหญ่',
    'บ้านเลตองคุ',
    'บ้านแม่จันทะ',
    'บ้านหม่องกั๊วะ',
    'อาทรอุทิศ',
    'บ้านลาดเรือ',
    'บ้านรักไทย',
    'บ้านนุชเทียน',
    'บ้านโป่งตะแบก',
    'ตะโกปิดทอง',
    'บ้านถ้ำหิน',
    'บ้านแม่น้ำน้อย',
    'เฮงเคลไทย',
    'บ้านต้นมะม่วง',
    'มิตรมวลชน 2',
    'วิจิตรวิทยาคาร',
    'บ้านปิล็อกคี่',
    'สหธนาคารกรุงเทพ',
    'สุนทรเวช',
    'บ้านเรดาร์',
    'บ้านทิไล่ป้า',
    'นเรศวรบ้านห้วยโสก',
    'บ้านโป่งลึก',
    'บ้านย่านซื่อ',
    'บ้านเขาจ้าว',
    'บ้านท่าวังหิน',
    'นเรศวรห้วยผึ้ง',
    'นเรศวรป่าละอู',
    'บ้านแพรกตะคร้อ',
    'บ้านคลองน้อย',
    'บ้านป่าหมาก',
    'บ้านควนมีชัย',
    'บ้านเขาวัง',
    'บ้านไร่ยาว',
    'บ้านยูงงาม',
    'บ้านยางโพรง',
    'บ้านกอเตย',
    'เทคนิคมีนบุรีอนุสรณ์ 1',
    'บ้านคลองวาย',
    'บ้านในวง',
    'สันตินิมิตร',
    'บ้านพันวาล',
    'บ้านห้วยเหมือง',
    'สิริราษฎร์',
    'บ้านสวนเพชร',
    'บ้านควนสามัคคี',
    'เชิญ พิศลยบุตร',
    'บ้านประกอบออก',
    'มหาราช 1',
    'บ้านสำนักเอาะ',
    'บ้านหาดทราย',
    'สื่อมวลชนกีฬา',
    'ทุ่งไม้ด้วน 2',
    'บ้านทุ่งสบายใจ',
    'บ้านบาโรย',
    'ยูงทองรัฐประชาสรรค์',
    'บ้านส้านแดง',
    'บ้านทุ่งตะเซะพัฒนา',
    'สันติราษฎร์ประชาบำรุง',
    'บ้านหินจอก',
    'บ้านควนตะแบก',
    'พีระยานุเคราะห์ฯ 4',
    'เฉลิมพระเกียรติ 50 พรรษา สมเด็จพระเทพฯ',
    'สังวาลย์วิท 4',
    'นิคมพิทักษ์ราษฎร์',
    'บ้านปาโจแมเราะ',
    'โรงงานยาสูบ 2',
    'การท่าอากาศยานฯ',
    'บ้านตืองอช่างกลปทุมวันอนุสรณ์13',
    'บ้านละโอ',
    'บ้านลีนานนท์',
    'บ้านไอร์บือแต'
]